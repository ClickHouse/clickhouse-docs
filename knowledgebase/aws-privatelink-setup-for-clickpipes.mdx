...
8. **Recommended For MySQL**
    - If you are using MySQL, it is recommended to set `skip_name_resolve=1` as a parameter in the RDS. Due to repeated
      health checks from the NLB, MySQL will block the NLB Hosts. Setting this parameter completely avoids the DNS host
      name lookup and so it never blocks the NLB hosts.
