...
- Use [type hints](/sql-reference/data-types/newjson) on JSON paths you know ahead of time. Hints bypass the discriminator column and store the path like a regular typed column, with the same performance and no overhead.
...
If your dynamic fields are homogeneous key-value pairs — all values share the same type — [`Map(String, T)`](/sql-reference/data-types/map) is simpler and more efficient than a JSON column. Common examples: string tags (`Map(String, String)`), numeric metrics (`Map(String, Float64)`), or feature flags (`Map(String, Bool)`).
