...
|------|-------------|----------|
| **Wide** | Each column is stored in separate file(s), each with its own marks file | Queries selecting a subset of columns |
| **Compact** | All columns are stored in a single file with a single marks file | Ingestion performance and queries needing all columns |
