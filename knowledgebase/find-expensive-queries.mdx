...
:::note[Distributed execution is not the only source of differing `query_id` values]
A single query can also spawn internal sub-queries that are logged with their own `query_id` and `is_initial_query = 0` — most commonly the internal queries ClickHouse runs to evaluate [views](https://clickhouse.com/docs/operations/system-tables/query_views_log). These internal identifiers are assigned by ClickHouse and may not follow the same format as the one reported to your client; for view-related steps you may see `query_id` values that include a label such as `queryView...`. When you want the query exactly as you submitted it, filter on `is_initial_query = 1` (or `query_id = initial_query_id`). This is server-side behaviour of `query_log` and is the same from any client, including the Cloud SQL Console.
:::
