...
Note that when using ClickHouse Cloud and its default [SharedMergeTree](https://clickhouse.com/docs/en/cloud/reference/shared-merge-tree) table engine, using `insert_quorum_parallel` is not required — all inserts to SharedMergeTree are quorum inserts (by design).
