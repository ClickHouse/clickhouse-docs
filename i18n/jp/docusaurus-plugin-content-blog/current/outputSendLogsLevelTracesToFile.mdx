---
title: clickhouse-client を使用してログレベルのトレースをファイルに出力する方法
description: "clickhouse-client を使用してログレベルのトレースをファイルに出力する方法"
date: 2024-11-21
tags: ['データエクスポート']
keywords: ['ログレベルのトレース', 'clickhouse-client']
---
...
複数のステートメントや複数行に対して、ClickHouse Client を使用して `send_logs_level` の出力結果をファイルに出力するにはどうすればよいですか？

## 回答

* たとえば `send_logs_level_example.sql` という名前で、ステートメントを記述した SQL ファイルを作成します。
