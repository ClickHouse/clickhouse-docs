---
title: ClickHouseでPIVOTはできるでしょうか？
description: "ClickHouseにはPIVOT句はありませんが、集約関数コンビネータを使うことで、これに近い機能を実現できます。ここでは、英国の住宅価格データセットを使って、その方法を見ていきます。"
date: 2024-12-11
tags: ['データモデリング', 'データの基本概念']
keywords: ['PIVOT', 'Aggregate Function Combinators']
---

{frontMatter.description}

{/* 省略 */}

## はじめに \{#introduction\}

ClickHouse にはピボット演算子はありませんが、[集約関数コンビネータ](/sql-reference/aggregate-functions/combinators)、特に [`-Map` 接尾辞](/sql-reference/aggregate-functions/combinators#-map) を持つものを使うことで、同様の処理を行えます。

この記事では、その方法について説明します。
同じ内容を扱った動画もあり、以下から視聴できます:

<iframe width="560" height="315" src="https://www.youtube.com/embed/nlRMOmwYtF4?si=0TZSNg-uo7zjiO52" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen />

## 集約関数コンビネータを理解する \{#understanding-aggregate-function-combinators\}

簡単な例から始めましょう。ここでは [clickhouse-local](/operations/utilities/clickhouse-local) を使用します。次のコマンドを実行して起動します。

```bash
clickhouse -m --output_format_pretty_row_numbers=0
```

次のクエリは `sumMap` 関数を呼び出します。この関数はマップを受け取り、各キーに対応する値を合計します。

```sql
SELECT sumMap(map('ClickHouse', 1, 'ClickBench', 2));
```

```text
┌─sumMap(map('ClickHouse', 1, 'ClickBench', 2))─┐
│ {'ClickBench':2,'ClickHouse':1}               │
└───────────────────────────────────────────────┘
```

これは渡したマップをそのまま返すだけなので、あまり興味深い例ではありません。
では、複数行にわたるマップに対して `sumMap` を呼び出してみましょう。

```sql
WITH values AS (
  SELECT map('ClickHouse', 3) AS value
  UNION ALL
  SELECT map('ClickBench', 2, 'ClickHouse', 4) AS value
)
SELECT sumMap(value)
FROM values;
```

```text
┌─sumMap(value)───────────────────┐
│ {'ClickBench':2,'ClickHouse':7} │
└─────────────────────────────────┘
```

キー `ClickHouse` は両方の行で現れており、その値が合計されています。キー `ClickBench` は1行にのみ存在するため、その単一の値を合計することになり、その値がそのまま返されます。

また、`maxMap` を使ってキーごとの最大値を求めることもできます。

```sql
WITH values AS (
  SELECT map('ClickHouse', 3) AS value
  UNION ALL
  SELECT map('ClickBench', 2, 'ClickHouse', 4) AS value
)
SELECT maxMap(value)
FROM values;
```

```text
┌─maxMap(value)───────────────────┐
│ {'ClickBench':2,'ClickHouse':4} │
└─────────────────────────────────┘
```

または、`avgMap` を使用してキーごとの平均値を計算することもできます。

```sql
WITH values AS (
  SELECT map('ClickHouse', 3) AS value
  UNION ALL
  SELECT map('ClickBench', 2, 'ClickHouse', 4) AS value
)
SELECT avgMap(value)
FROM values;
```

```text
┌─avgMap(value)─────────────────────┐
│ {'ClickBench':2,'ClickHouse':3.5} │
└───────────────────────────────────┘
```

ここまでで、これらの関数コンビネータがどのように機能するのか、イメージをつかんでいただけたのではないでしょうか。

## 実際のユースケース: 英国住宅価格データセット \{#real-world-application-uk-housing-prices-dataset\}

次に、[ClickHouse SQL Playground](https://clickhouse.com/blog/announcing-the-new-sql-playground) 上の、より大規模なデータセットに対してこれらを使ってみます。

ClickHouse SQL Playground には clickhouse-client を使って接続できます。

```bash
clickhouse client -m \
  -h sql-clickhouse.clickhouse.com \
  -u demo \
  --secure
```

これから `uk_price_paid` テーブルに対してクエリを実行するので、そのテーブル内のデータを確認してみましょう。

```sql
SELECT * FROM uk.uk_price_paid LIMIT 1 FORMAT Vertical;
```

```text
行 1:
──────
価格:     145000
日付:      2008-11-19
郵便番号1:
郵便番号2:
種類:      半戸建て
新築:    0
期間:  借地権
住所1:
住所2:
通り:    CURLEW DRIVE
地域:  SCARBOROUGH
町:      SCARBOROUGH
地区:  SCARBOROUGH
州:    NORTH YORKSHIRE
カテゴリ:  0
```

上で見たように、このテーブルにはイギリスの不動産売買に関連するさまざまなフィールドが含まれています。

### 10年ごとのグルーピングと集約 \{#grouping-and-aggregating-by-decade\}

このデータセットで、各10年ごとに郡ごとにグループ化した中央値価格を算出してみましょう。

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    county,
    medianMap(map(year, price)) AS medianPrices
FROM uk.uk_price_paid
GROUP BY ALL
ORDER BY max(price) DESC
LIMIT 10;
```

```text
    ┌─county─────────────┬─medianPrices───────────────────────────────────────┐
 1. │ GREATER LONDON     │ {1990:89972.5,2000:215000,2010:381500,2020:485000} │
 2. │ TYNE AND WEAR      │ {1990:46500,2000:93000,2010:130000,2020:139000}    │
 3. │ WEST MIDLANDS      │ {1990:50000,2000:110000,2010:149950,2020:185000}   │
 4. │ GREATER MANCHESTER │ {1990:47000,2000:97000,2010:141171,2020:178000}    │
 5. │ MERSEYSIDE         │ {1990:46750,2000:94972.5,2010:128000,2020:149000}  │
 6. │ HERTFORDSHIRE      │ {1990:86500,2000:193000,2010:315000,2020:415000}   │
 7. │ WEST YORKSHIRE     │ {1990:48995,2000:99950,2010:139000,2020:164950}    │
 8. │ BRIGHTON AND HOVE  │ {1990:70000,2000:173000,2010:288000,2020:387000}   │
 9. │ DORSET             │ {1990:76500,2000:182000,2010:250000,2020:315000}   │
10. │ HAMPSHIRE          │ {1990:79950,2000:177500,2010:260000,2020:335000}   │
    └────────────────────┴────────────────────────────────────────────────────┘
```

### 結果のフィルタリング \{#filtering-results\}

結果を2010年以降のデータのみに絞り込むことができます。

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    county,
    medianMap(map(year, price)) AS medianPrices
FROM uk.uk_price_paid
WHERE year >= 2010
GROUP BY ALL
ORDER BY max(price) DESC
LIMIT 10;
```

```text
    ┌─county─────────────┬─medianPrices────────────────┐
 1. │ GREATER LONDON     │ {2010:384975,2020:485919.5} │
 2. │ TYNE AND WEAR      │ {2010:130000,2020:140000}   │
 3. │ WEST MIDLANDS      │ {2010:146500,2020:185000}   │
 4. │ GREATER MANCHESTER │ {2010:140000,2020:177500}   │
 5. │ MERSEYSIDE         │ {2010:130000,2020:150000}   │
 6. │ HERTFORDSHIRE      │ {2010:315000,2020:415000}   │
 7. │ WEST YORKSHIRE     │ {2010:140000,2020:162500}   │
 8. │ BRIGHTON AND HOVE  │ {2010:287500,2020:387000}   │
 9. │ DORSET             │ {2010:255750,2020:315000}   │
10. │ HAMPSHIRE          │ {2010:265000,2020:330000}   │
    └────────────────────┴─────────────────────────────┘
```

## 複数の集約を組み合わせる \{#combining-multiple-aggregations\}

また、10 年ごとの最大価格を求めたい場合は、先ほど見た `maxMap` 関数を使って求めることができます。

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    county,
    medianMap(map(year, price)) AS medianPrices,
    maxMap(map(year, price)) AS maxPrices
FROM uk.uk_price_paid
WHERE year >= 2010
GROUP BY ALL
ORDER BY max(price) DESC
LIMIT 10;
```

```text
    ┌─county─────────────┬─medianPrices──────────────┬─maxPrices───────────────────────┐
 1. │ GREATER LONDON     │ {2010:385000,2020:485250} │ {2010:594300000,2020:630000000} │
 2. │ TYNE AND WEAR      │ {2010:130000,2020:141000} │ {2010:448300979,2020:93395000}  │
 3. │ WEST MIDLANDS      │ {2010:149000,2020:184250} │ {2010:415000000,2020:104500000} │
 4. │ GREATER MANCHESTER │ {2010:140000,2020:175000} │ {2010:107086856,2020:319186000} │
 5. │ MERSEYSIDE         │ {2010:129950,2020:150000} │ {2010:300000000,2020:93395000}  │
 6. │ HERTFORDSHIRE      │ {2010:315000,2020:415000} │ {2010:254325163,2020:93395000}  │
 7. │ WEST YORKSHIRE     │ {2010:138500,2020:165000} │ {2010:246300000,2020:109686257} │
 8. │ BRIGHTON AND HOVE  │ {2010:285000,2020:387000} │ {2010:200000000,2020:71540000}  │
 9. │ DORSET             │ {2010:250000,2020:315000} │ {2010:150000000,2020:20230000}  │
10. │ HAMPSHIRE          │ {2010:264000,2020:330000} │ {2010:150000000,2020:48482500}  │
    └────────────────────┴───────────────────────────┴─────────────────────────────────┘
```

## マップ値への関数適用 \{#applying-functions-to-map-values\}

別の方法として、`avgMap` を使用して平均価格を計算できます。
これらの値には小数点以下の桁数が多いため、[`mapApply`](/sql-reference/functions/tuple-map-functions#mapapply) 関数を使ってマップ内の各値に [`floor`](/sql-reference/functions/rounding-functions#floor) 関数を適用し、小数部を切り捨てて値を整形できます。

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    county,
    medianMap(map(year, price)) AS medianPrices,
    mapApply((k, v) -> (k, floor(v)), avgMap(map(year, price))) AS avgPrices
FROM uk.uk_price_paid
WHERE year >= 2010
GROUP BY ALL
ORDER BY max(price) DESC
LIMIT 10;
```

```text
    ┌─郡─────────────────┬─中央値価格────────────────┬─平均価格──────────────────┐
 1. │ グレーター・ロンドン │ {2010:382000,2020:490000} │ {2010:626091,2020:807240} │
 2. │ タイン・アンド・ウィア │ {2010:127000,2020:140000} │ {2010:176955,2020:225770} │
 3. │ ウェスト・ミッドランズ │ {2010:148500,2020:183000} │ {2010:204128,2020:257226} │
 4. │ グレーター・マンチェスター │ {2010:140000,2020:177500} │ {2010:195592,2020:251165} │
 5. │ マージーサイド     │ {2010:127995,2020:150000} │ {2010:182194,2020:206062} │
 6. │ ハートフォードシャー │ {2010:317500,2020:415000} │ {2010:414134,2020:529409} │
 7. │ ウェスト・ヨークシャー │ {2010:140000,2020:164500} │ {2010:185121,2020:234870} │
 8. │ ブライトン・アンド・ホーヴ │ {2010:285000,2020:387000} │ {2010:372285,2020:527184} │
 9. │ ドーセット         │ {2010:250000,2020:315000} │ {2010:305581,2020:370739} │
10. │ ハンプシャー       │ {2010:265000,2020:330000} │ {2010:335945,2020:425196} │
    └────────────────────┴───────────────────────────┴───────────────────────────┘
```

## 柔軟なグループ化：郡、地区、郵便番号 \{#flexible-grouping-counties-districts-and-postcodes\}

いくつか別のフィールドでグループ化してみましょう。
今回は、郡と地区ごとに、10 年ごとの価格の中央値を計算します。

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    county,
    district,
    medianMap(map(year, price)) AS medianPrices
FROM uk.uk_price_paid
WHERE year >= 2010
GROUP BY ALL
ORDER BY max(price) DESC
LIMIT 10
```

```text
    ┌─county─────────────┬─district───────────────┬─medianPrices────────────────┐
 1. │ GREATER LONDON     │ CROYDON                │ {2010:298475,2020:400000}   │
 2. │ GREATER LONDON     │ CITY OF WESTMINSTER    │ {2010:800000,2020:935000}   │
 3. │ GREATER LONDON     │ SOUTHWARK              │ {2010:437000,2020:540000}   │
 4. │ TYNE AND WEAR      │ NEWCASTLE UPON TYNE    │ {2010:144000,2020:162500}   │
 5. │ WEST MIDLANDS      │ WALSALL                │ {2010:137450,2020:162000}   │
 6. │ GREATER LONDON     │ CITY OF LONDON         │ {2010:725875,2020:840000}   │
 7. │ GREATER LONDON     │ HILLINGDON             │ {2010:329125,2020:439000}   │
 8. │ GREATER MANCHESTER │ MANCHESTER             │ {2010:144972.5,2020:190000} │
 9. │ GREATER LONDON     │ HAMMERSMITH AND FULHAM │ {2010:622250,2020:750000}   │
10. │ GREATER LONDON     │ ISLINGTON              │ {2010:500000,2020:640000}   │
    └────────────────────┴────────────────────────┴─────────────────────────────┘
```

また、年ごとにグループ化し、マップ内で `postcode1` と `postcode2` を連結することもできます：

```sql
WITH year(toStartOfInterval(date, toIntervalYear(10))) AS year
SELECT
    year,
    medianMap(map(postcode1 || ' ' || postcode2, price)) AS medianPrices
FROM uk.uk_price_paid
WHERE postcode1 LIKE 'NP1'
GROUP BY ALL;
```

```text
   ┌─year─┬─medianPrices────────────────────────────────────────────────────────┐
1. │ 1990 │ {'NP1 4PB':9000}                                                    │
2. │ 2000 │ {'NP1 4SR':28475,'NP1 7HZ':200000}                                  │
3. │ 2010 │ {'NP1 4PB':5000,'NP1 4QJ':1075000,'NP1 4SR':58000,'NP1 8BR':200000} │
4. │ 2020 │ {'NP1 5DW':140000}                                                  │
   └──────┴─────────────────────────────────────────────────────────────────────┘
```
