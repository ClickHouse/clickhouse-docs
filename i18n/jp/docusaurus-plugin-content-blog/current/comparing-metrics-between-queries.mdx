...
description: ClickHouse で 2 つのクエリのメトリクスを比較するためのクエリ。
tags: ['パフォーマンスと最適化']
keywords: [クエリの比較, メトリクスの比較, クエリ パフォーマンス]
---
...
## 2 つのクエリ間でメトリクスを比較する \{#compare-metrics-between-two-queries\}

2 つのクエリ間でメトリクスを比較するには、まず両方のクエリの `query_id` を取得する必要があります。
...
85行を取得。経過時間: 0.074秒。処理: 216万行、102.28 MB (2919万行/秒、1.38 GB/秒)
ピークメモリ使用量: 114.84 MiB
```