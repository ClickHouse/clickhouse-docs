...
この例では、[CTE](/sql-reference/statements/select/with) を使って、これら 2 つのクエリ内の各行の [cityHash](/sql-reference/functions/hash-functions#cityHash64) 値の合計を計算し、2 つの結果セットが同一であれば `1` を返します。
