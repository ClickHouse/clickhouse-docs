...
3. トレースを trace event 形式などの有用な形式に変換します。`llvm-xray convert --symbolize --instr_map=./build/programs/clickhouse --output-format=trace_event xray-log.clickhouse.ZqKprE |
