...
title: 'レッスン - デバッグの知見'
description: '遅いクエリ、メモリエラー、接続障害、設定上の問題など、最も一般的な ClickHouse の問題に対する解決策を紹介します。'
---
...
_本ガイドは、コミュニティミートアップから得られた知見集の一部です。実際の解決策や知見については、[特定の問題別に参照](./community-wisdom.md)できます。_
...
メモリ集約的な操作には外部集約を有効にします。処理速度は低下しますが、ディスクへのスピルによってメモリ不足によるクラッシュを防ぐことができます。`max_bytes_before_external_group_by`を使用することで、大規模な`GROUP BY`操作におけるメモリ不足クラッシュを防止できます。この設定の詳細については[こちら](/operations/settings/settings#max_bytes_before_external_group_by)を参照してください。
...
非同期挿入は、パフォーマンスを向上させるために小規模な挿入をサーバー側で自動的にバッチ処理します。データがディスクに書き込まれるのを待ってから確認応答を返すかどうかを設定できます。即座に返す方が高速ですが、耐久性は低くなります。最新バージョンでは、バッチ内の重複データを処理するための重複排除がサポートされています。
...
- [ClickHouse運用から得られた10の教訓](https://www.youtube.com/watch?v=liTgGiTuhJE)
