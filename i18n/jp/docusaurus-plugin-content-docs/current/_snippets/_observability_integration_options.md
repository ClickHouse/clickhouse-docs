...
Datadogは、サービスのアイドリング動作を考慮しながら適切なクラウドサービス監視を提供する専用API統合を開発中です。暫定的に、チームはClickHouse PrometheusエンドポイントによるOpenMetrics統合アプローチを使用して、運用上の分離とコスト効率の高い監視を実現できます。設定ガイダンスについては、[DatadogのPrometheusおよびOpenMetrics統合ドキュメント](https://docs.datadoghq.com/integrations/openmetrics/)を参照してください。
...
HyperDXには、Selects、Inserts、Infrastructureのタブを備えたClickHouseに特化したダッシュボードが付属しています。チームはLuceneまたはSQL構文を使用してシステムテーブルとログを検索し、Chart Explorerを介して詳細なシステム分析のためのカスタム可視化を作成することもできます。
このアプローチは、リアルタイムの本番アラートではなく、複雑な問題のデバッグ、パフォーマンス分析、深いシステムの内省に最適です。
