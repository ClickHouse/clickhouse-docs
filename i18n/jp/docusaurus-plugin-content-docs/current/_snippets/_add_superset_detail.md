...
`docker compose`を実行する前に、いくつかのタスクを完了する必要があります:
...
ガイドで作成するサンプル可視化の一部では、経度や緯度などの位置データを使用します。SupersetはMapbox地図をサポートしています。Mapboxの可視化機能を使用するには、Mapbox APIキーが必要です。[Mapbox無料プラン](https://account.mapbox.com/auth/signup/)に登録し、APIキーを生成してください。
