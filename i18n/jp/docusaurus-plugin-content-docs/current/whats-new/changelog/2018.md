...
sidebar_label: '2018'
title: '2018年の更新履歴'
description: '2018年の更新履歴'
doc_type: 'changelog'
keywords: ['ClickHouse 2018', '2018年の更新履歴', 'リリースノート', 'バージョン履歴', '旧バージョン']
---
...
- すべてのパーティションを一度にバックアップするために、`ALTER TABLE ... FREEZE`クエリでパーティションを省略する機能が追加されました。[#3514](https://github.com/ClickHouse/ClickHouse/pull/3514)
...
- 単純なキーを持つ`Join`型テーブルの読み取りがサポートされました。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3728)
...
* `VersionedCollapsingMergeTree` において、`FINAL` と組み合わせて使用した場合の `PREWHERE` のバグを修正しました。 [7167bfd7](https://github.com/ClickHouse/ClickHouse/commit/7167bfd7b365538f7a91c4307ad77e552ab4e8c1)
* `KILL QUERY` を使用して、テーブルのロック獲得待ちのためまだ開始されていないクエリをキャンセルできるようになりました。 [#3517](https://github.com/ClickHouse/ClickHouse/pull/3517)
* 時計が真夜中に戻される場合の日付と時刻の計算を修正しました（これはイランで発生し、モスクワでは 1981 年から 1983 年にかけて発生していました）。以前は、必要以上に 1 日早く時刻がリセットされてしまい、さらにテキスト形式での日付と時刻の書式が誤って出力される原因にもなっていました。 [#3819](https://github.com/ClickHouse/ClickHouse/pull/3819)
* データベース名を省略した `VIEW` やサブクエリで発生する一部の不具合を修正しました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3521)
* 内部の `MATERIALIZED VIEW` をロックしていなかったために、`MATERIALIZED VIEW` からの読み取りと `MATERIALIZED VIEW` の削除を同時に実行した際に発生していたレースコンディションを修正しました。 [#3404](https://github.com/ClickHouse/ClickHouse/pull/3404) [#3694](https://github.com/ClickHouse/ClickHouse/pull/3694)
* `Lock handler cannot be nullptr.` というエラーを修正しました。 [#3689](https://github.com/ClickHouse/ClickHouse/pull/3689)
* `compile_expressions` オプションが有効な場合（デフォルトで有効になっています）におけるクエリ処理を修正しました。`now` 関数のような非決定的な定数式は、もはや展開されなくなりました。 [#3457](https://github.com/ClickHouse/ClickHouse/pull/3457)
* `toDecimal32/64/128` 関数で、scale 引数に非定数を指定した場合にクラッシュする問題を修正しました。
* `Values` フォーマットで `NULL` 要素を含む配列を、`Nullable` なしの `Array` 型の列に挿入しようとした際に発生するエラーを修正しました（`input_format_values_interpret_expressions` = 1 の場合）。 [#3487](https://github.com/ClickHouse/ClickHouse/pull/3487) [#3503](https://github.com/ClickHouse/ClickHouse/pull/3503)
* ZooKeeper が利用できない場合に `DDLWorker` で継続的にエラーがログ出力される問題を修正しました。 [8f50c620](https://github.com/ClickHouse/ClickHouse/commit/8f50c620334988b28018213ec0092fe6423847e2)
* 引数に `Date` および `DateTime` 型をとる `quantile*` 関数の戻り値の型を修正しました。 [#3580](https://github.com/ClickHouse/ClickHouse/pull/3580)
* 式を伴わない単純なエイリアスのみを指定する `WITH` 句の不具合を修正しました。 [#3570](https://github.com/ClickHouse/ClickHouse/pull/3570)
* `enable_optimize_predicate_expression` が有効な場合に、名前付きサブクエリおよび修飾カラム名を含むクエリの処理を修正しました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3588)
* マテリアライズドビューを使用する際に発生していた `Attempt to attach to nullptr thread group` エラーを修正しました。 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3623)
* 特定の誤った引数を `arrayReverse` 関数に渡した際に発生するクラッシュを修正しました。 [73e3a7b6](https://github.com/ClickHouse/ClickHouse/commit/73e3a7b662161d6005e7727d8a711b930386b871)
* `extractURLParameter` 関数におけるバッファオーバーフローを修正しました。パフォーマンスを改善しました。ゼロバイトを含む文字列の正しい処理を追加しました。 [141e9799](https://github.com/ClickHouse/ClickHouse/commit/141e9799e49201d84ea8e951d1bed4fb6d3dacb5)
* `lowerUTF8` および `upperUTF8` 関数におけるバッファオーバーフローを修正しました。これらの関数で `FixedString` 型の引数を処理できるサポートを削除しました。 [#3662](https://github.com/ClickHouse/ClickHouse/pull/3662)
* `MergeTree` テーブルの削除時に稀に発生していた競合状態を修正しました。 [#3680](https://github.com/ClickHouse/ClickHouse/pull/3680)
* `Buffer` テーブルからの読み取りと対象テーブルに対する `ALTER` または `DROP` を同時に実行すると発生していたレースコンディションを修正しました。 [#3719](https://github.com/ClickHouse/ClickHouse/pull/3719)
* `max_temporary_non_const_columns` 制限を超えた場合に発生していたセグメンテーションフォルトを修正しました。 [#3788](https://github.com/ClickHouse/ClickHouse/pull/3788)

#### 改善点: {#improvements-1}


* サーバーは、処理済みの設定ファイルを `/etc/clickhouse-server/` ディレクトリには書き込みません。代わりに、`path` 内の `preprocessed_configs` ディレクトリに保存します。これは、`/etc/clickhouse-server/` ディレクトリが `clickhouse` ユーザーに対する書き込み権限を持たないことを意味し、その結果としてセキュリティが向上します。[#2443](https://github.com/ClickHouse/ClickHouse/pull/2443)
* `min_merge_bytes_to_use_direct_io` オプションはデフォルトで 10 GiB に設定されています。MergeTree ファミリーのテーブルで大きなパーツを生成するマージ処理は `O_DIRECT` モードで実行され、これによりページキャッシュの過度な追い出しが防止されます。[#3504](https://github.com/ClickHouse/ClickHouse/pull/3504)
* テーブル数が非常に多い場合でもサーバー起動を高速化。 [#3398](https://github.com/ClickHouse/ClickHouse/pull/3398)
* レプリカ間の接続にコネクションプールと HTTP `Keep-Alive` を追加しました。 [#3594](https://github.com/ClickHouse/ClickHouse/pull/3594)
...
* 互換性を維持するため、`join_default_strictness` オプションのデフォルト値は `ALL` に設定されています。 [120e2cbe](https://github.com/ClickHouse/ClickHouse/commit/120e2cbe2ff4fbad626c28042d9b28781c805afe)
* 無効または複雑な正規表現に対して `re2` ライブラリが `stderr` へ行っていたログ出力を削除しました。 [#3723](https://github.com/ClickHouse/ClickHouse/pull/3723)
* `Kafka` テーブルエンジンに、Kafka からの読み取りを開始する前に購読をチェックする機能と、テーブル用の `kafka_max_block_size` 設定を追加しました。 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3396)
* `cityHash64`、`farmHash64`、`metroHash64`、`sipHash64`、`halfMD5`、`murmurHash2_32`、`murmurHash2_64`、`murmurHash3_32`、`murmurHash3_64` 関数は、任意の数の引数およびタプルとして渡された引数に対して動作するようになりました。[#3451](https://github.com/ClickHouse/ClickHouse/pull/3451) [#3519](https://github.com/ClickHouse/ClickHouse/pull/3519)
* `arrayReverse` 関数が任意の型の配列で動作するようになりました。 [73e3a7b6](https://github.com/ClickHouse/ClickHouse/commit/73e3a7b662161d6005e7727d8a711b930386b871)
* `timeSlots` 関数にスロットサイズを指定するオプションのパラメータが追加されました。 [Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/3724)
* `FULL` および `RIGHT JOIN` の場合、右テーブルからの非結合データのストリームには `max_block_size` の設定値が使用されます。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3699)
* TLS を有効にするための `--secure` コマンドラインパラメータを `clickhouse-benchmark` と `clickhouse-performance-test` に追加しました。 [#3688](https://github.com/ClickHouse/ClickHouse/pull/3688) [#3690](https://github.com/ClickHouse/ClickHouse/pull/3690)
* `Buffer` 型テーブルの構造が出力先テーブルの構造と一致しない場合の型変換処理。 [Vitaly Baranov](https://github.com/ClickHouse/ClickHouse/pull/3603)
* `tcp_keep_alive_timeout` オプションを追加し、指定した時間間隔だけ非アクティブ状態が続いた後に keep-alive パケットを送信できるようにしました。 [#3441](https://github.com/ClickHouse/ClickHouse/pull/3441)
* `system.parts` テーブルで、パーティションキーが単一のカラムで構成されている場合に、その値を不要にクォートしていた処理を削除しました。 [#3652](https://github.com/ClickHouse/ClickHouse/pull/3652)
* `Date` および `DateTime` データ型で modulo 関数が利用できます。 [#3385](https://github.com/ClickHouse/ClickHouse/pull/3385)
* `POWER`、`LN`、`LCASE`、`UCASE`、`REPLACE`、`LOCATE`、`SUBSTR`、`MID` 関数にシノニムを追加しました。 [#3774](https://github.com/ClickHouse/ClickHouse/pull/3774) [#3763](https://github.com/ClickHouse/ClickHouse/pull/3763) いくつかの関数名は、SQL 標準との互換性のために大文字小文字を区別しません。SQL との互換性のために、シンタックスシュガーとして `SUBSTRING(expr FROM start FOR length)` を追加しました。 [#3804](https://github.com/ClickHouse/ClickHouse/pull/3804)
* `clickhouse-server` の実行可能コードに対応するメモリページを `mlock` し、メモリ上から強制的に追い出されるのを防止できる機能を追加しました。この機能はデフォルトでは無効になっています。 [#3553](https://github.com/ClickHouse/ClickHouse/pull/3553)
* `O_DIRECT` からの読み取り時の性能を改善（`min_bytes_to_use_direct_io` オプション有効時）。 [#3405](https://github.com/ClickHouse/ClickHouse/pull/3405)
* 定数キー引数と非定数デフォルト引数を持つ場合の `dictGet...OrDefault` 関数のパフォーマンスを改善しました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3563)
* `firstSignificantSubdomain` 関数が、`gov`、`mil`、`edu` ドメインも処理するようになりました。なお、[Igor Hatarist](https://github.com/ClickHouse/ClickHouse/pull/3601) によりパフォーマンスが改善されました。[#3628](https://github.com/ClickHouse/ClickHouse/pull/3628)
* `SYS-V init.d` スクリプトを使用して `clickhouse-server` を起動する際に使用するカスタム環境変数を、`/etc/default/clickhouse` 内で `CLICKHOUSE_PROGRAM_ENV` を定義することで指定できる機能。
...
* `system.metrics` テーブルに `VersionInteger` メトリクスが追加され、`system.build_options` には `VERSION_INTEGER` という行が追加されました。この行には、`18016000` のような ClickHouse バージョンの数値形式が格納されます。 [#3644](https://github.com/ClickHouse/ClickHouse/pull/3644)
* `Date` 型を数値と比較できないようにしました。これは、日付を囲む引用符を誤って省略してしまう `date = 2018-12-17` のような潜在的なエラーを防ぐためです。 [#3687](https://github.com/ClickHouse/ClickHouse/pull/3687)
* `rowNumberInAllBlocks` のようなステートフル関数の動作を修正しました。これらはクエリ解析時からカウントを開始していたため、以前は出力結果が実際より 1 大きい数値になっていました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3729)
* `force_restore_data` ファイルを削除できない場合には、エラーメッセージが表示されます。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3794)
...
- ARM上でのビルドに関する修正を行いました。[#3709](https://github.com/ClickHouse/ClickHouse/pull/3709)
...
### ClickHouseリリース18.14.18、2018年12月4日 {#clickhouse-release-18-14-18-2018-12-04}

#### バグ修正: {#bug-fixes-3}

- `range`型の辞書に対する`dictGet...`関数において、引数の一方が定数で他方が定数でない場合に発生するエラーを修正しました。[#3751](https://github.com/ClickHouse/ClickHouse/pull/3751)
...
### ClickHouseリリース18.14.17、2018年11月30日 {#clickhouse-release-18-14-17-2018-11-30}

#### バグ修正: {#bug-fixes-4}

- ODBCブリッジプロセスがメインサーバープロセスと共に終了しない問題を修正しました。[#3642](https://github.com/ClickHouse/ClickHouse/pull/3642)
...
### ClickHouseリリース18.14.15、2018年11月21日 {#clickhouse-release-18-14-15-2018-11-21}
...
### ClickHouseリリース18.14.14、2018年11月20日 {#clickhouse-release-18-14-14-2018-11-20}
...
- クラスタ設定でセキュア接続が有効になっている場合（`<secure>`フラグ）の`ON CLUSTER`クエリを修正しました。[#3465](https://github.com/ClickHouse/ClickHouse/pull/3465)
- `SAMPLE`、`PREWHERE`、およびエイリアス列を使用するクエリのエラーを修正しました。[#3543](https://github.com/ClickHouse/ClickHouse/pull/3543)
- `min_bytes_to_use_direct_io`設定が有効な場合に発生する稀な`unknown compression method`エラーを修正しました。[3544](https://github.com/ClickHouse/ClickHouse/pull/3544)

#### パフォーマンスの改善: {#performance-improvements}

- AMD EPYCプロセッサで実行する際のUInt16型またはDate型の列による`GROUP BY`を含むクエリのパフォーマンス低下を修正しました。[Igor Lapko](https://github.com/ClickHouse/ClickHouse/pull/3512)
...
- 外部データベースをクエリする際に不正なクエリ（空の`WHERE`句を含む）が生成される問題を修正しました。[hotid](https://github.com/ClickHouse/ClickHouse/pull/3477)
...
- LIMITクエリにおける`Block structure mismatch in UNION stream: different number of columns`エラーを修正しました。[#2156](https://github.com/ClickHouse/ClickHouse/issues/2156)
- Nested構造内に配列を含むテーブルでデータをマージする際のエラーを修正しました。[#3397](https://github.com/ClickHouse/ClickHouse/pull/3397)
- `merge_tree_uniform_read_distribution`設定が無効になっている場合（デフォルトでは有効）の不正なクエリ結果を修正しました。[#3429](https://github.com/ClickHouse/ClickHouse/pull/3429)
- Native形式でDistributedテーブルへ挿入する際のエラーを修正しました。[#3411](https://github.com/ClickHouse/ClickHouse/issues/3411)

### ClickHouse リリース 18.14.10、2018-10-23 {#clickhouse-release-18-14-10-2018-10-23}

- `compile_expressions`設定（式のJITコンパイル）はデフォルトで無効になっています。[#3410](https://github.com/ClickHouse/ClickHouse/pull/3410)
- `enable_optimize_predicate_expression`設定はデフォルトで無効になっています。
...
- データ挿入時に複数の`MATERIALIZED VIEW`で並列処理が利用可能になりました。`parallel_view_processing`設定を参照してください。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3208)
...
* `ORDER BY` および `LIMIT` を含むクエリにおけるメモリ使用量を大幅に削減しました。`max_bytes_before_remerge_sort` 設定を参照してください。 [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* `JOIN`（`LEFT`、`INNER`、...）が明示されていない場合、`INNER JOIN` が仮定されます。 [#3147](https://github.com/ClickHouse/ClickHouse/pull/3147)
* 修飾アスタリスクが `JOIN` を含むクエリでも正しく動作するようになりました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3202)
* `ODBC` テーブルエンジンは、リモートデータベースの SQL 方言に応じて識別子の引用方法を正しく選択するようになりました。 [Alexandr Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/3210)
* `compile_expressions` 設定（式の JIT コンパイル）はデフォルトで有効になっています。
* `DROP DATABASE/TABLE IF EXISTS` と `CREATE DATABASE/TABLE IF NOT EXISTS` を同時に実行した場合の動作を修正しました。以前は、`CREATE DATABASE ... IF NOT EXISTS` クエリで「File ... already exists」というエラーメッセージが返されることがあり、また `CREATE TABLE ... IF NOT EXISTS` および `DROP TABLE IF EXISTS` クエリで `Table ... is creating or attaching right now` が返されることがありました。 [#3101](https://github.com/ClickHouse/ClickHouse/pull/3101)
* 右辺が定数である LIKE および IN 式は、MySQL または ODBC テーブルからのクエリ実行時にリモートサーバーへ渡されるようになりました。 [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* MySQL および ODBC テーブルに対するクエリでは、`WHERE` 句内の定数式を含む比較条件がリモートサーバー側に渡されるようになりました。以前は、リテラル定数との比較のみが渡されていました。 [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* 端末における `Pretty` 形式の行幅を、漢字などの表意文字を含む文字列についても正しく計算。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3257)。
* `ALTER UPDATE` クエリには `ON CLUSTER` を指定できます。
* `JSONEachRow` 形式のデータ読み取り性能を向上。 [#3332](https://github.com/ClickHouse/ClickHouse/pull/3332)
* 互換性確保のために `LENGTH` および `CHARACTER_LENGTH` 関数の同義語を追加しました。`CONCAT` 関数は大文字と小文字を区別しなくなりました。 [#3306](https://github.com/ClickHouse/ClickHouse/pull/3306)
* `DateTime` 型の同義語として `TIMESTAMP` を追加しました。 [#3390](https://github.com/ClickHouse/ClickHouse/pull/3390)
* サーバーログでは、ログ行がクエリに関連していない場合でも、常に query&#95;id 用のスペースが確保されています。これにより、サードパーティ製ツールを使ってサーバーのテキストログを解析しやすくなります。
* クエリによるメモリ消費量は、消費量が次の整数ギガバイトの閾値を超えた時点でログに記録されます。 [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* Native プロトコルを使用するクライアントライブラリが、`INSERT` クエリに対してサーバーが想定する列数よりも誤って少ない列を送信してしまう場合に備え、互換モードを追加しました。この問題は `clickhouse-cpp` ライブラリを使用する場合に発生する可能性がありました。これまでは、このような状況になるとサーバーがクラッシュしていました。 [#3171](https://github.com/ClickHouse/ClickHouse/pull/3171)
* `clickhouse-copier` のユーザー定義の WHERE 句では、`partition_key` エイリアス（ソーステーブルのパーティションによる追加フィルタリング用）を使用できるようになりました。これは、コピー中にパーティショニング方式がわずかに変更される場合に有用です。 [#3166](https://github.com/ClickHouse/ClickHouse/pull/3166)
* 高負荷時におけるデータ読み取り速度を自動的に低減するため、`Kafka` エンジンのワークフローがバックグラウンドのスレッドプールに移行されました。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215)。
* `Cap'n'Proto` フォーマットにおける、`struct` のような構造の `Tuple` および `Nested` 値の読み取りサポート。 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3216)
* `firstSignificantSubdomain` 関数におけるトップレベルドメインのリストに、`biz` ドメインが含まれるようになりました。 [decaseal](https://github.com/ClickHouse/ClickHouse/pull/3219)
* 外部辞書の設定では、`null_value` はそのデータ型のデフォルト値として解釈されます。 [#3330](https://github.com/ClickHouse/ClickHouse/pull/3330)
* `Decimal` 型に対する `intDiv` および `intDivOrZero` 関数のサポート。 [b48402e8](https://github.com/ClickHouse/ClickHouse/commit/b48402e8712e2b9b151e0eef8193811d433a1264)
...
* `if` 関数に `UUID` 型用の特殊化を追加しました。 [#3366](https://github.com/ClickHouse/ClickHouse/pull/3366)
* `MergeTree table` から読み込む際の `open` および `close` システムコールの回数を減らしました。 [#3283](https://github.com/ClickHouse/ClickHouse/pull/3283)
* `TRUNCATE TABLE` クエリはどのレプリカ上でも実行できます（クエリはリーダーレプリカに転送されます）。 [Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/3375)

#### バグ修正: {#bug-fixes-10}


* `range_hashed` 辞書向けの `Dictionary` テーブルに関する不具合を修正しました。この問題はバージョン 18.12.17 で発生していました。 [#1702](https://github.com/ClickHouse/ClickHouse/pull/1702)
* `range_hashed` 辞書をロードする際に発生していたエラー（メッセージ `Unsupported type Nullable (...)`）を修正しました。このエラーはバージョン 18.12.17 で発生しました。 [#3362](https://github.com/ClickHouse/ClickHouse/pull/3362)
* 互いに近接して配置された多数の頂点を持つポリゴンに対して、不正確な計算が累積することにより発生していた `pointInPolygon` 関数のエラーを修正しました。 [#3331](https://github.com/ClickHouse/ClickHouse/pull/3331) [#3341](https://github.com/ClickHouse/ClickHouse/pull/3341)
* データパーツをマージした後、生成されたパーツのチェックサムが、別のレプリカで同じマージを行った結果のチェックサムと異なる場合、マージ結果は削除され、そのデータパーツが別のレプリカからダウンロードされます（これは正しい動作です）。しかし、データパーツのダウンロード後、そのパーツが既に存在するというエラーのため、ワーキングセットに追加できないことがありました（マージ後もしばらく遅れてからデータパーツが削除されるため）。これにより、同じデータを循環的にダウンロードしようとする問題が発生していました。 [#3194](https://github.com/ClickHouse/ClickHouse/pull/3194)
* クエリによる総メモリ消費量の計算が誤っていた問題を修正しました（この誤った計算により、`max_memory_usage_for_all_queries` 設定が正しく動作せず、`MemoryTracking` メトリクスの値も誤っていました）。この不具合はバージョン 18.12.13 で発生していました。 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3344)
* `CREATE TABLE ... ON CLUSTER ... AS SELECT ...` の動作を修正しました。この不具合はバージョン 18.12.13 で発生していました。 [#3247](https://github.com/ClickHouse/ClickHouse/pull/3247)
* `JOIN` がリモートサーバー上でのみ実行される場合に、クエリを発行したサーバーで不要なデータ構造を準備してしまっていた問題を修正しました。 [#3340](https://github.com/ClickHouse/ClickHouse/pull/3340)
* `Kafka` エンジンで、データ読み取り開始時に例外発生後にデッドロックが起きる問題と、完了時にロックが発生する問題を修正しました。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215)。
* `Kafka` テーブルに対して、オプションの `schema` パラメータ（`Cap'n'Proto` フォーマットのスキーマ）が指定されていませんでした。[Vojtech Splichal](https://github.com/ClickHouse/ClickHouse/pull/3150)
* ZooKeeper サーバーのアンサンブルに、接続は受け付けるもののハンドシェイクに応答する代わりにすぐに接続を閉じてしまうサーバーが含まれている場合、ClickHouse は別のサーバーへの接続を試みます。以前は、この状況で `Cannot read all data. Bytes read: 0. Bytes expected: 4.` というエラーが発生し、サーバーが起動できませんでした。 [8218cf3a](https://github.com/ClickHouse/ClickHouse/commit/8218cf3a5f39a43401953769d6d12a0bb8d29da9)
* ZooKeeper サーバーの集合に、DNS クエリでエラーが返されるサーバーが含まれている場合、それらのサーバーは無視されます。 [17b8e209](https://github.com/ClickHouse/ClickHouse/commit/17b8e209221061325ad7ba0539f03c6e65f87f29)
* `input_format_values_interpret_expressions = 1` の場合に、`VALUES` 形式でデータを挿入する際の `Date` と `DateTime` 間の型変換を修正しました。以前は、Unix Epoch からの経過日数を表す数値と Unix タイムスタンプとの間で変換が行われており、その結果、予期しない結果が発生していました。 [#3229](https://github.com/ClickHouse/ClickHouse/pull/3229)
* `Decimal` と整数型の相互変換を修正しました。 [#3211](https://github.com/ClickHouse/ClickHouse/pull/3211)
* `enable_optimize_predicate_expression` 設定の不具合を修正しました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3231)
* CSV 形式での浮動小数点数のパース時に、`;` などデフォルト以外の CSV セパレータを使用している場合に発生していたエラーを修正しました [#3155](https://github.com/ClickHouse/ClickHouse/pull/3155)
* `arrayCumSumNonNegative` 関数を修正しました（累積値が 0 未満の場合に負の値を加算しないようになりました）。[Aleksey Studnev](https://github.com/ClickHouse/ClickHouse/pull/3163)
* `Distributed` テーブルを基盤とする `Merge` テーブルにおける、`PREWHERE` 使用時の動作を修正しました。 [#3165](https://github.com/ClickHouse/ClickHouse/pull/3165)
* `ALTER UPDATE` クエリのバグ修正。
* バージョン 18.12 で発生した `odbc` テーブル関数の不具合を修正しました。[#3197](https://github.com/ClickHouse/ClickHouse/pull/3197)
* `StateArray` コンビネータ使用時の集約関数の動作を修正しました。 [#3188](https://github.com/ClickHouse/ClickHouse/pull/3188)
* `Decimal` 値を 0 で除算するとクラッシュする問題を修正しました。 [69dd6609](https://github.com/ClickHouse/ClickHouse/commit/69dd6609193beb4e7acd3e6ad216eca0ccfb8179)
* `Decimal` と整数引数を使用する演算における型の出力を修正しました。 [#3224](https://github.com/ClickHouse/ClickHouse/pull/3224)
* `Decimal128` 型に対する `GROUP BY` で発生していたセグメンテーションフォルトを修正しました。 [3359ba06](https://github.com/ClickHouse/ClickHouse/commit/3359ba06c39fcd05bfdb87d6c64154819621e13a)
* `log_query_threads` 設定（クエリ実行の各スレッドに関する情報のログ出力）は、`log_queries` オプション（クエリに関する情報のログ出力）が 1 に設定されている場合にのみ有効となるようになりました。`log_query_threads` オプションはデフォルトで有効化されているため、以前はクエリのログ出力が無効化されていてもスレッドに関する情報がログに記録されていました。 [#3241](https://github.com/ClickHouse/ClickHouse/pull/3241)
...
* バージョン 18.12.17 のサーバーとそれ以前のサーバーを同一クラスターで混在稼働させた場合の互換性の問題を修正しました。固定長および可変長の両方の GROUP BY キーを使用する分散クエリにおいて、集約対象のデータ量が多い場合に、返されるデータが常に完全に集約されず（2 つの異なる行が同じ集約キーを持つ）、不正な結果となることがありました。 [#3254](https://github.com/ClickHouse/ClickHouse/pull/3254)
* クエリがテストで宣言された置換の一部しか含まない場合の `clickhouse-performance-test` における置換処理を修正しました。 [#3263](https://github.com/ClickHouse/ClickHouse/pull/3263)
* `FINAL` と `PREWHERE` を併用した際に発生していたエラーを修正しました。 [#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* `ALTER` で追加された列に対して `PREWHERE` を使用した際に発生していたエラーを修正しました。 [#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* `DEFAULT` および `MATERIALIZED` の式に対して、`arrayJoin` が使用されていないことを確認するチェックを追加しました。以前は、データ挿入時に `arrayJoin` によりエラーが発生していました。 [#3337](https://github.com/ClickHouse/ClickHouse/pull/3337)
* `PREWHERE` 句に `arrayJoin` が含まれていないことを確認するチェックを追加しました。以前は、クエリ実行時に `Size ... does not match` や `Unknown compression method` といったメッセージが表示されることがありました。 [#3357](https://github.com/ClickHouse/ClickHouse/pull/3357)
* 等価比較からなる AND 連鎖を対応する IN 式に置き換える最適化の後、まれなケースで発生する可能性があったセグメンテーションフォルトを修正しました。 [liuyimin-bytedance](https://github.com/ClickHouse/ClickHouse/pull/3339)
* `clickhouse-benchmark` に対する軽微な修正。以前はクライアント情報がサーバーに送信されていませんでしたが、現在はシャットダウン時および反復回数の制限を行う際に、実行されたクエリ数がより正確に算出されるようになりました。 [#3351](https://github.com/ClickHouse/ClickHouse/pull/3351) [#3352](https://github.com/ClickHouse/ClickHouse/pull/3352)

#### 後方互換性のない変更: {#backward-incompatible-changes-1}

- `allow_experimental_decimal_type`オプションを削除しました。`Decimal`データ型はデフォルトで使用できます。[#3329](https://github.com/ClickHouse/ClickHouse/pull/3329)
...
- `clickhouse`ソースに対して`invalidate_query`(外部辞書の更新が必要かどうかを確認するクエリを指定する機能)が実装されました。[#3126](https://github.com/ClickHouse/ClickHouse/pull/3126)
- 範囲の境界を定義する`range_hashed`外部辞書キーとして、`UInt*`、`Int*`、`DateTime`データ型(`Date`型と共に)を使用できる機能が追加されました。`NULL`を使用して開いた範囲を指定できるようになりました。[Vasily Nemkov](https://github.com/ClickHouse/ClickHouse/pull/3123)
...
- `Ubuntu Precise`および旧バージョンでClickHouseを実行できなくなっていた非互換性(`glibc`バージョンへの不要な依存関係)を修正しました。この非互換性はバージョン18.12.13で発生していました。[#3130](https://github.com/ClickHouse/ClickHouse/pull/3130)
...
- `enable_optimize_predicate_expression`オプションがデフォルトで有効になりました(これはかなり楽観的です)。カラム名の検索に関連するクエリ解析エラーが発生した場合は、`enable_optimize_predicate_expression`を0に設定してください。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3107)
...
* `DECIMAL(digits, scale)` データ型（`Decimal32(scale)`、`Decimal64(scale)`、`Decimal128(scale)`）を追加しました。これを有効にするには、設定 `allow_experimental_decimal_type` を使用します。 [#2846](https://github.com/ClickHouse/ClickHouse/pull/2846) [#2970](https://github.com/ClickHouse/ClickHouse/pull/2970) [#3008](https://github.com/ClickHouse/ClickHouse/pull/3008) [#3047](https://github.com/ClickHouse/ClickHouse/pull/3047)
* `GROUP BY` のための新しい `WITH ROLLUP` 修飾子（代替構文：`GROUP BY ROLLUP(...)`）。 [#2948](https://github.com/ClickHouse/ClickHouse/pull/2948)
* `JOIN` を含むクエリでは、SQL 標準に準拠して、アスタリスク（`*`）はすべてのテーブルのカラム一覧に展開されます。ユーザー設定レベルで `asterisk_left_columns_only` を 1 に設定することで、従来の動作を復元できます。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2787)
* テーブル関数との JOIN のサポートを追加しました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
* clickhouse-client で Tab キーによるオートコンプリートをサポート。 [Sergey Shcherbin](https://github.com/ClickHouse/ClickHouse/pull/2447)
* clickhouse-client で Ctrl+C を押すと、入力したクエリを取り消せるようになりました。 [#2877](https://github.com/ClickHouse/ClickHouse/pull/2877)
* `join_default_strictness` 設定（値: `"`, `'any'`, `'all'`）を追加しました。これにより、`JOIN` に対して `ANY` または `ALL` を指定する必要がなくなります。 [#2982](https://github.com/ClickHouse/ClickHouse/pull/2982)
* クエリ処理に関連するサーバーログの各行には、クエリ ID が含まれます。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `clickhouse-client` でクエリ実行ログを取得できるようになりました（`send_logs_level` 設定を使用してください）。分散クエリ処理では、すべてのサーバーからログが集約されます。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `system.query_log` および `system.processes`（`SHOW PROCESSLIST`）テーブルで、クエリ実行時に変更されたすべての設定に関する情報（`Settings` データのネスト構造）が取得できるようになりました。`log_query_settings` という設定を追加しました。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `system.query_log` と `system.processes` テーブルで、クエリ実行に参加しているスレッド数に関する情報（`thread_numbers` 列を参照）が表示されるようになりました。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* ネットワーク経由の読み取りおよび書き込み、ディスクの読み取りおよび書き込みに要した時間、ネットワークエラーの発生回数、そしてネットワーク帯域幅が制限されている場合の待機に要した時間を計測する `ProfileEvents` カウンタを追加しました。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `rusage` に由来するシステムメトリクス（ユーザー空間およびカーネル空間での CPU 使用量、ページフォールト、コンテキストスイッチに関する情報を取得可能）と、taskstats メトリクス（I/O 待ち時間、CPU 待ち時間、ページキャッシュの有無それぞれについての読み取りおよび書き込みデータ量に関する情報を取得可能）を含む `ProfileEvents` カウンタを追加しました。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `ProfileEvents` カウンタはグローバルに、クエリ単位および各クエリ実行スレッドごとに適用されるため、クエリごとのリソース消費を詳細にプロファイルできます。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* 各クエリ実行スレッドの情報を含む `system.query_thread_log` テーブルおよび `log_query_threads` 設定を追加しました。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `system.metrics` および `system.events` テーブルに組み込みのドキュメントが追加されました。 [#3016](https://github.com/ClickHouse/ClickHouse/pull/3016)
* `arrayEnumerateDense` 関数を追加しました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2975)
* `arrayCumSumNonNegative` 関数と `arrayDifference` 関数が追加されました。 [Aleksey Studnev](https://github.com/ClickHouse/ClickHouse/pull/2942)
* `retention` 集約関数を追加しました。 [Sundy Li](https://github.com/ClickHouse/ClickHouse/pull/2887)
* 集約関数の状態をプラス演算子で加算（マージ）したり、非負の定数を掛けたりできるようになりました。 [#3062](https://github.com/ClickHouse/ClickHouse/pull/3062) [#3034](https://github.com/ClickHouse/ClickHouse/pull/3034)
* MergeTree ファミリーに属するテーブルで、仮想カラム `_partition_id` が利用できるようになりました。 [#3089](https://github.com/ClickHouse/ClickHouse/pull/3089)

#### 実験的機能: {#experimental-features-1}

- `LowCardinality(T)` データ型を追加しました。このデータ型は値のローカル辞書を自動的に作成し、辞書を展開することなくデータ処理を可能にします。[#2830](https://github.com/ClickHouse/ClickHouse/pull/2830)
- JITコンパイルされた関数のキャッシュと、コンパイル前の使用回数を記録するカウンターを追加しました。式をJITコンパイルするには、`compile_expressions` 設定を有効にしてください。[#2990](https://github.com/ClickHouse/ClickHouse/pull/2990) [#3077](https://github.com/ClickHouse/ClickHouse/pull/3077)

#### 改善: {#improvements-4}


* 放置されたレプリカが存在する場合にレプリケーションログが際限なく蓄積し続ける問題を修正しました。長時間遅延しているレプリカ向けに効果的な復旧モードを追加しました。
* `GROUP BY` において、集約列が複数あり、そのうち 1 つが文字列型で、他が固定長型のデータ型である場合のパフォーマンスを改善しました。
...
* バイナリ形式における文字列および配列の読み込み性能が向上しました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2955)
* 単一サーバー上に非常に多数のテーブルが存在する場合に、`system.tables` および `system.columns` へのクエリのパフォーマンスが向上し、メモリ消費量が削減されました。 [#2953](https://github.com/ClickHouse/ClickHouse/pull/2953)
* エラーで終了するクエリのストリームが大量に発生する場合のパフォーマンス問題を修正しました（`perf top` に `_dl_addr` 関数は表示されるものの、サーバーの CPU 使用率は高くない状況）。 [#2938](https://github.com/ClickHouse/ClickHouse/pull/2938)
...
* `UUID` データ型の機能の改善。 [#3074](https://github.com/ClickHouse/ClickHouse/pull/3074) [#2985](https://github.com/ClickHouse/ClickHouse/pull/2985)
* `UUID` データ型が The-Alchemist 辞書でサポートされました。[#2822](https://github.com/ClickHouse/ClickHouse/pull/2822)
* `visitParamExtractRaw` 関数はネストされた構造に対しても正しく動作します。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2974)
* `input_format_skip_unknown_fields` 設定が有効な場合、`JSONEachRow` 形式のオブジェクトフィールドは正しくスキップされます。 [BlahGeek](https://github.com/ClickHouse/ClickHouse/pull/2958)
* 条件付きの `CASE` 式では、`ELSE` を省略できるようになりました。これは `ELSE NULL` と同じ意味になります。 [#2920](https://github.com/ClickHouse/ClickHouse/pull/2920)
* ZooKeeper を使用する際の操作タイムアウトを構成できるようになりました。 [urykhy](https://github.com/ClickHouse/ClickHouse/pull/2971)
* `LIMIT n, m` 構文のオフセットは、`LIMIT n OFFSET m` として指定できます。 [#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
* `LIMIT` の代替として `SELECT TOP n` 構文を使用できます。 [#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
* system テーブルへの書き込み用キューのサイズを増やし、`SystemLog parameter queue is full` エラーの発生頻度を低減しました。
* `windowFunnel` 集約関数は、複数の条件を満たすイベントもサポートするようになりました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2801)
* `JOIN` の `USING` 句で重複列を使用できます。 [#3006](https://github.com/ClickHouse/ClickHouse/pull/3006)
* `Pretty` フォーマットでは、列のパディング幅による整列に上限が設けられました。`output_format_pretty_max_column_pad_width` 設定を使用してください。この値を超える幅のデータであっても、値自体は完全に表示されますが、テーブル内の他のセルが過度に広くなることはありません。 [#3003](https://github.com/ClickHouse/ClickHouse/pull/3003)
* `odbc` テーブル関数で、データベース名やスキーマ名を指定できるようになりました。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2885)
* `clickhouse-client` の設定ファイルに指定されたユーザー名を利用できるようにしました。 [Vladimir Kozbin](https://github.com/ClickHouse/ClickHouse/pull/2909)
* `ZooKeeperExceptions` カウンターは、`ZooKeeperUserExceptions`、`ZooKeeperHardwareExceptions`、`ZooKeeperOtherExceptions` の 3 種類のカウンターに分けられました。
* `ALTER DELETE` クエリはマテリアライズドビューに対しても使用できます。
* 非常に多数の `ReplicatedMergeTree` テーブルが存在する場合でも定期的な負荷スパイクが発生しないよう、これらのテーブルに対するクリーンアップスレッドの定期実行にランダム性を持たせました。
* `ATTACH TABLE ... ON CLUSTER` クエリのサポート。[#3025](https://github.com/ClickHouse/ClickHouse/pull/3025)

#### バグ修正: {#bug-fixes-13}


* `Dictionary` テーブルで、`Size of offsets does not match size of column` または `Unknown compression method` という例外がスローされる問題を修正しました。このバグはバージョン 18.10.3 で導入されました。 [#2913](https://github.com/ClickHouse/ClickHouse/issues/2913)
* マージ対象のデータパーツのうち一つが空（これらのパーツは、すべてのデータが削除された場合に、マージ中または `ALTER DELETE` 実行時に生成される）であり、かつマージに `vertical` アルゴリズムが使用されている場合に、`CollapsingMergeTree` テーブルをマージする処理で発生していたバグを修正しました。 [#3049](https://github.com/ClickHouse/ClickHouse/pull/3049)
* `Memory` テーブルに対して `DROP` または `TRUNCATE` と `SELECT` が同時に実行された際に発生する競合状態を修正しました。この問題が原因でサーバーがクラッシュする可能性がありました。このバグはバージョン 1.1.54388 で導入されました。 [#3038](https://github.com/ClickHouse/ClickHouse/pull/3038)
* `Replicated` テーブルへの挿入時に `Session is expired` エラーが返されるとデータ損失が発生する可能性があった問題を修正しました（データ損失は `ReplicatedDataLoss` メトリックで検出できます）。このエラーはバージョン 1.1.54378 で発生していました。 [#2939](https://github.com/ClickHouse/ClickHouse/pull/2939) [#2949](https://github.com/ClickHouse/ClickHouse/pull/2949) [#2964](https://github.com/ClickHouse/ClickHouse/pull/2964)
* `JOIN ... ON` 中に発生していたセグメンテーションフォルトを修正。 [#3000](https://github.com/ClickHouse/ClickHouse/pull/3000)
* `WHERE` 句が `WHERE table.column` のような完全修飾列名のみで構成されている場合に、列名の検索で発生していたエラーを修正しました。 [#2994](https://github.com/ClickHouse/ClickHouse/pull/2994)
* リモートサーバーに対して、サブクエリを含む IN 式からなる単一の列のみを要求した場合に、分散クエリの実行時に発生していた「Not found column」エラーを修正しました。 [#3087](https://github.com/ClickHouse/ClickHouse/pull/3087)
* 1つのシャードがローカルで、もう一方がリモートであり、かつ `PREWHERE` への移動最適化が有効になっている場合に、分散クエリで発生していた `Block structure mismatch in UNION stream: different number of columns` エラーを修正しました。 [#2226](https://github.com/ClickHouse/ClickHouse/pull/2226) [#3037](https://github.com/ClickHouse/ClickHouse/pull/3037) [#3055](https://github.com/ClickHouse/ClickHouse/pull/3055) [#3065](https://github.com/ClickHouse/ClickHouse/pull/3065) [#3073](https://github.com/ClickHouse/ClickHouse/pull/3073) [#3090](https://github.com/ClickHouse/ClickHouse/pull/3090) [#3093](https://github.com/ClickHouse/ClickHouse/pull/3093)
* 一部の非凸多角形に対して、`pointInPolygon` 関数を修正しました。 [#2910](https://github.com/ClickHouse/ClickHouse/pull/2910)
* `nan` と整数を比較した際に不正な結果となる問題を修正しました。 [#3024](https://github.com/ClickHouse/ClickHouse/pull/3024)
* まれにセグメンテーションフォルトを引き起こす可能性のあった `zlib-ng` ライブラリのエラーを修正しました。 [#2854](https://github.com/ClickHouse/ClickHouse/pull/2854)
* `AggregateFunction` 列を持つテーブルへの挿入時に、集約関数の状態が単純なものではなく（メモリを個別に割り当てる）、かつ 1 つの挿入リクエストが複数の小さなブロックを生成する場合に発生していたメモリリークを修正しました。 [#3084](https://github.com/ClickHouse/ClickHouse/pull/3084)
* 同一の `Buffer` または `MergeTree` テーブルを同時に作成および削除しようとした際に発生する競合状態を修正しました。
* タプルなどの、ある種の複雑な型を要素に持つタプル同士を比較する際に発生し得たセグメンテーションフォルトの問題を修正しました。 [#2989](https://github.com/ClickHouse/ClickHouse/pull/2989)
* 特定の `ON CLUSTER` クエリの実行時に発生する可能性があったセグメンテーションフォルトを修正しました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2960)
* `Nullable` 配列要素における `arrayDistinct` 関数のエラーを修正しました。 [#2845](https://github.com/ClickHouse/ClickHouse/pull/2845) [#2937](https://github.com/ClickHouse/ClickHouse/pull/2937)
* `enable_optimize_predicate_expression` オプションが、`SELECT *` を使用するケースも正しくサポートするようになりました。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2929)
* ZooKeeper セッションの再初期化時に発生するセグメンテーションフォルトを修正しました。 [#2917](https://github.com/ClickHouse/ClickHouse/pull/2917)
* ZooKeeper を使用している際に生じる可能性のあったブロッキングの問題を修正しました。
* `SummingMergeTree` でネストされたデータ構造を追加するコードの誤りを修正しました。
* 集約関数の状態のためにメモリを割り当てる際にはアラインメントが正しく考慮されるようになり、これにより集約関数の状態を実装する際にアラインメントを必要とする演算を使用できるようになりました。 [chenxing-xc](https://github.com/ClickHouse/ClickHouse/pull/2808)

#### セキュリティ修正: {#security-fix}

- ODBCデータソースの安全な使用。ODBCドライバとのやり取りには、別個の`clickhouse-odbc-bridge`プロセスを使用します。サードパーティのODBCドライバのエラーが、サーバの安定性や脆弱性の問題を引き起こすことはなくなりました。[#2828](https://github.com/ClickHouse/ClickHouse/pull/2828) [#2879](https://github.com/ClickHouse/ClickHouse/pull/2879) [#2886](https://github.com/ClickHouse/ClickHouse/pull/2886) [#2893](https://github.com/ClickHouse/ClickHouse/pull/2893) [#2921](https://github.com/ClickHouse/ClickHouse/pull/2921)
...
- JOINを含むクエリにおいて、アスタリスク文字はSQL標準に準拠して、すべてのテーブルの列リストに展開されます。ユーザ設定レベルで`asterisk_left_columns_only`を1に設定することで、以前の動作に戻すことができます。
...
- ZooKeeperとやり取りするためのライブラリのインターフェースが抽象として宣言されました。[#2950](https://github.com/ClickHouse/ClickHouse/pull/2950)
...
- 同時実行される`Set`または`Join`のエラーを修正しました。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2823)
- サブクエリ内の`UNION ALL`クエリにおいて、いずれかの`SELECT`クエリに重複するカラム名が含まれている場合に発生していた`Block structure mismatch in UNION stream: different number of columns`エラーを修正しました。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2094)
...
- アロケータを置き換えました:`tcmalloc`の代わりに`jemalloc`を使用するようになりました。一部のシナリオでは、最大20%の速度向上が見られます。ただし、最大20%遅くなるクエリも存在します。一部のシナリオではメモリ消費量が約10%削減され、安定性が向上しました。高負荷時において、ユーザー空間およびシステムのCPU使用率はわずかに増加します。[#2773](https://github.com/ClickHouse/ClickHouse/pull/2773)
...
- JOIN ON 構文における ON 式のサポートを追加しました:
...
- サーバーがバージョンのパッチコンポーネントをクライアントに渡すようになりました。パッチバージョンコンポーネントに関するデータは `system.processes` および `query_log` に含まれています。[#2646](https://github.com/ClickHouse/ClickHouse/pull/2646)
...
### ClickHouseリリース18.4.0、2018年7月28日 {#clickhouse-release-18-4-0-2018-07-28}
...
- `remote`または`cluster table function`の引数として、テーブルの代わりにテーブル関数を使用する機能を追加 [#2708](https://github.com/ClickHouse/ClickHouse/pull/2708)。
...
- `Kafka`から読み取る際に任意のメッセージ区切り文字を追加できるようになりました [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2701)。

#### 改善: {#improvements-8}

- `ALTER TABLE t DELETE WHERE`クエリは、WHERE条件の影響を受けなかったデータパートを書き換えなくなりました [#2694](https://github.com/ClickHouse/ClickHouse/pull/2694)。
- `ReplicatedMergeTree`テーブルの`use_minimalistic_checksums_in_zookeeper`オプションがデフォルトで有効になりました。この設定はバージョン1.1.54378(2018年4月16日)で追加されました。1.1.54378より古いバージョンはインストールできなくなりました。
- `ON CLUSTER`を指定した`KILL`および`OPTIMIZE`クエリの実行をサポート [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2689)。

#### バグ修正: {#bug-fixes-16}

- IN式を使用した集計における`Column ... is not under an aggregate function and not in GROUP BY`エラーを修正しました。このバグはバージョン18.1.0で発生しました。([bbdd780b](https://github.com/ClickHouse/ClickHouse/commit/bbdd780be0be06a0f336775941cdd536878dd2c2))
...
- 非レプリケートMergeTreeテーブルに対する`ALTER TABLE t DELETE WHERE`クエリのサポート([#2634](https://github.com/ClickHouse/ClickHouse/pull/2634))。
- `uniq*`ファミリーの集約関数における任意の型のサポート([#2010](https://github.com/ClickHouse/ClickHouse/issues/2010))。
- 比較演算子における任意の型のサポート([#2026](https://github.com/ClickHouse/ClickHouse/issues/2026))。
- `users.xml`ファイルで`10.0.0.1/255.255.255.0`形式のサブネットマスクの設定が可能になりました。これは、途中にゼロを含むIPv6ネットワークのマスクを使用するために必要です([#2637](https://github.com/ClickHouse/ClickHouse/pull/2637))。
- `arrayDistinct`関数を追加([#2670](https://github.com/ClickHouse/ClickHouse/pull/2670))。
- SummingMergeTreeエンジンがAggregateFunction型のカラムで動作するようになりました([Constantin S. Pan](https://github.com/ClickHouse/ClickHouse/pull/2566))。
...
- 一時テーブルに対するTRUNCATEコマンドを修正([Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2624))。
- レスポンスの読み取り中にネットワークエラーが発生した際にZooKeeperクライアントライブラリで発生する稀なデッドロックを修正([c315200](https://github.com/ClickHouse/ClickHouse/commit/c315200e64b87e44bdf740707fc857d1fdf7e947))。
- Nullable型へのCASTにおけるエラーを修正([#1322](https://github.com/ClickHouse/ClickHouse/issues/1322))。
- 区間の境界が一致する場合の`maxIntersection()`関数の誤った結果を修正([Michael Furmur](https://github.com/ClickHouse/ClickHouse/pull/2657))。
- 関数引数内のOR式チェーンの誤った変換を修正([chenxing-xc](https://github.com/ClickHouse/ClickHouse/pull/2663))。
- 別のサブクエリ内に`IN (subquery)`式を含むクエリのパフォーマンス低下を修正([#2571](https://github.com/ClickHouse/ClickHouse/issues/2571))。
- 大文字でない`CAST`関数を使用する分散クエリにおける異なるバージョンのサーバー間の非互換性を修正([fe8c4d6](https://github.com/ClickHouse/ClickHouse/commit/fe8c4d64e434cacd4ceef34faa9005129f2190a5))。
- 外部DBMSへのクエリにおける識別子の引用符の欠落を修正([#2635](https://github.com/ClickHouse/ClickHouse/issues/2635))。

#### 後方互換性のない変更: {#backward-incompatible-changes-6}

- 数値ゼロを含む文字列からDateTimeへの変換は機能しません。例:`SELECT toDateTime('0')`。これは、テーブルでの`DateTime DEFAULT '0'`や辞書での`<null_value>0</null_value>`が機能しない理由でもあります。解決策:`0`を`0000-00-00 00:00:00`に置き換えてください。
...
- レプリケートされたデータの送受信時における読み書き用ソケットのタイムアウトが非常に短い(1秒)問題を修正しました。この問題により、ネットワークやディスクに負荷がある場合に大きなパーツのダウンロードができず、パーツのダウンロードが循環的に試行される結果となっていました。このエラーはバージョン 1.1.54388 で発生していました。
- テーブルに重複したデータブロックを挿入した場合に ZooKeeper で chroot を使用する際の問題を修正しました。
- Nullable 要素を持つ配列に対して `has` 関数が正しく動作するようになりました ([#2115](https://github.com/ClickHouse/ClickHouse/issues/2115))。
- 分散クエリで使用する際に `system.tables` テーブルが正しく動作するようになりました。`metadata_modification_time` と `engine_full` カラムは非仮想カラムになりました。これらのカラムのみをテーブルからクエリした場合に発生していたエラーを修正しました。
- 空のデータブロックを挿入した後の空の `TinyLog` テーブルの動作を修正しました ([#2563](https://github.com/ClickHouse/ClickHouse/issues/2563))。
- ZooKeeper のノードの値が NULL の場合でも `system.zookeeper` テーブルが動作するようになりました。

### ClickHouse リリース 1.1.54390, 2018-07-06 {#clickhouse-release-1-1-54390-2018-07-06}

#### 新機能: {#new-features-11}

- クエリを `multipart/form-data` 形式(`query` フィールド内)で送信できるようになりました。これはクエリ処理のために外部データも送信する場合に便利です ([Olga Hvostikova](https://github.com/ClickHouse/ClickHouse/pull/2490))。
- CSV 形式でデータを読み取る際に、シングルクォートまたはダブルクォートの処理を有効または無効にする機能を追加しました。これは `format_csv_allow_single_quotes` と `format_csv_allow_double_quotes` 設定で構成できます ([Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2574))。
- 非レプリケート版の `MergeTree` でパーティションを指定せずに `OPTIMIZE TABLE ... FINAL` を使用できるようになりました ([Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2599))。

#### 改善: {#improvements-10}

- テーブルインデックスを使用できる場合の IN 演算子使用時におけるパフォーマンスの向上、メモリ消費量の削減、および正確なメモリ消費量の追跡を実現しました ([#2584](https://github.com/ClickHouse/ClickHouse/pull/2584))。
- データパーツを追加する際の冗長なチェックサム検証を削除しました。これは多数のレプリカが存在する場合に重要です。なぜなら、このような場合、検証の総数が N^2 に等しくなっていたためです。
...
- 非常に多数の式が存在する場合のクエリ解析パフォーマンスを改善しました ([#2572](https://github.com/ClickHouse/ClickHouse/pull/2572))。
- `ReplicatedMergeTree` テーブルにおけるマージ用データパーツの選択を高速化しました。ZooKeeper セッションの回復を高速化しました ([#2597](https://github.com/ClickHouse/ClickHouse/pull/2597))。
- `MergeTree` テーブルの `format_version.txt` ファイルが存在しない場合に再作成されるようになりました。これは、ファイルなしでディレクトリ構造をコピーした後に ClickHouse を起動する場合に有用です ([Ciprian Hacman](https://github.com/ClickHouse/ClickHouse/pull/2593))。

#### バグ修正: {#bug-fixes-19}


- ZooKeeperとの連携時に、サーバー再起動前にテーブルのセッションおよび読み取り専用状態を復旧できなくなる可能性があったバグを修正しました。
- ZooKeeperとの連携時に、セッションが中断された場合に古いノードが削除されない可能性があったバグを修正しました。
...
- 主キー列が同じサイズの符号付き整数と符号なし整数間で型を変換する関数内に配置されている場合のMergeTreeテーブルのインデックスに関するバグを修正しました([#2603](https://github.com/ClickHouse/ClickHouse/pull/2603))。
- `macros`が使用されているが設定ファイルに存在しない場合のセグメンテーション違反を修正しました([#2570](https://github.com/ClickHouse/ClickHouse/pull/2570))。
- クライアント再接続時のデフォルトデータベースへの切り替えに関する問題を修正しました([#2583](https://github.com/ClickHouse/ClickHouse/pull/2583))。
- `use_index_for_in_with_subqueries`設定が無効化されている場合に発生していたバグを修正しました。

#### セキュリティ修正: {#security-fix-1}

- MySQLに接続している場合にファイルの送信ができなくなりました(`LOAD DATA LOCAL INFILE`)。
...
- `Kafka`エンジンからマテリアライズドビューへデータを書き込む際の競合状態を修正しました([Yangkuan Liu](https://github.com/ClickHouse/ClickHouse/pull/2448))。
...
- 分散クエリでバージョン1.1.54388(またはそれ以降)のサーバーと古いバージョンのサーバーを同時に使用し、クエリに`AS`キーワードを含まない`cast(x, 'Type')`式があり、`cast`という単語が大文字でない場合、`Not found column cast(0, 'UInt8') in block`のようなメッセージで例外がスローされます。解決策:クラスタ全体でサーバーを更新してください。
...
- ZooKeeperクライアントライブラリのエラーを修正しました。このエラーは、ZooKeeper設定で空でない`chroot`プレフィックスが使用されている場合に、ウォッチの喪失、分散DDLキューのフリーズ、レプリケーションキューの速度低下を引き起こしていました。
...
- 左辺に配列が指定された場合のIN式の特殊な解釈を削除しました。以前は、式`arr IN (set)`は「少なくとも1つの`arr`要素が`set`に属する」と解釈されていました。新しいバージョンで同じ動作を得るには、`arrayExists(x -> x IN (set), arr)`と記述してください。
...
### ClickHouse リリース 1.1.54370, 2018-03-16 {#clickhouse-release-1-1-54370-2018-03-16}
...
- リモートテーブルへのクエリで辞書を使用する機能を復元しました。これは、リクエスト元サーバーに辞書が存在しない場合でも機能します。この機能はリリース1.1.54362で失われていました。
- `SELECT * FROM remote('server2', default.table) WHERE col IN (SELECT col2 FROM default.table)`のようなクエリで、`IN`の右辺がローカルではなくリモートの`default.table`を使用すべき場合の動作を復元しました。この動作はバージョン1.1.54358で破損していました。
- `Not found column ... in block`の不要なエラーレベルのログ出力を削除しました。

### ClickHouse リリース 1.1.54362, 2018-03-11 {#clickhouse-release-1-1-54362-2018-03-11}

#### 新機能: {#new-features-16}


* 空の集合（`SELECT count(*) FROM table WHERE 0` など）に対する `GROUP BY` なしの集約は、SQL 標準に準拠して、集約関数の値が null 値の 1 行だけの結果を返すようになりました。従来の動作（空の結果を返す）に戻すには、`empty_result_for_aggregation_by_empty_set` を 1 に設定してください。
* `UNION ALL` に対する型変換を追加しました。SQL 標準に準拠し、`UNION ALL` の各 `SELECT` 句で異なるエイリアス名を使用できるようになりました。
* `LIMIT BY` 句では任意の式がサポートされています。以前は、`SELECT` の結果として得られる列のみを使用できました。
* `MergeTree` テーブルのインデックスは、主キーを構成する列からなる式のタプルに対して `IN` が適用される場合に使用されます。例: `WHERE (UserID, EventDate) IN ((123, '2000-01-01'), ...)` (Anastasiya Tsarkova)。
* クラスター間コピーおよびデータの再シャーディングを行うツール `clickhouse-copier` を追加しました（ベータ版）。
* コンシステントハッシュ関数 `yandexConsistentHash`、`jumpConsistentHash`、`sumburConsistentHash` を追加しました。これらは、後続の再シャーディング時のネットワークトラフィック量を削減するためのシャーディングキーとして利用できます。
* 追加された関数: `arrayAny`、`arrayAll`、`hasAny`、`hasAll`、`arrayIntersect`、`arrayResize`。
* `arrayCumSum` 関数を追加しました（Javi Santana）。
* `parseDateTimeBestEffort`、`parseDateTimeBestEffortOrZero`、`parseDateTimeBestEffortOrNull` 関数を追加し、多種多様な形式のテキストを含む文字列から DateTime を読み取れるようにしました。
* データは、更新時に外部ディクショナリから部分的に再読み込みできます（前回のロード時よりも指定フィールドの値が大きいレコードだけを読み込む）（Arsen Hakobyan）。
* `cluster` テーブル関数を追加しました。例: `cluster(cluster_name, db, table)`。`remote` テーブル関数では、クラスタ名を識別子として指定することで、これを第1引数として渡すことができます。
* `remote` および `cluster` テーブル関数は、`INSERT` クエリ内で使用できます。
* `system.tables` テーブルに `create_table_query` および `engine_full` 仮想カラムを追加しました。`metadata_modification_time` カラムは仮想カラムです。
* `system.tables` および `system.databases` テーブルに `data_path` 列と `metadata_path` 列を追加し、`system.parts` および `system.parts_columns` テーブルに `path` 列を追加しました。
* `system.part_log` テーブルのマージに関する追加情報を追加しました。
* `system.query_log` テーブルに対して任意のパーティションキーを使用できるようになりました（Kirill Shvakov）。
* `SHOW TABLES` クエリで一時テーブルも表示されるようになりました。`system.tables` に一時テーブルと `is_temporary` 列が追加されました (zhang2014)。
* `DROP TEMPORARY TABLE` および `EXISTS TEMPORARY TABLE` クエリを追加しました（zhang2014）。
* 一時テーブルに対する `SHOW CREATE TABLE` のサポート (zhang2014)。
* 内部プロセスが使用する設定向けに `system_profile` 構成パラメータを追加しました。
* `MongoDB` 辞書で `object_id` を属性として読み込むためのサポート（Pavel Litvinenko）。
* `MongoDB` ソースを使用する外部ディクショナリのデータ読み込み時に、デフォルト値として `null` を解釈する機能（Pavel Litvinenko）。
* `Values` フォーマットで、シングルクォートなしの Unix タイムスタンプから `DateTime` 値を読み込む。
* `remote` テーブル関数では、一部のレプリカに要求されたテーブルが存在しない場合のフェイルオーバーをサポートしています。
* `clickhouse-server` を実行する際、設定はコマンドラインから上書きできます。例：`clickhouse-server -- --logger.level=information`。
* `FixedString` 型の引数に対する `empty` 関数を実装しました。この関数は、文字列がすべてヌルバイトのみで構成されている場合に 1 を返します (zhang2014)。
* いくつかのアドレスで待ち受けできない場合でも終了せず、少なくとも 1 つの待ち受けアドレスに対してリッスンを行うための `listen_try` 設定パラメータを追加しました（IPv4 または IPv6 のサポートが無効化されているシステムで有用です）。
* `VersionedCollapsingMergeTree` テーブルエンジンを追加。
* `library` 辞書ソースにおける行および任意の数値型のサポート。
* `MergeTree` テーブルは主キーなしでも使用できます（その場合は `ORDER BY tuple()` を指定する必要があります）。
* 引数が `NULL` でない場合、`Nullable` 型は非 `Nullable` 型に `CAST` できます。
* `RENAME TABLE` は `VIEW` に対しても使用できます。
* `throwIf` 関数を追加しました。
* `odbc_default_field_size` オプションを追加しました。これにより、ODBC ソースから読み込まれる値の最大サイズを増やすことができます（デフォルトは 1024 です）。
* `system.processes` テーブルおよび `SHOW PROCESSLIST` に、`is_cancelled` 列と `peak_memory_usage` 列が追加されました。

#### 改善点: {#improvements-15}

- `INSERT SELECT`クエリや`SELECT`サブクエリの中間データに対して、結果の制限とクォータが適用されなくなりました。
- サーバー起動時に`Replicated`テーブルのステータスをチェックする際の`force_restore_data`の誤トリガーが減少しました。
...
- 要素の1つとして空配列を含む定数多次元配列の場合における`arrayElement`関数のパフォーマンスが向上しました。例:`[[1], []][x]`。
...
- `Float`データ型に対して、テキスト形式の空行を数値0として解析する機能が追加されました。この機能は以前利用可能でしたが、リリース1.1.54342で失われていました。
...
- 1億項目を超えるカーディナリティに対する`uniqHLL12`および`uniqCombined`関数の精度の低さを修正しました(Alex Bocharov)。
...
- 正規表現を使用する関数におけるわずかなパフォーマンス低下を修正しました。
...
- テーブルにおける多次元配列とタプル（`Tuple`データ型）の格納サポートを追加しました。
- `DESCRIBE`および`INSERT`クエリでのテーブル関数のサポート。`DESCRIBE`でのサブクエリのサポートを追加しました。例：`DESC TABLE remote('host', default.hits)`; `DESC TABLE (SELECT 1)`; `INSERT INTO TABLE FUNCTION remote('host', default.hits)`。`INSERT INTO`に加えて`INSERT INTO TABLE`のサポート。
...
- `toTimeZone`、`timeDiff`、`toQuarter`、`toRelativeQuarterNum`関数を追加しました。`toRelativeHour`/`Minute`/`Second`関数は、引数として`Date`型の値を受け取ることができます。`now`関数名は大文字と小文字を区別します。
...
- `system.settings`テーブルにドキュメントを含むカラムを追加しました（Kirill Shvakov）。
- `MergeTree`テーブルの各データパートにおけるカラムサイズに関する情報を含む`system.parts_columns`テーブルを追加しました。
- ロード済みの`CatBoost`機械学習モデルに関する情報を含む`system.models`テーブルを追加しました。
...
- 文字列引数からの集約関数 `min`、`max`、`any`、`anyLast`、`anyHeavy`、`argMin`、`argMax` のパフォーマンスを改善しました。
- 関数 `isInfinite`、`isFinite`、`isNaN`、`roundToExp2` のパフォーマンスを改善しました。
- テキスト形式での `Date` および `DateTime` 型の値の解析とフォーマットのパフォーマンスを改善しました。
- 浮動小数点数の解析のパフォーマンスと精度を改善しました。
- 左側と右側の部分が `USING` に含まれない同一名の列を持つ場合の `JOIN` のメモリ使用量を削減しました。
- 計算の安定性を犠牲にすることで、集約関数 `varSamp`、`varPop`、`stddevSamp`、`stddevPop`、`covarSamp`、`covarPop`、`corr` のパフォーマンスを改善しました。従来の関数は `varSampStable`、`varPopStable`、`stddevSampStable`、`stddevPopStable`、`covarSampStable`、`covarPopStable`、`corrStable` という名前で利用可能です。
...
- `DISTINCT`、`JOIN`、`uniq`集約関数および外部ディクショナリに対する`UUID`データ型の不足していたサポートを追加しました(Evgeniy Ivanov)。`UUID`のサポートはまだ不完全です。
...
- レプリケーションキューの更新における非原子性を修正しました。これにより、サーバーが再起動するまでレプリカが同期しなくなる可能性がありました。
...
- `Nullable`型のサポートを再設計しました。サーバークラッシュにつながる可能性のあるバグを修正しました。`NULL`サポートに関連するほぼすべての他のバグを修正しました:INSERT SELECTにおける誤った型変換、HAVINGおよびPREWHEREにおけるNullableの不十分なサポート、`join_use_nulls`モード、`OR`演算子の引数としてのNullable型など。
...
- `-Array`コンビネータを持つ集約関数のn項バリアントの引数における配列サイズの等価性チェックの不足を追加しました。
...
- `systemd`のサポートを追加しました(Vladimir Smirnov)。一部のOSイメージとの非互換性のため、デフォルトでは無効になっており、手動で有効化できます。
...
- 混乱を避けるため、`runningIncome`関数を`runningDifferenceStartingWithFirstvalue`に名前変更しました。
- テーブルなしでFROMの直後にARRAY JOINが指定される場合の`FROM ARRAY JOIN arr`構文を削除しました(Amos Bird)。
- デモンストレーション目的でのみ使用されていた`BlockTabSeparated`形式を削除しました。
- 集約関数`varSamp`、`varPop`、`stddevSamp`、`stddevPop`、`covarSamp`、`covarPop`、`corr`の状態形式を変更しました。これらの集約関数の状態をテーブルに保存している場合(`AggregateFunction`データ型または対応する状態を持つマテリアライズドビューを使用)、feedback@clickhouse.comまでご連絡ください。
- 以前のサーバーバージョンには文書化されていない機能がありました。集約関数がパラメータに依存する場合でも、AggregateFunctionデータ型でパラメータなしで指定できました。例:`AggregateFunction(quantiles(0.5, 0.9), UInt64)`の代わりに`AggregateFunction(quantiles, UInt64)`。この機能は失われました。文書化されていませんでしたが、将来のリリースで再びサポートする予定です。
