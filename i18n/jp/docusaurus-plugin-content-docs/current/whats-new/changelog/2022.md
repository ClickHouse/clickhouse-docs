...
### <a id="2212"></a> ClickHouse リリース 22.12, 2022-12-15. [プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.12/), [動画](https://www.youtube.com/watch?v=sREupr6uc2k) \{#a-id2212a-clickhouse-release-2212-2022-12-15\}
...
* 起動時のロード失敗を回避するため、`CREATE` クエリ内の UDF を置き換えるようにしました。さらに、カラムの `DEFAULT` 式として UDF を使用できるようになりました。 [#43539](https://github.com/ClickHouse/ClickHouse/pull/43539) ([Antonio Andelic](https://github.com/antonio2368))。
* 次のクエリにおけるパーツを削除する方法を変更しました：TRUNCATE TABLE、ALTER TABLE DROP PART、ALTER TABLE DROP PARTITION。これらのクエリは、古いパーツを覆う空のパーツを作成するようになりました。これにより、TRUNCATE クエリは後続の排他ロックの取得なしに動作し、その結果、同時実行中の読み取りがロックされなくなります。また、これらすべてのクエリで永続性も実現しました。リクエストが成功した場合、その後にパーツが「復活」して現れることはありません。なお、原子性はトランザクションスコープ内でのみ保証されます。[#41145](https://github.com/ClickHouse/ClickHouse/pull/41145)（[Sema Checherinda](https://github.com/CheSema)）。
...
* クライアントで STDIN から読み込む際、進捗表示に読み取り済みの行数を表示するようにしました。 [#43423](https://github.com/ClickHouse/ClickHouse/issues/43423) をクローズ。[#43442](https://github.com/ClickHouse/ClickHouse/pull/43442)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* テーブル関数 `s3` および `S3` エンジンから読み込む際にプログレスバーを表示するようにしました。 [#43454](https://github.com/ClickHouse/ClickHouse/pull/43454) ([Kseniia Sumarokova](https://github.com/kssenii)).
* プログレスバーには読み取り行数と書き込み行数の両方が表示されます。 [#43496](https://github.com/ClickHouse/ClickHouse/pull/43496) ([Ilya Yatsishin](https://github.com/qoega)).
* `filesystemAvailable` および関連する関数でディスク名を指定するオプション引数を 1 つサポートするようにし、`filesystemFree` を `filesystemUnreserved` に変更しました。[#35076](https://github.com/ClickHouse/ClickHouse/issues/35076) をクローズ。[#42064](https://github.com/ClickHouse/ClickHouse/pull/42064)（[flynn](https://github.com/ucasfl)）。
* LDAP 連携: `search_limit` のデフォルト値を 256 に増やし、任意の値に変更できるようにするための LDAP サーバー設定オプションを追加しました。[#42276](https://github.com/ClickHouse/ClickHouse/issues/42276) をクローズ。[#42461](https://github.com/ClickHouse/ClickHouse/pull/42461) ([Vasily Nemkov](https://github.com/Enmk))。
...
* クエリで指定された接続タイムアウト設定を Distributed async INSERT に対して適用できるようにしました。 [#43156](https://github.com/ClickHouse/ClickHouse/pull/43156) ([Azat Khuzhin](https://github.com/azat)).
...
* TTL ルールに従って、完全に期限切れとなったパーツを優先的に削除するようにしました（[#42869](https://github.com/ClickHouse/ClickHouse/issues/42869) を参照）。[#43222](https://github.com/ClickHouse/ClickHouse/pull/43222)（[zhongyuankai](https://github.com/zhongyuankai)）。
...
* ストレージ `S3` およびテーブル関数 `s3` で、`Parquet`、`Arrow`、`ORC` 形式のネスト型サブカラムの読み取りをサポートしました。 [#43329](https://github.com/ClickHouse/ClickHouse/pull/43329) ([chen](https://github.com/xiedeyantu)).
...
* `trace_type = 'ProfileEvent'` のプロファイルイベントを、各インクリメントのたびに、その時点のスタック、プロファイルイベント名、およびインクリメントの値とともに `system.trace_log` に記録できるようにしました。これは設定 `trace_profile_events` を有効にすることで利用でき、クエリのパフォーマンス調査に使用できます。 [#43639](https://github.com/ClickHouse/ClickHouse/pull/43639) ([Anton Popov](https://github.com/CurtizJ)).
...
* 複数の JOIN を含む最適化が行われている場合でも、クエリ内のエラーを正しく報告できるようにしました。 [#43583](https://github.com/ClickHouse/ClickHouse/pull/43583) ([Salvatore](https://github.com/tbsal)).
...
#### バグ修正（公式 stable または prestable リリースにおけるユーザーにとって目に見える誤動作） \{#bug-fix-user-visible-misbehavior-in-official-stable-or-prestable-release\}
...
### <a id="2211"></a> ClickHouse リリース 22.11（2022-11-17）。[Presentation](https://presentations.clickhouse.com/2022-release-22.11/)、[Video](https://www.youtube.com/watch?v=LR-fckOOaFo) \{#a-id2211a-clickhouse-release-2211-2022-11-17\}
...
* ファイルシステムおよび S3 の再帰的なディレクトリ走査用に `**` グロブパターンのサポートを追加しました。 [#36316](https://github.com/ClickHouse/ClickHouse/issues/36316) を解決しました。 [#42376](https://github.com/ClickHouse/ClickHouse/pull/42376)（[SmitaRKulkarni](https://github.com/SmitaRKulkarni)）。
...
* 適用された行レベルポリシーを `system.query_log` に追加しました。 [#39819](https://github.com/ClickHouse/ClickHouse/pull/39819) ([Vladimir Chebotaryov](https://github.com/quickhouse)).
...
* モジュロ演算に基づいて常に非負の結果を返す関数 `pmod` を追加しました。 [#42755](https://github.com/ClickHouse/ClickHouse/pull/42755) ([李扬](https://github.com/taiyang-li)).
...
* パーティション全体のみを対象に古いパーツを最適化するための `min_age_to_force_merge_on_partition_only` 設定を追加。 [#42659](https://github.com/ClickHouse/ClickHouse/pull/42659) ([Antonio Andelic](https://github.com/antonio2368)).
...
* 他の型の中で `Object` 型をサポートしました（例: `Array(JSON)`）。[#36969](https://github.com/ClickHouse/ClickHouse/pull/36969)（[Anton Popov](https://github.com/CurtizJ)）。
...
* 取得されたパーツについて、それに包含される既存パーツを削除して、レプリケーション遅延の増大を防ぐようにしました。 [#39737](https://github.com/ClickHouse/ClickHouse/pull/39737) ([Azat Khuzhin](https://github.com/azat)).
* `/dev/tty` が利用可能な場合、clickhouse-client および clickhouse-local の進捗は STDERR に書き込まず、端末に直接表示されます。これにより、STDERR がファイルにリダイレクトされている場合でも進捗を確認でき、そのファイルが端末のエスケープシーケンスで汚染されることがありません。進捗は `--progress false` で無効にできます。これにより [#32238](https://github.com/ClickHouse/ClickHouse/issues/32238) がクローズされました。 [#42003](https://github.com/ClickHouse/ClickHouse/pull/42003)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* base64 符号化関数に `FixedString` 型入力のサポートを追加しました。 [#42285](https://github.com/ClickHouse/ClickHouse/pull/42285) ([ltrk2](https://github.com/ltrk2)).
...
* 関連する関数で返されるトークン数の最大値を設定できるように、関数 `tokens` をリファクタリングしました（デフォルトでは無効）。 [#42673](https://github.com/ClickHouse/ClickHouse/pull/42673) ([李扬](https://github.com/taiyang-li)).
...
* クエリプラン上で `read-in-order` 最適化を実装しました。これはデフォルトで有効になっています。以前の AST ベースのバージョンを使用するには、`query_plan_read_in_order = 0` を設定してください。[#42829](https://github.com/ClickHouse/ClickHouse/pull/42829) ([Nikolai Kochetov](https://github.com/KochetovNicolai)).
* S3 へのバックアップ時に、S3 のマルチパートアップロードにおけるパート数上限（最大 10 000 個）に達してエラーが発生するのを避けるため、アップロードパートのサイズを指数関数的に増加させるようにしました。 [#42833](https://github.com/ClickHouse/ClickHouse/pull/42833) ([Vitaly Baranov](https://github.com/vitlibar)).
...
* `INFORMATION_SCHEMA` テーブルを変更し、ClickHouse が MySQL 互換プロトコルを使用して自身に接続できるようにしました。エイリアスではなくカラムを追加しました（[#9769](https://github.com/ClickHouse/ClickHouse/issues/9769) に関連）。これにより、さまざまな MySQL クライアントとの互換性が向上します。[#43198](https://github.com/ClickHouse/ClickHouse/pull/43198)（[Filatenkov Artur](https://github.com/FArthur-cmd)）。
* Power BI が MySQL プロトコル経由で接続する場合の互換性を向上させるため、いくつかの関数を追加しました [#42612](https://github.com/ClickHouse/ClickHouse/pull/42612)（[Filatenkov Artur](https://github.com/FArthur-cmd)）。
* ダッシュボード変更時の操作性を改善 [#42872](https://github.com/ClickHouse/ClickHouse/pull/42872) ([Vladimir C](https://github.com/vdimir)).
...
### <a id="2210"></a> ClickHouse リリース 22.10（2022-10-25） [発表資料](https://presentations.clickhouse.com/2022-release-22.10/)、[動画](https://www.youtube.com/watch?v=sz9SES5-mdc) \{#a-id2210a-clickhouse-release-2210-2022-10-25\}
...
* クライアントのプロンプトにおける `{database}` マクロのサポートを削除しました。データベースが未指定の場合に正しく表示されず、`USE` 文でも更新されていませんでした。これにより [#25891](https://github.com/ClickHouse/ClickHouse/issues/25891) がクローズされます。 [#42508](https://github.com/ClickHouse/ClickHouse/pull/42508) ([Alexey Milovidov](https://github.com/alexey-milovidov))。
...
* HTTP プロトコルの非標準拡張である HTTP の `Accept-Encoding` ヘッダーに `lz4`、`bz2`、`snappy` 圧縮方式のサポートを追加しました。 [#42071](https://github.com/ClickHouse/ClickHouse/pull/42071) ([Nikolay Degterinsky](https://github.com/evillique)).
...
* `allow_experimental_analyzer` 設定のもとで、クエリ解析およびプラン作成用の新しいインフラストラクチャを追加しました。[#31796](https://github.com/ClickHouse/ClickHouse/pull/31796)（[Maksim Kita](https://github.com/kitaisreal)）。
...
* 以前の接続が期限切れになった場合に、新しい ZooKeeper への接続を再確立する処理を、より迅速に行えるよう改善しました。以前は、デフォルトで毎分実行されるタスクがあり、そのためテーブルがその間およそ 1 分間読み取り専用状態になる可能性がありました。 [#41092](https://github.com/ClickHouse/ClickHouse/pull/41092) ([Nikita Mikhaylov](https://github.com/nikitamikhaylov))。
* これで、プロジェクションを zero-copy レプリケーション（zero-copy レプリケーションは本番環境向けではない機能です）と併用できるようになりました。 [#41147](https://github.com/ClickHouse/ClickHouse/pull/41147) ([alesapin](https://github.com/alesapin)).
...
* `async_insert_max_data_size` または `async_insert_busy_timeout_ms` をクエリ単位のスコープで変更できるようにしました。たとえば、ユーザーがデータをまれにしか挿入せず、デフォルト設定を調整するためのサーバー設定にアクセスできない場合などに役立ちます。 [#40668](https://github.com/ClickHouse/ClickHouse/pull/40668) ([Nikita Mikhaylov](https://github.com/nikitamikhaylov))。
* リモートファイルシステムからの読み取り処理を改善し、読み取り/書き込み用スレッドプールのサイズを設定可能にしました。 [#41070](https://github.com/ClickHouse/ClickHouse/issues/41070) をクローズしました。 [#41011](https://github.com/ClickHouse/ClickHouse/pull/41011)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* WindowTransform/arratReduce*/initializeAggregation/aggregate 関数のバージョンにおいて、すべてのコンビネータの組み合わせをサポートするようにしました。以前は、`ForEach/Resample/Map` のようなコンビネータはこれらの箇所では動作せず、使用すると `State function ... inserts results into non-state column` のような例外が発生していました。 [#41107](https://github.com/ClickHouse/ClickHouse/pull/41107) ([Kruglov Pavel](https://github.com/Avogar)).
...
* 各ディスクごとの予約に使用されていない空き容量を確認できるように、`system.disks` テーブルに `unreserved_space` カラムを追加しました。 [#41254](https://github.com/ClickHouse/ClickHouse/pull/41254) ([filimonov](https://github.com/filimonov)).
...
* IN 演算子で decimal 型と浮動小数点リテラルを比較できるようにしました。 [#41544](https://github.com/ClickHouse/ClickHouse/pull/41544) ([liang.huang](https://github.com/lhuang09287750)).
...
* ClickHouse は、キャッシュが非同期に更新されるまでの一定期間（デフォルトでは 15 秒間）、古い DNS レコードをキャッシュしてしまう可能性がありました。この期間中でも、ClickHouse は接続の確立を試みてエラーを発生させることがありました。この挙動は修正されました。 [#41707](https://github.com/ClickHouse/ClickHouse/pull/41707) ([Nikita Mikhaylov](https://github.com/nikitamikhaylov))。
...
* 無効な証明書を提示するセキュアサーバーへの接続を許可するのは、クライアントが &#39;--accept-certificate&#39; フラグを指定した場合にのみとしました。 [#41743](https://github.com/ClickHouse/ClickHouse/pull/41743) ([Yakov Olkhovskiy](https://github.com/yakov-olkhovskiy)).
...
* テキストフォーマットに対する `DateTime` 型推論を改善しました。これにより、`date_time_input_format` 設定が考慮され、数値をタイムスタンプとみなして日時を推論しようとしなくなりました。 [#41389](https://github.com/ClickHouse/ClickHouse/issues/41389) をクローズしました。 [#42206](https://github.com/ClickHouse/ClickHouse/issues/42206) をクローズしました。 [#41912](https://github.com/ClickHouse/ClickHouse/pull/41912)（[Kruglov Pavel](https://github.com/Avogar)）。
* `perform_ttl_move_on_insert` = false の設定で挿入を行う際に表示されていた紛らわしい警告を削除しました。 [#41980](https://github.com/ClickHouse/ClickHouse/pull/41980) ([Vitaly Baranov](https://github.com/vitlibar))。
* ユーザーが `count(*)` と同様に `countState(*)` を記述できるようにしました。これにより [#9338](https://github.com/ClickHouse/ClickHouse/issues/9338) がクローズされました。[#41983](https://github.com/ClickHouse/ClickHouse/pull/41983)（[Amos Bird](https://github.com/amosbird)）。
...
* レポートをより便利にするために、Sentry の設定で環境名として任意の文字列を指定できるオプションを追加しました。 [#42037](https://github.com/ClickHouse/ClickHouse/pull/42037) ([Nikita Mikhaylov](https://github.com/nikitamikhaylov))。
* CSV から読み込む範囲外の Date 値のパースを修正。 [#42044](https://github.com/ClickHouse/ClickHouse/pull/42044) ([Andrey Zvonov](https://github.com/zvonand)).
* `parseDataTimeBestEffort` は、日付と時刻の間にカンマを挟んだ形式をサポートするようになりました。 [#42038](https://github.com/ClickHouse/ClickHouse/issues/42038) をクローズしました。 [#42049](https://github.com/ClickHouse/ClickHouse/pull/42049) ([flynn](https://github.com/ucasfl)).
* `ReplicatedMergeTree` における古い（stale）レプリカの復旧プロセスを改善しました。失われたレプリカが、正常なレプリカには存在しない一部のパーツを保持しているものの、それらのパーツが正常なレプリカのレプリケーションキュー上では将来的に生成される予定になっている場合、失われたレプリカはそれらのパーツを切り離さずに保持するようになりました。 [#42134](https://github.com/ClickHouse/ClickHouse/pull/42134) ([Alexander Tokmakov](https://github.com/tavplubix))。
...
* Minio に大きなパーツをアップロードする際、&#39;Complete Multipart Upload&#39; に時間がかかる場合があります。Minio は 10 秒ごとにハートビートを送信します（[https://github.com/minio/minio/pull/7198](https://github.com/minio/minio/pull/7198) を参照）。しかし ClickHouse では送信/受信タイムアウトがデフォルトで 5 秒に[設定されている](https://github.com/ClickHouse/ClickHouse/blob/cc24fcd6d5dfb67f5f66f5483e986bd1010ad9cf/src/IO/S3/PocoHTTPClient.cpp#L123)ため、その前にタイムアウトが発生してしまいます。[#42321](https://github.com/ClickHouse/ClickHouse/pull/42321)（[filimonov](https://github.com/filimonov)）。
* Decimal などの複合型を持つ集計状態型を扱う際に、ごくまれに不正なキャストが行われてしまう問題を修正しました。これにより [#42408](https://github.com/ClickHouse/ClickHouse/issues/42408) が修正されました。 [#42417](https://github.com/ClickHouse/ClickHouse/pull/42417) ([Amos Bird](https://github.com/amosbird)).
...
* 範囲内のすべてのパーツが指定した閾値より古い場合に、それらのパーツをマージします。閾値は `min_age_to_force_merge_seconds` で設定できます。この変更により [#35836](https://github.com/ClickHouse/ClickHouse/issues/35836) がクローズされます。 [#42423](https://github.com/ClickHouse/ClickHouse/pull/42423)（[Antonio Andelic](https://github.com/antonio2368)）。これは、大部分のロジックを実装した [@fastio](https://github.com/fastio) による [#39550i](https://github.com/ClickHouse/ClickHouse/pull/39550) の続きです。
...
#### バグ修正（公式 stable または prestable リリースにおけるユーザーにとって目に見える不具合） \{#bug-fix-user-visible-misbehavior-in-official-stable-or-prestable-release-2\}
...
### <a id="229"></a> ClickHouse 22.9 リリース（2022-09-22）。[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.9/)、[動画](https://www.youtube.com/watch?v=rK2BsaaaOCA) \{#a-id229a-clickhouse-release-229-2022-09-22\}
...
* 単一レベルの集約結果を生成する際に `max_block_size` の値を考慮するようになりました。これにより、クエリプランのステップをより多くのスレッドで実行できるようになります。 [#39138](https://github.com/ClickHouse/ClickHouse/pull/39138) ([Nikita Taranov](https://github.com/nickitat)).
...
* `WHERE` 句を適用した結果、ソートキーを構成する一部のカラムが常に定数になる場合における `optimize_read_in_order` のサポートを改善しました。例えば、`table` のストレージ定義が `MergeTree ORDER BY (a, b)` の場合の `SELECT ... FROM table WHERE a = 'x' ORDER BY a, b` のようなクエリです。 [#38715](https://github.com/ClickHouse/ClickHouse/pull/38715) ([Anton Popov](https://github.com/CurtizJ))。
* ソートの前に、`full_sorting_join` の結合済みストリーム同士を相互にフィルタリングするようにしました。 [#39418](https://github.com/ClickHouse/ClickHouse/pull/39418) ([Vladimir C](https://github.com/vdimir)).
* 空のリテラルの処理をスキップすることで、LZ4 の伸長処理を最適化しました。 [#40142](https://github.com/ClickHouse/ClickHouse/pull/40142) ([Nikita Taranov](https://github.com/nickitat)).
...
* vectorscan をバージョン 5.4.8 にアップグレードし、正規表現評価を高速化する多数のパフォーマンス最適化を取り込みました。 [#41270](https://github.com/ClickHouse/ClickHouse/pull/41270) ([Robert Schulze](https://github.com/rschu1ze)).
* 非常に高い並列度での動作時に、VFS（S3 など）向けのローカルファイルシステムキャッシュをスキップしてしまっていた誤ったフォールバック動作を修正しました。 [#40420](https://github.com/ClickHouse/ClickHouse/pull/40420) ([Kseniia Sumarokova](https://github.com/kssenii)).
* ROW POLICY のフィルタが常に false の場合、データを一切読み込まずに直ちに空の結果を返します。これにより [#24012](https://github.com/ClickHouse/ClickHouse/issues/24012) が解決されました。[#40740](https://github.com/ClickHouse/ClickHouse/pull/40740)（[Amos Bird](https://github.com/amosbird)）。
...
* S3 ObjectStorage 向けに、S3 API 呼び出しごとに分割された詳細な S3 プロファイルイベント `DiskS3*` を追加。 [#41532](https://github.com/ClickHouse/ClickHouse/pull/41532) ([Sergei Trifonov](https://github.com/serxa))。
...
* ODBC および JDBC テーブルで CONSTRAINT の使用を許可。 [#34551](https://github.com/ClickHouse/ClickHouse/pull/34551) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
...
* スレッド間での OpenTelemetry のトレースコンテキスト伝播を改善しました。 [#39010](https://github.com/ClickHouse/ClickHouse/pull/39010) ([Frank Chen](https://github.com/FrankChen021)).
...
* エラー発生後における Replicated ユーザーアクセスストレージの復旧処理を改善しました。 [#39977](https://github.com/ClickHouse/ClickHouse/pull/39977) ([Vitaly Baranov](https://github.com/vitlibar)).
* `EmbeddedRocksDB` に有効期限 (TTL) のサポートを追加しました。 [#39986](https://github.com/ClickHouse/ClickHouse/pull/39986) ([Lloyd-Pottiger](https://github.com/Lloyd-Pottiger)).
...
* Parquet 形式でのネストされたデータ構造のサポートを改善 [#40485](https://github.com/ClickHouse/ClickHouse/pull/40485) ([Arthur Passos](https://github.com/arthurpassos)).
* Avro からの Array(Record) をフラット化された Nested テーブルに読み込めるようにしました。 [#40534](https://github.com/ClickHouse/ClickHouse/pull/40534) ([Kruglov Pavel](https://github.com/Avogar)).
...
* `GROUPING SET` が使用されている場合は projection 機能を無効にします。誤った結果を生成してしまう問題がありました。この変更により [#40635](https://github.com/ClickHouse/ClickHouse/issues/40635) が修正されます。[#40726](https://github.com/ClickHouse/ClickHouse/pull/40726)（[Amos Bird](https://github.com/amosbird)）。
* テーブル定義で使用された場合にメタデータを破損させるおそれがあった `APPLY` カラムトランスフォーマーの誤ったフォーマットを修正しました。これにより [#37590](https://github.com/ClickHouse/ClickHouse/issues/37590) が解決されました。[#40727](https://github.com/ClickHouse/ClickHouse/pull/40727)（[Amos Bird](https://github.com/amosbird)）。
* `formatDateTime` でタイムゾーンオフセットをフォーマットするための `%z` 書式指定子をサポートしました。 [#40736](https://github.com/ClickHouse/ClickHouse/pull/40736) ([Cory Levy](https://github.com/LevyCory))。
...
* ファイルシステムキャッシュ用ディレクトリにステータスファイルを作成し、キャッシュディレクトリが異なるサーバーやキャッシュインスタンス間で共有されないようにします。 [#40820](https://github.com/ClickHouse/ClickHouse/pull/40820) ([Kseniia Sumarokova](https://github.com/kssenii)).
...
* ClickHouse Keeper：長時間に及ぶコミット処理中のシャットダウンを修正し、許可されるリクエストサイズの上限を引き上げ。[#40941](https://github.com/ClickHouse/ClickHouse/pull/40941)（[Antonio Andelic](https://github.com/antonio2368)）。
...
* 分散テーブルへの INSERT のオブザーバビリティを改善。[#41034](https://github.com/ClickHouse/ClickHouse/pull/41034)（[Frank Chen](https://github.com/FrankChen021)）。
* S3 とのやり取り向けの、より低レベルなメトリクスを追加。 [#41039](https://github.com/ClickHouse/ClickHouse/pull/41039) ([mateng915](https://github.com/mateng0915)).
...
* サーバー再起動なしに HTTP ハンドラーの変更を即時に適用できるようにしました。 [#41177](https://github.com/ClickHouse/ClickHouse/pull/41177) ([Azat Khuzhin](https://github.com/azat)).
...
* テストにおける SQL 関数のカバレッジを計測してレポートします。 [#40593](https://github.com/ClickHouse/ClickHouse/issues/40593). [#40647](https://github.com/ClickHouse/ClickHouse/pull/40647) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
...
* 高レベルな共通サーバーコンポーネント向けの組み込みリファレンスドキュメントのプロトタイプ。[#40649](https://github.com/ClickHouse/ClickHouse/pull/40649)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* 変更されたパフォーマンステストに含まれるすべてのクエリをチェックし、すべての変更クエリが漏れなくテストされるようにします。 [#40322](https://github.com/ClickHouse/ClickHouse/pull/40322) ([Nikita Taranov](https://github.com/nickitat))。
* TGZ パッケージを修正しました。 [#40681](https://github.com/ClickHouse/ClickHouse/pull/40681) ([Mikhail f. Shiryaev](https://github.com/Felixoid)).
* デバッグシンボルを修正。 [#40873](https://github.com/ClickHouse/ClickHouse/pull/40873) ([Azat Khuzhin](https://github.com/azat)).
* CI 設定を拡張して、x86 SSE2 のみを対象としたビルドを作成できるようにしました。古いハードウェアや組み込み用途のハードウェアで有用です。 [#40999](https://github.com/ClickHouse/ClickHouse/pull/40999) ([Robert Schulze](https://github.com/rschu1ze)).
...
* メインの ClickHouse コードから `dlopen` を削除しましたが、library-bridge と odbc-bridge にはまだ残っています。 [#41428](https://github.com/ClickHouse/ClickHouse/pull/41428) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* メインの ClickHouse バイナリでは `dlopen` を許可しないでください。有害であり、安全性に問題があります。当社では使用していませんが、一部のライブラリでは「プラグイン」を実装するために使用される場合があります。制御不能で危険なサードパーティ製ライブラリをプロセスのアドレス空間に読み込むという時代遅れの手法は正気の沙汰ではないため、断固として非推奨とします。[#41429](https://github.com/ClickHouse/ClickHouse/pull/41429)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
...
* 社内製 DWARF パーサーでの DWARF-5 サポート。 [#40710](https://github.com/ClickHouse/ClickHouse/pull/40710) ([Azat Khuzhin](https://github.com/azat)).
* テスト用のフォールトインジェクションを ZooKeeper クライアントに追加しました [#30498](https://github.com/ClickHouse/ClickHouse/pull/30498) ([Alexander Tokmakov](https://github.com/tavplubix))。
* debug および tsan 構成向けに、S3 ストレージを利用したステートレステストを追加 [#35262](https://github.com/ClickHouse/ClickHouse/pull/35262) ([Kseniia Sumarokova](https://github.com/kssenii)).
...
### <a id="228"></a> ClickHouse リリース 22.8（2022-08-18）。[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.8/)、[動画](https://www.youtube.com/watch?v=yob7AnaBJz0) \{#a-id228a-clickhouse-release-228-2022-08-18\}
...
* MergeTree テーブルに対する SQL 標準の `DELETE FROM` 構文と、MergeTree ファミリー向けの論理削除 (lightweight delete) 実装をサポートしました。[#37893](https://github.com/ClickHouse/ClickHouse/pull/37893) ([Jianmei Zhang](https://github.com/zhangjmruc)) ([Alexander Gololobov](https://github.com/davenger))。注意: この新機能により ClickHouse が HTAP DBMS になるわけではありません。
...
* AVX512VBMI2 の圧縮ストアを使用して数値カラムでのフィルタリングを最適化しました。[#39633](https://github.com/ClickHouse/ClickHouse/pull/39633)（[Guo Wangyang](https://github.com/guowangy)）。AVX512 VBMI2 を搭載したシステムでは、この PR により、SSB ベンチマーククエリ 3.1、3.2、3.3（SF=100）で約 6% のパフォーマンス向上が見られます。Intel Icelake Xeon 8380 × 2 ソケット構成でテスト済みです。[#40033](https://github.com/ClickHouse/ClickHouse/pull/40033)（[Robert Schulze](https://github.com/rschu1ze)）。
* マルチスレッド環境での関数式を用いたインデックス解析を最適化しました。 [#39812](https://github.com/ClickHouse/ClickHouse/pull/39812) ([Guo Wangyang](https://github.com/guowangy)).
...
* `timeSlots` が DateTime64 に対応しました。DateTime64 使用時には、サブセカンド精度の継続時間とスロットサイズを利用できます。 [#37951](https://github.com/ClickHouse/ClickHouse/pull/37951) ([Andrey Zvonov](https://github.com/zvonand)).
...
* カスタム TLD において `!`/`*`（感嘆符/アスタリスク）をサポートするようにしました（`cutToFirstSignificantSubdomainCustom()`/`cutToFirstSignificantSubdomainCustomWithWWW()`/`firstSignificantSubdomainCustom()`）。[#39496](https://github.com/ClickHouse/ClickHouse/pull/39496)（[Azat Khuzhin](https://github.com/azat)）。
* NATS への TLS 接続のサポートを追加しました。 [#39525](https://github.com/ClickHouse/ClickHouse/issues/39525) を実装しました。 [#39527](https://github.com/ClickHouse/ClickHouse/pull/39527)（[Constantine Peresypkin](https://github.com/pkit)）。
* `clickhouse-obfuscator`（テストおよび負荷生成のためのデータベース難読化ツール）に、事前学習済みモデルを利用するための新しい `--save` および `--load` パラメータが追加されました。これにより [#39534](https://github.com/ClickHouse/ClickHouse/issues/39534) が解決されました。 [#39541](https://github.com/ClickHouse/ClickHouse/pull/39541)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
...
* 外部集約が有効な場合の集計プロジェクションの構築処理を修正しました。このケースはまれであり、設定を変更することで容易に回避できるため、改善としてマークしています。この修正により [#39667](https://github.com/ClickHouse/ClickHouse/issues/39667) が解決されます。 [#39671](https://github.com/ClickHouse/ClickHouse/pull/39671) ([Amos Bird](https://github.com/amosbird))。
...
* スタックトレース内のアドレスを非表示にするための設定パラメータを追加しました。セキュリティ向上に多少寄与する可能性はありますが、一般的には有害であり、使用すべきではありません。[#39690](https://github.com/ClickHouse/ClickHouse/pull/39690) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* ネストされた関数データのメモリセグメントが適切にアラインされるように、AggregateFunctionDistinct のプレフィックスサイズを変更しました。 [#39696](https://github.com/ClickHouse/ClickHouse/pull/39696) ([Pxl](https://github.com/BiteTheDDDDt)).
...
* Play UI: textarea 内での Tab キー入力を認識しつつ、同時に Tab キーによるフォーカス移動を妨げないようにしました。 [#40053](https://github.com/ClickHouse/ClickHouse/pull/40053) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* クライアントはサーバー側で測定した経過時間を表示するようになります。これは、リモートデータセンター間で ClickHouse サービスの性能を比較する際に重要です。これにより [#38070](https://github.com/ClickHouse/ClickHouse/issues/38070) が解決されます。背景となる動機については [こちら](https://github.com/ClickHouse/ClickBench/blob/main/hardware/benchmark-cloud.sh#L37) も参照してください。[#39968](https://github.com/ClickHouse/ClickHouse/pull/39968)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
...
* `clickhouse-benchmark` で、サーバー側の時間情報が利用可能な場合はデフォルトでそれを表示するようにしました（ClickHouse バージョン 22.8 以降）。これはクラウド環境間のパフォーマンスを正しく比較するために必要です。この動作は、新しい `--client-side-time` コマンドラインオプションで変更できます。`--randomize` コマンドラインオプションを、`--randomize 1` から引数なしの形式に変更しました。 [#40193](https://github.com/ClickHouse/ClickHouse/pull/40193) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* クエリの複雑性制限が設定され、その制限に達した回数をカウントするカウンタ（ProfileEvents）を追加しました（`overflow_mode` が `break` の場合と `throw` の場合で別々のカウンタ）。たとえば、`read_overflow_mode = 'break'` で `max_rows_to_read` を設定している場合、`OverflowBreak` カウンタの値を見ることで、不完全な結果かどうかを判別できます。 [#40205](https://github.com/ClickHouse/ClickHouse/pull/40205) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
...
* 関数 multi(Fuzzy)Match(Any|AllIndices|AnyIndex)() の使用時にパターンキャッシュのメモリ消費量が増え続けないようにしました。 [#40264](https://github.com/ClickHouse/ClickHouse/pull/40264) ([Robert Schulze](https://github.com/rschu1ze)).
...
* 最近のタイムゾーン変更をサポートするため、tzdata を 2022b に更新しました。[https://github.com/google/cctz/pull/226](https://github.com/google/cctz/pull/226) を参照してください。チリの 2022 年の夏時間（DST）の開始は、9 月 4 日から 9 月 11 日に延期されました。イランは 2022-09-21 に標準時へ戻した後、DST（夏時間）の適用を恒久的に廃止する予定です。1977 年における Asia/Tehran の過去のタイムゾーン情報に関する修正も含まれています。イランが標準時を採用したのは 1946 年ではなく 1935 年です。1977 年には 03-21 23:00 から 10-20 24:00 まで DST を実施しており、1978 年の切り替えは 03-20 と 10-20 ではなく 03-24 と 08-05 に行われました。また、1979 年春の切り替えは 03-21 ではなく 05-27 でした（[https://data.iana.org/time-zones/tzdb/NEWS](https://data.iana.org/time-zones/tzdb/NEWS)）。（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* 以前のパッケージでは、`systemd.service` ファイルを `/etc` にインストールしていました。そこにあるファイルは `conf` としてマークされており、削除されず、自動的に更新もされません。この PR でそれらをクリーンアップします。 [#39323](https://github.com/ClickHouse/ClickHouse/pull/39323) ([Mikhail f. Shiryaev](https://github.com/Felixoid))。
* LSan が正しく動作するようにしました。 [#39430](https://github.com/ClickHouse/ClickHouse/pull/39430) ([Azat Khuzhin](https://github.com/azat)).
* TSAN は clang-14 との組み合わせで問題があります（[https://github.com/google/sanitizers/issues/1552](https://github.com/google/sanitizers/issues/1552), [https://github.com/google/sanitizers/issues/1540](https://github.com/google/sanitizers/issues/1540)）ので、ここでは TSAN バイナリを clang-15 でビルドします。[#39450](https://github.com/ClickHouse/ClickHouse/pull/39450)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
* ClickHouse のツールを個別の実行可能プログラムとしてビルドするオプションを削除しました。これにより [#37847](https://github.com/ClickHouse/ClickHouse/issues/37847) が修正されました。[#39520](https://github.com/ClickHouse/ClickHouse/pull/39520)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
...
* RISC-V 64 向けビルドを CI で実行できるよう準備しました。これは [#40141](https://github.com/ClickHouse/ClickHouse/issues/40141) に対応するものです。[#40197](https://github.com/ClickHouse/ClickHouse/pull/40197)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* 関数登録用マクロインターフェース（`FUNCTION_REGISTER*`）を簡素化し、registerFunctions.cpp 内で `extern` 関数を追加して呼び出すステップを不要にしました。これにより、新しい関数のインクリメンタルビルドが高速化されます。 [#38615](https://github.com/ClickHouse/ClickHouse/pull/38615) ([Li Yin](https://github.com/liyinsg)).
* Docker: 現在は、Docker イメージ内の entrypoint.sh が、マルチディスク構成用の設定で検出したすべてのフォルダに対して `chown` コマンドを生成して実行します [#17717](https://github.com/ClickHouse/ClickHouse/issues/17717)。[#39121](https://github.com/ClickHouse/ClickHouse/pull/39121)（[Nikita Mikhaylov](https://github.com/nikitamikhaylov)）。
...
* プロジェクションの ORDER BY と一致する ORDER BY の動作を修正しました（以前は結果をソートせずに返していました）。 [#38725](https://github.com/ClickHouse/ClickHouse/pull/38725) ([Azat Khuzhin](https://github.com/azat)).
* `GROUP BY` 文で、テーブルのカラムまたは式をシャドーイングしてしまう場合には関数を最適化しないようにしました。[#37032](https://github.com/ClickHouse/ClickHouse/issues/37032) を修正。[#39103](https://github.com/ClickHouse/ClickHouse/pull/39103)（[Anton Kozlov](https://github.com/tonickkozlov)）。
...
* クエリ最適化時のカラムプルーニングにおける位置引数を修正しました。[#38433](https://github.com/ClickHouse/ClickHouse/issues/38433) をクローズしました。[#39293](https://github.com/ClickHouse/ClickHouse/pull/39293) ([Kseniia Sumarokova](https://github.com/kssenii)).
* Protobuf/CapnProto フォーマットにおいて、空メッセージを含む場合にスキーマ推論で空の `Tuple` 型のカラムが作成されてしまう不具合を修正しました。[#39051](https://github.com/ClickHouse/ClickHouse/issues/39051) をクローズしました。Protobuf および CapnProto フォーマットのスキーマ推論時に、サポートされていない型のフィールドをスキップできるようにする 2 つの新しい設定 `input_format_{protobuf/capnproto}_skip_fields_with_unsupported_types_in_schema_inference` を追加しました。[#39357](https://github.com/ClickHouse/ClickHouse/pull/39357)（[Kruglov Pavel](https://github.com/Avogar)）。
* （Window View は実験的な機能です）`CREATE WINDOW VIEW .. ON CLUSTER ... INNER` で発生していたセグメンテーションフォールトを修正しました。[#39363](https://github.com/ClickHouse/ClickHouse/issues/39363) をクローズしました。[#39384](https://github.com/ClickHouse/ClickHouse/pull/39384)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* `insert into` 関数をキャンセルした場合の WriteBuffer の finalize の動作を修正しました（以前のバージョンでは `std::terminate` を呼び出す可能性がありました）。 [#39458](https://github.com/ClickHouse/ClickHouse/pull/39458) ([Kruglov Pavel](https://github.com/Avogar))。
* スパースシリアライズ形式での `Object` 型カラムの保存処理を修正しました。 [#39464](https://github.com/ClickHouse/ClickHouse/pull/39464) ([Anton Popov](https://github.com/CurtizJ)).
...
* `min_bytes_to_use_direct_io` の設定により `O_DIRECT` が有効な場合に、`local_filesystem_read_method='pread_threadpool'` 設定時の非同期読み取りでまれに発生していた不具合を修正しました。 [#39506](https://github.com/ClickHouse/ClickHouse/pull/39506) ([Anton Popov](https://github.com/CurtizJ)).
* （FreeBSD のみ）FreeBSD 上で ClickHouse を起動する際に発生していた &quot;Code: 49. DB::Exception: FunctionFactory: the function name &#39;&#39; is not unique. (LOGICAL&#95;ERROR)&quot; を修正しました。 [#39551](https://github.com/ClickHouse/ClickHouse/pull/39551) ([Alexander Gololobov](https://github.com/davenger)).
* `splitByChar` に最近導入された引数「maxsplit」が正しく動作していなかったバグを修正しました。 [#39552](https://github.com/ClickHouse/ClickHouse/pull/39552) ([filimonov](https://github.com/filimonov))。
* `enable_optimize_predicate_expression` を有効にした場合の ASOF JOIN におけるバグを修正し、[#37813](https://github.com/ClickHouse/ClickHouse/issues/37813) をクローズ。[#39556](https://github.com/ClickHouse/ClickHouse/pull/39556)（[Vladimir C](https://github.com/vdimir)）。
* `ON CLUSTER` または `Replicated` データベースおよび `ReplicatedMergeTree` を用いた `CREATE/DROP INDEX` クエリの動作を修正しました。以前はすべてのレプリカで実行されており、エラーや DDL キューがスタックする原因となっていました。[#39511](https://github.com/ClickHouse/ClickHouse/issues/39511) を修正。[#39565](https://github.com/ClickHouse/ClickHouse/pull/39565)（[Alexander Tokmakov](https://github.com/tavplubix)）。
...
* 分散テーブル読み取り時のデフォルトデータベース解決に関するバグを修正しました。 [#39674](https://github.com/ClickHouse/ClickHouse/pull/39674) ([Anton Kozlov](https://github.com/tonickkozlov)).
...
* タプルと `IN` 演算子を組み合わせた場合の誤ったインデックス解析を修正しました。これにより、誤ったクエリ結果が返される可能性がありました。 [#39752](https://github.com/ClickHouse/ClickHouse/pull/39752) ([Anton Popov](https://github.com/CurtizJ)).
* `EmbeddedRocksDB` テーブルでパラメータを用いたキーによるフィルタリングが正しく動作するよう修正しました。 [#39757](https://github.com/ClickHouse/ClickHouse/pull/39757) ([Antonio Andelic](https://github.com/antonio2368)).
...
* (NFS のみ) root-squash が有効なボリュームでの NFS 上の mkdir が正しく動作しない不具合を修正。[#39898](https://github.com/ClickHouse/ClickHouse/pull/39898) ([Constantine Peresypkin](https://github.com/pkit)).
* DETACH/DROP 時に Prometheus メトリクスからディクショナリが削除されるようにしました。 [#39926](https://github.com/ClickHouse/ClickHouse/pull/39926) ([Azat Khuzhin](https://github.com/azat)).
* StorageFile における仮想カラムの読み取り処理を修正。[#39907](https://github.com/ClickHouse/ClickHouse/issues/39907) をクローズ。[#39943](https://github.com/ClickHouse/ClickHouse/pull/39943)（[flynn](https://github.com/ucasfl)）。
* フェッチ時の過剰なメモリ使用を修正しました。これにより [#39915](https://github.com/ClickHouse/ClickHouse/issues/39915) が修正されます。[#39990](https://github.com/ClickHouse/ClickHouse/pull/39990)（[Nikolai Kochetov](https://github.com/KochetovNicolai)）。
...
* 特定の組み合わせで定数カラムと非定数カラムを使用した場合に、`EXCEPT` および `INTERSECT` 演算子がクラッシュする可能性がありました。 [#40020](https://github.com/ClickHouse/ClickHouse/pull/40020) ([Duc Canh Le](https://github.com/canhld94)).
* INSERT が遅すぎる、またはマージ／ミューテーションに時間がかかりすぎる場合に発生していた &quot;Part directory doesn&#39;t exist&quot; および &quot;`tmp_<part_name>` ... No such file or directory&quot; エラーを修正しました。さらに、パーツのフェッチの前回の試行が失敗したにもかかわらず `tmp-fetch_<part_name>` ディレクトリがクリーンアップされていなかった場合に、ログにエラーや警告が一切出力されないままレプリケーションキューのエントリが滞留する可能性があった問題も修正しました。 [#40031](https://github.com/ClickHouse/ClickHouse/pull/40031) ([Alexander Tokmakov](https://github.com/tavplubix)).
...
* ArrowColumn フォーマットの Dictionary(X) および Dictionary(Nullable(X)) から、それぞれ ClickHouse の LowCardinality(X) および LowCardinality(Nullable(X)) への変換ロジックを修正しました。 [#40037](https://github.com/ClickHouse/ClickHouse/pull/40037) ([Arthur Passos](https://github.com/arthurpassos)).
* タスクスケジューリング失敗時に S3 への書き込みで発生する可能性があるデッドロックを修正。 [#40070](https://github.com/ClickHouse/ClickHouse/pull/40070) ([Maksim Kita](https://github.com/kitaisreal)).
...
* 逆引き DNS 解決の不具合を修正。 [#40134](https://github.com/ClickHouse/ClickHouse/pull/40134) ([Arthur Passos](https://github.com/arthurpassos))。
...
### <a id="227"></a> ClickHouse リリース 22.7、2022-07-21。 [プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.7/)、[動画](https://www.youtube.com/watch?v=IOJyo14BpTQ) \{#a-id227a-clickhouse-release-227-2022-07-21\}
...
* 完全なソートマージ結合アルゴリズムを追加しました。 [#35796](https://github.com/ClickHouse/ClickHouse/pull/35796) ([Vladimir C](https://github.com/vdimir)).
...
* 設定 `additional_table_filters` を導入しました。この設定を使用すると、テーブルに対して追加のフィルタ条件を指定でき、データの読み取り直後に適用されます。例: `select number, x, y from (select number from system.numbers limit 5) f any left join (select x, y from table_1) s on f.number = s.x settings additional_table_filters={'system.numbers : 'number != 3', 'table_1' : 'x != 2'}`。クエリ結果に対する追加のフィルタ条件を指定する設定 `additional_result_filter` を導入しました。[#37918](https://github.com/ClickHouse/ClickHouse/issues/37918) をクローズしました。[#38475](https://github.com/ClickHouse/ClickHouse/pull/38475) ([Nikolai Kochetov](https://github.com/KochetovNicolai))。
...
* 関数 `translate(string, from_string, to_string)` と `translateUTF8(string, from_string, to_string)` を追加しました。文字列中の特定の文字を別の文字に変換する関数です。 [#38935](https://github.com/ClickHouse/ClickHouse/pull/38935) ([Nikolay Degterinsky](https://github.com/evillique))。
...
* `send_logs_source_regexp` 設定を追加しました。指定した正規表現に一致するログソース名のサーバーのテキストログを送信します。空文字列の場合はすべてのソースが対象です。 [#39161](https://github.com/ClickHouse/ClickHouse/pull/39161) ([Amos Bird](https://github.com/amosbird))。
* `Hive` テーブルに対する `ALTER` をサポートしました。 [#38214](https://github.com/ClickHouse/ClickHouse/pull/38214) ([lgbo](https://github.com/lgbo-ustc)).
...
* 配列およびタプルの両方に対して、L2 距離二乗および L2 ノルムの関数を追加しました。 [#38545](https://github.com/ClickHouse/ClickHouse/pull/38545) ([Julian Gilyadov](https://github.com/israelg99)).
...
* `FINAL` を含むクエリの並列実行性を向上させました。 [#36396](https://github.com/ClickHouse/ClickHouse/pull/36396) ([Nikita Taranov](https://github.com/nickitat))。
* [#35616](https://github.com/ClickHouse/ClickHouse/pull/35616) で導入されていた、結合における大きなパフォーマンス劣化を修正しました。ssb クエリのような一般的な結合クエリが、ほぼ 3 か月もの間 10 倍も遅くなっていたにもかかわらず、誰からも報告されなかったのは興味深いことです。[#38052](https://github.com/ClickHouse/ClickHouse/pull/38052)（[Amos Bird](https://github.com/amosbird)）。
* Intel hyperscan ライブラリから vectorscan へ移行しました。これにより、非 x86 プラットフォームにおける多くの文字列マッチング処理が高速化されます。 [#38171](https://github.com/ClickHouse/ClickHouse/pull/38171) ([Robert Schulze](https://github.com/rschu1ze)).
* 集約後に実行されるクエリプランのステップの並列実行性を高めました。 [#38295](https://github.com/ClickHouse/ClickHouse/pull/38295) ([Nikita Taranov](https://github.com/nickitat)).
...
* LZ4 の伸長処理向けに、AVX-512 VBMI で最適化された `copyOverlap32Shuffle` を追加しました。これにより、LZ4 の伸長処理のパフォーマンスが向上しました。 [#37891](https://github.com/ClickHouse/ClickHouse/pull/37891) ([Guo Wangyang](https://github.com/guowangy)).
...
* ベンチマークをより安定させるため、分岐を 32 バイト境界に揃えました。 [#38988](https://github.com/ClickHouse/ClickHouse/pull/38988) ([Guo Wangyang](https://github.com/guowangy))。これにより、Intel プロセッサ環境で平均して 1～2% の性能向上が得られます。
...
* テーブル `system.asynchronous_metric_log` のストレージ使用効率をさらに最適化しました。これにより [#38134](https://github.com/ClickHouse/ClickHouse/issues/38134) がクローズされました。[YouTube 動画](https://www.youtube.com/watch?v=0fSp9SF8N8A) も参照してください。[#38428](https://github.com/ClickHouse/ClickHouse/pull/38428)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
...
* * * 設定 `multiple_joins_try_to_keep_original_names` を追加し、複数の JOIN を書き換える際に識別子の名前が書き換えられないようにしました。[#34697](https://github.com/ClickHouse/ClickHouse/issues/34697) をクローズ。[#38149](https://github.com/ClickHouse/ClickHouse/pull/38149)（[Vladimir C](https://github.com/vdimir)）。
* トレースビジュアライザーの UX を改善。 [#38169](https://github.com/ClickHouse/ClickHouse/pull/38169) ([Sergei Trifonov](https://github.com/serxa)).
* AArch64 向けにスタックトレースの収集とクエリプロファイラを有効化しました。 [#38181](https://github.com/ClickHouse/ClickHouse/pull/38181) ([Maksim Kita](https://github.com/kitaisreal)).
...
* GCS をプロキシトンネル経由で使用する際の問題を修正しました。[#38726](https://github.com/ClickHouse/ClickHouse/pull/38726) ([Azat Khuzhin](https://github.com/azat))。
...
* `EXPLAIN AST` に新しいオプション `optimize = 1` が追加されました。有効にすると、書き換え後の AST を表示し、無効の場合は元のクエリの AST を表示します。デフォルトでは無効です。[#38910](https://github.com/ClickHouse/ClickHouse/pull/38910)（[Igor Nikonov](https://github.com/devcrafter)）。
...
* S3 の単一オブジェクトは、`RemoveObjectRequest` を使用して削除されるようになりました。GCP では `removeFileIfExists` を使用できず、その結果 `remove` 機能のおよそ半分が事実上動作しない問題がありましたが、これに対する互換性を実装しました。GCS がサポートしていない `DeleteObjects` S3 API を自動的に検出するようになりました。これにより、設定で `support_batch_delete=0` を明示的に指定しなくても GCS を使用できるようになります。[#37882](https://github.com/ClickHouse/ClickHouse/pull/37882)（[Vladimir Chebotarev](https://github.com/excitoon)）。
...
* 非推奨の Ordinary データベースを使用している場合に、INSERT クエリが存在していても、ALTER クエリの後に続く SELECT クエリで発生していた不要な待機を解消しました。 [#38864](https://github.com/ClickHouse/ClickHouse/pull/38864) ([Azat Khuzhin](https://github.com/azat)).
* `EXPLAIN AST` に新しいオプション `rewrite` が追加されました。有効にすると書き換え後の AST を表示し、そうでない場合は元のクエリの AST を表示します。デフォルトでは無効です。 [#38910](https://github.com/ClickHouse/ClickHouse/pull/38910)（[Igor Nikonov](https://github.com/devcrafter)）。
...
* `clickhouse-keeper`: リアルタイムダイジェストの計算および検証のサポートを追加しました。デフォルトでは無効です。 [#37555](https://github.com/ClickHouse/ClickHouse/pull/37555) ([Antonio Andelic](https://github.com/antonio2368)).
...
* clearOldLogs: 並行削除時には KEEPER&#95;EXCEPTION を報告しないようにしました。 [#39016](https://github.com/ClickHouse/ClickHouse/pull/39016) ([Raúl Marín](https://github.com/Algunenano)).
* clickhouse-keeper の改善：keeper サーバーに関するメタデータをディスクに永続化します。 [#39069](https://github.com/ClickHouse/ClickHouse/pull/39069) ([Antonio Andelic](https://github.com/antonio2368))。これにより、すべての keeper ノードを同時にシャットダウンまたは再起動する場合の運用が容易になります。
* ファイルシステムキャッシュ使用時にディスク容量が不足しても、例外を発生させずに処理を継続するようにしました。 [#39106](https://github.com/ClickHouse/ClickHouse/pull/39106) ([Kseniia Sumarokova](https://github.com/kssenii)).
* k8s からの SIGTERM シグナルの処理。 [#39130](https://github.com/ClickHouse/ClickHouse/pull/39130) ([Timur Solodovnikov](https://github.com/tsolodov)).
...
* ディスクが回転式でない場合は `system.errors` のカウンターをインクリメントしないようにしました。 [#39216](https://github.com/ClickHouse/ClickHouse/pull/39216) ([Raúl Marín](https://github.com/Algunenano)).
* `system.query_log` 内の `INSERT` クエリに対するメトリクス `result_bytes` は、挿入されたバイト数を示します。以前は値が誤っており、`result_rows` と同じ値が保存されていました。[#39225](https://github.com/ClickHouse/ClickHouse/pull/39225)（[Ilya Yatsishin](https://github.com/qoega)）。
...
* サーバー起動時のファイルシステムキャッシュ初期化時に例外を再スローし、エラーメッセージを改善しました。 [#39386](https://github.com/ClickHouse/ClickHouse/pull/39386) ([Kseniia Sumarokova](https://github.com/kssenii)).
...
* Functions `multiMatch[Fuzzy](AllIndices/Any/AnyIndex)` で、needle 引数が空の場合でも論理エラーを発生させないようにしました。 [#39012](https://github.com/ClickHouse/ClickHouse/pull/39012) ([Robert Schulze](https://github.com/rschu1ze)).
...
* `{0..10}` 形式の場合のグロブの展開を正しく行うよう修正し、[#38498](https://github.com/ClickHouse/ClickHouse/issues/38498) を解決しました。現在の実装はシェルの動作に類似しており、その点については @rschu1ze が [こちら](https://github.com/ClickHouse/ClickHouse/pull/38502#issuecomment-1169057723) で言及しています。 [#38502](https://github.com/ClickHouse/ClickHouse/pull/38502)（[Heena Bansal](https://github.com/HeenaBansal2009)）。
* 定数の map 引数を使用した場合に発生する `mapUpdate`、`mapFilter` 関数のクラッシュを修正しました。[#38547](https://github.com/ClickHouse/ClickHouse/issues/38547) をクローズしました。[#38553](https://github.com/ClickHouse/ClickHouse/pull/38553)（[hexiaoting](https://github.com/hexiaoting)）。
* クエリ最適化のための `toHour` の単調性に関する情報を修正しました。これにより、インデックス解析の誤りに起因する不正確なクエリ結果が発生する可能性がありました。この修正は [#38333](https://github.com/ClickHouse/ClickHouse/issues/38333) を解決します。 [#38675](https://github.com/ClickHouse/ClickHouse/pull/38675) ([Amos Bird](https://github.com/amosbird))。
...
* 並列読み取りバッファを使用した S3 のシーク可能な読み取りを修正しました（クエリ実行時のメモリ使用量に影響していた問題）。[#38258](https://github.com/ClickHouse/ClickHouse/issues/38258) をクローズ。[#38802](https://github.com/ClickHouse/ClickHouse/pull/38802)（[Kseniia Sumarokova](https://github.com/kssenii)）。
...
* 秒単位を使用する設定プロファイルに関する不具合を修正。[#38896](https://github.com/ClickHouse/ClickHouse/pull/38896)（[Raúl Marín](https://github.com/Algunenano)）。
* Nullable なパーティションキーがある場合に、パーティションプルーニングが誤って行われる問題を修正しました。注意：おそらく Nullable なパーティションキーは使用していないはずです — これはそもそも使うべきでないマイナーな機能です。Nullable キーはそもそも意味を成さず、この機能はごく一部の非常に特殊なユースケースのためだけに存在します。これにより [#38941](https://github.com/ClickHouse/ClickHouse/issues/38941) が修正されました。[#38946](https://github.com/ClickHouse/ClickHouse/pull/38946) ([Amos Bird](https://github.com/amosbird))。
* フェッチ時の `fsync_part_directory` を改善しました。 [#38993](https://github.com/ClickHouse/ClickHouse/pull/38993) ([Azat Khuzhin](https://github.com/azat)).
...
* `tuple` 関数内で `LowCardinality` 型を保持するようにしました。以前は `LowCardinality` 型が失われ、作成されたタプルの要素は `LowCardinality` の基になる型に変換されていました。 [#39113](https://github.com/ClickHouse/ClickHouse/pull/39113) ([Anton Popov](https://github.com/CurtizJ)).
...
* `PREWHERE` と読み取り順序最適化を使用する `SELECT` クエリで発生していた `Not found column Type in block` エラーを修正しました。 [#39157](https://github.com/ClickHouse/ClickHouse/pull/39157) ([Yakov Olkhovskiy](https://github.com/yakov-olkhovskiy)).
* リモートファイルシステムでのハードリンク作成時に発生し得る、極めてまれなレースコンディションを修正しました。これを再現する唯一の方法は、バックアップを同時に実行することです。 [#39190](https://github.com/ClickHouse/ClickHouse/pull/39190) ([alesapin](https://github.com/alesapin)).
...
* （MaterializedPostgreSQL - 実験的機能）。レプリケーションの初期化中に例外が発生した場合に起き得る、MaterializedPostgreSQL データベースエンジンでのセグメンテーションフォールトを修正しました。 [#36939](https://github.com/ClickHouse/ClickHouse/issues/36939) をクローズ。 [#39272](https://github.com/ClickHouse/ClickHouse/pull/39272)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* PostgreSQL データベースエンジンからのテーブルメタデータ取得の不具合を修正しました。 [#33502](https://github.com/ClickHouse/ClickHouse/issues/33502) をクローズ。 [#39283](https://github.com/ClickHouse/ClickHouse/pull/39283) ([Kseniia Sumarokova](https://github.com/kssenii))。
...
* `UNION` のサブクエリにおけるカラム順の誤りを修正しました（サブセレクト内でカラムが重複している場合に誤った結果が生成される可能性があった問題）。 [#37887](https://github.com/ClickHouse/ClickHouse/pull/37887) ([Azat Khuzhin](https://github.com/azat)).
* ドットを含むカラム名での MODIFY ALTER COLUMN の誤動作を修正。[#37907](https://github.com/ClickHouse/ClickHouse/issues/37907) をクローズ。[#37971](https://github.com/ClickHouse/ClickHouse/pull/37971)（[Kruglov Pavel](https://github.com/Avogar)）。
* S3 にデータを格納している `MergeTree` テーブルからのスパースカラムの読み取りを修正しました。 [#37978](https://github.com/ClickHouse/ClickHouse/pull/37978) ([Anton Popov](https://github.com/CurtizJ)).
...
* 後方互換性モードでタプル関数をリテラルに書き換えるようにしました。 [#38096](https://github.com/ClickHouse/ClickHouse/pull/38096) ([Anton Kozlov](https://github.com/tonickkozlov)).
...
* `CREATE TABLE nested_name_tuples (`a` Tuple(x String, y Tuple(i Int32, j String))) ENGINE = Memory;` のようなクエリに関する軽微な問題を修正しました。 [#38136](https://github.com/ClickHouse/ClickHouse/pull/38136) ([lgbo](https://github.com/lgbo-ustc))。
...
* （Window View は実験的な機能です）誤った構造を持つ WINDOW VIEW で発生する LOGICAL&#95;ERROR を修正しました。 [#38205](https://github.com/ClickHouse/ClickHouse/pull/38205) ([Azat Khuzhin](https://github.com/azat)).
* OAUTHBEARER のリフレッシュコールバックが設定されている場合に発生するクラッシュを修正するために、librdkafka サブモジュールを更新しました。 [#38225](https://github.com/ClickHouse/ClickHouse/pull/38225) ([Rafael Acevedo](https://github.com/racevedoo)).
...
* MaterializedPostgreSQL - 実験的機能。MaterializedPostgreSQL で発生しうる `Invalid number of rows in Chunk` エラーを修正。[#37323](https://github.com/ClickHouse/ClickHouse/issues/37323) をクローズ。 [#38360](https://github.com/ClickHouse/ClickHouse/pull/38360)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* 接続文字列設定を使用する RabbitMQ の構成を修正しました。[#36531](https://github.com/ClickHouse/ClickHouse/issues/36531) をクローズ。[#38365](https://github.com/ClickHouse/ClickHouse/pull/38365)（[Kseniia Sumarokova](https://github.com/kssenii)）。
...
* `DISTINCT` と `LIMIT` を含む分散クエリで結果が誤る可能性がある問題を修正。Fixes [#38282](https://github.com/ClickHouse/ClickHouse/issues/38282). [#38371](https://github.com/ClickHouse/ClickHouse/pull/38371)（[Anton Popov](https://github.com/CurtizJ)）。
* 0 バイトを含むパターンに対する countSubstrings() および position() の誤った結果を修正。 [#38589](https://github.com/ClickHouse/ClickHouse/pull/38589) ([Robert Schulze](https://github.com/rschu1ze)).
* これにより、IPv4/IPv6 表現の値が不正なテーブルに対しても、clickhouse-server を起動し、テーブルのアタッチやデタッチを行うことが可能になりました。Issue [#35156](https://github.com/ClickHouse/ClickHouse/issues/35156) に対する適切な修正です。[#38590](https://github.com/ClickHouse/ClickHouse/pull/38590)（[alesapin](https://github.com/alesapin)）。
...
### <a id="226"></a> ClickHouse リリース 22.6（2022-06-16）[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.6/)、[動画](https://www.youtube.com/watch?v=0fSp9SF8N8A) \{#a-id226a-clickhouse-release-226-2022-06-16\}
...
* `GROUPING` 関数を追加しました。これにより、`ROLLUP`、`CUBE`、`GROUPING SETS` を使用するクエリ内のレコードの曖昧さを解消できます。[#19426](https://github.com/ClickHouse/ClickHouse/issues/19426) をクローズしました。[#37163](https://github.com/ClickHouse/ClickHouse/pull/37163)（[Dmitry Novik](https://github.com/novikd)）。
...
* d3js ベースの OpenTelemetry トレース可視化ツールを追加しました。 [#37810](https://github.com/ClickHouse/ClickHouse/pull/37810)（[Sergei Trifonov](https://github.com/serxa)）。
...
* JOIN において、より多くのフィルタをプッシュダウンできるようになりました。 [#37472](https://github.com/ClickHouse/ClickHouse/pull/37472) ([Amos Bird](https://github.com/amosbird))。
* ワイドパーツを読み取る際には、必要なカラムのマークのみをロードするよう最適化しました。 [#36879](https://github.com/ClickHouse/ClickHouse/pull/36879) ([Anton Kozlov](https://github.com/tonickkozlov)).
* `MergeTree` テーブルで実験的設定 `ratio_of_defaults_for_sparse_serialization` を有効化することで使用可能な疎なカラムを、集約関数の引数として使用している場合の集約処理のパフォーマンスを改善しました。[#37617](https://github.com/ClickHouse/ClickHouse/pull/37617) ([Anton Popov](https://github.com/CurtizJ))。
* 2 つの引数を取る `COALESCE` 関数を最適化。 [#37666](https://github.com/ClickHouse/ClickHouse/pull/37666) ([Anton Popov](https://github.com/CurtizJ)).
* `multiIf` に条件が 1 つしかない場合は、よりパフォーマンスの高い `if` 関数に置き換えます。 [#37695](https://github.com/ClickHouse/ClickHouse/pull/37695) ([Anton Popov](https://github.com/CurtizJ)).
...
* 集約状態の破棄処理をスレッドプールにポストして実行できるようになりました。`LIMIT` を含み、集約状態が大きいクエリでは大きな高速化が得られます。例えば `select uniq(number) from numbers_mt(1e7) group by number limit 100` はおよそ 2.5 倍高速になりました。 [#37855](https://github.com/ClickHouse/ClickHouse/pull/37855) ([Nikita Taranov](https://github.com/nickitat)).
* 単一列のソート性能を改善。 [#37195](https://github.com/ClickHouse/ClickHouse/pull/37195) ([Maksim Kita](https://github.com/kitaisreal)).
* ソートキューを特殊化し、単一列ソートのパフォーマンスを向上しました。 [#37990](https://github.com/ClickHouse/ClickHouse/pull/37990) ([Maksim Kita](https://github.com/kitaisreal)).
* 配列のノルムおよび距離関数のパフォーマンスを 2～4 倍向上させました。 [#37394](https://github.com/ClickHouse/ClickHouse/pull/37394) ([Alexander Gololobov](https://github.com/davenger)).
* 動的ディスパッチを用いて数値比較関数のパフォーマンスを改善しました。 [#37399](https://github.com/ClickHouse/ClickHouse/pull/37399) ([Maksim Kita](https://github.com/kitaisreal)).
...
* ORDER BY 句に複数のカラムがある場合の MergeTree テーブルへの挿入のパフォーマンスを改善しました。 [#35762](https://github.com/ClickHouse/ClickHouse/pull/35762) ([Maksim Kita](https://github.com/kitaisreal)).
* テーブルが多数ある場合のバックグラウンド処理での過剰な CPU 使用を修正。 [#38028](https://github.com/ClickHouse/ClickHouse/pull/38028) ([Maksim Kita](https://github.com/kitaisreal)).
* 動的ディスパッチを用いて `not` 関数のパフォーマンスを向上しました。[#38058](https://github.com/ClickHouse/ClickHouse/pull/38058) ([Maksim Kita](https://github.com/kitaisreal))。
...
* フィルタビットマスク生成関数を AVX-512 命令を用いた一体化実装に改善しました。 [#37588](https://github.com/ClickHouse/ClickHouse/pull/37588) ([yaqi-zhao](https://github.com/yaqi-zhao)).
...
* 複数ディスクを利用した Hive ファイルキャッシュに対応しました。 [#37279](https://github.com/ClickHouse/ClickHouse/pull/37279) ([lgbo](https://github.com/lgbo-ustc))。
...
* ユーザーが、付与されたロールに由来する権限を確認できるようにしました。 [#36941](https://github.com/ClickHouse/ClickHouse/pull/36941) ([nvartolomei](https://github.com/nvartolomei)).
* 積分を数値積分で求めるのではなく、CDF 関数を使用します。これにより実行速度が向上し、精度も向上します。これによって [#36714](https://github.com/ClickHouse/ClickHouse/issues/36714) が修正されました。[#36953](https://github.com/ClickHouse/ClickHouse/pull/36953)（[Nikita Mikhaylov](https://github.com/nikitamikhaylov)）。
* 関数に `Nothing` 型のデフォルト実装を追加しました。これにより、引数のいずれかが `Nothing` の場合、ほとんどの関数は `Nothing` 型のカラムを返すようになりました。これは、引数として空配列を受け取る `arrayMap` / `arrayFilter` などの関数で発生していた問題も解決します。以前は `select arrayMap(x -> 2 * x, []);` のようなクエリは、ラムダ内の関数が `Nothing` 型を扱えないために失敗していましたが、現在はこのようなクエリは `Array(Nothing)` 型の空配列を返します。さらに、`arrayFilter` / `arrayFill` などの関数で Nullable 型の配列もサポートするようになりました。以前は `select arrayFilter(x -> x % 2, [1, NULL])` のようなクエリは失敗していましたが、現在は動作します（ラムダの結果が `NULL` の場合、その値は結果に含まれません）。[#37000](https://github.com/ClickHouse/ClickHouse/issues/37000) をクローズしました。[#37048](https://github.com/ClickHouse/ClickHouse/pull/37048)（[Kruglov Pavel](https://github.com/Avogar)）。
* 現在、シャードにローカルレプリカがある場合には、ローカル読み取り用のプランと、すべてのリモートレプリカから読み取るプランの 2 つを作成します。両方のプランは同じイニシエータを共有しており、そのイニシエータが読み取りを調整します。 [#37204](https://github.com/ClickHouse/ClickHouse/pull/37204) ([Nikita Mikhaylov](https://github.com/nikitamikhaylov)).
* 設定オプション「mark&#95;cache&#95;size」が明示的に設定されていないことを理由としてサーバーの起動を中止しないようにしました。 [#37326](https://github.com/ClickHouse/ClickHouse/pull/37326) ([Robert Schulze](https://github.com/rschu1ze))。
* カラム定義において、型名の直後に `NULL`/`NOT NULL` を指定できるようになりました。 [#37337](https://github.com/ClickHouse/ClickHouse/pull/37337) ([Igor Nikonov](https://github.com/devcrafter)).
* ファイルセグメント PARTIALLY&#95;DOWNLOADED に対する read buffer の取得を最適化。 [#37338](https://github.com/ClickHouse/ClickHouse/pull/37338) ([xiedeyantu](https://github.com/xiedeyantu)).
* ストレステスト時に発生していた問題を修正するため、短絡評価を行う関数の処理を改善しました。 [#37384](https://github.com/ClickHouse/ClickHouse/pull/37384) ([Kruglov Pavel](https://github.com/Avogar)).
...
* ゼロコピー レプリケーションでのパート取得中に極めてまれに発生するデッドロックを修正し、[#37423](https://github.com/ClickHouse/ClickHouse/issues/37423) を解決。[#37424](https://github.com/ClickHouse/ClickHouse/pull/37424)（[metahys](https://github.com/metahys)）。
* 不明なデータ形式のストレージを作成できないようにしました。 [#37450](https://github.com/ClickHouse/ClickHouse/pull/37450) ([Kruglov Pavel](https://github.com/Avogar)).
...
* play.html の query div を、高さ 20% を超えて拡張できるように変更しました。非常に長いクエリの場合、textarea 要素を拡張できると便利ですが、これまでは div の高さが固定のため、textarea を拡張すると下にある data div が隠れてしまっていました。この修正により、textarea 要素を拡張した際に data div が上下に押し出され、拡張された textarea に隠れないようになります。また、ユーザーが query の textarea のサイズを調整しても、query ボックスの幅が常に 100% のまま維持されるようにしました。 [#37488](https://github.com/ClickHouse/ClickHouse/pull/37488) ([guyco87](https://github.com/guyco87)).
...
* clickhouse-keeper の改善: 破損したログをタイムスタンプ付きのフォルダに移動します。 [#37565](https://github.com/ClickHouse/ClickHouse/pull/37565) ([Antonio Andelic](https://github.com/antonio2368)).
* 以降のマージ処理では、TTL により期限切れになった列を書き込まないようにしました（以前は、そのパーツに対する最初の merge/optimize の場合のみ TTL で期限切れになった列を書き込まず、2 回目以降のマージでは期限切れ列も書き込んでいました）。 [#37570](https://github.com/ClickHouse/ClickHouse/pull/37570) ([Azat Khuzhin](https://github.com/azat))。
* LowCardinality や Sparse カラムを含む場合の補助関数 `dumpColumnStructure` の結果が、より正確になりました。以前のバージョンでは、これらの関数は結果を返す前に引数を通常のカラムに変換していました。この変更は [#6935](https://github.com/ClickHouse/ClickHouse/issues/6935) への対応のために必要なものでした。[#37633](https://github.com/ClickHouse/ClickHouse/pull/37633)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* clickhouse-keeper: watch 用に一意なセッション ID のみを保存。 [#37641](https://github.com/ClickHouse/ClickHouse/pull/37641) ([Azat Khuzhin](https://github.com/azat)).
* 起こり得る「Cannot write to finalized buffer」エラーを修正。 [#37645](https://github.com/ClickHouse/ClickHouse/pull/37645) ([Azat Khuzhin](https://github.com/azat)).
...
* `system.build_options` テーブルで BoringSSL のバージョン関連情報を参照できるようにしました。 [#37850](https://github.com/ClickHouse/ClickHouse/pull/37850) ([Bharat Nallan](https://github.com/bharatnc)).
...
* タイムアウト後に壊れた detached パーツをクリーンアップします。 Closes [#25195](https://github.com/ClickHouse/ClickHouse/issues/25195)。 [#37975](https://github.com/ClickHouse/ClickHouse/pull/37975) ([Kseniia Sumarokova](https://github.com/kssenii)).
* 現在、MergeTree テーブルエンジンファミリーでは、移動に失敗したパーツは即座に削除されるようになりました。 [#37994](https://github.com/ClickHouse/ClickHouse/pull/37994) ([alesapin](https://github.com/alesapin))。
* ReplicatedMergeTree のマージで `always_fetch_merged_part` 設定が有効になっている場合、マージ時に他のレプリカ上のパーツを探す頻度が低くなり、[Zoo]Keeper への負荷も軽減されます。 [#37995](https://github.com/ClickHouse/ClickHouse/pull/37995) ([alesapin](https://github.com/alesapin)).
...
### <a id="225"></a> ClickHouse 22.5 リリース（2022-05-19）。[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.5/)、[動画](https://www.youtube.com/watch?v=jkXmXrmjaKQ) {#a-id225a-clickhouse-release-225-2022-05-19}
...
* 実験的機能: [PR](https://github.com/ClickHouse/ClickHouse/pull/36376) 内のテスト中に、あるキャッシュクラスが 2 回初期化されていることを発見し、その結果として例外がスローされました。この問題の原因はまだ明確ではありませんが、ClickHouse 内でディスクを繰り返しロードするコード上のロジックが存在しているはずなので、この状況に対しては特別な判定処理を行う必要があります。 [#36737](https://github.com/ClickHouse/ClickHouse/pull/36737) ([Han Shukai](https://github.com/KinderRiven)).
...
### <a id="224"></a> ClickHouse リリース 22.4, 2022-04-19. [プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.4/), [動画](https://www.youtube.com/watch?v=aFQs_zoYoXY) {#a-id224a-clickhouse-release-224-2022-04-19}
...
### <a id="223"></a> ClickHouse リリース v22.3-lts（2022-03-17）。[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.3/)、[動画](https://www.youtube.com/watch?v=GzeANZzPras) {#a-id223a-clickhouse-release-v223-lts-2022-03-17}
...
### <a id="222"></a> ClickHouse リリース v22.2（2022-02-17）。[プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.2/)、[動画](https://www.youtube.com/watch?v=6EG1gwhSTPg) {#a-id222a-clickhouse-release-v222-2022-02-17}
...
* FINAL を含むクエリに対してデータスキッピングインデックスを適用すると、誤った結果になる可能性があります。本リリースでは、FINAL を含むクエリに対してはデータスキッピングインデックスをデフォルトで無効化しました（新しい設定 `use_skip_indexes_if_final` が導入されており、デフォルトでは無効になっています）。[#34243](https://github.com/ClickHouse/ClickHouse/pull/34243) ([Azat Khuzhin](https://github.com/azat))。
...
### <a id="221"></a> ClickHouse リリース v22.1, 2022-01-18. [プレゼンテーション](https://presentations.clickhouse.com/2022-release-22.1/), [動画](https://www.youtube.com/watch?v=gP7I2SUBXig) {#a-id221a-clickhouse-release-v221-2022-01-18}
...
#### バグ修正（公式の stable または prestable リリースでのユーザーにとって目に見える不具合） {#bug-fix-user-visible-misbehavior-in-official-stable-or-prestable-release-4}
