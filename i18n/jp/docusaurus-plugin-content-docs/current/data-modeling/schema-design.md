...
ClickHouse には、データセットの型を自動的に判別するスキーマ推論機能があります。これは Parquet を含むすべてのデータ形式でサポートされています。この機能を利用し、s3 テーブル関数と [`DESCRIBE`](/sql-reference/statements/describe-table) コマンドを用いて、データに対する ClickHouse の型を特定できます。以下の例では、`stackoverflow/parquet/posts` フォルダ内のすべてのファイルを読み込むために、glob パターン `*.parquet` を使用しています。
...
> The [s3 テーブル関数](/sql-reference/table-functions/s3) allows data in S3 to be queried in-place from ClickHouse. This function is compatible with all of the file formats ClickHouse supports.
...
`PostTypeId` と `CreationDate` のカラムをオーダリングキーとして選択します。

今回のケースでは、ユーザーが常に `PostTypeId` でフィルタリングすることを想定しています。このカラムのカーディナリティは 8 であり、ordering key の最初のエントリとして論理的な選択です。日付の粒度でのフィルタリングで十分と判断し (datetime フィルターにも引き続き有効です) 、キーの第2部分として `toDate(CreationDate)` を使用します。日付は 16 ビットで表現できるため、索引のサイズも小さくなり、フィルタリングが高速化されます。最後のキーエントリは `CommentCount` で、最もコメントの多い投稿を見つける (最終的なソート) のに役立ちます。
