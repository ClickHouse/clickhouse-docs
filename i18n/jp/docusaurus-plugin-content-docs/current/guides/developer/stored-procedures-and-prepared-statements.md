...
従来型のリレーショナルデータベースの利用経験がある場合、ClickHouse でもストアドプロシージャやプリペアドステートメントを探しているかもしれません。
...
マテリアライズドビューは、従来ストアドプロシージャで実行されていた高コストな集計処理を事前計算するのに最適です。従来のデータベースから移行する場合、マテリアライズドビューは、ソーステーブルにデータが挿入される際に自動的にデータを変換・集計する**INSERTトリガー**として考えることができます。
...
複雑なビジネスロジック、ETLワークフロー、または複数ステップのプロセスの場合、言語クライアントを使用してClickHouseの外部でロジックを実装することが可能です。
...
def process_order(order_id: int, customer_id: int, order_total: Decimal) -> tuple[str, int]:
    """
    ストアドプロシージャに含まれるようなビジネスロジックを使用して注文を処理します。
    戻り値: (status_message, loyalty_points)

    注意: ClickHouseは分析処理に最適化されており、OLTPトランザクション向けではありません。
    ...
    # ステップ2: ティアに基づいて割引を計算(Pythonでのビジネスロジック)
    ...
    # 分析データベースでは、UPDATEよりもINSERTを使用することを推奨
    ...
    # ステップ6: ティアのアップグレードを確認(Pythonでのビジネスロジック)
...
3. **更新** - MySQLは`UPDATE`文を使用します。ClickHouseでは、可変データに対して[ReplacingMergeTree](/engines/table-engines/mergetree-family/replacingmergetree)または[CollapsingMergeTree](/engines/table-engines/mergetree-family/collapsingmergetree)を使用した`INSERT`を推奨します
...
クエリパラメータは**一般的なテキスト置換ではありません**。以下の特定の制限事項があります：

1. **主にSELECT文を対象としています** - SELECTクエリで最も良好にサポートされます
