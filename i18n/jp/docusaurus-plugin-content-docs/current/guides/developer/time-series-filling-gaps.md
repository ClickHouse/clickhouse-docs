...
sidebar_label: 'タイムシリーズ - ギャップフィル'
sidebar_position: 10
description: 'タイムシリーズデータのギャップを埋める。'
keywords: ['タイムシリーズ', 'ギャップフィル']
title: 'タイムシリーズデータのギャップを埋める'
---


# タイムシリーズデータのギャップを埋める

タイムシリーズデータを扱う際、データの欠落や非活性によりギャップが生じることがあります。  
通常、データをクエリする際にこれらのギャップを存在させたくはありません。この場合、`WITH FILL`句が役立ちます。  
このガイドでは、`WITH FILL`を使用してタイムシリーズデータのギャップを埋める方法について説明します。

## セットアップ {#setup}

以下の表は、GenAI画像サービスによって生成された画像のメタデータを保存しています：
...
2023年3月24日の`00:24:03`から`00:24:04`の間に作成された画像を調べるので、これらの時点のパラメータを作成します：
...
次に、データを100msのバケットにグループ化し、そのバケット内で作成された画像のカウントを返すクエリを記述します：
...
結果セットには、画像が作成されたバケットのみが含まれていますが、タイムシリーズ分析のためには、エントリーがない場合でも各100msバケットを返したいかもしれません。

## WITH FILL {#with-fill}

`WITH FILL`句を使用してこれらのギャップを埋めることができます。  
ギャップを埋めるサイズを示す`STEP`も指定します。  
これは、`DateTime`型のデフォルトで1秒ですが、私たちは100msの長さのギャップを埋めたいので、ステップ値として100msの間隔を設定しましょう：
...
ギャップが`count`カラムに0の値で埋められたことがわかります。

## WITH FILL...FROM {#with-fillfrom}

ただし、時間範囲の最初にまだギャップがあり、`FROM`を指定することで修正できます：
...
しかし、時間範囲の最後にいくつかのバケットがまだ不足しており、`TO`値を提供することで埋めることができます。  
`TO`は含まれないため、最後の時刻を少し加えて、その時刻が含まれるようにします：
...
これで、`00:24:03.000`から`00:24:05.000`までのすべての100msに対してエントリーが得られました。

## 累積カウント {#cumulative-count}

次に、バケット全体の作成された画像の累積カウントを維持したいとしましょう。  
これを実現するために、`cumulative`カラムを追加します：
...
累積カラムの値が期待どおりに機能していません。

## WITH FILL...INTERPOLATE {#with-fillinterpolate}

`count`カラムに`0`がある行は、累積カラムにも`0`があるため、以前の累積カラムの値を使用する方が良いでしょう。  
これを実現するために、次のように`INTERPOLATE`句を使用します：
...
これで、一段と良くなりました。  
最後に、`bar`関数を使用してバーチャートを追加し、新しいカラムを`INTERPOLATE`句に追加するのを忘れないようにします。
