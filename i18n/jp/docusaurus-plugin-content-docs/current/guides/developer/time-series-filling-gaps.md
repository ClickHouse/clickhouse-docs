...
時系列データを扱う際、欠損データや非アクティブな期間が原因で、データに空白（ギャップ）が生じることがあります。
多くの場合、データをクエリするときにはこうしたギャップは存在してほしくありません。このような場合に役立つのが `WITH FILL` 句です。
このガイドでは、`WITH FILL` を使用して時系列データのギャップを補完する方法について説明します。
...
次に、データを100ミリ秒のバケットにグループ化し、そのバケット内で作成された画像の件数を返すクエリを記述します:
...
結果セットには画像が作成されたバケットのみが含まれますが、時系列分析では、エントリが存在しない場合でも100ミリ秒ごとのすべてのバケットを返したい場合があります。
...
`DateTime`型のデフォルトは1秒ですが、100msの長さのギャップを埋めたいので、ステップ値として100msの間隔を設定しましょう:
...
結果から、`00:24:03.000` から `00:24:03.500` までのバケットがすべて表示されるようになったことが確認できます。
...
すべてのギャップが補完され、`00:24:03.000` から `00:24:05.000` まで 100 ミリ秒ごとのエントリが揃いました。


## 累積カウント {#cumulative-count}

バケット全体で作成された画像数の累積カウントを保持したい場合を考えてみましょう。
...
cumulative カラムの値が期待通りに動作していません。
