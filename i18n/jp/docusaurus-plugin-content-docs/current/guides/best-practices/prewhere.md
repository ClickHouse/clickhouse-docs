...
sidebar_label: 'PREWHERE の最適化'
sidebar_position: 21
description: 'PREWHERE は不要なカラムデータの読み取りを避けることで I/O を削減します。'
title: 'PREWHERE の最適化はどのように機能しますか？'
doc_type: 'guide'
...
[PREWHERE 句](/sql-reference/statements/select/prewhere) は、ClickHouse におけるクエリ実行のための最適化機能です。不要なデータの読み取りを避け、フィルタ対象でない列をディスクから読み込む前に無関係なデータを除外することで、I/O を削減しクエリ速度を向上させます。
...
<br />
<br />① クエリには`town`列に対するフィルタが含まれており、この列はテーブルのプライマリキーの一部であり、したがってプライマリインデックスの一部でもあります。

② クエリを高速化するため、ClickHouseはテーブルのプライマリインデックスをメモリにロードします。

③ インデックスエントリをスキャンして、述語に一致する行を含む可能性のあるtown列のグラニュールを特定します。

④ これらの関連する可能性のあるグラニュールが、クエリに必要な他の列の位置的に整列されたグラニュールとともにメモリにロードされます。

⑤ 残りのフィルタはクエリ実行時に適用されます。

ご覧のように、PREWHEREを使用しない場合、実際に一致する行がわずかであっても、フィルタリングの前に関連する可能性のあるすべての列がロードされます。
...
ClickHouseは、クエリのPREWHEREバージョンと非PREWHEREバージョンの両方で同じ数の行を処理することに注意してください。ただし、PREWHERE最適化が適用されると、処理されるすべての行に対してすべてのカラム値をロードする必要はありません。
:::
...
バージョン[23.11](https://clickhouse.com/blog/clickhouse-release-23-11#column-statistics-for-prewhere)以降、オプションのカラム統計により、カラムサイズだけでなく実際のデータ選択性に基づいてフィルタ処理順序を選択することで、さらなる改善が可能です。
...
ClickHouseはこのクエリで231万行を処理し、**23.36 MB**のカラムデータを読み取りました。
...
処理された行数は同じ(231万行)ですが、PREWHEREにより、ClickHouseが読み取ったカラムデータは3分の1以下に削減され、23.36 MBではなくわずか6.74 MBとなりました。その結果、総実行時間が3分の1に短縮されました。
...
プラン出力は非常に冗長なため、ここではほとんどを省略しています。要点として、3つのカラム述語すべてが自動的にPREWHEREに移動されたことが示されています。

これを自分で再現する際、クエリプランにおいて、これらの述語の順序がカラムのデータ型サイズに基づいていることがわかります。カラム統計を有効にしていないため、ClickHouseはPREWHERE処理順序を決定する際の代替基準としてサイズを使用します。
...
- フィルタリングの順序は重要です。小さく選択性の高いカラムを最初に配置してください。
