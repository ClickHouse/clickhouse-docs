...
sidebar_position: 20
description: 'ClickHouse は処理レーンと max_threads 設定を使用して、クエリの実行を並列化します。'
title: 'ClickHouse によるクエリの並列実行方法'
doc_type: 'guide'
...
ClickHouse は[高速性のために設計されています](/concepts/why-clickhouse-is-so-fast)。利用可能なすべての CPU コアを使い、データを複数の処理レーンに分散し、多くの場合ハードウェアの限界近くまで性能を引き出しながら、クエリを高度に並列化された形で実行します。
 
このガイドでは、ClickHouse におけるクエリの並列実行がどのように動作するか、そして大規模なワークロードでパフォーマンスを向上させるために、それをどのようにチューニングし監視できるかを説明します。
...
ClickHouseが①テーブルのプライマリキーに対するフィルタを含む集約クエリを実行する際、②プライマリインデックスをメモリにロードし、③処理が必要なグラニュールと安全にスキップできるグラニュールを識別します:
...
`8`コアを搭載したマシンでは、より多くのレーンがデータを並列処理するため、クエリ処理のスループットはおよそ2倍になります(ただし、メモリ使用量もそれに応じて増加します):
...
<br />
<br />
クエリを最初に受信したサーバーは、すべてのシャードからのサブ結果を収集し、それらを最終的なグローバル結果に統合します。

シャード間でクエリ負荷を分散することで、特に高スループット環境において並列処理の水平スケーリングが可能になります。

:::note ClickHouse Cloudではシャードの代わりに並列レプリカを使用
ClickHouse Cloudでは、この同じ並列処理が[並列レプリカ](https://clickhouse.com/docs/deployment-guides/parallel-replicas)によって実現されます。これはシェアードナッシングクラスタにおけるシャードと同様に機能します。各ClickHouse Cloudレプリカ(ステートレスなコンピュートノード)は、データの一部を並列に処理し、独立したシャードと同様に最終結果に貢献します。
:::


## クエリの並列処理の監視 {#monitoring-query-parallelism}

これらのツールを使用して、クエリが利用可能なCPUリソースを完全に活用しているかを確認し、活用していない場合の診断を行います。

ここでは59個のCPUコアを持つテストサーバー上で実行しており、ClickHouseのクエリ並列処理能力を十分に示すことができます。

サンプルクエリがどのように実行されるかを観察するために、集計クエリの実行中にすべてのトレースレベルのログエントリを返すようClickHouseサーバーに指示できます。このデモンストレーションでは、クエリの述語を削除しました。そうしないと、3つのグラニュールのみが処理され、ClickHouseが複数の並列処理レーンを使用するには十分なデータ量ではないためです:
...
- ① ClickHouseは3つのデータ範囲にわたって3,609個のグラニュール(トレースログではmarksとして示される)を読み取る必要があります。
...
注意: 上記の演算子プランは下から上に読みます。各行は物理実行プランのステージを表し、下部のストレージからのデータ読み取りから始まり、上部の最終処理ステップで終わります。`× 59`とマークされた演算子は、59個の並列処理レーンにわたって重複しないデータ領域で同時に実行されます。これは`max_threads`の値を反映しており、クエリの各ステージがCPUコア間でどのように並列化されるかを示しています。
...
注意: 視覚化は左から右に読みます。各行は、ブロックごとにデータをストリーミングし、フィルタリング、集計、最終処理ステージなどの変換を適用する並列処理レーンを表します。この例では、`max_threads = 4`設定に対応する4つの並列レーンを確認できます。

### 処理レーン間の負荷分散 {#load-balancing-across-processing-lanes}

上記の物理プランの`Resize`演算子は、処理レーン間でデータブロックストリームを[再分割および再分配](/academic_overview#4-2-multi-core-parallelization)し、均等に利用されるようにします。この再バランシングは、クエリ述語に一致する行数がデータ範囲によって異なる場合に特に重要です。そうしないと、一部のレーンが過負荷になり、他のレーンがアイドル状態になる可能性があります。作業を再分配することで、高速なレーンが低速なレーンを効果的に支援し、全体的なクエリ実行時間を最適化します。
...
上記の出力に示されているように、クエリは231万行を処理し、13.66MBのデータを読み取りました。これは、インデックス解析フェーズにおいて、ClickHouseが処理対象として**282個のグラニュール**を選択したためです。各グラニュールには8,192行が含まれており、合計で約231万行になります:
...
設定された`max_threads`の値に関係なく、ClickHouseは十分なデータがある場合にのみ追加の並列処理レーンを割り当てます。`max_threads`の「max」は上限を意味しており、使用されるスレッド数を保証するものではありません。
...
共有ストレージを使用するクラスタの場合(例: ClickHouse Cloud):
...
本番環境でこれらの設定を変更することは推奨しません。ここでは、`max_threads` が実際の並列度を常に決定するわけではない理由を示すためだけに表示しています。
:::

デモンストレーションのために、これらの設定を一時的に上書きして同時実行性を最大まで高めた場合の物理プランを確認してみましょう。
...
この設定により、ClickHouse は設定された `max_threads` を厳密に遵守しつつ、59 本の同時ストリームを使ってデータをスキャンするようになりました。

これは、小さなデータセットに対するクエリでは、ClickHouse が意図的に並行性を制限することを示しています。設定の上書きはテスト目的にのみ使用し、本番環境では使用しないでください。非効率な実行やリソース競合を引き起こす可能性があります。
...
ClickHouseがクエリを並列実行する仕組みや、大規模環境で高性能を実現する方法についてより深く理解したい場合は、以下のリソースをご参照ください:
