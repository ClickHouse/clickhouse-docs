...
title: 'sumArray'
description: 'sumArrayコンビネーターの使用例'
...
[`Array`](/sql-reference/aggregate-functions/combinators#-array) コンビネーターは、[`sum`](/sql-reference/aggregate-functions/reference/sum) 関数に適用することで、配列内のすべての要素の合計を計算します。この操作は `sumArray` 集約コンビネーター関数を使用して行われます。

`sumArray` 関数は、データセット内の複数の配列にわたるすべての要素の合計を計算する必要がある場合に便利です。

## 使用例 {#example-usage}

この例では、異なる製品カテゴリの毎日の売上データセットを使用して、`sumArray` がどのように機能するかを示します。各日のすべてのカテゴリの合計売上を計算します。
...
`sumArray` 関数は、それぞれの `category_sales` 配列内のすべての要素を合計します。たとえば、`2024-01-01` では、`100 + 200 + 150 = 450` となります。これは `arraySum` と同じ結果を示します。
