...
コンビネータを[`quantilesTiming`](/sql-reference/aggregate-functions/reference/quantiletiming)
関数に適用することで、条件が真である行について、配列内のタイミング値の分位数を計算できます。
この計算には`quantilesTimingArrayIf`集約コンビネータ関数を使用します。
