...
## ClickHouse デプロイメントを作成する \{#1-create-a-clickhouse-deployment\}
...
## TLS 証明書を作成する \{#2-create-tls-certificates\}
:::note
...
## 証明書と鍵を保存するディレクトリを作成して設定する。 \{#3-create-and-configure-a-directory-to-store-certificates-and-keys\}
...
## ClickHouse Keeper を使用して基本クラスタで環境を構成する \{#4-configure-the-environment-with-basic-clusters-using-clickhouse-keeper\}
...
1. ClickHouse サーバーの `config.xml` の `<clickhouse>` タグ内に以下を追加します
    ...
    :::note
    ClickHouse Keeper が ClickHouse サーバーに埋め込まれている場合（上記のとおり）、Keeper は [ClickHouse ノードで TLS インターフェースを設定する](#5-configure-tls-interfaces-on-clickhouse-nodes) の OpenSSL セクションで定義されたサーバーの OpenSSL 設定を使用します。ClickHouse Keeper をスタンドアロンプロセスとして実行する場合は、同じ CA 証明書とノード証明書/鍵の設定を含む `<openSSL>` セクションを Keeper 設定ファイルに追加する必要があります。詳細については、以下の [スタンドアロンの ClickHouse Keeper 向けに OpenSSL を設定する](#configure-openssl-for-standalone-clickhouse-keeper) を参照してください。
    :::
...
## ClickHouse ノードで TLS インターフェースを設定する \{#5-configure-tls-interfaces-on-clickhouse-nodes\}
...
3. ClickHouse Keeper の正常性を確認します
...
## OpenSSL の検証モードと証明書 handler \{#openssl-verification-modes\}
...
### 検証モード \{#verification-modes\}

`<openSSL>` の `<server>` または `<client>` セクション内で `<verificationMode>` を設定します。

| Mode      | Description                                                                                          |
| --------- | ---------------------------------------------------------------------------------------------------- |
| `none`    | 証明書の検証は行いません。接続は暗号化されますが、相手先の身元は確認されません。テスト用途でのみ使用してください。                                            |
| `relaxed` | 相手先から証明書が提示された場合は検証しますが、証明書が提供されなくても失敗しません。                                                          |
| `once`    | サーバー側では、初回のハンドシェイク時にのみクライアント証明書を検証し、再ネゴシエーションは行いません。クライアント側では、`relaxed` と同じように動作します。                 |
| `strict`  | 相手先の証明書を必須とし、完全に検証します。証明書がない場合、有効期限が切れている場合、または信頼された CA によって署名されていない場合、接続は失敗します。プロダクション環境での使用を推奨します。 |

### 無効な証明書の handler \{#invalid-certificate-handlers\}
...
| Handler                    | 説明                                       |
| -------------------------- | ---------------------------------------- |
| `RejectCertificateHandler` | 証明書が無効な場合は接続を拒否します。これはデフォルト設定であり、推奨されます。 |
| `AcceptCertificateHandler` | 証明書が無効でも接続を受け入れます。これはテスト時にのみ使用してください。    |

### 例: 証明書検証の無効化 \{#disabling-certificate-verification\}
