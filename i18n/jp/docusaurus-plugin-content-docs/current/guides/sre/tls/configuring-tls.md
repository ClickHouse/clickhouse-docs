...
<VerticalStepper headerLevel="h2">
  ...
  4. 各ノードについて、証明書要求 (CSR) を作成し、秘密鍵を生成します:
     ```bash
  ...
  :::note
  これは各ノードで実行する必要があります。各ホストでは適切な証明書と秘密鍵を使用してください。
  :::
  ...
  6. 証明書とパスを指定して OpenSSL を構成します
     ...
     詳細については、[このページ](/operations/server-configuration-parameters/settings/#openssl)を参照してください
  ...
  3. ClickHouse Keeper の正常性を確認します
     通常の [4 letter word (4lW)](/guides/sre/keeper/reference#four-letter-word-commands) コマンドは、TLS なしでは `echo` を使って実行できないため、ここでは `openssl` を使用する方法を説明します。
...
## スタンドアロンの ClickHouse Keeper 向けに OpenSSL を設定する \{#1-create-a-clickhouse-deployment\}
...
## OpenSSL の検証モードと証明書 handler \{#2-create-tls-certificates\}
