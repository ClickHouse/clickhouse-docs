...
SQL ベースのワークフローの利用を推奨します。両方の設定方法は同時に利用できるため、サーバー設定ファイルでアカウントやアクセス権を管理している場合でも、SQL ベースのワークフローへスムーズに移行できます。
...
:::note
ClickHouse Cloud コンソールのユーザーを管理したい場合は、この[ページ](/cloud/security/manage-cloud-users) を参照してください。
:::
...
3.  `default`ユーザーの[権限を制限](/operations/settings/permissions-for-queries)し、そのユーザーに対してSQLベースのアクセス制御とアカウント管理を無効化します。
...
:::note
行ポリシーは、読み取り専用アクセス権を持つユーザーに対してのみ有効です。ユーザーがテーブルを変更したり、テーブル間でパーティションをコピーできる場合、行ポリシーの制限は無効化されます。
:::
...
- 少なくとも1つのユーザーアカウントに対してSQLベースのアクセス制御とアカウント管理を有効化します。
  ...
  デフォルトでは、すべてのユーザーに対してSQLベースのアクセス制御とアカウント管理が無効になっています。`users.xml`設定ファイルで少なくとも1人のユーザーを設定し、[`access_management`](/operations/settings/settings-users.md#access_management-user-setting)、`named_collection_control`、`show_named_collections`、`show_named_collections_secrets`の各設定値を1に設定する必要があります。
...
1.  `users.xml`ファイルの`<default>`ユーザー配下でSQLユーザーモードを有効化します:
    ...
    :::note
    `default`ユーザーは新規インストール時に作成される唯一のユーザーであり、デフォルトでノード間通信に使用されるアカウントでもあります。

    本番環境では、SQL管理ユーザーでノード間通信を設定し、`<secret>`、クラスター認証情報、および/またはノード間HTTPおよびトランスポートプロトコル認証情報でノード間通信を設定した後、`default`アカウントがノード間通信に使用されるため、このユーザーを無効化することを推奨します。
    :::
...
`ALTER`文はいくつかのカテゴリに分類されており、階層構造を持つものと、階層構造を持たず明示的に定義する必要があるものがあります。
...
`GRANT ALTER on *.* TO my_user`を使用すると、最上位レベルの`ALTER TABLE`と`ALTER VIEW`にのみ影響し、その他の`ALTER`文は個別に付与または取り消す必要があります。
...
ユーザーまたはロールにサブ権限が付与されている場合、そのサブ権限を直接取り消すことも、そのサブ権限が継承している上位レベルの権限を取り消すこともできます。
...
権限は、`WITH GRANT OPTION` を持っているだけでなく、付与しようとしている権限自体も保持しているユーザーによって付与されなければなりません。
...
これでユーザーは `ALTER COLUMN` とそのすべてのサブ特権を付与または取り消せるようになりました。
...
0 行のセット。経過時間: 0.004 秒。

サーバーから例外を受信しました (バージョン 22.5.1):
コード: 497. DB::Exception: chnode1.marsnet.local:9440 から受信。DB::Exception: my_user: 権限が不足しています。このクエリを実行するには、my_db.my_table に対する ALTER DROP COLUMN(column2) の権限が必要です。(ACCESS_DENIED)
```

5. 権限を付与して alter 権限管理者をテストする
...
8. `ALTER` 管理ユーザーが持っていない権限を付与しようとした場合、その権限が管理ユーザーに対して付与されている権限のサブ権限には該当しないことをテストします。
...
**概要**
`ALTER` の権限は、テーブルおよびビューに対する `ALTER` については階層構造になっていますが、それ以外の `ALTER` ステートメントについては階層構造ではありません。権限は個別の細かな粒度でも、権限のグループ単位でも設定でき、同様の単位で取り消すこともできます。権限を付与または取り消すユーザーは、ユーザー（実行中のユーザー自身を含む）に対して権限を設定するための `WITH GRANT OPTION` を持ち、かつその権限自体も既に持っていなければなりません。実行中のユーザーは、自身が `WITH GRANT OPTION` を持っていない権限については、自分自身からその権限を取り消すことはできません。
