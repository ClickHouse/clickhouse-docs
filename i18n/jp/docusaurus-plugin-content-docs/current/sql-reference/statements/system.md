...
または、最小値と最大値をスペースで区切って指定することで、一様分布に従うランダムな秒数を指定できます：
...
いずれの場合も `ALL` パラメータを使用します:

```sql
SYSTEM INSTRUMENT REMOVE ALL
```

またはサブクエリから得られる ID の集合：

```sql
SYSTEM INSTRUMENT REMOVE (SELECT id FROM system.instrumentation WHERE handler = 'log')
```

インストルメンテーションポイント ID は、[`system.instrumentation`](../../operations/system-tables/instrumentation.md) システムテーブルから取得できます。
