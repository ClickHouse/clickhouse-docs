...
すべての [内部Dictionary](./create/dictionary/overview.md) を再読み込みします。
...
`AvroConfluent` 形式で使用される、URL 単位の Confluent スキーマレジストリのキャッシュをクリアします。これにより、スキーマ取得キャッシュ (id → schema) とスキーマ登録キャッシュ (subject + schema → id) の両方が破棄されるため、以降の読み取りおよび書き込みはレジストリサーバーに直接アクセスするようになります。これは、スキーマレジストリ側でスキーマが削除または書き換えられた場合や、テストでレジストリの冪等性を検証する場合に有用です。
...
次のいずれかの方法で単一のインストルメンテーションポイントを削除します:

```sql
SYSTEM INSTRUMENT REMOVE ID
```

`ALL` キーワードを使用してすべてを削除します:
...
または、指定した `function_name` に一致するすべてのインストルメンテーションポイントを対象とする場合：
