...
| [`SOURCE`](./sources/overview.md)           | Dictionary のデータソースを定義します（例: ClickHouse テーブル、HTTP、PostgreSQL）。         |
| [`LAYOUT`](./layouts/overview.md)           | Dictionary をメモリ上にどのように格納するかを制御します（例: `FLAT`, `HASHED`, `CACHE`）。      |
...
- [Layouts](/sql-reference/statements/create/dictionary/layouts) — Dictionary がメモリ上でどのように格納されるか
