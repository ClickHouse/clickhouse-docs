...
キー構造は `<key>` 要素内で設定します。キー フィールドは、Dictionary の[属性](#dictionary-key-and-fields)と同じ形式で指定します。例:
