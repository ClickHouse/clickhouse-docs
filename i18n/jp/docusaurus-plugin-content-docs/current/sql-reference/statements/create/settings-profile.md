...
`ON CLUSTER` 句を指定すると、クラスター全体に対して設定プロファイルを作成できます。詳細は [Distributed DDL](../../../sql-reference/distributed-ddl.md) を参照してください。
...
`max_memory_usage` 設定に対する値と制約を持つ `max_memory_usage_profile` 設定プロファイルを作成し、ユーザー `robin` に割り当てます:
