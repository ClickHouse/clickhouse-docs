...
認証方式に対して、有効期限日と、必要に応じて有効期限の時刻を指定できます。文字列をパラメータとして受け取ります。日時の指定には `YYYY-MM-DD [hh:mm:ss] [timezone]` 形式を使用することを推奨します。このとき、`[timezone]` には `+09:00` のような数値オフセット、または `UTC`、`GMT`、`Z`、`MSK`、`MSD` のいずれかを指定する必要があります。`Asia/Tokyo` のような名前付き IANA タイムゾーンは認識されません (下記の注記を参照してください) 。デフォルトでは、このパラメータは `'infinity'` です。
...
:::note
日時文字列は `parseDateTimeBestEffort` によって解析されます。この関数が認識するタイムゾーントークンは `UTC`、`GMT`、`Z`、`MSK`、`MSD`、および `+09:00` や `-05:00` のような数値オフセットのみです。`Asia/Tokyo` や `Europe/London` のような名前付き IANA タイムゾーンはサポートされておらず、固定オフセットは夏時間を採用する地域では IANA ゾーンと同等ではないため、エンコードする対象の日付に対して正しいオフセットを計算する必要があります。
:::
