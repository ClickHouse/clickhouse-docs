...
`ALP()` — 10進スケーリングに基づく浮動小数点データ向けのアダプティブな可逆圧縮方式です。ALP は各値を 10 のべき乗を用いて厳密なスケーリング済み整数として表現しようとし、その結果得られた整数を Frame-of-Reference とビットパッキングで圧縮します。厳密に表現できない値は、例外として生の値のまま保存されます。10進数（例：測定値、通貨）に由来する数値に対して最も効果的です。`Float32` と `Float64` をサポートします。詳細は [ALP: Adaptive lossless floating-point compression](https://ir.cwi.nl/pub/33334) を参照してください。

:::note
このコーデックは実験的機能であり、使用するには `SET allow_experimental_codecs = 1` を設定する必要があります。
:::
