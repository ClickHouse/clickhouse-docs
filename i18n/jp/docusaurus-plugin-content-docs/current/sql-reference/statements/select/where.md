---
description: '`WHERE` 句に関する ClickHouse ドキュメント'
sidebar_label: 'WHERE'
...
## 条件としての UInt8 カラムの使用 \{#using-uint8-columns-as-a-condition\}
...
### 条件として UInt8 カラムを使う \{#example-uint8-column-as-condition\}

[前の例](#example-filtering-with-logical-operators) のテーブルを使って、カラム名をそのまま条件として使用できます：
