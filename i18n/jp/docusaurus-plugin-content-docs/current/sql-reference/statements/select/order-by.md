...
これを実現するには、`timestamp` 列を補完する際に、`sensor_id` 列をソートプレフィックスとして使用します。
...
* ブログ記事: [ClickHouse で時系列データを扱う方法](https://clickhouse.com/blog/working-with-time-series-data-and-functions-ClickHouse)