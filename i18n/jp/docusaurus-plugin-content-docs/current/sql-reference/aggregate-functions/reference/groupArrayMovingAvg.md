...
この関数はウィンドウサイズをパラメータとして受け取れます。指定しない場合、ウィンドウサイズにはカラム内の行数と同じ値が使用されます。
...
この関数は[0 への丸め (rounding towards zero)](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero) を使用します。結果のデータ型において有効でない小数部分を切り捨てます。
