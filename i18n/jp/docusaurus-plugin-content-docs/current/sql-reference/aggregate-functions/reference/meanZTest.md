...
両方のサンプルの値は `sample_data` カラムに含まれています。`sample_index` が 0 の場合、その行の値は第1母集団からのサンプルに属します。それ以外の場合は第2母集団からのサンプルに属します。
帰無仮説は、母集団の平均が等しいというものです。正規分布に従うと仮定します。母集団の分散は等しくない場合があり、いずれの分散も既知であるとします。
