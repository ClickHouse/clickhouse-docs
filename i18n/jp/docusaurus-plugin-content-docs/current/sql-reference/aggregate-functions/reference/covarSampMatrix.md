...
* `x1[, x2, ...]` — 標本共分散行列を計算する対象となる 1 つ以上の引数。[`(U)Int*`](/sql-reference/data-types/int-uint) または [`Float*`](/sql-reference/data-types/float) または [`Decimal`](/sql-reference/data-types/decimal)

**返される値**

標本共分散行列を返します。[`Array(Array(Float64))`](/sql-reference/data-types/array)
