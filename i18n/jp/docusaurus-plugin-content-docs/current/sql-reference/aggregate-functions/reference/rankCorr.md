...
* `x` — 任意の値。[`Float*`](/sql-reference/data-types/float)
* `y` — 任意の値。[`Float*`](/sql-reference/data-types/float)

**返り値**

`x` と `y` の順位に対する順位相関係数を返します。相関係数の値は -1 から +1 の範囲です。[`Float64`](/sql-reference/data-types/float)
