...
両サンプルの値は `sample_data` カラムにあります。`sample_index` が 0 の場合、その行の値は第1母集団からのサンプルに属します。そうでない場合は第2母集団からのサンプルに属します。
帰無仮説は、母集団の平均が等しいというものです。分散が等しい正規分布であると仮定します。
...
* `sample_index` — サンプルを識別するインデックス。[Integer](../../../sql-reference/data-types/int-uint.md)。

**パラメータ**

* `confidence_level` — 信頼区間を計算するために使用する信頼水準。[Float](../../../sql-reference/data-types/float.md)。
