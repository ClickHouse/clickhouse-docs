---
description: 'covarSamp と似ていますが、計算誤差が小さくなる代わりに遅く動作します。'
...
`Σ((x - x̅)(y - y̅)) / (n - 1)` の値を計算します。 [covarSamp](../reference/covarsamp.md) と似ていますが、計算誤差が小さくなる代わりに遅く動作します。
...
- `x` と `y` の標本共分散。 `n <= 1` の場合、 `inf` が返されます。 [Float64](../../data-types/float.md)。
