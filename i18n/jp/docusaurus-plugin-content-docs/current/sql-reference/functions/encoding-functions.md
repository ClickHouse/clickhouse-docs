...
* `mode` — オプション。先頭バイトを witness version として取り除かずにデコードするには `'raw'` を渡します。非 SegWit アドレス (例: Cosmos SDK) の場合はこれを使用します。[`String`](/sql-reference/data-types/string)
...
* `hrp` — コードの「human-readable part」を指定する、`1 - 83` 文字の小文字英字から成る文字列。通常は &#39;bc&#39; または &#39;tb&#39;。[`String`](/sql-reference/data-types/string) または [`FixedString`](/sql-reference/data-types/fixedstring)
* `data` — エンコード対象のバイナリデータを表す文字列。[`String`](/sql-reference/data-types/string) または [`FixedString`](/sql-reference/data-types/fixedstring)
* `witver_or_variant` — 任意。UInt* の witness version (デフォルト = 1、Bech32 の場合は `0`、Bech32m の場合は `1` 以上) または String のエンコーディング variant のいずれかを指定します: `'bech32'` (BIP173) または `'bech32m'` (BIP350)。文字列の variant を使用する場合、witness version バイトは前置されません。これは Cosmos SDK などの非 SegWit アドレスで必要です。[`UInt*`](/sql-reference/data-types/int-uint) または [`String`](/sql-reference/data-types/string)
...
**Cosmos SDK アドレス (BIP173、witness version を含まない)**
