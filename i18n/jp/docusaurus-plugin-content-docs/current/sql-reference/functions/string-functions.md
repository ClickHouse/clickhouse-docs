...
* `index` — 省略可能。どの正規表現グループを抽出するかを示す 0 以上の整数です。デフォルト値は、`pattern` に少なくとも 1 つのキャプチャグループが含まれている場合は `1`、キャプチャグループがない場合は `0` (マッチ全体) です。[`(U)Int*`](/sql-reference/data-types/int-uint)
