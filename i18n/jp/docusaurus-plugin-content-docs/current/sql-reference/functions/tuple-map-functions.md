...
- `key_n` — マップエントリのキー。 [Map](../data-types/map.md) のキータイプとしてサポートされている任意の型。
- `value_n` — マップエントリの値。 [Map](../data-types/map.md) の値タイプとしてサポートされている任意の型。
...
- `keys` — マップを作成するためのキーの配列またはマップ。 [Array](../data-types/array.md) または [Map](../data-types/map.md)。 `keys` が配列の場合、NULL値を含まない限り、`Array(Nullable(T))` または `Array(LowCardinality(Nullable(T)))` 型を受け入れます。
...
入力文字列のキーと値のペアは、キーの後にキー-値区切り文字があり、値が続きます。
キー値ペアはペア区切り文字で区切られています。
キーと値は引用符で囲むことができます。
...
- キー-値ペアの [Map(String, String)](../data-types/map.md) 型。
...
エスケープシーケンスを使用してエスケープシーケンスがサポートされていない:
...
`toString` でシリアル化されたマップ文字列のキー-値ペアを復元するには:
...
非標準のエスケープシーケンスは、そのまま（バックスラッシュを含む）返されます。ただし、以下のいずれかである場合を除きます：
`\\`, `'`, `"`, `バックティック`, `/`, `=` または ASCII 制御文字（c &lt;= 31）。

この関数は、事前エスケープと事後エスケープが適さないユースケースを満たします。たとえば、次の入力文字列を考慮してください: `a: "aaaa\"bbb"`。期待される出力は: `a: aaaa\"bbbb`。
- 事前エスケープ：事前エスケープすると、出力は `a: "aaaa"bbb"` になり、次に `extractKeyValuePairs` は `a: aaaa` に出力されます。
- 事後エスケープ： `extractKeyValuePairs` は `a: aaaa\` と出力し、事後エスケープはそのまま保持します。

先頭のエスケープシーケンスは、キーではスキップされ、値については無効とみなされます。

**例**

エスケープシーケンスがサポートされたエスケープシーケンス:
...
引数は [maps](../data-types/map.md) または [tuples](../data-types/tuple.md#tuplet1-t2) の二つの [arrays](../data-types/array.md#data-type-array) であり、最初の配列の項目はキーを表し、2 番目の配列には各キーの値が含まれています。すべてのキー配列は同じタイプである必要があり、すべての値配列は同じタイプに昇格される項目を含む必要があります（[Int64](../data-types/int-uint.md#int-ranges)、[UInt64](../data-types/int-uint.md#uint-ranges) または [Float64](../data-types/float.md#float32-float64)）。共通の昇格型が結果配列の型として使用されます。

**返される値**

- 引数に応じて、1 つの [map](../data-types/map.md) または [tuple](../data-types/tuple.md#tuplet1-t2) を返します。最初の配列にはソートされたキーが含まれ、2 番目の配列には対応する値が含まれます。
...
整数キーを持つマップに欠落したキー-値ペアを補完します。
最大キーを指定することで、キーを最大値を超えて拡張することをサポートします。
具体的には、この関数は、最小キーから最大キー（または指定された `max` 引数）までのシリーズを形成するキーを持つマップを返し、対応する値を持ちます。
指定されたキーの値がない場合、デフォルト値が値として使用されます。
...
配列引数の場合、各行における `keys` と `values` の要素数は同じでなければなりません。

**引数**

引数は [Maps](../data-types/map.md) または二つの [Arrays](../data-types/array.md#data-type-array) であり、最初の配列と2番目の配列は各キーのキーと値を含む必要があります。
...
- 引数に応じて、ソートされた順序のキーと、対応するキーの値を持つ [Map](../data-types/map.md) または [Tuple](../data-types/tuple.md#tuplet1-t2) の二つの [Arrays](../data-types/array.md#data-type-array)。
...
指定したキーが指定したマップに含まれているかどうかを返します。
...
- `key` — キー。タイプは `map` のキータイプと一致する必要があります。

**返される値**

- `map` が `key` を含む場合は `1` を、含まない場合は `0` を返します。 [UInt8](../data-types/int-uint.md)。
...
- `map` が指定したパターンのような `key` を含む場合は `1` を、含まない場合は `0` を返します。
...
文字列キーを持つマップと LIKE パターンを提供すると、この関数はキーがパターンに一致する要素を持つマップを返します。
...
- 指定されたパターンに一致する要素を含むマップ。要素が一致しない場合は空のマップが返されます。
...
- 元のマップから得られるマップを返します。各要素に対して `func(map1[i], ..., mapN[i])` が適用されます。
...
- `func(map1[i], ..., mapN[i])` が `0` 以外の値を返すマップ要素のみを含むマップを返します。
...
同じキーを持つ要素が 2 つ以上の入力マップに存在する場合、すべての要素が結果マップに追加されますが、最初の要素のみが演算子 `[]` でアクセス可能です。
...
`map` に少なくとも一つのキー-値ペアが存在し、`func(key, value)` が 0 以外の値を返す場合は 1 を、そうでない場合は 0 を返します。
...
`func(key, value)` が `map` のすべてのキー-値ペアで 0 以外の値を返す場合は 1 を、そうでない場合は 0 を返します。
...
マップの要素を昇順にソートします。
`func` 関数が指定されている場合は、マップのキーと値に適用される `func` 関数の結果によってソート順序が決まります。
...
詳しくは、`arraySort` 関数の [リファレンス](../../sql-reference/functions/array-functions.md#array_functions-sort) を参照してください。

## mapPartialSort {#mappartialsort}

追加の `limit` 引数を使用して、要素を昇順にソートします。 
`func` 関数が指定されている場合は、マップのキーと値に適用される `func` 関数の結果によってソート順序が決まります。
...
詳しくは、関数 [arrayReverseSort](../../sql-reference/functions/array-functions.md#array_functions-reverse-sort) を参照してください。

## mapPartialReverseSort {#mappartialreversesort}

追加の `limit` 引数を使用して、要素を降順にソートします。
`func` 関数が指定されている場合は、マップのキーと値に適用される `func` 関数の結果によってソート順序が決まります。
