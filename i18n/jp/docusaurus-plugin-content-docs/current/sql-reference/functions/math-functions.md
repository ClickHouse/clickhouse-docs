...
この上限は `10^-15` 未満に保たれ、最大値の `256` では `10^-154` 未満に保たれます。

この関数は決定論的です。証人は `n` からシードされるため、同じ `(n, rounds)` の組み合わせからは常に
同じ結果が得られます。`4^(-rounds)` の上限は、一様ランダムな証人を用いた場合の入力ごとの確率です。
一方、ここでの決定論的なシードでは、これは入力全体にわたる割合を表します。つまり、その
証人列をすり抜ける合成数は、再現可能に `1` を返します。
