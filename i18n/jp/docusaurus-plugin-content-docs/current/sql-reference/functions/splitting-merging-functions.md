...
指定されたトークナイザを使って文字列をトークンに分割します。
デフォルトのトークナイザは、英数字ではない ASCII 文字を区切りとして使用します。

`splitByString` トークナイザの場合、トークンが [prefix code](https://en.wikipedia.org/wiki/Prefix_code) を構成しないときには、より長いセパレータを優先してマッチさせたいケースが多いでしょう。
そのためには、セパレータを長さの降順で指定してください。
例えば、separators = `['%21', '%']` のとき、文字列 `%21abc` は `['abc']` としてトークン化されますが、separators = `['%', '%21']` の場合は `['21ac']` としてトークン化されます（これはおそらく意図しない結果です）。
