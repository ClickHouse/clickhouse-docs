...
* `unicode_word` は、Unicode の単語境界ルール (UAX #29 に類似) を使用して文字列をトークンに分割します。ASCII の英数字とアンダースコアは、コネクタ (文字に対する `:`、同種の文字に対する `.` および `'`) とともにトークンを構成します。ASCII 以外の Unicode 文字は、1 文字ずつのトークンになります。
