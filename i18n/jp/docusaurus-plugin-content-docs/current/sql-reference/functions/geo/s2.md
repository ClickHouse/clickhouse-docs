...
* `s2Rect1PointLow`, `s2Rect1PointHi` — 最初の長方形に対応する S2 ポイントインデックスの下限値および上限値。[UInt64](../../data-types/int-uint.md)。
* `s2Rect2PointLow`, `s2Rect2PointHi` — 2 番目の長方形に対応する S2 ポイントインデックスの下限値および上限値。[UInt64](../../data-types/int-uint.md)。
...
この長方形と指定された長方形との交差部分全体を含む、最小の長方形を返します。S2 システムでは、長方形は緯度・経度空間の長方形を表す `S2LatLngRect` という種類の S2Region で表現されます。
...
* `s2Rect1PointLow`, `s2Rect1PointHi` — 第1の長方形に対応する Low および High の S2 ポイントインデックス。[UInt64](../../data-types/int-uint.md)。
* `s2Rect2PointLow`, `s2Rect2PointHi` — 第2の長方形に対応する Low および High の S2 ポイントインデックス。[UInt64](../../data-types/int-uint.md)。

**返される値**

* `s2UnionRect2PointLow` — 指定された長方形同士の共通部分を含む長方形に対応する Low S2 セル ID。[UInt64](../../data-types/int-uint.md)。
* `s2UnionRect2PointHi` — 指定された長方形同士の共通部分を含む長方形に対応する High S2 セル ID。[UInt64](../../data-types/int-uint.md)。
