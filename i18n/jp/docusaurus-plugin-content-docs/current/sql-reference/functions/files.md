...
- `path` — [user_files_path](../../operations/server-configuration-parameters/settings.md#user_files_path) からの相対ファイルパス。ワイルドカード `*`、`**`、`?`、`{abc,def}`、`{N..M}` をサポートします。ここで `N`、`M` は数値、`'abc'`、`'def'` は文字列です。
...
ファイル a.txt と b.txt からデータを文字列としてテーブルに挿入する:
