...
- `[, guess]` — 内部収益率の初期推定値（定数値）（オプション、デフォルトは0.1）。[`Float*`](/sql-reference/data-types/float)

**戻り値**

内部収益率を返します。計算が収束しない場合、入力配列が空または要素が1つのみの場合、すべてのキャッシュフローがゼロの場合、またはその他の計算エラーが発生した場合は`NaN`を返します。[`Float64`](/sql-reference/data-types/float)
...
不規則な間隔で発生する一連のキャッシュフローに対する拡張内部収益率（XIRR）を計算します。XIRRは、すべてのキャッシュフローの正味現在価値（NPV）がゼロとなる割引率です。
...
- 'ACT_365F' - Actual/365 Fixed：日付間の実際の日数を365で除算した値を使用
- 'ACT_365_25' - Actual/365.25：日付間の実際の日数を365.25で除算した値を使用
...
デフォルト方式（`start_from_zero` = true）:

$$
\sum_{i=0}^{N-1} \frac{values_i}{(1 + rate)^i}
$$

Excel互換方式（`start_from_zero` = false）:
...
- `[, start_from_zero]` — NPV計算を期間`0`から開始するか（true）、期間`1`から開始するか（false、Excel互換）を指定するオプションのブール値パラメータ。デフォルト: true。[`Bool`](/sql-reference/data-types/boolean)
...
- [ClickHouseの金融関数に関する動画](https://www.youtube.com/watch?v=BePLPVa0w_o)
