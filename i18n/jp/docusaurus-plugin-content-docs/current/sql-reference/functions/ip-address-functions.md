...
プレフィックス長付きの IPv4 アドレス (CIDR 形式) を受け取り、そのサブネットのアドレス範囲を、最初と最後の IPv4 アドレスからなる 2 要素のタプルとして返します。
...
32 ビット整数を IPv4 アドレスの文字列表現 (ドット区切り 10 進数表記の A.B.C.D 形式) に変換します。
入力はビッグエンディアンのバイト順として解釈されます。
...
* `num` — UInt32 型の数値として表現された IPv4 アドレス。[`UInt32`](/sql-reference/data-types/int-uint)

**戻り値**

IPv4 アドレスを表す値、またはフォーマットが無効な場合は `0` を返します。[`String`](/sql-reference/data-types/string)
...
32 ビット整数を、IPv4 アドレスの文字列表現 (ドット区切りの 10 進表記 (A.B.C.D 形式) ) に変換します。
...
* `num` — UInt32 型の数値として表された IPv4 アドレス。[`UInt32`](/sql-reference/data-types/int-uint)
...
ドット区切り 10進表記 (A.B.C.D 形式) の IPv4 アドレス文字列を、対応する 32 ビット整数表現に変換します ([`IPv4NumToString`](#IPv4NumToString) の逆の変換) 。
IPv4 アドレスの形式が不正な場合は、例外がスローされます。
...
IPv4 アドレスを返します。無効なアドレスの場合は `NULL` を返します。[`Nullable(UInt32)`](/sql-reference/data-types/nullable)
...
CIDR プレフィックス長付きの IPv6 アドレスを受け取り、そのサブネット内のアドレス範囲を、最小アドレスと最大アドレスからなる 2 つの IPv6 値のタプルとして返します。
...
サブネットの範囲を表す 2 つの IPv6 アドレスを含むタプルを返します。[`Tuple(IPv6, IPv6)`](/sql-reference/data-types/tuple)
...
バイナリ形式 (FixedString(16)) の IPv6 アドレスを、標準的な文字列表現に変換します。
...
IPv6 アドレスを標準テキスト表現からバイナリ形式 (`FixedString(16)`) に変換します。
`::ffff:111.222.33.44.` の形式の IPv4 マップド IPv6 アドレスを受け付けます。
IPv6 アドレスの形式が無効な場合は、例外がスローされます。

入力文字列に有効な IPv4 アドレスが含まれている場合、対応する IPv6 アドレスを返します。
HEX 表記は大文字・小文字のどちらでもかまいません。
...
IPv6 アドレスを、その標準的なテキスト表現からバイナリ形式 (`FixedString(16)`) に変換します。
...
* `string` — IPv6 アドレスを表す文字列。[`String`](/sql-reference/data-types/string)
...
`::ffff:111.222.33.44.` 形式の、IPv4アドレスをマッピングしたIPv6アドレスを受け付けます。
IPv6アドレスの形式が不正な場合は、`NULL` を返します。
...
IPv6 アドレスをバイナリ形式で返します。無効な場合には `NULL` を返します。[`Nullable(FixedString(16))`](/sql-reference/data-types/nullable)
...
IPv6 アドレスをバイナリ形式で表現した `FixedString(16)` 型の値を受け取ります。
指定したバイト数だけ切り詰めたアドレスをテキスト形式で返します。
...
* `x` — バイナリ形式の IPv6 アドレス。[`FixedString(16)`](/sql-reference/data-types/fixedstring) または [`IPv6`](/sql-reference/data-types/ipv6)
* `bytesToCutForIPv6` — IPv6 に対して削除するバイト数。[`UInt8`](/sql-reference/data-types/int-uint)
* `bytesToCutForIPv4` — IPv4 に対して削除するバイト数。[`UInt8`](/sql-reference/data-types/int-uint)

**返り値**

指定されたバイト数を削除したテキスト形式の IPv6 アドレスを含む文字列を返します。[`String`](/sql-reference/data-types/string)
...
[Classless Inter-Domain Routing (CIDR)](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) 表記で表されているネットワークに、指定した IP アドレスが含まれているかどうかを判定します。
...
アドレスと CIDR の IP バージョンが一致する場合は `1` を返し、それ以外の場合は `0` を返します。[`UInt8`](/sql-reference/data-types/int-uint)
...
IPv4 アドレス向けの関数については [`isIPv4String`](#isIPv4String) を参照してください。
...
`string` が IPv6 アドレスであれば `1`、それ以外の場合は `0` を返します。[`UInt8`](/sql-reference/data-types/int-uint)
...
文字列または UInt32 型で表現された IPv4 アドレスを IPv4 型に変換します。
[`IPv4StringToNum`](/sql-reference/functions/ip-address-functions#IPv4StringToNum) および [`IPv4NumToString`](/sql-reference/functions/ip-address-functions#IPv4NumToString) 関数と似ていますが、引数として文字列型と符号なし整数型の両方をサポートします。
...
IPv4 アドレスを返します。[`IPv4`](/sql-reference/data-types/ipv4)
...
IPv4 アドレスを表す文字列または UInt32 形式の値を [`IPv4`](../data-types/ipv4.md) 型に変換します。
IPv4 アドレスの形式が不正な場合は `0.0.0.0` (0 IPv4) または指定されたデフォルトの IPv4 アドレスを返します。
...
* `default` — 省略可。文字列が不正な IPv4 アドレスの場合に返す値。[`IPv4`](/sql-reference/data-types/ipv4)

**戻り値**

文字列を変換して得られる対応する IPv4 アドレス、または変換に失敗した場合はデフォルト値を返します。[`IPv4`](/sql-reference/data-types/ipv4)
...
成功した場合は IPv4 アドレスを返し、それ以外の場合は `NULL` を返します。[`IPv4`](/sql-reference/data-types/ipv4) または [`NULL`](/sql-reference/syntax#null)
...
[`toIPv4`](#toIPv4) と同様ですが、変換エラーが発生した場合、例外をスローする代わりに IPv4 のゼロアドレス (`0.0.0.0`) を返します。
...
成功した場合は IPv4 アドレスを返し、そうでない場合はゼロ値の IPv4 アドレス (`0.0.0.0`) を返します。[`IPv4`](/sql-reference/data-types/ipv4)
...
IPv6 アドレスを表す文字列または `UInt128` 形式の値を [`IPv6`](../data-types/ipv6.md) 型に変換します。
文字列の場合、IPv6 アドレスの形式が無効なときは空の値を返します。
IPv6 アドレスをバイナリ形式 (すなわち `FixedString(16)`) に相互変換する [`IPv6StringToNum`](/sql-reference/functions/ip-address-functions#IPv6StringToNum) および [`IPv6NumToString`](/sql-reference/functions/ip-address-functions#IPv6NumToString) 関数と同様です。
...
IPv6 アドレスの形式が無効な場合は、`::` (全ゼロの IPv6 アドレス) または指定されたデフォルトの IPv6 を返します。
...
IPv6 アドレスを返します。`string` 引数の形式が不正な場合は、`::` または指定された省略可能な `default` を返します。[`IPv6`](/sql-reference/data-types/ipv6)
...
入力値を型 `IPv6` の値に変換しますが、エラーが発生した場合には `NULL` を返します。
...
成功時は IPv6 アドレスを返し、失敗時は `NULL` を返します。[`IPv6`](/sql-reference/data-types/ipv6) または [`NULL`](/sql-reference/syntax#null)
...
[`toIPv6`](#toIPv6) に似ていますが、変換エラー時に例外をスローする代わりにゼロの IPv6 アドレス (`::`) を返します。
...
* `x` — IPv6 または IPv4 アドレスを表す文字列。[`String`](/sql-reference/data-types/string)

**戻り値**

成功した場合は IPv6 アドレスを返し、失敗した場合はゼロ値の IPv6 アドレス (`::`) を返します。[`IPv6`](/sql-reference/data-types/ipv6)
