...
この値は決定論的です。入力タグが同一であれば、常に同じキーが生成されます。
`limitk` や `limit_ratio` などのサンプリングオペレーターのソートキーとして使用することを想定しています。
...
グループに関連付けられたタグから算出される、安定した `UInt64` ハッシュ値。[`UInt64`](/sql-reference/data-types/int-uint)
...
この関数は [throwIf()](/sql-reference/functions/other-functions#throwIf) に似ていますが、
使用するエラーコードとエラーメッセージの形式が異なります。
...
* `condition` — 判定する条件。通常は関数 [count()](/sql-reference/aggregate-functions/reference/count#count) を含みます。[`UInt8`](/sql-reference/data-types/int-uint)
* `group` — タグのグループ。[`UInt64`](/sql-reference/data-types/int-uint)
