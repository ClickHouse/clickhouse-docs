---
description: 'Amazon S3 上の Apache Hudi テーブルに読み取り専用のテーブル形式インターフェイスを提供する。'
sidebar_label: 'hudi'
sidebar_position: 85
...
| `extra_credentials`                          | 省略可能なパラメータ。ClickHouse Cloud でロールベースのアクセスに使用する `role_arn` を渡すために使用します。設定手順については、[Secure S3](/cloud/data-sources/secure-s3) を参照してください。                                                                                                                                  |
