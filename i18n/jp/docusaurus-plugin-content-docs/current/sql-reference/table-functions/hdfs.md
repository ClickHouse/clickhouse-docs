...
`use_hive_partitioning` 設定値を 1 にすると、ClickHouse はパス (`/name=value/`) 内の Hive スタイルのパーティショニングを検出し、クエリ内でパーティション列を仮想列として利用できるようにします。これらの仮想列の名前は、パーティションパス内の名前と同じになります。
