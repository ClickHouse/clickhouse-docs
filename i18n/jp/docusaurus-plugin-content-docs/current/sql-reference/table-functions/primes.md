...
* 最も高速なのは、デフォルトのステップ（`1`）を使う単純な範囲クエリおよびポイントフィルタクエリで、例えば `primes(N)` や `primes() LIMIT N` です。これらの形式では、最適化された素数ジェネレーターを使用して、非常に大きな素数を効率的に計算します。例えば、次のクエリはほぼ瞬時に実行されます。
