---
description: '指定されたスキーマに基づいてランダムなデータを生成します。そのデータを使ってテスト用テーブルにデータを投入できます。すべてのデータ型がサポートされているわけではありません。'
...
指定したスキーマに従ってランダムなデータを生成します。
そのデータを使ってテスト用テーブルにデータを投入できます。
...
| `name`              | 対応するカラムの名前。                                                                          |
| `TypeName`          | 対応するカラムの型。                                                                            |
| `random_seed`       | 安定した結果を生成するために手動でランダムシードを指定します。`NULL`の場合、シードはランダムに生成されます。 |
| `max_string_length` | 生成されるすべての文字列の最大長。デフォルトは`10`。                                        |
...
[generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure)と組み合わせた使用例:
...
`structure`引数を省略した場合(この場合、構造はランダムになります):
...
ランダム構造とランダムデータの両方に乱数シードを指定する場合は、次のようにします：
...
:::note
`max_array_length` に十分大きな値を設定した `generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` は、複合型（`Array`、`Tuple`、`Map`、`Nested`）のネストの深さが最大 16 階層になり得るため、非常に大きな出力を生成する可能性があります。
:::
