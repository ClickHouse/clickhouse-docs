...
MergeTree テーブルのインデックスファイルおよびマークファイルの内容を表します。テーブルの内部状態を調査する目的で使用できます。
...
| 引数          | 説明                                              |
| ------------- | ------------------------------------------------- |
| `database`    | インデックスとマークの読み取り元となるデータベース名。   |
| `table`       | インデックスとマークの読み取り元となるテーブル名。      |
| `with_marks`  | マークを含む列を結果に含めるかどうか。 |
| `with_minmax` | 最小値・最大値インデックスを結果に含めるかどうか。      |


## 戻り値 {#returned_value}

ソーステーブルのプライマリインデックスおよびmin-maxインデックス(有効な場合)の値を持つカラム、ソーステーブルのデータパート内のすべての可能なファイルに対するマーク(有効な場合)の値を持つカラム、および以下の仮想カラムを含むテーブルオブジェクト:

- `part_name` - データパートの名前。
- `mark_number` - データパート内の現在のマークの番号。
- `rows_in_granule` - 現在のグラニュール内の行数。

マークカラムは、カラムがデータパートに存在しない場合、またはそのサブストリームの1つに対するマークが書き込まれていない場合(例: コンパクトパート内)に、`(NULL, NULL)`値を含むことがあります。
