...
**正規化**。
文字列を `Time64` にパースする際、時刻部分は正規化されますが、検証は行われません。
...
**飽和**。
時刻 (time-of-day) 部分は、部分への変換やテキストへのシリアル化時に [-999:59:59.xxx, 999:59:59.xxx] の範囲に制限されます。
保存されている数値自体はこの範囲を超える場合がありますが、部分の抽出 (時、分、秒) およびテキスト表現では、飽和させた値が使用されます。
...
## Date との加算 \{#addition-with-date\}

[Time64](time64.md) 値は [Date](date.md) または [Date32](date32.md) 値に加算でき、`Time64` と同じ小数位精度を持つ [DateTime64](datetime64.md) を生成します:
...
サポートされているすべての組み合わせと結果の型の詳細については、[日付と時刻の加算](../operators/index.md#date-time-addition)を参照してください。
