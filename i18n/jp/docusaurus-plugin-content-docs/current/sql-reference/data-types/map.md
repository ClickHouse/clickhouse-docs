...
他のデータベースと異なり、ClickHouse におけるマップではキーは一意である必要はありません。つまり、同じキーを持つ要素を 2 つ含むことができます。
 (これは、マップ が内部的には `Array(Tuple(K, V))` として実装されているためです。) 

マップ `m` からキー `k` に対応する値を取得するには、構文 `m[k]` を使用できます。
また、`m[k]` はマップ を走査するため、この操作の実行時間は マップ のサイズに比例します。
...
:::note
`with_buckets` シリアライゼーションを使用する場合、マップ値内のキーの順序は元の挿入順と異なることがあります。キーはハッシュに基づいて各バケットに分散され、挿入順ではなくバケット順で再構築されます。`basic` シリアライゼーションでは、挿入されたマップのキー順序は保持されます。
:::
