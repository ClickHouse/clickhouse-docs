...
:::tip[カラム名へのドットの使用は避ける]
ドットを含むカラム名、共通のドット区切りプレフィックスを持つカラム、および `Array` 型のカラムは、それぞれ、`flatten_nested = 1` (デフォルト) の場合にフラット化された `Nested` 構造の一部として解釈されることがあります。これにより、挿入時の予期しない配列長の検証や名前変更に関する制約が発生する可能性があります。

可能であればカラム名にドットを使用するのは避けてください。
`Nested` のセマンティクスが明示的に必要な場合を除き、カラム名のドットの代わりにアンダースコア (`_`) や別の区切り文字を使用してください。
:::
