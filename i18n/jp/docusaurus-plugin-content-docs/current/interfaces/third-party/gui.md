...
[CHouse UI](https://chouse-ui.com) は、**本番環境で ClickHouse を運用するチーム**向けに構築された、オープンソースのセルフホスト型 ClickHouse Web インターフェイスです。多くのツールは、クエリワークスペース、ダッシュボード、AI アシスタント、クラスター監視といった機能のどれか 1 つには優れていますが、CHouse UI はそれらを*組み合わせた*ものです。チーム向けのアクセスレイヤーに、複数クラスターのフリート監視と自律型の読み取り専用 AI SRE を組み合わせています。データベース認証情報への直接アクセスを必要とするクライアントとは異なり、認証情報はサーバー側で暗号化して保存され、独自の **Role-Based Access Control (RBAC)** レイヤーでアクセスを制御するため、ブラウザーが ClickHouse のパスワードに触れることはありません。
...
* **マルチクラスターのフリート** - 設定済みのすべてのクラスターを 1 つの画面で監視できます (ステータス、メモリ、アクティブなクエリ、例外、トレンドのスパークライン) 。各カードは個別にポーリングされ、バックエンドのスナップショットポーラーによって支えられています。
* **Chouse AI — Fleet Doctor** - 自律型の読み取り専用 AI SRE です。保護された `system.*` 専用の `SELECT` ツール (ClickHouse `readonly=1`) でフリートをスキャンし、根本原因を特定して、負荷の高いクエリの詳細分析と推奨される書き換えを含む構造化レポートを作成します。クラスターを変更することはありません。
* **監視タブ内の AI** - Query Logs の行で使える「Optimize with Chouse AI」 (書き換え + 変更前→変更後の `EXPLAIN` 見積もり + SQL ワークスペースで開く) に加え、`system.errors` の行やパーツログのエントリで使えるワンクリックの「Diagnose」も利用できます。
* **しきい値アラート** - ノードメモリ使用率、クエリごとのメモリ使用量、長時間実行クエリのルールを Slack とメールに配信し、しきい値超過時には自律的な根本原因分析が添付されます。
* **フルワークスペース** - Monaco SQL エディター、スキーマエクスプローラー、強制終了対応のライブクエリビュー、ClickHouse ネイティブの監視 (メモリ内訳、パーツ/マージ、レプリカ遅延、レイテンシのパーセンタイル) 、およびデータのインポート/エクスポート。

オープンソース (Apache 2.0) で、オンプレミスを優先した設計です。すべての機能が標準で含まれており、有料 tier はありません。

[CHouse UI ソースコード](https://github.com/daun-gatal/chouse-ui)

### clickhouse-flow \{#clickhouse-flow\}

[clickhouse-flow](https://github.com/MikeAmputer/clickhouse-flow) は、ClickHouse のテーブル、ビュー、materialized view 間のデータフローと依存関係を可視化するオープンソースツールです。

機能:

* ClickHouse のメタデータから スキーマ グラフを自動的に構築します。
* materialized view を介したデータフローを可視化します。
* スキーマ 構造を探索するためのインタラクティブな UI。
* ドキュメント作成や共有向けに、図を PDF または SVG としてエクスポートします。
* 開発環境ですばやくセットアップできる、Docker ベースのデプロイに対応しています。
...
[1bench](https://1bench.dev) は、複数のデータベースに対応し、ClickHouse を第一級にサポートするネイティブのデスクトップ GUI です。サーバー概要、スキーマ 管理、ベクトル検索、大規模な結果セットの閲覧まで幅広く対応します。

機能:

* 接続時にサーバー概要を表示 — バージョン、稼働時間、実行中のクエリ、進行中のマージ、パーツ数とストレージサイズ、レプリカの状態、クラスターとノードをひと目で確認できます。
* ビジュアルクエリビルダー (カラム選択、フィルター、並べ替え、制限) に加え、構文ハイライト付きの Monaco SQL エディタと、接続ごとのクエリ履歴を備えています。
* `MergeTree` 系、`ORDER BY`、`PARTITION BY`、`SETTINGS`、`Nullable()` の自動ラップに対応したビジュアルな `CREATE TABLE` ウィザード。
* ClickHouse の型をネイティブに処理 — `Nullable`、`Array`、`LowCardinality`、ネストされたオブジェクトに対応。
* ベクトル検索をサポート — `Array(Float32)` の埋め込みカラムをコンパクトなベクトルセルとして表示し、2D 埋め込み可視化と `cosineDistance` による類似検索に対応します。
* 結果テーブル内でのインラインデータ編集とバッチ保存に加え、ClickHouse のネイティブフォーマットを使用した CSV/JSON/SQL のエクスポートとインポートに対応。
* 接続オプション: HTTP/HTTPS、ファイアウォールの内側にあるプライベートクラスター向けの SSH トンネル、安全にプロダクション環境を参照するための任意の読み取り専用モード。
* ClickHouse Cloud と セルフホスト の両方に対応しています。