...
ClickHouse テーブルのカラムのデータ型は、対応する Arrow のデータフィールドと一致している必要はありません。データの挿入時には、ClickHouse は上の表に従ってデータ型を解釈し、その後、データを [キャスト](/sql-reference/functions/type-conversion-functions#CAST) して ClickHouse テーブルのカラムに設定されたデータ型に変換します。
