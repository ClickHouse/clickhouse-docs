---
alias: []
description: 'CustomSeparated フォーマットに関するドキュメント'
input_format: true
...
[Template](../Template/Template.md) と同様ですが、すべての列名と型を出力または読み取りを行い、[format_custom_escaping_rule](../../../operations/settings/settings-formats.md/#format_custom_escaping_rule) 設定で指定されたエスケープ規則と、以下の設定で指定された区切り文字を使用します：
...
:::note
フォーマット文字列に由来するエスケープ規則や区切り文字に関する設定は使用しません。
:::

[`CustomSeparatedIgnoreSpaces`](../CustomSeparated/CustomSeparatedIgnoreSpaces.md) フォーマットもあり、これは [TemplateIgnoreSpaces](../Template//TemplateIgnoreSpaces.md) と同様です。
...
`CustomSeparated` 形式を使ってデータを読み込みます:
...
| Setting                                                                                                                                                        | 説明                                                                                                                         | 既定値 |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------|--------|
| [input_format_custom_detect_header](../../../operations/settings/settings-formats.md/#input_format_custom_detect_header)                                       | 存在する場合、名前と型を含むヘッダー行を自動検出します。                                                                    | `true` |
...
| [input_format_custom_allow_variable_number_of_columns](../../../operations/settings/settings-formats.md/#input_format_custom_allow_variable_number_of_columns) | CustomSeparated フォーマットで可変個数の列を許可し、余分な列を無視して、欠損している列にはデフォルト値を使用します。        | `false`|