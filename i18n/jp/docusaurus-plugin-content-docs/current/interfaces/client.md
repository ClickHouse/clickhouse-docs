...
| `--inline-insert-data`          | `INSERT ... VALUES` (およびその他のインラインフォーマット) は、データを Native フォーマット のブロックに変換せず、クエリテキスト内でそのまま送信します。サーバー側でインラインデータを直接解析するため、テーブル構造やカラムのデフォルト値をクライアントへ返送するラウンドトリップを回避できます。これにより、ネイティブプロトコル 経由で多数の小規模な INSERT を行う場合のパフォーマンスが向上することがあります。[`send_table_structure_on_insert_with_inline_data`](/operations/settings/settings#send_table_structure_on_insert_with_inline_data) を自動的に `0` に設定します。インラインデータと外部データ (stdin または `INFILE` からのデータ) は併用できません。 |
...
| `--echo [ <bool> ]`               | 実行前に各クエリを表示します。オプションで真偽値を指定できます。                                                                                                             | 対話型モードでは `true`、非対話 (batch) モードでは `false` |
| `--echo-formatted [ <bool> ]`     | 表示するクエリをフォーマットします。オプションで真偽値を指定できます。                                                                                                          | 対話型モードでは `true`、非対話 (batch) モードでは `false` |
| `--echo-query-id [ <bool> ]`      | 実行前に クエリ ID を表示します。オプションで真偽値を指定できます。                                                                                                       | 対話型モードでは `true`、非対話 (batch) モードでは `false` |
| `--highlight [ --hilite ] <bool>` | コマンドプロンプトと表示するクエリの構文ハイライトを切り替えます。                                                                                                            | `true`                                   |
