...
ClickHouse は、このデータの構造をあらかじめ指定しなくても読み取ることができます。
...
さらに、現在キャッシュされている全スキーマを保持しているシステムテーブル [schema&#95;inference&#95;cache](../operations/system-tables/schema_inference_cache.md) と、全ソースまたは特定ソースのスキーマキャッシュを削除できるシステムクエリ `SYSTEM CLEAR SCHEMA CACHE [FOR File/S3/URL/HDFS]` があります。
...
配列に異なる型の値が含まれており、設定 `input_format_json_infer_array_of_dynamic_from_array_of_different_types` が有効になっている場合（デフォルトで有効）、その配列は `Array(Dynamic)` 型になります:
...
この設定を有効にすると、ネストされた JSON オブジェクトを文字列として読み取ります。
JSON オブジェクト型を使用せずにネストされた JSON オブジェクトを読み取る場合に使用します。

この設定はデフォルトで有効になっています。

注意: この設定を有効にしても、`input_format_json_try_infer_named_tuples_from_objects` 設定が無効になっている場合にのみ効果があります。
...
データが null のみで構成されており、ClickHouse がクォートで囲まれた値の型を判別できない場合、ClickHouse はその値を String 型として扱います。
...
ヘッダーが検出されるのは、少なくとも1つのカラムが String 以外の型である場合のみです。すべてのカラムが String 型の場合、ヘッダーは検出されません。
...
データに NULL しか含まれておらず ClickHouse が型を判別できない場合、ClickHouse はその列を String 型として扱います。
...
`input_format_tsv_use_best_effort_in_schema_inference` 設定を無効化した場合の例：
...
Values 形式では、ClickHouse は行からカラム値を抽出し、リテラルの解析と同様の再帰的パーサーを使用して解析します。
...
Template と同様に、Regexp フォーマットでは ClickHouse はまず指定された正規表現に従って行からすべてのカラム値を抽出し、その後、指定されたエスケープ規則に従って各値のデータ型を推論しようとします。
...
#### input\_format\_max\_rows\_to\_read\_for\_schema\_inference/input\_format\_max\_bytes\_to\_read\_for\_schema\_inference \{#input-format-max-rows-to-read-for-schema-inference\}
...
この設定を有効にすると、テキスト形式のスキーマ推論において、ClickHouse は文字列フィールドから `DateTime` または `DateTime64` 型を推論しようとします。
サンプルデータ内のあるカラムについて、すべてのフィールドが日時として正常にパースされた場合、結果の型は `DateTime` か `DateTime64(9)`（いずれかの datetime に小数部が含まれていた場合）となります。
...
Avro フォーマットでは、ClickHouse はデータからスキーマを読み取り、次の型対応に従って ClickHouse のスキーマに変換します。
...
Arrow 形式では、ClickHouse はスキーマをデータから読み取り、次の型対応に基づいて ClickHouse スキーマに変換します。
...
このフォーマットでは、ClickHouse はデータ内の JSON オブジェクト全体を `String` 型の単一のカラムとして読み取ります。このフォーマットで推論されるデータ型は常に `String` で、カラム名は `json` です。
...
形式や構造を指定せずに、このファイルに対して中身を確認したりクエリを実行したりできます。
