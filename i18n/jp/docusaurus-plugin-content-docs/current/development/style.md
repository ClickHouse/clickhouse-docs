...
ただし、内側の `statement` に中括弧または `else` が含まれる場合は、外側のブロックを中括弧で記述する必要があります。
...
この例は、http://home.tamk.fi/~jaalto/course/coding-style/doc/unmaintainable-code/ から引用したものです。
...
変数を値渡しする場合、`const` を使用しても通常は意味がありません。

**11.** unsigned。

必要な場合は `unsigned` を使用する。
...
ログは、基本的にアプリケーションコード内でのみ使用するようにしてください。
