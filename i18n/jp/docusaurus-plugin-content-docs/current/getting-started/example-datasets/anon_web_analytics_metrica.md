...
### 圧縮された hits TSV ファイルをダウンロードする \{#download-the-hits-compressed-tsv-file\}
...
### visits テーブルのデータをインポートする \{#import-the-visits-data\}
...
`hits` と `visits` のデータセットは ClickHouse のテストルーチンで使用されており、これはそのテストスイートに含まれるクエリの 1 つです。残りのテストについては、このページの末尾にある *Next Steps* セクションを参照してください。
...
[A Practical Introduction to Sparse Primary Indexes in ClickHouse](/guides/best-practices/sparse-primary-indexes.md) では、`hits` データセットを用いて、従来のリレーショナルデータベースと比較した ClickHouse の索引の違い、ClickHouse がスパースなプライマリ索引をどのように構築・利用しているか、そして索引設計のベストプラクティスについて解説しています。
...
:::note
このテストスイートでは、データベース名として `test` が使用され、テーブル名は `hits` および `visits` になっています。必要に応じてデータベース名やテーブル名を変更するか、テストファイル中の SQL を編集してください。
:::