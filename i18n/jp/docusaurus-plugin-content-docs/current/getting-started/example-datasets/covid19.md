...
COVID-19 Open-Data は、最大規模の COVID-19 疫学データベースを構築するとともに、強力で網羅的な共変量セットを提供することを目指しています。人口統計、経済、疫学、地理、健康、入院、モビリティ、政府対応、天候などに関する、オープンかつ公開ソースでライセンスされたデータを含みます。
...
:::note
以下のコマンドは、[ClickHouse Cloud](https://clickhouse.cloud) の **Production** インスタンス上で実行されました。ローカル環境にインストールした ClickHouse でも簡単に実行できます。
:::
...
5. 処理はかなり速く終わります — 何行挿入されたか確認してみましょう:
...
6. Covid-19 の累計症例数がどれくらい記録されているか確認してみましょう。
...
7. データの日付には 0 が多く含まれていることに気付くでしょう。これは週末であったり、そもそも毎日数値が報告されていなかった日です。ウィンドウ関数を使って、新規症例数の1日あたりの平均を平滑化できます。
...
8. このクエリは、各ロケーションの最新の値を取得します。すべての国が毎日報告しているわけではないため `max(date)` は使えず、代わりに `ROW_NUMBER` を用いて最後の行を取得します:
...
9. `lagInFrame` を使用して、日ごとの新規症例数の `LAG` を算出できます。このクエリでは、`US_DC` のロケーションでフィルタしています。
...
10. このクエリは、日ごとの新規症例数の変化率を計算し、結果セットに単純な `increase` または `decrease` 列を追加します。
...
:::note
[GitHub リポジトリ](https://github.com/GoogleCloudPlatform/covid-19-open-data) に記載されているとおり、このデータセットは 2022 年 9 月 15 日をもって更新が終了しています。
:::
