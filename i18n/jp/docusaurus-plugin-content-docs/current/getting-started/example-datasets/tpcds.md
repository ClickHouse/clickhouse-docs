...
可能な限り TPC-DS 仕様のルールに忠実に従います。
仕様で定義されている抽象データ型は、ClickHouse のネイティブデータ型に次のようにマッピングされます。

| TPC-DS Type    | ClickHouse Type                       |
| -------------- | ------------------------------------- |
| `identifier`   | `Int64`（または日付/時間ディメンションキーには `UInt32`） |
| `integer`      | `Int32`                               |
...
NULL 許容性については、仕様で &quot;N&quot;（not null）とマークされているカラムには、null 値を含まないため `Nullable` ラッパーは付きません。
それ以外のカラムはすべて `Nullable()` でラップされます。
文字列型については、`Nullable` は `LowCardinality` の内側に配置されます（例：`LowCardinality(Nullable(String))`）。
...
:::note
SQL 標準に従った正しい結果を得るには、次の設定を有効にしておく必要があります。
...
::::note
このクエリは、https://github.com/ClickHouse/ClickHouse/issues/95299 の影響により、そのままでは動作しません。以下のようにわずかに修正した代替の記述であれば動作します：
...
::::note
`stddev_samp` が単一の値に対して呼び出された場合、クエリは `NULL` ではなく `nan` を返します。該当 issue: https://github.com/ClickHouse/ClickHouse/issues/94683。それ以外のケースでは、結果は正しいです。
::::
...
::::note
このクエリは、https://github.com/ClickHouse/ClickHouse/issues/95299 により、そのままでは動作しません。次のように軽微な修正を加えた代替クエリであれば動作します。
...
::::note
https://github.com/ClickHouse/ClickHouse/issues/94858 により、クエリはそのままでは動作しません。軽微な修正を加えた次のような代替の書き方であれば動作します。
...
::::note
このクエリは、https://github.com/ClickHouse/ClickHouse/issues/94858 により、そのままでは動作しません。軽微な修正を加えた次の代替の書き方であれば動作します。
...
::::note
このクエリは https://github.com/ClickHouse/ClickHouse/issues/94976 により、そのままでは動作しません。次のようにわずかに修正した代替の記述であれば動作します：
...
::::note
このクエリは、https://github.com/ClickHouse/ClickHouse/issues/94671 に起因する問題により、そのままでは動作しません。以下のように、わずかに修正した代替の書き方であれば動作します。
...
::::note
このクエリは、https://github.com/ClickHouse/ClickHouse/issues/95299 により、そのままでは動作しません。次のように、わずかな修正を加えた代替クエリであれば動作します：
