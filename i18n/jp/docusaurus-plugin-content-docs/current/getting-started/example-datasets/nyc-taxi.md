...
ピックアップおよびドロップオフ日時、緯度経度、運賃の詳細、ニューヨークの地区名などのカラムがあることに注目してください。

いくつかクエリを実行してみましょう。次のクエリは、ピックアップ頻度が最も高い上位 10 地区を表示します。
...
データセットの説明およびダウンロード手順については、[https://github.com/toddwschneider/nyc-taxi-data](https://github.com/toddwschneider/nyc-taxi-data) および [http://tech.marksblogg.com/billion-nyc-taxi-rides-redshift.html](http://tech.marksblogg.com/billion-nyc-taxi-rides-redshift.html) を参照してください。

ダウンロードすると、CSV ファイルの非圧縮データが約 227 GB になります。1 Gbit 回線では、ダウンロードに約 1 時間かかります（s3.amazonaws.com から並列ダウンロードすることで、1 Gbit 帯域の少なくとも半分は活用できます）。
一部のファイルは最後までダウンロードされない場合があります。ファイルサイズを確認し、疑わしいものは再ダウンロードしてください。
...
Intel(R) Xeon(R) CPU E5-2650 v2 @ 2.60GHz × 2（物理コア合計 16）、128 GiB RAM、8×6 TB HDD（ハードウェア RAID-5）

実行時間は 3 回実行したうちの最短値です。ただし 2 回目以降の実行では、クエリはファイルシステムキャッシュからデータを読み取ります。それ以上のキャッシュは行われず、各実行でデータは都度読み出されて処理されます。
