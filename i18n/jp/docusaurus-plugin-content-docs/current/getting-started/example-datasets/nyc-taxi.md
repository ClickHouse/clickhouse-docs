---
description: '2009年以降にニューヨーク市から出発した、タクシーおよび配車サービス（Uber、Lyft など）の数十億件の乗車データ'
sidebar_label: 'ニューヨークタクシーデータ'
slug: /getting-started/example-datasets/nyc-taxi
title: 'ニューヨークタクシーデータ'
doc_type: 'guide'
keywords: ['サンプルデータセット', 'nyc taxi', 'チュートリアル', 'サンプルデータ', '入門']
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

ニューヨークのタクシーデータサンプルは、2009 年以降にニューヨーク市発で行われた 30 億件以上のタクシーおよびハイヤー車両（Uber、Lyft など）の乗車データで構成されています。この入門ガイドでは、300 万行のサンプルを使用します。

完全なデータセットは次のいずれかの方法で取得できます。

* S3 または GCS から ClickHouse Cloud にデータを直接挿入する
* 準備済みパーティションをダウンロードする
...
:::note
以下の例のクエリは、ClickHouse Cloud の **Production** インスタンス上で実行されました。詳細については
...
ユーザーはデータに慣れるために、データの一部（300万行）を取得できます。データはオブジェクトストレージ内のTSVファイルとして格納されており、`s3`テーブル関数を使用してClickHouse Cloudに簡単にストリーミングできます。
...
以下のコマンドは、S3バケットから3つのファイルを`trips_small`テーブルにストリーミングします（`{0..2}`構文は0、1、2の値を表すワイルドカードです）：
...
以下のコマンドは、GCSバケットから3つのファイルを`trips`テーブルにストリーミングします（`{0..2}`構文は0、1、2の値を表すワイルドカードです）：
...
:::note
以下の手順では、元のデータセットに関する情報と、セルフマネージド ClickHouse サーバー環境に準備済みパーティションをロードする方法について説明します。
:::
...
ダウンロードすると、CSV ファイルで約 227 GB の非圧縮データが取得されます。1 Gbit 接続では、ダウンロードに約 1 時間かかります(s3.amazonaws.com からの並列ダウンロードにより、1 Gbit チャネルの少なくとも半分を利用できます)。
一部のファイルが完全にダウンロードされない場合があります。ファイルサイズを確認し、疑わしいファイルは再ダウンロードしてください。
...
:::info
以下で説明するクエリを実行する場合は、テーブルの完全修飾名 `datasets.trips_mergetree` を使用する必要があります。
:::
...
実行時間は3回の実行のうち最良のものです。ただし、2回目の実行以降、クエリはファイルシステムキャッシュからデータを読み取ります。それ以上のキャッシュは発生せず、各実行でデータが読み出され処理されます。
...
クエリが線形にスケールしているため、ここには特に意外な点はありません。
...
この場合、クエリ処理時間は主にネットワークレイテンシーによって決まります。
クラスターがあるデータセンターとは別のデータセンターにあるクライアントからクエリを実行したため、約 20 ms のレイテンシーが追加されました。
