...
<VerticalStepper>
  ## 推奨事項を確認する \{#review-recommendations\}
  ...
  * [packages.clickhouse.com](https://packages.clickhouse.com/deb/pool/main/c/) からパッケージを手動でダウンロードしてインストールすることもできます。
  ...
  <details>
    ...
    # ClickHouse サーバー および client パッケージをインストール
  ...
  ## ClickHouse サーバー と client をインストールする \{#install-clickhouse-server-and-client\}
  ...
  ClickHouse サーバー に ClickHouse Keeper が同梱されているため、ClickHouse Keeper を別途インストールする必要はありません。
  :::
