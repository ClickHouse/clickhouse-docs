---
description: 'ClickHouse をソースからコンパイルする、または CI によって生成されたバイナリをインストールするための手順'
...
パッケージをコンパイルしてインストールすることも、パッケージをインストールせずにプログラムを直接使用することもできます。
...
データディレクトリとメタデータディレクトリを手動で作成し、目的のユーザーに対して`chown`で所有権を設定する必要があります。これらのパスはサーバー設定ファイル(src/programs/server/config.xml)で変更できます。デフォルトでは次のパスが使用されます:
...
リポジトリ](https://github.com/clickhouse/clickhouse/)内の各コミットに対して特殊なビルドを生成します。例えば、[サニタイザ](https://github.com/google/sanitizers)ビルド、最適化なし(Debug)
ビルド、クロスコンパイルビルドなどです。これらのビルドは通常、開発時にのみ有用ですが、特定の状況においてはユーザーにとっても有益な場合があります。

:::note
ClickHouseのCIは時間とともに進化しているため、CI生成ビルドをダウンロードする正確な手順は変わる可能性があります。
また、CIは古いビルド成果物を削除する場合があり、ダウンロードできなくなることがあります。
:::
