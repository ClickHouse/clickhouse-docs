---
description: 'ClickHouse をソースからコンパイルする、または CI 生成バイナリをインストールするための手順'
...
データディレクトリとメタデータディレクトリを手動で作成し、目的のユーザーに対して`chown`で所有権を設定する必要があります。これらのパスはサーバー設定ファイル(src/programs/server/config.xml)で変更できますが、デフォルトでは以下のようになっています:
...
ビルド、クロスコンパイルビルドなどです。これらのビルドは通常開発時にのみ有用ですが、特定の状況ではユーザーにとっても有用な場合があります。
...
- 緑色のチェック/黄色のドット/赤色のクロスをクリックして、CIチェックのリストを開きます。
