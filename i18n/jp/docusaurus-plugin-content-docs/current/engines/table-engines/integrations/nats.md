...
* メッセージサブジェクトへのパブリッシュまたはサブスクライブ
* 新しいメッセージを利用可能になり次第処理
...
証明書が期限切れ、自署名、存在しない、またはその他の理由で無効な場合は、`CLICKHOUSE_NATS_TLS_SECURE=0` を設定して検証を無効化します。
...
より具体的には、NATS エンジン向けのパスワードを設定できます。
