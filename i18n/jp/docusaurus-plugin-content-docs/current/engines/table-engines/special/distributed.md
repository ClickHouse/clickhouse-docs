---
description: 'Distributed エンジンを持つテーブル自体はデータを一切保存しませんが、複数のサーバーに対する分散クエリ処理を可能にします。読み取り処理は自動的に並列化されます。読み取り時には、リモートサーバー上のテーブル索引が存在する場合、それらが使用されます。'
...
| パラメータ         | 説明                                                                                                                                                                         | デフォルト値     |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `host`        | リモートサーバーのアドレスです。ドメイン名、IPv4 アドレス、IPv6 アドレスのいずれかを使用できます。ドメインを指定した場合、サーバーは起動時に DNS リクエストを実行し、その結果はサーバーの稼働中保持されます。DNS リクエストが失敗すると、サーバーは起動しません。DNS レコードを変更した場合は、サーバーを再起動してください。 | -          |
| `port`        | メッセージ交換に使用する TCP ポートです (設定内の `tcp_port`。通常は 9000 に設定されます) 。`http_port` と混同しないでください。                                                                                        | -          |
| `user`        | リモートサーバーへの接続に使用するユーザー名です。このユーザーには、指定したサーバーへの接続権限が必要です。アクセスは `users.xml` ファイルで設定します。詳細は、[Access rights](../../../guides/sre/user-management/index.md) のセクションを参照してください。      | `default`  |
| `password`    | リモートサーバーへの接続に使用するパスワードです (マスクされません) 。                                                                                                                                      | &#39;&#39; |
| `secure`      | セキュアな SSL/TLS 接続を使用するかどうかを指定します。通常はポートの指定も必要です (セキュアなデフォルトポートは `9440` です) 。サーバーは `<tcp_port_secure>9440</tcp_port_secure>` で待ち受け、正しい証明書で設定されている必要があります。                    | `false`    |
| `compression` | データ圧縮を使用します。                                                                                                                                                               | `true`     |
| `bind_host`   | このノードからリモートサーバーに接続する際に使用する送信元アドレスです。サポートされるのは IPv4 アドレスのみです。ClickHouse の分散クエリで使用する送信元 IP アドレスを設定する必要がある高度なデプロイ構成を想定しています。                                                  | -          |
...
* 特定のキーでデータを結合する必要があるクエリ (`IN` または `JOIN`) を使用する場合です。データがこのキーでシャーディングされていれば、`GLOBAL IN` や `GLOBAL JOIN` の代わりにローカル `IN` または `JOIN` を使用でき、はるかに効率的です。
* 多数の小規模クエリとともに多数のサーバー (数百台以上) を使用する場合です。例えば、個々のクライアント (Web サイト、広告主、またはパートナーなど) のデータに対するクエリです。小規模クエリがクラスター全体に影響しないようにするには、1 つのクライアントのデータを 1 つのシャードに配置するのが合理的です。あるいは、2 段階のシャーディングを設定することもできます。クラスター全体を「レイヤー」に分割し、各レイヤーは複数のシャードで構成できます。1 つのクライアントのデータは 1 つのレイヤーに配置されますが、必要に応じてそのレイヤーにシャードを追加でき、その内部ではデータがランダムに分散されます。各レイヤーに対して `Distributed` テーブルを作成し、グローバルクエリ用に単一の共有分散テーブルを作成します。
