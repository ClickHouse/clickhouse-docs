...
Transparent Huge Pages は常に `madvise` に設定してください。古いカーネル (5.9 より前) では、THP を `always` に設定すると著しいパフォーマンス低下を招くことがあります。これは、特に 64 GB 以上の RAM を搭載したシステムで、カーネルがメモリのデフラグに過剰な時間を費やすためです。カーネル 5.9 では proactive compaction が導入され、THP をはるかに適切に処理できるようになりましたが、THP が `always` に設定されていると ClickHouse は起動時に引き続き警告を表示するため、カーネルのバージョンにかかわらず、推奨される設定は `madvise` です。
