...
`queries_per_normalized_hash` – 個々の正規化クエリについての実行回数の最大値です。正規化クエリとは、リテラルをプレースホルダーに置き換えたクエリのことで、`SELECT 1` と `SELECT 2` は同じ正規化クエリと見なされます。この制限は、正規化クエリパターンごとに個別に追跡されます。
...
DDL 構文を使用する場合は、正規化されたクエリハッシュをキーとしてクォータを設定することもできます。これにより、それぞれの異なるクエリパターンに独立したクォータ枠が割り当てられます。
