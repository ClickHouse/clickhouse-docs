...
この設定は、パーツが外部から書き込まれ、メタデータの検出をストレージからリフレッシュする必要がある、非 Replicated MergeTree テーブルを対象としています。

MergeTree の設定 `refresh_parts_interval` は、基盤となるストレージからデータパーツの一覧を定期的にリフレッシュするためのものです（例: 外部から書き込まれたパーツを拾うため）。ここで重要な違いは、**レプリカ間で共有されるメタデータ** と **レプリカごとにローカルなメタデータ**（例: レプリカごとにローカルメタデータを持つ S3）の区別です。メタデータが共有されている場合にのみ、新しいパーツがすべてのレプリカから見えるようになります。オブジェクトストレージを使用しているという事実だけでは、メタデータ共有を意味しません。

- **オブジェクトストレージ（例: `disk = 's3'`）はメタデータ共有を意味しません。** メタデータがレプリカごとにローカルに保存されている場合（デフォルト）、各レプリカはオブジェクトストレージ内の blob へのポインタを独立して管理します。一方のレプリカで行われた変更は、他のレプリカからは見えません。その場合、`refresh_parts_interval` によってレプリカ間で新しいパーツが見えるようになることはありません。各レプリカが読み込むメタデータはレプリカローカルだからです。

- **パーツの自動リフレッシュには、ファイルシステムメタデータが共有されていることが必要です**（またはテーブルがテーブル専有かつ readonly なメタデータを使用しており、リフレッシュが適用可能であること）。`table_disk = true` をテーブルローカルなディスクと併用すること（例: `SETTINGS disk = disk(type=object_storage, ...), table_disk = true`）は、正しいセマンティクスを得る 1 つの方法です。この場合、テーブルがメタデータのライフサイクルを所有し、ストレージは readonly とみなされるため、`refresh_parts_interval` が実行され、外部から追加されたパーツを検出できます。

- **グローバルに定義されたディスクを使用する場合**（例: `storage_configuration` で `disk = 's3'` を指定）かつデフォルトでローカルメタデータを使用している場合、各レプリカは独自のメタデータ状態を持ちます。blob が S3 上に存在していても、`refresh_parts_interval` の観点ではストレージは共有とはみなされず、ClickHouse 外部または別のレプリカで作成された新しいパーツは検出されません。

パーツの自動リフレッシュを行うには、メタデータが共有されていることを確認するか、上記のように `table_disk = true` を設定したテーブルレベルディスクを使用してください。レプリカローカルなメタデータのまま `refresh_parts_interval` のみに依存しても、期待どおりにはパーツはリフレッシュされません。

:::note
`refresh_parts_interval` は ReplicatedMergeTree テーブルでは使用されません。
レプリケーション対応テーブルは、すでにレプリケーションメカニズムによってパーツを同期しています。
この設定が適用されるのは、パーツが外部から書き込まれ、メタデータのリフレッシュが必要な非レプリケーション対応の MergeTree テーブルのみです。
:::
...
| `use_environment_credentials`                                                                               | 存在する場合、環境変数 `AWS_ACCESS_KEY_ID`、`AWS_SECRET_ACCESS_KEY`、`AWS_SESSION_TOKEN` から AWS クレデンシャルを読み取ります。注意: 環境クレデンシャルはすべての S3 ディスクで共有されます。ディスクごとに異なるクレデンシャルを使用するには、代わりに各ディスクで `access_key_id` と `secret_access_key` を明示的に指定してください。 | `false`                                  |
