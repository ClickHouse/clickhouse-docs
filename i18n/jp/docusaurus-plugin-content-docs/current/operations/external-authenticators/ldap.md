...
ログインのたびに、ClickHouse は [LDAP サーバーの定義](#ldap-server-definition) で `bind_dn` パラータにより定義された DN に対して、提供された認証情報を用いて「バインド」を試み、成功した場合、そのユーザーは認証済みと見なされます。これは一般的に「シンプルバインド」方式と呼ばれます。
...
SQL 駆動の[Access Control and Account Management](/operations/access-rights#access-control-usage)が有効な場合、LDAP サーバーで認証されるユーザーも [CREATE USER](/sql-reference/statements/create/user) ステートメントを使用して作成できます。
