...
* `follow_referrals` — サーバーから返された LDAP リファラルを LDAP クライアントライブラリが自動的にたどることを許可するためのフラグ。デフォルトは `false`。注意: この設定は、主に Microsoft Active Directory 環境で、高位の base DN（例: `DC=example,DC=com`）に対する subtree 検索がリファラル／検索参照（例: `DC=DomainDnsZones,...`）を返す場合に重要となります。
  * クロスパーティション検索が明示的に必要であり、かつ環境がそれを許可するよう構成されている場合にのみ `true` を指定します。
  * リファラルをたどらない場合は `false` を指定します。これは AD ドメインルート検索に推奨されます。
