...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "minmax, uniq"},{"label": "自動統計をデフォルトで有効化"}]}, {"id": "row-2","items": [{"label": "25.10"},{"label": ""},{"label": "新しい設定"}]}]} />

適切なすべてのカラムに対して自動的に計算する統計タイプを指定する、カンマ区切りのリストです。
サポートされている統計タイプ: basic, tdigest, countmin, minmax, uniq。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "16"},{"label": "新しい設定。削除対象のいずれかのパーツがリモートディスク上にある場合に、同時実行のパーツ削除パスに入るしきい値を引き下げます。この場合、各削除では通常 1 回のネットワーク往復が発生します。以前の値 (100) は従来の `concurrent_part_removal_threshold` のデフォルト値に一致するため、古い `compatibility` モードでは以前の動作が維持されます。"}]}]} />

`concurrent_part_removal_threshold` と同じですが、削除対象の
パーツのうち少なくとも 1 つがリモートディスクに保存されている場合に使用されます。デフォルト値が低く
設定されているのは、リモートストレージ上での各パーツの削除では通常ネットワークの
往復が必要になるためです (たとえばオブジェクトストレージでは、パーツごとに 1 回の HTTP `DELETE` が発生します) 。
そのため、100 個のパーツであっても直列に削除すると、`DROP TABLE` が
数十秒にわたって停止する可能性があります。
...
マージ処理時に仮想カラム `_block_offset` を永続化します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "デフォルトで、min_age_to_force_merge_seconds が有効な場合でもパーツサイズを制限します"}]}, {"id": "row-2","items": [{"label": "25.1"},{"label": "0"},{"label": "min_age_to_force_merge の最大バイト数を制限する新しい設定を追加"}]}, {"id": "row-3","items": [{"label": "25.1"},{"label": "0"},{"label": "新しい設定"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "`with_buckets` マップのシリアライゼーションでバケット数を計算する際に、`sqrt` および `linear` 戦略で使用する係数を制御する設定を追加"}]}]} />

平均マップサイズからバケット数を計算する際に、`sqrt` および `linear` [map&#95;buckets&#95;strategy](#map_buckets_strategy) で使用される係数です。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "32"},{"label": "`with_buckets` シリアライゼーションの適用に必要な最小平均マップサイズ（1行あたりのキー数）を制御する設定を追加"}]}]} />

`with_buckets` シリアライゼーションを適用するために必要な最小平均マップサイズ (1行あたりのキー数) です。
平均マップサイズがこの値未満の場合、他のバケット設定にかかわらず、単一のバケットが使われます。
値を `0` にすると、このしきい値は無効になり、常にバケット化戦略が適用されます。
この設定は、小さなマップでは効果がほとんどないバケット化シリアライゼーションのオーバーヘッドを避けるのに役立ちます。

## map_buckets_strategy \{#map_buckets_strategy\}

<SettingsInfoBlock type="MergeTreeMapBucketsStrategy" default_value="sqrt" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "sqrt"},{"label": "'with_buckets' マップのシリアライゼーションでバケット数の選択方法を制御する設定を追加"}]}]} />

平均マップサイズに基づいて、`with_buckets` `Map` のシリアライゼーションにおけるバケット数の選択方法を制御します。

指定可能な値:

* constant — 平均マップサイズに関係なく、常に [max&#95;buckets&#95;in&#95;map](#max_buckets_in_map) をバケット数として使います。
* sqrt — `round(map_buckets_coefficient * sqrt(avg_map_size))` をバケット数として使い、値は `[1, max_buckets_in_map]` の範囲に収められます。
* linear — `round(map_buckets_coefficient * avg_map_size)` をバケット数として使い、値は `[1, max_buckets_in_map]` の範囲に収められます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "basic"},{"label": "Map シリアライゼーションのバージョンを制御する設定を追加"}]}]} />

`Map` カラムで使用するシリアライゼーション方式を制御します。

指定可能な値:

* basic — `Map` の標準的なシリアライゼーションを使います。
* with&#95;buckets — シリアライゼーション時にキーをバケットに分割します。バケットを使うと、Map から個々のキーを読み込む効率が向上します。

`with_buckets` シリアライゼーションのバケット数は、[max&#95;buckets&#95;in&#95;map](#max_buckets_in_map) と [map&#95;buckets&#95;strategy](#map_buckets_strategy) によって決まります。

## map_serialization_version_for_zero_level_parts \{#map_serialization_version_for_zero_level_parts\}

<SettingsInfoBlock type="MergeTreeMapSerializationVersion" default_value="basic" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "basic"},{"label": "レベル 0 パーツの Map シリアライゼーションバージョンを制御する設定を追加"}]}]} />

この設定では、挿入時に作成されるレベル 0 パーツに対して、
`Map` カラムの異なるシリアライゼーションバージョンを指定できます。
挿入時のパフォーマンス低下を避けるため、レベル 0 パーツでは `basic`
シリアライゼーションを維持しつつ、マージ済みパーツでは `with_buckets` を使用したい場合に有用です。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "32"},{"label": "'with_buckets' マップシリアル化の最大バケット数を制御する設定を追加"}]}]} />

`Map` シリアル化における最大バケット数です。`with_buckets` `Map` シリアル化で使用されます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "デフォルトで、データ型のシリアライゼーションバージョンをネストされた型にも伝播する"}]}]} />

true の場合、`string_serialization_version` のようなシリアライゼーションバージョンは、Array/Map/Nullable/JSON などのネストされた型の内部にも伝播されます。無効にすると、シリアライゼーションバージョンが影響するのは、この型のトップレベルのカラムと Tuple の el のみです。
...
他のレプリカからフェッチするパートの最小レベルです。このしきい値を下回るレベルのパーツは後回しにされます
(レプリケーションキューに保持され、恒久的にスキップされるのではなく、各スケジューリングサイクルで再評価されます) 。
1 を使うと、レベル 0 の (未マージの) パーツのフェッチを後回しにでき、インジェストが多いときのレプリケーションのオーバーヘッドを削減できます。
デフォルト: 0 (レベルに関係なく、すべてのパーツをフェッチします) 。
...
replicated&#95;fetches&#95;min&#95;part&#95;level 未満のレベルのパートでも、この時間 (秒) が経過するとフェッチされます。
タイムアウトを無効にするには 0 を使います (最小レベル未満のパーツは、マージされるまで無期限に延期されます) 。
デフォルト: 300 (5 分後に強制的にフェッチ) 。
