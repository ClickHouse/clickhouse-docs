...
`b.id` のような複合識別子が、エイリアス `b` を持つテーブルのカラム `id` としても、別のカラムの Tuple サブカラム `b.id` としても解釈できる場合は、エイリアスのプレフィックスとしての解釈 (`b` のカラム `id`) を優先します。デフォルトでは、新しいアナライザはサブカラムを優先します。以前のアナライザの名前解決に合わせるには、これを有効にしてください。
...
単一の `UNIQUE KEY` 行の順序を保持するバイナリエンコーディングの最大サイズ (バイト単位) 。
