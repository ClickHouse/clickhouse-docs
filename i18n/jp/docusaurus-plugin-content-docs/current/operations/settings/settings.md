...
増分読み取り1回あたりに消費する Paimon スナップショットの最大数。0 は無制限を意味します。
...
Paimon の増分モードにおける、クエリレベルの対象スナップショット読み取りです。&gt;0 の場合、リーダーはコミット済みウォーターマークを進めずに、指定された snapshot&#95;id の差分のみを取得します。
デフォルト: -1 (無効)
