...
`aiEmbed` が送信する 1 回の HTTP リクエストに含めるテキストの最大数です。API 呼び出しのオーバーヘッドを減らすため、テキストはこのサイズごとのバッチにまとめられます。たとえば、一意のテキストが 500 個あり、バッチサイズが 100 の場合、HTTP リクエストは 5 回になります。
...
AI 関数がクエリごとに送信できる HTTP リクエストの最大数です。無効にするには 0 に設定します。
...
1 つのクエリ内における、すべての AI 関数 API 呼び出しの入力 (提示) トークンの合計最大数です。これはプロバイダーの応答に基づいて累積的に追跡されます。各呼び出しの入力トークン数は事前には分からないため、この上限は 1 回の呼び出し分の入力トークン数だけ超過する可能性があることに注意してください。無効にするには 0 に設定します。

この上限は、応答内で `usage` オブジェクトを返すプロバイダー (OpenAI、Anthropic、vLLM) に対してのみ適用されます。トークン使用量を返さないプロバイダー (特に HuggingFace TEI) ではカウンターが 0 のままになるため、そのような呼び出しの上限を設定するには代わりに `ai_function_max_api_calls_per_query` を使用してください。
...
単一のクエリ内で、すべての AI 関数 API 呼び出しにおける出力 (completion) トークンの合計数の最大値です。プロバイダーからのレスポンスに基づいて累積で追跡されます。呼び出しの出力トークン数は事前にわからないため、この制限は 1 回の呼び出し分の出力トークン数だけ超過する可能性がある点に注意してください。無効にするには 0 に設定します。

この制限は、レスポンス内で `usage` オブジェクトを返すプロバイダー (OpenAI、Anthropic、vLLM) に対してのみ適用されます。出力トークンを生成しない埋め込み関数 (特に aiEmbed) には適用されません。
...
個々の API リクエストごとに、一時的なエラーに対する再試行の最大回数。各再試行では、`ai_function_retry_initial_delay_ms` を初期遅延とする指数バックオフを使用します。
...
AI 関数 (AI チャット補完および埋め込み API 呼び出し) が行う個々の HTTP リクエストのタイムアウト (秒) 。リクエストがこの時間内に完了しない場合は失敗と見なされ、`ai_function_max_retries` に従って再試行されることがあります。
...
失敗した AI 関数 API リクエストの最初の再試行までの初期遅延時間 (ミリ秒) です。遅延は以降の再試行ごとに 2 倍になります (指数バックオフ) 。たとえば、デフォルト設定では 1000ms、2000ms、4000ms となります。
...
true (デフォルト) の場合、再試行をすべて使い切っても AI 関数呼び出しが回復不能な失敗に終わると、クエリは例外で中止されます。false の場合、失敗した行にはカラム型のデフォルト値 (String の場合は空文字列) が設定され、処理は継続されます。
...
true の場合 (デフォルト) 、AI 関数のクォータ制限 (`ai_function_max_input_tokens_per_query`、`ai_function_max_output_tokens_per_query`、または `ai_function_max_api_calls_per_query`) を超えると、例外を送出してクエリを中止します。false の場合、残りの行にはカラム型のデフォルト値が設定されます (String の場合は空文字列) 。
...
実験的な AI 関数 (例: `aiGenerateContent`) を有効にします。これらの関数は AI プロバイダーに外部 HTTP リクエストを送信します。
...
Iceberg compaction の実行時に古いデータファイルをクリーンアップできるようにします。
...
実験的な Iceberg コマンド `ALTER TABLE ... EXECUTE expire_snapshots` の実行を許可します。
...
Paimon* テーブルエンジンを使用するテーブルの作成を許可します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "polyglot SQL transpiler ダイアレクトを有効にするための New setting。"}]}]}/>

polyglot SQL transpiler を有効にします。30 を超えるダイアレクト (MySQL、PostgreSQL、SQLite、Snowflake、DuckDB など) の SQL を ClickHouse SQL に変換します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "実験的な lazy posting list apply モードを制御する新しい設定"}]}]} />

true に設定すると、テキスト索引クエリで lazy posting list apply モードを使用できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "索引解析の前に、`coalesce(a_1, ..., a_N) <op> const` 形式の述語（同等の `ifNull` や、定数が左側にある場合も含む）を論理和に書き換え、各 `a_i` に対するカラムごとの主キーおよびスキップ索引を使用できるようにする新しい設定です。`coalesce(a, 42, b)` や `coalesce(a, b, 42)` のような、一部が定数の形式も処理されます。"}]}]} />

索引解析の前に、`coalesce(a_1, ..., a_N) <op> const` 形式の述語 (同等の `ifNull` や、定数が左側にある場合も含む) を論理和 `(a_1 <op> const) OR (a_1 IS NULL AND a_2 <op> const) OR ... OR (a_1 IS NULL AND ... AND a_{N-1} IS NULL AND a_N <op> const)` に書き換えることで、各 `a_i` に対するカラムごとの主キーおよびスキップ索引を使用できるようにします。`coalesce(a, 42, b)` や `coalesce(a, b, 42)` のような、一部が定数の形式も処理されます。引数リストは `coalesce` 自体と同様に正規化されます (`NULL` リテラルは削除され、最初の非 `Nullable` の引数より後ろの引数は削除されます) 。また、末尾に非 `NULL` の定数がある場合は、最後の分岐として出力されます。この書き換えは、索引の絞り込みに対して追加的にのみ適用されます。ランタイムフィルターでは引き続き元の述語が使用されます。
...
パーティション式が `LIMIT BY` カラムの決定論的関数である場合、パーティションごとに独立した `LIMIT BY` の評価を別々のスレッドで有効にします。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "新しい設定。26.5 より前は、`RANK` および `DENSE_RANK` ウィンドウ関数に渡された引数は暗黙的に無視されていました（`allow_rank_dense_rank_arguments = 1` と同等）。26.5 以降は、SQL 標準ではこれらの関数は引数を 0 個しか取らないため、デフォルトで引数を拒否し、`NUMBER_OF_ARGUMENTS_DOESNT_MATCH` を返します。従来の動作を復元するには、これを `1` に設定してください。"}]}]} />

後方互換性のために、`RANK` および `DENSE_RANK` ウィンドウ関数に引数を渡せるようにします。

SQL 標準では、`RANK` と `DENSE_RANK` は引数を 0 個取ります。つまり、これらの関数は
`OVER (ORDER BY ...)` ウィンドウのみに基づいて行を順位付けします。26.5 より前の ClickHouse バージョンでは、
`RANK(x) OVER (...)` のようなクエリでも引数が暗黙的に受け入れられ、そのまま無視されていました。これはユーザーの混乱を招く原因となっていました
 (見えている引数が順位付けに影響しているように見えても、実際には影響していなかったためです) 。

この設定が `false` の場合 (デフォルト) 、`RANK` と `DENSE_RANK` はあらゆる引数を拒否し、
`NUMBER_OF_ARGUMENTS_DOESNT_MATCH` をスローします。`true` に設定すると、従来の緩やかな動作が
復元され、26.5 より前と同様に引数は暗黙的に無視されます。
...
パーティションおよびパーツに対する操作のうち、正常に適用されたものについて、該当するパーツ情報の表示を有効または無効にします。
...
[`ALTER`](../../sql-reference/statements/alter/index.md)、[`OPTIMIZE`](../../sql-reference/statements/optimize.md)、[`TRUNCATE`](../../sql-reference/statements/truncate.md) クエリによってレプリカ上で実行されるアクションに対する待機動作を指定できます。
...
:::note
`alter_sync` は `Replicated` および `SharedMergeTree` テーブルにのみ適用され、`Replicated` または `Shared` ではないテーブルの alter には何の影響もありません。
:::
...
有効にすると、analyzer は通常の (マテリアライズされておらず、パラメーター化もされていない) ビューをその定義サブクエリに置き換え、述語プッシュダウンやカラム剪枝など、境界をまたぐ最適化を可能にします。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.10"},{"label": "1"},{"label": "この設定は、#87303 後のリグレッションに対する修正として追加されました。このリグレッションでは、ROW POLICY と PREWHERE が FINAL の前に適用されていました。PR #91065 でこの設定が導入され、PR #97279 で正しさを保証する true がデフォルトになりました。互換性のために修正前の false の動作へ戻らないよう、{true, true} として記録されています。"}]}]} />
...
統計情報に基づいて、並列レプリカでの実行への自動切り替えを有効にします。`enable_analyzer = 1`、`enable_parallel_replicas != 0`、`parallel_replicas_local_plan = 1` を有効にし、`cluster_for_parallel_replicas` を指定する必要があります。
0 - 無効、1 - 有効、2 - 統計情報の収集のみが有効 (並列レプリカでの実行への切り替えは無効) 。
...
クライアントがレスポンスを待たずに接続を閉じたときに、HTTP 読み取り専用クエリ（例: SELECT）をキャンセルします。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "best_effort"},{"label": "使いやすさの向上"}]}, {"id": "row-2","items": [{"label": "25.6"},{"label": "basic"},{"label": "Allow to use different DateTime parsing mode in String to DateTime cast"}]}]} />

String から DateTime へのCAST時に、日付と時刻のテキスト表現を解析するパーサーを選択できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.10"},{"label": "1"},{"label": "ClickHouse Cloud の設定"}]}]}/>

Cloud で許可されるデータベースエンジン。1 - DDL を Replicated データベースを使用するように書き換えます。2 - DDL を Shared データベースを使用するように書き換えます
...
- 3 - 明示的にリモートディスクが指定されている場合を除き、DDL を SharedMergeTree を使用するように書き換える
- 4 - 3 と同じですが、さらに Distributed の代わりに Alias を使用します（Alias テーブルは Distributed テーブルの宛先テーブルを指すため、対応するローカルテーブルを使用します）

公開部分を最小化するための UInt64
...
Replicated データベースでは、Replicated テーブルのみ作成できるようにします
...
ClickHouse Cloud でのみ有効です。分散キャッシュの読み取りリクエストが失敗した場合に再試行する回数です。
...
ClickHouse Cloud でのみ有効です。分散キャッシュからあらゆる種類の応答を受信するまでの待機時間をミリ秒単位で指定します。
...
ClickHouse Cloud でのみ有効です。分散キャッシュへの書き込みリクエストが失敗した場合に再試行する回数です。
...
Cloud におけるデフォルト値: `none_only_active`。
...
デフォルトでは、`Distributed` テーブルにデータを挿入する際、ClickHouse サーバーはクラスターのノードにデータをバックグラウンドモードで送信します。`distributed_foreground_insert=1` の場合、データは同期的に処理され、すべてのシャードにデータが保存された後にのみ `INSERT` 操作が成功します（`internal_replication` が true の場合は、各シャードにつき少なくとも 1 つのレプリカに保存された後）。
...
有効にすると、分散索引分析はコーディネーターでのみ実行されます。
これにより、述語にサブクエリ (例: `IN (SELECT ...)`) が含まれる場合に発生する O(N^2) のクエリ生成を防げます。
そうしないと、各フォロワーのレプリカがそれぞれ独立して独自の分散索引分析をトリガーするためです。
ただし、サブクエリで大規模なテーブルが使われる場合は、分散索引分析の効率が低下します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "動的ディスク設定（`disk()` 関数）で `from_env` 置換を許可する新しい設定です。セキュリティ上の理由により、デフォルトでは無効です。"}]}]} />

動的ディスク設定 (つまり `disk()` 関数の引数) で `from_env` 置換を使用できるようにします。
テーブルストレージの定義時にユーザーが任意の環境変数を読み取るのを防ぐため、デフォルトでは無効になっています。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "動的ディスク設定（`disk()` 関数）で `include` を許可するための新しい設定です。デフォルトでは無効です。"}]}]} />

動的ディスク設定 (具体的には `disk()` 関数の引数) で `include` を使用できるようにします。
デフォルトでは無効です。
...
デフォルト実装を使用して [Dynamic](../../sql-reference/data-types/dynamic.md) カラムに関数を適用する場合、
実際の型がその関数と互換性のない行での動作を制御します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "BLOB ストレージ の読み取り操作を system.blob_storage_log に記録するための新しい設定"}]}]} />

BLOB ストレージ の読み取り操作に関する情報を system.blob&#95;storage&#95;log テーブルに書き込みます。
`enable_blob_storage_log` も有効にする必要があります。
...
`write-through` キャッシュを有効または無効にします。`false` に設定すると、書き込み操作に対する `write-through` キャッシュは無効になります。`true` に設定すると、サーバー設定のキャッシュディスク構成セクションで `cache_on_write_operations` が有効になっている場合に限り、`write-through` キャッシュが有効になります。
...
マテリアライズド共通テーブル式を有効にします。enable&#95;global&#95;with&#95;statement より優先されます
...
非相関の EXISTS サブクエリをスカラーサブクエリとして実行します。スカラーサブクエリと同様に、キャッシュが使用され、結果には定数畳み込みが適用されます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "ピークメモリ使用量を削減するために、INSERT 中にプロジェクションパーツを同期的に確定するための新しい設定。"}]}]} />

有効にすると、プロジェクションパーツは INSERT 中に同期的に確定され、S3 アップロードの並列性は低下するものの、ピークメモリ使用量を削減できます。デフォルトでは、各プロジェクションの出力ストリームは、パート全体 (すべてのプロジェクションを含む) が確定されるまで維持されます。これにより S3 アップロードを重ねて実行できますが、ピークメモリはプロジェクション数に比例して増加します。この設定が影響するのは INSERT パスのみです。merge と mutation では、すでにプロジェクションは同期的に確定されます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "30"},{"label": "HTTP リクエストヘッダー全体の読み取りにかかる合計時間を制限するための新しい設定です。slowloris 攻撃から保護します。"}]}]} />

すべての HTTP リクエストヘッダーを読み取るための最大時間 (秒) です。これはヘッダー解析フェーズ全体に対する総制限時間であり、読み取りごとのタイムアウトではありません。クライアントがヘッダーデータを少しずつゆっくり送信して接続を開いたままにしようとする slowloris 型の攻撃を防ぎます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "10485760"},{"label": "authentication 前の HTTP リクエストヘッダー全体のサイズを制限する新しい設定。"}]}]} />

すべての HTTP リクエストヘッダーの合計サイズの上限を、バイト単位で指定します (名前と値の合計) 。
...
Iceberg のデータファイルのコンパクションにおけるしきい値。
...
Iceberg における コンパクション データファイルのしきい値。
...
`history.expire.max-ref-age-ms` プロパティが存在しない場合に、`expire_snapshots` で使用される Iceberg テーブルプロパティのデフォルト値です。
...
そのプロパティが存在しない場合に `expire_snapshots` で使用される、Iceberg テーブルプロパティ `history.expire.max-snapshot-age-ms` のデフォルト値。
...
Iceberg テーブルプロパティ `history.expire.min-snapshots-to-keep` が存在しない場合に、`expire_snapshots` で使用されるデフォルト値。
...
Iceberg におけるコンパクション対象のデータファイル数のしきい値。
...
JOIN が次のいずれかの制限に達した場合に、ClickHouse が実行する動作を定義します。

* [max&#95;bytes&#95;in&#95;join](/operations/settings/settings#max_bytes_in_join)
* [max&#95;rows&#95;in&#95;join](/operations/settings/settings#max_rows_in_join)

この設定が適用されるのは、[`join_algorithm`](/operations/settings/settings#join_algorithm) の値が
`hash` または `parallel_hash` の場合のみです。その他の
アルゴリズム (たとえば `partial_merge`、`grace_hash`、`auto`) では、これらの
制限は別の方法で処理されます。たとえば、ディスクへのスピル、再パーティション化、または
戦略の切り替えです。詳細は
[`join_algorithm`](/operations/settings/settings#join_algorithm) を参照してください。

設定可能な値:

* `THROW` — ClickHouse は例外を発生させてクエリを停止します。
* `BREAK` — ClickHouse はクエリを停止し、例外は発生させません。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "デフォルトでは INSERT 時の統計情報の構築を無効にし、代わりにマージに依存します"}]}, {"id": "row-2","items": [{"label": "24.6"},{"label": "1"},{"label": "INSERT 時の統計情報のマテリアライズを無効にできる新しい設定を追加しました"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "256000000"},{"label": "遅延評価 FINAL 最適化で使用するSetの最大バイト数に関する新しい設定"}]}]} />

遅延評価 FINAL 最適化で使用するSetの最大バイト数です。これを超えると、通常の FINAL にフォールバックします。
...
テーブルの結合時に使用される右側のデータ構造 (通常はハッシュテーブル) の最大サイズ (バイト単位) 。

この設定は、[SELECT ... JOIN](/sql-reference/statements/select/join)
操作および [Join table engine](/engines/table-engines/special/join) に適用されます。

クエリに複数の JOIN が含まれている場合、ClickHouse はすべての
中間結果に対してこの設定を確認します。制限に到達した場合の動作は、
選択した [`join_algorithm`](/operations/settings/settings#join_algorithm) に依存します。アルゴリズムごとの動作 (spill、再パーティション、
切り替え、または [`join_overflow_mode`](/operations/settings/settings#join_overflow_mode) に応じた throw/break) については、その設定を参照してください。
...
この設定の値が、現在同時に処理されているクエリ数以下である場合、例外をスローします。

例: `max_concurrent_queries_for_all_users` はすべてのユーザーに対して 99 に設定でき、データベース管理者はサーバーが過負荷のときでも調査のためのクエリを実行できるよう、自身に対しては 100 に設定できます。

1 つのクエリまたはユーザーについてこの設定を変更しても、他のクエリには影響しません。
...
増分読み取りごとに消費する Paimon スナップショットの最大数。0 は無制限を意味します。
...
テーブルへ挿入する際に形成されるブロックの最大サイズ (行数) 。

この設定は、次の 2 つのコンテキストでブロック形成を制御します。

1. フォーマットのパース: サーバーが任意のインターフェイス (HTTP、インラインデータ付きの clickhouse-client、gRPC、PostgreSQL ワイヤプロトコル) から行ベースの入力フォーマット (CSV、TSV、JSONEachRow など) をパースする際、次の場合にブロックが出力されます:

   * `min_insert_block_size_rows` と `min_insert_block_size_bytes` の両方に到達した場合、または
   * `max_insert_block_size_rows` または `max_insert_block_size_bytes` のいずれかに到達した場合

   注意: clickhouse-client または clickhouse-local でファイルから読み取る場合、データのパースはクライアント側で行われ、この設定はクライアント側に適用されます。

2. INSERT 操作: INSERT クエリの実行中およびデータが materialized view を経由して流れる際、この設定の動作は `use_strict_insert_block_limits` に依存します:

   * 有効な場合: 次の場合にブロックが出力されます:
     * 最小しきい値 (AND) : `min_insert_block_size_rows` と `min_insert_block_size_bytes` の両方に到達した場合
     * 最大しきい値 (OR) : `max_insert_block_size_rows` または `max_insert_block_size_bytes` のいずれかに到達した場合

   * 無効な場合: `min_insert_block_size_rows` または `min_insert_block_size_bytes` のいずれかに到達した時点でブロックが出力されます。`max_insert_block_size` の設定は適用されません。
...
最終パートのフラッシュを遅延させるストリーム (カラム) の最大数です。デフォルトは自動です（基盤となるストレージが並列書き込みをサポートしている場合は 100。たとえば S3。サポートしていない場合は無効）。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "4294967296"},{"label": "利用可能な空きメモリ量に応じて insert スレッド数を制限するための新しい設定"}]}]} />

`max_threads_min_free_memory_per_thread` と同じですが、`max_threads` ではなく `max_insert_threads` に適用されます。挿入パイプラインは通常、読み取りパイプラインよりもスレッドごとに大きなバッファ (MergeTree パーツ、圧縮ブロック) を保持するため、デフォルト値はより大きくなっています。

空きメモリ量が `max_insert_threads` にこの値を掛けた値を下回る場合、`max_insert_threads` は収まる値まで減らされ、最小値は `1` です。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "ランダム分布関数の形状パラメータを制限するための新しい設定です。極端な入力によるハングを防ぎます。"}]}]} />

`randChiSquared`、`randStudentT`、`randFisherF` などのランダム分布関数における形状パラメータの最大値です。極端なパラメータ値による計算時間の著しい長期化を防ぎます。

## max_rand_distribution_trials \{#max_rand_distribution_trials\}

<SettingsInfoBlock type="UInt64" default_value="1000000000" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000000"},{"label": "極端な入力でハングするのを防ぐため、ランダム分布関数の試行回数を制限する新しい設定。"}]}]} />

`randBinomial` や `randNegativeBinomial` などのランダム分布関数で許可される試行回数の最大値です。これにより、試行回数が多い場合に計算時間が極端に長くなるのを防ぎます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "10000000"},{"label": "lazy FINAL 最適化でSet内に含められる最大行数の新しい設定"}]}]} />

lazy FINAL 最適化でSet内に含められる最大行数。この値を超えると、通常の FINAL にフォールバックします。
...
テーブルを結合する際に使用される右側のデータ構造 (通常はハッシュ
テーブル) 内の行数を制限します。

この設定は [SELECT ... JOIN](/sql-reference/statements/select/join)
操作および [Join](/engines/table-engines/special/join) テーブルエンジンに適用されます。

クエリに複数の JOIN が含まれる場合、ClickHouse はすべての
中間結果に対してこの設定をチェックします。制限に達した場合の動作は、
選択した [`join_algorithm`](/operations/settings/settings#join_algorithm) に依存します。アルゴリズムごとの挙動 (spill、再パーティション、
切り替え、または [`join_overflow_mode`](/operations/settings/settings#join_overflow_mode)
に従った throw/break) については、その設定を参照してください。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "skip_unavailable_shards が有効な場合に、黙ってスキップできるシャード数を制限するための新しい設定。"}]}]}/>

`skip_unavailable_shards` が有効な場合、黙ってスキップできるシャードの最大数を制限します。
利用できないシャードの数がこの値を超える場合、黙ってスキップする代わりに例外がスローされます。

値が 0 の場合は制限なしを意味します（デフォルトの動作 — 利用できないすべてのシャードをスキップできます）。

## max_skip_unavailable_shards_ratio \{#max_skip_unavailable_shards_ratio\}

<SettingsInfoBlock type="Float" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "skip_unavailable_shards が有効な場合に、黙ってスキップできるシャードの比率を制限するための新しい設定。"}]}]}/>

`skip_unavailable_shards` が有効な場合、黙ってスキップできるシャードの最大比率（0〜1）を制限します。
利用できないシャードの総シャード数に対する比率がこの値を超える場合、黙ってスキップする代わりに例外がスローされます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "ピークメモリ使用量を削減するため、`UNION` ステップで同時にアクティブなデータストリーム数を制限する新しい設定。"}]}]} />

`UNION` ステップで同時にアクティブなデータストリーム数を制限します (`UNION DISTINCT` は `UNION ALL` ステップの後に `DISTINCT` ステップを実行することで実装されているため、`UNION ALL` と `UNION DISTINCT` の両方に適用されます) 。`UNION` クエリに多数のサブクエリがある場合、各サブクエリが同時に読み取りバッファを開くため、メモリ使用量はサブクエリ数に比例して増加します。この設定では `Concat` プロセッサを挿入してパイプラインを絞り込み、同時にアクティブになるストリーム数がこの値を超えないようにすることで、ピークメモリを大幅に削減します。実際の上限は、この値と `max_threads * max_streams_for_union_step_to_max_threads_ratio` の小さい方です (どちらか一方が 0 の場合、その値は無視されます) 。両方が 0 の場合、絞り込みは適用されません。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "8"},{"label": "新しい設定: UNION ステップで同時にアクティブなストリーム数の上限は、min(max_streams_for_union_step, max_threads * max_streams_for_union_step_to_max_threads_ratio) として計算されます。いずれかが 0 の場合、その項目は無効になります。"}]}]} />

この比率に `max_threads` を掛けた値によって、`UNION` ステップで同時にアクティブなストリーム数の上限が決まります (`UNION ALL` と `UNION DISTINCT` の両方に適用されます) 。実際の上限は、この計算値と `max_streams_for_union_step` のうち小さい方です (どちらか一方が 0 の場合は無視されます) 。たとえば、`max_threads = 8` でこの比率を 1 に設定すると、同時にアクティブになるストリームは最大 8 個です。この比率ベースの上限を無効にするには、0 に設定します。
...
サーバーがメモリ逼迫状態にある場合に `max_threads` を引き下げ、メモリ制限に達する可能性が高い高並列クエリの開始を防ぎます。

空きメモリは、サーバーの `max_server_memory_usage` から、グローバル メモリトラッカーで現在追跡されているメモリ使用量を差し引いて計算されます。この空きメモリが `max_threads` にこの値を掛けた値より小さい場合、`N * value <= free_memory` を満たす最大の N となるように `max_threads` が引き下げられます。最小値は `1` です。

この制限を無効にするには、`0` に設定します。

たとえば、デフォルト値が 1 GiB で空きメモリが 32 GiB の場合、`max_threads` の上限は 32 になります。空きメモリが 1 GiB の場合は 1 まで下がります。

この設定は、読み取り側の並列処理 (`SELECT`、`UNION`、`INTERSECT`/`EXCEPT`、および `INSERT ... SELECT` の `SELECT` 側) に適用されます。書き込み側については、`max_insert_threads_min_free_memory_per_thread` を参照してください。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "WKB ジオメトリの解析時の要素数を制限し、不正なデータによる過剰なメモリ割り当てを防ぐための新しい設定。"}]}]} />

`readWKB` および関連関数による解析時に、単一の WKB ジオメトリ要素内で許可されるポイント、リング、またはポリゴンの最大数です。これにより、不正な WKB データに起因する過剰なメモリ割り当てを防ぎます。ハードコードされた上限 (1 億) を使用するには、0 に設定してください。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0.5"},{"label": "lazy FINAL 最適化で処理を続行するために索引解析でフィルタリングされるマークの最小比率に関する新しい設定"}]}]} />

lazy FINAL 最適化において、索引解析でフィルタリングされるマークの最小比率です。この割合未満しかマークがフィルタリングされない場合は、通常の FINAL にフォールバックします。値 `0` はこのチェックを無効にします。
...
1. フォーマットのパース: サーバーが任意のインターフェイス (HTTP、インラインデータ付きの clickhouse-client、gRPC、PostgreSQL ワイヤプロトコル) から行ベースの入力フォーマット (CSV、TSV、JSONEachRow など) をパースする際、ブロックは次の場合に出力されます:
...
2. INSERT 操作: INSERT クエリの実行中およびデータが materialized view を経由して流れる際、この設定の動作は `use_strict_insert_block_limits` に依存します:
   ...
   * 無効な場合 (デフォルト) : ブロックは `min_insert_block_size_rows` または `min_insert_block_size_bytes` のいずれかに到達した時点で出力されます。`max_insert_block_size` 設定は適用されません。
...
不要な Dictionary 属性の取得を避けるため、`tupleElement(dictGet('dict', ('a', 'b', 'c'), key), 2)` を `dictGet('dict', 'b', key)` に書き換えます。位置指定 (`.1`、`.2`、...) および名前指定 (`.b`) によるアクセスをサポートし、デフォルト引数が定数 Tuple または定数で構成された `tuple(...)` の場合は `dictGetOrDefault` にも適用されます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "書き換え前に型の互換性が確認されるようになったため、arrayExists から has への書き換え最適化はデフォルトで有効になりました。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "16777216"},{"label": "キャッシュミス時にユーザー空間ページキャッシュを埋めるために使用される、単一の結合読み取りのサイズに上限を設ける新しい設定。"}]}]} />

`readBigAt` がユーザー空間ページキャッシュを埋める際、連続したキャッシュミスは、基盤ストレージからの単一の読み取りに結合されます。この設定は、1 回の結合読み取りのサイズをバイト単位で制限します。より長い連続ミスは複数回の読み取りに分割されます。これにより、並列コールドリード時に一時バッファが使用する一時的なメモリ量を抑えます。

値を大きくすると、オブジェクトストレージに対するコールドスキャン時の HTTP リクエスト数が減ります。値を小さくすると、一時的なメモリ使用量のピークを抑えられます。
...
Paimon のインクリメンタルモードにおける、クエリレベルの特定スナップショット読み取りです。&gt;0 の場合、リーダーはコミット済みウォーターマークを進めずに、指定した snapshot&#95;id の差分のみを取得します。
デフォルト: -1 (無効)
...
v25.4 以降、`ReplicatedMergeTree` または `SharedMergeTree` をログソースとする `INSERT ... SELECT` も、レプリカ間で並列化できます。有効にするには、次を設定します。
...
並列レプリカで、`MergeTree` テーブル上のシンプルなビューに対して、ビューの内部クエリではなく外側のクエリを実行できるようにし、ノード間の並列化を向上させます。各分岐がそれぞれ異なる `MergeTree` テーブルを読み取る `UNION ALL` ビューにも適用されます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "新しい設定。無効にすると、並列読み込み用のレプリカは、ローカルレプリカを必ず含めることなく、ロードバランシングアルゴリズムのみに従って選択されます。"}]}]} />

有効時 (デフォルト) には、並列読み込みに使用されるレプリカ群にローカルレプリカが常に含まれます。
無効時には、ローカルレプリカは優先されず、レプリカはロードバランシングアルゴリズムのみに従って選択されます。
これにより、`max_parallel_replicas = 1` のクエリを別のホストに送ることが可能になり、多数の短いクエリがクラスター全体に分散している場合のキャッシュ局所性を改善できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": ""},{"label": "polyglot transpiler のソース SQL ダイアレクトを指定するための New SETTING。"}]}]}/>

polyglot transpiler のソース SQL ダイアレクト (例: 'sqlite'、'mysql'、'postgresql'、'snowflake'、'duckdb') 。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1000"},{"label": "lazy join indexing 最適化でクエリプランの使用を許可する limit の最大値を制御する新しい設定を追加しました。0 の場合、制限はありません"}]}]} />

JOIN における 遅延索引最適化で、クエリプランの使用を許可する limit の最大値を制御します。0 の場合、制限はありません。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "10000"},{"label": "projection matcher（現時点では aggregate projection）において、内容ハッシュ化された IN 句の Set のコストに上限を設ける新しい設定を追加しました。制限を超える Set は非一致として扱われます。ゼロを指定すると内容ハッシュ比較が完全に無効になり（互換性維持用の値）、IN-set を含むノードでは projection match は決して成功しません。"}]}]} />

projection matcher が 2 つの Set が等しいかどうかを判定する際に、`IN` 句の Set の内容ハッシュを計算して比較する対象となる最大行数です。これを超える Set は非一致として扱われ、projection はスキップされます。ゼロを指定すると内容ハッシュ比較が完全に無効になり、`IN` 句の Set を含むノードでは projection match は決して成功しません。

aggregate projection matcher (および、`IN` 句の Set を比較する必要がある今後の projection matcher) で使用されます。内容ハッシュの計算コストは、Set の要素数に対して `O(N log N)` です。この設定により、クエリまたは projection に多数の `IN` 句が現れる場合でも、プランニング時に発生するコストの上限を抑えられます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "3"},{"label": "JOIN で遅延索引最適化を有効にするために、左側で必要となるペイロードカラムの最小数を制御します"}]}]} />

JOIN で遅延索引最適化を有効にするために、左側で必要となるペイロードカラムの最小数を制御します。0 はこの最適化が無効であることを示します。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "集合ベースの索引解析を使用して、ReplacingMergeTree に対する FINAL を用いた読み込みを最適化するための新しい設定"}]}]} />

主キーの集合を構築し、それを索引解析に使用することで、ReplacingMergeTree に対する FINAL を用いた読み込みを最適化します。
...
<VersionHistory
  rows={[
  {
    ...
    items: [
      ...
      {
        label:
          "有効にすると、read-in-order 中に各ブロックの後で仮想行を出力し、MergingSortedTransform でソースの優先順位をより頻繁に付け替えられるようになります。"
      }
    ]
  }
]}
...
`read_in_order_use_virtual_row` と併せて有効にすると、各パーツの先頭だけでなく、読み込んだ各ブロックの後にも仮想行を出力します。
これにより、`MergingSortedTransform` はソースの優先順位をより頻繁に付け替えられるようになり、下流のフィルターで多くの行が破棄され、データがパーツ間で不均一に分散している場合に有効です。
なお、これを有効にすると、読み取り時の `read_in_order_use_buffering` 最適化と事前マージ (`read_in_order_two_level_merge_threshold`) は無効になります。
...
復元時にテーブルエンジンを Replicated*MergeTree から Shared*MergeTree に置き換えます。
...
S3 エンドポイントのスタイルを指定します。設定可能な値: auto, virtual&#95;hosted, path。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "INSERT クエリにインラインデータが含まれる場合に、サーバーがテーブル構造を送信するかどうかを制御する新しい設定。"}]}]} />

無効にすると、INSERT クエリにインラインデータが含まれている場合、サーバーはネイティブプロトコル経由でテーブル構造とカラムの基本値をクライアントに返送しません。代わりに、サーバー自身がインラインデータを解析します。これにより、ネイティブプロトコル経由で多数の小規模な挿入を行う場合のパフォーマンスが向上することがあります。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "50"},{"label": "SharedMergeTree で逐次整合性を使用するクエリにおける、散発的な UNFINISHED エラーを減らすための新しい設定。"}]}]} />

`SharedMergeTree` で `select_sequential_consistency` を使用する際の、パーツ更新に対するミリ秒単位の初期バックオフです。ClickHouse Cloud でのみ利用できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1000"},{"label": "SharedMergeTree で逐次整合性を使用するクエリにおける、散発的な UNFINISHED エラーを減らすための新しい設定。"}]}]} />

`SharedMergeTree` で `select_sequential_consistency` を使用する際の、パーツ更新に対する最大バックオフ時間 (ミリ秒) 。ClickHouse Cloud でのみ利用できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "10"},{"label": "SharedMergeTree で逐次整合性を使用するクエリにおいて、断続的に発生する UNFINISHED エラーを減らすための新しい設定。"}]}]} />

`SharedMergeTree` で `select_sequential_consistency` を使用する際の、パーツ更新時の最大再試行回数です。ClickHouse Cloud でのみ利用できます。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "`show_data_lake_catalogs_in_system_tables` から名称変更され、既定では `system.tables`、`system.columns`、`system.completions` で `MySQL` および `PostgreSQL` データベースも非表示にするよう対象が拡張されました。これらのテーブルを列挙するには高コストなリモート呼び出しが必要になるためです。以前の動作を前提としていたユーザーは、この設定を `true` にする必要があります。旧名は別名として維持されます。"}]}]} />

システムテーブルにリモートデータベース (データレイクのカタログ、MySQL、PostgreSQL) を表示できるようにします。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0.2"},{"label": "遅延ポスティングリスト の密度しきい値に関する新しい設定"}]}]} />

遅延ポスティングリスト モードでアルゴリズムを選択するための密度しきい値です。
しきい値未満: leapfrog 積集合。しきい値以上: 総当たり bitmap。
...
辞書スキャンによるテキスト索引の LIKE 評価が有効な場合に読み取る、大きなポスティングの最大数。
...
辞書スキャンによるテキスト索引の LIKE 評価を使用するために必要な、LIKE/ILIKE パターン内の英数字の needle の最小長です。
このしきい値より短いパターンは一致する辞書トークンが多くなりすぎるため、コストの高いスキャンを避けるためにスキップされます。
...
&#39;materialize&#39; (デフォルト) は、ポスティングリストを Roaring Bitmap に即時デコードします。
&#39;lazy&#39; は、カーソルベースのオンデマンドデコードを使用します (V2 索引フォーマットと allow&#95;experimental&#95;text&#95;index&#95;lazy&#95;apply が必要です) 。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": ""},{"label": "url テーブル関数および URL テーブルエンジンで相対 URL を解決するためのベース URL を指定する新しい設定。"}]}]} />

[url](../../sql-reference/table-functions/url.md) テーブル関数および [URL](../../engines/table-engines/special/url.md) テーブルエンジンで、相対 URL の解決に使用するベース URL です。

設定すると、相対 URL は次のように解決されます。

* パス相対 URL (例: `data.csv`) : RFC 3986 に従ってベース URL のパスと結合されます。ベースパスの最後の `/` より後ろの部分は相対 URL で置き換えられるため、末尾のスラッシュの有無が重要です。`https://example.com/dir/` + `data.csv` = `https://example.com/dir/data.csv`、一方、`https://example.com/dir` + `data.csv` = `https://example.com/data.csv` です。ベースにパスがない場合 (例: `https://example.com`) は、`/` が挿入されます: `https://example.com/data.csv`。相対 URL 内のドットセグメント (`./` および `../`) は正規化されます: `https://example.com/dir/` + `../a.csv` = `https://example.com/a.csv`。
* ホスト相対 URL (例: `/test/data.csv`) : ベース URL のスキームとホストを基準に解決されます。
* スキーム相対 URL (例: `//other.com/test/data.csv`) : ベース URL のスキームを使って解決されます。
* クエリのみの参照 (例: `?x=1`) : ベース URL のパスに追加されます (既存のクエリ文字列やフラグメントは置き換えられます) 。
* フラグメントのみの参照 (例: `#frag`) : ベース URL に追加され、既存のクエリ文字列は保持されます (既存のフラグメントは置き換えられます) 。
* 空の参照: フラグメントを除いたベース URL を返します。
...
サーバーの同時実行制御（グローバルサーバー設定 `concurrent_threads_soft_limit_num` および `concurrent_threads_soft_limit_ratio_to_cores` を参照）に従います。無効にすると、サーバーが過負荷状態であっても、より多くのスレッドを使用できるようになります（通常の利用では推奨されず、主にテストに必要です）。
...
リモートクエリ向けの hedged requests ロジックを有効にします。これにより、クエリに対して異なるレプリカへ複数の接続を確立できるようになります。
新しい接続は、レプリカとの既存の接続が `hedged_connection_timeout` 以内に確立されなかった場合、
または `receive_data_timeout` 以内にデータを受信しなかった場合に有効になります。クエリは、空でない progress パケット（または `allow_changing_replica_until_first_data_packet` の場合は data パケット）を最初に送信した接続を使用し、
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "TopK フィルタリングでデータスキッピングインデックスをデフォルトで使用できるようにする"}]}, {"id": "row-2","items": [{"label": "25.12"},{"label": "0"},{"label": "新しい設定"}]}]} />

TopK フィルタリングにデータスキッピングインデックスを使用できるようにします。

有効にすると、`ORDER BY <column> LIMIT n` クエリ内のカラムに minmax スキップ索引が存在する場合、オプティマイザは最終結果に関係しないグラニュールをスキップするために minmax 索引を使用しようとします。これにより、クエリレイテンシを削減できる場合があります。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "挿入時に最小および最大の挿入境界を厳密に適用するための新しい設定。min < max の場合、max の制限が優先されます。"}]}]} />

有効にすると、挿入ブロックサイズの最小制限と最大制限の両方が厳密に適用されます。
...
**注**: max の設定が min の設定より小さい場合は、max の制限が優先されるため、min のしきい値に達する前にブロックが生成されます。

**注**: この設定は非同期挿入では自動的に無効になります。これは、非同期挿入ではエントリごとの重複排除トークンが付与されますが、これが厳密な制限の適用に必要なブロック分割と両立しないためです。
...
転置テキスト索引の辞書をスキャンして LIKE/ILIKE クエリを評価できるようにします。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "TopK クエリに対する動的フィルタリング最適化をデフォルトで有効にする"}]}, {"id": "row-2","items": [{"label": "25.12"},{"label": "0"},{"label": "New setting."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "既定の Variant 実装で型の不一致時の動作を制御するための新しい設定"}]}]} />

既定の実装で [Variant](../../sql-reference/data-types/variant.md) カラムに関数を適用する場合に、
実際の型がその関数と互換性のない行をどのように扱うかを制御します。
