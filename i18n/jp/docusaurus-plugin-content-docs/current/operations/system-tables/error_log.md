...
* `code` ([Int32](/sql-reference/data-types/int-uint)) — エラーコード。
* `error` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — エラー名。
* `value` ([UInt64](/sql-reference/data-types/int-uint)) — 時間インターバル内に発生したエラー数。
* `remote` ([UInt8](/sql-reference/data-types/int-uint)) — リモート例外 (つまり、分散クエリのいずれかで受信したもの) 。
* `last_error_time` ([DateTime](/sql-reference/data-types/datetime)) — 最後のエラーが発生した時刻。
* `last_error_message` ([String](/sql-reference/data-types/string)) — 最後に発生したエラーのメッセージ。
* `last_error_query_id` ([String](/sql-reference/data-types/string)) — 最後のエラーを引き起こしたクエリの ID (利用可能な場合) 。
* `last_error_trace` ([Array(UInt64)](/sql-reference/data-types/array)) — 呼び出されたメソッドが格納されている物理アドレスのリストを示すスタックトレース。
