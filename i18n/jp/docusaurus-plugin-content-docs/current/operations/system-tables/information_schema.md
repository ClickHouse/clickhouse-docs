...
[system.columns](../../operations/system-tables/columns.md) システムテーブルから読み取られたカラムと、ClickHouse でサポートされていない、または意味をなさないものの、標準上必要であるため含まれているカラム (常に `NULL`) を含みます。
...
制約によって制限されている [system.tables](../../operations/system-tables/tables.md) システムテーブル内のカラムを含みます。
