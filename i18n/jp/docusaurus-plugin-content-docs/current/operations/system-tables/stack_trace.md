---
description: 'すべてのサーバースレッドのスタックトレースを含むsystem テーブルです。開発者がサーバーの内部状態を調査できるようにします。'
...
* `thread_name` ([String](../../sql-reference/data-types/)) — スレッド名。
* `thread_id` ([UInt64](../../sql-reference/data-types/)) — スレッド識別子。
* `query_id` ([String](../../sql-reference/data-types/)) — このスレッドが属するクエリの ID。
* `trace` ([Array(UInt64)](../../sql-reference/data-types/)) — このスレッドのスタックトレースです。基本的にはアドレスの配列です。
* `untracked_memory` ([Int64](../../sql-reference/data-types/)) — まだ親の MemoryTracker に反映されていないメモリ割り当て量を表す、スレッドごとのアトミックでないカウンターです。前回の flush 以降、割り当てた量より解放した量のほうが多い場合は、負の値になることがあります。
...
ClickHouse オブジェクトファイルからシンボルを取得:
...
ClickHouse のソースコード内のファイル名と行番号を取得する:
...
* [イントロスペクション関数](../../sql-reference/functions/introspection.md) — 利用可能なイントロスペクション関数とその使い方。
