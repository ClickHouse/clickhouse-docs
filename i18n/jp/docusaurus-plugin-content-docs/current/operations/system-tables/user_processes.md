---
description: 'システムテーブルは、メモリ使用量とユーザーのProfileEventsの概要を取得するのに役立つ情報を含んでいます。'
...
このシステムテーブルは、メモリ使用量とユーザーのProfileEventsの概要を取得するのに使用できます。

カラム:

- `user` ([String](../../sql-reference/data-types/string.md)) — ユーザー名。
- `memory_usage` ([Int64](/sql-reference/data-types/int-uint#integer-ranges)) — ユーザーのすべてのプロセスが使用するRAMの合計。この合計には、一部の専用メモリが含まれない場合があります。[max_memory_usage](/operations/settings/settings#max_memory_usage)設定を参照してください。
...
- `ProfileEvents` ([Map(String, UInt64)](../../sql-reference/data-types/map)) — ユーザーのためのさまざまなメトリックを測定するProfileEventsの概要。これらの説明はテーブル[system.events](/operations/system-tables/events)にあります。
