---
description: 'ZooKeeper 接続履歴（補助 ZooKeeper を含む）を表示します。'
...
`system.zookeeper&#95;connection&#95;log` テーブルは、ZooKeeper への接続履歴（補助 ZooKeeper を含む）を示します。各行には、接続に関する 1 件のイベント情報が含まれます。
...
* `event_time_microseconds` ([Date](../../sql-reference/data-types/datetime64.md)) - エントリの時刻（マイクロ秒精度）。
...
* `index` ([UInt8](../../sql-reference/data-types/int-uint.md)) — ClickHouse が接続または切断した ZooKeeper ノードのインデックス。ZooKeeper の設定で定義されたインデックスです。
* `client_id` ([Int64](../../sql-reference/data-types/int-uint.md)) — 接続セッション ID。
* `keeper_api_version` ([UInt8](../../sql-reference/data-types/int-uint.md)) — Keeper API バージョン。
* `enabled_feature_flags` ([Array(Enum16)](../../sql-reference/data-types/array.md)) — 有効化されている機能フラグ。ClickHouse Keeper にのみ適用されます。取りうる値は `FILTERED_LIST`, `MULTI_READ`, `CHECK_NOT_EXISTS`, `CREATE_IF_NOT_EXISTS`, `REMOVE_RECURSIVE` です。
