...
パーツのチェックサムで報告される、MergeTree family のすべてのテーブルに格納された非圧縮バイトの総量です。これは `system.tables` の `total_bytes_uncompressed` カラムと同じ情報源に基づいており、マークや主キーインデックスなど、非圧縮で格納されるファイルは含みません。

### TotalUncompressedBytesOfMergeTreeTablesSystem \{#totaluncompressedbytesofmergetreetablessystem\}

システムデータベース内の MergeTree family のテーブルに格納されている、パーツのチェックサムで報告される非圧縮バイト総量です。これは `system.tables` の `total_bytes_uncompressed` カラムと同じ情報源に基づくもので、マークや主キーインデックスなどの非圧縮で保存されるファイルは含みません。
