...
* `symbol` ([String](../../sql-reference/data-types/)) — バイナリ内のシンボル名です。マングルされています。`demangle(symbol)` を適用すると、可読な名前を取得できます。
* `symbol_demangled` ([Nullable(String)](../../sql-reference/data-types/)) — XRay インストルメンテーションで使用される、デマングル済みのシンボルです。
* `function_id` ([Nullable(Int32)](../../sql-reference/data-types/)) — XRay インストルメンテーション マップ内の関数 ID です。
* `address_begin` ([UInt64](../../sql-reference/data-types/)) — バイナリ内のシンボルの開始アドレスです。
* `address_end` ([UInt64](../../sql-reference/data-types/)) — バイナリ内のシンボルの終了アドレスです。
