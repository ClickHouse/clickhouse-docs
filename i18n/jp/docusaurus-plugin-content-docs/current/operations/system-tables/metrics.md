...
キャッシュされた JIT コンパイル済み関数の実行可能/データセクション向けに `JITModuleMemoryManager` が保持する、予約済みのページブロック容量です (ページ単位に切り上げられ、2 倍の余裕を見込んで確保されます) 。実際に使用中のマシンコードのバイト数ではありません (実際の使用量はこれより小さくなります) 。`posix_memalign` 経由で割り当てられますが、これは jemalloc によって捕捉されるため、専用の JIT アリーナ内で計上され、`jemalloc.jit_arena.active_bytes` の一部となります。

### CompiledExpressionCacheCount \{#compiledexpressioncachecount\}

JITコンパイル済みマシンコードのキャッシュ内のエントリ総数
...
Keeper サーバーの読み取り用スレッドプールのスレッド数。

### KeeperReadThreadsActive \{#keeperreadthreadsactive\}

Keeper サーバーの読み取り用スレッドプール内でアクティブなスレッド数。

### KeeperReadThreadsScheduled \{#keeperreadthreadsscheduled\}

Keeper サーバーの読み取り用スレッドプールで、キューに入っている、または実行中のジョブ数。実際の読み取りタスクはこのスレッドプールでは別の仕組みでスケジュールされるため、このメトリクスには意味がありません。
