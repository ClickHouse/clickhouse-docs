---
description: '現在有効なすべてのロールを含み、現在のユーザーの現在のロールおよびそのロールに付与されているロールを含むシステムテーブル'
...
現在有効なすべてのロールを含むシステムテーブルであり、現在のユーザーの現在のロールおよびそのロールに付与されているロールを含みます。
...
- `is_default` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — `enabled_role` がデフォルトのロールかどうかを示すフラグ。