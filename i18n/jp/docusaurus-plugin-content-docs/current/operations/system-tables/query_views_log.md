---
description: 'クエリの実行時に実行される依存ビューに関する情報（ビューの種類や実行時間など）を保持するシステムテーブル。'
...
クエリを実行する際に実行された従属ビューに関する情報を含みます。たとえば、ビューの種類や実行時間などです。

ロギングを開始するには、次のようにします。
...
データのフラッシュ間隔は、[query_views_log](../../operations/server-configuration-parameters/settings.md#query_views_log) サーバー設定セクションの `flush_interval_milliseconds` パラメータで設定します。強制的にフラッシュするには、[SYSTEM FLUSH LOGS](/sql-reference/statements/system#flush-logs) クエリを使用します。
...
`query_views_log` テーブルに記録されるクエリ数を減らすには、[log_queries_probability](/operations/settings/settings#log_queries_probability) 設定を使用できます。
...
- `view_duration_ms` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — ビュー実行時間（すべてのステージの合計）をミリ秒単位で表したもの。
...
- `view_type` ([Enum8](../../sql-reference/data-types/enum.md)) — ビューの種類。値:
  - `'Default' = 1` — [Default views](/sql-reference/statements/create/view#normal-view)。このログには現れないはずです。
...
- `peak_memory_usage` ([Int64](../../sql-reference/data-types/int-uint.md)) — このビューのコンテキストにおける、確保されたメモリ量と解放されたメモリ量の差分の最大値。
- `ProfileEvents` ([Map(String, UInt64)](../../sql-reference/data-types/array.md)) — 各種メトリクスを計測する ProfileEvents。これらの説明はテーブル [system.events](/operations/system-tables/events) に記載されています。
- `status` ([Enum8](../../sql-reference/data-types/enum.md)) — ビューのステータス。値:
  - `'QueryStart' = 1` — ビュー実行の正常な開始。このログには現れないはずです。
  - `'QueryFinish' = 2` — ビュー実行の正常な終了。
