---
'description': 'システム テーブルは、致命的なエラーのスタックトレースに関する情報を含んでいます。'
'keywords':
- 'system table'
- 'crash_log'
...
致命的なエラーに関するスタックトレースの情報を含みます。テーブルはデフォルトではデータベースに存在せず、致命的なエラーが発生したときのみ作成されます。
...
- `trace_full` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — クラッシュ時のスタックトレース。各要素はClickHouseサーバープロセス内の呼び出されたメソッドを含んでいます。
...
- `build_id` ([String](../../sql-reference/data-types/string.md)) — コンパイラによって生成されたBuildID。
