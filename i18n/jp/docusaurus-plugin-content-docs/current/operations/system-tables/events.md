...
外部処理 (ソート/集約/join) によって書き込まれたデータ量 (非圧縮、つまり圧縮前)
...
Keeper によって処理された読み取りリクエストのバッチ数

### KeeperReadBatchTotalRequests \{#keeperreadbatchtotalrequests\}

Keeper がバッチ処理した読み取りリクエストの総数
...
要求されたスナップショットがローカルで利用できなくなったため、スナップショットの読み取りが延期された回数です。NuRaft は最新のスナップショットで再試行します。
...
フォロワーにスナップショットを提供する際の、RemoteSnapshotLoader で発生したリモート読み取りエラー数
...
クエリに含まれる分片数です。これは、すべての分散テーブルおよびテーブル関数にまたがって合計されます。1 つのホストが複数のテーブルに現れる場合は、複数回カウントされます。この数は想定される分片の総数を表し、`skip_unavailable_shards` 設定によってスキップされる分片も含まれます。
...
lazy posting list モードで、出力領域がすでに確定していた (OR では全ビット1、AND では全ビット0) ためにスキップされた パック済みブロック の数。
...
lazy posting list モードで、実際に読み込まれてデコードされたセグメント数 (キャッシュミス) 。

### TextIndexLazySegmentsPrepared \{#textindexlazysegmentsprepared\}

lazy posting listモードで準備されたセグメント数 (ディスクから読み込まれた、またはキャッシュされたもの) 。

### TextIndexLazySegmentsSkippedDense \{#textindexlazysegmentsskippeddense\}

lazy posting list モードにおいて、ブロックをデコードする代わりに、完全に dense なセグメント全体をパディングした数 (OR では `memset`、AND ではインクリメント) 。

### TextIndexLazySegmentsSkippedResolved \{#textindexlazysegmentsskippedresolved\}

lazy posting list モードで、出力領域がすでに確定していた (OR では全ビット1、AND では全ビット0) ためにスキップされたセグメント数。
...
ZooKeeper watch コールバックから送出された例外の数。
