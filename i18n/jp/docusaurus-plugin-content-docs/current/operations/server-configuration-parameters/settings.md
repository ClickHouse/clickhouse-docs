...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />ディスク (S3、Azure、GCS) 接続に対する SO&#95;RCVBUF オプションのサイズです。0 より大きい値を設定すると、受信バッファに関するカーネルの TCP 自動調整を無効にして、この値で固定します。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻しても、自動調整が復元されるのは新規に作成された接続のみです。既存のプールされた接続は、再作成されるまで固定のバッファサイズを保持します。

## disk_connections_sndbuf \{#disk_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />ディスク (S3、Azure、GCS) 接続の `SO_SNDBUF` オプションのサイズです。0 より大きい値を設定すると、送信バッファに対するカーネルの TCP 自動調整を無効にして、この設定で上書きします。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻しても、自動調整が復元されるのは新しく作成される接続だけです。既存のプール済み接続は、再作成されるまで固定のバッファサイズを保持します。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />一般的な HTTP 接続における `SO_RCVBUF` オプションのサイズです。0 より大きい値に設定すると、受信バッファに対するカーネルの TCP 自動調整よりこの設定が優先されます。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻しても、自動調整が復元されるのは新しく作成された接続のみです。既存のプールされた接続では、再作成されるまで固定のバッファサイズが維持されます。

## http_connections_sndbuf \{#http_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />一般的な HTTP 接続における `SO_SNDBUF` オプションのサイズです。0 より大きい値に設定すると、送信バッファに対するカーネルの TCP 自動調整を重写します。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻した場合、自動調整が復元されるのは新規に作成される接続のみです。既存のプールされた接続は、再作成されるまで固定のバッファサイズを保持します。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />ストレージ接続 (レプリケーション、分散クエリ) に対する `SO_RCVBUF` オプションのサイズです。0 より大きい値を設定すると、受信バッファに対するカーネルの TCP 自動調整を上書きします。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻しても、自動調整が復元されるのは新しく作成された接続のみです。既存のプールされた接続は、再作成されるまで固定のバッファサイズを保持します。

## storage_connections_sndbuf \{#storage_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />ストレージ接続 (レプリケーション、分散クエリ) の `SO_SNDBUF` オプションのサイズです。0 より大きい値を設定すると、送信バッファに対するカーネルの TCP 自動調整よりこの設定が優先されます。0 = カーネルのデフォルト (自動調整) 。注意: この設定を 0 に戻しても、自動調整が復元されるのは新しく作成される接続のみです。既存のプールされた接続は、再作成されるまで固定バッファサイズを保持します。
