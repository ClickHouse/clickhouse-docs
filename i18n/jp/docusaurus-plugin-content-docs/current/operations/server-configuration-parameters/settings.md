...
| `disallow_config_defined_profiles_for_sql_defined_users` | 有効にすると、デフォルトプロファイルを除き、config で定義された設定プロファイル (`users.xml` などの XML 設定ファイルで定義されたもの) が SQL で定義されたユーザーに適用されるのを防ぎます。SQL で定義されたユーザーに付与されたロールに割り当てられたプロファイルもチェックされます。                                                                                                                                                                                                  | `false` |
