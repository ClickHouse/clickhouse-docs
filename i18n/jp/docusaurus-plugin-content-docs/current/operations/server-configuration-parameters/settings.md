...
<SettingsInfoBlock type="UInt64" default_value="10" />リモートのIcebergカタログから最新のメタデータを非同期に取得するためのスレッドプールのサイズです。このプールは、すべてのアクティブなテーブルで共有されます。
...
<SettingsInfoBlock type="UInt64" default_value="0" />同時に保持できる軽量スナップショットの最大数。0 は無制限を意味します。スナップショット数がこの上限に達すると、新しいスナップショットを作成しようとした際に例外が送出されます。
...
<SettingsInfoBlock type="UInt64" default_value="5000" />Parquet メタデータファイルキャッシュの最大エントリ数。0 の場合は無効です。

## parquet_metadata_cache_policy \{#parquet_metadata_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />Parquet メタデータキャッシュのポリシー名。

## parquet_metadata_cache_size \{#parquet_metadata_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="536870912" />Parquet メタデータキャッシュの最大サイズ（バイト単位）。0 の場合は無効です。

## parquet_metadata_cache_size_ratio \{#parquet_metadata_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" />Parquet メタデータキャッシュにおける保護キュー (SLRU ポリシーの場合) のサイズを、キャッシュ全体のサイズに対する比率で表したものです。
