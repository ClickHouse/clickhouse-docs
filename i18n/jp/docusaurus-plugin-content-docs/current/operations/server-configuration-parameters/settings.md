...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />`/webterminal` HTTPエンドポイントで実験的なWebターミナルインターフェイスを有効にします。WebSocket 経由で、ブラウザ内で対話型の `clickhouse-client` セッションを提供します。
