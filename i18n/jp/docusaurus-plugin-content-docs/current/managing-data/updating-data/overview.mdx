...
|--------------------------------------------------------------|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Update mutation](/sql-reference/statements/alter/update)    | `ALTER TABLE [table] UPDATE`   | データを即座にディスクに更新する必要がある場合に使用します（例: コンプライアンス対応）。`SELECT` のパフォーマンスに悪影響を与えます。                                                                                                                |
| [On-the-fly updates](/guides/developer/on-the-fly-mutations) | `ALTER TABLE [table] UPDATE`   | 少量のデータを更新する場合に使用します。以降のすべての `SELECT` クエリで更新後のデータが即座に返されますが、ディスク上では最初は内部的に更新済みとしてマークされるだけです。`SET apply_mutations_on_fly = 1;` を使用して有効にします。 |
| [Lightweight updates](/sql-reference/statements/update)      | `UPDATE [table] SET ... WHERE` | 少量のデータ（テーブルの約 10% まで）を更新する場合に使用します。列全体を書き換えることなく、即時可視化のためのパッチ用パーツを作成します。`SELECT` クエリにオーバーヘッドを追加しますが、レイテンシは予測可能です。                              |
...
|--------|----------------|-----------------|-------|
| [Mutations](/sql-reference/statements/alter/update) | ベースライン | ベースライン | 完了後はフルスピードで実行される; データは物理的に書き換えられる |
| [On-the-fly mutations](/guides/developer/on-the-fly-mutations) | 可変 | 可変 | 即時に可視化される; 多数の更新が蓄積するとパフォーマンスが低下する |
| [Lightweight updates](/sql-reference/statements/update#lightweight-update-requirements) | 7–18% (平均 ~12%) | +20–210% | クエリに対して最も効率的; テーブルの ≤10% を更新する場合に最適 |
