...
| ------------------------------------------------------------------------------ | ---------------------------- | ----------------------------------------------------------------------------------------------- |
| [論理削除](/guides/developer/lightweight-delete)                     | `DELETE FROM [table]`        | 少量のデータを削除する場合に使用します。行は以降のすべての SELECT クエリから即座に除外されますが、当初は内部的に削除済みとしてマークされるだけであり、ディスク上からは削除されません。 |
...
| [テーブルを空にする](/sql-reference/statements/truncate)                           | `TRUNCATE TABLE [db.table]`  | テーブルからすべてのデータを効率的に削除します。                                                                        |
| [パーティションの削除](/sql-reference/statements/alter/partition#drop-partitionpart) | `DROP PARTITION`             | パーティション内のすべてのデータを効率的に削除します。                                                                     |
...
* [ClickHouse における更新および削除の扱い方](https://clickhouse.com/blog/handling-updates-and-deletes-in-clickhouse)