...
description: 'Bun 向けの chDB のインストール方法'
...
これらのファイルに対して直接`SELECT`クエリを記述し、データの内容を確認することもできます:
...
これから、これらのCSVファイルのデータをテーブルに格納します。
...
ここでは、各選手が獲得した最大ポイントとその時点でのランキングを取得するクエリを記述します：
...
興味深いことに、このリストの選手の中には、多くのポイントを獲得しながらも、そのポイント総数で1位になっていない選手がいます。
...
以下のクエリは、初めてトップ10にランクインした日と最後にトップ10にランクインした日の間の日数が最も少ないプレイヤーを検索します：
