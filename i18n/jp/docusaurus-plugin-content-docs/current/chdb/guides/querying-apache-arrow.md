...
description: 'このガイドでは、chDB を使用して Apache Arrow テーブルに対してクエリを実行する方法を説明します'
keywords: ['chdb', 'Apache Arrow']
doc_type: 'guide'
---

[Apache Arrow](https://arrow.apache.org/) は、データコミュニティで広く利用されている標準化されたカラム指向のインメモリフォーマットです。
このガイドでは、`Python` テーブル関数を使用して Apache Arrow に対してクエリを実行する方法を説明します。
...
続いて、PyArrow、pandas、ipythonをインストールします:
...
Pythonスクリプトやお好みのノートブック環境でもコードを実行できます。
...
:::note
より多くのファイルをダウンロードする場合は、`aws s3 ls`を使用してすべてのファイルのリストを取得し、上記のコマンドを更新してください。
:::
...
それでは、chDBからArrowテーブルにクエリを実行してみましょう。
まず、chDBをインポートします:
...
それでは、もう少し興味深い処理を行ってみましょう。
次のクエリは`quadkey`と`tile.*`列を除外し、残りのすべての列の平均値と最大値を計算します:
