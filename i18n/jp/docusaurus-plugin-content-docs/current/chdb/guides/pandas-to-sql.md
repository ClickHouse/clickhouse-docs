...
| `df.head(10)` | `LIMIT 10` |
| `df.tail(10)` | 少し複雑 (ORDER BY ... DESC LIMIT 10) |
| `df.drop_duplicates()` | `SELECT DISTINCT *` |

### ソート (ORDER BY) \{#sorting-order-by\}
...
| `df.dropna()` | `WHERE col IS NOT NULL` (各列ごとに) |
...
## pandas ユーザーのためのヒント \{#tips\}

### 1. SQL の操作として捉える \{#think-in-sql\}

DataStore のコードを書くときは、どんな SQL を発行したいのかを考えてください。
...
### 3. SQL の機能を活用する \{#leverage-sql-features\}

DataStore を使えば、pandas の構文で SQL の強力な機能を活用できます。
