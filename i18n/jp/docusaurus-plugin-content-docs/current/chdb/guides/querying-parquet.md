...
description: 'chDB を使って Parquet ファイルをクエリする方法を説明します。'
keywords: ['chdb', 'parquet']
doc_type: 'guide'
---

世界中の多くのデータは Amazon S3 バケットに保存されています。
このガイドでは、chDB を使ってそれらのデータにクエリを実行する方法を説明します。
...
この出力から、このParquetファイルには4000万行以上のデータがあり、42の行グループに分割され、各行には15列のデータがあることがわかります。
...
上記のクエリを調整して`ParquetMetadata`を削除し、全レビューにおける最も多い`star_rating`を計算することができます。
...
Amazonの商品を気に入っている人が多いか、気に入らない場合は評価を投稿しないだけのようです。
