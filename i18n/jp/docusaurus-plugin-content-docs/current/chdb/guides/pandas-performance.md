...
DataStore は、多くの操作で pandas より大幅に高い性能を発揮します。このガイドでは、その理由と、ワークロードを最適化する方法を説明します。
...
### 1. 高負荷なワークロード向けにパフォーマンスモードを有効化する \{#use-performance-mode\}
...
詳細については、[Performance Mode](../configuration/performance-mode.md) を参照してください。


### 2. CSV ではなく Parquet を使用する \{#use-parquet\}
...
### 4. 必要なカラムのみを選択 \{#select-only-needed-columns\}
...
### 5. SQLの集約機能を活用する \{#leverage-sql-aggregations\}
...
### 6. クエリ全体の実行ではなく head() を使用する \{#use-head\}
...
### 8. explain() を使用して最適化する \{#use-explain\}
...
詳細は [Execution Engine](../configuration/execution-engine.md) および [Performance Mode](../configuration/performance-mode.md) を参照してください。
:::