...
| **行の順序** | 保持される | 保持される（自動）；[performance mode](../configuration/performance-mode.md) では保証されない |
...
- **performance mode**（`config.use_performance_mode()`）では、いかなる操作においても行の順序は保証されません。[Performance Mode](../configuration/performance-mode.md) を参照してください。
