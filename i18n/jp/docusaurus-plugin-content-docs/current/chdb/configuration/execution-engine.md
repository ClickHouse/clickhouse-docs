...
負荷の高い集約ワークロードを実行していて、pandas の出力との厳密な互換性（行順、MultiIndex、dtype の補正）を必要としない場合は、[Performance Mode](performance-mode.md) の使用を検討してください。エンジンを自動的に `chdb` に設定し、pandas 互換性を保つためのオーバーヘッドをすべて排除します。
:::
