...
これは、組み込みのClickHouseエンジンを使用してSQL文を実行するメインクエリ関数です。様々な出力形式をサポートし、インメモリデータベースまたはファイルベースのデータベースで動作します。
...
| `path`          | str  | `""`       | データベースファイルパス。デフォルトはインメモリデータベース。<br/>ファイルパスまたはインメモリデータベースの場合は`":memory:"`を指定可能                                                                                                                                                                                               |
...
chDB関連のエラーに対する基底例外クラス。

この例外は、chDBのクエリ実行が失敗した場合やエラーが発生した場合に発生します。標準のPython Exceptionクラスを継承し、基盤となるClickHouseエンジンからのエラー情報を提供します。
...
セッションはクエリの状態を保持します。
pathがNoneの場合、一時ディレクトリを作成してデータベースパスとして使用し、セッションが閉じられると一時ディレクトリは削除されます。
データを保持するために、パスを指定してそのパスにデータベースを作成することもできます。

接続文字列を使用してパスやその他のパラメータを渡すこともできます。
...
:::note 接続文字列の引数処理
"[file:test.db?param1=value1&param2=value2](file:test.db?param1=value1&param2=value2)"のようなクエリパラメータを含む接続文字列では、
"param1=value1"が起動引数としてClickHouseエンジンに渡されます。

詳細については、`clickhouse local –help –verbose`を参照してください

特殊な引数の処理:

- "mode=ro"はclickhouseでは"–readonly=1"となります（読み取り専用モード）
  :::

:::warning 重要

- 同時に存在できるセッションは1つのみです。新しいセッションを作成する場合は、既存のセッションを閉じる必要があります。
...
このメソッドは、クリーンアッププロセス中に発生する可能性のある例外を抑制しながらセッションを閉じようとします。エラー処理のシナリオや、セッションの状態に関係なくクリーンアップを確実に実行する必要がある場合に特に有用です。
...
:::note
このメソッドは例外を発生させないため、finallyブロックやデストラクタで安全に呼び出すことができます。
:::
...
... finally:
...     session.cleanup()  # エラーに関係なく安全にクリーンアップ
```

**関連項目**

- [`close()`](#chdb-session-session-close) - エラー伝播を伴う明示的なセッションクローズ
...
このメソッドは基盤となる接続を閉じ、グローバルセッション状態をリセットします。
このメソッドを呼び出した後、セッションは無効になり、それ以降のクエリには使用できません。
...
:::note
このメソッドは、セッションがコンテキストマネージャとして使用される場合、またはセッションオブジェクトが破棄される場合に自動的に呼び出されます。
:::
...
このメソッドは、セッションのデータベースに対してSQLクエリを実行し、指定された形式で結果を返します。このメソッドは様々な出力形式をサポートし、クエリ間でセッション状態を維持します。
...
メモリにロードします。大きな結果セットの場合は、ストリーミング結果のために
[`send_query()`](#chdb-session-session-send_query) の使用を検討してください。
:::
...
すべてを一度にメモリにロードすることなく結果を反復処理できる
ストリーミング結果オブジェクトを返します。これは大きな
結果セットに特に有用です。
...
:::warning
返された StreamingResult オブジェクトは、データベースへの接続を維持するため、速やかに消費するか適切に保存する必要があります。
:::
...
| `format`  | str  | `"CSV"`    | 結果の出力形式。サポートされている形式:<br/>• `"CSV"` - カンマ区切り値（文字列）<br/>• `"JSON"` - JSON形式（文字列）<br/>• `"Arrow"` - Apache Arrow形式（バイト）<br/>• `"Dataframe"` - Pandas DataFrame（pandasが必要）<br/>• `"Arrowtable"` - PyArrow Table（pyarrowが必要） |
...
このメソッドは、SQLクエリを実行し、すべてを一度にメモリに読み込むことなく結果を反復処理できるStreamingResultオブジェクトを返します。大きな結果セットを処理する場合に最適です。
...
7項目のタプルのリストを返します。各タプルには次の内容が含まれます：
...
| 戻り値の型 | 説明                                                                                     |
| ----------- | ----------------------------------------------------------------------------------------------- |
| `tuple`     | 最大'size'個の行タプルを含むタプル。結果セットが使い果たされた場合はより少ない行を含む可能性があります |

:::note
このメソッドはDB-API 2.0仕様に従います。結果セットが使い果たされた場合は'size'より少ない行を返します。
:::
...
この関数は、chdbクエリ結果をPandas DataFrame形式に変換します。
まずPyArrow Tableに変換し、その後DataFrameに変換することで、
Pandas APIを使用した便利なデータ分析機能を提供します。
...
:::note
この関数は、大規模なデータセットでのパフォーマンスを向上させるため、
ArrowからPandasへの変換にマルチスレッドを使用します。
:::
...
## DataFrameの統合 {#dataframe-integration}
...
:::note
chDB/ClickHouseは従来のトランザクションをサポートしていないため、この操作は何も実行しません。DB-API 2.0準拠のために提供されています。
:::
...
カーソルは、SQL文の実行、クエリ結果の管理、結果セットのナビゲーションを行うメソッドを提供します。パラメータバインディングや一括操作をサポートし、DB-API 2.0仕様に準拠しています。
...
このメソッドは、異なるパラメータ値で同じSQLクエリを効率的に複数回実行します。
特に一括INSERT操作に有用です。
...
| 戻り値の型     | 説明                                            |
| --------------- | ------------------------------------------------------ |
| `tuple or None` | タプルとしての次の行、または利用可能な行がない場合はNone |
...
このメソッドは、パラメータ置換後の最終的なSQLクエリを表示します。
デバッグやログ記録の目的に有用です。
...
| パラメータ | 型              | デフォルト | 説明                                  |
| --------- | --------------- | ---------- | ------------------------------------- |
| `query`   | str             | _必須_     | パラメータプレースホルダを含むSQLクエリ |
...
:::note
このメソッドはPsycopgで使用されているDB-API 2.0の拡張に従っています。
:::
...
| 戻り値の型 | 説明                                                           |
| ----------- | ------------------------------------------------------------- |
| `None`      | 複数の結果セットがサポートされていないため、常にNoneを返します |

:::note
chDB/ClickHouseは単一のクエリからの複数の結果セットをサポートしていません。
このメソッドはDB-API 2.0準拠のために提供されていますが、常にNoneを返します。
:::
...
:::note
このメソッドは何も実行しませんが、DB-API 2.0仕様で必要とされています。
chDBは内部的にパラメータサイズを自動的に処理します。
:::
...
このモジュールは、Python Database API Specification v2.0に従い、
chDBにおけるデータベース関連エラーを処理するための完全な例外クラス階層を提供します。
...
:::note
これらの例外クラスはPython DB API 2.0仕様に準拠しており、
...
これは警告を除く、chdbにおけるすべてのエラー例外の基底クラスです。操作の正常な完了を妨げるすべてのデータベースエラー状態の親クラスとして機能します。
...
この例外は、以下を含む整合性制約に違反するデータベース操作が行われた場合に送出されます:
...
- トランザクションサポートのない接続で`rollback()`を要求
- データベースバージョンでサポートされていない高度なSQL機能の使用
- 現在のドライバで実装されていないメソッドの呼び出し
- 無効化されたデータベース機能の使用を試行
...
これはすべてのchdb関連例外の基底クラスです。Pythonの組み込みExceptionクラスを継承し、データベース操作の例外階層のルートとして機能します。

:::note
この例外クラスは、データベース例外処理に関するPython DB API 2.0仕様に準拠しています。
:::
...
挿入時のデータ切り捨てなど、重要な警告に対して発生する例外。

この例外は、データベース操作が完了したものの、アプリケーションが注意すべき重要な警告がある場合に発生します。
一般的なシナリオには以下が含まれます:
...
:::note
これは警告例外に関するPython DB API 2.0仕様に準拠しています。
:::
...
指定されたオブジェクトから新しい文字列オブジェクトを作成します。encodingまたはerrorsが指定されている場合、オブジェクトは指定されたエンコーディングとエラーハンドラを使用してデコードされるデータバッファを公開する必要があります。
それ以外の場合は、`object._\_str_\_()`(定義されている場合)または`repr(object)`の結果を返します。

- encodingのデフォルトは'utf-8'です。
- errorsのデフォルトは'strict'です。
...
数値または文字列を整数に変換します。引数が指定されていない場合は0を返します。xが数値の場合、x._\_int_\_()を返します。浮動小数点数の場合、ゼロ方向に切り捨てられます。

xが数値でない場合、またはbaseが指定されている場合、xは指定された基数の整数リテラルを表す文字列、bytes、またはbytearrayインスタンスである必要があります。リテラルの前には'+'または'-'を付けることができ、空白で囲むこともできます。baseのデフォルトは10です。有効な基数は0および2から36です。
base 0は、文字列から基数を整数リテラルとして解釈することを意味します。
...
指定されたオブジェクトから新しい文字列オブジェクトを作成します。encodingまたはerrorsが指定されている場合、オブジェクトは指定されたエンコーディングとエラーハンドラを使用してデコードされるデータバッファを公開する必要があります。
それ以外の場合は、object._\_str_\_()（定義されている場合）またはrepr(object)の結果を返します。
encodingのデフォルトは'utf-8'です。
errorsのデフォルトは'strict'です。
...
DB-API 2.0型比較用の拡張frozenset。

このクラスはfrozensetを拡張し、DB-API 2.0型比較のセマンティクスをサポートします。
個々の項目を等価演算子と不等価演算子の両方を使用してセットと比較できる、柔軟な型チェックを可能にします。

これはSTRING、BINARY、NUMBERなどの型定数に使用され、field_typeが単一の型値である場合に「field_type == STRING」のような比較を可能にします。

**例**

```pycon
>>> string_types = DBAPISet([FIELD_TYPE.STRING, FIELD_TYPE.VAR_STRING])
>>> FIELD_TYPE.STRING == string_types  # Trueを返します
>>> FIELD_TYPE.INT != string_types     # Trueを返します
>>> FIELD_TYPE.BLOB in string_types    # Falseを返します
```
...
このクラスはfrozensetを拡張し、DB-API 2.0の型比較セマンティクスをサポートします。
等価演算子および不等価演算子の両方を使用して、個々の項目をセットと比較できる
柔軟な型チェックを可能にします。
...
このモジュールは、chDBでユーザー定義関数（UDF）を作成・管理する機能を提供します。カスタムPython関数を記述することで、SQLクエリから呼び出せるchDBの機能を拡張できます。
...
| パラメータ     | 型 | デフォルト値    | 説明                                                             |
| ------------- | ---- | ---------- | ----------------------------------------------------------------------- |
| `return_type` | str  | `"String"` | 関数の戻り値の型。ClickHouseデータ型のいずれかを指定する必要があります |

**注意事項**

1. 関数はステートレスである必要があります。UDFのみがサポートされており、UDAFはサポートされていません。
2. デフォルトの戻り値の型はStringです。戻り値の型はClickHouseデータ型のいずれかである必要があります。
...
この関数は辞書のリストを受け取り、各キーがカラムに対応し、各値がカラム値のリストとなる辞書に変換します。
...
| 戻り値の型            | 説明                                                                 |
| ---------------------- | --------------------------------------------------------------------------- |
| `Dict[str, List[Any]]` | キーがカラム名、値がカラム値のリストとなる辞書 |
...
この関数は値のリストを検査し、リスト内のすべての値を表現できる最も適切なデータ型を決定します。整数、符号なし整数、10進数、浮動小数点型を考慮し、値がいずれの数値型でも表現できない場合、またはすべての値がNoneの場合は「string」をデフォルトとします。
...
| パラメータ     | 型                   | デフォルト    | 説明                                                                    |
| ------------- | ---------------------- | ---------- | ------------------------------------------------------------------------------ |
| `column_data` | `Dict[str, List[Any]]` | _必須_ | キーが列名、値が列の値のリストである辞書 |
...
指定された列から指定された行数を読み取り、オブジェクトのリストを返します。
各オブジェクトは列の値のシーケンスです。
...
| 戻り値の型 | 説明               |
| ----------- | ------------------------- |
| `bytes`     | 最終的なシリアル化されたデータ |

---

#### **abstractmethod** `write` {#write}

データの列をブロックに保存します。サブクラスで実装する必要があります。
...
| パラメータ   | 型              | 説明                                                |
| ----------- | ----------------- | ---------------------------------------------------------- |
| `col_names` | `List[str]`       | 書き込まれる列名のリスト                |
| `columns`   | `List[List[Any]]` | 列データのリスト。各列はリストで表されます |
...
...     print(f"クエリが失敗しました: {e}")
クエリが失敗しました: Table 'non_existent_table' doesn't exist
```
...
イテラブルが指定された場合、タプルはイテラブルの項目から初期化されます。
...
指定されたオブジェクトから新しい文字列オブジェクトを作成します。encodingまたは
errorsが指定された場合、オブジェクトは指定されたエンコーディングとエラーハンドラを使用して
デコードされるデータバッファを公開する必要があります。
それ以外の場合は、object._\_str_\_()の結果（定義されている場合）
またはrepr(object)を返します。
