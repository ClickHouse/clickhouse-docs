...
chDB は、ClickHouse の機能をアプリケーションに直接組み込むためのネイティブな C/C++ API を提供します。この API は、単純なクエリだけでなく、永続接続やクエリ結果のストリーミングなどの高度な機能もサポートします。
