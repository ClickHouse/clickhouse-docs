...
chDBは、ClickHouseの機能をアプリケーションに直接組み込むためのネイティブC/C++ APIを提供します。このAPIは、シンプルなクエリから永続的な接続やストリーミングクエリ結果などの高度な機能までサポートしています。
...
### ステップ2：ヘッダーのインクルード {#include-headers}
...
int main() {
    try {
        ...
        // 集計クエリ
        std::cout << "カウント: " << db.query("SELECT COUNT(*) FROM test") << "\n";
