...
sidebar_label: 'Marimo ノートブックと chDB によるデータ探索'
title: 'Marimo ノートブックと chDB によるデータ探索'
description: 'このガイドでは、ClickHouse Cloud またはローカルファイルのデータを Marimo ノートブックで探索するための chDB の設定および使用方法を説明します'
...
このガイドでは、ClickHouse を基盤とする高速なインプロセス SQL OLAP エンジンである [chDB](/docs/chdb) を利用して、Marimo ノートブック上で ClickHouse Cloud のデータセットを探索する方法を学びます。

**前提条件:**

* Python 3.8 以上
* 仮想環境
* 動作している ClickHouse Cloud サービスと [接続情報](/docs/cloud/guides/sql-console/gather-connection-details)

:::tip
まだ ClickHouse Cloud アカウントをお持ちでない場合は、[サインアップ](https://console.clickhouse.cloud/signUp?loc=docs-marimo-chdb) して
トライアルを開始し、$300 相当の無料クレジットを入手できます。
:::
...
* Marimo のリアクティブな実行モデルを活用してインタラクティブにデータ探索を行う方法

このガイドでは、スターターデータセットの 1 つとして ClickHouse Cloud 上で利用可能な UK Property Price データセットを使用します。
このデータセットには、1995 年から 2024 年までのイギリス国内における住宅の売買価格に関するデータが含まれています。
...
認証情報の漏洩リスクを軽減するため、Cloudのユーザー名とパスワードをローカルマシンの環境変数として追加することを推奨します。
ターミナルから以下のコマンドを実行して、ユーザー名とパスワードを環境変数として追加します:
...
<Image size='md' img={image_5} alt='Marimoのハローワールド' />
...
チャートを動的データフレームの上に移動して、ドロップダウンのあるセルの下に表示されるようにすることができます。
...
箱ひげ図は中央値、四分位数、外れ値を示し、平均価格だけでは得られない詳細な理解を提供します。
...
ここでは集計を行わないことに注意してください - 分布を構築するために、すべての個別の取引データが必要です：
...
```
セルの右上にあるオプションボタンを選択すると、コードを非表示にすることができます。
...
本ガイドでは、Marimoノートブックを使用してchDBでClickHouse Cloud内のデータを探索する方法を説明しました。
...
chDBとMarimoのリアクティブ実行モデルを活用することで、データサイエンティストはClickHouseの強力なSQL機能とPandasやPlotlyなどの使い慣れたPythonツールを組み合わせて利用でき、インタラクティブなウィジェットと自動依存関係追跡により、探索的分析をより効率的かつ再現可能なものにすることができます。
