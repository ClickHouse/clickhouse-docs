...
title: 'mcp-agent と ClickHouse MCP Server を使って AI エージェントを構築する方法'
pagination_prev: null
pagination_next: null
description: 'mcp-agent と ClickHouse MCP Server を使って AI エージェントを構築する方法について学びます'
...
このガイドでは、[ClickHouse の MCP Server](https://github.com/ClickHouse/mcp-clickhouse) を使って
[ClickHouse の SQL playground](https://sql.clickhouse.com/) と対話できる、[mcp-agent](https://github.com/lastmile-ai/mcp-agent) を用いた AI エージェントの構築方法を紹介します。

:::note Example notebook
この例は、[examples リポジトリ](https://github.com/ClickHouse/examples/blob/main/ai/mcp/mcp-agent/mcp-agent.ipynb) 内のノートブックとして参照できます。
:::
...
async with app.run() as mcp_agent_app:
    ...
    data_agent = Agent(
        ...
        instruction="""ClickHouseデータベースを使用して質問に回答できます。""",
    ...
    async with data_agent:
        ...
        result = await llm.generate_str(
            message="2025年の英国の不動産価格について教えてください。ClickHouseを使用して算出してください。"
        )
...
2025-10-10 11:26:51,449 - mcp.server.lowlevel.server - INFO - CallToolRequest型のリクエストを処理中
...
- ClickHouseのUK Price Paidテーブルに基づくと、2025年にこれまで記録された取引は376,633件で、平均価格は£362,283、中央値は£281,000です。データには2025年1月から8月のみが含まれているようです(したがって2025年は不完全です)。平均値を歪める極端な外れ値(最小£100、最大£127,700,000)が存在します。
```



私が行った計算（方法）
ClickHouse の uk.price-paid テーブルに対して集計処理を実行しました:
- uk.uk_price_paid_simple_partitioned を使った 2025 年全体のサマリ（件数、平均、中央値、最小値、最大値）
- 2025 年の月別内訳（取引件数、平均、中央値）
- 2025 年の平均価格が高い上位の町（取引件数が 50 件以上の町）
- 年比較: 2024 年 vs 2025 年（件数、平均、中央値）
...
- 平均価格による上位の町（2025 年、取引件数 ≥50 の町）
  ...
  - EAST MOLESEY、BEACONSFIELD、ESHER、CHALFONT ST GILES、THAMES DITTON もトップ 10 に含まれており、いずれも平均価格の高い通勤・富裕層向けの町です。
- 年比較（2024 年 vs 記録されている 2025 年）
...
- 物件タイプ別（2025 年）
  - detached: 取引 85,362 件, 平均 £495,714, 中央値 £415,000
  - semi-detached: 取引 107,580 件, 平均 £319,922, 中央値 £270,000
  - flat: 取引 62,975 件, 平均 £298,529, 中央値 £227,000
  - terraced: 取引 112,832 件, 平均 £286,616, 中央値 £227,000
  - other: 取引 7,884 件, 平均 £1,087,765（中央値 £315,000）— 少数グループおよび外れ値の影響に注意

重要な注意点とデータ品質に関する補足
- 2025 年のデータセットは一部のみ（1〜8 月）と見られます。したがって「2025 年」の合計値は通年の数字ではありません。
- 大きな外れ値が存在します（例: 最大 £127.7M、および最小 £100）。これらには入力ミスや非標準的なレコードが含まれている可能性があり、平均値を押し上げます。この場合、多くのケースで中央値の方が頑健な指標となります。
- 「other」物件タイプの平均値は、件数が少ないこと・異質なレコード・外れ値のために不安定です。
- is_new、duration などのメタデータによるフィルタリングは行っていません。これらのフィルタ（例えば新築物件やリースホールドを除外するなど）を適用すると結果は変わり得ます。
- テーブルは Price Paid スタイルの取引レコード（記録された売買取引）であり、売出価格や評価額を直接表すものではありません。
...
- 2025年を通じたトレンドを示すため、前月比または3ヶ月移動中央値を計算します。
- 月別の前年同月比（YoY）成長率を算出します（例：2025年3月 vs 2024年3月）。
- 単純な外挿または時系列モデリングを使用して2025年全体の予測を行います（ただし、欠損月/外れ値の処理方法を決定した後の方が望ましい）。

ご希望であれば、以下を実行できます：

- 極端な外れ値を除去した後、同じ集計を再実行し、クリーニングされた結果を表示します。
- 月別YoY成長率とチャートを作成します（CSV形式またはJSON形式の集計データを返却可能で、チャート化できます）。
  次にどれを実行しますか？
