...
sidebar_label: 'Ollama 連携'
title: 'Ollama 対応 ClickHouse MCP サーバーのセットアップ'
pagination_prev: null
pagination_next: null
description: 'このガイドでは、Ollama 対応の ClickHouse MCP サーバーをセットアップする方法を説明します。'
...
> 本ガイドでは、OllamaでClickHouse MCPサーバーを使用する方法を説明します。
...
Mac、Windows、またはLinux向けのOllamaは、[ダウンロードページ](https://ollama.com/download)から入手できます。
...
このコマンドは、モデルが存在しない場合にローカルマシンにダウンロードします。
...
この出力から、デフォルトのqwen3モデルが82億のパラメータを持つことがわかります。


## MCPHostのインストール {#install-mcphost}

本稿執筆時点(2025年7月)では、OllamaをMCPサーバーと併用するネイティブ機能は提供されていません。
ただし、[MCPHost](https://github.com/mark3labs/mcphost)を使用することで、OllamaモデルをMCPサーバーと連携して実行できます。
...
ClickHouse MCPサーバーを設定したら、以下のコマンドを実行してMCPHostを起動できます:

```bash
mcphost --model ollama:qwen3
```

または、特定の設定ファイルを使用する場合:
...
その後、ClickHouse SQLプレイグラウンドで利用可能なデータベースやテーブルについて、モデルに質問できます。

小規模なモデルを使用する場合（デフォルトのqwen3モデルは80億パラメータ）、実行してほしい内容をより具体的に指定する必要があります。
