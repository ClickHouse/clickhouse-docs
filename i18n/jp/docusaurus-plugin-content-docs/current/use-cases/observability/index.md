...
ClickHouse は、オブザーバビリティ用途において、比類のない速度、スケール、そしてコスト効率を実現します。このガイドでは、ニーズに応じて選べる 2 つのアプローチを紹介します。



## ClickStack - ClickHouseオブザーバビリティスタック {#clickstack}

ClickHouse Observability Stackは、ほとんどのユーザーに**推奨されるアプローチ**です。

**ClickStack**は、ClickHouseとOpenTelemetry (OTel)をベースに構築された本番環境対応のオブザーバビリティプラットフォームで、ログ、トレース、メトリクス、セッションを単一の高性能でスケーラブルなソリューションに統合し、シングルノードデプロイメントから**マルチペタバイト**規模まで対応します。
...
| [データ取り込み](/use-cases/observability/clickstack/ingesting-data)    | ClickStackへのデータ取り込みガイドライン     |
...
| [データ管理](/observability/managing-data)                         | オブザーバビリティ用のClickHouseデプロイメントには必ず大規模なデータセットが含まれ、これらを管理する必要があります。ClickHouseはデータ管理を支援する機能を提供しています。 |
