...
このガイドで使用するサンプルデータセットは、パブリックな ClickHouse Playground である [sql.clickhouse.com](https://sql.clickhouse.com) 上にホストされており、ローカルの ClickStack デプロイメントから接続できます。
...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: 空欄のままにしてください
  ...
  <Image img={edit_demo_source} alt="デモソースを編集" size="lg" />
  ...
  右上の時間選択ツールを使用して、過去`1日間`のすべてのデータを表示するように時間範囲を調整します。
  ...
  ユーザーが支払いを完了できないという報告された問題に相関付けられる、最も明白なエラークラスターをクリックします：`Failed to place order`。
  ...
  この問題はインフラストラクチャに関連していないと考えられます。エラー発生前後の期間において、メトリクスに顕著な変化は確認されていません。インフラストラクチャタブを閉じてください。
  ...
  ### より迅速な問題解決のためのイベントデルタ

  イベントデルタは、パフォーマンスやエラー率の変化を特定のデータサブセットに関連付けることで異常を検出し、根本原因の迅速な特定を可能にします。
  ...
  期間が`1ms`を超えるイベントを選択し、`Filter by selection`フィルターを適用すると、「通常」のイベントと約0msの高密度スパングループの違いを分析できます：

  <Image img={step_17} alt="手順 17" size="lg" />

  データのサブセットに対して分析を実行した結果、選択範囲外の「バックグラウンド」スパンのほとんどがVisaトランザクションであり、キャッシュエラーにより0msのレスポンスと関連付けられていることが確認できます。
  ...
  * キャッシュが満杯です
  * これによりリソース消費が増加しました
  * この問題により、Visa決済は完了できなくなるか、少なくとも完了までにかなり時間がかかる状態になっていました。
  ...
  * `Group By`: `<空>`
  ...
  キャッシュサイズが4～5時間かけて増加し (おそらくソフトウェアのデプロイ後) 、最大サイズ`100,000`に到達したことが確認できます。`Sample Matched Events`から、エラーがキャッシュのこの上限到達と相関付けられており、その後サイズが`0`として記録され、レスポンスも`0s`で返されていることがわかります。

  <Image img={step_20} alt="手順 20" size="lg" />
  ...
  * キャッシュが最大サイズに達すると、決済を拒否し、サイズが`0`であると報告しました。
  ...
  たとえば、サポートチームが支払いの問題に遭遇したユーザーのメールアドレス `Ronny.Windler@gmail.com` を提供した場合、ログやトレースを直接検索するよりも、そのユーザーのセッションから始める方が効果的であることが多いです。
  ...
  `SpanAttributes.userEmail: Ronny.Windler` を検索して顧客のセッションを特定します。セッションを選択すると、左側に当該顧客のセッションに関連するブラウザイベントとスパンが表示され、右側にユーザーのブラウザ操作が再現されます:
