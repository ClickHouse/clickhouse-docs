...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: 空欄のままにしてください
  ...
  右上の時間選択ツールを使用して、過去`2日間`のすべてのデータを表示するように時間範囲を調整します。
  ...
  * キャッシュが満杯です
  * これによりリソース消費が増加しました
  * この問題により、Visa決済を完了できなくなるか、少なくとも完了までに非常に長い時間がかかるようになっていました。
  ...
  * キャッシュが最大サイズに達すると、決済を拒否し、自身のサイズを`0`として報告しました。
  ...
  たとえば、サポートチームが支払いの問題に遭遇したユーザーのメールアドレス `Eddie86@yahoo.com` を提供した場合、ログやトレースを直接検索するよりも、そのユーザーのセッションから始める方が効果的であることが多いです。
  ...
  `SpanAttributes.userEmail: Eddie` を検索して顧客のセッションを特定します。セッションを選択すると、左側に当該顧客のセッションに関連するブラウザイベントとスパンが表示され、右側にユーザーのブラウザ操作が再現されます:
