...
:::note
カラムレベルの TTL を指定するには、ユーザー自身でスキーマを定義する必要があります。これは OTel collector 側では指定できません。
:::
