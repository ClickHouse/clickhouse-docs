...
| ClickHouse Embedded | ClickStack UI が ClickHouse サーバーバイナリに直接バンドルされた構成。追加コンポーネントは不要。             | ClickStack の試用、既存の ClickHouse データの探索、デモ  | アラートなし、ダッシュボード/検索の永続化なし、カスタマイズ可能なクエリ設定なし、イベントパターンなし | [ClickHouse Embedded](/use-cases/observability/clickstack/deployment/clickhouse-embedded) |