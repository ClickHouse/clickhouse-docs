...
:::note[TL;DR]
OpenTelemetry Collector の CloudWatch receiver を使用して、AWS CloudWatch ログを ClickStack に転送します。名前付きロググループと自動検出をサポートしています。デモ用データセットと事前構築済みダッシュボードが含まれています。
:::
...
<VerticalStepper headerLevel="h4">
  ...
  **必要なIAM権限：**
  ...
  <details>
    ...
    設定を編集する前に、実際の名前を選べるようにし、リージョンが正しいことも確認できるよう、現在のリージョンに存在するロググループを一覧表示してください:
    ...
    以下の例1の`groups.named`ブロックで、このリストの名前をそのまま使用してください。上記のアカウントの場合、named-groupsセクションは次のようになります:
    ...
    あるいは、対象のグループが共通のプレフィックス (ここでは `/aws-glue/`) を持っている場合は、個別に列挙する代わりに、`prefix: /aws-glue/` を指定した例2を使用してください。
  </details>
