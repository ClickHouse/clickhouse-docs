...
`elasticdump`はデータ移行において以下の利点を提供します:

- Elasticsearch REST APIと直接やり取りし、適切なデータエクスポートを保証します。
...
`elasticdump`が配置されているサーバーにClickHouseが[インストールされている](/install)ことを確認してください。**ClickHouseサーバーは起動しないでください** - これらの手順ではクライアントのみが必要です。
