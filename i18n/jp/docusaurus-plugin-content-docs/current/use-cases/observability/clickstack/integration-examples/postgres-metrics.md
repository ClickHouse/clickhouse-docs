...
```bash
# ゲージメトリクス（接続数、データベースサイズ）をダウンロード {#download-gauge-metrics-connections-database-size}
...
# 積算メトリクス（コミット、ロールバック、各種操作）をダウンロード {#download-sum-metrics-commits-rollbacks-operations}
