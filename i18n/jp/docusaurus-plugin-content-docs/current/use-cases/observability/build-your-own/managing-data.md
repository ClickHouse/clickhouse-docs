...
例として、`otel_logs` テーブルが日単位でパーティション分割されているとします。構造化ログのデータセットでデータが投入されている場合、このテーブルには複数日分のデータが格納されます。
...
`LogAttributes` から新しい列 `Size` を抽出して作成したいとします。`ALTER TABLE` でデフォルト値を指定することで、この列をスキーマに追加できます。
