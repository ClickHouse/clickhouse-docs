...
本ガイドは、ClickHouseオープンソースを評価する企業チームに対して、本番環境デプロイメントにおける監視とオブザーバビリティ機能に関する包括的な情報を提供します。企業のお客様からは、すぐに使える監視機能、DatadogやAWS CloudWatchなどのツールを含む既存のオブザーバビリティスタックとの統合、およびClickHouseの監視とセルフホスト型デプロイメントとの比較について、よくお問い合わせをいただきます。

### Prometheusベースの統合アーキテクチャ {#prometheus}

ClickHouseは、デプロイメントモデルに応じて異なるエンドポイントを通じてPrometheus互換メトリクスを公開します。それぞれに固有の運用特性があります。

**セルフマネージド/OSS ClickHouse**

ClickHouseサーバー上の標準的な/metricsエンドポイントを介してアクセス可能な、サーバー直接のPrometheusエンドポイント。このアプローチは以下を提供します。
