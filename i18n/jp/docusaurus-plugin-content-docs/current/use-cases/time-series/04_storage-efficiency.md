...
Wikipedia の統計データセットに対するクエリ方法を確認したので、次は ClickHouse におけるストレージ効率の最適化に注目していきます。
...
`project`列と`subproject`列を例に見てみましょう。これらの列はString型ですが、ユニーク値の数は比較的少なくなっています:
...
また、`hits`列にはUInt64型を使用していますが、これは8バイトを消費する一方で、最大値は比較的小さくなっています:
...
この値を考慮すると、代わりにUInt32を使用できます。これは4バイトしか消費せず、最大値として約40億まで格納できます:
...
これにより、この列のメモリ上のサイズが少なくとも半分に削減されます。ディスク上のサイズは圧縮により変わらないことに注意してください。ただし、小さすぎるデータ型を選択しないよう注意してください!
...
`time`カラムにはDeltaコーデックを使用しており、これは時系列データに適しています。

適切なソートキーを設定することでディスク容量を節約することもできます。
通常はパスでフィルタリングしたいため、ソートキーに`path`を追加します。
これにはテーブルの再作成が必要です。

以下は、初期テーブルと最適化されたテーブルの`CREATE`コマンドです。
...
最適化されたテーブルは、圧縮形式で約4分の1の容量しか使用していません。
