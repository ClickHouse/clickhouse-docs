...
ClickHouseでの時系列分析は、標準的なSQL集約関数とウィンドウ関数を使用して実行できます。
時系列データを扱う際には、通常、以下の3つの主要なメトリクスタイプがあります。

* 時間とともに単調増加するカウンターメトリクス（ページビューや総イベント数など）
* 上下に変動する時点測定値を表すゲージメトリクス（CPU使用率や温度など）
* 観測値をサンプリングしてバケットごとに集計するヒストグラム（リクエスト時間やレスポンスサイズなど）

これらのメトリクスに対する一般的な分析パターンには、期間間の値の比較、累積合計の計算、変化率の算出、分布の分析などが含まれます。
これらはすべて、集約関数、`sum() OVER`のようなウィンドウ関数、`histogram()`のような特殊関数を組み合わせることで実現できます。
...
次のクエリは、累計を作成する `sum() OVER` 句を使用した例です。`bar()` 関数は増加を視覚的に表現します。
...
時系列データの一般的なユースケースは、追跡されたイベントに基づいてヒストグラムを構築することです。
総ヒット数に基づいてページ数の分布を理解したいとします。ただし、10,000ヒットを超えるページのみを対象とします。
