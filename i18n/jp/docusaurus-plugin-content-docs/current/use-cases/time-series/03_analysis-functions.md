...
title: '分析関数 - 時系列'
sidebar_label: '分析関数'
description: 'ClickHouse における時系列データを分析するための関数。'
...
時系列データを扱う際には、通常、次の 3 種類のメトリクスを扱うことになります。

* 時間とともに単調に増加していくカウンターメトリクス（ページビュー数や総イベント数など）
* 任意の時点における値を表し増減するゲージメトリクス（CPU 使用率や温度など）
* 観測値をサンプリングし、バケットごとにカウントするヒストグラム（リクエストの処理時間やレスポンスサイズなど）

これらのメトリクスに対する一般的な分析パターンには、異なる期間間での値の比較、累積合計の計算、変化率の算出、分布の分析などがあります。
これらはすべて、集約関数の組み合わせ、`sum() OVER` のようなウィンドウ関数、`histogram()` のような専用関数を組み合わせることで実現できます。



## 期間比較による変化 {#time-series-period-over-period-changes}

時系列データを分析する際、期間ごとの値の変化を理解する必要があります。
...
以下のクエリは、"Weird Al" Yankovic の Wikipedia ページの閲覧数における日次変化を計算することで、これを実証しています。
trend 列は、前日と比較してトラフィックが増加したか（正の値）、減少したか（負の値）を示し、異常なスパイクや活動の低下を特定するのに役立ちます。
...
このクエリは、1時間ごとの合計値を1時間の秒数(3600)で除算することで、1秒あたりのページビュー率を計算します。
視覚的なバーにより、アクティビティのピーク時間帯を特定できます。
...
例えば、合計ヒット数が10,000を超えるページのみを対象に、総ヒット数に基づくページ数の分布を把握したい場合を考えてみましょう。
