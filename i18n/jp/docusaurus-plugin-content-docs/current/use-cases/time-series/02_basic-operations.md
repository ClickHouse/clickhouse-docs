...
title: '基本操作 - 時系列'
sidebar_label: '基本操作'
description: 'ClickHouse における基本的な時系列データ処理。'
...
ClickHouse には時系列データを扱うためのさまざまな手法が用意されており、異なる期間にわたってデータポイントを集計・グループ化・分析できます。
このセクションでは、時間ベースのデータを扱う際によく使われる基本的な操作について説明します。

一般的な操作には、時間間隔ごとのデータのグループ化、時系列データにおける欠損区間の処理、期間間の変化量の計算などがあります。
...
ここでは[`toDate()`](/sql-reference/functions/type-conversion-functions#todate)関数を使用しており、これは指定された時刻を日付型に変換します。また、時間単位でグループ化し、特定の日付でフィルタリングすることもできます:
...
多くの場合、一部の時間帯が欠損した疎なデータを扱います。これにより空のバケットが生じます。以下の例では、データを1時間間隔でグループ化します。これにより、一部の時間帯で値が欠損した統計が出力されます：
...
例えば、日単位ではなく、午後6時を起点とした24時間周期のウィンドウで総ヒット数を把握したいとします。
