...
| [Databricks Unity Catalog](/use-cases/data-lake/unity-catalog) | Delta, Iceberg |     ✅ Beta     |    ✅ Beta    | ✅ Beta | [Unity Catalog ガイド](/use-cases/data-lake/unity-catalog)       |
...
| [Microsoft OneLake](/use-cases/data-lake/onelake-catalog)      | Iceberg        |     ✅ Beta     |    ✅ Beta    | ✅ Beta | [OneLake カタログガイド](/use-cases/data-lake/onelake-catalog)       |

現在、すべてのカタログ連携では実験的またはベータ版の設定を有効にする必要があります。Microsoft OneLake と Databricks Unity Catalog を除き、すべてのカタログでは **読み取り専用** アクセスのみが提供されます。つまり、カタログ接続経由でテーブルに対してクエリを実行することはできますが、テーブルを作成したり書き込んだりすることはできません。カタログから ClickHouse にデータをロードして分析を高速化するには、[分析の高速化ガイド](/use-cases/data-lake/getting-started/accelerating-analytics)で説明しているように `INSERT INTO SELECT` を使用します。オープンなテーブル形式へデータを書き戻すには、[データ書き込みガイド](/use-cases/data-lake/getting-started/writing-data)で説明しているように、スタンドアロンの Iceberg テーブルを作成します。