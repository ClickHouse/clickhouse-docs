...
:::note
Unity Catalog との統合は、マネージドテーブルおよび外部テーブルの両方で動作します。
...
Databricks は、レイクハウス向けに複数のデータフォーマットをサポートしています。ClickHouse を使用すると、Unity Catalog テーブルに対して Delta と Iceberg の両方の形式でクエリを実行できます。

:::note
この機能は実験的なため、次の設定を有効にする必要があります。
