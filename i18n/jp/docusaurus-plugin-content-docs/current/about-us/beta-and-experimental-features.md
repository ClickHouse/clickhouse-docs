...
title: 'ベータ機能と実験的機能'
description: 'ClickHouse にはベータ機能および実験的機能があります。このドキュメントページではそれらの定義について説明します。'
slug: /beta-and-experimental-features
doc_type: 'reference'
---

ClickHouse はオープンソースであるため、ClickHouse の従業員だけでなくコミュニティからも多くのコントリビューションを受けています。これらのコントリビューションはしばしば異なるペースで開発されます。ある機能は、一般提供 (GA: Generally Available) と見なされるまでに、長いプロトタイピング段階を要したり、コミュニティからの十分なフィードバックと反復のための時間を必要としたりする場合があります。

どの機能が一般提供として分類されるかの確度が低いため、機能を **ベータ (Beta)** と **実験的 (Experimental)** の 2 つのカテゴリに区分しています。

**ベータ (Beta)** 機能は ClickHouse チームによって正式にサポートされています。**実験的 (Experimental)** 機能は、ClickHouse チームまたはコミュニティのいずれかによって主導される初期プロトタイプであり、公式にはサポートされていません。

以下のセクションでは、**ベータ (Beta)** 機能および **実験的 (Experimental)** 機能の特性について明示的に説明します。
...
- ClickHouseチームがベータ機能をサポートします

以下は、ClickHouse Cloudでベータ版として提供されており、ClickHouse Cloudサービスでご利用いただける機能です。

注：最近導入された機能を使用するには、ClickHouseの[compatibility](/operations/settings/settings#compatibility)設定で最新バージョンを使用していることを確認してください。


## 実験的機能 {#experimental-features}

- GA（一般提供）にならない可能性があります
- 削除される可能性があります
- 破壊的変更が導入される可能性があります
- 機能の動作が変更される可能性があります
- 意図的に有効化する必要があります
- ClickHouseチームは実験的機能を**サポートしていません**
- 重要な機能やドキュメントが不足している可能性があります
- クラウド環境では有効化できません

注意：ClickHouse Cloudでは、上記にベータ版として記載されているもの以外の実験的機能を有効化することはできません。
