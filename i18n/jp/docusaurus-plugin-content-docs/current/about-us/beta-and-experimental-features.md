...
title: 'ベータ機能と実験的機能'
description: 'ClickHouse にはベータ機能と実験的機能があります。このドキュメントページではそれらの定義について説明します。'
slug: /beta-and-experimental-features
doc_type: 'reference'
---

ClickHouse はオープンソースであるため、ClickHouse 社員だけでなくコミュニティからも多くのコントリビューションを受けています。これらのコントリビューションはしばしば異なるスピードで開発されます。ある機能は、一般提供 (GA) とみなされるまでに、長期にわたるプロトタイピングの段階や、コミュニティからの十分なフィードバックと反復のための時間を必要とする場合があります。

どのタイミングで機能を一般提供と分類できるかが明確でないため、機能を **Beta** と **Experimental** の 2 つのカテゴリに区分しています。

**Beta** 機能は、ClickHouse チームによって正式にサポートされる機能です。**Experimental** 機能は、ClickHouse チームまたはコミュニティによって推進される初期段階のプロトタイプであり、正式にはサポートされません。

以下のセクションでは、**Beta** 機能と **Experimental** 機能の特性について明確に説明します。
...
- ClickHouse チームによってベータ機能はサポートされています

以下は、ClickHouse Cloud においてベータと見なされ、ClickHouse Cloud サービスで利用可能な機能です。

注意: 最近導入された機能を使用するには、ClickHouse の [compatibility](/operations/settings/settings#compatibility) 設定で最新のバージョンを使用していることを必ず確認してください。
...
* 一般提供（GA）にならない可能性がある
* 削除される可能性がある
* 後方互換性のない変更が導入される可能性がある
* 将来的に機能が変更される可能性がある
* 明示的に有効化する必要がある
* ClickHouse チームは実験的機能を**サポートしていません**
* 重要な機能やドキュメントが不足している場合がある
* ClickHouse Cloud では有効化できない

注意: 上記で Beta として列挙されているもの以外の追加の実験的機能を ClickHouse Cloud で有効化することは許可されていません。

{/* 以下のタグ内のコンテンツは、ビルド時にソースから生成されるテーブルに置き換えられます。
...
| [merge&#95;selector&#95;algorithm](/operations/settings/merge-tree-settings#merge_selector_algorithm)                                                                                                                 | `シンプル`                  |
