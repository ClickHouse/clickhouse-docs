...
doc_type: 'reference'
keywords: ['ネイティブプロトコル', 'TCPプロトコル', 'クライアント／サーバー', 'プロトコル仕様', 'ネットワーキング']
---
...
`nested`が`false`になるまで、例外のリストが連続する場合があります。
...
:::tip
進捗状況は**差分**で報告されます。合計値を取得するには、クライアント側で累積してください。
:::
