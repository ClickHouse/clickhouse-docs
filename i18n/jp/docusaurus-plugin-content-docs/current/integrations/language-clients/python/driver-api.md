...
:::note
指定可能な引数が多く、そのほとんどがオプションであるため、ほとんどの API メソッドではキーワード引数を使用することを推奨します。
...
| transport&#95;settings | dict             | *None*     | このリクエストに含める HTTP ヘッダー用の任意の辞書です。各キーと値のペアは HTTP ヘッダーとして追加されます (例: `{'X-Custom-Header': 'value'}`) 。プロキシ認証、リクエストのトレーシング、または中間インフラストラクチャで必要なヘッダーの受け渡しに役立ちます。 |
...
| transport&#95;settings      | dict             | *None*     | このリクエストに含める HTTP ヘッダーの任意のディクショナリ。各キーと値のペアは HTTP ヘッダーとして追加されます (例: `{'X-Custom-Header': 'value'}`) 。プロキシ認証、リクエストトレーシング、または中間インフラストラクチャで必要なヘッダーの受け渡しに役立ちます。 |
...
| transport&#95;settings    | dict                              | *None*      | このリクエストに含める HTTP ヘッダーのオプションの辞書です。各キーと値のペアは HTTP ヘッダーとして追加されます (例: `{'X-Custom-Header': 'value'}`) 。プロキシ認証、リクエストのトレース、または中間インフラストラクチャで必要なヘッダーの受け渡しに役立ちます。 |
