...
## ClickHouseネイティブ形式でのエクスポート {#exporting-in-a-native-clickhouse-format}

ClickHouseノード間でデータをエクスポート・インポートする際に最も効率的なデータ形式は[Native](/interfaces/formats/Native)形式です。エクスポートは`INTO OUTFILE`句を使用して行います:
...
データをインポートする場合、小さなファイルや調査目的には[file()](/sql-reference/table-functions/file.md)を使用できます:
...
:::tip
`file()`関数を使用する場合、ClickHouse Cloudではファイルが存在するマシン上で`clickhouse client`からコマンドを実行する必要があります。別の方法として、[`clickhouse-local`](/operations/utilities/clickhouse-local.md)を使用してローカルでファイルを調査することもできます。
:::
...
ここではエクスポートにLZ4圧縮を使用しています。データをインポートする際にも同様に指定する必要があります:
...
複数の値をエクスポートすると破損したファイルが作成されるため、`LIMIT 1`を使用する必要があることに注意してください。
