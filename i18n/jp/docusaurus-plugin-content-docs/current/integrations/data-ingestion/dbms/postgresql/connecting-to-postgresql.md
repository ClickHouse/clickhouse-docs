...
**_以下の手順では、PostgreSQL CLI（psql）とClickHouse CLI（clickhouse-client）を使用します。PostgreSQLサーバーはLinux上にインストールされています。PostgreSQLデータベースが新規テストインストールの場合、以下は最小限の設定となります_**
