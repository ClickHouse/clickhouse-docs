...
処理スループット要件に見合うように、同時にアクティブな分片数を制限することを強く推奨します。"On Demand" の Kinesis ストリームでは、AWS がスループットに基づいて適切な数の分片を自動的に割り当てますが、
"Provisioned" ストリームでは、分片を過剰にプロビジョニングすると、後述するようなレイテンシの原因となるだけでなく、そのようなストリームに対する Kinesis の料金は「分片ごと」の課金となるため、コストも増加します。

プロデューサーアプリケーションが多くのアクティブな分片に継続的に書き込みを行う場合、pipe がそれらの分片を効率的に処理できるだけ十分なスケールになっていないと、レイテンシの原因となる可能性があります。Kinesis のスループット制限に基づき、
ClickPipes は分片データを読み取るためにレプリカごとに一定数の "worker" を割り当てます。例えば、最小サイズでは、1 つの ClickPipes レプリカにはこれらのワーカースレッドが 4 つ存在します。プロデューサーが
同時に 4 つを超える分片に書き込みを行っている場合、ワーカースレッドが空くまで、「追加」の分片からのデータは処理されません。特に、pipe が "enhanced fanout" を使用している場合、各ワーカースレッドは 1 つの
分片に 5 分間サブスクライブし、その間は他の分片を読み取ることができません。このため、5 分単位のレイテンシの「スパイク」が発生する可能性があります。
