...
4. `_resync`テーブルが元のテーブルと入れ替えられます。論理削除された行は、入れ替え前に元のテーブルから`_resync`テーブルに転送されます。

元のClickPipeのすべての設定は再同期ClickPipeに保持されます。元のClickPipeの統計情報はUIからクリアされます。
...
1. データソースタブで、再同期したいMongoDB ClickPipeをクリックします。
2. **設定**タブに移動します。
3. **再同期**ボタンをクリックします。
