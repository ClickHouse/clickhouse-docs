...
description: 'Postgres を ClickHouse Cloud とシームレスに連携します。'
...
ClickPipes を使用して、ソースとなる Postgres データベースから ClickHouse Cloud にデータを取り込むことができます。ソースの Postgres データベースは、オンプレミス環境のほか、Amazon RDS、Google Cloud SQL、Azure Database for Postgres、Supabase などを含むクラウド環境にホストされたものでも利用できます。



## 前提条件 {#prerequisites}

開始するには、まずPostgresデータベースが正しく設定されていることを確認する必要があります。ソースPostgresインスタンスに応じて、以下のいずれかのガイドに従ってください:
...
ソースPostgresデータベースの設定が完了したら、ClickPipeの作成を続行できます。
...
- **Sync interval**: ClickPipesがソースデータベースに変更をポーリングする間隔です。これは宛先ClickHouseサービスに影響を与えるため、コストに敏感なユーザーには、この値を高く設定すること(`3600`以上)を推奨します。
...
- **Pull batch size**: 1回のバッチで取得する行数です。これはベストエフォート設定であり、すべてのケースで尊重されるとは限りません。
...
7. ソースのPostgresデータベースから複製するテーブルを選択できます。テーブルの選択時に、宛先のClickHouseデータベースでのテーブル名の変更や、特定のカラムの除外も可能です。

   :::warning
   ClickHouseで順序キーをPostgresのプライマリキーとは異なる形で定義する場合は、関連するすべての[考慮事項](/integrations/clickpipes/postgres/ordering_keys)を必ず確認してください
   :::
