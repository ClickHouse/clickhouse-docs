---
title: 'Postgres ClickPipe の同期の制御'
description: 'Postgres ClickPipe の同期を制御するドキュメント'
...
このドキュメントでは、ClickPipe が **CDC (Running) モード** で動作している場合に、Postgres ClickPipe の同期を制御する方法について説明します。
...
ソースデータベースでトランザクションが実行されると、ClickPipeはトランザクションのCOMMITを受信するまで待機してから次に進みます。これにより同期間隔とプルバッチサイズの両方が**上書き**されます。
...
既存のClickPipeを編集する際は、パイプの**Settings**タブに移動し、パイプを一時停止してから、ここで**Configure**をクリックします:
