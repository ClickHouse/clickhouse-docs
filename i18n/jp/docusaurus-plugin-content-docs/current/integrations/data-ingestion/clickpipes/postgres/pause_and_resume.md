...
Postgres ClickPipe を一時停止したほうがよいシナリオがいくつかあります。たとえば、既存データを静止した状態で分析したい場合や、Postgres のアップグレード作業を行っている場合などです。ここでは、Postgres ClickPipe を一時停止および再開する方法を説明します。
...
4. **Metrics** タブを開きます。
5. 約 5 秒後（またはページを再読み込みした後）、ClickPipe のステータスが **Paused** になっていることを確認します。
...
1. **Data Sources** タブで、対象の Postgres ClickPipe をクリックします。ミラーのステータスは最初は **Paused** になっているはずです。
2. **Settings** タブに移動します。
...
5. **Metrics** タブに移動します。
6. 約 5 秒後（ページを更新した場合も同様）、ClickPipe のステータスが **Running** になっていることを確認します。