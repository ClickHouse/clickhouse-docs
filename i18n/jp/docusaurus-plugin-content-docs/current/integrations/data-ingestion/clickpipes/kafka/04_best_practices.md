...
* 多数のパーティションを使用すると、レイテンシが増加する可能性があります。ClickPipes では、各レプリカが一度に読み取るパーティション数が、レプリカサイズに応じて 10 (XS) から 240 (XL) までに制限されています。データがない、または少量しかないパーティションが多数ある場合、それらのメッセージの処理が遅延することがあります。
