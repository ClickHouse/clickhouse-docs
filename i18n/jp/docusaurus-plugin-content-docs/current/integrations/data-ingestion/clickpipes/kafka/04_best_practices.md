...
ClickPipes for Kafkaは水平および垂直スケーリングを行うように設計されています。デフォルトでは、1つのコンシューマーを持つコンシューマーグループを作成します。これはClickPipe作成時、または**Settings** -> **Advanced Settings** -> **Scaling**でいつでも設定できます。

ClickPipesはアベイラビリティゾーン分散アーキテクチャによる高可用性を提供します。
