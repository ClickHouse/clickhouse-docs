...
description: 'MySQL を ClickHouse Cloud とシームレスに接続する方法を説明します。'
...
# MySQL から ClickHouse へのデータ取り込み（CDC を使用）
...
ClickPipes を使用して、ソースとなる MySQL データベースから ClickHouse Cloud にデータを取り込むことができます。ソースの MySQL データベースは、オンプレミスでホストされていても、Amazon RDS や Google Cloud SQL などのサービスを利用してクラウド上でホストされていてもかまいません。
...
4. 前提条件の手順で設定したソースMySQLデータベースの接続詳細を入力します。
...
ソースMySQLデータベースが公開アクセス可能でない場合は、SSHトンネリングの詳細を指定できます。
...
- **Pull batch size**: 1回のバッチで取得する行数です。これはベストエフォート設定であり、すべてのケースで保証されるとは限りません。
...
6. ソースMySQLデータベースからレプリケートするテーブルを選択できます。テーブルを選択する際、宛先ClickHouseデータベースでテーブル名を変更したり、特定の列を除外したりすることもできます。
...
MySQLからClickHouse Cloudへのデータレプリケーションを行うClickPipeの設定が完了したら、最適なパフォーマンスを実現するためのデータのクエリ方法とモデリング方法に注力できます。MySQL CDCに関するよくある質問やトラブルシューティングについては、[MySQL FAQsページ](/integrations/data-ingestion/clickpipes/mysql/faq.md)を参照してください。
