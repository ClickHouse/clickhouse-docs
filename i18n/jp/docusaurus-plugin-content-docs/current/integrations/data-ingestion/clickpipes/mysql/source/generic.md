...
description: '任意の MySQL インスタンスを ClickPipes のソースとして構成する'
...
サイドバーにあるサポート対象プロバイダーのいずれかをご利用の場合は、そのプロバイダー専用のガイドを参照してください。
...
2. スキーマ権限を付与します。以下の例は`clickpipes`データベースの権限を示しています。レプリケートする各データベースとホストに対してこれらのコマンドを繰り返してください:
...
**内部認証局** - ITチームからルートCA証明書ファイルを取得してください。ClickPipes UIで新しいMySQL ClickPipeを作成する際にアップロードします。
...
ClickPipeの作成時に必要となるため、MySQLインスタンスのセットアップで使用した接続情報を必ず控えておいてください。
