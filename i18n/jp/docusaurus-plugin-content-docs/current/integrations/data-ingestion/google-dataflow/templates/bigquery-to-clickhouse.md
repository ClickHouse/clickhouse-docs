...
BigQuery から ClickHouse への Dataflow テンプレートは、BigQuery テーブルから ClickHouse テーブルへデータをインポートするバッチ パイプラインです。
このテンプレートは、テーブル全体を読み取ることも、指定した SQL クエリを使用して特定のレコードをフィルター処理することもできます。
...
| `maxInsertBlockSize`    | 挿入用ブロックの作成を制御する場合の、挿入時の最大ブロックサイズ(ClickHouseIOオプション)。                                                                                                                                                                                                                                    |          | `ClickHouseIO`オプション。                                                                                                                                                                                                                                         |
...
BigQueryデータセットをClickHouseに効果的にロードするために、パイプラインは以下の段階でカラム推論処理を実行します：
...
BigQueryの型は、ClickHouseテーブル定義に基づいて変換されます。したがって、以下の表は、
特定のBigQueryテーブル/クエリに対して、ターゲットとなるClickHouseテーブルに設定すべき推奨マッピングを示しています。
...
5. [テンプレートパラメータ](#template-parameters)セクションで説明されているように、BigQuery/ClickHouseIO関連の設定をカスタマイズおよび追加します
...
- DataFlowテンプレートを実行するために必要な設定、構成、および権限をセットアップするには、[このガイド](https://cloud.google.com/dataflow/docs/guides/templates/using-flex-templates#before-you-begin)の`Before you begin`セクションに従ってください。
...
- バッチサイズを減らす：Dataflowジョブ設定でバッチサイズを調整し、ClickHouseに送信するデータチャンクを小さくすることで、バッチあたりのメモリ消費量を削減します。これらの変更により、データ取り込み時のリソース使用量のバランスを取ることができます。
