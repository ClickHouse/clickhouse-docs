...
ClickHouse は、BigQuery からのバッチインポートや、Pub/Sub から ClickHouse へのストリーミング インジェストなど、特定のユースケース向けに設計された[事前定義済みテンプレート](./templates)を提供しています。これらのテンプレートはすぐに利用可能で、連携プロセスを簡素化するため、ノーコードソリューションを好む場合に最適な選択肢です。
