...
sidebar_position: 2
description: 'ユーザーは Google Dataflow Java Runner を使用して ClickHouse にデータを取り込むことが可能です'
...
Dataflow Java Runner を使用すると、カスタム Apache Beam パイプラインを Google Cloud の Dataflow サービス上で実行できます。この方式は柔軟性に最も優れており、高度な ETL ワークフローに適しています。
...
**注意**: このアプローチは、Beamフレームワークに関する知識とコーディングスキルを前提としています。ノーコードソリューションをご希望の場合は、[ClickHouseの事前定義テンプレート](./templates)の使用をご検討ください。
