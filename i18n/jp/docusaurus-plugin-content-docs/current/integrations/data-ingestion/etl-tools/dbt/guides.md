...
5. dbt は、要求どおりモデルを ClickHouse 上のビューとして表現します。これで、このビューに対して直接クエリを実行できます。このビューは `imdb_dbt` データベース内に作成されます。どのデータベースに作成されるかは、`clickhouse_imdb` プロファイル配下の `~/.dbt/profiles.yml` ファイル内にある schema パラメータの設定によって決まります。
   ...
   このビューに対してクエリを実行すると、先ほどのクエリの結果を、よりシンプルな構文で再現できます：
...
4. このテーブルからの結果が、以前の結果と整合していることを確認します。モデルがテーブルになったことで、レスポンス時間が大きく改善している点に注目してください:
    ...
    他にも、このモデルに対して任意のクエリを実行してみてください。たとえば、5 回を超えて出演している俳優のうち、平均ランキングが最も高いのは誰でしょうか。
...
1. まず、モデルをインクリメンタル型に変更します。この変更には次の要件があります:

   1. **unique&#95;key** - アダプターが行を一意に識別できるようにするため、unique&#95;key を指定する必要があります。この例では、クエリ内の `id` フィールドで十分です。これにより、マテリアライズされたテーブル内で行が重複しないことが保証されます。一意性制約の詳細については[こちら](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/configuring-incremental-models#defining-a-uniqueness-constraint-optional)を参照してください。
   2. **Incremental filter** - また、インクリメンタル実行時にどの行が変更されたかを dbt にどのように判定させるかを指定する必要があります。これは delta 式を指定することで行います。通常、これはイベントデータのタイムスタンプを用いて行われ、そのために `updated_at` タイムスタンプフィールドを使用します。行が挿入される際にデフォルトで now() の値が設定されるこのカラムにより、新しいロールを識別できます。加えて、新しい俳優が追加されるケースも識別する必要があります。既存のマテリアライズ済みテーブルを表す `{{this}}` 変数を使用すると、`where id > (select max(id) from {{ this }}) or updated_at > (select max(updated_at) from {{this}})` という式を構成できます。これを `{% if is_incremental() %}` 条件の内部に埋め込むことで、このフィルターがテーブルの初回構築時ではなく、インクリメンタル実行時にのみ使用されるようにします。インクリメンタルモデルにおける行のフィルタリングの詳細については、[dbt ドキュメントのこの解説](https://docs.getdbt.com/docs/building-a-dbt-project/building-models/configuring-incremental-models#filtering-rows-on-an-incremental-run)を参照してください。

   `actor_summary.sql` ファイルを次のように更新してください:
...
3. ここからは、インクリメンタル更新を説明するためにモデルにデータを追加します。`actors` テーブルに俳優「Clicky McClickHouse」を追加します。

   ```sql
   INSERT INTO imdb.actors VALUES (845466, 'Clicky', 'McClickHouse', 'M');
   ```

4. 「Clicky」にランダムな910本の映画で主演してもらいましょう：
...
5. 基盤となるソーステーブルに直接クエリを実行し、dbt モデルを介さずに、彼が現在実際に出演回数が最も多い俳優であることを確認します:
...
インクリメンタルモデルにおける大規模データセットの制約を克服するために、このアダプターは dbt の設定パラメーター `incremental_strategy` を使用します。これは `append` に設定できます。この値を設定すると、更新された行は一時テーブルを作成せずに、直接ターゲットテーブル（`imdb_dbt.actor_summary`）に挿入されます。
注意: Append only モードでは、データが不変であるか、重複が許容される必要があります。更新された行をサポートするインクリメンタルテーブルモデルが必要な場合は、このモードを使用しないでください。

このモードを説明するために、別の新しい俳優を追加し、`incremental_strategy='append'` を指定して `dbt run` を再度実行します。
...
2. さらに有名な俳優 Danny DeBito を追加します
...
### insert\_overwrite mode (experimental) \{#insert_overwrite-mode-experimental\}
...
7. ここでスナップショットに対してクエリを実行してみると、Clicky McClickHouse に対する行が 2 行あることに気付くはずです。以前のエントリには dbt&#95;valid&#95;to の値が設定されています。新しいエントリは、dbt&#95;valid&#95;from カラムに同じ値が記録され、dbt&#95;valid&#95;to の値は null になっています。もし新しい行があれば、それらもスナップショットに追記されます。
...
dbt には、CSV ファイルからデータをロードする機能があります。この機能はデータベースの大規模なエクスポートをロードする用途には適しておらず、コードテーブルや[ディクショナリ](/dictionary)で一般的に使用される小さなファイル向けに設計されています。たとえば、国コードを国名にマッピングする用途などです。簡単な例として、シード機能を使用してジャンルコードの一覧を生成し、アップロードします。
