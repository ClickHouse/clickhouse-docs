...
description: 'Databrain は、ClickHouse とシームレスに統合され、顧客向けダッシュボード、メトリクス、データ可視化を構築するための組み込み型アナリティクスプラットフォームです。'
...
[Databrain](https://usedatabrain.com) は、インタラクティブなダッシュボードや指標、データ可視化を作成して顧客と共有できる埋め込み型アナリティクスプラットフォームです。Databrain は HTTPS インターフェースを使用して ClickHouse に接続し、モダンで使いやすいインターフェースを通じて ClickHouse データを簡単に可視化や分析できるようにします。

<Image size="md" img={databrain_01} alt="ClickHouse データの可視化を表示している Databrain ダッシュボードインターフェース" border />
...
:::tip
許可リストに追加するIPアドレスの最新リストについては、[DatabrainのIPホワイトリスト化ドキュメント](https://docs.usedatabrain.com/guides/datasources/allow-access-to-our-ip)を参照してください。
:::
...
接続に使用するClickHouseユーザーが必要な権限を持っていることを確認します:
...
1. 接続後、Databrain内のワークスペースに移動します。
...
2. 保存されたメトリクスをドラッグアンドドロップして、ダッシュボードにメトリクスを追加します。
...
- [ClickHouse連携ガイド](https://docs.usedatabrain.com/guides/datasources/connecting-data-sources-to-databrain/clickhouse)
