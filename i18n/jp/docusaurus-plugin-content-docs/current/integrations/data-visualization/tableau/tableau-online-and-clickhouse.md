...
Tableau Online は、公式の MySQL データソースを使用し、MySQL インターフェイス経由で ClickHouse Cloud またはオンプレミスの ClickHouse 環境に接続できます。
...
<Image
  ...
  alt='MySQLオプションがハイライトされているTableau Onlineコネクタ選択画面'
  border
/>
<br />
...
ClickHouse CloudインスタンスのMySQLユーザー認証情報と、ダウンロードしたルート証明書のパスを入力します。
