...
## UI で作成したユーザー定義関数 \{#ui-udfs\}

<PrivatePreviewBadge />

ClickHouse Cloud では、UI からユーザー定義関数を作成できるよう設定できます。

:::note
この機能の試用にご興味がある場合は、プライベートプレビューへの登録について [サポート](https://clickhouse.com/support/program) までお問い合わせください。
:::

この例では、特定のタイムスタンプが通常の営業時間内かどうかをチェックする、シンプルな実行可能ユーザー定義関数 `isBusinessHours` を同様に作成します。
前回は SQL を使用して作成しましたが、今回は Python を使用し、UI から設定します。

### Python ファイルを作成する \{#create-python-file\}

ローカルに新しいファイル `main.py` を作成します：
...
4. 変更後のコードを含む zip をアップロードするか、設定を変更してから **Create new version** をクリックします。

これで、UI から最初のユーザー定義関数を追加し、その関数が実行されることを確認し、必要に応じて新しいバージョンを作成する方法を確認できました。