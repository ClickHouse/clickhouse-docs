...
Agent Builder の capabilities セクションで **ウェブ検索** をオンにします。有効にすると、ユーザーの質問とエージェントへの指示に基づいて、いつ検索を実行するかをエージェントが判断します。検索が実行され、結果が収集され、最も関連性の高いコンテンツがモデルコンテキストに渡されます。
