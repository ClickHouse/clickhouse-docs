...
ClickHouse Cloud において、*ウェアハウス* は同じデータを共有するサービスの集合です。
...
2. リフレッシャブルmaterialized view は、ウェアハウス内の読み書き可能（RW）サービス上で**のみ**実行されます。読み取り専用（RO）サービス上では**実行されません**。
...
7. **現在、1 つのウェアハウスあたりサービス数は 5 つまでというソフトリミットがあります。** 1 つのウェアハウス内で 5 つを超えるサービスが必要な場合は、サポートチームにお問い合わせください。
