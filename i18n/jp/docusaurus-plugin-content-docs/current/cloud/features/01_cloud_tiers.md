...
ClickHouse Cloud では複数のティアを利用できます。
ティアは組織単位で割り当てられます。そのため、同じ組織内のサービスはすべて同じティアに属します。
このページでは、どのティアがそれぞれのユースケースに適しているかを説明します。
...
厳格なセキュリティおよびコンプライアンス要件を持つ、大規模でミッションクリティカルなデプロイメント向けです。

* Scale に含まれるすべてに加えて、**さらに**
* 柔軟なスケーリング: 標準プロファイル (`1:4 vCPU:memory ratio`) に加え、`HighMemory (1:8 ratio)` のカスタムプロファイルを利用可能。
* 最高レベルのパフォーマンスとレジリエンス保証を提供します。
* エンタープライズグレードのセキュリティに対応:
  * シングルサインオン (SSO)
  * 強化された暗号化: AWS および GCP サービス向け。サービスはデフォルトで当社のキーにより暗号化されており、ユーザーのキーにローテーションして Customer Managed Encryption Keys (CMEK) を有効化できます。
* スケジュール済みアップグレードに対応: データベースとクラウドの両方のリリースについて、アップグレードを行う曜日および時間帯を選択できます。
* [HIPAA](/cloud/security/compliance-overview#hipaa-since-2024) および PCI コンプライアンスを提供します。

:::note
3 つすべてのティアにおいて、単一レプリカのサービスは固定されたサイズ (`8 GiB`, `12 GiB`) を前提としています。
:::
