...
- APIアドレス「https://api.clickhouse.cloud/v1」を入力し、「Enter」キーを押します:
...
- `baseURL`の下にある「Add new value」の空欄をクリックし、組織IDとサービスIDを入力します:
...
- 「OpenAPI spec for ClickHouse Cloud」の下で、フォルダ > V1 > organizationsを展開します
...
- 中央フレームメニューのParamsの下で、`organizationid`が必要です。
