...
title: 'API キーの管理'
description: 'ClickHouse Cloud は OpenAPI を利用した API を提供しており、アカウントおよびサービスのさまざまな側面をプログラムによって管理できます。'
doc_type: 'guide'
...
ClickHouse Cloud は OpenAPI を利用した API を提供しており、アカウントやサービスの各種項目をプログラムから管理できます。
...
1. 左側のメニューの **API Keys** タブを使用して、API キーを作成および管理できます。

<Image img={image_01} size="sm" alt="API Keys タブ" border />

2. **API Keys** ページには、最初に以下のような「最初の API キーを作成する」ためのプロンプトが表示されます。最初のキーを作成した後は、右上に表示される `New API Key` ボタンを使用して新しいキーを作成できます。
...
:::note
権限は ClickHouse Cloud の[事前定義ロール](/cloud/security/console-roles)に対応しています。`developer` ロールは対象サービスに対する読み取り専用権限を持ち、`admin` ロールは読み取りおよび書き込みのフル権限を持ちます。
:::
...
5. ClickHouse Cloud API は、[HTTP Basic Authentication](https://developer.mozilla.org/en-US/docs/Web/HTTP/Authentication) を使用して API キーの有効性を検証します。次に示すのは、`curl` を使用して ClickHouse Cloud API にリクエストを送信する際の API キーの使用例です。
...
6. **API Keys** ページに戻ると、キー名、Key ID の末尾4桁、権限、ステータス、有効期限、作成者が表示されます。この画面から、キー名・権限・有効期限を編集できます。また、この画面からキーを無効化したり削除したりすることもできます。

<br />

:::note
API キーの削除は取り消しできない操作です。そのキーを使用しているサービスは、直ちに ClickHouse Cloud へのアクセス権を失います。
:::
