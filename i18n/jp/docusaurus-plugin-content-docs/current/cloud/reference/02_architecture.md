...
- 実質無制限のストレージ容量
- データの手動共有が不要
- データ保存コストの大幅な削減、特にアクセス頻度の低いデータにおいて効果的
...
- セットアップ、モニタリング、バックアップ、および請求処理は自動的に実行されます。
...
GCPとAzureの場合、サービスはオブジェクトストレージの分離を備えています(すべてのサービスが独自のバケットまたはストレージコンテナを持ちます)。


## コンピュート間分離 {#compute-compute-separation}

[コンピュート間分離](/cloud/reference/warehouses)により、ユーザーは複数のコンピュートノードグループを作成できます。各グループは独自のサービスURLを持ち、すべて同じ共有オブジェクトストレージを使用します。これにより、同じデータを共有しながら、読み取りと書き込みなどの異なるユースケースでコンピュートを分離できます。また、必要に応じてコンピュートグループを独立してスケーリングできるため、より効率的なリソース活用が実現します。
