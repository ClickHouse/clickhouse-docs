...
このページでは、ClickHouse Cloudのディザスタリカバリに関する推奨事項と、障害発生時の復旧手順について説明します。
ClickHouse Cloudは現在、自動フェイルオーバーや複数の地理的リージョン間での自動同期には対応していません。

:::tip
お客様のサービス規模と構成における具体的なRTOを把握するため、定期的にバックアップからのリストアテストを実施してください。
:::
...
**RPO (Recovery Point Objective、目標復旧時点)**: 障害発生後に許容される最大データ損失を時間で測定したもの。例: RPOが30分の場合、障害発生時にデータベースは30分以内のデータまで復元可能である必要があります。これは当然、バックアップの取得頻度に依存します。

**RTO (Recovery Time Objective、目標復旧時間)**: 障害発生後、通常運用を再開するまでに許容される最大ダウンタイム。例: RTOが30分の場合、障害発生時にチームは30分以内にデータとアプリケーションを復元し、通常運用を再開できる必要があります。

**データベースバックアップとスナップショット**: バックアップは、データの別コピーを用いた耐久性のある長期ストレージを提供します。スナップショットはデータの追加コピーを作成せず、通常より高速で、より優れたRPOを提供します。
...
プライマリサービスのデータが破損した場合、バックアップを使用して新しいサービスへ復元できます。
...
:::note
この機能は現在、PCI/HIPAAサービスでは利用できません
:::
...
バックアップはサイズが大きくなるほどスケールメリットが働き、一部の内部サービスでは最大1PBのバックアップが10時間未満で完了することを確認しています。
...
## リカバリプロセス {#recovery-process}

このセクションでは、各ケースで利用可能なリカバリオプションとその手順について説明します。

### プライマリサービスのデータ破損 {#primary-service-data-corruption}

この場合、データはバックアップから同じリージョン内の別のサービスに[復元できます](/cloud/manage/backups/overview#restore-a-backup)。
...
3. 新しいサービスに名前を付けて、このバックアップから復元します
...
なお、リージョン間のデータ転送料金が発生することにご注意ください。

プライマリリージョンがダウンした場合、別のリージョンにあるバックアップを異なるリージョンの新しいサービスに復元できます。

バックアップを別のサービスに復元した後は、DNS、ロードバランサー、または接続文字列の設定を更新して、新しいサービスを参照するようにする必要があります。
これには以下が含まれる場合があります:
