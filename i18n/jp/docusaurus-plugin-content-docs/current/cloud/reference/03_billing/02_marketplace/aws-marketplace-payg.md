...
description: 'AWS Marketplace（PAYG）経由で ClickHouse Cloud を契約します。'
...
[AWS Marketplace](https://aws.amazon.com/marketplace) の PAYG（従量課金）パブリックオファーから ClickHouse Cloud の利用を開始しましょう。
...
### アカウントの設定 {#set-up-your-account}

この時点では設定は完了しておらず、ClickHouse Cloud組織はまだマーケットプレイス経由で請求されていないことに注意してください。マーケットプレイスサブスクリプションの「アカウントを設定」をクリックして、ClickHouse Cloudにリダイレクトし、設定を完了する必要があります。

<Image img={aws_marketplace_payg_4} alt='アカウントの設定' border />

ClickHouse Cloudにリダイレクトされたら、既存のアカウントでログインするか、新しいアカウントで登録できます。この手順は、ClickHouse Cloud組織をAWS Marketplaceの請求に紐付けるために非常に重要です。
...
<Image img={aws_marketplace_payg_6} size='md' alt='開始前の確認' />

<Image img={aws_marketplace_payg_7} size='md' alt='開始前の確認(続き)' />

</details>

既存のClickHouse Cloudユーザーの場合は、認証情報を使用してログインするだけです。

### 組織にマーケットプレイスサブスクリプションを追加 {#add-marketplace-subscription}
...
この手順を完了すると、組織がこのAWSサブスクリプションに接続され、すべての使用量がAWSアカウント経由で請求されます。
