...
- **Data Catalog 連携:** 外部のデータカタログをデータソースとして接続できるようになりました。ClickHouse Cloud は AWS Glue と Unity Catalog（今後さらに追加予定）をサポートしています。接続が完了すると、カタログはデータベースとして表示され、データを複製することなく Iceberg テーブルを直接読み取ることができます。ご利用を希望される場合は、サポートにお問い合わせのうえ、アクセスをリクエストしてください。
- **ClickPipes:**
  - MongoDB コネクタは Public Beta に昇格し、すべてのサービスティアで、新規および既存の ClickHouse Cloud のお客様にご利用いただけるようになりました。新機能の概要については[ブログ記事](https://clickhouse.com/blog/mongodb-cdc-connector-clickpipes-beta)を参照し、[ドキュメント](/integrations/clickpipes/mongodb)を確認してセットアップを開始してください。
  - [S3 ClickPipe](/integrations/clickpipes/object-storage/s3/overview) は、OVHcloud における S3 API 互換オブジェクトストレージサービスである OVH Object Storage と互換性を持つようになりました。
