...
- ソート句`ORDER BY ALL`(v23.12で導入)は`ORDER BY *`に置き換えられました。以前の構文は、`all`という列を持つテーブルに対してエラーが発生しやすいものでした。[#59450](https://github.com/ClickHouse/ClickHouse/pull/59450) ([Robert Schulze](https://github.com/rschu1ze))。
...
- `file/s3/hdfs/url/azureBlobStorage`エンジンでスキーマ推論中にファイル形式が不明な場合、自動的に検出を試みるようにしました。[#50576](https://github.com/ClickHouse/ClickHouse/issues/50576)を解決します。[#59092](https://github.com/ClickHouse/ClickHouse/pull/59092) ([Kruglov Pavel](https://github.com/Avogar))。
...
- 空のパーティションに関連するZooKeeperノードを削除するクエリ`ALTER TABLE table FORGET PARTITION partition`を追加しました。[#59507](https://github.com/ClickHouse/ClickHouse/pull/59507) ([Sergei Trifonov](https://github.com/serxa))。
- tarアーカイブとしてのバックアップの読み取りと書き込みをサポートしました。[#59535](https://github.com/ClickHouse/ClickHouse/pull/59535) ([josh-hildred](https://github.com/josh-hildred))。
...
- コーデック`LZ4HC`は新しいレベル2を受け入れるようになりました。これは以前の最小レベル3よりも高速ですが、圧縮率は低くなります。以前のバージョンでは、`LZ4HC(2)`以下は`LZ4HC(3)`と同じでした。作成者: [Cyan4973](https://github.com/Cyan4973)。[#60090](https://github.com/ClickHouse/ClickHouse/pull/60090) ([Alexey Milovidov](https://github.com/alexey-milovidov))。
...
- テーブルのプライマリキーにほとんど無用なカラムが含まれている場合、それらをメモリに保持しません。これは新しい設定`primary_key_ratio_of_unique_prefix_values_to_skip_suffix_columns`によって制御され、デフォルト値は`0.9`です。これは、複合プライマリキーにおいて、カラムが全体の少なくとも0.9の割合で値を変更する場合、その後のカラムはロードされないことを意味します。[#60255](https://github.com/ClickHouse/ClickHouse/pull/60255) ([Alexey Milovidov](https://github.com/alexey-milovidov))。
...
* wingfuzz によって発見された KQL の問題を修正 [#59626](https://github.com/ClickHouse/ClickHouse/pull/59626) ([Yong Wang](https://github.com/kashwy)).
...
* rabbitmq: ackもnackもされないメッセージが残存する問題を修正 [#59775](https://github.com/ClickHouse/ClickHouse/pull/59775) ([Kseniia Sumarokova](https://github.com/kssenii)).
* analyzer での `const` および `LowCardinality` に対する関数実行と、`GROUP BY const` の組み合わせに関する不具合を修正しました [#59986](https://github.com/ClickHouse/ClickHouse/pull/59986)（[Azat Khuzhin](https://github.com/azat)）。
...
* MergeTree 向けの `finished_mutations_to_keep=0` の動作を修正（ドキュメントでは 0 はすべてを保持することを意味するとしている）[#60031](https://github.com/ClickHouse/ClickHouse/pull/60031)（[Azat Khuzhin](https://github.com/azat)）。
* `DROP` 時に `s3queue` テーブルで発生する可能性のある例外を修正 [#60036](https://github.com/ClickHouse/ClickHouse/pull/60036) ([Kseniia Sumarokova](https://github.com/kssenii))。
* PartsSplitter における同一パートに対する不正な範囲 [#60041](https://github.com/ClickHouse/ClickHouse/pull/60041)（[Maksim Kita](https://github.com/kitaisreal)）。
* ハードコードされた 4096 の代わりに、DDLLogEntry でコンテキストからの max&#95;query&#95;size を使用するよう変更 [#60083](https://github.com/ClickHouse/ClickHouse/pull/60083)（[Kruglov Pavel](https://github.com/Avogar)）。
...
* 文字列表現の bool を真の bool 値にキャストできるようになりました [#60160](https://github.com/ClickHouse/ClickHouse/pull/60160) ([Robert Schulze](https://github.com/rschu1ze)).
* `system.s3queue_log` の問題を修正 [#60166](https://github.com/ClickHouse/ClickHouse/pull/60166) ([Kseniia Sumarokova](https://github.com/kssenii)).
* NULL 許容の集約関数名を使用する arrayReduce を修正 [#60188](https://github.com/ClickHouse/ClickHouse/pull/60188) ([Raúl Marín](https://github.com/Algunenano)).
...
* LRUResource Cache のバグ（Hive キャッシュ）を修正 [#60262](https://github.com/ClickHouse/ClickHouse/pull/60262) ([shanfengp](https://github.com/Aed-p))。
* s3queue: バグを修正（フレークしていたテスト test&#95;storage&#95;s3&#95;queue/test.py::test&#95;shards&#95;distributed も修正） [#60282](https://github.com/ClickHouse/ClickHouse/pull/60282) ([Kseniia Sumarokova](https://github.com/kssenii)).
* IPv6 を扱うハッシュ関数における未初期化値の使用と無効な結果の問題を修正 [#60359](https://github.com/ClickHouse/ClickHouse/pull/60359) ([Kruglov Pavel](https://github.com/Avogar))。
* 並列レプリカの変更時に再解析を強制する [#60362](https://github.com/ClickHouse/ClickHouse/pull/60362) ([Raúl Marín](https://github.com/Algunenano)).
...
* max&#95;parallel&#95;replicas を 0 に設定できないようにしました。意味をなさない設定であるためです [#60430](https://github.com/ClickHouse/ClickHouse/pull/60430) ([Kruglov Pavel](https://github.com/Avogar)).
...
* CREATE TABLE でスカラーサブクエリの計算を可能な限り回避するようにしました。 [#60464](https://github.com/ClickHouse/ClickHouse/pull/60464) ([Nikolai Kochetov](https://github.com/KochetovNicolai)).
...
* s3Cluster 内のキー検査を正しく行うよう修正 [#60477](https://github.com/ClickHouse/ClickHouse/pull/60477) ([Antonio Andelic](https://github.com/antonio2368)).
* 多数の行がエラーによりスキップされる場合に発生する並列パースのデッドロックを修正 [#60516](https://github.com/ClickHouse/ClickHouse/pull/60516) ([Kruglov Pavel](https://github.com/Avogar))。
* kql の複合演算子に対する max&#95;query&#95;size の修正: [#60534](https://github.com/ClickHouse/ClickHouse/pull/60534) ([Yong Wang](https://github.com/kashwy)).
...
* 日付型については数値ヒントを出力しないようにしました [#60577](https://github.com/ClickHouse/ClickHouse/pull/60577) ([Raúl Marín](https://github.com/Algunenano)).
* フィルタで非決定論的関数を使用する場合の MergeTree からの読み取りを修正 [#60586](https://github.com/ClickHouse/ClickHouse/pull/60586) ([Kruglov Pavel](https://github.com/Avogar))。
* 不正な互換性設定値型に起因する論理エラーを修正 [#60596](https://github.com/ClickHouse/ClickHouse/pull/60596) ([Kruglov Pavel](https://github.com/Avogar)).
* 混在する x86-64 / ARM クラスターにおける集約関数状態の不一致を修正 [#60610](https://github.com/ClickHouse/ClickHouse/pull/60610) ([Harry Lee](https://github.com/HarryLeeIBM))。
...
* SQL/JSON から無意味な記述を削除 [#60738](https://github.com/ClickHouse/ClickHouse/pull/60738) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* 集約関数 quantileGK における誤った sanitize チェックを削除しました [#60740](https://github.com/ClickHouse/ClickHouse/pull/60740)（[李扬](https://github.com/taiyang-li)）。
...
* サブクエリで `allow_experimental_analyzer` の値が異なる場合に発生していたクラッシュを修正 [#60770](https://github.com/ClickHouse/ClickHouse/pull/60770)（[Dmitry Novik](https://github.com/novikd)）。
* S3 からの読み取り時の再帰を削除 [#60849](https://github.com/ClickHouse/ClickHouse/pull/60849) ([Antonio Andelic](https://github.com/antonio2368)).
* HashedDictionaryParallelLoader でエラー発生時にハングする可能性があった問題を修正しました [#60926](https://github.com/ClickHouse/ClickHouse/pull/60926) ([vdimir](https://github.com/vdimir))。
...
* RangeHashedDictionary 向けの dictGetOrDefault におけるデフォルト引数の遅延評価を修正 [#61196](https://github.com/ClickHouse/ClickHouse/pull/61196) ([Kruglov Pavel](https://github.com/Avogar)).
...
* スタンドアロンバイナリ用 Keeper の再設定を修正 [#61233](https://github.com/ClickHouse/ClickHouse/pull/61233)（[Antonio Andelic](https://github.com/antonio2368)）。
...
* 集約関数 `uniqExact` が誤った結果を返す可能性のある不具合を修正 [#61257](https://github.com/ClickHouse/ClickHouse/pull/61257) ([Anton Popov](https://github.com/CurtizJ))。
* show database に関する不具合を修正しました [#61269](https://github.com/ClickHouse/ClickHouse/pull/61269) ([Raúl Marín](https://github.com/Algunenano))。
* MATERIALIZED カラムを使用した RabbitMQ ストレージにおける論理エラーを修正 [#61320](https://github.com/ClickHouse/ClickHouse/pull/61320) ([vdimir](https://github.com/vdimir))。
...
* 失敗した RESTORE の後処理を修正 [#61466](https://github.com/ClickHouse/ClickHouse/pull/61466) ([Vitaly Baranov](https://github.com/vitlibar)).
...
* ウィンドウビューで発生するクラッシュを修正 [#61526](https://github.com/ClickHouse/ClickHouse/pull/61526)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* ネイティブでない整数型での `repeat` の不具合を修正 [#61527](https://github.com/ClickHouse/ClickHouse/pull/61527) ([Antonio Andelic](https://github.com/antonio2368)).
...
* 位置を定数として指定した文字列検索を修正 [#61547](https://github.com/ClickHouse/ClickHouse/pull/61547) ([Antonio Andelic](https://github.com/antonio2368)).
* DateTime64 で使用した際にエラーが発生していた addDays を修正 [#61561](https://github.com/ClickHouse/ClickHouse/pull/61561)（[Shuai li](https://github.com/loneylee)）。
* 非同期インサートでの重複排除時の `system.part_log` を修正 [#61620](https://github.com/ClickHouse/ClickHouse/pull/61620)（[Antonio Andelic](https://github.com/antonio2368)）。
* system.parts に対する Non-ready set を修正。[#61666](https://github.com/ClickHouse/ClickHouse/pull/61666)（[Nikolai Kochetov](https://github.com/KochetovNicolai)）。
