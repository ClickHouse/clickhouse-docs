...
ClickHouse Cloud コンソールの Managed Postgres インスタンス画面にある **監視** タブから、CPU、メモリ、IOPS、ストレージ使用量を直接監視できます。

さらに、**クエリインサイト** タブでは、クエリの詳細な分析のために [Query Performance Insights](https://clickhouse.com/blog/postgres-query-insights-clickhouse-cloud) を参照できます。
...
Managed Postgres に対する Terraform のサポートは現在提供されていません。インスタンスの作成と管理には ClickHouse Cloud コンソールまたは [OpenAPI](openapi.md) の利用を推奨します。

## 拡張機能と設定 \{#extensions-and-configuration\}

### どの拡張機能がサポートされていますか？ \{#extensions-supported\}

Managed Postgres には、PostGIS、pgvector、pg&#95;cron などのよく利用されているものを含む、90 を超える PostgreSQL 拡張機能が含まれています。利用可能な拡張機能の一覧とインストール手順については、[拡張機能](/cloud/managed-postgres/extensions) ドキュメントを参照してください。
...
この仕組みにより、`PREPARE` (または拡張クエリの `Parse`) を実行した特定のバックエンドに紐づく **サーバー側プリペアドステートメント** は機能しません。対応する `EXECUTE` が別のバックエンドに送られると、次のようなエラーが発生します。
...
* **PgBouncer `max_client_conn`** は、プーラー内で同時に開ける **クライアント** 接続数の上限です。PgBouncer は、多数のクライアント接続を、はるかに少ない数のバックエンド接続に多重化します。
