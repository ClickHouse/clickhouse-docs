...
データマスキングはデータ保護のための手法であり、元のデータの形式と構造は保ったまま、あらゆる個人を特定できる情報 (PII) や機密情報を取り除いたデータに置き換えることを指します。
...
基本的なデータマスキングのユースケースでは、`replace`ファミリーの関数を使用してデータを便利にマスクできます:
...
上記のクエリでは、`\3`を使用して3番目のキャプチャグループを結果の文字列に代入しており、次のような結果が生成されます:
...
[`VIEW`](/sql-reference/statements/create/view)は、前述の文字列関数と組み合わせて使用することで、機密データを含むカラムに変換を適用し、ユーザーに提示する前にマスク化できます。
この方法では、元のデータは変更されず、ビューをクエリするユーザーにはマスク化されたデータのみが表示されます。
...
ビューから返されるデータが一部マスクされており、機微な情報が難読化されていることに注意してください。
閲覧者が持つ情報への権限レベルに応じて難読化のレベルを変えた複数のビューを作成することもできます。

ユーザーがマスク済みデータを返すビューにのみアクセスでき、元のマスクされていないデータを保持するテーブルにはアクセスできないようにするには、[Role Based Access Control](/cloud/security/console-roles) を使用して、特定のロールにはそのビューに対する `SELECT` 権限のみが付与されるようにします。
...
次に、そのビューに対する `SELECT` 権限をロールに付与します。

```sql
GRANT SELECT ON masked_orders TO masked_orders_viewer;
```

ClickHouse のロールは加算的に適用されるため、マスクされたビューのみを参照できるべきユーザーが、いかなるロール経由であってもベーステーブルに対する `SELECT` 権限を持たないようにする必要があります。

そのため、安全を期すためにベーステーブルへのアクセス権を明示的に取り消すべきです。
...
これにより、`masked_orders_viewer` ロールを持つユーザーはビュー上のマスク済みデータのみを参照でき、テーブル上の元の非マスクデータを参照することはできません。


## `MATERIALIZED`カラムとカラムレベルのアクセス制限を使用する {#materialized-ephemeral-column-restrictions}

別のビューを作成したくない場合は、マスク化されたバージョンのデータを元のデータと並行して保存できます。
...
ユーザーがマスクされたデータを含む列にのみアクセスできるようにするには、再度 [Role Based Access Control](/cloud/security/console-roles) を使用して、特定のロールに `orders` のマスクされた列に対する `SELECT` 権限のみを付与するように設定します。
...
マスクされていない機密カラムを [`EPHEMERAL`](/sql-reference/statements/create/table#ephemeral) としてマークできます。
これにより、この種のカラムがテーブルに保存されないことが保証されます。
...
先ほどと同じクエリを実行すると、テーブルに挿入されているのはマテリアライズされたマスク済みデータだけであることが分かります。
...
ログデータを特にマスキングしたいClickHouse OSSユーザーは、[クエリマスキングルール](/operations/server-configuration-parameters/settings#query_masking_rules)（ログマスキング）を使用してデータをマスキングできます。
...
なお、クエリ結果のデータはマスキングされないことに注意してください。
