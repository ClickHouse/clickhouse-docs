...
:::important
ClickHouse CloudサービスのIPアクセスリストの作成を省略した場合、サービスへのトラフィックは一切許可されません。ClickHouseサービスのIPアクセスリストを`Allow from anywhere`に設定している場合、パブリックIPを探索するインターネットクローラーやスキャナーによって、サービスがアイドル状態からアクティブ状態に定期的に移行される可能性があり、わずかながら予期しないコストが発生することがあります。
:::
...
フィルタを作成したら、指定した範囲内からサービスへの接続が可能であることを確認し、許可された範囲外からの接続が拒否されることを確認してください。簡単な `curl` コマンドで検証できます：

```bash title="許可リスト外からのアクセスは拒否される"
...
```bash title="許可リスト内からのアクセスは許可される"
curl https://<HOSTNAME>.clickhouse.cloud:8443
```
