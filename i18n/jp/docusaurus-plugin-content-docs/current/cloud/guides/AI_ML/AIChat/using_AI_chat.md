...
## データ利用同意の承諾（初回） \{#consent\}

1. 初回利用時は、データの取り扱いおよびサードパーティ LLM サブプロセッサに関する説明を含む同意ダイアログが表示されます。
2. 内容を確認し、同意して続行します。拒否した場合、パネルは開きません。
...
Agent モードでは、展開可能な中間の「思考」またはプランニングステップが表示される場合があります。これは、アシスタントがどのように回答を形成しているかを可視化するためのものです。必要に応じて折りたたんだり展開したりできます。

<Image img={img_thinking} alt="思考ステップ" size="md"/>

## 新しいチャットを開始する \{#new-chats\}

コンテキストをクリアして新しいセッションを開始するには、「New Chat」ボタンをクリックします。

## チャット履歴を表示する \{#history\}

1. 下部のセクションに最近のチャットが一覧表示されます。
2. 以前のチャットを選択して、そのメッセージを読み込みます。
3. ゴミ箱アイコンを使用して会話を削除します。

<Image img={img_history} alt="チャット履歴リスト" size="md"/>

## 生成された SQL の扱い \{#sql-actions\}
