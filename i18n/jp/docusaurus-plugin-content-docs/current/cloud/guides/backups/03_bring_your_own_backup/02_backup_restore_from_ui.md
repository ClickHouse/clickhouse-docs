...
# ユーザーインターフェースを使用したバックアップ/リストア {#ui-experience}
...
新しく作成したサービスのARN(ClickHouse Cloudコンソールのサービス設定ページから取得)をIAMロールの信頼ポリシーに追加します。これは上記のAWS手順セクションの[ステップ2](#create-iam-role)と同じです。新しいサービスがS3バケットにアクセスできるようにするために必要です。
...
ポップアップダイアログで、前のセクションで作成したGCPバケットパス、HMACキー、シークレットを入力します。
...
新しく作成されたサービスのSQLコンソールからリストアコマンドを実行して、バックアップをリストアします。
...
- b. `key1`と`key2`が表示されます。各キーの下に`Connection string`フィールドがあります。
