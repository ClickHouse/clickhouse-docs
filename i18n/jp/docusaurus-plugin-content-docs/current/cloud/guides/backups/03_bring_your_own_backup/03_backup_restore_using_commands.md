---
sidebar_label: 'コマンドで行うバックアップとリストア'
slug: /cloud/manage/backups/backup-restore-via-commands
title: 'コマンドを使用してバックアップを作成またはリストアする'
description: '独自のバケットを使用し、コマンドでバックアップを作成またはリストアする方法を説明するページ'
sidebar_position: 3
doc_type: 'guide'
...
`BACKUP` および `RESTORE` コマンドを使用すると、[ユーザーインターフェイス経由](/cloud/manage/backups/backup-restore-via-ui)でバックアップまたはリストアを行うことに加えて、
バックアップを保存先のバケットにエクスポートできます。
