...
## 標準オンボーディング とは何ですか？ \{#what-is-standard-onboarding\}
...
標準オンボーディング では、お客様は専用の AWS アカウント／GCP プロジェクトを用意し、CloudFormation または Terraform で初期スタックを実行して、ClickHouse Cloud が以降のセットアップをオーケストレーションするために必要な最小限の IAM 権限と信頼ポリシーを作成するだけです。その後のすべての手順 (インフラストラクチャのプロビジョニングやサービスの起動を含む) は、ClickHouse Cloud の ウェブコンソールを通じて管理されます。
