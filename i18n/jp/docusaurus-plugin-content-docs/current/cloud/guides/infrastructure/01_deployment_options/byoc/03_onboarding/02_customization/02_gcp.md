...
1. ClickHouse Kubernetes (GKE) クラスタ用に、[ClickHouse BYOC のサポート対象リージョン](/cloud/reference/byoc/supported-regions)で、少なくとも 1 つのプライベートサブネットを割り当てます。GKE クラスタノードに十分な IP アドレスを確保できるように、サブネットの CIDR 範囲は最低でも `/24` (例: 10.0.0.0/24) であることを確認してください。
