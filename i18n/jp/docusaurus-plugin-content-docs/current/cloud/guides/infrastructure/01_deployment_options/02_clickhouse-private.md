...
## オンボーディング プロセス \{#onboarding-process\}

ClickHouse Private に関心のあるお客様は、ユースケースの確認のためのレビューコールを設定するには、[お問い合わせ](https://clickhouse.com/company/contact)ください。最小規模要件を満たし、サポート対象の構成を前提とするリクエストが検討対象となります。なお、オンボーディングの対応可能件数には限りがあります。セットアップでは、AWS ECR から取得したイメージと Helm チャートを使用し、環境ごとのインストールガイドに従って進めます。