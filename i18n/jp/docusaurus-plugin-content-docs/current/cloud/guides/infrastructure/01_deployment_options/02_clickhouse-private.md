...
description: 'ClickHouse Private サービス概要'
doc_type: 'reference'
---
...
### 多様なユースケースと条件下での検証済み {#tested-proven-through-variety-of-use-cases}
...
お客様は[弊社](https://clickhouse.com/company/contact?loc=nav)にお問い合わせいただくことで、オンボーディングを開始できます。条件を満たすお客様には、詳細な環境構築ガイドと、デプロイ用のイメージおよびHelmチャートへのアクセスを提供いたします。
...
- サーバーノードはUbuntuで実行
- Operatorにはx86ノードグループが必要
- Cloud Kubernetes Serviceクラスターと同じリージョンにある[Cloud Object Storage](https://cloud.ibm.com/docs/cloud-object-storage?topic=cloud-object-storage-getting-started-cloud-object-storage)
- Ingressが必要な場合は、NLBも設定
