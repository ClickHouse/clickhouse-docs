---
title: Pandas DataFramesを使用したchDBのクエリ方法
sidebar_label: Pandasのクエリ
slug: /chdb/guides/pandas
description: Pandas DataFramesをchDBでクエリする方法を学びましょう
keywords: [chdb, pandas]
---

[Pandas](https://pandas.pydata.org/)は、Pythonにおけるデータ操作と分析のための人気ライブラリです。  
chDBのバージョン2では、Pandas DataFramesのクエリ性能を改善し、`Python`テーブル関数を導入しました。  
...
次にchDBをインストールします。バージョン2.0.2以上を確保してください：

```bash
pip install "chdb>=2.0.2"
```

次にPandasと他のいくつかのライブラリをインストールします：
...
また、Pythonスクリプトやお気に入りのノートブックでコードを使用することもできます。
...
次に、マッチのJSONファイルの1つをDataFrameに読み込みます：
...
次に、イベントのJSONファイルの1つをロードし、そのDataFrameに`match_id`というカラムも追加します：
...
## Pandas DataFrameをクエリする {#querying-pandas-dataframes}

次に、これらのDataFrameを使用してchDBでクエリを実行する方法を見てみましょう。  
ライブラリをインポートします：
...
Pandas DataFramesをクエリするには、`Python`テーブル関数を使用します：
...
したがって、`matches_df`のカラムをリストする場合、次のように書くことができます：
...
次に、2回以上の試合を裁いた審判を特定するには、次のクエリを書きます：
...
## Pandas DataFramesを結合する {#joining-pandas-dataframes}

クエリでDataFrameを結合することもできます。  
たとえば、試合の概要を取得するために次のクエリを書くことができます：
...
## DataFrameからテーブルをポピュレートする {#populating-a-table-from-a-dataframe}

DataFrameからClickHouseテーブルを作成してポピュレートすることもできます。  
chDBでテーブルを作成するには、Stateful Session APIを使用する必要があります。
...
その後、`events_df`に基づいて`events`テーブルを作成します：
...
## Pandas DataFrameとテーブルを結合する {#joining-a-pandas-dataframe-and-table}

最後に、`matches_df` DataFrameを`statsbomb.events`テーブルと結合するようにクエリを更新できます：
