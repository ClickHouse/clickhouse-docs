...
[JupySQL](https://jupysql.ploomber.io/en/latest/quick-start.html) は、Jupyter ノートブックと IPython シェルで SQL を実行できる Python ライブラリです。このガイドでは、chDB と JupySQL を使用してデータをクエリする方法を学びます。
...
JupySQL を IPython で使用することができます。以下のコマンドで IPython を起動します：

```bash
ipython
```

または、以下のコマンドで Jupyter Lab を起動します：
...
:::note
Jupyter Lab を使用している場合、ガイドの残りの手順に従う前にノートブックを作成する必要があります。
:::

## データセットのダウンロード {#downloading-a-dataset}

[Jeff Sackmann の tennis_atp](https://github.com/JeffSackmann/tennis_atp) データセットの 1 つを使用します。このデータセットには、選手およびその時系列のランクに関するメタデータが含まれています。まず、ランキングファイルをダウンロードしましょう：
...
そして、chDB の接続を作成します。永続化したデータはすべて `atp.chdb` ディレクトリに保存されます：

```python
conn = dbapi.connect(path="atp.chdb")
```

次に、`sql` マジックをロードして、chDB に接続します：
...
`atp_rankings` プレフィックスのファイルを一 bunch ダウンロードしました。`DESCRIBE` 句を使ってスキーマを理解しましょう：
...
これらのファイルに直接対して `SELECT` クエリを書くこともできます。データの形式を確認してみましょう：
...
データの形式がちょっと変です。日付をクリーンアップして、`REPLACE` 句を使用してクリーンアップした `ranking_date` を返しましょう：
...
## CSV ファイルを chDB にインポートする {#importing-csv-files-into-chdb}

次に、これらの CSV ファイルからデータをテーブルに保存します。デフォルトのデータベースはディスク上にデータを永続化しないため、まず別のデータベースを作成する必要があります：

```python
%sql CREATE DATABASE atp
```

次に、CSV ファイルのデータ構造からスキーマを派生させて `rankings` という名前のテーブルを作成します：
...
良さそうです - 出力は、CSV ファイルを直接クエリしたときと同じです。

プレーヤーのメタデータに対しても同じプロセスを行います。今回は、すべてのデータが単一の CSV ファイルにあるので、そのファイルをダウンロードします：
...
その後、CSV ファイルの内容に基づいて `players` という名前のテーブルを作成します。`dob` フィールドも `Date32` タイプにクリーンアップします。

> ClickHouse では、`Date` タイプは 1970 年以降の日付のみをサポートしています。`dob` カラムには 1970 年以前の日付が含まれているため、代わりに `Date32` タイプを使用します。
...
データの取り込みが完了しました。さあ、楽しい部分、データをクエリする時間です！

テニス選手は、参加したトーナメントでのパフォーマンスに基づいてポイントを受け取ります。
各選手のポイントは 52 週間の移動平均です。各選手の最大ポイントとその時のランキングを見つけるクエリを作成します：
...
このリストの中には、ポイント数では 1 位にならなかったにもかかわらず、多くのポイントを累積した選手がいるのは非常に興味深いです。

## クエリの保存 {#saving-queries}

クエリは、`%%sql` マジックの同じ行に `--save` パラメータを使用して保存できます。`--no-execute` パラメータは、クエリの実行をスキップします。
...
保存したクエリを実行すると、実行する前に共通テーブル式（CTE）に変換されます。次のクエリでは、選手が 1 位だったときに得た最大ポイントを計算します：
...
クエリにパラメータを使用することもできます。パラメータは通常の変数です：

```python
rank = 10
```

その後、クエリ内で `{{variable}}` 構文を使用できます。以下のクエリは、選手が最初にトップ 10 にランク入りした日と最後にランク入りした日との間に最も少ない日数を持っている選手を見つけます：
...
ヒストグラムを作成する前に、各プレーヤーが達成したトップ 100 内のランキングを計算するクエリを書いて（保存して）みましょう。これを使用して、各ランキングを達成した選手の数をカウントするヒストグラムを作成できます：
...
<img src={require('./images/players_per_rank.png').default} class="image" alt="セルフマネージド ClickHouse の移行" style={{width: '90%', padding: '30px'}}/>
