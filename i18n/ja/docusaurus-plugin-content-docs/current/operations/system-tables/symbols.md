---
description: "C++の専門家やClickHouseエンジニアに役立つ、`clickhouse`バイナリのイントロスペクション情報を含むシステムテーブル。"
slug: /operations/system-tables/symbols
title: "symbols"
keywords: ["システムテーブル", "symbols"]
---

`clickhouse`バイナリのイントロスペクション情報を含みます。アクセスするにはイントロスペクション権限が必要です。このテーブルはC++の専門家やClickHouseエンジニアにのみ役立ちます。

カラム:

- `symbol` ([String](../../sql-reference/data-types/string.md)) — バイナリ内のシンボル名。マングルされています。`demangle(symbol)`を適用することで可読名を取得できます。
