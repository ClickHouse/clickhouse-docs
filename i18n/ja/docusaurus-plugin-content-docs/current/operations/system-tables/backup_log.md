---
description: "システムテーブルで `BACKUP` および `RESTORE` 操作に関するログエントリを含みます。"
...
`BACKUP` および `RESTORE` 操作に関するログエントリを含みます。
...
- `event_time_microseconds` ([DateTime64](../../sql-reference/data-types/datetime64.md)) — マイクロ秒精度のエントリ時刻。
...
- `total_size` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — バックアップに保存されたファイルの総サイズ。
- `num_entries` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — バックアップ内のエントリ数、つまりバックアップがフォルダとして保存されている場合はフォルダ内のファイル数、アーカイブとして保存されている場合はアーカイブ内のファイル数です。増分バックアップの場合や空のファイルや重複が含まれている場合は `num_files` と異なります。常に true であるのは: `num_entries <= num_files`。
- `uncompressed_size` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — バックアップの未圧縮サイズ。
- `compressed_size` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — バックアップの圧縮サイズ。バックアップがアーカイブとして保存されていない場合、`uncompressed_size` と等しくなります。
- `files_read` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — 復元操作中に読み取られたファイルの数。
- `bytes_read` ([UInt64](../../sql-reference/data-types/int-uint.md#uint-ranges)) — 復元操作中に読み取られたファイルの総サイズ。
