---
description: "クラスタ上で実行された分散DDLクエリ（ON CLUSTER句を使用）の情報を含むシステムテーブルです。"
slug: /operations/system-tables/distributed_ddl_queue
title: "distributed_ddl_queue"
keywords: ["システムテーブル", "distributed_ddl_queue"]
---

クラスタ上で実行された[分散DDLクエリ（ON CLUSTER句）](../../sql-reference/distributed-ddl.md)に関する情報を含みます。
...
- `query_duration_ms` ([UInt64](../../sql-reference/data-types/int-uint.md)) — クエリ実行の所要時間（ミリ秒単位）。
...
クエリID: f544e72a-6641-43f1-836b-24baa1c9632a
...
status:            Finished
exception_code:    630
exception_text:    Code: 630. DB::Exception: test_dbを削除または名前変更できません、現在このデータベースに依存するテーブルが存在します:
query_finish_time: 2023-09-01 16:15:14
query_duration_ms: 154

2 行がセットにあります。経過時間: 0.025 秒。
```
