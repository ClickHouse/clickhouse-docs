---
description: "サーバーが知っている各テーブルのメタデータを含むシステムテーブル。"
...
[Temporary tables](../../sql-reference/statements/create/table.md#temporary-tables) は、作成されたセッションでのみ `system.tables` に表示されます。空の `database` フィールドで、`is_temporary` フラグがオンの状態で表示されます。
...
- `uuid` ([UUID](../../sql-reference/data-types/uuid.md)) — テーブル uuid (Atomic database)。
...
- `dependencies_table` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) - テーブル依存関係 ([materialized views](../../sql-reference/statements/create/view.md#materialized-view) 現在のテーブル)。
...
- `total_rows` ([Nullable](../../sql-reference/data-types/nullable.md)([UInt64](../../sql-reference/data-types/int-uint.md))) - 総行数、テーブル内の行数を迅速に正確に判断できる場合、そうでない場合は `NULL`（基になる `Buffer` テーブルを含む）。

- `total_bytes` ([Nullable](../../sql-reference/data-types/nullable.md)([UInt64](../../sql-reference/data-types/int-uint.md))) - 総バイト数、ストレージ内のテーブルの正確なバイト数を迅速に判断できる場合、そうでない場合は `NULL`（基になるストレージを含まない）。

    - テーブルがディスクにデータを保存している場合、ディスク上の使用スペース（つまり圧縮されたもの）を返します。
    - テーブルがメモリにデータを保存している場合、メモリ内で使用されているバイト数の近似値を返します。

- `total_bytes_uncompressed` ([Nullable](../../sql-reference/data-types/nullable.md)([UInt64](../../sql-reference/data-types/int-uint.md))) - 総未圧縮バイト数、ストレージ上のパートチェックサムから正確なバイト数を迅速に判断できる場合、そうでない場合は `NULL`（基になるストレージ（ある場合）を考慮しない）。

- `lifetime_rows` ([Nullable](../../sql-reference/data-types/nullable.md)([UInt64](../../sql-reference/data-types/int-uint.md))) - サーバー起動以降にINSERTされた総行数（`Buffer` テーブルのみ）。

- `lifetime_bytes` ([Nullable](../../sql-reference/data-types/nullable.md)([UInt64](../../sql-reference/data-types/int-uint.md))) - サーバー起動以降にINSERTされた総バイト数（`Buffer` テーブルのみ）。

- `comment` ([String](../../sql-reference/data-types/string.md)) - テーブルに関するコメント。

- `has_own_data` ([UInt8](../../sql-reference/data-types/int-uint.md)) — テーブル自体がディスク上にデータを保存しているか、他のソースのみを参照しているかを示すフラグ。

- `loading_dependencies_database` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) - データベースの読み込み依存関係（現在のオブジェクトの前にロードする必要があるオブジェクトのリスト）。

- `loading_dependencies_table` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) - テーブルの読み込み依存関係（現在のオブジェクトの前にロードする必要があるオブジェクトのリスト）。
