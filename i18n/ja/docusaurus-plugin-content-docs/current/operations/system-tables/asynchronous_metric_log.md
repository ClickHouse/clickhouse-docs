---
description: "システムテーブル `system.asynchronous_metrics` の履歴値を含み、タイムインターバル（デフォルトは1秒）ごとに保存されます。"
...
`system.asynchronous_metrics` の履歴値を含み、タイムインターバル（デフォルトは1秒）ごとに保存されます。デフォルトで有効です。
...
- [system.asynchronous_metrics](../system-tables/asynchronous_metrics.md) — 定期的にバックグラウンドで計算されるメトリックを含む。
- [system.metric_log](../system-tables/metric_log.md) — テーブル `system.metrics` と `system.events` からのメトリック値の履歴を含み、定期的にディスクにフラッシュされる。
