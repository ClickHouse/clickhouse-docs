---
description: "ClickHouse Keeper または ZooKeeper が設定されている場合のみ存在するシステムテーブルです。設定で定義された Keeper クラスターのデータを公開します。"
slug: /operations/system-tables/zookeeper
title: "zookeeper"
keywords: ["システムテーブル", "zookeeper"]
---

テーブルは ClickHouse Keeper または ZooKeeper が設定されていない限り存在しません。`system.zookeeper` テーブルは、設定で定義された Keeper クラスターのデータを公開します。  
クエリは、以下に示すように `WHERE` 句に 'path =' 条件または `path IN` 条件を設定する必要があります。これは、データを取得したい子のパスに対応します。

クエリ `SELECT * FROM system.zookeeper WHERE path = '/clickhouse'` は、`/clickhouse` ノードのすべての子のデータを出力します。  
ルートノードのすべてのデータを出力するには、`path = '/'` と記述します。  
...
これを使用して、Keeper パスクエリのバッチを実行できます。
...
- `ctime` (DateTime) — ノード作成時のタイムスタンプ。
- `mtime` (DateTime) — ノードの最終修正時のタイムスタンプ。
