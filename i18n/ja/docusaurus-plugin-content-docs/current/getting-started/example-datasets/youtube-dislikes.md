---
description: "YouTube動画の嫌悪投票のコレクションです。"
slug: /getting-started/example-datasets/youtube-dislikes
sidebar_label: YouTubeの嫌悪投票
title: "YouTubeの嫌悪投票データセット"
---

2021年11月、YouTubeはすべての動画から公開の***嫌悪票***カウントを削除しました。クリエイターは依然として嫌悪票の数を見ることができますが、視聴者は動画の***いいね***の数のみを見ることができます。

:::important
データセットには45.5億件を超えるレコードがあるため、リソースがそのボリュームを処理できない限り、以下のコマンドを単にコピー＆ペーストすることは避けてください。以下のコマンドは[ClickHouse Cloud](https://clickhouse.cloud)の**Production**インスタンス上で実行されました。
:::

データはJSON形式であり、[archive.org](https://archive.org/download/dislikes_youtube_2021_12_video_json_files)からダウンロードできます。同じデータがS3にも用意されているため、ClickHouse Cloudインスタンスにより効率的にダウンロードできます。

以下は、ClickHouse Cloudにテーブルを作成し、データを挿入する手順です。

:::note
以下の手順は、ローカルインストールのClickHouseでも簡単に機能します。唯一の変更は、`s3cluster`の代わりに`s3`関数を使用することです（クラスターが設定されている場合を除き - その場合は`default`をクラスターの名前に変更してください）。
:::

## ステップバイステップの指示 {#step-by-step-instructions}

1. データがどのようになっているか見てみましょう。`s3cluster`テーブル関数はテーブルを返すので、結果を`DESCRIBE`できます：
...
ClickHouseはJSONファイルから以下のスキーマを推論します：
...
2. 推論されたスキーマに基づいて、データ型をクリーンアップし、主キーを追加しました。以下のテーブルを定義します：
...
3. 次のコマンドは、S3ファイルからレコードを`youtube`テーブルにストリーミングします。

:::important
これにより非常に多くのデータが挿入されます - 46.5億行。データセット全体を取得したくない場合は、単に希望する行数の`LIMIT`句を追加してください。
:::
...
当社の`INSERT`コマンドに関するいくつかのコメント：

- `parseDateTimeBestEffortUSOrZero`関数は、受信する日付フィールドが正しい形式でない可能性がある場合に便利です。`fetch_date`が正しく解析されない場合、`0`に設定されます。
- `upload_date`カラムには有効な日付が含まれていますが、"4時間前"のような文字列も含まれています - これは有効な日付ではありません。元の値を`upload_date_str`に保存し、`toDate(parseDateTimeBestEffortUSOrZero(upload_date::String))`で解析を試みることにしました。解析に失敗した場合は、単に`0`を取得します。
- `ifNull`を使用して、テーブル内に`NULL`値が存在しないようにしました。受信した値が`NULL`の場合、`ifNull`関数は値を空文字列に設定します。

4. ClickHouse CloudのSQLコンソールで新しいタブを開き（または新しい`clickhouse-client`ウィンドウを開き）、カウントが増加する様子を見てください。サーバーリソースに応じて、46.5B行を挿入するのにしばらく時間がかかります（設定を調整せずに、約4.5時間かかります）。
...
5. データが挿入されると、お気に入りの動画やチャンネルの嫌悪票の数を数えてみてください。ClickHouseがアップロードした動画の数を見てみましょう：
...
1行の結果。所要時間: 0.570秒。処理行数: 237.57千行、5.77 MB (416.54千行/s、10.12 MB/s)。
```

:::note
上記のクエリは、主キーの最初のカラムとして`uploader`を選んだため、237k行だけを処理すればよく、非常に迅速に実行されます。
:::

6. ClickHouseの動画のいいねと嫌悪票を見てみましょう：
...
84行の結果があります。所要時間: 0.013秒。処理行数: 155.65千行、16.94 MB (11.96百万行/s、1.30 GB/s)。
```

7. `title`または`description`フィールドに**ClickHouse**を含む動画を検索してみましょう：
...
このクエリはすべての行を処理し、2つの文字列カラムを解析する必要があります。それでも、4.15M行/秒という良好なパフォーマンスを得ています：

```response
1174行の結果があります。所要時間: 1099.368秒。処理行数: 4.56億行、1.98 TB (4.15百万行/s、1.80 GB/s)。
```
...
### コメントが無効になっている場合、誰かが実際にいいねや嫌悪をクリックする可能性は下がるか？ {#if-someone-disables-comments-does-it-lower-the-chance-someone-will-actually-click-like-or-dislike}

コメントが無効になったとき、人々は動画に対する感情を表現するためにいいねや嫌悪をクリックする可能性が高くなるのでしょうか？
...
22行の結果があります。所要時間: 8.460秒。処理行数: 4.56億行、77.48 GB (538.73百万行/s、9.16 GB/s)。
```

コメントを有効にすることは、エンゲージメントの率が高いことと相関しているようです。


### 動画の数は時間と共にどう変化するか - 注目のイベント {#how-does-the-number-of-videos-change-over-time---notable-events}
...
コビドの周りにアップロード者が急増したことが顕著です [こちら](https://www.theverge.com/2020/3/27/21197642/youtube-with-me-style-videos-views-coronavirus-cook-workout-study-home-beauty) をご覧ください。


### 時間とともに字幕が増加すること - いつ {#more-subtitles-over-time-and-when}

音声認識の進化により、字幕を作成するのがこれまでにないほど容易になりました。YouTubeは2009年末に自動キャプションを追加しました - その時の急増が見られるでしょうか？
...
データ結果は2009年に急増していることを示しています。その時点でYouTubeは、他の人の動画のために字幕をアップロードできるコミュニティキャプション機能を削除していました。このことが、聴覚障害者と耳の不自由な視聴者向けの字幕をクリエイターに追加させるための非常に成功したキャンペーンを促しました。


### 時間とともに主要なアップローダー {#top-uploaders-over-time}
...
### 視聴回数が増えるにつれていいね率はどう変化するか？ {#how-do-like-ratio-changes-as-views-go-up}
...
### 視聴回数はどのように分布しているか？ {#how-are-views-distributed}
