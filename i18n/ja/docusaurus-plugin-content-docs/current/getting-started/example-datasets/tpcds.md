...
[Star Schema Benchmark (SSB)](star-schema.md)と同様に、TPC-DSは[TPC-H](tpch.md)に基づいていますが、異なるアプローチを採用しています。つまり、24のテーブルを使用して複雑なスノーフレークスキーマにデータを保存することにより、必要な結合の数を増やしました（8テーブルではなく24テーブル）。
データ分布は偏りがあり（例: 正規分布やポアソン分布）、99のレポートクエリとランダム置換を含んでいます。

リファレンス
- [TPC-DSの作成](https://dl.acm.org/doi/10.5555/1182635.1164217) (Nambiar)、2006年

最初に、TPC-DSリポジトリをチェックアウトしてデータジェネレーターをコンパイルします：
...
その後、クエリを生成します（同じスケールファクターを使用）：

```bash
./dsqgen -DIRECTORY ../query_templates/ -INPUT ../query_templates/templates.lst  -SCALE 1 # out/query_0.sqlに99のクエリを生成
```

次に、ClickHouseでテーブルを作成します。tools/tpcds.sqlの元のテーブル定義を使用するか、適切に定義された主キーインデックスとLowCardinality型のカラムを持つ「調整済み」のテーブル定義を使用できます。
...
::::warning
TPC-DSは相関サブクエリを多く使用していますが、執筆時点（2024年9月）ではClickHouseではサポートされていません（[issue #6697](https://github.com/ClickHouse/ClickHouse/issues/6697)）。そのため、上記の多くのベンチマーククエリはエラーで失敗します。
::::
