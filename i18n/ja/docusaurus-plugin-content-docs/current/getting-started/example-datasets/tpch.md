...
卸売業者の内部データウェアハウスをモデルとした人気のベンチマークです。
データは3rd正規形で保存され、クエリ実行時に多くの結合が必要になります。
年数が経過し、データが均一かつ独立に分布しているという非現実的な仮定にもかかわらず、TPC-Hは最も人気のあるOLAPベンチマークとして現在に至っています。

**参照**

- [TPC-H](https://www.tpc.org/tpc_documents_current_versions/current_specifications5.asp)
- [意思決定支援とWebコマースのための新しいTPCベンチマーク](https://doi.org/10.1145/369275.369291) (Poess et. al., 2000)
- [TPC-Hの分析：影響力のあるベンチマークから得られた隠れたメッセージと教訓](https://doi.org/10.1007/978-3-319-04936-6_5) (Boncz et. al.), 2013
...
スケールファクター100の詳細なテーブルサイズは以下の通りです：

| テーブル   | 行数         | ClickHouseで圧縮されたサイズ |
|------------|--------------|--------------------------|
| nation     | 25           | 2 kB                     |
| region     | 5            | 1 kB                     |
...
私たちはTPC-H規格のルールにできるだけ忠実に従います：
- 主キーは仕様書の1.4.2.2節で言及されたカラムにのみ作成されます。
- 代入パラメータは仕様書の2.1.x.4節におけるクエリ検証のための値に置き換えられます。
- 1.4.2.1節に従い、テーブル定義ではオプションの`NOT NULL`制約は使用せず、`dbgen`によってデフォルトで生成されても無視します。
  ClickHouseにおける`SELECT`クエリの性能は、`NOT NULL`制約の有無によって影響を受けません。
- 1.3.1節に従い、仕様書の抽象データ型を実装するためにClickHouseのネイティブデータ型（例：`Int32`、`String`）を使用します。
  これによる効果は、可読性の向上のみです。`dbgen`が生成するSQL-92データ型（例：`INTEGER`、`VARCHAR(40)`）もClickHouseで動作します。
...
CREATE TABLE orders  (
    ...
    o_comment        String)
ORDER BY (o_orderkey);
-- 以下は公式のTPC-Hルールに準拠しない代替オーダーキーですが、"Quantifying TPC-H Choke Points and Their Optimizations"の4.5節で推奨されています：
...
:::note
tpch-kitを使用して自分でテーブルを生成するのではなく、公開されているS3バケットからデータをインポートすることもできます。上記の`CREATE`ステートメントを使用して、空のテーブルを最初に作成してください。
...
:::note
正しい結果を生成するためには、[`join_use_nulls`](../../operations/settings/settings.md#join_use_nulls)を有効にする必要があります。
:::

クエリは`./qgen -s <scaling_factor>`によって生成されます。スケールファクター`s = 100`のクエリの例：

**正確性**

クエリの結果は、特に記載がない限り、公式結果に一致します。確認するために、スケールファクター=1のTPC-Hデータベースを生成し（`dbgen`、上記参照）、[tpch-kitの期待される結果](https://github.com/gregrahn/tpch-kit/tree/master/dbgen/answers)と比較します。
...
::::note
2025年2月現在、このクエリは相関サブクエリのため、すぐに実行できません。対応する問題: https://github.com/ClickHouse/ClickHouse/issues/6697

この代替の構文は機能し、参照結果を返すことが確認されています。
...
::::note
2025年2月現在、このクエリはDecimalの加算に関するバグのため、すぐに実行できません。対応する問題: https://github.com/ClickHouse/ClickHouse/issues/70136
...
FROM (
    ...
    WHERE
        ...
        AND n1.n_regionkey = r.region_key
        AND r_name = 'AMERICA'
