---
description: "2009年からニューヨーク市で発生した数十億のタクシーおよびハイヤー（Uber, Lyftなど）乗車データ"
...
ニューヨークタクシーデータは、2009年以降にニューヨーク市で発生した30億件以上のタクシーおよびハイヤー（Uber、Lyftなど）乗車データで構成されています。このデータセットは以下の方法で取得できます。

- S3またはGCSからClickHouse Cloudにデータを直接挿入する
- 準備されたパーティションをダウンロードする

## tripsテーブルの作成 {#create-the-table-trips}

タクシー乗車用のテーブルを作成することから始めます：
...
## オブジェクトストレージからのデータの直接読み込み {#load-the-data-directly-from-object-storage}

データに慣れるための小さなサブセットを取得しましょう。データはオブジェクトストレージにあるTSVファイルで、`s3`テーブル関数を使用してClickHouse Cloudに簡単にストリームできます。

同じデータがS3とGCSの両方に保存されています。どちらかのタブを選択してください。

<Tabs groupId="storageVendor">
<TabItem value="gcs" label="GCS" default>

以下のコマンドを使用して、GCSバケットから`trips`テーブルに3つのファイルをストリーミングします（`{0..2}`構文は、値0、1、2のワイルドカードです）：
...
以下のコマンドを使用して、S3バケットから`trips`テーブルに3つのファイルをストリーミングします（`{0..2}`構文は、値0、1、2のワイルドカードです）：
...
ピックアップおよびドロップオフの日付、地理座標、運賃の詳細、ニューヨークの近隣地域などのカラムがあることに注意してください：
...
いくつかのクエリを実行してみましょう。このクエリでは、最も頻繁にピックアップされた上位10の近隣地区を示しています：
...
:::note
以下の手順では、元のデータセットに関する情報と、準備されたパーティションをセルフマネージドのClickHouseサーバー環境に読み込む方法について提供します。
:::

データセットの説明やダウンロード手順については、https://github.com/toddwschneider/nyc-taxi-data および http://tech.marksblogg.com/billion-nyc-taxi-rides-redshift.html を参照してください。

ダウンロードの結果、約227GBの圧縮されていないCSVファイルのデータが得られます。ダウンロードには1Gbit接続で約1時間かかります（s3.amazonaws.comからの並列ダウンロードにより、1Gbitの帯域幅の少なくとも半分が回復されます）。一部のファイルが完全にダウンロードされない場合があります。ファイルサイズを確認し、疑わしいものは再度ダウンロードしてください。

```bash
$ curl -O https://datasets.clickhouse.com/trips_mergetree/partitions/trips_mergetree.tar
# チェックサムを検証
$ md5sum trips_mergetree.tar
# チェックサムが次の値と等しいはずです: f3b8d469b41d9a82da064ded7245d12c
$ tar xvf trips_mergetree.tar -C /var/lib/clickhouse # ClickHouseデータディレクトリのパス
$ # アンパックされたデータの権限を確認し、必要に応じて修正
...
## シングルサーバーでの結果 {#results-on-single-server}
...
Two Intel(R) Xeon(R) CPU E5-2650 v2 @ 2.60GHz, 16物理コア、128 GiB RAM、8x6 TB HD（ハードウェアRAID-5）

実行時間は3回の実行のうちの最良です。ただし、2回目の実行から、クエリはファイルシステムキャッシュからデータを読み取ります。以降、キャッシュは発生しません：データは各実行で読み込まれ、処理されます。
...
ここでは驚きはありません。クエリは線形にスケールします。

私たちはまた、140台のサーバークラスターからの結果を持っています：
...
この場合、クエリ処理時間は主にネットワークのレイテンシによって決まります。
クラスターとは異なるデータセンターにあるクライアントを使用してクエリを実行したため、約20msのレイテンシが追加されました。
