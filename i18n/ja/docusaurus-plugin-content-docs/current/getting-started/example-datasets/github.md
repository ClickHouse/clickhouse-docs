...
**これらのクエリはどのリポジトリでも機能するはずです。自由に探索し、あなたの発見を報告してください。** 実行時間に関する一部のガイドライン（2022年11月現在）：
...
`INSERT INTO SELECT`および[s3関数](/integrations/s3/s3-table-functions/)を使用してデータを挿入します。例えば、以下ではClickHouseファイルをそれぞれのテーブルに挿入します：
...
このクエリには、リネームを考慮した[行ごとのコミット履歴を取得する](#line-by-line-commit-history-of-a-file)というより複雑な変種も存在します。
...
これは、金曜日に生産性が低下することに合致しています。週末にコードをコミットしている人々を見るのは素晴らしいですね！私たちの貢献者に大感謝！
...
### リポジトリ内での古いコード行 {#oldest-lines-of-code-in-the-repository}
...
これは、最近のファイルにバイアスがかかっているためです。新しいファイルにはコミットの機会が少なくなります。1年以上前のファイルに制限した場合はどうでしょうか？
...
`sign = -1` はコードの削除を示しています。句読点および空行の挿入は除外します。
...
### 曜日ごとの最高貢献者は誰か？ {#who-is-the-highest-percentage-contributor-per-day-of-week}
...
分析は現在のファイルに制限します。簡潔さのために、結果は深さ 2 でルートフォルダごとに 5 ファイルに制限します。必要に応じて調整してください。
...
ただし、これは「書き直し」の概念を捉えられません。これは、コミットのいずれかで大部分のファイルが変更されている場合です。これにはより複雑なクエリが必要です。50% 以上のファイルが削除され、50% 以上が追加された場合を「書き直し」と見なします。このクエリを調整して、私たちが何を「書き直し」と見なすかを解釈することができます。
...
### 誰がテスト / CPP コード / コメントを最も多く書くか？ {#who-tends-to-write-more-tests--cpp-code--comments}
...
これを計算するために、まず各著者の時間に対するコメント比率を出します。これは、[誰がより多くのテスト / CPP コード / コメントを書く傾向があるか？](#who-tends-to-write-more-tests--cpp-code--comments) と似ています。これを各著者の開始日と結合し、週のオフセットに対するコメント比率を計算します。

全著者の平均を計算した後、我々はこれらの結果を毎10週のサンプリングで取得します。
...
20 行の結果。経過時間: 0.167 秒。処理した行数：15.07百万行、101.74 MB (90.51百万行/s、610.98 MB/s.)
```

嬉しいことに、我々のコメント % は非常に安定しており、著者が寄与する長さに関わらず低下しません。

### コードが書き直されるまでの平均時間とメディアン（コードの減衰の半減期）は何か？ {#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay}

我々は、[最も多く書き直されたファイルや、最も多くの著者によるファイルのリスト](#list-files-that-were-rewritten-most-number-of-times) と同じ原則を使用して書き直しを特定できますが、すべてのファイルを考慮します。ウィンドウ関数を使用して、各ファイルの書き直し間の時間を計算します。これにより、全ファイルの平均と中央値を計算できます。
...
1 行の結果。経過時間: 0.388 秒。処理した行数：266.05千行、22.85 MB (685.82千行/s、58.89 MB/s.)
```

### コードが再書きされる可能性が最も高いタイミングはいつか？ {#what-is-the-worst-time-to-write-code-in-sense-that-the-code-has-highest-chance-to-be-re-written}

これは、[コードが書き直されるまでの平均時間とメディアン（コードの減衰の半減期）](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) および [最も多く書き直されたファイルのリスト](#list-files-that-were-rewritten-most-number-of-times) と類似しているが、曜日ごとに集約する点が異なります。必要に応じて調整してください。例えば、年の月を考慮しても良いです。
...
7 行の結果。経過時間: 0.466 秒。処理した行数：7.54百万行、701.52 MB (16.15百万行/s、1.50 GB/s.)
```

### 最も「粘着性」がある著者のコードはどれか？ {#which-authors-code-is-the-most-sticky}

「粘着性」を、著者のコードが再書きされるまでの滞留期間と定義します。前の質問 [コードが書き直されるまでの平均時間とメディアン（コードの減衰の半減期）](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) と同様の指標を使用し、すなわち、ファイルへの50％の追加と50％の削除です。著者ごとの平均書き直し時間を計算し、ファイル数が2つ以上の貢献者のみを考慮します。
...
10 行の結果。経過時間: 0.555 秒。処理した行数：7.54百万行、720.60 MB (13.58百万行/s、1.30 GB/s.)
```

### 著者による連続コミット日数の最長記録 {#most-consecutive-days-of-commits-by-an-author}

このクエリでは、最初に著者がコミットした日を計算する必要があります。ウィンドウ関数を使用し、著者ごとにパーティションを分割することで、コミットの間隔の日数を計算できます。各コミットのために、最後のコミットからの経過日数が1日であれば連続カウント（1）とし、そうでなければ0とします。この結果を `consecutive_day` に格納します。

次に、各著者の最長の連続値の配列関数を計算します。まず、著者ごとのすべての `consecutive_day` 値を集計するために `groupArray` 関数が使用されます。この1と0の配列は、0の値で分割されます。最後に、最も長いサブ配列を計算します。
...
10 行の結果。経過時間: 0.025 秒。処理した行数：62.78千行、395.47 KB (2.54百万行/s、16.02 MB/s.)
```

### ファイルの行ごとのコミット履歴 {#line-by-line-commit-history-of-a-file}

ファイルは名前を変更することができます。この際、`path` 列がファイルの新しいパスに設定され、`old_path` は以前の場所を表します。例えば:
...
1 行の結果。経過時間: 0.135 秒。処理した行数：266.05千行、20.73 MB (1.98百万行/s、154.04 MB/s.)
```

これにより、ファイルの完全な履歴を確認することが難しくなります。すべての行やファイルの変更に接続する単一の値がないためです。

これに対処するために、ユーザー定義関数（UDF）を使用できます。これらは現在、再帰的にはできないため、ファイルの履歴を特定するには、各関数が明示的に呼ばれる一連の UDF を定義する必要があります。

これにより、名前変更を最大深度で追跡することができます。以下の例は5の深さです。このファイルが名前を変更される回数がこれ以上であることは考えにくいため、今のところはこれで十分です。
...
`file_path_history('src/Storages/StorageReplicatedMergeTree.cpp')` を呼び出すことで、名前変更の履歴を辿ることができます。各関数は `old_path` で次のレベルを呼び出します。そして、結果は `arrayConcat` を使って組み合わせられます。
...
1 行の結果。経過時間: 0.074 秒。処理した行数：344.06千行、6.27 MB (4.65百万行/s、84.71 MB/s.)
```

この機能を使用して、ファイルの完全な履歴のコミットを集約することができます。この例では、各 `path` 値のためのコミットを1つ示しています。
...
3 行の結果。経過時間: 0.170 秒。処理した行数：611.53千行、41.76 MB (3.60百万行/s、246.07 MB/s.)
```
...
これは、現在のところ配列関数で状態を保持できないため、正確な結果を得るのが特に困難です。これは `arrayFold` または `arrayReduce` によって可能になります。これにより、各イテレーションにおいて状態を保持できるようになります。

十分な高レベルの分析を行うには、以下のような近似的な解決策が考えられます。
...
└─────────────────┴──────────────────────┴───────────────────────────────────────────────────────────────┘
20 行の結果。経過時間: 0.547 秒。処理した行数：7.88百万行、679.20 MB (14.42百万行/s、1.24 GB/s.)
```

ここでは、正確かつ改善された解決策を歓迎します。

## 関連コンテンツ {#related-content}

- ブログ: [Git コミットと我々のコミュニティ](https://clickhouse.com/blog/clickhouse-git-community-commits)
...
- ブログ: [ClickHouse + Grafana を用いたオープンソース GitHub アクティビティの物語](https://clickhouse.com/blog/introduction-to-clickhouse-and-grafana-webinar)
```
