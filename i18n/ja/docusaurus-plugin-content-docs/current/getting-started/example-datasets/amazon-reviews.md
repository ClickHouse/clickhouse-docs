...
description: Amazon製品の顧客レビュー150M以上
---

このデータセットには、Amazon製品の顧客レビューが150M以上含まれています。データはAWS S3のsnappy圧縮されたParquetファイルにあり、サイズは合計49GB（圧縮後）です。これをClickHouseに挿入する手順を見ていきましょう。

:::note
以下のクエリは、[ClickHouse Cloud](https://clickhouse.cloud)の**Production**インスタンスで実行されました。
:::

1. データをClickHouseに挿入することなく、インプレースでクエリを実行できます。いくつかの行を取得して、どのようなものか見てみましょう。
...
2. ClickHouseにこのデータを格納するために、新しい`MergeTree`テーブル`amazon_reviews`を定義します。
...
3. 次の`INSERT`コマンドでは、`s3Cluster`テーブル関数を使用しており、これによりクラスターのすべてのノードを使用して複数のS3ファイルを並行して処理できます。また、`https://datasets-documentation.s3.eu-west-3.amazonaws.com/amazon_reviews/amazon_reviews_*.snappy.parquet`で始まる任意のファイルを挿入するためにワイルドカードを使用します。
...
:::tip
ClickHouse Cloudでは、クラスターの名前は`default`です。`default`をクラスターの名前に変更するか、クラスターがない場合は`s3Cluster`の代わりに`s3`テーブル関数を使用してください。
:::

5. このクエリはあまり時間がかかりません - 平均して毎秒300,000行です。5分ほどで全ての行が挿入されるはずです。
...
ORDER BY size DESC;
```
元のデータは約70Gでしたが、ClickHouseで圧縮されて約30Gを占めています。
...
7. いくつかのクエリを実行してみましょう…こちらはデータセット内で最も役立つレビュー上位10件です。
...
10行がセットに入っています。経過時間: 0.897秒。150.96百万行を処理し、15.36 GB（168.36百万行/s、17.13 GB/s）。
```

8. Amazonでレビュー数が最も多い商品上位10件はこちらです。
...
10行がセットに入っています。経過時間: 20.059秒。150.96百万行を処理し、12.78 GB（7.53百万行/s、637.25 MB/s）。
```

9. 各製品の月ごとの平均レビュー評価はこちらです（実際の[Amazonの面接質問](https://datalemur.com/questions/sql-avg-review-ratings)!）：
...
20行がセットに入っています。経過時間: 43.055秒。150.96百万行を処理し、13.24 GB（3.51百万行/s、307.41 MB/s）。
ピークメモリ使用量: 41.73 GiB。
```

10. 製品カテゴリごとの投票数の合計はこちらです。このクエリは、`product_category`が主キーに含まれているため、高速です。
...
43行がセットに入っています。経過時間: 0.201秒。150.96百万行を処理し、754.79 MB（750.85百万行/s、3.75 GB/s）。
```

11. レビュー内で最も頻繁に発生する**「awful」**という単語を含む商品を見つけましょう。これは大きなタスクであり、151M以上の文字列を解析する必要があります：
...
クエリはわずか4秒で完了します - これは印象的です - そして結果は面白いです：
...
50行がセットに入っています。経過時間: 4.072秒。150.96百万行を処理し、68.93 GB（37.07百万行/s、16.93 GB/s）。
```

12. 同じクエリを再実行できますが、今回はレビューで**awesome**を検索します：
...
```response
```
(The output continues, but it may not be needed as the prompt requests only up to a certain point.)
```plaintext
...
50 行のセット。経過時間: 4.079 秒。150.96 百万行を処理しました。68.95 GB (37.01 百万行/秒, 16.90 GB/秒)。
ピークメモリ使用量: 2.18 GiB。
```
