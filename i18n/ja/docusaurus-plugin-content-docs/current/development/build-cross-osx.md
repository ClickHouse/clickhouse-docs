...
sidebar_position: 20
sidebar_label: macOS用にLinux上でビルドする
---

# Linux上でmacOS用にClickHouseをビルドする方法

これは、Linuxマシンを持ち、そのマシンを使用してOS Xで動作する`clickhouse`バイナリをビルドしたい場合の手順です。
主なユースケースは、Linuxマシン上で実行される継続的インテグレーションチェックです。
...
以下のセクションでは、`x86_64` macOS用のClickHouseのビルド手順を示します。
ARMアーキテクチャをターゲットにしている場合は、すべての`x86_64`の表記を`aarch64`に置き換えてください。
例えば、`x86_64-apple-darwin`を`aarch64-apple-darwin`に置き換えます。
...
生成されるバイナリはMach-O実行可能形式となり、Linux上では実行できません。
