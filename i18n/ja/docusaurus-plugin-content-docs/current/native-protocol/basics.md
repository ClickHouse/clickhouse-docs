...
この文書は、ClickHouse TCPクライアントのバイナリプロトコルについて説明します。
...
// OOMやmake()でのランタイム例外を防ぐためにnをチェックします。
...
:::tip
ClickHouseは**リトルエンディアン**を固定サイズの整数に使用します。
:::
...
ブール値は1バイトで表現され、`1`は`true`、`0`は`false`です。
