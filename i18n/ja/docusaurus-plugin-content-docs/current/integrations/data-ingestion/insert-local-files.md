...
`clickhouse-client`を使用してローカルファイルをClickHouseサービスにストリームすることができます。これにより、多くの強力で便利なClickHouse関数を使用してデータを前処理することが可能になります。例を見てみましょう...

1. `comments.tsv`という名前のTSVファイルがあり、そこにはいくつかのHacker Newsコメントが含まれており、ヘッダ行にはカラム名が含まれています。データを挿入する際に[入力フォーマット](/interfaces/formats)を指定する必要があり、ここでは`TabSeparatedWithNames`です：
...
2. Hacker Newsデータのためのテーブルを作成しましょう：
...
3. `author`カラムを小文字に変換したいので、これは[`lower`関数](/sql-reference/functions/string-functions/#lower-lcase)を使って簡単にできます。また、`comment`文字列をトークンに分割し、その結果を`tokens`カラムに保存したいので、これは[`extractAll`関数](/sql-reference/functions/string-search-functions/#extractallhaystack-pattern)を使用して行うことができます。これをすべて1つの`clickhouse-client`コマンドで行います - `comments.tsv`ファイルが`<`演算子を使用して`clickhouse-client`にパイプされることに注意してください：
...
:::note
`input`関数はここで便利で、`hackernews`テーブルへのデータ挿入の際にデータを変換することを可能にします。`input`への引数は、受信する生データのフォーマットであり、これは他のテーブル関数（受信データのスキーマを指定する場合）でもよく見られます。
:::

4. これで完了です！データはClickHouseに格納されました：
...
5. 別のオプションとして、`cat`のようなツールを使用してファイルを`clickhouse-client`にストリームすることもできます。例えば、次のコマンドは`<`演算子を使用するのと同じ結果を得られます：
...
`clickhouse-client`のインストール方法については、[ドキュメントページ](/interfaces/cli)を訪れてください。

## 関連コンテンツ {#related-content}

- ブログ: [ClickHouseにデータを取り込む - 第1部](https://clickhouse.com/blog/getting-data-into-clickhouse-part-1)
- ブログ: [大規模な実世界データセットの探求: ClickHouseにおける100年以上の気象記録](https://clickhouse.com/blog/real-world-data-noaa-climate-data)
