...
keywords: [ clickhouse, Apache Spark, jdbc, マイグレーション, データ ]
---
...
このセクションでは、Sparkでの[ClickHouse公式JDBCコネクタ](/integrations/java/jdbc-driver)の使用方法について詳述します。
...
```java
 public static void main(String[] args) {
        ...
        //---------------------------------------------------------------------------------------------------
        // JDBCメソッドを使用してClickHouseにdfを書き込む
        //---------------------------------------------------------------------------------------------------
        ...
        //---------------------------------------------------------------------------------------------------
        // saveメソッドを使用してClickHouseにdfを書き込む
        //---------------------------------------------------------------------------------------------------
...
```sql
   ...
   -- resultTableはdf.createTempViewやSpark SQLで作成できる
...
Spark JDBCを使用する場合、Sparkは単一のパーティションを使ってデータを読み込みます。高い同時実行性を実現するためには、`partitionColumn`、`lowerBound`、`upperBound`、`numPartitions`を指定する必要があります。これにより、複数のワーカーから並行して読み込む際にテーブルをどのようにパーティション化するかを説明します。
詳細については、Apache Sparkの公式ドキュメントで[ JDBC構成](https://spark.apache.org/docs/latest/sql-data-sources-jdbc.html#data-source-option)を参照してください。

## JDBCの制限 {#jdbc-limitations}

* 現在、JDBCを使用してデータを挿入できるのは既存のテーブルのみです（DataFrameを挿入する際に自動的にテーブルを作成する方法はなく、Sparkが他のコネクタで行っているようにはできません）。
