...
description: JupySQLはJupyter用のマルチプラットフォームデータベースツールです。
---

# ClickHouseとJupySQLの使用
このガイドでは、ClickHouseとの統合を紹介します。

JupySQLを使用してClickHouse上でクエリを実行します。
データがロードされたら、SQLプロットを通じて可視化します。

JupySQLとClickHouseの統合は、clickhouse_sqlalchemyライブラリの使用によって実現されます。このライブラリは、両システム間の容易な通信を可能にし、ユーザーがClickHouseに接続してSQL方言を渡すことを可能にします。接続が成功すると、ユーザーはClickhouseネイティブUIまたはJupyterノートブックから直接SQLクエリを実行できます。
...
**次のステージのためにClickhouseが起動してアクセス可能であることを確認する必要があります。ローカル版とクラウド版のいずれかを使用できます。**

**注意:** 接続先のインスタンスタイプに応じて接続文字列を調整する必要があります（url、ユーザー名、パスワード）。以下の例ではローカルインスタンスを使用しています。詳細を学ぶには、[こちらのガイド](/getting-started/quick-start)を確認してください。
...
![ヒストグラムの例](images/jupysql-plot-1.png)
...
![ヒストグラムの2番目の例](images/jupysql-plot-1.png)
