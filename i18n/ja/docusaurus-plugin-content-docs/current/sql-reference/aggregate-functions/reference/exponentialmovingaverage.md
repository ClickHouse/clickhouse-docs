...
各 `value` は、決定された `timeunit` に対応します。半減期 `x` は、指数的重みが半分に減衰する時間の遅延を示します。この関数は重み付け平均を返します：古い時間点ほど、対応する値が考慮される重みが小さくなります。
...
- `timeunit` — 時間単位。[整数](../../../sql-reference/data-types/int-uint.md)、[浮動小数点数](../../../sql-reference/data-types/float.md) または [小数](../../../sql-reference/data-types/decimal.md)。timeunit はタイムスタンプ（秒）ではなく、時間間隔のインデックスです。[intDiv](../../functions/arithmetic-functions.md#intdiva-b) を使用して計算できます。
...
- 最新の時間点での過去 `x` 時間の値の[指数的に平滑化された移動平均](https://ja.wikipedia.org/wiki/%E7%A7%BB%E5%8B%95%E5%B9%B3%E5%9D%87#%E5%BC%95%E6%95%B0%E7%A7%BB%E5%8B%95%E5%B9%B3%E5%9D%87)を返します。
...
-- toRelativeHourNum を使って timeunit を計算
SELECT
    value,
    time,
