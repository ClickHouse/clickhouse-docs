...
description: "配列をさらに大きな配列に集約します。"
keywords: ["groupArrayArray", "array_concat_agg"]
---
# groupArrayArray

配列をさらに大きな配列に集約します。 
[`groupArray`](/sql-reference/aggregate-functions/reference/grouparray) 関数を [Array](/sql-reference/aggregate-functions/combinators#-array) 組み合わせ関数と組み合わせます。

エイリアス: `array_concat_agg`

**例**

ユーザーのブラウジングセッションを記録したデータがあります。各セッションは、特定のユーザーが訪れたページのシーケンスを記録しています。 
`groupArrayArray` 関数を使用して、各ユーザーのページ訪問パターンを分析できます。
