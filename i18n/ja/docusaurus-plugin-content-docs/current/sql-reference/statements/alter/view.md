...
# ALTER TABLE ... MODIFY QUERY 文

`ALTER TABLE ... MODIFY QUERY` 文を使用することで、[マテリアライズドビュー](../create/view.md#materialized)作成時に指定された `SELECT` クエリを変更することができます。この操作は、データの取り込みプロセスを中断することなく実行できます。

このコマンドは、`TO [db.]name` 句を使って作成されたマテリアライズドビューを変更するために作られています。基盤のストレージテーブルの構造や、マテリアライズドビューのカラム定義は変更されないため、`TO [db.]name` 句を使わずに作成されたマテリアライズドビューに対するこのコマンドの適用は非常に限られています。
...
-- 新しい計測値 `cost` を追加しましょう
-- そして新しい次元 `browser` を追加します。

ALTER TABLE events
  ADD COLUMN browser String,
  ADD COLUMN cost Float64;

-- カラムはマテリアライズドビューとTO（受信テーブル）で一致する必要はないので、
-- 次の変更は挿入を妨げません。
...
-- 新しいカラム `browser` と `cost` は空っぽのままです。まだマテリアライズドビューを変更していないためです。
...
-- !!! `MODIFY ORDER BY` 中に主キーが暗黙的に導入されました。
...
-- !!! カラムの定義は変更されていませんが、問題ありません。MATERIALIZED VIEWをクエリしているのではなく、
-- TO（ストレージ）テーブルをクエリしているからです。
-- SELECT セクションは更新されます。
...
適用範囲は非常に限られており、`SELECT` セクションを変更することのみで、新しいカラムを追加することはできません。
...
## ALTER LIVE VIEW 文 {#alter-live-view-statement}

`ALTER LIVE VIEW ... REFRESH` 文は、[ライブビュー](../create/view.md#live-view)をリフレッシュします。[ライブビューの強制リフレッシュ](../create/view.md#live-view-alter-refresh)を参照してください。

## ALTER TABLE ... MODIFY REFRESH 文 {#alter-table--modify-refresh-statement}

`ALTER TABLE ... MODIFY REFRESH` 文は、[リフレッシュ可能なマテリアライズドビュー](../create/view.md#refreshable-materialized-view)のリフレッシュパラメータを変更します。[リフレッシュパラメータの変更](../create/view.md#changing-refresh-parameters)を参照してください。
