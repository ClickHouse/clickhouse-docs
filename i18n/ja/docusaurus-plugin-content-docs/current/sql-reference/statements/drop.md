...
既存のエンティティを削除します。`IF EXISTS`句が指定されている場合、エンティティが存在しないときにエラーを返しません。`SYNC`修飾子が指定されている場合、エンティティは遅延なく削除されます。
...
- 複数のテーブルを一度に削除することは原子的な操作ではありません。すなわち、1つのテーブルの削除が失敗した場合、後続のテーブルは削除されません。
