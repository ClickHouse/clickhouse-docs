...
- 入力配列と同じ長さの配列を返します。各値は系列内の対応する要素の可能な異常のスコアを表します。非ゼロのスコアは可能な異常を示します。[配列](../data-types/array.md)。
...
- 系列データの周期に等しい実数値。データポイントの数が4未満の場合はNaNです。[Float64](../data-types/float.md)。
...
シリーズデータをSTL [(ローカルポリノミアル回帰に基づく季節-トレンド分解手法)](https://www.wessa.net/download/stl.pdf) を使用して、季節、トレンド、残差成分に分解します。
...
- 季節成分を含む最初の配列、トレンドを含む第二の配列、残差成分を含む第三の配列、基準（季節 + トレンド）成分を含む第四の配列からなる4つの配列を返します。[配列](../data-types/array.md)。
