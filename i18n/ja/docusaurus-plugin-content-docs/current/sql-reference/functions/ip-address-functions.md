...
UInt32 数値を受け取り、それをビッグエンディアンのIPv4アドレスとして解釈します。 A.B.C.d 形式（小数形式のドット区切りの数字）で対応するIPv4アドレスを含む文字列を返します。

エイリアス: `INET_NTOA`.

## IPv4StringToNum {#IPv4StringToNum}

[IPv4NumToString](#IPv4NumToString) の逆の関数です。IPv4アドレスの形式が無効な場合、例外をスローします。
...
バイナリ形式でIPv6アドレスを含む FixedString(16) 値を受け取ります。テキスト形式でこのアドレスを含む文字列を返します。IPv6 マッピングされた IPv4 アドレスは、形式 ::ffff:111.222.33.44 で出力されます。
...
入力文字列に有効な IPv4 アドレスが含まれている場合、その IPv6 の同等物を返します。HEX は大文字または小文字を使用できます。
...
`UInt32` 数値を受け取ります。 [ビッグエンディアン](https://en.wikipedia.org/wiki/Endianness) として解釈される IPv4 アドレスとみなします。 バイナリ形式のIPv6アドレスを含む `FixedString(16)` の値を返します。例:
...
バイナリ形式のIPv6アドレスを含む FixedString(16) 値を受け取ります。指定されたバイト数が削除されたアドレスをテキスト形式で含む文字列を返します。例えば:
...
IPv4とUInt8の値を受け取り、[CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing)を含みます。サブネットの下限範囲および上限範囲を含む2つのIPv4を含むタプルを返します。
...
IPv6とUInt8の値を受け取り、CIDRを含みます。サブネットの下限範囲および上限範囲を含む2つのIPv6を含むタプルを返します。
...
[`IPv4StringToNum`](#IPv4StringToNum) に似ていますが、IPv4 アドレスの文字列形式を取り、[IPv4](../data-types/ipv4.md) 型の値を返します。
...
- `string` がIPv4アドレスに変換されます。 [IPv4](../data-types/ipv4.md).
...
`toIPv4` と同様ですが、IPv4 アドレスが無効な形式の場合、`0.0.0.0` (0 IPv4) または提供されたIPv4デフォルトを返します。
...
- 現在のIPv4アドレスに変換された `string`、または `string` が無効なアドレスの場合は null。 [String](../data-types/string.md).
...
IPv6 アドレスをバイナリ形式に変換する [IPv6StringToNum](#ipv6stringtonum) 関数に類似しています。

入力文字列に有効な IPv4 アドレスが含まれている場合、その IPv4 アドレスのIPv6の同等物が返されます。
...
[`toIPv6`](#toipv6) と同様ですが、IPv6 アドレスが無効な形式の場合、`::` (0 IPv6) または提供されたIPv6デフォルトを返します。
...
- IPv6アドレス [IPv6](../data-types/ipv6.md)、他は`::`または提供されたオプションのデフォルトが返されます。
...
- IP アドレス。 [IPv6](../data-types/ipv6.md)、または `string` が無効な形式の場合は null。
...
- IP アドレス。 [IPv6](../data-types/ipv6.md)、または `string` が無効な形式の場合は `::`。
...
入力文字列がIPv4アドレスであるかどうかを判定します。`string`がIPv6アドレスの場合、0を返します。
...
入力文字列がIPv6アドレスであるかどうかを判定します。`string`がIPv4アドレスの場合、0を返します。
...
特定のIPアドレスが、[CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) 表記で表されたネットワークに含まれているかどうかを判断します。 true の場合は `1`、そうでない場合は `0` を返します。
...
この関数は、IPv4 および IPv6 のアドレス（およびネットワーク）を文字列として受け取ります。アドレスのIPバージョンとCIDRが一致しない場合、0を返します。

**引数**

- `address` — IPv4 または IPv6 アドレス。 [String](../data-types/string.md).
- `prefix` — CIDR の形式の IPv4 または IPv6 ネットワークプレフィックス。 [String](../data-types/string.md).
