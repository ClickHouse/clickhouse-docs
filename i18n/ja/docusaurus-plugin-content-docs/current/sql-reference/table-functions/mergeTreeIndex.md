...
MergeTreeテーブルのインデックスとマークファイルの内容を表します。これは内部利用のために使用できます。
...
- `database` - インデックスとマークを読み取るデータベースの名前。
- `table` - インデックスとマークを読み取るテーブルの名前。
- `with_marks` - 結果にマークを持つカラムを含めるかどうか。

**戻り値**

ソーステーブルの主キーの値を持つカラム、マークの値を持つカラム（有効な場合）、および仮想カラムを含むテーブルオブジェクト：

- `part_name` - データパートの名前。
- `mark_number` - データパート内の現在のマークの番号。
- `rows_in_granule` - 現在のグラニュール内の行の数。

カラムがデータパートに存在しない場合や、そのサブストリームのいずれかに対するマークが書き込まれていない場合（例：コンパクトなパーツ）の場合、マークカラムは `(NULL, NULL)` 値を含むことがあります。
