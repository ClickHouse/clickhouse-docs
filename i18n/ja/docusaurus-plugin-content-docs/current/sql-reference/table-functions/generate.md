...
指定されたスキーマを持つランダムなデータを生成します。
テストテーブルにデータを挿入することを可能にします。
...
- `random_seed` — 安定した結果を得るためにランダムシードを手動で指定します。NULLの場合は、シードがランダムに生成されます。
- `max_string_length` — 生成されるすべての文字列の最大文字列長。デフォルトは `10`。
- `max_array_length` — 生成されるすべての配列またはマップの最大要素数。デフォルトは `10`。
...
[getRandomStructure](../../sql-reference/functions/other-functions.md#generaterandomstructure)との組み合わせ:
...
`structure`引数が欠落している場合（この場合、構造はランダム）:
...
ランダムな構造とランダムなデータの両方にランダムシードを使用:
...
**注意:** `generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` は、複雑なタイプ（`Array`、`Tuple`、`Map`、`Nested`）のネストの深さが大きくなる可能性があるため、十分に大きな `max_array_length` を指定すると非常に大きな出力を生成する可能性があります（最大16まで）。

## 関連コンテンツ {#related-content}
- ブログ: [ClickHouseにおけるランダムデータの生成](https://clickhouse.com/blog/generating-random-test-distribution-data-for-clickhouse)
