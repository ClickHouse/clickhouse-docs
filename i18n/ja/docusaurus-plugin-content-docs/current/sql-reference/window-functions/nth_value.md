...
指定された順序のフレーム内で、n番目の行（オフセット）に対して評価された最初の非NULL値を返します。
...
- `x` — カラム名。
- `offset` — 現在の行と評価するn番目の行。

**返される値**

- 指定された順序のフレーム内で、n番目の行（オフセット）に対して評価された最初の非NULL値。

**例**

この例では、`nth-value`関数を使用してプレミアリーグサッカー選手の架空の給与データセットから3番目に高い給与を求めます。
