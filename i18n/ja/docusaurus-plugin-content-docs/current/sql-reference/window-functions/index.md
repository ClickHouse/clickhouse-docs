...
実行できる計算の一部は集約関数で行うことができますが、ウィンドウ関数は行を1つの出力にグループ化しません - 各行は依然として返されます。

## 標準ウィンドウ関数 {#standard-window-functions}

ClickHouseは、ウィンドウとウィンドウ関数を定義するための標準的な文法をサポートしています。以下のテーブルは、機能が現在サポートされているかどうかを示しています。

| 機能                                                                 | サポートされていますか？                                                                                                                                                                       |
|-----------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ad hocウィンドウ仕様（`count(*) over (partition by id order by time desc)`） | ✅                                                                                                                                                                                  |
| ウィンドウ関数を含む式（例: `(count(*) over ()) / 2)`                | ✅                                                                                                                                                                                   |
| `WINDOW`句（`select ... from table window w as (partition by id)`）   | ✅                                                                                                                                                                                   |
| `ROWS`フレーム                                                         | ✅                                                                                                                                                                                   |
| `RANGE`フレーム                                                        | ✅（デフォルト）                                                                                                                                                                      |
| `DateTime`の`RANGE OFFSET`フレーム用の`INTERVAL`構文                   | ❌（代わりに秒数を指定してください（`RANGE`は任意の数値型で機能します）。）                                                                                                                             |
| `GROUPS`フレーム                                                       | ❌                                                                                                                                                                               |
| フレーム内で集約関数を計算する（`sum(value) over (order by time)`）   | ✅（すべての集約関数がサポートされています）                                                                                                                                                       |
| `rank()`, `dense_rank()`, `row_number()`                               | ✅ <br/>エイリアス: `denseRank()`                                                                                                                                                                                   |
| `percent_rank()`                                                       | ✅  データセット内のパーティション内での値の相対的な地位を効率的に計算します。この関数は、`ifNull((rank() OVER(PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER(PARTITION BY x) - 1, 0), 0)`のような冗長で計算資源を多く消費する手動SQL計算を効果的に置き換えます。<br/>エイリアス: `percentRank()`| 
| `lag/lead(value, offset)`                                             | ❌ <br/> 次のいずれかのワークアラウンドを使用できます:<br/> 1) `any(value) over (.... rows between <offset> preceding and <offset> preceding)`、または`leading`を使用する場合、<br/> 2) `lagInFrame/leadInFrame`、類似ですがウィンドウフレームを尊重します。`lag/lead`と同じ動作を得るには、`rows between unbounded preceding and unbounded following`を使用します。                                                                 |
| ntile(buckets)                                                        | ✅ <br/> ウィンドウを指定します（例: `partition by x order by y rows between unbounded preceding and unbounded following`）。 |
...
指定された`metric_column`の非負導関数を`timestamp_column`により求めます。 
`INTERVAL`は省略可能で、デフォルトは`INTERVAL 1 SECOND`です。
計算される値は、各行に対して以下のようになります:
- 最初の行に対して`0`、
...
- `ORDER BY` - 集約関数の計算中にグループ内の行を順序付ける方法を定義します。
...
- [`row_number()`](./row_number.md) - 現在の行をそのパーティション内で1から始めて番号付けします。
- [`first_value(x)`](./first_value.md) - 順序付けられたフレーム内で評価された最初の値を返します。
- [`last_value(x)`](./last_value.md) - 順序付けられたフレーム内で評価された最後の値を返します。
- [`nth_value(x, offset)`](./nth_value.md) - 順序付けられたフレーム内でn番目の行（オフセット）に照らして評価された最初の非NULL値を返します。
- [`rank()`](./rank.md) - 隙間のある状態で現在の行をそのパーティション内でランク付けします。
- [`dense_rank()`](./dense_rank.md) - 隙間のない状態で現在の行をそのパーティション内でランク付けします。
- [`lagInFrame(x)`](./lagInFrame.md) - 順序付けられたフレーム内で現在の行の前の指定された物理的オフセット行で評価された値を返します。
- [`leadInFrame(x)`](./leadInFrame.md) - 順序付けられたフレーム内で現在の行の後のオフセット行で評価された値を返します。

## 例 {#examples}

ウィンドウ関数がどのように使用できるかのいくつかの例を見てみましょう。
...
### カラムによるパーティション分け {#partitioning-by-column}
...
```sql
-- フレームはパーティションの境界によって制約されています（BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING）
SELECT
    part_key,
    value,
    order,
...
```sql
-- 短い形式 - 制約式なし、順序なし、
-- `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`の等価なもの
SELECT
    part_key,
    value,
    order,
...
```sql
-- 短い形式（フレームがパーティションの開始と現在の行で制約されます）
-- `ORDER BY order ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`の等価なもの
SELECT
    part_key,
    value,
    order,
...
```sql
-- フレームがパーティションの開始と現在の行で制約されますが、順序が逆
SELECT
    part_key,
    value,
    order,
...
```sql
-- first_valueとlast_valueはフレームを尊重します
SELECT
...
温度は秒単位の精度で保存されていますが、`Range`と`ORDER BY toDate(ts)`を使用することで、サイズが10ユニットのフレームを作成し、`toDate(ts)`のおかげで単位は日になります。
...
ウィンドウ関数の初期サポートのロードマップは[こちらのイシュー](https://github.com/ClickHouse/ClickHouse/issues/18097)で確認できます。

ウィンドウ関数に関連するすべてのGitHubのイシューには[comp-window-functions](https://github.com/ClickHouse/ClickHouse/labels/comp-window-functions)タグがあります。
...
- ブログ: [ClickHouseでの時間系列データの操作](https://clickhouse.com/blog/working-with-time-series-data-and-functions-ClickHouse)
- ブログ: [Gitコミットシーケンスのウィンドウおよび配列関数](https://clickhouse.com/blog/clickhouse-window-array-functions-git-commits)
- ブログ: [ClickHouseへのデータの取り込み - 第3部 - S3を使用](https://clickhouse.com/blog/getting-data-into-clickhouse-part-3-s3)
