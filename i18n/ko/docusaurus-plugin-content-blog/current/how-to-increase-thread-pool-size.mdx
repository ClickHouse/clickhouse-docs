...
ClickHouse는 쿼리를 처리하고 머지 및 뮤테이션과 같은 백그라운드 작업을 수행하기 위해 **Global Thread 풀**의 스레드를 사용합니다. 쿼리를 처리할 유휴 스레드가 없으면 풀에 새 스레드가 생성됩니다.
...
위의 설정과 글로벌 스레드 풀(Global Thread Pool)에 영향을 미치는 기타 설정에 대한 자세한 내용은 [문서](/operations/server-configuration-parameters/settings#max_thread_pool_size)를 참고하십시오.