---
date: 2023-10-25
title: QUOTA 및 쿼리 복잡도(Query Complexity) 소개
tags: ['Managing Cloud']
keywords: ['Quotas', 'Query Complexity']
description: 'QUOTA 및 쿼리 복잡도(Query Complexity)는 사용자가 ClickHouse에서 수행할 수 있는 작업을 제한하고 관리하는 강력한 수단입니다. 이 KB 문서에서는 이 두 가지 서로 다른 접근 방식을 적용하는 방법을 예시와 함께 설명합니다.'
---
...
## Quota와 쿼리 복잡도에 대하여 \{#about-quotas-and-query-complexity\}

[Quota](https://clickhouse.com/docs/operations/quotas)와 [쿼리 복잡도](https://clickhouse.com/docs/operations/settings/query-complexity)는 ClickHouse에서 사용자가 수행할 수 있는 작업을 제한하고 관리하는 강력한 방법입니다.

Quota는 일정 시간 구간 내에서 제약을 적용하는 반면, 쿼리 복잡도는 시간 구간과 관계없이 적용됩니다.

이 KB 문서에서는 이 두 가지 서로 다른 접근 방식을 적용하는 예제를 보여 줍니다.

## 샘플 데이터 \{#the-sample-data\}

이 예제에서는 다음과 같은 간단한 샘플 테이블을 사용합니다:
...
## QUOTA 사용 \{#using-quotas\}

이 예제에서는 10초마다 결과 행(row)을 최대 10개까지만 조회할 수 있도록 QUOTA를 적용할 역할(role)을 생성합니다.
...
이제 `user_with_quota` 사용자로 로그인하십시오
...
사용자는 새로운 10개 행 결과 집합 할당량(resultset allowance)을 다시 얻으려면 5초를 더 기다려야 합니다.

## 쿼리 복잡도 사용 \{#using-query-complexity\}

이 예제에서는 각 쿼리당 1개의 행만 반환되도록 제한하는 Query Complexity `SETTING`을 적용할 역할을 생성합니다.
...
이제 `user_with_query_complexity` 사용자로 로그인하세요:
...
결과 집합에서 1개를 초과하는 행을 가져오려고 할 때마다 쿼리 복잡도 제약 조건이 작동합니다.