...
:::note
단일 쿼리는 서로 다른 `query_id` 값을 가진 여러 행으로 기록될 수 있습니다. 분산 보조 쿼리와 내부 뷰 단계에는 `is_initial_query = 0`이 설정됩니다. 제출된 형태 그대로 쿼리를 보려면 `is_initial_query = 1`(또는 `query_id = initial_query_id`)로 필터링하십시오. 또한 ClickHouse가 할당하는 내부 `query_id` 값에는 `queryView...`와 같은 레이블이 포함될 수 있다는 점에 유의하십시오. 자세한 내용은 [`query_log` 참고](/operations/system-tables/query_log)를 참조하십시오.
:::
...
## 클러스터 전체 모니터링 쿼리 \{#cluster-wide-monitoring-queries\}

다음 쿼리는 ClickHouse 클러스터를 모니터링하는 데 유용합니다. `clusterAllReplicas()`를 사용해 모든 노드의 데이터를 집계합니다.

:::note
이 쿼리는 클러스터 이름이 `default`라고 가정합니다. 클러스터 이름이 다르면 `'default'`와 `default`를 실제 클러스터 이름으로 바꾸십시오.
:::

### 분당 및 초당 새로 생성된 파트의 평균(지난 1시간) \{#avg-new-parts-per-minute-and-second\}
...
### CPU 및 메모리 집약적 쿼리(클러스터 전체) \{#cpu-and-memory-intensive-queries-cluster-wide\}
...
### 완료 예상 시간이 포함된 진행 중인 머지 \{#merges-in-progress-with-eta\}

이 쿼리는 클러스터에서 현재 실행 중인 머지와 완료까지의 예상 시간을 보여줍니다:
...
### 정규화된 해시별 가장 많이 사용되는 쿼리 \{#most-common-queries-by-normalized-hash\}
...
### 이벤트 유형 및 날짜별 오류 수 \{#error-counts-by-event-type-and-date\}

클러스터 전반에서 발생한 파트 생성 오류를 분석합니다:
...
### 노드별 테이블 수 \{#number-of-tables-by-node\}

클러스터 노드 전반의 테이블 분포를 확인합니다:
...
### async insert 작업 확인 \{#check-for-async-insert-operations\}

async insert 동작을 모니터링합니다:
...
## 파트 및 머지 분석 \{#parts-and-merges-analysis\}

### 테이블별 현재 활성 파트 수 \{#currently-active-parts-by-table\}

클러스터 전체에서 테이블별 활성 파트 수를 확인할 수 있습니다:
...
### 파트가 과도하게 많은 파티션 \{#partitions-with-too-many-parts\}

파트 수가 너무 많아 쿼리 성능에 영향을 줄 수 있는 파티션을 찾습니다:
...
### 분리된 파트 \{#detached-parts\}

조사가 필요할 수 있는 분리된 파트가 있는지 확인하세요:
...
## 시스템 정보 쿼리 \{#system-information-queries\}

### 노드별 클러스터 전체 메모리 사용량 \{#cluster-wide-memory-usage-by-node\}

노드 전반의 메모리 사용량을 모니터링합니다:
...
### 클러스터에서 쿼리 실행하기 \{#running-queries-on-the-cluster\}
...
### 기본값에서 변경된 설정 \{#modified-settings-from-defaults\}

기본값에서 변경된 설정을 확인하십시오:
...
### 복제 큐 상태 \{#replication-queue-status\}

복제된 테이블에서는 복제 큐를 확인하십시오:
