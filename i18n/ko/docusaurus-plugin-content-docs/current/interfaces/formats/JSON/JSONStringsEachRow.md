---
alias: []
description: 'JSONStringsEachRow 형식에 대한 문서'
input_format: false
...
[`JSONEachRow`](./JSONEachRow.md)와 다른 점은 데이터 필드가 타입이 지정된 JSON 값이 아닌 문자열로 출력된다는 점뿐입니다.
...
다음과 같은 데이터가 들어 있는 JSON 파일 `football.json`을 사용합니다.
...
`JSONStringsEachRow` 형식을 사용하여 데이터를 읽습니다:
