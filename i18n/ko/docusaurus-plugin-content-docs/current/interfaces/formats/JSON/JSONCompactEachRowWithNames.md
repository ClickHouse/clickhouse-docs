...
[`JSONCompactEachRow`](./JSONCompactEachRow.md) 포맷과는, [`TabSeparatedWithNames`](../TabSeparated/TabSeparatedWithNames.md) 포맷과 유사하게 컬럼 이름이 포함된 헤더 행도 함께 출력한다는 점에서 다릅니다.
...
입력 데이터의 컬럼이 이름을 기준으로 테이블의 컬럼에 매핑됩니다. 그리고 [`input_format_skip_unknown_fields`](/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) 설정이 1로 설정된 경우, 이름을 알 수 없는 컬럼은 건너뜁니다.
