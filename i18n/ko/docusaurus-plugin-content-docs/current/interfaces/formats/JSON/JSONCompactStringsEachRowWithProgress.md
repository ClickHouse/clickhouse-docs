---
alias: []
description: 'JSONCompactStringsEachRowWithProgress 형식 설명서'
input_format: true
...
[`JSONCompactEachRowWithProgress`](/interfaces/formats/JSONCompactEachRowWithProgress)와 유사하지만, 모든 값이 문자열 형태로 변환됩니다.
이는 모든 데이터 타입에 대해 일관된 문자열 표현이 필요할 때 유용합니다.

주요 특징:

- `JSONCompactEachRowWithProgress`와 동일한 구조
- 모든 값이 문자열로 표현됨 (숫자, 배열 등도 모두 따옴표로 감싼 문자열)
- 진행 상황 업데이트, 합계, 예외 처리 포함
- 문자열 기반 데이터를 선호하거나 요구하는 클라이언트에 유용함
