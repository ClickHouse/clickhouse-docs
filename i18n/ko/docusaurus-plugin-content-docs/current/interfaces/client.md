...
| `--inline-insert-data`          | 데이터를 네이티브 형식의 블록으로 변환하는 대신 `INSERT ... VALUES`(및 다른 인라인 형식)를 쿼리 텍스트에 있는 그대로 전송합니다. 서버가 인라인 데이터를 직접 구문 분석하므로 테이블 구조와 컬럼 기본값을 클라이언트로 다시 보내는 왕복 과정을 피할 수 있습니다. 이는 네이티브 프로토콜을 통해 여러 개의 작은 삽입을 수행할 때 성능을 향상시킬 수 있습니다. [`send_table_structure_on_insert_with_inline_data`](/operations/settings/settings#send_table_structure_on_insert_with_inline_data)를 자동으로 `0`으로 설정합니다. 인라인 데이터와 외부 데이터(stdin 또는 `INFILE`에서 제공되는 데이터)는 함께 사용할 수 없습니다. |
...
| `--echo [ <bool> ]`               | 실행 전에 각 쿼리를 출력합니다. 선택적 불리언 값을 받습니다.                                                                                                           | 대화형 모드에서는 `true`, 비대화형(배치) 모드에서는 `false` |
| `--echo-formatted [ <bool> ]`     | echo된 쿼리를 포맷합니다. 선택적 불리언 값을 받습니다.                                                                                                             | 대화형 모드에서는 `true`, 비대화형(배치) 모드에서는 `false` |
| `--echo-query-id [ <bool> ]`      | 실행 전에 쿼리 id를 출력합니다. 선택적 불리언 값을 받습니다.                                                                                                          | 대화형 모드에서는 `true`, 비대화형(배치) 모드에서는 `false` |
| `--highlight [ --hilite ] <bool>` | 명령 프롬프트와 echo된 쿼리의 구문 강조를 켜거나 끕니다.                                                                                                            | `true`                                   |
