...
이 설정을 활성화하면 (`input_format_json_try_infer_named_tuples_from_objects`가 활성화된 경우) JSON 객체에서 이름이 지정된 Tuple을 추론하는 동안 모호한 경로에 대해 예외를 발생시키는 대신 `String` 타입을 사용할 수 있습니다.
이를 통해 모호한 경로가 있더라도 JSON 객체를 이름이 지정된 Tuple로 읽을 수 있습니다.
