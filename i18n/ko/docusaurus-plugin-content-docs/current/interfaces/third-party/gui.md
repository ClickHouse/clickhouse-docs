...
[Dory](https://github.com/dorylab/dory)는 AI에 최적화된 SQL 워크스페이스로, ClickHouse에 대한 일급 지원을 제공하며 AI 기능이 내장되어 있습니다.

기능:

* SQL 생성, 설명, 디버깅을 위한 AI Copilot
* 통합 워크스페이스에서 여러 ClickHouse 클러스터를 관리하고 쿼리
* 스키마를 인식하는 SQL 자동 완성과 멀티 탭 쿼리 워크스페이스
* 필터링 및 시각화를 포함한 쿼리 결과의 대화형 탐색
* 데이터셋 이해를 위한 AI 기반 테이블 요약
* SSH 터널을 지원하는 Direct ClickHouse 연결
* 라이트, 다크 등 다양한 테마를 지원하는 현대적인 개발자 친화적 인터페이스
...
[1bench](https://1bench.dev)는 ClickHouse를 강력하게 지원하는 멀티 데이터베이스용 네이티브 데스크톱 GUI로, 서버 개요, 스키마 관리, 벡터 검색, 대규모 결과 세트 탐색 기능을 제공합니다.

기능:

* 연결 시 서버 개요 제공 — 버전, 업타임, 실행 중인 쿼리, 활성 병합, 파트 및 스토리지 크기, 레플리카 상태, 클러스터와 노드를 한눈에 확인할 수 있습니다.
* 시각적 쿼리 빌더(컬럼 선택기, 필터, 정렬, 제한)와 함께, 구문 강조와 연결별 쿼리 이력을 제공하는 Monaco SQL 에디터를 사용할 수 있습니다.
* `MergeTree` 변형, `ORDER BY`, `PARTITION BY`, `SETTINGS`, `Nullable()` 자동 래핑을 지원하는 시각적 `CREATE TABLE` 마법사를 제공합니다.
* 네이티브 ClickHouse 타입 처리 — `Nullable`, `Array`, `LowCardinality`, 중첩 객체를 지원합니다.
* 벡터 검색 지원 — `Array(Float32)` 임베딩 컬럼을 간결한 벡터 셀로 렌더링하고, 2D 임베딩 시각화와 `cosineDistance`를 통한 유사 항목 찾기를 제공합니다.
* 결과 테이블에서 인라인 데이터 편집과 일괄 저장을 지원하며, ClickHouse의 네이티브 형식을 사용한 CSV/JSON/SQL 내보내기 및 가져오기를 지원합니다.
* 연결 옵션: HTTP/HTTPS, 방화벽 뒤의 비공개 클러스터를 위한 SSH 터널, 안전한 프로덕션 탐색을 위한 선택적 읽기 전용 모드.
* ClickHouse Cloud 및 셀프 호스팅 환경에서 사용할 수 있습니다.