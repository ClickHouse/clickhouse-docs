...
ClickHouse Cloud를 사용하면 패치와 업그레이드를 직접 관리할 필요가 없습니다. 수정 사항, 신규 기능, 성능 향상이 포함된 업그레이드를 주기적으로 적용합니다. ClickHouse에 어떤 새로운 기능이 추가되었는지에 대한 전체 목록은 [Cloud changelog](/whats-new/cloud)를 참고하십시오.
