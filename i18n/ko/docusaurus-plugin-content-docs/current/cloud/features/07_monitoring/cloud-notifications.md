...
현재 결제(결제 실패, 사용량이 ascertain 임계값을 초과한 경우 등)와 관련된 알림과 스케일링 이벤트(스케일링 완료, 스케일링 차단 등)와 관련된 알림을 전송합니다.

:::note
크레딧 임계값 알림은 현재 약정 사용 금액 계약을 체결한 조직에서만 제공됩니다. 종량제(PAYG) 조직에는 이러한 알림이 전송되지 않습니다.
:::