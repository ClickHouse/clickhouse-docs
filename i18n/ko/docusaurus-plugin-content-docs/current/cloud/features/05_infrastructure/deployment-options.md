...
ClickHouse Cloud는 프로비저닝, 확장, 유지 관리, 업데이트를 처리하므로, 사용자는 데이터 분석과 애플리케이션 개발에만 온전히 집중할 수 있습니다.
사용량 기반 과금과 자동 확장을 제공하여 분석 워크로드에 대해 안정적이고 비용 효율적인 성능을 보장합니다. AWS, GCP, Azure에서 사용할 수 있으며, 마켓플레이스를 통한 직접 청구 옵션을 지원합니다.

[ClickHouse Cloud](/getting-started/quick-start/cloud)에 대해 더 알아보십시오.

## 사용자 소유 클라우드 사용 \{#byoc\}

ClickHouse 사용자 소유 클라우드 사용(BYOC)을 사용하면 조직은 관리형 서비스 계층을 활용하면서도 자체 클라우드 환경 내에서 ClickHouse를 배포하고 관리할 수 있습니다. 이 옵션은 완전 관리형 ClickHouse Cloud 환경과 자가 관리형 배포의 완전한 제어 사이의 격차를 메워 줍니다. ClickHouse BYOC를 사용하면 사용자는 특정 규정 준수 및 규제 요구 사항을 충족하기 위해 데이터, 인프라, 보안 정책에 대한 제어를 유지하는 동시에 패치, 모니터링, 스케일링과 같은 운영 작업은 ClickHouse에 맡길 수 있습니다. 이 모델은 프라이빗 클라우드 배포의 유연성과 관리형 서비스의 이점을 함께 제공하므로, 엄격한 보안, 거버넌스, 데이터 레지던시 요구 사항이 있는 엔터프라이즈의 대규모 배포에 적합합니다.

[사용자 소유 클라우드 사용](/cloud/reference/byoc/overview)에 대해 자세히 알아보세요.
