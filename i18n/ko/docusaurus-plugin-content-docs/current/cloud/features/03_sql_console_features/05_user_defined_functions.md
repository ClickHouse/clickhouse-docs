...
이 예시에서는 특정 타임스탬프가 일반적인 업무 시간에 해당하는지 검사하는 간단한 실행형 사용자 정의 함수 `isBusinessHours`를 동일하게 생성합니다.
이전에는 SQL을 사용하여 생성했지만, 이번에는 Python을 사용해 작성하고 UI를 통해 구성합니다.

### Python 파일 만들기 \{#create-python-file\}

로컬에 새 파일 `main.py`를 만드십시오:
...
1. Cloud 콘솔 홈에서 왼쪽 하단 메뉴의 조직 이름을 클릭하세요.
...
### UDF 테스트 \{#test-your-udf\}

1. 페이지 왼쪽 상단에서 **Settings - return to your service view**를 클릭하여 SQL Console 홈 화면으로 돌아가십시오
2. 왼쪽 메뉴에서 **SQL Console**을 클릭하십시오
3. 다음 쿼리를 입력하십시오:
...
### 새 버전 만들기 \{#create-new-version\}

1. Cloud 콘솔 홈페이지에서 왼쪽 하단 메뉴의 조직 이름을 클릭합니다.
