...
3. 방화벽 설정으로 인해 다른 리전에서 PSC로의 액세스가 차단되지 않도록 해야 합니다.
4. GCP 리전 간 데이터 전송 비용이 발생할 수 있음에 유의해야 합니다.
...
2. 서비스 엔드포인트를 생성합니다.
3. ClickHouse Cloud 서비스에 &quot;Endpoint ID&quot;를 추가합니다.
4. ClickHouse 서비스 허용 목록에 &quot;Endpoint ID&quot;를 추가합니다.
...
또한 ClickHouse는 필요한 GCP PSC 엔드포인트 또는 DNS 레코드 구성에 대해 책임을 지지 않습니다.
...
:::important
PSC 엔드포인트 IP 주소에 직접 연결하는 것만으로는 충분하지 않습니다. ClickHouse Cloud는 DNS 이름을 기준으로 연결을 라우팅하므로(TLS 핸드셰이크에서 SNI를 통해), 프라이빗 DNS 호스트 이름(`privateDnsHostname`)이 엔드포인트 IP 주소로 확인되도록 DNS를 구성해야 합니다. 그렇지 않으면 ClickHouse Cloud는 해당 연결이 어떤 서비스를 대상으로 하는지 식별할 수 없으며, 연결이 실패합니다.
:::
...
Private Service Connect가 활성화된 각 서비스에는 퍼블릭 엔드포인트와 프라이빗 엔드포인트가 있습니다. Private Service Connect를 사용해 연결하려면 프라이빗 엔드포인트를 사용해야 하며, 이는 [Obtain GCP service attachment for Private Service Connect](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect) 단계에서 확인한 `privateDnsHostname`입니다.
...
DNS&#95;NAME - [Private Service Connect용 GCP 서비스 어태치먼트 가져오기](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect) 단계에서 얻은 `privateDnsHostname` 값을 사용하십시오
