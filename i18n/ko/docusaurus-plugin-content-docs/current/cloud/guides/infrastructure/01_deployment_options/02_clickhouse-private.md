...
* 백업 및 스케일링 작업을 포함한 프로그래밍 방식 작업을 위한 API
...
ClickHouse Private에 관심이 있는 고객은 사용 사례 검토를 위한 상담 일정을 예약하려면 [문의할 수 있습니다](https://clickhouse.com/company/contact). 최소 규모 요구 사항을 충족하고 지원되는 구성을 대상으로 하는 요청만 검토되며, 온보딩 가능 수량에는 제한이 있다는 점에 유의하십시오. 설정은 AWS ECR에서 가져온 이미지와 Helm 차트를 사용하여 환경별 설치 가이드를 따라 진행됩니다.