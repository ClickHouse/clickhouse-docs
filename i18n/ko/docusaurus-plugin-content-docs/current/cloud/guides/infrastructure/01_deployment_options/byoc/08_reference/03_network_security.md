...
* Tailscale이 엔지니어를 고객 엔드포인트로 라우팅하기 전에, 엔지니어는 내부 승인 워크플로를 통해 액세스를 요청해야 합니다
* 액세스는 시간 제한이 있으며 자동으로 만료됩니다
* 모든 액세스는 감사 대상이며 로그로 기록됩니다

전체 데이터 액세스 정책(엔지니어가 볼 수 있는 범위, 인증서 기반 인증, 고객 측 감사 포함)에 대해서는 [ClickHouse data access](/cloud/reference/byoc/reference/clickhouse_data_access)를 참조하십시오.
...
BYOC에서 Tailscale이 구현되는 방식에 대한 더 자세한 기술 정보는 [Building ClickHouse BYOC on AWS blog post](https://clickhouse.com/blog/building-clickhouse-byoc-on-aws#tailscale-connection)를 참조하십시오. 연결 후 ClickHouse 엔지니어가 읽을 수 있는 정보와 ClickHouse가 해당 접근을 어떻게 감사하는지에 대해서는 [ClickHouse data access](/cloud/reference/byoc/reference/clickhouse_data_access)를 참조하십시오.
...
ClickHouse Cloud 엔지니어는 문제 해결을 위해 Tailscale을 통한 액세스 권한이 필요합니다. BYOC 배포 환경에 대해서는 just-in-time 인증서 기반 인증이 프로비저닝됩니다. 전체 액세스 정책은 [ClickHouse data access](/cloud/reference/byoc/reference/clickhouse_data_access)를 참조하십시오.
