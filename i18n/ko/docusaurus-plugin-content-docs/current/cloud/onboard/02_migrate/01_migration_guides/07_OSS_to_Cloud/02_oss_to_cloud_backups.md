...
title: '백업 명령을 사용해 자가 관리형 ClickHouse에서 ClickHouse Cloud로 마이그레이션하기'
...
### MergeTree 테이블에서 ReplicatedMergeTree 테이블로 \{#mergetree-to-replicatedmergetree\}
...
클러스터를 실행 중이라면 테이블이 이미 `ReplicatedMergeTree` 엔진을 사용하고 있을 가능성이 높습니다.
...
### ReplicatedMergeTree를 사용하는 분산 테이블 \{#distributed-tables\}

구성에서 여러 세그먼트에 걸쳐 분산 테이블을 사용하는 경우, 각 노드에 로컬 `ReplicatedMergeTree` 테이블과 쿼리 진입점 역할을 하는 `Distributed` 테이블이 필요합니다.

다음 명령어를 실행하여 모든 클러스터 노드에 로컬 복제된 테이블을 생성하십시오:
...
다음으로 그 위에 `Distributed` 테이블을 생성합니다:
...
각 세그먼트는 독립적으로 백업해야 합니다. 각 세그먼트의 노드에 연결한 다음
세그먼트마다 고유한 대상 경로로 백업 명령어를 실행합니다.
...
:::note[단일 노드 배포]
분산 테이블을 사용하지 않는 경우, 단일 명령어로 데이터베이스 전체를 백업할 수 있습니다:
...
각 세그먼트의 백업을 ClickHouse Cloud 서비스에 하나씩 복원합니다. `ROLE_ARN`은
[&quot;S3 데이터에 안전하게 액세스하기&quot;](/cloud/data-sources/secure-s3)에서 얻은 값으로 설정하십시오.
두 번째 복원부터(및 그 이후의 모든 복원에는) `SETTINGS allow_non_empty_tables=true`를 사용하여,
충돌로 인해 실패하지 않고 세그먼트 데이터가 이미 복원된 테이블에 추가되도록 하십시오:
...
:::note[비분산 배포]
분산 테이블을 사용하지 않는다면, 다음 단일 명령어로 데이터베이스를 복원하십시오:
...
복원이 완료되면 Cloud에서 데이터를 사용할 수 있는지 확인할 수 있습니다:
...
ClickHouse Cloud는 내부적으로 `SharedMergeTree`를 사용하므로 이전 분산 테이블은 더 이상 필요하지 않습니다. 해당 테이블을 삭제하고, 쿼리에서 기존 테이블 이름을 그대로 사용할 수 있도록 동일한 이름의 뷰로 대체할 수 있습니다:
...
분산 테이블이 아닌 `ReplicatedMergeTree` 테이블은 `SharedMergeTree`로 복원됩니다:
