...
* **두 개의 윈도 기반의 더 스마트한 자동 스케일링 추천 기능:** ClickHouse Cloud는 이제 수직 자동 스케일링에 두 개의 조회 기간 윈도를 결합한 방식을 사용합니다. 기존의 단일 30시간 윈도 대신 3시간 &quot;소형 윈도&quot;와 30시간 &quot;대형 윈도&quot;를 함께 사용합니다. 이를 통해 스케일 다운 지연 시간을 최대 30시간에서 최소 3시간까지 줄여, 스케일 업 응답성은 유지하면서도 변동성이 큰 워크로드의 인프라 비용을 절감할 수 있습니다. 자세한 내용은 [블로그 게시물](https://clickhouse.com/blog/smarter-auto-scaling)을 참조하십시오.
* **Cloud Console의 모니터링:** 새로운 개요 대시보드와 인프라 대시보드를 통해 ClickHouse 서버의 동작을 더 잘 파악할 수 있습니다. 이제 관리자 사용자는 ClickHouse Cloud 사용 중 발생하는 일반적인 문제에 대해 이메일 알림을 받습니다. 자세한 내용은 [블로그 게시물](https://clickhouse.com/blog/clickhouse-cloud-new-monitoring-capabilities)을 참조하십시오.
* **컴퓨트-컴퓨트 분리(warehouses):** 기본 서비스(상위 서비스라고도 함)의 자동 유휴화 지원이 이제 공개 프리뷰로 순차적으로 제공되고 있습니다. 이 기능이 아직 조직에 활성화되지 않았다면, 액세스를 위해 [지원팀에 문의](https://clickhouse.com/support/program)하십시오. 이 기능이 일반 제공 단계로 전환되면, 기존 기본 서비스에는 자동 유휴화를 켤 수 있는 옵션이 제공되고 새 기본 서비스에는 기본적으로 자동 유휴화가 활성화됩니다.
...
* **GCP의 Bring Your Own Cloud (BYOC)가 이제 일반 제공됩니다:** ClickHouse BYOC는 이제 Google Cloud에서 일반 제공(GA)되므로, 자체 GCP 프로젝트에 ClickHouse 서비스를 배포하고 엄격한 데이터 상주 요구 사항을 준수할 수 있습니다. 자세한 내용은 [문서](/cloud/reference/byoc/overview)와 [블로그 게시물](https://clickhouse.com/blog/byoc-gcp-ga)을 참조하십시오. 액세스를 요청하려면 [문의](https://clickhouse.com/cloud/bring-your-own-cloud)하십시오.
* **선불 크레딧 및 결제 수단 변경에 대한 청구 알림:** ClickHouse Cloud는 이제 약정 계약에 따라 조직에 선불 크레딧이 추가되거나 결제 수단(신용카드 또는 마켓플레이스 구독)이 업데이트될 때 알림을 보냅니다. 알림은 기본적으로 UI와 이메일에 대해 활성화되어 있으며, Slack으로도 전송되도록 구성할 수 있습니다.
...
### 수평 확장(일반 제공) \{#horizontal-scaling-ga\}

수평 확장이 이제 일반 제공됩니다(Generally Available). API와 Cloud 콘솔을 통해 레플리카를 추가하여 서비스를 수평으로 확장할 수 있습니다. 자세한 내용은 [문서](/cloud/features/autoscaling/horizontal#manual-horizontal-scaling)를 참고하십시오.
...
* [수평 스케일링](/cloud/features/autoscaling/horizontal#manual-horizontal-scaling). 더 많은 병렬 처리가 필요한 워크로드를 이제 최대 10개의 레플리카로 구성할 수 있습니다(구성을 위해 지원팀에 문의하십시오)
