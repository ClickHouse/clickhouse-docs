...
ClickHouse가 관리하는 Postgres는 로컬 NVMe 스토리지를 기반으로 구축되어, 기존의 네트워크 연결 스토리지 아키텍처에서 발생하는 추가 비용 부담 없이 프로덕션급 성능과 네이티브 ClickHouse 통합을 제공합니다. 이 페이지에서는 서비스의 요금 모델, 사용 가능한 인스턴스 타입, 그리고 티어별 비교를 설명합니다.
...
제품이 베타 기간 동안 발전함에 따라 일반 제공(GA)에 앞서 가격 및 제공 방식이 조정될 수 있습니다. 다음 사항에 유의하십시오.

* 네트워크 이그레스 요금은 GA 이후 도입될 예정입니다. 데이터베이스와 동일한 위치에 배치된 애플리케이션은 egress 비용이 거의 발생하지 않을 것으로 예상됩니다.
* 현재 기준을 정하는 중인 한도를 초과하는 보존 기간에는 GA 시점부터 추가 백업 요금이 적용될 수 있습니다.
* Postgres와 ClickHouse가 동일한 리전에 함께 배치된 경우, ClickPipes를 통한 네이티브 CDC는 GA 이후에도 무료이거나 매우 낮은 요금으로 유지될 것으로 예상합니다. 이는 통합된 OLTP + OLAP 플랫폼이라는 비전과도 부합합니다.
* 스케일링, failover 및 대기 인스턴스 프로비저닝 시에는 데이터베이스를 계속 온라인 상태로 유지하기 위해 두 개의 인스턴스가 잠시 병렬로 실행됩니다. 전환이 완료되는 동안 두 인스턴스 모두에 대한 요금이 일시적으로 중복 청구될 수 있습니다. 이 기간은 인스턴스 유형과 스토리지 볼륨에 따라 달라집니다.
* 선택한 인스턴스 유형을 지정한 리전에서 일시적으로 사용할 수 없는 경우, 데이터베이스를 계속 온라인 상태로 유지하기 위해 이에 준하는 인스턴스 유형이 프로비저닝될 수 있습니다. 이 경우 프로비저닝된 인스턴스의 요율로 과금됩니다.
* 베타 기간 동안 실제 고객의 사용 패턴, 워크로드 특성, 인프라 요구 사항에 대해 더 많이 파악하게 되면, 기존 가격은 GA 시점에 가까워질수록 조정되거나 변경될 수 있습니다.