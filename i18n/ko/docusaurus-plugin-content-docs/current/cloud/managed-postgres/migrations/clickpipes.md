...
이제 ClickHouse Cloud는 외부 PostgreSQL 데이터베이스를 Managed Postgres 서비스로 마이그레이션할 수 있도록 ClickPipes를 제공합니다. 이 기본 제공 통합을 사용하면 소스 데이터베이스에 연결하고, 스키마를 내보내며, 이를 Managed Postgres로 가져오고, 지속적 복제를 설정하는 과정을 간소화할 수 있습니다.
