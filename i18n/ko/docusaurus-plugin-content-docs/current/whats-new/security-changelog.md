...
ClickHouse 서버에서 힙 버퍼 오버플로 문제가 발견되었습니다. ClickHouse 서버에 데이터를 로드할 수 있는 권한을 가진 악의적인 사용자는 잘못된 CapnProto 객체를 삽입하여 ClickHouse 서버를 비정상 종료시킬 수 있습니다.
...
ClickHouse 서버에서 힙 버퍼 오버플로 문제가 발견되었습니다. 공격자는 기본적으로 포트 8123에서 수신 대기하는 HTTP 엔드포인트로 특정 방식으로 조작한 HTTP 요청을 전송하여, 힙 기반 버퍼 오버플로우를 유발하고 ClickHouse 서버 프로세스를 비정상 종료시킬 수 있습니다. 이 공격에는 인증이 필요하지 않습니다.
...
제보: Yandex Information Security Team 소속 Andrey Krasichkov
...
제보: Yandex Information Security Team 소속 Andrey Krasichkov 및 Evgeny Sidorov
