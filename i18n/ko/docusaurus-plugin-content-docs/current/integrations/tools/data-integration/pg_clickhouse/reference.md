...
pg&#95;clickhouse를 사용해 보는 가장 간단한 방법은 [Docker image]를 사용하는 것입니다. 이 Docker 이미지는 pg&#95;clickhouse 및 [re2][re2
extension] 확장 기능이 포함된 표준 PostgreSQL Docker 이미지를 기반으로 합니다.
...
* 라이브러리 버전(PostgreSQL 18 이상에서 `PG_MODULE_MAGIC`으로 정의됨)은 전체 시맨틱 버전을 포함하며, `pgch_version()` 함수 또는 Postgres [`pg_get_loaded_modules()`] 함수의 출력에서 확인할 수 있습니다.
* 확장 기능 버전(컨트롤 파일에서 정의됨)은 메이저 및 마이너 버전만 포함하며, `pg_catalog.pg_extension` 테이블, `pg_available_extension_versions()` 함수의 출력, 그리고 `\dx
...
반면 마이너 또는 메이저 버전이 증가하는 릴리스에는 SQL 업그레이드 스크립트가 함께 제공되며, 해당 확장 기능을 포함하는 모든 기존 데이터베이스는 업그레이드를 적용하기 위해 `ALTER EXTENSION pg_clickhouse UPDATE`를 실행해야 합니다.
...
* `dbname`: 연결 시 사용할 ClickHouse 데이터베이스입니다. 기본값은
  &quot;default&quot;입니다.
* `fetch_size`: HTTP 스트리밍의 대략적인 바이트 단위 배치 크기입니다. 배치는
  행 경계에서 분할됩니다. 기본값은 `50000000`(50 MB)입니다. `0`은
  스트리밍을 비활성화하고 전체 응답을 버퍼링합니다. 외부 테이블은 이 값을
  재정의할 수 있습니다.
...
[CREATE FOREIGN TABLE]을 사용하여 ClickHouse 데이터베이스의 데이터를 조회할 수 있는
외부 테이블을 생성합니다:
...
* `database`: 원격 데이터베이스의 이름입니다. 기본값은 외부 서버에
  정의된 데이터베이스입니다.
* `fetch_size`: HTTP 스트리밍을 위한 대략적인 배치 크기(바이트 단위)입니다. 서버 수준의
  `fetch_size`를 재정의합니다. 기본값은 `50000000`(50 MB)입니다. `0`으로 설정하면
  스트리밍이 비활성화되고 전체 응답이 버퍼링됩니다.
* `table_name`: 원격 테이블의 이름입니다. 기본값은 외부 테이블에 지정된
  이름입니다.
* `engine`: ClickHouse 테이블에서 사용하는 [테이블 엔진(table engine)]입니다.
...
* `column_name`: ClickHouse 쪽 컬럼의 이름으로, 쿼리와 삽입 구문을 다시 생성할 때
  PostgreSQL 속성 이름보다 우선하여 사용됩니다. 따옴표로 묶지 않은 소문자
  PostgreSQL 컬럼 이름을 대소문자를 구분하는 ClickHouse 컬럼에 매핑할 때
  유용합니다. 예를 들면 다음과 같습니다.
...
* `SimpleAggregateFunction`: [SimpleAggregateFunction Type] 컬럼에 적용되는
  집계 함수의 이름입니다. 데이터 타입을 함수에 전달되는 ClickHouse 타입에
  매핑하고, 적절한 컬럼 옵션으로 집계 함수 이름을 지정하십시오.
...
[DROP FOREIGN TABLE]을(를) 사용하여 외부 테이블을 삭제합니다:
...
[SET]을 사용하여 pg&#95;clickhouse 사용자 지정 구성 매개변수를 설정합니다.
...
기본값은 `join_use_nulls 1, group_by_use_nulls 1, final 1`입니다. 빈 문자열로 설정하면
ClickHouse 서버의 설정으로 되돌아갑니다.
...
정규 표현식 함수와 연산자를 푸시다운할지 여부를 제어합니다. 기본값은 푸시다운 사용이며,
푸시다운되지 않게 하려면 이 매개변수를 false로 설정하십시오:

```sql
SET pg_clickhouse.pushdown_regex = 'false';
```

자세한 내용은 [정규 표현식](#regular-expressions)을 참고하십시오.
...
기본적으로 어떤 역할에도 이 함수에 대한 `EXECUTE` 권한이 없으므로, [GRANT]로
권한을 부여할 때는 임시 ClickHouse 쿼리를 실제로 실행해야 하는
역할(예: 전용 ClickHouse 관리자 역할)에만 부여하는 것이 좋습니다:
...
`pg_clickhouse`는 조건절(`HAVING`, `WHERE`)에서 사용되는 PostgreSQL 내장 함수 중 일부를 푸시다운합니다. 이 함수 집합은 다음과 같이 ClickHouse의 해당 함수에 매핑됩니다:
...
* `sin`, `cos`, `tan`, `atan`, `atan2`, `sinh`, `cosh`, `tanh`, `asinh`, `degrees`, `radians`, `pi`: 동일한 이름의 [ClickHouse 수학 함수](https://clickhouse.com/docs/sql-reference/functions/math-functions)입니다.
  `asin`, `acos`, `atanh`, `acosh`는 푸시다운되지 않습니다. 범위를 벗어난 입력에 대해 ClickHouse는 `NaN`을 반환하지만, PG
  는 오류를 발생시킵니다.
* `date_part`:
...
* `regexp_replace`: [replaceRegexpOne](https://clickhouse.com/docs/sql-reference/functions/string-replace-functions#replaceRegexpOne) 또는 `g` 플래그가 지정된 경우 [replaceRegexpOne](https://clickhouse.com/docs/sql-reference/functions/string-replace-functions#replaceRegexpAll)
...
* `json_extract_path`: [toJSONString](https://clickhouse.com/docs/sql-reference/functions/json-functions#toJSONString) + [하위 컬럼 구문](https://clickhouse.com/docs/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns)
* `jsonb_extract_path_text`: [하위 컬럼 구문](https://clickhouse.com/docs/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns)
* `jsonb_extract_path`: [toJSONString](https://clickhouse.com/docs/sql-reference/functions/json-functions#toJSONString) + [하위 컬럼 구문](https://clickhouse.com/docs/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns)
...
* `to_char(timestamp[tz], fmt)`: [formatDateTime](https://clickhouse.com/docs/sql-reference/functions/date-time-functions#formatDateTime)
  `fmt`가 문자열 상수이고 모든 키워드에 정확히 대응하는
  ClickHouse 등가 표현이 있는 경우입니다. 지원되는 키워드는 Compatibility
  Notes 아래의 [to&#95;char()](#to_char)를 참조하십시오. 그렇지 않으면 함수는
...
* `CURRENT_DATE`:
  [now](https://clickhouse.com/docs/sql-reference/functions/date-time-functions#now) 및
...
* `CURRENT_DATABASE`: PostgreSQL 함수에서 값으로 전달됩니다.
* `CURRENT_SCHEMA`: PostgreSQL 함수에서 값으로 전달됩니다.
* `CURRENT_CATALOG`: PostgreSQL 함수에서 값으로 전달됩니다.
* `CURRENT_USER`: PostgreSQL 함수의 반환값으로 전달됩니다.
* `USER`: PostgreSQL 함수에서 값으로 전달되는 값입니다.
...
* `!~*` (대소문자를 구분하지 않는 정규식 불일치): [match](https://clickhouse.com/docs/sql-reference/functions/string-search-functions#match)
* `->>` (JSON/JSONB 요소를 텍스트로 추출): [sub-column syntax](https://clickhouse.com/docs/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns)
* `->` (JSON/JSONB 추출): [toJSONString](https://clickhouse.com/docs/sql-reference/functions/json-functions#toJSONString) + [sub-column syntax](https://clickhouse.com/docs/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns)

### 사용자 정의 함수 \{#custom-functions\}

`pg_clickhouse`에서 생성한 이 사용자 정의 함수들은 PostgreSQL에 대응하는 함수가 없는 일부 ClickHouse 함수에 대해 원격 쿼리 푸시다운(쿼리 푸시다운)을 제공합니다. 이러한 함수들 중 어떤 것이든 푸시다운할 수 없으면 예외를 발생시킵니다.
...
pg&#95;clickhouse는 select core와 서드파티
확장 기능의 함수를 인식하여, 해당 함수를 ClickHouse의 동등한 함수로 푸시다운합니다.
...
이러한 [ordered-set aggregate functions]는 *direct 인수*를 매개변수로, `ORDER BY` 표현식을 인수로 전달하여 ClickHouse의 [Parametric
...
다음 PostgreSQL [윈도우 함수]는 해당하는 경우 프레임 지정도 포함한 `OVER
...
* `min` / `max` (`OVER` 절과 함께 사용)

순위 함수(`row_number`, `rank`, `dense_rank`, `ntile`, `cume_dist`,
`percent_rank`)는 ClickHouse에서 이러한 함수에 대한 프레임 지정을 허용하지 않으므로
푸시다운 시 프레임 절을 생략합니다.
...
[pg&#95;clickhouse.pushdown&#95;regex](#pg_clickhousepushdown_regex)가 true인 경우(기본값),
pg&#95;clickhouse는 정규 표현식을 ClickHouse에서 대응되는 형태로 푸시다운하고,
기본적인 수준의 호환성을 보장하려고 합니다. 다만 두 시스템의 차이점과
pg&#95;clickhouse가 이를 어떻게 처리하는지는 반드시 유의해야 합니다.

* PostgreSQL은 [POSIX Regular Expressions]를 지원하고 ClickHouse는
  [RE2 Regular Expressions][RE2]를 지원합니다. 동작 차이에 유의하십시오.
  정규 표현식이 ClickHouse에서 평가되는 경우(예: `WHERE` 절)에는 RE2로 작성하고,
  Postgres에서 평가되는 경우(예: `SELECT` 절)에는 POSIX로 작성하십시오.

* pg&#95;clickhouse는 Postgres의 [Regex flags]를 ClickHouse 정규 표현식 앞에
  `(?)` 형태로 붙여 푸시다운합니다. 예시는 다음과 같습니다.
  ...
  여기에는 `-s`가 포함된다는 점에 유의하십시오. 이는 ClickHouse에서 기본적으로
  활성화되는 `s`를 비활성화하여 Postgres 정규 표현식의 동작과 맞추기 위한 것입니다.
  Postgres 함수 호출의 플래그에 `s`가 포함된 경우 pg&#95;clickhouse는 `-s`를
  추가하지 않습니다. 안타깝게도 이 동작은 Postgres 24 및 이전 버전의 일부
  정규 표현식과의 호환성을 깨뜨립니다.

* ClickHouse에서 평가될 때 사용할 수 있는, 즉 두 시스템이 모두 지원하는 플래그는
  다음뿐입니다.

  * `i`: 대소문자를 구분하지 않음
  * `m`: 멀티라인 모드:
  * `s`: `.`이 `\n`과 일치하도록 함
  * `p`: 부분적 줄바꿈 민감 매칭(`s`와 동일하게 처리됨)
...
* 예외는 `regexp_replace()`로, 이 함수는 `g` 플래그도 지원합니다. `g`가
  설정되면 pg&#95;clickhouse는 `replaceRegexpOne()` 대신
  `replaceRegexpAll()`을 사용하고, 다른 플래그를 앞에 붙이기 전에 `g`를 제거합니다.

* Postgres의 `regexp_replace()`에서 replacement 인수는 전체 일치를 가리킬 때
  `\&`를 지원하지만, ClickHouse에서는 전체 일치에 `\0`을 사용합니다.
  함수가 ClickHouse로 푸시다운될 때는 반드시 `\0`을 사용하십시오.

모호함을 완전히 피하려면
[pg&#95;clickhouse.pushdown&#95;regex](#pg_clickhousepushdown_regex)를 설정해
Postgres 정규 표현식이 ClickHouse로 푸시다운되지 않도록 하고,
pg&#95;clickhouse가 ClickHouse 호환 [RE2] 정규 표현식의
[직접 푸시다운](#re2)을 지원하는 [re2 확장 기능] 사용을 고려하십시오.

### `to_char()` \{#to_char\}

`timestamp` 및 `timestamp with time zone`에 대한 PostgreSQL [`to_char()`]는 포맷 인수가 NULL이 아닌 문자열 상수이고, 그 안의 모든 PostgreSQL 키워드에 바이트 단위까지 동일한 ClickHouse 대응 [formatDateTime]이 있을 때만 ClickHouse [formatDateTime]으로 푸시다운됩니다. 포맷이 동적이거나(`Const`가 아님) 지원되지 않는 키워드 또는 수정자를 하나라도 포함하면, 해당 호출은 PostgreSQL에서 로컬로 평가됩니다. 부분 번역으로 푸시다운을 시도하는 경우는 없으므로 출력은 PG 호환성을 유지합니다.

`numeric`, `interval` 및 기타 타임스탬프가 아닌 타입에 대한 2개 인수 형태의 `to_char()`는 푸시다운되지 않습니다. ClickHouse [formatDateTime]은 날짜-시간 값만 포맷합니다.

#### 변환된 키워드 \{#translated-keywords\}

| PostgreSQL                 | ClickHouse | 의미                           |
| -------------------------- | ---------- | ---------------------------- |
| `YYYY`, `yyyy`             | `%Y`       | 4자리 연도                       |
| `YY`, `yy`                 | `%y`       | 2자리 연도                       |
| `MM`, `mm`                 | `%m`       | 앞자리를 0으로 채운 월 (01–12)        |
| `DD`, `dd`                 | `%d`       | 앞자리를 0으로 채운 일 (01–31)        |
| `DDD`, `ddd`               | `%j`       | 앞자리를 0으로 채운 연중 일수 (001–366)  |
| `HH24`, `hh24`             | `%H`       | 앞자리를 0으로 채운 24시간제 시각 (00–23) |
| `HH`, `hh`, `HH12`, `hh12` | `%I`       | 앞자리를 0으로 채운 12시간제 시각 (01–12) |
| `MI`, `mi`                 | `%i`       | 앞자리를 0으로 채운 분 (00–59)        |
| `SS`, `ss`                 | `%S`       | 앞자리를 0으로 채운 초 (00–59)        |
| `Q`, `q`                   | `%Q`       | 분기 (1–4)                     |
...
`"..."`로 감싼 텍스트는 있는 그대로 처리되며, 리터럴 `%`는 ClickHouse의 지정자 접두사를 이스케이프하기 위해 `%%`로 두 번 써야 합니다. 따옴표 밖의 `\"`도 리터럴 `"`로 그대로 처리됩니다. `"..."` 내부에서는 백슬래시가 `"`만 이스케이프하고, 그 외의 백슬래시 시퀀스는 리터럴 텍스트로 처리됩니다.
...
[ClickHouse settings]: https://clickhouse.com/docs/operations/settings/settings "ClickHouse 문서: 세션 설정"
...
[PREPARE notes]: https://www.postgresql.org/docs/current/sql-prepare.html#SQL-PREPARE-NOTES "PostgreSQL 문서: PREPARE 관련 참고 사항"

[query parameters]: https://clickhouse.com/docs/guides/developer/stored-procedures-and-prepared-statements#alternatives-to-prepared-statements-in-clickhouse "ClickHouse 문서: ClickHouse에서 prepared statement의 대안"

[underlying bug]: https://github.com/ClickHouse/ClickHouse/issues/85847 "ClickHouse/ClickHouse#85847 multipart form의 일부 쿼리에서 settings를 읽지 않는 문제"

[fixed]: https://github.com/ClickHouse/ClickHouse/pull/85570 "ClickHouse/ClickHouse#85570 multipart를 사용하는 HTTP 수정"

[BYTEA]: https://www.postgresql.org/docs/current/datatype-binary.html "PostgreSQL 문서: 이진 데이터 타입"

[GRANT]: https://www.postgresql.org/docs/current/sql-grant.html "PostgreSQL 문서: GRANT"

[String]: https://clickhouse.com/docs/sql-reference/data-types/string "ClickHouse 문서: String"
...
[POSIX Regular Expressions]: https://www.postgresql.org/docs/18/functions-matching.html#FUNCTIONS-POSIX-REGEXP "PostgreSQL 문서: POSIX 정규 표현식"
...
[`to_char()`]: https://www.postgresql.org/docs/current/functions-formatting.html "PostgreSQL 문서: 데이터 타입 형식 지정 함수"

[formatDateTime]: https://clickhouse.com/docs/sql-reference/functions/date-time-functions#formatDateTime "ClickHouse 문서: formatDateTime"