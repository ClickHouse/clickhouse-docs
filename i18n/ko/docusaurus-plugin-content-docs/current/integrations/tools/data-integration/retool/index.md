...
description: '풍부한 사용자 인터페이스를 갖춘 웹 및 모바일 앱을 빠르게 빌드하고, 복잡한 작업을 자동화하며, AI와 연동할 수 있습니다. 이 모든 것은 사용자의 데이터로 구동됩니다.'
...
# Retool을 ClickHouse에 연결하기 \{#connecting-retool-to-clickhouse\}
...
Retool 계정에 로그인한 후 _Resources_ 탭으로 이동합니다. "Create New" -> "Resource"를 선택합니다:
...
설정 마법사에서 "Driver name"으로 `com.clickhouse.jdbc.ClickHouseDriver`가 선택되어 있는지 확인합니다:
...
인스턴스에서 SSL 사용이 필요하거나 ClickHouse Cloud를 사용하는 경우 연결 문자열에 `&ssl=true`를 추가하여 `jdbc:clickhouse://HOST:PORT/DATABASE?user=USERNAME&password=PASSWORD&ssl=true`와 같이 설정합니다.
