---
description: 'ClickHouse gRPC 인터페이스 문서'
...
인터페이스 사양은 [clickhouse_grpc.proto](https://github.com/ClickHouse/ClickHouse/blob/master/src/Server/grpc_protos/clickhouse_grpc.proto)에 정의되어 있습니다.

## gRPC 구성 \{#grpc-interface-configuration\}

gRPC 인터페이스를 사용하려면 기본 [서버 구성](../../operations/configuration-files.md)에서 `grpc_port`를 설정합니다. 기타 구성 옵션은 아래 예제를 참조하십시오.
...
제공된 [사양](https://github.com/ClickHouse/ClickHouse/blob/master/src/Server/grpc_protos/clickhouse_grpc.proto)을 사용하여 gRPC가 지원하는 프로그래밍 언어라면 어느 것이든 클라이언트를 작성할 수 있습니다.
또는 내장 Python 클라이언트를 사용할 수도 있습니다. 이 클라이언트는 리포지토리의 [utils/grpc-client/clickhouse-grpc-client.py](https://github.com/ClickHouse/ClickHouse/blob/master/utils/grpc-client/clickhouse-grpc-client.py)에 위치합니다. 내장 클라이언트를 사용하려면 [grpcio 및 grpcio-tools](https://grpc.io/docs/languages/python/quickstart) Python 모듈이 필요합니다.
...
* `--format OUTPUT_FORMAT, -f OUTPUT_FORMAT` – 결과 출력 [형식](../../interfaces/formats.md)입니다. 대화형 모드의 기본값: `PrettyCompact`.
* `--debug` – 디버그 정보를 표시하도록 활성화합니다.

클라이언트를 대화형 모드로 실행하려면 `--query` 인수 없이 실행합니다.
