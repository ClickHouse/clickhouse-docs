...
:::note
이 프로바이더는 활발히 개발 중입니다. 현재 릴리스는 LINQ 쿼리(조인, 서브쿼리, 집합 연산 포함), `SaveChanges` / `BulkInsertAsync`를 통한 `INSERT`, 전체 DDL(`CREATE` / `ALTER` / `DROP`)을 포함한 마이그레이션, 그리고 ClickHouse 전용 테이블 엔진 설정을 지원합니다. `UPDATE` / `DELETE`는 지원되지 않습니다.
:::
...
#### 지원되는 LINQ 연산 \{#ef-core-linq\}

**쿼리:** `Where`, `OrderBy`, `Take`, `Skip`, `Select`, `First`, `Single`, `Any`, `All`, `Count`, `Distinct`, `AsNoTracking`

**GROUP BY 및 집계:** `Count`, `LongCount`, `Sum`, `Average`, `Min`, `Max`와 함께 사용하는 `GroupBy` — `HAVING`(`.GroupBy()` 뒤의 `.Where()`)을 포함하며, 단일 프로젝션의 여러 집계와 집계 결과에 대한 `OrderBy`도 지원합니다.

**조인:** `Join`(INNER), `GroupJoin`/`SelectMany` 패턴(LEFT 및 CROSS). LEFT JOIN은 일치하는 항목이 없는 행에 실제 `null`을 반환합니다(아래의 [LEFT JOIN null semantics](#ef-core-join-nulls) 참조).

**서브쿼리:** 프로젝션 내의 상관 `Contains` / `IN`, `Any` / `EXISTS`, `All`, 그리고 스칼라 서브쿼리.
...
**수학 함수:** 표준 `Math` 및 `MathF` 메서드는 해당하는 ClickHouse 함수로 변환되며, 산술, 로그, 삼각 및 유틸리티 함수를 포함합니다.

##### LEFT JOIN의 NULL 의미 체계 \{#ef-core-join-nulls\}

프로바이더는 JOIN 동작에 대한 Entity Framework의 기대에 맞추기 위해 모든 연결 경로에 `set_join_use_nulls=1`을 자동으로 주입합니다.

ClickHouse 서버 또는 프로필에서 이 설정 변경을 허용하지 않는 경우(예: `readonly=1` 프로필) 다음과 같이 비활성화하십시오:

```csharp
optionsBuilder.UseClickHouse(connectionString, o => o.DisableJoinNullSemantics());
```

옵트아웃을 활성화하면 LEFT JOIN은 ClickHouse 컬럼의 기본값을 반환하므로, EF의 null 기반 탐색 감지 기능이 더 이상 예상대로 작동하지 않습니다. `== null` 대신 `0` / `""`과 명시적으로 비교하십시오.
...
`ToTable(name, t => ...)` Fluent API를 통해 ClickHouse 테이블 엔진과 엔진별 절을 구성합니다. 엔진을 구성하지 않으면 프로바이더는 엔터티의 기본 키(primary key)에서 파생된 `ORDER BY`를 사용하는 `MergeTree`를 기본값으로 사용합니다.
...
| Engine                                  | 체이닝 메서드                                                                                                   | 참고                                |
| --------------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `MergeTree`                             | `HasMergeTreeEngine()`                                                                                    | 별도 구성하지 않으면 기본값으로 사용됩니다           |
| `ReplacingMergeTree`                    | `HasReplacingMergeTreeEngine("Version", "IsDeleted")` 또는 `HasReplacingMergeTreeEngine<T>(e => e.Version)` | `Version` / `IsDeleted` 컬럼은 선택 사항 |
| `SummingMergeTree`                      | `HasSummingMergeTreeEngine(…)` 또는 `HasSummingMergeTreeEngine<T>(e => new { … })`                          | 합산할 컬럼은 선택 사항                     |
| `AggregatingMergeTree`                  | `HasAggregatingMergeTreeEngine()`                                                                         | —                                 |
| `CollapsingMergeTree`                   | `HasCollapsingMergeTreeEngine("Sign")` 또는 `HasCollapsingMergeTreeEngine<T>(e => e.Sign)`                  | `Sign` 컬럼은 `Int8`이어야 합니다          |
| `VersionedCollapsingMergeTree`          | `HasVersionedCollapsingMergeTreeEngine("Sign", "Version")` 또는 `<T>(e => e.Sign, e => e.Version)`          | —                                 |
| `GraphiteMergeTree`                     | `HasGraphiteMergeTreeEngine("config_section")`                                                            | —                                 |
| `Log`, `TinyLog`, `StripeLog`, `Memory` | `HasLogEngine()`, `HasTinyLogEngine()`, `HasStripeLogEngine()`, `HasMemoryEngine()`                       | ORDER BY / PARTITION BY 없음        |

**엔진 절:** `WithOrderBy`, `WithPartitionBy`, `WithPrimaryKey`, `WithSampleBy`, `WithTtl`, `WithSettings`. 모두 `HasXxxEngine()`에서 반환된 엔진 빌더에 적용됩니다.

**컬럼 수준 기능:** `HasCodec`, `HasTtl`, `HasComment`, `HasDefault` — 모두 마이그레이션에 반영됩니다.

**데이터 스키핑 인덱스** — `HasIndex(...).HasSkippingIndexType(...)`를 통해:
...
ClickHouse에는 이에 해당하는 기능이 없으므로 일반(스키핑이 아닌) 인덱스는 별다른 경고 없이 무시됩니다. 또한 ClickHouse는 고유성을 강제하지 않으므로 고유 인덱스를 사용하면 예외가 발생합니다.

#### 마이그레이션 \{#ef-core-migrations\}

일반적인 EF Core 마이그레이션 워크플로는 다음과 같습니다:
...
| `ALTER TABLE MODIFY COLUMN`            | 형식 변경과 어노테이션 추가/제거(CODEC, TTL, COMMENT, DEFAULT)를 처리합니다         |
...
| Feature                              | Reason                                                                                                          |
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| 외래 키                                 | ClickHouse는 외래 키를 강제하지 않습니다. 마이그레이션에서는 `AddForeignKey`를 허용하지 않으며, 모델 유효성 검사기는 모델 빌드 시 경고를 출력합니다.                |
| 고유 제약 조건 / 고유 인덱스                    | ClickHouse는 고유성을 강제하지 않습니다. 고유 인덱스는 마이그레이션 시 오류를 발생시킵니다.                                                        |
| 서버 생성 값(auto-increment / `IDENTITY`) | ClickHouse에는 이에 해당하는 기능이 없습니다.                                                                                  |
| `Nested(…)` 컬럼                       | 매핑된 CLR 형식으로는 아직 지원되지 않습니다.                                                                                     |
| JSON으로 저장되는 소유 엔터티(`.ToJson()`)      | 소유 엔터티의 구조적 JSON 매핑은 아직 구현되지 않았습니다. 대신 `Json` 컬럼에서는 `JsonNode` / `string`을 사용하십시오([JSON 컬럼](#ef-core-json) 참조). |

마이그레이션 외에도 이 프로바이더는 아직 다음을 지원하지 않습니다.

* **`UPDATE` / `DELETE`**
* **트랜잭션**: `BeginTransaction`은 아무 작업도 수행하지 않는 빈 작업입니다. ClickHouse는 ACID 트랜잭션을 지원하지 않습니다.
* **JSON 경로 쿼리 변환**: LINQ의 `entity.Data["key"]`는 ClickHouse의 `data.key` SQL 구문으로 변환되지 않습니다. JSON이 아닌 컬럼으로 필터링하고, JSON은 메모리에서 확인하십시오.
