...
클라이언트 저장소에는 환경 변수를 사용하는 예제가 여러 개 포함되어 있으며, 예를 들어 [ClickHouse Cloud에서 테이블을 생성하는 예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/create_table_cloud.ts), [async insert를 사용하는 예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/async_insert.ts) 등이 있습니다.
...
`command`, `exec`, `insert`, `select`와 같이 쿼리나 구문을 전송하는 모든 메서드는 결과에 `query_id`를 포함합니다. 이 고유 식별자는 쿼리마다 클라이언트에서 할당되며, [서버 구성](/operations/server-configuration-parameters/settings)에서 활성화된 경우 `system.query_log`에서 데이터를 조회하거나, 장시간 실행되는 쿼리를 취소하는 데 유용하게 사용할 수 있습니다([예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/cancel_query.ts) 참고). 필요하다면 `query_id`는 `command`/`query`/`exec`/`insert` 메서드의 매개변수에서 사용자가 재정의할 수 있습니다.
...
[소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/array_json_each_row.ts).
...
**예시:** (Node.js 전용) 기존 `on('data')` 방식을 사용하여 `JSONEachRow` 형식으로 쿼리 결과를 스트리밍합니다. 이는 `for await const` 구문과 서로 바꿔 사용할 수 있습니다. [소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_streaming_json_each_row.ts).
...
Insert 메서드는 기본적으로 `await`과 함께 사용하도록 설계되었습니다. 그러나 입력 스트림을 먼저 지정한 뒤, 스트림이 완료되었을 때에만(이때 `insert` promise도 resolve됨) `insert` 연산을 `await`하도록 할 수도 있습니다. 이는 이벤트 리스너와 같은 시나리오에서 유용할 수 있으나, 클라이언트 측에서 다양한 엣지 케이스를 처리해야 하므로 오류 처리가 복잡해질 수 있습니다. 대신 [이 예시](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/async_insert_without_waiting.ts)에 나와 있듯이 [비동기 insert(async inserts)](/optimize/asynchronous-inserts) 사용을 고려하십시오.

:::tip
이 메서드로는 모델링하기 어려운 커스텀 INSERT 문이 있는 경우 [command 메서드](./js.md#command-method) 사용을 고려하십시오.

사용 예시는 [INSERT INTO ... VALUES](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_values_and_functions.ts) 및 [INSERT INTO ... SELECT](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_from_select.ts) 예제에서 확인할 수 있습니다.
:::
...
[소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_file_stream_csv.ts). 관련 항목: [파일 스트리밍](./js.md#streaming-files-nodejs-only).
...
자세한 내용은 [소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_exclude_columns.ts)를 참고하십시오.

**예시**: 클라이언트 인스턴스에 설정된 데이터베이스가 아닌 다른 데이터베이스에 데이터를 INSERT합니다. [소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_into_different_db.ts).
...
[소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/create_table_cloud.ts).
...
[소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/create_table_single_node.ts).
...
* [Dynamic/Variant/JSON 사용 예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/dynamic_variant_json.ts)
* [Time/Time64 사용 예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/time_time64.ts)
...
하지만 `DateTime` 또는 `DateTime64` 컬럼을 사용하는 경우, 문자열과 JS Date 객체 두 형식 모두를 사용할 수 있습니다. JS Date 객체는 `date_time_input_format`을 `best_effort`로 설정한 상태에서 `insert`에 그대로 전달할 수 있습니다. 자세한 내용은 이 [예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_js_dates.ts)를 참고하십시오.
...
자세한 내용은 [이 예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/insert_decimals.ts)를 참조하십시오.
...
**예시:** 매개변수를 사용하는 쿼리입니다.
[소스 코드](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/query_with_parameter_binding.ts)
.
...
readonly=1 USER에 적용되는 제한 사항을 더 잘 보여 주는 [예제](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/read_only_user.ts)를 참고하십시오.
