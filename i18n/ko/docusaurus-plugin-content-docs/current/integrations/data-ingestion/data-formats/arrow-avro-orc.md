...
keywords: ['Apache Avro', 'Apache Arrow', 'ORC 형식', '열 지향 형식', '빅데이터 형식']
doc_type: 'guide'
---

Apache는 널리 사용되는 [Avro](https://avro.apache.org/), [Arrow](https://arrow.apache.org/), [Orc](https://orc.apache.org/)를 포함해 분석 환경에서 활발히 사용되는 여러 데이터 형식을 출시했습니다. ClickHouse는 이 목록에 있는 모든 형식을 사용한 데이터 가져오기와 내보내기를 지원합니다.
...
### Avro 및 ClickHouse 데이터 타입 \{#avro-and-clickhouse-data-types\}

Avro 파일을 가져오거나 내보낼 때 [데이터 타입 매핑](/interfaces/formats/Avro#data-type-mapping)을 고려하십시오. Avro 파일에서 데이터를 로드할 때는 명시적 형 변환을 사용하여 데이터 타입을 변환하십시오.
...
또한 수동으로 변환해야 하는 데이터 타입이 있는지 확인하려면 [데이터 타입 매핑](/interfaces/formats/Arrow#data-types-matching)을(를) 참조하십시오.
...
ClickHouse에서는 동일한 ArrowStream 형식을 사용하여 Arrow 스트림도 읽을 수 있습니다.
...
ClickHouse는 다양한 시나리오와 플랫폼을 지원하기 위해 텍스트 및 바이너리 형식을 포함한 여러 형식을 지원합니다. 다음 문서에서 더 많은 형식과 이를 다루는 방법을 살펴보십시오:
