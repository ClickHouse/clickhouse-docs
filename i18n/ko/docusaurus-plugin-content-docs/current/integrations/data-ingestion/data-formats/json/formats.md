...
가장 널리 사용되는 JSON 데이터 형식 중 하나는 [이 예시](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)와 같이 JSON 배열 안에 JSON 객체들의 목록을 두는 방식입니다.
...
JSON 객체 목록을 가져오려면 [`JSONEachRow`](/interfaces/formats/JSONEachRow) 형식을 사용할 수 있습니다([list.json](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json) 파일의 데이터를 삽입하는 경우):
...
일부 상황에서는 JSON 객체 목록이 배열 요소가 아니라 객체 속성으로 인코딩될 수 있습니다(예시는 [objects.json](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/objects.json)을 참조하십시오):
...
공간을 절약하기 위해 JSON 파일이 객체 대신 배열로 인코딩되는 경우가 있습니다. 이 경우 [JSON 배열 목록](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/arrays.json)을 다루게 됩니다:
...
일부 경우에는 데이터가 행 단위가 아니라 컬럼 단위로 인코딩될 수 있습니다. 이 경우 상위 JSON 객체에 값이 포함된 컬럼들이 들어 있습니다. [다음 파일](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns.json)을 살펴보십시오:
...
객체 대신 [컬럼 배열](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/columns-array.json)을 다룰 때는 [`JSONCompactColumns`](/interfaces/formats/JSONCompactColumns) 포맷을 사용할 경우 더 간결한 형식도 지원합니다:
...
JSON 객체를 파싱하지 않고 단일 `String`(또는 `JSON`) 컬럼에 그대로 저장하고자 하는 경우가 있습니다. 서로 다른 구조를 가진 JSON 객체 목록을 처리할 때 유용합니다. 예를 들어, 상위 목록 안에 서로 다른 JSON 객체가 여러 개 들어 있는 [이 파일](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/custom.json)을 살펴보겠습니다.
...
[중첩 JSON 객체](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)를 다루는 경우, 명시적인 스키마를 별도로 정의하고 복합 타입([`Array`](/sql-reference/data-types/array.md), [`JSON`](/integrations/data-formats/json/overview), [`Tuple`](/sql-reference/data-types/tuple.md))을 사용하여 데이터를 로드할 수 있습니다:
...
[다음 설정 옵션](/operations/settings/settings-formats.md/#input_format_import_nested_json)을 활성화하면 [중첩된 JSON 키](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list-nested.json)를 참조할 수 있습니다:
...
이 테이블에는 3개의 컬럼을 가진 [원본 JSON 데이터](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/list.json)를 그대로 삽입할 수 있습니다:
...
BSON 데이터를 가져올 때는 [BSONEachRow](/interfaces/formats/BSONEachRow) 형식을 사용합니다. [이 BSON 파일](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.bson)에서 데이터를 가져오겠습니다:
