...
사용하는 JSON이 매우 동적이고 고유한 키가 많으며, 동일한 키에 여러 타입이 사용되는 것을 알고 있다면, 데이터가 줄바꿈으로 구분된 JSON 형식이라 하더라도 각 키마다 컬럼을 추론하려고 `JSONEachRow`와 스키마 추론을 사용하는 것은 권장하지 않습니다.

다음 예제는 위에서 사용한 [Python PyPI 데이터셋](https://clickpy.clickhouse.com/)의 확장 버전입니다. 여기서는 임의의 키-값 쌍을 가진 `tags` 컬럼을 추가했습니다.
