...
1. `comments.tsv`라는 TSV 파일에 일부 Hacker News 댓글 데이터가 있고, 헤더 행에 컬럼 이름이 포함되어 있다고 가정합니다. 데이터를 삽입할 때는 [input format](/interfaces/formats)을 지정해야 하며, 여기서는 `TabSeparatedWithNames`를 사용합니다:
...
3. `author` 컬럼의 값을 소문자로 변환하려고 합니다. 이는 [`lower` 함수](/sql-reference/functions/string-functions#lower)로 간단히 처리할 수 있습니다. 또한 `comment` 문자열을 토큰으로 분리하여 그 결과를 `tokens` 컬럼에 저장하려고 하며, 이는 [`extractAll` 함수](/sql-reference/functions/string-search-functions#extractAll)를 사용하여 수행할 수 있습니다. 이러한 작업 전체를 하나의 `clickhouse-client` 명령으로 실행합니다. `comments.tsv` 파일이 `<` 연산자를 사용해 `clickhouse-client`로 전달되는 방식에 주목하십시오:
...
:::note
`input` 함수는 데이터를 `hackernews` 테이블에 삽입하는 동시에 변환할 수 있어서 여기에서 유용합니다. `input`의 인수는 들어오는 원시 데이터의 형식이며, 다른 많은 테이블 함수(table functions)에서도 이를 볼 수 있습니다(들어오는 데이터에 대한 스키마를 지정하는 곳).
:::

4. 이제 완료되었습니다! 데이터가 ClickHouse에 적재되었습니다:
...
5. 또 다른 방법은 `cat`과 같은 도구를 사용하여 파일을 `clickhouse-client`로 스트리밍하는 것입니다. 예를 들어, 다음 명령은 `<` 연산자를 사용하는 것과 동일한 결과를 제공합니다.
...
`clickhouse-client`를 로컬 운영 체제에 설치하는 방법에 대한 자세한 내용은 [`clickhouse-client` 문서 페이지](/interfaces/cli)를 참조하십시오.
