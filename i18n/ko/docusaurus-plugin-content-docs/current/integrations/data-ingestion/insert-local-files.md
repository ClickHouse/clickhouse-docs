---
'sidebar_label': '로컬 파일 삽입'
'sidebar_position': 2
'title': '로컬 파일 삽입'
'slug': '/integrations/data-ingestion/insert-local-files'
'description': '로컬 파일 삽입에 대해 알아보세요'
'show_related_blogs': true
'doc_type': 'guide'
'keywords':
- 'insert local files ClickHouse'
- 'ClickHouse local file import'
- 'clickhouse-client file upload'
---


# 로컬 파일 삽입

`clickhouse-client`를 사용하여 로컬 파일을 ClickHouse 서비스로 스트리밍할 수 있습니다. 이를 통해 강력하고 편리한 ClickHouse 함수를 사용하여 데이터를 전처리할 수 있습니다. 예를 살펴보겠습니다...

1. `comments.tsv`라는 TSV 파일이 있고, 해당 파일에는 일부 Hacker News 댓글이 포함되어 있으며, 헤더 행에는 컬럼 이름이 포함되어 있다고 가정해 보겠습니다. 데이터를 삽입할 때 [입력 형식](/interfaces/formats)을 지정해야 합니다. 우리의 경우는 `TabSeparatedWithNames`입니다:
...
2. Hacker News 데이터를 위한 테이블을 생성해 보겠습니다:
...
3. `author` 컬럼을 소문자로 변환하고 싶습니다. 이는 [`lower` 함수](/sql-reference/functions/string-functions#lower)를 사용하여 쉽게 수행할 수 있습니다. 또한 `comment` 문자열을 토큰으로 분할하고 결과를 `tokens` 컬럼에 저장하고 싶습니다. 이는 [`extractAll` 함수](/sql-reference/functions/string-search-functions#extractAll)를 사용하여 할 수 있습니다. 이 모든 작업을 하나의 `clickhouse-client` 명령어에서 수행할 수 있습니다 - `comments.tsv` 파일이 `<` 연산자를 사용하여 `clickhouse-client`로 파이프된 것을 주목하세요:
...
:::note
`input` 함수는 우리가 `hackernews` 테이블에 삽입할 때 데이터를 변환할 수 있게 해주므로 유용합니다. `input`의 인자는 들어오는 원시 데이터의 형식이며, 이는 다른 많은 테이블 함수에서도 볼 수 있습니다 (들어오는 데이터의 스키마를 지정하는 곳).
:::

4. 그게 전부입니다! 데이터가 ClickHouse에 올라갔습니다:
...
5. 또 다른 옵션은 `cat`와 같은 도구를 사용하여 파일을 `clickhouse-client`로 스트리밍하는 것입니다. 예를 들어, 다음 명령은 `<` 연산자를 사용하는 것과 동일한 결과를 생성합니다:
...
자세한 내용은 [클릭하우스 클라이언트 문서 페이지](/interfaces/cli)를 방문하여 `clickhouse-client`를 로컬 운영 체제에 설치하는 방법을 확인하세요.
