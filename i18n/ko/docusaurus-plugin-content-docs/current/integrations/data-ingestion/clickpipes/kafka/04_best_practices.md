...
* 개별 메시지는 기본적으로 가장 작은 XS 레플리카 크기로 실행할 때 2MB(비압축 기준), 더 큰 레플리카 크기에서는 8MB(비압축 기준)로 제한됩니다. 이 한도를 초과하는 메시지는 오류와 함께 거부됩니다. 더 큰 메시지 크기가 필요하면 지원팀에 문의하십시오.
...
* 배치 크기가 최대 한도에 도달했을 때(행 100,000개 또는 파드 메모리 1GB당 28MB)
