...
:::tip
단계별 지침은 [지속적 수집을 위한 Unordered 모드 구성](/integrations/clickpipes/object-storage/s3/unordered-mode)을 참조하십시오.
:::

S3 ClickPipe를 사용해 암시적인 순서가 없는 파일도 수집하도록 구성할 수 있습니다. 이를 위해 버킷에 연결된 [Amazon SQS](https://aws.amazon.com/sqs/) 큐를 설정하면 되며, 선택적으로 [Amazon EventBridge](https://aws.amazon.com/eventbridge/)를 이벤트 라우터로 사용할 수 있습니다. 이렇게 하면 ClickPipes가 객체 생성 이벤트를 수신하고, 파일 이름 규칙과 관계없이 새 파일을 수집할 수 있습니다.

:::note
Unordered 모드는 Amazon S3에서만 **지원되며**, 공개 버킷(public bucket) 또는 S3 호환 서비스에서는 **지원되지 않습니다**. 이 모드를 사용하려면 버킷에 연결된 [Amazon SQS](https://aws.amazon.com/sqs/) 큐를 설정해야 하며, 선택적으로 [Amazon EventBridge](https://aws.amazon.com/eventbridge/)를 이벤트 라우터로 사용할 수 있습니다.
:::
...
파일은 `max insert bytes` 또는 `max file count`에 설정된 임곗값에 도달하거나, 구성 가능한 간격(기본적으로 30초)이 지나면 수집됩니다. 특정 파일이나 특정 시점에서 수집을 시작하는 것은 **불가능하며**, ClickPipes는 항상 선택한 경로의 모든 파일을 로드합니다. DLQ가 구성된 경우, 실패한 메시지는 DLQ의 `maxReceiveCount` 매개변수에 설정된 횟수만큼 다시 큐에 넣어지고 재처리됩니다.
...
[Amazon EventBridge](https://aws.amazon.com/eventbridge/)를 통해 S3 이벤트 알림을 SQS로 전송할 수도 있습니다. 이 방식은 대부분의 사용 사례에 권장됩니다. EventBridge는 더 다양한 이벤트 필터링을 지원하고 여러 대상으로 팬아웃할 수 있으며, S3의 prefix별 event type당 알림 1개 규칙 제한을 받지 않기 때문입니다. 단계별 지침은 [지속적 수집을 위한 비순차 모드 구성](/integrations/clickpipes/object-storage/s3/unordered-mode)을 참조하십시오.
...
객체 스토리지 ClickPipes는 [수직 오토스케일링 설정](/cloud/features/autoscaling/vertical#configuring-vertical-auto-scaling)에 의해 결정되는 최소 ClickHouse 서비스 크기를 기준으로 확장됩니다. ClickPipe의 크기는 파이프를 생성할 때 결정됩니다. 이후 ClickHouse 서비스 설정을 변경해도 ClickPipe 크기에는 영향을 주지 않습니다.
