...
:::note
도움이 필요하면 [리포지토리에 이슈를 등록하거나](https://github.com/ClickHouse/clickhouse-kafka-connect/issues) [ClickHouse 공개 Slack](https://clickhouse.com/slack)에서 질문을 남기십시오.
:::
**ClickHouse Kafka Connect Sink**는 Kafka 토픽의 데이터를 ClickHouse 테이블로 전달하는 Kafka 커넥터입니다.
...
| `port`                                          | ClickHouse 포트 — 기본값은 클라우드의 HTTPS용 8443이지만, HTTP(셀프 호스티드의 기본값)에서는 8123이어야 합니다                                                                                                                           | `8443`                                                   |
| `ssl`                                           | ClickHouse에 대한 SSL 연결 활성화                                                                                                                                                                              | `true`                                                   |
...
| `errors.retry.timeout`                          | Kafka Connect의 최대 재시도 시간(밀리초)입니다. 재시도를 하지 않으려면 `0`을 사용합니다. 무한 재시도를 하려면 `-1`을 사용합니다. 권장 값은 &quot;10000&quot; ms(10초)보다 큰 값입니다. 타임아웃                                                                     | `"0"`                                                    |
...
| `tolerateStateMismatch`                         | 커넥터가 AFTER&#95;PROCESSING에 저장된 현재 오프셋보다 &quot;이전&quot;인 레코드를 드롭하도록 허용합니다(예: 오프셋 5가 전송되었으나, 마지막으로 기록된 오프셋이 250인 경우). 장애 후 수집을 복구하는 데 사용해야 하며, 작업이 완료되면 다시 `"false"`로 되돌려야 합니다.                          | `"false"`                                                |
...
| `bufferCount` (v1.3.6부터)                        | ClickHouse로 플러시하기 전에 메모리에 버퍼링할 레코드 수입니다. `0`은 내부 버퍼링을 비활성화합니다. `exactlyOnce=true`에서는 버퍼링이 지원되지 않습니다.                                                                                                   | `"0"`                                                    |
| `bufferFlushTime` (v1.3.6부터)                    | `exactlyOnce=false`일 때 flush 전에 레코드를 버퍼링하는 최대 시간(밀리초)입니다. `0`은 시간 기반 flush를 비활성화합니다. 기본값은 `0`입니다. 시간 기반 임곗값에만 필요합니다. `bufferCount > 0`일 때만 적용됩니다.                                                      | `"0"`                                                    |
| `reportInsertedOffsets` (v1.3.6부터)              | `exactlyOnce=false`일 때 `preCommit`에서 `currentOffsets` 대신 성공적으로 삽입된 오프셋만 반환하도록 활성화합니다. 단, `ignorePartitionsWhenBatching=true`인 경우에는 적용되지 않으며, 이 경우에도 `currentOffsets`가 반환됩니다.                           | `"false"`                                                |
...
* (2) - struct에 `oneof`와 같은 union이 포함된 경우, 컨버터는 필드 이름에 접두사/접미사를 추가하지 않도록 설정해야 합니다. 이를 위해 `ProtobufConverter`에 대해 `generate.index.for.unions=false` [설정](https://docs.confluent.io/platform/current/schema-registry/connect.html#protobuf)을 사용합니다.
...
시작할 때 사용할 수 있는 가장 기본적인 구성입니다. Kafka Connect를 분산 모드로 실행하고 있고, SSL이 활성화된 ClickHouse 서버가 `localhost:8443`에서 실행 중이며, 데이터는 스키마리스 JSON 형식이라고 가정합니다.
...
:::note
위의 커넥터 구성에서는 워커 설정에서 `connector.client.config.override.policy=All`을 사용해 클라이언트 재정의를 활성화해야 합니다. 자세한 내용은 [Kafka Connect 문서](https://docs.confluent.io/platform/current/connect/references/allconfigs.html#override-the-worker-configuration)를 참조하십시오.
:::
...
###### Avro 타입 대응 \{#avro-type-mapping\}

아래의 타입 대응은 Kafka Connect의 공식 Avro 직렬화/역직렬화 구현인 `io.confluent.connect.avro.AvroConverter`에서 정의합니다. 변환 로직에 대한 자세한 정보는 Kafka Connect [문서](https://docs.confluent.io/platform/current/connect/userguide.html#avro)를 참조하십시오.
...
| Avro 유형 | Kafka Connect 유형 | 지원 여부 | 비고                                                                                                                                                                                                                                                  |
| ------- | ---------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| null    | *N/A*            | ❌     | 독립 실행형 타입으로는 지원되지 않지만 union에서는 사용할 수 있습니다.                                                                                                                                                                                                          |
| boolean | BOOLEAN          | ✅     |                                                                                                                                                                                                                                                     |
| int     | INT8/INT16/INT32 | ✅     | 기본값은 INT32입니다. schema에 `connect.type=int8` 속성이 있으면 INT8로 해석되며, `connect.type=int16`인 경우에도 동일하게 INT16으로 해석됩니다.                                                                                                                                       |
| long    | INT64            | ✅     |                                                                                                                                                                                                                                                     |
| float   | FLOAT32          | ✅     |                                                                                                                                                                                                                                                     |
| double  | FLOAT64          | ✅     |                                                                                                                                                                                                                                                     |
| bytes   | BYTES            | ✅     |                                                                                                                                                                                                                                                     |
...
| record  | STRUCT           | ✅     |                                                                                                                                                                                                                                                     |
| enum    | STRING           | ✅     |                                                                                                                                                                                                                                                     |
| array   | ARRAY/MAP        | ✅     | 기본값은 ARRAY입니다. 필드가 원래 `AvroData.fromConnectSchema`를 통해 생성된 경우 MAP으로 해석됩니다([소스](https://github.com/confluentinc/schema-registry/blob/174907bfc0d9424e8d02e788f450f4afcdda1750/avro-data/src/main/java/io/confluent/connect/avro/AvroData.java#L943)) |
| map     | MAP              | ✅     |                                                                                                                                                                                                                                                     |
| union   | STRUCT/`<T>`     | ⚠️    | 기본값은 STRUCT입니다. `flatten.singleton.unions=true`인 경우 union 정의의 단일 타입 `T`로 해석됩니다([문서](https://docs.confluent.io/cloud/current/connectors/reference/connector-configuration.html#value-converter-flatten-singleton-unions) 참조)                         |
| fixed   | BYTES            | ⚠️    | 고정 `decimal` 논리 타입은 지원되지 않습니다(아래 참조).                                                                                                                                                                                                               |

Kafka Connect 타입과 ClickHouse 타입 간의 대응은 [지원되는 데이터 타입](#supported-data-types)을 참조하십시오.
...
###### Protobuf 타입 대응 \{#proto-type-mapping\}

아래 타입 대응은 Kafka Connect의 공식 Protobuf 직렬화/역직렬화 구현인 `io.confluent.connect.protobuf.ProtobufConverter`에서 정의합니다. 변환 로직에 관한 자세한 내용은 Kafka Connect [문서](https://docs.confluent.io/platform/current/connect/userguide.html#json-schema-and-protobuf)를 참조하십시오.
...
| string                                  | STRING                                  | ✅         |                                                                                                                                                                 |
| bytes                                   | BYTES                                   | ✅         |                                                                                                                                                                 |
| enum                                    | INT32/STRING                            | ✅         | 기본값은 STRING입니다. `int.for.enums=true`이면 INT32로 확인됩니다([schema registry 문서](https://docs.confluent.io/platform/current/schema-registry/connect.html#protobuf) 참조). |
| message                                 | STRUCT                                  | ⚠️        | 아래의 지원되지 않는 스키마 섹션을 참조하십시오                                                                                                                                   |
| repeated T (where T is not a map entry) | ARRAY                                   | ✅         |                                                                                                                                                                 |
| `map<K, V>`                             | MAP                                     | ✅         |                                                                                                                                                                 |
| oneof                                   | STRUCT                                  | ⚠️        | oneof를 ClickHouse 스키마로 변환하는 아래 섹션을 참조하십시오                                                                                                                    |
...
###### `oneof` 필드를 ClickHouse 컬럼으로 변환할 때의 참고 사항 \{#oneof-translation\}

이 커넥터는 Protobuf 유니온(`oneof`)을 ClickHouse Variant 유형으로 변환하는 기능을 지원하지 않습니다. 대신 `oneof` 필드를 ClickHouse 테이블 스키마에 각각의 널 허용 필드로 나열하십시오.
...
커넥터는 다음 Protobuf 스키마를 지원하지 않습니다:
...
CH 버전 26.1부터는 `allow_experimental_nullable_tuple_type=1`로 설정된 경우 이 스키마를 지원합니다([이 문서 페이지](https://clickhouse.com/docs/operations/settings/settings#allow_experimental_nullable_tuple_type) 참조).
...
내부 버퍼링을 사용하면 싱크 작업이 여러 `poll()` 호출의 레코드를 누적한 후 더 큰 배치로 ClickHouse에 플러시할 수 있습니다. 이는 각 poll이 파티션별로 작은 배치를 많이 생성하는 워크로드에서 처리량을 높일 수 있습니다.

주요 동작:

* `bufferCount`는 플러시하기 전에 버퍼링할 레코드 수를 제어합니다.
* `bufferFlushTime`은 버퍼링된 레코드를 플러시하기 전까지의 최대 대기 시간(밀리초)을 설정합니다.
* `bufferFlushTime`은 `bufferCount > 0`일 때만 적용됩니다.
* `bufferCount=0` 및 `bufferFlushTime=0`이면 버퍼링이 비활성화된 상태(기본 동작)로 유지됩니다.
* `exactlyOnce=true`인 경우 버퍼링은 지원되지 않습니다.

버퍼링이 exactly-once 모드와 호환되지 않는 이유:
버퍼링은 배치 경계를 변경하므로 ClickHouse 블록 중복 제거와 커넥터 오프셋 상태 머신이 깨집니다.
이 문제를 해결하려면 커넥터 구성에서 `exactlyOnce=false`로 exactly-once 모드를 비활성화하거나 `bufferCount=0`으로 버퍼링을 비활성화하십시오.
...
:::note
위 속성을 사용하려면 worker 설정에서 `connector.client.config.override.policy=All`을 통해 클라이언트 재정의를 활성화해야 합니다. 자세한 내용은 [Kafka Connect 문서](https://docs.confluent.io/platform/current/connect/references/allconfigs.html#override-the-worker-configuration)를 참조하십시오.
:::
...
:::note
위의 커넥터 구성에서는 worker 구성에서 `connector.client.config.override.policy=All`을 설정해 클라이언트 override를 활성화해야 합니다. 자세한 내용은 [Kafka Connect 문서](https://docs.confluent.io/platform/current/connect/references/allconfigs.html#override-the-worker-configuration)를 참조하십시오.
:::
...
:::note
이 조정은 정확히 한 번 처리 보장에 영향을 줄 수 있습니다.
:::
