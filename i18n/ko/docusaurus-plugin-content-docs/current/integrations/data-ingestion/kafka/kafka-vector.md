...
Vector는 [sources](https://vector.dev/docs/introduction/concepts/#sources)를 사용하여 푸시 또는 풀 모델로 데이터를 수집합니다. 한편 [sinks](https://vector.dev/docs/introduction/concepts/#sinks)는 이벤트의 목적지를 정의합니다. 따라서 Kafka source와 ClickHouse sink를 사용합니다. Kafka는 sink로 지원되지만 ClickHouse source는 제공되지 않는다는 점에 유의해야 합니다. 이로 인해 Vector는 ClickHouse에서 Kafka로 데이터를 전송하려는 경우에는 적합하지 않습니다.
