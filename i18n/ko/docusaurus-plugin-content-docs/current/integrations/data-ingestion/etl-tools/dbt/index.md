...
| 페이지                                                                                           | 설명                                                |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| [기능 및 구성](/integrations/dbt/features-and-configurations)                  | 사용 가능한 기능과 전반적인 설정에 대한 설명                         |
| [머티리얼라이제이션](/integrations/dbt/materializations)                                        | 사용 가능한 머티리얼라이제이션과 해당 설정                     |
| [가이드](/integrations/dbt/guides)                                                            | ClickHouse와 함께 dbt를 사용하는 방법에 대한 가이드               |
| [머티리얼라이제이션: materialized&#95;view](/integrations/dbt/materialization-materialized-view) | materialized&#95;view 머티리얼라이제이션에 대한 구체적인 문서 |
...
## dbt 개념과 지원되는 머티리얼라이즈 \{#concepts-and-supported-materializations\}

dbt는 모델(model)이라는 개념을 도입합니다. 이는 잠재적으로 여러 테이블을 조인할 수 있는 SQL 문으로 정의됩니다. 모델은 여러 가지 방식으로 &quot;머티리얼라이즈(materialize)&quot;될 수 있습니다. 머티리얼라이즈는 모델의 SELECT 쿼리에 대한 빌드 전략을 의미합니다. 머티리얼라이즈를 구현하는 코드는 새로운 릴레이션을 생성하거나 기존 릴레이션을 업데이트하기 위해, 사용 중인 SELECT 쿼리를 다른 SQL 문으로 감싸는 보일러플레이트 SQL입니다.

dbt는 5가지 유형의 머티리얼라이즈를 제공합니다. 이들은 모두 `dbt-clickhouse`에서 지원됩니다:
...
추가적인 구문과 절을 통해 기반 데이터가 변경될 때 이러한 모델을 어떻게 업데이트해야 하는지 정의합니다. dbt는 일반적으로 성능이 문제가 되기 전까지는 view 머티리얼라이즈로 시작할 것을 권장합니다. table 머티리얼라이즈는 모델의 쿼리 결과를 테이블로 저장하여, 더 많은 스토리지를 사용하는 대신 쿼리 시간 성능을 향상시킵니다. incremental 방식은 이를 더 확장하여, 이후 기반 데이터의 업데이트가 대상 테이블에 반영되도록 합니다.

[현재 ClickHouse 어댑터](https://github.com/silentsokolov/dbt-clickhouse)는 **dictionary**, **분산 테이블**, **distributed incremental** 머티리얼라이즈도 지원합니다. 또한 이 어댑터는 dbt [snapshots](https://docs.getdbt.com/docs/building-a-dbt-project/snapshots#check-strategy)과 [seeds](https://docs.getdbt.com/docs/building-a-dbt-project/seeds)를 지원합니다.

다음은 `dbt-clickhouse`의 [실험적 기능](/beta-and-experimental-features)입니다:

| 유형                                      | 지원 여부                         | 세부 정보                                                                                                                                                                                |
| --------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Materialized View 머티리얼라이즈       | 예. 명시적 대상 지정을 사용한 생성은 Beta 단계 | [materialized view](/sql-reference/statements/create/view#materialized-view)를 생성합니다.                                                                                                 |
| 분산 테이블 머티리얼라이즈       | 예, 실험적                        | [분산 테이블](/engines/table-engines/special/distributed)을 생성합니다.                                                                                                              |
| Distributed incremental 머티리얼라이즈 | 예, 실험적                        | 분산 테이블과 동일한 아이디어를 기반으로 하는 incremental 모델입니다. 모든 전략이 지원되는 것은 아니므로, 자세한 내용은 [해당 문서 섹션](/integrations/dbt/materializations#materialization-distributed-incremental)을 참조하십시오. |
| Dictionary 머티리얼라이즈              | 예, 실험적                        | [dictionary](/engines/table-engines/special/dictionary)를 생성합니다.                                                                                                                      |
