...
`partitionColumn`, `lowerBound`, `upperBound`, `numPartitions`를 지정해야 하며, 이는 여러 워커가 병렬로
데이터를 읽을 때 테이블을 어떻게 파티션할지 정의합니다.
자세한 내용은 Apache Spark의 공식 문서
[JDBC configurations](https://spark.apache.org/docs/latest/sql-data-sources-jdbc.html#data-source-option)를 참조하십시오.
