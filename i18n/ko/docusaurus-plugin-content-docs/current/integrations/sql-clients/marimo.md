...
title: 'ClickHouse와 함께 marimo 사용하기'
doc_type: 'guide'
keywords: ['marimo', 'notebook', '데이터 분석', 'python', '시각화']
integration:
...
## 1. SQL 지원이 포함된 marimo 설치 \{#install-marimo-sql\}
...
<Image img={add_db_panel} size="sm" border alt="새 데이터베이스 추가" />

데이터베이스 세부 정보를 입력하는 창이 표시됩니다.

<Image img={add_db_details} size="md" border alt="데이터베이스 세부 정보 입력" />

그다음 연결을 설정하기 위해 실행할 수 있는 셀이 생성됩니다.

<Image img={run_cell} size="md" border alt="셀을 실행하여 ClickHouse에 연결" />

## 3. SQL 실행 \{#run-sql\}

연결을 설정한 후 새 SQL 셀을 생성하고 ClickHouse 엔진을 선택합니다.

<Image img={choose_sql_engine} size="md" border alt="SQL 엔진 선택" />
...
<Image img={dropdown_cell_chart} size="lg" border alt="드롭다운, 테이블, 차트의 조합" />
...
<Image img={run_app_view} size="md" border alt="앱 뷰 실행" />
