...
[Databrain](https://usedatabrain.com)은 고객과 대화형 대시보드, 메트릭 및 데이터 시각화를 구축하고 공유할 수 있도록 지원하는 임베디드 애널리틱스 플랫폼입니다. Databrain은 HTTPS 인터페이스를 사용해 ClickHouse에 연결되므로, 현대적이고 사용하기 쉬운 인터페이스를 통해 ClickHouse 데이터를 손쉽게 시각화하고 분석할 수 있습니다.

<Image size="md" img={databrain_01} alt="ClickHouse 데이터 시각화를 보여주는 Databrain 대시보드 인터페이스" border />

<br />

이 가이드에서는 Databrain을 ClickHouse 인스턴스에 연결하는 단계를 안내합니다.
...
3. 허용 목록에 Databrain의 IP 주소를 추가합니다.

:::tip
허용 목록에 추가해야 하는 최신 IP 주소 목록은 [Databrain의 IP 허용 문서](https://docs.usedatabrain.com/guides/datasources/allow-access-to-our-ip)를 참고하십시오.
:::
...
* **임베디드 애널리틱스**: 애플리케이션에 대시보드와 메트릭을 직접 삽입합니다.
