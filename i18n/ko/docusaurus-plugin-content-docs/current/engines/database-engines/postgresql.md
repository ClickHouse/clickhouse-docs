...
ClickHouse 데이터베이스가 PostgreSQL 서버와 데이터를 교환하는 예:
...
PostgreSQL에서 테이블 구조를 수정했다고 가정해 보십시오:

```sql
postgre> ALTER TABLE test_table ADD COLUMN data Text
```

데이터베이스를 생성할 때 `use_table_cache` 매개변수를 `1`로 설정했기 때문에 ClickHouse의 테이블 구조가 캐시되었고, 그 결과 변경되지 않았습니다:
...
테이블을 detach한 뒤 다시 attach하자, 구조가 업데이트되었습니다:
