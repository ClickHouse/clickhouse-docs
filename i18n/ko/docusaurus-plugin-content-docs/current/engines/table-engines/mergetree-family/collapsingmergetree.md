---
description: 'MergeTree에서 상속하며, 머지 과정에서 행을 축약하는 로직이 추가된 엔진입니다.'
keywords: ['업데이트', '축약']
...
사용자 활동의 마지막 상태만 필요하므로, 아래에 표시된 것처럼 원래 &quot;state&quot; 행과 삽입한 &quot;cancel&quot;
행을 삭제하여 객체의 잘못된(이전) 상태를 하나로 축약할 수 있습니다:
...
ClickHouse가 데이터 [파트](/concepts/glossary#parts)를 머지할 때,
동일한 정렬 키(`ORDER BY`)를 가진 연속된 각 행 그룹은 최대 2개의 행으로 축약됩니다.
...
따라서 축약 동작은 통계 계산 결과를 변경하지 않아야 합니다.
변경 내용은 점진적으로 축약되어, 결국 거의 모든 객체에 대해 마지막 상태만 남게 됩니다.
...
`CollapsingMergeTree` 테이블에서 완전히 &quot;축약&quot;된 데이터를 얻어야 한다면 집계가 필요합니다.
축약을 최종적으로 마무리하려면, `GROUP BY` 절과 sign을 반영하는 집계 함수를 사용하여 쿼리를 작성하십시오.
...
`uniq` 집계는 객체가 적어도 하나의 축약되지 않은(non-collapsed) 상태를 가진 경우 계산할 수 있습니다.
`min` 및 `max` 집계는 `CollapsingMergeTree`가 축약된 상태의 이력을 저장하지 않기 때문에 계산할 수 없습니다.
...
위에서 반환된 데이터를 살펴보며 축약이 실제로 발생했는지 확인해 보겠습니다...
두 개의 `INSERT` 쿼리로 두 개의 데이터 파트를 생성했습니다.
`SELECT` 쿼리는 두 개의 스레드에서 수행되었고, 행의 순서는 임의였습니다.
그러나 데이터 파트 간 머지가 아직 발생하지 않았기 때문에 축약은 **발생하지 않았으며**,
...
집계가 필요하지 않고 축약을 강제로 수행하려는 경우, `FROM` 절에 `FINAL` 수정자를 사용할 수도 있습니다.
