...
* `unicode_word`는 Unicode 단어 경계 규칙(UAX #29와 유사)을 사용해 문자열을 토큰으로 분리합니다. ASCII 영숫자와 밑줄은 연결 문자와 함께 토큰을 구성합니다(문자의 경우 `:`, 같은 타입의 문자에 대해서는 `.` 및 `'`). 비-ASCII Unicode 문자는 한 글자짜리 토큰이 됩니다.
