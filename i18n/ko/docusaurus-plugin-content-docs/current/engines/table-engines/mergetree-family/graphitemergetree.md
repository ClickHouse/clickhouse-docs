---
description: 'Graphite 데이터의 희소화(thinning) 및 집계/평균화(롤업)를 위해 설계되었습니다.'
...
이 엔진은 [Graphite](http://graphite.readthedocs.io/en/latest/index.html) 데이터의 희소화(thinning)와 집계/평균화(롤업)를 위해 설계되었습니다. Graphite의 데이터 저장소로 ClickHouse를 사용하려는 개발자에게 유용할 수 있습니다.

롤업이 필요하지 않다면 Graphite 데이터를 저장하기 위해 어떤 ClickHouse 테이블 엔진이든 사용할 수 있지만, 롤업이 필요하다면 `GraphiteMergeTree`를 사용하십시오. 이 엔진은 저장 공간 사용량을 줄이고 Graphite에서 실행되는 쿼리의 효율을 높입니다.
...
이러한 컬럼의 이름은 롤업(롤업) 설정에서 지정해야 합니다.
...
롤업에 대한 설정은 서버 구성의 [graphite&#95;rollup](../../../operations/server-configuration-parameters/settings.md#graphite) 매개변수로 정의합니다. 매개변수 이름은 임의로 지정할 수 있습니다. 여러 개의 구성을 만들어 서로 다른 테이블에 사용할 수 있습니다.
...
### 규칙 유형 없이 구성한 예시 \{#configuration-example\}
