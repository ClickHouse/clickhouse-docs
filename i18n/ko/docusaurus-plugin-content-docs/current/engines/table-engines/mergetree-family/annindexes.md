...
#### 벡터 유사성 인덱스 생성 \{#creating-a-vector-similarity-index\}

벡터 유사성 인덱스는 새 테이블에서 다음과 같이 생성할 수 있습니다:
...
* `L2Distance`, [Euclidean distance](https://en.wikipedia.org/wiki/Euclidean_distance)로, 유클리드 공간에서 두 점을 잇는 선분의 길이를 나타내며,
* `cosineDistance`, [cosine distance](https://en.wikipedia.org/wiki/Cosine_similarity#Cosine_distance)로, 0이 아닌 두 벡터 사이의 각도를 나타내거나
* `dotProduct`는 [dot product](https://en.wikipedia.org/wiki/Dot_product)(내적)으로, 두 벡터의 대응 요소를 곱한 값의 합을 나타냅니다. 정규화된 데이터에서는 `cosineDistance`와 동일합니다.

정규화된 데이터에서는 일반적으로 `L2Distance`가 가장 좋은 선택이며, 그렇지 않은 경우에는 스케일을 보정하기 위해 `cosineDistance` 사용을 권장합니다.

:::note
거리 함수 `L2Distance`와 `cosineDistance`의 경우 값이 작을수록 유사도가 높음을 의미하는 반면, `dotProduct`의 경우 값이 클수록 유사도가 높음을 의미합니다.
따라서 `L2Distance` 및 `cosineDistance`로 구성된 벡터 인덱스는 `SELECT [...] ORDER BY [...] ASC` 쿼리에서만 사용할 수 있으며(`ASC`는 `ORDER BY`의 기본값), `dotProduct`로 구성된 벡터 인덱스는 `SELECT [...] ORDER BY [...] DESC` 쿼리에서만 사용할 수 있습니다.
:::
...
대부분의 사용 사례에서 모든 HNSW 전용 매개변수의 기본값은 충분히 적절하게 작동합니다.
