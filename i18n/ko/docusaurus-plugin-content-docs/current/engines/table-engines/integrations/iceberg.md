...
`IcebergS3`의 경우, 선택 사항인 `extra_credentials` 매개변수를 사용하여 ClickHouse Cloud에서 역할 기반 액세스용 `role_arn`을 전달할 수 있습니다. 구성 단계는 [Secure S3](/cloud/data-sources/secure-s3)를 참조하십시오.
...
다음 표는 스키마 추론 과정에서 Iceberg 데이터 타입이 ClickHouse 데이터 타입에 어떻게 매핑되는지(읽기용) 보여줍니다.
...
| `timestamp`        | `DateTime64(6)`        | 마이크로초, 타임존 없음                |
| `timestamptz`      | `DateTime64(6, 'UTC')` | 마이크로초, UTC 타임존               |
| `timestamp_ns`     | `DateTime64(9)`        | 나노초, 타임존 없음 (Iceberg v3에서만)  |
| `timestamptz_ns`   | `DateTime64(9, 'UTC')` | 나노초, UTC 타임존 (Iceberg v3에서만) |
