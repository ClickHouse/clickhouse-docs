...
이 엔진은 [Amazon S3](https://aws.amazon.com/s3/) 생태계와의 통합을 제공합니다. 이 엔진은 [HDFS](/engines/table-engines/integrations/hdfs) 엔진과 유사하지만 S3에 특화된 기능을 제공합니다.
...
* `partition_strategy` – 옵션: `WILDCARD` 또는 `HIVE`. `WILDCARD`는 경로에 `{_partition_id}`가 포함되어 있어야 하며, 이 값이 파티션 키로 대체됩니다. `HIVE`는 와일드카드를 허용하지 않으며, 경로를 테이블 루트로 간주하고, 파일 이름은 Snowflake ID, 확장자는 파일 형식인 Hive 스타일 파티션 디렉터리를 생성합니다. 기본값은 `WILDCARD`입니다.
