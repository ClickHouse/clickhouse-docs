---
description: 'JOIN 연산에 사용할 수 있도록 미리 준비해 두는 선택적 데이터 구조입니다.'
sidebar_label: 'Join'
sidebar_position: 70
...
`INSERT` 쿼리를 사용하여 `Join`-엔진 테이블에 데이터를 추가할 수 있습니다. 테이블이 `ANY` 엄격성 모드로 생성된 경우 중복 키에 대한 데이터는 무시됩니다. `ALL` 엄격성 모드에서는 모든 행이 추가됩니다.
