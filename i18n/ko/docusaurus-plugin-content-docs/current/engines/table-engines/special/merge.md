---
description: '`Merge` 엔진은 (`MergeTree`와 혼동하지 말아야 함) 자체적으로 데이터를 저장하지 않고,
  여러 개의 다른 테이블에서 동시에 읽을 수 있도록 합니다.'
sidebar_label: 'Merge'
sidebar_position: 30
...
`Merge` 엔진은 (`MergeTree`와 혼동하지 말아야 함) 자체적으로 데이터를 저장하지 않고, 여러 개의 다른 테이블에서 동시에 읽을 수 있도록 합니다.

읽기 작업은 자동으로 병렬 처리됩니다. 테이블에 쓰는 작업은 지원되지 않습니다. 읽을 때는 실제로 읽는 테이블에 인덱스가 있으면 해당 인덱스를 사용합니다.
