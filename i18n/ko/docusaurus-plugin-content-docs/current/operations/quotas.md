...
:::note ClickHouse Cloud의 쿼터
쿼터는 ClickHouse Cloud에서 지원되지만, [DDL 구문](/sql-reference/statements/create/quota)을 사용하여 생성해야 합니다. 아래에 설명한 XML 구성 방식은 **지원되지 않습니다**.
:::

쿼터를 사용하면 일정 기간 동안 리소스 사용량을 제한하거나 리소스 사용량을 추적할 수 있습니다.
쿼터는 일반적으로 &#39;users.xml&#39;인 사용자 설정에서 구성합니다.

시스템에는 단일 쿼리의 복잡성을 제한하는 기능도 있습니다. [쿼리 복잡성에 대한 제한](../operations/settings/query-complexity.md) 섹션을 참고하십시오.

쿼리 복잡성 제한과 달리 쿼터는 다음과 같습니다.
...
기본적으로 쿼터는 리소스 사용량을 제한하지 않고 매 시간마다 이를 추적합니다.
...
&#39;statbox&#39; 쿼터의 경우, 제한은 매 1시간과 매 24시간(86,400초)에 대해 설정됩니다. 시간 간격은 구현에 따라 정의된 고정된 시점부터 계산됩니다. 즉, 24시간 간격이 자정부터 시작된다고 보장되지는 않습니다.

간격이 끝나면 수집된 값은 모두 초기화됩니다. 다음 1시간에 대해서는 쿼터 계산이 다시 처음부터 시작됩니다.
...
`queries_per_normalized_hash` – 단일 정규화 쿼리의 최대 실행 횟수입니다. 정규화 쿼리는 리터럴이 플레이스홀더로 대체된 쿼리이므로 `SELECT 1`과 `SELECT 2`는 동일한 정규화 쿼리로 간주됩니다. 이 제한은 개별 정규화 쿼리 패턴마다 서로 독립적으로 추적됩니다.

하나 이상의 시간 구간에서 제한을 초과하면, 어떤 제한을 어떤 구간에서 초과했는지와 새 구간이 언제 시작되는지(언제 다시 쿼리를 전송할 수 있는지)를 설명하는 텍스트와 함께 예외가 발생합니다.

쿼터는 &quot;quota key&quot; 기능을 사용하여 여러 키별 리소스 사용량을 서로 독립적으로 보고할 수 있습니다. 다음은 이에 대한 예시입니다:
...
DDL 구문을 사용할 때는 정규화된 쿼리 해시를 기준으로 쿼터 키를 지정할 수도 있으므로, 각기 다른 쿼리 패턴마다 자체적인 독립 쿼터 버킷이 할당됩니다:

```sql
CREATE QUOTA my_quota KEYED BY normalized_query_hash FOR INTERVAL 1 hour MAX queries = 100 TO my_user;
```

이 예시에서 사용자는 서로 다른 각 정규화된 쿼리를 시간당 최대 100회까지 실행할 수 있습니다. `SELECT number FROM numbers(1)`와 `SELECT number FROM numbers(2)`는 정규화된 형태가 같으므로 동일한 버킷을 공유하지만, `SELECT number, number FROM numbers(1)`는 별도의 버킷을 사용합니다.
