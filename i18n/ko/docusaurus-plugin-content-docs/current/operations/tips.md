...
transparent huge pages는 항상 `madvise`로 설정하십시오. 오래된 커널(5.9 이전)에서는 THP를 `always`로 설정하면 성능이 크게 저하될 수 있습니다. 커널이 메모리 조각 모음을 수행하는 데 과도한 시간을 소비하기 때문이며, 특히 RAM이 64 GB 이상인 시스템에서 더욱 그렇습니다. 커널 5.9에서는 THP를 훨씬 더 잘 처리하는 proactive 압축이 도입되었지만, ClickHouse는 여전히 시작 시 THP가 `always`로 설정되어 있으면 경고를 표시하므로 커널 버전과 관계없이 `madvise`를 권장 설정으로 사용하십시오.
