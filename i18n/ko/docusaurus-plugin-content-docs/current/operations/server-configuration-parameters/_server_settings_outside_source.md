...
| `disallow_config_defined_profiles_for_sql_defined_users` | 이 설정을 활성화하면 기본 프로필을 제외하고, XML 설정 파일(`users.xml` 등)에 정의된 구성 기반 설정 프로필이 SQL로 정의된 사용자에게 적용되지 않도록 합니다. SQL로 정의된 사용자에게 부여된 역할에 할당된 프로필도 검사합니다.                                                                                                                                                                                                                      | `false` |
