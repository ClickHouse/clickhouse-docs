...
<SettingsInfoBlock type="UInt64" default_value="10" />원격 iceberg 카탈로그에서 최신 메타데이터를 비동기적으로 가져오기 위한 스레드 풀의 크기입니다. 이 풀은 현재 활성 상태인 모든 테이블이 공유합니다.
...
<SettingsInfoBlock type="UInt64" default_value="0" />동시에 보관할 수 있는 경량 스냅샷의 최대 개수입니다. 0은 제한이 없음을 의미합니다. 스냅샷 수가 이 한도에 도달하면 새 스냅샷을 생성하려고 할 때 예외가 발생합니다.
