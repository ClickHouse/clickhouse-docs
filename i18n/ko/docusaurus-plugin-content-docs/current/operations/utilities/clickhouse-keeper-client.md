...
* `--password=PASSWORD` — 인증에 사용할 비밀번호입니다. `CLICKHOUSE_KEEPER_PASSWORD` 환경 변수 또는 XML 구성 파일의 `<zookeeper><password>` 아래에서도 설정할 수 있습니다.
* `--identity=IDENTITY` — `digest` 인증 방식에 사용할 ID입니다. `CLICKHOUSE_KEEPER_IDENTITY` 환경 변수 또는 XML 구성 파일의 `<zookeeper><identity>` 아래에서도 설정할 수 있습니다.
...
## 환경 변수 \{#clickhouse-keeper-client-env\}

* `CLICKHOUSE_KEEPER_PASSWORD` — 명령줄에서 `--password`를 지정하지 않은 경우 기본 비밀번호로 사용됩니다.
* `CLICKHOUSE_KEEPER_IDENTITY` — 명령줄에서 `--identity`를 지정하지 않은 경우 기본 ID로 사용됩니다.

## 인증 \{#clickhouse-keeper-client-auth\}

인증이 필요한 Keeper 서버에 연결할 때 비밀번호는 다음 우선순위에 따라 조회됩니다(먼저 일치하는 항목이 적용됩니다):

1. `--password` 명령줄 인수
2. `CLICKHOUSE_KEEPER_PASSWORD` 환경 변수
3. `--config-file`로 지정된 XML 구성 파일의 `<zookeeper><password>`

동일한 우선순위가 `--identity` / `CLICKHOUSE_KEEPER_IDENTITY` / `<zookeeper><identity>`에도 적용됩니다.

인증 설정이 포함된 XML 구성 파일 예시:
...
## 예시 \{#clickhouse-keeper-client-example\}
