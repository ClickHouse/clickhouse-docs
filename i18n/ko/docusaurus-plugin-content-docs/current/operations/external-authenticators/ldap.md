...
* `follow_referrals` — 서버에서 반환된 LDAP referral을 LDAP 클라이언트 라이브러리가 자동으로 따라가도록 허용하는 플래그입니다. 기본값은 `false`입니다. 참고: 이 설정은 Microsoft Active Directory 환경에서, 상위 수준 base DN(예: `DC=example,DC=com`)에 대한 subtree 검색이 referral/검색 reference(예: `DC=DomainDnsZones,...`)를 반환할 수 있는 경우에 주로 관련이 있습니다.
  * 교차 파티션 검색이 명시적으로 필요하고, 환경이 이를 허용하도록 구성된 경우에만 `true`를 지정하십시오.
  * referral을 따라가지 않으려면 `false`를 지정하십시오. AD 도메인 루트 검색에서는 이를 권장합니다.
