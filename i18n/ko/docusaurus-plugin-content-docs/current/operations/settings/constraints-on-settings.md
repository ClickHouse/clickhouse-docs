---
description: '`user.xml` 설정 파일의 `profiles` 섹션에서 설정에 대한 제약 조건을 정의할 수 있으며, 이를 통해 사용자가 `SET` 쿼리를 사용해 일부 설정을 변경하지 못하도록 할 수 있습니다.'
...
ClickHouse에서 설정에 대한 「제약 조건(constraints)」이란 설정에 부여할 수 있는 제한이나 규칙을 의미합니다. 이러한 제약은 데이터베이스의 안정성, 보안 및 예측 가능한 동작을 유지하기 위해 적용될 수 있습니다.

## CONSTRAINT 정의 \{#defining-constraints\}

설정에 대한 CONSTRAINT은 `user.xml` 설정 파일의 `profiles` 섹션에서 정의할 수 있습니다.
이를 통해 [`SET`](/sql-reference/statements/set) SQL 문을 사용하여 일부 설정을 변경하는 것을 금지합니다.

CONSTRAINT은 다음과 같이 정의합니다.
...
사용자가 CONSTRAINT를 위반하는 설정을 시도하면 예외가 발생하고
설정은 변경되지 않은 상태로 유지됩니다.

## CONSTRAINT의 유형 \{#types-of-constraints\}

ClickHouse에서 지원되는 CONSTRAINT 유형은 다음과 같습니다.
...
`min` 및 `max` CONSTRAINT은 숫자 설정에 대한 상한과 하한을 지정하며,
서로 조합하여 사용할 수 있습니다.

`disallowed` CONSTRAINT은 특정 설정에 대해 허용되지 않아야 하는
특정 값을 지정하는 데 사용할 수 있습니다.

`readonly` 또는 `const` CONSTRAINT은
해당 설정을 전혀 변경할 수 없음을 의미합니다.

`changeable_in_readonly` CONSTRAINT 유형은 `readonly` 설정이 `1`로 설정된 경우에도
