...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "match_case"},{"label": "새 설정."}]}]} />
...
<VersionHistory
  rows={[
  {
    ...
    items: [
      ...
      {
        label:
          "지원되지 않는 CH 타입을 UNKNOWN_TYPE 예외 대신 Parquet (arrow) 바이너리로 변환하는 새 설정입니다."
      }
    ]
  }
]}
...
변환할 수 없는 출력 타입을 원시 바이너리 데이터로 출력합니다. false이면 이러한 타입에서 UNKNOWN&#95;TYPE 예외가 발생합니다.
