...
AvroConfluent 포맷의 경우 Confluent 스키마 레지스트리 HTTP 클라이언트의 연결 타임아웃(초)입니다. 스키마 가져오기와 스키마 등록 모두에 사용됩니다. 0보다 크고 600(10분)보다 작아야 합니다.

## format_avro_schema_registry_max_retries \{#format_avro_schema_registry_max_retries\}

<SettingsInfoBlock type="UInt64" default_value="5" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "5"},{"label": "Confluent 스키마 레지스트리와 통신할 때 발생하는 일시적 장애(전송 타임아웃, 연결 거부, DNS 오류, HTTP 5xx/408/429)에 대한 최대 재시도 횟수를 제어하는 새로운 설정입니다. 재시도를 비활성화하려면 0으로 설정하십시오. 이전 동작(재시도 없음)은 `compatibility = '26.5'`로 유지됩니다."}]}]} />

AvroConfluent 포맷의 경우: Confluent 스키마 레지스트리와 통신할 때 발생하는 일시적 장애(전송 타임아웃, 연결 거부, DNS 오류, HTTP 5xx/408/429)에 대한 최대 재시도 횟수입니다. 재시도를 비활성화하려면 0으로 설정하십시오. 허용되는 최대값은 20입니다. 스키마 검증 오류(HTTP 409, 잘못된 Avro JSON)는 재시도하지 않습니다.

## format_avro_schema_registry_receive_timeout \{#format_avro_schema_registry_receive_timeout\}

<SettingsInfoBlock type="UInt64" default_value="1" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "AvroConfluent 포맷에 사용되는 Confluent 스키마 레지스트리 HTTP 클라이언트의 수신 타임아웃(초)을 제어하는 새로운 설정입니다."}]}]} />

AvroConfluent 포맷의 경우 Confluent 스키마 레지스트리 HTTP 클라이언트의 수신 타임아웃(초)입니다. 스키마 가져오기와 스키마 등록 모두에 사용됩니다. 값은 0보다 크고 600(10분)보다 작아야 합니다.

## format_avro_schema_registry_retry_initial_backoff_ms \{#format_avro_schema_registry_retry_initial_backoff_ms\}

<SettingsInfoBlock type="UInt64" default_value="100" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "100"},{"label": "실패한 Confluent 스키마 레지스트리 요청을 재시도하기 전에 적용할 초기 백오프(밀리초)를 제어하는 새로운 설정입니다. 백오프는 재시도할 때마다 2배로 증가하며 최대 10초로 제한됩니다. `format_avro_schema_registry_max_retries = 0`일 때는 적용되지 않습니다(`compatibility = '26.5'`로 복원되는 26.6 이전 동작)."}]}]} />

AvroConfluent 포맷에서 실패한 Confluent 스키마 레지스트리 요청을 재시도하기 전에 적용하는 초기 백오프 시간(밀리초)입니다. 백오프는 이후 재시도할 때마다 2배로 증가하며 최대 10초로 제한됩니다. 값은 0보다 커야 하며 60000 이하여야 합니다.

## format_avro_schema_registry_send_timeout \{#format_avro_schema_registry_send_timeout\}

<SettingsInfoBlock type="UInt64" default_value="1" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "AvroConfluent 포맷에서 사용하는 Confluent 스키마 레지스트리 HTTP 클라이언트의 전송 제한 시간(초)을 제어하는 새로운 설정입니다."}]}]} />

AvroConfluent 포맷에서 사용하는 Confluent 스키마 레지스트리 HTTP 클라이언트의 전송 제한 시간(초)입니다. 스키마 가져오기와 스키마 등록 모두에 사용됩니다. 값은 0보다 크고 600(10분)보다 작아야 합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "auto"},{"label": "대소문자가 정확히 일치해야 하는 대신, 먼저 입력 컬럼 이름을 대소문자를 구분해 일치시키고, 실패하면 대소문자를 구분하지 않고 일치시킵니다."}]}, {"id": "row-2","items": [{"label": "26.4"},{"label": "match_case"},{"label": "새 설정."}]}]} />

다양한 형식(JSONEachRow, CSVWithNames, JSONColumns, BSONEachRow, RowBinaryWithNames 등을 포함하되 이에 국한되지 않음)을 통해 데이터를 수집할 때 컬럼 이름 매칭 모드를 정의합니다.
지원되는 모드는 다음과 같습니다.
...
:::note
스트리밍 insert의 경우 `min_insert_block_size_rows=0` 및 `min_insert_block_size_bytes=0`도 설정해야 합니다. 그렇지 않으면 파싱된 블록이 해당 임계값에 도달할 때까지 block squashing 단계에서 메모리에 계속 누적되어, 제때 insert가 수행되지 않을 수 있습니다.
:::

**예시: Wikipedia recent changes를 ClickHouse로 스트리밍**
...
<VersionHistory
  rows={[
  {
    ...
    items: [
      ...
      {
        label:
          "지원되지 않는 CH 타입을 UNKNOWN_TYPE 예외 대신 arrow binary로 변환하는 새 설정입니다."
      }
    ]
  }
]}
...
변환할 수 없는 타입은 raw binary 데이터로 출력합니다. false이면 이러한 타입에서 UNKNOWN&#95;TYPE 예외가 발생합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": ""},{"label": "AvroConfluent 출력을 쓸 때 Confluent Schema Registry에서 스키마를 등록할 subject 이름을 지정하는 새로운 설정입니다."}]}]} />

AvroConfluent 출력 형식의 경우, Confluent Schema Registry에서 스키마를 등록할 subject 이름입니다. AvroConfluent 출력을 쓸 때 필요합니다.
...
대화형 모드에서는 이 설정을 명시적으로 지정하지 않으면, 터미널이 UTF-8을 지원하지 않을 때(`LC_ALL`, `LC_CTYPE`, `LANG` 환경 변수로 판단) `clickhouse-client`가 자동으로 `ASCII`로 전환됩니다.
