...
0이 아닌 값으로 설정하고 `join_algorithm`이 `hash`, `parallel_hash`, `default` 또는 `auto`인 경우, 오른쪽 데이터가 이 바이트 수를 초과하면 디스크로 스필할 수 있도록 해시 조인(hash join)이 자동으로 grace hash join으로 전환됩니다. 0(기본값)으로 설정하면 자동 스필이 비활성화됩니다. 이 설정을 사용하면 조인을 통한 read in order 최적화가 방지됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "새 설정: 해시 조인의 스필 임계값으로 사용하는 가용 메모리 비율입니다. 절대값 설정인 `max_bytes_before_external_join`와 함께 사용되며, 둘 중 더 작은 값이 적용됩니다."}]}]} />
...
`max_bytes_before_external_join`와 `max_bytes_ratio_before_external_join`가 모두 설정된 경우, 계산된 임계값 중 더 작은 값이 사용됩니다. 비율이 `0`(기본값)이면 절대값 설정만 적용됩니다.
