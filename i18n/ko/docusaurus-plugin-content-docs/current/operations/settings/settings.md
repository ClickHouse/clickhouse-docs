...
Iceberg 컴팩션 중 오래된 데이터 파일 정리를 허용합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.3"},{"label": "0"},{"label": "catalog_type = 'unity'인 실험적 데이터베이스 엔진 DataLakeCatalog의 사용을 허용합니다"}]}]}/>
...
이 설정은 추출된 튜플 서브컬럼이 `Nullable`이 될 수 있는지 여부를 제어하지 않습니다(예: Dynamic, Variant, JSON 또는 Tuple 컬럼에서 추출된 경우).
추출된 튜플 서브컬럼이 `Nullable`이 될 수 있는지 제어하려면 `allow_nullable_tuple_in_extracted_subcolumns`를 사용하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "새 설정."}]}]} />

Paimon* 테이블 엔진을 사용해 테이블을 생성할 수 있도록 허용합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "polyglot SQL transpiler dialect를 활성화하는 새 설정입니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "실험적인 지연 포스팅 리스트 적용 모드를 제어하는 새로운 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "MergeTree 계열 엔진에서 실험적 `UNIQUE KEY` 절 사용을 제어하는 새 설정"}]}]} />

MergeTree 계열 엔진에서 `UNIQUE KEY` 절이 있는 테이블을 생성할 수 있도록 허용합니다.
...
<VersionHistory
  rows={[
  {
    ...
    items: [
      { label: "26.4" },
      { label: "0" },
      { label: "Iceberg 고아 파일 제거 기능의 사용 여부를 제어하는 새 설정" }
    ]
  }
]}
/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "인덱스 분석 전에 `coalesce(a_1, ..., a_N) <op> const` 형태의 프레디케이트(마찬가지로 `ifNull` 형태이거나 상수가 왼쪽에 있는 경우 포함)를 논리합으로 재작성하는 새로운 설정입니다. 이를 통해 각 `a_i`에 대한 컬럼별 기본 키(primary key)와 스킵 인덱스를 사용할 수 있습니다. `coalesce(a, 42, b)` 및 `coalesce(a, b, 42)`와 같은 부분 상수 형태도 처리됩니다."}]}]} />

인덱스 분석 전에 `coalesce(a_1, ..., a_N) <op> const` 형태의 프레디케이트(마찬가지로 `ifNull` 형태이거나 상수가 왼쪽에 있는 경우 포함)를 `(a_1 <op> const) OR (a_1 IS NULL AND a_2 <op> const) OR ... OR (a_1 IS NULL AND ... AND a_{N-1} IS NULL AND a_N <op> const)`라는 논리합으로 재작성합니다. 이렇게 하면 각 `a_i`에 대한 컬럼별 기본 키(primary key)와 스킵 인덱스를 사용할 수 있습니다. `coalesce(a, 42, b)` 및 `coalesce(a, b, 42)`와 같은 부분 상수 형태도 처리됩니다. 인수 목록은 `coalesce` 자체와 동일하게 정규화되며(`NULL` 리터럴은 제거되고, 첫 번째 비-`Nullable` 인수 뒤의 인수는 제거됨), 끝에 비-`NULL` 상수가 있으면 이를 마지막 분기로 생성합니다. 이 재작성은 인덱스 가지치기에만 추가로 적용되며, 런타임 필터링에는 계속 원래 프레디케이트가 사용됩니다.

## allow_limit_by_partitions_independently \{#allow_limit_by_partitions_independently\}

<SettingsInfoBlock type="Bool" default_value="1" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1"},{"label": "파티션 표현식이 `LIMIT BY` 컬럼의 결정적 함수일 때, 파티션별 `LIMIT BY` 평가를 독립적으로 수행하도록 활성화하는 새 설정입니다"}]}]} />

파티션 표현식이 `LIMIT BY` 컬럼의 결정적 함수일 때, 별도의 스레드에서 파티션별 `LIMIT BY` 평가를 독립적으로 수행하도록 활성화합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "추출된 Tuple 서브컬럼이 널 허용(Nullable) 가능인지 여부를 제어하는 새로운 설정입니다."}]}]} />

`Tuple(...)` 타입의 추출된 서브컬럼을 `Nullable(Tuple(...))` 타입으로 허용할지 여부를 제어합니다.

* `false`: 서브컬럼이 없는 행에 대해서는 기본 Tuple 값을 사용하여 `Tuple(...)`을 반환합니다.
* `true`: 서브컬럼이 없는 행에 대해서는 `NULL`을 사용하여 `Nullable(Tuple(...))`을 반환합니다.

이 설정은 추출된 서브컬럼의 동작만 제어합니다.
테이블에서 `Nullable(Tuple(...))` 컬럼을 생성할 수 있는지 여부는 제어하지 않으며, 이는 `allow_experimental_nullable_tuple_type` 설정으로 제어됩니다.

ClickHouse는 서버 시작 시 로드된 이 설정 값을 사용합니다.
`SET` 또는 쿼리 수준의 `SETTINGS`로 변경하더라도 추출된 서브컬럼 동작은 변경되지 않습니다.
추출된 서브컬럼 동작을 변경하려면 시작 프로필 설정(예: users.xml)에서 `allow_nullable_tuple_in_extracted_subcolumns`를 업데이트한 다음 서버를 다시 시작해야 합니다.
...
[`ALTER`](../../sql-reference/statements/alter/index.md), [`OPTIMIZE`](../../sql-reference/statements/optimize.md) 또는 [`TRUNCATE`](../../sql-reference/statements/truncate.md) 쿼리에 의해 레플리카에서 실행될 작업에 대해 대기 동작을 지정할 수 있습니다.
...
:::note
`alter_sync`는 `Replicated` 및 `SharedMergeTree` 테이블에만 적용되며, `Replicated` 또는 `Shared` 테이블이 아닌 테이블을 변경할 때는 아무 동작도 하지 않습니다.
:::
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.10"},{"label": "1"},{"label": "#87303 이후 발생한 회귀로 인해 row policy와 PREWHERE가 FINAL 이전에 적용되던 문제를 수정하기 위해 이 설정이 추가되었습니다. PR #91065에서 이 설정이 도입되었고, PR #97279에서 정확성을 안전하게 보장하는 true가 기본값이 되었습니다. 호환성이 수정 이전의 false 동작으로 되돌아가지 않도록 {true, true}로 기록됩니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.2"},{"label": "200"},{"label": "비동기 삽입 타임아웃의 최소값(밀리초)입니다. async_insert_busy_timeout_ms는 async_insert_busy_timeout_max_ms의 별칭입니다"}]}]}/>
...
수집된 통계에 기반하여 병렬 레플리카를 사용한 실행으로 자동 전환하도록 활성화합니다. 이를 위해서는 `enable_analyzer = 1`, `enable_parallel_replicas != 0`, `parallel_replicas_local_plan = 1`을 설정하고 `cluster_for_parallel_replicas`를 지정해야 합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "enable"},{"label": "기본적으로 모든 동기 및 비동기 INSERT에 대해 중복 제거를 활성화합니다."}]}]} />
...
ClickHouse Cloud에서만 적용됩니다. 분산 캐시 읽기 요청이 실패했을 때 재시도할 최대 횟수입니다.
...
ClickHouse Cloud에서만 적용됩니다. 분산 캐시 쓰기 요청이 실패했을 때 재시도할 최대 횟수입니다.
...
[Distributed](/engines/table-engines/special/distributed) 테이블에 데이터를 동기식으로 삽입할지 여부를 활성화하거나 비활성화합니다.

기본적으로 `Distributed` 테이블에 데이터를 삽입하면 ClickHouse 서버는 데이터를 클러스터 노드로 백그라운드 모드에서 전송합니다. `distributed_foreground_insert=1`이면 데이터가 동기식으로 처리되며, 모든 샤드에 데이터가 저장된 후에만 `INSERT` 작업이 성공합니다(`internal_replication`이 true이면 각 샤드에 대해 최소 하나의 레플리카).
...
활성화하면 분산 인덱스 분석은 코디네이터에서만 실행됩니다.
이렇게 하면 조건자에 서브쿼리(예: `IN (SELECT ...)`)가 포함된 경우 O(N^2)로 쿼리가 생성되는 현상을 방지할 수 있습니다.
그렇지 않으면 각 팔로워 레플리카가 자체적으로 분산 인덱스 분석을 각각 트리거하기 때문입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "레플리카에서 실행할 분산 쿼리 계획을 직렬화하는 새 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "기본 Dynamic 구현에서 타입 불일치 처리 동작을 제어하는 새 설정"}]}]} />

기본 구현을 사용해 [Dynamic](../../sql-reference/data-types/dynamic.md) 컬럼에 함수를 적용할 때,
실제 타입이 해당 함수와 호환되지 않는 행을 어떻게 처리할지 제어합니다:

* `true` (기본값) — 예외를 발생시킵니다.
* `false` — 해당 행에 대해서는 대신 `NULL`을 반환합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "blob storage 읽기 작업을 system.blob_storage_log에 기록하는 새 설정"}]}]} />

blob storage 읽기 작업 정보를 system.blob&#95;storage&#95;log 테이블에 기록합니다.
`enable_blob_storage_log`도 함께 활성화해야 합니다.
...
구체화된 공통 테이블 표현식을 활성화합니다. 이 설정은 `enable_global_with_statement`보다 우선 사용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "INSERT 중 PROJECTION 파트를 동기적으로 최종 처리하여 최대 메모리 사용량을 줄이는 새 설정입니다."}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "30"},{"label": "HTTP 요청 헤더를 읽는 전체 시간을 제한하여 slowloris 공격을 방지하는 새로운 설정입니다."}]}]} />

모든 HTTP 요청 헤더를 읽는 최대 시간을 초 단위로 지정합니다. 이는 각 읽기 작업별 타임아웃이 아니라 전체 헤더 구문 분석 단계에 적용되는 총 마감 시간입니다. 클라이언트가 헤더 데이터를 아주 천천히 조금씩 보내 연결을 계속 열린 상태로 유지하는 slowloris 유형의 공격을 방지합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "10485760"},{"label": "인증 전에 전체 HTTP 요청 헤더 크기를 제한하는 새 설정입니다."}]}]} />

모든 HTTP 요청 헤더의 최대 총 크기(이름과 값을 합산)이며, 단위는 바이트입니다.
...
두 번의 백그라운드 compaction 작업 사이의 최소 지연 시간입니다.
...
Iceberg에서 데이터 파일 compaction에 적용되는 임계값입니다.
...
Iceberg에서 compaction 대상 데이터 파일에 대한 임계값입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "9223372036854775807"},{"label": "새 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "432000000"},{"label": "새 설정."}]}]}/>

해당 속성이 없으면 `expire_snapshots`에서 사용하는 Iceberg 테이블 속성 `history.expire.max-snapshot-age-ms`의 기본값입니다.

## iceberg_expire_default_min_snapshots_to_keep \{#iceberg_expire_default_min_snapshots_to_keep\}

<SettingsInfoBlock type="Int64" default_value="1" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "새 설정."}]}]}/>

해당 속성이 없으면 `expire_snapshots`에서 사용하는 Iceberg 테이블 속성 `history.expire.min-snapshots-to-keep`의 기본값입니다.
...
Iceberg에서 compaction할 데이터 파일 수의 임계값입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "READ 작업에서 원격 catalog로부터 가져오기를 방지하기 위해 캐시된 메타데이터 버전을 사용할 수 있도록 하는 새 설정"}]}]}/>
...
<VersionHistory
  rows={[
  {
    ...
    items: [
      { label: "26.4" },
      { label: "259200" },
      { label: "기본 orphan 파일 수명 임곗값에 대한 새 설정" }
    ]
  }
]}
/>

Iceberg 테이블에서 orphan 파일을 제거할 때 사용하는 기본 수명 임곗값(초)입니다. 이 값보다 최신인 파일은 orphan 파일로 간주되지 않습니다. `remove_orphan_files()` 프로시저 호출에서 `older_than` 인수가 생략된 경우 사용됩니다. 기본값은 259200(3일)입니다.
...
JOIN이 다음 제한 중 하나에 도달했을 때 ClickHouse가 수행할 작업을 정의합니다:

* [max&#95;bytes&#95;in&#95;join](/operations/settings/settings#max_bytes_in_join)
* [max&#95;rows&#95;in&#95;join](/operations/settings/settings#max_rows_in_join)

이 설정은 [`join_algorithm`](/operations/settings/settings#join_algorithm)이 `hash` 또는 `parallel_hash`인 경우에만 적용됩니다. 다른
알고리즘(예: `partial_merge`, `grace_hash`, `auto`)은 이러한
제한을 다르게 처리합니다 — 디스크로 스필하거나, 다시 파티셔닝하거나, 전략을
전환합니다 — 자세한 내용은
[`join_algorithm`](/operations/settings/settings#join_algorithm)을 참조하십시오.

가능한 값:

* `THROW` — ClickHouse가 예외를 발생시키고 쿼리를 중단합니다.
* `BREAK` — ClickHouse가 쿼리를 중단하고 예외를 발생시키지 않습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "기본적으로 INSERT 시 통계를 생성하지 않고, 대신 병합에 의존합니다"}]}, {"id": "row-2","items": [{"label": "24.6"},{"label": "1"},{"label": "INSERT 시 통계의 머티리얼라이즈를 비활성화할 수 있도록 하는 새 설정이 추가되었습니다"}]}]} />

활성화된 경우 INSERT 시 통계가 생성되어 함께 삽입됩니다. 비활성화하면 통계는 병합 작업 동안 또는 명시적인 MATERIALIZE STATISTICS 실행 시 생성 및 저장됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "해시 조인의 디스크 스필을 자동으로 제어하는 새 설정입니다. 0이 아닌 값으로 설정하면 스필이 활성화되고 바이트 임계값이 설정됩니다."}]}]} />

0이 아닌 값으로 설정하고 `join_algorithm`이 `hash`, `parallel_hash`, `default` 또는 `auto`인 경우, 오른쪽 데이터가 이 바이트 수를 초과하면 디스크로 스필할 수 있도록 해시 조인(hash join)이 자동으로 grace hash join으로 전환됩니다. 0(기본값)으로 설정하면 이 절대 바이트 임계값은 비활성화되지만, `max_bytes_ratio_before_external_join`(기본값은 `0.5`)을 통해 자동 스필이 계속 발생할 수 있습니다. 자동 스필을 완전히 비활성화하려면 둘 다 `0`으로 설정하십시오. 이 설정을 사용하면 조인을 통한 read in order 최적화가 방지됩니다.
...
테이블)의 최대 크기(바이트 단위)입니다.

이 설정은 [SELECT ... JOIN](/sql-reference/statements/select/join)
연산과 [Join table engine](/engines/table-engines/special/join)에 적용됩니다.

쿼리에 여러 조인이 포함되어 있으면 ClickHouse는 모든 중간 결과에 대해 이 설정을 확인합니다. 제한에 도달하면 동작은 선택한
[`join_algorithm`](/operations/settings/settings#join_algorithm)에 따라 달라집니다. 알고리즘별 동작(스필, 재파티션, 전환 또는
[`join_overflow_mode`](/operations/settings/settings#join_overflow_mode)에 따른 throw/break)은 해당 설정을 참조하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0.5"},{"label": "새 설정: 해시 조인의 스필 임계값으로 사용하는 가용 메모리 비율입니다. 기본값은 `0.5`이며, `max_bytes_ratio_before_external_group_by` 및 `max_bytes_ratio_before_external_sort`와 동일합니다. 절대값 설정인 `max_bytes_before_external_join`와 함께 사용되며, 둘 중 더 작은 값이 적용됩니다."}]}]} />

`JOIN`에 사용할 수 있는 가용 메모리 비율입니다. 이 비율에 도달하면 오른쪽 데이터를 디스크에 스필하기 위해 해시 조인이 grace hash join으로 전환됩니다.

예를 들어 `0.6`으로 설정하면 실행 시작 시 `JOIN`은 오른쪽 해시 테이블에 가용 메모리의 `60%`(서버/사용자/병합 기준)를 사용할 수 있습니다. 그 이후에는 디스크로 스필을 시작합니다.

`max_bytes_before_external_join`와 `max_bytes_ratio_before_external_join`가 모두 설정된 경우, 계산된 임계값 중 더 작은 값이 사용됩니다. 비율이 `0`이면 절대값 설정만 적용됩니다.
...
예시: `max_concurrent_queries_for_all_users`는 모든 사용자에 대해 99로 설정할 수 있으며, 데이터베이스 관리자는 서버가 과부하 상태일 때에도 조사용 쿼리를 실행할 수 있도록 자신에 대해서는 100으로 설정할 수 있습니다.
...
증분 읽기 시 소비할 Paimon 스냅샷의 최대 개수입니다. 0은 제한이 없음을 의미합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "4294967296"},{"label": "사용 가능한 여유 메모리를 기준으로 삽입 스레드 수를 제한하는 새 설정"}]}]} />

`max_threads_min_free_memory_per_thread`와 동일하지만, `max_threads`가 아니라 `max_insert_threads`에 적용됩니다. 기본값이 더 큰 이유는 삽입 파이프라인이 일반적으로 읽기 파이프라인보다 스레드별 버퍼(MergeTree 파트, 압축 블록)를 더 크게 유지하기 때문입니다.

여유 메모리 양이 `max_insert_threads`에 이 값을 곱한 값보다 작으면, `max_insert_threads`는 이에 맞게 최소 `1`까지 줄어듭니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "난수 분포 함수의 형상 매개변수를 제한하여 극단적인 입력으로 인한 멈춤을 방지하는 새로운 설정입니다."}]}]} />

`randChiSquared`, `randStudentT`, `randFisherF`와 같은 난수 분포 함수에서 분포 형상 매개변수의 최댓값입니다. 이 설정은 극단적인 매개변수 값으로 인해 계산 시간이 과도하게 길어지는 것을 방지합니다.

## max_rand_distribution_trials \{#max_rand_distribution_trials\}

<SettingsInfoBlock type="UInt64" default_value="1000000000" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000000"},{"label": "난수 분포 함수의 시도 횟수를 제한하는 새 설정으로, 극단적인 입력으로 인한 멈춤 현상을 방지합니다."}]}]} />

`randBinomial` 및 `randNegativeBinomial`과 같은 난수 분포 함수에 허용되는 최대 시도 횟수입니다. 시도 횟수가 매우 클 때 계산 시간이 지나치게 길어지는 것을 방지합니다.
...
쿼리에 여러 개의 조인이 포함된 경우 ClickHouse는 각
중간 결과마다 이 설정을 확인합니다. 제한에 도달하면 수행할 동작은 선택한
[`join_algorithm`](/operations/settings/settings#join_algorithm)에 따라 달라집니다. 알고리즘별 동작(spill, re-partition, switch 또는
[`join_overflow_mode`](/operations/settings/settings#join_overflow_mode)에 따른 throw/break)은 해당
설정을 참조하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "skip_unavailable_shards가 활성화되었을 때 조용히 스킵할 수 있는 샤드 수를 제한하는 새 설정입니다."}]}]}/>

`skip_unavailable_shards`가 활성화되면 조용히 스킵할 수 있는 샤드의 최대 개수를 제한합니다.
사용할 수 없는 샤드 수가 이 값을 초과하면 조용히 스킵하는 대신 예외가 발생합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "skip_unavailable_shards가 활성화되었을 때 조용히 스킵할 수 있는 샤드 비율을 제한하는 새 설정입니다."}]}]}/>

`skip_unavailable_shards`가 활성화되면 조용히 스킵할 수 있는 샤드의 최대 비율(0~1)을 제한합니다.
사용할 수 없는 샤드의 비율이 전체 샤드 대비 이 값을 초과하면, 조용히 스킵하는 대신 예외가 발생합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "최대 메모리 사용량을 줄이기 위해 `UNION` 단계에서 동시에 활성화될 수 있는 데이터 스트림 수를 제한하는 새 설정입니다."}]}]} />

`UNION` 단계에서 동시에 활성화될 수 있는 데이터 스트림 수를 제한합니다(`UNION DISTINCT`는 `UNION ALL` 단계 뒤에 `DISTINCT` 단계가 이어지는 방식으로 구현되므로 `UNION ALL`과 `UNION DISTINCT` 모두에 적용됩니다). `UNION` 쿼리에 서브쿼리가 많으면 모든 서브쿼리가 동시에 읽기 버퍼를 열게 되어, 메모리 사용량이 서브쿼리 수에 비례해 증가합니다. 이 설정은 `Concat` 프로세서를 삽입해 파이프라인을 좁힘으로써 한 번에 활성 상태가 되는 스트림 수를 이 값 이하로 제한하며, 그 결과 최대 메모리 사용량을 크게 줄입니다. 실제 제한값은 이 값과 `max_threads * max_streams_for_union_step_to_max_threads_ratio` 중 더 작은 값입니다(둘 중 하나가 0이면 해당 값은 무시됩니다). 둘 다 0이면 파이프라인 축소가 적용되지 않습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "8"},{"label": "새 설정: UNION 단계에서 동시에 활성 상태가 될 수 있는 스트림 수 제한은 min(max_streams_for_union_step, max_threads * max_streams_for_union_step_to_max_threads_ratio)로 계산되며, 둘 중 하나가 0이면 해당 입력은 비활성화됩니다."}]}]} />

이 비율에 `max_threads`를 곱한 값이 `UNION` 단계에서 동시에 활성 상태가 될 수 있는 스트림 수의 제한을 결정합니다(`UNION ALL`과 `UNION DISTINCT` 모두에 적용됨). 실제 제한은 이 계산값과 `max_streams_for_union_step` 중 더 작은 값입니다(둘 중 하나가 0이면 무시됨). 예를 들어 `max_threads = 8`이고 이 비율을 1로 설정하면 최대 8개의 스트림이 활성화됩니다. 이 비율 기반 제한을 비활성화하려면 0으로 설정하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1073741824"},{"label": "사용 가능한 여유 메모리를 기준으로 스레드 수를 제한하는 새 설정"}]}]} />

서버가 메모리 압박 상태일 때 `max_threads`를 낮춰, 메모리 한도에 도달할 가능성이 높은 고병렬 쿼리가 시작되지 않도록 합니다.

여유 메모리는 서버의 `max_server_memory_usage`에서 전역 메모리 추적기(global memory tracker)가 현재 추적 중인 메모리를 뺀 값으로 계산됩니다. 이 여유 메모리가 `max_threads`에 이 값을 곱한 값보다 작으면, `N * value <= free_memory`를 만족하는 가장 큰 N이 되도록 `max_threads`를 줄이며, 최소값은 `1`입니다.

이 제한을 비활성화하려면 `0`으로 설정하십시오.

예를 들어 기본값인 1 GiB에서 여유 메모리가 32 GiB이면 `max_threads`의 상한은 32가 됩니다. 여유 메모리가 1 GiB이면 1로 낮아집니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "WKB geometry 파싱 시 요소 수를 제한하여, 손상된 데이터로 인한 과도한 메모리 할당을 방지하는 새 설정입니다."}]}]} />

`readWKB` 및 관련 함수가 파싱할 때 단일 WKB geometry 요소에 허용되는 최대 포인트, 링 또는 폴리곤 수입니다. 이 설정은 손상된 WKB 데이터로 인한 과도한 메모리 할당을 방지합니다. 하드코딩된 제한값(1억)을 사용하려면 0으로 설정하십시오.
...
불필요한 딕셔너리 속성을 조회하지 않도록 `tupleElement(dictGet('dict', ('a', 'b', 'c'), key), 2)`를 `dictGet('dict', 'b', key)`로 재작성합니다. 포지션 기반(`.1`, `.2`, ...) 및 이름 기반(`.b`) 접근을 지원하며, 기본 인수가 상수 튜플 또는 상수로 이루어진 `tuple(...)`인 경우에는 `dictGetOrDefault`에도 적용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "이제 재작성 전에 타입 호환성을 확인하므로, 기본적으로 arrayExists를 has로 재작성하는 최적화가 활성화됩니다."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "이 최적화는 프로덕션 환경에서 사용할 준비가 되어 있습니다"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "16777216"},{"label": "캐시 미스 발생 시 사용자 공간 페이지 캐시를 채우는 데 사용되는 단일 병합 읽기의 크기를 제한하는 새 설정입니다."}]}]} />

`readBigAt`가 사용자 공간 페이지 캐시를 채울 때 연속된 캐시 미스는 하위 스토리지에서 한 번의 읽기로 병합됩니다. 이 설정은 병합 읽기 1회의 크기를 바이트 단위로 제한하며, 더 긴 미스 구간은 여러 번의 읽기로 나뉩니다. 병렬 콜드 읽기 시 임시 버퍼의 일시적인 메모리 사용량을 제한합니다.

값이 클수록 객체 스토리지에서 콜드 스캔 시 HTTP 요청 수가 줄어들고, 값이 작을수록 일시적인 최대 메모리 사용량이 줄어듭니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "-1"},{"label": "새 설정."}]}]} />

Paimon 증분 모드에서 쿼리 수준으로 특정 snapshot을 대상으로 읽습니다. &gt;0이면 리더는 커밋된 워터마크를 앞으로 진행하지 않고 지정된 snapshot&#95;id에 대한 delta만 가져옵니다.
기본값: -1(비활성화됨)
...
v25.4부터 `ReplicatedMergeTree` 또는 `SharedMergeTree` 소스의 `INSERT ... SELECT`도 레플리카 전체에 걸쳐 병렬화할 수 있습니다. 이를 활성화하려면 다음과 같이 설정하십시오.
...
병렬 레플리카가 `MergeTree` 테이블에 대한 단순 뷰에서 뷰의 내부 쿼리 대신 외부 쿼리를 실행하도록 허용하여, 노드 간 병렬 처리를 개선합니다. 이 설정은 각 분기가 서로 다른 `MergeTree` 테이블을 읽는 `UNION ALL` 뷰에도 적용됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "새 설정. 비활성화하면 병렬 읽기용 레플리카는 로컬 레플리카를 집합에 강제로 포함하지 않고, 로드 밸런싱 알고리즘에 따라서만 선택됩니다."}]}]} />

활성화하면(기본값) 병렬 읽기에 사용되는 레플리카 집합에 항상 로컬 레플리카가 포함됩니다.
비활성화하면 로컬 레플리카에 별도의 우선순위를 두지 않으며, 레플리카는 로드 밸런싱 알고리즘에 따라서만 선택됩니다.
이렇게 하면 `max_parallel_replicas = 1`인 쿼리를 다른 호스트로 보낼 수 있으므로, 많은 짧은 쿼리가 클러스터 전체에 분산되는 경우 캐시 지역성을 개선할 수 있습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": ""},{"label": "polyglot transpiler용 소스 SQL dialect를 지정하는 새 설정."}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "모든 서브쿼리에 `use_query_cache`가 전파되도록 활성화하는 새로운 설정입니다. 이 설정이 없으면 각 서브쿼리에서 `SETTINGS use_query_cache = true`를 명시적으로 설정한 경우에만 서브쿼리가 캐시됩니다."}]}]} />

활성화하면 서브쿼리 결과를 [쿼리 캐시](../query-cache.md)에 쓰고 여기서 읽을 수 있습니다. 이렇게 하면 `use_query_cache`가 모든 서브쿼리로 전파됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "1000"},{"label": "지연 JOIN 인덱싱 최적화에 쿼리 플랜을 사용할 수 있는 최대 제한값을 제어하는 새로운 설정이 추가되었습니다. 0이면 제한이 없습니다"}]}]} />

JOIN의 지연 인덱싱 최적화에 쿼리 플랜을 사용할 수 있는 최대 제한값을 제어합니다. 0이면 제한이 없습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "3"},{"label": "JOIN에서 지연 인덱싱 최적화를 활성화하는 데 필요한 왼쪽 측 페이로드 컬럼의 최소 개수를 제어합니다"}]}]} />

JOIN에서 지연 인덱싱 최적화를 활성화하는 데 필요한 왼쪽 측 페이로드 컬럼의 최소 개수를 제어합니다. 0이면 최적화가 비활성화됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "정렬 키가 조인에서 보존되는 쪽만 참조할 때 ORDER BY ... LIMIT n을 LEFT/RIGHT 조인을 통해 아래로 푸시하는 쿼리 플랜 수준 최적화를 활성화하는 새 설정입니다."}]}]} />

정렬 키가 조인에서 보존되는 쪽(LEFT/RIGHT)의 컬럼만 참조하는 경우 `ORDER BY ... LIMIT n`을 조인을 통해 아래로 푸시하는 쿼리 플랜 수준 최적화를 켜거나 끕니다. 이 최적화는 조인 전에 보존되는 쪽 입력이 생성해야 하는 행 수를 제한합니다.
...
[query profiler](../../operations/optimizing-performance/sampling-query-profiler.md)의 실제 클록 타이머 주기를 설정합니다. 실제 클록 타이머는 벽시계 시간을 계산합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "10"},{"label": "재귀 CTE에서 `getLeastSupertype`를 반복 적용해 컬럼 타입을 추론할 때의 최대 반복 횟수"}]}]} />

재귀 CTE에서 컬럼 타입을 추론할 때의 최대 반복 횟수입니다. 컬럼 타입은 수렴할 때까지 UNION ALL의 비재귀 측과 재귀 측에 `getLeastSupertype`를 반복적으로 적용하여 결정됩니다. 타입 확장을 비활성화하고 비재귀 부분의 타입만 사용하려면 0으로 설정하십시오.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.2"},{"label": "0"},{"label": "새 설정"}]}]}/>

RESTORE 중에 테이블 엔진을 Replicated*MergeTree에서 Shared*MergeTree로 교체합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "auto"},{"label": "새 설정."}]}]} />

S3 엔드포인트 스타일을 강제로 지정합니다. 가능한 값: auto, virtual&#95;hosted, path.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "인라인 데이터가 포함된 INSERT 쿼리에 대해 서버가 테이블 구조를 전송할지 제어하는 새로운 설정입니다."}]}]} />

이 설정이 비활성화되어 있고 INSERT 쿼리에 인라인 데이터가 포함되어 있으면, 서버는 네이티브 프로토콜을 통해 테이블 구조와 컬럼 기본값을 클라이언트에 다시 전송하지 않습니다. 대신 서버가 인라인 데이터를 직접 파싱합니다. 이렇게 하면 네이티브 프로토콜을 통한 다수의 소규모 삽입에서 성능이 향상될 수 있습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "`show_data_lake_catalogs_in_system_tables`에서 이름이 변경되었으며, 기본적으로 `system.tables`, `system.columns`, `system.completions`에서 `MySQL` 및 `PostgreSQL` 데이터베이스도 숨기도록 범위가 확대되었습니다. 해당 테이블을 열거하려면 비용이 큰 원격 호출이 필요하기 때문입니다. 이전 동작에 의존하던 사용자는 이 설정을 `true`로 지정해야 합니다. 이전 이름은 별칭으로 유지됩니다."}]}]} />

시스템 테이블에 원격 데이터베이스(데이터 레이크 카탈로그, MySQL, PostgreSQL)를 표시할 수 있도록 합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0.2"},{"label": "지연 포스팅 리스트 밀도 임계값에 대한 새로운 설정"}]}]} />

지연 포스팅 리스트 모드에서 알고리즘을 선택할 때 사용하는 밀도 임계값입니다.
임계값 미만: leapfrog intersection. 임계값 이상: brute-force bitmap.
...
딕셔너리 스캔을 통한 텍스트 인덱스 LIKE 평가가 활성화된 경우 읽을 수 있는 큰 포스팅의 최대 개수입니다.

이 설정을 사용하려면 `use_text_index_like_evaluation_by_dictionary_scan`도 활성화해야 합니다.

## text_index_like_min_pattern_length \{#text_index_like_min_pattern_length\}

<SettingsInfoBlock type="UInt64" default_value="4" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "4"},{"label": "새 설정"}]}]} />

딕셔너리 스캔 방식의 텍스트 인덱스 LIKE 평가를 사용하기 위해 필요한 LIKE/ILIKE 패턴 내 영숫자 needle의 최소 길이입니다.
이 임계값보다 짧은 패턴은 너무 많은 딕셔너리 토큰과 일치하므로, 비용이 많이 드는 스캔을 피하기 위해 건너뜁니다.
...
텍스트 인덱스 쿼리 실행 중 포스팅 리스트를 적용하는 방식을 제어합니다.
&#39;materialize&#39;(기본값)는 포스팅 리스트를 Roaring Bitmaps로 미리 디코딩합니다.
&#39;lazy&#39;는 필요할 때 디코딩하는 커서 기반 방식을 사용합니다(V2 인덱스 포맷 및 allow&#95;experimental&#95;text&#95;index&#95;lazy&#95;apply 필요).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": ""},{"label": "url 테이블 함수와 URL 테이블 엔진에서 상대 URL을 해석할 때 사용할 기준 URL을 지정하는 새 설정입니다."}]}]} />

[url](../../sql-reference/table-functions/url.md) 테이블 함수와 [URL](../../engines/table-engines/special/url.md) 테이블 엔진에서 상대 URL을 해석하는 데 사용하는 기준 URL입니다.

설정하면 상대 URL은 다음과 같이 해석됩니다:

* 경로 상대 URL(예: `data.csv`): RFC 3986에 따라 기준 URL 경로와 병합됩니다. 기준 경로에서 마지막 `/` 뒤의 모든 내용이 상대 URL로 대체되므로, 끝의 슬래시 유무가 중요합니다: `https://example.com/dir/` + `data.csv` = `https://example.com/dir/data.csv`, 하지만 `https://example.com/dir` + `data.csv` = `https://example.com/data.csv`입니다. 기준 URL에 경로가 없으면(예: `https://example.com`) `/`가 삽입됩니다: `https://example.com/data.csv`. 상대 URL의 점 세그먼트(`./` 및 `../`)는 정규화됩니다: `https://example.com/dir/` + `../a.csv` = `https://example.com/a.csv`.
* 호스트 상대 URL(예: `/test/data.csv`): 기준 URL의 스킴과 호스트를 기준으로 해석됩니다.
* 스킴 상대 URL(예: `//other.com/test/data.csv`): 기준 URL의 스킴을 사용해 해석됩니다.
* 쿼리만 있는 참조(예: `?x=1`): 기준 URL 경로에 추가되며(기존 쿼리/프래그먼트는 대체됨).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.12"},{"label": "1"},{"label": "기본적으로 동시성 제어 사용"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "21.9"},{"label": "1"},{"label": "기본적으로 Hedged Requests 기능 사용"}]}]}/>

원격 쿼리에 대해 hedged requests 로직을 활성화합니다. 이를 통해 하나의 쿼리에 대해 서로 다른 레플리카와 여러 연결을 설정할 수 있습니다.
기존 레플리카 연결이 `hedged_connection_timeout` 내에 설정되지 않았거나
`receive_data_timeout` 내에 데이터를 수신하지 못한 경우 새 연결이 설정됩니다. 쿼리는 비어 있지 않은 progress 패킷(또는 `allow_changing_replica_until_first_data_packet`인 경우 data 패킷)을
가장 먼저 보낸 첫 번째 연결을 사용하며, 다른 연결은 취소됩니다. `max_parallel_replicas > 1`인 쿼리도 지원됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "기본적으로 TopK 필터링에 데이터 스키핑 인덱스 사용 활성화"}]}, {"id": "row-2","items": [{"label": "25.12"},{"label": "0"},{"label": "새로운 설정."}]}]} />
...
역텍스트 인덱스 딕셔너리를 스캔하여 LIKE/ILIKE 쿼리를 평가할 수 있도록 합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "TopK 쿼리에 대한 동적 필터링 최적화를 기본적으로 활성화합니다"}]}, {"id": "row-2","items": [{"label": "25.12"},{"label": "0"},{"label": "새로운 설정입니다."}]}]} />

`ORDER BY <column> LIMIT n` 쿼리를 실행할 때 동적 필터링 최적화를 활성화합니다.

이 설정을 활성화하면, 쿼리 실행기는 최종 결과 집합의 `top N` 행에 포함되지 않을 그래뉼과 행을 건너뛰려고 시도합니다. 이 최적화는 동적으로 동작하며, 지연 시간(latency) 개선 정도는 데이터 분포와 쿼리에 포함된 다른 프레디케이트 존재 여부에 따라 달라집니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "기본 Variant 구현에서 타입 불일치 시 동작을 제어하는 새 설정"}]}]} />

기본 구현에서 [Variant](../../sql-reference/data-types/variant.md) 컬럼에 함수를 적용할 때,
