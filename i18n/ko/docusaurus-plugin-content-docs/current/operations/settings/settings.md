...
증분 읽기마다 소비할 Paimon 스냅샷의 최대 개수입니다. 0은 제한이 없음을 의미합니다.
...
Paimon 증분 모드에서 쿼리 수준으로 특정 스냅샷(snapshot)을 읽기 위한 설정입니다. 값이 0보다 크면 리더는 커밋된 워터마크를 앞으로 진행하지 않고 지정된 snapshot&#95;id의 변경분(delta)만 가져옵니다.
기본값: -1 (비활성화)
