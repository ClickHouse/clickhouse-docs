...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "256"},{"label": "새로운 설정: 단일 UNIQUE KEY 행에 대한 정렬 순서 보존 바이너리 인코딩의 최대 크기(바이트)"}]}]} />

단일 `UNIQUE KEY` 행에 대한 정렬 순서 보존 바이너리 인코딩의 최대 크기(바이트)입니다.
