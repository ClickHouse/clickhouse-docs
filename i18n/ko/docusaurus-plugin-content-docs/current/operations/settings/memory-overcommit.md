---
description: '쿼리에 대해 더 유연한 메모리 한도를 설정할 수 있도록 하는 실험적인 기법입니다.'
...
## User 오버커밋 트래커 \{#user-overcommit-tracker\}

User 오버커밋 트래커는 사용자의 쿼리 목록에서 오버커밋 비율이 가장 높은 쿼리를 찾습니다.
...
## Global 오버커밋 트래커 \{#global-overcommit-tracker\}

Global 오버커밋 트래커는 전체 쿼리 목록에서 오버커밋 비율이 가장 큰 쿼리를 찾습니다.
이때 오버커밋 비율은 할당된 바이트 수를 `memory_overcommit_ratio_denominator` 설정 값으로 나눈 값으로 계산됩니다.

해당 쿼리의 `memory_overcommit_ratio_denominator` 값이 0이면, 오버커밋 트래커는 그 쿼리를 선택하지 않습니다.
