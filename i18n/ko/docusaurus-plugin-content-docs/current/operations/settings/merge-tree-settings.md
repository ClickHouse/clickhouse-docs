...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "minmax, uniq"},{"label": "기본적으로 자동 통계를 활성화"}]}, {"id": "row-2","items": [{"label": "25.10"},{"label": ""},{"label": "새 설정"}]}]} />

적합한 모든 컬럼에서 자동으로 계산할 통계 타입의 쉼표로 구분된 목록입니다.
지원되는 통계 타입: tdigest, countmin, minmax, nullcount, uniq.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "16"},{"label": "새 설정입니다. 제거 대상 파트 중 하나라도 원격 디스크에 있으면 동시 파트 제거 경로로 전환하는 임계값을 더 낮게 설정합니다. 이 경우 각 제거 작업에는 일반적으로 네트워크 왕복이 1회 필요합니다. 이전 값(100)은 기존 `concurrent_part_removal_threshold` 기본값과 같으므로, 이전 `compatibility` 모드에서는 기존 동작이 유지됩니다."}]}]} />

`concurrent_part_removal_threshold`와 동일하지만, 제거되는
파트 중 하나 이상이 원격 디스크에 저장되어 있을 때 사용됩니다. 기본값이 더 낮은
이유는 원격 스토리지에서 파트를 제거할 때마다 일반적으로 네트워크
왕복이 필요하기 때문입니다(예: 객체 스토리지에서는 파트당 HTTP `DELETE`
1회). 따라서 파트를 100개만 직렬로 제거해도 `DROP TABLE`이
수십 초 동안 지연될 수 있습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "기본적으로 min_age_to_force_merge_seconds가 설정된 경우에도 파트 크기 제한"}]}, {"id": "row-2","items": [{"label": "25.1"},{"label": "0"},{"label": "min_age_to_force_merge의 최대 바이트 수를 제한하는 새로운 설정 추가"}]}, {"id": "row-3","items": [{"label": "25.1"},{"label": "0"},{"label": "새로운 설정"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "'with_buckets' 맵 직렬화에서 버킷 수를 계산할 때 `sqrt` 및 `linear` 전략에 사용되는 계수를 제어하는 설정을 추가했습니다"}]}]} />

평균 맵 크기에서 버킷 수를 계산할 때 `sqrt` 및 `linear` [map&#95;buckets&#95;strategy](#map_buckets_strategy)에 사용되는 계수입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "32"},{"label": "`with_buckets` 직렬화를 적용하는 데 필요한 최소 평균 맵 크기(행당 키 수)를 제어하는 설정을 추가했습니다"}]}]} />

`with_buckets` 직렬화를 적용하는 데 필요한 최소 평균 맵 크기(행당 키 수)입니다.
평균 맵 크기가 이 값보다 작으면 다른 버킷 설정과 관계없이 단일 버킷이 사용됩니다.
값이 `0`이면 이 임곗값이 비활성화되며, 항상 버킷 기반 직렬화가 적용됩니다.
이 설정은 이점이 거의 없는 작은 맵에 대해 버킷 기반 직렬화의 오버헤드를 피하는 데 유용합니다.

## map_buckets_strategy \{#map_buckets_strategy\}

<SettingsInfoBlock type="MergeTreeMapBucketsStrategy" default_value="sqrt" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "sqrt"},{"label": "'with_buckets' `Map` 직렬화의 버킷 수 선택 전략을 제어하는 설정 추가"}]}]} />
...
* sqrt — `round(map_buckets_coefficient * sqrt(avg_map_size))`를 버킷 수로 사용하며, 값은 `[1, max_buckets_in_map]` 범위로 제한됩니다.
* linear — `round(map_buckets_coefficient * avg_map_size)`를 버킷 수로 사용하며, 값은 `[1, max_buckets_in_map]` 범위로 제한됩니다.
...
`with_buckets` 직렬화에서 버킷 수는 [max&#95;buckets&#95;in&#95;map](#max_buckets_in_map) 및 [map&#95;buckets&#95;strategy](#map_buckets_strategy)에 따라 결정됩니다.

## map_serialization_version_for_zero_level_parts \{#map_serialization_version_for_zero_level_parts\}

<SettingsInfoBlock type="MergeTreeMapSerializationVersion" default_value="basic" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "basic"},{"label": "제로 레벨 파트의 Map 직렬화 버전을 제어하는 설정을 추가했습니다"}]}]} />

이 설정을 사용하면
삽입 시 생성되는 제로 레벨 파트의 `Map`
컬럼에 대해 서로 다른 직렬화 버전을 지정할 수 있습니다.
삽입 시 성능 저하를 방지하기 위해 제로 레벨 파트에는 `basic`
직렬화를 유지하면서, 머지된 파트에는 `with_buckets`를
사용하려는 경우에 유용합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "32"},{"label": "'with_buckets' Map 직렬화에 대한 최대 버킷 수를 제어하는 설정 추가"}]}]} />

`Map` 직렬화의 최대 버킷 수입니다. `with_buckets` `Map` 직렬화와 함께 작동합니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "기본적으로 타입 직렬화 버전을 중첩 타입에 전파합니다"}]}]} />

true이면 string&#95;serialization&#95;version과 같은 직렬화 버전이 Array/맵/널 허용/JSON 등의 중첩 타입 내부에도 전파됩니다. 비활성화하면 직렬화 버전은 이 타입의 최상위 컬럼과 Tuple 요소에만 적용됩니다.
...
다른 레플리카에서 가져올 최소 파트 레벨입니다. 이 임곗값보다 레벨이 낮은 파트는 연기됩니다
(복제 대기열에 유지되며 각 스케줄링 주기마다 다시 평가되고, 영구적으로 건너뛰지는 않습니다).
1을 사용하면 레벨 0(머지되지 않은) 파트 가져오기를 연기하여 대량 수집 중 복제 오버헤드를 줄일 수 있습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "300"},{"label": "새 설정"}]}]}/>

replicated&#95;fetches&#95;min&#95;part&#95;level보다 낮은 파트도 어쨌든 가져오게 되는 제한 시간이며, 단위는 초입니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "false로 설정하면 공통 접두사를 공유하는 점 표기 이름의 Array 컬럼은 기존 Nested 의미 체계에서처럼 오프셋 파일을 공유하지 않고, 서로 독립적인 컬럼으로 처리됩니다"}]}]} />

활성화되면(기본값) 공통 접두사를 공유하는 점 표기 이름의 Array 컬럼(예: n.a 및 n.b)은
Nested 구조의 일부로 처리됩니다. 즉, 디스크에서 하나의 오프셋 파일(예: n.size0)을 공유하며,
INSERT 시 배열 크기가 서로 같은지 검증합니다.
비활성화되면 각 Array 컬럼은 각각 독립적인 오프셋 파일을 사용하고, 점으로 구분된 이름은 더 이상 특별한
의미를 갖지 않으며, 같은 접두사를 공유하는 점 표기 Array 컬럼과 스칼라 컬럼이 함께 존재할 수 있습니다
(예: n.a Array(String)와 함께 n UInt32). 이 설정은 테이블 생성 후에는 변경할 수 없습니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "auto"},{"label": "설정을 auto로 지정: 레플리카당 최대 머지 작업 수 * 활성 레플리카 수"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "100"},{"label": "New setting"}]}]} />

코디네이터가 준비하여 워커에 분산해야 하는 머지 작업 항목의 개수입니다.
&#39;auto&#39;로 설정하면, 단일 레플리카에서 허용되는 최대 머지 작업 수에 활성 레플리카 수를 곱한 값이 됩니다.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1800"},{"label": "기본 replica set 백그라운드 업데이트 간격을 30분으로 늘림"}]}, {"id": "row-2","items": [{"label": "26.2"},{"label": "300"},{"label": "새 설정"}]}]} />

레플리카가 백그라운드에서 replica set을 업데이트하려고 시도하는 간격입니다. 다음 실행 시점에는 [0, value]초 범위에서 균등하게 지터가 적용됩니다. 예외: value = 0은 이 규칙을 따르지 않으며,
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "TTL에 의해 만료된 행을 제거해야 하는 머지에 대해 수직 병합 알고리즘 허용"}]}]}/>

`true`인 경우, 수직 병합에서 TTL로 만료된 행 삭제를 최적화합니다. 수평 병합을 강제로 수행하는 대신,
TTL 필터를 평가하여 병합 알고리즘에 전달하고, 이 알고리즘은 행 소스에 skip 플래그를 설정합니다.
