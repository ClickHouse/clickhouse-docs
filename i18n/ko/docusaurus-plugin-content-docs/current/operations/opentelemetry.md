...
ClickHouse는 [ClickHouse Keeper](../guides/sre/keeper/overview) 요청(ZooKeeper와 호환되는 코디네이션 서비스)에 대해 OpenTelemetry 트레이싱을 지원합니다. 이 기능은 클라이언트 요청 전송부터 서버 측 처리에 이르기까지 Keeper 작업의 전체 수명 주기에 대한 상세한 가시성을 제공합니다.
