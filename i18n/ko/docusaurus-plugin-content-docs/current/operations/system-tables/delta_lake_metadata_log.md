---
description: 'Delta Lake 테이블에서 읽어 온 메타데이터 파일 정보를 담은 시스템 테이블입니다. 각 항목은 루트 메타데이터 JSON 파일 하나를 나타냅니다.'
...
[`delta_lake_log_metadata`](../../operations/settings/settings.md#delta_lake_log_metadata) 설정을 사용하면 어떤 메타데이터 이벤트를 로그에 기록할지 제어할 수 있습니다.
...
* `event_time` ([DateTime](/sql-reference/data-types/datetime)) — 이벤트 발생 시간입니다.
...
* `content` ([String](/sql-reference/data-types/string)) — JSON 형식의 내용입니다.
