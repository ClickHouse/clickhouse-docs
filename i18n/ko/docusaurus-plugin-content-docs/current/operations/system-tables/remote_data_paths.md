...
* `disk_name` ([String](../../sql-reference/data-types/)) — 디스크 이름.
* `path` ([String](../../sql-reference/data-types/)) — 디스크 경로.
* `cache_base_path` ([String](../../sql-reference/data-types/)) — 캐시 파일의 기본 디렉터리.
* `local_path` ([String](../../sql-reference/data-types/)) — ClickHouse 파일 경로이며, 메타데이터 경로로도 사용됩니다.
* `remote_path` ([String](../../sql-reference/data-types/)) — 객체 스토리지의 Blob 경로로, ClickHouse 파일이 연결된 경로입니다.
* `size` ([UInt64](../../sql-reference/data-types/)) — 파일 크기(압축된 기준).
* `common_prefix_for_blobs` ([String](../../sql-reference/data-types/)) — 객체 스토리지 내 Blob의 공통 접두사.
* `cache_paths` ([Array(String)](../../sql-reference/data-types/)) — 해당 Blob에 대응하는 캐시 파일.
