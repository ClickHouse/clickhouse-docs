...
파트 체크섬에 보고된 비압축 바이트의 총량으로, MergeTree 엔진 계열의 모든 테이블에 저장된 값을 나타냅니다. 이는 `system.tables`의 `total_bytes_uncompressed` 컬럼과 동일한 소스이며, 마크와 기본 키 인덱스처럼 비압축 상태로 저장되는 파일은 포함하지 않습니다.

### TotalUncompressedBytesOfMergeTreeTablesSystem \{#totaluncompressedbytesofmergetreetablessystem\}

system 데이터베이스의 MergeTree 엔진 계열 테이블에 저장된 비압축 바이트의 총량으로, 파트 체크섬 기준으로 집계됩니다. 이는 `system.tables`의 `total_bytes_uncompressed` 컬럼과 동일한 소스이며, 마크와 기본 키 인덱스처럼 비압축 상태로 저장되는 파일은 포함하지 않습니다.
