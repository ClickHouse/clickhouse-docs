---
description: '일반 함수와 집계 함수에 대한 정보를 포함하는 시스템 테이블입니다.'
keywords: ['system table', 'functions']
slug: /operations/system-tables/functions
title: 'system.functions'
doc_type: 'reference'
---

## 설명 \{#description\}

일반 함수와 집계 함수에 대한 정보를 포함합니다.

## 컬럼 \{#columns\}

{/*AUTOGENERATED_START*/ }

* `name` ([String](../../sql-reference/data-types/)) — 함수 이름입니다.
* `is_aggregate` ([UInt8](../../sql-reference/data-types/)) — 함수가 집계 함수인지 여부입니다.
* `case_insensitive` ([UInt8](../../sql-reference/data-types/)) — 함수 이름을 대소문자를 구분하지 않고 사용할 수 있는지 여부입니다.
* `alias_to` ([String](../../sql-reference/data-types/)) — 함수 이름이 별칭인 경우의 원래 함수 이름입니다.
* `create_query` ([String](../../sql-reference/data-types/)) — 더 이상 사용되지 않습니다.
* `origin` ([Enum8(&#39;System&#39; = 0, &#39;SQLUserDefined&#39; = 1, &#39;ExecutableUserDefined&#39; = 2, &#39;WasmUserDefined&#39; = 3)](../../sql-reference/data-types/)) — 더 이상 사용되지 않습니다.
* `description` ([String](../../sql-reference/data-types/)) — 함수가 수행하는 작업에 대한 개략적인 설명입니다.
* `syntax` ([String](../../sql-reference/data-types/)) — 함수 시그니처입니다.
* `arguments` ([String](../../sql-reference/data-types/)) — 함수 인수(arguments)입니다.
* `parameters` ([String](../../sql-reference/data-types/)) — 함수 매개변수(parameters)입니다(집계 함수에서만 사용됩니다).
* `returned_value` ([String](../../sql-reference/data-types/)) — 함수가 반환하는 값입니다.
* `examples` ([String](../../sql-reference/data-types/)) — 사용 예입니다.
* `introduced_in` ([String](../../sql-reference/data-types/)) — 함수가 처음 도입된 ClickHouse 버전입니다.
* `categories` ([String](../../sql-reference/data-types/)) — 함수의 범주입니다.
* `deterministic` ([Nullable(UInt8)](../../sql-reference/data-types/)) — 동일한 인수에 대해 함수가 같은 결과를 반환하는지 여부입니다. 알 수 없는 경우에는 NULL입니다(예: 집계 함수 또는 사용자 정의 함수).
* `higher_order` ([Nullable(UInt8)](../../sql-reference/data-types/)) — 함수가 고차 함수인지 여부입니다. 즉, 최소 하나의 람다 표현식을 인수로 받습니다(예: arrayMap, arrayFilter, mapApply). 알 수 없는 경우에는 NULL입니다.

{/*AUTOGENERATED_END*/ }

## 예시 \{#example\}

```sql title="Query"
 SELECT name, is_aggregate, deterministic, case_insensitive, alias_to FROM system.functions LIMIT 5;
```

```text title="Response"
┌─name─────────────────────┬─is_aggregate─┬─deterministic─┬─case_insensitive─┬─alias_to─┐
│ BLAKE3                   │            0 │                1 │                0 │          │
│ sipHash128Reference      │            0 │                1 │                0 │          │
│ mapExtractKeyLike        │            0 │                1 │                0 │          │
│ sipHash128ReferenceKeyed │            0 │                1 │                0 │          │
│ mapPartialSort           │            0 │                1 │                0 │          │
└──────────────────────────┴──────────────┴──────────────────┴──────────────────┴──────────┘

5 rows in set. Elapsed: 0.002 sec.
```