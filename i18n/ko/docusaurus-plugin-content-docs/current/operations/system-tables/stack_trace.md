...
스택 프레임을 분석하려면 `addressToLine`, `addressToLineWithInlines`, `addressToSymbol`, `demangle` [인트로스펙션 함수](../../sql-reference/functions/introspection.md)를 사용하십시오.

## 컬럼 \{#columns\}

{/*AUTOGENERATED_START*/ }

* `thread_name` ([String](../../sql-reference/data-types/)) — 스레드 이름입니다.
* `thread_id` ([UInt64](../../sql-reference/data-types/)) — 스레드 식별자입니다.
* `query_id` ([String](../../sql-reference/data-types/)) — 이 스레드가 속한 쿼리의 ID입니다.
* `trace` ([Array(UInt64)](../../sql-reference/data-types/)) — 이 스레드의 스택 트레이스입니다. 기본적으로 주소 배열에 불과합니다.
* `untracked_memory` ([Int64](../../sql-reference/data-types/)) — 아직 상위 MemoryTracker에 전파되지 않은 메모리 할당을 위한 스레드별 atomic-less 카운터입니다. 마지막 플러시 이후 할당된 양보다 해제된 양이 더 많으면 음수일 수 있습니다.
