...
서로 다른 크기 클래스(빈, bin)에서 jemalloc 할당자를 통해 수행된 메모리 할당 정보를 모든 arena에서 집계한 내용입니다.
...
* `index` ([UInt16](../../sql-reference/data-types/)) — 크기 기준으로 정렬된 빈의 인덱스입니다.
* `large` ([UInt8](../../sql-reference/data-types/)) — 큰 할당이면 True, 작은 할당이면 False입니다.
* `size` ([UInt64](../../sql-reference/data-types/)) — 이 빈에서의 할당 크기입니다.
...
* `curregs` ([Int64](../../sql-reference/data-types/)) — 이 크기 클래스에 대한 현재 region 개수입니다.
