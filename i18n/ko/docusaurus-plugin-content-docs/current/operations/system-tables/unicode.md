...
`system.unicode` 테이블은 유니코드 문자와 그 속성(https://unicode-org.github.io/icu/userguide/strings/properties.html)에 대한 정보를 제공하는 가상 테이블입니다. 이 테이블은 요청 시 실시간으로 생성됩니다.
...
* `code_point` ([String](../../sql-reference/data-types/)) — U+XXXX 형식으로 표현되는 유니코드 코드 포인트입니다.
* `code_point_value` ([Int32](../../sql-reference/data-types/)) — 유니코드 코드 포인트의 정수 값입니다.
* `notation` ([String](../../sql-reference/data-types/)) — 문자 표기(코드 포인트의 시각적 표현)입니다.
