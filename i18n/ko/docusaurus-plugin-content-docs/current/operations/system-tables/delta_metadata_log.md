...
`system.delta_lake_metadata_log` 테이블은 ClickHouse가 읽는 Delta Lake 테이블의 메타데이터 접근 및 파싱 이벤트를 기록합니다. 각 메타데이터 파일에 대한 자세한 정보를 제공하므로 디버깅, 감사, 그리고 Delta 테이블 구조 변화의 이해에 유용합니다.
