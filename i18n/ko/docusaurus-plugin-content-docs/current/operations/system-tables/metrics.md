...
Keeper 서버 읽기 작업을 위한 스레드 풀의 스레드 수입니다.

### KeeperReadThreadsActive \{#keeperreadthreadsactive\}

Keeper 서버 읽기 작업을 위한 스레드 풀의 활성 스레드 수입니다.

### KeeperReadThreadsScheduled \{#keeperreadthreadsscheduled\}

Keeper 서버 읽기용 스레드 풀에서 큐에 대기 중이거나 현재 실행 중인 작업 수입니다. 실제로는 큰 의미가 없는 메트릭이며, 이 스레드 풀의 실제 읽기 작업은 다른 메커니즘을 통해 스케줄링됩니다.
