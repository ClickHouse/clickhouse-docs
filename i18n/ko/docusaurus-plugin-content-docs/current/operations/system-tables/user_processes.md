---
description: '메모리 사용량 및 사용자 ProfileEvents 개요를 파악하는 데 유용한 정보를 제공하는 시스템 테이블입니다.'
...
이 시스템 테이블은 사용자별 메모리 사용량과 ProfileEvents에 대한 개요를 확인하는 데 사용할 수 있습니다.

## 컬럼 \{#columns\}

{/*AUTOGENERATED_START*/ }

* `user` ([String](../../sql-reference/data-types/)) — 사용자 이름입니다.
* `memory_usage` ([Int64](../../sql-reference/data-types/)) — 해당 사용자의 모든 프로세스가 사용하는 RAM의 합계입니다. 일부 유형의 전용 메모리는 포함되지 않을 수 있습니다. `max&#95;memory&#95;usage` 설정을 참고하십시오.
* `peak_memory_usage` ([Int64](../../sql-reference/data-types/)) — 해당 사용자의 최대 메모리 사용량입니다. 사용자의 쿼리가 실행되지 않을 때 이 값이 초기화될 수 있습니다.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](../../sql-reference/data-types/)) — 사용자에 대한 다양한 메트릭을 측정하는 ProfileEvents의 요약입니다. 이에 대한 설명은 `system.events` 테이블에서 확인할 수 있습니다.
