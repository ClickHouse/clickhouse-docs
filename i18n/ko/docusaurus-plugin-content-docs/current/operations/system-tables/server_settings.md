...
* `changed` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — 해당 설정이 `config.xml`에 지정되었는지 여부를 표시합니다.
* `description` ([String](../../sql-reference/data-types/string.md)) — 서버 설정에 대한 간단한 설명.
* `type` ([String](../../sql-reference/data-types/string.md)) — 서버 설정 값의 형식.
* `changeable_without_restart` ([Enum8](../../sql-reference/data-types/enum.md)) — 설정을 서버가 실행 중인 동안 변경할 수 있는지 여부입니다. 값:
  * `'No' `
  * `'IncreaseOnly'`
  * `'DecreaseOnly'`
  * `'Yes'`
* `is_obsolete` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) - 설정이 더 이상 사용되지 않는지 여부를 표시합니다.

**예시**

다음 예시는 이름에 `thread_pool`이 포함된 서버 설정 정보를 조회하는 방법을 보여줍니다.
