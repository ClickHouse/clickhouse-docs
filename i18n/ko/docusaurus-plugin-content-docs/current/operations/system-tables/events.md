...
&#39;max&#95;network&#95;bandwidth&#95;for&#95;all&#95;users&#39; 스로틀러를 통과한 바이트 수입니다.
...
분산 캐시의 쓰기 버퍼 이벤트입니다. 객체 스토리지에 기록된 바이트 수입니다.

### DistrCacheObjectStorageWriteMicroseconds \{#distrcacheobjectstoragewritemicroseconds\}

분산 캐시 쓰기 버퍼 이벤트입니다. 객체 스토리지에 쓰기 위해 소요된 시간입니다.
...
분산 캐시 연결 이벤트입니다. 풀에서 오래된 연결을 제거하는 데 걸린 시간
...
분산 캐시 이벤트입니다. 해시로 선택된 서버가 변경되어(예: 서버 등록 해제로 인해) 읽기 또는 쓰기 중 발생한 서버 전환 횟수입니다.

### DistrCacheStaleReconnections \{#distrcachestalereconnections\}

분산 캐시 연결 이벤트입니다. 오래된 유효하지 않은(피어 측에서 닫힌) 풀링 연결로 인해 발생한 재연결 횟수입니다.
...
분산 캐시 쓰기 버퍼 이벤트입니다. 분산 캐시에 기록된 바이트 수를 나타냅니다.
...
분산 캐시 쓰기 버퍼 이벤트입니다. WriteBufferFromDistributedCache::writeToFileSegment에 소요된 시간입니다.

### DistrCacheWriteReconnectsAfterTimeout \{#distrcachewritereconnectsaftertimeout\}

분산 캐시 쓰기 버퍼 이벤트입니다. 타임아웃 후 재연결된 횟수입니다.
...
요청한 스냅샷을 로컬에서 더 이상 사용할 수 없어 보류된 스냅샷 읽기 횟수입니다. NuRaft는 최신 스냅샷으로 다시 시도합니다.
...
쿼리에 사용된 세그먼트 수로, 모든 분산 테이블과 테이블 함수에 걸쳐 합산한 값입니다. 하나의 호스트가 여러 테이블에 나타나면 여러 번 계산됩니다. 이 값은 예상되는 전체 세그먼트 수를 나타내며, `skip_unavailable_shards` 설정으로 건너뛴 세그먼트도 포함됩니다.
...
### 지연 포스팅 리스트 모드에서 출력 영역이 이미 결정되어(OR의 경우 모두 1, AND의 경우 모두 0) 건너뛴 패킹된 블록 수 \{#textindexlazyblocksskippedresolved\}

지연 포스팅 리스트 모드에서 출력 영역이 이미 결정되어(OR의 경우 모두 1, AND의 경우 모두 0) 건너뛴 패킹된 블록 수입니다.
...
지연 포스팅 리스트 모드에서 준비된 세그먼트 수(디스크에서 읽거나 캐시에서 가져옴).

### TextIndexLazySegmentsSkippedDense \{#textindexlazysegmentsskippeddense\}

지연 포스팅 리스트 모드에서 블록을 디코딩하는 대신 세그먼트 전체를 패딩해 처리한 fully-dense 세그먼트 수입니다(OR의 경우 memset, AND의 경우 increment).

### TextIndexLazySegmentsSkippedResolved \{#textindexlazysegmentsskippedresolved\}

지연 포스팅 리스트 모드에서 출력 영역이 이미 확정된 상태(OR의 경우 전부 1, AND의 경우 전부 0)여서 건너뛴 세그먼트 수입니다.
...
&#39;max&#95;network&#95;bandwidth&#95;for&#95;user&#39; 스로틀링에 맞추기 위해 쿼리가 대기한 총 시간입니다.
...
ZooKeeper watch 콜백에서 발생한 예외 수입니다.
...
레플리카가 로그 엔트리를 처리할 때까지 대기하는 동안 전송된 watch 알림 수입니다(SYNC REPLICA, ALTER, 뮤테이션).
...
WorkloadEntityKeeperStorage로 전달된 watch 알림 횟수입니다.
