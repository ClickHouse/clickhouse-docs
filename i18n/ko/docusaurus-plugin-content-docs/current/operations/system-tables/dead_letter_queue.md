...
* `table_engine` ([Enum8(&#39;Kafka&#39; = 1, &#39;RabbitMQ&#39; = 2)](/sql-reference/data-types/enum)) — 스트림 유형입니다. 가능한 값은 &#39;Kafka&#39;, &#39;RabbitMQ&#39;입니다.
* `event_date` ([Date](/sql-reference/data-types/date)) — 메시지 소비 날짜입니다.
* `event_time` ([DateTime](/sql-reference/data-types/datetime)) — 메시지 소비 일시입니다.
* `event_time_microseconds` ([DateTime64(6)](/sql-reference/data-types/datetime64)) — 마이크로초 정밀도의 쿼리 시작 시간입니다.
* `database` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — Kafka 테이블이 속한 ClickHouse 데이터베이스입니다.
* `table` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — ClickHouse 테이블 이름입니다.
* `error` ([String](/sql-reference/data-types/string)) — 오류 텍스트입니다.
* `raw_message` ([String](/sql-reference/data-types/string)) — 메시지 본문입니다.
* `kafka_topic_name` ([String](/sql-reference/data-types/string)) — Kafka 토픽 이름입니다.
* `kafka_partition` ([UInt64](/sql-reference/data-types/int-uint)) — 토픽의 Kafka 파티션입니다.
* `kafka_offset` ([UInt64](/sql-reference/data-types/int-uint)) — 메시지의 Kafka 오프셋입니다.
* `kafka_key` ([String](/sql-reference/data-types/string)) — 메시지의 Kafka 키입니다.
* `rabbitmq_exchange_name` ([String](/sql-reference/data-types/string)) — RabbitMQ exchange 이름입니다.
* `rabbitmq_message_id` ([String](/sql-reference/data-types/string)) — RabbitMQ 메시지 ID입니다.
* `rabbitmq_message_timestamp` ([DateTime](/sql-reference/data-types/datetime)) — RabbitMQ 메시지 타임스탬프입니다.
* `rabbitmq_message_redelivered` ([UInt8](/sql-reference/data-types/int-uint)) — RabbitMQ 재전송 플래그입니다.
* `rabbitmq_message_delivery_tag` ([UInt64](/sql-reference/data-types/int-uint)) — RabbitMQ delivery tag입니다.
* `rabbitmq_channel_id` ([String](/sql-reference/data-types/string)) — RabbitMQ 채널 ID입니다.
