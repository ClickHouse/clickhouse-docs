...
`clickhouse` 바이너리에 대한 내부 검사(내부 검사) 정보를 포함합니다. 접근하려면 내부 검사 권한이 필요합니다.
...
* `symbol` ([String](../../sql-reference/data-types/)) — 바이너리 내 심볼 이름입니다. 맹글링된 상태입니다. 사람이 읽을 수 있는 이름을 얻으려면 `demangle(symbol)`을 적용할 수 있습니다.
* `symbol_demangled` ([Nullable(String)](../../sql-reference/data-types/)) — XRay 계측에 사용되는 디맹글된 심볼입니다.
* `function_id` ([Nullable(Int32)](../../sql-reference/data-types/)) — XRay 계측 맵의 함수 ID입니다.
* `address_begin` ([UInt64](../../sql-reference/data-types/)) — 바이너리 내 심볼의 시작 주소입니다.
* `address_end` ([UInt64](../../sql-reference/data-types/)) — 바이너리 내 심볼의 종료 주소입니다.
