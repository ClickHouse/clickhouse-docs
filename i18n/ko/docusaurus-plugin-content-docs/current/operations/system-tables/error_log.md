...
* `code` ([Int32](/sql-reference/data-types/int-uint)) — 오류 코드입니다.
* `error` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — 오류 이름입니다.
* `value` ([UInt64](/sql-reference/data-types/int-uint)) — 시간 인터벌 동안 발생한 오류 수입니다.
* `remote` ([UInt8](/sql-reference/data-types/int-uint)) — 원격 예외입니다(즉, 분산 쿼리 중 하나에서 수신된 예외).
* `last_error_time` ([DateTime](/sql-reference/data-types/datetime)) — 마지막 오류가 발생한 시간입니다.
* `last_error_message` ([String](/sql-reference/data-types/string)) — 마지막 오류 메시지입니다.
* `last_error_query_id` ([String](/sql-reference/data-types/string)) — 마지막 오류를 일으킨 쿼리의 ID입니다(사용할 수 있는 경우).
* `last_error_trace` ([Array(UInt64)](/sql-reference/data-types/array)) — 호출된 메서드가 저장된 물리적 주소 목록을 나타내는 스택 트레이스입니다.
