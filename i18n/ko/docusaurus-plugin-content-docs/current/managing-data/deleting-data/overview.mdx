...
| [삭제 뮤테이션](/sql-reference/statements/alter/delete)                        | `ALTER TABLE [table] DELETE` | 데이터가 디스크에서 즉시 삭제되어야 할 때(예: 규정 준수를 위해) 사용합니다. SELECT 성능에 부정적인 영향을 줍니다.                                |
| [테이블 비우기](/sql-reference/statements/truncate)                           | `TRUNCATE TABLE [db.table]`  | 테이블의 모든 데이터를 효율적으로 제거합니다.                                                                            |
| [파티션 삭제](/sql-reference/statements/alter/partition#drop-partitionpart) | `DROP PARTITION`             | 파티션의 모든 데이터를 효율적으로 제거합니다.                                                                            |
