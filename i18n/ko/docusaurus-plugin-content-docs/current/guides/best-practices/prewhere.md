...
[PREWHERE 절](/sql-reference/statements/select/prewhere)은 ClickHouse의 쿼리 실행 최적화 기능입니다. 불필요한 데이터 읽기를 피하고, 디스크에서 필터링에 사용되지 않는 컬럼을 읽기 전에 관련 없는 데이터를 걸러내어 I/O를 줄이고 쿼리 속도를 향상합니다.

이 가이드에서는 PREWHERE의 동작 방식, 그 영향을 측정하는 방법, 그리고 최상의 성능을 위해 이를 조정하는 방법을 설명합니다.
...
① 쿼리에는 테이블의 기본 키에 포함되어 있고, 따라서 프라이머리 인덱스에도 포함된 `town` 컬럼에 대한 필터가 있습니다.

②  PREWHERE 절 없이 실행한 경우와 마찬가지로, 쿼리를 가속하기 위해 ClickHouse는 프라이머리 인덱스를 메모리에 로드합니다.
...
* PREWHERE는 선택도가 높은 필터가 있는 와이드 테이블과 대규모 스캔에서 가장 큰 효과를 발휘합니다.