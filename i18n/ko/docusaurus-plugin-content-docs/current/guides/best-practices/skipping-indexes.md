...
* 특수화된 **tokenbf&#95;v1** *(지원 중단됨)*). Bloom filter 튜닝과 관련된 세 가지 파라미터를 받습니다: (1) 필터의 크기(바이트 단위, 필터가 클수록 오탐이 줄어들지만 저장 공간이 더 필요함), (2) 적용할 해시 함수의 개수(마찬가지로 해시 함수가 많을수록 오탐이 줄어듦), (3) Bloom filter 해시 함수에 사용할 시드 값입니다. 이러한 파라미터가 Bloom filter 동작에 어떤 영향을 주는지에 대한 자세한 내용은 [여기](https://hur.st/bloomfilter/) 계산기를 참조하십시오.
...
* 특수화된 **ngrambf&#95;v1** *(지원 중단됨)*. 이 인덱스는 token 인덱스와 동일한 방식으로 동작합니다. Bloom filter 설정 전에 하나의 파라미터를 추가로 받는데, 인덱싱할 ngram의 크기입니다. ngram은 길이가 `n`인 임의의 문자 시퀀스로, 문자열 `A short string`에 대해 ngram 크기가 4라면 다음과 같이 인덱싱됩니다:
  ```text
...
> 전체 텍스트 검색 워크로드에는 지원 중단된 *tokenbf&#95;v1* 또는 *ngrambf&#95;v1* 인덱스보다 전용 **text index**([전체 텍스트 검색용 Text index](engines/table-engines/mergetree-family/textindexes.md) 참조)를 사용하는 것이 좋습니다.
> text index는 진정한 역인덱스를 제공하며, token 기반 Bloom filter 인덱스와 비교해 더 나은 검색 성능, 더 예측 가능한 동작, 그리고 더 높은 유연성과 성능을 제공합니다.
