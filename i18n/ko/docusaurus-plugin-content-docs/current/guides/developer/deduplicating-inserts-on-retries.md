...
'description': '삽입 작업을 재시도할 때 중복 데이터를 방지합니다.'
'keywords':
- 'deduplication'
- 'deduplicate'
- 'insert retries'
...
Here we see that two parts have been inserted into the `dst` table. 2 blocks from select -- 2 parts on insert. The parts contain different data.
