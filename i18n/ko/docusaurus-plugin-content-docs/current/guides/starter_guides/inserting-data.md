...
이 클라이언트들은 INSERT가 올바르게 수행되도록 최적화되어 있으며, 예를 들어 [Go 클라이언트](/integrations/language-clients/go/clickhouse-api#async-insert)처럼 직접적으로 또는 쿼리, USER, 연결 수준 SETTINGS에서 비동기 INSERT를 활성화했을 때 간접적으로 비동기 INSERT를 네이티브로 지원합니다.
