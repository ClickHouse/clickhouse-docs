...
이 예제에서는 커맨드라인에서 배치 모드의 `clickhouse-client`를 사용하여 CSV 파일을 ClickHouse에 삽입하는 방법을 보여줍니다. 배치 모드의 `clickhouse-client`를 사용해 커맨드라인으로 데이터를 삽입하는 추가 정보와 예제는 「[Batch mode](/interfaces/client#batch-mode)」를 참고하십시오.
