...
title: 'sumIf'
description: 'sumIf 조합자(combinator)를 사용하는 예제'
...
이 예제에서는 성공 여부 플래그와 함께 판매 데이터를 저장하는 테이블을 만들고,
`sumIf`를 사용하여 성공한 트랜잭션의 총 판매 금액을 계산합니다.
...
이 예제에서는 [ClickHouse playground](https://sql.clickhouse.com/)에서 제공하는 `stock` 테이블을 사용하여 2002년 상반기의 가격 방향별 거래량을 계산합니다.
