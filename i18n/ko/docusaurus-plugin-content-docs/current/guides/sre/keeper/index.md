...
ClickHouse Keeper는 Zookeeper와 거의 동일한 4lw(four-letter word) 명령도 제공합니다. 각 명령은 `mntr`, `stat` 등과 같이 네 글자로 구성됩니다. 몇 가지 유용한 명령이 있습니다. `stat`는 서버와 연결된 클라이언트에 대한 일반적인 정보를 제공하고, `srvr`는 서버에 대한 보다 상세한 정보를 제공하며, `cons`는 연결에 대한 보다 상세한 정보를 제공합니다.

4lw 명령에는 `four_letter_word_white_list`라는 허용 목록 설정이 있으며, 기본값은 `conf,cons,crst,envi,ruok,srst,srvr,stat,wchs,dirs,mntr,isro,rcvr,apiv,csnp,lgif,rqld,ydld`로 설정됩니다.
...
* `ftfl`: 모든 기능 플래그와 각 플래그가 해당 Keeper 인스턴스에서 활성화되어 있는지 여부를 나열합니다.
...
* `ydld`: 리더십을 양도하고 팔로워가 되도록 요청합니다. 이 요청을 받은 서버가 리더인 경우, 먼저 쓰기 작업을 일시 중지하고, 후임자(현재 리더는 후임자가 될 수 없음)가 최신 로그를 모두 따라잡을 때까지 대기한 뒤 사임합니다. 후임자는 자동으로 선택됩니다. 요청이 전송되면 `Sent yield leadership request to leader.` 를 반환하고, 요청이 전송되지 못하면 `Failed to send yield leadership request to leader.` 를 반환합니다. 노드가 이미 팔로워인 경우 결과는 요청이 전송된 경우와 동일합니다.
...
ZooKeeper에서 ClickHouse Keeper로 중단 없이 매끄럽게 마이그레이션하는 것은 불가능합니다. ZooKeeper 클러스터를 중지하고, 데이터를 변환한 뒤, ClickHouse Keeper를 시작해야 합니다. `clickhouse-keeper-converter` 도구는 ZooKeeper 로그와 스냅샷을 ClickHouse Keeper 스냅샷으로 변환합니다. ZooKeeper 3.4 이상이 필요합니다.

#### 마이그레이션 전 준비 \{#pre-migration-preparation\}

마이그레이션을 수행하려면 데이터 수집을 중지해야 합니다. 시작하기 전에 메인터넌스 기간을 계획하십시오.

ZooKeeper를 중지하기 전에 조정 메타데이터를 수정하는 ClickHouse 백그라운드 작업을 중지하십시오. 예:

```sql
SYSTEM STOP MERGES;
```

마이그레이션 전에 비교용 지표를 기록해 두어 이후에 일관성을 검증할 수 있도록 하십시오.


#### 마이그레이션 단계 \{#migration-steps\}

1. 모든 ClickHouse 노드로의 데이터 수집을 중지하십시오.

2. 모든 ClickHouse 노드에서 백그라운드 작업을 모두 중지하십시오([위](#pre-migration-preparation) 참조).

3. 모든 ZooKeeper 노드를 중지하십시오.
...
6. 스냅샷을 모든 ClickHouse Keeper 노드에 복사하십시오. 어떤 노드도 시작되기 전에 모든 노드에 스냅샷이 있어야 합니다. 스냅샷 없이 노드가 시작되면 빈 상태로 스스로를 리더로 선출할 수 있습니다.

7. 새 Keeper 클러스터를 가리키도록 ClickHouse 설정을 업데이트하십시오.

8. 모든 노드에서 ClickHouse Keeper를 시작한 다음 ClickHouse를 다시 시작하십시오.

9. 일관성이 유지되는지 확인하기 위해 메트릭을 마이그레이션 전 기준값과 비교하십시오.

10. 백그라운드 작업을 재개하고 데이터 수집을 다시 시작하십시오.


#### 여러 ZooKeeper 클러스터 통합하기 \{#consolidating-multiple-zookeeper-clusters\}

여러 ZooKeeper 클러스터(예를 들어 세그먼트 그룹마다 1개씩)를 운영하는 경우, 이를 단일 ClickHouse Keeper 클러스터로 통합할 수 있습니다. 공식 `clickhouse-keeper-converter` 도구는 1:1 변환(하나의 ZooKeeper 클러스터를 하나의 Keeper 스냅샷으로 변환)만 지원하므로, 통합하려면 여러 스냅샷을 병합할 수 있도록 `clickhouse-keeper-converter`의 소스 코드를 수정해야 합니다.

1. 각 ZooKeeper 클러스터에서 `clickhouse-keeper-converter`를 개별적으로 실행하고, 각 출력은 서로 다른 디렉터리에 기록합니다.
2. 스냅샷 파일을 순차적으로 역직렬화합니다. 병합할 때는 서로 다른 원본 클러스터의 네임스페이스 사이에서 노드 ID 충돌이 발생하지 않도록 `numChildren` 값을 다시 계산합니다.
3. 병합된 출력을 대상 ClickHouse Keeper 스냅샷 디렉터리에 기록합니다.

#### 암호화 및 ACL 처리 \{#handling-encryption-and-acls\}

ClickHouse Keeper는 ZooKeeper와 동일한 ACL 스킴(`world`, `auth`, `digest`)을 지원합니다. 변환 시 ACL을 처리하는 방법은 ZooKeeper 설정에 따라 달라집니다.

* **완전히 암호화되었거나 전혀 암호화되지 않은 경우**: 바로 변환하십시오. 변환 도구는 기존 ACL 정보를 유지합니다.
* **부분적으로 암호화된 경우**: 변환 전에 슈퍼 관리자 계정에 권한을 부여하고, 영향을 받는 경로에서 `setAcl -R`로 ACL을 제거하십시오. 변환한 후 필요하면 ClickHouse Keeper에서 암호화를 다시 활성화하십시오.

#### 마이그레이션 확인 \{#verifying-the-migration\}

ClickHouse Keeper를 시작하고 ClickHouse를 다시 시작한 후, 마이그레이션이 성공했는지 확인하려면 마이그레이션 전 기준값과 핵심 메트릭을 비교하십시오.

여러 ZooKeeper 클러스터를 통합할 때는 다음을 구분하십시오:

* **공통 경로**: 여러 원본 클러스터에 동일한 데이터로 존재하는 경로입니다. 병합된 출력에서는 이러한 경로를 중복 제거해야 합니다.
* **구별되는 경로**: 특정 클러스터에만 존재하는 경로입니다(예: 각 세그먼트 그룹의 `/clickhouse/tables` 아래). 이러한 경로는 올바른 원본의 내용을 유지해야 합니다.

비교를 위해 대규모 ZooKeeper 트리를 직접 순회하지 마십시오. 대신 변환 중에 변환된 모든 경로를 파일에 출력하십시오.

#### 마이그레이션 후 튜닝 \{#post-migration-tuning\}

마이그레이션 후에는 더 큰 클러스터나 더 높은 처리량을 위해 다음 설정을 조정하는 것을 고려하십시오.

| 설정                                    | 기본값     | 권장값     | 참고                                         |
| ------------------------------------- | ------- | ------- | ------------------------------------------ |
| `max_requests_batch_size`             | `100`   | `10000` | data part 수가 많거나 세그먼트가 많은 클러스터에서는 값을 늘리십시오 |
| `force_sync`                          | `true`  | `false` | 비동기 로그 쓰기는 처리량을 향상시킵니다                     |
| `compress_logs`                       | `false` | `true`  | Raft 로그 파일을 압축하여 디스크 I/O를 줄입니다             |
| `compress_snapshots_with_zstd_format` | `true`  | —       | 기본적으로 이미 활성화되어 있으며 zstd로 스냅샷을 압축합니다        |

이 설정은 [Keeper 구성](#internal-coordination-settings)의 `coordination_settings` 아래에서 지정합니다.
...
클러스터에서 노드를 추가/제거하려면 쿼럼이 필요합니다. 클러스터의 노드를 너무 많이 동시에 잃고
...
6. 복구 모드에서는, 리더 노드는 새로운 노드들과 쿼럼을 달성할 때까지 `mntr` 명령에 대해 오류 메시지를 반환하며, 클라이언트와 팔로워로부터 오는 모든 요청을 거부합니다.
7. 쿼럼이 달성되면 리더 노드는 정상 동작 모드로 돌아가며, 모든 요청을 수락합니다. Raft 기준으로 `mntr`로 검증했을 때 `zk_server_state`가 `leader`를 반환해야 합니다.
...
또한 최신 로그에는 `keeper_server.latest_log_storage_disk`를, 최신 스냅샷에는 `keeper_server.latest_snapshot_storage_disk`를 사용할 수 있습니다.
...
ClickHouse Keeper는 ZooKeeper와의 완전한 호환성을 목표로 하지만, 아직 구현되지 않은 기능이 일부 있습니다(개발은 진행 중입니다):
