...
<VerticalStepper headerLevel="h2">
  ...
  uv를 사용하지 않으려면 MCP 서버 구성을 업데이트하여 다른 패키지 관리자를 사용해야 합니다.

  ## Claude Desktop 다운로드하기 \{#download-claude-desktop\}

  또한 Claude Desktop 앱을 설치해야 하며, [Claude Desktop 웹사이트](https://claude.ai/desktop)에서 다운로드할 수 있습니다.
  ...
  <Image img={ClaudeDesktopConfig} alt="Claude Desktop 설정 파일" size="md" />

  해당 버튼을 클릭하면 설정 파일(`claude_desktop_config.json`)이 포함된 디렉터리로 이동합니다.
  ...
  이제 Claude가 ClickHouse MCP 서버를 사용하게 하는 질문을 할 준비가 되었습니다.
  예를 들어, `SQL playground에서 가장 흥미로운 dataset은 무엇인가요?`라고 물어볼 수 있습니다.
