...
<VerticalStepper headerLevel="h2">
  ...
  uv를 사용하지 않으려면 다른 패키지 관리자를 사용하도록 MCP 서버 구성을 업데이트해야 합니다.
  ...
  ClickHouse MCP 서버를 구성하려면 MCP 서버를 Open API 엔드포인트로 변환해야 합니다.
  ...
  <Image img={AddConnection} alt="Open WebUI - 연결로 OpenAI 추가" size="md" />
  ...
  <Image img={Conversation} alt="Open WebUI - ClickHouse MCP 서버와 대화" size="md" />
</VerticalStepper>