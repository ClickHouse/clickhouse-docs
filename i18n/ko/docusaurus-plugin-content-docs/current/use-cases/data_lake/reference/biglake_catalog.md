...
description: '이 가이드에서는 ClickHouse와 Lakehouse 런타임 카탈로그(BigLake 메타스토어)를 사용하여 Google Cloud Storage에 있는 데이터를 쿼리하는 단계를 안내합니다.'
...
ClickHouse는 여러 카탈로그(Unity, Glue, Polaris 등)와의 통합을 지원합니다. 이 가이드에서는 ClickHouse를 통해 [Lakehouse 런타임 카탈로그, 즉 BigLake 메타스토어](https://docs.cloud.google.com/biglake/docs/)에 있는 Iceberg 테이블을 쿼리하는 단계를 안내합니다.
...
## Lakehouse 런타임 카탈로그와 ClickHouse 간 연결 생성 \{#creating-a-connection\}
...
## ClickHouse를 사용하여 Lakehouse 런타임 카탈로그 테이블 쿼리하기 \{#querying-lakehouse-runtime-tables\}
...
## Lakehouse에서 ClickHouse로 데이터 로드하기 \{#loading-data-into-clickhouse\}
