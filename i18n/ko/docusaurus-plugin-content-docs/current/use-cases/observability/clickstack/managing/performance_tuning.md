...
import OtelLogsSchema from '@site/i18n/ko/docusaurus-plugin-content-docs/current/use-cases/observability/clickstack/ingesting-data/_snippets/_schema_otel_logs.md';
import OtelTracesSchema from '@site/i18n/ko/docusaurus-plugin-content-docs/current/use-cases/observability/clickstack/ingesting-data/_snippets/_schema_otel_traces.md';
...
그러나 사용자가 계속해서 원래 속성 경로(예: `ResourceAttributes.k8s.pod.name:"checkout-675775c4cc-f2p9c"`)로 쿼리하더라도, **ClickStack는 내부적으로 materialized `PodName` 컬럼을 사용하도록 쿼리를 자동으로 재작성합니다**. 즉, 다음 프레디케이트를 사용합니다:

```sql
PodName = 'checkout-675775c4cc-f2p9c'
```

이렇게 하면 대시보드, 알림 또는 저장된 쿼리를 변경하지 않고도 사용자가 최적화의 이점을 누릴 수 있습니다.

:::note
기본적으로 materialized 컬럼은 `SELECT *` 쿼리에서 제외됩니다. 이렇게 하면 쿼리 결과를 항상 테이블에 다시 삽입할 수 있다는 불변 조건이 유지됩니다.
:::
...
이러한 인덱스는 세 가지 일반적인 패턴에 중점을 둡니다:

* TraceId, 세션 식별자, 속성 키 또는 값과 같은 카디널리티가 높은 문자열 필터링
* [`*AttributeItems`](#map-direct-read-optimization) 컬럼의 [text indexes](#text-indexes)로 가속되는 맵 하위 키 필터링
* 스팬 지속 시간과 같은 숫자 범위 필터링

logs 테이블은 Bloom 필터 대신 전체적으로 `text(tokenizer = 'array')` 인덱스를 사용하며, 전문 검색을 위해 `lower(Body)`에 `text(tokenizer = 'splitByNonAlpha')` 인덱스를 추가합니다. 전체 DDL은 [「ClickStack에서 사용하는 테이블 및 스키마」](/use-cases/observability/clickstack/ingesting-data/schemas#logs)를 참조하십시오.
...
[Text indexes](/engines/table-engines/mergetree-family/textindexes)는 블룸 필터의 대안이 될 수 있습니다. 블룸 필터는 확률적 구조이므로 그래뉼을 확실하게 제외할 수 있지만, 거짓 양성 비율이 있기 때문에 제외되지 않은 그래뉼은 여전히 로드한 뒤 `WHERE` 조건으로 평가해야 합니다. 텍스트 인덱스는 토큰을 파트 내 정확한 오프셋에 매핑하는 역인덱스입니다. 텍스트 인덱스는 그래뉼이 아니라 오프셋을 평가하고 거짓 양성을 만들지 않으므로, 일반적으로 기본 컬럼을 로드하지 않고도 `WHERE` 조건을 처리할 수 있습니다. 이는 [직접 읽기](https://github.com/ClickHouse/clickhouse-docs/pull/6356/%E2%80%A6)라고 하는 최적화입니다. 데이터 로드는 대개 쿼리 시간에 가장 큰 영향을 주는 요소이므로, 직접 읽기는 쿼리 지연 시간을 의미 있게 줄일 수 있습니다.

또한 텍스트 인덱스 자체도 쿼리할 수 있으므로, ClickStack에서 자동 완성과 기타 내부 검사 기능을 지원합니다.

두 가지 토크나이저로 대부분의 ClickStack 패턴을 처리할 수 있습니다:

| 토크나이저             | 사용 대상                               | 일반적인 컬럼                           |
| ----------------- | ----------------------------------- | --------------------------------- |
| `array`           | `Array(String)` 요소를 하나의 전체 토큰으로 인덱싱 | `mapKeys(...)`, `*AttributeItems` |
| `splitByNonAlpha` | 일반 문장 문자열에 대한 단어 수준 전문 검색           | `Body`, `lower(Body)`, `SpanName` |

#### 맵 및 배열 컬럼용 배열 토크나이저 \{#array-tokenizer\}

기본 로그 스키마는 `array` 토크나이저를 사용해 `mapKeys`와 구체화된 항목 배열을 인덱싱합니다:
...
속성 키로 필터링하면 해당 맵 컬럼 전체를 스캔하지 않고도 이를 포함하지 않는
모든 행을 걸러낼 수 있습니다. 이것이 [맵 직접 읽기 최적화](#map-direct-read-optimization)를
효율적으로 만드는 메커니즘입니다.

#### 로그 본문을 위한 `splitByNonAlpha` \{#text-index-body\}

`Body` 컬럼의 전문 검색은 `splitByNonAlpha` 텍스트
인덱스를 사용하면 성능상 이점을 얻을 수 있습니다. ClickStack은 대소문자를 구분하지 않는
Lucene 검색에서 이 인덱스를 사용할 수 있도록 `lower(Body)`에 이 인덱스를 정의합니다:

```sql
INDEX idx_lower_body lower(Body) TYPE text(tokenizer = 'splitByNonAlpha')
```

ClickStack가 `lower(Body)`의 `text(tokenizer = 'splitByNonAlpha')` 인덱스를 감지하면, `error` 또는
`"connection refused"`와 같이 컬럼을 명시하지 않은 Lucene 쿼리를
`hasAllTokens(lower(Body), lower(...))`로 재작성합니다. 이렇게 하면
전체 `Body` 컬럼을 읽지 않고도 인덱스로 처리할 수 있습니다. 대부분의
관측성 로그 워크로드에서는 이것이 가장 큰 단일 필터링 성능 향상을 제공합니다.

:::note 텍스트 인덱스와 `tokenbf_v1`
이전 `tokenbf_v1` 인덱스 유형은 (`lower(SpanName)`에 대해 기본 traces 스키마에서 여전히 사용됨)
기능적으로는 유사하지만 ClickHouse 26.2
이상에서는 Deprecated되었습니다. 새 전문 검색 인덱스에는 `text(tokenizer = ...)`를 사용해야 합니다.
:::

토크나이저 옵션, 전처리기, 검증에 대한 자세한 참고 정보는 [전문 검색 문서](/engines/table-engines/mergetree-family/textindexes)를 참조하십시오.

#### 기본 logs 스키마의 텍스트 인덱스 \{#text-indexes-in-default-logs-schema\}

업스트림과 동기화되는 기본 `otel_logs` 스키마(schema)에는 위에서 설명한 모든 텍스트 인덱스가 기본으로 포함되어 있습니다. 즉, `TraceId`에는 `text(tokenizer = 'array')`가, 각 `mapKeys(...)` 및 `*AttributeItems` 배열에는 `text(tokenizer = 'array')`가, 그리고 전문 검색을 위해 `lower(Body)`에는 `text(tokenizer = 'splitByNonAlpha')`가 적용됩니다. 표준 DDL은 [&quot;ClickStack에서 사용하는 테이블과 스키마&quot;](/use-cases/observability/clickstack/ingesting-data/schemas#logs)를 참조하십시오. 동일한 스키마를 아래에도 다시 수록했습니다.

<OtelLogsSchema />

### 최소-최대 인덱스 \{#min-max-indexes\}
...
### 스킵 인덱스 구체화(Materialize) \{#materialize-skip-index\}

스킵 인덱스를 추가한 이후에는 새로 수집된 데이터에만 적용됩니다. 인덱스를 명시적으로 구체화하기 전에는 과거 데이터에는 이 인덱스의 효과가 적용되지 않습니다.

이미 스킵 인덱스를 추가한 경우, 예를 들어 다음과 같습니다:
...
스킵 인덱스를 구체화하는 작업은 mutation으로 처리됩니다. 진행 상황은 system 테이블을 통해 모니터링할 수 있습니다.
...
**문자열 컬럼에는 지원되는 경우 텍스트 인덱스를 우선 사용하고, 그렇지 않으면 블룸 필터를 사용하십시오.** 텍스트 인덱스는 블룸 필터와 동일한 동등 비교 및 `IN` 필터를 가속할 뿐만 아니라, 전문 검색과 [Map 직접 읽기 최적화](#map-direct-read-optimization)에 사용되는 토큰 기반 프레디케이트(`hasToken`, `hasAllTokens`, `has`)도 지원합니다. 아직 텍스트 인덱스를 지원하지 않는 구형 클러스터에서는 블룸 필터가 여전히 좋은 선택지입니다.
...
## Optimization 3. 맵 직접 읽기 \{#map-direct-read-optimization\}

`LogAttributes['k8s.pod.name'] =
'checkout'`와 같은 맵 하위 키로 필터링하면, ClickHouse는 프레디케이트를 평가하기 위해 디스크에서 `LogAttributes` 맵 컬럼 전체를 읽고 각 행을 풀어야 합니다. [자주 쿼리되는 속성 구체화하기](#materialize-frequently-queried-attributes)는 미리 알고 있는 키에는 이 문제를 해결해 주지만, 사용자가 필요할 때마다 임시로 필터링하는 임의의 속성까지 처리하도록 확장되지는 않습니다.

스키마에 `mapKeys`와 `mapValues`에 대한 인덱스가 있더라도, 이러한 인덱스는 행에 특정 키가 있는지, 특정 값이 있는지는 알려줄 수 있지만, 그 키와 값이 같은 항목에 속하는지는 알려줄 수 없습니다. 다시 말해, `mapKeys`는 `mapContainsKey(ResourceAttributes, 'foo')`에는 답할 수 있고 `mapValues`는 `mapContainsValue(ResourceAttributes, 'bar')`에는 답할 수 있지만, 어느 쪽도 `ResourceAttributes['foo'] = 'bar'`에는 답하지 못합니다.

키와 값을 하나의 `Array(String)` 컬럼으로 이어 붙이면, 맵 직접 읽기 최적화를 통해 기본 맵을 로드하지 않고도 `ResourceAttributes['foo'] = 'bar'`에 답할 수 있습니다. 맵은 대체로 크기가 크고, 볼륨이 늘어날수록 함께 커집니다. 여기에 애플리케이션 수준의 쿼리 재작성을 결합하면, 모든 맵 하위 키에 대한 동등성 필터가 해당 인덱스를 사용하는 단일 `has(...)` 호출로 바뀌며, 쿼리 시점에 맵 역직렬화가 필요하지 않습니다. 또한 실제로 드는 저장소 비용은 텍스트 인덱스에 대해서만 발생하는데, 기본 컬럼이 `ALIAS` 컬럼이어서 저장되지 않기 때문입니다.

이 최적화는 자동으로 적용됩니다. ClickStack은 기본 logs 및 trace 테이블에 필요한 컬럼과 인덱스를 포함하며, 연결된 ClickHouse 서버가 기반 기능을 지원하면 런타임에 맵 첨자 필터를 재작성합니다. 스키마에 이러한 컬럼이 없거나, 기본값 외에 추가로 가속하려는 맵 컬럼이 있다면, 계속 읽고 이를 활성화하십시오.

### 스키마 \{#map-direct-read-schema\}

가속하려는 각 맵 컬럼마다 ClickStack은
...
`ALIAS` 형태는 배열이 디스크에 바이트를 전혀 추가하지 않음을 의미합니다. ClickHouse는 이를
쿼리 시점과 인덱스를 빌드할 때 계산합니다. `ALIAS` 컬럼의 `text(tokenizer = 'array')` 스킵 인덱스는
`key=value` 쌍마다 토큰 하나를 저장하며, ClickHouse는 이를 사용해 소스 맵을 건드리지 않고
그래뉼을 걸러냅니다:
...
기존 테이블에 인덱스를 생성한 후에는 과거 데이터에서도 사용할 수 있도록 이를 구체화하십시오([&quot;스킵 인덱스 구체화&quot;](#materialize-skip-index) 참조).

기본 ClickStack 스키마는 다음 컬럼과 인덱스를 제공합니다:

| 테이블           | ALIAS 컬럼                                                             | 텍스트 인덱스                                                            |
| ------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------ |
...
### 쿼리 재작성 \{#map-direct-read-rewrite\}

사용자가 ClickStack UI 또는 SDK를 통해 맵 하위 키를 기준으로 필터링하면 ClickStack은 다음과 같이 재작성합니다:
...
재작성된 형태는 `LogAttributeItems`의 텍스트 인덱스를 활용해 `key=value` 토큰을 포함하지 않는
행 전체를 걸러내고, 일치하지 않는 행에 대해서는 원본 `LogAttributes` 맵을
역직렬화하지도 않습니다. 카디널리티가 높은
관측성 워크로드에서는 일반적으로 맵 서브스크립트 접근 방식보다 I/O를
10배가량 줄여 줍니다.

이 재작성은 자동으로 수행되므로, `LogAttributes['key']`를 참조하는 저장된 쿼리, 대시보드, 및 알림은
아무런 변경 없이도 성능 향상을 얻습니다.

### ClickHouse 버전 요구 사항 \{#map-direct-read-version\}

쿼리 재작성에는 텍스트 인덱스가 적용된 배열 컬럼에서 직접
토큰 수준 프루닝을 지원하는 ClickHouse 버전이 필요합니다. ClickStack은
연결된 서버의 버전(`SELECT version()`, 연결별로 캐시됨)을 감지하고, 서버가
임계값 이상일 때만 재작성된 형태를 사용합니다. 이전 버전의 서버에서는 자동으로
원래의 맵 서브스크립트 형식으로 돌아갑니다.
...
:::note 왜 ALIAS이고 MATERIALIZED는 아닌가
items 배열은 이미 맵 컬럼에 들어 있는 데이터를 그대로 비추는 창입니다.
이를 두 번 저장하면 — 한 번은 맵에, 한 번은 배열에 — 새로운 쿼리 패턴을
지원하지도 못한 채 쓰기 I/O만 두 배로 늘어나게 됩니다. `ALIAS` 컬럼의 텍스트 인덱스는
...
기본 키는 압축 및 저장 레이아웃에도 영향을 미치지만, 주된 목적은 쿼리 성능입니다. ClickStack에서는 기본 제공되는 기본 키가 가장 일반적인 관측성(액세스 패턴) 패턴과 강력한 압축을 위해 이미 최적화되어 있습니다. 로그, 트레이스, 메트릭 테이블의 기본 키는 일반적인 워크플로에서 좋은 성능을 내도록 설계되어 있습니다.
...
아래는 로그와 트레이스용 ClickStack 테이블의 기본 기본 키입니다.
...
다른 데이터 타입의 테이블에서 사용되는 기본 키는 [&quot;Tables and schemas used by ClickStack&quot;](/use-cases/observability/clickstack/ingesting-data/schemas)를 참조하십시오. 트레이스 테이블은 서비스 이름과 스팬 이름으로 필터링한 다음 타임스탬프로 필터링하도록 최적화되어 있습니다. 로그 테이블은 5분 시간 버킷을 앞에 두어 시간 범위 스캔이 먼저 프라이머리 인덱스에 도달한 다음, 각 버킷 내에서 서비스 이름으로 범위를 좁히도록 구성되어 있으며, 이는 일반적인 &quot;서비스 X에서 최근 N분 동안 무슨 일이 있었는가&quot; 워크플로에 적합한 레이아웃입니다. 최적의 순서는 기본 키의 순서대로 필터를 적용하는 것이지만, 어떤 순서로든 이 컬럼들로 필터링하더라도 ClickHouse가 [읽기 전에 데이터를 프루닝(pruning)](/optimize/skipping-indexes)하므로 쿼리는 여전히 큰 이점을 얻을 수 있습니다.
...
다음 절차를 사용하면 이전 데이터를 유지하면서도 투명하게 쿼리할 수 있습니다. 필요하다면 ClickStack UI에서 기존 키를 사용해 쿼리할 수 있고, 동시에 새로운 데이터는 사용자의 액세스 패턴에 최적화된 새 테이블을 통해 제공됩니다. 이 접근 방식은 수집 파이프라인을 수정할 필요가 없도록 보장하며, 데이터는 계속 기본 테이블 이름으로 전송되고 모든 변경 사항은 사용자에게 투명하게 처리됩니다.
...
<VerticalStepper headerLevel="h4">
  ...
  #### ClickStack UI가 Merge 테이블을 읽도록 업데이트

  ClickStack UI에서 로그 데이터 소스의 테이블로 `otel_logs_merge`를 사용하도록 구성합니다.
...
### block 컬럼을 활용한 행 조회 가속화

기본 ClickStack 로그 스키마는 쿼리 성능에 직접적인 영향을 주지는 않지만,
ClickStack UI에서 행 세부 정보 조회를 실질적으로 더 빠르게 해 주는
두 가지 MergeTree 설정을 활성화합니다:

```sql
SETTINGS enable_block_number_column = 1, enable_block_offset_column = 1
```

이 설정을 사용하면 테이블의 모든 행에는 part 내에서 해당 행을 고유하게 식별하는
암묵적인 `(_block_number, _block_offset)` 쌍이 부여됩니다. ClickStack UI에서 로그 행을 클릭해 상세 패널을 열면, ClickStack은
해당 단일 행을 가져오기 위해 후속 쿼리를 실행합니다. 블록 컬럼이 없으면
행을 구분할 수 있을 만큼 충분한 컬럼이 행의 `WHERE` 절에 포함되어야 하며, 일반적으로 프라이머리 키
외에 `Body`와 `SeverityText`가 여기에 해당합니다. 블록 컬럼이 있으면,
프라이머리 키에 `_block_number`와 `_block_offset`만 있으면 충분합니다. `Body`와 같은 큰
컬럼은 lookup 시 전혀 읽지 않으므로, 결과적으로 쿼리 속도가 빨라집니다.

ClickStack은 테이블의 `CREATE` 문에서 이 설정을 감지하고,
두 컬럼이 모두 활성화되어 있으면 더 간결한 `WHERE` 절을 자동으로 생성합니다. 별도의
애플리케이션 설정 변경은 필요하지 않습니다.

기존 로그 또는 트레이스 테이블에서 이 최적화를 사용하려면:
...
머지로 다시 작성되기 전까지 계속 이전의 행별 조회 방식을 사용합니다.
...
## Optimization 6. Exploiting PROJECTION

PROJECTION은 materialized 컬럼, 스킵 인덱스(skip index), 기본 키, materialized view를 모두 검토한 이후에 고려할 수 있는 최종 단계의 고급 최적화입니다. PROJECTION과 materialized view는 비슷해 보일 수 있지만, ClickStack에서 이들은 서로 다른 목적을 가지며 서로 다른 시나리오에서 사용할 때 가장 효과적입니다.

<iframe width="560" height="315" src="https://www.youtube.com/embed/6CdnUdZSEG0?si=1zUyrP-tCvn9tXse" title="YouTube 동영상 플레이어" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen />

실질적으로 **PROJECTION은 테이블의 추가적인 숨겨진 복사본**으로 볼 수 있으며, 동일한 행을 **서로 다른 물리적 순서**로 저장합니다. 이를 통해 PROJECTION은 기본 테이블의 `ORDER BY` 키와는 별도의 자체 기본 인덱스를 가지게 되고, 원래 정렬 방식과 맞지 않는 접근 패턴에 대해서 ClickHouse가 데이터를 더 효과적으로 걸러낼 수 있도록 합니다.

Materialized view는 다른 정렬 키를 가진 별도의 대상 테이블에 행을 명시적으로 기록하여 유사한 효과를 얻을 수 있습니다. 핵심 차이는 **PROJECTION은 ClickHouse에 의해 자동으로, 투명하게 관리된다는 점**이며, materialized view는 ClickStack에서 명시적으로 등록하고 선택해야 하는 테이블이라는 점입니다.

쿼리가 기본 테이블을 대상으로 실행되면 ClickHouse는 기본 레이아웃과 사용 가능한 PROJECTION들을 평가하고, 각자의 기본 인덱스를 샘플링한 뒤, 올바른 결과를 반환하면서 가장 적은 그래뉼을 읽는 레이아웃을 선택합니다. 이 결정은 쿼리 분석기가 자동으로 수행합니다.

따라서 ClickStack에서 PROJECTION은 다음과 같은 **순수한 데이터 재정렬**에 가장 적합합니다.

* 접근 패턴이 기본 키와 근본적으로 다른 경우
...
* [PROJECTION 가이드](/data-modeling/projections)
* [PROJECTION을 사용할 때](/data-modeling/projections#when-to-use-projections)
* [Materialized view와 PROJECTION 비교](/managing-data/materialized-views-versus-projections)
