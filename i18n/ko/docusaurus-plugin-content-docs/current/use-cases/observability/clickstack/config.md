...
* `BETA_CH_OTEL_JSON_SCHEMA_ENABLED`
  ...
  * **안내:**
    * **베타 기능**을 활성화합니다. JSON 타입 schema는 일반적인 관측성 워크로드에는 **권장되지 않습니다**. 비교 내용과 각각이 적합한 경우는 [Map vs JSON type](/use-cases/observability/clickstack/ingesting-data/schema/map-vs-json)를 참조하십시오.
...
* `CLICKHOUSE_ENDPOINT`
  ...
  * **안내:**
    ...
    * collector가 ClickHouse로 데이터를 전송하려면 필수입니다
...
* `OPAMP_SERVER_URL`
  ...
  * **설명:** collector를 관리하는 데 사용되는 OpAMP 서버의 URL입니다(예: HyperDX 인스턴스). 기본 포트는 `4320`입니다.
  * **안내:**
...
* `OTLP_AUTH_TOKEN`
  ...
  * **안내:**
    * 운영 환경에서 독립 실행형 collector 이미지를 사용하는 경우 설정을 권장합니다

* `HYPERDX_OTEL_EXPORTER_CLICKHOUSE_DATABASE`
  * **기본값:** `default`
  * **설명:** collector가 텔레메트리 데이터를 기록하는 ClickHouse 데이터베이스입니다.
  * **안내:**
...
* `OTEL_AGENT_FEATURE_GATE_ARG`
  * **기본값:** `<empty string>`
  * **설명:** collector에서 활성화할 기능 플래그를 설정합니다. `--feature-gates=clickhouse.json`으로 설정하면 collector에서 JSON 타입에 대한 베타 지원이 활성화되어 schema가 해당 타입으로 생성되도록 합니다. HyperDX에서 JSON 지원을 활성화하려면 [`BETA_CH_OTEL_JSON_SCHEMA_ENABLED`](#hyperdx)도 참고하십시오.
  * **안내:**
    * **베타 기능**을 활성화합니다. JSON 타입 schema는 일반적인 관측성 워크로드에는 **권장되지 않습니다**. 비교와 각 타입이 적합한 사용 사례는 [Map vs JSON type](/use-cases/observability/clickstack/ingesting-data/schema/map-vs-json)를 참고하십시오.
    * JSON 타입을 사용하여 새 테이블을 생성하려면 `--feature-gates=clickhouse.json`으로 설정하십시오."
