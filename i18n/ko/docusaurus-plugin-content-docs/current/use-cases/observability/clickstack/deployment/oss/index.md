...
| ClickHouse Embedded | ClickStack UI가 ClickHouse 서버 바이너리에 직접 번들로 포함됩니다. 추가 컴포넌트가 필요하지 않습니다.      | ClickStack 체험, 기존 ClickHouse 데이터 탐색, 데모                          | 알림 없음, 대시보드/검색 결과 영속성 없음, 쿼리 설정 커스터마이징 불가, 이벤트 패턴 미지원 | [ClickHouse Embedded](/use-cases/observability/clickstack/deployment/clickhouse-embedded) |