...
title: 'ClickStack - 로그, 트레이스, 메트릭 샘플'
sidebar_position: 0
...
이 가이드에서는 샘플 데이터 세트를 사용해 ClickStack Open Source와 Managed ClickStack을 모두 살펴봅니다.
...
<Tabs groupId="sample-logs">
  <TabItem value="managed-clickstack" label="관리형 ClickStack" default>
    <VerticalStepper headerLevel="h3">
      ...
      ### 샘플 데이터 로드하기 \{#load-sample-data-managed\}
      ...
      ### 세션 탐색하기 \{#explore-sessions-managed\}
      ...
      ### 트레이스 탐색하기 \{#explore-traces-managed\}
  ...
  <TabItem value="oss-clickstack" label="ClickStack 오픈소스">
    ...
    <VerticalStepper headerLevel="h3">
      ### ClickStack UI(HyperDX)로 이동하기 \{#navigate-to-the-hyperdx-ui\}

      [http://localhost:8080](http://localhost:8080)을 방문하여 ClickStack UI에 접근하십시오.

      <Image img={hyperdx} alt="ClickStack UI" size="lg" />

      ### 수집 API key 복사하기 \{#copy-ingestion-api-key\}
      ...
      ### 샘플 데이터 다운로드하기 \{#download-sample-data\}
