...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: 비워 두십시오
  ...
  오른쪽 상단의 시간 선택기를 사용하여 이전 `2일`의 모든 데이터를 표시하도록 시간을 조정하세요.
  ...
  이러한 오류 클러스터는 서로 다른 서비스에서 비롯된 것으로 보입니다.
  ...
  * `Timespan`: `지난 1일`
  ...
  오류가 발생하면 응답이 `0s`로 반환된다는 점에 유의하십시오.
  ...
  * 이로 인해 캐시가 커질수록 리소스 사용량이 증가했으며, 구현이 비효율적이었던 것으로 보입니다
  ...
  * 최대 크기에 도달하자 캐시는 결제 요청을 거부하고, 자신의 크기를 `0`으로 보고했습니다.
  ...
  예를 들어, 지원 팀에서 결제 문제를 겪은 사용자의 이메일 `Eddie86@yahoo.com`을 제공한 경우, 로그나 트레이스를 직접 검색하기보다 해당 사용자의 세션부터 확인하는 것이 더 효과적입니다.
  ...
  `SpanAttributes.userEmail: Eddie`를 검색하여 고객의 세션을 찾으세요. 세션을 선택하면 왼쪽에 해당 고객 세션의 브라우저 이벤트 및 관련 스팬이 표시되며, 오른쪽에는 사용자의 브라우저 경험이 재현됩니다:
