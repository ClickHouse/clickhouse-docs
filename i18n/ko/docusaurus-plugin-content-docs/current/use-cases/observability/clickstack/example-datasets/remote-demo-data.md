...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: 비워 두십시오
  ...
  오류 클러스터는 `Failed to place order`라는 이름의 패턴을 포함해 결제 실패와 관련된 문제를 보여줍니다. 추가 클러스터에서는 카드 결제 처리 문제와 캐시가 가득 찬 문제도 나타납니다.

  <Image img={step_4} alt="4단계" size="lg" />

  이러한 오류 클러스터는 서로 다른 서비스에서 발생한 것일 가능성이 높습니다.

  ### 오류 패턴 탐색

  사용자가 결제를 완료할 수 없다는 우리가 보고한 문제와 연관된 가장 명확한 오류 클러스터를 클릭하세요: `Failed to place order`.
  ...
  지속 시간이 `1ms`보다 긴 이벤트를 선택하고 `Filter by selection` 필터를 적용하면, &quot;정상&quot; 이벤트와 지속 시간이 약 0ms인 고밀도 스팬 그룹 간의 차이를 분석할 수 있습니다:

  <Image img={step_17} alt="17단계" size="lg" />

  데이터 하위 집합을 분석해 보면, 선택 범위 밖의 &quot;background&quot; 스팬은 대부분 visa 트랜잭션이며, 캐시 오류로 인해 0ms 응답과 연관되어 있음을 확인할 수 있습니다.
  ...
  * 이 문제로 인해 Visa 결제가 완료되지 않거나, 완료되더라도 매우 오랜 시간이 걸렸습니다.
  ...
  * 이로 인해 캐시 크기가 커질수록 리소스 사용량이 증가했는데, 구현이 비효율적이었기 때문인 것으로 보입니다
  * 캐시가 커질수록 Visa 결제 성능이 저하되었습니다
  * 최대 크기에 도달하자 캐시는 결제 요청을 거부하고, 자신의 크기를 `0`으로 보고했습니다.
  ...
  예를 들어, 지원 팀에서 결제 문제를 겪은 사용자의 이메일 `Ronny.Windler@gmail.com`을 제공한 경우, 로그나 트레이스를 직접 검색하기보다 해당 사용자의 세션부터 확인하는 것이 더 효과적입니다.
  ...
  `SpanAttributes.userEmail: Ronny.Windler`를 검색하여 고객의 세션을 찾으세요. 세션을 선택하면 왼쪽에 해당 고객 세션의 브라우저 이벤트 및 관련 스팬이 표시되며, 오른쪽에는 사용자의 브라우저 경험이 재현됩니다:

  <Image img={step_22} alt="22단계" size="lg" />

  ### 세션 재생

  ▶️ 버튼을 눌러 세션을 재생할 수 있습니다. `Highlighted`와 `All Events` 간 전환을 통해 스팬의 세분화 수준을 조정할 수 있으며, `Highlighted`는 주요 이벤트와 오류를 강조 표시합니다.
