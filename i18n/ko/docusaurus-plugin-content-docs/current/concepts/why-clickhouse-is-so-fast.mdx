---
sidebar_position: 1
sidebar_label: 'ClickHouse는 왜 그렇게 빠를까요?'
description: '빠르게 동작하도록 설계되었습니다. 개발 프로세스 전반에서 쿼리 실행 성능은 항상 최우선 순위였지만, ClickHouse가 실제 프로덕션 시스템이 될 수 있도록 사용자 친화성, 확장성, 보안과 같은 다른 중요한 특성도 함께 고려되었습니다.'
title: 'ClickHouse는 왜 그렇게 빠를까요?'
slug: /concepts/why-clickhouse-is-so-fast
keywords: ['아키텍처', 'VLDB', 'Performance']
...
아키텍처 관점에서 보면 데이터베이스는 (최소한) 스토리지 계층과 쿼리 처리 레이어로 구성됩니다. 스토리지 계층은 테이블 데이터의 저장, 로드 및 유지 관리를 담당하고, 쿼리 처리 레이어는 사용자 쿼리를 실행합니다. 다른 데이터베이스와 비교했을 때 ClickHouse는 두 계층 모두에서 매우 빠른 INSERT 및 SELECT 쿼리를 가능하게 하는 혁신을 제공합니다.
