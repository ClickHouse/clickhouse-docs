...
버전별 레이블은 PR이 반드시 도달해야 하는 *가장 오래된* 릴리스를 지정합니다. 즉, 명시된 릴리스에만 백포트되는 것이 아니라 해당 릴리스 **및 그보다 최신인 모든 활성 릴리스 브랜치에도** 백포트됩니다. 예를 들어, 개발 브랜치에 머지된 PR에 `v25.3-must-backport`가 지정되어 있으면 `25.3`과 그 이후의 모든 활성 릴리스(`25.4`, `25.5`, …)로 백포트됩니다. 여러 버전별 레이블이 있으면 가장 낮은 버전이 적용되는데, 이미 그보다 최신 버전까지 모두 포함하기 때문입니다.
...
| `v{VER}-must-backport` (예: `v25.3-must-backport`) | 해당 릴리스 브랜치와 **그보다 최신인 모든 활성 릴리스 브랜치**에 백포트합니다. 즉, 버전은 PR이 반드시 도달해야 하는 *가장 오래된* 릴리스를 나타냅니다. 이러한 레이블이 여러 개 있으면 가장 낮은 버전이 우선합니다. 해당 브랜치들에서는 `rolling-out` 제외 규칙보다 우선합니다 |
...
#### 3. 롤아웃 브랜치 처리 \{#rolling-out-branch-handling\}

릴리스 PR에 `rolling-out` 레이블이 있으면 일반 백포트 레이블(`pr-must-backport`, `pr-critical-bugfix`)은 해당 브랜치를 건너뜁니다. 봇은 해당 브랜치에 대해 이전에 생성된 체리픽 또는 백포트 PR을 설명 댓글과 함께 닫습니다. 버전별 레이블(예: `v25.3-must-backport`)은 항상 이 동작을 재정의합니다 — 명시된 릴리스와 그 레이블이 확장 적용되는 그보다 최신의 모든 활성 릴리스 브랜치에 대해 그렇습니다. `pr-must-backport-force`는 모든 브랜치에서 `rolling-out` 검사를 우회합니다.
