---
description: 'ClickHouse의 서드파티 사용 방식과 서드파티 라이브러리를 추가하고 유지 관리하는 방법을 설명하는 페이지.'
...
* 서브모듈을 업데이트했으며 이전 패치를 다시 적용해야 하는 경우입니다. 이때 예전 PR을 그대로 다시 만드는 것은 과도합니다. 대신 이전 커밋들을 새 버전에 해당하는 새 `ClickHouse/` 브랜치로 단순히 체리픽 하십시오. 여러 개의 커밋으로 구성된 PR은 필요하다면 하나의 커밋으로 squash 해도 됩니다. 최선의 경우 커스텀 패치를 업스트림에 기여해 두었다면 새 버전에서는 해당 패치를 생략할 수 있습니다.

서브모듈을 업데이트한 후에는 포크 저장소의 새 hash를 가리키도록 ClickHouse의 서브모듈 참조를 업데이트하십시오.
