...
**13.** `[]` 연산자 사이를 구분하기 위해 공백을 사용하지 마십시오.
...
**20.** 문자열 리터럴에는 비ASCII 문자를 사용할 수 있습니다.
...
변수를 값으로 전달할 때는 `const`를 사용하는 것이 보통 의미가 없습니다.

**11.** unsigned.

필요한 경우 `unsigned`를 사용합니다.
...
함수가 힙에서 생성된 객체의 소유권을 가져오는 경우, 인수 타입을 `shared_ptr` 또는 `unique_ptr`로 지정합니다.
...
드문 경우이지만(루프에서 값을 갱신하는 경우) 인수를 통해 값을 반환해야 할 수 있습니다. 이때 인수는 참조 타입이어야 합니다.
...
초기화를 위해 생성자 인수가 필요하다면, 보통은 기본 생성자를 작성하지 말아야 합니다.
