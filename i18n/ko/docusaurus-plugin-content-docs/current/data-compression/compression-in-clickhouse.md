...
다음 쿼리를 사용하여 각 컬럼의 현재 압축된 크기 및 압축되지 않은 크기를 측정할 수 있습니다. 먼저 정렬 키가 없는 초기 스키마 `posts`의 크기를 살펴보겠습니다.
...
최적화된 타입과 정렬 키를 사용한 테이블 `posts_v3`에 대해 이 쿼리를 반복 실행하면, 압축되지 않은 크기와 압축된 크기가 상당히 줄어든 것을 확인할 수 있습니다.
