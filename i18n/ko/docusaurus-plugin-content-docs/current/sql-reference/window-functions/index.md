...
집계 함수로 수행하는 계산과 유사한 계산을 수행하는 데 사용할 수 있지만, 윈도우 함수는 행을 단일 출력으로 그룹화하지 않으므로 개별 행이 그대로 반환됩니다.

## 표준 윈도우 함수 \{#standard-window-functions\}

ClickHouse는 윈도우와 윈도우 함수에 대한 표준 SQL 문법을 지원합니다.
...
| `DateTime` `RANGE OFFSET` 프레임용 `INTERVAL` 구문                       | ❌     | 대신 초 단위 숫자를 지정하십시오(`RANGE`는 모든 숫자 유형에서 사용할 수 있습니다).                                                                                                                                                                                                                                                                      |
| `GROUPS` 프레임                                                       | ❌     |                                                                                                                                                                                                                                                                                                                          |
| 프레임에 대해 집계 함수를 계산 (`sum(value) OVER (ORDER BY time)`)              | ✅     | 모든 집계 함수가 지원됩니다.                                                                                                                                                                                                                                                                                                         |
| `rank()`, `dense_rank()`/`denseRank()`, `row_number()`             | ✅     |                                                                                                                                                                                                                                                                                                                          |
| `percent_rank()`/`percentRank()`                                   | ✅     | 파티션 내에서 값의 상대적 순위를 효율적으로 계산합니다. `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`처럼 더 장황하고 계산 비용이 큰 수동 SQL 계산을 대체합니다.                                                                                                                                           |
| `cume_dist()`                                                      | ✅     | 값 그룹 내에서 값의 누적 분포를 계산합니다. 현재 행의 값보다 작거나 같은 값을 가진 행의 비율을 반환합니다.                                                                                                                                                                                                                                                           |
| `lag/lead(value, offset)`                                          | ✅     | 다음 우회 방법 중 하나를 사용할 수도 있습니다:<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)` 또는 `lead`의 경우 `PRECEDING` 대신 `FOLLOWING` 사용 <br /> 2) `lagInFrame/leadInFrame`은 유사한 함수이지만 윈도우 프레임을 따릅니다. `lag/lead`와 동일한 동작을 얻으려면 `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`을 사용하십시오. |
| `ntile(buckets)`                                                   | ✅     | 예를 들어 윈도우는 `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`처럼 지정하십시오.                                                                                                                                                                                                               |
...
## 윈도우 함수로만 사용할 수 있는 함수 \{#functions\}

다음 함수는 윈도우 함수로만 사용할 수 있습니다. 대부분은 표준 SQL 함수이며, `lagInFrame`, `leadInFrame`, `nonNegativeDerivative`는 ClickHouse 확장 기능입니다.

| Function                                                                                                   | Description                                                                    |
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| [`row_number()`](./row_number.md)                                                                          | 현재 행에 파티션 내에서 1부터 시작하는 번호를 매깁니다.                                               |
| [`first_value(x)`](./first_value.md)                                                                       | 정렬된 프레임 내에서 평가된 첫 번째 값을 반환합니다.                                                 |
| [`last_value(x)`](./last_value.md)                                                                         | 정렬된 프레임 내에서 평가된 마지막 값을 반환합니다.                                                  |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | 정렬된 프레임의 n번째 행(offset)에 대해 평가된 첫 번째 NULL이 아닌 값을 반환합니다.                         |
| [`rank()`](./rank.md)                                                                                      | 현재 행의 파티션 내 순위를 반환하며, 순위 사이에 공백이 있을 수 있습니다.                                    |
| [`dense_rank()`](./dense_rank.md)                                                                          | 현재 행의 파티션 내 순위를 반환하며, 순위 사이에 공백이 없습니다.                                         |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | 정렬된 프레임 내에서 현재 행보다 지정된 물리적 오프셋만큼 앞에 있는 행에서 평가된 값을 반환합니다.                       |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | 정렬된 프레임 내에서 현재 행보다 지정된 오프셋만큼 뒤에 있는 행에서 평가된 값을 반환합니다.                           |
| [`nonNegativeDerivative(metric_column, timestamp_column[, INTERVAL X UNITS])`](./nonNegativeDerivative.md) | `timestamp_column`을 기준으로 `metric_column`의 음수가 아닌 미분값을 계산합니다. ClickHouse 전용입니다. |
...
### 이동/슬라이딩 평균(행 3개 기준) \{#moving--sliding-average-per-3-rows\}
...
### 이동 / 슬라이딩 평균(10일 기준) \{#moving--sliding-average-per-10-days\}

온도는 초 단위 precision으로 저장되지만, `Range`와 `ORDER BY toDate(ts)`를 사용하면 크기가 10인 frame을 만들고, `toDate(ts)`를 사용하므로 단위는 일입니다.
