...
:::note
선택적 수정자 `RESPECT NULLS`를 `first_value(column_name)` 뒤에 사용하면 `NULL` 인수가 건너뛰어지지 않도록 할 수 있습니다.
...
이 예제에서는 `last_value` 함수를 사용하여 프리미어리그 축구 선수들의 연봉에 대한 가상 데이터셋에서 가장 낮은 연봉을 받는 선수를 찾습니다.
