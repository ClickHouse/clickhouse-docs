---
description: 'lagInFrame 윈도우 함수에 대한 설명서'
...
ClickHouse 윈도우 함수 `lagInFrame`은 윈도 프레임 설정을 그대로 따릅니다.
...
* `x` — 컬럼 이름.
* `offset` — 적용할 오프셋 값. [(U)Int*](../data-types/int-uint.md). (선택 사항 - 기본값은 `1`).
* `default` — 계산된 행이 윈도우 프레임의 경계를 벗어날 때 반환할 값. (선택 사항 - 생략 시 컬럼 타입의 기본값).
...
이 예시는 특정 주식의 과거 데이터를 살펴보고, `lagInFrame` 함수를 사용하여 종가의 일간 증감과 변동률을 계산합니다.
