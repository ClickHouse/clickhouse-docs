---
description: '지정된 그리드 상의 시계열 데이터에 대해 PromQL과 유사한 idelta를 계산하는 집계 FUNCTION입니다.'
...
* `staleness` — 고려 대상 샘플의 최대 staleness(초 단위)를 지정합니다. staleness 윈도우는 좌측은 열린 구간, 우측은 닫힌 구간입니다. [`UInt32`](/sql-reference/data-types/int-uint)
...
지정된 그리드에 대한 idelta 값을 반환합니다. 반환되는 배열은 각 시간 그리드 지점마다 하나의 값을 포함합니다. 특정 그리드 지점에 대해 instant delta 값을 계산할 수 있을 만큼 윈도우 내에 샘플이 충분하지 않으면 값은 NULL입니다. [`Array(Nullable(Float64))`](/sql-reference/data-types/array) — 널 허용(Nullable) `Float64` 값의 배열입니다.
