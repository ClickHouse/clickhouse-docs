---
description: 'N개의 변수에 대한 표본 공분산 행렬을 반환합니다.'
...
* `x1[, x2, ...]` — 표본 공분산 행렬을 계산할 하나 이상의 매개변수입니다. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float) 또는 [`Decimal`](/sql-reference/data-types/decimal) 형식입니다.

**반환 값**

표본 공분산 행렬을 반환합니다. [`Array(Array(Float64))`](/sql-reference/data-types/array)
