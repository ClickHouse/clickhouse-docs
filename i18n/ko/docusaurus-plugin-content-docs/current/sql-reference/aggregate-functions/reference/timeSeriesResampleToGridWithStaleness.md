---
'description': '지정된 그리드로 시계열 데이터를 다시 샘플링하는 집계 함수.'
...
Example:
The following 쿼리 re-samples time series data to the grid [90, 105, 120, 135, 150, 165, 180, 195, 210] by choosing the value no older than 30 sec for each point on the grid:
...
Also it is possible to pass multiple samples of timestamps and values as Arrays of equal size. The same 쿼리 with array arguments:
