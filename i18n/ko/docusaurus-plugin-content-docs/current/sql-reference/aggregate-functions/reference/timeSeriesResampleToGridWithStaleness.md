---
description: '지정된 그리드에 맞춰 시계열 데이터를 재샘플링하는 집계 FUNCTION입니다.'
...
* `start_timestamp` — 그리드의 시작 지점을 지정합니다. [`UInt32`](/sql-reference/data-types/int-uint) 또는 [`DateTime`](/sql-reference/data-types/datetime)
* `end_timestamp` — 그리드의 끝 지점을 지정합니다. [`UInt32`](/sql-reference/data-types/int-uint) 또는 [`DateTime`](/sql-reference/data-types/datetime)
* `grid_step` — 그리드 간격(초 단위)을 지정합니다. [`UInt32`](/sql-reference/data-types/int-uint)
* `staleness_window` — 가장 최근 샘플의 최대 허용 지연 시간(초 단위)을 지정합니다. [`UInt32`](/sql-reference/data-types/int-uint)
...
지정된 그리드에 맞춰 재샘플링된 시계열 값을 반환합니다. 반환되는 배열에는 각 시간 그리드 시점마다 하나의 값이 포함됩니다. 특정 그리드 시점에 대한 샘플이 없으면 해당 값은 NULL입니다. [`Array(Nullable(Float64))`](/sql-reference/data-types/array)
