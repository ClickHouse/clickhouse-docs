---
description: '결과는 varSamp의 제곱근과 동일합니다. 이 함수는 수치적으로 안정적인 알고리즘을 사용합니다.'
...
* `x` — 표본 분산의 제곱근을 계산할 값. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float) 또는 [`Decimal*`](/sql-reference/data-types/decimal)

**반환 값**

`x`의 표본 분산의 제곱근을 반환합니다. [`Float64`](/sql-reference/data-types/float)
