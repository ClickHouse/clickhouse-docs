...
> • 다각형 인수가 상수이고 점이 인덱싱된 키 컬럼을 사용해 표현된 경우(예: `x, y`가 `PRIMARY KEY`의 일부이거나 `minmax` 인덱스로 커버되는 테이블에서 `pointInPolygon((x, y), constant_polygon)`), ClickHouse는 기본 키와 `minmax` 데이터 스키핑 인덱스를 모두 사용해 관련 없는 그래뉼을 제외할 수 있습니다.
