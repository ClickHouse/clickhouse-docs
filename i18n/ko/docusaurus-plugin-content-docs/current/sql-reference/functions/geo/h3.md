...
H3의 실험적 포함 모드를 사용하여, 제공된 geometry가 ring 또는 (multi-)polygon일 때 이를 덮는 지정된 해상도의 육각형을 반환합니다.
...
* `geometry`에는 다음 [Geo 데이터 타입](../../data-types/geo.md) 또는 그 기반 primitive 타입 중 하나를 사용할 수 있습니다:
  * [Ring](../../data-types/geo.md#ring)
  * [Polygon](../../data-types/geo.md#polygon)
  * [MultiPolygon](../../data-types/geo.md#multipolygon)
* `resolution` — 인덱스 해상도입니다. 범위: `[0, 15]`. [UInt8](../../data-types/int-uint.md).
* `flags` — 포함 모드입니다. 범위: `[0, 3]`. `0`, `1`, `2`, `3`과 같은 정수 리터럴 또는 [UInt32](../../data-types/int-uint.md) 값(예: `toUInt32(2)`)을 사용하십시오. 값은 H3 API에 `UInt32`로 전달됩니다. 다른 네이티브 정수 타입은 정확한 형 변환을 통해 변환됩니다.

**반환 값**

* 선택한 포함 모드에 따라 H3 인덱스로 이루어진 배열이 반환됩니다. [Array](../../data-types/array.md)([UInt64](../../data-types/int-uint.md)).
