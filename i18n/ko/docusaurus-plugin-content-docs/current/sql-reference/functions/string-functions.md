...
UTF-8 문자열에 Unicode 케이스 폴딩을 적용하여, 대소문자를 구분하지 않는 비교에 적합한 소문자와 유사한 정규화 형태로 변환합니다.

표준 Unicode 케이스 폴딩을 적용합니다. 케이스 폴딩의 영향을 받지 않는 호환 문자는 유지됩니다
(예: 로마 숫자, 동그라미 숫자). 다만 `ﬃ`와 같은 일부 합자는 Unicode 케이스 폴딩 자체에서 확장되므로 여전히 분해됩니다.
...
케이스 폴딩이 적용된 UTF-8 문자열입니다. [`String`](/sql-reference/data-types/string)
...
* `s` — 자연 정렬 키로 변환할 문자열입니다. [`String`](/sql-reference/data-types/string)

**반환 값**

`s`로부터 생성된 자연 정렬 키 문자열을 반환합니다. [`String`](/sql-reference/data-types/string)
...
UTF-8 문자열을 [NFKC&#95;Casefold 정규화 형식](https://unicode.org/reports/tr44/#NFKC_Casefold)에 따라 정규화합니다. 이 형식은 NFKC 정규화를 적용한 후 대소문자 접기를 수행합니다.
이는 식별자의 대소문자를 구분하지 않는 비교에 유용합니다.
...
UTF-8 문자열의 NFKC&#95;Casefold 정규화된 형태를 반환합니다. [`String`](/sql-reference/data-types/string)
...
* `index` — 선택 사항입니다. 어떤 정규 표현식 그룹을 추출할지 나타내는 0 이상의 정수입니다. 기본값은 `pattern`에 캡처 그룹이 하나 이상 포함된 경우 `1`이고, 캡처 그룹이 없는 경우 `0`(전체 일치)입니다. [`(U)Int*`](/sql-reference/data-types/int-uint)
...
NFD를 통해 문자를 분해하고 결합 마크(Unicode category Mn)를 제거한 다음, NFC를 통해 다시 조합하여 UTF-8 문자열에서 발음 구별 부호(악센트)를 제거합니다.
...
분음 부호가 제거된 UTF-8 문자열. [`String`](/sql-reference/data-types/string)
