...
결과 타입은 인수들 중 최대 비트 수와 동일한 비트를 가진 정수입니다. 인수 중 하나라도 부호 있는 타입이면 결과는 부호 있는 숫자가 됩니다. 인수가 부동 소수점 숫자인 경우 `Int64`로 형변환됩니다.
...
`x`에서 값이 1인 비트의 개수를 반환합니다. [`UInt8`](../data-types/int-uint.md).

:::note
이 함수는 입력 값을 더 큰 타입으로 변환하지 않습니다([부호 확장(sign extension)](https://en.wikipedia.org/wiki/Sign_extension)).
...
두 수의 비트 표현 사이의 [해밍 거리](https://en.wikipedia.org/wiki/Hamming_distance)를 반환합니다.
...
* `x` — 해밍 거리(Hamming distance) 계산을 위한 첫 번째 수. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float)
* `y` — 해밍 거리(Hamming distance) 계산을 위한 두 번째 수. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float)

**반환 값**

`x`와 `y`의 해밍 거리를 [`UInt8`](/sql-reference/data-types/int-uint) 값으로 반환합니다.
...
* `a` — 비트 NOT 연산을 적용할 값입니다. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float) 또는 [`String`](/sql-reference/data-types/string)

**반환 값**

`~a`, 즉 비트가 반전된 `a`의 결과를 반환합니다.
...
* `N` — 왼쪽으로 회전할 비트 위치 수입니다. [`UInt8/16/32/64`](/sql-reference/data-types/int-uint)
...
* `a` — 회전할 값입니다. [`(U)Int8/16/32/64`](/sql-reference/data-types/int-uint)
* `N` — 오른쪽으로 회전할 비트 수입니다. [`UInt8/16/32/64`](/sql-reference/data-types/int-uint)
...
* `N` — 시프트할 위치의 개수입니다. [`UInt8/16/32/64`](/sql-reference/data-types/int-uint)
...
* `a` — 시프트할 값입니다. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`String`](/sql-reference/data-types/string) 또는 [`FixedString`](/sql-reference/data-types/fixedstring)
* `N` — 시프트할 자리 수입니다. [`UInt8/16/32/64`](/sql-reference/data-types/int-uint)
...
* `s` — 슬라이스할 String 또는 FixedString입니다. [`String`](/sql-reference/data-types/string) 또는 [`FixedString`](/sql-reference/data-types/fixedstring)
* `offset` —
...
* `length`가 8의 배수가 아니면, 결과는 오른쪽이 0으로 패딩됩니다.
  [`(U)Int8/16/32/64`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float)

**반환 값**

추출된 비트를 이진 시퀀스로 표현한 문자열을 반환합니다. 결과는 항상 바이트 경계(8비트의 배수)에 맞추어 패딩됩니다. [`String`](/sql-reference/data-types/string)
...
임의의 숫자를 [2진수 형태](https://en.wikipedia.org/wiki/Binary_number)로 변환한 다음, 지정된 위치에 있는 비트의 값을 반환합니다. 비트 위치는 오른쪽에서 왼쪽으로 0부터 셉니다.
...
`a`의 이진 표현에서 위치 `i`에 있는 비트 값을 반환합니다. [`UInt8`](/sql-reference/data-types/int-uint)
...
지정된 위치의 모든 비트에 대해 [논리곱](https://en.wikipedia.org/wiki/Logical_conjunction) (AND 연산자) 결과를 반환합니다.
...
* `a` — 정수 값입니다. [`(U)Int8/16/32/64`](/sql-reference/data-types/int-uint)
* `index1, ...` — 하나 이상의 비트 위치입니다. [`(U)Int8/16/32/64`](/sql-reference/data-types/int-uint)

**반환 값**

논리곱 연산 결과를 반환합니다. [`UInt8`](/sql-reference/data-types/int-uint)
...
숫자에서 지정된 위치의 모든 비트에 대한 [논리합](https://en.wikipedia.org/wiki/Logical_disjunction) (OR 연산자) 결과를 반환합니다.
...
논리 합의 결과를 반환합니다. [`UInt8`](/sql-reference/data-types/int-uint)
...
* `a` — 첫 번째 값입니다. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float)
* `b` — 두 번째 값입니다. [`(U)Int*`](/sql-reference/data-types/int-uint) 또는 [`Float*`](/sql-reference/data-types/float)
