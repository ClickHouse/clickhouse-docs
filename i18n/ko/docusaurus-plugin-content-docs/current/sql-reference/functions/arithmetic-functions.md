...
Float32 또는 Float64, 또는 BFloat16 타입 인자가 무한대가 아니고 `NaN`도 아니면 `1`을 반환하고,
그 외의 경우 `0`을 반환합니다.
...
* `x` — 유한한 값인지 확인할 숫자. [`Float*`](/sql-reference/data-types/float) 또는 [`BFloat16`](/sql-reference/data-types/float)
...
Float32 또는 Float64, 또는 BFloat16 형식의 인자가 무한대일 경우 `1`을 반환하고, 그렇지 않은 경우 `0`을 반환합니다.
...
* `x` — 무한대인지 확인할 숫자. [`Float*`](/sql-reference/data-types/float) 또는 [`BFloat16`](/sql-reference/data-types/float)
...
Float32, Float64 또는 BFloat16 형 인자가 `NaN`이면 `1`을 반환하고, 그렇지 않으면 `0`을 반환합니다.
...
* `x` — `NaN`인지 평가할 인수. [`Float*`](/sql-reference/data-types/float) 또는 [`BFloat16`](/sql-reference/data-types/float)
