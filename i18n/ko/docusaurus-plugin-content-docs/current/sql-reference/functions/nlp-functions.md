...
:::warning
이 함수는 Experimental 기능이며, 향후 릴리스에서 예기치 않게 이전 버전과 호환되지 않는 방식으로 변경될 수 있습니다.
유효에 하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
...
:::warning
이 FUNCTION은 실험적 기능이며, 향후 릴리스에서 예측할 수 없고 하위 호환되지 않는 방식으로 변경될 수 있습니다.
이를 사용하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
...
:::warning
이 함수는 실험적 기능이며, 향후 릴리스에서 이전 버전과 호환되지 않는 예측 불가능한 방식으로 변경될 수 있습니다.
...
:::warning
이 함수는 Experimental 상태이며, 향후 릴리스에서 이전 버전과 호환되지 않는 예측 불가능한 방식으로 변경될 수 있습니다.
이를 유효하게 하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
...
:::warning
이 FUNCTION은 실험적 기능이며, 향후 릴리스에서 예측할 수 없는 하위 호환되지 않는 방식으로 변경될 수 있습니다.
유효에 하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
...
이 함수가 작동하려면 딕셔너리가 필요하며, [GitHub](https://github.com/vpodpecan/lemmagen3/tree/master/src/lemmagen3/models)에서 얻을 수 있습니다.
로컬 파일에서 딕셔너리를 로드하는 방법에 대한 자세한 내용은 [「Defining Dictionaries」 페이지](/sql-reference/statements/create/dictionary/sources/local-file)를 참조하십시오.

:::warning
이 함수는 실험적 기능이며, 향후 릴리스에서 이전 버전과 호환되지 않는 예기치 않은 방식으로 변경될 수 있습니다.
사용하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
...
:::warning
이 함수는 실험적 기능이며, 향후 릴리스에서 예측할 수 없고 이전 버전과 호환되지 않는 방식으로 변경될 수 있습니다.
활성화하려면 `allow_experimental_nlp_functions = 1`로 설정하십시오.
:::
