...
:::note
이 기능은 ClickHouse Cloud에서 비공개 프리뷰로 지원됩니다.
...
실행형 사용자 정의 함수(UDF)의 구성은 하나 이상의 xml 파일에 위치할 수 있습니다.
...
| `stderr_reaction`             | 명령의 stderr 출력을 처리하는 방법입니다. 값: `none`(무시), `log`(모든 stderr를 즉시 로그에 기록), `log_first`(종료 후 처음 4 KiB를 로그에 기록), `log_last`(종료 후 마지막 4 KiB를 로그에 기록), `throw`(stderr 출력이 있으면 즉시 예외를 발생시킴)입니다. `log_first` 또는 `log_last`를 0이 아닌 종료 코드와 함께 사용하는 경우 stderr 내용이 예외 메시지에 포함됩니다                                               | Optional | `log_last`                |
| `check_exit_code`             | true인 경우, ClickHouse는 명령의 종료 코드를 검사합니다. 0이 아닌 종료 코드는 예외를 발생시킵니다                                                                                                                                                                                                                                                  | Optional | `true`                    |
