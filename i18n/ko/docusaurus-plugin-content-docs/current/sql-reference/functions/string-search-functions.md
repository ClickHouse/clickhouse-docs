...
**토큰화 없이 그대로(AS-IS) 검색할 needle을 배열로 지정합니다**
...
[`like`](#like)와 유사하지만, 대소문자를 구분하지 않고 검색합니다. 선택적 `ESCAPE` 절도 지원합니다(`like` 참조).
...
* `escape_character` — `\` 대신 이스케이프 문자로 사용할 수 있는 선택적 단일 문자 문자열입니다. 기본값: `\`. [`String`](/sql-reference/data-types/string)
...
## ESCAPE 절 \{#escape-clause\}

선택적인 `ESCAPE` 절은 사용자 지정 이스케이프 문자를 지정합니다(반드시 ASCII 문자 1개여야 합니다).
지정하면 사용자 지정 이스케이프 문자가 `%` 및 `_` 메타 문자를 이스케이프할 때 기본 백슬래시를 대체합니다.
이스케이프 문자는 `%`(리터럴 퍼센트), `_`(리터럴 밑줄), 자기 자신(리터럴 이스케이프 문자)의 세 가지를 이스케이프할 수 있습니다.
사용자 지정 이스케이프 문자를 사용하면 백슬래시는 더 이상 특별한 의미를 갖지 않으며 리터럴 문자로 처리됩니다.
...
* `escape_character` — `\` 대신 사용할 이스케이프 문자로 지정하는 선택적 단일 문자 문자열입니다. 기본값: `\`. [`String`](/sql-reference/data-types/string)
...
[`position`](#position)과 비슷하지만, `haystack` 문자열에서 여러 `needle` 부분 문자열의 위치(바이트 단위, 1부터 시작)를 배열으로 반환합니다.
...
대소문자를 구분하지 않고 문자열이 패턴과 일치하지 않는지 확인합니다. 패턴에는 SQL LIKE 매칭을 위한 특수 문자 `%` 및 `_`를 포함할 수 있습니다. 선택적 `ESCAPE` 절도 지원합니다(`like` 참조).
...
[`like`](#like)와 유사하지만 결과를 부정합니다. 선택적 `ESCAPE` 절도 지원합니다(`like` 참조).
...
* `haystack` — 검색을 수행할 문자열입니다. [`String`](/sql-reference/data-types/string) 또는 [`FixedString`](/sql-reference/data-types/fixedstring)
* `pattern` — 비교에 사용할 LIKE 패턴입니다. [`String`](/sql-reference/data-types/string)
* `escape_character` — `\` 대신 이스케이프 문자로 사용할 선택적 단일 문자 문자열입니다. 기본값: `\`. [`String`](/sql-reference/data-types/string)
