...
Limit the maximum number of 쿼리 for the current user with 123 쿼리 in 15 months constraint:

```sql
CREATE QUOTA qA FOR INTERVAL 15 month MAX queries = 123 TO CURRENT_USER;
```

For the default user limit the maximum execution time with half a second in 30 minutes, and limit the maximum number of 쿼리 with 321 and the maximum number of errors with 10 in 5 quarters:
