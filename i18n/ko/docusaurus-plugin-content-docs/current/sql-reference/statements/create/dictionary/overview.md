...
| [`SOURCE`](./sources/)                      | 딕셔너리의 데이터 소스를 정의합니다(예: ClickHouse 테이블, HTTP, PostgreSQL).            |
| [`LAYOUT`](./layouts/)                      | 딕셔너리가 메모리에 저장되는 방식을 제어합니다(예: `FLAT`, `HASHED`, `CACHE`).             |
...
- [Layouts](./layouts/) — 딕셔너리가 메모리에 저장되는 방식
- [Sources](./sources/) — 데이터 소스에 연결하기
