...
캐시 딕셔너리의 경우, 캐시 내 데이터의 만료 [lifetime](../lifetime.md)을 설정할 수 있습니다. 셀에 데이터를 로드한 이후 `lifetime`보다 더 많은 시간이 경과하면 해당 셀의 값은 사용되지 않고 키는 만료된 것으로 간주됩니다. 이 키는 다음에 다시 사용해야 할 때 재요청됩니다. 이 동작은 `allow_read_expired_keys` 설정으로 구성할 수 있습니다.
