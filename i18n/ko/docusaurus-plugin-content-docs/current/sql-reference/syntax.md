...
:::tip[컬럼 이름에 점(.) 사용을 피하십시오]
컬럼 이름에 점이 포함되어 있거나, 공통된 점(.) 접두사를 공유하는 컬럼, 그리고 `Array` 타입 컬럼은 `flatten_nested = 1`(기본값)일 때 평탄화된 `Nested` 구조의 일부로 해석될 수 있습니다. 이로 인해 INSERT 시 예기치 않은 배열 길이 검증이 발생하거나 이름 변경에 제약이 생길 수 있습니다.

가능하면 컬럼 이름에 점을 사용하지 마십시오.
`Nested` 의미가 명시적으로 필요한 경우가 아니라면, 컬럼 이름에서 점 대신 밑줄(`_`)이나 다른 구분 기호를 사용하십시오.
:::
