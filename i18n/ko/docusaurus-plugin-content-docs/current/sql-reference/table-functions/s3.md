...
| `partition_strategy`                    | 선택적 매개변수입니다. 지원되는 값: `wildcard` 또는 `hive`. `wildcard`는 경로에 `{_partition_id}`가 포함되어 있어야 하며, 이는 파티션 키로 대체됩니다. `hive`는 와일드카드를 허용하지 않고, 경로를 테이블 루트로 가정하며, 파일 이름은 Snowflake ID, 확장자는 파일 포맷인 Hive 스타일의 파티션 디렉터리를 생성합니다. 기본값은 `file_like_engine_default_partition_strategy` 설정에 따릅니다(`26.6`보다 오래된 `compatibility` 설정에서는 `wildcard`, 그 외에는 `hive`). |
...
| `extra_credentials`                     | 선택적 매개변수입니다. ClickHouse Cloud에서 역할 기반 접근을 위해 `role_arn`을 전달할 때 사용됩니다. 구성 단계는 [Secure S3](/cloud/data-sources/secure-s3)를 참고하십시오.                                                                                                                                                                                                           |
...
`wildcard`: 파일 경로의 `{_partition_id}` 와일드카드를 실제 파티션 키로 대체합니다. `26.6`보다 이전의 `compatibility` 설정에서만 기본값으로 선택되며, 그 외의 경우 기본값은 `hive`입니다(`file_like_engine_default_partition_strategy` 설정 참고).
...
`use_hive_partitioning`을 1로 설정하면 ClickHouse는 경로(`/name=value/`)에서 Hive 스타일 파티셔닝을 감지하고, 쿼리에서 파티션 컬럼을 가상 컬럼으로 사용할 수 있도록 허용합니다. 이 가상 컬럼들은 파티션 경로에 있는 컬럼과 동일한 이름을 가집니다.
