...
지정된 클러스터의 여러 노드와 병렬로 Amazon S3에 있는 Apache [Hudi](https://hudi.apache.org/) 테이블의 파일을 처리할 수 있습니다. 이니시에이터에서는 클러스터의 모든 노드에 대한 연결을 만들고 각 파일을 동적으로 분배합니다. 워커 노드에서는 이니시에이터에 다음으로 처리할 작업을 요청하고 이를 처리합니다. 모든 작업이 완료될 때까지 이 과정이 반복됩니다.
...
| `extra_credentials`                          | 선택적인 매개변수입니다. ClickHouse Cloud에서 역할 기반 액세스를 위해 `role_arn`을 전달하는 데 사용됩니다. 구성 단계는 [Secure S3](/cloud/data-sources/secure-s3)를 참조하십시오.                                                                                                                                           |
