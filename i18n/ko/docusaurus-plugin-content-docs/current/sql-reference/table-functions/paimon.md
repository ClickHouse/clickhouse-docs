...
Amazon S3, Azure, HDFS 또는 로컬에 저장된 Apache [Paimon](https://paimon.apache.org/) 테이블에 대해 읽기 전용 테이블과 유사한 인터페이스를 제공합니다.
...
인수에 대한 설명은 각각 테이블 함수 `s3`, `azureBlobStorage`, `HDFS`, `file`의 인수 설명과 일치합니다.
`format`은 Paimon 테이블의 데이터 파일 형식을 나타냅니다.

`paimonS3`의 경우, ClickHouse Cloud에서 역할 기반 액세스를 위한 `role_arn`을 전달하는 데 선택적 `extra_credentials` 매개변수를 사용할 수 있습니다. 구성 단계는 [Secure S3](/cloud/data-sources/secure-s3)를 참조하세요.
