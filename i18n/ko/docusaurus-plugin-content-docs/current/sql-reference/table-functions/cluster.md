...
`cluster_name`에는 `{}`로 둘러싸인 매크로(치환)를 포함할 수 있습니다. 치환 값은 서버 구성 파일의 [macros](../../operations/server-configuration-parameters/settings.md#macros) 섹션에서 가져옵니다.
...
`cluster` 및 `clusterAllReplicas` 테이블 함수는 각 요청마다 서버 연결이 재설정되므로 `분산` 테이블을 생성하는 것보다 효율성이 떨어집니다. 대량의 쿼리를 처리하는 경우에는 항상 미리 `분산` 테이블을 생성하고, `cluster` 및 `clusterAllReplicas` 테이블 함수를 사용하지 않도록 합니다.
