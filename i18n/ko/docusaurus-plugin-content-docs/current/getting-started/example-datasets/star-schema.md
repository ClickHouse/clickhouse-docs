---
description: 'Star Schema Benchmark (SSB) 데이터 세트와 쿼리'
sidebar_label: 'Star Schema Benchmark'
slug: /getting-started/example-datasets/star-schema
title: 'Star Schema Benchmark (SSB, 2009)'
doc_type: 'guide'
keywords: ['예제 데이터 세트', 'star schema', '샘플 데이터', '데이터 모델링', '벤치마크']
---

Star Schema Benchmark는 [TPC-H](tpch.md)의 테이블과 쿼리를 대략적으로 기반으로 하지만, TPC-H와 달리 star schema 레이아웃을 사용합니다.
대부분의 데이터는 거대한 fact 테이블에 저장되며, 이 fact 테이블은 여러 개의 작은 dimension 테이블로 둘러싸여 있습니다.
쿼리는 fact 테이블을 하나 이상의 dimension 테이블과 조인하여 `MONTH = 'JANUARY'`와 같은 필터 조건을 적용합니다.
...
그런 다음 데이터를 생성합니다. 매개변수 `-s`는 스케일 팩터를 지정합니다. 예를 들어 `-s 100`인 경우 6억 행이 생성됩니다.
