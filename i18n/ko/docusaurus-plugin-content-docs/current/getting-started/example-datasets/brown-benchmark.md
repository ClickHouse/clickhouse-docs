---
description: '머신이 생성한 로그 데이터용 새로운 분석 벤치마크'
sidebar_label: 'Brown University 벤치마크'
...
`MgBench`는 머신이 생성한 로그 데이터를 위한 새로운 분석 벤치마크로, [Andrew Crotty](http://cs.brown.edu/people/acrotty/)가 제안했습니다.
...
## 벤치마크 쿼리 실행 \{#run-benchmark-queries\}
...
아래의 쿼리 3.5는 UNION을 사용합니다. SELECT 쿼리 결과를 결합하는 방식을 설정합니다. 이 설정은 UNION ALL 또는 UNION DISTINCT를 명시적으로 지정하지 않은 UNION과 함께 사용할 때만 적용됩니다.
...
데이터는 [Playground](https://sql.clickhouse.com)에서 대화형 쿼리 실행을 위해서도 사용할 수 있으며, [예제](https://sql.clickhouse.com?query_id=1MXMHASDLEQIP4P1D1STND)를 참고하십시오.
