...
### 히트(hits) 데이터 가져오기 \{#import-the-hits-data\}
...
### visits 데이터 가져오기 \{#import-the-visits-data\}
...
## JOIN 예시 \{#an-example-join\}

hits 및 visits 데이터 세트는 ClickHouse 테스트
루틴에서 사용되며, 이 쿼리는 해당 테스트 스위트에 포함된 쿼리 중 하나입니다. 나머지
테스트는 이 페이지 맨 아래에 있는 *Next Steps* 섹션에
나와 있습니다.
...
[A Practical Introduction to Sparse Primary Indexes in ClickHouse](/guides/best-practices/sparse-primary-indexes.md)에서는 hits 데이터셋을 사용하여 전통적인 관계형 데이터베이스와 비교했을 때 ClickHouse 인덱싱의 차이점, ClickHouse가 희소 기본 인덱스(sparse primary index)를 구성하고 사용하는 방식, 그리고 인덱싱 모범 사례를 설명합니다.

이 테이블을 대상으로 하는 추가 쿼리 예시는 ClickHouse [stateful tests](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql)에서 확인할 수 있습니다.

:::note
테스트 스위트는 데이터베이스 이름으로 `test`를 사용하고, 테이블 이름은 `hits`와 `visits`입니다. 데이터베이스와 테이블 이름을 변경하거나 테스트 파일의 SQL을 편집해도 됩니다.
:::