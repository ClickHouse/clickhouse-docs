...
<VerticalStepper headerLevel="h3">
  ...
  다음 명령을 실행하여 테이블을 생성하고 원격 CSV 파일에서 직접 데이터를 삽입하세요. [`url`](/sql-reference/table-functions/url) 함수를 통해 콘텐츠에 접근합니다.
  스키마는 자동으로 추론됩니다:
  ...
  [`DESCRIBE TABLE`](/sql-reference/statements/describe-table) 명령어를 사용하여 할당된 타입을 확인할 수 있습니다.
  ...
  이 테이블에 데이터를 삽입하려면 `INSERT INTO, SELECT` 명령어를 사용하세요.
  ...
  score 필드는 스토리의 인기도 메트릭을 제공하며, `id` 필드와 `||` 연결 연산자를 사용하여 원본 게시물로 연결되는 링크를 생성할 수 있습니다.
...
<VerticalStepper headerLevel="h3">
  ...
  인덱스의 머티리얼라이즈는 시간이 소요됩니다(인덱스가 생성되었는지 확인하려면 시스템 테이블 `system.data_skipping_indices`를 사용하세요).
