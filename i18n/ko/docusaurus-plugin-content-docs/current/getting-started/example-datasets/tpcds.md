...
먼저 TPC-DS 저장소를 체크아웃한 다음 데이터 생성기를 컴파일합니다:
...
TPC-DS 사양의 규칙을 가능한 한 엄격하게 따릅니다.
사양에 정의된 추상 데이터 타입은 ClickHouse의 고유 데이터 타입으로 다음과 같이 매핑됩니다.

| TPC-DS Type    | ClickHouse Type                                            |
| -------------- | ---------------------------------------------------------- |
| `identifier`   | `Int64`이며, 단 `*_date_sk` 및 `*_time_sk` 컬럼은 `UInt32`를 사용합니다 |
| `integer`      | `Int64`                                                    |
...
::::note
다음 설정을 활성화해야 SQL 표준에 부합하는 올바른 결과를 얻을 수 있습니다:
...
::::note
단일 값에 대해 `stddev_samp`를 호출하면 이 쿼리는 `NULL` 대신 `nan`을 반환합니다. 관련 이슈: https://github.com/ClickHouse/ClickHouse/issues/94683. 이 점을 제외하면 결과는 올바릅니다.
::::
...
::::note
해당 쿼리는 https://github.com/ClickHouse/ClickHouse/issues/94858에 보고된 문제로 인해 기본 설정만으로는 바로 작동하지 않습니다. 아래와 같이 일부를 수정한 대체 쿼리는 정상적으로 작동합니다:
...
::::note
https://github.com/ClickHouse/ClickHouse/issues/94858 이슈 때문에 이 쿼리는 기본 설정만으로는 바로 동작하지 않습니다. 아래와 같이 소폭 수정한 대체 쿼리는 정상적으로 동작합니다:
...
::::note
https://github.com/ClickHouse/ClickHouse/issues/94976 이슈 때문에 쿼리가 그대로는 동작하지 않습니다. 다음과 같이 약간 수정한 대안 쿼리는 정상적으로 동작합니다:
...
::::note
해당 쿼리는 https://github.com/ClickHouse/ClickHouse/issues/94671 때문에 기본 설정만으로는 동작하지 않습니다. 다음과 같이 소폭 수정한 대체 쿼리는 정상적으로 동작합니다:
