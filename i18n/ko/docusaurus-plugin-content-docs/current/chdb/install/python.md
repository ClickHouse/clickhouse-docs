---
title: 'Python용 chDB 설치하기'
sidebar_label: 'Python'
slug: /chdb/install/python
description: 'Python용 chDB를 설치하는 방법'
...
- Python 3.8+ 
- 지원되는 플랫폼: macOS 및 Linux (x86_64 및 ARM64)
...
### 기본적인 Python 사용 \{#basic-python-usage\}
...
## 데이터 입력 방식 \{#data-input\}

### 파일 기반 데이터 소스 \{#file-based-data-sources\}

chDB는 파일을 직접 쿼리할 수 있도록 70개 이상의 데이터 형식을 지원합니다:
...
### 출력 형식 예시 \{#output-format-examples\}
...
### DataFrame 작업 \{#dataframe-operations\}

#### 레거시 DataFrame API \{#legacy-dataframe-api\}
...
#### Python 테이블 엔진(권장) \{#python-table-engine-recommended\}
...
### 상태 저장 세션(Stateful sessions) \{#stateful-sessions\}

세션은 여러 작업에 걸쳐 쿼리 상태를 유지하여 복잡한 워크플로우를 구현할 수 있게 합니다.
...
### 세션의 고급 기능 \{#advanced-session-features\}
...
기존 Python 애플리케이션과의 호환성을 제공하는 표준 데이터베이스 인터페이스입니다.
...
사용자 정의 Python 함수로 SQL 기능을 확장합니다:
...
#### 커스텀 반환 타입을 사용하는 고급 UDF \{#advanced-udf-custom-return-types\}
...
1. **상태 없는 함수(Stateless Functions)**: UDF는 부작용이 없는 순수 함수여야 합니다.
2. **함수 내부에서 모듈 import**: 필요한 모든 모듈은 UDF 내부에서 `import`해야 합니다.
3. **문자열 입력/출력**: 모든 UDF 매개변수는 문자열입니다(TabSeparated 형식).
4. **에러 처리**: 견고한 UDF를 위해 `try-catch` 블록을 포함해야 합니다.
5. **성능**: UDF는 각 행에 대해 호출되므로 성능을 최적화해야 합니다.
...
메모리 사용량을 일정하게 유지하면서 대규모 데이터셋을 처리합니다:
...
### Python 테이블 엔진 \{#python-table-engine\}

#### Pandas DataFrame을 쿼리하기 \{#query-pandas-dataframes\}
...
#### PyReader를 사용한 사용자 정의 데이터 소스 \{#custom-data-sources-pyreader\}

특정 데이터 소스를 위한 사용자 정의 데이터 리더를 구현합니다:
...
chDB는 다른 임베디드 엔진보다 일관되게 더 높은 성능을 발휘합니다.

- **DataFrame 연산**: 분석 쿼리에서 기존 DataFrame 라이브러리보다 2~5배 더 빠릅니다.
- **Parquet 처리**: 선도적인 열 지향(columnar) 엔진과 견줄 만한 성능을 제공합니다.
- **메모리 효율성**: 대안 대비 메모리 사용량이 더 적습니다.
...
- **이슈 및 지원 요청**: [GitHub 리포지토리](https://github.com/chdb-io/chdb/issues)에서 이슈를 등록하십시오