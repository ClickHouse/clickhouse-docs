...
chDB-node는 두 가지 쿼리 모드를 지원합니다. 단순 작업을 위한 독립 실행형 쿼리와 데이터베이스 상태 유지를 위한 세션 기반 쿼리입니다.

### 독립 실행형 쿼리 \{#standalone-queries\}

상태 유지가 필요하지 않은 간단한 일회성 쿼리의 경우:
...
### 세션 기반 쿼리 \{#session-based-queries\}
...
### 외부 데이터 처리 \{#processing-external-data\}
