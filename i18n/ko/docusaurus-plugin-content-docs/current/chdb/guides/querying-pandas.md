...
[Pandas](https://pandas.pydata.org/)는 Python에서 데이터 처리와 분석을 위해 널리 사용되는 라이브러리입니다.
chDB 버전 2에서는 Pandas DataFrame을 쿼리하는 성능을 개선하고 `Python` 테이블 함수(table function)를 도입했습니다.
이 가이드에서는 `Python` 테이블 함수를 사용하여 Pandas를 쿼리하는 방법을 살펴봅니다.
...
이제 Pandas와 몇 가지 다른 라이브러리를 설치하겠습니다:

```bash
pip install pandas requests ipython
```

이 가이드의 나머지 부분에서 실행할 명령은 `ipython`을 사용합니다. 다음 명령을 실행하여 `ipython`을 시작하십시오:

```bash
ipython
```

Python 스크립트나 즐겨 사용하는 노트북(notebook) 환경에서도 이 코드를 사용할 수 있습니다.

## URL에서 Pandas DataFrame 생성하기 \{#creating-a-pandas-dataframe-from-a-url\}

[StatsBomb GitHub 저장소](https://github.com/statsbomb/open-data/tree/master?tab=readme-ov-file)에서 일부 데이터를 쿼리해 보겠습니다.
먼저 requests와 pandas를 임포트합니다:
...
이제 경기 JSON 파일 중 하나를 DataFrame으로 로드합니다.
...
다음으로, 이벤트 JSON 파일 중 하나를 로드하고, 해당 DataFrame에 `match_id`라는 컬럼을 또한 추가합니다:
...
다음으로 chDB를 사용하여 이러한 DataFrame에 쿼리를 수행하는 방법을 살펴보겠습니다.
라이브러리를 임포트합니다:
...
`Python` 테이블 함수(table function)를 사용해 Pandas DataFrame에 대해 쿼리를 수행할 수 있습니다.
...
따라서 `matches_df`의 컬럼을 나열하려면 다음과 같이 하면 됩니다:
...
다음 쿼리를 사용해 두 경기 이상을 주관한 심판이 누구인지 확인할 수 있습니다:
...
## Pandas DataFrame 조인하기 \{#joining-pandas-dataframes\}

쿼리에서 DataFrame을 조인할 수도 있습니다.
예를 들어, 경기에 대한 개요를 얻기 위해 다음과 같은 쿼리를 작성할 수 있습니다:
...
## DataFrame으로 테이블 채우기 \{#populating-a-table-from-a-dataframe\}

DataFrame을 사용하여 ClickHouse 테이블을 생성하고 데이터를 채울 수도 있습니다.
chDB에 테이블을 CREATE하려면 Stateful Session API를 사용해야 합니다.

이제 session 모듈을 import합니다:
...
그러면 패스를 가장 많이 받은 선수를 반환하는 쿼리를 실행할 수 있습니다.
...
## Pandas DataFrame와 테이블 조인하기 \{#joining-a-pandas-dataframe-and-table\}

마지막으로, 조인 쿼리를 업데이트하여 `matches_df` DataFrame을 `statsbomb.events` 테이블과 조인할 수도 있습니다:
