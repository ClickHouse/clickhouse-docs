---
description: '开始使用 Moose Stack —— 一种代码优先的方式，在 ClickHouse 之上构建类型安全的 schema，并支持本地开发'
sidebar_label: 'Moose OLAP（TypeScript / Python）'
sidebar_position: 25
slug: /interfaces/third-party/moose-olap
title: '使用 Moose OLAP 在 ClickHouse 上开发'
keywords: ['Moose']
doc_type: 'guide'
---
...
[Moose OLAP](https://docs.fiveonefour.com/moose/olap) 是 [Moose Stack](https://docs.fiveonefour.com/moose) 的核心模块。Moose Stack 是一个面向 TypeScript 和 Python 的开源开发工具包，用于构建实时分析后端。 

Moose OLAP 提供对开发者友好的抽象以及类似 ORM 的功能，并且为 ClickHouse 原生构建。
...
- **实时流式处理**：内置支持将 ClickHouse 与 Kafka 或 Redpanda 集成,实现流式数据摄取
- **REST API**：轻松基于 ClickHouse 表和视图生成完整的 REST API 文档
...
或者按照本指南在现有的 ClickHouse 或 ClickHouse Cloud 部署上快速启动并运行 Moose OLAP,仅需不到 5 分钟。

### 前置要求 {#prerequisites}

- **Node.js 20+** 或 **Python 3.12+** - TypeScript 或 Python 开发必需
- **Docker Desktop** - 用于本地开发环境
- **macOS/Linux** - Windows 通过 WSL2 支持
...
还没有运行 ClickHouse?使用 ClickHouse Playground 试用 Moose OLAP!
...
启动开发服务器以启动本地 ClickHouse 实例，所有生产表将根据您的代码定义自动重建：

```bash
moose dev
````

**重要提示**：您的生产 ClickHouse 不会受到影响。这将创建一个本地开发环境。

### 为本地数据库填充数据 {#step-6-seed-database}
...
现在您已经在代码中定义了表，您可以获得与 Web 应用程序中 ORM 数据模型相同的优势——在分析数据之上构建 API 和物化视图时具有类型安全和自动完成功能。接下来，您可以尝试：
...
- **参考应用程序**:查看开源参考应用程序 [Area Code](https://github.com/514-labs/area-code):这是一个入门代码库,包含构建功能丰富、企业级应用程序所需的所有必要构建模块,适用于需要专用基础设施的场景。其中包含两个示例应用程序:面向用户的分析应用和运营数据仓库。
...
- **观看教程**:在 [Youtube](https://www.youtube.com/channel/UCmIj6NoAAP7kOSNYk77u4Zw) 上观看视频教程、演示以及 Moose Stack 功能深度解析
