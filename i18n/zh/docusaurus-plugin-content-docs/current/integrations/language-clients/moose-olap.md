---
description: '开始使用 Moose Stack——一种代码优先的方法，在 ClickHouse 之上基于类型安全的 schema 和本地开发进行构建'
...
[Moose OLAP](https://docs.fiveonefour.com/moose/olap) 是 [Moose Stack](https://docs.fiveonefour.com/moose) 的核心模块。Moose Stack 是一个开源开发工具包，用于在 Typescript 和 Python 中构建实时分析后端。 

Moose OLAP 为 ClickHouse 原生打造，为开发者提供友好的抽象层和类似 ORM 的功能。
...
或者按照本指南在现有的 ClickHouse 或 ClickHouse Cloud 部署上快速启动 Moose OLAP,只需不到 5 分钟。
...
还没有部署 ClickHouse?可以使用 ClickHouse Playground 来试用 Moose OLAP!
...
**重要提示**：您的生产环境 ClickHouse 不会受到任何影响。这将创建一个本地开发环境。
...
现在您已经在代码中定义了表，您可以获得与 Web 应用程序中 ORM 数据模型相同的优势——在分析数据之上构建 API 和物化视图时具有类型安全和自动补全功能。接下来，您可以尝试：
...
- **参考应用**:查看开源参考应用 [Area Code](https://github.com/514-labs/area-code):这是一个入门代码仓库,包含构建功能丰富、企业级应用所需的全部基础模块,适用于需要专用基础设施的场景。其中包含两个示例应用:面向用户的分析应用和运营数据仓库。
...
- **参与贡献**:在 [GitHub](https://github.com/514-labs/moose) 上查看代码、为 Moose Stack 贡献代码并报告问题
