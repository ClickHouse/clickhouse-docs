...
description: '用于连接 ClickHouse 的官方 JS 客户端。'
title: 'ClickHouse JS'
doc_type: 'reference'
integration:
  - support_level: 'core'
...
该客户端使用 TypeScript 编写，并为客户端公共 API 提供类型定义。

它零依赖，针对极致性能进行了优化，并在多种 ClickHouse 版本和配置（本地部署单节点、本地部署集群以及 ClickHouse Cloud）下进行了测试。

针对不同的运行环境，提供了两个版本的客户端：
...
客户端源代码托管在 [ClickHouse-JS GitHub 仓库](https://github.com/ClickHouse/clickhouse-js) 中。



## 环境要求（Node.js）{#environment-requirements-nodejs}

运行客户端需要在环境中提供 Node.js。
该客户端与所有[维护中](https://github.com/nodejs/release#readme)的 Node.js 版本兼容。

一旦某个 Node.js 版本接近生命周期终止（End-Of-Life），客户端将停止对其提供支持,因为该版本被视为过时且不安全。
...
该客户端可能也能与更早的版本配合使用;但这仅为尽力支持,不提供保证。如果您使用的 ClickHouse 版本早于 23.3,请参阅 [ClickHouse 安全策略](https://github.com/ClickHouse/ClickHouse/blob/master/SECURITY.md) 并考虑升级。


## 示例 {#examples}

我们致力于通过客户端代码库中的[示例](https://github.com/ClickHouse/clickhouse-js/blob/main/examples)来涵盖客户端使用的各种场景。
...
客户端实例可以在实例化时进行[预配置](./js.md#configuration)。
...
| **pathname**?: string                                                    | 客户端解析 ClickHouse URL 后要添加的可选路径名。 | `''`                    | [带路径名的代理文档](./js.md#proxy-with-a-pathname)                                |
...
| **session_id**?: string                                                  | 随每个请求发送的可选 ClickHouse 会话 ID。                          | -                       | -                                                                                          |
| **keep_alive**?: `{ **enabled**?: boolean }`                             | 在 Node.js 和 Web 版本中默认启用。                                | -                       | -                                                                                          |
| **http_headers**?: `Record<string, string>`                              | 用于传出 ClickHouse 请求的附加 HTTP 标头。                           | -                       | [带身份验证的反向代理文档](./js.md#reverse-proxy-with-authentication)        |
...
| **set_basic_auth_header**?: boolean <br/><ExperimentalBadge/>              | 使用基本身份验证凭据设置 `Authorization` 标头。 | `true`        | [HTTP 代理文档中此设置的用法](./js.md#custom-httphttps-agent-experimental-nodejs-only) |

### URL 配置 {#url-configuration}

:::important
URL 配置将_始终_覆盖硬编码的值,在这种情况下将记录警告。
:::
...
- (2) 任何以 `clickhouse_setting_` 或 `ch_` 为前缀的参数都将删除此前缀,并将其余部分添加到客户端的 `clickhouse_settings` 中。例如,`?ch_async_insert=1&ch_wait_for_async_insert=1` 将等同于:
...
注意:`clickhouse_settings` 的布尔值应在 URL 中以 `1`/`0` 的形式传递。
...
以下示例演示如何设置与 ClickHouse Cloud 的连接。它假定 `url`(包括协议和端口)和 `password` 值通过环境变量指定,并使用 `default` 用户。

**示例:** 使用环境变量创建 Node.js 客户端实例进行配置。
...
你应当尽快开始读取 `ResultSet`，因为它会保持响应流处于打开状态，从而占用底层连接。客户端不会对传入数据进行缓冲，以避免应用程序可能产生的过高内存占用。

另外，如果结果集过大，无法一次性装入内存，你可以调用 `stream` 方法，以流式模式处理数据。每个响应分块都会被转换为一个相对较小的行数组（该数组的大小取决于客户端从服务器接收到的具体分块大小——该大小可能会变化——以及单行的大小），并按分块逐个处理。

请参考[支持的数据格式](./js.md#supported-data-formats)列表，以确定在你的场景中用于流式处理的最合适格式。例如，如果你希望流式处理 JSON 对象，可以选择 [JSONEachRow](/interfaces/formats/JSONEachRow)，此时每一行都会被解析为一个 JS 对象；或者选择更紧凑的 [JSONCompactColumns](/interfaces/formats/JSONCompactColumns) 格式，使每一行成为一个紧凑的值数组。另请参阅：[流式处理文件](./js.md#streaming-files-nodejs-only)。

:::important
如果 `ResultSet` 或其流未被完全读取，在经过 `request_timeout` 设定的空闲时间后将被销毁。
:::
...
**示例：**（Node.js/Web）一个查询返回 `JSONEachRow` 格式的数据集，读取整个流并将内容解析为 JS 对象。
...
**示例：**（仅限 Node.js）使用经典的 `on('data')` 方式，以 `JSONEachRow` 格式流式处理查询结果。此方式可以与 `for await const` 语法互换使用。[源代码](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_streaming_json_each_row.ts)。
...
**示例：**（仅限 Node.js）使用经典的 `on('data')` 模式以 `CSV` 格式流式返回查询结果。此方式可以与 `for await const` 语法互换使用。
...
**示例:**(仅限 Node.js)使用 `for await const` 语法以 `JSONEachRow` 格式流式处理查询结果为 JS 对象。这可以与经典的 `on('data')` 方法互换使用。
...
有关更多详细信息,请参阅 [Node.js 仓库中的此问题](https://github.com/nodejs/node/issues/31979)。
:::
...
返回类型是最小化的,因为我们不期望从服务器返回任何数据,并且会立即消耗完响应流。

如果向 insert 方法提供了空数组,则不会将 insert 语句发送到服务器;相反,该方法将立即以 `{ query_id: '...', executed: false }` 解析。在这种情况下,如果方法参数中未提供 `query_id`,则结果中将是空字符串,因为返回客户端生成的随机 UUID 可能会造成混淆,因为具有此类 `query_id` 的查询不会存在于 `system.query_log` 表中。

如果 insert 语句已发送到服务器,则 `executed` 标志将为 `true`。

#### Node.js 中的 Insert 方法和流式处理 {#insert-method-and-streaming-in-nodejs}

它可以与 `Stream.Readable` 或普通的 `Array<T>` 一起使用,具体取决于为 `insert` 方法指定的[数据格式](./js.md#supported-data-formats)。另请参阅有关[文件流式处理](./js.md#streaming-files-nodejs-only)的部分。

Insert 方法应该被等待;但是,可以指定输入流并稍后等待 `insert` 操作,仅在流完成时(这也将解析 `insert` promise)。这对于事件监听器和类似场景可能很有用,但错误处理可能并不简单,客户端会有很多边缘情况。相反,请考虑使用[异步插入](/optimize/asynchronous-inserts),如[此示例](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/async_insert_without_waiting.ts)所示。
...
此接口未来可能会发生变化。另请参阅:[所有客户端方法的基本参数](./js.md#base-parameters-for-all-client-methods)。

### Command 方法 {#command-method}

该方法用于执行没有输出的语句、不适用格式子句的语句,或者您不关心响应的语句。此类语句的示例包括 `CREATE TABLE` 或 `ALTER TABLE`。
...
await client.command({
  query: `
    ...
    ORDER BY (id)
  `,
  // 建议在集群环境中使用,以避免在响应代码发送后发生查询处理错误,
  // 而 HTTP 头已经发送到客户端的情况
...
并且需要获取结果,可以使用 `exec` 作为 `command` 的替代方法。

`exec` 返回一个可读流,该流必须在应用程序端被消费或销毁。
...
在应用程序启动时，ping 是检查服务器是否可用的一个有用工具，尤其是在 ClickHouse Cloud 中，实例可能处于空闲状态，并会在收到 ping 后被唤醒：在这种情况下，你可能希望在两次调用之间加入一定延迟并重试几次。

请注意，默认情况下，Node.js 版本使用 `/ping` 端点，而 Web 版本则使用简单的 `SELECT 1` 查询来实现类似的效果，这是因为 `/ping` 端点不支持 CORS。
...
**示例：** 如果你希望在调用 `ping` 方法时同时校验凭据，或指定 `query_id` 等其他参数，可以按如下方式使用：

```ts
const result = await client.ping({
  select: true /* query_id、abort_signal、http_headers 或任何其他查询参数 */
})
```

`ping` 方法允许使用大多数标准的 `query` 方法参数——参见 `PingParamsWithSelectQuery` 的类型定义。

### Close（仅限 Node.js） {#close-nodejs-only}

关闭所有已打开的连接并释放资源。在 Web 版本中这是一个无操作（no-op）。
...
客户端代码库中提供了多个使用常见数据格式(NDJSON、CSV、Parquet)的文件流式传输示例。
...
如果您将 `format` 指定为 JSON 系列格式之一（`JSONEachRow`、`JSONCompactEachRow` 等），客户端将在网络通信过程中对数据进行序列化和反序列化。

以"原始"文本格式（`CSV`、`TabSeparated` 和 `CustomSeparated` 系列）提供的数据在网络传输时不会进行额外转换。

:::tip
JSON 作为通用格式与 [ClickHouse JSON 格式](/interfaces/formats/JSON)之间可能存在混淆。

客户端支持使用 [JSONEachRow](/interfaces/formats/JSONEachRow) 等格式流式传输 JSON 对象（有关其他支持流式传输的格式,请参阅表格概览;另请参阅客户端代码库中的 `select_streaming_` [示例](https://github.com/ClickHouse/clickhouse-js/tree/main/examples/node)）。

需要注意的是,[ClickHouse JSON](/interfaces/formats/JSON) 等格式在响应中表示为单个对象,客户端无法对其进行流式传输。
:::
...
对于 Parquet，select 查询的主要用例很可能是将结果流写入文件。请参阅客户端仓库中的[示例](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_parquet_as_file.ts)。

`JSONEachRowWithProgress` 是一种仅支持输出的格式，能够在流中报告进度。有关更多详细信息，请参阅[此示例](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_json_each_row_with_progress.ts)。
...
由于客户端在插入值时不进行额外的类型转换,`Date`/`Date32` 类型列只能以字符串形式插入。
...
但是,如果使用 `DateTime` 或 `DateTime64` 列,则可以同时使用字符串和 JS Date 对象。将 `date_time_input_format` 设置为 `best_effort` 后,可以直接将 JS Date 对象传递给 `insert`。更多详细信息请参阅此[示例](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/insert_js_dates.ts)。
...
然而,当使用 `JSON*` 格式查询数据时,ClickHouse 默认会将 Decimal 类型作为 _数字_ 返回,这可能导致精度损失。为避免这种情况,可以在查询中将 Decimal 类型转换为字符串:
...
虽然服务器可以接受数字形式的输入,但在 `JSON*` 系列输出格式中会以字符串形式返回,以避免整数溢出,因为这些类型的最大值超过了 `Number.MAX_SAFE_INTEGER`。
...
这些设置可以在客户端实例级别进行配置,从而应用于发送到 ClickHouse 的每个请求:
...
包含所有受支持的 ClickHouse 设置的类型声明文件可以在[此处](https://github.com/ClickHouse/clickhouse-js/blob/main/packages/client-common/src/settings.ts)找到。

:::important
请确保执行查询的用户具有更改设置的足够权限。
:::
...
您可以创建参数化查询,并从客户端应用程序传递参数值。这样可以避免在客户端对查询中的特定动态值进行格式化。

按常规方式格式化查询,然后将要从应用程序参数传递到查询的值放在大括号中,格式如下:
...
查看 https://clickhouse.com/docs/interfaces/cli#cli-queries-with-parameters-syntax 了解更多详情。
...
- `DEBUG` - 响应信息(不包含授权标头和主机信息)
- `INFO` - 大多数情况下未使用,将在客户端初始化时打印当前日志级别
- `WARN` - 非致命错误;失败的 `ping` 请求被记录为警告,因为底层错误包含在返回结果中
...
请参阅代码仓库中的 [基础](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/basic_tls.ts) 和 [双向](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/mutual_tls.ts) TLS 完整示例。

### Keep-alive 配置(仅限 Node.js){#keep-alive-configuration-nodejs-only}

客户端默认在底层 HTTP 代理中启用 Keep-Alive,这意味着已连接的套接字将被重用于后续请求,并且会发送 `Connection: keep-alive` 头。空闲的套接字默认会在连接池中保留 2500 毫秒(请参阅[关于调整此选项的说明](./js.md#adjusting-idle_socket_ttl))。
...
如果您要修改 `keep_alive.idle_socket_ttl`,请记住它应该始终与您的服务器/负载均衡器 Keep-Alive 配置保持同步,并且应该**始终低于**该配置,以确保服务器永远不会首先关闭打开的连接。

#### 调整 `idle_socket_ttl` {#adjusting-idle_socket_ttl}

客户端将 `keep_alive.idle_socket_ttl` 设置为 2500 毫秒,因为这可以被认为是最安全的默认值;在服务器端,如果不修改 `config.xml`,`keep_alive_timeout` 在 [23.11 之前的 ClickHouse 版本中可能设置为低至 3 秒](https://github.com/ClickHouse/ClickHouse/commit/1685cdcb89fe110b45497c7ff27ce73cc03e82d1)。

:::warning
如果您对性能满意且没有遇到任何问题,建议**不要**增加 `keep_alive.idle_socket_ttl` 设置的值,因为这可能会导致潜在的 "Socket hang-up" 错误;此外,如果您的应用程序发送大量查询且它们之间的停机时间不多,默认值应该足够,因为套接字不会空闲足够长的时间,客户端会将它们保留在连接池中。
:::
...
在这种情况下,`keep_alive_timeout` 为 10 秒,您可以尝试将 `keep_alive.idle_socket_ttl` 增加到 9000 甚至 9500 毫秒,以使空闲套接字保持打开的时间比默认值稍长。请注意潜在的 "Socket hang-up" 错误,这将表明服务器在客户端之前关闭了连接,请降低该值直到错误消失。
...
- 在 ClickHouse 服务器配置中适当降低 `keep_alive.idle_socket_ttl` 设置。在某些情况下,例如客户端与服务器之间存在较高网络延迟时,将 `keep_alive.idle_socket_ttl` 再降低 200-500 毫秒可能会有所帮助,从而避免出站请求获取到服务器即将关闭的套接字的情况。
...
- 如果此错误发生在没有数据进出的长时间运行查询期间(例如长时间运行的 `INSERT FROM SELECT`),这可能是由于负载均衡器关闭了空闲连接。您可以尝试通过组合使用以下 ClickHouse 设置,在长时间运行的查询期间强制传入一些数据:
  ...
  const client = createClient({
    ...
    /** 这些设置组合使用可以避免在没有数据进出的长时间运行查询(如 `INSERT FROM SELECT` 及类似查询)情况下出现负载均衡器超时问题,
     *  因为连接可能会被负载均衡器标记为空闲并突然关闭。
     *  在本例中,我们假设负载均衡器的空闲连接超时为 120 秒,因此将 110 秒设置为"安全"值。 */
    clickhouse_settings: {
  ...
  但请注意,在最新的 Node.js 版本中,接收的标头总大小限制为 16KB;在接收到一定数量的进度标头后(在我们的测试中约为 70-80 个),将会抛出异常。
...
如果您的 ClickHouse 部署前面有带身份验证的反向代理,您可以使用 `http_headers` 设置在其中提供必要的标头:
...
:::warning
这是一个实验性功能,在未来的版本中可能会以不向后兼容的方式进行更改。客户端提供的默认实现和设置应该足以满足大多数使用场景。仅在您确定需要时才使用此功能。
:::


默认情况下,客户端将使用客户端配置中提供的设置(如 `max_open_connections`、`keep_alive.enabled`、`tls`)来配置底层 HTTP(s) 代理,该代理负责处理与 ClickHouse 服务器的连接。此外,如果使用了 TLS 证书,底层代理将配置必要的证书,并强制使用正确的 TLS 认证头。

从 1.2.0 版本开始,可以向客户端提供自定义 HTTP(s) 代理,以替换默认的底层代理。这在复杂的网络配置场景下可能很有用。如果提供了自定义代理,则适用以下条件:

- `max_open_connections` 和 `tls` 选项将_不起作用_,并将被客户端忽略,因为它们是底层代理配置的一部分。
- `keep_alive.enabled` 仅用于控制 `Connection` 头的默认值(`true` -> `Connection: keep-alive`,`false` -> `Connection: close`)。
- 虽然空闲保持连接的套接字管理仍然有效(因为它不绑定到代理,而是绑定到特定的套接字本身),但现在可以通过将 `keep_alive.idle_socket_ttl` 值设置为 `0` 来完全禁用它。
...
const client = createClient({
  ...
  http_agent: agent,
  // 使用自定义 HTTPS 代理时,客户端不会使用默认的 HTTPS 连接实现;需要手动提供头信息
  http_headers: {
    "X-ClickHouse-User": "username",
    "X-ClickHouse-Key": "password"
  },
  // 重要:授权头与 TLS 头冲突;需要禁用它。
...
当使用证书_和_自定义 _HTTPS_ 代理时,可能需要通过 `set_basic_auth_header` 设置(在 1.2.0 版本中引入)禁用默认的授权头,因为它与 TLS 头冲突。所有 TLS 头都需要手动提供。


## 已知限制（Node.js/web）{#known-limitations-nodejsweb}

- 结果集不提供数据映射器,因此仅使用语言原始类型。计划在 [RowBinary 格式支持](https://github.com/ClickHouse/clickhouse-js/issues/216)中添加某些数据类型映射器。
- [Decimal\* 和 Date\* / DateTime\* 数据类型存在一些注意事项](./js.md#datedate32-types-caveats)。
- 使用 JSON\* 系列格式时,大于 Int32 的数字将表示为字符串,因为 Int64+ 类型的最大值超过了 `Number.MAX_SAFE_INTEGER`。详情请参阅[整数类型](./js.md#integral-types-int64-int128-int256-uint64-uint128-uint256)部分。
...
- 为降低应用程序内存消耗,建议在适用场景下对大批量插入(如从文件插入)和查询使用流式处理。对于事件监听器等类似场景,[异步插入](/optimize/asynchronous-inserts)是另一个不错的选择,可以最小化甚至完全避免客户端批处理。异步插入示例可在[客户端代码库](https://github.com/ClickHouse/clickhouse-js/tree/main/examples)中找到,文件名以 `async_insert_` 为前缀。
- 客户端默认不启用请求或响应压缩。但在查询或插入大型数据集时,可以考虑通过 `ClickHouseClientConfigOptions.compression` 启用压缩(可仅针对 `request` 或 `response`,或同时针对两者)。
- 压缩会带来明显的性能开销。为 `request` 或 `response` 启用压缩将分别对查询或插入速度产生负面影响,但可减少应用程序的网络流量传输。
