...
description: '使用 Java 连接 ClickHouse 的方案'
...
Java 客户端是一个实现自有 API 的库，用于屏蔽与 ClickHouse 服务器进行网络通信的细节。目前仅支持 HTTP 接口。该库提供了处理不同 ClickHouse 数据格式以及其他相关功能的实用工具。

Java 客户端最早在 2015 年开发。随着时间推移，其代码库变得非常难以维护，API 设计也较为混乱，且难以进一步优化。为此，我们在 2024 年将其重构为一个新组件 `client-v2`。它提供了更清晰的 API、更精简的代码库和更多性能优化，并对 ClickHouse 格式（主要是 RowBinary 和 Native）有更好的支持。JDBC 将在不久的将来使用该客户端。  
...
| Http Proxy                        |     ✔     |     ✔     |                            |
| POJO SerDe                        |     ✔     |     ✗     |                            |
| Connection Pool                   |     ✔     |     ✔     | 在使用 Apache HTTP Client 时可用 |
| Named Parameters                  |     ✔     |     ✔     |                            |
| Retry on failure                  |     ✔     |     ✔     |                            |
| Failover                          |     ✗     |     ✔     |                            |
| Load-balancing                    |     ✗     |     ✔     |                            |
...
JDBC 驱动继承其底层客户端实现所具备的相同特性。其他 JDBC 功能列在其[页面](/integrations/language-clients/java/jdbc)中。

### 兼容性

* 此仓库中的所有项目都会针对所有 ClickHouse 的[活动 LTS 版本](https://github.com/ClickHouse/ClickHouse/pulls?q=is%3Aopen+is%3Apr+label%3Arelease)进行测试。
* [支持策略](https://github.com/ClickHouse/ClickHouse/blob/master/SECURITY.md#security-change-log-and-support)
* 我们建议持续升级客户端，以避免错过安全修复和新功能改进。
...
我们的 Java 语言客户端使用 [SLF4J](https://www.slf4j.org/) 进行日志记录。你可以使用任何兼容 SLF4J 的日志框架，例如 `Logback` 或 `Log4j`。
例如，如果你使用 Maven，可以在 `pom.xml` 文件中添加如下依赖：
