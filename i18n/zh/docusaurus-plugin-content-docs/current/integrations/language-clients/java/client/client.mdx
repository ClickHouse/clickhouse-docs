...
>
  <Version>
    ...
    * 检查客户端证书中的 CN 是否与该 USER 匹配：
    ...
    <Tabs groupId="client-config">
      ...
      <TabItem value="server-settings" label="服务器设置">
        | 方法                                              | 参数                                | 描述                                                                                                     | 默认值       | 键                        |
        | ----------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------ | --------- | ------------------------ |
  ...
  <Version>
    ...
    **RowBinary 编码**
    ...
    | node&#95;discovery&#95;interval | `0`     | 节点发现的时间间隔 (毫秒) 。值为 0 或负数表示只进行一次发现。                        |
    ...
    | node&#95;check&#95;interval   | `0`                                      | 以毫秒为单位的节点检查间隔，负数将被视为 0。仅当自上次检查以来已过去指定的时间时，才会检查该节点的状态。<br />`health_check_interval` 和 `node_check_interval` 的区别在于，`health_check_interval` 选项会调度一个后台任务，对节点列表 (全部节点或故障节点) 执行状态检查，而 `node_check_interval` 则指定针对某个特定节点，自上次检查以来两次检查之间至少需要间隔的时间。             |
    ...
    | 属性                                 | 默认值    | 说明                                                                                                                         |
    | ---------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------- |
    | failover                           | `0`    | 单个请求允许执行故障转移的最大次数。为零或负值时表示不进行故障转移。发生故障转移时，请求会根据负载均衡策略被发送到其他节点，以从故障中恢复。                                                     |
