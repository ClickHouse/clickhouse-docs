...
`Client.raw_query` 方法允许通过客户端连接直接使用 ClickHouse HTTP 查询接口。返回值是未经处理的 `bytes` 对象。该方法提供了一个便捷的封装,通过最简化的接口实现参数绑定、错误处理、重试和设置管理:
...
| fmt           | str              | _None_     | 结果字节的 ClickHouse 输出格式。(如未指定,ClickHouse 使用 TSV)                                                                                         |
...
调用者需负责处理返回的 `bytes` 对象。请注意,`Client.query_arrow` 只是对此方法的简单封装,使用 ClickHouse `Arrow` 输出格式。

### 客户端 `raw_stream` 方法 {#client-rawstream-method}

`Client.raw_stream` 方法具有与 `raw_query` 方法相同的 API,但返回一个 `io.IOBase` 对象,该对象可用作 `bytes` 对象的生成器/流源。目前 `query_arrow_stream` 方法使用了该方法。

### 客户端 `raw_insert` 方法 {#client-rawinsert-method}

`Client.raw_insert` 方法允许通过客户端连接直接插入 `bytes` 对象或 `bytes` 对象生成器。由于不对插入负载进行任何处理,因此性能很高。该方法提供了指定设置和插入格式的选项:

| 参数         | 类型                                   | 默认值     | 描述                                                                                 |
| ------------ | -------------------------------------- | ---------- | ------------------------------------------------------------------------------------ |
| table        | str                                    | _必需_     | 简单表名或带数据库限定的表名                                                          |
| column_names | Sequence[str]                          | _None_     | 插入块的列名。如果 `fmt` 参数不包含列名,则为必需                                      |
| insert_block | str, bytes, Generator[bytes], BinaryIO | _必需_     | 要插入的数据。字符串将使用客户端编码进行编码。                                        |
| settings     | dict                                   | _None_     | 参见[设置说明](driver-api.md#settings-argument)。                                    |
| fmt          | str                                    | _None_     | `insert_block` 字节的 ClickHouse 输入格式。(如未指定,ClickHouse 使用 TSV)             |

调用者需负责确保 `insert_block` 采用指定的格式并使用指定的压缩方法。ClickHouse Connect 将这些原始插入用于文件上传和 PyArrow 表,并将解析委托给 ClickHouse 服务器。
...
ClickHouse Connect 在多线程、多进程和事件循环驱动/异步应用程序中表现良好。所有查询和插入处理都在单个线程内执行,因此操作通常是线程安全的。(在底层对某些操作进行并行处理是未来可能的增强功能,以克服单线程带来的性能损失,但即使在这种情况下也会保持线程安全。)

由于每个执行的查询或插入分别在其自己的 `QueryContext` 或 `InsertContext` 对象中维护状态,这些辅助对象不是线程安全的,不应在多个处理流之间共享。有关上下文对象的更多讨论,请参阅 [QueryContexts](advanced-querying.md#querycontexts) 和 [InsertContexts](advanced-inserting.md#insertcontexts) 部分。
...
注意:与常规 `Client` 不同,`AsyncClient` 默认强制将 `autogenerate_session_id` 设置为 `False`。
...
对于大型多线程应用程序,使用独立的连接池可能更为合适。可以通过 `pool_mgr` 关键字参数向 `clickhouse_connect.get_client` 主函数提供自定义连接池:
