...
ClickHouse 支持对大多数查询值进行[服务器端绑定](/interfaces/client#cli-queries-with-parameters)，其中绑定的值作为 HTTP 查询参数，与查询本身分开发送。如果 ClickHouse Connect 检测到形如 `{<name>:<datatype>}` 的绑定表达式，将自动添加相应的查询参数。对于服务器端绑定，`parameters` 参数应为一个 Python 字典。
...
| transport&#95;settings | dict             | *None*     | 要随此请求一起发送的可选 HTTP 标头字典。每个键值对都会添加为一个 HTTP 标头 (例如，`{'X-Custom-Header': 'value'}`) 。可用于代理身份验证、请求跟踪，或传递中间基础设施所需的标头。 |
...
| transport&#95;settings      | dict             | *None*     | 可选的 HTTP 标头字典，会随此请求一并发送。每个键值对都会添加为一个 HTTP 标头 (例如 `{'X-Custom-Header': 'value'}`) 。可用于代理身份验证、请求跟踪，或传递中间基础设施所需的标头。               |
...
| transport&#95;settings    | dict                              | *None*      | 可选的、要随此请求一起包含的 HTTP 头字典。每个键值对都会作为一个 HTTP 头添加 (例如 `{'X-Custom-Header': 'value'}`) 。这对于代理身份验证、请求跟踪或传递中间基础设施所需的头很有用。                 |
