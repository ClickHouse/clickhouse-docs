...
ClickHouse Connect 在 `InsertContext` 中执行所有插入操作。`InsertContext` 包含作为参数传递给客户端 `insert` 方法的所有值。此外,当首次构造 `InsertContext` 时,ClickHouse Connect 会检索插入列所需的数据类型,以便进行高效的 Native 格式插入。通过为多次插入重用 `InsertContext`,可以避免这种"预查询",从而更快速、更高效地执行插入操作。

可以使用客户端的 `create_insert_context` 方法获取 `InsertContext`。该方法接受与 `insert` 函数相同的参数。请注意,重用时只应修改 `InsertContext` 的 `data` 属性。这与其设计目的一致,即提供一个可重用的对象,用于向同一表重复插入新数据。
...
`InsertContext` 包含在插入过程中更新的可变状态,因此不是线程安全的。

### 写入格式 {#write-formats}

写入格式目前仅针对有限数量的类型实现。在大多数情况下,ClickHouse Connect 会通过检查第一个(非空)数据值的类型来自动确定列的正确写入格式。例如,如果向 `DateTime` 列插入数据,并且该列的第一个插入值是 Python 整数,ClickHouse Connect 将直接插入该整数值,假定它实际上是 Unix 时间戳(秒)。
...
| JSON/Object('json')   | dict                    | string            | 字典或 JSON 字符串都可以插入到 JSON 列中(注意 `Object('json')` 已弃用)                                      |
| Variant               | object                  |                   | 目前所有变体都作为字符串插入并由 ClickHouse 服务器解析                                                      |
| Dynamic               | object                  |                   | 警告 -- 目前任何插入到 Dynamic 列的数据都会作为 ClickHouse String 持久化                                    |

### 专用插入方法 {#specialized-insert-methods}

ClickHouse Connect 为常见数据格式提供了专用插入方法:

- `insert_df` -- 插入 Pandas DataFrame。此方法的第二个参数需要一个 `df` 参数(必须是 Pandas DataFrame 实例),而不是 Python Sequence of Sequences 的 `data` 参数。ClickHouse Connect 自动将 DataFrame 作为列式数据源处理,因此不需要也不提供 `column_oriented` 参数。
...
- `insert_df_arrow` -- 插入基于 Arrow 的 Pandas DataFrame 或 Polars DataFrame。ClickHouse Connect 将自动判断 DataFrame 是 Pandas 还是 Polars 类型。如果是 Pandas,将执行验证以确保每列的 dtype 后端都基于 Arrow,如果有任何列不是则会抛出错误。
...
将 Python `datetime.datetime` 对象插入 ClickHouse `DateTime` 或 `DateTime64` 列时,ClickHouse Connect 会自动处理时区信息。由于 ClickHouse 内部将所有 DateTime 值存储为无时区的 Unix 时间戳(自纪元以来的秒数或小数秒),时区转换会在插入时自动在客户端完成。

#### 带时区的 datetime 对象 {#timezone-aware-datetime-objects}

如果插入带时区的 Python `datetime.datetime` 对象,ClickHouse Connect 会自动调用 `.timestamp()` 将其转换为 Unix 时间戳,该时间戳会正确处理时区偏移量。这意味着您可以插入任意时区的 datetime 对象,它们都会被正确存储为对应的 UTC 时间戳。
...
在此示例中,三个 datetime 对象代表不同的时间点,因为它们具有不同的时区。每个对象都将被正确转换为相应的 Unix 时间戳并存储在 ClickHouse 中。

:::note
使用 pytz 时,必须使用 `localize()` 方法为不带时区信息的 datetime 对象附加时区信息。直接将 `tzinfo=` 传递给 datetime 构造函数会使用不正确的历史偏移量。对于 UTC,`tzinfo=pytz.UTC` 可以正常工作。更多信息请参阅 [pytz 文档](https://pythonhosted.org/pytz/#localized-times-and-date-arithmetic)。
:::

#### 不带时区信息的 datetime 对象 {#timezone-naive-datetime-objects}

如果插入不带时区信息的 Python `datetime.datetime` 对象(即没有 `tzinfo` 的对象),`.timestamp()` 方法会将其解释为系统本地时区。为避免歧义,建议:

1. 插入时始终使用带时区信息的 datetime 对象,或
2. 确保系统时区设置为 UTC,或
3. 在插入前手动转换为 epoch 时间戳
...
ClickHouse 列可以定义时区元数据(例如 `DateTime('America/Denver')` 或 `DateTime64(3, 'Asia/Tokyo')`)。此元数据不影响数据的存储方式(仍以 UTC 时间戳存储),但会控制从 ClickHouse 查询数据时所使用的时区。

向此类列插入数据时,ClickHouse Connect 会将 Python datetime 转换为 Unix 时间戳(如果存在时区信息则将其纳入计算)。查询数据时,ClickHouse Connect 将返回转换为列时区的 datetime,无论插入时使用的是什么时区。
...
# 插入纽约时间（美国东部夏令时间 10:30 AM，对应 UTC 14:30）
...
`clickhouse_connect.driver.tools` 包提供了 `insert_file` 方法,支持直接从文件系统向现有 ClickHouse 表插入数据。数据解析由 ClickHouse 服务器负责。`insert_file` 接受以下参数:

| 参数         | 类型            | 默认值            | 描述                                                                                                                      |
| ------------ | --------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------- |
| client       | Client          | _必需_            | 用于执行插入操作的 `driver.Client`                                                                                         |
| table        | str             | _必需_            | 要插入数据的 ClickHouse 表。可使用完整表名(包含数据库名)。                                                                 |
| file_path    | str             | _必需_            | 数据文件的本地文件系统路径                                                                                                  |
| fmt          | str             | CSV, CSVWithNames | 文件的 ClickHouse 输入格式。如果未提供 `column_names`,则默认为 CSVWithNames                                               |
| column_names | Sequence of str | _None_            | 数据文件中的列名列表。对于已包含列名的格式无需提供此参数                                                                          |
...
| compression  | str             | _None_            | ClickHouse 支持的压缩类型(zstd、lz4、gzip),用于 Content-Encoding HTTP 头                                                    |

对于包含不一致数据或非常规格式日期/时间值的文件,此方法支持应用于数据导入的设置(例如 `input_format_allow_errors_num` 和 `input_format_allow_errors_ratio`)。
