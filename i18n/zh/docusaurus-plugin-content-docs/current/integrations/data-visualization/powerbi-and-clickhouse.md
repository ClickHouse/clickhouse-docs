...
Microsoft Power BI 可以从 [ClickHouse Cloud](https://clickhouse.com/cloud) 或自行管理的部署中查询数据，或将数据加载到内存中。

你可以使用多种 Power BI 产品来可视化你的数据：

* Power BI Desktop：Windows 桌面应用，用于创建仪表板和可视化视图
* Power BI Service：在 Azure 中以 SaaS 形式提供，用于承载在 Power BI Desktop 中创建的仪表板
...
如果您的使用场景数据量较小,可以选择导入模式,所有数据将被加载到 Power BI 中。
:::
...
最后,您应该能在导航器视图中看到数据库和表。选择所需的表并单击"加载"以从 ClickHouse 导入数据。
...
导入完成后,您的 ClickHouse 数据应该可以像往常一样在 Power BI 中访问。
...
在此示例中,`pageviews` 表包含一个 UInt64 列,默认被识别为"二进制"类型。
