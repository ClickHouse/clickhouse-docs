...
- 'integrate'
- 'ui'
'description': 'Looker是一款企业级BI、数据应用和嵌入式分析平台，可帮助您实时探索和共享见解。'
'title': 'Looker'
---
...
导航至 管理 -> 数据库 -> 连接，然后点击右上角的“添加连接”按钮。

<Image size="md" img={looker_01} alt="在 Looker 的数据库管理界面中添加新连接" border />
<br/>

为您的数据源选择一个名称，并从方言下拉列表中选择 `ClickHouse`。在表单中输入您的凭据。
...
首先测试您的连接，一旦完成，连接到您的新 ClickHouse 数据源。
...
1. 以下数据类型默认以字符串形式处理：
   * Array - 由于 JDBC 驱动程序的限制，序列化无法按预期工作
   * Decimal* - 可以在模型中更改为数字
   ...
   * Geo types
     ...
     * Point
     * Ring
2. [对称聚合特性](https://cloud.google.com/looker/docs/reference/param-explore-symmetric-aggregates)不被支持
