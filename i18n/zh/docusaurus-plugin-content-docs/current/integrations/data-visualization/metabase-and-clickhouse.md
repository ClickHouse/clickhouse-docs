...
keywords: ['Metabase']
description: 'Metabase 是一款易于使用的开源可视化界面工具，用于探索和分析您的数据。'
...
Metabase 是一款易用的开源 UI 工具，用于对数据进行分析和提问。Metabase 是一个 Java 应用，只需<a href="https://www.metabase.com/start/oss/jar" target="_blank">下载 JAR 文件</a>，然后使用 `java -jar metabase.jar` 运行即可。Metabase 通过 JDBC 驱动程序连接到 ClickHouse，您需要下载该驱动并将其放入 `plugins` 目录中：
...
5. 输入您的 ClickHouse 数据库连接详细信息。如果您的 ClickHouse 服务器配置了 SSL,请启用安全连接。例如:
   ...
   <Image
     size='md'
     img={metabase_02}
     alt='ClickHouse 数据库的 Metabase 连接详细信息表单'
     border
   />
...
3. 例如,以下是在名为 `uk_price_paid` 的表上运行的 SQL 查询,该查询返回 1995 年至 2022 年按年份统计的平均支付价格:
...
1. 点击 **+ New** 并选择 **Question**。您可以通过选择数据库和表来构建查询。例如,以下查询针对 `default` 数据库中名为 `uk_price_paid` 的表。这是一个简单的查询示例,用于计算大曼彻斯特郡内各城镇的平均价格:
...
3. 在结果下方,点击 **Visualization** 按钮将可视化方式更改为条形图(或其他可用的图表类型):
