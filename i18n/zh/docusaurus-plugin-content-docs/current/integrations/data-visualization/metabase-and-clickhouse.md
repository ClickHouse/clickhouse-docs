...
keywords: ['Metabase']
description: 'Metabase 是一款易用的开源 UI 工具，可用于对你的数据进行提问和分析。'
title: '连接 Metabase 与 ClickHouse'
...
Metabase 是一款易于使用的开源 UI 工具，可用于对你的数据进行查询和分析。Metabase 是一个 Java 应用，只需<a href="https://www.metabase.com/start/oss/jar" target="_blank">下载 JAR 文件</a>，然后使用 `java -jar metabase.jar` 命令运行即可启动。Metabase 通过 JDBC 驱动程序连接到 ClickHouse，你需要下载该驱动并将其放入 `plugins` 目录中：
...
<Image
  size='md'
  img={metabase_08}
  alt='显示 ClickHouse 数据的 Metabase 饼图可视化'
  border
/>
<p />
...
1. 如果您没有 `plugins` 文件夹,请在 `metabase.jar` 所在目录下创建一个 `plugins` 子文件夹。
...
2. 在右上角点击 **+ New** 菜单,您可以提问、运行 SQL 查询以及构建仪表板:
...
3. 例如,以下是在名为 `uk_price_paid` 的表上运行的 SQL 查询,返回 1995 年至 2022 年按年份统计的平均支付价格:
...
## 5. 提出查询 {#5-ask-a-question}

1. 点击 **+ New** 并选择 **Question**。您可以从选择数据库和表开始构建查询。例如,以下查询针对 `default` 数据库中名为 `uk_price_paid` 的表。这是一个简单的查询示例,用于计算大曼彻斯特郡内各城镇的平均价格:

   <Image
     size='md'
     img={metabase_06}
     alt='Metabase 查询构建器界面,显示英国价格数据'
     border
   />
...
3. 在结果下方,点击 **Visualization** 按钮将可视化图表更改为条形图(或其他可用的图表类型):
...
如需了解更多关于 Metabase 以及如何构建仪表板的信息,请<a href="https://www.metabase.com/docs/latest/" target="_blank">访问 Metabase 文档</a>。
