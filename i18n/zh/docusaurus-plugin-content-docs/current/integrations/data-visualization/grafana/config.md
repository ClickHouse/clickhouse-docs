...
修改配置最简单的方式是在 Grafana UI 的插件配置页面中进行，但也可以通过 [YAML 文件预配置数据源](https://grafana.com/docs/grafana/latest/administration/provisioning/#data-sources)。

本页列出了 ClickHouse 插件中可用的配置选项，并提供了使用 YAML 预配置数据源时可用的配置示例片段。
...
jsonData:
  ...
  tlsAuthWithCACert: <boolean> # 如果提供了 CA 证书，则设置为 true。验证自签名 TLS 证书时必需。
...
请注意，从 UI 保存配置时会添加 `version` 属性。该属性显示保存配置时使用的插件版本。

### HTTP 协议 {#http-protocol}

如果选择通过 HTTP 协议连接，将显示更多设置选项。
...
头可以是明文或安全的。
所有头键以明文形式存储，而安全头值保存在安全配置中（类似于 `password` 字段）。
...
jsonData:
  ...
  dialTimeout: 10 # 连接服务器时的拨号超时时间,单位为秒。默认为 "10"。
  queryTimeout: 60 # 运行查询时的查询超时时间,单位为秒。默认为 60。此设置需要用户权限,如果遇到权限错误,请尝试将其设置为 "0" 以禁用。
...
还需要配置这些默认值以启用[数据链接](./query-builder.md#data-links),这是一项支持强大可观测性工作流的功能。
...
虽然日志不需要 OpenTelemetry,但使用统一的日志/追踪数据集有助于通过[数据链接](./query-builder.md#data-links)实现更流畅的可观测性工作流。
...
这将自动覆盖默认列以使用所选的 OTel 模式版本。
虽然不需要 OpenTelemetry,但使用其模式进行追踪时,此功能效果最佳。
...
与表定义的别名相比,这种方法的优势在于您可以轻松更新它们,而无需修改表。在某些模式中,这可能有数千个条目,可能会使底层表定义变得混乱。它还允许隐藏您希望用户忽略的列。
