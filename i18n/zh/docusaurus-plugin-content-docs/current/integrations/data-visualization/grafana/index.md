...
借助 Grafana，您可以通过仪表板对所有数据进行探索和共享。
Grafana 需要一个插件才能连接到 ClickHouse，该插件可在 Grafana 的 UI 中轻松安装。
...
您的数据源现已可以使用!了解更多关于如何使用[查询构建器](./query-builder.md)构建查询的信息。

有关配置的更多详细信息,请查看[插件配置](./config.md)文档。

如果您需要查找本文档中未包含的更多信息,请访问 [GitHub 上的插件仓库](https://github.com/grafana/clickhouse-datasource)。


## 升级插件版本 {#upgrading-plugin-versions}

从 v4 开始,配置和查询可以随新版本发布而升级。

v3 的配置和查询在打开时会自动迁移到 v4。虽然旧的配置和仪表板可以在 v4 中加载,但迁移结果不会持久化,除非在新版本中重新保存。如果您在打开旧配置/查询时遇到任何问题,请放弃更改并[在 GitHub 上报告问题](https://github.com/grafana/clickhouse-datasource/issues)。

如果配置/查询是用较新版本创建的,则插件无法降级到旧版本。
