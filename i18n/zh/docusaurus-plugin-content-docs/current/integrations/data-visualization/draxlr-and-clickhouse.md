...
- 'integrate'
- 'ui'
'description': 'Draxlr是一个具有数据可视化和分析功能的商业智能工具。'
'title': '将Draxlr连接到ClickHouse'
---
...
5. 在表单中添加连接详细信息。
  ...
  <Image size="md" img={draxlr_01} alt="Draxlr 连接表单显示 ClickHouse 数据库配置选项" border />

6. 点击 **Next** 按钮，并等待连接建立。如果连接成功，您将看到表格页面。

## 4. 探索您的数据 {#4-explore-your-data}

1. 点击列表中的一个表。

2. 这将带您进入探索页面，以查看表中的数据。

3. 您可以开始添加过滤器，进行连接并添加排序。

  <Image size="md" img={draxlr_02} alt="Draxlr 数据探索界面显示过滤器和排序选项" border />

4. 您还可以使用 **Graph** 按钮选择图表类型以可视化数据。
...
2. 点击 **Raw Query** 按钮并在文本区域中输入您的查询。
  ...
  <Image size="md" img={draxlr_03} alt="Draxlr SQL 查询界面针对 ClickHouse" border />
...
1. 执行查询后，点击 **Save Query** 按钮。
  ...
  <Image size="md" img={draxlr_04} alt="Draxlr 保存查询对话框与仪表板选项" border />
...
2. 您可以通过点击左侧边栏上的 **Add +** 按钮来添加一个新的仪表板。

3. 要添加新小部件，请点击右上角的 **Add** 按钮。

4. 您可以从已保存查询的列表中选择一个查询，并选择可视化类型，然后点击 **Add Dashboard Item** 按钮。

## 了解更多 {#learn-more}
要了解有关 Draxlr 的更多信息，您可以访问 [Draxlr documentation](https://draxlr.notion.site/draxlr/Draxlr-Docs-d228b23383f64d00a70836ff9643a928) 网站。
