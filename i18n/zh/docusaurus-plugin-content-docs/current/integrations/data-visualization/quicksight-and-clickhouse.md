...
description: 'Amazon QuickSight 以统一的商业智能 (BI) 赋能数据驱动型组织。'
title: 'QuickSight'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
QuickSight 可以通过官方 MySQL 数据源，在 Direct Query 模式下使用 MySQL 接口连接到本地部署的 ClickHouse（23.11+）。
...
但是,建议将其分配给单独的配置文件供 MySQL 用户使用,而不是使用默认配置文件。
...
点击"Edit/Preview data",您应该能够看到解析出的表结构,或者如果您选择使用自定义 SQL 访问数据,则可以调整您的自定义 SQL:
...
现在您可以继续发布数据集并创建新的可视化了!


## 已知限制 {#known-limitations}

- SPICE 导入功能无法正常工作,请使用直接查询模式。详见 [#58553](https://github.com/ClickHouse/ClickHouse/issues/58553)。
