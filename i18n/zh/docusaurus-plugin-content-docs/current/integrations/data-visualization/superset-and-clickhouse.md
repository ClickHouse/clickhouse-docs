...
keywords: ['superset']
description: 'Apache Superset 是一款开源的数据探索和可视化平台。'
...
<a href="https://superset.apache.org/" target="_blank">Apache Superset</a> 是一个由 Python 编写的开源数据探索与可视化平台。Superset 使用 ClickHouse 提供的 Python 驱动程序连接到 ClickHouse。下面来看它的工作原理……



## Goal {#goal}

在本指南中，您将使用来自 ClickHouse 数据库的数据在 Superset 中构建仪表板。该仪表板效果如下：
...
4. 点击 **CONNECT** 按钮,然后点击 **FINISH** 按钮以完成设置向导,您应该能在数据库列表中看到您的数据库。


## 4. 添加数据集 {#4-add-a-dataset}

1. 要在 Superset 中使用 ClickHouse 数据,需要先定义一个**_数据集_**。在 Superset 顶部菜单中选择 **Data**,然后从下拉菜单中选择 **Datasets**。

2. 点击添加数据集按钮。选择新建的数据库作为数据源,即可看到该数据库中定义的表:
...
3. 点击对话框底部的 **ADD** 按钮,该表将显示在数据集列表中。现在您可以开始构建仪表板并分析 ClickHouse 数据了!


## 5. 在 Superset 中创建图表和仪表板 {#5--creating-charts-and-a-dashboard-in-superset}

如果您熟悉 Superset,那么接下来的内容会让您感到轻车熟路。如果您是 Superset 的新手,那么……它就像世界上许多其他优秀的可视化工具一样——入门很快,但细节和技巧需要在使用过程中逐步掌握。

1. 首先创建一个仪表板。在 Superset 的顶部菜单中,选择 **Dashboards**。点击右上角的按钮添加新仪表板。以下仪表板命名为 **UK property prices**:
...
3. Superset 饼图需要一个 **Dimension** 和一个 **Metric**,其余设置为可选项。您可以为维度和指标选择自己的字段,本示例使用 ClickHouse 字段 `district` 作为维度,`AVG(price)` 作为指标。
