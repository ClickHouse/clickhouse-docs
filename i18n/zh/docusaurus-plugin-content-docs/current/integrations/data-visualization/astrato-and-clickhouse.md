...
- 'embedded analytics'
- 'Astrato'
'description': 'Astrato 为企业和数据业务带来了真正的自服务商业智能，通过把分析放在每个用户的手中，使他们能够构建自己的仪表板、报告和数据应用，从而使数据问题的回答不再依赖于
  IT 支持。Astrato 加速了采纳，提升了决策速度，并将分析、嵌入式分析、数据输入和数据应用统一在一个平台上。Astrato 将行动和分析结合在一起，引入实时回写，互动
  ML 模型，通过支持推送 SQL 来加速分析，超越仪表板。'
'title': '连接 Astrato 到 ClickHouse'
---
...
Astrato 使用下推 SQL 直接查询 ClickHouse Cloud 或本地部署。这意味着您可以访问所需的所有数据，受益于 ClickHouse 的行业领先性能。
...
- 在侧边栏中选择 **数据**，然后选择 **数据连接** 标签
（或导航到此链接：https://app.astrato.io/data/sources）
​
- 单击屏幕右上角的 **新建数据连接** 按钮。
...
- 单击 **测试连接**。如果连接成功，给数据连接一个 **名称** 并单击 **下一步**。

- 设置对数据连接的 **用户访问**，然后单击 **连接**。
...
在我们的数据视图编辑器中，您将看到 ClickHouse 中的所有表和模式，选择一些以开始。

<Image size="lg" img={astrato_4a_clickhouse_data_view} alt="Astrato 连接到 ClickHouse 用户访问" border />

现在您已经选择了数据，前往定义 **数据视图**。单击网页右上角的定义。

在这里，您可以连接数据，并 **创建管理维度和度量** - 理想的选择以在各个团队之间推动业务逻辑的一致性。

<Image size="lg" img={astrato_4b_clickhouse_data_view_joins} alt="Astrato 连接到 ClickHouse 用户访问" border />

**Astrato 智能地建议连接** 使用您的元数据，包括利用 ClickHouse 中的键。我们的建议连接使您能够轻松开始，利用您良好管理的 ClickHouse 数据，而无需重新发明轮子。我们还会向您显示 **连接质量**，让您有机会详细查看 Astrato 的所有建议。
...
透明性和准确性是 Astrato 的核心。我们确保生成的每个查询都是可见的，让您保持完全的控制。所有计算直接在 ClickHouse 中进行，利用其速度，同时保持强大的安全性和治理。
...
一个美丽的完整仪表板或数据应用程序不再遥远。要查看我们构建的更多内容，请访问我们网站上的演示图库。 https://astrato.io/gallery
