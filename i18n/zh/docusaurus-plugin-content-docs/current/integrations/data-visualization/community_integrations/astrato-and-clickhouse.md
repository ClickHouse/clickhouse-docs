...
description: 'Astrato 通过将分析能力交到每一位用户手中，为企业和数据驱动型业务带来真正的自助式 BI，使用户能够自行构建仪表板、报表和数据应用，在无需 IT 协助的情况下解答数据问题。Astrato 加速部署与采用、提升决策速度，并在一个平台中统一分析、嵌入式分析、数据录入和数据应用。Astrato 将行动与分析融为一体，支持实时回写、与 ML 模型交互，并借助 AI 加速分析——在 Astrato 对下推 SQL 的支持下，使能力远超传统仪表板。'
...
Astrato 使用 Pushdown SQL 直接查询 ClickHouse Cloud 或本地部署的 ClickHouse。这意味着你可以依托 ClickHouse 行业领先的性能，访问所需的全部数据。
...
<Image
  ...
  alt='Astrato 连接到 ClickHouse 必填字段'
  border
/>
...
<Image
  ...
  alt='Astrato 连接到 ClickHouse 用户访问权限'
  border
/>
...
在数据视图编辑器中,您将看到 ClickHouse 中的所有表和架构,选择其中一些即可开始使用。
...
在这里,您可以连接数据,并**创建受治理的维度和度量** —— 这对于在不同团队之间保持业务逻辑的一致性非常理想。
...
**Astrato 会智能建议连接方式**,利用您的元数据,包括 ClickHouse 中的键。我们建议的连接方式让您可以轻松上手,直接使用治理良好的 ClickHouse 数据,无需重复造轮子。我们还会显示**连接质量**,以便您可以详细查看 Astrato 提供的所有建议。
...
透明度和准确性是 Astrato 的核心。我们确保生成的每个查询都是可见的,让您保持完全控制。所有计算都直接在 ClickHouse 中进行,充分利用其速度优势,同时保持强大的安全性和治理能力。
