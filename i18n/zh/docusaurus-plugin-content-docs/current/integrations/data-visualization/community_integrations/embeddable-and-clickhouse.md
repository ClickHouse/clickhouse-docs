...
description: 'Embeddable 是一个开发者工具包，用于在你的应用中直接构建快速、交互式且高度可自定义的分析体验。'
...
在 [Embeddable](https://embeddable.com/) 中，你可以在代码中定义 [Data Models](https://docs.embeddable.com/data-modeling/introduction) 和 [Components](https://docs.embeddable.com/development/introduction)（存储在你自己的代码仓库中），并使用我们的 **SDK** 将它们接入功能强大的 Embeddable **零代码构建器**，供你的团队使用。

最终，你可以在自己的产品中直接向客户交付快速、交互式的分析功能；由你的产品团队负责设计；由你的工程团队负责构建；由你的客户团队和数据团队负责维护——这才是它应有的样子。

内置的行级安全保证每位用户只能看到其被授权访问的精确数据。而两级、可完全配置的缓存机制则让你能够在大规模场景下提供快速的实时分析。
