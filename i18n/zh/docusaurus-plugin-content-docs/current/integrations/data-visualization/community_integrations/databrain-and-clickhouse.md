...
[Databrain](https://usedatabrain.com) 是一个嵌入式分析平台，使您能够构建并与客户共享交互式仪表盘、指标和数据可视化。Databrain 通过 HTTPS 接口连接到 ClickHouse，让您能够借助现代且易于使用的界面，轻松对 ClickHouse 数据进行可视化和分析。
...
3. **使用合适的数据类型**：确保 ClickHouse schema 使用了最优的数据类型
