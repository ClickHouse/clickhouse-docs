...
keywords: ['clickhouse', 'Databrain', '连接', '集成', 'ui', '分析', '嵌入式', '仪表板', '可视化']
...
[Databrain](https://usedatabrain.com) 是一个嵌入式分析平台，使你能够为客户构建并共享交互式仪表盘、指标和数据可视化。Databrain 通过 HTTPS 接口连接到 ClickHouse，使你能够使用现代、易用的界面轻松对 ClickHouse 数据进行可视化和分析。

<Image size="md" img={databrain_01} alt="显示 ClickHouse 数据可视化的 Databrain 仪表盘界面" border />

<br/>

本指南将逐步讲解如何将 Databrain 连接到你的 ClickHouse 实例。
...
- **多租户和单租户**: 连接 ClickHouse 数据库,支持单租户和多租户架构
- **报告调度**: 安排自动化报告并通过电子邮件发送给利益相关者
- **AI 驱动的洞察**: 使用 AI 从数据生成摘要和洞察
