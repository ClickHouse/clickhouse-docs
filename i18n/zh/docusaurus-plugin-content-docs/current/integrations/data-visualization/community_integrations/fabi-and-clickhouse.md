...
keywords: ['ClickHouse', 'Fabi.ai', '连接', '集成', 'notebook', '界面', '分析']
description: 'Fabi.ai 是一个一体化协作数据分析平台。你可以利用 SQL、Python、AI 和无代码功能，以前所未有的速度构建仪表板和数据工作流'
...
<a href="https://www.fabi.ai/" target="_blank">Fabi.ai</a> 是一个一体化协同数据分析平台。你可以利用 SQL、Python、AI 和无代码功能，以前所未有的速度构建仪表板和数据工作流。结合 ClickHouse 的规模扩展能力与强大性能，你可以在几分钟内基于海量数据集构建并共享你的第一个高性能仪表板。

<Image size="md" img={fabi_01} alt="Fabi.ai 数据探索与工作流平台" border />

## 获取连接详细信息 \{#gather-your-connection-details\}
...
1. 首次创建账户时，系统会提示您连接数据库；如果您已有账户，请点击任意 Smartbook 左侧的 数据源 面板，然后选择 Add Data Source。
...
3. 恭喜！您已成功将 ClickHouse 集成到 Fabi.ai。

## 查询 ClickHouse \{#querying-clickhouse\}

将 Fabi.ai 连接到 ClickHouse 后，前往任意一个 [Smartbook](https://docs.fabi.ai/analysis_and_reporting/smartbooks) 并创建一个 SQL 单元。如果你的 Fabi.ai 实例只连接了一个数据源，SQL 单元会默认使用 ClickHouse；否则，你可以从数据源下拉菜单中选择要查询的数据源。
