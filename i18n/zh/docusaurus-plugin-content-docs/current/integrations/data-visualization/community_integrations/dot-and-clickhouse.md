...
description: 'AI 聊天机器人 | Dot 是一款智能虚拟数据助手，能够回答业务数据问题、检索定义和相关数据资产，甚至在 ClickHouse 的支持下协助进行数据建模。'
title: 'Dot'
doc_type: 'guide'
---
...
它可以直接连接到 ClickHouse，让你可以用自然语言提出与数据相关的问题、探索数据、验证假设并解答“为什么”的问题——这一切都可以直接在 Slack、Microsoft Teams、ChatGPT 或其原生 Web 界面中完成。



## 前置条件 {#pre-requisites}

- 一个 ClickHouse 数据库,可以是自托管部署或 [ClickHouse Cloud](https://clickhouse.com/cloud)
...
- **深入分析原因**: 通过追问了解趋势和异常现象。
- **集成到您的工作环境**: 支持 Slack、Microsoft Teams、ChatGPT 或 Web 应用。
- **结果可靠**: Dot 会根据您的 schema 和定义验证查询,最大程度减少错误。
- **可扩展**: 基于查询下推技术构建,将 Dot 的智能与 ClickHouse 的速度完美结合。
