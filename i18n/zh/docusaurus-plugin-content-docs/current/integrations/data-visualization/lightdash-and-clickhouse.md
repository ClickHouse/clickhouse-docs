...
description: 'Lightdash 是一款构建在 dbt 之上的现代开源 BI 工具，团队可以通过语义层探索和可视化来自 ClickHouse 的数据。了解如何将 Lightdash 连接到 ClickHouse，以获得由 dbt 驱动的高速、可治理分析。'
...
Lightdash 是为现代数据团队打造的 **AI 优先 BI 平台**，将 dbt 的开放性与 ClickHouse 的高性能相结合。通过将 ClickHouse 连接到 Lightdash，团队可以获得基于 dbt 语义层的 **AI 驱动自助式分析体验**，从而确保每个问题都能通过受治理且一致的指标得到解答。

开发者喜欢 Lightdash 的开放架构、受版本控制的 YAML 模型，以及可以直接融入其工作流的各种集成能力——从 GitHub 到 IDE。

这一合作将 **ClickHouse 的速度** 与 **Lightdash 的开发者体验**相结合，使基于 AI 的数据探索、可视化与洞察自动化变得前所未有的简单。
...
由于此过程对所有数据仓库都相同,我们在此不做详细说明——您可以按照官方 Lightdash 指南导入 dbt 项目:
...
在 Lightdash 中,所有**指标**和**维度**都直接在您的 dbt 模型 `.yml` 文件中定义。这使您的业务逻辑具有版本控制、一致性和完全透明性。
...
### 从表中查询您的数据 {#querying-your-data-from-tables}

一旦您的 dbt 项目连接并与 Lightdash 同步,您就可以直接从**表**(或"探索")开始探索数据。
...
Lightdash 中的 **AI Agents** 使数据探索真正实现自助式。  
用户无需编写查询,只需用自然语言提问 — 例如 _"我们的月度收入增长是多少?"_ — AI Agent 会自动生成正确的可视化,引用您在 dbt 中定义的指标和模型以确保准确性和一致性。

它由您在 dbt 中使用的同一语义层驱动,这意味着每个答案都保持受治理、可解释且快速 — 全部由 ClickHouse 提供支持。
...
要了解将 dbt 项目连接到 Lightdash 的更多信息,请访问 [Lightdash 文档 → ClickHouse 设置](https://docs.lightdash.com/get-started/setup-lightdash/connect-project#clickhouse?utm_source=clickhouse&utm_medium=partner&utm_campaign=integration_docs)。
