...
* **`HEX([my_string])`** *(在 v0.2.1 中添加)* — 返回一个包含参数十六进制表示形式的字符串。等价于 [`hex()`](/sql-reference/functions/encoding-functions/#hex)。
...
* **`REAL([my_number])`** — 将字段转换为浮点类型（Float64）。详情参见[`此处`](/sql-reference/data-types/decimal/#operations-and-result-type)。
