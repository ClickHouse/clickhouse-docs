...
借助云服务商提供的基础设施,EMQX Cloud 服务于全球数十个国家和地区,为 5G 和万物互联应用提供低成本、安全可靠的云服务。
...
- 您已准备好一个 ClickHouse Cloud 实例用于持久化设备数据。
...
如果您是 EMQX Cloud 新用户,请访问 [EMQX Cloud 注册](https://accounts.emqx.com/signup?continue=https%3A%2F%2Fwww.emqx.com%2Fen%2Fcloud) 页面,点击"开始免费试用"注册账户。
...
登录后,在账户菜单下点击"Cloud console",您将看到创建新部署的绿色按钮。
...
选择专业版并选择 `N.Virginial` 区域,然后点击 `Create Now`。只需几分钟,您就能获得一个完全托管的 MQTT 代理:
...
在左侧菜单中点击"Authentication & ACL",然后在子菜单中点击"Authentication"。点击右侧的"Add"按钮,为后续的 MQTT 连接设置用户名和密码。这里我们将使用 `emqx` 和 `xxxxxx` 作为用户名和密码。
...
返回概览页面并向下滚动到页面底部,您将看到 NAT 网关组件。点击 Subscribe 按钮并按照说明操作。请注意,NAT Gateway 是一项增值服务,但同样提供 14 天免费试用。
...
创建完成后,您将在该组件中看到公网 IP 地址。请注意,如果您在 ClickHouse Cloud 设置期间选择了"从特定位置连接",则需要将此 IP 地址添加到白名单中。


## 将 EMQX Cloud 与 ClickHouse Cloud 集成 {#integration-emqx-cloud-with-clickhouse-cloud}

[EMQX Cloud 数据集成](https://docs.emqx.com/en/cloud/latest/rule_engine/introduction.html#general-flow)用于配置处理和响应 EMQX 消息流及设备事件的规则。数据集成不仅提供了清晰灵活的"可配置"架构解决方案,还简化了开发流程,提升了用户体验,并降低了业务系统与 EMQX Cloud 之间的耦合度。同时,它还为定制 EMQX Cloud 的专有功能提供了优质的基础设施支持。
...
EMQX Cloud 提供了与主流数据系统的 30 多种原生集成,ClickHouse 就是其中之一。
...
在左侧菜单中点击"数据集成",然后点击"查看所有资源"。您可以在数据持久化部分找到 ClickHouse,也可以直接搜索 ClickHouse。

点击 ClickHouse 卡片创建新资源。

- 备注:为此资源添加备注说明。
- 服务器地址:您的 ClickHouse Cloud 服务地址,请记得包含端口号。
- 数据库名称:在前面步骤中创建的 `emqx` 数据库。
- 用户:连接 ClickHouse Cloud 服务的用户名。
- 密钥:连接密码。
...
在创建资源时,您会看到一个弹出窗口,点击"新建"将进入规则创建页面。
...
该规则将从 `temp_hum/emqx` 主题读取消息,并通过添加 client_id、topic 和 timestamp 信息来增强 JSON 对象。
...
这是向 ClickHouse 插入数据的模板,您可以看到其中使用了变量。

### 查看规则详情 {#view-rules-details}

点击"确认"和"查看详情"。现在,所有配置都已完成。您可以从规则详情页面查看数据集成的运行情况。
...
我们将模拟温度和湿度数据,通过 MQTT X 将这些数据上报到 EMQX Cloud,然后使用 EMQX Cloud 数据集成功能将数据保存到 ClickHouse Cloud。
...
您可以使用任何 MQTT 客户端或 SDK 来发布消息。在本教程中,我们将使用 [MQTT X](https://mqttx.app/),这是 EMQ 提供的一款用户友好的 MQTT 客户端应用程序。
...
- Name:连接名称。可以使用任意名称。
- Host:MQTT broker 连接地址。您可以从 EMQX Cloud 概览页面获取。
- Port:MQTT broker 连接端口。您可以从 EMQX Cloud 概览页面获取。
- Username/Password:使用上面创建的凭据,在本教程中应为 `emqx` 和 `xxxxxx`。
...
点击右上角的 "Connect" 按钮,连接应该会建立成功。

现在您可以使用此工具向 MQTT broker 发送消息。
输入:
...
点击右侧的发送按钮。您可以更改温度值并向 MQTT broker 发送更多数据。
...
现在可以查看 ClickHouse Cloud 上的数据了。理想情况下,您使用 MQTTX 发送的数据将传输到 EMQX Cloud,并通过原生数据集成功能持久化到 ClickHouse Cloud 的数据库中。
...
您无需编写任何代码,现在就可以将 MQTT 数据从 EMQX Cloud 传输到 ClickHouse Cloud。借助 EMQX Cloud 和 ClickHouse Cloud,您无需管理基础设施,只需专注于编写物联网应用程序,数据将安全地存储在 ClickHouse Cloud 中。
