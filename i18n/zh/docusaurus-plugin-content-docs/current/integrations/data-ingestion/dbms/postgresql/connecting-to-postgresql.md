...
- 'table'
- 'engine'
'description': '页面描述将 PostgreSQL 连接到 ClickHouse 的各种方法'
---
...
# 连接 ClickHouse 和 PostgreSQL

本页面涵盖了将 PostgreSQL 与 ClickHouse 集成的以下选项：

- 使用 [ClickPipes](/integrations/clickpipes/postgres)，由 PeerDB 提供支持的 ClickHouse Cloud 的托管集成服务 - 现已公开测试版！
- 使用 [PeerDB](https://github.com/PeerDB-io/peerdb)，一个专门设计用于 PostgreSQL 数据库复制到自管理的 ClickHouse 和 ClickHouse Cloud 的开源 CDC 工具。
...
2. 创建一个用户以便从 ClickHouse 连接。为了演示，这个例子授予完全的超级用户权限。
```sql
...
6. 要配置 PostgreSQL 以允许使用新用户对新数据库进行复制的连接，请将以下条目添加到 `pg_hba.conf` 文件中。用 PostgreSQL 服务器的子网或 IP 地址更新地址行：
```text
...
查看 ClickHouse [Cloud Endpoints API](/cloud/get-started/query-endpoints) 获取出站流量详情。
:::
...
5. 让我们看看在 ClickHouse 表中添加行会发生什么：
```sql
...
6. 在 ClickHouse 中添加的行应该出现在 PostgreSQL 表中：
```sql
...
这个例子演示了使用 `PostgreSQL` 表引擎在 PostgreSQL 和 ClickHouse 之间的基本集成。
查看 [PostgreSQL 表引擎文档页面](/engines/table-engines/integrations/postgresql) 以了解更多功能，例如指定模式、仅返回子集列和连接多个副本。还可以查看 [ClickHouse 和 PostgreSQL - 数据的天堂似的匹配 - 第 1 部分](https://clickhouse.com/blog/migrating-data-between-clickhouse-postgres) 博客。
...
PostgreSQL 数据库引擎利用 PostgreSQL 的复制功能创建一个数据库的副本，包含所有或部分的模式和表。
本文旨在说明使用一个数据库、一个模式和一个表的集成基本方法。

***在以下过程中，使用 PostgreSQL CLI (psql) 和 ClickHouse CLI (clickhouse-client)。PostgreSQL 服务器安装在 Linux 上。如果 PostgreSQL 数据库是新的测试安装，则以下具有最低设置***
...
wal_level = logical
```
_*ClickHouse 需要最低 `logical` wal 级别和最低 `2` 个复制槽_

2. 使用管理员账户创建一个用户以便从 ClickHouse 连接：
```sql
CREATE ROLE clickhouse_user SUPERUSER LOGIN PASSWORD 'ClickHouse_123';
```
_*为了演示，授予了完全的超级用户权限。*
...
7. 配置 PostgreSQL 以允许使用新用户对新数据库进行复制的连接。以下是需要添加到 `pg_hba.conf` 文件中的最小条目：
...
```
_*为了演示，使用的是明文密码身份验证方法。根据 PostgreSQL 文档更新地址行，使用子网或服务器地址*
...
|user     |连接到 PostgreSQL 的用户名    |clickhouse_user                      |
|password |连接到 PostgreSQL 的密码      |ClickHouse_123                       |
|settings |数据库引擎的附加设置          | materialized_postgresql_tables_list = 'table1'|
...
### 4. 总结 {#4-summary}
本集成指南专注于如何简单地复制一个具有表的数据库，然而，还有更多高级选项，包括复制整个数据库或向现有复制中添加新表和模式。虽然此复制不支持 DDL 命令，但引擎可以设置为检测更改，并在进行结构更改时重新加载表。
...
- 博客: [ClickHouse 和 PostgreSQL - 数据的天堂似的匹配 - 第 1 部分](https://clickhouse.com/blog/migrating-data-between-clickhouse-postgres)
- 博客: [ClickHouse 和 PostgreSQL - 数据的天堂似的匹配 - 第 2 部分](https://clickhouse.com/blog/migrating-data-between-clickhouse-postgres-part-2)
