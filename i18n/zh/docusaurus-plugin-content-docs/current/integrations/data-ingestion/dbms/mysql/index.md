...
本页涵盖使用 `MySQL` 表引擎，从 MySQL 表中读取数据。

:::note
对于 ClickHouse Cloud，您还可以使用 [MySQL ClickPipe](/integrations/clickpipes/mysql) （目前处于私人预览）轻松将数据从 MySQL 表移动到 ClickHouse。
:::

## 使用 MySQL 表引擎将 ClickHouse 连接到 MySQL {#connecting-clickhouse-to-mysql-using-the-mysql-table-engine}

`MySQL` 表引擎允许您将 ClickHouse 连接到 MySQL。可以在 ClickHouse 或 MySQL 表中执行 **SELECT** 和 **INSERT** 语句。本文说明了如何使用 `MySQL` 表引擎的基本方法。
...
5. 根据需要授予权限。（出于演示目的，给 `mysql_clickhouse` 用户授予管理员权限。）
```sql
...
请查看 ClickHouse [Cloud Endpoints API](//cloud/get-started/query-endpoints.md) 获取出站流量详细信息。
:::
...
```
  ...
  :::note
  请查看 [MySQL 表引擎](/engines/table-engines/integrations/mysql.md) 文档页面以获取完整的参数列表。
  :::
...
2. 注意 MySQL 表中的现有行已出现在 ClickHouse 表中，以及您刚刚添加的新行：
```sql
  SELECT
      id,
      column1
