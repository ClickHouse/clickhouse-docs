...
# 将 dlt 连接至 ClickHouse

<PartnerBadge/>

<a href="https://dlthub.com/docs/intro" target="_blank">dlt</a> 是一个开源库，你可以将其添加到 Python 脚本中，将来自各种且往往杂乱的数据源的数据加载为结构良好、实时更新的数据集。
...
:::note
此命令将初始化您的数据管道,使用 chess 作为数据源,ClickHouse 作为目标。
:::
...
要将数据加载到 ClickHouse 中,您需要创建一个 ClickHouse 数据库。以下是您应执行的大致步骤:
...
port = 9000                              # ClickHouse HTTP 端口,默认为 9000
...
```bash
# 将此配置保持在 toml 文件的顶部，在任何 section 开始之前。
...
1. `Clickhouse` 具有实验性的 `object` 数据类型,但我们发现它的行为有些不可预测,因此 dlt clickhouse 目标会将复杂数据类型加载到文本列中。如果您需要此功能,请通过我们的 Slack 社区联系我们,我们会考虑添加该功能。
...
5. `Clickhouse` 在使用 float 或 double 数据类型时,在某些条件下可能会产生舍入误差。如果您无法接受舍入误差,请务必使用 decimal 数据类型。例如,在加载器文件格式设置为 `jsonl` 的情况下,将值 12.7001 加载到 double 列中将可预见地产生舍入误差。
...
dlt 支持在将数据加载到 ClickHouse 时使用 Google Cloud Storage (GCS) 作为暂存区域。这由 ClickHouse 的 <a href="https://clickhouse.com/docs/sql-reference/table-functions/gcs">GCS 表函数</a>自动处理,dlt 在底层使用该函数。
