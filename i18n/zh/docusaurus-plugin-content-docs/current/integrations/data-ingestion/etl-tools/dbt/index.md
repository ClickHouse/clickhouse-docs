...
dbt 提供了多种安装命令行界面 (CLI) 的选项,详细信息请参见[此处](https://docs.getdbt.com/dbt-cli/install/overview)。我们推荐使用 `pip` 安装 dbt 和 dbt-clickhouse。
...
您的 CD 步骤可以简单到只需针对生产 ClickHouse 集群运行 `dbt build`。

#### 更完整的 CI/CD 阶段:使用最新数据,仅测试受影响的模型 {#more-complete-ci-stage}

一种常见的策略是使用 [Slim CI](https://docs.getdbt.com/best-practices/best-practice-workflows#run-only-modified-models-to-test-changes-slim-ci) 作业,其中仅重新部署修改过的模型(及其上游和下游依赖项)。这种方法使用生产运行的构件(即 [dbt manifest](https://docs.getdbt.com/reference/artifacts/manifest-json))来减少项目的运行时间,并确保各环境之间不会出现 schema 漂移。

为了保持开发环境同步并避免针对过时的部署运行模型,您可以使用 [clone](https://docs.getdbt.com/reference/commands/clone) 或 [defer](https://docs.getdbt.com/reference/node-selection/defer)。


我们建议为测试环境（即预发布环境）使用独立的 ClickHouse 集群或服务，以避免影响生产环境的运行。为确保测试环境具有代表性，重要的是要使用生产数据的一个子集，并以一种防止不同环境之间出现 schema 漂移的方式运行 dbt。

- 如果在测试时不需要最新数据，可以将生产数据的备份恢复到预发布环境中。
- 如果在测试时需要最新数据，可以结合使用 [`remoteSecure()` 表函数](/sql-reference/table-functions/remote) 和可刷新的物化视图，以所需的频率写入数据。另一种选择是使用对象存储作为中间层，定期从生产服务写入数据，然后使用对象存储表函数或 ClickPipes（用于持续摄取）将其导入预发布环境。

为 CI 测试使用独立环境还可以让你在不影响生产环境的情况下进行手动测试。例如，你可能希望将某个 BI 工具指向该环境进行测试。

对于部署（即 CD 步骤），我们建议复用生产环境部署生成的构件，仅更新发生变更的模型。为此，需要将对象存储（例如 S3）配置为 dbt 构件的中间存储。一旦完成配置，你可以运行类似 `dbt build --select state:modified+ --state path/to/last/deploy/state.json` 的命令，根据自上次在生产中运行以来的变更，有选择地最小化地重建所需的模型。
...
- 如果在 ClickHouse 插入语句中将 Ephemeral 模型/CTE 放置在 `INSERT INTO` 之前,它们将无法工作,请参见 https://github.com/ClickHouse/ClickHouse/issues/30323。这不应影响大多数模型,但在模型定义和其他 SQL 语句中放置 Ephemeral 模型时应谨慎。<!-- TODO review this limitation, looks like the issue was already closed and the fix was introduced in 24.10 -->
