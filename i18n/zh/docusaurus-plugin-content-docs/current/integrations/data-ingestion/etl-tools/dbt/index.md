...
| 页面                                                                        | 描述                                             |
| ------------------------------------------------------------------------- | ---------------------------------------------- |
| [功能和配置](/integrations/dbt/features-and-configurations)                    | 可用功能与通用配置概述                                    |
| [物化类型](/integrations/dbt/materializations)                                | 可用的 物化 类型及其配置                     |
| [指南](/integrations/dbt/guides)                                            | 将 dbt 与 ClickHouse 结合使用的指南                     |
| [Materialized views](/integrations/dbt/materialization-materialized-view) | 关于 materialized&#95;view 物化 的专门文档 |
...
dbt 引入了 模型 的概念。它被定义为一条 SQL 语句，可能会关联多张表。一个 模型 可以通过多种方式进行“物化”。物化表示该 模型 的 SELECT 查询的构建策略。物化背后的代码是样板 SQL 代码，它会将你的 SELECT 查询包装在一条语句中，以便创建新的关系对象或更新已有的关系对象。

dbt 提供 5 种物化方式，`dbt-clickhouse` 全部支持：

* **view** (默认) ：模型 在数据库中以 view 的形式构建。在 ClickHouse 中，它会构建为一个 [view](/sql-reference/statements/create/view)。
* **table**：模型 在数据库中以表的形式构建。在 ClickHouse 中，它会构建为一张 [table](/sql-reference/statements/create/table)。
* **ephemeral**：模型 不会直接在数据库中构建，而是作为 CTE (Common Table Expressions，公用表表达式) 被内联进依赖它的其他 模型 中。
* **incremental**：模型 起初会以表的形式物化，在后续运行中，dbt 会向该表插入新增行并更新发生变化的行。
* **materialized view**：模型 在数据库中构建为 materialized view。在 ClickHouse 中，它会构建为一个 [materialized view](/sql-reference/statements/create/view#materialized-view)。

还可以通过额外的语法和子句来定义当底层数据发生变化时，这些 模型 应如何更新。dbt 一般建议先从 view 物化开始使用，直到性能成为瓶颈。table 物化通过将 模型 查询的结果保存为一张表，在增加存储开销的前提下，提供了查询时的性能提升。incremental 方式在此基础上进一步扩展，使得对底层数据的后续更新可以被捕获到目标表中。

当前用于 ClickHouse 的[适配器](https://github.com/silentsokolov/dbt-clickhouse)还额外支持 **dictionary**、**分布式表** 和 **distributed incremental** 物化方式。该适配器还支持 dbt 的 [snapshots](https://docs.getdbt.com/docs/building-a-dbt-project/snapshots#check-strategy) 和 [seeds](https://docs.getdbt.com/docs/building-a-dbt-project/seeds)。

以下是在 `dbt-clickhouse` 中的[实验特性](/beta-and-experimental-features)：

| 类型                         | 是否支持                       | 详情                                                                                                                                                       |
| -------------------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Materialized View 物化       | 是。使用显式 target 创建处于 Beta 阶段 | 创建一个 [materialized view](/sql-reference/statements/create/view#materialized-view)。                                                                       |
| 分布式表 物化       | 是，Experimental             | 创建一个 [分布式表](/engines/table-engines/special/distributed)。                                                                                    |
| Distributed incremental 物化 | 是，Experimental             | 基于与 分布式表 相同理念的 incremental 模型。请注意，并非所有策略都受支持，更多信息请参见[其文档章节](/integrations/dbt/materializations#materialization-distributed-incremental)。 |
| Dictionary 物化              | 是，Experimental             | 创建一个 [dictionary](/engines/table-engines/special/dictionary)。                                                                                            |
...
- 引擎必须是[受支持的引擎](/integrations/dbt/materializations#supported-table-engines)之一。
- 必须具备足够的权限才能访问该数据库。
