...
keywords: ['vector', '日志收集', '可观测性', '数据摄取', '数据管道']
---
...
1. 以下 `access_log` 属性将日志以 **combined** 格式发送到 `/var/log/nginx/my_access.log`。
...
1. 以下 **vector.toml** 文件定义了一个类型为 **file** 的 **source**,用于追踪 **my_access.log** 文件的末尾内容,同时还定义了一个 **sink** 指向上面定义的 **access_logs** 表:
...
2. 使用上述配置启动 Vector。访问 Vector [文档](https://vector.dev/docs/)以获取有关定义 sources 和 sinks 的更多详细信息。
