...
description: '使用 BladePipe 数据管道将数据流式写入 ClickHouse'
...
<a href="https://www.bladepipe.com/" target="_blank">BladePipe</a> 是一款具备亚秒级延迟的实时端到端数据集成工具，可在各个平台之间实现无缝数据流转。 

ClickHouse 是 BladePipe 的内置连接器之一，使用户可以将来自各种数据源的数据自动集成到 ClickHouse 中。本文将逐步演示如何实时向 ClickHouse 加载数据。
...
2. 要将 ClickHouse 用作目标,请确保用户具有 SELECT、INSERT 和常用 DDL 权限。
   :::

3. 在 BladePipe 中,点击"DataSource">"Add DataSource"。

4. 选择 `ClickHouse`,填写 ClickHouse 主机和端口、用户名和密码等配置信息,然后点击"Test Connection"。
...
在本教程中,我们使用 MySQL 实例作为数据源,并介绍将 MySQL 数据加载到 ClickHouse 的过程。
...
2. 选择已添加的 MySQL 和 ClickHouse 实例,然后点击"Test Connection"以确保 BladePipe 已连接到这些实例。接着,选择要迁移的数据库。
