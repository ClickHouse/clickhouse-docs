...
title: '将 BladePipe 接入 ClickHouse'
doc_type: 'guide'
---
...
<a href="https://www.bladepipe.com/" target="_blank">BladePipe</a> 是一款具有亚秒级延迟的实时端到端数据集成工具，可在各个平台之间实现无缝数据流动。 

ClickHouse 是 BladePipe 内置连接器之一，用户可以借此将来自各类数据源的数据自动集成到 ClickHouse 中。本文将逐步演示如何实时将数据加载到 ClickHouse 中。
...
2. 按照 <a href="https://doc.bladepipe.com/productOP/byoc/installation/install_worker_docker" target="_blank">安装 Worker (Docker)</a> 或 <a href="https://doc.bladepipe.com/productOP/byoc/installation/install_worker_binary" target="_blank">安装 Worker (二进制)</a> 中的说明下载并安装 BladePipe Worker。
...
1. BladePipe 支持 ClickHouse 版本 `20.12.3.3` 及以上。
...
5. 点击底部的 "Add DataSource",即可添加 ClickHouse 实例。
...
2. 选择 `MySQL`,填写 MySQL 主机和端口、用户名和密码等配置信息,然后点击"Test Connection"。
...
2. 选择已添加的 MySQL 和 ClickHouse 实例,点击"Test Connection"确保 BladePipe 已连接到这些实例。然后,选择要迁移的数据库。
...
1. 停止 MySQL 实例的数据写入,并等待 ClickHouse 合并数据。
   :::note
   由于 ClickHouse 自动合并的时机不可预测,您可以通过运行 `OPTIMIZE TABLE xxx FINAL;` 命令手动触发合并。请注意,手动合并不一定总能成功。

或者,您可以运行 `CREATE VIEW xxx_v AS SELECT * FROM xxx FINAL;` 命令创建视图,并在该视图上执行查询以确保数据已完全合并。
:::
