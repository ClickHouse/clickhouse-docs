...
description: 'Azure Synapse 与 ClickHouse 集成简介'
...
[Azure Synapse](https://azure.microsoft.com/en-us/products/synapse-analytics) 是一项集成分析服务，将大数据、数据科学和数据仓库融合在一起，以实现快速、大规模的数据分析。
在 Synapse 中，Spark 池提供按需、可扩展的 [Apache Spark](https://spark.apache.org) 集群，使用户能够运行复杂的数据转换、机器学习任务，以及与外部系统的集成。

本文将说明在 Azure Synapse 中使用 Apache Spark 时，如何集成 [ClickHouse Spark 连接器](/integrations/apache-spark/spark-native-connector)。
...
:::info
使用 ClickHouse Cloud 时,请确保设置[所需的 Spark 设置](/integrations/apache-spark/spark-native-connector#clickhouse-cloud-settings)。  
:::


## 设置验证 {#setup-verification}

要验证依赖项和配置是否已成功设置,请访问您会话的 Spark UI,并进入 `Environment` 选项卡。
