...
此连接器利用 ClickHouse 特有的优化功能，例如高级分区和谓词下推，以提升查询性能和数据处理效率。该连接器基于 [ClickHouse 官方 JDBC 连接器](https://github.com/ClickHouse/clickhouse-java)，并管理其自身的 catalog。

在 Spark 3.0 之前，Spark 缺少内置的 catalog 概念，因此用户通常依赖 Hive Metastore 或 AWS Glue 等外部 catalog 系统。使用这些外部方案时，用户必须在 Spark 中访问数据源表之前手动注册这些表。自从 Spark 3.0 引入 catalog 概念后，Spark 现在可以通过注册 catalog 插件自动发现数据表。

Spark 的默认 catalog 为 `spark_catalog`，表通过 `{catalog name}.{database}.{table}` 进行标识。借助新的 catalog 功能，现在可以在单个 Spark 应用中添加并使用多个 catalog。

## 在 Catalog API 和 TableProvider API 之间进行选择 {#choosing-between-apis}

ClickHouse Spark 连接器支持两种访问模式：**Catalog API** 和 **TableProvider API**（基于格式的访问）。理解二者的差异有助于为您的使用场景选择合适的方案。

### Catalog API 与 TableProvider API 对比 {#catalog-vs-tableprovider-comparison}

| 功能 | Catalog API | TableProvider API |
|---------|-------------|-------------------|
| **配置方式** | 通过 Spark 配置集中管理 | 通过每次操作的选项单独配置 |
| **表发现** | 通过 catalog 自动发现 | 需要手动指定表 |
| **DDL 操作** | 完整支持（CREATE、DROP、ALTER） | 支持有限（仅自动创建表） |
| **Spark SQL 集成** | 原生支持（`clickhouse.database.table`） | 需要指定数据格式（format） |
| **适用场景** | 长期、稳定连接并集中管理配置 | 临时、动态或短期访问 |
...
| 0.4.0   | Spark 3.2, 3.3            | 无依赖项                |
| 0.3.0   | Spark 3.2, 3.3            | 无依赖项                |
| 0.2.1   | Spark 3.2                 | 无依赖项                |
| 0.1.2   | Spark 3.2                 | 无依赖项                |
...
或者，你也可以将所需的 JAR 文件放入 `$SPARK_HOME/jars/` 目录，或在使用 `spark-submit` 命令时，通过 `--jars` 参数将它们作为 Spark 选项直接传入。
这两种方式都能确保 ClickHouse connector 在 Spark 环境中可用。
...
在使用 Spark 的 Shell 工具（Spark SQL CLI、Spark Shell CLI 和 spark-submit 命令）时，可以通过传入所需的 JAR 来注册依赖：
...
注意：对于仅使用 SQL 的使用场景，推荐在生产环境中使用 [Apache Kyuubi](https://github.com/apache/kyuubi)。
...
在 [Sonatype OSS Snapshots Repository](https://s01.oss.sonatype.org/content/repositories/snapshots/com/clickhouse/) 中找到所有每日构建的 SNAPSHOT JAR 文件。

:::important
请务必包含带有 `all` 分类器的 [clickhouse-jdbc JAR](https://mvnrepository.com/artifact/com.clickhouse/clickhouse-jdbc)，
因为该 connector 依赖于 [clickhouse-http](https://mvnrepository.com/artifact/com.clickhouse/clickhouse-http-client)
...
要访问你的 ClickHouse 表，必须使用以下配置新建一个 Spark catalog：
...
* 将配置传递给你的 `spark-submit` 命令（或 `spark-shell`/`spark-sql` CLI 命令）。
* 在初始化上下文时添加配置。

:::important
在使用 ClickHouse 集群时，你需要为每个实例设置唯一的 catalog 名称。
例如：
...
通过这种方式，你可以在 Spark SQL 中通过 `clickhouse1.<ck_db>.<ck_table>` 访问 clickhouse1 中的表 `<ck_db>.<ck_table>`，并通过 `clickhouse2.<ck_db>.<ck_table>` 访问 clickhouse2 中的表 `<ck_db>.<ck_table>`。

:::

## 使用 TableProvider API（基于格式的访问） {#using-the-tableprovider-api}

除了基于目录的方式之外，ClickHouse Spark 连接器还通过 TableProvider API 提供**基于格式的访问模式**。
...
TableProvider API 提供了多项强大的特性：

#### 自动创建数据表 {#automatic-table-creation}

当向一个不存在的表写入数据时，connector 会根据合适的 schema 自动创建该表。该 connector 提供了智能的默认配置：

* **Engine**：如果未指定，默认使用 `MergeTree()`。可以通过 `engine` 选项指定其他 engine（例如 `ReplacingMergeTree()`, `SummingMergeTree()` 等）
* **ORDER BY**：**必需** —— 创建新表时必须显式指定 `order_by` 选项。connector 会校验所有指定的列都存在于 schema 中。
...
:::important
**必须指定 ORDER BY**：通过 TableProvider API 创建新表时，`order_by` 选项是**必填**的。你必须显式指定用于 ORDER BY 子句的列（或列集）。连接器会验证所有指定列是否存在于 schema 中，如果有任意列缺失，将抛出错误。
...
使用基于格式的 API 时，可以使用以下连接选项：

#### 连接选项 {#connection-options}

| 选项         | 描述                                             | 默认值         | 必填     |
|--------------|--------------------------------------------------|----------------|----------|
| `host`       | ClickHouse 服务器的主机名                        | `localhost`    | 是       |
...
| `timezone`   | 日期/时间操作使用的时区                         | `server`       | 否       |
...
|-----------------------------|-----------------------------------------------------------------------------|-------------------|----------|
| `order_by`                  | 用于 ORDER BY 子句的列。多个列时使用逗号分隔                                 | N/A               | **Yes**  |
| `engine`                    | ClickHouse 表引擎（例如 `MergeTree()`, `ReplacingMergeTree()`, `SummingMergeTree()` 等） | `MergeTree()`     | No       |
| `settings.allow_nullable_key` | 允许在 ORDER BY 中使用 Nullable 键（用于 ClickHouse Cloud）              | Auto-detected**   | No       |
| `settings.<key>`            | 任意 ClickHouse 表设置                                                     | N/A               | No       |
| `cluster`                   | 分布式表使用的集群名称                                                      | N/A               | No       |
| `clickhouse.column.<name>.variant_types` | Variant 列对应的 ClickHouse 类型列表，使用逗号分隔（例如 `String, Int64, Bool, JSON`）。类型名称区分大小写。逗号后的空格可选。 | N/A | No |

\* 在创建新表时，`order_by` 选项是必需的。所有指定的列必须存在于 schema 中。  
\** 如果 ORDER BY 包含 Nullable 列且未显式提供此选项，则会自动设置为 `1`。

:::tip
**最佳实践**：对于 ClickHouse Cloud，如果你的 ORDER BY 列可能为 Nullable，建议显式设置 `settings.allow_nullable_key=1`，因为 ClickHouse Cloud 需要启用此设置。
:::
...
* **`append`**：向已有表中追加数据
* **`overwrite`**：替换表中的所有数据（清空表）

:::important
**不支持分区覆盖写入**：该连接器当前不支持分区级别的覆盖写入操作（例如，将 `overwrite` 模式与 `partitionBy` 一起使用时）。该功能正在开发中。有关该功能的进展，请参阅 [GitHub issue #34](https://github.com/ClickHouse/spark-clickhouse-connector/issues/34)。
:::
...
Catalog API 和 TableProvider API 都支持配置 ClickHouse 特有的选项（而非连接器选项）。在创建表或执行查询时，这些选项会被传递给 ClickHouse。

ClickHouse 选项允许您配置 ClickHouse 特有的设置，例如 `allow_nullable_key`、`index_granularity`，以及其他表级或查询级设置。它们不同于连接器选项（例如 `host`、`database`、`table`），后者用于控制连接器如何连接到 ClickHouse。

### 使用 TableProvider API {#using-tableprovider-api-options}

在使用 TableProvider API 时，请采用 `settings.&lt;key&gt;` 选项格式：
...
使用 Catalog API 时，请在 Spark 配置中使用 `spark.sql.catalog.<catalog_name>.option.<key>` 格式：
...
在连接到 [ClickHouse Cloud](https://clickhouse.com) 时，请确保启用 SSL 并配置合适的 SSL 模式。例如：
...
:::important
**不支持分区覆盖写入**：Catalog API 当前不支持分区级别的覆盖写入操作（例如使用带有 `partitionBy` 的 `overwrite` 模式）。该功能正在开发中。请参见 [GitHub issue #34](https://github.com/ClickHouse/spark-clickhouse-connector/issues/34) 以了解该功能的最新进展。
:::
...
可以使用 Spark SQL 在 ClickHouse 实例上执行 DDL 操作，所有更改会立即持久化到
ClickHouse 中。
Spark SQL 允许以与在 ClickHouse 中完全相同的方式编写查询，
...
上述示例展示了 Spark SQL 查询，您可以在应用程序中通过任一 API（Java、Scala、PySpark 或 shell）来运行这些查询。


## 使用 VariantType {#working-with-varianttype}

:::note
VariantType 在 Spark 4.0 及以上版本中可用，并且需要在启用了实验性 JSON/Variant 类型的 ClickHouse 25.3 及以上版本中使用。
:::

该连接器支持 Spark 的 `VariantType` 用于处理半结构化数据。VariantType 会映射到 ClickHouse 的 `JSON` 和 `Variant` 类型，从而能够高效地存储和查询模式灵活的数据。

:::note
本节专门介绍 VariantType 的映射和用法。若要查看所有受支持数据类型的完整概览，请参阅[受支持的数据类型](#supported-data-types)一节。
:::

### ClickHouse 类型映射 {#clickhouse-type-mapping}

| ClickHouse Type | Spark Type | 描述 |
|----------------|------------|------|
...
从 ClickHouse 读取数据时，`JSON` 和 `Variant` 列会自动映射为 Spark 的 `VariantType`：
...
可以通过 JSON 或 Variant 列类型将 VariantType 数据写入 ClickHouse：
...
可以使用 Spark SQL 的 DDL 语句创建 VariantType 表：
...
如果未指定 `variant_types` 属性，该列将默认为 ClickHouse 的 `JSON` 类型，该类型只接受 JSON 对象：
...
#### 支持多种类型的 Variant 类型 {#variant-type-multiple-types}

若要支持基本类型、数组和 JSON 对象，请在 `variant_types` 属性中指定类型：
...
- **原生类型（Primitives）**：`String`、`Int8`、`Int16`、`Int32`、`Int64`、`UInt8`、`UInt16`、`UInt32`、`UInt64`、`Float32`、`Float64`、`Bool`
- **数组（Arrays）**：`Array(T)`，其中 T 为任意受支持的类型，包括嵌套数组
...
VariantType 的写入支持因格式而异：

| 格式    | 支持情况    | 说明                                                                                                                             |
| ----- | ------- | ------------------------------------------------------------------------------------------------------------------------------ |
| JSON  | ✅ 完全支持  | 同时支持 `JSON` 和 `Variant` 类型。推荐用于 VariantType 类型数据                                                                               |
| Arrow | ⚠️ 部分支持 | 支持写入 ClickHouse 的 `JSON` 类型。不支持 ClickHouse 的 `Variant` 类型。完整支持将取决于 https://github.com/ClickHouse/ClickHouse/issues/92752 的解决进展 |
...
:::tip
如果需要向 ClickHouse 的 `Variant` 类型写入数据，请使用 JSON 格式。Arrow 格式仅支持写入 `JSON` 类型。
:::


### 最佳实践 {#varianttype-best-practices}

1. **针对仅包含 JSON 的数据使用 JSON 类型**：如果只存储 JSON 对象，请使用默认的 JSON 类型（不要设置 `variant_types` 属性）
2. **显式指定类型**：使用 `Variant()` 时，显式列出计划存储的所有类型
3. **启用实验特性**：确保在 ClickHouse 中启用了 `allow_experimental_json_type = 1`
4. **写入时使用 JSON 格式**：对于 VariantType 数据，推荐使用 JSON 格式进行写入，以获得更好的兼容性
5. **根据查询模式进行考虑**：JSON/Variant 类型支持 ClickHouse 的 JSON 路径查询，可用于高效过滤
6. **通过列提示提升性能**：在 ClickHouse 中使用 JSON 字段时，添加列提示可以提升查询性能。目前尚不支持通过 Spark 添加列提示。关于该特性的进展请参阅 [GitHub issue #497](https://github.com/ClickHouse/spark-clickhouse-connector/issues/497)。
...
:::note
**使用配置**：这些是 Spark 级别的配置选项，适用于 Catalog API 和 TableProvider API。可以通过以下两种方式进行设置：
...
| 键                                                  | 默认值                                                | 描述                                                                                                                                                                                                 | 自版本起  |
| -------------------------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| spark.clickhouse.ignoreUnsupportedTransform        | false                                              | ClickHouse 支持使用复杂表达式作为分片键或分区值，例如 `cityHash64(col_1, col_2)`，但 Spark 当前不支持这些用法。若为 `true`，则忽略这些不受支持的表达式；否则会快速失败并抛出异常。注意，当启用 `spark.clickhouse.write.distributed.convertLocal` 时，忽略不受支持的分片键可能会导致数据损坏。 | 0.4.0 |
| spark.clickhouse.read.compression.codec            | lz4                                                | 用于在读取数据时执行解压的编解码器。支持的编解码器：none、lz4。                                                                                                                                                                | 0.5.0 |
| spark.clickhouse.read.distributed.convertLocal     | true                                               | 读取分布式表时，改为读取对应的本地表而不是分布式表本身。若设置为 `true`，则忽略 `spark.clickhouse.read.distributed.useClusterNodes`。                                                                                                   | 0.1.0 |
| spark.clickhouse.read.fixedStringAs                | binary                                             | 按指定的 Spark 数据类型读取 ClickHouse 的 FixedString 类型。支持的类型：binary、string                                                                                                                                  | 0.8.0 |
...
| spark.clickhouse.read.splitByPartitionId           | true                                               | 如果为 `true`，则基于虚拟列 `_partition_id` 而不是分区值来构造输入分区过滤条件。当前在基于分区值组装 SQL 谓词时存在已知问题。此功能要求 ClickHouse Server v21.6 及更高版本。                                                                                  | 0.4.0 |
| spark.clickhouse.useNullableQuerySchema            | false                                              | 如果为 `true`，则在通过执行 `CREATE/REPLACE TABLE ... AS SELECT ...` 创建表时，将查询的 schema 中所有字段标记为 Nullable。注意，此配置依赖于 SPARK-43390（在 Spark 3.5 中可用），否则其行为始终等同于 `true`。                                            | 0.8.0 |
| spark.clickhouse.write.batchSize                   | 10000                                              | 写入 ClickHouse 时每个批次包含的记录数。                                                                                                                                                                         | 0.1.0 |
| spark.clickhouse.write.compression.codec           | lz4                                                | 用于写出数据时的压缩编解码器。支持的编解码器：none、lz4。                                                                                                                                                                   | 0.3.0 |
| spark.clickhouse.write.distributed.convertLocal    | false                                              | 在写入分布式表时，将数据写入对应的本地表，而不是分布式表本身。若为 `true`，则忽略 `spark.clickhouse.write.distributed.useClusterNodes`。                                                                                                 | 0.1.0 |
| spark.clickhouse.write.distributed.useClusterNodes | true                                               | 在向分布式表写入数据时，将数据写入集群的所有节点。                                                                                                                                                                          | 0.1.0 |
...
| spark.clickhouse.write.localSortByPartition        | 与 spark.clickhouse.write.repartitionByPartition 相同 | 如果为 `true`，则会在写入前按分区进行本地排序。如果未设置，则等同于 `spark.clickhouse.write.repartitionByPartition`。                                                                                                             | 0.3.0 |
| spark.clickhouse.write.maxRetry                    | 3                                                  | 单个批量写入因可重试错误码失败时的最大重试次数。                                                                                                                                                                           | 0.1.0 |
| spark.clickhouse.write.repartitionByPartition      | true                                               | 是否在写入前按照 ClickHouse 的分区键对数据重新分区，以符合 ClickHouse 表的分布。                                                                                                                                               | 0.3.0 |
| spark.clickhouse.write.repartitionNum              | 0                                                  | 若需要在写入前对数据重新分区以满足 ClickHouse 表的分布要求，可使用此配置指定重新分区的分区数；当该值小于 1 时，表示无此要求。                                                                                                                             | 0.1.0 |
| spark.clickhouse.write.repartitionStrictly         | false                                              | 如果为 `true`，Spark 会在写入前将传入记录严格地分布到各个分区，以满足所需的数据分布要求，然后再将记录传递给数据源表。否则，Spark 可能会应用某些优化来加速查询，但可能会破坏该分布要求。注意，此配置依赖 SPARK-37523（在 Spark 3.4 中提供），如果没有应用该补丁，则其行为始终等同于 `true`。                             | 0.3.0 |
...
本节概述了 Spark 与 ClickHouse 之间的数据类型映射关系。以下表格可作为快速参考，用于在从 ClickHouse 读取数据到 Spark，以及从 Spark 向 ClickHouse 插入数据时进行数据类型转换。

### 从 ClickHouse 读取数据到 Spark {#reading-data-from-clickhouse-into-spark}

| ClickHouse 数据类型                                               | Spark 数据类型                 | 支持      | 是否为基础类型 | 说明                                               |
|-------------------------------------------------------------------|--------------------------------|-----------|----------------|----------------------------------------------------|
| `Nothing`                                                         | `NullType`                     | ✅         | 是             |                                                    |
| `Bool`                                                            | `BooleanType`                  | ✅         | 是             |                           |
...
| `Decimal`                                                         | `DecimalType`                  | ✅         | 是             | 精度和小数位数最高到 `Decimal128`                  |
...
| `Map`                                                             | `MapType`                      | ✅         | 否             | 键类型被限制为 `StringType`                        |
| `IntervalYear`                                                    | `YearMonthIntervalType(Year)`  | ✅         | 是             |                                                    |
| `IntervalMonth`                                                   | `YearMonthIntervalType(Month)` | ✅         | 是             |                                                    |
| `IntervalDay`, `IntervalHour`, `IntervalMinute`, `IntervalSecond` | `DayTimeIntervalType`          | ✅         | 否             | 会使用对应的具体 interval 子类型                   |
| `JSON`, `Variant`                                                 | `VariantType`                  | ✅         | 否             | 需要 Spark 4.0+ 和 ClickHouse 25.3+。可通过 `spark.clickhouse.read.jsonAs=string` 以 `StringType` 读取 |
| `Object`                                                          |                                | ❌         |                |                                                    |
| `Nested`                                                          |                                | ❌         |                |                                                    |
| `Tuple`                                                           | `StructType`                   | ✅         | 否             | 支持具名和匿名 tuple。具名 tuple 按名称映射到 struct 字段，匿名 tuple 使用 `_1`、`_2` 等。支持嵌套 struct 和可为空（Nullable）的字段 |
| `Point`                                                           |                                | ❌         |                |                                                    |
| `Polygon`                                                         |                                | ❌         |                |                                                    |
| `MultiPolygon`                                                    |                                | ❌         |                |                                                    |
| `Ring`                                                            |                                | ❌         |                |                                                    |
...
| Spark 数据类型                      | ClickHouse 数据类型 | 是否支持 | 是否为基本类型 | 备注                                   |
|-------------------------------------|----------------------|-----------|--------------|----------------------------------------|
...
| `CharType`                          | `String`             | ✅         | 是           |                                        |
| `DecimalType`                       | `Decimal(p, s)`      | ✅         | 是           | 精度和小数位最高支持到 `Decimal128`    |
| `DateType`                          | `Date`               | ✅         | 是           |                                        |
...
| `VariantType`                       | `JSON` or `Variant`  | ✅         | 否           | 需要 Spark 4.0+ 和 ClickHouse 25.3+。默认为 `JSON` 类型。使用 `clickhouse.column.<name>.variant_types` 属性指定包含多种类型的 `Variant`。 |
| `Object`                            |                      | ❌         |              |                                        |
| `Nested`                            |                      | ❌         |              |                                        |
...
请访问我们的 [GitHub 仓库](https://github.com/ClickHouse/spark-clickhouse-connector) 来提交 issue、提出改进建议，
或创建 pull request。
我们欢迎一切贡献！在开始之前，请先查看仓库中的贡献指南。
