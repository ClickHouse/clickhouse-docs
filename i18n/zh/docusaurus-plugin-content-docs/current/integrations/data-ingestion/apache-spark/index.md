...
1. [Spark Connector](./apache-spark/spark-native-connector) - Spark Connector 实现了 `DataSourceV2` 接口，并具有独立的 Catalog 管理。目前，这是集成 ClickHouse 和 Spark 的推荐方式。
...
这两种方案都已成功通过测试，并与包括 Java、Scala、PySpark 和 Spark SQL 在内的多种 API 完全兼容。