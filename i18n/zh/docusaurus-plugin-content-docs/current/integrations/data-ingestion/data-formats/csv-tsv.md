...
ClickHouse 支持从 CSV 导入数据并导出为 CSV。由于 CSV 文件在格式上可能各不相同，包括首行列名、自定义分隔符以及转义字符，ClickHouse 提供了相应的格式和设置，以高效应对每一种情况。
...
这里,我们使用 `FORMAT CSV` 子句让 ClickHouse 识别文件格式。我们还可以使用 [url()](/sql-reference/table-functions/url.md) 函数直接从 URL 加载数据,或使用 [s3()](/sql-reference/table-functions/s3.md) 函数从 S3 文件加载数据。
...
在这种情况下,ClickHouse 在导入数据时会跳过文件的第一行。
...
如果我们从该文件加载数据，ClickHouse 会将 `Nothing` 视为 String 类型（这是正确的）：
...
如果我们希望 ClickHouse 将 `Nothing` 视为 `NULL`，可以通过以下选项进行设置：

```sql
SET format_csv_null_representation = 'Nothing'
```

现在 `NULL` 已经出现在我们期望的位置：
...
此外还有 [TabSeparatedWithNames](/interfaces/formats/TabSeparatedWithNames) 格式,用于处理带有标题行的 TSV 文件。与 CSV 类似,可以使用 [input_format_tsv_skip_first_lines](/operations/settings/settings-formats.md/#input_format_tsv_skip_first_lines) 选项跳过前 X 行。
...
注意 ClickHouse 仅用 **约 1** 秒就将 3600 万行数据保存到 CSV 文件中。
...
现在 ClickHouse 根据(第二个)标题行识别列类型,而不是进行推断。


## 自定义分隔符、分隔符和转义规则 {#custom-delimiters-separators-and-escaping-rules}

在复杂场景中,文本数据可能采用高度自定义的格式,但仍然具有结构。ClickHouse 为此类场景提供了专门的 [CustomSeparated](/interfaces/formats/CustomSeparated) 格式,允许设置自定义转义规则、分隔符、行分隔符以及起始/结束符号。
...
我们还可以使用 [CustomSeparatedWithNames](/interfaces/formats/CustomSeparatedWithNames) 来正确导出和导入表头。如需处理更复杂的场景,请参阅[正则表达式和模板](templates-regex.md)格式。


## 处理大型 CSV 文件 {#working-with-large-csv-files}

CSV 文件可能很大,ClickHouse 能够高效处理任意大小的文件。大型文件通常以压缩格式提供,ClickHouse 可以直接处理,无需在处理前解压缩。在插入数据时可以使用 `COMPRESSION` 子句:
...
如果省略 `COMPRESSION` 子句,ClickHouse 仍会尝试根据文件扩展名推断压缩格式。同样的方法也可用于直接将文件导出为压缩格式:
