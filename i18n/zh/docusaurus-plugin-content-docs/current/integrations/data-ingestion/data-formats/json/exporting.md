...
description: '如何从 ClickHouse 导出 JSON 格式数据'
...
几乎任何用于导入的 JSON 格式也都可以用于导出。最常用的是 [`JSONEachRow`](/interfaces/formats/JSONEachRow)：
...
也可以使用 [`JSONCompactEachRow`](/interfaces/formats/JSONCompactEachRow)，通过省略列名来节省磁盘空间：
...
获取数据及其结构的更高效方式是使用 [`JSONCompactEachRowWithNamesAndTypes`](/interfaces/formats/JSONCompactEachRowWithNamesAndTypes) 格式:
...
该格式使用紧凑的 JSON 格式,并在开头添加包含列名和类型的两个标题行。此格式可用于将数据导入到另一个 ClickHouse 实例(或其他应用程序)中。
