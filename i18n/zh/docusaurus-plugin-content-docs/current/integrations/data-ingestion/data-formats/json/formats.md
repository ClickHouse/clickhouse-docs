...
前面的加载 JSON 数据示例假设使用 [`JSONEachRow`](/interfaces/formats/JSONEachRow)（`NDJSON`）。此格式会将每一行 JSON 中的键作为列来读取。例如：
...
虽然这通常是 JSON 最常用的格式，但用户也可能遇到其他格式，或者需要将整个 JSON 作为单个对象来读取。
...
我们之前的示例展示了 `JSONEachRow` 如何读取换行符分隔的 JSON,其中每一行作为单独的对象读取并映射到表的一行,每个键映射到一列。这对于 JSON 结构可预测且每列具有单一类型的场景非常理想。

相比之下,`JSONAsObject` 将每一行视为单个 `JSON` 对象,并将其存储在类型为 [`JSON`](/sql-reference/data-types/newjson) 的单列中,这使其更适合处理嵌套的 JSON 数据以及键是动态的且可能具有多种类型的场景。

对于逐行插入,使用 `JSONEachRow`;对于存储灵活或动态的 JSON 数据,使用 [`JSONAsObject`](/interfaces/formats/JSONAsObject)。

将上述示例与以下查询进行对比,该查询将相同的数据作为每行一个 JSON 对象读取:
...
`JSONAsObject` 对于使用单个 JSON 对象列向表中插入行非常有用,例如:
...
`JSONAsObject` 格式对于读取对象结构不一致的换行符分隔 JSON 也很有用。例如,如果某个键在不同行中的类型不同(有时可能是字符串,有时则是对象)。在这种情况下,ClickHouse 无法使用 `JSONEachRow` 推断出稳定的模式,而 `JSONAsObject` 允许在不进行严格类型约束的情况下摄取数据,将每个 JSON 行作为整体存储在单列中。例如,请注意 `JSONEachRow` 在以下示例中如何失败:
...
从服务器收到异常（版本 24.12.1）：
代码：636。DB::Exception: 来自 sql-clickhouse.clickhouse.com:9440 的响应。DB::Exception: 无法从 JSONEachRow 格式文件中提取表结构。错误：
代码：117。DB::Exception: JSON 对象中存在歧义数据：在某些对象中，路径 &#39;record.subject&#39; 的类型为 &#39;String&#39;，而在另一些对象中为 &#39;Tuple(`$type` String, cid String, uri String)&#39;。您可以启用设置 input&#95;format&#95;json&#95;use&#95;string&#95;type&#95;for&#95;ambiguous&#95;paths&#95;in&#95;named&#95;tuples&#95;inference&#95;from&#95;objects，以便对路径 &#39;record.subject&#39; 使用 String 类型。(INCORRECT&#95;DATA) (version 24.12.1.18239 (official build))
要增加为结构推断而读取的最大行数/字节数，请使用设置 input&#95;format&#95;max&#95;rows&#95;to&#95;read&#95;for&#95;schema&#95;inference/input&#95;format&#95;max&#95;bytes&#95;to&#95;read&#95;for&#95;schema&#95;inference。
您可以手动指定结构：（在文件/URI bluesky/file&#95;0001.json.gz 中）。(CANNOT&#95;EXTRACT&#95;TABLE&#95;STRUCTURE)
...
最常见的 JSON 数据形式之一是在 JSON 数组中包含 JSON 对象列表,如[此示例](../assets/list.json)所示:
...
我们使用 [FROM INFILE](/sql-reference/statements/insert-into.md/#inserting-data-from-a-file) 子句从本地文件加载数据,可以看到导入成功:
...
现在,我们可以使用 [`file()`](/sql-reference/functions/files.md/#file) 函数检查将从原始 JSON 文件中加载的数据:
...
在处理[列数组](../assets/columns-array.json)而非对象时,还支持使用 [`JSONCompactColumns`](/interfaces/formats/JSONCompactColumns) 格式的更紧凑形式:
...
在某些情况下,您可能希望将 JSON 对象保存到单个 `String`(或 `JSON`)列中,而不对其进行解析。当需要处理具有不同结构的 JSON 对象列表时,这种方式非常有用。以[此文件](../assets/custom.json)为例,其中父列表内包含多个不同的 JSON 对象:
...
现在可以使用 [`JSONAsString`](/interfaces/formats/JSONAsString) 格式将文件中的数据加载到此表中,以保留 JSON 对象而不对其进行解析:
...
然后可以使用 [JSON 函数](/sql-reference/functions/json-functions.md)来查询已保存的对象:
...
这样就可以使用点表示法引用嵌套 JSON 对象的键(注意需要用反引号包裹这些键):
...
ClickHouse 支持从 [BSON](https://bsonspec.org/) 编码文件导入数据以及导出数据到该格式文件。此格式被一些数据库管理系统使用,例如 [MongoDB](https://github.com/mongodb/mongo) 数据库。

要导入 BSON 数据,我们使用 [BSONEachRow](/interfaces/formats/BSONEachRow) 格式。下面从[此 BSON 文件](../assets/data.bson)导入数据:
