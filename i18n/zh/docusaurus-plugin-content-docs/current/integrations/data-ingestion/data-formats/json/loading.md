...
以下示例展示了如何加载结构化和半结构化 JSON 数据的基本方法。对于更复杂的 JSON（包括嵌套结构），请参阅 [**设计 JSON 模式**](/integrations/data-formats/json/schema) 指南。
...
:::note 尽可能使用静态结构
在列具有固定名称和类型,且不期望出现新列的情况下,在生产环境中始终优先使用静态定义的结构。

JSON 类型适用于高度动态的数据,其中列的名称和类型可能会发生变化。此类型在原型设计和数据探索中也很有用。
:::

下面显示了一个简单的表结构,其中 **JSON 键映射到列名**:
...
ClickHouse 可以加载多种格式的 JSON 数据,自动从扩展名和内容推断类型。我们可以使用 [S3 函数](/sql-reference/table-functions/s3)读取上述表的 JSON 文件:
...
这些示例假定使用 `JSONEachRow` 格式。ClickHouse 也支持其他常见的 JSON 格式，加载这些格式的示例见[此处](/integrations/data-formats/json/other-formats)。
...
考虑以下来自上述 [Python PyPI 数据集](https://clickpy.clickhouse.com/)扩展版本的示例。在这里,我们添加了一个包含随机键值对的任意 `tags` 列。
...
这里的 tags 列是不可预测的,因此我们无法对其建模。要加载这些数据,我们可以使用之前的模式,但需要添加一个类型为 [`JSON`](/sql-reference/data-types/newjson) 的 `tags` 列:
...
注意这里加载数据时的性能差异。JSON 列在插入时需要进行类型推断,如果列中存在多种类型,还需要额外的存储空间。尽管 JSON 类型可以通过配置(参见[设计 JSON 模式](/integrations/data-formats/json/schema))来获得与显式声明列相当的性能,但它在开箱即用时是有意设计为灵活的。然而,这种灵活性是有代价的。
...
如果您的数据结构是已知且一致的,即使数据采用 JSON 格式,通常也不需要使用 JSON 类型。具体来说,如果您的数据具有以下特征:


* **键已知的扁平结构**：使用标准列类型，例如 String。
* **可预测的嵌套结构**：为这类结构使用 Tuple、Array 或 Nested 类型。
* **结构可预测但字段类型多样**：可以考虑使用 Dynamic 或 Variant 类型。

你也可以组合使用这些方法，就像上面的示例那样：对可预测的顶层键使用静态列，对负载中动态变化的部分使用单个 JSON 列。
