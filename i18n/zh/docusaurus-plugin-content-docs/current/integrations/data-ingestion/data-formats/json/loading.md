...
以下示例演示了如何加载结构化和半结构化 JSON 数据的基本方法。对于更复杂的 JSON(包括嵌套结构),请参阅 [**设计 JSON 模式**](/integrations/data-formats/json/schema) 指南。
...
:::note 尽可能使用静态模式
在列具有固定名称和类型,且不期望出现新列的情况下,生产环境中应始终优先使用静态定义的模式。
...
:::note 排序键
我们在这里通过 `ORDER BY` 子句选择了排序键。有关排序键及其选择方法的更多详细信息,请参见[此处](/data-modeling/schema-design#choosing-an-ordering-key)。
:::

ClickHouse 可以加载多种格式的 JSON 数据,并能从扩展名和内容自动推断类型。我们可以使用 [S3 函数](/sql-reference/table-functions/s3)读取上述表的 JSON 文件:
...
注意,我们不需要指定文件格式。相反,我们使用通配符模式读取存储桶中的所有 `*.json.gz` 文件。ClickHouse 会根据文件扩展名和内容自动推断格式为 `JSONEachRow`(ndjson)。如果 ClickHouse 无法检测格式,可以通过参数函数手动指定格式。
...
2 行结果。耗时 0.005 秒。处理了 8.19 千行，908.03 KB（1.63 百万行/秒，180.38 MB/秒）。

````

也可以使用 [`FORMAT` 子句](/sql-reference/statements/select/format) 内联加载行数据,例如:
...
这些示例假定使用 `JSONEachRow` 格式。系统同样支持其他常见的 JSON 格式，加载这些格式的示例见[此处](/integrations/data-formats/json/other-formats)。


## 加载半结构化 JSON {#loading-semi-structured-json}

我们之前的示例加载的 JSON 是静态的,具有已知的键名和类型。但实际情况往往并非如此——键可能会被添加,或其类型可能会改变。这在可观测性数据等用例中很常见。

ClickHouse 通过专用的 [`JSON`](/sql-reference/data-types/newjson) 类型来处理这种情况。

考虑以下来自上述 [Python PyPI 数据集](https://clickpy.clickhouse.com/)扩展版本的示例。这里我们添加了一个任意的 `tags` 列,其中包含随机的键值对。
...
这里的 tags 列是不可预测的,因此我们无法对其进行建模。要加载这些数据,我们可以使用之前的模式,但需要添加一个类型为 [`JSON`](/sql-reference/data-types/newjson) 的 `tags` 列:
...
注意这里加载数据时的性能差异。JSON 列在插入时需要进行类型推断,如果列中存在多种类型,还需要额外的存储空间。尽管 JSON 类型可以通过配置(参见[设计 JSON 模式](/integrations/data-formats/json/schema))来获得与显式声明列相当的性能,但它在开箱即用时有意保持灵活性。然而,这种灵活性是有代价的。
...
- 具有**不可预测的键**,可能随时间变化。
- 包含**类型可变的值**(例如,某个路径有时包含字符串,有时包含数字)。
- 需要模式灵活性,严格类型化不可行时。
...
* **可预测的嵌套结构**：对这类结构使用 Tuple、Array 或 Nested 类型。
* **结构可预测但类型不同**：可以考虑使用 Dynamic 或 Variant 类型。

你也可以像上面的示例那样组合使用这些方法：为可预测的顶层键使用静态列，为负载中动态变化的部分使用单个 JSON 列。
