...
description: '介绍如何在 ClickHouse 中处理 Avro、Arrow 和 ORC 数据的页面'
...
Apache 提供了多种在分析环境中广泛使用的数据格式，包括常用的 [Avro](https://avro.apache.org/)、[Arrow](https://arrow.apache.org/) 和 [ORC](https://orc.apache.org/)。ClickHouse 支持对这些格式的数据进行导入和导出。
...
在导入或导出 Avro 文件时需要考虑[数据类型映射](/interfaces/formats/Avro#data-type-mapping)。从 Avro 文件加载数据时,使用显式类型转换来进行类型转换:
...
当 Kafka 消息使用 Avro 格式时,ClickHouse 可以使用 [AvroConfluent](/interfaces/formats/AvroConfluent) 格式和 [Kafka](/engines/table-engines/integrations/kafka.md) 引擎来读取这些数据流:
...
另一种列式格式是 [Apache Arrow](https://arrow.apache.org/),ClickHouse 同样支持该格式的导入和导出。要从 [Arrow 文件](assets/data.arrow)导入数据,可以使用 [Arrow](/interfaces/formats/Arrow) 格式:
...
现在可以通过将 ClickHouse 的输出通过管道传递给该脚本来流式传输数据:
...
另外,请参阅[数据类型匹配](/interfaces/formats/ORC)以及[附加设置](/interfaces/formats/Parquet#format-settings)来调整导出和导入操作。


## 延伸阅读 {#further-reading}

ClickHouse 支持多种格式,包括文本和二进制格式,以满足各种应用场景和平台需求。您可以在以下文章中了解更多格式及其使用方法:
...
另外,请参阅 [clickhouse-local](https://clickhouse.com/blog/extracting-converting-querying-local-files-with-sql-clickhouse-local) —— 这是一个功能完备的便携式工具,无需 ClickHouse 服务器即可处理本地或远程文件。
