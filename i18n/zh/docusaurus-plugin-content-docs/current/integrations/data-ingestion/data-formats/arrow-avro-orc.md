...
Apache 发布了多种在分析场景中广泛使用的数据格式，其中包括常用的 [Avro](https://avro.apache.org/)、[Arrow](https://arrow.apache.org/) 和 [Orc](https://orc.apache.org/)。ClickHouse 支持使用上述任一格式导入和导出数据。
...
要从 [Avro 文件](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.avro) 导入数据，应在 `INSERT` 语句中使用 [Avro](/interfaces/formats/Avro) 格式：
...
在导入或导出 Avro 文件时，请注意[数据类型匹配](/interfaces/formats/Avro#data-type-mapping)。从 Avro 文件加载数据时，请使用显式类型转换进行转换：
...
另一种列式数据格式是 [Apache Arrow](https://arrow.apache.org/)，ClickHouse 也支持使用它进行导入和导出。要从 [Arrow 文件](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.arrow) 导入数据，我们使用 [Arrow](/interfaces/formats/Arrow) 格式：
...
[Apache ORC](https://orc.apache.org/) 格式是一种列式存储格式，通常用于 Hadoop。ClickHouse 支持使用 [ORC 格式](/interfaces/formats/ORC) 导入和导出 [ORC 数据](https://clickhouse-docs-assets.s3.us-east-1.amazonaws.com/data.orc)：
