...
ClickHouse 支持多种二进制格式，可以带来更高的性能和更好的空间利用率。二进制格式在字符编码方面也是安全的，因为数据是以二进制形式保存的。

我们将使用 some_data [表](assets/some_data.sql)和[数据](assets/some_data.tsv)进行演示，您可以在自己的 ClickHouse 实例上自行复现。
...
这将创建一个原生格式的 [data.clickhouse](assets/data.clickhouse) 文件。
...
在导出数据到原生格式(以及大多数其他格式)时,可以使用 `COMPRESSION` 子句启用压缩:
...
这里使用了 LZ4 压缩进行导出。在导入数据时需要指定相应的压缩方式:
...
此格式不支持自动模式推断,因此在加载前进行探索时,必须显式定义模式:
...
这种情况下可以使用 [RawBLOB 格式](/interfaces/formats/RawBLOB)。该格式仅可直接用于单列表:
...
注意,必须使用 `LIMIT 1`,因为导出多个值会生成损坏的文件。
...
这会将数据保存到 [proto.bin](assets/proto.bin) 文件。ClickHouse 还支持导入 Protobuf 数据以及嵌套消息。如需处理单个 Protocol Buffer 消息,可以使用 [ProtobufSingle](/interfaces/formats/ProtobufSingle)(此格式会省略长度分隔符)。
...
ClickHouse 支持的另一种流行二进制序列化格式是 [Cap'n Proto](https://capnproto.org/)。与 `Protobuf` 格式类似,我们需要在示例中定义一个架构文件([`schema.capnp`](assets/schema.capnp)):
...
现在我们可以使用 [CapnProto](/interfaces/formats/CapnProto) 格式和该架构进行导入和导出:
