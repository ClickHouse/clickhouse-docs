---
sidebar_label: 'Binary 和 Native'
slug: /integrations/data-formats/binary-native
title: '在 ClickHouse 中使用 Native 和 Binary 格式'
description: '介绍如何在 ClickHouse 中使用 Native 和 Binary 格式的页面'
...
# 在 ClickHouse 中使用 Native 和 Binary 格式

ClickHouse 支持多种二进制格式，可以带来更好的性能和空间效率。二进制格式在字符编码方面也是安全的，因为数据是以二进制形式保存的。

我们将使用 some_data 的[表](assets/some_data.sql)和[数据](assets/some_data.tsv)进行演示，你也可以在自己的 ClickHouse 实例上复现这些示例。



## 以 ClickHouse Native 格式导出 {#exporting-in-a-native-clickhouse-format}
...
在导出数据到 Native 格式(以及大多数其他格式)时,可以使用 `COMPRESSION` 子句启用压缩:
...
这里使用了 LZ4 压缩进行导出。导入数据时需要指定相应的压缩方式:
...
另一种支持的二进制格式是 [RowBinary](/interfaces/formats/RowBinary),它允许以二进制行格式导入和导出数据:
...
此格式不支持自动模式推断,因此在加载前探索数据时,需要显式定义模式:
...
注意,这里必须使用 `LIMIT 1`,因为导出多个值会生成损坏的文件。
...
要使用 [Protocol Buffers](/interfaces/formats/Protobuf),首先需要定义一个 [schema 文件](assets/schema.proto):
...
该 schema 文件的路径(本例中为 `schema.proto`)需在 [Protobuf](/interfaces/formats/Protobuf) 格式的 `format_schema` 设置选项中指定:
...
这会将数据保存到 [proto.bin](assets/proto.bin) 文件。ClickHouse 还支持导入 Protobuf 数据及嵌套消息。如需处理单个 Protocol Buffer 消息,可使用 [ProtobufSingle](/interfaces/formats/ProtobufSingle)(此格式会省略长度分隔符)。
...
另外,您还可以了解 [clickhouse-local](https://clickhouse.com/blog/extracting-converting-querying-local-files-with-sql-clickhouse-local) —— 一个功能完备的便携式工具,无需启动 ClickHouse 服务器即可处理本地/远程文件。
