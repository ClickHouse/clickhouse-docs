...
可以使用 [SQLInsert](/interfaces/formats/SQLInsert) 格式将数据转储为 SQL 格式。ClickHouse 会以 `INSERT INTO <table name> VALUES(...` 形式写入数据,并使用 [`output_format_sql_insert_table_name`](/operations/settings/settings-formats.md/#output_format_sql_insert_table_name) 设置选项指定表名:
...
ClickHouse 提供 [Values](/interfaces/formats/Values) 格式,它与 SQLInsert 类似,但省略了 `INSERT INTO table VALUES` 部分,仅返回值集合:
...
## 从 SQL 转储文件插入数据 {#inserting-data-from-sql-dumps}

要读取 SQL 转储文件,请使用 [MySQLDump](/interfaces/formats/MySQLDump) 格式:
...
默认情况下,ClickHouse 会跳过未知列(由 [input_format_skip_unknown_fields](/operations/settings/settings-formats.md/#input_format_skip_unknown_fields) 选项控制),并处理转储文件中首个找到的表的数据(当多个表被转储到单个文件时)。DDL 语句将被跳过。要从 MySQL 转储文件将数据加载到表中([mysql.sql](assets/mysql.sql) 文件):
