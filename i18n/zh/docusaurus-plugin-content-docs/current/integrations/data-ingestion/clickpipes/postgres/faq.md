...
对于 Postgres 13 或更低版本，CTID 范围扫描较慢，因此这些设置就更加关键。在这种情况下，可以考虑按以下步骤提升性能：

1. **删除现有 pipe**：这是应用新设置所必需的。
2. **删除 ClickHouse 上的目标表**：确保由之前的 pipe 创建的表已被移除。
3. **使用优化后的设置创建新的 pipe**：通常做法是将每个分区的快照行数提高到 100 万到 1000 万之间，具体取值取决于你的需求以及 Postgres 实例可承受的负载。

这些调整应能显著提升初始加载的性能，尤其是对于较旧的 Postgres 版本。如果你使用的是 Postgres 14 或更高版本，由于增强了对 CTID 范围扫描的支持，这些设置的影响会小一些。
