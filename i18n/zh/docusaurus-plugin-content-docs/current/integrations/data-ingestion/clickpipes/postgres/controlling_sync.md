...
description: '介绍如何控制 Postgres ClickPipe 同步的文档'
...
本文档介绍如何在 ClickPipe 处于 **CDC（运行）模式** 时控制 Postgres ClickPipe 的同步。


## 概述 {#overview}

数据库 ClickPipes 的架构由两个并行进程组成——从源数据库拉取数据和向目标数据库推送数据。拉取进程由同步配置控制,该配置定义了数据拉取的频率以及每次拉取的数据量。这里的"每次"是指一个批次——因为 ClickPipe 以批次方式拉取和推送数据。
