...
本文档介绍了 Postgres ClickPipe 中并行快照/初始加载的工作机制，以及可用于控制该过程的快照参数。


## 概述 {#overview-pg-snapshot}

初始加载是 CDC ClickPipe 的第一个阶段,在启动 CDC 之前,ClickPipe 会将源数据库中表的历史数据同步到 ClickHouse。通常情况下,开发人员会以单线程方式执行此操作——例如使用 pg_dump 或 pg_restore,或使用单个线程从源数据库读取数据并写入 ClickHouse。
