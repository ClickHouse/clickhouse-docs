...
4. 使用你想要复制的表创建一个 [publication](https://www.postgresql.org/docs/current/logical-replication-publication.html)。强烈建议仅将所需的表包含在 publication 中，以避免性能开销。
   ...
   :::warning
   包含在 publication 中的任意表必须定义有 **primary key**，*或者* 已将其 **replica identity** 配置为 `FULL`。有关范围设置的指导，请参阅 [Postgres 常见问题](../faq.md#how-should-i-scope-my-publications-when-setting-up-replication)。
   :::
