...
本文档介绍如何设置 Neon Postgres，以便在 ClickPipes 中用作复制源。
...
转到 **Settings** 选项卡,然后进入 **Logical Replication** 部分。
...
如果您使用的是 Neon 企业版,可以将 [ClickPipes IP 地址](../../index.md#list-of-static-ips)添加到白名单,以允许 ClickPipes 向您的 Neon Postgres 实例复制数据。
要进行此操作,请点击 **Settings** 选项卡,然后进入 **IP Allow** 部分。
...
现在我们已经准备好用户、发布并启用了复制，可以复制连接详情来创建新的 ClickPipe。
