...
如果你使用的是支持的提供商之一（见侧边栏），请参考该提供商的专用指南。
...
2. 进入 Flags 并将 `cloudsql.logical_decoding` 更改为 on,将 `wal_sender_timeout` 更改为 0。这些更改需要重启 Postgres 服务器。
...
如果您使用 SSH 隧道,则需要将 [ClickPipes IP](../../index.md#list-of-static-ips) 添加到跳板服务器/堡垒主机的防火墙规则中。
