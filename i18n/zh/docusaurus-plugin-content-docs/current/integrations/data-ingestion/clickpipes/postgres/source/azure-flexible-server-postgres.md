...
**如果** `wal_level` 已设置为 `logical`，则**无需**执行以下步骤。如果你是从其他数据复制工具迁移过来，这个设置通常已经预先配置好了。
...
请按照以下步骤将 [ClickPipes IP](../../index.md#list-of-static-ips) 添加到您的网络中。
...
现在你可以[创建 ClickPipe](../index.md)，并开始从你的 Postgres 实例向 ClickHouse Cloud 摄取数据。
请务必记录你在设置 Postgres 实例时使用的连接信息，因为在创建 ClickPipe 的过程中会用到这些信息。