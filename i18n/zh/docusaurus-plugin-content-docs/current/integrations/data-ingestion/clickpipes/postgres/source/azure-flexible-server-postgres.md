...
description: '将 Azure Flexible Server for Postgres 设置为 ClickPipes 数据源'
slug: /integrations/clickpipes/postgres/source/azure-flexible-server-postgres
title: 'Azure Flexible Server for Postgres 数据源配置指南'
...
如果 `wal_level` 已设置为 `logical`,则**无需**执行以下步骤。如果您正在从其他数据复制工具迁移,此设置通常已预先配置好。
...
2. 为 `clickpipes_user` 授予对要复制表所在模式的只读访问权限。以下示例展示了如何为 `public` 模式设置权限。如果需要授予对多个模式的访问权限,可以为每个模式运行这三条命令。
...
1. 进入**网络**选项卡,将 [ClickPipes IP 地址](../../index.md#list-of-static-ips)添加到 Azure Flexible Server Postgres 的防火墙中;如果您使用 SSH 隧道,则添加到跳板机/堡垒主机的防火墙中。
...
请务必记录设置 Postgres 实例时使用的连接详细信息，因为在创建 ClickPipe 时需要用到这些信息。
