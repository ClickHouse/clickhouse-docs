...
description: '将任意 Postgres 实例配置为 ClickPipes 源'
...
如果您使用的是侧边栏中列出的受支持提供商之一，请参阅该提供商的相应指南。
...
1. 要在您的 Postgres 实例上启用复制,需要确保设置了以下参数:
...
3. 如果您按照上述说明对配置进行了任何更改,则必须重启 Postgres 实例以使更改生效。


## 创建具有权限和发布的用户 {#creating-a-user-with-permissions-and-publication}

让我们为 ClickPipes 创建一个新用户,为其授予适用于 CDC 的必要权限,
并创建一个用于复制的发布。
...
如果您是自行部署,需要按照以下步骤允许 ClickPipes IP 地址连接到 ClickPipes 用户。如果您使用的是托管服务,可以参照服务提供商的文档执行相同操作。
...
建议进行此配置更改,以确保大型事务/提交不会导致复制槽被删除。
...
如需获取关于此值的更佳建议,请联系 ClickPipes 团队。
...
现在您可以[创建 ClickPipe](../index.md),并开始从 Postgres 实例向 ClickHouse Cloud 导入数据。
请务必记录设置 Postgres 实例时使用的连接详细信息,在创建 ClickPipe 时将需要用到这些信息。
