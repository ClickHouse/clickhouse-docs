...
description: '将任意 Postgres 实例配置为 ClickPipes 的源'
...
让我们为 ClickPipes 创建一个新用户,为其授予适用于 CDC 的必要权限,并创建一个用于数据复制的发布。
...
请确保将 `clickpipes_user` 和 `clickpipes_password` 替换为您期望使用的用户名和密码。
