...
在某些场景下，暂停 Postgres ClickPipe 会很有用。例如，你可能希望在数据保持静态时对现有数据进行分析，或者你正在对 Postgres 执行升级操作。下面介绍如何暂停和恢复 Postgres ClickPipe。
...
1. 在"数据源"选项卡中,点击您要恢复的 Postgres ClickPipe。此时镜像的状态应为 **已暂停**。
2. 前往 **设置** 选项卡。
3. 点击 **恢复** 按钮。
