...
管道配置完成后,会进入 `Setup` 状态。在此状态下会创建目标 ClickHouse 表。


## 快照 {#snapshot}

设置完成后,系统将进入 `Snapshot` 状态(除非是仅 CDC 的管道,此类管道会直接转换到 `Running` 状态)。`Snapshot`、`Initial Snapshot` 和 `Initial Load`(更常用)这几个术语可以互换使用。在此状态下,系统会对源 MongoDB 集合进行快照并将数据加载到 ClickHouse 中。oplog 的保留设置应当考虑初始加载所需的时间。当触发重新同步或向现有管道添加新表时,管道也会进入 `Snapshot` 状态。


## 运行中 {#running}

初始加载完成后,管道进入 `Running` 状态(除非是仅快照管道,会转换为 `Completed` 状态)。此时管道开始执行 `Change-Data Capture`(变更数据捕获)。在此状态下,系统开始将源 MongoDB 集群的变更流式传输到 ClickHouse。有关控制 CDC 的信息,请参阅[控制 CDC 的文档](./sync_control)。
