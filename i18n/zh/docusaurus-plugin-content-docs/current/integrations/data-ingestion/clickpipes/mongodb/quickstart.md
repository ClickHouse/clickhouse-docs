...
本指南介绍了通过 ClickPipes 将 MongoDB 中的 JSON 数据复制到 ClickHouse 时的常见模式。
...
MongoDB CDC 连接器使用原生 JSON 数据类型将 MongoDB 文档复制到 ClickHouse。ClickHouse 中得到的表 `t1` 将包含以下行：
...
- `_peerdb_version`：跟踪行的版本号；当行被更新或删除时递增
- `_peerdb_is_deleted`：标记行是否已删除

### ReplacingMergeTree 表引擎 {#replacingmergetree-table-engine}

ClickPipes 使用 `ReplacingMergeTree` 表引擎系列将 MongoDB 集合映射到 ClickHouse。通过该引擎，更新操作被建模为针对给定主键（`_id`）插入文档的新版本（`_peerdb_version`），从而能够高效地将更新、替换和删除操作作为版本化插入来处理。

`ReplacingMergeTree` 会在后台异步清除重复数据。要确保同一行不存在重复数据，请使用 [`FINAL` 修饰符](/sql-reference/statements/select/from#final-modifier)。例如：
...
来自 MongoDB 的删除操作会作为新行传播，并通过 `_peerdb_is_deleted` 列标记为已删除。通常您需要在查询中过滤掉这些行：
...
在 ClickHouse 中,JSON 中的每个字段都具有 `Dynamic` 类型。Dynamic 类型允许 ClickHouse 在不预先知道类型的情况下存储任意类型的值。您可以使用 `toTypeName` 函数验证这一点:
...
要检查字段的底层数据类型,可以使用 `dynamicType` 函数。请注意,不同行中相同字段名称可能具有不同的数据类型:
...
[常规函数](https://clickhouse.com/docs/sql-reference/functions/regular-functions)对 Dynamic 类型的工作方式与对常规列的工作方式相同:
...
ClickHouse 中的[聚合函数](https://clickhouse.com/docs/sql-reference/aggregate-functions/combinators)不能直接用于 Dynamic 类型。例如,如果您尝试直接对 Dynamic 类型使用 `sum` 函数,将收到以下错误:
...
要使用聚合函数,请使用 `CAST` 函数或 `::` 语法将字段转换为适当的类型:
...
:::note
从 Dynamic 类型转换为底层数据类型(由 `dynamicType` 确定)性能非常高,因为 ClickHouse 在内部已经以底层类型存储该值。
:::
...
您可以在 JSON 表之上创建普通视图,以封装展平/类型转换/转换逻辑,从而像查询关系表一样查询数据。普通视图是轻量级的,因为它们仅存储查询本身,而不存储底层数据。例如:
...
此方法的关键优势在于,使用 `FINAL` 关键字的查询仅在刷新期间运行一次,从而消除了对目标表的后续查询使用 `FINAL` 的需要。
...
现在可以如下查询 `flattened_t1_final` 视图：
