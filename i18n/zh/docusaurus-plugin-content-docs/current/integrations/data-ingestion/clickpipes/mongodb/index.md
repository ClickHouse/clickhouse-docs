...
:::info
通过 ClickPipes 将数据从 MongoDB 导入 ClickHouse Cloud 目前处于公测阶段。
:::
...
你可以使用 ClickPipes 将 MongoDB 数据库中的数据导入 ClickHouse Cloud。源 MongoDB 数据库可以部署在本地，也可以托管在 MongoDB Atlas 等云服务上。
...
5. 在此您可以为 ClickPipe 选择目标数据库。您可以选择现有数据库或创建新数据库。
...
- 使用 MongoDB 原生的 Change Streams API 进行 CDC,该 API 依赖 MongoDB oplog 来捕获实时变更。
