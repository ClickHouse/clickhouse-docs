---
sidebar_label: '将 MongoDB 数据导入 ClickHouse'
...
:::info
通过 ClickPipes 将数据从 MongoDB 导入 ClickHouse Cloud 目前处于公开测试阶段。
:::
...
您可以使用 ClickPipes 将 MongoDB 数据库中的数据摄取到 ClickHouse Cloud 中。源 MongoDB 数据库可以部署在本地数据中心，或通过 MongoDB Atlas 等服务在云中托管。



## 前提条件 {#prerequisites}

开始之前,您需要确保 MongoDB 数据库已正确配置复制功能。具体配置步骤取决于您部署 MongoDB 的方式,请参考以下相关指南:
...
如果您的源 MongoDB 数据库无法公开访问,您可以指定 SSH 隧道详细信息。
...
- **同步间隔**: ClickPipes 轮询源数据库以获取更改的时间间隔。这会影响目标 ClickHouse 服务,对于成本敏感的用户,我们建议将此值设置得较高(超过 `3600`)。
- **拉取批次大小**: 单次批处理中获取的行数。这是一个尽力而为的设置,可能并非在所有情况下都会被遵守。
- **并行快照表数量**: 初始快照期间并行获取的表的数量。当您有大量表并希望控制并行获取的表数量时,此设置非常有用。
...
完成 ClickPipe 的配置以将数据从 MongoDB 复制到 ClickHouse Cloud 后,您可以专注于如何查询和建模数据以实现最佳性能。
...
- 使用 MongoDB 原生的 Change Streams API 进行 CDC,该 API 依赖 MongoDB oplog 捕获实时变更。
- 默认情况下,MongoDB 文档会以 JSON 类型复制到 ClickHouse 中。这样可以灵活管理模式,并能够使用 ClickHouse 中丰富的 JSON 操作符进行查询和分析。您可以在[此处](https://clickhouse.com/docs/sql-reference/data-types/newjson)了解更多关于查询 JSON 数据的信息。
- 目前暂不支持自助配置 PrivateLink。如果您使用 AWS 并需要 PrivateLink,请联系 db-integrations-support@clickhouse.com 或创建支持工单,我们将与您协作启用该功能。
