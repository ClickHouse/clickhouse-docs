...
ClickPipes 会根据数据摄取过程中遇到的错误类型,将错误存储在两个不同的表中。
...
- **使用 ClickPipes for Kafka 时是否有办法处理错误或故障?**
  ...
  是的,ClickPipes for Kafka 在从 Kafka 消费数据时遇到任何操作问题(包括网络问题、连接问题等)导致的故障时,会自动重试。如果遇到格式错误的数据或无效的模式,ClickPipes 会将记录存储在 record_error 表中并继续处理。
