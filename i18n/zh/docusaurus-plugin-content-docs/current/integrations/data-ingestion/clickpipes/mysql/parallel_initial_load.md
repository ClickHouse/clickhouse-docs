...
MySQL ClickPipe 使用源表中的某一列对源表进行逻辑分区。该列称为**分区键列**。它用于将源表划分为多个分区,然后 ClickPipe 可以并行处理这些分区。
...
此设置控制并行处理的分区数量。默认值为 4,这意味着 ClickPipe 将并行读取源表的 4 个分区。可以增加此值以加快初始加载速度,但建议根据源实例的规格将其保持在合理范围内,以避免源数据库负载过重。ClickPipe 将根据源表的大小和每个分区的行数自动调整分区数量。
