...
description: '将任意 MariaDB 实例设置为 ClickPipes 的源'
...
如果你使用的是侧边栏中列出的受支持服务提供商之一，请参阅该提供商的专用指南。
...
**自托管 MariaDB 11.4 及更高版本** - 如果您的服务器已设置 `ssl_ca`,请遵循上述选项。否则,请咨询您的 IT 团队以配置合适的证书。作为最后手段,可以使用 ClickPipes UI 中的"跳过证书验证"开关(出于安全原因不推荐使用)。
...
现在您可以[创建 ClickPipe](../index.md) 并开始将 MariaDB 实例中的数据导入到 ClickHouse Cloud。
