...
如果您使用的是受支持的服务商之一（见侧边栏），请参考该服务商的相应指南。
...
binlog_format = ROW      -- 默认值
binlog_row_image = FULL  -- 默认值
expire_logs_days = 1     -- 或更高;0 表示永久保留日志
```
...
**无服务器访问权限的自托管 MySQL** - 联系您的 IT 团队获取证书。作为最后手段,可以在 ClickPipes UI 中使用"跳过证书验证"开关(出于安全原因不推荐使用)。
