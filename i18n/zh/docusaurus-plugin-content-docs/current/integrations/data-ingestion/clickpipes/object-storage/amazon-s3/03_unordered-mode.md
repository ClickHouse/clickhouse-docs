...
<VerticalStepper type="numbered" headerLevel="h2">
  ...
  **2.** 在 AWS Console 中，导航至 **IAM &gt; Roles &gt; Create role**。选择 **Custom trust policy**，粘贴以下内容，并将 `<ch-cloud-arn>` 替换为上一步复制的 IAM 角色 ARN：
  ...
  **3.** 为 IAM 角色创建内联策略，授予其从 S3 读取对象及管理 SQS 队列消息所需的[必需权限](/integrations/clickpipes/object-storage/s3/overview#permissions)。将 `<bucket-arn>` 和 `<sqs-queue-arn>` 替换为相应的值：
