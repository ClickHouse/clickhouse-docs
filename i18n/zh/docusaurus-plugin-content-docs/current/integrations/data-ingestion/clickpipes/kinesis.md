...
6. 在下一步中,您可以选择是将数据导入到新的 ClickHouse 表还是重用现有表。按照屏幕上的说明修改表名、模式和设置。您可以在顶部的示例表中实时预览您的更改。
...
ClickPipes 以批处理方式将数据插入 ClickHouse。这样做是为了避免在数据库中创建过多的数据部分(parts),从而导致集群性能问题。
...
ClickPipes for Kinesis 设计为支持水平和垂直扩展。默认情况下,我们创建一个包含单个消费者的消费者组。这可以在创建 ClickPipe 期间配置,或在任何其他时间点通过 **Settings** -> **Advanced Settings** -> **Scaling** 进行配置。
