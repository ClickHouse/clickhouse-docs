...
3. 记下 **Resource ID** 值。对于 RDS,它应该类似于 `db-xxxxxxxxxxxxxx`,对于 Aurora 集群,它应该类似于 `cluster-xxxxxxxxxxxxxx`。我们将此值称为 `{RDS_RESOURCE_ID}`。这是将在 IAM 策略中使用的资源 ID,用于授予对 RDS 实例的访问权限。
