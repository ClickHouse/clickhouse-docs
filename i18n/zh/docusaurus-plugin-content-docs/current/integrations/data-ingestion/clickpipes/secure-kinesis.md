...
description: '本文展示 ClickPipes 客户如何利用基于角色的访问控制对 Amazon Kinesis 进行身份验证，并安全访问其数据流。'
doc_type: 'guide'
...
本文演示 ClickPipes 客户如何利用基于角色的访问机制对 Amazon Kinesis 进行身份验证，并安全访问其数据流。
...
在深入了解安全 Kinesis 访问的配置之前,理解其工作机制非常重要。以下概述了 ClickPipes 如何通过在客户 AWS 账户中代入角色来访问 Amazon Kinesis 数据流。

<Image img={secure_kinesis} alt='Secure Kinesis' size='lg' border />

使用这种方法,客户可以在单一位置(代入角色的 IAM 策略)集中管理对其 Kinesis 数据流的所有访问权限,而无需逐个修改每个数据流的访问策略。
...
- 4. 创建后复制新的 **IAM Role Arn**。这是访问您的 Kinesis 流所需的信息。
