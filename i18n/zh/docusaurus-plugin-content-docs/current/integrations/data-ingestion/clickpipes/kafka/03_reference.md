---
sidebar_label: '参考'
description: '说明 Kafka ClickPipes 所支持的格式、数据源、投递语义、身份验证机制以及实验性功能的详细信息'
...
| Confluent Cloud  | <Confluentsvg class="image" alt="Confluent Cloud logo" style={{width: '3rem'}}/>            | 流式 | 稳定 | 通过我们的直接集成,释放 Confluent 和 ClickHouse Cloud 的组合优势。          |
...
- Tuple 和 Array,其元素可使用上述任意类型(包括 Nullable 类型,仅支持一级深度)
...
Avro 中的 Nullable 类型通过使用 `(T, null)` 或 `(null, T)` 的联合模式定义,其中 T 是基础 Avro 类型。在模式推断期间,此类联合类型将映射到 ClickHouse 的 "Nullable" 列。请注意,ClickHouse 不支持 `Nullable(Array)`、`Nullable(Map)` 或 `Nullable(Tuple)` 类型。这些类型的 Avro null 联合类型将映射为非 nullable 版本(Avro Record 类型映射为 ClickHouse 命名 Tuple)。这些类型的 Avro "null" 值将按以下方式插入:

- 对于 null Avro 数组,插入空 Array
- 对于 null Avro Map,插入空 Map
- 对于 null Avro Record,插入所有值为默认值/零值的命名 Tuple

#### Variant 类型支持 {#variant-type-support}

ClickPipes 在以下情况下支持 Variant 类型:

- Avro 联合类型。如果您的 Avro 模式包含具有多个非 null 类型的联合类型,ClickPipes 将推断出相应的 variant 类型。除此之外,Avro 数据不支持 Variant 类型。
...
- 如果列实际包含 JSON 字符串对象,则 Avro String 和 Bytes 类型可以分配给 JSON 列。
