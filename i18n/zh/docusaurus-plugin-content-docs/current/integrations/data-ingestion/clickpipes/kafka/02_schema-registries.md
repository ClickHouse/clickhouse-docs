...
- 如果 Avro 模式中缺少 ClickHouse 目标映射中定义的字段，则对应的 ClickHouse 列将使用“零值”填充，例如 0 或空字符串。请注意，目前在 ClickPipes 插入中不会评估 DEFAULT 表达式（这是一个临时性限制，待 ClickHouse 服务器默认值处理机制更新后将被移除）。
