...
description: '如何将 ClickPipes 与 schema registry 集成以进行模式管理'
...
## Kafka ClickPipes 支持的注册中心 {#supported-schema-registries}

支持与 Confluent Schema Registry API 兼容的 Schema 注册中心,包括:

- Confluent Schema Registry
- Redpanda Schema Registry

ClickPipes 目前暂不支持 AWS Glue Schema Registry 或 Azure Schema Registry。如需支持这些 Schema 注册中心,[请联系我们的团队](https://clickhouse.com/company/contact?loc=clickpipes)。
...
- 如果 Avro 模式缺少 ClickHouse 目标映射中定义的字段,ClickHouse 列将填充"零"值,例如 0 或空字符串。请注意,ClickPipes 插入操作目前不会计算 DEFAULT 表达式(这是一个临时限制,待 ClickHouse 服务器默认处理逻辑更新后将解除)。
- 如果 Avro 模式字段与 ClickHouse 列不兼容,该行/消息的插入将失败,失败信息将记录在 ClickPipes 错误表中。请注意,系统支持多种隐式类型转换(例如数值类型之间的转换),但并非所有转换都支持(例如,Avro 记录字段无法插入到 Int32 类型的 ClickHouse 列中)。
