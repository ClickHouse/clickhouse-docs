...
doc_type: 'guide'
keywords: ['Confluent HTTP Sink 连接器', 'HTTP Sink ClickHouse', 'Kafka HTTP 连接器', 'ClickHouse HTTP 集成', 'Confluent Cloud HTTP Sink']
---
...
- **自行管理:** 下载 Confluent 软件包并在本地安装。按照[此处](https://docs.confluent.io/kafka-connect-http/current/overview.html)记录的安装说明来安装连接器。
...
- `auth.type` - 如果在 ClickHouse 中启用了安全性,请设置为 BASIC。目前不支持其他与 ClickHouse 兼容的身份验证机制。
...
- `value.converter` - 根据主题上的数据类型进行设置。此数据不需要模式。这里的格式必须与参数 `http.api.url` 中指定的 FORMAT 一致。最简单的方法是使用 JSON 和 org.apache.kafka.connect.json.JsonConverter 转换器。通过转换器 org.apache.kafka.connect.storage.StringConverter 将值视为字符串也是可行的 - 尽管这需要用户在 insert 语句中使用函数提取值。如果使用 io.confluent.connect.avro.AvroConverter 转换器,ClickHouse 也支持 [Avro 格式](/interfaces/formats/Avro)。

完整的设置列表,包括如何配置代理、重试和高级 SSL,可以在[这里](https://docs.confluent.io/kafka-connect-http/current/connector_config.html)找到。


Github 示例数据的配置文件可以在[此处](https://github.com/ClickHouse/clickhouse-docs/tree/main/docs/integrations/data-ingestion/kafka/code/connectors/http_sink)找到,假设 Connect 以独立模式运行,且 Kafka 托管在 Confluent Cloud 中。
