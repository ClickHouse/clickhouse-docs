...
准备好目标表。下面的示例中，我们为了简洁使用了精简版的 GitHub 模式。请注意，虽然这里使用的是 MergeTree 表引擎，但此示例可以很容易地改写为 [MergeTree family](../../../engines/table-engines/mergetree-family/index.md) 中的任意成员。
...
或者，如果 Kafka 集群使用 SASL 进行身份验证，请使用以下配置：
...
该数据集包含 200,000 行，因此应该可以在几秒钟内完成摄取。如果你想使用更大的数据集，可以查看 GitHub 上 [ClickHouse/kafka-samples](https://github.com/ClickHouse/kafka-samples) 仓库中的 [large datasets 部分](https://github.com/ClickHouse/kafka-samples/tree/main/producer#large-datasets)。
...
在创建时，materialized view 会连接到 Kafka 引擎并开始读取数据，将行插入到目标表中。该过程会持续运行，之后写入 Kafka 的消息也会被消费。您可以随时重新运行插入脚本，将更多消息插入到 Kafka 中。
...
重新统计 GitHub 中的行数，以确认它已经增加了 100。正如上图所示，先通过 Kafka 表引擎将行插入到 Kafka，然后再由同一个引擎重新读取这些行，并通过我们的 materialized view 将它们插入到 GitHub 目标表中！
...
如果你向最初在 [Kafka to ClickHouse](#kafka-to-clickhouse) 部分中创建的原始 github topic 插入数据，这些文档就会自动出现在 &quot;github&#95;clickhouse&quot; topic 中。使用原生 Kafka 工具进行确认。例如，下面我们使用 [kcat](https://github.com/edenhill/kcat)，向托管在 Confluent Cloud 上的 github topic 插入 100 行数据：
...
虽然这是一个复杂的示例，但它展示了 materialized view 与 Kafka 引擎结合使用时的强大威力。
...
任何设置变更都应进行测试。我们建议监控 Kafka consumer 的 lag（消费滞后），以确保你的扩展规模是合适的。
