---
'title': '将 ClickHouse 与 Kafka 集成使用命名集合'
...
3. `<kafka>` 中的部分包含扩展的 Kafka 配置选项。有关更多选项，请参阅 [librdkafka 配置](https://github.com/confluentinc/librdkafka/blob/master/CONFIGURATION.md)。
...
这完成了通过命名集合将 ClickHouse 与 Kafka 集成的设置。通过将 Kafka 配置集中在 ClickHouse `config.xml` 文件中，您可以更轻松地管理和调整设置，确保高效的集成。
