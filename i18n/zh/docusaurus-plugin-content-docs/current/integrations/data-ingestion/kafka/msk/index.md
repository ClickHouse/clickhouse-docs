---
sidebar_label: 'Amazon MSK 与 Kafka Connector Sink 集成'
sidebar_position: 1
...
> 注意：视频中展示的策略较为宽松，仅适用于快速入门。有关最小权限 IAM 的指导，请参阅下文。
...
- MSK 代理可公开访问。请参阅开发者指南的[公共访问](https://docs.aws.amazon.com/msk/latest/developerguide/public-access.html)部分。
...
1. 如果连接器需要与公共 ClickHouse 实例通信,请[启用互联网访问](https://docs.aws.amazon.com/msk/latest/developerguide/msk-connect-internet-access.html)。
...
使用满足您配置需求的最小权限集。从以下基准配置开始,仅在使用可选服务时添加相应权限。
...
- 仅在使用 AWS Glue Schema Registry 时添加 Glue 权限块。
- 仅在从 Secrets Manager 获取凭证/信任存储时添加 Secrets Manager 权限块。需限定 ARN 范围。
- 仅在从 S3 加载工件(例如信任存储)时添加 S3 权限块。需限定到存储桶/前缀范围。
...
提高性能的一种方法是通过在 **worker** 配置中添加以下内容来调整批次大小和从 Kafka 获取的记录数量:

```yml
consumer.max.poll.records=[记录数量]
consumer.max.partition.fetch.bytes=[记录数量 * 单条记录字节大小]
```

您使用的具体值会因所需的记录数量和记录大小而异。例如,默认值为:
...
为了使 MSK Connect 能够连接到 ClickHouse,我们建议将您的 MSK 集群部署在私有子网中,并通过 NAT 网关连接以访问互联网。下面提供了相关配置说明。请注意,虽然支持公有子网,但不推荐使用,因为需要持续为您的 ENI 分配弹性 IP 地址,[AWS 在此处提供了更多详细信息](https://docs.aws.amazon.com/msk/latest/developerguide/msk-connect-internet-access.html)

1. **创建私有子网:** 在您的 VPC 中创建一个新子网,将其指定为私有子网。该子网不应具有直接访问互联网的权限。
1. **创建 NAT 网关:** 在您 VPC 的公有子网中创建 NAT 网关。NAT 网关使私有子网中的实例能够连接到互联网或其他 AWS 服务,但阻止互联网主动与这些实例建立连接。
1. **更新路由表:** 添加一条路由,将发往互联网的流量定向到 NAT 网关
1. **确保安全组和网络 ACL 配置:** 配置您的[安全组](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html)和[网络 ACL(访问控制列表)](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html)以允许相关流量。
   ...
   1. 对于自托管的 ClickHouse,打开服务器中配置的端口(HTTP 默认为 8123)。
1. **将安全组附加到 MSK:** 确保这些安全组已附加到您的 MSK 集群和 MSK Connect 工作节点。
1. **连接到 ClickHouse Cloud:**
   1. 公共端点 + IP 白名单:需要从私有子网通过 NAT 出站。
   1. 在可用的情况下使用私有连接(例如 VPC 对等连接/PrivateLink/VPN)。确保已启用 VPC DNS 主机名/解析,并且 DNS 可以解析私有端点。
1. **验证连接性(快速检查清单):**
