---
sidebar_label: '使用 Kafka Connector Sink 的 Amazon MSK 集成'
sidebar_position: 1
slug: /integrations/kafka/cloud/amazon-msk/
description: 'ClickHouse 官方 Kafka 连接器与 Amazon MSK 的集成'
...
> 注意：视频中展示的策略较为宽松，仅用于快速入门。有关最小权限 IAM 的指导，请参见下文。
...
- 仅在从 S3 加载工件(如信任存储)时添加 S3 权限块。需限定到存储桶/前缀范围。
...
提高性能的一种方法是通过在 **worker** 配置中添加以下内容来调整批次大小和从 Kafka 获取的记录数:

```yml
consumer.max.poll.records=[记录数]
consumer.max.partition.fetch.bytes=[记录数 * 单条记录字节大小]
```

具体使用的值会根据所需的记录数和记录大小而有所不同。例如,默认值为:
...
1. **创建私有子网:** 在 VPC 中创建一个新子网,将其指定为私有子网。该子网不应具有直接访问互联网的权限。
1. **创建 NAT 网关:** 在 VPC 的公有子网中创建一个 NAT 网关。NAT 网关使私有子网中的实例能够连接到互联网或其他 AWS 服务,但阻止互联网主动与这些实例建立连接。
