...
description: 'ClickHouse 提供的官方 Kafka 连接器。'
...
:::note
如果您需要任何帮助,请在[代码仓库中提交问题](https://github.com/ClickHouse/clickhouse-kafka-connect/issues)或在 [ClickHouse 公共 Slack](https://clickhouse.com/slack) 中提问。
:::
...
- 支持第三方状态存储:当前默认使用内存存储,但可以使用 KeeperMap(即将添加 Redis 支持)。
...
- 在 Connect 属性文件中将插件目录路径添加到 [plugin.path](https://kafka.apache.org/documentation/#connectconfigs_plugin.path) 配置中,以便 Confluent Platform 能够找到该插件。
...
| `port`                                           | ClickHouse 端口 - 默认为 8443(云端 HTTPS),但对于 HTTP(自托管的默认方式)应为 8123                                                                                                       | `8443`                                                   |
...
| `key.converter` (必需\* - 参见描述)   | 根据键的类型进行设置。如果传递键(且未在 worker 配置中定义),则此处为必需。                                                                                                                 | `"org.apache.kafka.connect.storage.StringConverter"`     |
| `value.converter` (必需\* - 参见描述) | 根据主题上的数据类型进行设置。支持:JSON、String、Avro 或 Protobuf 格式。如果未在 worker 配置中定义,则此处为必需。                                                                                   | `"org.apache.kafka.connect.json.JsonConverter"`          |
| `value.converter.schemas.enable`                 | 连接器值转换器的模式支持                                                                                                                                                                                                           | `"false"`                                                |
| `errors.tolerance`                               | 连接器错误容忍度。支持:none、all                                                                                                                                                                                                    | `"none"`                                                 |
| `errors.deadletterqueue.topic.name`              | 如果设置(配合 errors.tolerance=all),将为失败的批次使用死信队列(参见[故障排除](#troubleshooting))                                                                                                                | `""`                                                     |
...
| `bypassRowBinary`                                | 允许禁用基于模式的数据(Avro、Protobuf 等)使用 RowBinary 和 RowBinaryWithDefaults - 仅应在数据存在缺失列且 Nullable/Default 不可接受时使用                          | `"false"`                                                |
...
| `tolerateStateMismatch`                          | 允许连接器丢弃"早于"当前存储的 AFTER_PROCESSING 偏移量的记录(例如,如果发送偏移量 5,而最后记录的偏移量为 250)                                                             | `"false"`                                                |
| `ignorePartitionsWhenBatching`                   | 在收集消息进行插入时将忽略分区(但仅当 `exactlyOnce` 为 `false` 时)。性能说明:连接器任务越多,每个任务分配的 Kafka 分区越少 - 这可能意味着收益递减。 | `"false"`                                                |

### 目标表 {#target-tables}


ClickHouse Connect Sink 从 Kafka 主题中读取消息并将其写入相应的表。ClickHouse Connect Sink 仅向已存在的表写入数据。请在开始向表中插入数据之前，确保已经在 ClickHouse 中创建了具有合适表结构的目标表。

每个主题在 ClickHouse 中都需要一个对应的专用目标表。目标表名称必须与源主题名称完全一致。

### 预处理 {#pre-processing}

如果需要在消息发送到 ClickHouse Kafka Connect Sink 之前对其进行转换，请使用 [Kafka Connect Transformations](https://docs.confluent.io/platform/current/connect/transforms/overview.html)。

### 支持的数据类型 {#supported-data-types}

**在声明了模式的情况下：**

| Kafka Connect 类型                      | ClickHouse 类型          | 是否支持  | 是否为基础类型 |
| --------------------------------------- | ------------------------ | --------- | --------- |
| STRING                                  | String                   | ✅        | 是        |
| STRING                                  | JSON。见下文 (1)         | ✅        | 是        |
| INT8                                    | Int8                     | ✅        | 是        |
...
- (1) - 只有当在 ClickHouse 中将 `input_format_binary_read_json_as_string=1` 配置为启用时才支持 JSON。该设置仅对 RowBinary 格式族生效，并且会影响插入请求中的所有列，因此这些列都必须是字符串类型。在这种情况下，连接器会将 STRUCT 转换为 JSON 字符串。

- (2) - 当结构体包含类似 `oneof` 的 union 时，应将转换器配置为不要在字段名称上添加前缀/后缀。对于 `ProtobufConverter`，可以使用 `generate.index.for.unions=false` [设置](https://docs.confluent.io/platform/current/schema-registry/connect.html#protobuf)。

**在未声明模式的情况下：**

记录会被转换为 JSON，并作为 [JSONEachRow](/interfaces/formats/JSONEachRow) 格式中的值发送到 ClickHouse。

### 配置示例 {#configuration-recipes}

以下是一些常见的配置示例，帮助你快速开始使用。

#### 基本配置 {#basic-configuration}

这是帮助你快速上手的最基本配置——假设你在分布式模式下运行 Kafka Connect，并且有一个启用了 SSL、运行在 `localhost:8443` 上的 ClickHouse 服务器，数据为无模式的 JSON。
...
请注意:如果遇到类缺失的问题,并非所有环境都自带 protobuf 转换器,您可能需要使用包含依赖项的备用 jar 版本。
...
如果使用 Confluent Platform,可通过运行 CLI 命令查看日志:

```bash
confluent local services connect log
```

有关更多详细信息,请参阅官方[教程](https://docs.confluent.io/platform/current/connect/logging.html)。
...
该连接器与 Kafka Connect 框架集成，并公开任务生命周期和错误跟踪的指标。
...
2. **跟踪错误率**：监视 `errors-total` 和 `records-skip-total` 以检测数据质量问题
...
本节介绍 ClickHouse Kafka Connect Sink 的性能调优策略。在处理高吞吐量用例或需要优化资源利用率并最小化滞后时，性能调优至关重要。
...
在调优之前,理解数据如何流经连接器非常重要:

1. **Kafka Connect 框架**在后台从 Kafka 主题拉取消息
2. **连接器轮询**框架内部缓冲区中的消息
...
Kafka Connect(框架)在后台从 Kafka 主题拉取消息,独立于连接器运行:

- **`fetch.min.bytes`**: 框架将数据传递给连接器之前的最小数据量(默认值:1 字节)
- **`fetch.max.bytes`**: 单次请求中拉取的最大数据量(默认值:52428800 / 50 MB)
...
**重要提示**：Kafka Connect 的 fetch 设置针对的是压缩数据，而 ClickHouse 接收的是解压后的数据。请根据压缩比来平衡这些设置。

**权衡取舍**：
- **更大的批次** = 更好的 ClickHouse 写入性能、更少的数据分片、更低的开销
- **更大的批次** = 更高的内存占用、可能增加端到端延迟
...
当连接器发送的批次相对较小，或者您希望将批处理职责转移到 ClickHouse 以进一步优化数据写入时，异步插入是一项强大的功能。

##### 何时使用异步插入 {#when-to-use-async-inserts}

在以下情况下建议启用异步插入：

- **大量小批次**：连接器频繁发送小批次数据（每批次 < 1000 行）
- **高并发写入**：多个连接器任务同时写入同一张表
- **分布式部署**：在不同主机上运行多个连接器实例
- **分片创建开销**：遇到"分片过多"错误
- **混合工作负载**：同时进行实时数据写入和查询操作

在以下情况下**不建议**使用异步插入：

- 已经以可控频率发送大批次数据（每批次 > 10,000 行）
- 需要数据立即可见（查询必须立即看到数据）
- 使用 `wait_for_async_insert=0` 的精确一次语义与您的需求冲突
- 您的场景更适合通过改进客户端批处理来优化

##### 异步插入的工作原理 {#how-async-inserts-work}

启用异步插入后，ClickHouse 的处理流程如下：

1. 从连接器接收插入查询
2. 将数据写入内存缓冲区（而非立即写入磁盘）
...
这可以显著减少创建的数据分片数量，并提高整体吞吐量。

##### 启用异步插入 {#enabling-async-inserts}

在 `clickhouseSettings` 配置参数中添加异步插入设置：
...
- **`wait_for_async_insert=1`**（推荐）：连接器在确认前等待数据刷新到 ClickHouse 存储，提供数据交付保证
- **`wait_for_async_insert=0`**：连接器在缓冲后立即确认，性能更好，但在刷新前服务器崩溃时可能丢失数据
...
- **`async_insert_max_data_size`**（默认值：10485760 / 10 MB）：触发刷新的最大缓冲区大小
- **`async_insert_busy_timeout_ms`**（默认值：1000）：触发刷新的最大等待时间（毫秒）
- **`async_insert_stale_timeout_ms`**（默认值：0）：自上次插入后触发刷新的时间（毫秒）
- **`async_insert_max_query_number`**（默认值：100）：触发刷新的最大查询数

**权衡取舍**：

- **优势**：更少的数据分片、更好的合并性能、更低的 CPU 开销、高并发下的吞吐量提升
- **注意事项**：数据无法立即查询、端到端延迟略有增加
- **风险**：如果设置 `wait_for_async_insert=0`，服务器崩溃时可能丢失数据；大缓冲区可能造成内存压力

##### 异步插入与精确一次语义 {#async-inserts-with-exactly-once}

当结合使用 `exactlyOnce=true` 与异步插入时：
...
**重要提示**：使用 exactly-once 时必须配置 `wait_for_async_insert=1`，以确保只有在数据持久化后才提交偏移量。
...
每个任务处理主题分区的一个子集。任务越多，并行度越高，但需要注意：

- 最大有效任务数 = 主题分区数
- 每个任务维护自己到 ClickHouse 的连接
- 任务越多，开销越高，可能导致资源争用

**建议**：将 `tasks.max` 设置为与主题分区数相等，然后根据 CPU 和吞吐量指标进行调整。
...
**警告**：仅在 `exactlyOnce=false` 时使用。此设置可以通过创建更大的批次来提高吞吐量，但会丧失分区级别的顺序保证。
...
**建议**：对于多个高流量主题，为每个主题部署一个连接器实例，以最大化并行插入吞吐量。
...
SETTINGS
    -- 增加最大插入线程数以实现并行写入数据部分
    max_insert_threads = 4,
    -- 允许使用 quorum 插入以提高可靠性（ReplicatedMergeTree）
    insert_quorum = 2
```
...
#### 监控和性能故障排查 {#monitoring-performance}
...
| 超时错误                | 批次大小过大,网络慢            | 减小批次大小,增加 `socket_timeout`,检查网络                 |
| CPU 使用率高            | 过多小数据分片                 | 启用异步插入,增加合并设置                                   |
...
2. **优先使用较大批次**:在可能的情况下,每次插入的目标行数为 10,000-100,000 行
3. **使用异步插入**:在发送大量小批次或高并发情况下使用
4. **始终使用 `wait_for_async_insert=1`**:在精确一次语义下使用
5. **水平扩展**:将 `tasks.max` 增加到分区数量
6. **每个高流量主题使用一个连接器**:以获得最大吞吐量
7. **持续监控**:跟踪消费者延迟、分片数量和合并活动
8. **充分测试**:在生产部署之前,始终在实际负载下测试配置更改
...
默认情况下，Kafka 键不会存储在 value 字段中，但您可以使用 `KeyToValue` 转换将键移动到 value 字段（在新的 `_key` 字段名下）：
