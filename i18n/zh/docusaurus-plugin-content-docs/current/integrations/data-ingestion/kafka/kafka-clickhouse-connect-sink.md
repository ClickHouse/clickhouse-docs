...
description: '来自 ClickHouse 的官方 Kafka 连接器。'
title: 'ClickHouse Kafka Connect Sink'
doc_type: 'guide'
keywords: ['ClickHouse Kafka Connect Sink', 'Kafka ClickHouse 连接器', '官方 ClickHouse 连接器', 'ClickHouse Kafka 集成']
---
...
:::note
如果您需要任何帮助,请[在代码仓库中提交问题](https://github.com/ClickHouse/clickhouse-kafka-connect/issues)或在 [ClickHouse 公共 Slack](https://clickhouse.com/slack) 中提问。
:::
...
- 开箱即用的精确一次(exactly-once)语义。该功能由名为 [KeeperMap](https://github.com/ClickHouse/ClickHouse/pull/39976) 的 ClickHouse 核心新特性提供支持(连接器将其用作状态存储),并支持极简架构。
...
连接器以单个 JAR 文件的形式分发,其中包含运行插件所需的所有类文件。
...
| `tolerateStateMismatch`                          | 允许连接器丢弃"早于"AFTER_PROCESSING 存储的当前偏移量的记录(例如,如果发送偏移量 5,而最后记录的偏移量为 250)                                                             | `"false"`                                                |
| `ignorePartitionsWhenBatching`                   | 在收集消息进行插入时将忽略分区(但仅当 `exactlyOnce` 为 `false` 时)。性能说明:连接器任务越多,每个任务分配的 Kafka 分区越少 - 这可能意味着收益递减 | `"false"`                                                |

### 目标表 {#target-tables}


ClickHouse Connect Sink 从 Kafka 主题读取消息并将其写入相应的表中。ClickHouse Connect Sink 将数据写入现有表。请确保在开始插入数据之前,已在 ClickHouse 中创建了具有适当架构的目标表。

每个主题都需要在 ClickHouse 中有一个专用的目标表。目标表名称必须与源主题名称匹配。

### 预处理 {#pre-processing}

如果您需要在将出站消息发送到 ClickHouse Kafka Connect Sink 之前对其进行转换,请使用 [Kafka Connect Transformations](https://docs.confluent.io/platform/current/connect/transforms/overview.html)。
...
#### 使用死信队列的基本配置 {#basic-configuration-with-dlq}
...
请注意:如果遇到缺少类的问题,并非所有环境都自带 protobuf 转换器,您可能需要使用包含依赖项的备用 jar 版本。
...
该连接器支持以不同 ClickHouse 格式使用字符串转换器:[JSON](/interfaces/formats/JSONEachRow)、[CSV](/interfaces/formats/CSV) 和 [TSV](/interfaces/formats/TabSeparated)。
...
- `running-task-count`：当前运行的任务数
- `paused-task-count`：当前暂停的任务数
- `failed-task-count`：失败的任务数
...
当连接器发送相对较小的批次,或者您希望通过将批处理责任转移到 ClickHouse 来进一步优化数据摄取时,异步插入是一项强大的功能。
...
- **数据分区创建开销**:遇到"数据分区过多"错误
- **混合工作负载**:将实时数据摄取与查询工作负载相结合
...
- **`wait_for_async_insert=0`**:连接器在缓冲后立即确认。性能更好,但在刷新之前服务器崩溃可能会导致数据丢失。
...
- **`async_insert_stale_timeout_ms`**(默认值:0):自上次插入以来刷新前的时间(毫秒)
...
- **风险**:如果 `wait_for_async_insert=0`,服务器崩溃时可能丢失数据;大缓冲区可能导致内存压力
...
如果您的连接器配置为订阅多个主题，使用 `topic2TableMap` 将主题映射到表，并且在插入时遇到瓶颈导致消费者滞后，请考虑为每个主题创建单独的连接器。

出现这种情况的主要原因是当前批次是[串行](https://github.com/ClickHouse/clickhouse-kafka-connect/blob/578ac07e8be1a920aaa3b26e49183595c3edd04b/src/main/java/com/clickhouse/kafka/connect/sink/ProxySinkTask.java#L95-L100)插入到每个表中的。
...
1. **消费者滞后**：使用 Kafka 监控工具跟踪每个分区的滞后情况
...
3. **使用异步插入**,当发送许多小批次或在高并发情况下
4. **始终使用 `wait_for_async_insert=1`**,以实现精确一次语义
5. **水平扩展**:将 `tasks.max` 增加到分区数量
6. **每个高流量主题使用一个连接器**,以获得最大吞吐量
7. **持续监控**:跟踪消费者延迟、分片数量和合并活动
8. **彻底测试**:在生产部署之前,始终在实际负载下测试配置更改
...
这很可能是因为数据中的字段与表中的字段不匹配——这在 CDC（以及 Debezium 格式）中尤为常见。
...
这将把数据从嵌套 JSON 转换为扁平化 JSON(使用 `_` 作为分隔符)。表中的字段将遵循 "field1_field2_field3" 格式(例如 "before_id"、"after_id" 等)。

#### "我想在 ClickHouse 中使用 Kafka 键" {#i-want-to-use-my-kafka-keys-in-clickhouse}

默认情况下,Kafka 键不会存储在值字段中,但您可以使用 `KeyToValue` 转换将键移动到值字段(使用新的 `_key` 字段名):
