...
description: '在 Kafka Connect 与 ClickHouse 中使用 JDBC Sink 连接器'
title: 'JDBC 连接器'
doc_type: 'guide'
...
:::note
此连接器仅适用于简单数据且由基本数据类型(如 int)组成的场景。不支持 ClickHouse 特定类型(如 maps)。
:::

在我们的示例中,使用的是 Confluent 发行版的 Kafka Connect。

下面介绍一个简单的安装过程,从单个 Kafka 主题拉取消息并将行插入到 ClickHouse 表中。对于没有 Kafka 环境的用户,我们推荐使用 Confluent Cloud,它提供了慷慨的免费套餐。

请注意,JDBC 连接器需要 schema(不能在 JDBC 连接器中使用纯 JSON 或 CSV)。虽然 schema 可以编码在每条消息中,但[强烈建议使用 Confluent schema registry](https://www.confluent.io/blog/kafka-connect-deep-dive-converters-serialization-explained/#json-schemas) 以避免相关开销。提供的插入脚本会自动从消息中推断 schema 并将其插入到注册表中 - 因此该脚本可以重复用于其他数据集。Kafka 的键假定为字符串类型。有关 Kafka schema 的更多详细信息,请参见[此处](https://docs.confluent.io/platform/current/schema-registry/index.html)。
...
如果您使用 confluent-hub 安装方法,本地配置文件将自动更新。

要从 Kafka 向 ClickHouse 发送数据,我们使用连接器的 Sink 组件。

#### 2. 下载并安装 JDBC 驱动程序 {#2-download-and-install-the-jdbc-driver}

从[此处](https://github.com/ClickHouse/clickhouse-java/releases)下载并安装 ClickHouse JDBC 驱动程序 `clickhouse-jdbc-<version>-shaded.jar`。按照[此处](https://docs.confluent.io/kafka-connect-jdbc/current/#installing-jdbc-drivers)的详细说明将其安装到 Kafka Connect 中。其他驱动程序可能有效,但尚未经过测试。

:::note

常见问题:文档建议将 jar 复制到 `share/java/kafka-connect-jdbc/`。如果您遇到 Connect 找不到驱动程序的问题,请将驱动程序复制到 `share/confluent-hub-components/confluentinc-kafka-connect-jdbc/lib/`。或者修改 `plugin.path` 以包含驱动程序 - 见下文。
...
- `batch.size` - 单批次发送的行数。请确保将此值设置为适当的较大数值。根据 ClickHouse [建议](/sql-reference/statements/insert-into#performance-considerations),应将 1000 视为最小值。
- `tasks.max` - JDBC Sink 连接器支持运行一个或多个任务。这可用于提高性能。与批次大小一起,这是提高性能的主要手段。
...
确保已创建该表,如果该表已从之前的示例中存在,则将其删除。下面显示了与精简 Github 数据集兼容的示例。请注意,不包含任何当前不支持的 Array 或 Map 类型:
...
此脚本可用于将任何 ndjson 文件插入 Kafka 主题。它会自动尝试推断模式。提供的示例配置仅插入 10k 条消息 - 如有需要可[在此处修改](https://github.com/ClickHouse/clickhouse-docs/tree/main/docs/integrations/data-ingestion/kafka/code/producer/github.config#L25)。此配置还会在插入 Kafka 时从数据集中移除任何不兼容的 Array 字段。
...
Kafka Connect 将开始消费消息并将行插入 ClickHouse。请注意,关于"[JDBC Compliant Mode] Transaction is not supported."的警告是预期行为,可以忽略。

对目标表"Github"执行简单查询即可确认数据已插入。
...
* [Kafka Connect JDBC Sink 深入解析：处理主键](https://rmoff.net/2021/03/12/kafka-connect-jdbc-sink-deep-dive-working-with-primary-keys/)
* [Kafka Connect 实战：JDBC Sink](https://www.youtube.com/watch?v=b-3qN_tlYR4&t=981s) - 适合更习惯看视频而非阅读的读者。
* [Kafka Connect 深入解析 – 转换器和序列化详解](https://www.confluent.io/blog/kafka-connect-deep-dive-converters-serialization-explained/#json-schemas)
