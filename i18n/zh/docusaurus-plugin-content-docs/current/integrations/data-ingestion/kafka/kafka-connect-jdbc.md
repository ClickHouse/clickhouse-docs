...
description: '使用 Kafka Connect 和 ClickHouse 的 JDBC 连接器 Sink'
title: 'JDBC 连接器'
doc_type: 'guide'
...
:::note
此连接器仅适用于简单数据且由基本数据类型(如 int)组成的场景。不支持 ClickHouse 特定类型(如 map)。
:::
...
常见问题:文档建议将 jar 文件复制到 `share/java/kafka-connect-jdbc/`。如果您遇到 Connect 找不到驱动程序的问题,请将驱动程序复制到 `share/confluent-hub-components/confluentinc-kafka-connect-jdbc/lib/`。或者修改 `plugin.path` 以包含驱动程序 - 见下文。
...
确保已创建该表，如果该表已从之前的示例中存在，请先删除它。下面显示了与精简 Github 数据集兼容的示例。请注意，不包含任何当前不支持的 Array 或 Map 类型：
...
* [Kafka Connect 深入解析 – JDBC Source 连接器](https://www.confluent.io/blog/kafka-connect-deep-dive-jdbc-source-connector)
* [Kafka Connect JDBC Sink 深入解析：主键处理](https://rmoff.net/2021/03/12/kafka-connect-jdbc-sink-deep-dive-working-with-primary-keys/)
* [Kafka Connect 实战：JDBC Sink](https://www.youtube.com/watch?v=b-3qN_tlYR4&t=981s) - 适合更喜欢“看视频而不是阅读”的读者。
* [Kafka Connect 深入解析 – 转换器与序列化详解](https://www.confluent.io/blog/kafka-connect-deep-dive-converters-serialization-explained/#json-schemas)
