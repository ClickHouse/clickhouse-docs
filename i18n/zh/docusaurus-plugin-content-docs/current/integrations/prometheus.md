...
该功能支持将 [Prometheus](https://prometheus.io/) 集成到 ClickHouse Cloud 服务中进行监控。通过 [ClickHouse Cloud API](/cloud/manage/api/api-overview) 端点访问 Prometheus 指标，允许用户安全地连接并将指标导出到他们的 Prometheus 指标收集器中。这些指标可以与仪表板集成，例如 Grafana、Datadog 进行可视化。

要开始使用，请 [生成一个 API 密钥](/cloud/manage/openapi)。
...
使用您的 ClickHouse Cloud API 密钥进行基础认证：
...
ClickHouse Cloud 提供了一个特殊的指标 `ClickHouse_ServiceInfo`，这是一个始终值为 `1` 的 `gauge`。该指标包含所有的 **指标标签** 以及以下标签：

| 标签                    | 描述                                             |
|------------------------|---------------------------------------------------|
| clickhouse_cluster_status | 服务状态。可以是以下之一: [`awaking` \| `running` \| `degraded` \| `idle` \| `stopped`] |
| clickhouse_version      | 服务运行的 ClickHouse 服务器的版本               |
| scrape                  | 表示上次抓取的状态。可以是 `full` 或 `partial`  |
| full                    | 表示上次抓取指标时没有错误                       |
| partial                 | 表示上次抓取指标时有一些错误，仅返回了 `ClickHouse_ServiceInfo` 指标。 |

请求以检索指标的操作不会恢复处于空闲状态的服务。如果服务处于 `idle` 状态，则只会返回 `ClickHouse_ServiceInfo` 指标。

对于 ClickPipes，还有一个类似的 `ClickPipes_Info` 指标 `gauge`，除了 **指标标签** 外，还包含以下标签：
...
Prometheus 服务器在给定的间隔内从配置的目标收集指标。以下是 Prometheus 服务器使用 ClickHouse Cloud Prometheus 端点的示例配置：
...
请注意，`honor_labels` 配置参数需要设置为 `true`，以确保实例标签能够正确填充。此外，上述示例中的 `filtered_metrics` 设置为 `true`，但应根据用户偏好进行配置。

## 与 Grafana 集成 {#integrating-with-grafana}

用户有两种主要方式与 Grafana 集成：

- **指标端点** - 此方法的优势在于不需要任何额外的组件或基础设施。此选项仅限于 Grafana Cloud，只需 ClickHouse Cloud Prometheus 端点 URL 和凭据即可。
- **Grafana Alloy** - Grafana Alloy 是 OpenTelemetry (OTel) Collector 的中立分发版，取代了 Grafana Agent。可以作为抓取器使用，部署在您自己的基础设施中，并兼容任何 Prometheus 端点。

我们在下面提供了使用这些选项的说明，重点介绍与 ClickHouse Cloud Prometheus 端点相关的具体细节。

### 使用指标端点的 Grafana Cloud {#grafana-cloud-with-metrics-endpoint}

- 登录到您的 Grafana Cloud 帐户
- 通过选择 **指标端点** 添加新连接
- 配置抓取 URL 指向 Prometheus 端点，并使用基本认证配置您的 API 密钥/秘密连接
- 测试连接以确保您能够连接
...
配置后，您应该在下拉菜单中看到可以选择配置仪表板的指标：

<Image img={prometheus_grafana_dropdown} size="md" alt="Grafana 指标浏览器下拉菜单" border/>

<br />

<Image img={prometheus_grafana_chart} size="md" alt="Grafana 指标浏览器图表" border/>

### 使用 Alloy 的 Grafana Cloud {#grafana-cloud-with-alloy}

如果您使用的是 Grafana Cloud，可以通过导航到 Grafana 中的 Alloy 菜单并按照屏幕上的说明进行安装：

<Image img={prometheus_grafana_alloy} size="md" alt="Grafana Alloy" border/>

<br />

这应配置 Alloy 以通过带有身份验证令牌的 `prometheus.remote_write` 组件将数据发送到 Grafana Cloud 端点。用户只需修改 Alloy 配置（在 Linux 中位于 `/etc/alloy/config.alloy`）以包括 ClickHouse Cloud Prometheus 端点的抓取器。

以下显示了使用 `prometheus.scrape` 组件从 ClickHouse Cloud 端点抓取指标的 Alloy 配置示例，以及自动配置的 `prometheus.remote_write` 组件。请注意，`basic_auth` 配置组件包含我们的 Cloud API 密钥 ID 和秘密（分别作为用户名和密码）。
...
请注意，`honor_labels` 配置参数需要设置为 `true`，以确保实例标签能够正确填充。

### 自管理的 Grafana 与 Alloy {#grafana-self-managed-with-alloy}

自管理的 Grafana 用户可以在 [这里](https://grafana.com/docs/alloy/latest/get-started/install/) 找到安装 Alloy 代理的说明。我们假设用户已配置 Alloy 将 Prometheus 指标发送到所需的目标。下面的 `prometheus.scrape` 组件导致 Alloy 抓取 ClickHouse Cloud 端点。我们假设 `prometheus.remote_write` 接收抓取到的指标。如果不存在，请调整 `forward_to` 键以指向目标位置。
...
配置后，您应该在指标浏览器中看到与 ClickHouse 相关的指标：
...
您可以使用 Datadog [代理](https://docs.datadoghq.com/agent/?tab=Linux) 和 [OpenMetrics 集成](https://docs.datadoghq.com/integrations/openmetrics/) 从 ClickHouse Cloud 端点收集指标。以下是该代理和集成的简单示例配置。不过，请注意您可能只想选择您最关心的指标。下面的通用示例将导出数千个指标实例组合，Datadog 将其视为自定义指标。
