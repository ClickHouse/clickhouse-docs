...
DataGrip 可从 https://www.jetbrains.com/datagrip/ 下载
...
1. 启动 DataGrip,在 **Data Sources and Drivers** 对话框的 **Data Sources** 选项卡上,单击 **+** 图标

<Image
  img={datagrip_5}
  size='lg'
  border
  alt='DataGrip Data Sources 选项卡,+ 图标已高亮显示'
/>
...
- 切换到 **Drivers** 选项卡并加载 ClickHouse 驱动程序
  ...
  为了减小下载大小,DataGrip 不自带驱动程序。在 **Drivers** 选项卡上,从 **Complete Support** 列表中选择 **ClickHouse**,然后展开 **+** 号。从 **Provided Driver** 选项中选择 **Latest stable** 驱动程序:

<Image
  img={datagrip_1}
  size='lg'
  border
  alt='DataGrip Drivers 选项卡显示 ClickHouse 驱动程序安装过程'
/>
...
:::tip
在 **Host** 字段中仅输入主机名(例如 `your-host.clickhouse.cloud`),不要包含 `https://` 等协议前缀。

对于 ClickHouse Cloud 连接,必须在主机下方的 **URL** 字段中添加 `?ssl=true`。完整的 JDBC URL 格式如下:

`jdbc:clickhouse://your-host.clickhouse.cloud:8443/default?ssl=true`

ClickHouse Cloud 要求所有连接使用 SSL 加密。如果缺少 `?ssl=true` 参数,即使凭据正确也会出现"Connection reset"错误。
