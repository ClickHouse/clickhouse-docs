...
description: 'marimo 是用于数据交互的下一代 Python 笔记本'
title: '将 marimo 与 ClickHouse 配合使用'
doc_type: '指南'
...
[marimo](https://marimo.io/) 是一个开源的 Python 响应式笔记本，内置 SQL。每当你运行某个单元格或与界面元素交互时，marimo 会自动运行受影响的单元格（或将其标记为已失效），从而保持代码与输出一致，并在问题出现之前预防错误。每个 marimo 笔记本都以纯 Python 形式存储，可作为脚本执行，也可部署为应用程序。
...
现在,您可以在数据框中查看结果。我们希望可视化从指定上车地点出发的最昂贵下车地点。marimo 提供了多个 UI 组件来帮助您实现这一目标。我们将使用下拉菜单选择地点,并使用 altair 进行图表绘制。
...
marimo 的响应式执行模型扩展到 SQL 查询,因此对 SQL 的更改将自动触发依赖单元格的下游计算(或可选地将单元格标记为过时状态以处理高开销的计算)。因此,当查询更新时,图表和表格会相应变化。

您还可以切换到应用视图,以获得简洁的界面来探索数据。
