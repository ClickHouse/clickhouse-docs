...
DBeaver 提供多个版本。在本指南中，我们使用 [DBeaver Community](https://dbeaver.io/)。请在[此处](https://dbeaver.com/edition/)查看不同版本及其功能。DBeaver 通过 JDBC 连接到 ClickHouse。

:::note
请使用 23.1.0 或更高版本的 DBeaver，以更好地支持 ClickHouse 中的 `Nullable` 列。
:::
...
DBeaver 可从 https://dbeaver.io/download/ 获取
...
<Image
  ...
  size='md'
  border
  alt='设置主机名、端口、用户、密码和数据库名称'
/>

- 默认情况下 **SSL > Use SSL** 属性未设置。如果您要连接到 ClickHouse Cloud 或需要在 HTTP 端口上使用 SSL 的服务器,请启用 **SSL > Use SSL**:

<Image img={dbeaver_use_ssl} size='md' border alt='如需要则启用 SSL' />
...
如果 DBeaver 检测到您未安装 ClickHouse 驱动程序,它将提示为您下载:
