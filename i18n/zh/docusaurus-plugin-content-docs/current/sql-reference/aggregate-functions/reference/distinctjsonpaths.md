...
计算存储在 [JSON](../../data-types/newjson.md) 列中的不重复路径列表。
...
计算存储在 [JSON](../../data-types/newjson.md) 列中的不重复路径及其类型的列表。
...
如果 JSON 声明包含具有指定类型的路径，即使输入数据没有这些路径的值，这些路径也会始终包含在 `distinctJSONPaths/distinctJSONPathsAndTypes` 函数的结果中。
