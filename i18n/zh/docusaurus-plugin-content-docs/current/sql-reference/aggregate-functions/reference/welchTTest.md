...
两个样本的取值都在 `sample_data` 列中。如果 `sample_index` 等于 0，则该行中的值属于第一总体的样本，否则属于第二总体的样本。

原假设认为两个总体的均值相等。假定样本服从正态分布，总体方差可以不相等。
...
* [Welch t 检验（Welch&#39;s t-test）](https://en.wikipedia.org/wiki/Welch%27s_t-test)
