...
两个样本的值在 `sample_data` 列中。如果 `sample_index` 等于 0，则该行中的值属于第一个总体的样本。否则，它属于第二个总体的样本。零假设是总体的均值相等。假设服从正态分布。总体可能具有不等的方差，并且方差是已知的。
...
- `confidence_level` — 为计算置信区间而设定的置信水平。 [浮点数](../../../sql-reference/data-types/float.md)。
