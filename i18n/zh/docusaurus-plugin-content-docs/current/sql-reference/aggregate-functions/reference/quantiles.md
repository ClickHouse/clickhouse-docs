...
所有的分位数函数也有对应的分位数函数：`quantiles`, `quantilesDeterministic`, `quantilesTiming`, `quantilesTimingWeighted`, `quantilesExact`, `quantilesExactWeighted`, `quantileExactWeightedInterpolated`, `quantileInterpolatedWeighted`, `quantilesTDigest`, `quantilesBFloat16`, `quantilesDD`。这些函数在一次调用中计算所有列出级别的分位数，并返回一个结果值的数组。

## quantilesExactExclusive {#quantilesexactexclusive}

精确计算数值数据序列的 [分位数](https://en.wikipedia.org/wiki/Quantile)。

为了获得精确的值，所有传入的值都会被组合成一个数组，然后进行部分排序。因此，该函数消耗 `O(n)` 内存，其中 `n` 是传入值的数量。然而，对于少量值，该函数的效率非常高。
...
在处理一组级别时，该函数的工作效率优于 [quantileExactExclusive](../../../sql-reference/aggregate-functions/reference/quantileexact.md#quantileexactexclusive)。
...
- 对于数值数据类型输入，使用 [Float64](../../../sql-reference/data-types/float.md)。
- 如果输入值具有 `Date` 类型，则使用 [Date](../../../sql-reference/data-types/date.md)。
- 如果输入值具有 `DateTime` 类型，则使用 [DateTime](../../../sql-reference/data-types/datetime.md)。
...
在处理一组级别时，该函数的工作效率优于 [quantileExactInclusive](../../../sql-reference/aggregate-functions/reference/quantileexact.md#quantileexactinclusive)。
...
`quantilesGK` 类似于 `quantileGK`，但允许我们同时在不同级别计算分位数并返回一个数组。
