...
所有分位数函数都有对应的 quantiles 系列函数：`quantiles`、`quantilesDeterministic`、`quantilesTiming`、`quantilesTimingWeighted`、`quantilesExact`、`quantilesExactWeighted`、`quantileExactWeightedInterpolated`、`quantileInterpolatedWeighted`、`quantilesTDigest`、`quantilesBFloat16`、`quantilesDD`。这些函数在一次遍历中计算所有指定级别的分位数,并返回包含结果值的数组。
...
在处理多个分位数级别时,比 [quantileExactExclusive](../../../sql-reference/aggregate-functions/reference/quantileexact.md#quantileexactexclusive) 更高效。
...
- `expr` — 列值表达式,结果为数值[数据类型](/sql-reference/data-types)、[Date](../../../sql-reference/data-types/date.md) 或 [DateTime](../../../sql-reference/data-types/datetime.md)。

**参数**

- `level` — 分位数级别。可能的值:(0, 1) — 不包含边界。[Float](../../../sql-reference/data-types/float.md)。

**返回值**

- 指定级别的分位数[数组](../../../sql-reference/data-types/array.md)。
...
此函数等同于 Excel 的 [PERCENTILE.INC](https://support.microsoft.com/en-us/office/percentile-inc-function-680f9539-45eb-410b-9a5e-c1355e5fe2ed) 函数([R7 类型](https://en.wikipedia.org/wiki/Quantile#Estimating_quantiles_from_a_sample))。

在处理多个分位数级别时,比 [quantileExactInclusive](../../../sql-reference/aggregate-functions/reference/quantileexact.md#quantileexactinclusive) 更高效。
...
- `level` — 分位数级别。可能的值:[0, 1] — 包含边界。[Float](../../../sql-reference/data-types/float.md)。
...
`quantilesGK` 的工作方式与 `quantileGK` 类似,但允许同时计算不同级别的分位数并返回一个数组。
