...
在此示例中，使用了 DateTime 类型的参数。该函数以秒为单位返回值。
...
在此示例中，使用了 Date 类型的参数。该函数以天为单位返回值。
