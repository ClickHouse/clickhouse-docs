---
description: '根据 `key` 数组中指定的键，对一个或多个 `value` 数组求和。返回一个由数组构成的 tuple：首先是按排序顺序排列的键数组，其次是针对相应键汇总后的值数组，且不会发生溢出。'
...
* `value1`、`value2`、...：对每个键进行求和的值的 [Array](../../data-types/array.md)。

传递一个由键数组和值数组组成的元组，与分别传入一个键数组和若干值数组是等价的。

:::note
对每一行进行汇总时，`key` 数组与所有 `value` 数组中的元素数量必须相同。
:::

**返回值**

* 返回一个数组形式的元组：第一个数组包含按排序顺序排列的键，后续的数组包含针对相应键求和后的值。

**示例**

首先我们创建一个名为 `sum_map` 的表，并向其中插入一些数据。键数组和值数组分别作为名为 `statusMap` 的 [Nested](../../data-types/nested-data-structures/index.md) 类型列存储，同时也一并作为名为 `statusMapTuple` 的 [Tuple](../../data-types/tuple.md) 类型列存储，用于演示上述函数两种不同语法的用法。
...
接下来，我们使用 `sumMap` 函数查询该表，同时演示数组和元组类型两种语法：
