...
该函数绘制值 `x` 的频率直方图，以及这些值在区间 `[min_x, max_x]` 内的重复率 `y`。所有落入同一桶的 `x` 的重复次数将被平均，因此数据应预先聚合。负重复次数将被忽略。

如果未指定区间，则将最小 `x` 用作区间起始，最大 `x` 用作区间结束。否则，区间外的值将被忽略。
...
- `buckets` — 段的数量。类型：[Integer](../../../sql-reference/data-types/int-uint.md)。
- `min_x` — 区间起始。可选参数。
- `max_x` — 区间结束。可选参数。
