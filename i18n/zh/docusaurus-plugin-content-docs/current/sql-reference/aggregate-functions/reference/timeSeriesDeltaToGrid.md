...
该聚合函数接收由时间戳和值组成的时间序列数据对，并在由起始时间戳、结束时间戳和步长描述的等间隔时间网格上，从这些数据中计算[类似 PromQL 的 delta](https://prometheus.io/docs/prometheus/latest/querying/functions/#delta)。对于网格上的每个点，用于计算 `delta` 的样本会在指定的时间窗口内进行选取。
...
* `staleness` - 指定被考虑样本的最大“陈旧度”（以秒为单位）。陈旧度窗口为左开右闭区间。
...
返回值：
指定网格上的 `delta` 值，类型为 `Array(Nullable(Float64))`。返回的数组为每个时间网格点包含一个值。如果在窗口内的样本不足以计算某个网格点的 delta 值，则该值为 NULL。
...
还可以将多组时间戳和值样本作为长度相同的 `Array` 传入。使用数组参数的同一个查询如下：
