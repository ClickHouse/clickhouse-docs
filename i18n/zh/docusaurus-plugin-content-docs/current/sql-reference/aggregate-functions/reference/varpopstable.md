---
'description': '返回总体方差。与 varPop 不同，该函数使用一种数值稳定的算法。它的运行速度较慢，但提供了更低的计算误差。'
...
返回总体方差。与 [`varPop`](../reference/varpop.md) 不同，此函数使用[数值稳定](https://en.wikipedia.org/wiki/Numerical_stability)算法。它运行较慢，但提供更低的计算误差。
