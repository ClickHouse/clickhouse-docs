---
description: '在指定网格上，对时间序列数据计算类似 PromQL 的 rate 的聚合函数。'
...
该聚合函数接收时间戳和值成对的时间序列数据，并在由起始时间戳、结束时间戳和步长描述的规则时间网格上，从这些数据计算出[类似 PromQL 的 rate](https://prometheus.io/docs/prometheus/latest/querying/functions/#rate)。对于网格上的每个点，用于计算 `rate` 的样本都限定在指定的时间窗口内。
...
* `staleness` - 指定所考虑样本允许的最大“陈旧度”，以秒为单位。陈旧度窗口是左开右闭区间。
...
返回值：
在指定网格上的 `rate` 值，类型为 `Array(Nullable(Float64))`。返回的数组针对每个时间网格点包含一个值。如果在窗口内没有足够的样本来为某个网格点计算 rate 值，则该值为 NULL。

示例：
下面的查询在网格 [90, 105, 120, 135, 150, 165, 180, 195, 210] 上计算 `rate` 值：
...
还可以将多个时间戳和值样本以长度相同的数组形式传入。使用数组参数的同一查询如下：
...
:::note
这是一个实验性函数，可通过设置 `allow_experimental_ts_to_grid_aggregate_function=true` 来启用。
:::
