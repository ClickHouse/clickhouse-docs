...
- `codec` - [字符串](../../../sql-reference/data-types/string.md)，包含一个[压缩编解码器](/sql-reference/statements/create/table#column_compression_codec)或多个以逗号分隔的编解码器。
- `block_size_bytes` - 压缩数据的块大小。这类似于同时设置[`max_compress_block_size`](../../../operations/settings/merge-tree-settings.md#max_compress_block_size)和[`min_compress_block_size`](../../../operations/settings/merge-tree-settings.md#min_compress_block_size)。默认值为1 MiB（1048576字节）。
