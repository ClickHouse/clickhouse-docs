...
一个聚合函数，用于接收时间戳和值成对的时间序列数据，并且最多只存储最近的 2 个样本。该聚合函数旨在与物化视图和用于存储针对网格对齐时间戳的重采样时间序列数据的聚合表一起使用。

聚合表仅为每个对齐的时间戳存储最后 2 个值。这样可以在只读取远少于原始表中存储数据量的情况下，计算类似 PromQL 的 `irate` 和 `idelta`。

:::warning
该函数为实验性功能，可通过设置 `allow_experimental_ts_to_grid_aggregate_function=true` 启用。
:::
...
* `timestamp` — 样本的时间戳。[`DateTime`](/sql-reference/data-types/datetime) 或 [`DateTime64`](/sql-reference/data-types/datetime64) 或 [`(U)Int*`](/sql-reference/data-types/int-uint) 或 [`Int*`](/sql-reference/data-types/int-uint)
* `value` — 对应时间戳的时间序列值。[`Float32`](/sql-reference/data-types/float) 或 [`Float64`](/sql-reference/data-types/float)

**返回值**

返回一对长度在 0 到 2 之间且等长的数组。第一个数组包含采样后时间序列的时间戳，第二个数组包含时间序列对应的数值。[`Tuple(Array(DateTime), Array(Float64))`](/sql-reference/data-types/tuple)

**示例**

**原始数据示例表，以及一个用于存储重采样数据的表**
