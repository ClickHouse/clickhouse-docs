...
* 返回 `x` 和 `y` 的秩之间的秩相关系数。相关系数的取值范围为 -1 到 +1。若传入的参数少于两个，函数将抛出异常。值接近 +1 表示较强的线性关系，且当一个随机变量增大时，另一个随机变量也随之增大。值接近 -1 表示较强的线性关系，但当一个随机变量增大时，另一个随机变量减小。值接近或等于 0 表示两个随机变量之间不存在相关性。
