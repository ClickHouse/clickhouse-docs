...
每个 `value` 对应一个确定的 `timeunit`。
半衰期 `x` 是指数权重衰减到一半时对应的时间延迟。
该函数返回加权平均值：时间点越久远，其对应值所占的权重就越小。
...
在最新时刻返回基于过去 `x` 个时间单位内数值计算的指数平滑移动平均值。[`Float64`](/sql-reference/data-types/float)
