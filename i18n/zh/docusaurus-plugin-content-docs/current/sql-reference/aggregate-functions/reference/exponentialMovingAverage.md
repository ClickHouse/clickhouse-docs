---
description: '计算在指定时间段内数值的指数移动平均值。'
...
每个 `value` 对应一个特定的 `timeunit`。半衰期 `x` 是指数权重衰减到一半所对应的时间滞后。该函数返回一个加权平均值：时间点越久远，其对应的数值权重就越小。
...
* 返回在最新时间点上，过去 `x` 个时间单位内数值的[指数平滑移动平均](https://en.wikipedia.org/wiki/Moving_average#Exponential_moving_average)。
