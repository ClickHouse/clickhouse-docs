...
在一个查询中使用多个具有不同分位水平的 `quantile*` 函数时，其内部状态不会被合并（也就是说，该查询的执行效率会低于理论最优）。在这种情况下，请使用 [quantiles](../../../sql-reference/aggregate-functions/reference/quantiles.md#quantiles) 函数。
...
* `level` — 分位数的级别。可选参数。0 到 1 之间的常量浮点数。建议使用 `[0.01, 0.99]` 范围内的 `level` 值。默认值：0.5。当 `level=0.5` 时，函数计算[中位数](https://en.wikipedia.org/wiki/Median)。
...
:::note\
如果没有向函数传递任何值（在使用 `quantileTimingIf` 时），则返回 [NaN](/sql-reference/data-types/float#nan-and-inf)。这样做的目的是将这些情况与结果为零的情况区分开来。有关 `NaN` 值排序的说明，请参阅 [ORDER BY 子句](/sql-reference/statements/select/order-by)。
:::
