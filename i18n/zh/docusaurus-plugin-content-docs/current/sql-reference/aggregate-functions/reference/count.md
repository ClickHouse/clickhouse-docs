...
ClickHouse 支持 `COUNT(DISTINCT ...)` 语法。这种写法的行为取决于 [count&#95;distinct&#95;implementation](../../../operations/settings/settings.md#count_distinct_implementation) 设置。它定义执行该操作时使用哪个 [uniq*](/sql-reference/aggregate-functions/reference/uniq) 函数。默认使用 [uniqExact](/sql-reference/aggregate-functions/reference/uniqExact.md) 函数。
...
此示例表明，`count(DISTINCT num)` 会根据 `count_distinct_implementation` 设置的值由 `uniqExact` 函数执行。
