...
* `x1[, x2, ...]` — 一个或多个参数，用于计算样本协方差矩阵。[`(U)Int*`](/sql-reference/data-types/int-uint) 或 [`Float*`](/sql-reference/data-types/float) 或 [`Decimal`](/sql-reference/data-types/decimal)

**返回值**

返回样本协方差矩阵。[`Array(Array(Float64))`](/sql-reference/data-types/array)
