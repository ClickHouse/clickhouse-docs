---
'description': "Calculates the value of `\tΣ((x - x̄)(y - ȳ)) / (n - 1)`"
...
:::note
此函数使用数值不稳定的算法。如果在计算中需要 [数值稳定性](https://en.wikipedia.org/wiki/Numerical_stability)，请使用 [`covarSampStable`](../reference/covarsamp.md) 函数。它的速度较慢，但提供了更低的计算误差。
:::
...
- `x` 和 `y` 之间的样本协方差。在 `n <= 1` 的情况下，返回 `nan`。[Float64](../../data-types/float.md)。
