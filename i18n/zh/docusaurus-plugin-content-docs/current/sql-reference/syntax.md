在本节中，我们将看看 ClickHouse 的 SQL 语法。 
ClickHouse 使用基于 SQL 的语法，但提供了许多扩展和优化。

## 查询解析 {#query-parsing}

ClickHouse 有两种类型的解析器：
- _完整的 SQL 解析器_（递归下降解析器）。
- _数据格式解析器_（快速流解析器）。

完整的 SQL 解析器在除 `INSERT` 查询的所有情况下使用，而 `INSERT` 查询则同时使用这两种解析器。
...
您还可以通过使用 [`input_format_values_interpret_expressions`](../operations/settings/settings-formats.md#input_format_values_interpret_expressions) 设置来启用数据的完整解析器。
...
（默认为 1 MB），其余部分使用流解析。
这样做是为了避免大 `INSERT` 查询的问题，推荐的方式是在 ClickHouse 中插入数据。

在 `INSERT` 查询中使用 [`Values`](../interfaces/formats.md/#data-format-values) 格式时， 
可能看起来数据的解析与 `SELECT` 查询中的表达式相同，但事实并非如此。
`Values` 格式是非常有限的。
...
- 空格符号包括空格、制表符、换行符、回车和分页符。

## 注释 {#comments}

ClickHouse 支持 SQL 风格和 C 风格的注释：

- SQL 风格的注释以 `--`、`#!` 或 `# ` 开头，并持续到行的末尾。在 `--` 和 `#!` 后的空格可以省略。
- C 风格的注释从 `/*` 到 `*/`，可以是多行的。空格也不是必需的。

## 关键字 {#keywords}

ClickHouse 中的关键字可以根据上下文是 _区分大小写_ 或 _不区分大小写_。

关键字在以下情况下是 **不区分大小写** 的：

- SQL 标准。例如，`SELECT`、`select` 和 `SeLeCt` 都是有效的。
- 在某些流行的 DBMS（MySQL 或 Postgres）中的实现。例如，`DateTime` 与 `datetime` 是相同的。

:::note
您可以在 [system.data_type_families](/operations/system-tables/data_type_families) 表中检查数据类型名称是否区分大小写。
:::

与标准 SQL 相反，所有其他关键字（包括函数名）是 **区分大小写** 的。

此外，关键字不是保留的。 
它们仅在相应的上下文中被视为关键字。 
如果您使用与关键字同名的 [标识符](#identifiers)，请将它们用双引号或反引号括起来。 

例如，如果表 `table_name` 有一列名为 `"FROM"`，则以下查询是有效的：
...
如果要使用与关键字相同的标识符或您想在标识符中使用其他符号，请使用双引号或反引号将其括起来，例如 `"id"`、 `` `id` ``。

:::note
对带引号标识符的转义规则同样适用于字符串字面值。有关更多详细信息，请参见 [字符串](#string)。
:::

## 字面量 {#literals}

在 ClickHouse 中，字面量是直接在查询中表示的值。
换言之，它是一个在查询执行期间不变的固定值。
...
- 使用前导单引号，其中单引号字符 `'`（仅此字符）可以转义为 `''`，或者
- 使用前导反斜杠和下表中列出的支持的转义序列。

:::note
反斜杠失去其特殊含义，即在前缀为其他字符时，它被逐字解释。
:::

| 支持的转义                    | 描述                                                             |
|------------------------------|-------------------------------------------------------------------|
| `\xHH`                       | 8 位字符规范，后跟任意数量的十六进制数字 (H)。                   | 
| `\N`                         | 保留，不做任何操作（例如 `SELECT 'a\Nb'` 返回 `ab`）              |
| `\a`                         | 响铃                                                             |
| `\b`                         | 退格                                                             |
| `\e`                         | 转义字符                                                         |
| `\f`                         | 分页符                                                           |
...
- 首先作为 64 位有符号数字，使用 [strtoull](https://en.cppreference.com/w/cpp/string/byte/strtoul) 函数。
- 如果不成功，则作为 64 位无符号数字，使用 [strtoll](https://en.cppreference.com/w/cpp/string/byte/strtol) 函数。
- 如果仍不成功，则作为浮点数，使用 [strtod](https://en.cppreference.com/w/cpp/string/byte/strtof) 函数。
- 否则返回错误。
...
| **浮点数**                          | `123.456`，`inf`，`nan`                        |
| **十六进制**                        | `0xc0fe`                                       |
| **支持与 SQL 标准兼容的十六进制字符串** | `x'c0fe'`                                      |
| **二进制**                          | `0b1101`                                       |
| **支持与 SQL 标准兼容的二进制字符串** | `b'1101'`                                      |
...
一个数组必须至少包含一个项，而一个元组必须至少有两个项。

:::note
当元组出现在 `SELECT` 查询的 `IN` 子句中时，还有一种特殊情况。 
...
`NULL` 用于指示值缺失。 
要在表字段中存储 `NULL`，必须是 [Nullable](../sql-reference/data-types/nullable.md) 类型。

:::note
有关 `NULL` 应注意以下事项：

- 根据数据格式（输入或输出），`NULL` 可能有不同的表示。有关更多信息，请参见 [数据格式](/interfaces/formats)。
- `NULL` 的处理是微妙的。例如，如果比较操作的至少一个参数是 `NULL`，则该操作的结果也为 `NULL`。乘法、加法和其他操作也是如此。我们建议查看每个操作的文档。
...
Heredoc 被定义为位于两个 `$` 符号之间的自定义字符串字面量。
...
查询参数使您能够编写包含抽象占位符而不是具体标识符的通用查询。 
当执行带有查询参数的查询时， 
所有占位符都会解析并替换为实际的查询参数值。
...
可以在查询中通过 `{<name>: <datatype>}` 引用查询参数，其中 `<name>` 是查询参数名称，`<datatype>` 是转换后的数据类型。

<details>
<summary>使用 SET 命令的示例</summary>

例如，以下 SQL 定义了名称为 `a`、`b`、`c` 和 `d` 的参数，且各自具有不同的数据类型：
...
如果您使用 `clickhouse-client`，参数指定为 `--param_name=value`。例如，以下参数的名称为 `message`，它作为 `String` 被检索：
...
如果查询参数代表数据库、表、函数或其他标识符的名称，请使用 `Identifier` 作为其类型。例如，以下查询返回来自名为 `uk_price_paid` 的表的行：
...
:::note
查询参数不是可以在任意 SQL 查询的任意位置使用的通用文本替代。 
它们主要用于在 `SELECT` 语句中替代标识符或字面量。
:::

## 函数 {#functions}

函数调用的写法类似于标识符，后跟一对括号中的参数列表（可以为空）。 
与标准 SQL 相比，括号是必需的，即使参数列表为空。 
例如： 
...
运算符在查询解析过程中被转换为其对应的函数，考虑它们的优先级和结合性。
...
在 `CREATE` 查询中，数据类型和表引擎的写法与标识符或函数的写法相同。 
换句话说，它们可能包含参数列表，也可能不包含。
...
它还可以包含一个 [别名](#expression-aliases)。
...
| 语法部分  | 描述                                                                                                                                      | 示例                                                                   | 备注                                                                                                                                          |
|------------|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| `AS`       | 定义别名的关键字。您可以在 `SELECT` 子句中为表名或列名定义别名，而无需使用 `AS` 关键字。 | `SELECT table_name_alias.column_name FROM table_name table_name_alias`。 | 在 [CAST](/sql-reference/functions/type-conversion-functions#cast) 函数中，`AS` 关键字有另一种含义。请参阅函数的描述。                       |
...
- 别名对查询或子查询是全局的，您可以在查询的任何部分为任何表达式定义别名。例如：

```sql
SELECT (1 AS n) + 2, n`.
```

- 别名在子查询和子查询之间不可见。例如，执行以下查询时，ClickHouse 会生成异常 `Unknown identifier: num`：
...
在前面的示例中，我们声明了表 `t` 和列 `b`。 
然后，在选择数据时，我们定义了 `sum(b) AS b` 别名。 
由于别名是全局的， 
ClickHouse 在表达式 `argMax(a, b)` 中将文字 `b` 替换为表达式 `sum(b)`。 
这个替换导致了异常。
