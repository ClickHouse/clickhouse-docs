...
:::tip[避免在列名中使用点号]
当 `flatten_nested = 1` (默认值) 时，包含点号的列名、共享相同点号前缀的列以及类型为 `Array` 的列，都可能被解释为扁平化的 `Nested` 结构的一部分。这可能会在插入时导致意外的数组长度校验以及重命名限制。

如有可能，请避免在列名中使用点号。
除非你明确需要 `Nested` 语义，否则在列名中应使用下划线 (`_`) 或其他分隔符来代替点号。
:::
