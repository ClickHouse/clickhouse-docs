...
ClickHouse 在查询解析阶段根据运算符的优先级、优先性和结合性将运算符转化为其对应的函数。

## 访问运算符 {#access-operators}

`a[N]` – 访问数组的一个元素。对应的函数是 `arrayElement(a, N)`。

`a.N` – 访问元组的一个元素。对应的函数是 `tupleElement(a, N)`。

## 数值取反运算符 {#numeric-negation-operator}

`-a` – 对应的函数是 `negate(a)`。

对于元组取反：[tupleNegate](../../sql-reference/functions/tuple-functions.md#tuplenegate)。
...
`a BETWEEN b AND c` – 与 `a >= b AND a <= c`等价。

`a NOT BETWEEN b AND c` – 与 `a < b OR a > c`等价。
...
从给定日期提取部分。例如，可以从给定日期提取月份，或者从时间提取秒。

`part` 参数指定要提取的日期部分。可用的值如下：
...
创建一个 [Interval](../../sql-reference/data-types/special-data-types/interval.md) 类型的值，以便在与 [Date](../../sql-reference/data-types/date.md) 和 [DateTime](../../sql-reference/data-types/datetime.md) 类型的值进行算术操作时使用。
...
设置 `INTERVAL` 值时，您还可以使用字符串文字。例如，`INTERVAL 1 HOUR` 与 `INTERVAL '1 hour'` 或 `INTERVAL '1' hour` 是相同的。

:::tip    
不同类型的时间间隔不能组合。不能使用 `INTERVAL 4 DAY 1 HOUR` 这样的表达式。指定较小或等于时间间隔最小单位的单位，例如，`INTERVAL 25 HOUR`。您可以使用连续的操作，如下面的示例所示。
:::
...
:::note    
`INTERVAL` 语法或 `addDays` 函数始终是首选。简单的加法或减法（语法如 `now() + ...`）不考虑时间设置。例如，夏令时。
:::
...
语法 `SELECT a AND b` — 计算 `a` 和 `b` 的逻辑连接，使用函数 [and](/sql-reference/functions/logical-functions#and)。

## 逻辑或运算符 {#logical-or-operator}

语法 `SELECT a OR b` — 计算 `a` 和 `b` 的逻辑析取，使用函数 [or](/sql-reference/functions/logical-functions#or)。

## 逻辑取反运算符 {#logical-negation-operator}
...
条件运算符计算 `b` 和 `c` 的值，然后检查条件 `a` 是否满足，然后返回相应的值。如果 `b` 或 `C` 是 [arrayJoin()](/sql-reference/functions/array-join) 函数，则每一行将会复制，不考虑“a”条件。
...
所有二元运算符都有左结合性。例如，`1 + 2 + 3` 被转化为 `plus(plus(1, 2), 3)`。
有时这并不是您预期的那样。例如，`SELECT 4 > 2 > 3` 的结果将为 0。

为了提高效率，`and` 和 `or` 函数接受任意数量的参数。相应的 `AND` 和 `OR` 运算符链被转化为对这些函数的单个调用。
...
可以通过启用 [optimize_functions_to_subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) 设置进行优化。当 `optimize_functions_to_subcolumns = 1` 时，该函数仅读取 [null](../../sql-reference/data-types/nullable.md#finding-null) 子列，而不是读取和处理整个列数据。查询 `SELECT n IS NULL FROM table` 被转化为 `SELECT n.null FROM TABLE`。
...
可以通过启用 [optimize_functions_to_subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) 设置进行优化。当 `optimize_functions_to_subcolumns = 1` 时，该函数仅读取 [null](../../sql-reference/data-types/nullable.md#finding-null) 子列，而不是读取和处理整个列数据。查询 `SELECT n IS NOT NULL FROM table` 被转化为 `SELECT NOT n.null FROM TABLE`。
