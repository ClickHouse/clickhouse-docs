...
:::note
从 25.10 版本开始，可以像使用其他任意运算符一样使用 `<=>`。
在 25.10 版本之前，它只能在 JOIN 表达式中使用，例如：
...
它的行为类似常规相等运算符（`=`），但会将 `NULL` 值视为可比较。
两个 `NULL` 值被认为相等，将 `NULL` 与任意非 `NULL` 值比较时，结果返回 0（false）而不是 `NULL`。
...
`a GLOBAL NOT IN ...` – 等同于 `globalNotIn (a, b)` 函数。
...
`a != ANY (subquery)` – 等价于 `a NOT IN (SELECT singleValueOrNull(*) FROM subquery)`。
...
`a != ALL (subquery)` – 等同于 `notIn (a, subquery)` 函数。
...
从给定的日期中提取各个组成部分。例如，你可以从日期中获取月份，或者从时间中获取秒数。

`part` 参数指定要从日期中提取的部分。可用的取值如下：

* `DAY` — 月份中的天数。可能的取值范围: 1–31。
* `MONTH` — 月份编号。可能的取值范围: 1–12。
* `YEAR` — 年份。
...
更多示例参见 [tests](https://github.com/ClickHouse/ClickHouse/blob/master/tests/queries/0_stateless/00619_extract.sql)。
...
在设置 `INTERVAL` 的值时，你也可以使用字符串字面量。例如，`INTERVAL 1 HOUR` 等同于 `INTERVAL '1 hour'` 或 `INTERVAL '1' hour`。

:::tip\
不同类型的时间间隔不能组合使用。你不能使用类似 `INTERVAL 4 DAY 1 HOUR` 的表达式。请使用小于或等于该时间间隔中最小单位的单位来指定间隔，例如 `INTERVAL 25 HOUR`。你可以使用连续运算，如下面的示例所示。
:::
...
:::note\
始终优先使用 `INTERVAL` 语法或 `addDays` 函数。简单的加减运算（例如 `now() + ...` 这样的语法）不会考虑时间设置，例如夏令时。
:::
...
语法为 `SELECT a AND b` — 通过函数 [and](/sql-reference/functions/logical-functions#and) 计算 `a` 与 `b` 的逻辑与结果。

## 逻辑 OR 运算符 {#logical-or-operator}

语法：`SELECT a OR b` —— 通过函数 [or](/sql-reference/functions/logical-functions#or) 计算 `a` 与 `b` 的逻辑或运算。
...
条件运算符会先对 b 和 c 求值，然后检查条件 a 是否满足，最后返回相应的值。如果 `b` 或 `c` 是 [arrayJoin()](/sql-reference/functions/array-join) 函数，则每一行都会被复制，与条件 a 是否成立无关。
...
`transform` 函数无法处理 `NULL`。


## 连接运算符 {#concatenation-operator}

`s1 || s2` – 等价于 `concat(s1, s2)` 函数。

## Lambda 创建运算符 {#lambda-creation-operator}

`x -> expr` – 等同于 `lambda(x, expr)` 函数。
...
有时这不会像你预期的那样工作。例如，`SELECT 4 > 2 > 3` 的结果为 0。

为了提高效率，`and` 和 `or` 函数可以接受任意数量的参数。相应的 `AND` 和 `OR` 运算符链会被合并为对这些函数的一次调用。
...
* 对于 [Nullable](../../sql-reference/data-types/nullable.md) 类型的值,`IS NULL` 运算符返回:
  * `1`,如果值为 `NULL`。
  * `0`,否则。
* 对于其他类型的值,`IS NULL` 运算符始终返回 `0`。

可以通过启用 [optimize&#95;functions&#95;to&#95;subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) 设置来优化。当将 `optimize_functions_to_subcolumns` 设置为 `1` 时,该函数只读取 [null](../../sql-reference/data-types/nullable.md#finding-null) 子列,而不是读取并处理整个列数据。查询 `SELECT n IS NULL FROM table` 会被转换为 `SELECT n.null FROM TABLE`。
...
可以通过启用 [optimize&#95;functions&#95;to&#95;subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) 设置来优化。启用 `optimize_functions_to_subcolumns = 1` 后，函数只读取 [null](../../sql-reference/data-types/nullable.md#finding-null) 子列，而不是读取并处理整列数据。查询 `SELECT n IS NOT NULL FROM table` 会被改写为 `SELECT NOT n.null FROM TABLE`。
