...
作为 `UNION` 一部分的查询可以用 `()` 括起来。[ORDER BY](../../../sql-reference/statements/select/order-by.md) 和 [LIMIT](../../../sql-reference/statements/select/limit.md) 作用于各个独立查询，而不是最终结果。如果你需要对最终结果进行转换，可以在 [FROM](../../../sql-reference/statements/select/from.md) 子句中将所有带有 `UNION` 的查询放入一个子查询中。
