---
description: 'DESCRIBE TABLE 的文档'
...
`DESCRIBE` 语句针对表中的每一列返回一行，包含如下 [String](../../sql-reference/data-types/string.md) 类型的值：

* `name` — 列名。
* `type` — 列类型。
* `default_type` — 列中 [默认表达式](/sql-reference/statements/create/table) 所使用的子句：`DEFAULT`、`MATERIALIZED` 或 `ALIAS`。如果没有默认表达式，则返回空字符串。
...
* `is_subcolumn` — 对于内部子列，该标志为 `1`。仅当通过 [describe&#95;include&#95;subcolumns](../../operations/settings/settings.md#describe_include_subcolumns) 设置启用子列描述时，才会包含在结果中。

[Nested](../../sql-reference/data-types/nested-data-structures/index.md) 数据结构中的所有列都会被单独描述。每个列的名称前都会加上父列名和一个点作为前缀。
