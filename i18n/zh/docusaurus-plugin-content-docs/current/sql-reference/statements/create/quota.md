...
键 `user_name`、`ip_address`、`forwarded_ip_address`、`client_key`、`client_key, user_name`、`client_key, ip_address` 和 `normalized_query_hash` 对应于 [system.quotas](../../../operations/system-tables/quotas.md) 表中的字段。

仅当 `KEYED BY` 为 `ip_address` 或 `forwarded_ip_address` 时，才能使用 `IPV4_PREFIX_BITS` 和 `IPV6_PREFIX_BITS` 选项。它们对应于 [system.quotas](../../../operations/system-tables/quotas.md) 表中的字段。

参数 `queries`、`query_selects`、`query_inserts`、`errors`、`result_rows`、`result_bytes`、`read_rows`、`read_bytes`、`written_bytes`、`execution_time`、`failed_sequential_authentications`、`queries_per_normalized_hash` 对应于 [system.quotas&#95;usage](../../../operations/system-tables/quotas_usage.md) 表中的字段。

`ON CLUSTER` 子句允许在集群上创建 QUOTA，参见 [Distributed DDL](../../../sql-reference/distributed-ddl.md)。
