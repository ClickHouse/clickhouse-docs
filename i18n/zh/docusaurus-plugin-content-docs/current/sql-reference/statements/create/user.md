...
用于为某个认证方法指定过期日期，以及可选的过期时间。它接受一个字符串作为参数。建议使用 `YYYY-MM-DD [hh:mm:ss] [timezone]` 格式表示日期时间，其中 `[timezone]` 必须是数值偏移量 (如 `+09:00`) ，或 `UTC`、`GMT`、`Z`、`MSK`、`MSD` 之一；不识别像 `Asia/Tokyo` 这样的具名 IANA 时区 (见下方说明) 。默认情况下，该参数为 `'infinity'`。
...
:::note
日期时间字符串由 `parseDateTimeBestEffort` 解析，它只识别时区标记 `UTC`、`GMT`、`Z`、`MSK`、`MSD`，以及像 `+09:00` 或 `-05:00` 这样的数值偏移量。不支持像 `Asia/Tokyo` 或 `Europe/London` 这样的具名 IANA 时区，而且对于采用夏令时的区域，固定偏移量并不等同于 IANA 时区，因此你必须根据要编码的具体日期计算正确的偏移量。
:::
