...
| [`SOURCE`](./sources/)                      | 定义字典的数据源（例如 ClickHouse 表、HTTP、PostgreSQL）。 |
| [`LAYOUT`](./layouts/)                      | 控制字典在内存中的存储方式（例如 `FLAT`、`HASHED`、`CACHE`）。 |
...
- [Layouts](./layouts/) — 字典在内存中的存储方式
- [Sources](./sources/) — 连接到数据源
