...
根据字典的 [布局](/sql-reference/dictionaries#storing-dictionaries-in-memory)，可以将一个或多个属性指定为字典键。
...
:::note
在 [ClickHouse Cloud](https://clickhouse.com) 的 SQL 控制台中使用时，创建字典时必须指定用户（`default` 或任何其他具有 `default_role` 角色的用户）和密码。
:::
...
有关详细信息，请参见 [字典源](/sql-reference/dictionaries#dbms)。
