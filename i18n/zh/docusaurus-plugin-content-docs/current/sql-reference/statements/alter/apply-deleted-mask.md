...
该命令会应用由[轻量级删除](/sql-reference/statements/delete)创建的掩码，并强制将标记为已删除的行从磁盘中移除。此命令属于重量级变更操作，从语义上等同于执行查询 `ALTER TABLE [db].name DELETE WHERE _row_exists = 0`。
