...
`SHOW CREATE (TABLE|DATABASE|USER)` 在以下设置开启时可显示秘密信息：
...
:::note
如果使用此语句获取系统表的 `CREATE` 查询，您将获得一个 *假* 查询，该查询仅声明表结构，但不能用于创建表。
:::
...
在本示例中，我们使用 `SHOW` 获取数据库名称中包含符号序列 'de' 的数据库名称：
...
在本示例中，我们使用 `SHOW TABLES` 语句找到所有名称中包含 'user' 的表：
...
数据库和表名可以以简略形式指定为 `<db>.<table>`，这意味着 `FROM tab FROM db` 和 `FROM db.tab` 是等效的。如果未指定数据库，则查询返回当前数据库中的列列表。

还有两个可选关键字：`EXTENDED` 和 `FULL`。`EXTENDED` 关键字当前没有效果，存在是为了与 MySQL 兼容。`FULL` 关键字使输出包括排序规则、注释和权限列。

`SHOW COLUMNS` 语句产生一个具有以下结构的结果表：

| 列名       | 描述                                                                                                                         | 类型               |
|------------|----------------------------------------------------------------------------------------------------------------------------|--------------------|
| `field`    | 列的名称                                                                                                                    | `String`           |
| `type`     | 列的数据类型。如果查询是通过 MySQL 网络协议进行的，则显示 MySQL 中的等效类型名称。                                        | `String`           |
...
| `extra`    | 额外的信息，目前未使用                                                                                                      | `String`           |
| `collation`|（仅当指定了 `FULL` 关键字时）列的排序规则，总是 `NULL`，因为 ClickHouse 没有按列的排序规则                              | `Nullable(String)` |
| `comment`  | （仅当指定了 `FULL` 关键字时）列的注释                                                                                    | `String`           |
| `privilege`| （仅当指定了 `FULL` 关键字时）您在此列上的权限，目前不可用                                                               | `String`           |

### 示例 {#examples-2}

在本示例中，我们将使用 `SHOW COLUMNS` 语句获取表 'orders' 中所有以 'delivery_' 开头的列的信息：
...
此语句主要是为了与 MySQL 兼容。系统表 [`system.tables`](../../operations/system-tables/tables.md)（用于主键）和 [`system.data_skipping_indices`](../../operations/system-tables/data_skipping_indices.md)（用于数据跳过索引）提供了相同的信息，但方式更符合 ClickHouse 的本质。
...
数据库和表名可以以简略形式指定为 `<db>.<table>`，即 `FROM tab FROM db` 和 `FROM db.tab` 是等效的。如果未指定数据库，则查询假设当前数据库为数据库。

可选关键词 `EXTENDED` 当前没有效果，存在是为了与 MySQL 兼容。
...
| `key_name`     | 索引的名称，如果索引是主键索引则为 `PRIMARY`。                                                              | `String`           |
| `seq_in_index` | 对于主键索引，从 `1` 开始的列的位置。对于数据跳过索引：始终为 `1`。                                         | `UInt8`            |
| `column_name`  | 对于主键索引，此列的名称。对于数据跳过索引：`''`（空字符串），见字段 "expression"。                         | `String`           |
| `collation`    | 列在索引中的排序：如果升序为 `A`，如果降序为 `D`，如果未排序为 `NULL`。                                      | `Nullable(String)` |
| `cardinality`  | 索引基数的估算（索引中唯一值的数量）。目前始终为 0。                                                      | `UInt64`           |
...
输出 [`system.processes`](/operations/system-tables/processes) 表的内容，该表包含当前正在处理的查询列表，排除了 `SHOW PROCESSLIST` 查询。
...
`WITH IMPLICIT` 修饰符允许显示隐式权限（例如，`GRANT SELECT ON system.one`）

`FINAL` 修饰符合并用户及其授予角色（包括继承）所有权限。

## SHOW CREATE USER {#show-create-user}

`SHOW CREATE USER` 语句显示创建用户时使用的参数 [用户创建](../../sql-reference/statements/create/user.md)。
...
`SHOW CREATE ROLE` 语句显示创建角色时使用的参数 [角色创建](../../sql-reference/statements/create/role.md)。
...
`SHOW CREATE ROW POLICY` 语句显示创建行策略时使用的参数 [行政策创建](../../sql-reference/statements/create/row-policy.md)。
...
`SHOW CREATE QUOTA` 语句显示创建配额时使用的参数 [配额创建](../../sql-reference/statements/create/quota.md)。
...
`SHOW CREATE SETTINGS PROFILE` 语句显示创建设置配置文件时使用的参数 [设置配置文件创建](../../sql-reference/statements/create/settings-profile.md)。
...
`SHOW USERS` 语句返回 [用户帐户](../../guides/sre/user-management/index.md#user-account-management) 名称列表。 
...
`SHOW ACCESS` 语句显示所有 [用户](../../guides/sre/user-management/index.md#user-account-management)、[角色](../../guides/sre/user-management/index.md#role-management)、[配置文件](../../guides/sre/user-management/index.md#settings-profiles-management) 等及其所有 [权限](../../sql-reference/statements/grant.md#privileges)。
...
当使用 `CHANGED` 子句时，查询仅返回与其默认值更改过的设置。
...
如果指定了 `LIKE` 或 `ILIKE` 子句，则查询返回名称匹配提供的 `<pattern>` 的系统函数列表。
