...
> • 当多边形参数为常量，且点由已建立索引的键列表示时 (例如，在 `x, y` 属于 `PRIMARY KEY` 或受 `minmax` 索引覆盖的表上使用 `pointInPolygon((x, y), constant_polygon)`) ，ClickHouse 可以同时使用主键和 `minmax` 数据 skipping 索引来裁剪无关的粒度块。
