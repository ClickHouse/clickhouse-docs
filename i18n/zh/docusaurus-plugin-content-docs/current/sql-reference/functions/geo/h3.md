...
* `h3index` — H3 六边形索引编号。[UInt64](../../data-types/int-uint.md)。
...
返回在指定分辨率下覆盖给定几何对象 (Ring 或 (多) Polygon) 的六边形，并使用 H3 Experimental 包含模式。
...
* `flags` — 包含模式。范围：`[0, 3]`。可使用 `0`、`1`、`2` 和 `3` 等整数字面量，或 [UInt32](../../data-types/int-uint.md) 值 (例如 `toUInt32(2)`) 。这些值会以 `UInt32` 形式传递给 H3 API。其他原生整数类型会通过精确类型转换进行转换。
...
返回成对 GeoCoord 点 (纬度/经度) 之间的“大圆 (great circle) ”或“半正矢”距离，单位为米。
