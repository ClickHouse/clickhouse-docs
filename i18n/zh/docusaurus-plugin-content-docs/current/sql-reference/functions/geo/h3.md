...
[H3](https://eng.uber.com/h3/) 是一个地理索引系统，地球表面被划分为一个均匀的六边形网格。该系统是分层的，也就是说，顶层的每个六边形（“父级”）可以拆分为七个均匀但更小的六边形（“子级”），以此类推。

层次的级别称为 `resolution`，可以取值范围为 `0` 到 `15`，其中 `0` 是 `base` 级别，具有最大和最粗糙的单元格。

经度和纬度结合可以转换为一个 64 位的 H3 索引，用于标识一个网格单元。

H3 索引主要用于地点分组和其他地理空间操作。

H3 系统的完整描述可在 [Uber 工程官方网站](https://eng.uber.com/h3/) 查阅。
...
- 1 — 这个数字是有效的 H3 索引。 [UInt8](../../data-types/int-uint.md)。
- 0 — 这个数字不是有效的 H3 索引。 [UInt8](../../data-types/int-uint.md)。
...
- 如果该索引无效，则函数返回一个随机值。使用 [h3IsValid](#h3isvalid) 验证该索引。 [UInt8](../../data-types/int-uint.md)。
...
计算给定 [H3](#h3-index) 六边形边的平均长度，单位为度。
...
- 给定 [H3](#h3-index) 六边形边的平均长度，单位为度。 [Float64](../../data-types/float.md)。
...
计算给定 [H3](#h3-index) 六边形边的平均长度，单位为米。
...
- 给定 [H3](#h3-index) 六边形边的平均长度，单位为米。 [Float64](../../data-types/float.md)。
...
计算给定 [H3](#h3-index) 六边形边的平均长度，单位为千米。
...
- 给定 [H3](#h3-index) 六边形边的平均长度，单位为千米。 [Float64](../../data-types/float.md)。
...
返回给定分辨率的点索引 `(lat, lon)` 的 [H3](#h3-index)。
...
- 发生错误时返回 0。 [UInt64](../../data-types/int-uint.md)。

注意：在 ClickHouse v25.4 或更早版本中，`geoToH3()` 的参数顺序为 `(lon, lat)`。根据 ClickHouse v25.5，输入值的顺序为 `(lat, lon)`。可以通过设置 `geotoh3_lon_lat_input_order = true` 恢复先前的行为。
...
返回对应于提供的 [H3](#h3-index) 索引的质心纬度和经度。
...
注意：在 ClickHouse v24.12 或更早版本中，`h3ToGeo()` 返回的值顺序为 `(lon, lat)`。根据 ClickHouse v25.1，返回值的顺序为 `(lat, lon)`。可以通过设置 `h3togeo_lon_lat_result_order = true` 恢复先前的行为。
...
返回与提供的 H3 索引对应的边界坐标对 `(lat, lon)` 的数组。
...
- 坐标对的数组 '(lat, lon)'。 [Array](../../data-types/array.md)([Float64](../../data-types/float.md), [Float64](../../data-types/float.md))。
...
列出给定六边形周围半径为 `k` 的所有 [H3](#h3-index) 六边形，顺序随机。
...
- H3 索引的数组。 [Array](../../data-types/array.md)([UInt64](../../data-types/int-uint.md))。
...
- 六边形基本单元号。 [UInt8](../../data-types/int-uint.md)。
...
- `index1` — 六边形索引号。 [UInt64](../../data-types/int-uint.md)。
- `index2` — 六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
- 六边形索引号。发生错误时返回 0。 [UInt64](../../data-types/int-uint.md)。
...
返回 [H3](#h3-index) 索引是否有 Class III 定向的分辨率。
...
- `1` — 索引有 Class III 定向的分辨率。 [UInt8](../../data-types/int-uint.md)。
...
- 包含与给定 H3 索引相交的十二面体面的数组。 [Array](../../data-types/array.md)([UInt64](../../data-types/int-uint.md))。
...
返回与给定输入 H3 索引对应的特定单元的确切面积，单位为平方米。
...
返回与给定输入 H3 索引对应的特定单元的确切面积，单位为平方弧度。
...
返回给定分辨率下包含给定 [H3](#h3-index) 索引的中心子六边形索引。
...
- 给定分辨率下包含给定 [H3](#h3-index) 的中心子六边形索引 [H3](#h3-index)。 [UInt64](../../data-types/int-uint.md)。
...
返回成对的 GeoCoord 点（纬度/经度）之间的“大圆”或“哈弗辛”距离，以米为单位。
...
- `lat1`, `lon1` — 点1 的纬度和经度，单位为度。 [Float64](../../data-types/float.md)。
- `lat2`, `lon2` — 点2 的纬度和经度，单位为度。 [Float64](../../data-types/float.md)。
...
返回成对的 GeoCoord 点（纬度/经度）之间的“大圆”或“哈弗辛”距离，以千米为单位。
...
返回成对的 GeoCoord 点（纬度/经度）之间的“大圆”或“哈弗辛”距离，以弧度为单位。
...
- 所有分辨率 0 的 H3 索引的数组。 [Array](../../data-types/array.md)([UInt64](../../data-types/int-uint.md))。
...
- `start` — 表示起始点的六边形索引号。 [UInt64](../../data-types/int-uint.md)。
- `end` — 表示终止点的六边形索引号。 [UInt64](../../data-types/int-uint.md)。

**返回值**

返回的 h3 索引数组表示两个提供的索引之间的线。 [Array](../../data-types/array.md)([UInt64](../../data-types/int-uint.md))。
...
返回以提供的原点 h3Index 为中心、长度为 k 的六边形环的索引。
...
- `index` — 表示原点的六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
根据提供的原点和目标返回单向边 H3 索引，发生错误时返回 0。
...
- `originIndex` — 原点六边形索引号。 [UInt64](../../data-types/int-uint.md)。
- `destinationIndex` — 目标六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
- `edge` — 表示单向边的六边形索引号。 [UInt64](../../data-types/int-uint.md)。

**返回值**

- 原点六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
- 目标六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
一个包含两个值的元组 `tuple(origin, destination)`：

- `origin` — 原点六边形索引号。 [UInt64](../../data-types/int-uint.md)。
- `destination` — 目标六边形索引号。 [UInt64](../../data-types/int-uint.md)。
...
- 坐标对的数组 '(lon, lat)'。 [Array](../../data-types/array.md)([Float64](../../data-types/float.md), [Float64](../../data-types/float.md))。
