...
**算法**
使用朴素贝叶斯 (朴素贝叶斯) 分类算法，并结合[拉普拉斯平滑](https://en.wikipedia.org/wiki/Additive_smoothing)，基于 n‑gram 概率来处理未见过的 n‑gram，具体方法参考[这份资料](https://web.stanford.edu/~jurafsky/slp3/4.pdf)。
...
你可以在[这里](https://github.com/nihalzp/ClickHouse-NaiveBayesClassifier-Models)找到用于创建语言检测 朴素贝叶斯 模型的示例源代码。
...
1. **根据分词模式，在每个文档的开头和结尾添加边界标记：**
   ...
   * **Byte**：`0x01` (开头) ，`0xFF` (结尾)
   * **Codepoint**：`U+10FFFE` (开头) ，`U+10FFFF` (结尾)
   * **Token**：`<s>` (开头) ，`</s>` (结尾)
