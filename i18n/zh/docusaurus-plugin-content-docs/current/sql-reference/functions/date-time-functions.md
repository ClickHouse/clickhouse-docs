...
本节中的大多数函数接受一个可选的时区参数，例如 `Europe/Amsterdam`。在这种情况下，指定的时区将被用作时区，而不是本地（默认）时区。
...
从一个年、月和日参数创建一个 [Date](../data-types/date.md)
- 从年、月和日参数，或者
- 从年和一年中的第几天参数。
...
- `day_of_year` — 年中的第几天。 [Integer](../data-types/int-uint.md)、[Float](../data-types/float.md) 或 [Decimal](../data-types/decimal.md)。
...
从年、月、日（或可选的年和一天）创建一个 [Date32](../../sql-reference/data-types/date32.md) 类型的日期。
...
:::note
如果省略 `month`，则 `day` 应取值在 `1` 和 `365` 之间，否则应取值在 `1` 和 `31` 之间。
:::
...
从年、月、日、小时、分钟和秒参数创建一个 [DateTime](../data-types/datetime.md)。
...
从其组件（年、月、日、小时、分钟、秒）创建一个 [DateTime64](../../sql-reference/data-types/datetime64.md) 数据类型的值。可选的亚秒级精度。
...
- `precision` — 可选的亚秒级组件精度（0-9）。 [Integer](../../sql-reference/data-types/int-uint.md)。
...
将第一个参数 'expr' 转换为类型 [DateTime64(6)](../data-types/datetime64.md)。如果提供第二个参数 'expr_time'，则将指定的时间添加到转换后的值。
...
返回当前会话的时区，即设置 [session_timezone](../../operations/settings/settings.md#session_timezone) 的值。如果在分布式表的上下文中执行该函数，则会生成一个具有与每个分片相关的值的普通列；否则，它会生成一个常量值。
...
返回服务器的时区，即设置 [timezone](../../operations/server-configuration-parameters/settings.md#timezone) 的值。如果在分布式表的上下文中执行该函数，则会生成一个具有与每个分片相关的值的普通列；否则，它会生成一个常量值。
...
将日期或带时间的日期转换为指定的时区。不会改变数据的内部值（Unix秒数），仅值的时区属性和字符串表示形式发生变化。
...
- `timezone` — 返回值的时区。 [String](../data-types/string.md)。该参数是常量，因为 `toTimezone` 更改列的时区（时区是 `DateTime*` 类型的属性）。
...
- [formatDateTime](#formatdatetime) - 支持非常量时区。
- [toString](type-conversion-functions.md#tostring) - 支持非常量时区。
...
返回相对于 [UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time) 的时区偏移量（秒）。该函数会考虑在指定日期和时间的 [夏令时](https://en.wikipedia.org/wiki/Daylight_saving_time) 和历史时区变化。使用 [IANA 时区数据库](https://www.iana.org/time-zones) 来计算偏移量。
...
- 相对于 UTC 的偏移量（秒）。 [Int32](../data-types/int-uint.md)。
...
- 给定日期/时间在一年中的天数（1 - 366）。 [UInt16](../data-types/int-uint.md)。
...
`toDayOfWeek()` 的双参数形式允许您指定一周是从星期一还是星期日开始，以及返回值是否应在 0 到 6 或 1 到 7 之间。如果省略模式参数，则默认为 0。可以将日期的时区指定为第三个参数。
...
- `mode` - 决定一周的第一天。可能的值为 0、1、2 或 3。详见上表的差异。
- `timezone` - 可选参数，它的行为与其他转换函数相同。

第一个参数还可以作为 [String](../data-types/string.md) 传递，格式由 [parseDateTime64BestEffort()](type-conversion-functions.md#parsedatetime64besteffort) 支持。字符串参数的支持仅出于与 MySQL 的兼容性，这是某些第三方工具所期望的。由于字符串参数的支持可能在未来依赖新的 MySQL 兼容性设置，并且因为字符串解析通常很慢，因此建议不使用它。

**返回值**

- 根据所选模式，给定日期/时间的一周中的天数（1-7）

**示例**

以下日期为 2023 年 4 月 21 日，星期五：
...
假设如果时钟提前，则由一小时并在凌晨2点发生；如果时钟后退，由一小时并在凌晨3点发生（这并不总是确切发生的时间 - 这取决于时区）。
...
* `enable_extended_results_for_datetime_functions = 0`时：
  ...
  * 函数 `toStartOfDay`、`toStartOfHour`、`toStartOfFifteenMinutes`、`toStartOfTenMinutes`、`toStartOfFiveMinutes`、`toStartOfMinute`、`timeSlot` 返回 `DateTime`。尽管这些函数可以接收扩展类型 `Date32` 和 `DateTime64` 的值作为参数，但传递超出常规范围（1970 年到 2149 年的 `Date` / 2106 年的 `DateTime`）的时间将产生错误结果。
...
将日期或带时间的日期向下取整到年的第一天。将日期作为 `Date` 对象返回。
...
将日期或带时间的日期向下取整到 ISO 年的第一天，这可能与“普通”年份不同。（见 [https://en.wikipedia.org/wiki/ISO_week_date](https://en.wikipedia.org/wiki/ISO_week_date)。）
...
- 给定日期/时间的季度第一天。 [Date](../data-types/date.md)。
...
- 给定日期/时间的月份第一天。 [Date](../data-types/date.md)。
...
:::note
解析不正确日期的行为是特定于实现的。 ClickHouse 可能返回零日期，抛出异常，或进行“自然”溢出。
:::
...
- 在给定日期之前或等于给定日期的最近的星期一的日期。 [Date](../data-types/date.md)。
...
将日期或带时间的日期向下取整到最近的星期日或星期一。返回日期。模式参数的工作方式与函数 `toWeek()` 中的模式参数完全相同。如果未指定模式，则默认为 0。
...
- 在给定日期之前或等于给定日期的最近的星期日或星期一的日期，具体取决于模式。 [Date](../data-types/date.md)。
...
模式参数的工作方式与函数 `toWeek()` 中的模式参数完全相同。如果未指定模式，则假设模式为 0。
...
- `t` - 一个 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)
- `mode` - 决定一周最后一天的模式，如 [toWeek](#toweek) 函数所述
- `timezone` - 可选参数，它的行为与其他转换函数相同

**返回值**

- 在给定日期之后或等于给定日期的最近的星期六或星期日的日期，具体取决于模式。 [Date](../data-types/date.md)。
...
- 给定日期/时间的一小时开始。 [DateTime](../data-types/datetime.md)。
...
- 给定日期/时间的一分钟开始。 [DateTime](../data-types/datetime.md)。
...
- 给定日期/时间的五分钟区间的开始。 [DateTime](../data-types/datetime.md)。
...
- 给定日期/时间的十分钟区间的开始。 [DateTime](../data-types/datetime.md)。
...
- 给定日期/时间的十五分钟区间的开始。 [DateTime](../data-types/datetime.md)。
...
(*) 小时间隔是特殊的：计算总是相对于当前日期的 00:00:00（零点）进行的。因此，只有 1 到 23 的小时值是有用的。

如果指定了单位 `WEEK`，则 `toStartOfInterval` 假设周一为一周的开始。注意，这种行为与函数 `toStartOfWeek` 的默认周日开始规则不同。
...
第二个重载模拟 TimescaleDB 的 `time_bucket()` 函数，对应于 PostgreSQL 的 `date_bin()` 函数，例如：
...
别名: `toTime` - 仅在启用 `use_legacy_to_time` 设置时可用。
...
- 日期时间，日期等于 `1970-01-02` 并保留时间。 [DateTime](../data-types/datetime.md)。

:::note
如果 `date` 输入参数包含次秒组件，它们将在返回的 `DateTime` 值中被丢弃，保留到秒的精度。
:::
...
将日期或带时间的日期转换为自某个过去的固定时间点以来的年份数。
...
- 从过去的固定参考点起的年数。 [UInt16](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的季度数。
...
- 从过去的固定参考点起的季度数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的月份数。
...
- 从过去的固定参考点起的月份数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的周数。
...
- 从过去的固定参考点起的周数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的天数。
...
- 从过去的固定参考点起的天数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的小时数。
...
- 从过去的固定参考点起的小时数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的分钟数。
...
- 从过去的固定参考点起的分钟数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为自某个过去的固定时间点以来经过的秒数。
...
- 从过去的固定参考点起的秒数。 [UInt32](../data-types/int-uint.md)。
...
将日期或带时间的日期转换为 ISO 年，以 UInt16 数字表示。
...
- `value` — 包含日期或带时间的值。 [Date](../data-types/date.md)， [Date32](../data-types/date32.md)， [DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)
...
- `value` 转换为当前的 ISO 周数。 [UInt8](../data-types/int-uint.md)。
...
此函数返回日期或日期时间的周数。 `toWeek()` 的两个参数形式使您能够指定一周是从周日还是周一开始，以及返回值应在 0 到 53 或 1 到 53 之间。如果省略模式参数，则默认模式为 0。
...
|------|------------|-------|-------------------|
| 0    | 周日       | 0-53  | 包含该年的一个周日 |
| 1    | 周一       | 0-53  | 包含该年 4 天或更多天 |
| 2    | 周日       | 1-53  | 包含该年的一个周日 |
| 3    | 周一       | 1-53  | 包含该年 4 天或更多天 |
| 4    | 周日       | 0-53  | 包含该年 4 天或更多天 |
| 5    | 周一       | 0-53  | 包含该年的一个周一 |
| 6    | 周日       | 1-53  | 包含该年 4 天或更多天 |
| 7    | 周一       | 1-53  | 包含该年的一个周一 |
| 8    | 周日       | 1-53  | 包含 1 月 1 日     |
| 9    | 周一       | 1-53  | 包含 1 月 1 日     |

对于具有“包含该年 4 天或更多天”含义的模式值，周数根据 ISO 8601:1988 编号：

- 如果包含 1 月 1 日的那周在新年中有 4 天或更多天，则它是第1周。
- 否则，它是前一年的最后一周，下一周是第1周。

对于具有“包含 1 月 1 日”含义的模式值，包含 1 月 1 日的那周是第1周。
无论这一周包含了新年中的多少天，即使只包含一天。
即，如果去年 12 月的最后一周包含了今年 1 月 1 日，那它将是明年的第1周。
...
- `t` – 日期或日期时间。
- `mode` – 可选参数，值范围为 \[0,9\]，默认值为 0。
- `Timezone` – 可选参数，行为与其他转换函数相同。

第一个参数也可以指定为 [String](../data-types/string.md)，格式由 [parseDateTime64BestEffort()](type-conversion-functions.md#parsedatetime64besteffort) 支持。仅出于与 MySQL 的兼容性原因，对字符串参数的支持是可以用的，因为某些第三方工具期望这样。由于未来可能会对字符串参数的支持取决于新的 MySQL 兼容性设置，并且字符串解析通常较慢，因此建议不要使用它。
...
返回日期的年份和周数。结果中的年份可能与日期参数中的年份不同，尤其是在年初和年末的周。

模式参数的工作方式类似于 `toWeek()` 的模式参数。对于单参数语法，使用模式值 0。

`toISOYear()` 是一个兼容性函数，相当于 `intDiv(toYearWeek(date,3),100)`。

:::warning
`toYearWeek()` 返回的周数可以与你从 `toWeek()` 获取的周数不同。 `toWeek()` 始终在给定年份的上下文中返回周数，如果 `toWeek()` 返回 `0`，则 `toYearWeek()` 返回前一年最后一周相应的值。请参见下面示例中的 `prev_yearWeek`。
:::
...
返回自 [0000年1月1日](https://en.wikipedia.org/wiki/Year_zero) 在 [ISO 8601 定义的先验公历](https://en.wikipedia.org/wiki/Gregorian_calendar#Proleptic_Gregorian_calendar) 中已过去的天数。该计算与 MySQL 的 [`TO_DAYS()`](https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html#function_to-days) 函数相同。
...
- `date` — 要计算自公元 0 年以来过去的天数的日期。 [Date](../data-types/date.md)， [Date32](../data-types/date32.md)， [DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
- `time_zone` — 字符串类型常量值或表示时区的表达式。 [String types](../data-types/string.md)

**返回值**

自 0000-01-01 以来过去的天数。 [UInt32](../data-types/int-uint.md)。
...
返回自 [0000年1月1日](https://en.wikipedia.org/wiki/Year_zero) 以来经过的天数对应的日期，在 [ISO 8601 定义的先验公历](https://en.wikipedia.org/wiki/Gregorian_calendar#Proleptic_Gregorian_calendar) 中。该计算与 MySQL 的 [`FROM_DAYS()`](https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html#function_from-days) 函数相同。

如果它无法在 [Date](../data-types/date.md) 类型的范围内表示，则结果是未定义的。
...
对应于自公元 0 年以来经过的天数的日期。 [Date](../data-types/date.md)。
...
返回 `startdate` 和 `enddate` 之间差异的 `unit` 组件。差异的计算使用 1 纳秒的精度。
例如，`2021-12-29` 和 `2022-01-01` 之间的差异，对于 `day` 单位为 3 天，对于 `month` 单位为 0 个月，对于 `year` 单位为 0 年。
...
- `timezone` — [时区名称](../../operations/server-configuration-parameters/settings.md#timezone)（可选）。如果指定，则应用于 `startdate` 和 `enddate`。如果未指定，则使用 `startdate` 和 `enddate` 的时区。如果不相同，则结果不确定。 [String](../data-types/string.md)。

**返回值**

表示 `enddate` 和 `startdate` 的差异，以 `unit` 表达。 [Int](../data-types/int-uint.md)。
...
差异使用相对单位计算，例如 `2021-12-29` 和 `2022-01-01` 之间的差异，对于 `day` 单位是 3 天（见 [toRelativeDayNum](#torelativedaynum)），对于 `month` 单位是 1 个月（见 [toRelativeMonthNum](#torelativemonthnum)）和对于 `year` 单位是 1 年（见 [toRelativeYearNum](#torelativeyearnum)）。

如果指定了单位 `week`，则 `date_diff` 假设周一为一周的开始。请注意，这种行为与函数 `toWeek()` 的默认周日开始规则不同。
...
- `unit` — 用于截断结果的间隔类型。 [String Literal](/sql-reference/syntax#string).
    可能的值：
...
- `value` — 日期与时间。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
- 否则，返回 [Date](../data-types/date.md)。

如果 unit 参数为 Day、Hour、Minute 或 Second，
- 且 value 参数为 Date32 或 DateTime64，则返回 [DateTime64](../data-types/datetime64.md)。
- 否则，返回 [DateTime](../data-types/datetime.md)。

如果 unit 参数为 Millisecond、Microsecond 或 Nanosecond，则返回 [DateTime64](../data-types/datetime64.md)，其刻度为 3 或 6 或 9（取决于 unit 参数）。
...
将时间间隔或日期间隔添加到提供的日期或包含时间的日期中。

如果加法结果超出数据类型的边界，则结果是未定义的。
...
- `unit` — 要添加的间隔类型。注意：这不是一个 [String](../data-types/string.md)，因此不应加引号。
    可能的值：
...
- `date` — 要将 `value` 添加到的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。

**返回值**

通过将 `value`（以 `unit` 表达）添加到 `date` 来获得的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
- `unit` — 要减去的间隔类型。注意：这不是一个 [String](../data-types/string.md)，因此不应加引号。
...
- `date` — 从中减去 `value` 的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。

**返回值**

通过从 `date` 中减去 `value`（以 `unit` 表达）来获得的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
日期或带时间的日期，其中指定的 `value`（以 `unit` 表达）添加到 `date`。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
通过从 `date` 中减去 `value`（以 `unit` 表达）获得的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
将时间间隔添加到提供的日期、带时间的日期或字符串编码的日期/带时间的日期中。
...
- `date` — 要添加 `interval` 的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md)、[DateTime64](../data-types/datetime64.md) 或 [String](../data-types/string.md)
- `interval` — 要添加的间隔。 [Interval](../data-types/special-data-types/interval.md)。

**返回值**

通过将 `interval` 添加到 `date` 来获得的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
从提供的日期、带时间的日期或字符串编码的日期/带时间的日期中减去时间间隔。
...
- `date` — 从中减去 `interval` 的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md)、[DateTime64](../data-types/datetime64.md) 或 [String](../data-types/string.md)
- `interval` — 要减去的间隔。 [Interval](../data-types/special-data-types/interval.md)。

**返回值**

通过从 `date` 中减去 `interval` 获得的日期或带时间的日期。 [Date](../data-types/date.md)、[Date32](../data-types/date32.md)、[DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
返回查询分析时当前日期和时间的次秒精度。该函数是一个常量表达式。
...
- `scale` - 刻度大小（精度）：10<sup>-precision</sup> 秒。有效范围：[ 0 : 9 ]。通常使用 - 3（默认）（毫秒），6（微秒），9（纳秒）。
- `timezone` — 返回值的 [时区名称](../../operations/server-configuration-parameters/settings.md#timezone)（可选）。 [String](../data-types/string.md)。

**返回值**

- 当前日期和时间的次秒精度。 [DateTime64](../data-types/datetime64.md)。
...
返回处理每个数据块时的当前日期和时间。与函数 [now](#now) 相比，它不是一个常量表达式，并且对于长时间运行的查询，在不同的数据块中返回的值将不同。

在长时间运行的 INSERT SELECT 查询中生成当前时间时，使用此函数是有意义的。
...
- 在处理每个数据块时的当前日期和时间。 [DateTime](../data-types/datetime.md)。
...
返回查询分析时的当前日期。这与 'toDate(now())' 相同，并且有别名：`curdate`，`current_date`。
...
:::note
尽管此函数可以接受扩展类型 `Date32` 和 `DateTime64` 的值作为参数，但传递的时间如果不在正常范围内（ `Date` 的年份为 1970 年到 2149 年 / `DateTime` 为 2106 年）将会产生错误结果。
:::
...
将日期或带时间的日期转换为包含年份和月份数字的 UInt32 数字（YYYY * 100 + MM）。接受第二个可选的时区参数。如果提供，时区必须是字符串常量。

此函数是 `YYYYMMDDToDate()` 函数的反义词。
...
将日期或带时间的日期转换为包含年份和月份数字的 UInt32 数字（YYYY * 10000 + MM * 100 + DD）。接受第二个可选的时区参数。如果提供，时区必须是字符串常量。
...
将日期或带时间的日期转换为包含年份和月份数字的 UInt64 数字（YYYY * 10000000000 + MM * 100000000 + DD * 1000000 + hh * 10000 + mm * 100 + ss）。接受第二个可选的时区参数。如果提供，时区必须是字符串常量。
...
此函数是 `toYYYYMMDD()` 函数的反义词。

如果输入不编码有效的 Date 值，则输出是未定义的。
...
- `yyyymmdd` - 代表年份、月份和日期的数字。 [Integer](../data-types/int-uint.md)、[Float](../data-types/float.md) 或 [Decimal](../data-types/decimal.md)。
...
与 `YYYYMMDDToDate()` 函数类似，但生成 [Date32](../data-types/date32.md)。
## YYYYMMDDhhmmssToDateTime {#yyyymmddhhmmsstodatetime}

将包含年份、月份、日期、小时、分钟和秒数字的数字转换为 [DateTime](../data-types/datetime.md)。

如果输入不编码有效的 DateTime 值，则输出是未定义的。

此函数是 `toYYYYMMDDhhmmss()` 函数的反义词。
...
- `yyyymmddhhmmss` - 代表年份、月份和日期的数字。 [Integer](../data-types/int-uint.md)、[Float](../data-types/float.md) 或 [Decimal](../data-types/decimal.md)。
- `timezone` - 返回值的 [Timezone](../../operations/server-configuration-parameters/settings.md#timezone)（可选）。
...
与 `YYYYMMDDhhmmssToDate()` 函数类似，但生成 [DateTime64](../data-types/datetime64.md)。

接受一个可选的 `precision` 参数，位于 `timezone` 参数之后。
...
- `date`: 要添加指定年份数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定季度数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定月份数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定周数的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定天数的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定小时数的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定分钟数的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md), [String](../data-types/string.md)。
...
- `date`: 要添加指定秒数的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
- `interval_2`: 第二个要添加的间隔。 [interval](../data-types/special-data-types/interval.md)。

**返回值**

- 返回一个间隔的元组。 [tuple](../data-types/tuple.md)([interval](../data-types/special-data-types/interval.md))。

:::note
相同类型的间隔将合并为一个单一的间隔。例如，如果传递 `toIntervalDay(1)` 和 `toIntervalDay(2)`，则结果将为 `(3)` 而不是 `(1,1)`。
:::
...
- `intervals`: 要添加到 `date` 的间隔的元组。 [tuple](../data-types/tuple.md)([interval](../data-types/special-data-types/interval.md))。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定年份数量。
...
- `date`: 从中减去指定年份数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定季度数量。
...
- `date`: 从中减去指定季度数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定月份数量。
...
- `date`: 从中减去指定月份数量的日期/带时间的日期。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定周数。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定天数。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定小时数。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定分钟数。
...
从一个日期、带时间的日期或字符串编码的日期/带时间的日期中减去指定秒数。
...
- `date_time`: 带时间的日期，从中减去指定毫秒数。 [DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
- `date_time`: 带时间的日期，从中减去指定微秒数。 [DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
- `date_time`: 带时间的日期，从中减去指定纳秒数。 [DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)、[String](../data-types/string.md)。
...
- `interval_1`: 第一个间隔或间隔的元组。 [interval](../data-types/special-data-types/interval.md)、[tuple](../data-types/tuple.md)([interval](../data-types/special-data-types/interval.md))。
- `interval_2`: 第二个要被否定的间隔。 [interval](../data-types/special-data-types/interval.md)。

**返回值**

- 返回间隔的元组。 [tuple](../data-types/tuple.md)([interval](../data-types/special-data-types/interval.md))。

:::note
相同类型的间隔将合并为一个单一的间隔。例如，如果传递 `toIntervalDay(2)` 和 `toIntervalDay(1)`，则结果将为 `(1)` 而不是 `(2,1)`。
:::
...
- `date`: 第一个间隔或间隔的元组。 [Date](../data-types/date.md)/[Date32](../data-types/date32.md)/[DateTime](../data-types/datetime.md)/[DateTime64](../data-types/datetime64.md)。
- `intervals`: 从 `date` 中减去的间隔的元组。 [tuple](../data-types/tuple.md)([interval](../data-types/special-data-types/interval.md))。
...
对于以 'StartTime' 开始并持续 'Duration' 秒的时间区间，它返回一个时间数组，包括从该区间向下舍入到 'Size' 秒的点。'Size' 是一个可选参数，默认设置为 1800（30 分钟）。
例如，当在相应的会话中搜索页面浏览量时，这是必要的。
接受 DateTime 和 DateTime64 作为 'StartTime' 参数。对于 DateTime， 'Duration' 和 'Size' 参数必须为 `UInt32`。对于 'DateTime64' ，它们必须为 `Decimal64`。
返回 DateTime/DateTime64 数组（返回类型与 'StartTime' 的类型匹配）。对于 DateTime64，返回值的刻度可能与 'StartTime' 的刻度不同——所有给定参数中取最高的刻度。
...
根据给定的格式字符串格式化时间。格式是一个常量表达式，因此您无法为单个结果列使用多个格式。

formatDateTime使用MySQL日期时间格式样式，参考 https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html#function_date-format。
...
使用替换字段，您可以为结果字符串定义一个模式。“示例”列显示 `2018-01-02 22:33:44` 的格式化结果。

| 占位符  | 描述                                                                                                                                                                                       | 示例       |
|---------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| %a      | 缩写的星期几名称 (周一-周日)                                                                                                                                                              | Mon        |
| %b      | 缩写的月份名称 (一月-十二月)                                                                                                                                                              | Jan        |
| %c      | 作为整数的月份 (01-12)，请参阅下面的“注释4”                                                                                                                                                  | 01         |
| %C      | 年份除以100并截断为整数 (00-99)                                                                                                                                                              | 20         |
| %d      | 零填充的月份日期 (01-31)                                                                                                                                                                    | 02         |
| %D      | 短的MM/DD/YY日期，相当于 %m/%d/%y                                                                                                                                                | 01/02/18   |
| %e      | 空格填充的月份日期 ( 1-31)，请参阅下面的“注释5”                                                                                                                                               | &nbsp; 2   |
| %f      | 分数秒，请参阅下面的“注释1”和“注释2”                                                                                                                                                | 123456     |
| %F      | 短的YYYY-MM-DD日期，相当于 %Y-%m-%d                                                                                                                                                       | 2018-01-02 |
| %g      | 两位数字的年份格式，符合ISO 8601，从四位数字符号中缩写而来                                                                                                                        | 18         |
| %G      | 适用于ISO周数的四位数字年份格式，从基于周的年份 [由ISO 8601定义](https://en.wikipedia.org/wiki/ISO_8601#Week_dates)标准计算，通常仅与 %V 一起使用                                   | 2018       |
| %h      | 12小时制的小时 (01-12)                                                                                                                                                                    | 09         |
| %H      | 24小时制的小时 (00-23)                                                                                                                                                                    | 22         |
| %i      | 分钟 (00-59)                                                                                                                                                                            | 33         |
| %I      | 12小时制的小时 (01-12)                                                                                                                                                                    | 10         |
| %j      | 年中的天数 (001-366)                                                                                                                                                                      | 002        |
| %k      | 24小时制的小时 (00-23)，请参阅下面的“注释4”                                                                                                                                        | 14         |
| %l      | 12小时制的小时 (01-12)，请参阅下面的“注释4”                                                                                                                                        | 09         |
| %m      | 作为整数的月份 (01-12)                                                                                                                                                                      | 01         |
| %M      | 完整的月份名称 (一月-十二月)，请参阅下面的“注释3”                                                                                                                                                     | January     |
| %n      | 换行符 ('')                                                                                                                                                                            |            |
| %p      | AM或PM标识                                                                                                                                                                          | PM         |
| %Q      | 季度 (1-4)                                                                                                                                                                              | 1          |
| %r      | 12小时制HH:MM AM/PM时间，相当于 %h:%i %p                                                                                                                                               | 10:30 PM   |
| %R      | 24小时制HH:MM时间，相当于 %H:%i                                                                                                                                                           | 22:33      |
| %s      | 秒 (00-59)                                                                                                                                                                            | 44         |
| %S      | 秒 (00-59)                                                                                                                                                                            | 44         |
| %t      | 横向制表符 (')                                                                                                                                                                            |            |
| %T      | ISO 8601时间格式 (HH:MM:SS)，相当于 %H:%i:%S                                                                                                                                                    | 22:33:44   |
| %u      | 以数字表示的ISO 8601星期几，星期一为1（1-7）                                                                                                                                             | 2          |
| %V      | ISO 8601周数 (01-53)                                                                                                                                                                       | 01         |
| %w      | 作为整数表示的星期几，星期日为0 (0-6)                                                                                                                                                     | 2          |
| %W      | 完整的星期几名称 (星期一-星期日)                                                                                                                                                          | Monday     |
| %y      | 年份的最后两位数字 (00-99)                                                                                                                                                              | 18         |
| %Y      | 年                                                                                       | 2018       |
| %z      | 与UTC的时差，格式为+HHMM或-HHMM                                                                                                                                                            | -0500      |
| %%      | 百分号符号                                                                                                                                                                            | %          |

注1: 在版本早于v23.4的ClickHouse中，如果格式化值是没有分数秒的Date、Date32或DateTime，或者是精度为0的DateTime64，则 %f 会打印单个零 (0)。可以使用设置 `formatdatetime_f_prints_single_zero = 1` 恢复之前的行为。

注2: 在版本早于v25.1的ClickHouse中，%f按DateTime64的刻度打印的数字数量，而不是固定的6位数字。可以使用设置 `formatdatetime_f_prints_scale_number_of_digits= 1` 恢复之前的行为。

注3: 在版本早于v23.4的ClickHouse中，%M会打印分钟 (00-59)，而不是完整的月份名称 (一月-十二月)。可以使用设置 `formatdatetime_parsedatetime_m_is_month_name = 0` 恢复之前的行为。

注4: 在版本早于v23.11的ClickHouse中，函数 `parseDateTime` 需要格式化器 %c（月份）和 %l/%k（小时）前导零，例如 `07`。在后来的版本中，前导零可以省略，例如 `7`。可以使用设置 `parsedatetime_parse_without_leading_zeros = 0` 恢复之前的行为。需要注意的是，函数 `formatDateTime` 默认仍会为 `%c` 和 `%l/%k` 打印前导零，以避免破坏现有用例。可以通过设置 `formatdatetime_format_without_leading_zeros = 1` 更改该行为。

注5: 在版本早于v25.5的ClickHouse中，函数 `parseDateTime` 对于格式化器 `%e` 要求单数字的日期用空格填充，例如 ` 3`。在后来的版本中，空格填充是可选的，例如 `3` 和 ` 3` 都可以工作。为保持以前的行为，请设置 `parsedatetime_e_requires_space_padding = 1`。同样，函数 `formatDateTime` 中的格式化器 `%e` 之前无条件空格填充单数字，但现在打印时不会带前导空格。为保持以前的行为，请设置 `formatdatetime_e_with_space_padding = 1`。
...
此外，`formatDateTime` 函数可以接受一个第三个字符串参数，其中包含时区名称。例如：`Asia/Istanbul`。在这种情况下，时间会根据指定的时区进行格式化。
...
与formatDateTime类似，但它按照Joda样式格式化日期时间，而不是MySQL样式。参考 https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html。
...
- `date_part` — 日期部分。可能值: 'year', 'quarter', 'month', 'week', 'dayofyear', 'day', 'weekday', 'hour', 'minute', 'second'。 [字符串](../data-types/string.md)。
...
- `date` — 带时间的日期或仅日期。 [日期](../data-types/date.md)、 [DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)。
...
此函数将Unix时间戳转换为日历日期和一天的时间。

可以通过两种方式调用：

当提供一个类型为 [整数](../data-types/int-uint.md) 的单一参数时，它返回类型为 [DateTime](../data-types/datetime.md) 的值，即表现得像 [toDateTime](../../sql-reference/functions/type-conversion-functions.md#todatetime)。
...
当给定两个或三个参数，其中第一个参数为类型为 [整数](../data-types/int-uint.md)、 [日期](../data-types/date.md)、 [Date32](../data-types/date32.md)、 [DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md) 的值，第二个参数为常量格式字符串，第三个参数为可选的常量时区字符串，该函数返回类型为 [字符串](/sql-reference/data-types/string) 的值，即它表现得像 [formatDateTime](#formatdatetime)。在这种情况下，使用 [MySQL的日期时间格式样式](https://dev.mysql.com/doc/refman/8.0/en/date-and-time-functions.html#function_date-format)。
...
与 [fromUnixTimestamp](#fromunixtimestamp) 相同，但是在第二种调用方式（两个或三个参数）时，格式化使用 [Joda样式](https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html)，而不是MySQL样式。
...
将文本形式为 `YYYY-MM-DD` 的 [Proleptic Gregorian calendar](https://en.wikipedia.org/wiki/Proleptic_Gregorian_calendar) 日期转换为 [Modified Julian Day](https://en.wikipedia.org/wiki/Julian_day#Variants) 的整数。该函数支持的日期范围为 `0000-01-01` 至 `9999-12-31`。如果参数无法解析为日期或日期无效，则会引发异常。
...
- `date` — 文本形式的日期。 [字符串](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- Modified Julian Day 数字。 [Int32](../data-types/int-uint.md)。
...
类似于 [toModifiedJulianDay()](#tomodifiedjulianday)，但不引发异常而是返回 `NULL`。
...
- Modified Julian Day 数字。 [Nullable(Int32)](../data-types/int-uint.md)。
...
将 [Modified Julian Day](https://en.wikipedia.org/wiki/Julian_day#Variants) 数字转换为文本形式 `YYYY-MM-DD` 的 [Proleptic Gregorian calendar](https://en.wikipedia.org/wiki/Proleptic_Gregorian_calendar) 日期。该函数支持的日期数字范围为 `-678941` 至 `2973483`（分别代表 0000-01-01 和 9999-12-31）。如果日期数字超出支持范围，则会引发异常。
...
- `day` — Modified Julian Day 数字。 [任何整数类型](../data-types/int-uint.md)。
...
类似于 [fromModifiedJulianDayOrNull()](#frommodifiedjuliandayornull)，但不引发异常而是返回 `NULL`。
...
- 文本形式的日期。 [Nullable(字符串)](../data-types/string.md)
...
将来自其他时区的DateTime/DateTime64类型值转换为UTC时区的时间戳。此函数主要是为了与Apache Spark和类似框架的兼容性而包含。
...
- `time_val` — 一个 DateTime/DateTime64类型的常量值或一个表达式。 [DateTime/DateTime64类型](../data-types/datetime.md)
- `time_zone` — 一个字符串类型的常量值或表达式，表示时区。 [字符串类型](../data-types/string.md)

**返回值**

- 文本形式的DateTime/DateTime64
...
将来自UTC时区的DateTime/DateTime64类型值转换为其他时区的时间戳。此函数主要是为了与Apache Spark和类似框架的兼容性而包含。
...
:::note
此函数的结果与 `now('UTC')` 相同。它只是为了支持MySQL而添加，推荐使用 [`now`](#now)。
:::
...
返回两个日期或带时间值的日期之间的差异。差异以秒为单位计算。这与 `dateDiff` 相同，并只是为了支持MySQL而添加。推荐使用 `dateDiff`。
...
- `first_datetime` — 一个 DateTime/DateTime64类型的常量值或一个表达式。 [DateTime/DateTime64类型](../data-types/datetime.md)
- `second_datetime` — 一个 DateTime/DateTime64类型的常量值或一个表达式。 [DateTime/DateTime64类型](../data-types/datetime.md)
