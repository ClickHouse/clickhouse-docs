...
`to<type>` 函数和 [cast](#cast) 在某些情况下表现不同，例如在 [LowCardinality](../data-types/lowcardinality.md) 的情况下：[cast](#cast) 会移除 [LowCardinality](../data-types/lowcardinality.md) 特性，而 `to<type>` 函数则不会。在 [Nullable](../data-types/nullable.md) 的情况下也是如此，这种行为与 SQL 标准不兼容，可以通过 [cast_keep_nullable](../../operations/settings/settings.md/#cast_keep_nullable) 设置进行更改。

:::note
如果将数据类型的值转换为更小的数据类型（例如从 `Int64` 转换为 `Int32`）或在不兼容的数据类型之间转换（例如从 `String` 转换为 `Int`），可能会导致潜在的数据丢失。请务必仔细检查结果是否如预期。
:::
...
将输入值转换为 [`Bool`](../data-types/boolean.md) 类型的值。如果发生错误，则会抛出异常。
...
将输入值转换为 [`Int8`](../data-types/int-uint.md) 类型的值。如果发生错误，则会抛出异常。
...
:::note
如果输入值无法在 [Int8](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。例如：`SELECT toInt8(128) == -128;`。
:::
...
:::note
该函数使用 [舍入到零](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero)，意味着它会截断数字的小数位。
:::
...
:::note
如果输入值无法在 [Int8](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。
:::
...
与 [`toInt8`](#toint8) 类似，该函数将输入值转换为 [Int8](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。如果未传递 `default` 值，则在发生错误时返回 `0`。
...
- `expr` — 返回数字或数字的字符串表示的表达式。[表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
...
- 如果成功，则返回 8 位整数值，否则返回传递的默认值（如果有），或者返回 `0`（如果没有）。[Int8](../data-types/int-uint.md)。
...
将输入值转换为 [`Int16`](../data-types/int-uint.md) 类型的值。如果发生错误，则会抛出异常。
...
- `expr` — 返回数字或数字的字符串表示的表达式。[表达式](/sql-reference/syntax#expressions)。
...
:::note
如果输入值无法在 [Int16](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。例如：`SELECT toInt16(32768) == -32768;`。
:::
...
:::note
如果输入值无法在 [Int16](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢，这不视为错误。
:::
...
与 [`toInt16`](#toint16) 类似，该函数将输入值转换为 [Int16](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。如果未传递 `default` 值，则在发生错误时返回 `0`。
...
- 如果成功，则返回 16 位整数值，否则返回传递的默认值（如果有），或者返回 `0`（如果没有）。[Int16](../data-types/int-uint.md)。
...
:::note
如果输入值无法在 [Int32](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。例如：`SELECT toInt32(2147483648) == -2147483648;`。
:::
...
:::note
如果输入值无法在 [Int32](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。
:::
...
与 [`toInt32`](#toint32) 类似，该函数将输入值转换为 [Int32](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。如果未传递 `default` 值，则在发生错误时返回 `0`。
...
- 如果成功，则返回 32 位整数值，否则返回传递的默认值（如果有），或者返回 `0`（如果没有）。[Int32](../data-types/int-uint.md)。
...
将输入值转换为 [`Int64`](../data-types/int-uint.md) 类型的值。如果发生错误，则会抛出异常。
...
:::note
如果输入值无法在 [Int64](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。例如：`SELECT toInt64(9223372036854775808) == -9223372036854775808;`。
:::
...
:::note
如果输入值无法在 [Int64](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。
:::
...
与 [`toInt64`](#toint64) 类似，该函数将输入值转换为 [Int64](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。如果未传递 `default` 值，则在发生错误时返回 `0`。
...
- 如果成功，则返回 64 位整数值，否则返回传递的默认值（如果有），或者返回 `0`（如果没有）。[Int64](../data-types/int-uint.md)。
...
将输入值转换为 [`Int128`](../data-types/int-uint.md) 类型的值。如果发生错误，则会抛出异常。
...
:::note
如果输入值无法在 [Int128](../data-types/int-uint.md) 的范围内表示，结果会发生溢出或下溢。这不视为错误。
:::
...
与 [`toInt128`](#toint128) 类似，该函数将输入值转换为 [Int128](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `0`。
...
与 [`toInt128`](#toint128) 类似，该函数将输入值转换为 [Int128](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `NULL`。
...
- 如果成功，则返回 128 位整数值，否则返回 `NULL`。[Int128](../data-types/int-uint.md) / [NULL](../data-types/nullable.md)。
...
将输入值转换为 [`Int256`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
:::note
该函数使用 [向零取整](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero)，意味着它会截断数字的小数位。
:::
...
- 如果成功，则返回 256 位整数值，否则返回 `0`。 [Int256](../data-types/int-uint.md)。
...
- 如果成功，则返回 256 位整数值，否则返回 `NULL`。 [Int256](../data-types/int-uint.md) / [NULL](../data-types/nullable.md)。
...
- `default`（可选）— 如果转换为 `Int256` 类型失败，则返回的默认值。 [Int256](../data-types/int-uint.md)。
...
- 如果成功，则返回 256 位整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [Int256](../data-types/int-uint.md)。
...
将输入值转换为 [`UInt8`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
- 如果成功，则返回 8 位无符号整数值，否则返回 `0`。 [UInt8](../data-types/int-uint.md)。
...
- 如果成功，则返回 8 位无符号整数值，否则返回 `NULL`。 [UInt8](../data-types/int-uint.md) / [NULL](../data-types/nullable.md)。
...
- `expr` — 返回数字或数字字符串表示的表达式。 [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md)。
- `default`（可选）— 如果转换为 `UInt8` 类型失败，则返回的默认值。 [UInt8](../data-types/int-uint.md)。
...
- 如果成功，则返回 8 位无符号整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [UInt8](../data-types/int-uint.md)。
...
将输入值转换为 [`UInt16`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
- 如果成功，则返回 16 位无符号整数值，否则返回 `0`。 [UInt16](../data-types/int-uint.md)。
...
- 如果成功，则返回 16 位无符号整数值，否则返回 `NULL`。 [UInt16](../data-types/int-uint.md) / [NULL](../data-types/nullable.md)。
...
- `expr` — 返回数字或数字字符串表示的表达式。 [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md)。
- `default`（可选）— 如果转换为 `UInt16` 类型失败，则返回的默认值。 [UInt16](../data-types/int-uint.md)。
...
- 如果成功，则返回 16 位无符号整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [UInt16](../data-types/int-uint.md)。
...
将输入值转换为 [`UInt32`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
- 如果成功，则返回 32 位无符号整数值，否则返回 `0`。 [UInt32](../data-types/int-uint.md)
...
- 如果成功，则返回 32 位无符号整数值，否则返回 `NULL`。 [UInt32](../data-types/int-uint.md) / [NULL](../data-types/nullable.md)。
...
- `expr` — 返回数字或数字字符串表示的表达式。 [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md)。
- `default`（可选）— 如果转换为 `UInt32` 类型失败，则返回的默认值。 [UInt32](../data-types/int-uint.md)。
...
- 如果成功，则返回 32 位无符号整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [UInt32](../data-types/int-uint.md)。
...
将输入值转换为 [`UInt64`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
:::note
如果输入值无法在 [UInt64](../data-types/int-uint.md) 的范围内表示，则结果会发生溢出或下溢。
这不被视为错误。
...
- `expr` — 返回数字或数字字符串表示的表达式。 [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md)。
- `defauult`（可选）— 如果转换为 `UInt64` 类型失败，则返回的默认值。 [UInt64](../data-types/int-uint.md)。
...
将输入值转换为 [`UInt128`](../data-types/int-uint.md) 类型的值。如出错则抛出异常。
...
:::note
如果输入值无法在 [UInt128](../data-types/int-uint.md) 的范围内表示，则结果会发生溢出或下溢。
这不被视为错误。
:::
...
像 [`toUInt128`](#touint128) 函数一样，这个函数将输入值转换为 [UInt128](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `0`。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
...
:::note
该函数采用[向零舍入](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero)的方法，这意味着它会截断数字的小数位数。
:::
...
像 [`toUInt128`](#touint128) 函数一样，这个函数将输入值转换为 [UInt128](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `NULL`。
...
像 [`toUInt128`](#toint128) 函数一样，这个函数将输入值转换为 [UInt128](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
- `default`（可选）— 解析为 `UInt128` 类型不成功时返回的默认值。 [UInt128](../data-types/int-uint.md)。
...
- 如果成功则返回 128 位无符号整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [UInt128](../data-types/int-uint.md)。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions)。
...
:::note
如果输入值无法在 [UInt256](../data-types/int-uint.md) 的范围内表示，则结果会发生溢出或下溢。
这不被视为错误。
:::
...
像 [`toUInt256`](#touint256) 函数一样，这个函数将输入值转换为 [UInt256](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `0`。
...
像 [`toUInt256`](#touint256) 函数一样，这个函数将输入值转换为 [UInt256](../data-types/int-uint.md) 类型的值，但在发生错误时返回 `NULL`。
...
像 [`toUInt256`](#touint256) 函数一样，这个函数将输入值转换为 [UInt256](../data-types/int-uint.md) 类型的值，但在发生错误时返回默认值。
如果没有传递 `default` 值，则在发生错误时返回 `0`。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
- `default`（可选）— 解析为 `UInt256` 类型不成功时返回的默认值。 [UInt256](../data-types/int-uint.md)。
...
- 如果成功则返回 256 位无符号整数值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [UInt256](../data-types/int-uint.md)。
...
像 [`toFloat32`](#tofloat32) 函数一样，这个函数将输入值转换为 [Float32](../data-types/float.md) 类型的值，但在发生错误时返回 `0`。
...
像 [`toFloat32`](#tofloat32) 函数一样，这个函数将输入值转换为 [Float32](../data-types/float.md) 类型的值，但在发生错误时返回 `NULL`。
...
像 [`toFloat32`](#tofloat32) 函数一样，这个函数将输入值转换为 [Float32](../data-types/float.md) 类型的值，但在发生错误时返回默认值。
如果没有传递 `default` 值，则在发生错误时返回 `0`。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
- `default`（可选）— 解析为 `Float32` 类型不成功时返回的默认值。 [Float32](../data-types/float.md)。

支持的参数：
- (U)Int8/16/32/64/128/256 的值或字符串表示。
- (U)Int8/16/32/128/256 的字符串表示。
- Float32/64 的值，包括 `NaN` 和 `Inf`。
- Float32/64 的字符串表示，包括 `NaN` 和 `Inf`（不区分大小写）。
...
- 如果成功则返回 32 位浮点值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [Float32](../data-types/float.md)。
...
像 [`toFloat64`](#tofloat64) 函数一样，这个函数将输入值转换为 [Float64](../data-types/float.md) 类型的值，但在发生错误时返回 `0`。
...
像 [`toFloat64`](#tofloat64) 函数一样，这个函数将输入值转换为 [Float64](../data-types/float.md) 类型的值，但在发生错误时返回 `NULL`。
...
像 [`toFloat64`](#tofloat64) 函数一样，这个函数将输入值转换为 [Float64](../data-types/float.md) 类型的值，但在发生错误时返回默认值。
如果没有传递 `default` 值，则在发生错误时返回 `0`。
...
- `expr` — 返回一个数字或数字字符串表示的表达式。 [表达式](/sql-reference/syntax#expressions) / [字符串](../data-types/string.md)。
- `default`（可选）— 解析为 `Float64` 类型不成功时返回的默认值。 [Float64](../data-types/float.md)。
...
- 如果成功则返回 64 位浮点值，否则返回传递的默认值（如果有）或 `0`（如果没有）。 [Float64](../data-types/float.md)。
...
将输入值转换为 [`BFloat16`](/sql-reference/data-types/float#bfloat16) 类型的值。 在发生错误时抛出异常。
...
- 16 位的 brain-float 值。 [BFloat16](/sql-reference/data-types/float#bfloat16)。
...
- 16 位的 brain-float 值，否则返回 `0`。 [BFloat16](/sql-reference/data-types/float#bfloat16)。

:::note
该函数在从字符串表示转换时可能会导致精度的静默丢失。
:::
...
- 16 位的 brain-float 值，否则返回 `NULL`（`\N`）。 [BFloat16](/sql-reference/data-types/float#bfloat16)。
...
如果参数是 [DateTime](../data-types/datetime.md) 或 [DateTime64](../data-types/datetime64.md)，则会截断并留下 DateTime 的日期部分：
...
如果参数是 [字符串](../data-types/string.md)，则作为 [日期](../data-types/date.md) 或 [DateTime](../data-types/datetime.md) 解析。如果它被解析为 [DateTime](../data-types/datetime.md)，则使用日期部分：
...
如果参数是一个数字，并且看起来像一个 UNIX 时间戳（大于 65535），则将其解释为 [DateTime](../data-types/datetime.md)，然后在当前时区下截断为 [日期](../data-types/date.md)。时区参数可以作为函数的第二个参数指定。截断为 [日期](../data-types/date.md) 取决于时区：
...
上面的示例展示了同一个 UNIX 时间戳在不同时区下可以被解释为不同的日期。

如果参数是一个数字，并且小于 65536，则将其解释为自 1970-01-01（第一个 UNIX 天）以来的天数，并转换为 [日期](../data-types/date.md)。这与 `日期` 数据类型的内部数字表示对应。示例：
...
如果参数不符合日期类型的范围，则会导致实现定义的行为，可能饱和到最大支持的日期或溢出：
```sql
...
与 [toDate](#todate) 相同，但如果收到无效参数则返回 [日期](../data-types/date.md) 的下限。仅支持 [字符串](../data-types/string.md) 参数。
...
与 [toDate](#todate) 相同，但如果收到无效参数则返回 `NULL`。仅支持 [字符串](../data-types/string.md) 参数。
...
像 [toDate](#todate) 一样，但如果不成功，则返回一个默认值，该值要么是第二个参数（如果指定），否则是 [日期](../data-types/date.md) 的下限。
...
- `expr` — 值。 [字符串](../data-types/string.md)、[Int](../data-types/int-uint.md)、[日期](../data-types/date.md) 或 [DateTime](../data-types/datetime.md)。
- `time_zone` — 时区。 [字符串](../data-types/string.md)。

:::note
如果 `expr` 是数字，则它被解释为自 Unix 纪元开始的秒数（作为 Unix 时间戳）。
如果 `expr` 是 [字符串](../data-types/string.md)，则它可以被解释为 Unix 时间戳，也可以作为日期/带时间的字符串表示。
因此，由于歧义，短数字字符串表示（最多四位数）被明确禁用。例如，字符串 `'1999'` 可能既是年份（未完成的日期/DateTime 字符串的字符串表示）也可能是 Unix 时间戳。较长的数字字符串是允许的。
:::

**返回值**

- 一个日期时间。 [DateTime](../data-types/datetime.md)
...
与 [toDateTime](#todatetime) 相同，但如果收到无效参数，则返回 [DateTime](../data-types/datetime.md) 的下限。仅支持 [字符串](../data-types/string.md) 参数。
...
与 [toDateTime](#todatetime) 相同，但如果收到无效参数，则返回 `NULL`。仅支持 [字符串](../data-types/string.md) 参数。
...
像 [toDateTime](#todatetime) 一样，但如果不成功，则返回一个默认值，该值可能是第三个参数（如果指定），否则是 [DateTime](../data-types/datetime.md) 的下限。
...
将参数转换为 [Date32](../data-types/date32.md) 数据类型。如果值超出范围，`toDate32` 返回 [Date32](../data-types/date32.md) 支持的边界值。如果参数具有 [日期](../data-types/date.md) 类型，则会考虑它的边界。
...
- 一个日历日期。 类型 [Date32](../data-types/date32.md)。
...
与 [toDate32](#todate32) 相同，但如果收到无效参数则返回 [Date32](../data-types/date32.md) 的最小值。
...
与 [toDate32](#todate32) 相同，但如果收到无效参数则返回 `NULL`。
...
将参数转换为 [Date32](../data-types/date32.md) 数据类型。如果值超出范围，`toDate32OrDefault` 返回 [Date32](../data-types/date32.md) 支持的下限值。如果参数具有 [日期](../data-types/date.md) 类型，则会考虑它的边界。如果收到无效参数，则返回默认值。
...
- `expr` — 值。 [字符串](../data-types/string.md)、[UInt32](../data-types/int-uint.md)、[浮点数](../data-types/float.md) 或 [DateTime](../data-types/datetime.md)。
- `scale` - 单位时间 (精度)： 10<sup>-precision</sup> 秒。有效范围：[ 0 : 9 ]。
- `timezone`（可选） - 指定的 datetime64 对象的时区。

**返回值**

- 日历日期和时间，带有亚秒级精度。 [DateTime64](../data-types/datetime64.md)。
...
在没有小数点的情况下，该值依然被视为以秒为单位的 Unix 时间戳：
...
像 [toDateTime64](#todatetime64) 函数一样，这个函数将输入值转换为 [DateTime64](../data-types/datetime64.md) 类型的值，但在收到无效参数时返回 [DateTime64](../data-types/datetime64.md) 的最小值。
...
- 一个日历日期和时间，带有亚秒级精度，否则返回 `DateTime64` 的最小值：`1970-01-01 01:00:00.000`。 [DateTime64](../data-types/datetime64.md)。
...
与 [toDateTime64](#todatetime64) 相似，此函数将输入值转换为 [DateTime64](../data-types/datetime64.md) 类型的值，但如果接收到无效参数，则返回 `NULL`。
...
- `scale` - Tick大小（精度）：10<sup>-precision</sup> 秒。有效范围：[ 0 : 9 ]。
- `timezone`（可选） - 指定的 DateTime64 对象的时区。

**返回值**

- 一日期和日期时间，具有亚秒精度，否则为 `NULL`。[DateTime64](../data-types/datetime64.md)/[NULL](../data-types/nullable.md)。
...
与 [toDateTime64](#todatetime64) 相似，此函数将输入值转换为 [DateTime64](../data-types/datetime64.md) 类型的值，但如果接收到无效参数，则返回 [DateTime64](../data-types/datetime64.md) 的默认值或提供的默认值。
...
- `scale` - Tick大小（精度）：10<sup>-precision</sup> 秒。有效范围：[ 0 : 9 ]。
- `timezone`（可选） - 指定的 DateTime64 对象的时区。
- `default`（可选） - 如果接收到无效参数，返回的默认值。[DateTime64](../data-types/datetime64.md)。

**返回值**

- 一日期和日期时间，具有亚秒精度，否则返回 `DateTime64` 的最小值或提供的 `default` 值（如果提供）。[DateTime64](../data-types/datetime64.md)。
...
将输入值转换为 [`Decimal(9, S)`](../data-types/decimal.md) 类型的值，其精度为 `S`。在出现错误时引发异常。
...
- `expr` — 返回数字或数字字符串表示的表达式。[Expression](/sql-reference/syntax#expressions)。
- `S` — 范围在 0 和 9 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
...
:::note
当 `expr` 的值超过 `Decimal32` 的范围时，可能会发生溢出：`( -1 * 10^(9 - S), 1 * 10^(9 - S) )`。
小数部分的多余位数将被丢弃（不进行四舍五入）。
整数部分的多余位数将导致异常。
:::

:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal32(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
...
- 如果成功，则返回类型为 `Decimal(9, S)` 的值，否则返回 `0` 和 `S` 位小数。[Decimal32(S)](../data-types/decimal.md)。
...
与 [`toDecimal32`](#todecimal32) 相似，此函数将输入值转换为 [Nullable(Decimal(9, S))](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- `expr` — 数字的字符串表示。[String](../data-types/string.md)。
- `S` — 范围在 0 和 9 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
- `default`（可选） — 如果转换为类型 `Decimal32(S)` 不成功，则返回的默认值。[Decimal32(S)](../data-types/decimal.md)。
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal32OrDefault(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
您可以使用字符串输入，以便操作使用底层整数类型：`toDecimal32OrDefault('1.15', 2) = 1.15`
:::

**返回值**

- 如果成功，则返回类型为 `Decimal(9, S)` 的值，否则返回传入的默认值（如果有）或 `0`（如果没有）。[Decimal32(S)](../data-types/decimal.md)。
...
将输入值转换为 [`Decimal(18, S)`](../data-types/decimal.md) 类型的值，其精度为 `S`。在出现错误时引发异常。
...
- `expr` — 返回数字或数字字符串表示的表达式。[Expression](/sql-reference/syntax#expressions)。
- `S` — 范围在 0 和 18 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
...
:::note
当 `expr` 的值超过 `Decimal64` 的范围时，可能会发生溢出：`( -1 * 10^(18 - S), 1 * 10^(18 - S) )`。
小数部分的多余位数将被丢弃（不进行四舍五入）。
整数部分的多余位数将导致异常。
:::
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal64(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
...
与 [`toDecimal64`](#todecimal64) 相似，此函数将输入值转换为 [Decimal(18, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Decimal(18, S)` 的值，否则返回 `0` 和 `S` 位小数。[Decimal64(S)](../data-types/decimal.md)。
...
与 [`toDecimal64`](#todecimal64) 相似，此函数将输入值转换为 [Nullable(Decimal(18, S))](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Nullable(Decimal(18, S))` 的值，否则返回相同类型的值 `NULL`。[Decimal64(S)](../data-types/decimal.md)。
...
与 [`toDecimal64`](#todecimal64) 相似，此函数将输入值转换为 [Decimal(18, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回默认值。
...
- `expr` — 数字的字符串表示。[String](../data-types/string.md)。
- `S` — 范围在 0 和 18 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
- `default`（可选） — 如果转换为类型 `Decimal64(S)` 不成功，则返回的默认值。[Decimal64(S)](../data-types/decimal.md)。
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal64OrDefault(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
您可以使用字符串输入，以便操作使用底层整数类型：`toDecimal64OrDefault('1.15', 2) = 1.15`
:::

**返回值**

- 如果成功，则返回类型为 `Decimal(18, S)` 的值，否则返回传入的默认值（如果有）或 `0`（如果没有）。[Decimal64(S)](../data-types/decimal.md)。
...
将输入值转换为 [`Decimal(38, S)`](../data-types/decimal.md) 类型的值，其精度为 `S`。在出现错误时引发异常。
...
- `expr` — 返回数字或数字字符串表示的表达式。[Expression](/sql-reference/syntax#expressions)。
- `S` — 范围在 0 和 38 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
...
:::note
当 `expr` 的值超过 `Decimal128` 的范围时，可能会发生溢出：`( -1 * 10^(38 - S), 1 * 10^(38 - S) )`。
小数部分的多余位数将被丢弃（不进行四舍五入）。
整数部分的多余位数将导致异常。
:::
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal128(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
...
与 [`toDecimal128`](#todecimal128) 相似，此函数将输入值转换为 [Decimal(38, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Decimal(38, S)` 的值，否则返回 `0` 和 `S` 位小数。[Decimal128(S)](../data-types/decimal.md)。
...
与 [`toDecimal128`](#todecimal128) 相似，此函数将输入值转换为 [Nullable(Decimal(38, S))](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Nullable(Decimal(38, S))` 的值，否则返回相同类型的值 `NULL`。[Decimal128(S)](../data-types/decimal.md)。
...
与 [`toDecimal128`](#todecimal128) 相似，此函数将输入值转换为 [Decimal(38, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回默认值。
...
- `expr` — 数字的字符串表示。[String](../data-types/string.md)。
- `S` — 范围在 0 和 38 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
- `default`（可选） — 如果转换为类型 `Decimal128(S)` 不成功，则返回的默认值。[Decimal128(S)](../data-types/decimal.md)。
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal128OrDefault(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
您可以使用字符串输入，以便操作使用底层整数类型：`toDecimal128OrDefault('1.15', 2) = 1.15`
:::

**返回值**

- 如果成功，则返回类型为 `Decimal(38, S)` 的值，否则返回传入的默认值（如果有）或 `0`（如果没有）。[Decimal128(S)](../data-types/decimal.md)。
...
将输入值转换为 [`Decimal(76, S)`](../data-types/decimal.md) 类型的值，其精度为 `S`。在出现错误时引发异常。
...
- `expr` — 返回数字或数字字符串表示的表达式。[Expression](/sql-reference/syntax#expressions)。
- `S` — 范围在 0 和 76 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
...
:::note
当 `expr` 的值超过 `Decimal256` 的范围时，可能会发生溢出：`( -1 * 10^(76 - S), 1 * 10^(76 - S) )`。
小数部分的多余位数将被丢弃（不进行四舍五入）。
整数部分的多余位数将导致异常。
:::
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal256(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
...
与 [`toDecimal256`](#todecimal256) 相似，此函数将输入值转换为 [Decimal(76, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Decimal(76, S)` 的值，否则返回 `0` 和 `S` 位小数。[Decimal256(S)](../data-types/decimal.md)。
...
与 [`toDecimal256`](#todecimal256) 相似，此函数将输入值转换为 [Nullable(Decimal(76, S))](../data-types/decimal.md) 类型的值，但在发生错误时返回 `0`。
...
- 如果成功，则返回类型为 `Nullable(Decimal(76, S))` 的值，否则返回相同类型的值 `NULL`。[Decimal256(S)](../data-types/decimal.md)。
...
与 [`toDecimal256`](#todecimal256) 相似，此函数将输入值转换为 [Decimal(76, S)](../data-types/decimal.md) 类型的值，但在发生错误时返回默认值。
...
- `expr` — 数字的字符串表示。[String](../data-types/string.md)。
- `S` — 范围在 0 和 76 之间的精度参数，指定数字的小数部分可以有多少位数字。[UInt8](../data-types/int-uint.md)。
- `default`（可选） — 如果转换为类型 `Decimal256(S)` 不成功，则返回的默认值。[Decimal256(S)](../data-types/decimal.md)。
...
:::warning
转换会丢弃多余的数字，并且在处理 Float32/Float64 输入时可能会以意想不到的方式运行，因为操作是使用浮点指令执行的。
例如：`toDecimal256OrDefault(1.15, 2)` 等于 `1.14` 因为 1.15 * 100 在浮点运算中是 114.99。
您可以使用字符串输入，以便操作使用底层整数类型：`toDecimal256OrDefault('1.15', 2) = 1.15`
:::

**返回值**

- 如果成功，则返回类型为 `Decimal(76, S)` 的值，否则返回传入的默认值（如果有）或 `0`（如果没有）。[Decimal256(S)](../data-types/decimal.md)。
...
用于在数字、字符串（但不是固定字符串）、日期和含时间的日期之间转换的函数。所有这些函数接受一个参数。

在进行字符串转换时，值的格式化或解析使用与 TabSeparated 格式（以及几乎所有其他文本格式）相同的规则。如果字符串无法解析，则会抛出异常，并取消请求。

在将日期转换为数字或反之时，日期对应于自 Unix 纪元开始以来的天数。当将含时间的日期转换为数字或反之时，含时间的日期对应于自 Unix 纪元开始以来的秒数。

toDate/toDateTime 函数的日期和含时间的日期格式定义如下：
...
作为例外，如果从 UInt32、Int32、UInt64 或 Int64 数字类型转换到 Date，并且该数字大于或等于 65536，则将该数字解释为 Unix 时间戳（而不是天数），并四舍五入到日期。这支持了常见的 `toDate(unix_timestamp)` 的写法，否则会导致错误，并需要写出更繁琐的 `toDate(toDateTime(unix_timestamp))`。

日期与含时间的日期之间的转换以自然方式进行：添加一个空时间或丢弃时间。

数字类型之间的转换使用与 C++ 中不同数字类型之间的赋值相同的规则。

此外，DateTime 参数的 toString 函数可以接受第二个 String 参数，其中包含时区的名称。例如：`Asia/Yekaterinburg` 在这种情况下，时间根据指定的时区进行格式化。
...
另请查看 `toUnixTimestamp` 函数。
## toFixedString {#tofixedstring}

将 [String](../data-types/string.md) 类型的参数转换为 [FixedString(N)](../data-types/fixedstring.md) 类型（固定长度 N 的字符串）。如果字符串的字节少于 N，则用空字节在右侧填充。如果字符串的字节多于 N，则抛出异常。
...
- 一个 N 长度的固定字符串 `s`。[FixedString](../data-types/fixedstring.md)。
...
接受一个字符串或固定字符串参数。返回在找到的第一个零字节处截断内容的字符串。
...
- `number` — 以字符串形式表示的值，[Int, UInt](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Decimal](../data-types/decimal.md)。
- `scale` — 小数位数，[UInt8](../data-types/int-uint.md)。
    * [Decimal](../data-types/decimal.md) 和 [Int, UInt](../data-types/int-uint.md) 类型的最大 scale 是 77（这是 Decimal 可能的最大有效数字）。
    * [Float](../data-types/float.md) 的最大 scale 是 60。

**返回值**

- 输入值表示为带有指定小数位数（scale）的 [String](../data-types/string.md)。如果请求的 scale 小于原始数字的 scale，则数字四舍五入。
...
通过将输入值视为 UInt8 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt8。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt8 的重解释值 `x`。[UInt8](/sql-reference/data-types/int-uint).
...
通过将输入值视为 UInt16 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt16。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt16 的重解释值 `x`。[UInt16](/sql-reference/data-types/int-uint).
...
通过将输入值视为 UInt32 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt32。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt32 的重解释值 `x`。[UInt32](/sql-reference/data-types/int-uint).
...
通过将输入值视为 UInt64 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt64。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt64 的重解释值 `x`。[UInt64](/sql-reference/data-types/int-uint).
...
通过将输入值视为 UInt128 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt128。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt128 的重解释值 `x`。[UInt128](/sql-reference/data-types/int-uint).
...
通过将输入值视为 UInt256 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 UInt256。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 UInt256 的重解释值 `x`。[UInt256](/sql-reference/data-types/int-uint).
...
通过将输入值视为 Int8 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int8。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int8 的重解释值 `x`。[Int8](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Int16 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int16。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int16 的重解释值 `x`。[Int16](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Int32 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int32。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int32 的重解释值 `x`。[Int32](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Int64 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int64。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int64 的重解释值 `x`。[Int64](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Int128 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int128。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int128 的重解释值 `x`。[Int128](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Int256 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Int256。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Int256 的重解释值 `x`。[Int256](/sql-reference/data-types/int-uint#integer-ranges).
...
通过将输入值视为 Float32 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Float32。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Float32 的重解释值 `x`。[Float32](../data-types/float.md)。
...
通过将输入值视为 Float64 类型的值来执行字节重解释。与 [`CAST`](#cast) 不同，函数不试图保留原始值——如果目标类型无法表示输入类型，则输出是无意义的。
...
- `x`: 值以字节重解释为 Float64。[(U)Int*](../data-types/int-uint.md)、[Float](../data-types/float.md)、[Date](../data-types/date.md)、[DateTime](../data-types/datetime.md)、[UUID](../data-types/uuid.md)、[String](../data-types/string.md) 或 [FixedString](../data-types/fixedstring.md)。

**返回值**

- 作为 Float64 的重解释值 `x`。[Float64](../data-types/float.md)。
...
接受字符串、固定字符串或数字值，并将字节解释为主机顺序（小端）。返回将解释的数字作为自 Unix 纪元开始以来的天数的日期。
...
:::note
如果提供的字符串不够长，函数将像字符串用必要数量的空字节填充一样工作。如果字符串比需要的长度长，则额外的字节将被忽略。
:::
...
这些函数接受一个字符串，并将放置在字符串开头的字节解释为主机顺序（小端）中的数字。返回将解释的日期和时间作为自 Unix 纪元开始以来的秒数。
...
此函数接受一个数字、日期或含时间的日期，并返回一个字符串，包含以主机顺序（小端）表示的对应值的字节。末尾的空字节会被删除。例如，UInt32 类型值 255 是一个长度为一字节的字符串。
...
此函数接受一个数字、日期或含时间的日期，并返回一个[FixedString](../data-types/fixedstring.md)，包含以主机顺序（小端）表示的对应值的字节。末尾的空字节会被删除。例如，UInt32 类型值 255 是一个长度为一字节的 FixedString。
...
:::note
除了这里列出的 UUID 函数外，还有专门的 [UUID 函数文档](../functions/uuid-functions.md)。
:::

接受一个 16 字节的字符串，并通过将每个 8 字节的半部解释为小端字节顺序返回一个 UUID。如果字符串不够长，则函数将像字符串用必要数量的空字节填充一样工作。如果字符串长度超过 16 字节，则末尾的额外字节将被忽略。
...
将输入值转换为指定的数据类型。与 [reinterpret](#reinterpret) 函数不同，`CAST` 尝试使用新数据类型表示相同的值。如果无法进行转换，则会引发异常。
支持多种语法变体。
...
:::note
如果输入值不符合目标类型的范围，则结果会溢出。例如，`CAST(-1, 'UInt8')` 返回 `255`。
:::
...
支持转换为 [Nullable](../data-types/nullable.md) 类型及其反向转换。
...
与 [cast](#cast) 的区别在于，`accurateCast` 不允许在转换期间发生数字类型的溢出，如果类型值 `x` 不适合类型 `T` 的范围。例：`accurateCast(-1, 'UInt8')` 将抛出异常。
...
将输入值 `x` 转换为指定的数据类型 `T`。始终返回 [Nullable](../data-types/nullable.md) 类型，并在转换值在目标类型中不可表示时返回 [NULL](/sql-reference/syntax#null)。
...
将输入值 `x` 转换为指定的数据类型 `T`。如果转换值在目标类型中不可表示，则返回默认类型值或指定的 `default_value`。
...
根据数字值和间隔单位（例如“秒”或“天”）创建 [Interval](../../sql-reference/data-types/special-data-types/interval.md) 数据类型值。
...
- `value` — 间隔的长度。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。

- `unit` — 要创建的间隔类型。[字符串文字](/sql-reference/syntax#string)。
    可能的值：
...
返回 `n` 年的 [IntervalYear](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 年数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 季度的 [IntervalQuarter](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 季度数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 个月的 [IntervalMonth](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 月数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 周的 [IntervalWeek](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 周数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 天的 [IntervalDay](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 天数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 小时的 [IntervalHour](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 小时数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 分钟的 [IntervalMinute](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 分钟数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 秒的 [IntervalSecond](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 秒数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 毫秒的 [IntervalMillisecond](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 毫秒数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
返回 `n` 微秒的 [IntervalMicrosecond](../data-types/special-data-types/interval.md) 数据类型的间隔。
...
- `n` — 微秒数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
- `n` — 纳秒数。整数或其字符串表示形式，以及浮点数。[(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md)。
...
此函数操作与 [formatDateTime](/sql-reference/functions/date-time-functions#formatdatetime) 函数相对。
...
根据 MySQL 风格格式字符串解析的输入字符串返回一个 [DateTime](../data-types/datetime.md) 值。
...
与 [parseDateTime](#parsedatetime) 相同，只是当遇到无法处理的日期格式时返回零日期。

## parseDateTimeOrNull {#parsedatetimeornull}

与 [parseDateTime](#parsedatetime) 相同，只是当遇到无法处理的日期格式时返回 `NULL`。

别名：`str_to_date`。

## parseDateTimeInJodaSyntax {#parsedatetimeinjodasyntax}

类似于 [parseDateTime](#parsedatetime)，只是格式字符串采用 [Joda](https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html) 而不是 MySQL 语法。

此函数操作与 [formatDateTimeInJodaSyntax](/sql-reference/functions/date-time-functions#formatdatetimeinjodasyntax) 函数相对。
...
根据 Joda 风格格式字符串解析的输入字符串返回一个 [DateTime](../data-types/datetime.md) 值。

**支持的格式说明符**

在 [formatDateTimeInJoda](/sql-reference/functions/date-time-functions#formatdatetime) 中列出的所有格式说明符都是支持的，除了：
- S: 秒的分数
- z: 时区
- Z: 时区偏移/ID
...
与 [parseDateTimeInJodaSyntax](#parsedatetimeinjodasyntax) 相同，只是当遇到无法处理的日期格式时返回零日期。

## parseDateTimeInJodaSyntaxOrNull {#parsedatetimeinjodasyntaxornull}

与 [parseDateTimeInJodaSyntax](#parsedatetimeinjodasyntax) 相同，只是当遇到无法处理的日期格式时返回 `NULL`。
...
根据 MySQL 风格格式字符串解析的输入字符串返回一个 [DateTime64](../data-types/datetime64.md) 值。
返回值的精度为 6。

## parseDateTime64OrZero {#parsedatetime64orzero}

与 [parseDateTime64](#parsedatetime64) 相同，只是当遇到无法处理的日期格式时返回零日期。

## parseDateTime64OrNull {#parsedatetime64ornull}

与 [parseDateTime64](#parsedatetime64) 相同，只是当遇到无法处理的日期格式时返回 `NULL`。
...
根据 Joda 风格格式字符串解析的输入字符串返回一个 [DateTime64](../data-types/datetime64.md) 值。
返回值的精度等于格式字符串中的 `S` 占位符数量（但最多为 6）。

## parseDateTime64InJodaSyntaxOrZero {#parsedatetime64injodasyntaxorzero}

与 [parseDateTime64InJodaSyntax](#parsedatetime64injodasyntax) 相同，只是当遇到无法处理的日期格式时返回零日期。

## parseDateTime64InJodaSyntaxOrNull {#parsedatetime64injodasyntaxornull}

与 [parseDateTime64InJodaSyntax](#parsedatetime64injodasyntax) 相同，只是当遇到无法处理的日期格式时返回 `NULL`。
...
- `time_zone` — 时区。函数根据时区解析 `time_string`。 [String](../data-types/string.md)。

**支持的非标准格式**

- 包含 9..10 位 [unix 时间戳](https://en.wikipedia.org/wiki/Unix_time) 的字符串。
- 包含日期和时间组件的字符串： `YYYYMMDDhhmmss`，`DD/MM/YYYY hh:mm:ss`，`DD-MM-YY hh:mm`，`YYYY-MM-DD hh:mm:ss` 等。
- 包含日期但不包含时间组件的字符串： `YYYY`，`YYYYMM`，`YYYY*MM`，`DD/MM/YYYY`，`DD-MM-YY` 等。
- 包含日期和时间的字符串： `DD`，`DD hh`，`DD hh:mm`。在这种情况下，`MM` 将替换为 `01`。
...
对于所有带分隔符的格式，该函数解析月份名称，表达为完整名称或月份名称的前 3 个字母。例如： `24/DEC/18`， `24-Dec-18`， `01-September-2018`。
如果未指定年份，则被认为等于当前年份。如果结果的 DateTime 恰好在未来（即使是在当前时刻之后的一秒），则当前年份将被替换为前一年。
...
此函数在 ISO 日期格式（例如 `YYYY-MM-DD hh:mm:ss`）和其他可以明确提取月份和日期组件的日期格式（例如 `YYYYMMDDhhmmss`，`YYYY-MM`，`DD hh` 或 `YYYY-MM-DD hh:mm:ss ±h:mm`）下表现得很像 [parseDateTimeBestEffort](#parsedatetimebesteffort)。如果无法明确提取月份和日期组件，例如 `MM/DD/YYYY`， `MM-DD-YYYY` 或 `MM-DD-YY`，则更喜欢美国日期格式，而不是 `DD/MM/YYYY`， `DD-MM-YYYY` 或 `DD-MM-YY`。作为上述格式的例外，如果月份大于 12 且小于或等于 31，则该函数将回退到 [parseDateTimeBestEffort](#parsedatetimebesteffort) 的行为，例如 `15/08/2020` 被解析为 `2020-08-15`。

## parseDateTimeBestEffortOrNull {#parsedatetimebesteffortornull}
## parseDateTime32BestEffortOrNull {#parsedatetime32besteffortornull}

与 [parseDateTimeBestEffort](#parsedatetimebesteffort) 相同，只是在遇到无法处理的日期格式时返回 `NULL`。

## parseDateTimeBestEffortOrZero {#parsedatetimebesteffortorzero}
## parseDateTime32BestEffortOrZero {#parsedatetime32besteffortorzero}

与 [parseDateTimeBestEffort](#parsedatetimebesteffort) 相同，只是在遇到无法处理的日期格式时返回零日期或零日期时间。
...
与 [parseDateTimeBestEffortUS](#parsedatetimebesteffortus) 函数相同，只是在遇到无法处理的日期格式时返回零日期（`1970-01-01`）或零日期时间（`1970-01-01 00:00:00`）。
...
- `time_string` — 包含要转换的日期或日期和时间的字符串。 [String](../data-types/string.md)。
- `precision` — 所需的精度。 `3` — 毫秒， `6` — 微秒。默认值为 `3`。可选。 [UInt8](../data-types/int-uint.md)。
...
与 [parseDateTime64BestEffort](#parsedatetime64besteffort) 相同，只是在模糊情况下更喜欢美国日期格式（`MM/DD/YYYY` 等）。

## parseDateTime64BestEffortOrNull {#parsedatetime64besteffortornull}

与 [parseDateTime64BestEffort](#parsedatetime64besteffort) 相同，只是在遇到无法处理的日期格式时返回 `NULL`。

## parseDateTime64BestEffortOrZero {#parsedatetime64besteffortorzero}

与 [parseDateTime64BestEffort](#parsedatetime64besteffort) 相同，只是在遇到无法处理的日期格式时返回零日期或零日期时间。

## parseDateTime64BestEffortUSOrNull {#parsedatetime64besteffortusornull}

与 [parseDateTime64BestEffort](#parsedatetime64besteffort) 相同，只是在模糊情况下更喜欢美国日期格式（`MM/DD/YYYY` 等），并在遇到无法处理的日期格式时返回 `NULL`。

## parseDateTime64BestEffortUSOrZero {#parsedatetime64besteffortusorzero}

与 [parseDateTime64BestEffort](#parsedatetime64besteffort) 相同，只是在模糊情况下更喜欢美国日期格式（`MM/DD/YYYY` 等），并在遇到无法处理的日期格式时返回零日期或零日期时间。
...
- `expr` — 导致一种支持的数据类型的 [表达式](/sql-reference/syntax#expressions)。
...
将 `DateTime64` 转换为具有固定秒精度的 `Int64` 值。输入值会根据其精度适当放大或缩小。
...
- `value` — 任何精度的 DateTime64 值。 [DateTime64](../data-types/datetime64.md)。
...
将 `DateTime64` 转换为具有固定毫秒精度的 `Int64` 值。输入值会根据其精度适当放大或缩小。
...
将 `DateTime64` 转换为具有固定微秒精度的 `Int64` 值。输入值会根据其精度适当放大或缩小。
...
将 `DateTime64` 转换为具有固定纳秒精度的 `Int64` 值。输入值会根据其精度适当放大或缩小。
...
将 `Int64` 转换为具有固定秒精度和可选时区的 `DateTime64` 值。输入值会根据其精度适当放大或缩小。
...
- 转换为精度为 `0` 的 DateTime64 的 `value`。 [DateTime64](../data-types/datetime64.md)。
...
将 `Int64` 转换为具有固定毫秒精度和可选时区的 `DateTime64` 值。输入值会根据其精度适当放大或缩小。
...
- 转换为精度为 `3` 的 DateTime64 的 `value`。 [DateTime64](../data-types/datetime64.md)。
...
将 `Int64` 转换为具有固定微秒精度和可选时区的 `DateTime64` 值。输入值会根据其精度适当放大或缩小。
...
- 转换为精度为 `6` 的 DateTime64 的 `value`。 [DateTime64](../data-types/datetime64.md)。
...
将 `Int64` 转换为具有固定纳秒精度和可选时区的 `DateTime64` 值。输入值会根据其精度适当放大或缩小。
...
- 转换为精度为 `9` 的 DateTime64 的 `value`。 [DateTime64](../data-types/datetime64.md)。
...
通过给定格式将任意表达式转换为字符串。与 formatRow 的不同之处在于，此函数在存在时修剪最后的 `\n`。
