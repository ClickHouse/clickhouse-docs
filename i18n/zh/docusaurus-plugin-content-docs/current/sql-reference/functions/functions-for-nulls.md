...
* `x` — 任意 Nullable 类型的原始值。[`Nullable(T)`](/sql-reference/data-types/nullable)

**返回值**

如果原始值不是 `NULL`，则返回对应的非 Nullable 类型值；如果输入值为 `NULL`，则返回一个任意值。[`Any`](/sql-reference/data-types)
...
返回第一个非 `NULL` 的参数；如果所有参数都是 `NULL`，则返回 `NULL`。[`Any`](/sql-reference/data-types) 或 [`NULL`](/sql-reference/syntax#null)
...
* `alt` — 当 `x` 为 `NULL` 时要返回的值。[`Any`](/sql-reference/data-types)

**返回值**

如果 `x` 不是 `NULL`，则返回 `x` 的值，否则返回 `alt`。[`Any`](/sql-reference/data-types)
...
如果 `x` 的数据类型是 `Nullable`，则返回 `1`，否则返回 `0`。[`UInt8`](/sql-reference/data-types/int-uint)
...
* `x` — 数值类型的值。[`UInt`](/sql-reference/data-types/int-uint)

**返回值**

当 `x` 为 `NULL` 或等于零时返回 `1`，否则返回 `0`。返回类型为 [`UInt8/16/32/64`](/sql-reference/data-types/int-uint) 或 [`Float32/Float64`](/sql-reference/data-types/float)
