...
计算两个值 `a` 和 `b` 的商。返回值类型始终为 [Float64](/sql-reference/data-types/float)。
整数除法由 `intDiv` 函数提供。
...
如果你不需要严格控制精度和/或需要更快的计算，请考虑使用 [divide](#divide)。
:::
...
在指定刻度下执行除法运算的结果。[`Decimal256`](/sql-reference/data-types/decimal)
...
你可以通过使用[三元运算符](/sql-reference/functions/conditional-functions#if)获得类似的结果：`isFinite(x) ? x : y`。
...
* `x` — 要检查是否为无穷大的值。[`Float*`](/sql-reference/data-types/float)
...
返回 `x` 和 `y` 中较小的一个值。[`Float64`](/sql-reference/data-types/float)
...
如果不需要精确控制小数精度和/或更看重计算速度，可以考虑使用 [multiply](#multiply)。
:::
...
* `result_scale` — 结果的小数精度。[`(U)Int*`](/sql-reference/data-types/int-uint)

**返回值**

按指定小数精度进行乘法运算的结果。类型：[`Decimal256`](/sql-reference/data-types/decimal)
