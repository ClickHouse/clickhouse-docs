...
- `cashflows` — 现金流数组。每个值表示支出（负值）或收入（正值）。[`Array(Int8/16/32/64)`](/sql-reference/data-types/array) 或 [`Array(Float*)`](/sql-reference/data-types/array)
- `[, guess]` — 可选的内部收益率初始估计值（常量值）（默认为 0.1）。[`Float*`](/sql-reference/data-types/float)

**返回值**

返回内部收益率，如果计算无法收敛、输入数组为空或只有一个元素、所有现金流为零或发生其他计算错误，则返回 `NaN`。[`Float64`](/sql-reference/data-types/float)
...
- `date` — 与现金流相对应的唯一日期的已排序数组。[`Array(Date)`](/sql-reference/data-types/array) 或 [`Array(Date32)`](/sql-reference/data-types/array)
...
计算一系列现金流的净现值（NPV），假设每次现金流之间的时间间隔相等。

默认模式（`start_from_zero` = true）：

$$
\sum_{i=0}^{N-1} \frac{values_i}{(1 + rate)^i}
$$

Excel 兼容模式（`start_from_zero` = false）：
...
- `[, start_from_zero]` — 可选布尔参数，指示 NPV 计算是从第 `0` 期（true）还是第 `1` 期（false，Excel 兼容）开始。默认值：true。[`Bool`](/sql-reference/data-types/boolean)
...
- `dates` — 与每笔现金流对应的日期数组。必须与现金流数组大小相同。[`Array(Date)`](/sql-reference/data-types/array) 或 [`Array(Date32)`](/sql-reference/data-types/array)
