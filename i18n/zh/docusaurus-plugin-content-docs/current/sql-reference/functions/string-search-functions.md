...
返回在 `haystack` 中找到的第一个 `needle` 在 `needles` 数组中的从 1 开始的索引（位置）。如果未找到任何 `needle`，则返回 0。搜索大小写不敏感，并基于 UTF-8 字符编码。[`UInt64`](/sql-reference/data-types/int-uint)
