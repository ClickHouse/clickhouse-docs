...
接受一个 UInt32 数字，将其解释为大端格式的 IPv4 地址。返回一个包含对应 IPv4 地址的字符串，格式为 A.B.C.d（以点分隔的十进制数字）。
...
由于使用 'xxx' 是非常不寻常的，未来可能会改变。我们建议您不要依赖于此片段的确切格式。

### IPv6NumToString(x) {#ipv6numtostringx}

接受一个包含 IPv6 地址的 FixedString(16) 值，格式为二进制。返回一个包含此地址的字符串，格式为文本。
...
如果输入字符串包含有效的 IPv4 地址，则返回其 IPv6 等效值。
HEX 可以是大写或小写。
...
接受一个 `UInt32` 数字，将其解释为 [大端](https://en.wikipedia.org/wiki/Endianness) 格式的 IPv4 地址。返回一个包含以二进制格式表示的 IPv6 地址的 `FixedString(16)` 值。示例：
...
接受一个包含 IPv6 地址的 FixedString(16) 值，以二进制格式返回一个字符串，移除指定字节数的地址。示例：
...
接受一个 IPv4 地址和一个包含 [CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) 的 UInt8 值。返回一个包含子网的下限范围和上限范围的两个 IPv4 的元组。
...
接受一个 IPv6 地址和一个包含 CIDR 的 UInt8 值。返回一个包含子网的下限范围和上限范围的两个 IPv6 的元组。
...
- `default`（可选）— 如果 `string` 格式无效要返回的值。 [IPv4](../data-types/ipv4.md)。
...
- `string` 转换为当前 IPv4 地址，或者如果 `string` 是无效地址则返回 null。 [String](../data-types/string.md)。
...
- `string` 转换为当前 IPv4 地址，或者如果 `string` 是无效地址则返回 `0.0.0.0`。 [String](../data-types/string.md)。
...
类似于 [IPv6StringToNum](#ipv6stringtonum) 和 [IPv6NumToString](#ipv6numtostringx) 函数，这些函数在二进制格式（即 `FixedString(16)`）之间转换 IPv6 地址。

如果输入字符串包含有效的 IPv4 地址，则返回该 IPv4 地址的 IPv6 等效值。
...
- `string` — IP 地址。 [String](../data-types/string.md)。
- `default`（可选）— 如果 `string` 格式无效要返回的值。 [IPv6](../data-types/ipv6.md)。
...
- IP 地址。 [IPv6](../data-types/ipv6.md)，或者如果 `string` 格式无效则返回 null。
...
- IP 地址。 [IPv6](../data-types/ipv6.md)，或者如果 `string` 格式无效则返回 `::`。
...
确定 IP 地址是否包含在以 [CIDR](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing) 表示的网络中。如果符合条件，则返回 `1`，否则返回 `0`。
...
此函数接受以字符串表示的 IPv4 和 IPv6 地址（和网络）。如果 IP 地址的版本与 CIDR 不匹配，则返回 `0`。
