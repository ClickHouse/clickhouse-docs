...
* `num` — 以 `UInt32` 无符号 32 位整数形式表示的 IPv4 地址。[`UInt32`](/sql-reference/data-types/int-uint)

**返回值**

返回表示 IPv4 地址的字符串，如果格式无效则返回 `0`。[`String`](/sql-reference/data-types/string)
...
返回 IPv4 地址字符串，并将最后一个八位组替换为 xxx。[`String`](/sql-reference/data-types/string)
...
返回 IPv4 地址的数值表示形式。[`UInt32`](/sql-reference/data-types/int-uint)
...
返回一个包含两个 IPv6 地址的元组，用于表示子网范围。[`Tuple(IPv6, IPv6)`](/sql-reference/data-types/tuple)
...
返回以文本格式表示的 IPv6 地址字符串。[`String`](/sql-reference/data-types/string)
...
返回 IPv6 地址的二进制格式。[`FixedString(16)`](/sql-reference/data-types/fixedstring)
...
以二进制格式返回 IPv6 地址；如果地址无效，则返回 `NULL`。[`Nullable(FixedString(16))`](/sql-reference/data-types/nullable)
...
接受一个包含 IPv6 地址二进制表示的 `FixedString(16)` 值。
返回一个字符串，内容为在文本格式下移除指定字节数后的该地址。
...
* `x` — 二进制格式的 IPv6 地址。[`FixedString(16)`](/sql-reference/data-types/fixedstring) 或 [`IPv6`](/sql-reference/data-types/ipv6)
* `bytesToCutForIPv6` — 对 IPv6 需要截断的字节数。[`UInt8`](/sql-reference/data-types/int-uint)
* `bytesToCutForIPv4` — 对 IPv4 需要截断的字节数。[`UInt8`](/sql-reference/data-types/int-uint)

**返回值**

返回一个字符串，包含在移除指定字节后得到的文本格式 IPv6 地址。[`String`](/sql-reference/data-types/string)
...
* `prefix` — 使用 CIDR 表示法的 IPv4 或 IPv6 网络前缀。[`String`](/sql-reference/data-types/string)
...
判断输入字符串是否为 IPv4 地址。
IPv6 版本请参阅 [`isIPv6String`](#isIPv6String)。
...
它类似于 [`IPv4StringToNum`](/sql-reference/functions/ip-address-functions#IPv4StringToNum) 和 [`IPv4NumToString`](/sql-reference/functions/ip-address-functions#IPv4NumToString) 函数，但同时支持字符串和无符号整数数据类型作为输入参数。
...
* `default` — 可选。当 `string` 不是有效 IPv4 地址时要返回的值。[`IPv4`](/sql-reference/data-types/ipv4)
...
* `x` — IPv4 地址的字符串或整数形式表示。[`String`](/sql-reference/data-types/string) 或 [`Integer`](/sql-reference/data-types/int-uint)
...
转换成功时返回 IPv6 地址，否则返回 `NULL`。[`IPv6`](/sql-reference/data-types/ipv6) 或 [`NULL`](/sql-reference/syntax#null)
...
如果成功，返回一个 IPv6 地址，否则返回全零 IPv6 地址 (`::`)。[`IPv6`](/sql-reference/data-types/ipv6)
