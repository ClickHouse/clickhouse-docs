...
用于在字符串中[搜索](string-search-functions.md)和[替换](string-replace-functions.md)的函数在单独的章节中进行说明。
...
* `c` — 当 `s` 不以该字符结尾时要追加的字符。[`String`](/sql-reference/data-types/string)
...
返回第一个字符的 ASCII 码点。如果 `s` 为空，结果为 `0`。如果第一个字符不是 ASCII 字符，或者不属于 UTF-16 的 Latin-1 补充区，则结果未定义。[`Int32`](/sql-reference/data-types/int-uint)
...
返回一个字符串，包含参数的编码结果。[`String`](/sql-reference/data-types/string) 或 [`FixedString`](/sql-reference/data-types/fixedstring)
...
* `encoded` — 要进行编码的字符串列或常量。如果字符串不是有效的 Base64 编码字符串，将抛出异常。[`String`](/sql-reference/data-types/string)
...
返回两个字符串的汉明距离。[`UInt64`](/sql-reference/data-types/int-uint)
...
（例如罗马数字、带圈数字），但请注意，某些连字（如 `ﬃ`）仍会被分解，因为 Unicode 大小写折叠本身会将其展开。
...
经过大小写折叠的 UTF-8 字符串。[`String`](/sql-reference/data-types/string)
...
非 [`String`](../data-types/string.md) 或 [`FixedString`](../data-types/fixedstring.md) 类型的参数，会先使用其默认序列化方式转换为字符串。
...
返回由参数拼接得到的 String。如果任一参数为 `NULL`，则函数返回 `NULL`。如果没有参数，则返回空字符串。[`Nullable(String)`](/sql-reference/data-types/nullable)
...
返回通过连接各参数生成的字符串。如果任一参数值为 `NULL`，则函数返回 `NULL`。如果未传递任何参数，则返回空字符串。[`String`](/sql-reference/data-types/string)
...
* `exp1, exp2, ...` — 要连接的表达式。类型不是 `String` 或 `FixedString` 的参数会通过其默认序列化方式转换为字符串。由于这会影响性能，不推荐使用非 String/FixedString 参数。[`Any`](/sql-reference/data-types)

**返回值**

返回由参数连接生成的 String。如果任一参数值为 `NULL`，则函数返回 `NULL`。[`String`](/sql-reference/data-types/string)
...
* `sep` — 要使用的分隔符。[`const String`](/sql-reference/data-types/string) 或 [`const FixedString`](/sql-reference/data-types/fixedstring)
* `exp1, exp2, ...` — 要连接的表达式。非 `String` 或 `FixedString` 类型的参数会使用其默认序列化方式转换为字符串。由于这会降低性能，不推荐使用非 `String`/`FixedString` 参数。[`String`](/sql-reference/data-types/string) 或 [`FixedString`](/sql-reference/data-types/fixedstring)

**返回值**

返回由参数连接而成的字符串。如果任意参数值为 `NULL`，则函数返回 `NULL`。[`String`](/sql-reference/data-types/string)
...
返回将字符串 `s` 从编码 `from` 转换为编码 `to` 后的结果字符串。[`String`](/sql-reference/data-types/string)
...
返回一个字符串，其中的 HTML 实体已被解码。[`String`](/sql-reference/data-types/string)
...
* `s` — 需要检查的字符串。[`String`](/sql-reference/data-types/string)
...
如果 `s` 的结尾在不区分大小写的情况下等于 `suffix`，则返回 `1`，否则返回 `0`。[`UInt8`](/sql-reference/data-types/int-uint)
...
* `html` — 包含要提取文本内容的 HTML 字符串。[`String`](/sql-reference/data-types/string)

**返回值**

返回提取出的文本内容，其中空白字符已规范化处理。[`String`](/sql-reference/data-types/string)
...
按照 IDNA 机制返回输入字符串的 ASCII 表示形式。[`String`](/sql-reference/data-types/string)
...
返回对 `s` 中每个单词的首字母转换为大写后的结果。[`String`](/sql-reference/data-types/string)
...
返回将 `s` 中每个单词首字母转换为大写后的字符串。[`String`](/sql-reference/data-types/string)
...
* `s` — 用于检查其 UTF-8 编码是否有效的字符串。[`String`](/sql-reference/data-types/string)

**返回值**

如果这组字节构成有效的 UTF-8 编码文本，则返回 `1`，否则返回 `0`。[`UInt8`](/sql-reference/data-types/int-uint)
...
返回一个按指定长度左侧填充后的字符串。[`String`](/sql-reference/data-types/string)
...
* `string` — 需要进行填充的输入字符串。[`String`](/sql-reference/data-types/string)
* `length` — 结果字符串的长度。如果该值小于输入字符串的长度，则输入字符串会被截断为 `length` 个字符。[`(U)Int*`](/sql-reference/data-types/int-uint)
* `pad_string` — 可选。用于填充输入字符串的字符串。如果未指定，则输入字符串会使用空格进行填充。[`String`](/sql-reference/data-types/string)
...
* `s` — 用于计算子字符串的 UTF-8 编码字符串。[`String`](/sql-reference/data-types/string) 或 [`FixedString`](/sql-reference/data-types/fixedstring)
...
字符串 `s` 的长度，以 Unicode 码点计。[`UInt64`](/sql-reference/data-types/int-uint)
...
* `s` — 要转换为小写形式的字符串。[`String`](/sql-reference/data-types/string)
...
返回一个小写形式的字符串。[`String`](/sql-reference/data-types/string)
...
* `s` — 要转换为自然排序键的字符串。[`String`](/sql-reference/data-types/string)

**返回值**

返回从 `s` 生成的自然排序键字符串。[`String`](/sql-reference/data-types/string)
...
根据 [NFKC&#95;Casefold 规范化形式](https://unicode.org/reports/tr44/#NFKC_Casefold)对 UTF-8 字符串进行规范化，即先执行 NFKC 规范化，再进行大小写折叠。
这对于标识符的大小写不敏感匹配非常有用。
...
返回 UTF-8 字符串经 NFKC&#95;Casefold 归一化后的结果。[`String`](/sql-reference/data-types/string)
...
去除变音符号后的 UTF-8 字符串。[`String`](/sql-reference/data-types/string)
...
一个字符串，其内容为将字符串 `s` 重复 `n` 次后的结果。如果 `n` 为负数，函数返回空字符串。[`String`](/sql-reference/data-types/string)
...
返回一个字符串，其中的 Unicode 码点序列被反转。[`String`](/sql-reference/data-types/string)
...
返回指定长度的右侧填充后的字符串。[`String`](/sql-reference/data-types/string)
...
返回输入字符串的 Soundex 代码。[`String`](/sql-reference/data-types/string)
...
* `min_cutoff_length` — 可选。如果指定，则仅返回长度大于或等于 `min_cutoff_length` 的 n-gram。默认值与 `min_ngram_length` 相同。不得小于 `min_ngram_length`，且不得大于 `max_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)

**返回值**

返回选取子字符串的数组。[`Array(String)`](/sql-reference/data-types/array)
...
* `s` — 输入字符串。[`String`](/sql-reference/data-types/string)
* `min_ngram_length` — 可选。要提取的 n-gram 的最小长度。默认值和最小值都是 3。[`UInt*`](/sql-reference/data-types/int-uint)
* `max_ngram_length` — 可选。要提取的 n-gram 的最大长度。默认值为 100。不得小于 `min_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)
* `min_cutoff_length` — 可选。如果指定，则只返回长度大于或等于 `min_cutoff_length` 的 n-gram。默认值与 `min_ngram_length` 相同。取值不得小于 `min_ngram_length`，且不得大于 `max_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)
...
* `min_ngram_length` — 可选。提取的 n-gram 的最小长度。默认值和最小值为 3。[`UInt*`](/sql-reference/data-types/int-uint)
* `max_ngram_length` — 可选。提取的 n-gram 的最大长度。默认值为 100。不得小于 `min_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)
* `min_cutoff_length` — 可选。若指定，则仅返回长度大于或等于 `min_cutoff_length` 的 n-gram。默认值与 `min_ngram_length` 相同。不得小于 `min_ngram_length`，且不得大于 `max_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)

**返回值**

返回选定 UTF-8 子字符串的 CRC32 哈希值数组。[`Array(UInt32)`](/sql-reference/data-types/array)
...
* `s` — 输入字符串。[`String`](/sql-reference/data-types/string)
* `min_ngram_length` — 可选。提取的 n-gram 的最小长度。默认值和最小值都是 3。[`UInt*`](/sql-reference/data-types/int-uint)
* `max_ngram_length` — 可选。提取的 n-gram 的最大长度。默认值为 100。值不能小于 `min_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)
* `min_cutoff_length` — 可选。如果指定，则只返回长度大于或等于 `min_cutoff_length` 的 n-gram。默认值与 `min_ngram_length` 相同。值不能小于 `min_ngram_length` 且不能大于 `max_ngram_length`。[`UInt*`](/sql-reference/data-types/int-uint)
...
返回字符串中唯一字节的数量。[`UInt16`](/sql-reference/data-types/int-uint)
...
* `s1` — 第一个输入的 UTF-8 字符串。[`String`](/sql-reference/data-types/string)
* `s2` — 第二个输入的 UTF-8 字符串。[`String`](/sql-reference/data-types/string)

**返回值**

返回两个 UTF-8 字符串之间的 Jaccard 相似系数。[`Float64`](/sql-reference/data-types/float)
...
返回从索引 `offset` 开始、长度最多为 `length` 个字节的 `s` 的子字符串。[`String`](/sql-reference/data-types/string)
...
* `count` — 在提取子串前要统计的分隔符出现次数。如果 `count` 为正数，则返回从左侧开始计数、最后一个分隔符左侧的所有内容。如果 `count` 为负数，则返回从右侧开始计数、最后一个分隔符右侧的所有内容。[`UInt`](/sql-reference/data-types/int-uint) 或 [`Int`](/sql-reference/data-types/int-uint)

**返回值**

返回 `s` 中在第 `count` 次出现的 `delim` 之前的子串。[`String`](/sql-reference/data-types/string)
...
* `delim` — 用作分隔符的字符。[`String`](/sql-reference/data-types/string)
...
返回 `s` 中第 `count` 次出现 `delim` 之前的子字符串。[`String`](/sql-reference/data-types/string)
...
返回从码点索引 `offset` 开始，长度为 `length` 个码点的 `s` 的子串。[`String`](/sql-reference/data-types/string)
...
返回在字符串两端移除指定字符后的结果。[`String`](/sql-reference/data-types/string)
...
* `input` — 要处理的字符串。[`String`](/sql-reference/data-types/string)
...
* `s` — 要修剪的字符串。[`String`](/sql-reference/data-types/string)
* `trim_characters` — 可选，要修剪的字符。如果未指定，则会删除常见的空白字符。[`String`](/sql-reference/data-types/string)
...
* `encoded` — 要解码的字符串列或常量。如果字符串不是有效的 Base32 编码格式，则在出错时返回空字符串。[`String`](/sql-reference/data-types/string)
...
根据输入值的 IDNA 规则返回输入字符串的 ASCII 表示形式，如果输入无效则返回空字符串。[`String`](/sql-reference/data-types/string)
...
返回输入值解码后的字符串，如果输入无效则返回空字符串。[`String`](/sql-reference/data-types/string)
...
返回由 `s` 转换得到的大写字符串。[`String`](/sql-reference/data-types/string)
