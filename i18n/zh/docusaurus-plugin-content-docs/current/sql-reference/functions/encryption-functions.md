...
这些函数实现了使用 AES（高级加密标准）算法的数据加密和解密。

密钥长度取决于加密模式。对于 `-128-`、`-196-` 和 `-256-` 模式，密钥长度分别为 16、24 和 32 字节。

初始化向量长度始终为 16 字节（超出 16 字节的字节会被忽略）。

请注意，这些函数在 ClickHouse 21.1 之前的工作速度较慢。
...
- `iv` — 初始化向量. 对于 `-gcm` 模式是必需的，其他模式是可选的. [String](/sql-reference/data-types/string).
- `aad` — 附加认证数据。它不会被加密，但会影响解密。仅在 `-gcm` 模式下有效，对于其他模式会抛出异常. [String](/sql-reference/data-types/string).
...
插入一些数据（请避免将密钥/初始化向量存储在数据库中，因为这会破坏整个加密概念），同时存储“提示”也是不安全的，仅用于示例目的：
...
在相同输入时将产生与 `encrypt` 相同的密文。但当 `key` 或 `iv` 超出正常长度时，`aes_encrypt_mysql` 将遵循 MySQL 的 `aes_encrypt` 做法：‘折叠’ `key` 并忽略多余的 `iv` 位。
...
- `mode` — 加密模式. [String](/sql-reference/data-types/string).
- `plaintext` — 需要被加密的文本. [String](/sql-reference/data-types/string).
- `key` — 加密密钥。如果密钥长度超过模式要求，将执行 MySQL 特定的密钥折叠. [String](/sql-reference/data-types/string).
...
在相同输入的情况下，`encrypt` 和 `aes_encrypt_mysql` 生成相同的密文：
...
但是当 `key` 或 `iv` 超出预期长度时，`encrypt` 会失败：
...
而 `aes_encrypt_mysql` 生成 MySQL 兼容的输出：
...
- `iv` — 初始化向量. 对于 `-gcm` 模式是必需的，其他模式是可选的. [String](/sql-reference/data-types/string).
- `aad` — 附加认证数据。如果此值不正确将无法解密。仅在 `-gcm` 模式下有效，对于其他模式会抛出异常. [String](/sql-reference/data-types/string).
...
请注意，只有一部分数据被正确解密，其他部分由于 `mode`、`key` 或 `iv` 在加密时不同而变为乱码。

## tryDecrypt {#trydecrypt}

与 `decrypt` 类似，但如果解密因使用错误的密钥而失败，则返回 NULL。

**示例**

让我们创建一个表，其中 `user_id` 是唯一用户 ID，`encrypted` 是加密字符串字段，`iv` 是解密/加密的初始向量。假设用户知道他们的 ID 和密钥以解密加密字段：
...
与 MySQL 加密兼容，并解密使用 [AES_ENCRYPT](https://dev.mysql.com/doc/refman/8.0/en/encryption-functions.html#function_aes-encrypt) 函数加密的数据。

在相同输入上将产生与 `decrypt` 相同的明文。但是当 `key` 或 `iv` 超出正常长度时，`aes_decrypt_mysql` 将遵循 MySQL 的 `aes_decrypt` 做法：‘折叠’ `key` 并忽略多余的 IV 位。
