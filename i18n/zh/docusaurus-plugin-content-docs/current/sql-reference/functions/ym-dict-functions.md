...
:::note
为了使下面的函数能够正常工作，服务器配置必须指定获取所有嵌入字典的路径和地址。这些字典在任何这些函数的首次调用时加载。如果无法加载参考列表，将抛出异常。

因此，本节中显示的示例将在默认情况下在 [ClickHouse Fiddle](https://fiddle.clickhouse.com/) 和快速发布及生产部署中抛出异常，除非首先进行了配置。
:::

有关创建参考列表的信息，请参见【“字典”】(../dictionaries#embedded-dictionaries)部分。

## 多个地理库 {#multiple-geobases}

ClickHouse 支持同时处理多个备选地理库（区域层级），以支持对某些区域所属国家的不同视角。

'clickhouse-server' 配置指定了与区域层级相关的文件：
...
接受一个区域 ID 和地理库，并返回相应语言的区域名称字符串。如果指定 ID 的区域不存在，则返回空字符串。
...
- `id` — 来自地理库的区域 ID。 [UInt32](../data-types/int-uint)。
- `geobase` — 字典键。请参见【多个地理库】(#multiple-geobases)。 [String](../data-types/string)。可选。

**返回值**

- 使用 `geobase` 指定的相应语言的区域名称。 [String](../data-types/string)。
- 否则，返回空字符串。
...
接受来自地理库的区域 ID。如果该区域是城市或城市的一部分，则返回适当城市的区域 ID。否则，返回 0。
...
将区域转换为地区（地理库中的类型 5）。在其他方面，此函数与【'regionToCity'】(#regiontocity)相同。
...
- 适当地区的区域 ID（如果存在）。 [UInt32](../data-types/int-uint)。
- 如果没有，则返回 0。
...
将区域转换为联邦区（地理库中的类型 4）。在其他方面，此函数与【'regionToCity'】相同。
...
将区域转换为国家（地理库中的类型 3）。在其他方面，此函数与【'regionToCity'】相同。
...
将区域转换为大陆（地理库中的类型 1）。在其他方面，此函数与【'regionToCity'】相同。
...
- 顶级大陆的标识符（即在区域层级中向上攀登时的那个）。 [UInt32](../data-types/int-uint)。
- 如果没有，则返回 0。
...
获取区域的人口。人口可以在与地理库相关的文件中记录。请参见【“字典”】(../dictionaries#embedded-dictionaries)部分。如果该区域没有记录人口，则返回 0。在地理库中，人口可能会被记录在子区域而不是父区域。
...
- `lhs` — 来自地理库的左侧区域 ID。 [UInt32](../data-types/int-uint)。
- `rhs` — 来自地理库的右侧区域 ID。 [UInt32](../data-types/int-uint)。
- `geobase` — 字典键。请参见【多个地理库】(#multiple-geobases)。 [String](../data-types/string)。可选。
...
接受一个 UInt32 数字——来自地理库的区域 ID。返回由传入区域及沿链的所有父区域 ID 组成的数组。
...
- 由传入区域及沿链的所有父区域组成的区域 ID 数组。 [Array](../data-types/array)([UInt32](../data-types/int-uint))。
