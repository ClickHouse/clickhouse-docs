...
此类型表示其他数据类型的联合。类型 `Variant(T1, T2, ..., TN)` 表示该类型的每一行具有类型 `T1` 或 `T2` 或 ... 或 `TN` 的值，或者为 none（`NULL` 值）。
...
### 将字符串列转换为变体列 {#converting-a-string-column-to-a-variant-column}
...
### 将普通列转换为变体列 {#converting-an-ordinary-column-to-a-variant-column}
...
注意：从 `String` 类型的转换总是通过解析进行的，如果你需要将 `String` 列转换为不进行解析的 `Variant` 的 `String`，可以执行以下操作：
```sql
...
### 将变体列转换为普通列 {#converting-a-variant-column-to-an-ordinary-column}
...
### 将变体转换为另一个变体 {#converting-a-variant-to-another-variant}
...
## 比较变体类型的值 {#comparing-values-of-variant-data}

`Variant` 类型的值只能与相同 `Variant` 类型的值进行比较。

操作符 `<` 对于具有底层类型 `T1` 和 `T2` 的值 `v1` 和 `v2` 的结果如下定义：
