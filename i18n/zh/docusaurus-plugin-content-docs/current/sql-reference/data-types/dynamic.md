---
'description': 'ClickHouse 中的 Dynamic 数据类型文档，允许在单个列中存储不同类型的值'
...
### 通过解析将字符串列转换为 Dynamic 列 {#converting-a-string-column-to-a-dynamic-column-through-parsing}
...
- 如果 `T1 = T2 = T`，结果将是 `d1.T < d2.T`（将比较基础值）。
...
**注意：** 由于 [特定工作](#using-dynamic-type-in-functions) 的函数在执行比较时不适用上述比较规则，例如 `<`/`>`/`=` 和其他函数。
...
## Dynamic 的 JSONExtract 函数 {#jsonextract-functions-with-dynamic}
