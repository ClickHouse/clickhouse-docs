---
description: 'ClickHouse 中 Dynamic 数据类型的文档，该类型可以在同一列中存储多种类型的值'
...
此类型允许在其中存储任意类型的值，而无需事先知道所有可能的类型。
...
其中 `N` 是一个可选参数，取值范围为 `0` 到 `254`，用于指示在一个单独存储的数据块中（例如 MergeTree 表的单个数据部分），类型为 `Dynamic` 的列内部最多可以有多少种不同的数据类型作为独立子列存储。如果超出该限制，所有具有新数据类型的值将统一以二进制形式存储在一个特殊的共享数据结构中。`max_types` 的默认值为 `32`。
...
## 将 Dynamic 嵌套类型作为子列读取 {#reading-dynamic-nested-types-as-subcolumns}

`Dynamic` 类型支持使用类型名称作为子列,从 `Dynamic` 列中读取单个嵌套类型。
因此,如果您有列 `d Dynamic`,可以使用语法 `d.T` 读取任何有效类型 `T` 的子列,
如果 `T` 可以位于 `Nullable` 内,则此子列的类型为 `Nullable(T)`,否则为 `T`。此子列的大小将与原始 `Dynamic` 列相同,并且在原始 `Dynamic` 列不具有类型 `T` 的所有行中将包含 `NULL` 值(如果 `T` 不能位于 `Nullable` 内,则为空值)。
...
要了解每一行中存储的是哪种变体类型，可以使用函数 `dynamicType(dynamic_column)`。它会为每一行返回一个包含值类型名称的 `String`（如果该行为 `NULL`，则返回 `'None'`）。
...
要从 `String` 列解析 `Dynamic` 类型值,可以启用 `cast_string_to_dynamic_use_inference` 设置:
...
如果 `K < N`,则最少见类型的值将被插入到单个特殊子列中,但仍然可以访问:
...
函数 `isDynamicElementInSharedData` 对存储在 `Dynamic` 内部特殊共享数据结构中的行返回 `true`。正如我们所见，结果列只包含两种未存储在共享数据结构中的类型。

如果 `K=0`，所有类型都会插入到同一个特殊子列中：
...
所有文本格式(TSV、CSV、CustomSeparated、Values、JSONEachRow 等)均支持读取 `Dynamic` 类型。在数据解析过程中,ClickHouse 会尝试推断每个值的类型,并在插入 `Dynamic` 列时使用推断出的类型。
...
大多数函数支持 `Dynamic` 类型的参数。在这种情况下,函数会针对 `Dynamic` 列内部存储的每种数据类型分别执行。
当函数的结果类型取决于参数类型时,使用 `Dynamic` 参数执行此类函数的结果将为 `Dynamic` 类型。当函数的结果类型不取决于参数类型时,结果将为 `Nullable(T)` 类型,其中 `T` 是该函数的常规结果类型。
...
如果某个函数无法对 `Dynamic` 列中的某个类型执行，则会抛出异常：
...
```text
接收到异常：
Code: 43. DB::Exception: 函数 plus 的参数类型 Array(Int64) 和 UInt8 非法：执行 'FUNCTION plus(__table1.d : 3, 1_UInt8 :: 1) -> plus(__table1.d, 1_UInt8) Dynamic : 0' 时。(ILLEGAL_TYPE_OF_ARGUMENT)
```
...
在 `ORDER BY` 和 `GROUP BY` 中,`Dynamic` 类型值的比较方式与 `Variant` 类型值类似:
对于 `Dynamic` 类型的值 `d1`(底层类型为 `T1`)和 `d2`(底层类型为 `T2`),运算符 `<` 的结果定义如下:

- 如果 `T1 = T2 = T`,结果为 `d1.T < d2.T`(比较底层值)。
- 如果 `T1 != T2`,结果为 `T1 < T2`(比较类型名称)。

默认情况下,`Dynamic` 类型不允许用于 `GROUP BY`/`ORDER BY` 键,如果要使用它,请注意其特殊的比较规则并启用 `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by` 设置。
...
**注意:** 具有不同数值类型的 Dynamic 类型值被视为不同的值,它们之间不进行比较,而是比较它们的类型名称。
...
**注意:** 上述比较规则不适用于 `<`/`>`/`=` 等比较函数的执行,因为函数对 `Dynamic` 类型有[特殊处理](#using-dynamic-type-in-functions)


## 达到 Dynamic 内存储的不同数据类型数量上限 {#reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic}

`Dynamic` 数据类型只能将有限数量的不同数据类型存储为独立子列。默认情况下,此上限为 32,但您可以在类型声明中使用 `Dynamic(max_types=N)` 语法来更改它,其中 N 的取值范围为 0 到 254(由于实现细节的限制,Dynamic 内部最多只能将 254 种不同的数据类型存储为独立子列)。
当达到上限时,所有插入到 `Dynamic` 列的新数据类型都将被插入到一个共享数据结构中,该结构以二进制形式存储不同数据类型的值。

让我们看看在不同场景下达到上限时会发生什么。

### 数据解析期间达到上限 {#reaching-the-limit-during-data-parsing}

在解析数据中的 `Dynamic` 值时,当当前数据块达到上限后,所有新值都将被插入到共享数据结构中:
...
如我们所见,在插入 3 种不同的数据类型 `Int64`、`Array(Int64)` 和 `String` 之后,所有新类型都被插入到特殊的共享数据结构中。

### MergeTree 表引擎中数据部分合并期间 {#during-merges-of-data-parts-in-mergetree-table-engines}

在 MergeTree 表中合并多个数据部分时,结果数据部分中的 `Dynamic` 列可能会达到可在内部独立子列中存储的不同数据类型的上限,从而无法将源部分中的所有类型都存储为子列。
在这种情况下,ClickHouse 会选择哪些类型在合并后保留为独立子列,哪些类型将被插入到共享数据结构中。在大多数情况下,ClickHouse 会尝试保留最常见的类型,并将最罕见的类型存储在共享数据结构中,但这取决于具体实现。

让我们看一个这样的合并示例。首先,创建一个带有 `Dynamic` 列的表,将不同数据类型的上限设置为 `3`,并插入具有 `5` 种不同类型的值:
...
每次插入都会创建一个独立的数据部分,其 `Dynamic` 列包含单一类型:
...
可以看到，ClickHouse 将最常见的类型 `UInt64` 和 `Array(UInt64)` 保留为子列，而将所有其他类型写入共享数据区。


## 使用 Dynamic 类型的 JSONExtract 函数 {#jsonextract-functions-with-dynamic}

所有 `JSONExtract*` 函数均支持 `Dynamic` 类型:
