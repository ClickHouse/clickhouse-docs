---
description: 'ClickHouse 中 Date32 数据类型的文档，该类型相较于 Date 类型提供了更大范围的日期存储能力'
...
日期类型。支持与 [DateTime64](../../sql-reference/data-types/datetime64.md) 相同的日期范围。以本机字节序的有符号 32 位整数存储，其数值表示自 `1900-01-01` 起的天数。**重要！** 0 表示 `1970-01-01`，负值表示 `1970-01-01` 之前的天数。

**示例**

创建一个带有 `Date32` 类型列的表并向其中插入数据：
