---
'description': 'ClickHouse 中浮点数据类型 Float32、Float64 和 BFloat16 的文档'
'sidebar_label': 'Float32 | Float64 | BFloat16'
'sidebar_position': 4
'slug': '/sql-reference/data-types/float'
'title': '浮点数类型 Float32 | Float64 | BFloat16'
---



:::note
如果您需要准确的计算，特别是处理需要高精度的金融或商业数据时，您应该考虑使用 [Decimal](../data-types/decimal.md) 代替。
...
- 计算的结果取决于计算方法（处理器类型和计算机系统的架构）。
- 浮点计算可能会得到诸如无穷大 (`Inf`) 和“非数字” (`NaN`) 这样的数字。在处理计算结果时应考虑这一点。
...
有关 `NaN` 排序的规则，请参阅 [ORDER BY 子句](../../sql-reference/statements/select/order-by.md) 部分。
