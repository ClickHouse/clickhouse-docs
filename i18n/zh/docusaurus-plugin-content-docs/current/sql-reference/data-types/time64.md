...
**饱和**。
在拆解为组件或序列化为文本时，时刻部分会被限制在区间 [-999:59:59.xxx, 999:59:59.xxx] 内。
...
[Time64](time64.md) 值可以与 [Date](date.md) 或 [Date32](date32.md) 值相加，生成一个与 `Time64` 具有相同小数位精度的 [DateTime64](datetime64.md)：
