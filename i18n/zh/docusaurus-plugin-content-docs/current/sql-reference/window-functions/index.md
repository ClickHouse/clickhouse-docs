...
窗口函数允许您在与当前行相关的一组行上执行计算。您可以执行的某些计算类似于使用聚合函数所能做的，但是窗口函数不会将行分组到单个输出中 - 仍然返回各个行。

## 标准窗口函数 {#standard-window-functions}

ClickHouse 支持定义窗口和窗口函数的标准语法。下表指示某个特性是否当前被支持。
...
| `ROWS` 框架                                                                     | ✅                                                                                                                                                                                   |
| `RANGE` 框架                                                                    | ✅ (默认)                                                                                                                                                                      |
| `INTERVAL` 语法用于 `DateTime` `RANGE OFFSET` 框架                             | ❌ (请改用指定秒数 (`RANGE` 与任意数值类型兼容）。)                                                                                                                                 |
| `GROUPS` 框架                                                                  | ❌                                                                                                                                                                               |
| 在框架上计算聚合函数 (`sum(value) over (order by time)`)                     | ✅ (所有聚合函数均被支持)                                                                                                                                                       |
| `rank()`, `dense_rank()`, `row_number()`                                         | ✅ <br/>别名: `denseRank()`                                                                                                                                                                                   |
| `percent_rank()` | ✅  高效计算数据集中某个值在分区中的相对排名。该函数有效地替代了更冗长且计算密集的手动 SQL 计算，表达为 `ifNull((rank() OVER(PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER(PARTITION BY x) - 1, 0), 0)` <br/>别名: `percentRank()`| 
| `lag/lead(value, offset)`                                                        | ❌ <br/> 您可以使用以下的替代方法：<br/> 1) `any(value) over (.... rows between <offset> preceding and <offset> preceding)`，或者 `following` 来实现 `lead` <br/> 2) `lagInFrame/leadInFrame`，其类似，但遵循窗口框架。要获取与 `lag/lead` 相同的行为，请使用 `rows between unbounded preceding and unbounded following`                                                                 |
| ntile(buckets) | ✅ <br/>  请按照以下方式指定窗口，(partition by x order by y rows between unbounded preceding and unbounded following). |
...
为每一行计算的值如下：
- 对于第一行为 `0`，
- ${\text{metric}_i - \text{metric}_{i-1} \over \text{timestamp}_i - \text{timestamp}_{i-1}}  * \text{interval}$ 对于 $i_{th}$ 行。
...
- [`row_number()`](./row_number.md) - 在其分区内从 1 开始为当前行编号。
- [`first_value(x)`](./first_value.md) - 返回在其有序框架内评估的第一个值。
- [`last_value(x)`](./last_value.md) - 返回在其有序框架内评估的最后一个值。
- [`nth_value(x, offset)`](./nth_value.md) - 返回在其有序框架中，对第 n 行（偏移）进行评估的第一个非 NULL 值。
- [`rank()`](./rank.md) - 在其分区内为当前行排名，存在间隙。
- [`dense_rank()`](./dense_rank.md) - 在其分区内为当前行排名，无间隙。
- [`lagInFrame(x)`](./lagInFrame.md) - 返回在有序框架中，当前行之前的特定物理偏移行所评估的值。
- [`leadInFrame(x)`](./leadInFrame.md) - 返回在有序框架中，当前行之后的偏移行所评估的值。
...
### 每个部门的最大/总工资 {#maximumtotal-salary-per-department}
...
温度以秒为精度存储，但使用 `Range` 和 `ORDER BY toDate(ts)` 我们形成大小为 10 个单位的框架，因为 `toDate(ts)` 单位为天。
...
窗口函数初步支持的路线图在 [该问题中](https://github.com/ClickHouse/ClickHouse/issues/18097)。
...
- 博客: [将数据引入 ClickHouse - 第 3 部分 - 使用 S3](https://clickhouse.com/blog/getting-data-into-clickhouse-part-3-s3)
