...
返回在其有序范围内评估的最后一个值。默认情况下，NULL 参数会被跳过，但是可以使用 `RESPECT NULLS` 修饰符来覆盖此行为。
...
:::note
在 `first_value(column_name)` 之后使用可选修饰符 `RESPECT NULLS` 将确保不跳过 `NULL` 参数。
...
在此示例中，`last_value` 函数用于从一个虚构的英超联赛足球运动员薪水数据集中找出薪水最低的足球运动员。
