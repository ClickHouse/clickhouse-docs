---
description: '提供类似表的接口，用于在 Azure Blob Storage 中选取/插入文件。类似于 S3 函数。'
...
提供类似于表的接口，用于在 [Azure Blob Storage](https://azure.microsoft.com/en-us/products/storage/blobs) 中查询和插入文件。此表函数与 [s3 函数](../../sql-reference/table-functions/s3.md) 类似。
...
| `partition_strategy`                        | 可选参数。支持的值：`WILDCARD` 或 `HIVE`。`WILDCARD` 要求路径中包含 `{_partition_id}`,该占位符将被分区键替换。`HIVE` 不允许使用通配符,假定路径为表根目录,并生成 Hive 风格的分区目录,使用 Snowflake ID 作为文件名,文件格式作为扩展名。默认值为 `WILDCARD`                                                                                                                                                                                                           |
| `partition_columns_in_data_file`            | 可选参数。仅与 `HIVE` 分区策略一起使用。告知 ClickHouse 是否期望分区列写入数据文件中。默认值为 `false`。                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `extra_credentials`                         | 使用 `client_id` 和 `tenant_id` 进行身份验证。如果提供了 extra_credentials,则它们的优先级高于 `account_name` 和 `account_key`。                                                                                                                                                                                                                                                                                                                                      |
...
使用以下语句将数据写入 Azure Blob Storage:
...
这是一个提示设置，用于让 ClickHouse 在读取时解析 Hive 风格的分区文件。它对写入操作没有影响。如需对称的读写操作，请使用 `partition_strategy` 参数。

当 `use_hive_partitioning` 设置为 1 时,ClickHouse 将检测路径中的 Hive 风格分区(`/name=value/`),并允许在查询中将分区列作为虚拟列使用。这些虚拟列的名称与分区路径中的名称相同,但以 `_` 开头。
...
共享访问签名 (SAS) 是一个 URI,用于授予对 Azure 存储容器或文件的受限访问权限。使用它可以在不共享存储账户密钥的情况下,提供对存储账户资源的限时访问。更多详情请参阅[此处](https://learn.microsoft.com/en-us/rest/api/storageservices/delegate-access-with-shared-access-signature)。

`azureBlobStorage` 函数支持共享访问签名 (SAS)。

[Blob SAS 令牌](https://learn.microsoft.com/en-us/azure/ai-services/translator/document-translation/how-to-guides/create-sas-tokens?tabs=Containers)包含对请求进行身份验证所需的所有信息,包括目标 blob、权限和有效期。要构造 blob URL,请将 SAS 令牌附加到 blob 服务端点。例如,如果端点是 `https://clickhousedocstest.blob.core.windows.net/`,则请求变为:
