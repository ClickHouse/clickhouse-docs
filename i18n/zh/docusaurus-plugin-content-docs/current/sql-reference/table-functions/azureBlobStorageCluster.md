---
'description': '允许在指定的集群中并行处理来自 Azure Blob 存储的文件。'
'sidebar_label': 'Azure Blob 存储集群'
'sidebar_position': 15
...
允许在指定集群中的多个节点上并行处理来自 [Azure Blob Storage](https://azure.microsoft.com/en-us/products/storage/blobs) 的文件。在发起节点上，它创建与集群中所有节点的连接，公开 S3 文件路径中的星号，并动态分派每个文件。在工作节点上，它向发起节点询问下一个待处理的任务并进行处理。此过程重复进行，直到所有任务完成。
...
| `container_name`     | 容器名称                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
| `blobpath`           | 文件路径。支持只读模式下以下通配符：`*`，`**`，`?`，`{abc,def}` 和 `{N..M}`，其中 `N`，`M` 是数字，`'abc'`，`'def'` 是字符串。                                                                                                                                                                                                                                                                                                                                                           |
| `account_name`       | 如果使用了 storage_account_url，则可以在此指定账户名称                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `account_key`        | 如果使用了 storage_account_url，则可以在此指定账户密钥                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `format`             | 文件的 [格式](/sql-reference/formats)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `compression`        | 支持的值：`none`，`gzip/gz`，`brotli/br`，`xz/LZMA`，`zstd/zst`。默认情况下，它将通过文件扩展名自动检测压缩（同于设置为 `auto`）。                                                                                                                                                                                                                                                                                                                                                |
...
类似于 [AzureBlobStorage](/engines/table-engines/integrations/azureBlobStorage) 表引擎，用户可以使用 Azurite 模拟器进行本地 Azure 存储开发。详细信息请见 [这里](https://learn.microsoft.com/en-us/azure/storage/common/storage-use-azurite?tabs=docker-hub%2Cblob-storage)。下面假设 Azurite 在主机名 `azurite1` 可用。
...
请参见 [azureBlobStorage](/sql-reference/table-functions/azureBlobStorage#using-shared-access-signatures-sas-sas-tokens) 以获取示例。
