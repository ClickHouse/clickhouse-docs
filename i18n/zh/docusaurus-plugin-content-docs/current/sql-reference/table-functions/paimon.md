---
description: '为存储在 Amazon S3、Azure、HDFS 或本地存储中的 Apache Paimon 表提供类似表的只读接口。'
...
提供对 Amazon S3、Azure、HDFS 或本地存储的 Apache [Paimon](https://paimon.apache.org/) 表的只读表接口。
...
`format` 指定 Paimon 表中数据文件的格式。

### 返回值 {#returned-value}

返回一个具有指定结构的表,用于读取指定 Paimon 表中的数据。
