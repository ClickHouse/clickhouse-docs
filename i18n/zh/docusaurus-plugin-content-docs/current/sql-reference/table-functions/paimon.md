...
为存储在 Amazon S3、Azure、HDFS 或本地的 Apache [Paimon](https://paimon.apache.org/) 表提供类似表的只读接口。
...
对于 `paimonS3`，可以使用可选参数 `extra_credentials` 传递 `role_arn`，以便在 ClickHouse Cloud 中进行基于角色的访问。有关配置步骤，请参见 [保护 S3](/cloud/data-sources/secure-s3)。
