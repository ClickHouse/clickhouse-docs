...
`cluster_name` 可以包含宏——用 `{}` 括起的替换占位符。替换后的值取自服务器配置文件中的 [macros](../../operations/server-configuration-parameters/settings.md#macros) 部分。
...
`host`、`port`、`user`、`password`、`compression`、`secure` 等连接设置从 `<remote_servers>` 配置段中获取。详情参见 [Distributed 引擎](../../engines/table-engines/special/distributed.md)。
