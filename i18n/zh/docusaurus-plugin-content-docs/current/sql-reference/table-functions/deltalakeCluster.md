...
允许在指定集群中的多个节点上，并行处理 Amazon S3 中 [Delta Lake](https://github.com/delta-io/delta) 表的文件。在发起节点上，它会创建与集群中所有节点的连接，并动态分派每个文件。在工作节点上，它会向发起节点请求下一个待处理任务并进行处理。该过程会重复进行，直到所有任务完成为止。
