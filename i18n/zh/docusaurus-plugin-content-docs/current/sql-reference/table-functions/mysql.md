...
| `on_duplicate_clause` | 添加到 `INSERT` 查询的 `ON DUPLICATE KEY on_duplicate_clause` 表达式。仅在 `replace_query = 0` 时可以指定（如果同时传递 `replace_query = 1` 和 `on_duplicate_clause`，ClickHouse 会生成异常）。<br/>    示例：`INSERT INTO t (c1,c2) VALUES ('a', 2) ON DUPLICATE KEY UPDATE c2 = c2 + 1;`<br/>    这里的 `on_duplicate_clause` 是 `UPDATE c2 = c2 + 1`。请参阅 MySQL 文档以找出可以与 `ON DUPLICATE KEY` 子句一起使用的 `on_duplicate_clause`。 |

参数也可以通过 [命名集合](operations/named-collections.md) 传递。在这种情况下，`host` 和 `port` 应单独指定。推荐在生产环境中使用这种方法。

简单的 `WHERE` 子句，例如 `=, !=, >, >=, <, <=` 目前在 MySQL 服务器上执行。

其余条件和 `LIMIT` 取样限制只有在查询到 MySQL 完成后才在 ClickHouse 中执行。
...
:::note
某些 MySQL 数据类型可以映射到不同的 ClickHouse 类型 - 这是通过查询级别设置 [mysql_datatypes_support_level](operations/settings/settings.md#mysql_datatypes_support_level) 来解决的。
:::

:::note
在 `INSERT` 查询中，为了区分表函数 `mysql(...)` 与带列名称列表的表名，您必须使用关键字 `FUNCTION` 或 `TABLE FUNCTION`。请参见下面的示例。
:::
