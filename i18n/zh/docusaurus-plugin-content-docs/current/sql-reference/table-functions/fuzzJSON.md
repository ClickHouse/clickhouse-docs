...
| `json_str` (字符串)                | 表示 JSON 格式结构数据的源字符串。                                                  |
| `random_seed` (UInt64)             | 用于生成稳定结果的手动随机种子。                                                    |
| `reuse_output` (布尔值)            | 将一个模糊处理过程的输出作为下一个模糊器的输入。                                      |
| `malform_output` (布尔值)          | 生成一个无法解析为 JSON 对象的字符串。                                              |
| `max_output_length` (UInt64)       | 生成或变异的 JSON 字符串的最大允许长度。                                            |
...
| `max_object_size` (UInt64)         | 单个 JSON 对象在同一层级中字段的最大允许数量。                                        |
| `max_string_value_length` (UInt64) | 字符串值的最大长度。                                                                |
| `min_key_length` (UInt64)          | 最小键长度。至少应为 1。                                                            |
| `max_key_length` (UInt64)          | 最大键长度。如果指定，必须大于或等于 `min_key_length`。                               |

## 返回值 {#returned_value}

一个表对象，包含一个包含变异 JSON 字符串的单列。
