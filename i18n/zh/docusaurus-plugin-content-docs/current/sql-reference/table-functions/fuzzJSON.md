...
| `reuse_output` (布尔值)              | 将模糊处理的输出作为下一个模糊器的输入进行重用。                                           |
| `malform_output` (布尔值)            | 生成一个无法解析为 JSON 对象的字符串。                                                     |
| `max_output_length` (UInt64)         | 生成或扰动 JSON 字符串的最大允许长度。                                                    |
...
| `max_object_size` (UInt64)           | 单层 JSON 对象中字段的最大允许数量。                                                       |
| `max_string_value_length` (UInt64)   | 字符串值的最大长度。                                                                       |
| `min_key_length` (UInt64)            | 最小键长度。必须至少为 1。                                                                |
| `max_key_length` (UInt64)            | 最大键长度。如果指定，必须大于或等于 `min_key_length`。                                   |

## 返回值 {#returned_value}

一个表对象，包含一列扰动的 JSON 字符串。
