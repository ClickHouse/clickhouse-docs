...
提供类似表的接口来选择/插入 [Amazon S3](https://aws.amazon.com/s3/) 和 [Google Cloud Storage](https://cloud.google.com/storage/) 中的文件。这个表函数类似于 [hdfs 函数](../../sql-reference/table-functions/hdfs.md)，但提供 S3 特定的功能。

如果您的集群中有多个副本，可以使用 [s3Cluster 函数](../../sql-reference/table-functions/s3Cluster.md) 来并行插入。

使用 `s3 表函数` 结合 [`INSERT INTO...SELECT`](../../sql-reference/statements/insert-into#inserting-the-results-of-select) 时，数据以流式方式读取和插入。在块不断从 S3 读取并推送到目标表时，只有少量数据块驻留在内存中。
...
对于 GCS，在看到 `access_key_id` 和 `secret_access_key` 的地方替换为您的 HMAC 密钥和 HMAC 秘密。
:::
...
| 参数                                   | 描述                                                                                                                                                                                                |
|----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `url`                                  | 指向文件的存储桶 url。支持只读模式下的通配符：`*`、`**`、`?`、`{abc,def}` 和 `{N..M}`，其中 `N`、`M` 为数字，`'abc'`、`'def'` 为字符串。有关更多信息，请参见 [这里](../../engines/table-engines/integrations/s3.md#wildcards-in-path)。                                            |
| `NOSIGN`                               | 如果提供此关键字作为凭据，所有请求将不被签名。                                                                                                                                                       |
...
| `headers`                              | 参数是可选的。允许在 S3 请求中传递头信息。以 `headers(key=value)` 的格式传入，例如 `headers('x-amz-request-payer' = 'requester')`。                                                              |

:::note GCS
GCS url 的格式为此，因为 Google XML API 的端点与 JSON API 不同：
...
还可以使用 [命名集合](operations/named-collections.md) 传递参数。在这种情况下，`url`、`access_key_id`、`secret_access_key`、`format`、`structure`、`compression_method` 的工作原理相同，并且支持一些额外的参数：
...
:::note
ClickHouse 使用文件名扩展名来确定数据的格式。例如，我们可以在没有 `CSVWithNames` 的情况下运行前面的命令：
...
ClickHouse 还可以确定文件的压缩方法。例如，如果文件以 `.csv.gz` 扩展名压缩，则 ClickHouse 会自动解压缩该文件。
:::

## 用法 {#usage}

假设我们在 S3 上有几个文件，其 URIs 如下：
...
:::tip
如果您的文件列表中包含带有前导零的数字范围，请分别为每个数字使用带大括号的构造，或使用 `?`。
:::

计算文件名为 `file-000.csv`、`file-001.csv`、...、`file-999.csv` 的文件中的总行数：
...
从现有表插入数据到文件 `test-data.csv.gz` 中：
...
Glob ** 可以用于递归目录遍历。考虑以下示例，它将递归获取 `my-test-bucket-768` 目录中的所有文件：

```sql
SELECT * FROM s3('https://clickhouse-public-datasets.s3.amazonaws.com/my-test-bucket-768/**', 'CSV', 'name String, value UInt32', 'gzip');
```

以下获取自 `my-test-bucket` 目录中的任何文件夹内的所有 `test-data.csv.gz` 文件：

```sql
SELECT * FROM s3('https://clickhouse-public-datasets.s3.amazonaws.com/my-test-bucket-768/**/test-data.csv.gz', 'CSV', 'name String, value UInt32', 'gzip');
```

注意。可以在服务器配置文件中指定自定义 URL 映射。例如：
```sql
...
对于生产用例，建议使用 [命名集合](operations/named-collections.md)。这里是示例：
```sql
...
如果在插入数据到 `S3` 表时指定了 `PARTITION BY` 表达式，则会为每个分区值创建一个单独的文件。将数据拆分为单独的文件有助于提高读取操作的效率。
...
```
结果是数据被写入了三个不同存储桶中的文件：`my_bucket_1/file.csv`、`my_bucket_10/file.csv` 和 `my_bucket_20/file.csv`。

## 访问公共存储桶 {#accessing-public-buckets}

ClickHouse 尝试从许多不同类型的来源获取凭据。
有时，在访问某些公共存储桶时，可能会出现问题，导致客户端返回 `403` 错误代码。
通过使用 `NOSIGN` 关键字，可以强制客户端忽略所有凭据，而不对请求进行签名，从而避免此问题。
...
对于非公共存储桶，用户可以向函数传递 `aws_access_key_id` 和 `aws_secret_access_key`。例如：

```sql
SELECT count() FROM s3('https://datasets-documentation.s3.eu-west-3.amazonaws.com/mta/*.tsv', '<KEY>', '<SECRET>','TSVWithNames')
```

这适用于一次性访问或凭据可以轻松轮换的情况。然而，这并不建议作为重复访问或凭据敏感的长期解决方案。在这种情况下，我们建议用户依赖基于角色的访问。

ClickHouse Cloud 中 S3 的基于角色的访问文档 [在这里](/cloud/security/secure-s3#access-your-s3-bucket-with-the-clickhouseaccess-role)。
...
更多示例可以在 [此处](/cloud/security/secure-s3#access-your-s3-bucket-with-the-clickhouseaccess-role)找到。

## 处理归档 {#working-with-archives}

假设我们在 S3 上有几个归档文件，其 URIs 如下：

- 'https://s3-us-west-1.amazonaws.com/umbrella-static/top-1m-2018-01-10.csv.zip'
- 'https://s3-us-west-1.amazonaws.com/umbrella-static/top-1m-2018-01-11.csv.zip'
- 'https://s3-us-west-1.amazonaws.com/umbrella-static/top-1m-2018-01-12.csv.zip'

使用 :: 提取这些归档中的数据是可能的。通配符可以用于 URL 部分以及 :: 之后的部分（负责存档内文件的名称）。
...
TAR
7Z
虽然 ZIP 和 TAR 归档可以从任何支持的存储位置访问，但 7Z 归档只能从安装了 ClickHouse 的本地文件系统读取。
:::

## 插入数据 {#inserting-data}

请注意，行只能插入到新文件中。没有合并周期或文件拆分操作。一旦文件写入，后续插入将失败。有关更多信息，请参见 [此处](/integrations/s3#inserting-data)。

## 虚拟列 {#virtual-columns}

- `_path` — 文件的路径。类型：`LowCardinality(String)`。在归档情况下，以格式 `"{path_to_archive}::{path_to_file_inside_archive}"` 显示路径。
- `_file` — 文件的名称。类型：`LowCardinality(String)`。在归档情况下显示归档内部文件的名称。
- `_size` — 文件的字节大小。类型：`Nullable(UInt64)`。如果文件大小未知，则值为 `NULL`。在归档情况下显示归档内部文件的未压缩文件大小。
...
## 访问请求者支付的存储桶 {#accessing-requester-pays-buckets}

要访问请求者支付的存储桶，必须在任何请求中传递头 `x-amz-request-payer = requester`。这通过将参数 `headers('x-amz-request-payer' = 'requester')` 传递给 s3 函数来实现。例如：
