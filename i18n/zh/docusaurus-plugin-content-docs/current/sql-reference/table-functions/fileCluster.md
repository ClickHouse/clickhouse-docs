---
description: '允许在集群中的多个节点上并行处理与指定路径匹配的文件。发起节点会与工作节点建立连接，展开文件路径中的通配符（globs），并将读文件任务分派给各个工作节点。每个工作节点都会向发起节点请求下一个要处理的文件，如此循环，直到所有任务完成（所有文件都被读取）。'
...
| `path`               | 从 [user&#95;files&#95;path](/operations/server-configuration-parameters/settings.md#user_files_path) 开始的文件相对路径。文件路径同样支持使用 [通配符](#globs-in-path)。 |
