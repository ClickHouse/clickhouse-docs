...
| [clusterAllReplicas](/sql-reference/table-functions/cluster)                           | 允许在不创建分布式表的情况下访问集群中在 `remote_servers` 配置节中配置的所有分片。                                                                    |
| [deltaLake](/sql-reference/table-functions/deltalake)                                  | 为存储在 Amazon S3 中的 Delta Lake 表提供只读的类表接口。                                                                              |
...
| [executable](/engines/table-functions/executable)                                      | `executable` 表函数基于在脚本中定义的、向 **stdout** 输出行的用户自定义函数（UDF）的输出创建一张表。                                                      |
...
| [format](/sql-reference/table-functions/format)                                        | 根据指定的输入格式从参数中解析数据。若未指定 structure 参数，则会从数据中自动推断结构。                                                                     |
...
| [generateRandom](/sql-reference/table-functions/generate)                              | 根据指定的 schema 生成随机数据，可用生成的数据填充测试表。并非所有数据类型都受支持。                                                                        |
...
| [paimon](/sql-reference/table-functions/paimon)                                        | 为存储在 Amazon S3、Azure、HDFS 或本地存储中的 Apache Paimon 表提供只读的类表接口。                                                           |
...
| [input](/sql-reference/table-functions/input)                                          | 一种表函数，用于将按给定结构发送到服务器的数据高效转换并插入到另一种结构的表中。                                                                              |
| [jdbc](/sql-reference/table-functions/jdbc)                                            | 返回一张通过 JDBC 驱动连接的表。                                                                                                   |
| [merge](/sql-reference/table-functions/merge)                                          | 创建一个临时 Merge 表。其结构通过对底层表的列取并集，并推导出它们的公共类型来确定。                                                                         |
...
| [timeSeriesSelector](/sql-reference/table-functions/timeSeriesSelector)                | 从 TimeSeries 表中读取时间序列数据，按选择器过滤，并限定时间戳在指定时间区间内。                                                                        |
...
| [generate&#95;series (generateSeries)](/sql-reference/table-functions/generate_series) | 返回一张仅包含一个名为 `generate_series` 的列（类型为 UInt64）的表，该列包含从 start 到 stop（包含端点）的所有整数。                                         |
...
| [arrowFlight](/sql-reference/table-functions/arrowflight)                              | 允许对通过 Apache Arrow Flight 服务器暴露的数据执行查询。                                                                               |
