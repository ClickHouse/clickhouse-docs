...
| [executable](/engines/table-functions/executable)                                      | `executable` 表函数根据你在脚本中定义的、将行输出到 **stdout** 的用户自定义函数（UDF）的输出创建一张表。                                                    |
...
| [fuzzQuery](/sql-reference/table-functions/fuzzQuery)                                  | 对给定的查询字符串施加随机扰动。                                                                                                      |
...
| [input](/sql-reference/table-functions/input)                                          | 一种表函数，用于高效地将按给定结构发送到服务器的数据转换后插入到具有另一种结构的表中。                                                                           |
...
| [timeSeriesMetrics](/sql-reference/table-functions/timeSeriesMetrics)                  | timeSeriesMetrics 返回表引擎为 TimeSeries 的表 `db_name.time_series_table` 所使用的指标表。                                           |
| [timeSeriesSelector](/sql-reference/table-functions/timeSeriesSelector)                | 从 TimeSeries 表中读取时间序列数据，根据选择器进行过滤，且时间戳位于指定时间区间内。                                                                      |
...
| [generate&#95;series (generateSeries)](/sql-reference/table-functions/generate_series) | 返回一张仅包含一个名为 `generate_series` 的列（UInt64）的表，该列包含从 start 到 stop（含）之间的所有整数。                                              |
| [odbc](/sql-reference/table-functions/odbc)                                            | 返回一个通过 ODBC 建立连接的表。                                                                                                   |
...
| [arrowFlight](/sql-reference/table-functions/arrowflight)                              | 支持查询由 Apache Arrow Flight 服务器提供的数据。                                                                                   |
| [url](/sql-reference/table-functions/url)                                              | 从指定的 `URL` 按给定的 `format` 和 `structure` 创建一张表                                                                          |
