---
description: '提供一个类表接口，用于在 Google
  Cloud Storage（GCS）中执行 `SELECT` 和 `INSERT` 操作。需要具备 `Storage Object User` IAM 角色。'
keywords: ['gcs', '存储桶']
sidebar_label: 'gcs'
sidebar_position: 70
...
| 参数                           | 描述                                                                                                     |
| ---------------------------- | ------------------------------------------------------------------------------------------------------ |
| `url`                        | 文件在 存储桶 中的路径。在只读模式下支持以下通配符：`*`、`**`、`?`、`{abc,def}` 和 `{N..M}`，其中 `N`、`M` 为数字，`'abc'`、`'def'` 为字符串。 |
| `NOSIGN`                     | 如果在凭证位置提供该关键字，则所有请求都不会进行签名。                                                                            |
| `hmac_key` and `hmac_secret` | 指定用于给定 端点 的凭证密钥。可选。                                                                              |
...
:::note GCS
GCS 路径采用此格式，是因为 Google XML API 的 端点 与 JSON API 不同：
...
2. 在 存储桶 名称中使用分区 ID 会在不同的 存储桶 中生成文件：
...
因此，数据将被写入位于不同 存储桶 中的三个文件：`my_bucket_1/file.csv`、`my_bucket_10/file.csv` 和 `my_bucket_20/file.csv`。
