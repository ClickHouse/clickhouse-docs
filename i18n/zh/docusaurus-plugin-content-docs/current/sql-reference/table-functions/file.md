...
:::note
如果你的文件列表中包含带前导零的数字范围，请分别使用花括号对每一位数字进行展开，或者使用 `?`。
:::
...
递归统计目录 `big_dir/` 中任意子目录下所有名为 `file002` 的文件的总行数：
...
当 `use_hive_partitioning` 被设置为 1 时，ClickHouse 会在路径中检测 Hive 风格的分区 (`/name=value/`) ，并允许在查询中将分区列作为虚拟列使用。这些虚拟列的名称与分区路径中的名称相同。
