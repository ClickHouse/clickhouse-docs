...
sidebar_label: '概览'
slug: /chdb
description: 'chDB 是一款由 ClickHouse 提供技术支持的进程内 SQL OLAP 引擎'
...
chDB 是一个由 [ClickHouse](https://github.com/clickhouse/clickhouse) 驱动的高速进程内 SQL OLAP 引擎。
当你希望在某种编程语言环境中使用 ClickHouse 的强大能力，而无需连接到 ClickHouse 服务器时，就可以使用它。
...
- **多种数据格式** - 支持 Parquet、CSV、JSON、Arrow、ORC 等[70 多种格式](/interfaces/formats)的输入输出
- **最小化数据拷贝** - 通过 [python memoryview](https://docs.python.org/3/c-api/memoryview.html) 实现 C++ 到 Python 的高效数据传输
- **丰富的 Python 生态集成** - 原生支持 Pandas、Arrow、DB API 2.0,无缝融入现有数据科学工作流
- **零依赖** - 无需安装外部数据库
...
- 如果您使用 [Go](install/go.md)、[Rust](install/rust.md)、[NodeJS](install/nodejs.md)、[Bun](install/bun.md) 或 [C and C++](install/c.md)，请查看相应的语言页面。
- 如果您使用 Python，请参阅[入门开发者指南](getting-started.md)或 [chDB 在线课程](https://learn.clickhouse.com/user_catalog_class/show/1901178)。此外还有以下常见任务的操作指南：
...
您可以观看由 ClickHouse 原创者 Alexey Milovidov 提供的 chDB 项目简介视频：
...
<Image img={dfBench} alt="DataFrame 性能测试结果" size="md"/>
...
chDB 采用 Apache License, Version 2.0 许可证。更多信息请参阅 [LICENSE](https://github.com/chdb-io/chdb/blob/main/LICENSE.txt)。
