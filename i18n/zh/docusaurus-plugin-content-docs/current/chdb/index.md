---
title: 'chDB'
sidebar_label: '概览'
slug: /chdb
description: 'chDB 是一款基于 ClickHouse 的进程内 SQL OLAP 引擎'
keywords: ['chdb', 'embedded', 'clickhouse-lite', 'in-process', 'in process']
doc_type: 'guide'
---

import Image from '@theme/IdealImage';
import dfBench from '@site/static/images/chdb/df_bench.png';


# chDB

chDB 是一款由 [ClickHouse](https://github.com/clickhouse/clickhouse) 驱动的高速进程内 SQL OLAP 引擎。
当您希望在某种编程语言环境中直接获得 ClickHouse 的强大能力，而无需连接到 ClickHouse 服务器时，就可以使用它。



## 主要特性 {#key-features}

- **进程内 SQL OLAP 引擎** - 基于 ClickHouse 构建,无需安装 ClickHouse 服务器
- **多种数据格式** - 输入和输出支持 Parquet、CSV、JSON、Arrow、ORC 以及 [70 多种格式](/interfaces/formats)
- **最小化数据复制** - 通过 [Python memoryview](https://docs.python.org/3/c-api/memoryview.html) 实现从 C++ 到 Python 的高效数据传输
- **丰富的 Python 生态系统集成** - 原生支持 Pandas、Arrow、DB API 2.0,无缝集成到现有数据科学工作流
- **零依赖** - 无需安装外部数据库


## chDB 支持哪些编程语言？ {#what-languages-are-supported-by-chdb}

chDB 提供以下语言绑定：

- [Python](install/python.md) - [API 参考](api/python.md)
- [Go](install/go.md)
- [Rust](install/rust.md)
- [NodeJS](install/nodejs.md)
- [Bun](install/bun.md)
- [C 和 C++](install/c.md)


## 如何开始？ {#how-do-i-get-started}

- 如果您使用 [Go](install/go.md)、[Rust](install/rust.md)、[NodeJS](install/nodejs.md)、[Bun](install/bun.md) 或 [C 和 C++](install/c.md),请查看相应的语言页面。
- 如果您使用 Python,请参阅[开发者入门指南](getting-started.md)或 [chDB 在线课程](https://learn.clickhouse.com/user_catalog_class/show/1901178)。此外还有以下常见任务指南:
  - [JupySQL](guides/jupysql.md)
  - [查询 Pandas](guides/querying-pandas.md)
  - [查询 Apache Arrow](guides/querying-apache-arrow.md)
  - [查询 S3 中的数据](guides/querying-s3-bucket.md)
  - [查询 Parquet 文件](guides/querying-parquet.md)
  - [查询远程 ClickHouse](guides/query-remote-clickhouse.md)
  - [使用 clickhouse-local 数据库](guides/clickhouse-local.md)


## 介绍视频 {#an-introductory-video}

您可以观看由 ClickHouse 原始创建者 Alexey Milovidov 提供的 chDB 项目简介视频：

<div class='vimeo-container'>
  <iframe
    width='560'
    height='315'
    src='https://www.youtube.com/embed/cuf_hYn7dqU?si=SzUm7RW4Ae5-YwFo'
    title='YouTube 视频播放器'
    frameborder='0'
    allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share'
    referrerpolicy='strict-origin-when-cross-origin'
    allowfullscreen
  ></iframe>
</div>


## 性能基准测试 {#performance-benchmarks}

chDB 在不同场景下均展现出卓越的性能表现:


* **[嵌入式引擎的 ClickBench 基准测试](https://benchmark.clickhouse.com/#)** - 综合性能对比
* **[DataFrame 处理性能](https://colab.research.google.com/drive/1FogLujJ_-ds7RGurDrUnK-U0IW8a8Qd0)** - 与其他 DataFrame 库的比较分析
* **[DataFrame Benchmark](https://benchmark.clickhouse.com/#)**



<Image img={dfBench} alt='DataFrame 基准测试结果' size="md"/>



## 关于 chDB {#about-chdb}

- 在[博客](https://clickhouse.com/blog/chdb-embedded-clickhouse-rocket-engine-on-a-bicycle)上阅读 chDB 项目诞生的完整故事
- 在[博客](https://clickhouse.com/blog/welcome-chdb-to-clickhouse)上了解 chDB 及其应用场景
- 学习 [chDB 在线课程](https://learn.clickhouse.com/user_catalog_class/show/1901178)
- 使用 [codapi 示例](https://antonz.org/trying-chdb/)在浏览器中体验 chDB
- 更多示例请参见 (https://github.com/chdb-io/chdb/tree/main/examples)


## 许可证 {#license}

chDB 基于 Apache License, Version 2.0 许可证发布。详情请参阅 [LICENSE](https://github.com/chdb-io/chdb/blob/main/LICENSE.txt)。
