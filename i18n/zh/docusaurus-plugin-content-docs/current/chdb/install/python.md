...
在 macOS 和 Linux (x86_64 和 ARM64) 上使用 Python 3.8+
...
### 文件查询 (Parquet, CSV, JSON, Arrow, ORC 和 60+) {#query-on-file-parquet-csv-json-arrow-orc-and-60}
...
### 表查询 (Pandas DataFrame, Parquet 文件/字节, Arrow 字节) {#query-on-table-pandas-dataframe-parquet-filebytes-arrow-bytes}
...
会话将保留查询的状态。所有的 DDL 和 DML 状态将保存在一个目录中。目录路径可以作为参数传递。如果没有传递，将创建一个临时目录。

如果未指定路径，临时目录将在 Session 对象删除时被删除。否则，路径将保持。

请注意，默认数据库是 `_local`，默认引擎是 `Memory`，这意味着所有数据将存储在内存中。如果您想将数据存储在磁盘上，您应该创建另一个数据库。
...
### 使用 UDF (用户自定义函数) 查询 {#query-with-udf-user-defined-functions}
...
1. 函数应无状态。仅支持 UDF，不支持 UDAF (用户自定义聚合函数)。
2. 默认返回类型为字符串。如果您希望更改返回类型，可以将返回类型作为参数传递。返回类型应为 [以下之一](/sql-reference/data-types)。
3. 函数应接受字符串类型的参数。由于输入为 TabSeparated，所有参数均为字符串。
4. 函数将对每行输入进行调用。示例:
```python
def sum_udf(lhs, rhs):
    return int(lhs) + int(rhs)
...
for line in sys.stdin:
    ...
    sys.stdout.flush()
```
5. 函数应为纯 Python 函数。您应导入 **在函数内部** 使用的所有 Python 模块。
```python
def func_use_json(arg):
    import json
    ...
```
6. 使用的 Python 解释器与运行脚本时使用的是相同的。您可以从 `sys.executable` 获取它。
...
2. `read` 方法应：
    1. 返回一个列表的列表，第一个维度是列，第二个维度是行，列的顺序应与 `read` 的第一个参数 `col_names` 相同。
    1. 在没有更多数据可读时返回一个空列表。
    1. 为有状态，游标应在 `read` 方法中更新。
3. 可选的 `get_schema` 方法可用于返回表的模式。其原型为 `def get_schema(self) -> List[Tuple[str, str]]:`，返回值为一个元组列表，每个元组包含列名和列类型。列类型应为 [以下之一](/sql-reference/data-types)。
...
1. Pandas DataFrame 性能最好，Arrow 表优于 PyReader
