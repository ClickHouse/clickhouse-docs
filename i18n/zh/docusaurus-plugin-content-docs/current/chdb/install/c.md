...
sidebar_label: 'C 和 C++'
slug: /chdb/install/c
description: '如何安装并在 C 和 C++ 中使用 chDB'
...
chDB 提供原生的 C/C++ API，使您可以将 ClickHouse 的功能直接嵌入到应用程序中。该 API 既支持简单查询，也支持持久连接、流式查询结果等高级功能。
...
public:
    ChDBConnection(const std::vector<std::string>& args) {
        ...
        if (!conn) {
            throw std::runtime_error("无法连接到 chDB");
        }
    }
