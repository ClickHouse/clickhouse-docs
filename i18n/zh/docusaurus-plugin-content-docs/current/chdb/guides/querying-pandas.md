...
'sidebar_label': '使用 Pandas 进行查询'
'slug': '/chdb/guides/pandas'
'description': '了解如何使用 chDB 查询 Pandas DataFrames'
'keywords':
- 'chdb'
- 'pandas'
---
...
[Pandas](https://pandas.pydata.org/) 是一个流行的数据操作和分析库，用于 Python。 在 chDB 的 2 版本中，我们提高了查询 Pandas DataFrames 的性能，并引入了 `Python` 表函数。 在本指南中，我们将学习如何使用 `Python` 表函数查询 Pandas。
...
现在我们将安装 chDB。 确保您安装版本为 2.0.2 或更高版本：

```bash
pip install "chdb>=2.0.2"
```

现在我们要安装 Pandas 和其他一些库：

```bash
pip install pandas requests ipython
```

我们将使用 `ipython` 运行本指南后面的命令，您可以通过运行以下命令来启动它：
...
我们将从 [StatsBomb GitHub 代码库](https://github.com/statsbomb/open-data/tree/master?tab=readme-ov-file) 查询一些数据。 让我们首先导入 requests 和 pandas：
...
然后，我们将加载其中一个比赛的 JSON 文件到 DataFrame 中：
...
接下来，我们将加载其中一个事件 JSON 文件，并向该 DataFrame 中添加一个名为 `match_id` 的列：
...
我们可以通过使用 `Python` 表函数来查询 Pandas DataFrames：
...
因此，如果我们想列出 `matches_df` 中的列，我们可以写如下内容：
...
然后，我们可以通过编写以下查询来找出哪些裁判执法了超过一场比赛：
...
我们还可以在查询中连接 DataFrames。 例如，要获取比赛的概述，我们可以写下以下查询：
...
我们还可以从 DataFrames 创建和填充 ClickHouse 表。 如果我们想在 chDB 中创建一个表，我们需要使用 Stateful Session API。
