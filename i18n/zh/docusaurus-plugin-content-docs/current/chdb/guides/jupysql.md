...
[JupySQL](https://jupysql.ploomber.io/en/latest/quick-start.html) 是一个 Python 库，可以让你在 Jupyter Notebook 和 IPython Shell 中运行 SQL。
在本指南中，我们将学习如何结合使用 chDB 和 JupySQL 来查询数据。

<div class="vimeo-container">
  <iframe width="560" height="315" src="https://www.youtube.com/embed/2wjl3OijCto?si=EVf2JhjS5fe4j6Cy" title="YouTube 视频播放器" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen />
</div>
...
可以在 IPython 中使用 JupySQL,运行以下命令启动:

```bash
ipython
```

或在 Jupyter Lab 中使用,运行以下命令启动:
...
:::note
如果使用 Jupyter Lab,需要先创建一个 notebook,然后再继续本指南的其余部分。
:::


## 下载数据集 {#downloading-a-dataset}

我们将使用 [Jeff Sackmann 的 tennis_atp](https://github.com/JeffSackmann/tennis_atp) 数据集,其中包含球员及其历史排名的元数据。
首先下载排名文件:
...
数据的格式有些不规范。
让我们清理日期字段,并使用 `REPLACE` 子句返回清理后的 `ranking_date`:
...
接下来我们将创建一个名为 `rankings` 的表,其模式将根据 CSV 文件中的数据结构自动推导:
...
看起来不错——输出结果与直接查询 CSV 文件时相同,符合预期。
...
```text
+-----------+------------+-----------+------+------------+-----+--------+-------------+
| player_id | 名 | 姓 | 惯用手 |    出生日期     | 国家代码 | 身高 | wikidata_id |
+-----------+------------+-----------+------+------------+-----+--------+-------------+
...
数据摄取已完成,现在进入有趣的部分——查询数据!

网球运动员根据他们在参加的锦标赛中的表现获得积分。
每位球员的积分按 52 周滚动周期计算。
我们将编写一个查询,找出每位球员累积的最高积分以及达到该积分时的排名:
...
有趣的是,此列表中的一些球员虽然积累了大量积分,但在达到该积分总数时并未排名第一。
...
以下查询查找首次进入前10排名与最后一次进入前10排名之间间隔天数最少的球员:
...
JupySQL 还提供了有限的图表绘制功能。
我们可以创建箱线图或直方图。

我们将创建一个直方图,但首先需要编写(并保存)一个查询,用于计算每位球员在前 100 名中所达到的排名。
我们可以使用该查询创建一个直方图,统计达到各个排名的球员数量:
