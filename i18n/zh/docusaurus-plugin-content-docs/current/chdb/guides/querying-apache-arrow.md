...
[Apache Arrow](https://arrow.apache.org/) 是一种标准化的列式内存格式规范，在数据社区中越来越受欢迎。
...
:::note
如果需要下载更多文件,可以使用 `aws s3 ls` 获取所有文件列表,然后更新上述命令。
:::

接下来,从 `pyarrow` 包中导入 Parquet 模块:

```python
import pyarrow.parquet as pq
```

然后将 Parquet 文件读取到 Apache Arrow 表中:
