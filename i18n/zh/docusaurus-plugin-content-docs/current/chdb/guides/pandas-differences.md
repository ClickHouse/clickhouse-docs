...
| **行顺序** | 保留 | 保留（自动）；在[性能模式](../configuration/performance-mode.md)下不保证 |
| **inplace** | 支持 | 不支持 |
| **索引** | 完全支持 | 简化实现 |
...
- 在 **性能模式**（`config.use_performance_mode()`）下：对于任何操作，都不保证行顺序。请参见[性能模式](../configuration/performance-mode.md)。
