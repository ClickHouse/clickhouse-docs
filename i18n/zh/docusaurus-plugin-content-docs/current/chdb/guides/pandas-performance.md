...
### 1. 为高负载场景启用性能模式 \{#use-performance-mode\}
...
完整说明请参见 [Performance Mode](../configuration/performance-mode.md)。


### 2. 使用 Parquet 而非 CSV \{#use-parquet\}
...
### 4. 仅选择必要的列 \{#select-only-needed-columns\}
...
### 5. 利用 SQL 聚合函数 \{#leverage-sql-aggregations\}
...
### 8. 使用 explain() 进行查询优化 \{#use-explain\}
...
若要在聚合类工作负载上获得最大吞吐量，请使用 `config.use_performance_mode()`。
详见 [Execution Engine](../configuration/execution-engine.md) 和 [Performance Mode](../configuration/performance-mode.md)。
:::