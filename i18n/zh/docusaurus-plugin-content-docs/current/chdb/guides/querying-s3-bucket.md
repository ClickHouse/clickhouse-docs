...
让我们从列出[包含 Amazon 评论的 S3 存储桶](/getting-started/example-datasets/amazon-reviews)中的所有文件开始。
...
我们还将使用 [`One`](/interfaces/formats/One) 输入格式,使文件不被解析,而是每个文件返回单独一行,这样我们可以通过 `_file` 虚拟列访问文件名,通过 `_path` 虚拟列访问路径。
...
接下来使用 `DESCRIBE` 子句查看这些 Parquet 文件的模式结构:
