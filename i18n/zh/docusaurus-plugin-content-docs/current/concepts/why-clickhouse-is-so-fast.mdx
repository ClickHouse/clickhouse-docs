---
sidebar_position: 1
sidebar_label: '为什么 ClickHouse 如此之快？'
description: '它从设计之初就以速度为目标。在开发过程中，查询执行性能始终是重中之重，但同时也兼顾了其他重要特性，例如易用性、可扩展性和安全性，从而使 ClickHouse 成为真正可用于生产环境的系统。'
...
在 2024 年 8 月，我们的第一篇研究论文在 VLDB 上被接收并发表。\
VLDB 是一个专注于超大规模数据库的国际会议，被普遍认为是数据管理领域最重要的会议之一。\
在数百篇投稿中，VLDB 的录用率通常约为 20%。

你可以阅读[论文的 PDF](https://www.vldb.org/pvldb/vol17/p3731-schulze.pdf)，或者查看我们的[网页版本](/docs/academic_overview)，其中对 ClickHouse 中最有趣、也是让它如此高速的关键架构和系统设计组件进行了简明扼要的介绍。

我们的 CTO、ClickHouse 的创建者 Alexey Milovidov 在会上做了论文报告 (演示文稿在[这里](https://raw.githubusercontent.com/ClickHouse/clickhouse-presentations/master/2024-vldb/VLDB_2024_presentation.pdf)) ，随后进行了问答环节 (时间很快就用完了！) 。\
你可以在这里观看录制的报告：
