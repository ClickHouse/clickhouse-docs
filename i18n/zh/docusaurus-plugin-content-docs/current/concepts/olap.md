...
[OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing) 是 Online Analytical Processing（联机分析处理）的缩写。它是一个广义术语，可以从技术和业务两个视角来理解。在更高层面上，你可以把这三个单词反过来理解：

**Processing** — 对一些源数据进行处理……

**Analytical** — ……以生成一些分析报表和洞察……
...
近年来,企业决策者开始认识到数据的价值。盲目决策的公司往往难以在竞争中保持领先。成功企业采用数据驱动的方法,这要求它们收集所有可能对业务决策有用的数据,并需要具备及时分析这些数据的能力。OLAP 数据库管理系统(DBMS)正是为此而生。
...
ClickHouse 是一个 OLAP 数据库管理系统,经常被用作这些 SaaS 解决方案的后端,用于分析特定领域的数据。然而,一些企业仍然不愿意与第三方提供商共享数据,因此自建数据仓库方案也是可行的选择。
...
即使数据库管理系统最初是纯 OLAP 或纯 OLTP,也会被迫朝 HTAP 方向发展以保持竞争力。ClickHouse 也不例外。最初,它被设计为[尽可能快的 OLAP 系统](/concepts/why-clickhouse-is-so-fast),目前仍不具备完整的事务支持,但已添加了一些功能,如一致性读写以及用于更新/删除数据的变更操作。
...
- 而单独存储列会增加行操作的成本,如追加或就地修改,成本与列数成正比(如果系统试图收集事件的所有细节以备不时之需,列数可能会非常庞大)。因此,大多数 OLTP 系统按行存储数据。
