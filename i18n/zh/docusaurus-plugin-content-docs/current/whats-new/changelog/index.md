...
* 将更多过滤条件下推到 JOIN 中。[#85556](https://github.com/ClickHouse/ClickHouse/pull/85556) ([Nikita Taranov](https://github.com/nickitat))。
...
* 在 PARTITION BY 与排序键匹配或为其前缀时,提高窗口函数的性能。[#87299](https://github.com/ClickHouse/ClickHouse/pull/87299) ([Nikita Taranov](https://github.com/nickitat))。
...
* Function `isValidASCII` 已针对正面结果（即全 ASCII 输入值）进行了优化。[#93347](https://github.com/ClickHouse/ClickHouse/pull/93347)（[Robert Schulze](https://github.com/rschu1ze)）。
...
* Reduce memory usage for AST（抽象语法树）。[#93601](https://github.com/ClickHouse/ClickHouse/pull/93601) ([Nikolai Kochetov](https://github.com/KochetovNicolai))。
...
* 为可暂停的 failpoint 新增 SYSTEM NOTIFY FAILPOINT 支持；在 SYSTEM WAIT FAILPOINT fp 中支持 PAUSE/RESUME。[#92368](https://github.com/ClickHouse/ClickHouse/pull/92368)（[Shaohua Wang](https://github.com/tiandiwonder)）。
...
* 将比平均情况更慢的后台任务写入日志（`background_schedule_pool_log.duration_threshold_milliseconds=30`），以避免产生过多的任务日志。[#92965](https://github.com/ClickHouse/ClickHouse/pull/92965)（[Azat Khuzhin](https://github.com/azat)）。
...
* 恢复 `clickhouse git-import` 工具——此前它在处理体积较大或无效的提交时会出错。参见 https://presentations.clickhouse.com/2020-matemarketing/。 [#93202](https://github.com/ClickHouse/ClickHouse/pull/93202) ([Alexey Milovidov](https://github.com/alexey-milovidov))。
...
* 以一定概率随机执行缓存正确性检查。[#93439](https://github.com/ClickHouse/ClickHouse/pull/93439)（[Kseniia Sumarokova](https://github.com/kssenii)）。
...
* 当 Keeper 检测到损坏的快照或不一致的变更日志时，将抛出异常，而不是手动中止或自动清理文件。这样可以通过依赖人工干预使 Keeper 的行为更加安全。[#94168](https://github.com/ClickHouse/ClickHouse/pull/94168) ([Antonio Andelic](https://github.com/antonio2368)).
...
* 在 S3Queue 有序模式下支持更通用的分区策略。 [#94321](https://github.com/ClickHouse/ClickHouse/pull/94321) ([Bharat Nallan](https://github.com/bharatnc)).
...
* 在 S3(Azure)Queue 的有序模式下，通过基于限制的追踪来清理失败节点（此前该操作仅在无序模式下同时对失败和已处理节点执行，因此现在也会在有序模式下执行，但只针对失败节点）。 [#94412](https://github.com/ClickHouse/ClickHouse/pull/94412) ([Kseniia Sumarokova](https://github.com/kssenii))。
