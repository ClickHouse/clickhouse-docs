...
此版本包含针对先前版本 1.1.54318 的错误修复:

- 修复了复制中可能导致数据丢失的竞态条件错误。此问题影响版本 1.1.54310 和 1.1.54318。如果您在使用 Replicated 表时使用了这些版本之一,强烈建议进行更新。此问题在日志中显示为类似 `Part ... from own log does not exist.` 的警告消息。即使您在日志中没有看到这些消息,此问题仍然存在。

### ClickHouse 版本 1.1.54318,2017-11-30 {#clickhouse-release-1-1-54318-2017-11-30}

此版本包含针对先前版本 1.1.54310 的错误修复:
...
- 添加了 `trunc` 函数。改进了舍入函数(`round`、`floor`、`ceil`、`roundToExp2`)的性能并修正了它们的工作逻辑。更改了 `roundToExp2` 函数对分数和负数的处理逻辑。
- ClickHouse 可执行文件现在对 libc 版本的依赖性降低。同一个 ClickHouse 可执行文件可以在各种 Linux 系统上运行。使用编译查询时仍然存在依赖性(通过设置 `compile = 1`,默认不使用)。
- 减少了查询动态编译所需的时间。
...
- 修复了当块中的第一列为 Array 类型时 concat 函数中出现的错误。
...
- INSERT 可以在分布式表中同步执行：只有在所有数据都保存到所有分片后才返回 OK。通过设置 insert_distributed_sync=1 来启用此功能。
- 添加了 UUID 数据类型，用于处理 16 字节标识符。
- 添加了 CHAR、FLOAT 和其他类型的别名，以兼容 Tableau。
...
- 通过合并多个源插入操作，提高了从分布式表进行异步插入的性能。要启用此功能，请使用设置 distributed_directory_monitor_batch_inserts=1。
...
- 优化了从分布式表读取时的流分配。
- 如果值不变，可以在只读模式下配置设置。
- 添加了检索 MergeTree 引擎非整数粒度的能力，以满足 preferred_block_size_bytes 设置中指定的块大小限制。目的是在处理具有大列的表的查询时减少 RAM 消耗并提高缓存局部性。
- 对于 `toStartOfHour(x) op сonstexpr` 这样的条件，高效使用包含 `toStartOfHour(x)` 等表达式的索引。
- 为 MergeTree 引擎添加了新设置（config.xml 中的 merge_tree 部分）：
  - replicated_deduplication_window_seconds 设置在复制表中对插入进行去重的允许秒数。
  - cleanup_delay_period 设置启动清理以删除过期数据的频率。
  - replicated_can_become_leader 可以防止副本成为领导者（并分配合并）。
...
- system.parts 表有一个新列，包含有关标记大小的信息（以字节为单位）。

#### 错误修复： {#bug-fixes-4}


- 使用 Merge 表的分布式表现在可以正确处理在 `_table` 字段上带有条件的 SELECT 查询。
- 修复了 ReplicatedMergeTree 在检查数据部分时的罕见竞态条件。
- 修复了启动服务器时"领导者选举"可能导致的冻结问题。
- 使用数据源的本地副本时,max_replica_delay_for_distributed_queries 设置会被忽略。此问题已修复。
- 修复了尝试清理不存在的列时 `ALTER TABLE CLEAR COLUMN IN PARTITION` 的错误行为。
- 修复了使用空数组或字符串时 multiIf 函数的异常。
- 修复了反序列化 Native 格式时的过度内存分配问题。
- 修复了 Trie 字典的错误自动更新。
- 修复了使用 SAMPLE 时从 Merge 表运行带有 GROUP BY 子句的查询时的异常。
- 修复了使用 distributed_aggregation_memory_efficient=1 时 GROUP BY 的崩溃问题。
- 现在可以在 IN 和 JOIN 的右侧指定 database.table。
- 并行聚合使用了过多的线程。此问题已修复。
- 修复了 "if" 函数处理 FixedString 参数的方式。
- 对于权重为 0 的分片,从分布式表执行 SELECT 工作不正确。此问题已修复。
- 运行 `CREATE VIEW IF EXISTS` 不再导致崩溃。
- 修复了设置 input_format_skip_unknown_fields=1 且存在负数时的错误行为。
- 修复了当字典中存在无效数据时 `dictGetHierarchy()` 函数的无限循环问题。
- 修复了在 IN 或 JOIN 子句中使用子查询和 Merge 表运行分布式查询时的 `Syntax error: unexpected (...)` 错误。
- 修复了对 Dictionary 表的 SELECT 查询的错误解释。
- 修复了在 IN 和 JOIN 子句中使用超过 20 亿个元素的数组时的 "Cannot mremap" 错误。
- 修复了以 MySQL 作为源的字典的故障转移。

#### 改进的 ClickHouse 开发和构建工作流: {#improved-workflow-for-developing-and-assembling-clickhouse-1}
...
- Dictionary 数据库引擎(此类型的数据库自动为所有连接的外部字典提供 Dictionary 表)。
- 可以通过向源发送请求来检查字典的更新。
- 限定列名
- 使用双引号引用标识符。
- HTTP 接口中的会话。
...
- 现在触发警报后,日志会打印完整的堆栈跟踪。
- 放宽了启动时对损坏/额外数据部分数量的验证(存在过多误报)。

#### 错误修复: {#bug-fixes-5}

- 修复了插入分布式表时连接"粘滞"的问题。
- GLOBAL IN 现在适用于从 Merge 表查询分布式表的情况。
- 在 Google Compute Engine 虚拟机上检测到的核心数不正确。此问题已修复。
- 更改了缓存外部字典的可执行源的工作方式。
- 修复了包含空字符的字符串比较。
- 修复了 Float32 主键字段与常量的比较。
- 以前,对字段大小的错误估计可能导致过大的分配。
- 修复了查询使用 ALTER 添加到表中的 Nullable 列时的崩溃问题。
- 修复了当行数少于 LIMIT 时按 Nullable 列排序的崩溃问题。
