...
- 新增支持加载 [CatBoost](https://yandex.com/dev/catboost/) 模型并将其应用于存储在 ClickHouse 中的数据。
- 新增支持与 UTC 偏移量为非整数的时区。
- 新增支持时间间隔的算术运算。
- Date 和 DateTime 类型的值范围扩展至 2105 年。
- 新增 `CREATE MATERIALIZED VIEW x TO y` 查询（指定用于存储物化视图数据的现有表）。
...
- 在创建表时，新增检查验证采样键表达式是否包含在主键中。

#### 错误修复： {#bug-fixes}

- 修复了同步插入 Distributed 表时的挂起问题。
- 修复了 Replicated 表中非原子性添加和删除数据部分的问题。
- 插入物化视图的数据不再进行不必要的去重。
...
- 原生协议支持 TLS（要启用，请在 `config.xml` 中设置 `tcp_ssl_port`）。
...
- ClickHouse 可执行文件现在对 libc 版本的依赖性降低。同一个 ClickHouse 可执行文件可以在各种 Linux 系统上运行。使用编译查询时仍然存在依赖性(使用设置 `compile = 1`,默认不使用)。
- 减少了查询动态编译所需的时间。
...
- 可以在 Distributed 表中同步执行 INSERT 操作：只有在所有数据都保存到所有分片后才会返回 OK。通过设置 insert_distributed_sync=1 来启用此功能。
...
- 为 MergeTree 引擎添加了新设置（config.xml 中的 merge_tree 部分）：
  - replicated_deduplication_window_seconds 设置在 Replicated 表中对插入进行去重的允许秒数。
  - cleanup_delay_period 设置启动清理以删除过期数据的频率。
  - replicated_can_become_leader 可以防止副本成为 leader（并分配合并操作）。
- 加速了从 ZooKeeper 中删除过期数据的清理过程。
...
- 使用 Merge 表的分布式表现在可以正确处理带有 `_table` 字段条件的 SELECT 查询。
- 修复了 ReplicatedMergeTree 在检查数据分片时的罕见竞态条件。
- 修复了服务器启动时"领导者选举"可能导致的冻结问题。
- 修复了使用数据源的本地副本时 max_replica_delay_for_distributed_queries 设置被忽略的问题。
- 修复了尝试清理不存在的列时 `ALTER TABLE CLEAR COLUMN IN PARTITION` 的错误行为。
- 修复了使用空数组或字符串时 multiIf 函数的异常。
- 修复了反序列化 Native 格式时的过度内存分配问题。
- 修复了 Trie 字典的错误自动更新问题。
...
- 放宽了启动时对损坏/额外数据分片数量的验证(存在过多误报)。

#### 错误修复: {#bug-fixes-5}

- 修复了向分布式表插入数据时连接"粘滞"的问题。
- GLOBAL IN 现在可用于从 Merge 表查询分布式表。
- 修复了在 Google Compute Engine 虚拟机上检测到的核心数不正确的问题。
- 更改了缓存外部字典的可执行源的工作方式。
- 修复了包含空字符的字符串比较问题。
- 修复了 Float32 主键字段与常量的比较问题。
- 以前,对字段大小的错误估计可能导致过大的内存分配。
- 修复了查询使用 ALTER 添加到表中的 Nullable 列时的崩溃问题。
- 修复了当行数少于 LIMIT 时按 Nullable 列排序的崩溃问题。
- 修复了仅由常量值组成的 ORDER BY 子查询问题。
