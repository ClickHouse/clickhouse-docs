...
- 现在可以使用参数配置 `uniqCombined` 聚合函数的精度(选择 HyperLogLog 单元格的数量)。[#3406](https://github.com/ClickHouse/ClickHouse/pull/3406)
...
* 对 `LowCardinality` 数据类型的修复和性能优化。使用 `LowCardinality(Nullable(...))` 的 `GROUP BY`。获取 `extremes` 的值。处理高阶函数。`LEFT ARRAY JOIN`。分布式 `GROUP BY`。返回 `Array` 的函数。`ORDER BY` 的执行。向 `Distributed` 表写入（nicelulu）。针对实现 `Native` 协议的旧客户端所执行的 `INSERT` 查询提供向后兼容性。为 `JOIN` 提供 `LowCardinality` 支持。单流模式下的性能提升。[#3823](https://github.com/ClickHouse/ClickHouse/pull/3823) [#3803](https://github.com/ClickHouse/ClickHouse/pull/3803) [#3799](https://github.com/ClickHouse/ClickHouse/pull/3799) [#3769](https://github.com/ClickHouse/ClickHouse/pull/3769) [#3744](https://github.com/ClickHouse/ClickHouse/pull/3744) [#3681](https://github.com/ClickHouse/ClickHouse/pull/3681) [#3651](https://github.com/ClickHouse/ClickHouse/pull/3651) [#3649](https://github.com/ClickHouse/ClickHouse/pull/3649) [#3641](https://github.com/ClickHouse/ClickHouse/pull/3641) [#3632](https://github.com/ClickHouse/ClickHouse/pull/3632) [#3568](https://github.com/ClickHouse/ClickHouse/pull/3568) [#3523](https://github.com/ClickHouse/ClickHouse/pull/3523) [#3518](https://github.com/ClickHouse/ClickHouse/pull/3518)
* 修复了 `select_sequential_consistency` 选项的行为。此前，当启用此设置并开始向新分区写入数据时，有时会返回不完整的结果。[#2863](https://github.com/ClickHouse/ClickHouse/pull/2863)
...
* 在 VIEW 中的子查询已经能够正确指定数据库。[#3521](https://github.com/ClickHouse/ClickHouse/pull/3521)
* 修复了在 `VersionedCollapsingMergeTree` 中使用 `FINAL` 时 `PREWHERE` 的一个错误。[7167bfd7](https://github.com/ClickHouse/ClickHouse/commit/7167bfd7b365538f7a91c4307ad77e552ab4e8c1)
* 现在可以使用 `KILL QUERY` 来取消那些尚未开始执行、正在等待表被锁定的查询。[#3517](https://github.com/ClickHouse/ClickHouse/pull/3517)
* 修正了在午夜将时钟向后拨时的日期和时间计算问题（这种情况会在伊朗发生，也曾在 1981 到 1983 年间发生在莫斯科）。此前，这会导致时间被错误地重置到比实际需要早一天，并且还会在文本格式中导致日期和时间格式化错误。[#3819](https://github.com/ClickHouse/ClickHouse/pull/3819)
* 修复了在某些省略数据库名的 `VIEW` 和子查询场景中的错误。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3521)
* 修复了在同时从 `MATERIALIZED VIEW` 读取并删除该 `MATERIALIZED VIEW` 时，由于未对其内部 `MATERIALIZED VIEW` 加锁而导致的竞争条件。 [#3404](https://github.com/ClickHouse/ClickHouse/pull/3404) [#3694](https://github.com/ClickHouse/ClickHouse/pull/3694)
* 修复了错误 `Lock handler cannot be nullptr.`。[#3689](https://github.com/ClickHouse/ClickHouse/pull/3689)
* 修复了在启用 `compile_expressions` 选项时的查询处理问题（该选项默认启用）。像 `now` 函数这样的非确定性常量表达式不再被展开。 [#3457](https://github.com/ClickHouse/ClickHouse/pull/3457)
* 修复了在 `toDecimal32/64/128` 函数中为 scale 参数指定非常量值时触发的崩溃问题。
* 修复了在 `input_format_values_interpret_expressions` = 1 的情况下，尝试以 `Values` 格式将包含 `NULL` 元素的数组插入到类型为 `Array` 但未使用 `Nullable` 的列时出现的错误。[#3487](https://github.com/ClickHouse/ClickHouse/pull/3487) [#3503](https://github.com/ClickHouse/ClickHouse/pull/3503)
* 修复了当 ZooKeeper 不可用时 `DDLWorker` 中持续记录错误日志的问题。[8f50c620](https://github.com/ClickHouse/ClickHouse/commit/8f50c620334988b28018213ec0092fe6423847e2)
* 修正了 `quantile*` 函数在参数类型为 `Date` 和 `DateTime` 时的返回类型。 [#3580](https://github.com/ClickHouse/ClickHouse/pull/3580)
* 修复了当 `WITH` 子句仅指定一个不包含表达式的简单别名时的处理。 [#3570](https://github.com/ClickHouse/ClickHouse/pull/3570)
* 在启用 `enable_optimize_predicate_expression` 时，修复了包含命名子查询和限定列名的查询的处理。 [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3588)
...
* 修复了在向 `arrayReverse` 函数传递某些不正确参数时可能发生的崩溃问题。[73e3a7b6](https://github.com/ClickHouse/ClickHouse/commit/73e3a7b662161d6005e7727d8a711b930386b871)
* 修复了 `extractURLParameter` 函数中的缓冲区溢出问题。提升了性能。为包含空字节的字符串添加了正确的处理。[141e9799](https://github.com/ClickHouse/ClickHouse/commit/141e9799e49201d84ea8e951d1bed4fb6d3dacb5)
...
* 修复了在删除 `MergeTree` 表时出现的罕见竞态条件。 [#3680](https://github.com/ClickHouse/ClickHouse/pull/3680)
* 修复了在从 `Buffer` 表读取的同时对目标表执行 `ALTER` 或 `DROP` 时出现的竞态条件。 [#3719](https://github.com/ClickHouse/ClickHouse/pull/3719)
* 修复了在超出 `max_temporary_non_const_columns` 限制时出现的段错误（segfault）。 [#3788](https://github.com/ClickHouse/ClickHouse/pull/3788)

#### 改进: {#improvements-1}


* 服务器不会将处理后的配置文件写入 `/etc/clickhouse-server/` 目录，而是将它们保存在 `path` 路径下的 `preprocessed_configs` 目录中。这样一来，`/etc/clickhouse-server/` 目录对 `clickhouse` 用户没有写入权限，从而提升了安全性。[#2443](https://github.com/ClickHouse/ClickHouse/pull/2443)
* `min_merge_bytes_to_use_direct_io` 选项默认设置为 10 GiB。对于来自 MergeTree 系列表的合并操作，如果会生成较大的数据部分，将以 `O_DIRECT` 模式执行，从而避免过度逐出页面缓存。[#3504](https://github.com/ClickHouse/ClickHouse/pull/3504)
* 在存在大量表的情况下，加快服务器启动速度。[#3398](https://github.com/ClickHouse/ClickHouse/pull/3398)
* 为副本间的连接添加了连接池和 HTTP `Keep-Alive`。[#3594](https://github.com/ClickHouse/ClickHouse/pull/3594)
* 如果查询语法无效，则通过 `HTTP` 接口返回 `400 Bad Request` 状态码（之前返回的是 500）。[31bc680a](https://github.com/ClickHouse/ClickHouse/commit/31bc680ac5f4bb1d0360a8ba4696fa84bb47d6ab)
* 出于兼容性考虑，`join_default_strictness` 选项的默认值为 `ALL`。[120e2cbe](https://github.com/ClickHouse/ClickHouse/commit/120e2cbe2ff4fbad626c28042d9b28781c805afe)
* 移除了 `re2` 库中针对无效或复杂正则表达式向 `stderr` 输出日志的行为。[#3723](https://github.com/ClickHouse/ClickHouse/pull/3723)
* 为 `Kafka` 表引擎新增功能：在开始从 Kafka 读取之前检查订阅；以及该表的 `kafka_max_block_size` 设置。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3396)
* `cityHash64`、`farmHash64`、`metroHash64`、`sipHash64`、`halfMD5`、`murmurHash2_32`、`murmurHash2_64`、`murmurHash3_32` 和 `murmurHash3_64` 函数现在可以接受任意数量的参数，以及以元组形式传入的参数。[#3451](https://github.com/ClickHouse/ClickHouse/pull/3451) [#3519](https://github.com/ClickHouse/ClickHouse/pull/3519)
...
* 对于 `FULL` 和 `RIGHT JOIN`，`max_block_size` 设置用于控制来自右表的未关联数据流。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3699)
...
* 当 `Buffer` 类型表的结构与目标表结构不匹配时执行类型转换。[Vitaly Baranov](https://github.com/ClickHouse/ClickHouse/pull/3603)
* 添加了 `tcp_keep_alive_timeout` 选项，用于在连接空闲达到指定时间间隔后启用 keep-alive 数据包。[#3441](https://github.com/ClickHouse/ClickHouse/pull/3441)
* 如果分区键仅包含单列，则不再在 `system.parts` 表中对其值使用不必要的引号。 [#3652](https://github.com/ClickHouse/ClickHouse/pull/3652)
* `modulo` 函数适用于 `Date` 和 `DateTime` 数据类型。[#3385](https://github.com/ClickHouse/ClickHouse/pull/3385)
* 为函数 `POWER`、`LN`、`LCASE`、`UCASE`、`REPLACE`、`LOCATE`、`SUBSTR` 和 `MID` 新增了同义函数名。 [#3774](https://github.com/ClickHouse/ClickHouse/pull/3774) [#3763](https://github.com/ClickHouse/ClickHouse/pull/3763) 为兼容 SQL 标准，某些函数名不区分大小写。新增了语法糖 `SUBSTRING(expr FROM start FOR length)` 以兼容 SQL。 [#3804](https://github.com/ClickHouse/ClickHouse/pull/3804)
* 新增了对 `clickhouse-server` 可执行代码对应的内存页进行 `mlock` 的功能，以防止其被强制从内存中移出。此功能默认禁用。 [#3553](https://github.com/ClickHouse/ClickHouse/pull/3553)
* 在启用 `min_bytes_to_use_direct_io` 选项时，从 `O_DIRECT` 读取时的性能得到提升。[#3405](https://github.com/ClickHouse/ClickHouse/pull/3405)
* 在 `dictGet...OrDefault` 函数中，当键参数为常量且默认值参数为非常量时，提升了性能。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3563)
...
* 通过在 `/etc/default/clickhouse` 中定义 `CLICKHOUSE_PROGRAM_ENV`，可以在使用 `SYS-V init.d` 脚本启动 `clickhouse-server` 时指定自定义环境变量。
  [Pavlo Bashynskyi](https://github.com/ClickHouse/ClickHouse/pull/3612)
* 修正 clickhouse-server init 脚本的返回代码。 [#3516](https://github.com/ClickHouse/ClickHouse/pull/3516)
* `system.metrics` 表现在包含 `VersionInteger` 指标，`system.build_options` 则新增了 `VERSION_INTEGER` 这一行，其中存放 ClickHouse 版本的数值形式，例如 `18016000`。[#3644](https://github.com/ClickHouse/ClickHouse/pull/3644)
* 移除了将 `Date` 类型与数字进行比较的能力，以避免出现诸如 `date = 2018-12-17` 这类潜在错误，即错误地省略了日期外侧的引号。[#3687](https://github.com/ClickHouse/ClickHouse/pull/3687)
* 修复了诸如 `rowNumberInAllBlocks` 等有状态函数的行为。此前由于在查询分析阶段就已开始执行，它们的输出结果会大 1。 [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3729)
...
- 修复了使用较大的 `max_query_size` 设置值时过度分配内存的问题(之前会一次性预分配 `max_query_size` 字节的内存块)。[#3720](https://github.com/ClickHouse/ClickHouse/pull/3720)

#### 构建变更: {#build-changes}

- 修复了使用操作系统软件包中的 LLVM/Clang 7 版本库进行构建的问题(这些库用于运行时查询编译)。[#3582](https://github.com/ClickHouse/ClickHouse/pull/3582)

### ClickHouse 版本 18.14.17, 2018-11-30 {#clickhouse-release-18-14-17-2018-11-30}

#### 错误修复: {#bug-fixes-4}

- 修复了 ODBC 桥接进程未随主服务器进程终止的情况。[#3642](https://github.com/ClickHouse/ClickHouse/pull/3642)
- 修复了当列列表与远程表的列列表不同时,向 `Distributed` 表进行同步插入的问题。[#3673](https://github.com/ClickHouse/ClickHouse/pull/3673)
- 修复了删除 MergeTree 表时可能导致崩溃的罕见竞态条件。[#3643](https://github.com/ClickHouse/ClickHouse/pull/3643)
- 修复了当查询线程创建失败并出现 `Resource temporarily unavailable` 错误时发生的查询死锁。[#3643](https://github.com/ClickHouse/ClickHouse/pull/3643)
- 修复了使用 `CREATE AS table` 语法且 `ENGINE` 子句在 `AS table` 之前指定时对 `ENGINE` 子句的解析问题(该错误导致忽略指定的引擎)。[#3692](https://github.com/ClickHouse/ClickHouse/pull/3692)

### ClickHouse 版本 18.14.15, 2018-11-21 {#clickhouse-release-18-14-15-2018-11-21}

#### 错误修复: {#bug-fixes-5}

- 修复了在反序列化 `Array(String)` 类型列时高估内存块大小导致"超出内存限制"错误的问题。该问题出现在 18.12.13 版本中。[#3589](https://github.com/ClickHouse/ClickHouse/issues/3589)
...
- 修复了在 AMD EPYC 处理器上执行时,对 UInt16 或 Date 类型列使用 `GROUP BY` 的查询性能回退问题。[Igor Lapko](https://github.com/ClickHouse/ClickHouse/pull/3512)
- 修复了处理长字符串的查询性能回退问题。[#3530](https://github.com/ClickHouse/ClickHouse/pull/3530)
...
* 显著降低了包含 `ORDER BY` 和 `LIMIT` 的查询的内存消耗。请参阅 `max_bytes_before_remerge_sort` 设置。 [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* 在未显式指定 `JOIN` 类型（如 `LEFT`、`INNER` 等）时，将默认使用 `INNER JOIN`。 [#3147](https://github.com/ClickHouse/ClickHouse/pull/3147)
* 在包含 `JOIN` 的查询中，限定的星号现在能正常工作。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3202)
* `ODBC` 表引擎现在能够根据远程数据库的 SQL 方言正确选择标识符的引用方式。[Alexandr Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/3210)
* 默认启用 `compile_expressions` 设置（表达式的 JIT 编译）。
* 修复了同时执行 `DROP DATABASE/TABLE IF EXISTS` 和 `CREATE DATABASE/TABLE IF NOT EXISTS` 时的行为。此前，`CREATE DATABASE ... IF NOT EXISTS` 查询可能会返回错误消息 &quot;File ... already exists&quot;，而 `CREATE TABLE ... IF NOT EXISTS` 和 `DROP TABLE IF EXISTS` 查询可能会返回 `Table ... is creating or attaching right now`。 [#3101](https://github.com/ClickHouse/ClickHouse/pull/3101)
* 在从 MySQL 或 ODBC 表查询时，右侧为常量的 `LIKE` 和 `IN` 表达式会被下推到远程服务器。 [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* 在从 MySQL 和 ODBC 表中查询时，`WHERE` 子句中与常量表达式的比较会被下推到远程服务器。此前，只有与常量的比较会被下推。[#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* 在终端中对 `Pretty` 格式进行正确的行宽计算，包括含有表意文字的字符串。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3257)。
* 可以在 `ALTER UPDATE` 查询中指定 `ON CLUSTER`。
* 改进了以 `JSONEachRow` 格式读取数据时的性能。[#3332](https://github.com/ClickHouse/ClickHouse/pull/3332)
* 为兼容性新增了 `LENGTH` 和 `CHARACTER_LENGTH` 函数的别名。`CONCAT` 函数不再区分大小写。[#3306](https://github.com/ClickHouse/ClickHouse/pull/3306)
* 为 `DateTime` 类型新增了别名 `TIMESTAMP`。 [#3390](https://github.com/ClickHouse/ClickHouse/pull/3390)
* 在服务器日志中始终会为 query&#95;id 预留位置，即使该日志行与某个查询无关也是如此。这样可以更方便地使用第三方工具解析服务器文本日志。
* 当查询的内存消耗超过下一个整数 GB 阈值时，会被记录到日志中。 [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* 为以下情况添加了兼容模式：使用 `Native` 协议的客户端库在执行 `INSERT` 查询时，由于错误而发送的列数少于服务器预期的列数。使用 `clickhouse-cpp` 库时可能会出现这种情况。此前，该场景会导致服务器崩溃。[#3171](https://github.com/ClickHouse/ClickHouse/pull/3171)
* 在 `clickhouse-copier` 的用户自定义 WHERE 表达式中，现在可以使用 `partition_key` 别名（用于按源表分区进行额外过滤）。当在复制过程中分区方案发生变化但仅作了小幅调整时，这一功能非常有用。[#3166](https://github.com/ClickHouse/ClickHouse/pull/3166)
* 为了在高负载下自动降低数据读取速度，已将 `Kafka` 引擎的工作流程移至后台线程池。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215)。
* 支持在 `Cap'n'Proto format` 中读取类似 `struct` 这样的结构里的 `Tuple` 和 `Nested` 值。 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3216)
...
* 为 `Decimal` 类型新增对 `intDiv` 和 `intDivOrZero` 函数的支持。[b48402e8](https://github.com/ClickHouse/ClickHouse/commit/b48402e8712e2b9b151e0eef8193811d433a1264)
* 为 `sumMap` 聚合函数的键新增对 `Date`、`DateTime`、`UUID` 和 `Decimal` 类型的支持。 [#3281](https://github.com/ClickHouse/ClickHouse/pull/3281)
* 为外部字典提供对 `Decimal` 数据类型的支持。 [#3324](https://github.com/ClickHouse/ClickHouse/pull/3324)
* 在 `SummingMergeTree` 表中添加对 `Decimal` 数据类型的支持。[#3348](https://github.com/ClickHouse/ClickHouse/pull/3348)
* 在 `if` 中为 `UUID` 增加了专门实现。 [#3366](https://github.com/ClickHouse/ClickHouse/pull/3366)
* 减少了从 `MergeTree` 表读取数据时进行的 `open` 和 `close` 系统调用次数。[#3283](https://github.com/ClickHouse/ClickHouse/pull/3283)
* 可以在任意副本上执行 `TRUNCATE TABLE` 查询（该查询会被传递到主副本）。[Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/3375)

#### 错误修复：{#bug-fixes-10}


* 修复了 `range_hashed` 字典的 `Dictionary` 表相关问题。该错误出现在 18.12.17 版本中。[#1702](https://github.com/ClickHouse/ClickHouse/pull/1702)
* 修复了在加载 `range_hashed` 字典时出现的错误（错误消息为 `Unsupported type Nullable (...)`）。此错误出现在 18.12.17 版本中。[#3362](https://github.com/ClickHouse/ClickHouse/pull/3362)
* 修复了 `pointInPolygon` 函数中的错误，这些错误是由于在多边形包含大量彼此非常接近的顶点时，计算误差累积所导致的。 [#3331](https://github.com/ClickHouse/ClickHouse/pull/3331) [#3341](https://github.com/ClickHouse/ClickHouse/pull/3341)
* 如果在合并数据分片后，所得分片的校验和与另一副本上执行相同合并操作得到的结果不同，则会删除该合并结果，并从另一副本下载数据分片（这是预期行为）。但是在下载数据分片后，由于出现“分片已存在”的错误，无法将其加入工作集（因为该数据分片是在合并后延迟一段时间才被删除）。这会导致反复循环地尝试下载同一份数据。 [#3194](https://github.com/ClickHouse/ClickHouse/pull/3194)
* 修复了查询总内存消耗计算不正确的问题（由于计算错误，`max_memory_usage_for_all_queries` 设置无法正常生效，且 `MemoryTracking` 指标的数值也不正确）。该错误出现在 18.12.13 版本中。[Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3344)
* 修复了 `CREATE TABLE ... ON CLUSTER ... AS SELECT ...` 的功能。该错误出现在 18.12.13 版本中。[#3247](https://github.com/ClickHouse/ClickHouse/pull/3247)
* 修复了这样的问题：当 `JOIN` 仅在远程服务器上执行时，发起查询的服务器上仍会不必要地准备用于 `JOIN` 的数据结构。 [#3340](https://github.com/ClickHouse/ClickHouse/pull/3340)
* 修复了 `Kafka` 引擎中的错误：在开始读取数据时发生异常后可能出现死锁，以及在读取完成时出现锁竞争问题 [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215)。
* 对于 `Kafka` 表，未传递可选的 `schema` 参数（即 `Cap'n'Proto` 格式的模式）。[Vojtech Splichal](https://github.com/ClickHouse/ClickHouse/pull/3150)
* 如果 ZooKeeper 服务器集群中存在某些服务器，它们接受连接但在完成握手响应前立即关闭连接，ClickHouse 会选择连接另一台服务器。此前，这种情况会产生错误 `Cannot read all data. Bytes read: 0. Bytes expected: 4.` 并导致服务器无法启动。[8218cf3a](https://github.com/ClickHouse/ClickHouse/commit/8218cf3a5f39a43401953769d6d12a0bb8d29da9)
* 如果 ZooKeeper 服务器集群中包含在 DNS 查询时返回错误的服务器，这些服务器将被忽略。[17b8e209](https://github.com/ClickHouse/ClickHouse/commit/17b8e209221061325ad7ba0539f03c6e65f87f29)
* 修复了在使用 `VALUES` 格式插入数据时（当 `input_format_values_interpret_expressions = 1` 时）`Date` 与 `DateTime` 之间的类型转换问题。此前，转换是在 Unix 纪元时间中的天数数值与 Unix 时间戳之间进行的，这会导致意外结果。[#3229](https://github.com/ClickHouse/ClickHouse/pull/3229)
...
* 修复了在使用非默认 CSV 分隔符（例如 `;`）且包含浮点数时出现的 CSV 格式解析错误 [#3155](https://github.com/ClickHouse/ClickHouse/pull/3155)
...
* 修复了带有 `StateArray` 组合器的聚合函数的工作行为。[#3188](https://github.com/ClickHouse/ClickHouse/pull/3188)
* 修复了在将 `Decimal` 值除以零时触发的崩溃。 [69dd6609](https://github.com/ClickHouse/ClickHouse/commit/69dd6609193beb4e7acd3e6ad216eca0ccfb8179)
* 修复了在使用 `Decimal` 与整数参数进行运算时的类型输出结果。 [#3224](https://github.com/ClickHouse/ClickHouse/pull/3224)
* 修复了在对 `Decimal128` 执行 `GROUP BY` 时出现的段错误。[3359ba06](https://github.com/ClickHouse/ClickHouse/commit/3359ba06c39fcd05bfdb87d6c64154819621e13a)
* `log_query_threads` 设置（记录查询执行中每个线程的信息）现在只有在 `log_queries` 选项（记录查询信息）被设置为 1 时才会生效。由于 `log_query_threads` 选项默认启用，此前即使禁用了查询日志，线程相关信息仍然会被记录。[#3241](https://github.com/ClickHouse/ClickHouse/pull/3241)
* 修复了 `quantiles` 聚合函数在分布式执行时的一个错误（错误信息：`Not found column quantile...`）。[292a8855](https://github.com/ClickHouse/ClickHouse/commit/292a885533b8e3b41ce8993867069d14cbd5a664)
* 修复了在由 18.12.17 版本服务器和更早版本服务器共同组成的集群上工作时的兼容性问题。对于包含定长和非定长 `GROUP BY` 键的分布式查询，如果需要聚合的数据量较大，返回的数据有时不会被完全聚合（会出现两行不同数据具有相同的聚合键）。[#3254](https://github.com/ClickHouse/ClickHouse/pull/3254)
* 修复了 `clickhouse-performance-test` 中对替换项的处理方式，当查询只包含测试中声明的部分替换项时也能被正确处理。 [#3263](https://github.com/ClickHouse/ClickHouse/pull/3263)
* 修复了在同时使用 `FINAL` 和 `PREWHERE` 时出现的错误。 [#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* 修复了在对通过 `ALTER` 添加的列使用 `PREWHERE` 时会出现的错误。[#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* 针对 `DEFAULT` 和 `MATERIALIZED` 表达式中不允许使用 `arrayJoin` 的情况增加了检查。此前，在插入数据时使用 `arrayJoin` 会导致错误。[#3337](https://github.com/ClickHouse/ClickHouse/pull/3337)
* 在 `PREWHERE` 子句中新增了对缺少 `arrayJoin` 情况的检查。此前，在执行查询时可能会出现诸如 `Size ... does not match` 或 `Unknown compression method` 之类的错误信息。[#3357](https://github.com/ClickHouse/ClickHouse/pull/3357)
* 修复了在一次将由相等判断构成的 `AND` 链替换为相应 `IN` 表达式的优化之后，在极少数情况下可能发生的段错误。 [liuyimin-bytedance](https://github.com/ClickHouse/ClickHouse/pull/3339)
* 对 `clickhouse-benchmark` 进行了一些小的改动：此前客户端信息不会发送到服务器；现在在退出程序以及限制迭代次数时，已执行查询数量的计算更加精确。[#3351](https://github.com/ClickHouse/ClickHouse/pull/3351) [#3352](https://github.com/ClickHouse/ClickHouse/pull/3352)

#### 向后不兼容的变更: {#backward-incompatible-changes-1}

- 移除了 `allow_experimental_decimal_type` 选项。`Decimal` 数据类型现已可供默认使用。[#3329](https://github.com/ClickHouse/ClickHouse/pull/3329)
...
- 为 `clickhouse` 数据源实现了 `invalidate_query`(用于指定查询以检查外部字典是否需要更新)。[#3126](https://github.com/ClickHouse/ClickHouse/pull/3126)
...
- `enable_optimize_predicate_expression` 选项默认启用(这相当乐观)。如果发生与搜索列名相关的查询分析错误,请将 `enable_optimize_predicate_expression` 设置为 0。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3107)
...
- 添加了插入数据时对 `Nested` 类型字段中数组元素大小的检查。[#3118](https://github.com/ClickHouse/ClickHouse/pull/3118)
- 修复了使用 `ODBC` 数据源和 `hashed` 存储更新外部字典时的错误。该错误出现在 18.12.13 版本。
- 修复了从包含 `IN` 条件的查询创建临时表时的崩溃问题。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3098)
- 修复了聚合函数处理包含 `NULL` 元素的数组时的错误。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3097)
...
* 添加了 `DECIMAL(digits, scale)` 数据类型（`Decimal32(scale)`、`Decimal64(scale)`、`Decimal128(scale)`）。要启用该类型，请使用设置项 `allow_experimental_decimal_type`。 [#2846](https://github.com/ClickHouse/ClickHouse/pull/2846) [#2970](https://github.com/ClickHouse/ClickHouse/pull/2970) [#3008](https://github.com/ClickHouse/ClickHouse/pull/3008) [#3047](https://github.com/ClickHouse/ClickHouse/pull/3047)
* 用于 `GROUP BY` 的新 `WITH ROLLUP` 修饰符（可选语法：`GROUP BY ROLLUP(...)`）。[#2948](https://github.com/ClickHouse/ClickHouse/pull/2948)
* 在包含 JOIN 的查询中，星号会按照 SQL 标准展开为所有表的全部列。你可以在用户配置级别将 `asterisk_left_columns_only` 设置为 1 来恢复旧的行为。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2787)
* 添加了对在表函数上执行 `JOIN` 的支持。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
* 在 clickhouse-client 中按 Tab 键即可自动补全。[Sergey Shcherbin](https://github.com/ClickHouse/ClickHouse/pull/2447)
* 在 clickhouse-client 中按下 Ctrl+C 会取消已输入的查询。 [#2877](https://github.com/ClickHouse/ClickHouse/pull/2877)
* 新增了 `join_default_strictness` 设置（取值：`"`, `'any'`, `'all'`）。这样在使用 `JOIN` 时就不必显式指定 `ANY` 或 `ALL`。[#2982](https://github.com/ClickHouse/ClickHouse/pull/2982)
* 与查询处理相关的服务器日志中的每一行都会显示查询 ID。[#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* 现在可以在 clickhouse-client 中获取查询执行日志（使用 `send_logs_level` 设置）。在分布式查询处理中，日志会从所有服务器逐级传递到客户端。[#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `system.query_log` 和 `system.processes`（`SHOW PROCESSLIST`）表现在在你执行查询时会包含所有已更改设置的信息（`Settings` 数据的嵌套结构）。新增 `log_query_settings` 设置。[#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `system.query_log` 和 `system.processes` 表现在会显示参与查询执行的线程数量信息（见 `thread_numbers` 列）。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* 添加了 `ProfileEvents` 计数器，用于统计通过网络进行读写、对磁盘进行读写所耗费的时间、网络错误的次数，以及在网络带宽受限时的等待时间。[#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* 添加了包含来自 rusage 的系统指标的 `ProfileEvents` 计数器（可以使用它们获取用户态和内核态 CPU 使用情况、缺页错误以及上下文切换信息），以及 taskstats 指标（使用这些指标可以获取 I/O 等待时间、CPU 等待时间，以及读取和写入的数据量，分别统计包含和不包含页缓存的情况）。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* `ProfileEvents` 计数器既应用于全局范围，也应用于每个查询以及每个查询执行线程，从而可以对每个查询的资源消耗进行细粒度分析。 [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
...
* 现在，您可以使用加号运算符对聚合函数的状态进行相加（合并），并将聚合函数的状态与非负常数相乘。[#3062](https://github.com/ClickHouse/ClickHouse/pull/3062) [#3034](https://github.com/ClickHouse/ClickHouse/pull/3034)
* MergeTree 系列中的表现在包含虚拟列 `_partition_id`。[#3089](https://github.com/ClickHouse/ClickHouse/pull/3089)

#### 实验性功能：{#experimental-features-1}

- 新增 `LowCardinality(T)` 数据类型。该数据类型自动创建值的本地字典，允许在不解包字典的情况下处理数据。 [#2830](https://github.com/ClickHouse/ClickHouse/pull/2830)
...
* 修复了在存在弃用副本时复制日志无限累积的问题。为延迟时间较长的副本新增了高效的恢复模式。
* 当在 `GROUP BY` 中包含多个聚合字段且其中一个为字符串类型、其余为固定长度类型时，性能得到了提升。
* 在使用 `PREWHERE` 以及隐式将表达式转移到 `PREWHERE` 时，性能得到了提升。
...
* 当单个服务器上存在非常大量的表时，对 `system.tables` 和 `system.columns` 的查询性能提升，内存消耗降低。[#2953](https://github.com/ClickHouse/ClickHouse/pull/2953)
* 修复了在大量查询流导致错误时出现的性能问题（在 `perf top` 中可以看到 `_dl_addr` 函数，但服务器几乎不占用 CPU）。 [#2938](https://github.com/ClickHouse/ClickHouse/pull/2938)
* 启用 `enable_optimize_predicate_expression` 时，将条件下推到视图中。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
...
* The-Alchemist 字典支持 `UUID` 数据类型。 [#2822](https://github.com/ClickHouse/ClickHouse/pull/2822)
* `visitParamExtractRaw` 函数已能正确处理嵌套结构。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2974)
...
* 对于包含条件的 `CASE` 表达式，现在可以省略 `ELSE`，其效果等同于 `ELSE NULL`。 [#2920](https://github.com/ClickHouse/ClickHouse/pull/2920)
* 在使用 ZooKeeper 时，现在可以配置操作超时时间。[urykhy](https://github.com/ClickHouse/ClickHouse/pull/2971)
* 你可以将 `LIMIT n, m` 中的偏移量写成 `LIMIT n OFFSET m`。[#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
* 您可以使用 `SELECT TOP n` 语法来替代 `LIMIT`。 [#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
...
* `JOIN` 的 `USING` 子句中现在可以使用重复列。 [#3006](https://github.com/ClickHouse/ClickHouse/pull/3006)
* `Pretty` 格式现在对按宽度对齐列的填充做了限制。请使用 `output_format_pretty_max_column_pad_width` 设置。如果某个值本身更宽，仍会完整显示，但表格中的其他单元格不会被拉得过宽。 [#3003](https://github.com/ClickHouse/ClickHouse/pull/3003)
* `odbc` 表函数现在允许指定数据库/架构名称。[Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2885)
* 增加了对使用在 `clickhouse-client` 配置文件中指定用户名的支持。[Vladimir Kozbin](https://github.com/ClickHouse/ClickHouse/pull/2909)
...
* 在对 `ReplicatedMergeTree` 表定期运行清理线程时增加了随机化机制，以避免在存在大量 `ReplicatedMergeTree` 表时出现周期性的负载峰值。
* 支持 `ATTACH TABLE ... ON CLUSTER` 查询。[#3025](https://github.com/ClickHouse/ClickHouse/pull/3025)

#### 错误修复: {#bug-fixes-13}


* 修复了 `Dictionary` 表中的一个问题（会抛出 `Size of offsets does not match size of column` 或 `Unknown compression method` 异常）。此错误首次出现在 18.10.3 版本中。[#2913](https://github.com/ClickHouse/ClickHouse/issues/2913)
* 修复了在合并 `CollapsingMergeTree` 表时的一个错误：当某个数据分片为空（如果在合并过程中或执行 `ALTER DELETE` 时所有数据都被删除，就会形成这样的分片），并且合并使用了 `vertical` 算法时，会触发该错误。 [#3049](https://github.com/ClickHouse/ClickHouse/pull/3049)
* 修复了在对 `Memory` 表执行 `DROP` 或 `TRUNCATE` 的同时进行 `SELECT` 时出现的竞态条件，该问题可能导致服务器崩溃。该缺陷首次出现在版本 1.1.54388 中。 [#3038](https://github.com/ClickHouse/ClickHouse/pull/3038)
* 修复了在向 `Replicated` 表插入数据时，如果返回 `Session is expired` 错误可能导致数据丢失的问题（可以通过 `ReplicatedDataLoss` 指标检测到数据丢失）。该错误出现在版本 1.1.54378 中。[#2939](https://github.com/ClickHouse/ClickHouse/pull/2939) [#2949](https://github.com/ClickHouse/ClickHouse/pull/2949) [#2964](https://github.com/ClickHouse/ClickHouse/pull/2964)
* 修复了在执行 `JOIN ... ON` 时出现的段错误（segfault）。 [#3000](https://github.com/ClickHouse/ClickHouse/pull/3000)
* 修复了在 `WHERE` 表达式仅由限定列名组成（例如 `WHERE table.column`）时，搜索列名出错的问题。 [#2994](https://github.com/ClickHouse/ClickHouse/pull/2994)
* 修复了在执行分布式查询时，如果从远程服务器只请求一列且该列为带子查询的 IN 表达式时，会出现“Not found column”错误的问题。 [#3087](https://github.com/ClickHouse/ClickHouse/pull/3087)
* 修复了在分布式查询中，当一个分片为本地而另一个为非本地，并且触发将条件下推到 `PREWHERE` 的优化时会出现的 `Block structure mismatch in UNION stream: different number of columns` 错误。 [#2226](https://github.com/ClickHouse/ClickHouse/pull/2226) [#3037](https://github.com/ClickHouse/ClickHouse/pull/3037) [#3055](https://github.com/ClickHouse/ClickHouse/pull/3055) [#3065](https://github.com/ClickHouse/ClickHouse/pull/3065) [#3073](https://github.com/ClickHouse/ClickHouse/pull/3073) [#3090](https://github.com/ClickHouse/ClickHouse/pull/3090) [#3093](https://github.com/ClickHouse/ClickHouse/pull/3093)
* 修复了在某些非凸多边形场景下的 `pointInPolygon` 函数。[#2910](https://github.com/ClickHouse/ClickHouse/pull/2910)
* 修复了在比较 `nan` 与整数时产生错误结果的问题。[#3024](https://github.com/ClickHouse/ClickHouse/pull/3024)
* 修复了 `zlib-ng` 库中的一个错误，该错误在极少数情况下可能导致进程段错误。 [#2854](https://github.com/ClickHouse/ClickHouse/pull/2854)
...
* 修复了在同时创建和删除同一个 `Buffer` 或 `MergeTree` 表时可能出现的竞态条件。
* 修复了在比较由某些复杂类型（例如 `tuple`）组成的元组时可能导致段错误的问题。 [#2989](https://github.com/ClickHouse/ClickHouse/pull/2989)
...
* `enable_optimize_predicate_expression` 选项现在已能在包含 `SELECT *` 的情况中被正确支持。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2929)
* 修复了在重新初始化 ZooKeeper 会话时发生的段错误。 [#2917](https://github.com/ClickHouse/ClickHouse/pull/2917)
* 修复了与 ZooKeeper 一起工作时可能出现的阻塞问题。
* 修复了在 `SummingMergeTree` 中添加嵌套数据结构时使用的不正确代码。
* 在为聚合函数状态分配内存时，会正确考虑内存对齐，从而可以在实现聚合函数状态时使用需要对齐的操作。[chenxing-xc](https://github.com/ClickHouse/ClickHouse/pull/2808)
...
- 在现有 `murmurHash2_32` 函数的基础上,新增了 `murmurHash2_64`、`murmurHash3_32`、`murmurHash3_64` 和 `murmurHash3_128` 函数。[#2791](https://github.com/ClickHouse/ClickHouse/pull/2791)
...
- 修复了当子查询内的 `UNION ALL` 查询中某个 `SELECT` 查询包含重复列名时出现的 `Block structure mismatch in UNION stream: different number of columns` 错误。[Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2094)
- 修复了连接到 MySQL 服务器时发生异常导致的内存泄漏问题。
- 修复了查询错误时 clickhouse-client 响应代码不正确的问题。
...
- 新增了使用表函数替代表作为 `remote` 或 `cluster table function` 参数的能力 [#2708](https://github.com/ClickHouse/ClickHouse/pull/2708)。
...
- 更改了版本号编号方案。现在第一部分包含发布年份(公元纪年,莫斯科时区,减去 2000),第二部分包含主要变更的编号(大多数版本都会递增),第三部分是补丁版本。除非变更日志中另有说明,否则版本仍然保持向后兼容。
...
- 将包含数字零的字符串转换为 DateTime 不起作用。示例:`SELECT toDateTime('0')`。这也是表中 `DateTime DEFAULT '0'` 以及字典中 `<null_value>0</null_value>` 不起作用的原因。解决方案:将 `0` 替换为 `0000-00-00 00:00:00`。
...
- 改进了在可以使用表索引时使用 IN 运算符的性能,降低了内存消耗,并正确跟踪内存消耗 ([#2584](https://github.com/ClickHouse/ClickHouse/pull/2584))。
...
- 改进了在存在大量表达式时的查询分析性能 ([#2572](https://github.com/ClickHouse/ClickHouse/pull/2572))。
...
- 修复了分析带有递归别名的查询时的错误。
- 修复了从 ReplacingMergeTree 读取数据时,当 PREWHERE 条件过滤所有行时的错误 ([#2525](https://github.com/ClickHouse/ClickHouse/issues/2525))。
...
- 修复了在对表执行 `REANAME` 后无法在非主副本上执行 `OPTIMIZE` 查询的问题。
...
- 在系统表中新增了数据部分未压缩形式的大小信息。
- 为分布式表新增了服务器到服务器的加密支持(在 `<remote_servers>` 的副本配置中设置 `<secure>1</secure>`)。
- 为 `ReplicatedMergeTree` 系列新增了表级配置,以最小化存储在 ZooKeeper 中的数据量:`use_minimalistic_checksums_in_zookeeper = 1`
...
- 如果 `query_log` 表被手动删除,会动态重新创建 (Kirill Shvakov)。
- `lengthUTF8` 函数运行速度更快 (zhang2014)。
- 当分片数量非常大时,改进了 `Distributed` 表中同步插入的性能 (`insert_distributed_sync = 1`)。
...
- 构建支持使用 `ninja` 代替 `make`,并且默认使用 `ninja` 构建发布版本。
- 重命名的软件包:`clickhouse-server-base` 改为 `clickhouse-common-static`;`clickhouse-server-common` 改为 `clickhouse-server`;`clickhouse-common-dbg` 改为 `clickhouse-common-static-dbg`。要安装,请使用 `clickhouse-server clickhouse-client`。为了向后兼容,旧名称的软件包仍将在仓库中加载。
...
- 移除了向 `Distributed` 表进行同步插入时的双重缓冲,该问题可能导致连接超时。
...
- 移除了多余的错误级别日志记录 `Not found column ... in block`。
...
* 对于空集（例如 `SELECT count(*) FROM table WHERE 0`）在未使用 `GROUP BY` 的情况下进行聚合时，现在会返回一行结果，其中聚合函数的值为 null，以符合 SQL 标准。若要恢复旧行为（返回空结果），请将 `empty_result_for_aggregation_by_empty_set` 设置为 1。
* 为 `UNION ALL` 增加了类型转换。在符合 SQL 标准的前提下，`UNION ALL` 中 `SELECT` 列位置允许使用不同的别名。
* `LIMIT BY` 子句中支持任意表达式。此前，只能使用 `SELECT` 结果中的列。
* 当 `IN` 运算符应用于由主键列组成的表达式元组时，将会使用 `MergeTree` 表的索引。例如：`WHERE (UserID, EventDate) IN ((123, '2000-01-01'), ...)`（Anastasiya Tsarkova）。
* 新增 `clickhouse-copier` 工具，用于在集群之间复制和重新分片数据（测试版）。
* 新增了一组一致性哈希函数：`yandexConsistentHash`、`jumpConsistentHash`、`sumburConsistentHash`。它们可以用作分片键，从而在后续重新分片时减少网络流量。
...
* 在更新期间，可以从外部字典中部分重新加载数据（仅加载其指定字段的值相较于上一次加载更大的记录）（Arsen Hakobyan）。
* 新增了 `cluster` 表函数。示例：`cluster(cluster_name, db, table)`。如果将集群名称以标识符形式指定，则 `remote` 表函数可以将其作为第一个参数。
* `remote` 和 `cluster` 表函数可以用于 `INSERT` 查询。
* 在 `system.tables` 表中添加了 `create_table_query` 和 `engine_full` 虚拟列。`metadata_modification_time` 列是虚拟列。
* 在 `system.tables` 和 `system.databases` 表中添加了 `data_path` 和 `metadata_path` 列，并在 `system.parts` 和 `system.parts_columns` 表中添加了 `path` 列。
* 在 `system.part_log` 表中新增了有关合并的更多信息。
* 现在可以为 `system.query_log` 表使用任意分区键（Kirill Shvakov）。
* `SHOW TABLES` 查询现在也会显示临时表。已在 `system.tables` 中添加临时表以及 `is_temporary` 列（zhang2014）。
* 新增对 `DROP TEMPORARY TABLE` 和 `EXISTS TEMPORARY TABLE` 查询的支持（zhang2014）。
* 为临时表提供 `SHOW CREATE TABLE` 支持（zhang2014）。
* 新增了用于内部进程相关设置的 `system_profile` 配置参数。
* 支持在 `MongoDB` 字典中将 `object_id` 加载为属性（Pavel Litvinenko）。
* 在使用 `MongoDB` 作为源为外部字典加载数据时，将 `null` 作为默认值读取（Pavel Litvinenko）。
* 在 `Values` 格式中从不带单引号的 Unix 时间戳读取 `DateTime` 值。
* 当某些副本上不存在所请求的表时，`remote` 表函数支持故障转移。
* 运行 `clickhouse-server` 时，可以在命令行中重写配置设置。例如：`clickhouse-server -- --logger.level=information`。
* 对 `FixedString` 参数实现了 `empty` 函数：如果字符串完全由空字节组成，则该函数返回 1（zhang2014）。
* 新增 `listen_try` 配置参数，在部分地址无法监听时，不退出进程并至少成功监听一个地址（对禁用 IPv4 或 IPv6 支持的系统非常有用）。
* 新增了 `VersionedCollapsingMergeTree` 表引擎。
* `library` 字典源现在支持行和任意数值类型。
* `MergeTree` 表也可以在没有主键的情况下使用（只需指定 `ORDER BY tuple()`）。
* 如果参数不是 `NULL`，则可以将 `Nullable` 类型转换（`CAST`）为非 `Nullable` 类型。
* 可以对 `VIEW` 执行 `RENAME TABLE` 操作。
* 新增了 `throwIf` 函数。
* 新增了 `odbc_default_field_size` 选项，用于扩展从 ODBC 源加载的值的最大尺寸（默认值为 1024）。
...
- 改进了 `arrayElement` 函数的性能，适用于常量多维数组中包含空数组作为元素之一的情况。示例：`[[1], []][x]`。
- 使用包含大量替换内容的配置文件时(例如，非常大的 IP 网络列表)，服务器现在启动更快。
...
- 修复了在对 `Merge` 表的查询中通过虚拟 `_table` 列过滤表时的错误。
- 修复了在 `Distributed` 表中使用 `ALIAS` 列时的错误。
- 修复了使包含 `quantile` 系列聚合函数的查询无法进行动态编译的错误。
...
- 改进了向 Replicated 表插入数据时对重复项的处理，使其不再拖慢复制队列的执行速度。

### ClickHouse 版本 1.1.54342，2018-01-22 {#clickhouse-release-1-1-54342-2018-01-22}

此版本包含针对先前版本 1.1.54337 的错误修复：

- 修复了 1.1.54337 中的回归问题：如果默认用户具有只读访问权限，则服务器会拒绝启动并显示消息 `Cannot create database in readonly mode`。
- 修复了 1.1.54337 中的回归问题：在使用 systemd 的系统上，无论配置如何，日志始终写入 syslog；watchdog 脚本仍然使用 init.d。
...
- 新增对在表中存储多维数组和元组（`Tuple` 数据类型）的支持。
...
- 新增 `format_schema_path` 配置参数（Marek Vavruşa）。用于指定 `Cap'n Proto` 格式的 schema。schema 文件只能位于指定的目录中。
...
- 新增对 `Nullable` 参数的聚合函数的支持，即使在函数返回非 `Nullable` 结果的情况下也是如此（由 Silviu Caragea 贡献）。示例：`groupArray`、`groupUniqArray`、`topK`。
- 为 `clickhouse-client` 新增 `max_client_network_bandwidth` 参数（Kirill Shvakov）。
- 允许具有 `readonly = 2` 设置的用户操作 TEMPORARY 表（CREATE、DROP、INSERT...）（Kirill Shvakov）。
...
- 新增对从文本中读取 `Date` 类型值的支持，该格式中月份和/或日期使用单个数字而不是两个数字指定（Amos Bird）。
...
- 通过降低计算稳定性要求,改进了聚合函数 `varSamp`、`varPop`、`stddevSamp`、`stddevPop`、`covarSamp`、`covarPop`、`corr` 的性能。原有的稳定版本函数可通过 `varSampStable`、`varPopStable`、`stddevSampStable`、`stddevPopStable`、`covarSampStable`、`covarPopStable`、`corrStable` 名称继续使用。
...
- 修复了在 FreeBSD 和 OS X 下内存分配器的错误行为。
...
- 为了实现动态代码生成,`clang` 和 `lld` 被嵌入到 `clickhouse` 二进制文件中。它们也可以通过 `clickhouse clang` 和 `clickhouse lld` 调用。
...
## [2017 年更新日志](./2017.md) {#changelog-for-2017}
