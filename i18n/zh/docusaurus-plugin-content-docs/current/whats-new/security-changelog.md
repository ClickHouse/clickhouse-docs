...
sidebar_position: 20
sidebar_label: '安全变更日志'
title: '安全变更日志'
description: '详细记录安全相关更新和变更的日志'
doc_type: 'changelog'
...
当启用库桥接功能时,clickhouse-library-bridge 会在本地主机上暴露一个 HTTP API。这使得 clickhouse-server 能够从指定路径动态加载库并在隔离进程中执行。结合 ClickHouse 表引擎允许将文件上传到特定目录的功能,配置不当的服务器可能被拥有访问这两个表引擎权限的攻击者利用,使其能够在 ClickHouse 服务器上执行任意代码。
...
在启用查询缓存的情况下使用 ClickHouse 时，如果在用户角色之间切换，存在获取不准确数据的风险。ClickHouse 建议使用存在漏洞版本的用户，在其应用程序动态切换不同角色时不要使用查询缓存功能。
...
ClickHouse Cloud 使用不同的版本编号，该漏洞的修复已应用于 v23.9.2.47475 版本。
...
ClickHouse Cloud 使用不同的版本编号，该漏洞的修复已应用于 v23.9.2.47551 版本。
...
ClickHouse 服务器中发现了堆缓冲区溢出漏洞。具有向 ClickHouse 服务器加载数据权限的恶意用户可以通过插入格式错误的 CapnProto 对象导致 ClickHouse 服务器崩溃。
...
拥有 CREATE DICTIONARY 权限的攻击者可以读取允许目录外的任意文件。
...
## 在 ClickHouse 19.14.3.3 版本中修复,2019-09-10 {#fixed-in-clickhouse-release-19-14-3-3-2019-09-10}
...
## 在 ClickHouse 版本 19.13.6.1 中修复,2019-09-20 {#fixed-in-clickhouse-release-19-13-6-1-2019-09-20}
