...
description: '介绍 clickhouse-static-files-disk-uploader 实用工具'
doc_type: 'guide'
---
...
生成一个数据目录，其中包含指定 ClickHouse 表的元数据。该元数据可用于在另一台服务器上创建一个 ClickHouse 表，该表包含由 `web` 磁盘作为存储后端的只读数据集。
...
| `--output-dir [dir]`     | `non-test` 模式下文件的输出目录                                              |
...
此步骤与将数据目录输出到本地文件系统类似,但需要添加 `--test-mode` 标志。不同的是,您需要通过 `--url` 标志指定目标 URL,而不是指定输出目录。
...
获取表元数据目录后,可以使用它在其他服务器上创建 ClickHouse 表。

请参阅[此 GitHub 仓库](https://github.com/ClickHouse/web-tables-demo)中的演示示例。在该示例中,我们使用 `web` 磁盘创建表,从而可以将表附加到其他服务器上的数据集。
