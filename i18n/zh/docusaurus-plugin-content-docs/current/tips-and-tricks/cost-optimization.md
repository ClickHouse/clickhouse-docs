...
Microsoft 使用一个单独的 MySQL 数据库实现了字符串到整数的映射系统。他们不再在 ClickHouse 中存储实际的字符串，而是只存储整数 ID。当用户通过 UI 运行查询并请求 `weather_answer` 的数据时，查询优化器会首先查询 MySQL 映射表以获取对应的整数 ID，然后将查询转换为使用该整数，再发送到 ClickHouse。
