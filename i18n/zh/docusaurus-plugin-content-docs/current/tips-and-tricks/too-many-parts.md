...
_本指南是从社区交流会中总结的系列经验之一。如需了解更多实际解决方案和深入见解,可以[按具体问题浏览](./community-wisdom.md)。_
_需要更多性能优化建议?请参阅[性能优化](./performance-optimization.md)社区经验指南。_


## 理解问题 {#understanding-the-problem}

ClickHouse 会抛出"Too many parts"错误以防止严重的性能下降。小数据分片会导致多个问题:查询性能下降(因为查询期间需要读取和合并更多文件)、内存使用增加(因为每个分片都需要在内存中保存元数据)、压缩效率降低(因为较小的数据块压缩效果较差)、I/O 开销增加(因为需要更多文件句柄和寻址操作),以及后台合并变慢(给合并调度器带来更多工作)。
...
SELECT
    ...
    CASE
        ...
        WHEN count() > 100 THEN '注意 - 数据分片增多 (>100)'
        ELSE '正常 - 数据分片数量合理'
