...
_本指南汇集了来自社区交流会的实践经验。如需了解更多实际解决方案和见解,您可以[按具体问题浏览](./community-wisdom.md)。_
_物化视图使用遇到问题?请参阅[物化视图](./materialized-views.md)社区实践指南。_
_如果您的查询速度较慢且需要更多示例,我们还提供了[查询优化](/optimize/query-optimization)指南。_


## 按基数排序(从低到高) {#cardinality-ordering}

当低基数列排在前面时,ClickHouse 的主索引效果最佳,这样可以高效地跳过大块数据。键中靠后的高基数列可在这些数据块内提供细粒度排序。应从具有较少唯一值的列(如 status、category、country)开始,以具有较多唯一值的列(如 user_id、timestamp、session_id)结束。
...
在 ORDER BY 子句中使用时间戳时,需要权衡基数与精度之间的取舍。微秒精度的时间戳会产生极高的基数(几乎每行都有一个唯一值),这会降低 ClickHouse 稀疏主索引的有效性。对时间戳进行舍入会产生较低的基数,从而实现更好的索引跳过,但会损失基于时间查询的精度。
...
在调试 ClickHouse 性能时,不要依赖平均查询时间或整体系统指标。相反,应识别特定查询缓慢的原因。系统可能整体平均性能良好,但个别查询却可能因内存耗尽、过滤不当或高基数操作而性能不佳。

据 ClickHouse 首席技术官 Alexey 所述:_"正确的做法是问自己为什么这个特定查询需要五秒钟来处理……我不关心中位数和其他查询是否处理得快。我只关心我的查询"_

当查询缓慢时,不要只看平均值。应该问"为什么这个特定查询很慢?"并检查实际的资源使用模式。


## 内存与行扫描 {#memory-and-row-scanning}

Sentry 是一个以开发者为先的错误跟踪平台,每天处理来自超过 400 万开发者的数十亿个事件。他们的核心洞察:_"在这种特定场景下,分组键的基数决定了内存使用"_ - 高基数聚合会因内存耗尽而严重影响性能,而非行扫描。

当查询失败时,需要判断是内存问题(分组过多)还是扫描问题(行数过多)。

像 `GROUP BY user_id, error_message, url_path` 这样的查询会为这三个字段的每个唯一组合创建独立的内存状态。随着用户数量、错误类型和 URL 路径的增加,很容易产生数百万个必须同时保存在内存中的聚合状态。

对于极端情况,Sentry 采用确定性采样。10% 的采样率可将内存使用量减少 90%,同时对大多数聚合保持约 5% 的精度:

```sql
WHERE cityHash64(user_id) % 10 = 0  -- 始终是相同的 10% 用户
```

这确保相同的用户出现在每次查询中,在不同时间段提供一致的结果。核心原理:`cityHash64()` 对相同的输入生成一致的哈希值,因此 `user_id = 12345` 将始终哈希到相同的值,确保该用户要么始终出现在 10% 的样本中,要么永远不出现 - 在查询之间不会出现不一致。
...
您不是在内存中存储每个唯一字符串,而是将关于这些字符串的问题的答案存储为整数。无论数据多样性如何,聚合状态都变得有界且极小。

来自 Sentry 工程团队:"这些重型查询的速度提高了 10 倍以上,我们的内存使用量降低了 100 倍(更重要的是,是有界的)。我们最大的客户在搜索回放时不再看到错误,我们现在可以支持任意规模的客户而不会耗尽内存。"
...
- [ClickHouse Performance Analysis](https://www.youtube.com/watch?v=lxKbvmcLngo) - Alexey Milovidov 讲解调试方法论
- [ClickHouse Meetup: Query Optimization Techniques](https://www.youtube.com/watch?v=JBomQk4Icjo) - 社区优化策略

**延伸阅读**：

- [查询优化指南](/optimize/query-optimization)
- [物化视图社区洞察](./materialized-views.md)
