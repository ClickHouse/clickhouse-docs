...
_如果您的查询速度较慢且希望了解更多优化示例，我们还提供了[查询优化](/optimize/query-optimization)指南。_
...
在调试 ClickHouse 性能时,不要依赖平均查询时间或整体系统指标。相反,应识别特定查询缓慢的原因。系统可能整体平均性能良好,但个别查询却可能因内存耗尽、过滤不当或高基数操作而变慢。
...
Sentry 是一个以开发者为中心的错误跟踪平台,每天处理来自 400 多万开发者的数十亿个事件。他们的核心洞察:_"在这种特定情况下,分组键的基数将决定内存使用"_ - 高基数聚合会通过内存耗尽而非行扫描导致性能下降。

当查询失败时,需要判断是内存问题(分组过多)还是扫描问题(行数过多)。

像 `GROUP BY user_id, error_message, url_path` 这样的查询会为这三个值的每个唯一组合创建一个独立的内存状态。随着用户数量、错误类型和 URL 路径的增加,很容易产生数百万个必须同时保存在内存中的聚合状态。
...
这确保相同的用户出现在每个查询中,在不同时间段提供一致的结果。核心洞察:`cityHash64()` 为相同的输入生成一致的哈希值,因此 `user_id = 12345` 将始终哈希为相同的值,确保该用户要么始终出现在 10% 的样本中,要么永远不出现 - 在查询之间不会出现闪烁。
