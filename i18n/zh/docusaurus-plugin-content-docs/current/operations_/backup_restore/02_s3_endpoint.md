...
本文介绍如何通过 S3 端点向 S3 存储桶备份数据或从中恢复数据。
...
增量备份需要从一个_基础_备份开始。S3 目标位置的第一个参数是 S3 端点,后面跟着存储桶中用于此备份的目录。在本示例中,目录名为 `my_backup`。
...
增量备份包含基础备份与当前表内容之间的差异数据。在执行增量备份之前添加更多数据:
...
此备份命令与基础备份类似,但增加了 `SETTINGS base_backup` 参数和基础备份的位置。请注意,增量备份的目标位置与基础备份不在同一目录中,它使用相同的端点,但存储桶中的目标目录不同。基础备份位于 `my_backup` 中,而增量备份将写入 `my_incremental`:
