...
- 受密码保护
- 针对[系统表、日志表或访问管理表](#system-backups)进行


## 备份类型 {#backup-types}

备份可以是全量备份或增量备份。全量备份是数据的完整副本,而增量备份是相对于上次全量备份的数据变化量。

全量备份的优势在于它是一种简单、独立(不依赖其他备份)且可靠的恢复方法。但是,全量备份可能需要较长时间才能完成,并且会占用大量存储空间。相比之下,增量备份在时间和空间方面都更加高效,但恢复数据时需要所有备份都可用。

根据您的需求,可以选择使用:

- **全量备份** 适用于较小的数据库或关键数据。
- **增量备份** 适用于较大的数据库或需要频繁且经济高效地执行备份的场景。
- **两者结合使用**,例如每周执行全量备份,每天执行增量备份。
...
更多详细信息请参阅["命名集合"](/operations/named-collections)。
