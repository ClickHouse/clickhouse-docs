...
也可以通过在 ClickHouse 存储配置中配置 S3 磁盘来对 S3 执行 `BACKUP`/`RESTORE` 操作。
按照上面为本地磁盘配置的方式,通过向 `/etc/clickhouse-server/config.d` 添加文件来配置磁盘。
...
- 如果您的表使用 S3 存储且磁盘类型不同,
  ...
  会下载并上传它们,这非常低效。在这种情况下,建议使用
  `BACKUP ... TO S3(<endpoint>)` 语法来处理此场景。
  :::


## 本地磁盘备份/恢复使用示例 {#usage-examples}
...
除 zip 外还可以使用其他格式。有关更多详细信息,请参阅下面的["tar 归档备份"](#backups-as-tar-archives)。
