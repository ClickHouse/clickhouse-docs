...
`ExternalDistributed` 引擎允许对存储在远程服务器上的 MySQL 或 PostgreSQL 数据执行 `SELECT` 查询。接受 [MySQL](../../../engines/table-engines/integrations/mysql.md) 或 [PostgreSQL](../../../engines/table-engines/integrations/postgresql.md) 引擎作为参数，因此可以实现分片。
...
- 列名称应与原始表中的名称相同，但您可以仅使用其中一些列，并可按任何顺序排列。
- 列类型可以与原始表中的不同。ClickHouse 尝试将值 [cast](/sql-reference/functions/type-conversion-functions#cast) 为 ClickHouse 数据类型。
...
在指定副本时，在读取每个分片时会选择可用副本中的一个。如果连接失败，则选择下一个副本，依此类推。如果所有副本的连接尝试均失败，则会以相同方式重复尝试多次。
