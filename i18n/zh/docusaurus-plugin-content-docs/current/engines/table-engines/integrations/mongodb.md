...
- `oid_columns` - 应在 WHERE 子句中视为 `oid` 的列的逗号分隔列表。默认为 `_id`。

:::tip
如果您使用的是 MongoDB Atlas 云服务，连接 URL 可以通过 'Atlas SQL' 选项获取。  
Seed list(`mongodb**+srv**`) 目前尚不支持，但将在未来的版本中添加。
:::
...
如果在 MongoDB 文档中未找到键（例如，列名不匹配），则将插入默认值或 `NULL`（如果列是可空的）。

### OID {#oid}

如果您希望将 `String` 在 WHERE 子句中视为 `oid`，只需将列名放入表引擎的最后一个参数中。  
这在通过 `_id` 列查询记录时可能是必要的，因为在 MongoDB 中，默认情况下它具有 `oid` 类型。  
如果表中的 `_id` 字段具有其他类型，例如 `uuid`，您需要指定空的 `oid_columns`，否则此参数的默认值 `_id` 将被使用。
...
在这种情况下，输出将为 `0`，因为 ClickHouse 不知道 `another_oid_column` 具有 `oid` 类型，因此我们来修复它：
...
这样的表达式被转换为 MongoDB 查询语言并在服务器端执行。  
您可以使用 [mongodb_throw_on_unsupported_query](../../../operations/settings/settings.md#mongodb_throw_on_unsupported_query) 禁用所有这些限制。  
在那种情况下，ClickHouse 尝试尽可能地转换查询，但可能会导致全表扫描和在 ClickHouse 端的处理。

:::note
显式设置文字的类型总是更好，因为 Mongo 要求严格的类型过滤。\
例如，您希望通过 `Date` 进行过滤：
