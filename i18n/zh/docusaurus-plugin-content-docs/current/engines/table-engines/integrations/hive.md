...
Hive引擎允许您在HDFS Hive表上执行 `SELECT` 查询。目前，它支持以下输入格式：

- 文本：仅支持简单的标量列类型，除了 `binary`

- ORC：支持简单的标量列类型，除了 `char`；仅支持复杂类型，如 `array`

- Parquet：支持所有简单的标量列类型；仅支持复杂类型，如 `array`
...
表结构可以与原始Hive表结构不同：
- 列名称应与原始Hive表中的相同，但您可以仅使用其中一些列，并可以按任何顺序使用，也可以使用从其他列计算得出的别名列。
- 列类型应与原始Hive表中的相同。
- 分区表达式应与原始Hive表一致，且分区表达式中的列应在表结构中。
...
### 如何使用本地缓存进行HDFS文件系统 {#how-to-use-local-cache-for-hdfs-filesystem}

我们强烈建议您为远程文件系统启用本地缓存。基准测试表明，启用缓存后，其速度几乎快2倍。
...
- enable: 如果为true，ClickHouse将在启动后为远程文件系统(HDFS)维护本地缓存。
- root_dir: 必填。存储远程文件系统本地缓存文件的根目录。
- limit_size: 必填。本地缓存文件的最大大小（以字节为单位）。
- bytes_read_before_flush: 控制从远程文件系统下载文件时在刷新到本地文件系统之前读取的字节数。默认值为1MB。
...
在ClickHouse中的表，从上面创建的Hive表中检索数据：
```sql
