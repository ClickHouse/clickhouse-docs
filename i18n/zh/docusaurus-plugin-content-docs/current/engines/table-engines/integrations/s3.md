---
description: '此引擎提供与 Amazon S3 生态系统的集成。类似于 HDFS 引擎，但提供 S3 特有的功能。'
sidebar_label: 'S3'
...
此引擎可与 [Amazon S3](https://aws.amazon.com/s3/) 生态系统集成。它与 [HDFS](/engines/table-engines/integrations/hdfs) 引擎类似，但提供 S3 特有的功能。
...
* `partition_strategy` – 选项：`WILDCARD` 或 `HIVE`。`WILDCARD` 要求路径中包含 `{_partition_id}`，该占位符会被分区键替换。`HIVE` 不允许使用通配符，假定路径为表的根路径，并生成 Hive 风格的分区目录，使用 Snowflake ID 作为文件名，并以文件格式作为扩展名。默认值为 `file_like_engine_default_partition_strategy` 设置 (在早于 `26.6` 的 `compatibility` 设置下为 `WILDCARD`，否则为 `HIVE`) 。
* `partition_columns_in_data_file` - 仅在使用 `HIVE` 分区策略时使用。用于告知 ClickHouse 是否应预期分区列会写入数据文件中。默认值为 `false`。
...
* `extra_credentials` - 可选。用于在 ClickHouse Cloud 中传递用于基于角色访问的 `role_arn`。配置步骤请参见 [Secure S3](/cloud/data-sources/secure-s3)。
...
`WILDCARD`：将文件路径中的 `{_partition_id}` 通配符替换为实际分区键。不支持读取。仅当 `compatibility` 设置早于 `26.6` 时才会默认选用；否则默认值为 `HIVE` (参见 `file_like_engine_default_partition_strategy` 设置) 。
...
:::note
如示例所示，目前暂不直接支持查询已分区的 S3 table，但可以通过使用 S3 table function 查询各个单独分区来实现。

在 S3 中写入分区数据的主要用例，是为了便于将这些数据传输到另一个 ClickHouse 系统中 (例如，从本地部署系统迁移到 ClickHouse Cloud) 。由于 ClickHouse dataset 通常非常大，而且网络可靠性有时并不理想，因此按子集传输 dataset 更为合理，这也正是进行分区写入的原因。
:::
...
#### 从分区查询数据 3 \{#select-from-partition-3\}
...
#### 从分区 45 查询数据 \{#select-from-partition-45\}
...
在 ClickHouse Cloud 中，您可以使用基于角色的访问方式向 S3 进行身份验证，而无需使用访问密钥。有关配置步骤，请参见 [Secure S3](/cloud/data-sources/secure-s3)。
