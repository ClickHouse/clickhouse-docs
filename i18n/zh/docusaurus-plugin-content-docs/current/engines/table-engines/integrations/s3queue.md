...
用于 Ordered 模式。自 `24.6` 版本起可用。如果存在多个 S3Queue 表副本，并且它们都使用 Keeper 中相同的元数据目录，那么 `s3queue_buckets` 的值至少应等于副本数量。如果同时使用了 `s3queue_processing_threads` 设置，那么将 `s3queue_buckets` 的值进一步增大是有意义的，因为它决定了 `S3Queue` 处理的实际并行度。
