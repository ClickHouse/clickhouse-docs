---
'description': '此引擎与Amazon S3中现有的Delta Lake表提供只读集成。'
...
请注意，Delta Lake 表必须已经存在于 S3 中，此命令不接受 DDL 参数来创建新表。
...
- `url` — 指向现有 Delta Lake 表的 Bucket url。
- `aws_access_key_id`, `aws_secret_access_key` - 用于 [AWS](https://aws.amazon.com/) 账户用户的长期凭证。您可以使用这些凭证来验证您的请求。此参数是可选的。如果未指定凭证，则使用配置文件中的凭证。
