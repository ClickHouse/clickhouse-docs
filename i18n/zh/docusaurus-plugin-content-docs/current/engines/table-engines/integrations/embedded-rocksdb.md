...
- `rocksdb_dir` - 已存在的 RocksDB 的目录路径或新创建的 RocksDB 的目标路径。使用指定的 `rocksdb_dir` 打开表。
- `read_only` - 当 `read_only` 设置为 true 时，使用只读模式。对于具有 TTL 的存储，压缩将不会被触发（无论是手动还是自动），因此不会删除过期条目。
- `primary_key_name` - 列表中的任何列名。
- 必须指定 `primary key`，它仅支持一个列作为主键。主键将以二进制形式序列化为 `rocksdb key`。
- 除主键以外的列将按相应顺序以二进制形式序列化为 `rocksdb` 值。
- 带有键 `equals` 或 `in` 过滤的查询将优化为从 `rocksdb` 执行多键查找。

引擎设置：

- `optimize_for_bulk_insert` - 表格优化为批量插入（插入管道将创建 SST 文件并导入到 rocksdb 数据库，而不是写入内存表）；默认值：`1`。
...
还有一个 `system.rocksdb` 表，它暴露了 rocksdb 统计信息：
...
默认情况下，微不足道的近似计数优化是关闭的，这可能会影响 `count()` 查询的性能。要启用该优化，请设置 `optimize_trivial_approximate_count_query = 1`。此外，该设置会影响 EmbeddedRocksDB 引擎的 `system.tables`，请开启此设置以查看 `total_rows` 和 `total_bytes` 的近似值。

## 支持的操作 {#supported-operations}

### 插入 {#inserts}

当新行插入到 `EmbeddedRocksDB` 时，如果密钥已经存在，则会更新值；否则，会创建一个新密钥。
...
该直接连接避免在内存中形成哈希表，并直接访问
EmbeddedRocksDB 中的数据。

在大连接中，您可能会看到使用直接连接时内存使用量大大降低，
因为不会创建哈希表。
...
:::tip
当 `join_algorithm` 设置为 `direct, hash` 时，在可能的情况下将使用直接连接，否则使用哈希连接。
:::
...
### 更多有关连接的信息 {#more-information-on-joins}
