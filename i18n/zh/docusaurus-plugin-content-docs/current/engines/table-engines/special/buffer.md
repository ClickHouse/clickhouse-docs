---
description: '在写入时先将要写入的数据缓存在 RAM 中，并定期刷新到另一张表。读取时会同时从缓冲区和另一张表中读取数据。'
...
在写入过程中，数据会被插入到一个或多个随机缓冲区中 (由 `num_layers` 配置) 。或者，如果要插入的数据分区片段足够大 (大于 `max_rows` 或 `max_bytes`) ，则会绕过缓冲区，直接写入目标表。

数据刷新的条件会针对 `num_layers` 个缓冲区中的每一个分别计算。例如，如果 `num_layers = 16` 且 `max_bytes = 100000000`，则最大 RAM 消耗为 1.6 GB。
...
创建一个与 `merge.hits` 结构相同、使用 Buffer 引擎的 `merge.hits_buffer` 表。向该表写入时，数据会先缓存在 RAM 中，之后再写入 &#39;merge.hits&#39; 表。系统会创建一个单独的缓冲区，并在满足以下任一条件时刷新数据：

* 自上次刷新起已过去 100 秒 (`max_time`) 或
* 已写入 100 万行 (`max_rows`) 或
* 已写入 100 MB 数据 (`max_bytes`) 或
* 已过去 10 秒 (`min_time`)，并且已写入 10,000 行 (`min_rows`) 和 10 MB (`min_bytes`) 数据

例如，如果只写入了一行数据，那么无论如何，100 秒后都会被刷新。但如果写入了很多行，数据就会更早刷新。

当服务器停止，或执行 `DROP TABLE` 或 `DETACH TABLE` 时，缓冲的数据也会被刷新到目标表。

你可以将数据库名和表名设置为单引号括起来的空字符串。这表示不存在目标表。在这种情况下，当达到数据刷新条件时，缓冲区只会被直接清空。这对于在内存中保留一个数据窗口可能会很有用。
