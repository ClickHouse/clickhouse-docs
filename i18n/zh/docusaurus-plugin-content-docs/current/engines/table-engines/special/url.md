...
* `_headers` - HTTP 请求头。类型：`Map(LowCardinality(String), LowCardinality(String))`。

## 解析相对 URL \{#resolving-relative-urls\}

[url&#95;base](/operations/settings/settings.md#url_base) 设置允许在 `URL` 引擎中使用相对 URL。设置 `url_base` 后，传递给该引擎的 URL 会按照 [RFC 3986](https://datatracker.ietf.org/doc/html/rfc3986) 的规则，以它为基准进行解析。有关解析规则的完整说明，请参见 [url 表函数文档](../../../sql-reference/table-functions/url.md#resolving-relative-urls)。
...
* [url&#95;base](/operations/settings/settings.md#url_base) - 用于解析传递给该引擎的相对 URL 的基础 URL。