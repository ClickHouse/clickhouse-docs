---
description: 'File 表引擎将数据保存在一个文件中，文件使用受支持的
  文件格式之一（如 `TabSeparated`、`Native` 等）。'
sidebar_label: 'File'
sidebar_position: 40
...
在 [clickhouse-local](../../../operations/utilities/clickhouse-local.md) 中，File 表引擎除了 `Format` 外还可以接收文件路径。可以使用数字或人类可读的名称 (例如 `0` 或 `stdin`、`1` 或 `stdout`) 来指定默认输入/输出流。可以根据额外的引擎参数或文件扩展名 (`gz`、`br` 或 `xz`) 来读写压缩文件。
