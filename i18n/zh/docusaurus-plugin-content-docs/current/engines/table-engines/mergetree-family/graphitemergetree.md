---
description: '用于对 Graphite 数据进行稀疏化和聚合/平均（rollup）处理。'
...
Rollup 的配置由服务器配置中的 [graphite&#95;rollup](../../../operations/server-configuration-parameters/settings.md#graphite) 参数定义。该参数名称可以是任意的。你可以创建多个配置，并将它们用于不同的表。
...
### 不含规则类型的配置示例 \{#configuration-example\}
...
:::note
数据 rollup 是在合并过程中完成的。通常不会对旧分区启动合并操作，因此要进行 rollup，需要使用 [optimize](../../../sql-reference/statements/optimize.md) 触发一次未计划的合并。也可以使用其他工具，例如 [graphite-ch-optimizer](https://github.com/innogames/graphite-ch-optimizer)。
:::