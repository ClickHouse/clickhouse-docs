...
<details markdown="1">
  ...
  `Sign` — 分配给某列的名称，该列用于表示行的类型，其中 `1` 表示“状态”行，`-1` 表示“撤销”行。[Int8](/sql-reference/data-types/int-uint)。
</details>
...
对于每一组具有相同排序键 (`ORDER BY`) 的连续行，最终最多只会保留两行：
`Sign` = `1` 的 `"状态"` 行和 `Sign` = `-1` 的 `"撤销"` 行。
换句话说，在 ClickHouse 中，条目会发生折叠。

对于每个生成的数据分区片段，ClickHouse 会保存：

|    |                                                                                            |
| -- | ------------------------------------------------------------------------------------------ |
| 1. | 如果 `"状态"` 行和 `"撤销"` 行的数量相同，且最后一行为 `"状态"` 行，则保存第一条 `"撤销"` 行和最后一条 `"状态"` 行。 |
| 2. | 如果 `"状态"` 行多于 `"撤销"` 行，则保存最后一条 `"状态"` 行。                                         |
| 3. | 如果 `"撤销"` 行多于 `"状态"` 行，则保存第一条 `"撤销"` 行。                                         |
| 4. | 在所有其他情况下，不保存任何行。                                                                           |

另外，当 `"状态"` 行比 `"撤销"` 行至少多 2 行，或者 `"撤销"` 行比 `"状态"` 行至少多 2 行时，合并会继续进行。
不过，ClickHouse 会将这种情况视为逻辑错误，并将其记录到服务器日志中。
如果相同的数据被多次插入，就有可能出现此错误。
因此，折叠不应改变统计结果。
变更会被逐步折叠，最终几乎只保留每个对象的最后状态。

需要 `Sign` 列，是因为合并算法不能保证具有相同排序键的所有行都处于同一个结果数据分区片段中，甚至不在同一台物理服务器上。
...
我们使用两个 `INSERT` 查询来创建两个不同的数据分区片段。

:::note
如果我们使用单个查询插入数据，ClickHouse 只会创建一个数据分区片段，并且之后不会执行任何合并操作。
:::
...
通过两条 `INSERT` 语句，我们创建了两个数据分区片段。
`SELECT` 语句在两个线程中执行，因此得到的行顺序是随机的。
然而，折叠**并没有发生**，因为这些数据分区片段尚未被合并，
而 ClickHouse 会在后台的某个未知时间点合并数据分区片段，这一点是无法预测的。
