---
description: '允许从备份中以只读模式立即挂载表/数据库。'
sidebar_label: '备份'
sidebar_position: 60
...
数据库备份允许从[备份](../../operations/backup)中以只读模式即时附加表或数据库。

数据库备份既支持增量备份，也支持非增量备份。
...
- `database_name_inside_backup` — 备份内的数据库名称。
- `backup_destination` — 备份目标。


## 使用示例 {#usage-example}

下面以 `Disk` 备份目标为例进行说明。首先在 `storage.xml` 中配置备份磁盘:
...
也可以像操作普通数据库一样操作这个 Backup 数据库。例如查询其中的表:
