...
'sidebar_label': 'AWS Glue目录'
'title': 'AWS Glue目录'
'pagination_prev': null
'pagination_next': null
'description': '在本指南中，我们将指导您执行使用ClickHouse和AWS Glue数据目录查询S3存储桶中数据的步骤。'
---
...
ClickHouse 支持与多个目录集成（Unity、Glue、Polaris 等）。在本指南中，我们将引导您通过使用 ClickHouse 和 Glue 数据目录查询 S3 存储桶中的数据。
...
要连接到 Glue 目录，您需要识别目录所在地区并提供访问密钥和秘密密钥。

:::note
当前，Glue 目录仅支持访问密钥和秘密密钥，但我们将在未来支持其他认证方法。
:::

## 在 Glue 数据目录和 ClickHouse 之间建立连接 {#connecting}

配置完 Unity 目录并完成认证后，在 ClickHouse 和 Unity 目录之间建立连接。
...
您可以看到上面的某些表不是 Iceberg 表，例如 `iceberg-benchmark.hitsparquet`。您将无法查询这些表，因为当前仅支持 Iceberg。
...
## 从您的数据湖加载数据到 ClickHouse {#loading-data-into-clickhouse}

如果您需要从 Databricks 将数据加载到 ClickHouse，请首先创建一个本地 ClickHouse 表：
