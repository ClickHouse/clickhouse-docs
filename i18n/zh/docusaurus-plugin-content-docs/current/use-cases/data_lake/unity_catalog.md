...
description: '在本指南中，我们将向您介绍如何使用 ClickHouse 和 Unity Catalog 查询
 位于 S3 存储桶中的数据。'
keywords: ['Unity', '数据湖']
...
ClickHouse 支持与多个目录服务集成（Unity、Glue、Polaris 等）。本指南将逐步引导您使用 ClickHouse 和 [Unity Catalog](https://www.databricks.com/product/unity-catalog) 来查询由 Databricks 管理的数据。

Databricks 的 Lakehouse 架构支持多种数据格式。借助 ClickHouse，您可以以 Delta 或 Iceberg 的形式查询 Unity Catalog 表。

:::note
由于该功能目前为实验性功能，您需要通过以下设置将其启用：
...
要使 ClickHouse 能够与 Unity Catalog 交互,需要确保 Unity Catalog 已配置为允许外部读取器访问。可以按照["启用 Unity Catalog 的外部数据访问"](https://docs.databricks.com/aws/en/external-access/admin)指南进行配置。
...
配置好 catalog 后,必须为 ClickHouse 生成凭据。根据与 Unity 的交互模式,可以使用以下两种方法:
...
如果您使用的是 Iceberg 客户端,则只会显示启用了 Uniform 的 Delta 表:
