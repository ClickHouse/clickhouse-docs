...
'sidebar_label': 'Unity目录'
'title': 'Unity目录'
'pagination_prev': null
'pagination_next': null
'description': '在本指南中，我们将指导您如何使用ClickHouse和Unity目录查询位于S3存储桶中的数据。'
---
...
:::note
与 Unity Catalog 的集成适用于受管理和外部表。
当前此集成仅支持 AWS。
:::

ClickHouse 支持与多个目录的集成（Unity、Glue、Polaris 等）。本指南将引导您通过 ClickHouse 查询由 Databricks 管理的数据，使用 [Unity Catalog](https://www.databricks.com/product/unity-catalog)。

Databricks 支持多种数据格式以用于他们的湖仓。通过 ClickHouse，您可以将 Unity Catalog 表查询为 Delta 和 Iceberg。

## 在 Databricks 中配置 Unity {#configuring-unity-in-databricks}

为了允许 ClickHouse 与 Unity Catalog 交互，您需要确保 Unity Catalog 配置为允许与外部读取器的交互。可以通过遵循[ "启用对 Unity Catalog 的外部数据访问"](https://docs.databricks.com/aws/en/external-access/admin)指南来实现。
...
一旦您的目录配置完成，您必须为 ClickHouse 生成凭据。根据您与 Unity 的交互模式，可以采用两种不同的方法：

* 对于 Iceberg 客户端，使用服务主体进行身份验证 [service principal](https://docs.databricks.com/aws/en/dev-tools/auth/oauth-m2m)。

* 对于 Delta 客户端，使用个人访问令牌（[PAT](https://docs.databricks.com/aws/en/dev-tools/auth/pat)）。

## 创建 Unity Catalog 与 ClickHouse 之间的连接 {#creating-a-connection-between-unity-catalog-and-clickhouse}
...
连接建立后，您可以开始通过 Unity catalog 进行查询。例如：
...
:::note 需要反引号
因为 ClickHouse 不支持超过一个命名空间，所以需要反引号。
:::
