...
description: '在本指南中，我们将带您逐步完成使用 ClickHouse 和 Nessie 目录
 查询数据的流程。'
...
ClickHouse 支持与多个目录集成（Unity、Glue、REST、Polaris 等）。本指南将引导您完成使用 ClickHouse 和 [Nessie](https://projectnessie.org/) 目录查询数据的步骤。

Nessie 是一个用于数据湖的开源事务目录，提供：

* **Git 风格的**基于分支和提交的数据版本控制
* **跨表事务**及可见性保证
* **REST API**，遵循 Iceberg REST 目录规范
* **开放数据湖**方案，支持 Hive、Spark、Dremio、Trino 等
* **可用于生产环境的**部署方案，可在 Docker 或 Kubernetes 上运行

:::note
由于该特性仍为实验性功能，您需要通过以下命令启用它：
...
对于本地开发和测试,您可以使用容器化的 Nessie 设置。此方法非常适合学习、原型开发和开发环境。
...
您可以使用官方的 [Nessie docker-compose 设置](https://projectnessie.org/guides/setting-up/),它提供了一个完整的环境,包含 Nessie、内存版本存储和用于对象存储的 MinIO。
...
:::note
Nessie 设置使用内存版本存储,需要先将示例数据加载到 Iceberg 表中。在尝试通过 ClickHouse 查询这些表之前,请确保环境已创建并填充了表。
:::
...
如果需要将 Nessie 目录中的数据加载到 ClickHouse,请先创建一个本地 ClickHouse 表:
