...
ClickHouse 支持与多个目录集成（OneLake、Unity、Glue、Polaris 等）。本指南将指导你完成使用 ClickHouse 和 [OneLake](https://learn.microsoft.com/en-us/fabric/onelake/onelake-overview) 查询存储在 Microsoft OneLake 中的数据的各个步骤。

Microsoft OneLake 为其 lakehouse 支持多种表格式。使用 ClickHouse，你可以查询 Iceberg 表。

:::note
由于此功能处于 beta 阶段，你需要通过以下方式启用该功能：
...
在 Microsoft Fabric 中对表执行查询之前，你需要收集以下信息：
...
对于 Data Item ID，我们建议使用你的 Lakehouse ID。根据我们的测试，它无法与 Warehouse ID 搭配使用。

如需了解如何查找这些值，请参阅 [Microsoft OneLake 的文档](http://learn.microsoft.com/en-us/fabric/onelake/table-apis/table-apis-overview#prerequisites)。

## 在 OneLake 与 ClickHouse 之间创建连接 \{#creating-a-connection-between-unity-catalog-and-clickhouse\}

在获取上述所需信息后，即可在 Microsoft OneLake 与 ClickHouse 之间创建连接，但在此之前需要先启用目录（catalogs）：
...
如果你使用 Iceberg 客户端，则只会显示已启用 Uniform 的 Delta 表：
...
如果需要将 OneLake 中的数据加载到 ClickHouse：
