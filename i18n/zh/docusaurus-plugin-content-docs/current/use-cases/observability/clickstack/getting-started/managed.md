...
<VerticalStepper headerLevel="h2">
  ...
  若要在 [ClickHouse Cloud](https://console.clickhouse.cloud) 中创建一个 Managed ClickStack 服务，请按照以下步骤完成注册：

  * 在[注册页面](https://console.clickhouse.cloud/signUp)创建账户
  * 选择使用邮箱，或通过 Google SSO、Microsoft SSO、AWS Marketplace、Google Cloud 或 Microsoft Azure 注册
  * 如果您使用邮箱和密码注册，请务必在 24 小时内通过邮件中的链接验证邮箱地址
  * 使用刚刚创建的用户名和密码登录

  <Image img={signup_page} size="md" alt="Cloud 注册页面" border />

  ## 选择您的使用场景 \{#select-your-use-case\}

  <UseCaseSelector />

  ## 指定云服务提供商、区域和数据规模 \{#specify-your-data-size\}
