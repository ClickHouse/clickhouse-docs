...
:::note Warning
Temporal 平台中的 OpenMetrics 支持目前处于[公开预览](https://docs.temporal.io/evaluate/development-production-features/release-stages#public-preview)阶段。更多信息请参阅其[官方文档](https://docs.temporal.io/cloud/metrics/openmetrics)。
:::

Temporal 为构建简单、复杂度高且具有高弹性的应用程序提供了一层抽象。


# 使用 ClickStack 监控 Temporal Cloud 指标 \{#temporal-metrics-clickstack\}

:::note[摘要]
本指南演示如何通过配置 OpenTelemetry collector 的 Prometheus receiver，使用 ClickStack 监控 Temporal Cloud。你将学习如何：

- 配置 OTel collector 以收集 Temporal Cloud 指标
- 使用自定义配置部署 ClickStack
- 使用预构建的仪表板可视化 Temporal Cloud 性能（打开的工作流数、每秒操作数（actions/sec）、活动命名空间数、任务积压情况）
...
本节介绍如何通过为 ClickStack OTel collector 配置 Prometheus receiver 来完成 ClickStack 的配置。

## 前提条件 \{#prerequisites\}

- 运行中的 ClickStack 实例
- 已有的 Temporal Cloud 账户
- 从 ClickStack 到 Temporal Cloud 的 HTTP 网络连通性

<VerticalStepper headerLevel="h4">
  #### 创建 Temporal Cloud 密钥

  确保您拥有 Temporal Cloud API 密钥。您可以按照 Temporal 文档中的[身份验证指南](https://docs.temporal.io/production-deployment/cloud/metrics/openmetrics/api-reference#authentication)来创建该密钥。

  :::important 密钥文件
  确保将这些凭据存储在名为 `temporal.key` 的文件中,该文件应与下文创建的配置文件位于同一目录下。此密钥应以纯文本形式存储,前后不能有空格。
  :::

  #### 创建自定义 OTel collector 配置

  ClickStack 允许您通过挂载自定义配置文件并设置环境变量来扩展 OpenTelemetry 收集器的基础配置。自定义配置会与 HyperDX 通过 OpAMP 管理的基础配置进行合并。
  ...
  * **按照 [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/resource/#service) 设置必需的 `service.name` 资源属性**
  * 通过专用 pipeline 将指标转发至 ClickHouse exporter

  :::note

  * 只需在自定义配置中定义新的 receiver、processor 和 pipeline 即可
  * `memory_limiter` 和 `batch` 处理器以及 `clickhouse` 导出器已经在基础 ClickStack 配置中预先定义好——你只需按名称引用它们即可
  * `resource` 处理器会按照 OpenTelemetry 语义约定设置所需的 `service.name` 属性
  * 对于多个 Temporal Cloud 账户，自定义 `service.name` 以区分它们（例如，`"temporal-prod"`、`"temporal-dev"`）
    :::
  ...
  3. 将 `temporal.key` 文件挂载为 `/etc/otelcol-contrib/temporal.key`
  4. 确保 ClickStack 与 Temporal 之间的网络互通

  所有命令均假定在示例目录中执行,该目录存储了 `temporal-metrics.yaml` 和 `temporal.key` 文件。
  ...
  2. 搜索名称以 `temporal` 开头的指标（例如 `temporal_cloud_v1_workflow_success_count`、`temporal_cloud_v1_poll_timeout_count`）
...
2. 在右上角点击省略号，然后选择 **Import Dashboard**
...
请检查环境变量 `CUSTOM_OTELCOL_CONFIG_FILE` 是否已正确设置：
...
验证 collector 是否可以访问 Temporal Cloud：
...
你应该会看到一系列 Prometheus 指标被打印出来，例如：
...
如果在日志中看到身份验证错误，请检查 API 密钥。

### 网络连通性问题 {#network-issues}

如果 ClickStack 无法访问 Temporal Cloud，请确保在 Docker Compose 文件或 `docker run` 命令中已允许[外部网络访问](https://docs.docker.com/engine/network/#drivers)。

## 后续步骤 {#next-steps}

如果希望进一步探索，可以参考以下步骤来拓展监控实践：

- 为关键指标（内存使用阈值、连接数上限、缓存命中率下降）[设置告警](/use-cases/observability/clickstack/alerts)
- 为特定使用场景创建更多仪表盘（复制延迟、持久化性能）
- 通过复制接收端配置并使用不同的 Endpoint 和 Service 名称，监控多个 Temporal Cloud 帐户