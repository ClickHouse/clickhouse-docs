...
description: '使用 OpenTelemetry 进行 ClickStack 数据接入 - ClickHouse 可观测性栈'
title: '使用 OpenTelemetry 接入数据'
doc_type: 'guide'
...
所有数据均通过一个 **OpenTelemetry (OTel) collector** 实例接入 ClickStack，该实例是日志、指标、跟踪和会话数据的主要入口。对于这一实例，我们建议使用 collector 的官方 [ClickStack 发行版](#installing-otel-collector)。

用户可以通过 [语言 SDK](/use-cases/observability/clickstack/sdks) 将数据发送到该 collector，或通过采集基础设施指标和日志的数据采集代理发送数据（例如以 [agent](/use-cases/observability/clickstack/ingesting-data/otel-collector#collector-roles) 角色运行的 OTel collector，或其他技术，如 [Fluentd](https://www.fluentd.org/) 或 [Vector](https://vector.dev/)）。
...
ClickStack OTel 采集器也可以独立部署,不依赖堆栈中的其他组件。
