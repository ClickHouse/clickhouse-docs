...
sidebar_position: 6
description: 'ClickStack 的 AWS Lambda - ClickHouse 可观测性堆栈'
title: 'AWS Lambda'
doc_type: 'guide'
...
OpenTelemetry 项目提供独立的 Lambda 层用于:
...
特定语言的自动插桩 Lambda 层会使用针对您特定语言的 OpenTelemetry 自动插桩包自动对您的 Lambda 函数代码进行插桩。

每种语言和区域都有其对应的层 ARN。

如果您的 Lambda 已经使用 OpenTelemetry SDK 进行了插桩,可以跳过此步骤。
...
_可以在 [OpenTelemetry Lambda Layers GitHub 仓库](https://github.com/open-telemetry/opentelemetry-lambda/releases)中找到这些层的最新版本。_

3. 在您的 Lambda 函数中的"Configuration" > "Environment variables"下配置以下环境变量。
...
collector Lambda 层允许您将日志、指标和追踪数据从 Lambda 函数转发到 ClickStack,而不会因导出器延迟而影响响应时间。
...
部署层后,您应该能够在 HyperDX 中看到从 Lambda 函数自动收集的追踪数据。`decouple` 和 `batching` 处理器可能会在遥测数据收集过程中引入延迟,因此追踪数据可能会延迟显示。要发出自定义日志或指标,您需要使用特定语言的 OpenTelemetry SDK 对代码进行插桩。
...
将 `OTEL_LOG_LEVEL` 环境变量设置为 `DEBUG` 以启用 OpenTelemetry SDK 的调试日志。这有助于确保自动埋点层正确地对您的应用程序进行埋点。

### 启用采集器调试日志 {#enabling-collector-debug-logs}

要调试采集器问题,您可以通过修改采集器配置文件来启用调试日志,添加 `logging` 导出器并将遥测日志级别设置为 `debug`,以启用采集器 Lambda 层的详细日志记录。
