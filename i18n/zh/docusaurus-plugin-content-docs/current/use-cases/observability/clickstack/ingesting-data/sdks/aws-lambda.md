...
特定语言的自动插桩 Lambda 层会使用针对您所用语言的 OpenTelemetry 自动插桩包,自动对您的 Lambda 函数代码进行插桩。
...
收集器 Lambda 层允许您将日志、指标和追踪数据从 Lambda 函数转发到 ClickStack,而不会因导出器延迟而影响响应时间。
...
部署层后,您应该能够在 HyperDX 中看到从 Lambda 函数自动收集的追踪数据。`decouple` 和 `batching` 处理器可能会在遥测数据收集过程中引入延迟,因此追踪数据可能会延迟显示。要发送自定义日志或指标,您需要使用特定语言的 OpenTelemetry SDK 对代码进行插桩。
...
如果您未看到手动定义的追踪或其他遥测数据,可能是使用了不兼容版本的 OpenTelemetry API 包。请确保您的 OpenTelemetry API 包版本不高于 AWS Lambda 中包含的版本。
...
要调试采集器问题,您可以通过修改采集器配置文件来启用调试日志,添加 `logging` 导出器并将遥测日志级别设置为 `debug`,以启用来自采集器 Lambda 层的更详细日志记录。
