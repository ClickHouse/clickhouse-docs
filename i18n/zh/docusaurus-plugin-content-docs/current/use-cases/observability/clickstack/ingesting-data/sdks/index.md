...
数据通常通过 **OpenTelemetry (OTel) 收集器（collector）** 发送到 ClickStack，可以由语言 SDK 直接发送，或通过作为代理运行的中间 OpenTelemetry 收集器（例如用于收集基础设施指标和日志）。

语言 SDK 负责在你的应用程序内部收集遥测数据——尤其是 **traces（调用链/追踪）** 和 **logs（日志）**——并通过 OTLP 端点将这些数据导出到 OpenTelemetry 收集器，再由其完成写入 ClickHouse 的摄取过程。

在基于浏览器的环境中，SDK 还可能负责收集 **session data（会话数据）**，包括 UI 事件、点击和页面导航，从而实现用户会话回放。 
...
虽然 ClickStack 提供了具有增强遥测和功能的专有语言 SDK,但您也可以无缝使用其现有的 OpenTelemetry SDK。
:::
...
要启用此功能,请配置 OpenTelemetry 收集器将资源标签转发到 Pod。详细的设置说明请参阅 [Kubernetes 集成指南](/use-cases/observability/clickstack/integrations/kubernetes#forwarding-resouce-tags-to-pods)。
