...
`userId`、`userEmail`、`userName` 和 `teamName` 将在会话 UI 中填充相应的值,但可以省略。可以指定任何其他附加值并用于搜索事件。
...
要了解更多信息以及强制跟踪特定请求,请参阅 [Google Cloud Run 文档](https://cloud.google.com/run/docs/trace)。
...
要启用未捕获异常的捕获,您需要将 `HDX_NODE_EXPERIMENTAL_EXCEPTION_CAPTURE` 环境变量设置为 1。
