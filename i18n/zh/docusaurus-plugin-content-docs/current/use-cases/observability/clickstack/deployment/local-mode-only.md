...
与[一体化镜像](/use-cases/observability/clickstack/deployment/docker-compose)类似,这个综合性 Docker 镜像打包了所有 ClickStack 组件:

- **ClickHouse**
- **HyperDX**
- **OpenTelemetry (OTel) 收集器**(在端口 `4317` 和 `4318` 上公开 OTLP)
- **MongoDB**(用于持久化应用程序状态)
