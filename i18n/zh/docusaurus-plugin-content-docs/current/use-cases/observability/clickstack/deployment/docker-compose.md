...
所有 ClickStack 组件均作为独立的 Docker 镜像单独分发:
...
这些镜像可以组合并使用 Docker Compose 在本地部署。
...
- `8888`:用于监控采集器自身的 Prometheus 指标端点

这些端口支持与各种遥测数据源集成,使 OpenTelemetry 采集器能够满足生产环境中多样化的数据采集需求。
...
此发行版可与 ClickHouse Cloud 配合使用。用户应:

- 从 `docker-compose.yaml` 文件中移除 ClickHouse 服务。如果是测试环境,此步骤为可选项,因为部署的 ClickHouse 实例将被忽略——尽管会浪费本地资源。如果移除该服务,请确保同时删除对该服务的所有引用,例如 `depends_on`。
- 通过在 compose 文件中设置环境变量 `CLICKHOUSE_ENDPOINT`、`CLICKHOUSE_USER` 和 `CLICKHOUSE_PASSWORD`,修改 OTel collector 以使用 ClickHouse Cloud 实例。具体操作是将环境变量添加到 OTel collector 服务中:

  ```shell
  otel-collector:
      image: ${OTEL_COLLECTOR_IMAGE_NAME}:${IMAGE_VERSION}
      environment:
        CLICKHOUSE_ENDPOINT: '<CLICKHOUSE_ENDPOINT>' # https 端点
