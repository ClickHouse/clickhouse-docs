...
HyperDX 的 Helm chart 可以在[这里](https://github.com/hyperdxio/helm-charts)找到,这是生产环境部署的**推荐**方法。
...
端口转发允许我们访问和配置 HyperDX。生产环境部署时,应通过 ingress 或负载均衡器暴露服务,以确保正确的网络访问、TLS 终止和可扩展性。端口转发最适合本地开发或一次性管理任务,不适用于长期运行或高可用性环境。
...
对于处理敏感数据(如 API 密钥或数据库凭据),请使用 Kubernetes Secret。HyperDX Helm chart 提供了默认的 Secret 文件,您可以修改并应用到集群中。

#### 使用预配置的 Secret {#using-pre-configured-secrets}

Helm chart 包含一个默认的 Secret 模板,位于 [`charts/hdx-oss-v2/templates/secrets.yaml`](https://github.com/hyperdxio/helm-charts/blob/main/charts/hdx-oss-v2/templates/secrets.yaml)。该文件提供了管理 Secret 的基础结构。
...
如果使用 ClickHouse Cloud,用户需要禁用由 Helm chart 部署的 ClickHouse 实例,并指定 Cloud 凭据:
...
默认情况下,此 chart 会同时安装 ClickHouse 和 OTel 采集器。但在生产环境中,建议您单独管理 ClickHouse 和 OTel 采集器。

要禁用 ClickHouse 和 OTel 采集器,请设置以下参数值:
...
默认情况下,chart 中配置了一个 cronjob 任务,负责检查是否应触发告警。以下是其配置选项:

| 参数                          | 描述                                                                                                                                                                                | 默认值            |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| `tasks.enabled`               | 启用/禁用集群中的 cron 任务。默认情况下,HyperDX 镜像会在进程内运行 cron 任务。如果您希望在集群中使用独立的 cron 任务,请将此项设置为 true。 | `false`           |
...
此命令将删除与该版本关联的所有资源，但持久化数据（如有）可能仍会保留。
