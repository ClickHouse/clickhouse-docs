...
sidebar_position: 5
description: '仅使用本地模式部署 ClickStack - ClickHouse 可观测性栈'
doc_type: 'guide'
...
与[一体化镜像](/use-cases/observability/clickstack/deployment/docker-compose)类似，这个综合型 Docker 镜像打包了所有 ClickStack 组件：

* **ClickHouse**
* **HyperDX**
* **OpenTelemetry (OTel) collector**（通过端口 `4317` 和 `4318` 对外暴露 OTLP）
* **MongoDB**（用于持久化应用状态）

**但是，此发行版 HyperDX 已禁用用户身份验证功能**
...
<VerticalStepper headerLevel="h3">
  ...
  本地模式会在本地的 8080 端口上部署 HyperDX UI。
  ...
  **在此部署模式下未启用身份验证，因此系统不会提示您创建用户。**

  连接到您自己的外部 ClickHouse 集群，例如 ClickHouse Cloud。

  <Image img={hyperdx_2} alt="创建登录" size="md" />

  创建一个 Source（数据源），保留所有默认值，并将 `Table` 字段填写为 `otel_logs`。其他所有设置应会被自动检测，此时即可点击 `Save New Source`。
...
对于本地模式镜像，用户只需要设置 `BETA_CH_OTEL_JSON_SCHEMA_ENABLED=true` 参数即可，例如：
