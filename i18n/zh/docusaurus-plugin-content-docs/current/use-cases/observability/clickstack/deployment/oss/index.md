...
| ClickHouse Embedded | ClickStack UI 直接打包进 ClickHouse 服务器二进制中。无需额外组件。                                          | 试用 ClickStack、探索现有 ClickHouse 数据、演示                                                        | 无告警、无看板/搜索持久化、无法自定义查询设置、无事件模式                                                  | [ClickHouse Embedded](/use-cases/observability/clickstack/deployment/clickhouse-embedded)             |