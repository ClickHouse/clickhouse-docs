...
为了在容器重启后持久化数据和设置,用户可以修改上述 Docker 命令来挂载路径 `/data/db`、`/var/lib/clickhouse` 和 `/var/log/clickhouse-server`。例如:
...
- **缺乏组件隔离:** 所有组件均在单个 Docker 容器内运行。这导致无法独立扩展和监控各组件,且任何 `cgroup` 限制都会全局应用于所有进程。因此,各组件可能会竞争 CPU 和内存资源。


## 自定义端口 {#customizing-ports-deploy}

如果需要自定义 HyperDX Local 运行的应用程序端口（8080）或 API 端口（8000），需要修改 `docker run` 命令来转发相应端口并设置若干环境变量。
...
此发行版可与 ClickHouse Cloud 配合使用。虽然本地 ClickHouse 实例仍会被部署(但会被忽略),但可以通过设置环境变量 `CLICKHOUSE_ENDPOINT`、`CLICKHOUSE_USER` 和 `CLICKHOUSE_PASSWORD` 来配置 OTel 收集器以使用 ClickHouse Cloud 实例。
...
连接到 HyperDX UI 后,导航至 [`Team Settings`](http://localhost:8080/team) 并创建到您的 ClickHouse Cloud 服务的连接,然后配置所需的数据源。有关示例流程,请参见[此处](/use-cases/observability/clickstack/getting-started#create-a-cloud-connection)。
