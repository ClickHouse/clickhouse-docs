...
2. **[Elastic Distributions of OpenTelemetry (EDOT SDKs)](https://www.elastic.co/docs/reference/opentelemetry/edot-sdks/)** – 这些是 Elastic 发布的标准 OpenTelemetry SDK 的定制发行版，适用于 .NET、Java、Node.js、PHP 和 Python。如果你的应用已经在使用 EDOT SDK，则无需重新为代码插桩。你只需重新配置 SDK，将遥测数据导出到 ClickStack 中包含的 OTLP Collector。有关更多详细信息，请参阅[“迁移 EDOT SDKs”](#migrating-edot-sdks)。

:::note 尽可能使用 ClickStack SDKs
虽然标准 OpenTelemetry SDKs 也受支持，但我们强烈建议针对各语言使用 [**ClickStack 分发的 SDKs**](/use-cases/observability/clickstack/sdks)。这些发行版包含额外的插桩、改进的默认配置，以及为与 ClickStack 管道和 HyperDX UI 无缝协同而设计的自定义扩展。通过使用 ClickStack SDKs，你可以启用诸如异常栈追踪等高级功能，而这些功能在原生 OpenTelemetry 或 EDOT SDKs 中是不可用的。
:::
...
配置遵循标准 OpenTelemetry 机制。对于 Python,通常通过环境变量完成,如 [OpenTelemetry 零代码插桩文档](https://opentelemetry.io/docs/zero-code/python/configuration/)中所述。
...
要迁移到 ClickStack,请更新端点以指向本地 OTLP Collector 并更改授权标头:
