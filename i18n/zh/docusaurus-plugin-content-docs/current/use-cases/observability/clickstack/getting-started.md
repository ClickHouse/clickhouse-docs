...
得益于预构建的 Docker 镜像，开始使用 **ClickStack** 十分简单。这些镜像基于官方的 ClickHouse Debian 软件包构建，并提供多个发行版，以满足不同的使用场景。
...
HyperDX 将自动连接到本地集群,并为日志、跟踪、指标和会话创建数据源,使您能够立即开始使用该产品。
...
要查找 HyperDX 的连接详情,请导航到 ClickHouse Cloud 控制台并点击侧边栏上的<b>连接</b>按钮。
...
这将暴露一个 OpenTelemetry 收集器(端口 4317 和 4318)以及 HyperDX UI(端口 8080)。
...
- [本地文件和指标](/use-cases/observability/clickstack/getting-started/local-data) - 加载本地文件并使用本地 OTel 收集器在 macOS 或 Linux 上监控系统。
...
自托管本地模式镜像预配置了 OpenTelemetry 收集器和 ClickHouse 服务器。这使得从应用程序中采集遥测数据并在 HyperDX 中可视化变得简单,只需最少的外部配置。要开始使用自托管版本,只需运行 Docker 容器并转发相应的端口:
...
### 完成连接凭据配置 {#complete-connection-credentials}
