...
ClickStack 架构围绕三个核心组件构建：**ClickHouse**、**HyperDX** 和 **OpenTelemetry (OTel) 采集器**。一个 **MongoDB** 实例提供应用状态存储。它们共同构成了一个为日志、指标和链路追踪进行了优化的高性能开源可观测性技术栈。
...
ClickStack 的核心是 ClickHouse,一个专为大规模实时分析而设计的列式数据库。它支持可观测性数据的摄取和查询,具备以下能力:
...
这种模块化架构使 ClickStack 能够提供开箱即用的可观测性平台,具有高性能、灵活性和开源特性。
