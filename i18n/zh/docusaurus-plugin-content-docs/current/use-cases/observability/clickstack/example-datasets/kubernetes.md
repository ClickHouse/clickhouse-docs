...
description: 'ClickStack 入门与 Kubernetes 监控'
doc_type: 'guide'
...
本指南可用于从 Kubernetes 系统收集日志和指标，并将其发送到 **ClickStack** 进行可视化和分析。作为演示数据，我们可以选择使用 ClickStack 基于官方 OpenTelemetry 演示构建的 fork 版本。
...
- 一个 **Kubernetes 集群**(推荐 v1.20 及以上版本),其中至少有一个节点可为 ClickHouse 提供至少 32 GiB 内存和 100GB 磁盘空间。
- **[Helm](https://helm.sh/)** v3 及以上版本
- **`kubectl`**,已配置为可与您的集群进行交互
...
此**步骤为可选项,适用于没有现有 Pod 需要监控的用户**。虽然已在 Kubernetes 环境中部署了现有服务的用户可以跳过此步骤,但此演示应用包含已插桩的微服务,这些服务会生成追踪和会话回放数据,使用户能够探索 ClickStack 的所有功能。

以下命令将在 Kubernetes 集群中部署 ClickStack 分支版本的 OpenTelemetry 演示应用程序栈,专为可观测性测试和展示插桩功能而定制。它包括后端微服务、负载生成器、遥测管道、支撑基础设施(例如 Kafka、Redis)以及与 ClickStack 的 SDK 集成。

所有服务都部署到 `otel-demo` 命名空间。每个部署包括:

- 使用 OTel 和 ClickStack SDK 自动插桩以收集追踪、指标和日志。
- 所有服务将其插桩数据发送到 `my-hyperdx-hdx-oss-v2-otel-collector` OpenTelemetry 收集器(未部署)
- [转发资源标签](/use-cases/observability/clickstack/integrations/kubernetes#forwarding-resouce-tags-to-pods)以通过环境变量 `OTEL_RESOURCE_ATTRIBUTES` 关联日志、指标和追踪。
...
部署演示后,请确认所有 Pod 已成功创建并处于 `Running` 状态:
...
安装 Helm Chart 后,您可以将 ClickStack 部署到集群中。您可以在 Kubernetes 环境中运行所有组件(包括 ClickHouse 和 HyperDX),也可以使用 ClickHouse Cloud,其中 HyperDX 也作为托管服务提供。
...
以下命令将 ClickStack 安装到 `otel-demo` 命名空间。该 Helm Chart 将部署:

- 一个 ClickHouse 实例
- HyperDX
- OTel Collector 的 ClickStack 发行版
- 用于存储 HyperDX 应用程序状态的 MongoDB
...
此 chart 还会安装 ClickHouse 和 OTel 收集器。对于生产环境,建议您使用 ClickHouse 和 OTel 收集器 operator,和/或使用 ClickHouse Cloud。
...
:::note
该 chart 目前始终会部署 HyperDX 和 MongoDB。虽然这些组件提供了替代访问路径,但它们未与 ClickHouse Cloud 身份验证集成。在此部署模型中,这些组件面向管理员使用,[提供对安全摄取密钥的访问](#retrieve-ingestion-api-key),该密钥是通过已部署的 OTel 收集器进行数据摄取所必需的,但不应向最终用户公开。
:::
...
要验证部署状态,请运行以下命令并确认所有组件都处于 `Running` 状态。注意:使用 ClickHouse Cloud 的用户在此列表中不会看到 ClickHouse:
...
:::note
即使使用 ClickHouse Cloud,仍然需要在 Kubernetes 集群中部署本地 HyperDX 实例。它提供由 HyperDX 捆绑的 OpAMP 服务器管理的数据摄取密钥,通过已部署的 OTel 收集器保护数据摄取安全 - 这是 ClickHouse Cloud 托管版本目前尚不支持的功能。
:::

出于安全考虑,该服务使用 `ClusterIP`,默认不对外公开。
...
### 获取数据摄取 API 密钥 {#retrieve-ingestion-api-key}

通过 ClickStack 收集器部署的 OTel 收集器的数据摄取使用摄取密钥进行保护。

导航至 [`Team Settings`](http://localhost:8080/team),从 `API Keys` 部分复制 `Ingestion API Key`。此 API 密钥可确保通过 OpenTelemetry 收集器进行的数据摄取是安全的。

<Image img={copy_api_key} alt='Copy API key' size='lg' />

### 创建 API 密钥 Kubernetes Secret {#create-api-key-kubernetes-secret}

使用数据摄取 API 密钥创建一个新的 Kubernetes Secret,以及一个包含通过 ClickStack Helm Chart 部署的 OTel 收集器位置的 ConfigMap。后续组件将使用此配置来允许数据摄取到通过 ClickStack Helm Chart 部署的收集器中:
...
# 创建一个 ConfigMap,指向上面部署的 ClickStack OTel 收集器
...
重启 OpenTelemetry 演示应用程序 Pod,以使 Ingestion API Key 生效。

```shell
kubectl rollout restart deployment -n otel-demo -l app.kubernetes.io/part-of=opentelemetry-demo
````

演示服务的追踪和日志数据现在应该开始流入 HyperDX。

<Image img={hyperdx_kubernetes_data} alt='HyperDX Kubernetes Data' size='lg' />

### 添加 OpenTelemetry Helm 仓库 {#add-otel-helm-repo}

为了收集 Kubernetes 指标,我们将部署一个标准的 OTel 收集器,并将其配置为使用上述 Ingestion API Key 将数据安全地发送到我们的 ClickStack 收集器。
...
# 将 k8sattributes 处理器添加到所有管道,并将必要的规则添加到 ClusterRole。
...
# 将 kubeletstats 接收器添加到指标管道,并将必要的规则添加到 ClusterRole。
...
导航到您的 HyperDX UI - 可以使用在 Kubernetes 中部署的实例,也可以通过 ClickHouse Cloud 访问。

<p/>
<details>
<summary>使用 ClickHouse Cloud</summary>

如果使用 ClickHouse Cloud,只需登录您的 ClickHouse Cloud 服务,并从左侧菜单中选择"HyperDX"。您将自动完成身份验证,无需创建用户。
...
从这里选择所需的源类型,然后选择相应的表,例如对于追踪,选择表 `otel_traces`。所有设置应自动检测。
...
在生产环境中,如果您未在 ClickHouse Cloud 中使用 HyperDX,我们建议使用带有 TLS 的 ingress。例如:
