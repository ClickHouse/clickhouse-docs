...
**以下指南假定你已经按照[一体化镜像的部署说明](/use-cases/observability/clickstack/getting-started)或[仅本地模式](/use-cases/observability/clickstack/deployment/local-mode-only)部署了 ClickStack，并完成了初始用户创建。或者，你也可以跳过所有本地设置，直接连接到我们托管的 ClickStack 演示环境 [play-clickstack.clickhouse.com](https://play-clickstack.clickhouse.com)，该演示环境使用的就是本数据集。**

本指南使用托管在公共 ClickHouse playground [sql.clickhouse.com](https://sql.clickhpouse.com) 上的示例数据集，你可以从本地部署的 ClickStack 连接到该数据集。

:::warning 在 ClickHouse Cloud 中与 HyperDX 配合使用时不受支持
当 HyperDX 托管在 ClickHouse Cloud 中时，不支持远程数据库。因此无法使用该数据集。
:::


它包含大约 40 小时的数据，这些数据来源于官方 OpenTelemetry (OTel) 演示的 ClickHouse 版本。数据会在每晚重放，并将时间戳调整到当前时间窗口，使用户能够通过 HyperDX 集成的日志、链路追踪和指标来探索系统行为。

:::note 数据差异
由于数据集每天从午夜开始重放，具体可视化效果可能会因你查看演示的时间而有所不同。
:::
...
在本演示中,我们将调查一起涉及销售望远镜及相关配件的电子商务网站的故障事件。
...
此演示使用官方 OpenTelemetry 演示的 [ClickStack 维护分支](https://github.com/ClickHouse/opentelemetry-demo)。
...
点击与我们报告的用户无法完成支付问题相关的最明显的错误聚类:`Failed to place order`。

这将显示与 `frontend` 服务相关的此错误的所有出现列表:

<Image img={step_5} alt='Step 5' size='lg' />

选择任何结果错误。日志元数据将详细显示。滚动浏览 `Overview` 和 `Column Values` 表明由于缓存导致的信用卡扣费问题:
...
选择 `Infrastructure` 选项卡以查看与 `frontend` 服务的底层 Pod 相关的指标,并将时间跨度扩大到 `1d`:
...
在 ClickStack 中,追踪会自动与日志和指标关联。让我们探索与所选日志关联的追踪,以识别相关的服务。
...
滚动到视图底部,我们可以看到 `payment` 服务导致了错误,该错误随后沿调用链向上传播。
...
选择 `Search` 切换到主搜索视图。将数据源切换为 `Traces` 并选择 `Results table` 视图。**确保时间跨度仍为最近一天。**
...
如果我们选择 `Event Patterns` 对追踪应用事件聚类,就可以立即看到 `payment` 服务的缓存问题。

<Image img={step_12} alt='Step 12' size='lg' />

### 探索追踪的基础设施 {#explore-infrastructure-for-a-trace}

点击 `Results table` 切换到结果视图。使用 `StatusCode` 过滤器和 `Error` 值过滤错误。
...
移除错误过滤器并从左侧 `Analysis Mode` 菜单中选择 `Event Deltas`。
...
- 缓存已满
- 这导致资源消耗增加
- 该问题阻止了 visa 支付完成 - 或者至少导致支付需要很长时间才能完成。
...
如果我们将 `Group By`(分组依据)设置为 `SpanAttributes['app.payment.card_type']`(只需输入 `card` 即可自动完成),我们可以看到相对于 Mastercard,Visa 交易的服务性能下降情况:

<Image img={step_19} alt='Step 19' size='lg' />

请注意,一旦发生错误,响应将在 `0s` 内返回。

### 探索指标以获取更多上下文 {#exploring-metrics-for-more-context}

最后,让我们将缓存大小绘制为指标,以查看它随时间的变化情况,从而获得更多上下文信息。
...
我们可以看到缓存大小在 4-5 小时内持续增长(可能是在软件部署之后),最终达到最大值 `100,000`。从 `Sample Matched Events`(样本匹配事件)中,我们可以看到错误与缓存达到此限制相关,之后缓存被记录为大小 `0`,响应也在 `0s` 内返回。
...
从左侧菜单导航到 `Client Sessions`(客户端会话)选项卡,然后确保数据源设置为 `Sessions`,时间段设置为 `Last 1 day`:
...
可以通过按 ▶️ 按钮来重放会话。在 `Highlighted`(突出显示)和 `All Events`(所有事件)之间切换可以实现不同程度的 span 粒度,前者突出显示关键事件和错误。
...
选择该 span 后,我们可以确认这是由内部错误引起的。通过点击 `Trace`(追踪)选项卡并滚动浏览连接的 span,我们能够确认该客户确实是我们缓存问题的受害者。

<Image img={step_24} alt='Step 24' size='lg' />

</VerticalStepper>

此演示演练了涉及电子商务应用程序中支付失败的真实事件,展示了 ClickStack 如何通过统一的日志、追踪、指标和会话重放来帮助发现根本原因 - 探索我们的[其他入门指南](/use-cases/observability/clickstack/sample-datasets)以深入了解特定功能。
