...
description: "ClickStack 本地及系统数据与指标入门指南"
doc_type: "指南"
keywords:
...
:::note Root 用户
我们以 root 用户身份运行收集器以访问所有系统日志——这是从 Linux 系统的受保护路径捕获日志所必需的。但是,不建议在生产环境中使用此方式。在生产环境中,应将 OpenTelemetry Collector 部署为本地代理,并仅授予访问目标日志源所需的最小权限。

注意,我们将主机的 `/var/log` 挂载到容器内的 `/host/var/log`,以避免与容器自身的日志文件发生冲突。
:::
...
在随后弹出的菜单中,从 `Output format` 下拉列表中选择 `Percentage`,然后点击 `Apply`。
