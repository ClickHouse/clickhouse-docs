...
如需在配置生产主机前测试集成,可使用包含示例日志的演示数据集。
...
如果您希望在配置现有环境之前先测试主机日志集成功能,可以使用["演示数据集"](/use-cases/observability/clickstack/integrations/host-logs#demo-dataset)部分中提供的预配置环境和示例数据进行测试。
...
:::note
对于演示数据集,请将时间范围设置为 **2025-11-11 00:00:00 - 2025-11-12 00:00:00 (UTC)**(根据您的本地时区调整)。导入的仪表板默认不会指定时间范围。
:::
