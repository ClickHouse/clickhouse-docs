...
title: '使用 ClickStack 构建可视化与仪表盘'
sidebar_label: '仪表盘'
pagination_prev: null
pagination_next: null
description: '使用 ClickStack 构建可视化与仪表盘'
doc_type: 'guide'
...
ClickStack 支持事件可视化，并内置对在 HyperDX 中绘制图表的支持。这些图表可以添加到仪表盘中，与其他用户共享。

可视化可以基于 traces、metrics、logs 或任意用户定义的宽事件模式创建。
...
对于以下示例,我们使用 [sql.clickhouse.com](https://sql.clickhouse.com) 上提供的远程数据集,该数据集在["远程演示数据集"](/use-cases/observability/clickstack/getting-started/remote-demo-data)指南中有详细描述。**用户也可以通过访问 [play-clickstack.clickhouse.com](https://play-clickstack.clickhouse.com) 来重现这些示例。**
...
:::note
可视化可以从任何数据源创建——指标、追踪或日志。ClickStack 将所有这些都视为宽事件。任何**数值列**都可以随时间绘制图表,**字符串**、**日期**或**数值**列可用于分组。
...
仪表板提供了一种将相关可视化图表分组的方式,使用户能够并排比较指标和探索模式,从而识别系统中的潜在根本原因。这些仪表板可用于临时调查或保存以进行持续监控。

全局过滤器可以在仪表板级别应用,并自动传播到该仪表板内的所有可视化图表。这允许在图表之间进行一致的下钻操作,并简化跨服务和遥测类型的事件关联分析。

下面我们使用日志和追踪数据源创建一个包含两个可视化图表的仪表板。这些步骤可以在 [play-clickstack.clickhouse.com](https://play-clickstack.clickhouse.com) 上重现,或通过连接到托管在 [sql.clickhouse.com](https://sql.clickhouse.com) 上的数据集在本地重现,具体说明请参见指南["远程演示数据集"](/use-cases/observability/clickstack/getting-started/remote-demo-data)。
...
要删除、编辑或复制可视化图表,请将鼠标悬停在其上方并使用相应的操作按钮。
...
仪表板可通过左侧菜单访问,内置搜索功能可快速定位特定仪表板。

<Image img={dashboard_search} alt='仪表板搜索' size='sm' />
...
此仪表板提供用于监控 ClickHouse 的可视化图表。要访问此仪表板,请从左侧菜单中选择它。
...
服务仪表板基于追踪数据显示当前活动的服务。这要求用户已收集追踪数据并配置了有效的追踪数据源。

服务名称从追踪数据中自动检测,并提供一系列预构建的可视化图表,分布在三个选项卡中:HTTP 服务、数据库和错误。

可视化图表可以使用 Lucene 或 SQL 语法进行过滤,并且可以调整时间窗口以进行针对性分析。
