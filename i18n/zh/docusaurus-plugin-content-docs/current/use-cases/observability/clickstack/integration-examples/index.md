...
description: 'ClickStack 数据接入 - ClickHouse 可观测性栈'
title: '集成指南'
doc_type: 'landing-page'
keywords: ['ClickStack data ingestion', 'observability data ingestion', 'ClickStack integration guides']
---

ClickStack 提供多种方式将可观测性数据接入到你的 ClickHouse 实例中。本节包含针对各类日志、链路追踪和指标数据源的快速入门指南。

:::note
其中部分集成指南使用 ClickStack 内置的 OpenTelemetry Collector 进行快速测试。对于生产环境部署，我们建议运行你自己的 OTel Collector，并将数据发送到 ClickStack 的 OTLP 端点。生产环境配置请参阅[发送 OpenTelemetry 数据](/use-cases/observability/clickstack/ingesting-data/opentelemetry)。
:::
