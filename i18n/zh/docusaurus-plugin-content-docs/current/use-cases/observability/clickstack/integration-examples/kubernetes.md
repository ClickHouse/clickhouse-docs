...
ClickStack 使用 OpenTelemetry (OTel) collector 从 Kubernetes 集群中收集日志、指标和 Kubernetes 事件，并将其转发到 ClickStack。我们支持原生的 OTel 日志格式，无需任何额外的厂商相关配置。

本集成涵盖以下内容：

- **日志 (Logs)**
- **基础设施监控指标 (Infra Metrics)**

:::note
如需发送应用级别指标或 APM/追踪数据（traces），还需要在应用程序中添加相应的语言集成。
:::

本指南假设你已经部署了一个[作为网关的 ClickStack OTel collector](/use-cases/observability/clickstack/ingesting-data/otel-collector)，并已通过摄取（ingestion）API 密钥进行安全防护。
...
此外,创建一个 ConfigMap 来指定您的 ClickStack OTel 收集器的位置:
...
现在,来自您的 Kubernetes 集群的指标、日志和 Kubernetes 事件应该会显示在 HyperDX 中。


## 将资源标签转发到 Pod(推荐) {#forwarding-resouce-tags-to-pods}

为了将应用程序级别的日志、指标和追踪与 Kubernetes 元数据(如 Pod 名称、命名空间等)关联,您需要使用 `OTEL_RESOURCE_ATTRIBUTES` 环境变量将 Kubernetes 元数据转发到应用程序。
    ...
    spec:
      containers:
        - name: app-container
          ...
          env:
            # ... 其他环境变量
            # 从 Downward API 收集 K8s 元数据并转发到应用程序
            - name: POD_NAME
              valueFrom:
                fieldRef:
            ...
            # 通过 OTEL_RESOURCE_ATTRIBUTES 将 K8s 元数据转发到应用程序
