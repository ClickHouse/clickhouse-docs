...
对于希望在配置生产系统之前测试 nginx 追踪集成的用户,我们提供了一个预生成的 Nginx 追踪样本数据集,其中包含真实的流量模式。

<VerticalStepper headerLevel="h4">

#### 启动 ClickStack {#start-clickstack}

如果您还没有运行 ClickStack,请使用以下命令启动:
...
在继续操作之前,请等待约 30 秒,以便 ClickStack 完全初始化。
...
:::note[时区显示]
HyperDX 以您浏览器的本地时区显示时间戳。演示数据时间跨度为 **2025-10-26 13:00:00 - 2025-10-27 13:00:00 (UTC)**。较宽的时间范围确保无论您身处何地都能看到演示追踪数据。看到追踪数据后，您可以将范围缩小到 24 小时以获得更清晰的可视化效果。
:::
