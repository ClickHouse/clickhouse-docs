...
:::note[TL;DR]
本指南介绍如何通过配置 OpenTelemetry 收集器采集 Redis 服务器日志,从而使用 ClickStack 监控 Redis。您将学习如何:

- 配置 OTel 收集器以解析 Redis 日志格式
- 使用自定义配置部署 ClickStack
- 使用预构建的仪表板可视化 Redis 指标(连接数、命令、内存、错误)
...
如果您想在配置现有设置之前测试 Redis 集成,可以使用["演示数据集"](/use-cases/observability/clickstack/integrations/redis#demo-dataset)部分中预配置的设置和示例数据进行测试。
...
首先,检查 Redis 日志配置。连接到 Redis 并检查日志文件位置:
...
- **Docker**: 通常记录到 stdout,但可配置为写入 `/data/redis.log`

如果 Redis 正在记录到 stdout,请通过更新 `redis.conf` 将其配置为写入文件:
...
- 从标准位置读取 Redis 日志
- 使用正则表达式解析 Redis 日志格式以提取结构化字段(`pid`、`role`、`timestamp`、`log_level`、`message`)
...
- 此配置使用 `start_at: beginning` 在采集器启动时读取所有现有日志,使您能够立即看到日志。对于生产部署,如果您希望避免在采集器重启时重新摄取日志,请更改为 `start_at: end`。
  :::
...
:::note
确保 ClickStack 采集器具有读取 Redis 日志文件的适当权限。在生产环境中,使用只读挂载(`:ro`)并遵循最小权限原则。
:::
...
对于希望在配置生产系统之前测试 Redis 集成的用户,我们提供了一个预生成的 Redis 日志样本数据集,包含真实的使用模式。
...
:::note[时区显示]
HyperDX 以浏览器本地时区显示时间戳。演示数据的时间跨度为 **2025-10-27 10:00:00 - 2025-10-28 10:00:00 (UTC)**。较宽的时间范围可确保无论您身处何地都能看到演示日志。看到日志后,可以将范围缩小到 24 小时,以获得更清晰的可视化效果。
:::
...
#### 仪表板将创建并预配置所有可视化内容 {#created-dashboard}
...
### 自定义配置无法加载 {#troubleshooting-not-loading}
...
如果 Redis 日志的格式不同，则可能需要调整 `regex_parser` 运算符中的正则表达式模式。标准格式为：
