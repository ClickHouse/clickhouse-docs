...
:::note[TL;DR]
本指南介绍如何通过配置 OpenTelemetry 采集器的 Redis 接收器,使用 ClickStack 监控 Redis 性能指标。您将学习:
...
首先，确认你可以连接到 Redis，并且 `INFO` 命令可以正常执行：
...
# 在同一网络中运行 ClickStack（在配置中将 endpoint 更新为 &quot;redis:6379&quot;）
...
- 为关键指标设置[告警](/use-cases/observability/clickstack/alerts)（内存使用阈值、连接限制、缓存命中率下降）
