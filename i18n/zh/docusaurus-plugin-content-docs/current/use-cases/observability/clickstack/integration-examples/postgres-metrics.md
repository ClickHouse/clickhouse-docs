...
:::note[要点速览]
本指南演示如何通过配置 OpenTelemetry collector 的 PostgreSQL receiver，使用 ClickStack 监控 PostgreSQL 性能指标。你将学会如何：

- 配置 OTel collector 来收集 PostgreSQL 指标
- 使用自定义配置部署 ClickStack
- 使用预构建的仪表板可视化 PostgreSQL 性能（事务、连接、数据库大小、缓存命中率）

如果你希望在为生产环境的 PostgreSQL 数据库配置之前先测试集成，可以使用提供的包含示例指标的演示数据集。
...
本节介绍如何通过为 ClickStack OTel collector 配置 PostgreSQL receiver，将您现有 PostgreSQL 实例的指标发送到 ClickStack。

如果您希望在配置自己现有环境之前先测试 PostgreSQL 指标集成，可以在[下一节](#demo-dataset)中使用我们预先配置的演示数据集进行测试。
...
PostgreSQL receiver 需要一个对统计视图具有只读访问权限的用户。为监控用户授予 `pg_monitor` 角色：
...
ClickStack 允许通过挂载自定义配置文件并设置环境变量来扩展基础 OpenTelemetry collector 配置。
...
:::note
`tls: insecure: true` 设置会在开发/测试环境中禁用 SSL 校验。对于启用 SSL 的生产 PostgreSQL，请移除此行或配置正确的证书。
:::
...
1. 进入 Metrics Explorer（指标浏览器）
2. 搜索以 `postgresql.` 开头的指标（例如：`postgresql.backends`、`postgresql.commits`）
3. 您应能看到按配置采集间隔出现的指标数据点

在确认指标稳定流入后，继续前往[仪表盘与可视化](#dashboards)部分导入预构建的仪表盘。
...
对于希望在配置生产系统之前测试 PostgreSQL 指标集成的用户，我们提供了一个预生成的数据集，其中包含具有真实负载模式的 PostgreSQL 指标。

:::note[仅数据库级指标]
此演示数据集仅包含数据库级指标，以保持示例数据轻量化。在监控真实的 PostgreSQL 数据库时，表和索引指标会自动采集。
:::
...
下载预生成的指标文件（包含 24 小时、具有真实负载模式的 PostgreSQL 指标）：
...
该数据集包含真实的模式：
- **早晨连接峰值（08:00）** - 登录高峰
- **缓存性能问题（11:00）** - Blocks_read 指标峰值
- **应用程序缺陷（14:00-14:30）** - 回滚率飙升至 15%
- **死锁事件（14:15，16:30）** - 罕见的死锁
...
前往 [Dashboards and visualization](#dashboards) 部分导入该仪表盘，即可一次性查看大量 PostgreSQL 指标。

:::note[时区显示]
HyperDX 会按照您浏览器的本地时区显示时间戳。演示数据的时间范围为 **2025-11-10 00:00:00 - 2025-11-11 00:00:00（UTC）**。请将时间范围设置为 **2025-11-09 00:00:00 - 2025-11-12 00:00:00**，以确保无论您身在何处都能看到演示指标。看到指标后，您可以将时间范围收窄到 24 小时，以获得更清晰的可视化效果。
:::
...
为帮助你开始使用 ClickStack 监控 PostgreSQL，我们提供了 PostgreSQL 指标的关键可视化视图。
...
在完成 PostgreSQL 指标监控配置之后：

- 为关键阈值（连接数上限、高回滚率、低缓存命中率）配置[告警](/use-cases/observability/clickstack/alerts)
- 启用 `pg_stat_statements` 扩展，实现查询级监控
- 通过复制接收端配置，并使用不同的端点（endpoint）和服务名称，监控多个 PostgreSQL 实例



## 部署到生产环境 {#going-to-production}

本指南在 ClickStack 内置的 OpenTelemetry Collector 基础上进行扩展，以便快速完成配置。对于生产环境部署，我们建议运行您自己的 OTel Collector，并将数据发送到 ClickStack 的 OTLP 端点。生产环境配置请参阅[发送 OpenTelemetry 数据](/use-cases/observability/clickstack/ingesting-data/opentelemetry)。
