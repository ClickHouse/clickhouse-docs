...
:::note[TL;DR]
本指南介绍如何通过配置 OpenTelemetry 采集器的 PostgreSQL 接收器,使用 ClickStack 监控 PostgreSQL 性能指标。您将学习:

- 配置 OTel 采集器以收集 PostgreSQL 指标
- 使用自定义配置部署 ClickStack
- 使用预构建仪表板可视化 PostgreSQL 性能指标(事务、连接、数据库大小、缓存命中率)

如果您希望在配置生产环境 PostgreSQL 数据库之前测试集成功能,可以使用包含示例指标的演示数据集。
...
本节介绍如何通过配置 ClickStack OTel 采集器的 PostgreSQL 接收器,将现有 PostgreSQL 安装的指标发送到 ClickStack。
...
#### 创建自定义 OTel 采集器配置 {#create-custom-config}

ClickStack 允许您通过挂载自定义配置文件并设置环境变量来扩展基础 OpenTelemetry 采集器配置。
...
:::note[仅包含数据库级指标]
此演示数据集仅包含数据库级指标,以保持示例数据轻量化。在监控真实 PostgreSQL 数据库时,表和索引指标会自动收集。
:::
...
下载预生成的指标文件(包含 24 小时真实模式的 PostgreSQL 指标):
...
:::note[时区显示]
HyperDX 会以您浏览器的本地时区显示时间戳。演示数据的时间范围为 **2025-11-10 00:00:00 - 2025-11-11 00:00:00 (UTC)**。请将时间范围设置为 **2025-11-09 00:00:00 - 2025-11-12 00:00:00**,以确保无论您身处何地都能看到演示指标。查看到指标后,您可以将范围缩小至 24 小时,以获得更清晰的可视化效果。
:::
...
为帮助您快速开始使用 ClickStack 监控 PostgreSQL,我们提供了 PostgreSQL 指标的核心可视化功能。

<VerticalStepper headerLevel="h4">

#### <TrackedLink href={useBaseUrl('/examples/postgres-metrics-dashboard.json')} download="postgres-metrics-dashboard.json" eventName="docs.postgres_metrics_monitoring.dashboard_download">下载</TrackedLink>仪表板配置文件 {#download}

#### 导入预构建仪表板 {#import-dashboard}

1. 打开 HyperDX 并导航至仪表板(Dashboards)部分
...
仪表板创建完成后,所有可视化组件均已预先配置:

<Image img={example_dashboard} alt='PostgreSQL 指标仪表板' />

:::note
对于演示数据集,请将时间范围设置为 **2025-11-10 00:00:00 - 2025-11-11 00:00:00 (UTC)**(请根据您的本地时区进行调整)。导入的仪表板默认不会指定时间范围。
:::
...
完成 PostgreSQL 指标监控配置后:

- 为关键阈值设置[告警](/use-cases/observability/clickstack/alerts)(如连接数限制、高回滚率、低缓存命中率)
- 使用 `pg_stat_statements` 扩展启用查询级监控
- 通过复制接收器配置并指定不同的端点和服务名称来监控多个 PostgreSQL 实例
