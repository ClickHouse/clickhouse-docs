...
:::note[TL;DR]
本指南介绍如何通过配置 OpenTelemetry 收集器采集 PostgreSQL 服务器日志,实现使用 ClickStack 监控 PostgreSQL。您将学习如何:

- 配置 PostgreSQL 以 CSV 格式输出日志,以便进行结构化解析
- 创建用于日志采集的自定义 OTel 收集器配置
- 使用自定义配置部署 ClickStack
- 使用预构建的仪表板可视化 PostgreSQL 日志洞察(错误、慢查询、连接)

如果您希望在配置生产环境 PostgreSQL 之前测试集成,可以使用包含示例日志的演示数据集。
...
PostgreSQL 支持多种日志格式。对于使用 OpenTelemetry 进行结构化解析,我们推荐使用 CSV 格式,该格式提供一致且可解析的输出。
...
:::note
本指南使用 PostgreSQL 的 `csvlog` 格式进行可靠的结构化解析。如果您使用 `stderr` 或 `jsonlog` 格式,需要相应调整 OpenTelemetry 收集器的配置。
:::
...
- 此配置使用 `start_at: end` 以避免在收集器重启时重新摄取日志。对于测试,可更改为 `start_at: beginning` 以立即查看历史日志。
...
3. 挂载您的 PostgreSQL 日志目录以便收集器可以读取
...
:::note
确保 ClickStack 收集器具有读取 PostgreSQL 日志文件的适当权限。在生产环境中,使用只读挂载(`:ro`)并遵循最小权限原则。
:::
...
1. 进入搜索视图
2. 将来源设置为 Logs
3. 通过 `source:postgresql` 过滤以查看 PostgreSQL 相关日志
