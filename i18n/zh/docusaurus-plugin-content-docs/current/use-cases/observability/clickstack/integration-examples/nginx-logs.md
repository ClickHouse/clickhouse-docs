...
本节介绍如何通过修改 ClickStack OTel 收集器配置,将现有 Nginx 安装配置为向 ClickStack 发送日志。
...
首先,配置 Nginx 以 JSON 格式输出日志,以便更容易解析。将此日志格式定义添加到您的 nginx.conf 中:
...
- 解析 JSON 日志条目
- 提取并保留原始日志时间戳
- 添加 source: Nginx 属性以便在 HyperDX 中进行过滤
...
对于希望在配置生产系统之前测试 nginx 集成的用户,我们提供了一个预生成的 nginx 访问日志示例数据集,其中包含真实的流量模式。
