...
description: '使用 ClickStack 配置告警'
doc_type: 'guide'
...
ClickStack 内置告警支持,使团队能够跨日志、指标和追踪实时检测和响应问题。

可以直接在 HyperDX 界面中创建告警,并与 Slack 和 PagerDuty 等主流通知系统集成。

告警功能可无缝处理您的 ClickStack 数据,帮助您跟踪系统健康状况、捕获性能衰退并监控关键业务事件。


## 告警类型 {#types-of-alerts}

ClickStack 支持两种互补的告警创建方式:**搜索告警**和**仪表板图表告警**。告警创建后,会附加到相应的搜索或图表上。
...
- 指定 `grouped by` 值。这允许对搜索进行聚合,例如按 `ServiceName` 聚合,从而可以从同一搜索触发多个告警。
...
可以为不同的服务类型创建 webhook,包括 Slack 和 PagerDuty,以及通用目标。
...
- Webhook URL。例如:`https://hooks.slack.com/services/<unique_path>`。有关更多详细信息,请参阅 [Slack 文档](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks/)。

**Slack (API)**

- Slack bot token。有关更多详细信息,请参阅 [Slack 文档](https://docs.slack.dev/authentication/tokens/#bot/)。

**PagerDuty API**

- PagerDuty integration key。有关更多详细信息,请参阅 [PagerDuty 文档](https://support.pagerduty.com/main/docs/api-access-keys)。
...
此视图还会显示告警评估历史记录。告警会按照定期时间间隔进行评估(由创建告警时设置的周期/持续时间定义)。在每次评估期间,HyperDX 会查询您的数据以检查是否满足告警条件:

- **红色条**:此次评估期间满足阈值条件,告警已触发(已发送通知)
...
以下是使用 HyperDX 的几种常见告警场景:

**错误:** 建议为默认的 `All Error Events` 和 `HTTP Status >= 400` 已保存搜索设置告警,以便在出现过多错误时接收通知。

**慢操作:** 您可以设置慢操作搜索(例如 `duration:>5000`),然后在出现过多慢操作时触发告警。

**用户事件:** 您还可以为面向客户的团队设置告警,以便在新用户注册或执行关键用户操作时接收通知。
