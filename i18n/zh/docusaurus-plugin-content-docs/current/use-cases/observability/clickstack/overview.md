...
**ClickStack** 是一款基于 ClickHouse 构建的生产级可观测性平台，将日志、追踪、指标和会话统一到单一的高性能解决方案中。ClickStack 专为监控和调试复杂系统而设计，使开发人员和 SRE 能够在无需在工具之间来回切换、也无需借助时间戳或关联 ID 手动拼接数据的情况下，对问题进行端到端追踪。

ClickStack 的核心是一条简单却强大的理念：所有可观测性数据都应作为宽且信息丰富的事件进行写入。这些事件按数据类型（日志、追踪、指标和会话）分别存储在 ClickHouse 表中，但在数据库层面依然可以被完全查询并进行交叉关联。

ClickStack 通过利用 ClickHouse 的列式架构、原生 JSON 支持以及完全并行化的执行引擎，高效处理高基数工作负载。这使得在海量数据集上实现亚秒级查询、对大时间范围进行快速聚合，以及对单条追踪进行深度检查成为可能。JSON 以压缩的列式格式存储，从而在无需人工干预或预先定义模式的情况下支持模式演进。
...
2. **OpenTelemetry collector** – 定制构建的预配置采集器，为日志、追踪和指标采用了预定义的架构
3. **ClickHouse** – 作为整个技术栈核心的高性能分析数据库
...
ClickStack 可与任何 ClickHouse 实例和 schema 开箱即用,只需最少的配置。无论您是从零开始还是与现有环境集成,都可以在几分钟内完成部署并投入使用。

### 用户友好且专为可观测性打造 {#user-friendly-purpose-built}

HyperDX UI 同时支持 SQL 和 Lucene 风格的语法,允许用户选择适合其工作流程的查询界面。该 UI 专为可观测性场景打造,经过优化以帮助团队快速定位根因并流畅地处理复杂数据。

### 端到端可观测性 {#end-to-end-observability}

ClickStack 提供全栈可见性,涵盖从前端用户会话到后端基础设施指标、应用程序日志和分布式追踪。这种统一视图支持对整个系统进行深度关联分析。

### 为 ClickHouse 深度优化 {#built-for-clickhouse}

技术栈的每一层都旨在充分发挥 ClickHouse 的能力。查询经过优化以充分利用 ClickHouse 的分析函数和列式存储引擎,确保对海量数据进行快速搜索和聚合。

### 原生 OpenTelemetry 支持 {#open-telemetry-native}

ClickStack 原生集成了 OpenTelemetry,通过 OpenTelemetry collector 端点采集所有数据。对于高级用户,它还支持使用原生文件格式、自定义数据管道或 Vector 等第三方工具直接将数据导入 ClickHouse。

### 开源且完全可定制 {#open-source-and-customizable}

ClickStack 完全开源,可以部署在任何环境中。Schema 灵活且用户可修改,UI 设计为可配置自定义 schema 而无需修改代码。所有组件(包括 collector、ClickHouse 和 UI)都可以独立扩展,以满足数据采集、查询或存储需求。
...
2. **OpenTelemetry collector**  
   定制化采集器,配置了针对 ClickHouse 数据摄取优化的预设模式。通过 OpenTelemetry 协议接收日志、指标和追踪数据,并使用高效的批量插入方式直接写入 ClickHouse。

3. **ClickHouse**  
   高性能分析数据库,作为宽事件的中央数据存储。ClickHouse 利用其列式引擎和对 JSON 的原生支持,提供大规模的快速搜索、过滤和聚合能力。

除了这三个组件外,ClickStack 还使用 **MongoDB 实例**来存储应用程序状态,例如仪表板、用户账户和配置设置。

完整的架构图和部署详情可参见[架构部分](/use-cases/observability/clickstack/architecture)。
