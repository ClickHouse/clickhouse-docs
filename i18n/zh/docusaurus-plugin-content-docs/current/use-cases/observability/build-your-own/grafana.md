---
title: '使用 Grafana'
description: '使用 Grafana 与 ClickHouse 实现可观测性'
...
Grafana 是 ClickHouse 可观测性数据的首选可视化工具，这一能力是通过 Grafana 官方的 ClickHouse 插件实现的。用户可以按照[此处](/integrations/grafana)的安装说明进行操作。

插件的 V4 版本在全新的查询构建体验中，将日志和追踪作为一等公民。这减少了 SRE 编写 SQL 查询的需要，并简化了基于 SQL 的可观测性，加速了这一新兴范式的发展。
...
在 Grafana 中配置 ClickHouse 数据源时,该插件允许用户为日志和追踪指定默认数据库和表,以及这些表是否符合 OTel 模式。这使得插件能够返回在 Grafana 中正确渲染日志和追踪所需的列。如果您对默认的 OTel 模式进行了更改并希望使用自己的列名,可以进行指定。对于时间(`Timestamp`)、日志级别(`SeverityText`)或消息正文(`Body`)等列使用默认的 OTel 列名意味着无需进行任何更改。
...
日志配置需要时间、日志级别和消息列才能正确渲染日志。

追踪配置稍微复杂一些(完整列表见[此处](/engines/table-engines/mergetree-family/mergetree#mergetree-data-storage))。此处所需的列是为了使构建完整追踪配置文件的后续查询能够被抽象化。这些查询假设数据的结构类似于 OTel,因此与标准模式有显著偏差的用户需要使用视图才能从此功能中受益。
...
查询构建器提供了一种简单的方式来修改查询,使用户无需编写 SQL。可以从查询构建器执行过滤操作,包括查找包含关键字的日志。希望编写更复杂查询的用户可以切换到 SQL 编辑器。只要返回适当的列,并选择 `logs` 作为查询类型,结果就会以日志形式渲染。日志渲染所需的列在[此处](https://grafana.com/developers/plugin-tools/tutorials/build-a-logs-data-source-plugin#logs-data-frame-format)列出。
...
如上所示,追踪 ID 被渲染为可点击的链接。点击追踪 ID 后,用户可以通过 `View Trace` 链接查看关联的 span。此操作将发出以下查询(假设使用 OTel 列)以检索所需结构的 span,并将结果渲染为瀑布图。
...
:::note
注意上述查询如何使用物化视图 `otel_traces_trace_id_ts` 来执行追踪 ID 查找。更多详细信息请参阅[加速查询 - 使用物化视图进行查找](/use-cases/observability/schema-design#using-materialized-views-incremental--for-fast-lookups)。
:::
...
除了统计数据外,折线图是可观测性用例中最常见的可视化形式。如果查询返回一个名为 `time` 的 `datetime` 字段和一个数值字段,ClickHouse 插件将自动渲染折线图。例如:
...
我们在前面的章节中探讨了使用 IP 字典为可观测性数据添加地理坐标。假设您有 `latitude` 和 `longitude` 列,可以使用 `geohashEncode` 函数对可观测性数据进行可视化。这将生成与 Grafana Geo Map 图表兼容的地理哈希。下面显示了一个示例查询和可视化:
