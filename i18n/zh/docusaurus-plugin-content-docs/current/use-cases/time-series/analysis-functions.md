...
在 ClickHouse 中，时间序列分析可以使用标准 SQL 聚合和窗口函数进行。 
在处理时间序列数据时，您通常会遇到三种主要类型的指标：

* 随时间单调增加的计数指标（如页面浏览量或总事件数）
* 表示时间点测量的表计指标，可以上下波动（如 CPU 使用率或温度）
* 采样观测结果并在桶中计数的直方图（如请求持续时间或响应大小）

这些指标的常见分析模式包括比较周期之间的值、计算累计总和、确定变化率和分析分布。 
所有这些都可以通过聚合、窗口函数如 `sum() OVER` 和专业函数如 `histogram()` 的组合来实现。

## 季度对季度变化 {#time-series-period-over-period-changes}

在分析时间序列数据时，我们常常需要理解值在时间段之间的变化。 
这对表计和计数指标都是至关重要的。 
[`lagInFrame`](/docs/sql-reference/window-functions/lagInFrame) 窗口函数让我们能够访问前一个时期的值，从而计算这些变化。

以下查询通过计算“怪异艾尔”扬科维奇的维基百科页面的每日变化来演示这一点。
趋势列显示与前一天相比，流量是增加（正值）还是减少（负值），有助于识别活动中的异常峰值或骤降。
...
以下查询通过使用 `sum() OVER` 子句创建运行总和，同时 `bar()` 函数提供增长的可视化表示。
...
在分析时间序列数据时，了解单位时间内事件的发生速率通常是有用的。 
此查询通过将每小时总数除以每小时的秒数（3600）来计算每秒的页面浏览率。 
可视化条形图有助于识别活动的高峰小时。
...
时间序列数据的一个流行用例是基于跟踪事件生成直方图。 
假设我们想要了解根据总点击量对多个页面的分布情况，只包括总点击量超过 10,000 次的页面。
...
然后，我们可以使用 [`arrayJoin()`](/docs/sql-reference/functions/array-join) 来处理数据，并使用 `bar()` 进行可视化：
