...
拥有全面的日期和时间数据类型对于有效的时间序列数据管理是必不可少的，ClickHouse 正是能够满足这一需求。
从紧凑的日期表示到具有纳秒精度的高精度时间戳，这些类型旨在平衡存储效率和不同时间序列应用的实际需求。

无论您是处理历史金融数据、物联网传感器读数，还是未来日期事件，ClickHouse 的日期和时间类型都提供了处理各种时间数据场景所需的灵活性。
支持的类型范围允许您在保持用例所需的精确度的同时优化存储空间和查询性能。

* [`Date`](/sql-reference/data-types/date) 类型在大多数情况下是足够的。此类型需要 2 字节来存储一个日期，并将范围限制为 `[1970-01-01, 2149-06-06]`。

* [`Date32`](/sql-reference/data-types/date32) 涵盖更广泛的日期范围。它需要 4 字节来存储一个日期，并将范围限制为 `[1900-01-01, 2299-12-31]`。

* [`DateTime`](/sql-reference/data-types/datetime) 存储具有秒精度的日期时间值，范围为 `[1970-01-01 00:00:00, 2106-02-07 06:28:15]`。它每个值需要 4 字节。

* 对于需要更高精度的情况，可以使用 [`DateTime64`](/sql-reference/data-types/datetime64)。该类型允许存储精确到纳秒的时间，范围为 `[1900-01-01 00:00:00, 2299-12-31 23:59:59.99999999]`。它每个值需要 8 字节。
...
我们可以使用 [`now()`](/sql-reference/functions/date-time-functions#now) 函数返回当前时间，并通过第一个参数使用 [`now64()`](/sql-reference/functions/date-time-functions#now64) 以指定的精度获取当前时间。
...
许多用例需要存储时区。我们可以将时区作为最后一个参数设置到 `DateTime` 或 `DateTime64` 类型中：
...
* `dt_2` 和 `dt64_2` 没有指定时区，因此它们使用服务器的本地时区，在此例中为 `Europe/London`。

在第二行中，我们插入的所有值都没有时区，因此使用了服务器的本地时区。
与第一行相同，`dt_1` 和 `dt_3` 转换为 `Europe/Berlin`，而 `dt_2` 和 `dt64_2` 使用服务器的本地时区。


## 日期和时间函数 {#time-series-date-time-functions}

ClickHouse 还提供了一组函数，让我们能够在不同的数据类型之间进行转换。
