...
keywords: ['时间序列', '存储效率', '压缩', '数据保留', 'TTL', '存储优化', '磁盘占用']
...
在探索了如何查询我们的 Wikipedia 统计数据集之后，让我们将重点放在优化其在 ClickHouse 中的存储效率上。
本节将演示一些实用技术，帮助在保持查询性能的同时减少存储占用。
...
鉴于此值,我们可以改用 UInt32,它仅占用 4 字节,且允许存储最大约 40 亿的值:
