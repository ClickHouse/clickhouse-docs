...
这里使用了 [`toDate()`](/sql-reference/functions/type-conversion-functions#todate) 函数,它将指定的时间转换为日期类型。或者,我们也可以按小时聚合并筛选特定日期:
...
ClickHouse 提供了 [`WITH FILL`](/docs/guides/developer/time-series-filling-gaps#with-fill) 修饰符来解决这个问题。它会将所有空缺的小时用零填充,以便更好地理解数据随时间的分布:
