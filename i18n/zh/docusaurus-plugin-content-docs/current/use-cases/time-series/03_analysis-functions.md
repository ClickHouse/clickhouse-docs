...
* 表示某一时刻测量值、可升可降的 Gauge 指标（例如 CPU 使用率或温度）
* 对观测值进行采样并按桶计数的 Histogram 指标（例如请求时长或响应大小）
...
[`lagInFrame`](/docs/sql-reference/window-functions/lagInFrame) 窗口函数允许我们获取上一时间区间的数值，用于计算这些变化。

下面的查询通过计算“Weird Al” Yankovic 的维基百科页面浏览量的日环比变化来演示这一点。
trend 列显示与前一天相比流量是增加（正值）还是减少（负值），从而帮助识别异常的流量峰值或骤降情况。
...
下面的查询通过使用 `sum() OVER` 子句来演示这一点，该子句会创建一个累积总和。`bar()` 函数提供了这种增长的可视化展示。
...
我们可以使用 `histogram()` 函数，根据桶（区间）的数量自动生成自适应直方图：
...
然后我们可以使用 [`arrayJoin()`](/docs/sql-reference/functions/array-join) 来处理数据，并使用 `bar()` 对其进行可视化：
