...
要高效管理时序数据，必须具备一套完善的日期和时间类型，而 ClickHouse 恰好提供了这些能力。
从紧凑的日期表示到具备纳秒级精度的高精度时间戳，这些类型在存储效率与不同时间序列应用的实际需求之间取得了平衡。

无论你是在处理历史金融数据、IoT 传感器读数，还是未来时间点的事件，ClickHouse 的日期和时间类型都能提供处理各类时间数据场景所需的灵活性。
丰富的数据类型选择使你能够在保持业务所需精度的前提下，同时优化存储空间和查询性能。

* 在大多数情况下，[`Date`](/sql-reference/data-types/date) 类型已经足够。该类型使用 2 个字节存储一个日期，取值范围为 `[1970-01-01, 2149-06-06]`。

* [`Date32`](/sql-reference/data-types/date32) 覆盖了更宽的日期范围。它使用 4 个字节存储一个日期，取值范围为 `[1900-01-01, 2299-12-31]`。

* [`DateTime`](/sql-reference/data-types/datetime) 以秒为精度存储日期时间值，取值范围为 `[1970-01-01 00:00:00, 2106-02-07 06:28:15]`。每个值占用 4 个字节。

* 在需要更高精度的场景下，可以使用 [`DateTime64`](/sql-reference/data-types/datetime64)。它允许以最高纳秒精度存储时间，取值范围为 `[1900-01-01 00:00:00, 2299-12-31 23:59:59.99999999]`。每个值占用 8 个字节。

下面我们创建一张用于存储多种日期类型的表：
