...
<VerticalStepper headerLevel="h2">
  ...
  可以在 [LlamaIndex &quot;LLMs&quot; 文档](https://docs.llamaindex.ai/en/stable/examples/)中找到配置凭据的相关说明。
  :::

  ## 初始化 MCP server \{#initialize-mcp-and-agent\}

  现在配置 ClickHouse MCP server，使其指向 ClickHouse SQL playground。
  你需要将这些 Python 函数转换成 Llama Index 工具：
  ...
  现在，您可以创建一个能够访问上述工具的 agent。将单次运行的最大工具调用次数设置为 10。如有需要，可自行修改此参数：
