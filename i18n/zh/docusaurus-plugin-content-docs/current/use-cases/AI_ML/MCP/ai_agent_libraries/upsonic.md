...
# 如何使用 Upsonic 和 ClickHouse MCP Server 构建 AI 智能体

在本指南中，你将学习如何构建一个 [Upsonic](https://github.com/Upsonic/Upsonic/tree/master) AI 智能体，使其可以通过 [ClickHouse 的 MCP Server](https://github.com/ClickHouse/mcp-clickhouse) 与
[ClickHouse 的 SQL playground](https://sql.clickhouse.com/) 进行交互。

:::note 示例 Notebook
该示例可以在 [examples 仓库](https://github.com/ClickHouse/examples/blob/main/ai/mcp/upsonic/upsonic.ipynb) 中作为 Notebook 找到。
:::
...
- 基于 ClickHouse 中的英国房价实付数据表,截至目前 2025 年已记录 376,633 笔交易,平均价格为
  £362,283,中位数价格为 £281,000。数据似乎仅包含 2025 年 1 月至 8 月的记录(因此 2025 年数据不完整)。存在极端
  离群值(最小值 £100,最大值 £127,700,000),导致平均值出现偏差。



我做了哪些计算（以及如何计算）
我在 ClickHouse 中对 uk.price-paid 表进行了聚合计算：
- 基于 uk.uk_price_paid_simple_partitioned 的 2025 年总体汇总（count、mean、median、min、max）
- 2025 年按月统计（transactions、mean、median）
- 2025 年按平均价格排序的高价城镇（交易量 ≥ 50 的城镇）
- 年度对比：2024 年 vs 2025 年（count、mean、median）
- 使用 uk.uk_price_paid 的 2025 年按房产类型统计（counts、avg、median）

关键数字（来自数据集）
- 2025 年总体（记录的交易）：transactions = 376,633；平均价格（mean）= £362,282.66；中位数价格（median）= £281,000；最小值（min）= £100；最大值（max）=
£127,700,000。
- 按月（2025）：（月份、交易量、平均价格、中位数价格）
  - 1 月：53,927 笔交易，平均 £386,053，中位数 £285,000
  - 2 月：58,740 笔交易，平均 £371,803，中位数 £285,000
  - 3 月：95,274 笔交易，平均 £377,200，中位数 £315,000
  - 4 月：24,987 笔交易，平均 £331,692，中位数 £235,000
  - 5 月：39,013 笔交易，平均 £342,380，中位数 £255,000
  - 6 月：41,446 笔交易，平均 £334,667，中位数 £268,500
  - 7 月：44,431 笔交易，平均 £348,293，中位数 £277,500
  - 8 月：18,815 笔交易，平均 £364,653，中位数 £292,999
  （数据集中仅包含 1–8 月。）
- 按平均价格排序的高价城镇（2025 年，交易量 ≥ 50 的城镇）
  ...
  - EAST MOLESEY、BEACONSFIELD、ESHER、CHALFONT ST GILES、THAMES DITTON 也位列前十（均为平均房价较高的通勤/富裕城镇）。
- 年度对比（按记录数据的 2024 vs 2025）
  - 2024 年：859,960 笔交易，平均 £390,879，中位数 £280,000
  - 2025 年：376,633 笔交易，平均 £362,283，中位数 £281,000
  （2025 年的交易笔数要低得多，因为数据集仅包含部分年度数据。）
- 按房产类型（2025）
  - detached：85,362 笔交易，平均 £495,714，中位数 £415,000
  - semi-detached：107,580 笔交易，平均 £319,922，中位数 £270,000
  - flat：62,975 笔交易，平均 £298,529，中位数 £227,000
  - terraced：112,832 笔交易，平均 £286,616，中位数 £227,000
  - other：7,884 笔交易，平均 £1,087,765（中位数 £315,000）— 注意小样本和离群值效应

重要注意事项和数据质量说明
- 2025 年的数据集似乎是不完整的（仅包含 1–8 月）。任何“2025 年”合计都不是全年数据。
- 存在较大的离群值（例如最大 £127.7M，最小 £100）。这些记录很可能包含数据录入错误或非标准记录，从而抬高均值。在这里，中位数通常是更稳健的度量。
- “other” 房产类型的平均值不稳定，因为计数较少、异质性强且存在离群值。
- 我没有按 is_new、duration 或其他元数据进行过滤；这些过滤条件会改变结果（例如排除新建房或租赁产权）。
- 这些表是 Price Paid 风格的交易记录（已记录成交），并不直接代表挂牌价或估值。
