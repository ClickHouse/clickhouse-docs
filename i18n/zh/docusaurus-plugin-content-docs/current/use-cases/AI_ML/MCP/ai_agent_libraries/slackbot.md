...
description: '学习如何构建一个可以与 ClickHouse MCP Server 交互的 SlackBot 代理。'
...
该机器人允许你在 Slack 中直接以自然语言查询 ClickHouse 中的数据。它使用
...
您可以按照 Slack 文档中["将应用添加到 Slack 工作区"](https://slack.com/intl/en-gb/help/articles/202035138-Add-apps-to-your-Slack-workspace)的说明进行操作。
...
在项目根目录创建一个 `.env` 文件,包含以下环境变量,以便您的应用程序连接到 [ClickHouse SQL 演练场](https://sql.clickhouse.com/)。
...
如果需要,您可以修改 ClickHouse 相关变量以使用您自己的 ClickHouse 服务器或 Cloud 实例。
...
机器人将在会话串中回复，如适用，会将所有先前的会话串消息作为上下文使用。
