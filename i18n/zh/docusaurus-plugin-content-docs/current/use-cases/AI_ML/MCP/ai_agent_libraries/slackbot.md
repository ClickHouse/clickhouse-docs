...
description: '了解如何构建一个能够与 ClickHouse MCP Server 交互的 SlackBot 代理。'
...
# 如何使用 ClickHouse MCP Server 构建 SlackBot 智能体

本指南将介绍如何构建一个 [SlackBot](https://slack.com/intl/en-gb/help/articles/202026038-An-introduction-to-Slackbot) 智能体。
通过该机器人，你可以在 Slack 中直接使用自然语言查询自己的 ClickHouse 数据。它基于
...
您可以按照 Slack 文档中["将应用添加到您的 Slack 工作区"](https://slack.com/intl/en-gb/help/articles/202035138-Add-apps-to-your-Slack-workspace)的说明进行操作。
...
在项目根目录下创建一个 `.env` 文件，包含以下环境变量，
以便您的应用程序连接到 [ClickHouse SQL 演练场](https://sql.clickhouse.com/)。
...
如果需要，您可以修改 ClickHouse 相关变量，
以使用您自己的 ClickHouse 服务器或 Cloud 实例。
...
2. **在 Slack 中:**
   ...
   - 在会话串中回复并提及机器人:`@yourbot how many contributions did these users make last week?`
   - 向机器人发送私信:`Show me all tables in the demo database.`

机器人将在会话串中回复,如果适用,会将所有先前的会话串消息作为上下文。

**会话串上下文:**
在会话串中回复时,机器人会加载所有先前的消息(当前消息除外)并将它们作为 AI 的上下文。

**工具使用:**
机器人仅使用通过 MCP 提供的工具(例如模式发现、SQL 执行),并且始终会显示所使用的 SQL 以及如何找到答案的摘要。
