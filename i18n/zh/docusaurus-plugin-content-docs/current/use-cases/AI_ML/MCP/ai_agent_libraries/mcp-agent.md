...
description: '了解如何使用 mcp-agent 和 ClickHouse MCP Server 构建 AI Agent'
...
在本指南中，您将学习如何构建一个 [mcp-agent](https://github.com/lastmile-ai/mcp-agent) AI 代理，使其能够通过 [ClickHouse 的 MCP Server](https://github.com/ClickHouse/mcp-clickhouse) 与
[ClickHouse 的 SQL 在线体验环境](https://sql.clickhouse.com/) 进行交互。

:::note 示例 notebook
该示例可作为 notebook 在 [examples 仓库](https://github.com/ClickHouse/examples/blob/main/ai/mcp/mcp-agent/mcp-agent.ipynb) 中找到。
:::
...
- 基于 ClickHouse 中的英国房价支付数据表,截至目前 2025 年已记录 376,633 笔交易,平均价格为 £362,283,中位数价格为 £281,000。数据似乎仅包含 2025 年 1 月至 8 月的记录(因此 2025 年数据不完整)。存在极端异常值(最小值 £100,最大值 £127,700,000),导致平均值偏高。
```
...
- 统计 2025 年的按月分布（交易量、均值、中位数）
- 统计 2025 年按平均价格排序的高价城镇（交易量 ≥ 50 的城镇）
- 年度对比：2024 vs 2025（count、mean、median）
- 使用 uk.uk_price_paid 按 2025 年房产类型分布统计（数量、avg、median）

关键数据（来自该数据集）
- 2025 年整体（已记录交易）：transactions = 376,633；mean price = £362,282.66；median price = £281,000；min = £100；max =
£127,700,000。
...
- 按平均价格排序的高价城镇（2025，交易量 ≥ 50 的城镇）
  ...
  - EAST MOLESEY、BEACONSFIELD、ESHER、CHALFONT ST GILES、THAMES DITTON 也在前 10 名之内（均为平均价格较高的通勤/富裕城镇）。
- 年度对比（2024 vs 2025，按已记录数据）
  - 2024：859,960 笔交易，mean £390,879，median £280,000
  - 2025：376,633 笔交易，mean £362,283，median £281,000
  （2025 年的计数要低得多，因为数据集只包含该年的一部分。）
- 按房产类型（2025）
  ...
  - other：7,884 笔交易，avg £1,087,765（median £315,000）— 注意小样本组与离群值效应

重要注意事项和数据质量说明
- 2025 年的数据集似乎是不完整的（仅包含 Jan–Aug 各月）。任何关于“2025”的总计都不是全年数据。
- 存在较大的离群值（例如，最大 £127.7M，最小 £100）。这些记录很可能包含录入错误或非标准记录，并会抬高
mean。median 在此场景下通常是更稳健的度量。
- “other” 房产类型的平均值不稳定，因为计数较少、分布不均且存在离群值。
- 我没有根据 is_new、duration 或其他元数据进行过滤；这些过滤条件可能会改变结果（例如排除新建房或
leasehold）。
- 这些表是 Price Paid 风格的交易记录（已成交销售）— 它们并不能直接代表挂牌价格或估值。
