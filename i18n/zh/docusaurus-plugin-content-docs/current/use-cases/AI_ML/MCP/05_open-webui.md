...
description: '本指南介绍如何使用 Docker 配置 Open WebUI 与 ClickHouse MCP 服务器。'
...
> 本指南介绍如何配置 [Open WebUI](https://github.com/open-webui/open-webui) 以使用 ClickHouse MCP 服务器,
...
您可以通过访问 http://localhost:8000/docs 查看已创建的端点列表

<Image img={Endpoints} alt='Open API 端点' size='md' />

要在 Open WebUI 中使用这些端点,需要导航到设置页面:
...
默认情况下,Open WebUI 使用 Ollama 模型,但我们也可以添加 OpenAI 兼容端点。
这些配置通过设置菜单完成,但这次我们需要点击 `Connections` 选项卡:
...
<Image
  img={AddConnection}
  alt='Open WebUI - 添加 OpenAI 连接'
  size='md'
/>

然后 OpenAI 模型将显示在顶部菜单中:
