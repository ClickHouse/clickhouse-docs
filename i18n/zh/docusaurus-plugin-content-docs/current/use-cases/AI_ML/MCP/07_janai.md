...
<VerticalStepper headerLevel="h2">
  ...
  在撰写本文时，MCP 服务器 仍然是 Jan.ai 中的实验性功能。
  可以通过打开实验性功能开关来启用它们：
  ...
  要安装 ClickHouse MCP 服务器，我们需要点击 `+` 图标，然后按如下方式填写表单：

  <Image img={MCPForm} alt="添加 MCP 服务器" size="md" />

  完成上述操作后，如果 ClickHouse MCP 服务器 还未启用，我们需要将其开关切换为启用：

  <Image img={MCPEnabled} alt="启用 MCP 服务器" size="md" />

  现在 ClickHouse MCP 服务器 的工具会出现在聊天对话框中：

  <Image img={MCPTool} alt="ClickHouse MCP 服务器 工具" size="md" />

  ## 使用 Jan.ai 与 ClickHouse MCP 服务器 对话 \{#chat-to-clickhouse-mcp-server\}
