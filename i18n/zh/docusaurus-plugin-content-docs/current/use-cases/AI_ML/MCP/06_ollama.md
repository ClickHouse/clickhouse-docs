...
> 本指南说明如何使用 ClickHouse MCP 服务器与 Ollama。
...
安装完成后,您可以通过以下方式将模型下载到本地机器:

```bash
ollama pull qwen3:8b
```

如果模型尚未存在,此命令将把模型下载到本地机器。
下载完成后,您可以通过以下方式运行模型:
...
不过,我们可以使用 [MCPHost](https://github.com/mark3labs/mcphost) 来运行 Ollama 模型与 MCP Servers。
...
二进制文件将安装在 `~/go/bin` 目录下,因此我们需要确保该目录在系统路径中。
...
:::note
理论上,您应该能够在 MCP 配置文件的 `environment` 键下提供这些变量,但我们发现这种方法无法正常工作。
:::
