...
1b. 具体取决于你使用的 MCP 客户端，你也可以直接编辑 JSON 配置
...
## 通过 OAuth 验证 ClickHouse Cloud 身份 {#authenticate-via-oauth}

1. 在首次会话时，Claude Code 会打开一个浏览器窗口。否则，您也可以在 Claude Code 中运行 `/mcp` 命令，并选择 `clickhouse_cloud` MCP 服务器来发起连接

2. 使用您的 ClickHouse Cloud 凭证进行身份验证
...
2. 恭喜!您现在可以从 Claude Code 使用 ClickHouse Cloud 远程 MCP 服务器了

<Image img={img7} alt='Claude Code MCP Usage' size='md' />

虽然本示例使用了 Claude Code,但您可以按照类似步骤使用任何支持 MCP 的 LLM 客户端。
