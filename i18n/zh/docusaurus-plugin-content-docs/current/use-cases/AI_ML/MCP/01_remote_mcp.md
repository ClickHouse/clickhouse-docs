...
> 本指南介绍如何启用和使用 ClickHouse Cloud 远程 MCP 服务器。本示例中我们将使用 Claude Code 作为 MCP 客户端,但您可以使用任何支持 MCP 的 LLM 客户端。
...
## 通过 OAuth 向 ClickHouse Cloud 进行身份验证 {#authenticate-via-oauth}

1. Claude Code 将在首次会话时打开浏览器窗口。您也可以在 Claude Code 中运行 `/mcp` 命令并选择 `clickhouse_cloud` MCP 服务器来触发连接
...
<Image img={img5} alt='Claude Code MCP 连接成功' size='md' />
