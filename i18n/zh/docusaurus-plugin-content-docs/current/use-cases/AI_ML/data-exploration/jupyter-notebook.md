...
description: '本指南介绍如何在 Jupyter Notebook 中安装和使用 chDB，从 ClickHouse Cloud 或本地文件中探索数据'
...
在本指南中，您将学习如何借助 [chDB](/chdb)——一个由 ClickHouse 驱动的高速进程内 SQL OLAP 引擎——在 Jupyter Notebook 中探索 ClickHouse Cloud 上的数据集。
...
我们将使用英国房产价格（UK Property Price）数据集，它在 ClickHouse Cloud 中作为入门数据集之一提供。
其中包含从 1995 年到 2024 年英国房屋成交价格的数据。
...
为了降低凭据泄露的风险,我们建议将您的 Cloud 用户名和密码设置为本地计算机上的环境变量。
在终端中运行以下命令将用户名和密码添加为环境变量:
...
浏览器将打开一个新窗口,显示 `localhost:8888` 上的 Jupyter 界面。
点击 `File` > `New` > `Notebook` 创建新的 Notebook。
...
现在您可以导入 chDB 并运行一个简单的查询来验证一切设置正确:
...
在 Jupyter notebook 中设置好英国房价支付数据集并运行 chDB 后,我们现在可以开始探索数据了。
...
编写以下查询以从您的 ClickHouse Cloud 服务获取英国房价支付数据并将其转换为 `pandas.DataFrame`:
...
根据数据量大小,此操作可能需要几秒钟。
在本例中,我们返回每年的平均价格,并按 `town='LONDON'` 进行筛选。
...
请注意，尽管在 ClickHouse 中 `date` 的类型是 `Date`，但在结果数据帧中，它的类型是 `uint16`。
chDB 在返回 DataFrame 时会自动推断最合适的类型。

现在我们已经以熟悉的形式获取了数据，让我们来看看伦敦房价是如何随时间变化的。

在一个新的单元格中运行以下命令，使用 matplotlib 绘制一张伦敦地区房价随时间变化的简单图表：
...
<details>
  ...
  也可以在一步中从多个数据源读取。可以使用下面这个带有 `JOIN` 的查询语句来实现：
...
尽管我们缺少 2020 年及之后的数据，但仍可以在 1995 年到 2019 年这段时间内对这两个数据集进行对比绘图。
在一个新的单元格中运行以下命令：
...
# 设置标题并每隔两年显示一次
plt.title('伦敦住房市场：销售量与价格随时间的变化', fontsize=14, pad=20)
...
与销售量不同,价格在2008年危机中几乎未受影响,并保持了上升趋势。令人惊叹!
```
...
使用英国房产价格数据集,我们展示了如何使用 `remoteSecure()` 函数查询远程 ClickHouse Cloud 数据,使用 `file()` 表引擎读取本地 CSV 文件,并将结果直接转换为 Pandas DataFrame 以进行分析和可视化。
通过 chDB,数据科学家可以利用 ClickHouse 强大的 SQL 功能,同时结合熟悉的 Python 工具(如 Pandas 和 matplotlib),从而轻松整合多个数据源进行综合分析。

虽然许多伦敦的数据科学家可能在短期内无法负担自己的住房,但至少他们可以分析这个将他们拒之门外的市场!
