...
description: '本指南介绍如何在 Marimo 笔记本中配置和使用 chDB，以探索来自 ClickHouse Cloud 或本地文件的数据'
...
在本指南中，您将学习如何借助 [chDB](/docs/chdb)（由 ClickHouse 驱动的高性能进程内 SQL OLAP 引擎），在 Marimo 笔记本中探索 ClickHouse Cloud 上的数据集。
...
:::tip
如果您还没有 ClickHouse Cloud 账户，可以[注册](https://console.clickhouse.cloud/signUp?loc=docs-marimo-chdb)试用，并获得价值 300 美元的免费额度开始使用。
:::
...
我们将使用 UK Property Price 数据集，该数据集作为入门数据集之一已在 ClickHouse Cloud 上提供。
它包含了 1995 年至 2024 年间英国房屋成交价格相关的数据。
...
为了降低凭据泄露的风险,我们建议您将 Cloud 用户名和密码作为环境变量添加到本地计算机。
...
将鼠标悬停在单元格上时，会出现两个带有"+"符号的圆圈。
点击这些圆圈即可添加新单元格。

添加一个新单元格并运行一个简单的查询，以检查所有配置是否正确：
...
在设置好英国房价数据集并在 Marimo notebook 中运行 chDB 后,我们现在可以开始探索数据了。
...
现在数据以熟悉的形式呈现给我们,让我们探索伦敦房产价格如何随时间变化。
...
现在,让我们查询所选年份的各个房产价格。
注意,这里我们不进行聚合——我们需要所有单独的交易记录来构建分布图:
...
title=f&#39;伦敦房价分布（{year_slider.value}）&#39;,
yaxis=dict(
title=&#39;价格（£）&#39;,
tickformat=&#39;,.0f&#39;
),
...
```
如果您点击单元格右上角的选项按钮,可以隐藏代码。
当您移动滑块时,图表会自动更新,这得益于 Marimo 的响应式执行:
...
我们以英国房产价格数据集为例,展示了如何使用 `remoteSecure()` 函数查询远程 ClickHouse Cloud 数据,并将查询结果直接转换为 Pandas DataFrame 进行分析和可视化。
借助 chDB 和 Marimo 的响应式执行模型,数据科学家可以充分利用 ClickHouse 强大的 SQL 能力,同时使用熟悉的 Python 工具(如 Pandas 和 Plotly),并通过交互式组件和自动依赖追踪功能,使探索性分析更加高效且易于复现。
