...
> 本指南介绍如何在 ClickHouse Cloud 控制台中启用和使用 AI 对话功能。
...
1. 您必须有权访问已启用 AI 功能的 ClickHouse Cloud 组织(如不可用,请联系您的组织管理员或支持团队)。


## 打开 AI 聊天面板 {#open-panel}

1. 导航至 ClickHouse Cloud 服务。
2. 在左侧边栏中,点击标有"Ask AI"的星形图标。
3. (快捷键) 按 <kbd>⌘</kbd> + <kbd>'</kbd> (macOS) 或 <kbd>Ctrl</kbd> + <kbd>'</kbd> (Linux/Windows) 切换打开状态。
...
1. 首次使用时,系统将显示一个同意对话框,说明数据处理方式和第三方 LLM 子处理器的相关信息。
2. 请审阅并接受以继续操作。如果拒绝,该面板将无法打开。
...
- **Agent**:基于 schema 和元数据的多步推理(服务必须处于活跃状态)。
...
1. 输入您的问题(例如"创建一个物化视图来按用户聚合每日事件")。
2. 按 <kbd>Enter</kbd> 键发送(使用 <kbd>Shift</kbd> + <kbd>Enter</kbd> 键换行)。
3. 在模型处理过程中,您可以点击"停止"按钮来中断。
