...
<details>
  ...
  <Image size="md" img={s3_3} alt="跳过为 IAM 用户分配用户组" border force />

  5. 除非您需要添加任何标签，否则选择 `Next: Review`
  ...
  <Image size="md" img={s3_5} alt="创建 IAM 用户（显示无权限警告）" border force />

  7. 用户已创建；单击 `show` 并复制 access key 和 secret key。
  ...
  <Image size="md" img={s3_a} alt="在阻止公共访问的情况下配置 S3 存储桶设置" border force />

  4. 在页面底部选择 `Create Bucket`

  <Image size="md" img={s3_b} alt="完成 S3 存储桶的创建" border force />
  ...
  <Image size="md" img={s3_d} alt="在 S3 存储桶中创建一个新文件夹" border force />
  ...
  10. 选中新建文件夹的复选框，点击 `Copy URL`，并将复制的 URL 保存下来，以便在下一节的 ClickHouse 存储配置中使用。
