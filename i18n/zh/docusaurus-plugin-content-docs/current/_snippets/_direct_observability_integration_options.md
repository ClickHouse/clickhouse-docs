...
:::note
由于与用于节省成本的空闲优化行为不兼容，并且受到 Cloud 代理层操作限制，不建议在 ClickHouse Cloud 部署中使用此集成。
:::

### 直接使用系统表 {#system-tables}

你可以通过连接 ClickHouse 系统表（尤其是 `system.query_log`）并直接进行查询来执行深入的查询性能分析。使用 SQL 控制台或 `clickhouse` 客户端，你的团队可以识别慢查询、分析资源使用情况，并在整个组织范围内跟踪使用模式。
