...
<details>
  ...
  在 ClickHouse Cloud 服务列表中选择你要使用的服务，并切换到 **Settings**。如果 IP 访问列表中不包含需要连接到你的 ClickHouse Cloud 服务的远程系统的 IP 地址或地址范围，则可以通过 **Add IPs** 来解决该问题：

  <Image size="md" img={ip_allow_list_check_list} alt="检查服务的 IP 访问列表中是否允许来自你 IP 地址的流量" border />

  添加需要连接到你的 ClickHouse Cloud 服务的单个 IP 地址或地址范围。根据需要修改表单，然后单击 **Save**。
