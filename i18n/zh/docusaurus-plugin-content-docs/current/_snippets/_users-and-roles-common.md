...
以 `default` 用户身份登出,然后以 `clickhouse_admin` 用户身份重新登录。
...
用户应具有必要的权限,而不应全部设为管理员用户。本文档的其余部分提供了示例场景及所需的角色配置。
...
##### 创建一个角色,将此角色的用户限制为仅能查看数据库 `db1` 中 `table1` 的 `column1`: {#create-column-role}
...
##### 创建一个角色,将此角色的用户限制为仅能查看选定的数据行,在本例中,仅查看 `column1` 中包含 `A` 的行 {#create-row-role}
...
##### 为其他角色授予显式权限以保持对所有行的访问 {#grant-other-roles-access}
    ...
    :::note
    将策略附加到表时,系统将应用该策略,只有已定义的用户和角色能够对表执行操作,其他所有用户的任何操作都将被拒绝。为了避免将限制性行策略应用于其他用户,必须定义另一个策略来允许其他用户和角色拥有常规访问权限或其他类型的访问权限。
    :::
...
##### 测试查询所有列的 `SELECT` 语句 {#test-select-all-columns}
...
:::note
访问被拒绝,因为查询指定了所有列,而该用户仅有权访问 `id` 和 `column1` 列
:::

##### 验证仅查询指定且允许访问的列的 `SELECT` 语句: {#verify-allowed-columns}
...
:::note
验证仅返回上述两行,`column1` 列值为 `B` 的行应被排除。
:::
...
用户可以被分配多个角色以组合所需的权限。当使用多个角色时,系统会组合这些角色来确定权限,最终效果是角色权限会累加生效。
...
在某些情况下,权限的交叉或组合可能会产生意外结果。可以使用管理员账户执行以下命令来定位问题
...
### 从角色中取消用户分配 {#unassign-a-user-from-a-role}
