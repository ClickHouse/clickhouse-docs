...
在互操作性设置对话框中，推荐 IAM 角色 **Storage Object Admin**；在第二步中选择该角色。

<Image size="md" img={GCS_create_service_account_2} alt="在 GCS 中选择 IAM 角色 Storage Object Admin" border />

第三步是可选的，本指南中不使用。您可以根据政策允许用户拥有这些权限。

<Image size="md" img={GCS_create_service_account_3} alt="为 GCS 中的新服务账户配置其他设置" border />

服务账户 HMAC 密钥将被显示。请保存此信息，因为它将在 ClickHouse 配置中使用。
