...
外部集成使组织能够保持既有的监控工作流程,充分利用团队对熟悉工具的专业知识,并将 ClickHouse 监控与更广泛的基础设施可观测性相集成,而无需中断当前流程或投入大量的重新培训成本。
...
ClickStack 是 ClickHouse 推荐的可观测性解决方案,用于深度系统分析和调试,使用 ClickHouse 作为存储引擎,为日志、指标和追踪提供统一平台。该方法依赖于 HyperDX(ClickStack 的用户界面),直接连接到您的 ClickHouse 实例内的系统表。
HyperDX 提供了一个专注于 ClickHouse 的仪表板,包含 Selects、Inserts 和 Infrastructure 选项卡。团队还可以使用 Lucene 或 SQL 语法搜索系统表和日志,并通过 Chart Explorer 创建自定义可视化以进行详细的系统分析。
