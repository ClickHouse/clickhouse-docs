...
keywords: ['JSON 导入', 'JSONEachRow 格式', '数据导入', 'JSON 导入', '数据格式']
doc_type: 'guide'
---
...
ClickHouse 支持多种[输入和输出数据格式](/interfaces/formats)。其中包含多种 JSON 变体，但用于数据摄取最常用的是 [JSONEachRow](/interfaces/formats/JSONEachRow)。该格式要求每行包含一个 JSON 对象，对象之间用换行符分隔。
...
- `input_format_skip_unknown_fields` 允许插入 JSON 数据,即使其中包含表结构中不存在的额外字段(通过丢弃这些字段)。
- `input_format_import_nested_json` 允许将嵌套的 JSON 对象插入到 [Nested](../../sql-reference/data-types/nested-data-structures/index.md) 类型的列中。

:::note
设置在 HTTP 接口中通过 `GET` 参数指定,在 `CLI` 接口中作为以 `--` 为前缀的额外命令行参数指定。
:::
