...
如果您决定不遵循建议并在 ClickHouse 上运行一些类键值查询,以下是一些建议:

- 点查询在 ClickHouse 中开销较大的关键原因是其主要 [MergeTree 表引擎家族](../..//engines/table-engines/mergetree-family/mergetree.md)的稀疏主索引。该索引无法指向每一行具体数据,而是指向每第 N 行,系统必须从相邻的第 N 行扫描到目标行,在此过程中读取大量额外数据。在键值场景中,通过 `index_granularity` 设置减小 N 的值可能会有所帮助。
- ClickHouse 将每个列保存在单独的文件集中,因此要组装一个完整的行,需要遍历所有这些文件。文件数量随列数线性增长,因此在键值场景中,应避免使用过多列,而是将所有数据载荷放入单个 `String` 列中,并使用某种序列化格式(如 JSON、Protobuf 或其他合适的格式)进行编码。
