...
列式数据库是分析型应用的首选，因为它允许在表中设置大量列以备不时之需，但在执行读取查询时，并不会为未使用的列付出代价 (传统 OLTP 数据库在查询时会读取所有数据，因为数据是按行而不是按列存储的) 。面向列的数据库是为大数据处理和数据仓库而设计的，通常可以通过由低成本硬件组成的分布式集群进行原生扩展以提高吞吐量。ClickHouse 通过[distributed](../../engines/table-engines/special/distributed.md)表和[replicated](../../engines/table-engines/mergetree-family/replication.md)表的组合来实现这一点。
