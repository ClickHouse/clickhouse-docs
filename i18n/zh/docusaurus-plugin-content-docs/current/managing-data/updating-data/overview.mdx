...
在处理更新时，分析型数据库与事务型数据库由于其底层设计理念和目标使用场景的不同而存在显著差异。
例如，PostgreSQL 是一种行式、符合 ACID 的关系型数据库，支持强大且具事务性的更新和删除操作，并通过多版本并发控制（[MVCC](https://en.wikipedia.org/wiki/Multiversion_concurrency_control)）等机制来确保数据的一致性和完整性。
这使得即便在高并发环境下，也能安全可靠地进行数据修改。

相反，ClickHouse 是一种列式数据库，针对读取密集型分析场景以及高吞吐的仅追加写入操作进行了优化。
虽然它原生支持就地更新和删除，但必须谨慎使用以避免高 I/O。
作为替代方案，可以通过重构表结构，将删除和更新转换为追加操作，使其以异步方式和/或在读取时处理，从而体现出其更侧重于高吞吐数据摄取和高效查询性能，而非实时数据变更。
...
| 方法 | 语法 | 使用场景 |
|--------------------------------------------------------------|--------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Update 变更](/sql-reference/statements/alter/update)    | `ALTER TABLE [table] UPDATE`   | 当数据必须立即更新到磁盘时使用（例如，为了满足合规要求）。会对 `SELECT` 性能产生负面影响。 |
| [On-the-fly updates](/guides/developer/on-the-fly-mutations) | `ALTER TABLE [table] UPDATE`   | 用于更新少量数据。后续所有 `SELECT` 查询都会立即返回更新后的数据，但这些行在磁盘上最初仅在内部被标记为已更新。通过 `SET apply_mutations_on_fly = 1;` 启用。 |
| [Lightweight updates](/sql-reference/statements/update)      | `UPDATE [table] SET ... WHERE` | 用于更新少量数据（最多约占整张表的 10%）。创建补丁分区片段以实现立即可见，而无需重写整个列。会为 `SELECT` 查询增加开销，但延迟可预测。 |
...
| [实时变更](/guides/developer/on-the-fly-mutations) | 可变 | 可变 | 结果即时可见；若累计大量更新则性能下降 |
