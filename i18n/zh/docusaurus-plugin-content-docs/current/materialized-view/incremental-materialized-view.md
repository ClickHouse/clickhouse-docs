...
增量物化视图（Materialized Views）允许你将计算成本从查询时转移到插入时，从而使 `SELECT` 查询更快。
...
在 ClickHouse 中，得益于 [`toStartOfDay`](/sql-reference/functions/date-time-functions#toStartOfDay) 函数，这个查询相对简单：
...
为了演示我们的物化视图，假设当前 `votes` 表为空，尚未接收到任何数据。我们的物化视图会对插入到 `votes` 的数据执行上述 `SELECT`，并将结果写入 `up_down_votes_per_day`：
...
这里的 `TO` 子句是关键，它表示结果将被写入到的目标表，即 `up_down_votes_per_day`。

我们可以使用之前的插入操作重新填充我们的 votes 表：
...
完成后，我们可以确认 `up_down_votes_per_day` 的行数——它应该是每天一行记录：
...
Null 表引擎是一种强大的优化手段——可以把它看作 `/dev/null`。当我们的 `posts_null` 表在插入行数据时接收到行数据，物化视图会计算并存储汇总统计信息——它本质上就像一个触发器。不过，原始数据不会被存储。虽然在我们的场景中，我们很可能仍然希望保存原始帖子，但这种方法可以在避免保存原始数据所带来的存储开销的同时，用于计算聚合结果。
...
在选择 ClickHouse 排序键时，你应当考虑数据的访问模式。应优先选择那些在过滤和聚合子句中经常被使用的列。对于那些访问模式更加多样、无法用同一组列来概括的场景，这种做法可能会比较受限。例如，考虑下面的 `comments` 表：
...
现在，如果该用户获得一个新徽章并插入一行数据，那么我们的视图将会更新：
...
在这种情况下，该视图仅会在插入 badge 但对应的用户行尚不存在时执行。若我们为该用户再插入一个 badge，就会插入一行记录，这与预期一致：
...
- **预先插入已 JOIN 的数据。** 确保在向源表插入行之前，JOIN 关联的表中的数据已经存在。JOIN 在插入时进行计算，因此缺失数据会导致行无法匹配或出现 null。

- **限制从 JOIN 中读取的列。** 仅从 JOIN 的表中选择所需列，以最小化内存使用并减少插入时的延迟（见下文）。
...
- **考虑插入量与插入频率。** 在中等插入负载下，JOIN 表现良好。对于高吞吐量摄取场景，考虑使用中间表、预先进行 JOIN，或其他方案，例如 Dictionaries 和 [Refreshable Materialized Views](/materialized-view/refreshable-materialized-view)。

### 在过滤和 JOIN 中使用源表 {#using-source-table-in-filters-and-joins-in-materialized-views}

在 ClickHouse 中使用物化视图时，非常重要的是要理解源表在物化视图查询执行期间是如何处理的。具体来说，物化视图查询中的源表会被替换为正在插入的数据块。如果没有正确理解这一行为，可能会导致一些出乎意料的结果。
...
在这个示例中，由子查询 `IN (SELECT id FROM t0)` 构建的集合只包含新插入的行，因此可以用来过滤 `t1`。
...
使用上述方法，我们可以优化该视图。接下来在 `users` 表上添加一个过滤条件，仅保留插入徽章行中出现的用户 ID：
...
在上述操作中，对于用户 ID `2936484`，只会从 `users` 表中检索出一行数据。该查找同样通过将 `Id` 作为表的排序键进行了优化。
...
假设我们希望创建一个统一的用户活动视图，通过合并这两个表以展示每个用户最近一次的活动：
...
假设我们有一个目标表用于接收此查询的结果。请注意这里使用了 [AggregatingMergeTree](/engines/table-engines/mergetree-family/aggregatingmergetree) 表引擎和 [AggregateFunction](/sql-reference/data-types/aggregatefunction) 数据类型，以确保结果被正确合并：
...
如果希望在向 `badges` 或 `comments` 中插入新行时，这个表能够自动更新，一种较为朴素的做法是尝试使用前面的 `UNION ALL` 查询来创建一个物化视图：
...
虽然这在语法上是正确的，但会产生非预期的结果——该视图只会在向 `comments` 表插入时被触发。例如：
...
向 `badges` 表插入数据不会触发该视图，从而导致 `user_activity` 无法接收更新：
...
为了解决这个问题，我们只需为每个 `SELECT` 语句创建一个 materialized view：
...
现在向任意一个表插入数据都会得到正确的结果。例如，如果我们向 `comments` 表插入数据：
...
同样，对 `badges` 表的插入也会反映到 `user_activity` 表中：
...
相反，考虑在启用 `parallel_view_processing=1` 的情况下向表中插入一行时会发生什么。启用该设置后，视图会并行执行，因此无法保证行到达目标表的顺序：
...
虽然在这里严格来说不需要使用 CTE，但出于示例目的，该视图仍然可以按预期工作：
