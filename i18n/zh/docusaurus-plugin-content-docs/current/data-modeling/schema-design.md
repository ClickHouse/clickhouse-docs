...
title: '模式设计'
description: '优化 ClickHouse 表结构以提升查询性能'
...
理解高效的模式设计是优化 ClickHouse 性能的关键，其中的诸多设计选择往往涉及取舍，且最优方案取决于所要支持的查询类型，以及数据更新频率、延迟要求和数据量等因素。本指南概述了用于优化 ClickHouse 性能的模式设计最佳实践和数据建模方法。


## Stack Overflow 数据集 {#stack-overflow-dataset}

在本指南的示例中,我们使用 Stack Overflow 数据集的一个子集。该数据集包含 2008 年至 2024 年 4 月期间 Stack Overflow 上的所有帖子、投票、用户、评论和徽章。这些数据以 Parquet 格式存储在 S3 存储桶 `s3://datasets-documentation/stackoverflow/parquet/` 下,采用以下模式:
...
ClickHouse 提供了表结构推断功能,可以自动识别数据集的类型。该功能支持所有数据格式,包括 Parquet。我们可以利用此功能,通过 s3 表函数和 [`DESCRIBE`](/sql-reference/statements/describe-table) 命令来识别数据的 ClickHouse 类型。请注意,下面我们使用通配符模式 `*.parquet` 来读取 `stackoverflow/parquet/posts` 文件夹中的所有文件。
...
我们指定了 MergeTree 作为表引擎。MergeTree 是您最常使用的 ClickHouse 表引擎。它是 ClickHouse 工具箱中的多面手,能够处理 PB 级数据,适用于大多数分析场景。其他表引擎则适用于诸如 CDC 等需要支持高效更新的场景。

`ORDER BY ()` 子句表示我们没有索引,更具体地说,数据没有排序。稍后会详细介绍。目前只需知道所有查询都需要进行全表扫描。
...
> 上述查询会加载 6000 万行数据。对于 ClickHouse 来说这算较小规模，但网络连接较慢的用户可能希望只加载一部分数据。只需通过 glob 模式指定要加载的年份即可，例如 `https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2008.parquet` 或 `https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/{2008, 2009}.parquet`。有关如何使用 glob 模式来定位部分文件，请参阅[此处](/sql-reference/table-functions/file#globs-in-path)。
...
磁盘上的数据越少意味着 I/O 越少,从而使查询和插入速度更快。在大多数情况下,任何压缩算法在 CPU 方面的开销都会被 I/O 减少所带来的收益所抵消。因此,在确保 ClickHouse 查询快速执行时,改进数据压缩应该是首要关注点。
...
ClickHouse 中的压缩受 3 个主要因素影响:排序键、数据类型和使用的编解码器。所有这些都通过表结构进行配置。

通过简单的类型优化过程,可以获得压缩和查询性能的最大初始改进。可以应用以下几条简单规则来优化表结构:

- **使用严格类型** - 我们的初始表结构对许多明显是数值的列使用了字符串类型。使用正确的类型将确保在过滤和聚合时具有预期的语义。这同样适用于日期类型,这些类型已在 Parquet 文件中正确提供。
- **避免可空列** - 默认情况下,上述列被假定为可空。Nullable 类型允许查询区分空值和 Null 值。这会创建一个单独的 UInt8 类型列。每次用户处理可空列时都必须处理这个额外的列。这会导致使用额外的存储空间,并且几乎总是对查询性能产生负面影响。仅当类型的默认空值与 Null 之间存在差异时才使用 Nullable。例如,对于 `ViewCount` 列中的空值使用值 0 可能对大多数查询来说已经足够,并且不会影响结果。如果空值需要被区别对待,通常也可以通过过滤器将它们从查询中排除。
- **对数值类型使用最小精度** - ClickHouse 具有多种为不同数值范围和精度设计的数值类型。始终致力于最小化用于表示列的位数。除了不同大小的整数(例如 Int16)之外,ClickHouse 还提供最小值为 0 的无符号变体。这些可以允许列使用更少的位,例如 UInt16 的最大值为 65535,是 Int16 的两倍。如果可能,优先使用这些类型而不是更大的有符号变体。
- **日期类型使用最小精度** - ClickHouse 支持多种日期和日期时间类型。Date 和 Date32 可用于存储纯日期,后者支持更大的日期范围,但代价是使用更多位。DateTime 和 DateTime64 提供对日期时间的支持。DateTime 限于秒级粒度并使用 32 位。DateTime64 如其名称所示,使用 64 位但提供高达纳秒级粒度的支持。一如既往,选择查询可接受的更粗粒度版本,最小化所需的位数。
...
- **使用枚举进行数据验证** - Enum 类型可用于高效编码枚举类型。根据需要存储的唯一值数量,枚举可以是 8 位或 16 位。如果您需要在插入时进行相关验证(未声明的值将被拒绝),或者希望执行利用枚举值自然排序的查询,可以考虑使用此类型,例如想象一个包含用户响应的反馈列 `Enum(':(' = 1, ':|' = 2, ':)' = 3)`。
...
| 列                       | 是否为数值 | 最小值, 最大值                                                     | 唯一值      | Null 值 | 注释                                                     | 优化类型                                                                                                                                                         |
| ----------------------- | ----- | ------------------------------------------------------------ | -------- | ------ | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| `CreationDate`          | 否     | 2008-07-31 21:42:52.667000000, 2024-03-31 23:59:17.697000000 | *        | 否      | 如果不需要毫秒级粒度，请使用 DateTime                                | DateTime                                                                                                                                                     |
| `分数`                    | 是     | -217, 34970                                                  | 3236     | 否      |                                                        | Int32                                                                                                                                                        |
| `ViewCount`             | 是     | 2, 13962748                                                  | 170867   | 否      |                                                        | UInt32                                                                                                                                                       |
| `正文`                    | 否     | -                                                            | *        | 否      |                                                        | 字符串                                                                                                                                                          |
| `OwnerUserId`           | 是     | -1, 4056915                                                  | 6256237  | 是      |                                                        | Int32                                                                                                                                                        |
| `OwnerDisplayName`      | 否     | -                                                            | 181251   | 是      | 将 Null 视为一个空字符串                                        | 字符串                                                                                                                                                          |
...
| `CommunityOwnedDate`    | 否     | 2008-08-12 04:59:35.017000000, 2024-04-01 05:36:41.380000000 | -        | 是      | 对于 Null 值,建议使用默认值 1970-01-01。如果不需要毫秒精度,请使用 DateTime 类型 | DateTime                                                                                                                                                     |
| `ClosedDate`            | 否     | 2008-09-04 20:56:44, 2024-04-06 18:49:25.393000000           | *        | 是      | 对于 Null 值,建议使用默认值 1970-01-01。如果不需要毫秒精度,请使用 DateTime 类型 | DateTime                                                                                                                                                     |
...
我们可以使用一个简单的 `INSERT INTO SELECT` 来填充该表，从之前的表中读取数据并将其插入到当前表中：

```sql
INSERT INTO posts_v2 SELECT * FROM posts

返回 0 行。用时：146.471 秒。已处理 5982 万行，83.82 GB（40.84 万行/秒，572.25 MB/秒）
```

在新的模式中我们不保留任何 `null`。上面的 `INSERT` 会将这些值隐式转换为其各自类型的默认值——整数为 0，字符串为空值。ClickHouse 还会自动将所有数值类型转换为其目标精度。
...
- 选择与常用过滤条件对应的列。如果某列在 `WHERE` 子句中频繁使用,则优先将这些列包含在键中,而不是那些使用频率较低的列。
...
返回 10 行。耗时:0.070 秒。处理了 5982 万行,569.21 MB(每秒 8.5255 亿行,8.11 GB/s)。
峰值内存使用:429.38 MiB。
```
...
在我们的场景中，我们希望用户始终通过 `PostTypeId` 进行过滤。它的基数为 8，是作为排序键首个条目的合理选择。我们意识到按日期粒度进行过滤通常就足够了（同时 datetime 过滤仍然会受益），因此将 `toDate(CreationDate)` 作为键的第二个组件。这样也会生成更小的索引，因为一个日期可以用 16 来表示，从而加快过滤。我们最后一个键条目是 `CommentCount`，用于帮助查找评论最多的帖子（最终排序）。
...
如果你对通过使用特定数据类型和合适的排序键所带来的压缩效果提升感兴趣，请参阅 [ClickHouse 中的压缩](/data-compression/compression-in-clickhouse)。如果还需要进一步提升压缩率，我们也推荐阅读章节 [选择合适的列压缩编解码器](/data-compression/compression-in-clickhouse#choosing-the-right-column-compression-codec)。
...
在下面列出的其他指南中,我们将探讨多种技术来重构更广泛的数据模式,以实现 ClickHouse 查询的最佳性能。在整个过程中,我们的目标是让 `Posts` 保持为中心表,大多数分析查询都通过它来执行。虽然其他表仍然可以单独查询,但我们假设大多数分析都需要在 `posts` 的上下文中进行。

> 在本节中,我们使用其他表的优化版本。虽然我们提供了这些表的模式定义,但为了简洁起见,我们省略了具体的决策过程。这些决策基于前面描述的规则,我们将推断这些决策的工作留给读者。

以下方法都旨在最小化 JOIN 的使用需求,从而优化读取并提高查询性能。虽然 ClickHouse 完全支持 JOIN,但我们建议谨慎使用(JOIN 查询中包含 2 到 3 张表是可以接受的),以实现最佳性能。

> ClickHouse 没有外键的概念。这并不禁止执行连接操作,但意味着引用完整性需要由用户在应用程序层面进行管理。在像 ClickHouse 这样的 OLAP 系统中,数据完整性通常在应用程序层面或数据摄取过程中进行管理,而不是由数据库本身强制执行,因为后者会产生显著的性能开销。这种方法提供了更大的灵活性和更快的数据插入速度。这与 ClickHouse 专注于超大数据集的读取和插入查询的速度和可扩展性的理念相一致。
