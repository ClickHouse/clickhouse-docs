...
在 ClickHouse 中，数据反规范化是一种利用扁平化表结构来减少 `JOIN` 操作、从而降低查询延迟的技术。
...
此过程减少了查询时对复杂连接的需求,可以显著加快读取操作速度,使其非常适合具有大量读取需求和复杂查询的应用程序。然而,它可能会增加写入操作和维护的复杂性,因为对重复数据的任何更改都必须传播到所有实例以保持一致性。
...
通常情况下,我们建议在以下场景中进行反规范化:

- 对变更频率较低的表进行反规范化,或者可以容忍数据在分析查询中可用之前存在延迟的表,即数据可以通过批处理完全重新加载。
- 避免对多对多关系进行反规范化。这可能导致单个源行发生变更时需要更新大量行。
- 避免对高基数关系进行反规范化。如果表中的每一行在另一个表中有数千个相关条目,这些条目需要表示为 `Array` ——可以是基本类型或元组。通常不建议使用包含超过 1000 个元组的数组。
...
作为一种方案,它面临一个主要挑战——写入性能和数据更新。更具体地说,反规范化实际上将数据关联的责任从查询时转移到了数据摄取时。虽然这可以显著提高查询性能,但会使数据摄取变得复杂,并且意味着如果用于组成某行的任何源行发生变化,数据管道就需要将该行重新插入 ClickHouse。这可能意味着源表中一行的更改会导致 ClickHouse 中的多行需要更新。在复杂的模式中,如果行是由复杂关联组成的,关联的嵌套组件中单行的更改可能意味着需要更新数百万行。

由于以下两个挑战,实时实现这一点往往不切实际,并且需要大量的工程工作:

1. 当表行发生变化时触发正确的关联语句。理想情况下,这不应导致关联的所有对象都被更新——而只是那些受到影响的对象。修改关联以高效地过滤到正确的行,并在高吞吐量下实现这一点,需要外部工具或工程支持。
...
_对于以下每个示例,假设存在一个需要在连接中使用两个表的查询。_

### Posts 和 Votes {#posts-and-votes}

帖子的投票表示为单独的表。下面显示了优化的模式以及加载数据的插入命令:
...
`PostLinks` 连接用户认为相关或重复的 `Posts`。以下查询显示了表结构和加载命令:
...
在大多数情况下,反规范化需要向父行添加单个列或统计信息。例如,我们可能只是希望用重复帖子的数量来丰富帖子数据,这时只需添加一个列即可。
...
作为示例,我们在下面演示将 `PostLinks` 反规范化到 `Posts` 上。

每个帖子可以包含多个指向其他帖子的链接,如前面 `PostLinks` 模式所示。作为 Nested 类型,我们可以如下表示这些链接帖子和重复帖子:
...
> 注意这里的执行时间。我们在大约 2 分钟内成功反规范化了 6600 万行数据。正如我们稍后将看到的,这是一个可以调度的操作。


请注意这里使用了 `groupArray` 函数，在进行关联之前，将 `PostLinks` 按每个 `PostId` 聚合成一个数组。然后对该数组进行过滤，得到两个子列表：`LinkedPosts` 和 `DuplicatePosts`，同时排除外连接产生的任何空结果。

我们可以选取几行记录来查看新的反规范化结构：
...
我们在上文中展示了如何在通过 `INSERT INTO SELECT` 加载数据后使用 ClickHouse 执行此转换。这适用于周期性批量转换。

假设可以接受周期性批量加载过程,用户在 ClickHouse 中有多种编排选项:

- **[可刷新物化视图](/materialized-view/refreshable-materialized-view)** - 可刷新物化视图可用于周期性调度查询,并将结果发送到目标表。在查询执行时,视图确保目标表以原子方式更新。这提供了一种 ClickHouse 原生的调度方式。
...
用户也可以选择在插入之前,在 ClickHouse 外部使用 [Apache Flink](https://flink.apache.org/) 等流式处理技术执行此操作。或者,可以使用增量[物化视图](/guides/developer/cascading-materialized-views)在数据插入时执行此过程。
