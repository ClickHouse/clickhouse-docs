...
无论是刚开始使用 ClickHouse，还是负责现有部署，用户几乎都不可避免地需要使用历史数据对表进行回填。在某些情况下，这相对简单，但在需要填充物化视图时，情况可能会更加复杂。本指南介绍了一些可用于完成该任务的流程，用户可以根据自身场景进行应用。

:::note
本指南假定用户已熟悉 [增量物化视图](/materialized-view/incremental-materialized-view) 的概念，以及[使用 s3 和 gcs 等表函数进行数据加载](/integrations/s3)。我们也建议用户阅读关于[从对象存储优化写入性能](/integrations/s3/performance)的指南，其中的建议可应用于本指南中的写入操作。
:::
...
本指南使用 PyPI 数据集。数据集中的每一行代表使用 `pip` 等工具下载 Python 包的一次记录。
...
我们假设用户正在消费此数据流(例如从 Kafka 或对象存储),用于此日期之后的数据。该数据的 schema 如下所示:
...
在所有场景中,我们都依赖"副本表和视图"这一概念。这些表和视图是用于实时流数据的表和视图的副本,允许在隔离环境中执行回填操作,并在发生故障时提供便捷的恢复方式。例如,我们有以下主 `pypi` 表和物化视图,用于计算每个 Python 项目的下载次数:
...
如果在第二次加载过程中的任意阶段发生故障，我们可以直接[截断](/managing-data/truncate) `pypi_v2` 和 `pypi_downloads_v2`，然后重新执行数据加载。

在数据加载完成后，我们可以使用 [`ALTER TABLE MOVE PARTITION`](/sql-reference/statements/alter/partition#move-partition-to-table) 子句，将数据从副本表移动到主表中。
...
:::note 分区名称
上面的 `MOVE PARTITION` 调用使用了分区名 `()`. 这表示该表只有一个分区（该表本身未分区）。对于已分区的表，用户需要多次调用 `MOVE PARTITION`——每个分区调用一次。当前分区的名称可以从 [`system.parts`](/operations/system-tables/parts) 表中获取，例如：`SELECT DISTINCT partition FROM system.parts WHERE (table = 'pypi_v2')`.
:::

现在我们可以确认 `pypi` 和 `pypi_downloads` 已包含完整数据。可以安全地删除 `pypi_downloads_v2` 和 `pypi_v2`。
...
返回 1 行。用时:0.007 秒。处理了 19.164 万行,1.53 MB(2734 万行/秒,218.74 MB/秒)。

SELECT count()
FROM pypi_v2
```

重要的是，`MOVE PARTITION` 操作既是轻量级的（利用硬链接），又是原子的，即要么失败，要么成功，不会出现中间状态。

我们在后文的回填场景中大量使用了这一过程。

请注意，此过程要求用户确定每次插入操作的大小。

更大的插入批次（即包含更多行）意味着所需的 `MOVE PARTITION` 操作会更少。不过，这需要与插入失败（例如由于网络中断）时进行恢复的成本进行权衡。用户可以通过将文件分批处理来降低风险，以配合这一过程。可以通过范围查询（例如 `WHERE timestamp BETWEEN 2024-12-17 09:00:00 AND 2024-12-17 10:00:00`）或通配模式来完成。例如，
...
:::note
当从对象存储加载数据时，ClickPipes 会采用这种方法，自动创建目标表及其物化视图的副本，从而避免用户需要手动执行上述步骤。通过同时使用多个工作线程，每个线程处理不同的子集（通过 glob 模式）并各自使用独立的副本表，可以在保证“精确一次”语义的前提下高速加载数据。感兴趣的读者可以在[这篇博客](https://clickhouse.com/blog/supercharge-your-clickhouse-data-loads-part3)中了解更多细节。
:::
...
如果历史数据位于独立的存储桶中,则不需要上述时间过滤器。如果没有时间或单调列可用,请将您的历史数据隔离存放。
...
在已经填充了大量数据并且正在持续插入数据的环境中,需要添加新的物化视图是很常见的情况。此时,可以用来标识数据流中某个时间点的时间戳或单调递增列非常有用,并且可以避免数据摄取的暂停。在下面的示例中,我们假设这两种情况都存在,优先选择避免摄取暂停的方法。
...
2. 运行 `INSERT INTO SELECT` 查询,使用视图的聚合查询从源表读取数据,插入到物化视图的目标表中。

可以进一步增强此方法,在步骤 (2) 中针对数据子集进行操作,和/或为物化视图使用重复的目标表(插入完成后将分区附加到原始表),以便在失败后更容易恢复。
...
最简单的方法是在主表上运行物化视图的查询,使用过滤器忽略最近添加的数据,通过 `INSERT INTO SELECT` 将结果插入到视图的目标表中。例如,对于上述视图:
...
在我们的案例中,这是一个相对轻量级的聚合操作,在 3 秒内完成,使用不到 600MiB 的内存。对于更复杂或运行时间更长的聚合操作,用户可以通过使用前面提到的重复表方法来提高此过程的容错性,即创建一个影子目标表,例如 `pypi_downloads_per_day_v2`,向其中插入数据,然后将其生成的分区附加到 `pypi_downloads_per_day`。
...
方法 (1) 是最简单的方法,通常已经足够。为简洁起见,我们不包含示例。

我们在下面进一步探讨方法 (2)。

#### 使用 Null 表引擎填充物化视图 {#using-a-null-table-engine-for-filling-materialized-views}

[Null 表引擎](/engines/table-engines/special/null) 提供了一个不持久化数据的存储引擎(可以将其视为表引擎世界中的 `/dev/null`)。虽然这看起来有些矛盾,但物化视图仍然会对插入到此表引擎的数据执行操作。这允许在不持久化原始数据的情况下构建物化视图——避免了 I/O 和相关的存储开销。
...
在倒数第二步中,我们使用[前文](#timestamp-or-monotonically-increasing-column-available)所述的简单 `INSERT INTO SELECT` 方法来回填 `pypi_downloads_per_day`。也可以使用[上文](#using-a-null-table-engine-for-filling-materialized-views)记录的 Null 表方法进行增强,并可选择使用副本表来提高容错性。

虽然此操作需要暂停插入,但中间操作通常可以快速完成,从而将数据中断降至最低。
```
