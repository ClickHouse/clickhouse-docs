...
> 关于 ClickHouse 为什么能把数据压缩得如此出色，我们推荐阅读[这篇文章](https://clickhouse.com/blog/optimize-clickhouse-codecs-compression-schema)。简单来说，作为一款列式数据库，ClickHouse 会按列顺序写入值。当这些值经过排序后，相同的值会彼此相邻，而压缩算法会利用数据中这种连续模式。在此基础之上，ClickHouse 还提供了 codec 和更细粒度的数据类型，使你可以更轻松地进一步调优压缩。
...
<summary>关于 compact 与 wide 分区的说明</summary>
...
完整的列级明细结果显示，通过在压缩前对数据进行排序并使用合适的数据类型，`Body`、`Title`、`Tags` 和 `CreationDate` 列的存储空间实现了可观的节省。
