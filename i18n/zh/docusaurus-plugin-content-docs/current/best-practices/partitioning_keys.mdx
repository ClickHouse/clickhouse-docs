...
每当向表中插入一组行时，ClickHouse 不会像[此处](/operations/settings/settings#max_insert_block_size)描述的那样创建一个包含所有插入行的单一分区片段，而是会针对插入行中每个唯一的分区键值创建一个新的分区片段：
...
启用分区后，ClickHouse 只会在分区内部对分区片段进行[合并](/merges)，而不会跨分区进行合并。我们在上面的示例表中对此进行了说明，见[此处](/parts)：
