...
当客户端批处理不可行时,ClickHouse 中的异步插入提供了一种强大的替代方案。这在可观测性工作负载中尤其有价值,在这些场景中,成百上千个代理持续发送数据——日志、指标、追踪——通常以小型实时负载的形式。在这些环境中,客户端缓冲数据会增加复杂性,需要一个集中式队列来确保可以发送足够大的批次。

:::note
不建议在同步模式下发送大量小批次,这会导致创建大量数据分片。这将导致查询性能不佳和["分片过多"](/knowledgebase/exception-too-many-parts)错误。
:::

异步插入通过将传入数据写入内存缓冲区,然后根据可配置的阈值将其刷新到存储,将批处理责任从客户端转移到服务器。这种方法显著减少了数据分片创建开销,降低了 CPU 使用率,并确保即使在高并发情况下数据摄取仍然保持高效。

核心行为通过 [`async_insert`](/operations/settings/settings#async_insert) 设置进行控制。

<Image img={async_inserts} size='lg' alt='Async inserts' />

启用时(1),插入会被缓冲,并且仅在满足以下刷新条件之一时才写入磁盘:
...
设置为 1(默认值)时,ClickHouse 仅在数据成功刷新到磁盘后才确认插入。这确保了强持久性保证,并使错误处理变得简单:如果在刷新过程中出现问题,错误会返回给客户端。建议在大多数生产场景中使用此模式,特别是当必须可靠地跟踪插入失败时。

[基准测试](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse)表明,无论运行 200 个还是 500 个客户端,它都能很好地扩展并发——这得益于自适应插入和稳定的数据分片创建行为。

设置 `wait_for_async_insert = 0` 启用"即发即忘"模式。在这种模式下,服务器在数据被缓冲后立即确认插入,而无需等待数据到达存储。

这提供了超低延迟的插入和最大吞吐量,非常适合高速、低关键性的数据。但是,这也有权衡:无法保证数据会被持久化,错误可能仅在刷新期间出现,并且难以追踪失败的插入。仅当您的工作负载可以容忍数据丢失时才使用此模式。

[基准测试还表明](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse),当缓冲区刷新不频繁时(例如每 30 秒),数据分片数量会大幅减少,CPU 使用率也会降低,但静默失败的风险仍然存在。
...
- 在用户级别启用异步插入。本示例使用 `default` 用户,如果您创建了其他用户,请替换为相应的用户名:
  ```sql
