...
特定版本标签会设定该 PR 必须到达的*最早*发布版本：它会被回移到该版本**以及之后所有更新的活跃发布分支**，而不只是回移到标签中指定的那个版本。例如，如果一个合并到开发分支的 PR 带有 `v25.3-must-backport` 标签，它会被回移到 `25.3` 以及其后的所有活跃发布版本 (`25.4`、`25.5`、…)。如果同时存在多个特定版本标签，则以最低版本为准，因为它已经涵盖了所有更新的版本。
...
| `v{VER}-must-backport` (e.g. `v25.3-must-backport`) | 回移到该发布分支**及所有更新的活跃发布分支**——该版本标记了 PR 必须到达的*最旧*发布版本。若存在多个此类标签，以最低版本为准。对这些分支会重写 `rolling-out` 的跳过规则 |
...
当发布 PR 带有 `rolling-out` 标签时，通用 回移 标签 (`pr-must-backport`、`pr-critical-bugfix`) 会跳过该分支。机器人会关闭此前为该分支创建的所有 cherry-pick 或 回移 PR，并附上说明原因的评论。特定版本标签 (例如 `v25.3-must-backport`) 始终会重写这一规则——对已命名的发布版本以及它扩展到的每个更新且仍处于活动状态的发布分支都是如此。`pr-must-backport-force` 会对所有分支绕过 `rolling-out` 检查。
