---
'description': '在 Linux 环境中为 macOS 系统交叉编译 ClickHouse 的指南'
'sidebar_label': '在 Linux 上构建 macOS'
'sidebar_position': 20
'slug': '/development/build-cross-osx'
'title': '在 Linux 上构建 macOS'
---


# 如何在 Linux 上为 macOS 构建 ClickHouse

这是针对您拥有一台 Linux 机器并希望使用它来构建将在 OS X 上运行的 `clickhouse` 二进制文件的情况。 
主要用例是在 Linux 机器上运行的持续集成检查。 
如果您想直接在 macOS 上构建 ClickHouse，请按照 [原生构建说明](../development/build-osx.md) 进行操作。

针对 macOS 的交叉构建基于 [构建说明](../development/build.md)，请先遵循它们。

以下部分提供了为 `x86_64` macOS 构建 ClickHouse 的详细步骤。 
如果您的目标是 ARM 架构，只需将所有 `x86_64` 的出现替换为 `aarch64`。 
