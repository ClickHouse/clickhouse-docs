...
[`Array`](/sql-reference/aggregate-functions/combinators#-array) 组合器可应用于 [`uniq`](/sql-reference/aggregate-functions/reference/uniq) 函数,使用 `uniqArray` 聚合组合器函数计算所有数组中不重复元素的近似数量。

当需要统计数据集中多个数组的不重复元素时,`uniqArray` 函数非常实用。它等同于使用 `uniq(arrayJoin())`,其中 `arrayJoin` 先将数组展平,然后 `uniq` 统计不重复元素的数量。
...
- `uniqArray` 返回 5,因为所有用户中共有 5 个唯一兴趣:'reading'、'gaming'、'music'、'sports'、'cooking'
- `uniq(arrayJoin())` 同样返回 5,表明两个函数都能统计所有数组中的唯一元素
