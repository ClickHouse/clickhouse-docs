...
title: 'countIf'
description: '使用 countIf 组合子（combinator）的示例'
...
[`If`](/sql-reference/aggregate-functions/combinators#-if) 组合器可应用于 [`count`](/sql-reference/aggregate-functions/reference/count) 函数,使用 `countIf` 聚合组合器函数来统计满足条件的行数。
