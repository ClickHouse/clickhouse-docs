...
在此示例中,我们将使用产品销售的示例数据集来演示 `argMaxIf` 的工作原理。我们将查找价格最高的产品名称,但仅限于销售次数至少为 10 次的产品。
...
`argMaxIf` 函数将返回所有销售次数至少为 10 次(sales_count >= 10)的产品中价格最高的产品名称。在本例中,它将返回 'Laptop',因为它在热门产品中价格最高(999.99)。
