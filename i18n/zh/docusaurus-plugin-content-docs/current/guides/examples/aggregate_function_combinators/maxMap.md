...
[`Map`](/sql-reference/aggregate-functions/combinators#-map) 组合器可以应用于 [`max`](/sql-reference/aggregate-functions/reference/max) 函数，以根据每个键计算 Map 中的最大值，使用 `maxMap` 聚合组合器函数。

## 示例用法 {#example-usage}

在本例中，我们将创建一个表，该表存储状态码及其在不同时间段内的计数，其中每行包含状态码与其对应计数的 Map。我们将使用 `maxMap` 来查找每个时间段内每个状态码的最大计数。
...
`maxMap` 函数将查找每个时间段内每个状态码的最大计数。例如：
