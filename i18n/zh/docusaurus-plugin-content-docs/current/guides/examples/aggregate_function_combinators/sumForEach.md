...
[`ForEach`](/sql-reference/aggregate-functions/combinators#-foreach) 组合器可应用于 [`sum`](/sql-reference/aggregate-functions/reference/sum) 聚合函数,将其从操作行值的聚合函数转换为操作数组列的聚合函数,对各行数组中的每个元素分别应用聚合操作。
