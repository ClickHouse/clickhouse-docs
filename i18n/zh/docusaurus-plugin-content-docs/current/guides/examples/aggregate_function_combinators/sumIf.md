...
[`If`](/sql-reference/aggregate-functions/combinators#-if) 组合器可以应用于 [`sum`](/sql-reference/aggregate-functions/reference/sum) 函数，以计算条件为真的行的值的总和，使用 `sumIf` 聚合组合函数。

## 示例用法 {#example-usage}

在这个示例中，我们将创建一个存储销售数据和成功标志的表，并使用 `sumIf` 计算成功交易的总销售额。
...
`sumIf` 函数只会对 `is_successful = 1` 的金额进行求和。在这种情况下，它将求和：100.50 + 200.75 + 300.00 + 175.25。
...
在这个示例中，我们将使用 [ClickHouse playground](https://sql.clickhouse.com/) 中可用的 `stock` 表，计算2002年上半年的按价格方向的交易量。
...
在这个示例中，我们将使用 [ClickHouse playground](https://sql.clickhouse.com/) 中可用的 `stock` 表，计算2006年三家当时最大的科技公司的按股票代码的交易量。
