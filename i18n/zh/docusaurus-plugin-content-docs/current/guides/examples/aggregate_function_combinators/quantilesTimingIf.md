...
函数，通过聚合组合器函数 `quantilesTimingIf` 来计算条件为 true 的行的时间值分位数。



## 示例用法 {#example-usage}

在本示例中，我们将创建一张表来存储不同端点的 API 响应时间，并使用 `quantilesTimingIf` 计算成功请求的响应时间分位数。
...
`quantilesTimingIf` 函数仅会对成功的请求（is&#95;successful = 1）计算分位数。
返回的数组按顺序包含以下分位数：
...
- [`If 组合子`](/sql-reference/aggregate-functions/combinators#-if)
