...
[`If`](/sql-reference/aggregate-functions/combinators#-if) 组合器可应用于 [`quantilesTiming`](/sql-reference/aggregate-functions/reference/quantiletiming) 函数,用于计算满足条件的行的时间值分位数,通过 `quantilesTimingIf` 聚合组合器函数实现。
