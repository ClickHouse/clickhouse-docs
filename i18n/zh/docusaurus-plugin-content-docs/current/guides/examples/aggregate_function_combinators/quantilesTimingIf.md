...
[`If`](/sql-reference/aggregate-functions/combinators#-if) 组合器可应用于 [`quantilesTiming`](/sql-reference/aggregate-functions/reference/quantiletiming) 函数，以计算满足条件的行的时间值分位数。该功能通过 `quantilesTimingIf` 聚合组合器函数实现。
...
`quantilesTimingIf` 函数仅计算成功请求(is_successful = 1)的分位数。
返回的数组按顺序包含以下分位数:
