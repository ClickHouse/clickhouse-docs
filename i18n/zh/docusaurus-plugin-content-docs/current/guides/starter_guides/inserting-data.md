---
title: '写入 ClickHouse 数据'
description: '如何向 ClickHouse 写入数据'
keywords: ['INSERT', 'Batch Insert']
sidebar_label: '写入 ClickHouse 数据'
...
这些事务每次可能只涉及少量行,由于可靠性保证会产生相当大的开销,从而限制了插入性能。

为了在实现高插入性能的同时保持强一致性保证,用户在向 ClickHouse 插入数据时应遵循下面描述的简单规则。
遵循这些规则将有助于避免用户首次使用 ClickHouse 时常见的问题,并避免尝试复制适用于 OLTP 数据库的插入策略。


## 插入数据的最佳实践 {#best-practices-for-inserts}

### 使用大批量插入 {#insert-in-large-batch-sizes}

默认情况下,每次向 ClickHouse 发送插入操作时,ClickHouse 都会立即创建一个存储部分(part),其中包含插入的数据以及需要存储的其他元数据。
因此,相比发送大量小批次插入,发送少量大批次插入可以减少所需的写入次数。
...
这样 ClickHouse 会将数据复制到任何可用的副本分片,并确保数据最终一致。

如果客户端负载均衡不方便,用户可以通过[分布式表](/engines/table-engines/special/distributed)进行插入,该表会在节点之间分配写入。同样,建议设置 `internal_replication=true`。
...
此外,被观测系统中存在事件激增的风险,在尝试客户端缓冲可观测性数据时,这可能会导致大量内存激增和相关问题。
...
(1) 异步接收插入查询。
(2) 首先将查询的数据写入内存缓冲区。
(3) 仅在下一次缓冲区刷新时,对数据进行排序并将其作为一个部分(part)写入数据库存储。
...
:::note
请注意,在刷新到数据库存储之前,数据无法通过查询进行检索,并且缓冲区刷新是可配置的。
...
这是与 OLTP 数据库的一个显著区别,使得从外部源加载数据变得更加容易——尤其是与[表函数](/sql-reference/table-functions)以及从磁盘文件加载数据的能力相结合时。
...
- [PostgreSQL 表引擎](/integrations/postgresql#using-the-postgresql-table-engine),可直接读取数据,如前面示例所示。当基于已知水位线(例如时间戳)的批量复制能够满足需求,或者仅需一次性迁移时,通常适合使用此方法。该方法可扩展至数千万行数据。对于需要迁移更大数据集的用户,建议使用多个请求,每个请求处理一部分数据。可以为每个数据块使用临时表,然后再将其分区移动到最终表中。这样可以在请求失败时进行重试。有关此批量加载策略的更多详细信息,请参见此处。
- 可以将数据从 PostgreSQL 导出为 CSV 格式。然后可以使用表函数从本地文件或通过对象存储将其插入 ClickHouse。
