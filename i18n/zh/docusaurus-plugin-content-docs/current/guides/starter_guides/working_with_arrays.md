...
> 在本指南中，您将学习如何在 ClickHouse 中使用数组，以及一些最常用的[数组函数](/sql-reference/functions/array-functions)。
...
在动态创建数组时,ClickHouse 会选择适合所有元素的最窄类型。
...
这可能与您习惯使用的其他编程语言不同,在那些语言中数组索引从零开始。


例如，对于一个数组，你可以通过以下写法选取其第一个元素：
...
尽管数组的索引是从 1 开始的，你仍然可以访问下标为 0 的元素。
返回值将是该数组类型的 *默认值*。
在下面的示例中，返回了一个空字符串，因为这是字符串数据类型的默认值：
...
您也可以使用 [`arrayEnumerate`](/sql-reference/functions/array-functions#arrayEnumerate) 函数返回元素索引组成的数组:
...
如果数组元素已按升序排序,则可以使用 [`indexOfAssumeSorted`](/sql-reference/functions/array-functions#indexOfAssumeSorted) 函数。
...
让我们看看在随机选择的某一天(例如 '2024-01-01')美国最繁忙的前 10 个机场。
我们希望了解每个机场有多少航班起飞。
...
[`arrayMap`](/sql-reference/functions/array-functions#arrayMap) 函数就是这样一个高阶函数的示例,它通过对原始数组的每个元素应用 lambda 函数来从提供的数组返回一个新数组。

运行下面使用 `arrayMap` 函数的查询以查看哪些航班延误或准点。
...
在上述查询中,`arrayMap` 函数接受单元素数组 `[DepDelayMinutes]` 并应用 lambda 函数 `d -> if(d >= 30, 'DELAYED', if(d >= 15, 'WARNING', 'ON-TIME'` 对其进行分类。
然后使用 `[DepDelayMinutes][1]` 提取结果数组的第一个元素。
...
在上述查询中,我们将 lambda 函数作为第一个参数传递给 [`arrayFilter`](/sql-reference/functions/array-functions#arrayFilter) 函数。
该 lambda 函数接受延误分钟数 (d),如果满足条件则返回 `1`,否则返回 `0`。
...
该查询分为两个主要阶段。
首先,它使用公共表表达式 (CTE) 创建一个名为 `airport_routes` 的临时数据集,该数据集查看 2024 年 1 月 1 日的所有航班,并为每个始发机场构建该机场所服务的所有唯一目的地的排序列表。
例如,在 `airport_routes` 结果集中,DEN 可能包含一个数组,其中包含其飞往的所有城市,如 `['ATL', 'BOS', 'LAX', 'MIA', ...]` 等。

在第二阶段,查询选取五个美国主要枢纽机场(`DEN`、`ATL`、`DFW`、`ORD` 和 `LAS`)并比较它们之间的每一个可能配对。
它使用交叉连接来实现这一点,交叉连接会创建这些机场的所有组合。
然后,对于每一对机场,它使用 `arrayIntersect` 函数来查找同时出现在两个机场列表中的目的地。
length 函数统计它们共有的目的地数量。


条件 `a1.Origin < a2.Origin` 确保每对机场只出现一次。
如果没有这个条件,您将得到 JFK-LAX 和 LAX-JFK 两个独立的结果,这是冗余的,因为它们代表相同的比较。
最后,查询对结果进行排序,以显示哪些机场对拥有最多的共享目的地,并仅返回前 10 个结果。
...
我们可以编写一个使用 `arrayJoin` 的查询来计算两个机场之间延误达到指定分钟数的航班数量。
...
请尝试将这些概念应用到您自己的数据集,并在 SQL playground 或其他示例数据集上尝试不同的查询。

数组是 ClickHouse 中的一项基础功能,可实现高效的分析查询——随着您对数组函数越来越熟悉,您会发现它们可以极大地简化复杂的聚合和时间序列分析。
