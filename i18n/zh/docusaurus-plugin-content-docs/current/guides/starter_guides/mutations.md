...
# 使用变更操作（mutations）更新和删除 ClickHouse 数据

尽管 ClickHouse 主要面向高吞吐量的分析型工作负载，但在某些情况下仍然可以修改或删除已有数据。这类操作称为“变更（mutations）”，通过 `ALTER TABLE` 命令执行。

:::tip
如果需要频繁执行更新操作，建议在 ClickHouse 中使用[去重](../developer/deduplication.md)，它允许在不生成变更事件的情况下更新和/或删除行。或者，可以使用[轻量级更新](/docs/sql-reference/statements/update)或[轻量级删除](/guides/developer/lightweight-delete)。
:::
...
:::note
要删除表中的所有数据，使用 `TRUNCATE TABLE [<database].]<table>` 命令更为高效。此命令也可以通过 `ON CLUSTER` 执行。
:::
...
- 轻量级删除默认为同步模式,会等待所有副本完成删除操作。该行为由 [`lightweight_deletes_sync` 设置](/operations/settings/settings#lightweight_deletes_sync)控制。
