...
本页汇总了 ClickHouse 数据跳过索引的示例,展示如何声明各种类型、何时使用以及如何验证其是否生效。所有功能均适用于 [MergeTree 系列表](/engines/table-engines/mergetree-family/mergetree)。
...
查看使用 `EXPLAIN` 和剪枝的[完整示例](/best-practices/use-data-skipping-indices-where-appropriate#example)。
...
`bloom_filter` 索引适用于"大海捞针"式的等值查询和 IN 成员判断。它接受一个可选参数,即假阳性率(默认值为 0.025)。
...
`ngrambf_v1` 索引将字符串拆分为 n-gram。它非常适用于 `LIKE '%...%'` 查询。支持 String/FixedString/Map 类型(通过 mapKeys/mapValues),并且可以调整大小、哈希函数数量和种子值。更多详细信息请参阅 [N-gram 布隆过滤器](/engines/table-engines/mergetree-family/mergetree#n-gram-bloom-filter)文档。
...
ngrambf_v1 的四个参数(n-gram 大小、位图大小、哈希函数数量、种子值)会显著影响性能和内存使用。使用以下函数可以根据预期的 n-gram 数量和期望的误报率来计算最优的位图大小和哈希函数数量:
...
-- 针对 4300 个 ngram、p_false = 0.0001 的示例计算
...
## 用于基于词搜索的 Token 布隆过滤器 (tokenbf_v1) {#token-bloom-filter-tokenbf-v1-for-word-based-search}
...
- 过滤值在数据块内分布稀疏
- 与 `ORDER BY` 列存在强相关性,或数据摄取模式将相似值分组在一起
- 在大型日志数据集上执行文本搜索(`ngrambf_v1`/`tokenbf_v1` 类型)

**应避免使用跳数索引的场景:**

- 大多数数据块可能包含至少一个匹配值(无论如何都会读取这些数据块)
- 对与数据排序无关的高基数列进行过滤

:::note 重要注意事项
如果某个值在数据块中出现哪怕一次,ClickHouse 也必须读取整个数据块。请使用真实数据集测试索引,并根据实际性能测量结果调整粒度和特定类型的参数。
:::
...
- 应使用 `EXPLAIN` 和追踪来验证布隆过滤器和其他跳数索引的效果;需调整颗粒度以平衡裁剪效果与索引大小。
