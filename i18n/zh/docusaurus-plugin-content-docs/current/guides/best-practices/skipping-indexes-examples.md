...
本页汇总了 ClickHouse 数据跳过索引的示例，展示如何声明各种索引类型、何时使用以及如何验证其是否生效。所有功能均适用于 [MergeTree 系列表](/engines/table-engines/mergetree-family/mergetree)。
...
| 索引类型                                | 描述                                       |
| --------------------------------------- | ------------------------------------------------- |
| **minmax**                              | 跟踪每个数据粒度中的最小值和最大值 |
| **set(N)**                              | 每个数据粒度最多存储 N 个不同的值        |
| **bloom_filter([false_positive_rate])** | 用于存在性检查的概率过滤器         |
| **ngrambf_v1**                          | 用于子串搜索的 N-gram 布隆过滤器        |
| **tokenbf_v1**                          | 用于全文搜索的基于分词的布隆过滤器   |

每个部分都提供了示例数据和示例，并演示如何在查询执行中验证索引的使用情况。
...
参见使用 `EXPLAIN` 和剪枝的[实际示例](/best-practices/use-data-skipping-indices-where-appropriate#example)。
...
创建/物化工作流程以及使用前后的效果对比请参见[基本操作指南](/optimize/skipping-indexes#basic-operation)。


## 通用布隆过滤器(标量) {#generic-bloom-filter-scalar}

`bloom_filter` 索引适用于"大海捞针"式的等值查询和 IN 成员判断。它接受一个可选参数,即误报率(默认值为 0.025)。
...
`ngrambf_v1` 索引将字符串拆分为 n-gram。它非常适合 `LIKE '%...%'` 查询。支持 String/FixedString/Map 类型(通过 mapKeys/mapValues),并且可以调整大小、哈希函数数量和种子值。更多详细信息请参阅 [N-gram 布隆过滤器](/engines/table-engines/mergetree-family/mergetree#n-gram-bloom-filter)文档。
...
[本指南](/use-cases/observability/schema-design#bloom-filters-for-text-search)展示了实际示例以及何时使用 token 和 ngram。
...
完整的调优指导请参阅[参数文档](/engines/table-engines/mergetree-family/mergetree#n-gram-bloom-filter)。


## 用于基于词的搜索的 Token Bloom 过滤器 (tokenbf_v1) {#token-bloom-filter-tokenbf-v1-for-word-based-search}
...
更多详细信息请参阅 [Token bloom filter](/engines/table-engines/mergetree-family/mergetree#token-bloom-filter) 和 [Bloom filter types](/optimize/skipping-indexes#skip-index-types) 页面。
...
此 [minmax 实例](/best-practices/use-data-skipping-indices-where-appropriate#example) 演示了 EXPLAIN 输出结构和剪枝计数。
...
:::note 重要注意事项
如果某个值在数据块中哪怕只出现一次,ClickHouse 也必须读取整个数据块。请使用真实数据集测试索引,并根据实际性能测量结果调整粒度和类型特定参数。
:::


## 临时忽略或强制使用索引 {#temporarily-ignore-or-force-indexes}

在测试和故障排查过程中,可以按名称为单个查询禁用特定索引。同时也提供了在需要时强制使用索引的设置。请参阅 [`ignore_data_skipping_indices`](/operations/settings/settings#ignore_data_skipping_indices)。
...
- 跳数索引仅支持 [MergeTree 系列表](/engines/table-engines/mergetree-family/mergetree);数据修剪在颗粒/数据块级别进行。
- 基于布隆过滤器的索引是概率性的(假阳性会导致额外读取,但不会跳过有效数据)。
- 应使用 `EXPLAIN` 和追踪来验证布隆过滤器和其他跳数索引的效果;需调整粒度以在修剪效果与索引大小之间取得平衡。
