...
sidebar_label: '查询并行度'
sidebar_position: 20
description: 'ClickHouse 使用 processing lanes 和 max_threads 设置来实现查询执行的并行化。'
...
ClickHouse 是[为速度而生的](/concepts/why-clickhouse-is-so-fast)。它以高度并行的方式执行查询，利用所有可用的 CPU 核心，将数据分布到多条处理通道中，并且经常将硬件推到接近极限的状态。
 
本指南将介绍 ClickHouse 中查询并行机制的工作原理，以及你如何通过调优或监控来提升大规模工作负载下的性能。

我们使用针对 [uk_price_paid_simple](/parts) 数据集的聚合查询来说明关键概念。



## 逐步解析:ClickHouse 如何并行化聚合查询 {#step-by-step-how-clickHouse-parallelizes-an-aggregation-query}

当 ClickHouse ① 执行带有主键过滤条件的聚合查询时,它会 ② 将主索引加载到内存中,以便 ③ 识别哪些数据颗粒需要处理,哪些可以安全跳过:
...
<br />
<br />
并行处理通道的数量 `n` 由 [max_threads](/operations/settings/settings#max_threads) 设置控制,默认情况下与服务器上 ClickHouse 可用的 CPU 核心数相匹配。在上面的示例中,我们假设有 `4` 个核心。
...
当表数据以[分片](/shards)形式分布在多个服务器上时,每个服务器并行处理其分片。在每个服务器内部,本地数据使用并行处理通道进行处理,就像上面描述的那样:
...
ClickHouse 的[嵌入式 Web UI](/interfaces/http)(可通过 `/play` 端点访问)可以将上述物理计划渲染为图形化可视化。在本示例中,我们将 `max_threads` 设置为 `4` 以保持可视化的紧凑性,仅显示 4 个并行处理通道:
...
请注意,上述物理计划中的 `Resize` 算子会[重新分区和重新分配](/academic_overview#4-2-multi-core-parallelization)数据块流到各个处理通道,以保持它们的均衡利用。当数据范围中匹配查询谓词的行数差异较大时,这种重新平衡尤为重要,否则某些通道可能会过载,而其他通道则处于空闲状态。通过重新分配工作,较快的通道可以有效地帮助较慢的通道,从而优化整体查询运行时间。


## 为什么 max_threads 并不总是生效 {#why-max-threads-isnt-always-respected}

如上所述,并行处理通道的数量 `n` 由 `max_threads` 设置控制,该值默认与服务器上 ClickHouse 可用的 CPU 核心数相匹配:
...
如上述输出所示,该查询处理了 231 万行并读取了 13.66MB 的数据。这是因为在索引分析阶段,ClickHouse 选择了 **282 个颗粒(granule)**进行处理,每个颗粒包含 8,192 行,总计约 231 万行:
...
无论配置的 `max_threads` 值是多少,ClickHouse 只有在数据量足够大时才会分配额外的并行处理通道。`max_threads` 中的 "max" 表示上限,而不是保证使用的线程数。

"数据量足够大"的含义主要由两个设置决定,这些设置定义了每个处理通道应处理的最小行数(默认为 163,840)和最小字节数(默认为 2,097,152):
...
:::warning 不要修改这些设置
我们不建议在生产环境中修改这些设置。这里展示这些设置只是为了说明为什么 `max_threads` 并不总是决定实际的并行度。
:::

出于演示目的，让我们在重写这些设置以强制使用最大并发度的情况下，检查物理执行计划：
...
现在 ClickHouse 使用 59 个并发流来扫描数据，并且完全遵守已配置的 `max_threads`。

这说明对于小数据集上的查询，ClickHouse 会有意限制并发度。仅在测试时临时覆盖设置，不要在生产环境中这样做，因为这可能导致执行低效或资源争用。
