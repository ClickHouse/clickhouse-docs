...
sidebar_position: 20
description: 'ClickHouse 使用处理通道和 max_threads 设置来并行执行查询。'
...
ClickHouse 是[为速度而打造的](/concepts/why-clickhouse-is-so-fast)。它以高度并行的方式执行查询，利用所有可用的 CPU 核心，将数据分发到多个处理通道，并且经常把硬件的性能压榨到接近极限。

本指南将介绍 ClickHouse 中查询并行机制的工作原理，以及如何对其进行调优或监控，以提升大规模工作负载下的性能。

我们将使用在 [uk_price_paid_simple](/parts) 数据集上的聚合查询来说明关键概念。
...
在具有 `8` 个核心的机器上,由于更多通道并行处理数据,查询处理吞吐量大约会翻倍(但内存使用量也会相应增加):
...
当表数据以[分片](/shards)形式分布在多个服务器上时,每个服务器并行处理其分片。在每个服务器内部,本地数据使用并行处理通道进行处理,正如上文所述:
...
在分片间分配查询负载可实现并行性的水平扩展,特别适用于高吞吐量环境。
...
返回 1 行。耗时:0.013 秒。已处理 231 万行,13.66 MB(每秒 1.7312 亿行,1.02 GB/秒)
峰值内存使用:27.24 MiB。
```
...
:::warning 请勿修改这些设置
我们不建议在生产环境中修改这些设置。此处展示它们仅用于说明为什么 `max_threads` 并不总是决定实际的并行度。
:::

为了演示,让我们检查覆盖这些设置以强制最大并发后的物理执行计划:
...
现在，ClickHouse 使用 59 个并发流来扫描数据，完全遵循已配置的 `max_threads`。

这表明，对于小数据集上的查询，ClickHouse 会有意限制并发度。仅在测试时使用设置覆盖——不要在生产环境中这样做——因为这可能导致执行效率低下或资源争用。
...
## 更多信息 {#where-to-find-more-information}

如果您想深入了解 ClickHouse 如何并行执行查询以及如何在大规模场景下实现高性能,请参考以下资源:

- [查询处理层 – VLDB 2024 论文(网页版)](/academic_overview#4-query-processing-layer) - 详细介绍 ClickHouse 的内部执行模型,包括调度、流水线和算子设计。
