...
keywords: ['去重', '数据去重', '插入重试', '插入操作']
doc_type: 'guide'
---

插入操作有时会因为超时等错误而失败。当插入失败时，数据有可能已经成功插入，也有可能没有。本指南介绍如何在重试插入时启用去重，从而避免相同数据被多次插入。

当执行插入重试时，ClickHouse 会尝试确定这些数据是否已经成功插入。如果插入的数据被标记为重复，ClickHouse 不会将其再次插入到目标表中。不过，用户仍然会收到一次成功的操作结果，就像数据已正常插入一样。
...
如果在重试序列期间发生的其他插入操作数量超过 `*_deduplication_window`,去重功能可能无法按预期工作。在这种情况下,相同的数据可能被多次插入。
...
设置 `insert_deduplicate=1` 可在查询级别启用去重。请注意,如果使用 `insert_deduplicate=0` 插入数据,即使使用 `insert_deduplicate=1` 重试插入,该数据也无法去重。这是因为使用 `insert_deduplicate=0` 插入时不会为数据块写入 `block_id`。
...
对于使用 `*MergeTree` 引擎的表,每个数据块都会被分配一个唯一的 `block_id`,它是该块中数据的哈希值。此 `block_id` 用作插入操作的唯一标识。如果在去重日志中找到相同的 `block_id`,该块将被视为重复数据而不会插入到表中。

这种方法适用于插入不同数据的场景。但是,如果需要有意多次插入相同的数据,则需要使用 `insert_deduplication_token` 设置来控制去重过程。此设置允许您为每次插入指定一个唯一的令牌,ClickHouse 使用该令牌来判断数据是否重复。

对于 `INSERT ... VALUES` 查询,将插入数据拆分为数据块的过程是确定性的,由相关设置决定。因此,用户在重试插入时应使用与初始操作相同的设置值。

对于 `INSERT ... SELECT` 查询,关键是查询的 `SELECT` 部分在每次操作中都要以相同的顺序返回相同的数据。请注意,这在实际使用中很难实现。为了确保重试时数据顺序稳定,请在查询的 `SELECT` 部分定义明确的 `ORDER BY` 子句。需要注意的是,在重试之间所选表可能会被更新:结果数据可能已发生变化,去重将不会生效。此外,在插入大量数据的情况下,插入后的数据块数量可能会超出去重日志窗口的容量,导致 ClickHouse 无法对这些块进行去重。


## 物化视图的插入去重 {#insert-deduplication-with-materialized-views}

当表具有一个或多个物化视图时,插入的数据会经过定义的转换后插入到这些视图的目标表中。转换后的数据在重试时也会进行去重。ClickHouse 对物化视图执行去重的方式与对目标表中插入数据的去重方式相同。
...
您还需要启用用户配置文件设置 [`deduplicate_blocks_in_dependent_materialized_views`](/operations/settings/settings#deduplicate_blocks_in_dependent_materialized_views)。
...
当向物化视图下的表插入数据块时,ClickHouse 会通过对组合了源表 `block_id` 和附加标识符的字符串进行哈希来计算 `block_id`。这确保了物化视图内的准确去重,使数据能够根据其原始插入进行区分,而不受到达物化视图目标表之前应用的任何转换的影响。
...
上述设置允许我们从表中查询一系列仅包含单行的数据块。这些小数据块不会被合并,在插入表之前保持原样。
...
这里我们看到两个数据分区已被插入到 `dst` 表中。查询产生 2 个数据块 -- 插入时生成 2 个数据分区。这些分区包含不同的数据。
...
这里我们看到 2 个数据分区已被插入到 `mv_dst` 表中。这些分区包含相同的数据,但它们没有被去重。
...
这里我们看到当重试插入时,所有数据都被去重了。去重对 `dst` 和 `mv_dst` 表都生效。
...
使用上述设置,select 会产生两个数据块——因此应该有两个数据块插入到表 `dst` 中。然而,我们看到只有一个数据块被插入到表 `dst` 中。这是因为第二个数据块被去重了。它具有相同的数据和去重键 `block_id`,该键是根据插入数据的哈希值计算得出的。这种行为不符合预期。虽然这种情况很少发生,但理论上是可能的。为了正确处理这种情况,用户必须提供 `insert_deduplication_token`。让我们通过以下示例来解决这个问题:
...
我们每次插入不同的数据。但是,相同的数据被插入到 `mv_dst` 表中。由于源数据不同,数据未进行去重。
