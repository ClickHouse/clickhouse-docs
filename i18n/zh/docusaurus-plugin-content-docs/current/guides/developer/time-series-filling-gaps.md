...
'sidebar_label': '时间序列 - 填充间隙'
'sidebar_position': 10
'description': '填充时间序列数据中的间隙'
'keywords':
- 'time series'
- 'gap fill'
'title': '时间序列数据的填充间隙'
...
在处理时间序列数据时，由于数据缺失或不活动，数据中可能存在空白。通常，当我们查询数据时，并不希望出现这些空白。在这种情况下，`WITH FILL` 子句可以派上用场。本指南讨论如何使用 `WITH FILL` 来填补时间序列数据中的空白。

## 设置 {#setup}

假设我们有以下一个表，其中存储了由 GenAI 图像服务生成的图像的元数据：
...
我们将探索2023年3月24日`00:24:03`到`00:24:04`之间创建的图像，因此让我们为这些时间点创建一些参数：
...
接下来，我们将编写一个查询，将数据分组到100毫秒的桶中，并返回在该桶中创建的图像计数：
...
结果集中仅包含创建图像的桶，但对于时间序列分析，我们可能希望返回每个100毫秒的桶，即使它没有任何条目。

## WITH FILL {#with-fill}

我们可以使用 `WITH FILL` 子句来填补这些空白。我们还将指定 `STEP`，即要填补的间隔的大小。对于 `DateTime` 类型，这默认是1秒，但我们希望填补长度为100毫秒的空白，因此将间隔100毫秒作为我们的步长值：
...
我们可以看到，缺口在 `count` 列中已被0值填补。

## WITH FILL...FROM {#with-fillfrom}

然而，在时间范围的开始仍然存在一个间隙，我们可以通过指定 `FROM` 来修复它：
...
从结果中可以看到，`00:24:03.000` 到 `00:24:03.500` 的桶现在都出现了。

## WITH FILL...TO {#with-fillto}

不过，我们仍然缺少一些来自时间范围结束的桶，我们可以通过提供 `TO` 值来填补。`TO` 不包括在内，因此我们会在结束时间上加一个小的值，以确保它被纳入：
...
所有的空白现在都已被填补，我们有从 `00:24:03.000` 到 `00:24:05.000` 每100毫秒的条目。
...
累计列中的值并没有如我们所希望的那样工作。

## WITH FILL...INTERPOLATE {#with-fillinterpolate}

任何在 `count` 列中为 `0` 的行在累计列中也为 `0`，而我们希望它使用累计列中的前一个值。我们可以通过使用 `INTERPOLATE` 子句来做到这一点，如下所示：
...
看起来好多了。现在为了结束这部分，让我们使用 `bar` 函数添加一个柱状图，并且不要忘记将我们的新列添加到 `INTERPOLATE` 子句中。
