...
sidebar_label: '时间序列 - 填补缺失'
sidebar_position: 10
description: '填补时间序列数据中的缺失值。'
...
在处理时间序列数据时，由于数据缺失或无活动期，数据中可能会出现空白。
通常，在查询数据时我们并不希望存在这些空白。在这种情况下，`WITH FILL` 子句就能派上用场。
本指南将介绍如何使用 `WITH FILL` 来填补时间序列数据中的空白。
...
接下来,编写一个查询将数据按 100 毫秒分组到时间桶中,并返回每个时间桶内创建的图像数量:
...
然而,时间范围的起始处仍存在缺口,我们可以通过指定 `FROM` 来解决:
...
现在所有间隙都已填充,我们得到了从 `00:24:03.000` 到 `00:24:05.000` 每 100 毫秒的条目。
