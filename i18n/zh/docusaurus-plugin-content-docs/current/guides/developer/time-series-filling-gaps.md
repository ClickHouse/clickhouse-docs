...
sidebar_label: '时间序列 - 填充缺口'
sidebar_position: 10
description: '填充时间序列数据中的缺口。'
...
在处理时间序列数据时，可能会因为数据缺失或无数据产生的时间段而出现空缺。
通常，在查询数据时我们并不希望这些空缺存在。在这种情况下，`WITH FILL` 子句就会派上用场。
...
我们将探索 2023 年 3 月 24 日 `00:24:03` 到 `00:24:04` 之间创建的图像,首先为这些时间点创建参数:
...
接下来,编写一个查询将数据按 100 毫秒分桶,并返回每个桶中创建的图像数量:
...
结果集仅包含有图像创建的时间桶,但在时间序列分析中,我们可能希望返回每个 100 毫秒的时间桶,即使其中没有任何数据。
...
从结果可以看出,从 `00:24:03.000` 到 `00:24:03.500` 的时间桶现在都已填充。


## WITH FILL...TO {#with-fillto}

不过,时间范围末尾仍然缺少一些时间桶,我们可以通过提供 `TO` 值来填充。
`TO` 不包含边界值,因此我们在结束时间上添加一个小的增量以确保将其包含在内:
...
现在所有间隙都已填充完毕,从 `00:24:03.000` 到 `00:24:05.000` 每 100 毫秒都有对应的条目。
...
cumulative 列中的值并未按预期方式工作。


## WITH FILL...INTERPOLATE {#with-fillinterpolate}

任何 `count` 列值为 `0` 的行,其 `cumulative` 列的值也为 `0`,但我们希望它使用 `cumulative` 列中的前一个值。
可以通过使用 `INTERPOLATE` 子句来实现,如下所示:
...
效果好多了。
最后,让我们使用 `bar` 函数添加一个条形图,记得将新列也添加到 `INTERPOLATE` 子句中。
