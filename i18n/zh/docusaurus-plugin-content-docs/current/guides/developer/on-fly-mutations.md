...
对于 `MergeTree` 系列表,可以通过启用查询级设置 `apply_mutations_on_fly` 来启用即时变更。
...
我们建议将 `apply_mutations_on_fly` 设置与其他 `MergeTree` 级别的设置(如 `number_of_mutations_to_throw` 和 `number_of_mutations_to_delay`)一起启用,以限制未物化变更的无限增长。
...
- `mutations_execute_nondeterministic_on_initiator` - 如果为 true,非确定性函数将在发起者副本上执行,并在 `UPDATE` 和 `DELETE` 查询中被替换为字面值。默认值:`false`。
- `mutations_execute_subqueries_on_initiator` - 如果为 true,标量子查询将在发起者副本上执行,并在 `UPDATE` 和 `DELETE` 查询中被替换为字面值。默认值:`false`。
- `mutations_max_literal_size_to_replace` - 在 `UPDATE` 和 `DELETE` 查询中可替换的序列化字面值的最大字节大小。默认值:`16384`(16 KiB)。
