...
title: '配置用于认证的 SSL 用户证书'
description: '本指南介绍用于配置 SSL 用户证书认证的简单、最小化设置。'
doc_type: 'guide'
...
本指南提供了一些简单且最小化的设置，用于通过 SSL 用户证书配置身份验证。本教程是在[配置 SSL-TLS 用户指南](../configuring-ssl.md)的基础上进行扩展的。

:::note
在使用 `https`、`native`、`mysql` 和 `postgresql` 接口时，支持 SSL 用户身份验证。

为实现安全的身份验证，ClickHouse 节点需要将 `<verificationMode>strict</verificationMode>` 设置为 `strict`（虽然将其设置为 `relaxed` 也可以用于测试目的）。

如果在 MySQL 接口前使用 AWS NLB，则必须联系 AWS 支持以启用以下未公开的选项：
...
2. 向新的证书用户授予权限:
   ...
   :::note
   我们建议使用 SQL 来定义用户和角色。但是,如果您当前在配置文件中定义用户和角色,用户配置如下所示:
...
2. 在 ClickHouse [客户端配置文件](/interfaces/cli.md#configuration_files)中配置 OpenSSL 证书和路径。
...
3. 运行 `clickhouse-client`。
   ...
   ```
   :::note
   注意:当配置文件中指定了证书时,传递给 clickhouse-client 的密码将被忽略。
   :::
...
本文介绍了为 SSL 证书身份验证创建和配置用户的基础知识。此方法可与 `clickhouse-client` 或任何支持 `https` 接口且允许设置 HTTP 请求头的客户端配合使用。生成的证书和密钥应妥善保管并严格限制访问权限,因为证书用于对 ClickHouse 数据库操作进行用户身份验证和授权。请像对待密码一样妥善保管证书和密钥。
