...
title: '将 ClickHouse 配置为使用 LDAP 进行认证和角色映射'
description: '介绍如何将 ClickHouse 配置为使用 LDAP 进行认证和角色映射'
keywords: ['LDAP 配置', 'LDAP 认证', '角色映射', '用户管理', 'SRE 指南']
doc_type: 'guide'
---
...
可以将 ClickHouse 配置为使用 LDAP 对 ClickHouse 数据库用户进行身份验证。本文档提供了一个简单示例，演示如何将 ClickHouse 与 LDAP 系统集成，以对一个公开可访问的目录服务进行身份验证。
...
3. 将 `<ldap>` 部分添加到 `<user_directories>` 部分以配置用户角色映射。此部分定义用户何时进行身份验证以及用户将获得什么角色。在此基本示例中,任何通过 LDAP 进行身份验证的用户都将获得 `scientists_role` 角色,该角色将在 ClickHouse 的后续步骤中定义。该部分应类似如下:
...
:::note
本节中的操作步骤假定已启用 ClickHouse 的 SQL 访问控制和账户管理功能。要启用该功能,请参阅 [SQL 用户和角色指南](index.md)。
:::
...
1. 使用 ClickHouse 客户端登录
   ...
   :::note
   使用步骤 1 中的 `ldapsearch` 命令可查看目录中的所有可用用户,所有用户的密码均为 `password`
   :::
