...
## 在 ClickHouse 中配置 LDAP 连接设置 \{#1-configure-ldap-connection-settings-in-clickhouse\}
...
3. 向 `<user_directories>` 部分添加 `<ldap>` 部分，以配置用户角色映射。该部分定义了用户何时通过身份验证，以及用户将获得什么角色。在这个基础示例中，任何通过 LDAP 进行身份验证的用户都会获得 `scientists_role` 角色，该角色将在后续步骤中在 ClickHouse 中定义。该部分应类似如下：
    ```xml
    <user_directories>
        <users_xml>
            <path>users.xml</path>
    ...
    |Parameter |Description                   |Example              |
    |----------|------------------------------|---------------------|
    |server    |前面 ldap_servers 部分中定义的标签名|test_ldap_server|
    |roles      |在 ClickHouse 中定义的、用户将被映射到的角色名称|scientists_role|
    |base_dn   |开始搜索包含该用户的组的基础路径        |dc=example,dc=com|
    |search_filter|用于识别所选组以映射用户的 LDAP 搜索过滤器    |`(&(objectClass=groupOfUniqueNames)(uniqueMember={bind_dn}))`|
    |attribute |值应从哪个属性名返回|cn|

4. 重启 ClickHouse 服务器以应用这些设置。

## 配置 ClickHouse 数据库角色和权限 \{#2-configure-clickhouse-database-roles-and-permissions\}

:::note
本节中的操作步骤假定 ClickHouse 中的 SQL 访问控制和账户管理功能已启用。如需启用，请查看 [SQL 用户与角色指南](index.md)。
:::
...
2. 将所需权限授予该角色。以下语句会向任何能够通过 LDAP 进行身份验证的用户授予管理员权限：
    ```sql
    GRANT ALL ON *.* TO scientists_role;
    ```

## 测试 LDAP 配置 \{#3-test-the-ldap-configuration\}
...
1. 使用 ClickHouse 客户端登录
    ...
    :::note
    使用步骤 1 中的 `ldapsearch` 命令查看目录中所有可用用户，并且所有用户的密码都是 `password`
    :::

2.  测试该用户是否已正确映射到 `scientists_role` 角色，并具有管理员权限
    ```sql
    SHOW DATABASES
    ```
