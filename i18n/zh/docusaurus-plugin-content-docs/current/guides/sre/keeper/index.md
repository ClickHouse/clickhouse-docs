...
7. 可以创建一个 `Distributed` 表来汇总表示两个分片上的数据。使用 `Distributed` 表引擎的表本身不存储任何数据，但允许在多个服务器上进行分布式查询处理。读操作会访问所有分片，写操作可以分布到各个分片上。在 `chnode1` 上运行以下查询：
   ```sql
...
8. 注意，对 `dist_table` 发起查询会返回来自两个分片的全部四行数据：
