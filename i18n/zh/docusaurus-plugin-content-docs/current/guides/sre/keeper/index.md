...
description: 'ClickHouse Keeper（或 clickhouse-keeper）可替代 ZooKeeper，用于实现复制与协同。'
...
ZooKeeper 是最早广为人知的开源协调系统之一。它使用 Java 实现，具有简单而强大的数据模型。ZooKeeper 的协调算法 ZooKeeper Atomic Broadcast（ZAB）不为读取操作提供线性一致性保证，因为每个 ZooKeeper 节点在本地提供读取服务。与 ZooKeeper 不同，ClickHouse Keeper 使用 C++ 编写，并采用 [RAFT 算法](https://raft.github.io/)[实现](https://github.com/eBay/NuRaft)。该算法为读取和写入操作提供线性一致性，并且在不同语言中有多个开源实现。

默认情况下，ClickHouse Keeper 提供与 ZooKeeper 相同的保证：线性一致的写入和非线性一致的读取。它具有兼容的客户端-服务器协议，因此任何标准 ZooKeeper 客户端都可以用于与 ClickHouse Keeper 交互。快照和日志的格式与 ZooKeeper 不兼容，但 `clickhouse-keeper-converter` 工具可以将 ZooKeeper 数据转换为 ClickHouse Keeper 快照。ClickHouse Keeper 中的服务器间协议也与 ZooKeeper 不兼容，因此无法构建混合的 ZooKeeper / ClickHouse Keeper 集群。

ClickHouse Keeper 以与 [ZooKeeper](https://zookeeper.apache.org/doc/r3.1.2/zookeeperProgrammers.html#sc_ZooKeeperAccessControl) 相同的方式支持访问控制列表（ACL）。ClickHouse Keeper 支持相同的权限集，并具有相同的内置方案：`world`、`auth` 和 `digest`。digest 认证方案使用 `username:password` 对，密码使用 Base64 编码。
...
ClickHouse Keeper 可以作为 ZooKeeper 的独立替代品使用，也可以作为 ClickHouse 服务器的内部组件使用。在这两种情况下，配置几乎是相同的 `.xml` 文件。

#### Keeper 配置设置 {#keeper-configuration-settings}

ClickHouse Keeper 的主要配置标签是 `<keeper_server>`，具有以下参数:
...
| `cgroups_memory_observer_wait_time` | 如果 `max_memory_usage_soft_limit` 未设置或设置为 `0`,则使用此时间间隔来观察物理内存量。一旦内存量发生变化,将通过 `max_memory_usage_soft_limit_ratio` 重新计算 Keeper 的内存软限制。 | `15`                                                                                                         |
| `http_control`                      | [HTTP 控制](#http-control)接口的配置。                                                                                                                                                                                           | -                                                                                                            |
| `digest_enabled`                    | 启用实时数据一致性检查                                                                                                                                                                                                                             | `True`                                                                                                       |
| `create_snapshot_on_exit`           | 在关闭时创建快照                                                                                                                                                                                                                   | -                                                                                                            |
| `hostname_checks_enabled`           | 为集群配置启用主机名合理性检查(例如,检查 localhost 是否与远程端点一起使用)                                                                                                                                           | `True`                                                                                                       |
...
| `election_timeout_lower_bound_ms`  | 如果跟随者在此时间间隔内没有收到来自领导者的心跳，则可以发起领导者选举。必须小于等于 `election_timeout_upper_bound_ms`，理想情况下二者不应相等。                                                                       | `1000`                                                                                                       |
| `election_timeout_upper_bound_ms`  | 如果跟随者在此时间间隔内没有收到来自领导者的心跳，则必须发起领导者选举。                                                                                                                                                | `2000`                                                                                                       |
| `rotate_log_storage_interval`      | 单个文件中要保存的日志记录数量。                                                                                                                                                                                         | `100000`                                                                                                     |
| `reserved_log_items`               | 在压缩（compaction）之前需要保留的协调日志记录数量。                                                                                                                                                                     | `100000`                                                                                                     |
| `snapshot_distance`                | ClickHouse Keeper 创建新快照的频率（按日志中的记录数量计）。                                                                                                                                                            | `100000`                                                                                                     |
| `snapshots_to_keep`                | 要保留的快照数量。                                                                                                                                                                                                       | `3`                                                                                                          |
| `stale_log_gap`                    | 当领导者认为跟随者已过时时使用的阈值，此时会向其发送快照而不是日志。                                                                                                                                                    | `10000`                                                                                                      |
| `fresh_log_gap`                    | 节点被视为“最新”（fresh）时的阈值。                                                                                                                                                                                      | `200`                                                                                                        |
| `max_requests_batch_size`          | 发送到 RAFT 之前，请求批次中（按请求数量计）的最大大小。                                                                                                                                                                | `100`                                                                                                        |
| `force_sync`                       | 对协调日志的每次写入调用 `fsync`。                                                                                                                                                                                       | `true`                                                                                                       |
| `quorum_reads`                     | 通过完整的 RAFT 共识流程将读请求按写请求的方式执行，并具有类似的速度特性。                                                                                                                                               | `false`                                                                                                      |
| `raft_logs_level`                  | 与协调相关的文本日志级别（trace、debug 等）。                                                                                                                                                                           | `system default`                                                                                             |
| `auto_forwarding`                  | 允许跟随者将写请求转发到领导者。                                                                                                                                                                                         | `true`                                                                                                       |
| `shutdown_timeout`                 | 等待内部连接结束并关闭的时间（毫秒）。                                                                                                                                                                                   | `5000`                                                                                                       |
| `startup_timeout`                  | 如果服务器在指定超时时间内未能连接到其他仲裁参与者，则会退出（毫秒）。                                                                                                                                                  | `30000`                                                                                                      |
| `async_replication`                | 启用异步复制。在保留所有写入和读取一致性保证的同时获得更好的性能。该设置默认禁用，以避免破坏向后兼容性。                                                                                                               | `false`                                                                                                      |
| `latest_logs_cache_size_threshold` | 最新日志条目的内存缓存总大小上限。                                                                                                                                                                                       | `1GiB`                                                                                                       |
| `commit_logs_cache_size_threshold` | 为后续提交所需日志条目的内存缓存总大小上限。                                                                                                                                                                             | `500MiB`                                                                                                     |
| `disk_move_retries_wait_ms`        | 在磁盘之间移动文件时发生故障后，两次重试之间的等待时间（毫秒）。                                                                                                                                                         | `1000`                                                                                                       |
| `disk_move_retries_during_init`    | 在初始化期间，在磁盘之间移动文件时发生故障后的重试次数。                                                                                                                                                                | `100`                                                                                                        |
| `experimental_use_rocksdb`         | 使用 RocksDB 作为后端存储。                                                                                                                                                                                              | `0`                                                                                                          |

仲裁配置位于 `<keeper_server>.<raft_configuration>` 部分，其中包含服务器的相关描述。

整个仲裁唯一的参数是 `secure`，用于为仲裁参与者之间的通信启用加密连接。如果节点之间的内部通信需要 SSL 连接，可以将该参数设置为 `true`，否则可以不指定。
...
:::note
当 ClickHouse Keeper 集群的拓扑发生变化时(例如替换服务器),请务必保持 `server_id` 到 `hostname` 的映射一致,避免混用或为不同服务器重用现有的 `server_id`(例如,如果依赖自动化脚本部署 ClickHouse Keeper,可能会发生这种情况)。

如果 Keeper 实例的主机可能发生变化,我们建议定义并使用主机名而不是原始 IP 地址。更改主机名等同于删除并重新添加服务器,在某些情况下这可能无法实现(例如没有足够的 Keeper 实例来形成仲裁)。
:::

:::note
`async_replication` 默认禁用以避免破坏向后兼容性。如果集群中的所有 Keeper 实例都运行支持 `async_replication` 的版本(v23.9+),我们建议启用它,因为它可以提高性能且没有任何缺点。
:::

三节点仲裁的配置示例可以在带有 `test_keeper_` 前缀的[集成测试](https://github.com/ClickHouse/ClickHouse/tree/master/tests/integration)中找到。服务器 #1 的配置示例:
...
ClickHouse Keeper 已捆绑到 ClickHouse 服务器包中,只需将 `<keeper_server>` 配置添加到 `/etc/your_path_to_config/clickhouse-server/config.xml` 并像往常一样启动 ClickHouse 服务器。如果要运行独立的 ClickHouse Keeper,可以使用类似的方式启动:

```bash
clickhouse-keeper --config /etc/your_path_to_config/config.xml
```

如果没有符号链接(`clickhouse-keeper`),可以创建它或将 `keeper` 指定为 `clickhouse` 的参数:
...
可以通过 telnet 或 nc 在客户端端口向 ClickHouse Keeper 发出命令。
...
* `srst`: 重置服务器统计信息。此命令会影响 `srvr`、`mntr` 和 `stat` 的输出结果。
...
* `cons`: 列出所有连接到此服务器的客户端的完整连接/会话详细信息。包括已接收/已发送的数据包数量、会话 ID、各操作的延迟、最近一次执行的操作等信息。
...
* `isro`: 检测服务器是否处于只读模式。如果服务器处于只读模式，将返回 `ro`，否则返回 `rw`。

```response
rw
```

* `wchs`: 列出服务器上所有监视项的简要信息。
...
* `wchc`: 按会话列出服务器上的 watch 详细信息。该命令会输出会话（连接）及其关联的 watch（路径）列表。注意，视 watch 数量多少而定，此操作可能开销较大（影响服务器性能），请谨慎使用。
...
* `wchp`: 按路径列出服务器上 watch 的详细信息。该命令会输出带有关联会话的路径（znode）列表。请注意，取决于 watch 的数量，此操作可能开销较大（即会影响服务器性能），请谨慎使用。
...
* `csnp`: 调度一个创建快照的任务。成功时返回该快照对应的最新已提交日志索引；失败时返回 `Failed to schedule snapshot creation task.`。注意，可以使用 `lgif` 命令来确认快照是否已完成。

```response
100
```

* `lgif`: Keeper 日志信息。`first_log_idx`：我在日志存储中的第一个日志索引；`first_log_term`：我的第一个日志任期；`last_log_idx`：我在日志存储中的最后一个日志索引；`last_log_term`：我的最后一个日志任期；`last_committed_log_idx`：我在状态机中最后一次提交的日志索引；`leader_committed_log_idx`：从我的视角看 leader 的已提交日志索引；`target_committed_log_idx`：应当提交到的目标日志索引；`last_snapshot_idx`：上一个快照中最大的已提交日志索引。
...
* `rqld`: 请求成为新的 leader。如果请求已发送则返回 `Sent leadership request to leader.`，如果请求未发送则返回 `Failed to send leadership request to leader.`。注意，如果节点已经是 leader，则行为与请求已发送时相同。

```response
已向 Leader 发送领导权请求。
```

* `ftfl`：列出所有 feature flag 以及它们在该 Keeper 实例上的启用状态。
...
* `ydld`: 请求让出主节点角色并成为从节点。如果接收该请求的服务器是主节点，它会先暂停写操作，等待继任者（当前主节点绝不会成为继任者）完成对最新日志的追赶，然后再主动卸任。继任者会被自动选出。如果请求已发送，则返回 `Sent yield leadership request to leader.`，如果请求未发送，则返回 `Failed to send yield leadership request to leader.`。注意，如果节点已经是从节点，结果与请求已发送时相同。

```response
已向 Leader 发送让出领导权请求。
```

* `pfev`: 返回所有已收集事件的值。对于每个事件，会返回事件名称、事件值以及事件描述。
...
4. 将快照复制到已配置 `keeper` 的 ClickHouse 服务器节点,或启动 ClickHouse Keeper 以替代 ZooKeeper。快照必须在所有节点上持久化,否则空节点可能启动更快并成为 leader。
...
### 丢失仲裁后的恢复 {#recovering-after-losing-quorum}

由于 ClickHouse Keeper 使用 Raft 协议,它可以容忍一定数量的节点故障,具体取决于集群规模。\
例如,对于 3 节点集群,如果只有 1 个节点崩溃,集群将继续正常工作。

集群配置可以动态调整,但存在一些限制。重新配置也依赖于 Raft,
因此要在集群中添加/删除节点,您需要满足仲裁条件。如果您同时丢失集群中过多节点且无法
再次启动它们,Raft 将停止工作,不允许您使用常规方式重新配置集群。
...
6. 在恢复模式下,leader 节点将为 `mntr` 命令返回错误消息,直到它与新节点达成仲裁,并且会拒绝来自客户端和 follower 的任何请求。
7. 达成仲裁后,leader 节点将返回正常操作模式,使用 Raft 接受所有请求——通过 `mntr` 验证,此时 `zk_server_state` 应返回 `leader`。


## 在 Keeper 中使用磁盘 {#using-disks-with-keeper}

Keeper 支持使用[外部磁盘](/operations/storing-data.md)的子集来存储快照、日志文件和状态文件。
...
在磁盘之间移动文件是安全的,即使 Keeper 在传输过程中停止也不会有数据丢失的风险。
...
如果启用了 `force_sync`,且未使用 `latest_log_storage_disk`,则 `log_storage_disk` 必须是 `local` 磁盘。
如果使用了 `latest_log_storage_disk`,则它必须始终是 `local` 磁盘。
如果禁用了 `force_sync`,则可以在任何配置中使用所有类型的磁盘。

Keeper 实例的一个可能的存储配置示例如下:
...
如果定义了分层磁盘配置(为最新文件使用单独的磁盘),Keeper 将在启动时尝试自动将文件移动到正确的磁盘。
...
为了减少从磁盘读取的数据量,Keeper 会在内存中缓存日志条目。
如果请求较大,日志条目会占用过多内存,因此会对缓存的日志数量进行限制。
该限制通过以下两个配置项控制:
...
检查(将 `127.0.0.1` 替换为您的 ClickHouse 服务器 IP 地址或主机名):
...
本指南提供了配置 ClickHouse Keeper 的简单最小化设置,并通过示例演示如何测试分布式操作。本示例在 Linux 上使用 3 个节点执行。

### 1. 使用 Keeper 设置配置节点 {#1-configure-nodes-with-keeper-settings}

1. 在 3 台主机(`chnode1`、`chnode2`、`chnode3`)上安装 3 个 ClickHouse 实例。(查看[快速入门](/getting-started/install/install.mdx)了解安装 ClickHouse 的详细信息。)
...
3. 将以下 ClickHouse Keeper 配置添加到所有三台服务器,并为每台服务器更新 `<server_id>` 设置;`chnode1` 为 `1`,`chnode2` 为 `2`,依此类推。
   ...
   | server_id             | raft 配置中使用的每个 ClickHouse Keeper 服务器的标识符                    | 1                                               |
...
5. 重启 ClickHouse 并验证每个 Keeper 实例是否正在运行。在每台服务器上执行以下命令。如果 Keeper 正在运行且健康,`ruok` 命令将返回 `imok`:
...
1.  在 `chnode1` 上使用 ClickHouse 客户端在新集群上创建一个新数据库。`ON CLUSTER` 子句会自动在两个节点上创建数据库。
    ```sql
...
7. 您可以创建一个 `Distributed` 表来表示两个分片上的数据。使用 `Distributed` 表引擎的表本身不存储任何数据,但支持在多个服务器上进行分布式查询处理。读取操作会访问所有分片,写入操作可以分布到各个分片。在 `chnode1` 上执行以下查询:
...
8. 注意查询 `dist_table` 会返回来自两个分片的全部四行数据:
...
本文介绍如何使用内置的 `{uuid}` 宏设置在 ClickHouse Keeper 或 ZooKeeper 中创建唯一条目。在频繁创建和删除表时,唯一路径非常有用,因为每次创建路径时都会使用新的 `uuid`,路径永不重用,从而避免了等待数分钟让 Keeper 垃圾回收删除路径条目。

### 示例环境 {#example-environment}

一个三节点集群,将在所有三个节点上配置 ClickHouse Keeper,并在其中两个节点上配置 ClickHouse。这为 ClickHouse Keeper 提供了三个节点(包括一个决胜节点),以及一个由两个副本组成的单分片 ClickHouse 集群。
...
:::note
注意,我们为 `shard` 和 `replica` 定义了宏,但 `{uuid}` 无需在此处定义,它是内置的。
:::
...
使用上述表的 UUID 从 ZooKeeper 获取表信息的示例命令
...
:::note
数据库必须为 `Atomic` 类型。如果从旧版本升级,`default` 数据库可能为 `Ordinary` 类型。
:::
...
:::note
如果此设置被关闭,您可以通过手动修改副本的 `raft_configuration` 部分来重新配置集群。请确保在所有副本上编辑文件,因为只有 leader 会应用更改。或者,您也可以通过任何兼容 ZooKeeper 的客户端发送 `reconfig` 查询。
:::
...
- `server_priority` 是一个非负整数,用于指定[在 leader 选举中应优先考虑哪些节点](https://github.com/eBay/NuRaft/blob/master/docs/leader_election_priority.md)。
...
您可以使用 `reconfig` 命令来添加新服务器、删除现有服务器以及更改现有服务器的优先级,以下是示例(使用 `clickhouse-keeper-client`):
...
- 仅支持增量重新配置。包含非空 `new_members` 的请求将被拒绝。
  ...
  ClickHouse Keeper 实现依赖 NuRaft API 来动态更改成员关系。NuRaft 每次只能添加或删除单个服务器。这意味着每次配置更改(`joining` 的每个部分、`leaving` 的每个部分)都必须单独处理。因此不提供批量重新配置功能,因为这会对最终用户造成误导。

  更改服务器类型(participant/learner)也不可行,因为 NuRaft 不支持此功能,唯一的方法是删除并重新添加服务器,这同样会造成误导。
...
- **重要提示**: 新节点必须以小于当前法定人数的批次添加,否则它们会在彼此之间选举出 Leader。本示例中采用逐个添加的方式。
