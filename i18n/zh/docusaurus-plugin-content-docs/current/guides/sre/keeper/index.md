...
ZooKeeper 是最早广为人知的开源协调系统之一。它用 Java 实现，具有简单而强大的数据模型。ZooKeeper 的协调算法 ZooKeeper Atomic Broadcast (ZAB) 不为读操作提供线性一致性保证，因为每个 ZooKeeper 节点本地提供读服务。与 ZooKeeper 不同，ClickHouse Keeper 使用 C++ 编写，并采用 [RAFT 算法](https://raft.github.io/)的[实现](https://github.com/eBay/NuRaft)。该算法允许对读写操作提供线性一致性，并且在不同语言中有多种开源实现。
...
| `server_id`                         | 唯一的服务器 ID，ClickHouse Keeper 集群中的每个节点都必须有唯一编号 (1、2、3……) 。                                                                               | -                                                                                                            |
...
| `http_control`                      | [HTTP 控制接口](#http-control) 接口的配置。                                                                                                   | -                                                                                                            |
...
ClickHouse Keeper 也提供了与 Zookeeper 几乎相同的 4lw 命令。每个命令由四个字母组成，例如 `mntr`、`stat` 等。有一些更有用的命令：`stat` 提供有关服务器和已连接客户端的通用信息，`srvr` 提供关于服务器的扩展详细信息，而 `cons` 提供关于连接的扩展详细信息。
...
* `csnp`: 调度一个快照创建任务。成功时返回已提交日志中该快照的最后索引；失败时返回 `Failed to schedule snapshot creation task.`。`lgif` 命令可帮助你判断快照是否已完成。
...
* `rqld`: 请求成为新的 leader。如果请求已发送，则返回 `Sent leadership request to leader.`；如果请求未发送，则返回 `Failed to send leadership request to leader.`。如果节点已经是 leader，则结果与请求已发送时相同。
...
* `ydld`: 请求让出领导权并成为 follower。如果接收请求的服务器是 leader，它会先暂停写入操作，等待接任节点 (当前 leader 永远不能成为接任节点) 完成对最新日志的追赶，然后再主动退位。接任节点将自动选出。如果请求已发送则返回 `Sent yield leadership request to leader.`，如果请求未发送则返回 `Failed to send yield leadership request to leader.`。如果节点已经是 follower，则结果与请求已发送时相同。
...
无法实现从 ZooKeeper 到 ClickHouse Keeper 的无缝迁移。需要先停止 ZooKeeper 集群、转换数据，然后再启动 ClickHouse Keeper。`clickhouse-keeper-converter` 工具可将 ZooKeeper 日志和快照转换为 ClickHouse Keeper 快照。它需要 ZooKeeper 3.4 或更高版本。

#### 迁移前准备 \{#pre-migration-preparation\}

迁移需要停止数据摄取。开始前，请预留维护窗口。
...
6. 将快照复制到所有 ClickHouse Keeper 节点。在任一节点启动之前，每个节点上都必须有该快照——如果某个节点在没有快照的情况下启动，可能会以空状态将自己选举为 领导者。

7. 更新 ClickHouse 配置，使其指向新的 Keeper 集群。

8. 在所有节点上启动 ClickHouse Keeper，然后重启 ClickHouse。

9. 将各项指标与迁移前的基线进行对比，以验证一致性。

10. 恢复后台任务，并重新开始数据摄取。


#### 合并多个 ZooKeeper 集群 \{#consolidating-multiple-zookeeper-clusters\}

如果您运行了多个 ZooKeeper 集群 (例如每个分片组一个) ，您可以将它们整合到一个 ClickHouse Keeper 集群中。官方 `clickhouse-keeper-converter` 工具仅支持一对一转换 (一个 ZooKeeper 集群对应一个 Keeper 快照) ，因此如需整合，则需要修改转换器源代码以合并多个快照：

1. 在每个 ZooKeeper 集群上分别运行 `clickhouse-keeper-converter`，并将各自的输出写入不同目录。
2. 按顺序反序列化这些快照文件。合并时，重新计算 `numChildren` 的值，以避免不同源集群的命名空间之间出现节点 ID 冲突。
3. 将合并后的输出写入目标 ClickHouse Keeper 快照目录。

#### 处理加密与 ACL \{#handling-encryption-and-acls\}

ClickHouse Keeper 支持与 ZooKeeper 相同的 ACL 方案 (`world`、`auth`、`digest`) 。转换过程中如何处理 ACL，取决于你的 ZooKeeper 配置：

* **完全加密或完全未加密**：可直接转换。转换器会保留现有 ACL 信息。
* **部分加密**：转换前，先为超级管理员账户授予权限，并使用 `setAcl -R` 清除受影响路径上的 ACL。转换完成后，如有需要，再在 ClickHouse Keeper 中重新启用加密。

#### 验证迁移 \{#verifying-the-migration\}

启动 ClickHouse Keeper 并重启 ClickHouse 后，请将关键指标与迁移前的基线对比，以确认迁移成功。

合并多个 ZooKeeper 集群时，请注意区分：

* **公共路径**：多个源集群中都存在且数据相同的路径——这些路径应在合并后的输出中去重。
* **差异路径**：仅存在于特定集群下的路径 (例如各分片组在 `/clickhouse/tables` 下的路径) ——这些路径必须从对应的源中保留下来。

不要直接遍历大型 ZooKeeper 树进行比较。应在转换过程中将所有转换后的路径输出到文件中。

#### 迁移后调优 \{#post-migration-tuning\}

迁移后，如果集群规模更大或需要更高处理量，请考虑调整以下设置：

| 设置                                    | 默认值     | 推荐值     | 说明                         |
| ------------------------------------- | ------- | ------- | -------------------------- |
| `max_requests_batch_size`             | `100`   | `10000` | 对于 part 数量较多或分片较多的集群，请增大此值 |
| `force_sync`                          | `true`  | `false` | 异步日志写入可提升处理量               |
| `compress_logs`                       | `false` | `true`  | 压缩 Raft 日志文件，以减少磁盘 I/O     |
| `compress_snapshots_with_zstd_format` | `true`  | —       | 默认已启用；使用 zstd 压缩快照         |

这些设置需在 [Keeper 配置](#internal-coordination-settings) 的 `coordination_settings` 下配置。

### 在丢失仲裁后的恢复 \{#recovering-after-losing-quorum\}
...
因此要向集群添加或移除节点，必须具备仲裁。如果在无法重启的情况下，集群中同时有太多节点宕机，
...
1. 选择一个 Keeper 节点作为新的 领导者。请注意，将使用该节点上的数据作为整个集群的数据，因此建议选择状态最新的节点。
...
5. 依次启动新节点上的 Keeper 实例，在启动下一个节点之前，确保 `mntr` 命令对 `zk_server_state` 的返回值为 `跟随者`。
6. 在恢复模式下，领导者 节点在与新节点达成仲裁之前，会对 `mntr` 命令返回错误信息，并拒绝来自客户端和 跟随者 的任何请求。
7. 达成仲裁后，领导者 节点会恢复到正常运行模式，使用 Raft 接受所有请求——可通过 `mntr` 进行验证，此时 `zk_server_state` 应返回 `领导者`。
...
此外，`keeper_server.latest_log_storage_disk` 可用于最新日志，`keeper_server.latest_snapshot_storage_disk` 可用于最新快照。
...
1. 让我们在 2 个节点上配置一个包含 2 个分片且每个分片只有 1 个副本的简单集群。第三个节点将用于满足 ClickHouse Keeper 的仲裁 (仲裁) 要求。在 `chnode1` 和 `chnode2` 上更新配置。下面的集群配置在每个节点上定义了 1 个分片，总计 2 个分片且不启用复制。在此示例中，一部分数据会位于一个节点上，另一部分数据会位于另一个节点上：
