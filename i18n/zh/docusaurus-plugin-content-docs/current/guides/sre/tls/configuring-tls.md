...
<VerticalStepper headerLevel="h2">
  ...
  对于此部署环境，每个节点都使用以下 ClickHouse Keeper 设置。每台服务器都会有自己的 `<server_id>`。 (例如，节点 `chnode1` 使用 `<server_id>1</server_id>`，以此类推。)
  ...
  6. 使用证书和路径配置 OpenSSL
     ...
     有关更多信息，请访问 [此页面](/operations/server-configuration-parameters/settings/#openssl)
  ...
  9. 禁用默认的 MySQL 和 PostgreSQL 模拟端口：
     ```xml
  ...
  3. 验证 ClickHouse Keeper 健康状况
     The typical [4 letter word (4lW)](/guides/sre/keeper/reference#four-letter-word-commands) 命令在不使用 TLS 的情况下无法通过 `echo` 使用，以下介绍如何结合 `openssl` 使用这些命令。
     * 启动一个 `openssl` 交互式会话
...
## 为独立运行的 ClickHouse Keeper 配置 OpenSSL \{#1-create-a-clickhouse-deployment\}
...
## OpenSSL 验证模式和证书 handler \{#2-create-tls-certificates\}
