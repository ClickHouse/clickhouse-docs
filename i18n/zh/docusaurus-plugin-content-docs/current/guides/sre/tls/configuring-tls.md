...
1. 在 ClickHouse 服务器的 `config.xml` 中，将以下内容添加到 `<clickhouse>` 标签内

    :::note
    对于生产环境，建议在 `config.d` 目录中使用单独的 `.xml` 配置文件。
    更多信息请参阅 https://clickhouse.com/docs/operations/configuration-files/
    :::
    ...
    :::note
    当 ClickHouse Keeper 嵌入在 ClickHouse 服务器中时（如上所示），Keeper 使用在[步骤 5.6](#5-configure-tls-interfaces-on-clickhouse-nodes)中定义的服务器 OpenSSL 配置。如果您将 ClickHouse Keeper 作为独立进程运行，则必须在 Keeper 配置文件中添加一个 `<openSSL>` 部分，并使用相同的 CA 证书和节点证书/密钥设置。有关详细信息，请参阅下方的[为独立 ClickHouse Keeper 配置 OpenSSL](#configure-openssl-for-standalone-clickhouse-keeper)。
    :::

2. 在所有节点上取消注释并更新 keeper 设置，并将 `<secure>` 标志设为 1：
    ```xml
    <zookeeper>
        <node>
...
3. 在 `chnode1` 和 `chnode2` 上更新并添加以下集群设置。`chnode3` 将用于 ClickHouse Keeper 仲裁。
    ...
    :::note
    对于此配置，仅配置了一个示例集群。测试示例集群必须删除、注释掉，或者如果存在一个正在测试的现有集群，则必须更新端口并添加 `<secure>` 选项。如果在安装期间或在 `users.xml` 文件中，已将 `default` 用户初始配置为带有密码，则必须设置 `<user>` 和 `<password>`。
    :::

    以下配置会在两台服务器上创建一个具有一个分片副本的集群（每个节点上各有一个）。
    ```xml
    <remote_servers>
        <cluster_1S_2R>
            <shard>
