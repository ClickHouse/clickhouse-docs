...
| 9000  | 原生协议端口 (也称为 ClickHouse TCP 协议) 。由 `clickhouse-server`、`clickhouse-client` 和原生 ClickHouse 工具等 ClickHouse 应用和进程使用。用于分布式查询时的服务器间通信。 |       | ✓   |
| 9004  | MySQL 兼容端口                                                                                                                       |       | ✓   |
| 9005  | PostgreSQL 兼容端口 (如果为 ClickHouse 启用了 SSL，则也用于安全通信) 。                                                                              |       | ✓   |
...
| 9234  | 推荐的 ClickHouse Keeper Raft 端口 (如果启用了 `<secure>1</secure>`，则也用于安全通信)                                                              |       | ✓   |
