...
sidebar_position: 20
title: '配置 SSL-TLS'
description: '本指南介绍如何通过简单的最小配置，使 ClickHouse 使用 OpenSSL 证书来验证连接。'
...
本指南提供了一组简单的最小配置，用于让 ClickHouse 使用 OpenSSL 证书来验证连接。作为演示，我们会创建一个自签名的证书颁发机构（CA）证书和密钥，并为各个节点签发证书，再通过相应配置建立连接。

:::note
TLS 的实现较为复杂，需要权衡许多选项才能确保部署的安全性和健壮性。本教程仅提供基础的 SSL/TLS 配置示例。请与贵组织的 PKI/安全团队协作，为贵组织生成合适的证书。

可查阅这篇[关于证书使用的基础教程](https://ubuntu.com/server/docs/security-certificates)，以获取入门级概览。
:::
...
:::note
使用自签名证书仅用于演示目的,不应在生产环境中使用。证书请求应由组织签名,并使用将在设置中配置的 CA 链进行验证。但是,这些步骤可用于配置和测试设置,之后可以替换为实际使用的证书。
:::
...
:::note
此操作必须在每个节点上执行。请在每个主机上使用相应的证书和密钥。
:::
...
2. 将 CA 证书、节点证书以及对应各节点的密钥复制到新建的 certs 目录中。
...
3. 更新并将以下集群设置添加到 `chnode1` 和 `chnode2`。`chnode3` 将用于 ClickHouse Keeper 仲裁。
   ...
   :::note
   在此配置中,仅配置了一个示例集群。测试示例集群必须删除或注释掉,或者如果存在正在测试的现有集群,则必须更新端口并添加 `<secure>` 选项。如果在安装时或在 `users.xml` 文件中为 `default` 用户初始配置了密码,则必须设置 `<user>` 和 `<password>`。
   :::

   以下配置创建一个包含一个分片的集群,该分片在两台服务器上各有一个副本(每个节点一个)。
...
6.  使用证书和路径配置 OpenSSL
    ...
    更多信息请访问 https://clickhouse.com/docs/operations/server-configuration-parameters/settings/#server_configuration_parameters-openssl
...
7.  在每个节点上为 SSL 配置 gRPC:
    ...
    更多信息请访问 https://clickhouse.com/docs/interfaces/grpc/
...
3. 验证 ClickHouse Keeper 运行状况
   在没有 TLS 的情况下,使用 `echo` 无法执行典型的 [4 字母命令 (4lW)](/guides/sre/keeper/index.md#four-letter-word-commands),以下是使用 `openssl` 执行这些命令的方法。
   - 使用 `openssl` 启动交互式会话
...
5. 使用 `https` 接口，通过 `https://chnode1.marsnet.local:8443/play` 登录 Play UI。
   ...
   :::note
   浏览器会显示证书不受信任，因为是从工作站访问，并且证书不在客户端机器的根 CA 证书存储中。
...
本文重点介绍了如何为 ClickHouse 环境配置 SSL/TLS。生产环境中的设置会因不同需求而异,例如证书验证级别、协议、加密算法等。通过本文,您应该已经充分了解了配置和实现安全连接所涉及的各个步骤。
