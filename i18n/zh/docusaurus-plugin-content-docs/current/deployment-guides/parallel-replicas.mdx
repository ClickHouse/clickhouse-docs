...
'keywords':
- 'parallel replica'
'description': '在本指南中，我们将首先讨论 ClickHouse 如何通过分布式表在多个分片上分配查询，然后讨论查询如何利用多个副本进行执行。'
---
...
ClickHouse 处理查询的速度极快，但这些查询是如何在多个服务器之间分配和并行执行的？

> 在本指南中，我们将首先讨论 ClickHouse 如何通过分布式表将查询分布到多个分片，然后讨论查询如何利用多个副本来执行。

## 分片架构 {#sharded-architecture}

在无共享架构中，集群通常被拆分为多个分片，每个分片包含整体数据的一个子集。一个分布式表位于这些分片之上，提供了完整数据的统一视图。

读取可以发送到本地表。查询执行将仅在指定的分片上进行，或者可以发送到分布式表，在这种情况下，每个分片都会执行给定的查询。查询分布式表的服务器将聚合数据并响应客户端：
...
<ol className="docs-ordered-list">
    <li>
        选择查询通过负载均衡器被任意发送到某个节点的分布式表（通过轮询策略或被路由到特定服务器）。此节点现在将充当协调者。
    </li>
    <li>
        节点将根据分布式表指定的信息找到每个需要执行查询的分片，并将查询发送到每个分片。
    </li>
    <li>
        每个分片会本地读取、过滤和聚合数据，然后将可合并状态发送回协调者。
    </li>
    <li>
...
当我们将副本引入时，过程相似，唯一的区别是每个分片只会有一个副本执行查询。这意味着可以更高效地并行处理更多查询。

## 非分片架构 {#non-sharded-architecture}

ClickHouse Cloud 的架构与上述架构非常不同。
（有关更多详细信息，请参见 ["ClickHouse Cloud Architecture"](https://clickhouse.com/docs/cloud/reference/architecture)）。由于计算和存储的分离，以及几乎无限的存储量，分片的需求变得不那么重要。

下图展示了 ClickHouse Cloud 的架构：

<Image img={image_2} size="md" alt="non sharded architecture" />

这种架构允许我们几乎瞬时地添加和移除副本，确保非常高的集群可扩展性。ClickHouse Keeper 集群（如右图所示）确保我们拥有元数据的单一真实来源。副本可以从 ClickHouse Keeper 集群获取元数据，并保持相同的数据。数据存储在对象存储中，SSD 缓存加快了查询速度。

但我们现在如何在多个服务器之间分配查询执行呢？在分片架构中，这很明显，因为每个分片实际上可以在数据的一个子集上执行查询。当没有分片时，情况是如何的呢？

## 引入并行副本 {#introducing-parallel-replicas}

为了通过多个服务器并行化查询执行，我们首先需要能够将我们的一个服务器指定为协调者。协调者负责创建需要执行的任务列表，确保它们被全部执行、聚合以及将结果返回给客户端。与大多数分布式系统一样，这将是接收初始查询的节点的角色。我们还需要定义工作单元。在分片架构中，工作单元是分片，数据的一个子集。使用并行副本时，我们会使用表的一个小部分，称为 [granules](/docs/guides/best-practices/sparse-primary-indexes#data-is-organized-into-granules-for-parallel-data-processing)，作为工作单元。

现在，让我们看看它在实践中的工作方式，借助下图：
...
<ol className="docs-ordered-list">
    <li>
        客户端的查询经过负载均衡器后发送到一个节点。该节点成为此查询的协调者。
    </li>
    <li>
        节点分析每个分片的索引，并选择合适的分片和 granules 进行处理。
    </li>
    <li>
        协调者将工作负载划分为一组可以分配给不同副本的 granules。
    </li>
    <li>
        每组 granules 由相应的副本处理，并在完成后将可合并的状态发送给协调者。
    </li>
    <li>
        最后，协调者合并所有副本的结果，然后将响应返回给客户端。
    </li>
</ol>

上述步骤概述了理论上并行副本的工作原理。然而，实际上，有许多因素可能导致这种逻辑无法完美运行：
...
<ol className="docs-ordered-list">
    ...
    <li>
        ClickHouse 中的复制是异步的，某些副本在某些时间点可能没有相同的分片。
    </li>
    <li>
        需要以某种方式处理副本间的尾延迟。
    </li>
    <li>
        基于每个副本上的活动，文件系统缓存因副本而异，意味着随机任务分配可能导致由于缓存局部性而降低性能。
    </li>
</ol>
...
为了解决上述（1）和（2）的问题，我们引入了公告的概念。让我们使用下图来可视化其工作原理：
...
<ol className="docs-ordered-list">
    ...
    <li>
        协调节点发送请求以获取集群中所有副本的公告。副本可能对表的当前分片有略微不同的视图。因此，我们需要收集这些信息以避免错误的调度决策。
    </li>
    <li>
        协调节点然后使用公告定义一组可以分配给不同副本的 granules。在这里，例如，我们可以看到，没有从第 3 个分片的 granules 被分配给第 2 个副本，因为该副本在其公告中未提供此分片。同时注意没有任务被分配给第 3 个副本，因为该副本没有提供公告。
    </li>
    <li>
        每个副本在其 granules 子集上处理查询并将可合并状态发送回协调者后，协调者合并结果并将响应发送给客户端。
    </li>
</ol>

### 动态协调 {#dynamic-coordination}

为了解决尾延迟问题，我们增加了动态协调。这意味着所有 granules 不是一次性发送给副本，而是每个副本都能够请求一个新任务（要处理的 granules 集合）给协调者。协调者将根据接收到的公告为副本提供 granules 集合。

假设我们处于处理阶段，所有副本都已发送了包含所有分片的公告。
...
<ol className="docs-ordered-list">
    <li>
        副本让协调节点知道它们能够处理任务，它们还可以指定它们可以处理多少工作。
    </li>
    <li>
        协调者将任务分配给副本。
    </li>
...
<ol className="docs-ordered-list">
    <li>
        副本 1 和 2 能够非常快速地完成任务。它们会请求协调节点的另一个任务。
    </li>
    <li>
        协调者将新任务分配给副本 1 和 2。
    </li>
...
<ol className="docs-ordered-list">
    <li>
        所有副本现在都完成了它们的任务处理。它们请求更多的任务。
    </li>
    <li>
        协调者使用公告检查剩余需要处理的任务，但没有剩余任务。
    </li>
    <li>
        协调者告诉副本一切都已处理。现在它将合并所有可合并的状态并响应查询。
    </li>
</ol>

### 管理缓存局部性 {#managing-cache-locality}

最后一个潜在问题是我们如何处理缓存局部性。如果查询被多次执行，我们如何确保相同的任务被路由到相同的副本？在前一个示例中，我们有以下任务分配：
...
为了确保相同的任务被分配给相同的副本，并能利用缓存，发生了两件事情。计算任务的分片 + granules 集合的哈希。应用任务分配时的副本数的模运算。

在纸面上这听起来不错，但在实际操作中，一旦某个副本负载突然增加，或者网络出现故障，如果相同的副本持续用于执行某些任务，就会引入尾延迟。如果 `max_parallel_replicas` 小于副本数量，则会随机选择副本进行查询执行。

### 任务窃取 {#task-stealing}

如果某个副本处理任务的速度比其他副本慢，那么其他副本将尝试通过哈希“窃取”原则上属于该副本的任务，以减少尾延迟。

### 限制 {#limitations}

这一功能已知存在一些限制，其中主要的限制在本节中进行了记录。

:::note
如果您发现与以下限制无关的问题，并怀疑并行副本是原因，请在 GitHub 上使用 `comp-parallel-replicas` 标签打开一个问题。
:::

| 限制                                        | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|-----------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 复杂查询                                    | 当前并行副本对于简单查询的工作效果很好。复杂的层次，如 CTE、子查询、JOIN、非平坦查询等，可能对查询性能产生负面影响。                                                                                                                                                                                                                                                                                                                                                |
| 小查询                                      | 如果您正在执行一个不处理大量行的查询，使用多个副本执行可能不会得到更好的性能，原因是副本间协调所需的网络时间可能会导致查询执行的额外周期。您可以通过使用设置 [`parallel_replicas_min_number_of_rows_per_replica`](/docs/operations/settings/settings#parallel_replicas_min_number_of_rows_per_replica) 来限制这些问题。                                           |
| 使用 FINAL 时并行副本被禁用                  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 高基数数据和复杂聚合                        | 需要发送大量数据的高基数聚合可能会显著减慢您的查询。                                                                                                                                                                                                                                                                                                                                                                           |
...
| 设置                                            | 描述                                                                                                                                                                                                                                                         |
|----------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `enable_parallel_replicas`                         | `0`: 禁用<br/> `1`: 启用 <br/>`2`: 强制使用并行副本，若未使用将抛出异常。                                                                                                                                                          |
| `cluster_for_parallel_replicas`                    | 用于并行复制的集群名称；如果您使用 ClickHouse Cloud，请使用 `default`。                                                                                                                                                                 |
| `max_parallel_replicas`                            | 用于查询执行的最大副本数，如果指定的数字低于集群中的副本数量，则节点将随机选择。该值也可以超承载以考虑水平扩展。 |
| `parallel_replicas_min_number_of_rows_per_replica` | 有助于基于需要处理的行数限制所用副本的数量，使用的副本数量由以下公式定义：<br/> `估计要读取的行数` / `每个副本的最小行数`。                                                               |
| `allow_experimental_analyzer`                      | `0`: 使用旧分析器<br/> `1`: 使用新分析器。 <br/><br/>并行副本的行为可能依据所使用的分析器而有所不同。                                                                                                                                    |

## 调查并行副本问题 {#investigating-issues-with-parallel-replicas}

您可以在 [`system.query_log`](/docs/operations/system-tables/query_log) 表中查看每个查询所使用的设置。您还可以查看 [`system.events`](/docs/operations/system-tables/events) 表，以查看服务器上发生的所有事件，并且可以使用 [`clusterAllReplicas`](/docs/sql-reference/table-functions/cluster) 表函数查看所有副本上的表（如果您是云用户，请使用 `default`）。
...
[`system.text_log`](/docs/operations/system-tables/text_log) 表也包含有关使用并行副本执行的查询的信息：
...
最后，您还可以使用 `EXPLAIN PIPELINE`。它突出展示 ClickHouse 将如何执行查询以及将用于查询执行的资源。让我们以以下查询为例：
