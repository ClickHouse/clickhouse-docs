...
{/*
  ## What is OLAP?                
  OLAP 场景需要在大型数据集上对复杂的分析型查询提供实时响应,具有以下特征：
  - 数据集可能非常庞大——数十亿或数万亿行
  - 数据组织在包含许多列的表中
  - 任何特定查询只需选择少数几列
  - 结果必须在毫秒或秒级返回
  ...
  在行式 DBMS 中，数据按行存储，与一行相关的所有值在物理上彼此相邻存储。

  在列式 DBMS 中，数据按列存储，同一列的值集中存储在一起。

  ## Why column-oriented databases work better in the OLAP scenario                                                                  

  列式数据库更适合 OLAP 场景：它们在处理大多数查询时至少快 100 倍。下文将详细解释其原因，但这一事实通过可视化演示更容易理解：
  ...
  ClickHouse 充分利用所有可用的系统资源，以尽可能快的速度处理每个分析型查询。这得益于分析能力的独特组合以及对实现最快 OLAP 数据库所需的底层细节的关注。

  有助于深入了解此主题的文章包括：
  - [ClickHouse 性能](/concepts/why-clickhouse-is-so-fast)
  - [ClickHouse 的独特功能](/about-us/distinctive-features.md)
  ...
  1.  对于分析型查询，只需读取少量表列。在列式数据库中，你可以只读取所需的数据。例如，如果你需要 100 列中的 5 列，可以预期 I/O 减少 20 倍。
  2.  由于数据以数据包形式读取，因此更容易压缩。列中的数据也更容易压缩。这进一步减少了 I/O 量。
  3.  由于 I/O 减少，更多数据可以装入系统缓存。

  例如，查询「统计每个广告平台的记录数」需要读取一个「广告平台 ID」列，该列未压缩时占用 1 字节。如果大部分流量不是来自广告平台，你可以预期该列至少有 10 倍的压缩率。使用快速压缩算法时，数据解压缩速度可以达到每秒至少数 GB 的未压缩数据。换句话说，此查询可以在单台服务器上以大约每秒数十亿行的速度处理。这种速度在实践中确实可以达到。

  ### CPU       

  由于执行查询需要处理大量行，因此对整个向量而不是单独的行分派所有操作，或者实现查询引擎使分派成本几乎为零，都会有所帮助。如果不这样做，在任何性能尚可的磁盘子系统下，查询解释器都会不可避免地使 CPU 停滞。因此，既按列存储数据，又在可能的情况下按列处理数据是有意义的。

  有两种方法可以做到这一点：

  1.  向量引擎。所有操作都是针对向量而不是单独的值编写的。这意味着你不需要频繁调用操作，分派成本可以忽略不计。操作代码包含优化的内部循环。

  2.  代码生成。为查询生成的代码包含所有间接调用。

  这在行式数据库中不会这样做，因为在运行简单查询时没有意义。但也有例外。例如，MemSQL 使用代码生成来减少处理 SQL 查询时的延迟。（相比之下，分析型 DBMS 需要优化吞吐量，而不是延迟。）
