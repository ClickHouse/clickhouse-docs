...
description: 'ClickHouse® 是一款列式 SQL 数据库管理系统 (DBMS)，用于联机分析处理 (OLAP)。它既有开源软件版本，也提供云服务。'
...
ClickHouse® 是一款高性能的列式 SQL 数据库管理系统（DBMS），面向联机分析处理（OLAP）。它既提供[开源软件](https://github.com/ClickHouse/ClickHouse)版本，也提供[云服务](https://clickhouse.com/cloud)版本。


## 什么是分析？ {#what-are-analytics}

分析，也称为 OLAP(在线分析处理),是指对海量数据集执行包含复杂计算(例如聚合、字符串处理、算术运算)的 SQL 查询。

与事务查询(或 OLTP,在线事务处理)不同——事务查询每次仅读写少量行,因此可在毫秒内完成——分析查询通常需要处理数十亿甚至数万亿行数据。

在许多使用场景中,[分析查询必须是"实时的"](https://clickhouse.com/engineering-resources/what-is-real-time-analytics),即在不到一秒的时间内返回结果。
...
由于每一列的值在磁盘上按顺序连续存储，在运行上面的查询时，不会加载不必要的数据。
由于按数据块进行的存储以及从磁盘到内存的传输方式与分析型查询的数据访问模式相契合，查询时只会从磁盘读取所需的列，从而避免为未使用的数据产生不必要的 I/O 开销。与需要读取整行（包括无关列）的行式存储相比，这样做[要快得多](https://benchmark.clickhouse.com/)：
...
ClickHouse 采用异步多主复制机制,确保数据冗余存储在多个节点上。数据写入任意可用副本后,其余所有副本会在后台获取数据副本。系统在不同副本上保持数据一致。大多数故障后的恢复可自动执行,复杂情况下则采用半自动方式。


## 基于角色的访问控制 {#role-based-access-control}

ClickHouse 通过 SQL 查询实现用户账户管理,并允许配置基于角色的访问控制,这与 ANSI SQL 标准和主流关系型数据库管理系统中的实现方式类似。
...
ClickHouse 提供了以准确性换取性能的方法。例如,其某些聚合函数可以近似计算唯一值数量、中位数和分位数。此外,查询可以在数据样本上运行以快速计算近似结果。最后,聚合可以仅针对有限数量的键运行,而不是针对所有键。根据键分布的偏斜程度,这可以提供相当准确的结果,同时使用的资源远少于精确计算。
...
要了解 ClickHouse 为何如此快速,请参阅[为什么 ClickHouse 如此快速?](/concepts/why-clickhouse-is-so-fast.mdx)指南。


<!--
## What is OLAP? {#what-is-olap}
OLAP 场景需要在大规模数据集上对复杂的分析查询提供实时响应,具有以下特征:
- 数据集规模可能非常庞大——数十亿甚至数万亿行
- 数据以包含大量列的表格形式组织
- 任何特定查询只需选择少数几列
- 查询结果必须在毫秒或秒级内返回
...
本文的其余部分将解释为什么列式数据库在这些场景中表现出色，以及为什么 ClickHouse 在这一类别中特别[优于](/concepts/why-clickhouse-is-so-fast/concepts/why-clickhouse-is-so-fast#storage-layer-concurrent-inserts-and-selects-are-isolated)其他数据库。
...
这些示例仅展示了数据的排列顺序。不同列的值是分开存储的,而同一列的数据存储在一起。
...
不同的数据存储顺序更适合不同的场景。数据访问场景是指执行什么查询、执行频率如何、比例如何;每种类型的查询读取多少数据——行数、列数和字节数;读取和更新数据之间的关系;数据的工作集大小以及数据的局部性使用程度;是否使用事务以及事务的隔离程度;对数据复制和逻辑完整性的要求;对每种类型查询的延迟和吞吐量的要求等等。

系统负载越高,根据使用场景的要求定制系统配置就越重要,这种定制也会变得越精细。没有任何系统能够同样适合显著不同的场景。如果一个系统能够适应广泛的场景,那么在高负载下,该系统要么对所有场景的处理都同样糟糕,要么只对一个或少数几个可能的场景表现良好。
...
- 查询提取大量行,但只涉及少量列。
- 对于简单查询,允许约 50 毫秒的延迟。
- 每个查询涉及一个大表;除了一个表之外,所有表都很小。
- 查询结果明显小于源数据。换句话说,数据经过过滤或聚合,因此结果可以放入单台服务器的内存中。
- 查询相对较少(通常每台服务器每秒数百个查询或更少)。
- 插入以相当大的批次进行(\> 1000 行),而不是单行插入。
- 不需要事务。

很容易看出,OLAP 场景与其他流行场景(如 OLTP 或键值访问)有很大不同。因此,如果想要获得良好的性能,尝试使用 OLTP 或键值数据库来处理分析查询是没有意义的。例如,如果尝试使用 MongoDB 或 Redis 进行分析,与 OLAP 数据库相比,性能会非常差。


### 输入/输出 {#inputoutput}

1.  对于分析查询，只需读取少量表列。在列式数据库中，您可以只读取所需的数据。例如，如果需要从 100 列中读取 5 列，I/O 可减少 20 倍。
2.  由于数据以数据包形式读取，压缩更加容易。列中的数据也更容易压缩。这进一步减少了 I/O 量。
3.  由于 I/O 减少，系统缓存可以容纳更多数据。

例如，查询"统计每个广告平台的记录数"需要读取一个"广告平台 ID"列，该列未压缩时占用 1 字节。如果大部分流量不是来自广告平台，该列可以实现至少 10 倍的压缩率。使用快速压缩算法时，数据解压缩速度可达到每秒至少数 GB 的未压缩数据。换句话说，此查询可以在单台服务器上以每秒约数十亿行的速度处理。这个速度在实践中确实可以达到。

### CPU {#cpu}

由于执行查询需要处理大量行，因此对整个向量而不是单独的行分派所有操作会有所帮助，或者实现查询引擎使其几乎没有分派成本。如果不这样做，即使使用性能尚可的磁盘子系统，查询解释器也不可避免地会使 CPU 停滞。因此，以列的形式存储数据并在可能的情况下按列处理数据是有意义的。

有两种方法可以做到这一点：

1.  向量引擎。所有操作都是针对向量而不是单独的值编写的。这意味着不需要频繁调用操作，分派成本可以忽略不计。操作代码包含优化的内部循环。

2.  代码生成。为查询生成的代码包含所有间接调用。

这在行式数据库中不会这样做，因为在运行简单查询时这样做没有意义。但也有例外。例如，MemSQL 使用代码生成来减少处理 SQL 查询时的延迟。（相比之下，分析型数据库管理系统需要优化吞吐量，而不是延迟。）
