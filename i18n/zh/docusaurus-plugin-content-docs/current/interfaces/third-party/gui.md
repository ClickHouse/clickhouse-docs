...
[1bench](https://1bench.dev) 是一款面向多种数据库的原生桌面 GUI，对 ClickHouse 提供一流支持——覆盖服务器概览、schema 管理、向量搜索以及大结果集浏览等功能。

功能：

* 连接后即可查看服务器概览——版本、运行时长、正在执行的查询、活动中的合并、parts 和存储大小、副本状态、集群与节点等信息一目了然。
* 提供可视化查询构建器 (列选择器、筛选器、排序、限制) ，并配备带语法高亮的 Monaco SQL 编辑器，以及按连接保存的查询历史。
* 提供可视化 `CREATE TABLE` 向导，支持 `MergeTree` 变体、`ORDER BY`、`PARTITION BY`、`SETTINGS` 和 `Nullable()` 自动包装。
* 原生支持 ClickHouse 类型——`Nullable`、`Array`、`LowCardinality`、嵌套对象。
* 支持向量搜索——将 `Array(Float32)` 嵌入列渲染为紧凑的向量单元，支持 2D 嵌入可视化，以及通过 `cosineDistance` 查找相似项。
* 支持在结果表中内联编辑数据并按批次保存，还支持使用 ClickHouse 原生格式进行 CSV/JSON/SQL 导出和导入。
* 连接选项包括：HTTP/HTTPS、用于连接防火墙后私有集群的 SSH 隧道，以及用于安全浏览生产环境的可选只读模式。
