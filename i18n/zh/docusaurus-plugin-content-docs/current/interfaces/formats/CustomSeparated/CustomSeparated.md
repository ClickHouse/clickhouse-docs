...
类似于 [Template](../Template/Template.md),但会打印或读取所有列的名称和类型,并使用 [format_custom_escaping_rule](../../../operations/settings/settings-formats.md/#format_custom_escaping_rule) 设置中的转义规则以及以下设置中的分隔符:
...
:::note
不使用格式字符串中的转义规则设置和分隔符。
:::

另外还有 [`CustomSeparatedIgnoreSpaces`](../CustomSeparated/CustomSeparatedIgnoreSpaces.md) 格式,类似于 [TemplateIgnoreSpaces](../Template//TemplateIgnoreSpaces.md)。
...
| 设置                                                                                                                                                        | 描述                                                                                                                  | 默认值 |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------- |
| [input_format_custom_detect_header](../../../operations/settings/settings-formats.md/#input_format_custom_detect_header)                                       | 启用自动检测包含名称和类型的表头（如果存在）。                                                           | `true`  |
...
| [input_format_custom_allow_variable_number_of_columns](../../../operations/settings/settings-formats.md/#input_format_custom_allow_variable_number_of_columns) | 允许 CustomSeparated 格式中的列数可变，忽略多余的列并对缺失的列使用默认值。 | `false` |
