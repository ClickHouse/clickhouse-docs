...
同时输出包含列名的标题行，类似于 [TabSeparatedWithNames](../TabSeparated/TabSeparatedWithNames.md)。
...
输入数据中的列将按名称映射到表中的列,
若设置 [`input_format_skip_unknown_fields`](../../../operations/settings/settings-formats.md/#input_format_skip_unknown_fields) 为 `1`,则名称未知的列将被跳过。
否则,将跳过第一行。
:::
