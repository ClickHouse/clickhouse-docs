...
对于 insert 查询，如果使用前缀或后缀，则该格式允许跳过某些列或字段 (见示例) 。
...
### 内联指定 \{#in-line-specification\}

厌倦了手动格式化 Markdown 表格吗？在本示例中，我们将介绍如何使用 `Template` 格式和内联指定设置来完成一个简单任务——从 `system.formats` 表中 `SELECT` 出一些 ClickHouse 格式的名称，并将其格式化为 Markdown 表格。这可以通过 `Template` 格式以及设置 `format_template_row_format` 和 `format_template_resultset_format` 轻松实现。
