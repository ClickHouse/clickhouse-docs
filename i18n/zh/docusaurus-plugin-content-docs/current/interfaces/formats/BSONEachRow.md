...
`BSONEachRow` 格式将数据解析为一系列二进制 JSON (BSON) 文档之间没有任何分隔符。
每一行被格式化为单个文档，每一列被格式化为具有列名作为键的单个 BSON 文档字段。

## 数据类型匹配 {#data-types-matching}

在输出中，它使用 ClickHouse 类型和 BSON 类型之间的以下对应关系：
...
| [String](/sql-reference/data-types/string.md)/[FixedString](/sql-reference/data-types/fixedstring.md) | `\x05` binary, `\x00` binary subtype 或者 \x02 字符串，如果设置了 output_format_bson_string_as_string   |
...
在输入中，它使用 BSON 类型和 ClickHouse 类型之间的以下对应关系：
...
其他 BSON 类型不受支持。此外，它在不同整数类型之间进行转换。
例如，可以将 BSON `int32` 值插入 ClickHouse 作为 [`UInt8`](../../sql-reference/data-types/int-uint.md)。

如 `Int128`/`UInt128`/`Int256`/`UInt256`/`Decimal128`/`Decimal256` 等大整数和小数可以从 BSON 二进制值中解析，采用 `\x00` 二进制子类型。
...
| 设置                                                                                                                                                                                               | 描述                                                                                      | 默认值  |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|----------|
| [`output_format_bson_string_as_string`](../../operations/settings/settings-formats.md/#output_format_bson_string_as_string)                                                                           | 对于字符串列，使用 BSON 字符串类型而不是二进制类型。                                   | `false`  |
| [`input_format_bson_skip_fields_with_unsupported_types_in_schema_inference`](../../operations/settings/settings-formats.md/#input_format_bson_skip_fields_with_unsupported_types_in_schema_inference) | 在格式 BSONEachRow 的模式推导中允许跳过不支持类型的列。 | `false`  |
