...
每行被格式化为单个文档，每列被格式化为单个 BSON 文档字段，列名作为键。

## 数据类型匹配 {#data-types-matching}

对于输出，它使用 ClickHouse 类型与 BSON 类型之间的以下对应关系：
...
| [String](/sql-reference/data-types/string.md)/[FixedString](/sql-reference/data-types/fixedstring.md)             | `\x05` binary, `\x00` binary subtype 或 \x02 string（如果设置了 output_format_bson_string_as_string） |
...
对于输入，它使用 BSON 类型与 ClickHouse 类型之间的以下对应关系：
...
其他 BSON 类型不被支持。此外，它会在不同的整数类型之间执行转换。
例如，可以将 BSON `int32` 值以 [`UInt8`](../../sql-reference/data-types/int-uint.md) 的形式插入 ClickHouse。

大整数和小数，例如 `Int128`/`UInt128`/`Int256`/`UInt256`/`Decimal128`/`Decimal256`，可以通过 `\x00` 二进制子类型从 BSON 二进制值中解析。 
...
| 设置                                                                                                                                                                                                | 描述                                                                                   | 默认值   |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------|----------|
| [`output_format_bson_string_as_string`](../../operations/settings/settings-formats.md/#output_format_bson_string_as_string)                                                                           | 对于字符串列使用 BSON 字符串类型，而不是二进制类型。                                          | `false`  |
| [`input_format_bson_skip_fields_with_unsupported_types_in_schema_inference`](../../operations/settings/settings-formats.md/#input_format_bson_skip_fields_with_unsupported_types_in_schema_inference) | 在格式 BSONEachRow 的模式推断中允许跳过具有不支持类型的列。                                   | `false`  |
