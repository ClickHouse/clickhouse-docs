...
| `DURATION`                              | [Interval](/sql-reference/data-types/special-data-types/interval.md) (纳秒/微秒/毫秒/秒)          | `DURATION`                 |
| `INT64`                                 | [Interval](/sql-reference/data-types/special-data-types/interval.md) (分钟/小时/天/周/月/季度/年)    | `INT64`                    |
...
ClickHouse 表列的数据类型不必与对应的 Arrow 数据字段完全一致。插入数据时，ClickHouse 会根据上表解释数据类型，然后将数据[转换](/sql-reference/functions/type-conversion-functions#CAST)为 ClickHouse 表列所设置的数据类型。
