...
[Apache Arrow](https://arrow.apache.org/) 提供了两种内置的列式存储格式。ClickHouse 支持这些格式的读写操作。
`Arrow` 是 Apache Arrow 的“文件模式”格式。它设计用于内存随机访问。
...
`DICTIONARY` 类型支持用于 `INSERT` 查询，对于 `SELECT` 查询，有一个 [`output_format_arrow_low_cardinality_as_dictionary`](/operations/settings/formats#output_format_arrow_low_cardinality_as_dictionary) 设置，允许将 [LowCardinality](/sql-reference/data-types/lowcardinality.md) 类型输出为 `DICTIONARY` 类型。
...
ClickHouse 表列的数据类型不必与相应的 Arrow 数据字段匹配。当插入数据时，ClickHouse 会根据上表解释数据类型，然后 [cast](/sql-reference/functions/type-conversion-functions#cast) 数据为 ClickHouse 表列所设置的数据类型。
...
| `input_format_arrow_case_insensitive_column_matching`                                                                    | 在匹配 Arrow 列与 CH 列时忽略大小写。                                                         | `0`          |
...
| `output_format_arrow_low_cardinality_as_dictionary`                                                                      | 启用将 LowCardinality 类型输出为 Dictionary Arrow 类型                                         | `0`          |
