...
CapnProto 消息采用严格类型且不具备自描述性,这意味着需要外部 schema 描述。Schema 会在运行时动态应用并针对每个查询进行缓存。

另请参阅 [格式 Schema](/interfaces/formats/#formatschema)。
...
您可以使用以下命令从 ClickHouse 表中查询数据并将其保存为 `CapnProto` 格式的文件:
...
如果您没有用于数据的外部 `CapnProto` 模式,仍然可以使用自动生成的模式以 `CapnProto` 格式输出/输入数据。
...
在这种情况下,ClickHouse 将使用 [structureToCapnProtoSchema](/sql-reference/functions/other-functions.md#structureToCapnProtoSchema) 函数根据表结构自动生成 CapnProto 模式,并使用该模式将数据序列化为 CapnProto 格式。

您还可以读取使用自动生成模式的 CapnProto 文件(在这种情况下,该文件必须使用相同的模式创建):
...
配置项 [`format_capn_proto_use_autogenerated_schema`](../../operations/settings/settings-formats.md/#format_capn_proto_use_autogenerated_schema) 默认启用,当未设置 [`format_schema`](/interfaces/formats#formatschema) 时生效。

您还可以使用配置项 [`output_format_schema`](/operations/settings/formats#output_format_schema) 在输入/输出过程中将自动生成的 schema 保存到文件。
