...
CapnProto 消息是严格类型的，并且不是自描述的，这意味着它们需要外部模式描述。该模式在运行时应用并为每个查询缓存。

另请参阅 [格式模式](/interfaces/formats/#formatschema)。

## 数据类型匹配 {#data_types-matching-capnproto}

下表显示了支持的数据类型以及它们如何与 ClickHouse [数据类型](/sql-reference/data-types/index.md) 在 `INSERT` 和 `SELECT` 查询中匹配。
...
如果您没有数据的外部 `CapnProto` 模式，您仍然可以使用自动生成的模式以 `CapnProto` 格式输入/输出数据。
...
在这种情况下，ClickHouse 将根据表结构使用函数 [structureToCapnProtoSchema](/sql-reference/functions/other-functions.md#structure_to_capn_proto_schema) 自动生成 CapnProto 模式，并将使用该模式对数据进行序列化。

您还可以读取带有自动生成模式的 CapnProto 文件（在这种情况下，该文件必须使用相同的模式创建）：
...
设置 [`format_capn_proto_use_autogenerated_schema`](../../operations/settings/settings-formats.md/#format_capn_proto_use_autogenerated_schema) 默认情况下是启用的，并且在未设置 [`format_schema`](/interfaces/formats#formatschema) 的情况下适用。
