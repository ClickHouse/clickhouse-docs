...
来自 [format_regexp](/operations/settings/settings-formats.md/#format_regexp) 设置的正则表达式应用于每一行导入的数据。正则表达式中的子模式数量必须与导入数据集中的列数相等。

导入数据的行必须由换行符 `'\n'` 或 DOS 风格换行符 `"\r\n"` 分隔。

每个匹配的子模式的内容根据相应数据类型的方法进行解析，遵循 [format_regexp_escaping_rule](/operations/settings/settings-formats.md/#format_regexp_escaping_rule) 设置。

如果正则表达式不匹配该行，并且 [format_regexp_skip_unmatched](/operations/settings/settings-formats.md/#format_regexp_escaping_rule) 设置为 1，该行将被静默跳过。否则，将抛出异常。
...
- `format_regexp_escaping_rule` — [字符串](/sql-reference/data-types/string.md)。支持以下转义规则：
  ...
  - 原始（作为一个整体提取子模式，无转义规则，类似于 [TSVRaw](/interfaces/formats/TabSeparated)）
