...
### 使用 ClickHouse Cloud 读取和写入数据 \{#basic-examples-cloud\}
...
[`structureToProtobufSchema`](/sql-reference/functions/other-functions#structureToProtobufSchema) 自动生成 Protobuf schema。然后它会使用该 schema 以 Protobuf 格式序列化数据。

你也可以使用自动生成的 schema 来读取 Protobuf 文件。在这种情况下，必须使用相同的 schema 来创建该文件：
