...
在此格式中,单个 JSON 对象被解释为单个 [JSON](/sql-reference/data-types/newjson.md) 值。如果输入包含多个 JSON 对象(以逗号分隔),则会被解释为多个独立的行。如果输入数据用方括号括起来,则会被解释为 JSON 数组。

此格式仅适用于解析具有单个 [JSON](/sql-reference/data-types/newjson.md) 类型字段的表。其余列必须设置为 [`DEFAULT`](/sql-reference/statements/create/table.md/#default) 或 [`MATERIALIZED`](/sql-reference/statements/create/view#materialized-view)。
