...
[Apache ORC](https://orc.apache.org/) 是一种列式存储格式,广泛应用于 [Hadoop](https://hadoop.apache.org/) 生态系统中。


## 数据类型匹配 {#data-types-matching-orc}

下表列出了 `INSERT` 和 `SELECT` 查询中支持的 ORC 数据类型及其对应的 ClickHouse [数据类型](/sql-reference/data-types/index.md)。
...
- ClickHouse 表列的数据类型不必与相应的 ORC 数据字段匹配。插入数据时,ClickHouse 会根据上表解释数据类型,然后将数据[转换](/sql-reference/functions/type-conversion-functions#cast)为 ClickHouse 表列设置的数据类型。
...
使用包含以下数据的 ORC 文件,文件名为 `football.orc`:
...
:::tip
ORC 是一种二进制格式,无法在终端以人类可读的形式显示。请使用 `INTO OUTFILE` 输出 ORC 文件。
:::
...
| 设置                                                                                                                                                                                              | 说明                                                                            | 默认值 |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------- |
| [`output_format_arrow_string_as_string`](/operations/settings/settings-formats.md/#output_format_arrow_string_as_string)                                                                             | 对 String 列使用 Arrow String 类型而非 Binary 类型。                            | `false` |
| [`output_format_orc_compression_method`](/operations/settings/settings-formats.md/#output_format_orc_compression_method)                                                                             | 输出 ORC 格式时使用的压缩方法。默认值                            | `none`  |
| [`input_format_arrow_case_insensitive_column_matching`](/operations/settings/settings-formats.md/#input_format_arrow_case_insensitive_column_matching)                                               | 在匹配 Arrow 列与 ClickHouse 列时忽略大小写。                       | `false` |
...
| [`input_format_arrow_skip_columns_with_unsupported_types_in_schema_inference`](/operations/settings/settings-formats.md/#input_format_arrow_skip_columns_with_unsupported_types_in_schema_inference) | 在 Arrow 格式的模式推断过程中允许跳过不支持类型的列。 | `false` |
