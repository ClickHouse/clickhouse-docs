...
Web 终端是一个浏览器内界面，通过 WebSocket 提供交互式的 `clickhouse-client` 会话。它可通过任意 ClickHouse HTTP 端口上的 `/webterminal` 路径访问。

访问任意 ClickHouse HTTP 端口上的 `/webterminal` (例如，`http://localhost:8123/webterminal`) 即可打开终端。

## 启用和禁用此功能 \{#enabling-the-feature\}

`/webterminal` 端点默认启用，由 `enable_webterminal` 服务器设置控制。要禁用该端点，请将此设置设为 `false`；此后，对 `/webterminal` 的请求将返回 HTTP 状态码 `403 Forbidden`。
...
:::note
`enable_webterminal` 已取代原先的 `allow_experimental_webterminal` 设置。如果未设置 `enable_webterminal`，为保持向后兼容，旧名称仍然有效。
:::
...
[`/play`](/interfaces/http) Web SQL UI 将 Web 终端嵌入为一个可停靠的面板。可通过侧边栏中的终端图标切换显示，也可在查询编辑器为空时按 `~` 键打开或关闭。`/play` 页面会在加载时检测 `/webterminal` 是否可用；当该端点不可用时，会隐藏终端控件 (例如当 `enable_webterminal` 设为 `false` 时) 。
...
该处理程序可在 ClickHouse 支持的所有平台上编译。嵌入式 `clickhouse-client` 运行器使用的伪终端层构建在可移植的 POSIX 基元 (`posix_openpt`/`grantpt`/`unlockpt`) 之上，并针对 Linux 提供了一条使用线程安全 `ptsname_r` 的特定实现路径。当端点不可用时 (例如将 `enable_webterminal` 设置为 `false`) ，ClickHouse 起始页和 `/play` 中指向 `/webterminal` 的链接会自动隐藏。