---
'description': 'ClickHouse中SSH接口的文档'
'keywords':
- 'client'
- 'ssh'
- 'putty'
...
ClickHouse 服务器允许使用 SSH 协议直接连接至其自身。任何客户端都是允许的。

在创建了一个 [由 SSH 密钥标识的数据库用户](/knowledgebase/how-to-connect-to-ch-cloud-using-ssh-keys):
```sql
CREATE USER abcuser IDENTIFIED WITH ssh_key BY KEY '<REDACTED>' TYPE 'ssh-ed25519';
```

您可以使用此密钥连接到 ClickHouse 服务器。这将打开一个伪终端 (PTY)，并启动 clickhouse-client 的交互会话。
...
主机密钥是 SSH 协议的一个组成部分。该密钥的公用部分存储在客户端的 `~/.ssh/known_hosts` 文件中，通常用于防止中间人攻击。当首次连接到服务器时，您将看到以下消息：
...
这实际上意味着：“您想要记住此主机的公钥并继续连接吗？”。

您可以通过传递选项告知您的 SSH 客户端不验证主机：
...
您可以以类似于普通 `clickhouse-client` 的方式向嵌入式客户端传递选项，但有一些限制。由于这是一个 SSH 协议，传递给目标主机参数的唯一方法是通过环境变量。
...
您可以通过这种方式更改任何用户级别的设置，并另外传递大多数普通的 `clickhouse-client` 选项（除了在此配置下没有意义的选项）。

重要：

如果同时传递了 `query` 选项和 SSH 命令，则后者将添加到待执行的查询列表中：
