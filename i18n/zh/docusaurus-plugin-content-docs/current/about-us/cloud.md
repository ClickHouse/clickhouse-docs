...
keywords: ['ClickHouse Cloud', '云数据库', '托管 ClickHouse', '无服务器数据库', '云端 OLAP']
doc_type: 'reference'
---
...
ClickHouse Cloud 是由广受欢迎的开源 OLAP 数据库 ClickHouse 的原始创建团队推出的云端服务。
...
- **快速创造价值**: 无需调整集群规模即可立即开始构建。
- **无缝扩展**: 自动扩展可根据工作负载变化进行调整,无需为峰值使用过度配置资源。
- **无服务器运维**: 规模调整、扩展、安全性、可靠性和升级均由我们负责,您只需专注业务。
- **透明定价**: 按实际使用量付费,并提供资源预留和扩展控制。
- **总拥有成本**: 最佳性价比和低管理开销。
- **广泛的生态系统**: 支持您常用的数据连接器、可视化工具、SQL 和语言客户端。
...
| **存储**                    | 作为开源和云托管产品,ClickHouse 可以部署在共享磁盘架构和无共享架构中。                                                                                                                                                                                                                                               | ✅               | ✅                 |
...
我们提供"快速"发布渠道,让您可以在常规发布计划之前订阅更新。有关更多详细信息,请参阅["快速发布渠道"](/manage/updates#fast-release-channel-early-upgrades)。

如果您依赖早期版本中的功能,在某些情况下,可以使用服务的兼容性设置恢复到之前的行为。
