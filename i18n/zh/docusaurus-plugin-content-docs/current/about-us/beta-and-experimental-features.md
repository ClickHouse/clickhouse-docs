---
sidebar_position: 1
sidebar_label: '测试版特性与实验性特性'
title: '测试版与实验性特性'
description: 'ClickHouse 提供测试版和实验性特性。本页文档讨论它们的定义。'
slug: /beta-and-experimental-features
doc_type: 'reference'
---

由于 ClickHouse 是开源项目，它不仅接收来自 ClickHouse 员工的贡献，还接收来自社区的贡献。这些贡献的开发节奏往往各不相同；某些特性可能需要较长的原型设计阶段，或者需要更多时间来获取足够的社区反馈并进行迭代，之后才能被视为一般可用（GA，Generally Available）。

由于特性何时会被归类为一般可用存在不确定性，我们将特性划分为两个类别：**测试版（Beta）** 和 **实验性（Experimental）**。

**测试版（Beta）** 特性由 ClickHouse 团队正式支持。**实验性（Experimental）** 特性是由 ClickHouse 团队或社区推动的早期原型，不属于正式支持范围。

下面的章节将明确说明 **测试版（Beta）** 和 **实验性（Experimental）** 特性的属性：



## Beta 功能 {#beta-features}

- 正在积极开发中，目标是实现普遍可用（GA）
- 主要已知问题可在 GitHub 上跟踪
- 功能在未来可能发生变化
- 在 ClickHouse Cloud 中可能已启用
- ClickHouse 团队为 Beta 功能提供支持

下面列出了在 ClickHouse Cloud 中被视为 Beta，且可在您的 ClickHouse Cloud 服务中使用的功能。

注意：请确保您在 ClickHouse 中使用的是当前版本的 [compatibility](/operations/settings/settings#compatibility) 设置，以便使用最近引入的功能。
...
* 该特性的具体功能可能会发生变化
* 需要明确手动启用
* ClickHouse 团队**不提供**对实验性功能的支持
* 可能缺少重要功能和文档
* 无法在云环境中启用

请注意：除上述列出的 Beta 功能外，ClickHouse Cloud 中不允许启用任何其他实验性功能。

{/* 下方标签内的内容会在构建阶段被由源数据生成的表格所替换
     请不要修改或移除这些标签
  */ }
...
| [merge&#95;selector&#95;algorithm](/operations/settings/merge-tree-settings#merge_selector_algorithm)                                                                                                                 | `简单`                    |
