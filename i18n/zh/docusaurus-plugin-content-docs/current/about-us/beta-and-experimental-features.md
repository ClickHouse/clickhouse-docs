因为 ClickHouse 是开源的，所以不仅 ClickHouse 员工，还有社区也作出了很多贡献。由于这些贡献的发展速度不同；某些特性可能需要较长的原型设计阶段，或者更多的时间来获得足够的社区反馈和迭代，从而被视为一般可用（GA）。

由于不确定何时特性被归类为一般可用，我们将特性分为两个类别：**Beta** 和 **Experimental**。

**Beta** 特性受到 ClickHouse 团队的正式支持。 **Experimental** 特性是由 ClickHouse 团队或社区推动的早期原型，并未获得正式支持。

以下部分明确描述了 **Beta** 和 **Experimental** 特性的属性：

## Beta 特性 {#beta-features}

- 在积极开发中，以使其一般可用 (GA)
- 主要已知问题可以在 GitHub 上跟踪
- 功能可能在将来发生变化
- 可能在 ClickHouse Cloud 中启用
- ClickHouse 团队支持 beta 特性

以下特性在 ClickHouse Cloud 中被认为是 Beta，并且可以在 ClickHouse Cloud Services 中使用，即使它们当前处于 ClickHouse 的设置 ```allow_experimental_*``` 下：

注意：请确保使用当前版本的 ClickHouse [compatibility](/operations/settings/settings#compatibility) 设置以使用最近引入的特性。

## Experimental 特性 {#experimental-features}
...
请注意：除了上述列为 Beta 的特性外，不允许在 ClickHouse Cloud 中启用其他实验性特性。
...
## Experimental 设置 {#experimental-settings}
