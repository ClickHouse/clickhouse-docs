...
在真正的列式数据库管理系统中,值本身不会附带存储额外的数据。这意味着必须支持定长值,以避免在值旁边存储其长度信息。例如,十亿个 UInt8 类型的值在未压缩状态下应该占用约 1 GB 空间,否则会严重影响 CPU 使用效率。即使在未压缩状态下,紧凑地存储数据(不含任何冗余信息)也至关重要,因为解压缩速度(CPU 使用率)主要取决于未压缩数据的体积。

这与那些能够分别存储不同列的值、但由于针对其他场景进行了优化而无法有效处理分析查询的系统形成对比,例如 HBase、BigTable、Cassandra 和 HyperTable。在这些系统中,您获得的吞吐量约为每秒十万行,而不是每秒数亿行。

最后,ClickHouse 是一个数据库管理系统,而不是单一数据库。它允许在运行时创建表和数据库、加载数据以及运行查询,而无需重新配置和重启服务器。
...
通过主键对数据进行物理排序,使得基于特定值或值范围提取数据时能够实现低延迟,通常在几十毫秒内即可完成。某些列式数据库管理系统,例如 SAP HANA 和 Google PowerDrill,只能在内存中运行。这种方式需要分配比实时分析实际所需更大的硬件预算。
...
大型查询会自动并行化执行,充分利用当前服务器上的所有可用资源。
...
ClickHouse 支持带有主键的表。为了快速执行主键范围内的查询,数据通过合并树进行增量排序。因此,数据可以持续不断地添加到表中。在写入新数据时不会产生锁定。
...
3.  对有限数量的随机键运行聚合,而不是对所有键进行聚合。在数据中键分布满足特定条件时,这种方法可以在使用更少资源的同时提供足够准确的结果。
...
ClickHouse 使用异步多主复制。数据写入任何可用副本后,所有其他副本会在后台获取数据副本。系统在不同副本上维护相同的数据。大多数故障后的恢复是自动执行的,在复杂情况下则采用半自动方式。
