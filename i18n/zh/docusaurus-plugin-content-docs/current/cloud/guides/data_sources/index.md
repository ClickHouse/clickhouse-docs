...
| 页面                                                           | 描述                                                            |
| -------------------------------------------------------------- | ---------------------------------------------------------------------- |
| [Cloud IP addresses](/manage/data-sources/cloud-endpoints-api) | 某些表函数和连接所需的网络配置信息 |
| [Accessing S3 data securely](/cloud/data-sources/secure-s3)    | 使用基于角色的访问控制安全访问 AWS S3 中的外部数据源         |
...
ClickHouse 支持多种表函数来访问外部数据源。更多信息请参阅 SQL 参考部分中的[表函数](/sql-reference/table-functions)。
