...
ClickHouse BYOC 针对不同使用场景提供了多种监控方式。

#### 可观测性仪表板 {#observability-dashboard}

ClickHouse Cloud 包含一个高级可观测性仪表板,可显示内存使用量、查询速率和 I/O 等指标。可在 ClickHouse Cloud Web 控制台界面的 **Monitoring** 部分访问该仪表板。
...
您可以使用来自系统表(如 `system.metrics`、`system.events` 和 `system.asynchronous_metrics` 等)的指标自定义仪表板,以详细监控服务器性能和资源利用情况。
...
ClickHouse BYOC 会在您的 Kubernetes 集群上部署 Prometheus 堆栈。您可以从中访问和抓取指标,并将其集成到您自己的监控堆栈中。
...
请改用上述部分中的 Prometheus 堆栈集成。除了 ClickHouse Server 指标外,它还提供更多指标,包括 K8S 指标和来自其他服务的指标。

ClickHouse Cloud 提供了一个 Prometheus 端点,您可以使用它来抓取监控指标。这允许与 Grafana 和 Datadog 等工具集成以进行可视化。
...
可以使用 ClickHouse 用户名和密码对进行身份验证。我们建议为抓取指标创建一个具有最小权限的专用用户。至少需要在所有副本上的 `system.custom_metrics` 表具有 `READ` 权限。例如：
...
另请参阅[这篇博客文章](https://clickhouse.com/blog/clickhouse-cloud-now-supports-prometheus-monitoring)和 [ClickHouse 的 Prometheus 配置文档](/integrations/prometheus)。
