...
ClickHouse BYOC 针对不同使用场景提供了多种方案。

#### 可观测性仪表板 {#observability-dashboard}

ClickHouse Cloud 包含一个高级可观测性仪表板,可显示内存使用量、查询速率和 I/O 等指标。可在 ClickHouse Cloud Web 控制台界面的 **Monitoring** 部分访问。
...
您可以使用系统表(如 `system.metrics`、`system.events` 和 `system.asynchronous_metrics` 等)中的指标自定义仪表板,以详细监控服务器性能和资源利用率。
...
#### 访问 BYOC Prometheus 技术栈 {#prometheus-access}

ClickHouse BYOC 在您的 Kubernetes 集群上部署了 Prometheus 技术栈。您可以从中访问和抓取指标,并将其集成到您自己的监控技术栈中。

请联系 ClickHouse 支持团队以启用私有负载均衡器并获取 URL。请注意,此 URL 仅可通过私有网络访问,且不支持身份验证。
...
请改用上述部分中的 Prometheus 技术栈集成。除 ClickHouse Server 指标外,它还提供更多指标,包括 K8S 指标和其他服务的指标。

ClickHouse Cloud 提供了 Prometheus 端点,可用于抓取监控指标。这允许与 Grafana 和 Datadog 等工具集成以实现可视化。
...
```bash
# HELP ClickHouse_CustomMetric_StorageSystemTablesS3DiskBytes system 数据库中存储在 `s3disk` 磁盘上的字节数
# TYPE ClickHouse_CustomMetric_StorageSystemTablesS3DiskBytes gauge
ClickHouse_CustomMetric_StorageSystemTablesS3DiskBytes{hostname="c-jet-ax-16-server-43d5baj-0"} 62660929
# HELP ClickHouse_CustomMetric_NumberOfBrokenDetachedParts 损坏的已分离数据分区数量
# TYPE ClickHouse_CustomMetric_NumberOfBrokenDetachedParts gauge
ClickHouse_CustomMetric_NumberOfBrokenDetachedParts{hostname="c-jet-ax-16-server-43d5baj-0"} 0
# HELP ClickHouse_CustomMetric_LostPartCount 最旧 mutation 的存在时长(秒)
...
可以使用 ClickHouse 用户名和密码进行身份验证。我们建议为采集指标创建一个具有最小权限的专用用户。至少需要在所有副本上的 `system.custom_metrics` 表具备 `READ` 权限。例如：
...
另请参阅[这篇博文](https://clickhouse.com/blog/clickhouse-cloud-now-supports-prometheus-monitoring)以及[ClickHouse 的 Prometheus 配置文档](/integrations/prometheus)。
