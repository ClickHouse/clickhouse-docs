---
title: 'ClickHouse 私有版'
slug: /cloud/infrastructure/clickhouse-private
keywords: ['私有', '本地部署']
description: 'ClickHouse 私有版概览'
doc_type: 'reference'
---
...
ClickHouse Private 是一个自部署软件包,包含与 ClickHouse Cloud 上运行的相同专有版本的 ClickHouse 以及我们的 ClickHouse Operator,配置为计算与存储分离。它部署在具有 S3 兼容存储的 Kubernetes 环境中。

该软件包目前支持 AWS 和 IBM Cloud,裸机部署即将推出。

:::note 注意
ClickHouse Private 专为具有最严格合规要求的大型企业而设计,提供对其专用基础设施的完全控制和管理能力。此选项仅可通过[联系我们](https://clickhouse.com/company/contact?loc=nav)获取。
:::
...
### 功能完备的路线图，定期发布新功能 {#full-featured-roadmap}
...
客户可以通过联系[我们](https://clickhouse.com/company/contact?loc=nav)来启动入门流程。对于符合条件的客户,我们将提供详细的环境构建指南,以及用于部署的镜像和 Helm chart 访问权限。


## 通用要求 {#general-requirements}

本节概述部署 ClickHouse Private 所需的资源。具体部署指南将在入门过程中提供。实例/服务器的类型和规模取决于具体使用场景。
