...
| `param_:name`         | 否       | 当请求体为流时使用的查询变量。将 `:name` 替换为您的变量名       | `?param_year=2024`    |
...
| `x-clickhouse-endpoint-upgrade` | 否       | 触发端点版本升级(需与版本请求头一起使用) | `1` 表示升级                              |
...
- 确保请求中包含有效的身份验证凭据
- 发送前验证 `queryEndpointId` 和 `queryVariables`
- 实现优雅的错误处理并提供适当的错误消息
...
1. 包含值为 `1` 的 `x-clickhouse-endpoint-upgrade` 请求头
2. 包含值为 `2` 的 `x-clickhouse-endpoint-version` 请求头
