...
字典是在 ClickHouse 中加速查找的一种常用手段。ClickHouse Cloud 当前支持来自 PostgreSQL、MySQL、远程和本地 ClickHouse 服务器、Redis、MongoDB 以及 HTTP 源的字典。
...
- 会话级别设置（通过 `SET` 语句设置）不会传递到 UDF 的执行上下文中
- 用户配置（profile）中的设置不会被 UDF 继承
- 查询级设置在 UDF 的执行环境中不生效
...
将此设置从 50GB 提升，以允许删除大小最高可达 1TB 的表/分区。
