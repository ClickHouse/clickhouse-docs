---
title: 'GCP 私有服务连接'
description: '本文档介绍如何使用 Google Cloud Platform (GCP) Private Service Connect (PSC) 连接到 ClickHouse Cloud，以及如何使用 ClickHouse Cloud IP 访问列表，限制仅通过 GCP PSC 地址访问您的 ClickHouse Cloud 服务。'
...
Private Service Connect (PSC) 是 Google Cloud 的一项网络功能,允许消费者在其虚拟私有云 (VPC) 网络内私密访问托管服务。同样,它允许托管服务提供商在其独立的 VPC 网络中托管这些服务,并向消费者提供私有连接。

服务提供商通过创建 Private Service Connect 服务向消费者发布其应用程序。服务消费者通过这些 Private Service Connect 类型之一直接访问这些 Private Service Connect 服务。
...
1. 必须在 PSC 级别显式启用 Global Access(参见下面的截图)。
1. 确保您的防火墙设置不会阻止来自其他区域对 PSC 的访问。
1. 请注意,您可能会产生 GCP 跨区域数据传输费用。

不支持跨区域连接。提供商和消费者区域必须相同。但是,您可以通过在 Private Service Connect (PSC) 级别启用 [Global Access](https://cloud.google.com/vpc/docs/about-accessing-vpc-hosted-services-endpoints#global-access),从 VPC 内的其他区域进行连接。
...
如果您已经为 ClickHouse 组织中的其他服务配置了 PSC，由于这种分组机制，您通常可以跳过大部分步骤，直接进入最后一步：[将"端点 ID"添加到 ClickHouse 服务允许列表](#add-endpoint-id-to-services-allow-list)。
...
:::note
以下代码示例展示了如何在 ClickHouse Cloud 服务中设置 Private Service Connect。在下面的示例中,我们将使用:
...
您需要获取 ClickHouse Cloud 服务的相关信息。您可以通过 ClickHouse Cloud 控制台或 ClickHouse API 来完成此操作。如果您打算使用 ClickHouse API,请在继续之前设置以下环境变量:
...
- 您可以从 ClickHouse 控制台(组织 -> 组织详情)获取您的组织 ID。
...
在 ClickHouse Cloud 控制台中,打开您想要通过 Private Service Connect 连接的服务,然后打开 **Settings** 菜单。点击 **Set up private endpoint** 按钮。记下 **Service name**(`endpointServiceId`)和 **DNS name**(`privateDnsHostname`)。您将在后续步骤中使用它们。
...
:::important
本节介绍通过 GCP PSC(Private Service Connect)配置 ClickHouse 的具体细节。GCP 相关步骤仅供参考,但可能会随时变化且不另行通知。请根据您的具体使用场景进行 GCP 配置。

请注意,ClickHouse 不负责配置所需的 GCP PSC 端点和 DNS 记录。

如有任何与 GCP 配置任务相关的问题,请直接联系 GCP Support。
:::
...
首先,我们将创建一个 Private Service Connection。
...
要创建 PSC Endpoint,请使用 **ADD ENDPOINT** 按钮。
...
您需要将从[获取 Private Service Connect 的 GCP 服务附件](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect)步骤中获取的"DNS 名称"指向 GCP Private Service Connect 端点的 IP 地址。这样可以确保您的 VPC/网络中的服务/组件能够正确解析该名称。
...
要删除端点,请打开 **Organization details -> Private Endpoints** 并点击删除按钮以移除该端点。

<Image
  img={gcp_pe_remove_private_endpoint}
  size='lg'
  alt='从 ClickHouse Cloud 移除私有端点'
  border
/>
...
您需要为每个应通过 Private Service Connect 访问的实例将端点 ID 添加到允许列表中。
...
- 最可能的原因是端点 ID 未添加到服务允许列表。请重新执行[_将端点 ID 添加到服务允许列表_步骤](#add-endpoint-id-to-services-allow-list)。
...
假设您尝试在 ClickHouse Cloud 中使用 [MySQL](/sql-reference/table-functions/mysql) 或 [PostgreSQL](/sql-reference/table-functions/postgresql) 表函数,并连接到托管在 GCP 上的数据库。GCP PSC 无法用于安全地建立此连接。PSC 是单向连接,它允许您的内部网络或 GCP VPC 安全地连接到 ClickHouse Cloud,但不允许 ClickHouse Cloud 连接到您的内部网络。
