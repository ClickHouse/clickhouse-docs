...
description: '本文档介绍如何使用 Google Cloud Platform (GCP) Private Service Connect (PSC) 连接到 ClickHouse Cloud，以及如何使用 ClickHouse Cloud IP 访问列表，禁止通过 GCP PSC 地址以外的地址访问你的 ClickHouse Cloud 服务。'
...
3. 确保你的防火墙设置不会阻止来自其他区域对 PSC 的访问。
4. 注意，你可能会产生 GCP 跨区域数据传输费用。
...
首先，您将创建一个 Private Service Connection (私有服务连接) 。
...
* **Target service**：使用在[获取用于 Private Service Connect 的 GCP 服务附件](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect) 步骤中获取的 `endpointServiceId`<sup>API</sup> 或 `Service name`<sup>console</sup>。
...
复制 ***PSC Connection ID***；您将在后续步骤中将其用作 ***Endpoint ID***。
...
:::important
仅直接连接到 PSC 端点 IP 地址是不够的。ClickHouse Cloud 会根据 DNS 名称 (通过 TLS 握手中的 SNI) 对连接进行路由，因此您必须配置 DNS，使私有 DNS 主机名 (`privateDnsHostname`) 解析到该端点 IP 地址。否则，ClickHouse Cloud 将无法识别该连接要访问哪个服务，连接也会失败。
:::
...
每个启用了 Private Service Connect 的服务都同时具有公共端点和私有端点。要通过 Private Service Connect 连接，你需要使用私有端点，即在[获取用于 Private Service Connect 的 GCP service attachment](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect) 步骤中获取的 `privateDnsHostname`。
