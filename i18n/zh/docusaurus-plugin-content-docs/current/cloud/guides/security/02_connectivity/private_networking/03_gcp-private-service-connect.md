---
title: 'GCP 私有服务连接'
description: '本文档介绍如何使用 Google Cloud Platform (GCP) Private Service Connect (PSC) 连接到 ClickHouse Cloud，以及如何通过 ClickHouse Cloud IP 访问列表，禁止来自非 GCP PSC 地址对您的 ClickHouse Cloud 服务的访问。'
...
Private Service Connect (PSC) 是 Google Cloud 的一项网络功能,允许消费者在其虚拟私有云 (VPC) 网络内私密访问托管服务。同样,它也允许托管服务提供方在其独立的 VPC 网络中托管这些服务,并向消费者提供私有连接。
...
1. 必须在 PSC 级别显式启用 Global Access(请参考下面的屏幕截图)。
...
您需要获取 ClickHouse Cloud 服务的相关信息。可以通过 ClickHouse Cloud 控制台或 ClickHouse API 来完成此操作。如果您要使用 ClickHouse API,请先设置以下环境变量:
...
对于任何与 GCP 配置任务相关的问题,请直接联系 GCP 支持。
:::
...
您需要将从[获取 Private Service Connect 的 GCP 服务附件](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect)步骤中获取的"DNS 名称"指向 GCP Private Service Connect 端点的 IP 地址。这样可以确保您 VPC/网络中的服务/组件能够正确解析该名称。
...
在 ClickHouse Cloud 控制台中,打开您想要通过 Private Service Connect 连接的服务,然后导航到 **Settings**。输入从[添加 Private Service 连接](#adding-a-private-service-connection)步骤中获取的 `Endpoint ID`。点击 **Create endpoint**。
...
将下面的 **ENDPOINT_ID** 替换为从[添加 Private Service 连接](#adding-a-private-service-connection)步骤中获取的 **Endpoint ID** 值
...
每个启用了 Private Link 的服务都有一个公共端点和一个私有端点。要使用 Private Link 进行连接,需要使用私有端点,该端点的 `privateDnsHostname` 可从[获取 Private Service Connect 的 GCP 服务附件](#obtain-gcp-service-attachment-and-dns-name-for-private-service-connect)中获取。
...
在此示例中,连接到 `xxxxxxx.yy-xxxxN.p.gcp.clickhouse.cloud` 主机名的请求将通过 Private Service Connect 路由。而连接到 `xxxxxxx.yy-xxxxN.gcp.clickhouse.cloud` 的请求将通过互联网路由。
...
如果使用 PSC 链接连接时遇到问题,请使用 `openssl` 检查连接性。确保 Private Service Connect 端点状态为 `Accepted`:
...
假设您尝试在 ClickHouse Cloud 中使用 [MySQL](/sql-reference/table-functions/mysql) 或 [PostgreSQL](/sql-reference/table-functions/postgresql) 表函数,并连接到托管在 GCP 中的数据库。GCP PSC 无法用于安全地建立此连接。PSC 是单向连接,它允许您的内部网络或 GCP VPC 安全地连接到 ClickHouse Cloud,但不允许 ClickHouse Cloud 连接到您的内部网络。
...
为此,请配置您的 GCP VPC 防火墙规则,以允许从 ClickHouse Cloud 连接到您的内部/私有数据库服务。请查看 [ClickHouse Cloud 区域的默认出口 IP 地址](/manage/data-sources/cloud-endpoints-api)以及[可用的静态 IP 地址](https://api.clickhouse.cloud/static-ips.json)。
