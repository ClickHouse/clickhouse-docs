...
1. AWS 账户。
2. 具有在 ClickHouse 端创建和管理私有端点所需权限的 [ClickHouse API key](/cloud/manage/openapi)。
...
获取用于配置 PrivateLink 的 `endpointServiceId` 和 `privateDnsHostname`：
