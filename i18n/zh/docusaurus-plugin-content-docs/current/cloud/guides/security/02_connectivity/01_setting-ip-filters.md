...
从 API 密钥列表中,点击 **Actions** 列下 API 密钥旁边的三个点,然后选择 **Edit**。在屏幕底部,您将找到 IP 访问列表和配置选项:
...
此截图显示了一个访问列表,允许来自某个 IP 地址范围的流量,描述为"NY Office range":
...
不建议这样做,但是允许的。我们建议您将构建在 ClickHouse 之上的应用程序公开给公众,并限制对后端 ClickHouse Cloud 服务的访问。
