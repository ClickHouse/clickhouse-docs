...
可以为具有 Service Read Only 和 Service Admin 权限的用户分配基本的 SQL 控制台角色。有关更多信息,请参阅[管理 SQL 控制台角色分配](/cloud/guides/sql-console/manage-sql-console-role-assignments)。本指南演示如何为 SQL 控制台用户创建自定义角色。

要为 SQL 控制台用户创建自定义角色并授予通用角色,请运行以下命令。电子邮件地址必须与控制台中用户的电子邮件地址匹配。
...
为 SQL 控制台用户 my.user@domain.com 创建角色并为其分配 database_developer 角色。
...
每当用户使用 SQL 控制台时,都会为其分配与其电子邮件地址关联的角色。
...
在[创建用户账户](/sql-reference/statements/create/user.md)时使用 SHA256_hash 方法来保护密码。ClickHouse 数据库密码必须包含至少 12 个字符,并满足复杂度要求:大写字母、小写字母、数字和/或特殊字符。
...
### 使用安全外壳协议 (SSH) 身份验证的数据库用户 {#database-ssh}
...
有关详细的示例演练,请参阅我们知识库中的[如何使用 SSH 密钥连接到 ClickHouse Cloud](/knowledgebase/how-to-connect-to-ch-cloud-using-ssh-keys)。
...
- 数据库角色是累加的。这意味着如果用户是两个角色的成员,该用户将拥有这两个角色授予的最大访问权限。添加角色不会导致访问权限丢失。
...
我们建议创建一个与个人关联的新用户账户,并授予该用户 default_role。这样用户执行的活动可以通过其用户 ID 进行识别,而 `default` 账户则保留用于紧急访问类型的活动。
