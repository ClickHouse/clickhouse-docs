...
title: '管理云用户'
description: '本页介绍管理员如何添加用户、管理权限分配以及移除用户'
doc_type: 'guide'
...
本指南适用于在 ClickHouse Cloud 中拥有 Organization Admin（组织管理员）角色的用户。
...
管理员可以一次邀请最多三 (3) 名用户,并在邀请时分配组织和服务级别角色。
...
2. 当用户登录 ClickHouse Cloud 时,系统将自动为其分配 `Member` 角色,该角色仅允许登录,不具有其他访问权限
3. 按照下方 `Manage user role assignments` 中的说明授予权限

### 强制仅使用 SAML 身份验证 {#enforce-saml}

一旦组织中至少有一个 SAML 用户被分配了组织管理员角色,请从组织中移除使用其他身份验证方法的用户,以强制组织仅使用 SAML 身份验证。


## 管理用户角色分配 {#manage-role-assignments}

拥有组织管理员角色的用户可以随时更新其他用户的权限。
...
选择下拉菜单项以调整所选用户的服务角色访问范围。选择 `Specific services` 时,您可以为每个服务单独控制用户的角色。
...
:::note 移除 SAML 用户
在身份提供商中从 ClickHouse 应用程序取消分配的 SAML 用户将无法登录 ClickHouse Cloud。该账户不会从控制台中自动移除,需要手动删除。
:::
