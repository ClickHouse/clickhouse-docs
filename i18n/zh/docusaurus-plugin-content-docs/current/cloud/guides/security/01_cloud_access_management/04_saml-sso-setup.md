...
<details>
  ...
  3. 在左下角,点击 **Organization** 下方的组织名称。4. 在弹出菜单中,选择 **Organization details**。5.
  记录您的 **Organization ID** 以便后续使用。
</details>
...
4.  输入组的名称和描述。此组将用于保持 SAML 应用及其相关书签应用之间的用户一致性。
...
<details>
   ...
   您将为每个组织设置一个应用程序集成,每个组织使用单独的登录 URL。
   ...
   12. 收集以下两项内容,然后转到上面的提交支持工单以完成流程:
     - Login URL
     - Certificate (Base64)
...
我们仅使用服务提供方发起的 SSO。这意味着用户需要访问 `https://console.clickhouse.cloud` 并输入电子邮件地址,然后会被重定向到 IdP 进行身份验证。已通过您的 IdP 完成身份验证的用户可以使用直接链接自动登录到您的组织,无需在登录页面输入电子邮件地址。
