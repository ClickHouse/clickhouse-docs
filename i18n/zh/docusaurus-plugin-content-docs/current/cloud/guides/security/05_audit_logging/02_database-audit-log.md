...
ClickHouse 主要将安全相关的数据库事件记录到会话日志和查询日志中。

[system.session_log](/operations/system-tables/session_log) 记录成功和失败的登录尝试,以及身份验证尝试的来源位置。此信息可用于识别针对 ClickHouse 实例的凭据填充攻击或暴力破解攻击。
...
[system.query_log](/operations/system-tables/query_log) 捕获 ClickHouse 实例中执行的查询活动。此信息可用于确定威胁行为者执行了哪些查询。
