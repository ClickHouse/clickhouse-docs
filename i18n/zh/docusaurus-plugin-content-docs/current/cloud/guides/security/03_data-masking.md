...
数据脱敏是一种用于数据保护的技术，通过将原始数据替换为另一种版本，在保留其格式和结构的同时，去除任何可识别个人身份的信息（PII）或其他敏感信息。

本指南将介绍如何在 ClickHouse 中对数据进行脱敏。
...
请注意，从该视图返回的数据经过部分脱敏处理，从而隐藏了敏感信息。
你也可以创建多个视图，根据查看者所拥有的权限级别，采用不同程度的脱敏。

为了确保用户只能访问返回脱敏数据的视图，而无法访问包含原始未脱敏数据的表，你应使用[基于角色的访问控制](/cloud/security/console-roles)，以确保只有特定角色被授予从该视图中进行 `SELECT` 的权限。
...
由于 ClickHouse 中的角色权限是叠加生效的，你必须确保那些本应只能看到脱敏视图的用户，在任何角色下都没有对基础表的 `SELECT` 权限。
...
为了确保用户只能访问包含已脱敏数据的列，可以再次使用[基于角色的访问控制](/cloud/security/console-roles)，以确保仅为特定角色授予在 `orders` 表中对已脱敏列执行 `SELECT` 的权限。
...
如果只希望在 `orders` 表中存储脱敏后的数据，
可以将包含敏感原始数据的列标记为 [`EPHEMERAL`](/sql-reference/statements/create/table#ephemeral)，
从而确保此类列不会被实际存储在表中。
...
如果我们像之前一样再次运行该查询，就会看到只有物化后的脱敏数据被插入到表中：
