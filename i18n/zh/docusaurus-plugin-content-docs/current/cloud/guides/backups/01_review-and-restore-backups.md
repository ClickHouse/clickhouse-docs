...
description: '介绍 ClickHouse Cloud 中的备份功能概览'
doc_type: 'guide'
---
...
本指南介绍 ClickHouse Cloud 中备份的工作原理、用于为服务配置备份的选项，以及如何从备份中恢复。



## 备份状态列表 {#backup-status-list}

您的服务将按照设定的计划进行备份，无论是默认的每日计划还是您选择的[自定义计划](/cloud/manage/backups/configurable-backups)。所有可用的备份可以从服务的**备份**选项卡中查看。在此处，您可以查看备份的状态、持续时间以及大小。您还可以通过**操作**列恢复特定的备份。
...
根据默认策略,ClickHouse Cloud 要求每天进行一次备份,保留 24 小时。如果选择需要保留更多数据或更频繁备份的计划,可能会产生额外的备份存储费用。

要了解备份成本,您可以从使用情况页面查看每个服务的备份成本(如下所示)。使用自定义计划运行备份几天后,您就可以了解成本情况并推算出备份的月度成本。
...
估算备份的总成本需要您设置计划。我们也在更新[价格计算器](https://clickhouse.com/pricing),以便您在设置计划之前获得月度成本估算。要估算成本,您需要提供以下信息:
...
备份恢复后,您将拥有两个相似的服务:**原始服务**(需要恢复的服务)和一个新的**已恢复服务**(从原始服务的备份中恢复)。
...
假设由于某种原因您无法使用新恢复的服务,例如,您仍有用户或应用程序连接到现有服务。您可以决定将新恢复的数据迁移到原始服务中。可以通过以下步骤完成迁移:
...
ClickHouse Cloud 通过 [共享目录](https://clickhouse.com/docs/cloud/reference/shared-catalog) 支持 `UNDROP` 命令。
