...
点击 UI 中备份列表上方的"访问或恢复备份"链接,以获取用于恢复备份的 SQL 命令。该命令如下所示:
...
这允许恢复以异步方式进行,因此即使连接断开,恢复操作也会继续运行。
需要注意的是,ASYNC 命令会立即返回成功状态。
但这并不意味着恢复操作已成功完成。
您需要监控 `system.backups` 表以查看恢复是否已完成以及成功或失败状态。
:::
...
生成 HMAC 密钥和密钥,这是基于密码身份验证所必需的。按照以下步骤生成密钥:
