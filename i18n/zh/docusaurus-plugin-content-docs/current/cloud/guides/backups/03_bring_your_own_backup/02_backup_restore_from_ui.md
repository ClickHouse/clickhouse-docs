...
title: '在 UI 中执行备份或恢复备份'
description: '介绍如何在 UI 中使用您自己的存储桶执行备份或恢复备份的页面'
sidebar_position: 2
doc_type: 'guide'
...
AWS 使用基于角色的身份验证,因此需要创建一个 IAM 角色,使 ClickHouse Cloud 服务能够代入该角色以写入此存储桶。

- a. 从 ClickHouse Cloud 服务设置页面的网络安全信息下获取 ARN,其格式类似于:
...
这可以通过为角色创建权限策略来完成,使用类似以下的 JSON,其中您需要在两处将资源替换为您的存储桶 ARN。
...
将新创建服务的 ARN(可从 ClickHouse Cloud 控制台的服务设置页面获取)添加到 IAM 角色的信任策略中。这与上述 AWS 步骤部分的[第二步](#create-iam-role)相同。此操作是必需的,以便新服务能够访问 S3 存储桶。

##### 获取用于恢复备份的 SQL 命令 {#obtain-sql-command-to-restore-backup}

点击用户界面中备份列表上方的"访问或恢复备份"链接,以获取用于恢复备份的 SQL 命令。该命令如下所示:
...
:::tip ASYNC 命令
对于恢复命令,您可以选择在末尾添加 `ASYNC` 命令以执行大型恢复操作。
这允许恢复操作异步执行,因此即使连接中断,恢复操作也会继续运行。
需要注意的是,ASYNC 命令会立即返回成功状态。
但这并不意味着恢复操作已成功完成。
您需要监控 `system.backups` 表以查看恢复操作是否已完成以及成功或失败状态。
:::
...
生成 HMAC 密钥和密钥,这是基于密码的身份验证所必需的。按照以下步骤生成密钥:
...
- c. 安全存储凭据:
  - I. 系统将显示 Access ID(您的 HMAC 密钥)和 Secret(您的 HMAC 密钥)。请保存这些值,因为在您关闭此窗口后,密钥将不会再次显示。
...
如果配置不同,自定义计划将用于将备份写入您的存储桶,而默认计划(每 24 小时备份一次)将用于 ClickHouse Cloud 所拥有的存储桶中的备份。
...
##### 创建新服务以恢复备份 {#gcp-create-new-service-to-restore-to}

创建一个新服务以将备份恢复到该服务。

##### 获取用于恢复备份的 SQL 命令 {#gcp-obtain-sql-command-to-restore-backup}

点击 UI 中备份列表上方的 `access or restore a backup` 链接以获取恢复备份的 SQL 命令。该命令应如下所示,您可以从下拉菜单中选择适当的备份以获取该特定备份的恢复命令。您需要将您的密钥访问密钥添加到命令中:
...
##### 创建存储账户 {#azure-create-a-storage-account}

在 Azure 门户中创建一个存储账户或选择一个现有的存储账户,用于存储您的备份。

##### 获取连接字符串 {#azure-get-connection-string}

- a. 在存储账户概览中,找到 `Security + networking` 部分,然后点击 `Access keys`。
- b. 在这里,您将看到 `key1` 和 `key2`。在每个密钥下方,您会找到 `Connection string` 字段。
- c. 点击 `Show` 显示连接字符串。复制该连接字符串,您将在 ClickHouse Cloud 中进行配置时使用。

</VerticalStepper>

#### 在 ClickHouse Cloud 中的操作步骤 {#azure-cloud-steps}

在 ClickHouse Cloud 控制台中按照以下步骤配置外部存储桶:

<VerticalStepper headerLevel="h5">

##### 更改外部备份配置 {#azure-configure-external-bucket}

在 `Settings` 页面上,点击 `Change external backup`

<Image img={change_external_backup} alt='更改外部备份配置' size='lg' />

##### 提供 Azure 存储账户的连接字符串和容器名称 {#azure-provide-connection-string-and-container-name-azure}

在下一个页面中,提供在上一节中创建的 Azure 存储账户的连接字符串和容器名称:

<Image
  img={azure_connection_details}
  alt='提供 Azure 存储账户的连接字符串和容器名称'
  size='md'
/>

##### 保存外部存储桶配置 {#azure-save-external-bucket}
...
##### 创建新服务用于恢复 {#azure-create-new-service-to-restore-to}

创建一个新服务用于恢复备份。目前,我们仅支持将备份恢复到新服务中。
...
:::tip ASYNC 命令
对于恢复命令,您还可以选择在末尾添加 `ASYNC` 命令以进行大型恢复。
这允许恢复异步执行,因此即使连接中断,恢复也会继续运行。
需要注意的是,ASYNC 命令会立即返回成功状态。
这并不意味着恢复已成功完成。
您需要监控 `system.backups` 表以查看恢复是否已完成以及是否成功或失败。
:::

##### 运行 SQL 命令恢复备份 {#azure-run-sql-command-to-restore-backup}
