---
sidebar_label: '使用命令进行备份和恢复'
slug: /cloud/manage/backups/backup-restore-via-commands
title: '使用命令创建或恢复备份'
description: '本文说明如何在使用自有存储桶时，通过命令执行备份和恢复'
sidebar_position: 3
doc_type: 'guide'
...
用户可以使用 `BACKUP` 和 `RESTORE` 命令将备份导出到其存储桶中,
也可以[通过用户界面](/cloud/manage/backups/backup-restore-via-ui)进行备份或恢复。
本指南提供了所有三个云服务提供商(CSP)的相关命令。


## 要求 {#requirements}

要将备份导出/恢复到您自己的 CSP 存储桶,您需要以下详细信息:
...
我们提供将备份导出到您的存储桶的功能,但不会清理或删除任何已写入的备份。您需要自行管理存储桶中备份的生命周期,包括根据需要进行删除、归档,或迁移到成本更低的存储以优化总体费用。

</details>

<details>
<summary>如果我将现有备份移动到其他位置,恢复过程会受到什么影响?</summary>

如果将任何备份移动到其他位置,则需要更新恢复命令以指向存储备份的新位置。

</details>

<details>
<summary>如果我更改了访问对象存储所需的凭据,该怎么办?</summary>
...
<details>
<summary>如果我更改了外部备份的导出位置,该怎么办?</summary>

您需要在界面中更新新位置,之后备份将开始写入新位置。旧备份将保留在原始位置。
...
要为服务禁用外部备份,请进入服务设置页面,然后点击"更改外部备份"。在后续页面中,点击"移除设置"以禁用该服务的外部备份。
