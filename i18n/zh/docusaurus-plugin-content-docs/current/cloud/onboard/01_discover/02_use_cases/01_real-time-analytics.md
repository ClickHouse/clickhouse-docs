...
title: '实时分析'
description: '了解如何使用 ClickHouse Cloud 构建实时分析应用，实现即时洞察并做出数据驱动的决策'
keywords: ['使用场景', '实时分析']
sidebar_label: '实时分析'
doc_type: 'guide'
---
...
实时分析系统构建在事件流之上,事件流由按时间顺序排列的一系列事件组成。事件是指已经发生的事情。它可以是电子商务网站上向购物车添加商品的操作、物联网(IoT)传感器发出的读数,或者足球比赛中的射门动作。

下面是一个事件示例(来自虚构的物联网传感器):
...
组织可以通过聚合和分析此类事件来发现有关其客户的洞察。传统上这是通过批处理分析完成的,在下一节中,我们将比较批处理分析和实时分析。


## 实时分析与批处理分析 {#real-time-analytics-vs-batch-analytics}

下图展示了从单个事件的角度来看，典型的批处理分析系统的工作方式：

<Image img={rta_0} size='md' border alt='批处理分析示意图' />

可以看到，从事件发生到我们处理并从中获得洞察之间存在相当大的时间间隔。传统上，这是数据分析的唯一方式，我们需要人为设定时间边界来批量处理数据。例如，我们可能会在每天结束时处理当天收集到的所有数据。这种方式适用于许多场景，但对于其他场景来说并不理想，因为我们处理的是陈旧数据，无法对数据做出足够快速的反应。

相比之下，在实时分析系统中，我们会在事件发生时立即做出反应，如下图所示：

<Image img={rta_1} size='md' border alt='实时分析示意图' />

现在我们几乎可以在事件生成的同时从中获得洞察。但这有什么用呢？


## 实时分析的优势 {#benefits-of-real-time-analytics}

在当今快节奏的环境中,组织依靠实时分析来保持敏捷性并快速响应不断变化的情况。实时分析系统可以在多个方面为企业带来价值。

### 更优的决策能力 {#better-decision-making}

通过实时分析获取可操作的洞察,可以提升决策质量。当业务运营人员能够实时看到正在发生的事件时,就能更容易地及时采取干预措施。
...
实时分析可以帮助企业开拓新的收入来源。组织可以开发以数据为中心的新产品和服务,为用户提供分析查询能力。这些产品往往具有足够的吸引力,使用户愿意为此付费。

此外,现有应用程序可以变得更具粘性,提高用户参与度和留存率。这将带来更高的应用使用率,从而为组织创造更多收入。

### 提升客户体验 {#improved-customer-experience}

借助实时分析,企业可以即时洞察客户的行为、偏好和需求。这使企业能够提供及时的帮助,实现个性化互动,并创造更具吸引力的体验,让客户持续回访。
...
该用例跨越多个行业:医疗保健、数字银行、金融服务、零售等。

[Instacart](https://www.instacart.com/) 是北美领先的在线杂货公司,拥有数百万活跃客户和购物者。该公司使用 ClickHouse 作为其欺诈检测平台 Yoda 的一部分。除了上述常见的欺诈类型外,它还会检测客户与购物者之间的串通行为。
...
> ClickHouse 专门针对分析查询进行设计和优化。这完全契合需要持续分析数据以发现潜在欺诈模式的应用场景。

### 时间敏感型决策 {#ftime-sensitive-decision-making}

时间敏感型决策是指用户或组织需要基于最新可用信息快速做出明智选择的场景。实时分析使用户能够在动态环境中做出明智的决策,无论他们是对市场波动做出反应的交易员、做出购买决策的消费者,还是适应实时运营变化的专业人士。
