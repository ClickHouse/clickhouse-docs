...
虽然用户可以直接在 ClickHouse 中或在插入前通过 SQL 查询对数据进行转换，ClickHouse 也可以在诸如 Python 之类的编程环境中通过 [chDB](/chdb) 使用。
这使得嵌入式 ClickHouse 可以以 Python 模块的形式对外提供，用于在 Notebook 环境中转换和操作大型数据帧。
因此，数据工程师可以在客户端侧完成转换工作，并将结果在集中式 ClickHouse 实例中物化为特征表。
...
对于那些在本地评估更方便的数据集，[ClickHouse local](/operations/utilities/clickhouse-local) 以及 [chDB](/chdb) 是很好的替代方案，它们允许用户配合 Pandas 等标准 Python 数据处理库来使用 ClickHouse。
...
离线特征存储用于模型训练。
这通常意味着特征本身是通过批处理数据转换流水线（如上一节所述）生成的，并且对这些特征的可用性通常没有严格的延迟要求。
...
向量存储是一种特定类型的数据库，针对向量（通常是对某段数据（如文本或图像）的嵌入表示）进行存储和检索进行了优化，这些向量以数值方式刻画其底层含义。
向量是当今生成式 AI 浪潮的核心，被广泛应用于无数应用场景中。
