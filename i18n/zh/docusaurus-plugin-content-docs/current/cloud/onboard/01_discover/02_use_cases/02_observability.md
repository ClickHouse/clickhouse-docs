...
title: '可观测性'
description: '使用 ClickHouse Cloud 实现可观测性、监控、日志记录以及分布式应用中的系统性能分析'
...
现代软件系统非常复杂。微服务、云基础设施以及分布式系统让我们越来越难以了解应用程序内部到底发生了什么。当出现问题时，团队需要能够快速定位问题发生的地方和原因。

这正是可观测性发挥作用的地方。它已经从简单的系统监控演变为理解系统行为的一整套方法。不过，要构建有效的可观测性并非易事——这既需要理解技术层面的概念，也要应对组织层面的挑战。
...
该领域已经历了显著的演进,可以通过两代不同的可观测性方法来理解。

第一代通常称为可观测性 1.0,围绕传统的"三大支柱"方法构建,即指标、日志和追踪。这种方法需要针对不同类型的遥测数据使用多个工具和数据存储。它通常要求工程师预先定义需要测量的内容,使得维护多个系统既昂贵又复杂。

现代可观测性,或称可观测性 2.0,采用了根本不同的方法。它基于为系统中的每个工作单元(例如一个 HTTP 请求和响应)收集广泛的结构化事件。这种方法捕获高基数数据,例如用户 ID、请求 ID、Git 提交哈希、实例 ID、Kubernetes Pod 名称、特定路由参数和供应商事务 ID。一个经验法则是:如果某个元数据能够帮助我们理解系统的行为方式,就添加它。
...
并实现了比之前解决方案快 4-30 倍的查询速度,90% 的查询在 300 毫秒内完成,从而实现了快速的问题调查。
...
## 实施可观测性面临的挑战 {#challenges-in-implementing-observability}

在组织内实施可观测性是深入了解系统性能和可靠性的变革性举措。然而,这一过程并非一帆风顺。当组织努力充分发挥可观测性的潜力时,会遇到各种可能阻碍进展的障碍。让我们来了解其中一些挑战。

### 数据量和可扩展性 {#data-volume-and-scalability}

实施可观测性的主要障碍之一是管理现代系统生成的海量遥测数据及其可扩展性问题。随着组织规模的扩大,需要监控的数据量也随之增长,这就需要能够高效处理大规模数据采集和实时分析的解决方案。

### 与现有系统的集成 {#integration-with-existing-systems}

与现有系统的集成是另一个重大挑战。许多组织在异构环境中运行,使用多种不同的技术,这使得可观测性工具必须能够与当前基础设施无缝集成。开放标准对于促进这种集成至关重要,可确保互操作性并降低在不同技术栈中部署可观测性解决方案的复杂性。

### 技能差距 {#skill-gaps}

技能差距也可能阻碍可观测性的成功实施。向高级可观测性解决方案的过渡通常需要数据分析和特定工具的专业知识。团队可能需要投资于培训或招聘来弥补这些差距,以充分发挥其可观测性平台的能力。
