...
| **ClickHouse CLI**             | ClickHouse CLI (`clickhousectl`) 让您能够安装和管理本地 ClickHouse 版本、启动服务器、运行查询，以及管理 ClickHouse Cloud 服务和资源。                                                               | ✅              | ✅                |
...
| **Scaling and optimization**   | 可根据工作负载无缝扩缩容，支持横向和纵向扩缩容。借助自动备份、复制和高可用性，ClickHouse Cloud 为用户提供最佳的资源分配。                                                                                           | ❌              | ✅                |
