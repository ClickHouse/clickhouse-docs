...
在上述查询中,我们将 BigQuery 表导出为 [Parquet 数据格式](https://parquet.apache.org/)。我们在 `uri` 参数中使用了 `*` 通配符。这样可以确保当导出数据超过 1GB 时,输出会自动分片为多个文件,并带有数字递增的后缀。

这种方法具有以下优势:

- Google 允许每天免费导出最多 50TB 数据到 GCS,用户只需支付 GCS 存储费用。
- 导出会自动生成多个文件,每个文件最多包含 1GB 的表数据。这有利于 ClickHouse 并行化导入操作。
- Parquet 作为列式存储格式,是一种更优的数据交换格式,因为它本身具有压缩特性,可以加快 BigQuery 的导出速度和 ClickHouse 的查询速度
...
或者,您可以设置 `SET input_format_null_as_default=1`,这样任何缺失或 NULL 值都将被替换为其各自列的默认值(如果指定了这些默认值)。
:::
