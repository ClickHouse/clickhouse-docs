...
doc_type: 'guide'
keywords: ['迁移', 'ClickHouse Cloud', 'OSS', '将自托管部署迁移到 Cloud']
---
...
本指南将介绍如何从自托管 ClickHouse 服务器迁移到 ClickHouse Cloud，以及如何在不同的 ClickHouse Cloud 服务之间迁移。[`remoteSecure`](/sql-reference/table-functions/remote) 函数可在 `SELECT` 和 `INSERT` 查询中使用，用于访问远程 ClickHouse 服务器，这使得迁移表就像编写一个内嵌 `SELECT` 的 `INSERT INTO` 查询一样简单。
...
:::note
如果源系统无法从外部网络访问,则可以推送数据而不是拉取数据,因为 `remoteSecure` 函数同时支持 SELECT 和 INSERT 操作。请参阅下一个选项。
:::
...
- 从已恢复的备份中迁移数据
- 将数据从开发服务复制到预发布服务(或从预发布环境复制到生产环境)
...
如果您之前导出了访问列表,则可以使用**共享**功能重新导入,否则请重新将您的条目添加到访问列表中。
