...
> 这是《从 PostgreSQL 迁移到 ClickHouse》指南的**第 2 部分**。通过一个实际示例，它演示了如何采用实时复制（CDC）方式高效完成迁移。文中涉及的许多概念同样适用于从 PostgreSQL 到 ClickHouse 的手动批量数据传输。

大多数来自 PostgreSQL 环境的 SQL 查询在 ClickHouse 中通常无需修改即可运行，并且往往执行得更快。
...
这里的示例涵盖了常见的查询模式,并展示了如何使用 ClickHouse 对其进行优化。这些示例在 PostgreSQL 和 ClickHouse 的等效资源(8 核,32GiB RAM)上使用完整的 [Stack Overflow 数据集](/getting-started/example-datasets/stackoverflow)(截至 2024 年 4 月)。

> 为简化起见,以下查询省略了数据去重技术的使用。
...
在可能的情况下,用户应充分利用 ClickHouse 的聚合函数。下面我们展示了使用 [argMax](/sql-reference/aggregate-functions/reference/argmax) 函数来计算每年浏览量最多的问题。
...
返回 17 行。耗时:0.677 秒。处理了 2437 万行,1.86 GB(3601 万行/秒,2.75 GB/秒)
峰值内存使用量:554.31 MiB。
```

这比对应的 Postgres 查询要简单得多（而且更快）：
...
条件函数和数组函数可以显著简化查询。下面的查询会计算从 2022 年到 2023 年间，出现次数超过 10000 次且百分比增幅最大的标签。注意，得益于条件函数、数组函数，以及在 HAVING 和 SELECT 子句中复用别名的能力，下面的 ClickHouse 查询非常简洁。
...
结果集中包含 5 行。耗时：0.247 秒。已处理 5.08 百万行，155.73 MB（20.58 百万行/秒，630.61 MB/秒）。
峰值内存使用量：403.04 MiB。
...
时间:116750.131 毫秒 (01:56.750)
````

[点击此处进入第 3 部分](/migrations/postgresql/data-modeling-techniques)
