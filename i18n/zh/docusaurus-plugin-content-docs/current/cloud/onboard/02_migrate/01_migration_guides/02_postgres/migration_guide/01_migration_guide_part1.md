...
> 这是从 PostgreSQL 迁移到 ClickHouse 指南的**第 1 部分**。通过一个实际案例，它演示了如何以实时复制（CDC）方式高效完成迁移。文中涉及的许多概念同样适用于从 PostgreSQL 到 ClickHouse 的手动批量数据传输。


## 数据集 {#dataset}

作为展示从 Postgres 迁移到 ClickHouse 的典型示例,我们使用[此处](/getting-started/example-datasets/stackoverflow)记录的 Stack Overflow 数据集。该数据集包含 2008 年至 2024 年 4 月期间 Stack Overflow 上的所有 `post`、`vote`、`user`、`comment` 和 `badge` 数据。该数据的 PostgreSQL 模式如下所示:

<Image
  img={postgres_stackoverflow_schema}
  size='lg'
  alt='PostgreSQL Stack Overflow 模式'
/>

_在 PostgreSQL 中创建表的 DDL 命令可在[此处](https://pastila.nl/?001c0102/eef2d1e4c82aab78c4670346acb74d83#TeGvJWX9WTA1V/5dVVZQjg==)获取。_

该模式虽然不一定是最优的,但使用了许多常用的 PostgreSQL 特性,包括主键、外键、分区和索引。
...
虽然对于 ClickHouse 来说这个数据集很小,但对于 Postgres 来说却相当庞大。上述内容为涵盖 2024 年前三个月的数据子集。

> 虽然我们的示例结果使用完整数据集来展示 Postgres 和 ClickHouse 之间的性能差异,但下文记录的所有步骤在功能上与较小的子集完全相同。希望将完整数据集加载到 Postgres 的用户请参见[此处](https://pastila.nl/?00d47a08/1c5224c0b61beb480539f15ac375619d#XNj5vX3a7ZjkdiX7In8wqA==)。由于上述模式中的外键约束,PostgreSQL 的完整数据集仅包含满足引用完整性的行。如有需要,可以将没有此类约束的 [Parquet 版本](/getting-started/example-datasets/stackoverflow)直接加载到 ClickHouse 中。
```
...
我们可以使用一个简单的 `INSERT INTO SELECT` 语句，从 PostgreSQL 读取数据并将其写入 ClickHouse：
...
增量加载同样可以被调度。如果 Postgres 表仅执行插入操作，并且存在自增 id 或时间戳，用户可以使用上述表函数的方法来加载增量数据，即在 `SELECT` 中应用 `WHERE` 子句。如果可以保证更新仅影响同一列，该方法也可用于支持更新。然而，若要支持删除，则需要对整张表进行完全重新加载，而随着表的增长，这可能难以实现。

我们使用 `CreationDate` 演示一次初始加载和一次增量加载（我们假设当行被更新时，该字段会随之更新）。
...
> ClickHouse 会将简单的 `WHERE` 子句（例如 `=`, `!=`, `>`, `>=`, `<`, `<=` 和 IN）下推到 PostgreSQL 服务器。因此，通过确保在用于标识变更集的列上建立索引，可以进一步提升增量加载的效率。

> 在使用查询复制时，一种检测 UPDATE 操作的可行方法是使用 [`XMIN` 系统列](https://www.postgresql.org/docs/9.1/ddl-system-columns.html)（事务 ID）作为水位线——该列发生变化即可视为有变更，从而将相应记录应用到目标表。采用这种方法的用户应注意，`XMIN` 值可能会发生回绕，并且在进行比较时需要对整张表执行全表扫描，从而使变更跟踪更加复杂。
