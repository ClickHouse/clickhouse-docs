...
# Amazon Redshift SQL 迁移指南



## 数据类型 {#data-types}

在 ClickHouse 和 Redshift 之间迁移数据的用户会立即注意到,ClickHouse 提供了更丰富的数据类型,且限制更少。Redshift 要求用户指定字符串的可能长度(即使是可变长度),而 ClickHouse 通过以字节形式存储字符串而无需编码,消除了这一限制和负担。因此,ClickHouse 的 String 类型没有长度限制或长度规范要求。

此外,用户可以使用 Arrays、Tuples 和 Enums——这些类型在 Redshift 中并非一等公民(尽管可以使用 `SUPER` 模拟 Arrays/Structs),这一直是用户的常见痛点。ClickHouse 还支持在查询时或表中持久化聚合状态。这使得数据可以预先聚合(通常使用物化视图),从而显著提升常见查询的性能。

下面列出了每种 Redshift 类型对应的 ClickHouse 类型:
...
| [`DECIMAL`](https://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html#r_Numeric_types201-decimal-or-numeric-type)       | [`UInt128`, `UInt256`, `Int128`, `Int256`](/sql-reference/data-types/int-uint), [`Decimal(P, S)`, `Decimal32(S)`, `Decimal64(S)`, `Decimal128(S)`, `Decimal256(S)`](/sql-reference/data-types/decimal) - （支持高精度和大范围取值）                           |
...
| [`GEOMETRY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                                | [地理空间数据类型](/sql-reference/data-types/geo)                                                                                                                                                                                                        |
| [`GEOGRAPHY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                               | [Geo 数据类型](/sql-reference/data-types/geo)（目前开发较少，例如尚不支持坐标系——可以通过[函数](/sql-reference/functions/geo/)进行模拟）                                                                                                                                         |
...
| [`VARBYTE`](https://docs.aws.amazon.com/redshift/latest/dg/r_VARBYTE_type.html) **                                                   | 将 [`String`](/sql-reference/data-types/string) 与 [`Bit`](/sql-reference/functions/bit-functions) 和 [编码](/sql-reference/functions/encoding-functions/#hex) 函数结合使用                                                                                 |



<sub><span>*</span> ClickHouse 还支持取值范围扩展的无符号整数类型，即 <a href='http://clickhouse.com/docs/sql-reference/data-types/int-uint'>`UInt8`、`UInt32`、`UInt32` 和 `UInt64`</a>。</sub><br />
<sub><span>**</span>ClickHouse 的 String 类型默认长度不受限制，但可以通过使用 <a href='http://clickhouse.com/docs/sql-reference/statements/create/table#constraints'>Constraints</a> 将其约束为特定长度。</sub>
...
在大多数情况下,假设您使用的是默认的 `COMPOUND` 类型,可以在 ClickHouse 中使用与 Redshift 相同的排序键列和顺序。向 Redshift 添加数据时,需要运行 `VACUUM` 和 `ANALYZE` 命令来重新排序新添加的数据并更新查询规划器的统计信息,否则未排序空间会不断增长。ClickHouse 则无需执行此类操作。

Redshift 为排序键提供了几个便捷功能。第一个是自动排序键(使用 `SORTKEY AUTO`)。虽然这适合快速入门,但当排序键达到最优时,显式排序键能够确保最佳性能和存储效率。第二个是 `INTERLEAVED` 排序键,它为排序键中的列子集赋予相同的权重,以便在查询使用一个或多个辅助排序列时提高性能。ClickHouse 支持显式[投影](/data-modeling/projections),通过略有不同的配置方式实现相同的效果。

用户需要注意,"主键"概念在 ClickHouse 和 Redshift 中具有不同的含义。在 Redshift 中,主键类似于传统 RDBMS 概念,旨在强制执行约束。但是,Redshift 并不严格强制执行主键约束,而是将其作为查询规划器和节点间数据分布的提示信息。在 ClickHouse 中,主键表示用于构建稀疏主索引的列,用于确保数据在磁盘上有序存储,在最大化压缩效率的同时避免主索引污染和内存浪费。
