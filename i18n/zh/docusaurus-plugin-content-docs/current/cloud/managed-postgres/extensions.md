...
Managed Postgres 包含一组精心挑选的扩展，用于增强数据库的功能。下面是可用扩展的列表。
...
| `tcn`                          | 1.0    | 基于触发器的变更通知                                            |
| `tds_fdw`                      | 2.0.5  | 用于查询 TDS 数据库 (Sybase 或 Microsoft SQL Server) 的外部数据封装器 |
...
`pg_stat_ch` 扩展为您的 Managed Postgres 实例提供查询级可观测性。它会采集每个查询的遥测数据 (时长、行数、缓冲区、WAL、错误) ，并将其发送到 ClickHouse Cloud UI，在其中呈现为慢查询、错误和工作负载视图。无需配置。

## pg_re2 扩展 \{#pg-re2\}

`pg_re2` 扩展为 Managed Postgres 增加了与 ClickHouse 兼容的正则表达式函数，基于 Google 的 RE2 引擎。在两者之间迁移查询，或运行需要两端具备一致模式匹配语义的混合工作负载时，这一扩展尤其有用。