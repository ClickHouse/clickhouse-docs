...
组织范围的可见性会使资源显示在 [市场](/cloud/features/ai-ml/agents/marketplace) 中。

## 共享智能体 \{#share-an-agent\}

点击智能体构建器底部的**共享**图标。图标上的徽标会显示当前拥有访问权限的用户或组数量。

<Image img={sharing} alt="智能体构建器面板底部，共享图标已高亮显示，徽标显示 1 个已共享主体" size="sm" />

此时会打开**共享**模态窗口。按名称或电子邮件搜索用户或组，选择访问级别 (**查看者**、**编辑者**或**所有者**) ，然后点击**保存更改**。如需向整个组织共享，请启用**与 ClickHouse 中的所有人共享**开关。

<Image img={shareAgentModal} alt="共享模态窗口，包含“用户和组权限”部分、搜索输入框、一个拥有“所有者”角色的现有用户、“与 ClickHouse 中的所有人共享”开关，以及“取消”和“保存更改”按钮" size="md" />
