...
为此，请在 ClickHouse Cloud 控制台中，进入你希望限制访问的特定服务的服务选项卡中的 **Settings**。
...
3. 服务类型 (只读或读写) 在创建时即已固定，之后无法在 Cloud Console 中修改。若要在只读和读写访问之间切换，请在该仓库中创建一个具有所需类型的新服务。
   :::
...
* **单副本主服务** 当前默认情况下，次级服务可以只有一个副本，但主服务必须至少有 2 个副本。
  要启用单副本主服务，请联系支持。此行为将在 2026 年第二季度默认启用。
* **主服务闲置**：此前，主服务默认不能自动闲置。自 2026 年 5 月起，主服务自动闲置已默认启用。作为发布的一部分，现有服务可使用该功能，而在发布后创建的新服务则默认启用该功能。
