...
当 Envoy 收到与此模式匹配的主机名时，它会基于该主机名计算路由哈希值，并根据计算出的哈希在哈希环上找到对应的 ClickHouse server。在服务没有正在发生变更 (例如服务器重启、扩缩容) 的前提下，Envoy 将始终选择同一台 ClickHouse server 进行连接。
