...
本文档详细说明了用于保护 ClickHouse 组织与服务安全的选项和最佳实践。
ClickHouse 致力于提供安全的分析型数据库解决方案，因此保护数据和服务的完整性是首要任务。
本文所述信息涵盖多种方法，旨在帮助用户确保其 ClickHouse 环境的安全。
...
使用电子邮件和密码或社交 SSO 登录的用户还可以配置多因素身份验证,通过 Authy 或 Google Authenticator 等身份验证器应用程序实现。
...
ClickHouse 数据库用户密码按照 NIST 800-63B 标准配置,最少 12 个字符,并需满足复杂度要求:大写字母、小写字母、数字和/或特殊字符。
...
ClickHouse Cloud 默认使用云服务商管理的 AES 256 密钥对静态数据进行加密。

了解更多关于[存储级加密](/cloud/security/cmek#storage-encryption)的信息。

### 透明数据加密 {#tde}

除存储加密外,ClickHouse Cloud 企业版客户还可以启用数据库级透明数据加密以获得额外保护。

了解更多关于[透明数据加密](/cloud/security/cmek#transparent-data-encryption-tde)的信息。

### 客户管理的加密密钥 {#cmek}

ClickHouse Cloud 企业版客户可以使用自己的密钥进行数据库级加密。

了解更多关于[客户管理的加密密钥](/cloud/security/cmek#customer-managed-encryption-keys-cmek)的信息。
...
ClickHouse 维护着完善的安全与合规计划。请定期查看最新的第三方审计报告。
