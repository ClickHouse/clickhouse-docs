...
title: '仪表板'
description: 'SQL 控制台的仪表板功能可让您基于已保存的查询汇总并共享可视化结果。'
doc_type: 'guide'
...
SQL Console 的仪表板功能允许您收集并共享来自已保存查询的可视化结果。您可以先保存并可视化查询，将这些可视化结果添加到仪表板中，并通过查询参数使仪表板具备交互性。
...
要与同事共享仪表板,请务必共享底层已保存的查询。用户至少需要对底层已保存查询具有只读访问权限,才能查看可视化内容。

### 交互性 {#interactivity}

使用[查询参数](/sql-reference/syntax#defining-and-using-query-parameters)使仪表板具有交互性。例如,您可以在 `WHERE` 子句中添加查询参数作为过滤器使用。

您可以在可视化设置中选择"过滤器"类型,通过 **Global** 过滤器侧边栏切换查询参数输入。您也可以通过链接到仪表板上的其他对象(如表)来切换查询参数输入。请参阅下方快速入门指南中的"[配置过滤器](/cloud/manage/dashboards#configure-a-filter)"部分。
...
导航到仪表板面板,然后点击"New Dashboard"。分配名称后,您就成功创建了第一个仪表板!

<Image img={dashboards_5} size='md' alt='创建新仪表板' border />

### 添加可视化 {#add-a-visualization}

现在有两个保存的查询:`queries over time` 和 `query count by query kind`。我们将第一个可视化为折线图。为可视化设置标题和副标题,并选择要可视化的查询。接下来,选择"Line"图表类型,并分配 x 轴和 y 轴。
...
让我们通过在查询类型上添加过滤器来使此仪表板具有交互性,这样您就可以仅显示与 Insert 查询相关的趋势。我们将使用[查询参数](/sql-reference/syntax#defining-and-using-query-parameters)来完成此任务。
...
假设您想将折线图的过滤器链接到表格。您可以返回可视化设置,将 query_kind 查询参数的值源更改为表格,并选择 query_kind 列作为要链接的字段。
