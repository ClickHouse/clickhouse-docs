...
The **查询洞察**功能通过可视化和表格，让 ClickHouse 的内置查询日志更易于使用。ClickHouse 的 `system.query_log` 表是查询优化、调试以及监控整体集群健康状况和性能的关键信息来源。

## 查询概览 \{#query-overview\}

选择某个服务后，左侧边栏中的 **Monitoring** 导航项会展开，显示一个新的 **查询洞察** 子项。点击该选项会打开新的查询洞察页面：
