...
构建交互式数据驱动应用，不仅需要快速的数据库、结构良好的数据以及经过优化的查询。
前端和微服务还需要一种简便的方式来使用这些查询返回的数据，最好是通过结构清晰的 API。

**Query API 端点** 功能允许您直接基于 ClickHouse Cloud 控制台中任何已保存的 SQL 查询创建 API 端点。
您可以通过 HTTP 访问 API 端点来执行已保存的查询，而无需使用原生驱动连接到您的 ClickHouse Cloud 服务。
