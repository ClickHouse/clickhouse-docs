...
<VerticalStepper headerLevel="h3">
  ...
  如果你的 Python 脚本导入了第三方包，则必须创建一个列出这些依赖的 `requirements.txt` 文件。例如：
  ...
  ### 打包依赖和本地文件 \{#bundle-dependencies\}

  要包含依赖包以及其他本地文件 (例如 wheel 文件、设置文件或数据文件) ，请将它们放在与 `main.py` 和 `requirements.txt` 相同的目录中。创建 ZIP 归档文件时，请将所有文件都包含进去：

  ```bash
  zip is_business_hours.zip main.py requirements.txt
  ```

  您可以在 Python 代码中使用 `os.path.dirname(os.path.abspath(__file__))` 来引用本地打包路径的基目录。该表达式会返回 ZIP 归档中 `main.py` 所在目录的绝对路径，从而使您可以访问其他打包文件：
  ...
  * 引用额外的脚本或数据文件
  ...
  :::warning[不允许使用符号链接]
  ClickHouse Cloud 会拒绝包含符号链接的 UDF 归档文件。请确保你的 ZIP 包中只包含普通文件和目录——含有符号链接的上传将无法通过验证。
  :::
  ...
  7. 点击 **Browse File**，上传在本教程开头创建的 `.zip` 文件。
  ...
  1. 点击页面左上角的 **Settings - return to your service view**，返回 SQL 控制台主页
  2. 点击左侧菜单中的 **SQL 控制台**
  3. 编写以下查询：
