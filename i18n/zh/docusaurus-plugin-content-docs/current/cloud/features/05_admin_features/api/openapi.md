...
title: '管理 API 密钥'
description: 'ClickHouse Cloud 提供了基于 OpenAPI 的 API，使您能够以编程方式管理您的账户及其服务的各个方面。'
doc_type: 'guide'
...
ClickHouse Cloud 提供了基于 OpenAPI 的 API，允许您以编程方式管理您的账户及服务的各个方面。
...
2. **API Keys** 页面初次打开时会显示一个提示，引导您创建第一个 API 密钥，如下所示。创建第一个密钥后，您可以使用右上角的 `New API Key` 按钮创建新的密钥。

<Image img={image_02} size="md" alt="API Keys 页面" border />

3. 要创建 API 密钥，请指定密钥名称、该密钥的权限以及过期时间，然后点击 `Generate API Key`。

<br />

:::note
权限与 ClickHouse Cloud 的[预定义角色](/cloud/security/console-roles)保持一致。developer 角色对分配的服务具有只读权限，而 admin 角色具有完整的读写权限。
:::
...
4. 下一步界面会显示您的 Key ID 和 Key secret。请复制这些值并将其保存在安全的位置，例如密码库。这些值在您离开该界面后将不会再次显示。

<Image img={image_04} size="md" alt="API 密钥详情" border />

5. ClickHouse Cloud API 使用 [HTTP Basic Authentication](https://developer.mozilla.org/en-US/docs/Web/HTTP/Authentication) 来验证您的 API 密钥是否有效。下面是一个示例，展示如何使用 API 密钥通过 `curl` 向 ClickHouse Cloud API 发送请求：
...
6. 返回 **API Keys** 页面，您将看到密钥名称、Key ID 的后四位字符、权限、状态、到期日期以及创建者。您可以在此界面编辑密钥名称、权限和到期时间，也可以在此界面禁用或删除密钥。
...
使用您的 API 密钥和 API 密文,配合基础 URL `https://api.clickhouse.cloud/v1`。
