...
slug: /manage/updates
title: '升级'
description: '使用 ClickHouse Cloud 时，您无需操心打补丁和升级。我们会定期发布包含修复、新功能和性能优化的升级版本。'
doc_type: 'guide'
...
在 ClickHouse Cloud 中，你无需担心打补丁和升级。我们会定期发布升级版本，其中包含修复、新功能和性能改进。有关 ClickHouse 最新变更的完整列表，请参阅我们的[云服务更新日志](/whats-new/cloud)。

:::note
我们正在引入一种新的升级机制，我们称之为 “make before break”（或 MBB）。通过这种新方式，我们会在升级操作中先添加更新后的副本，然后再移除旧副本。这将带来更加平滑的升级过程，对正在运行的工作负载干扰更小。

作为这一变更的一部分，与升级事件相关的历史系统表数据最长将保留 30 天。此外，AWS 或 GCP 上服务中早于 2024 年 12 月 19 日的系统表数据，以及 Azure 上服务中早于 2025 年 1 月 14 日的系统表数据，在迁移到新的组织层级时将不会被保留。
:::
...
创建服务时,[`compatibility`](/operations/settings/settings#compatibility) 设置将被设定为服务初始配置时 ClickHouse Cloud 所提供的最新 ClickHouse 版本。

`compatibility` 设置允许您使用先前版本中的设置默认值。当服务升级到新版本时,`compatibility` 设置中指定的版本不会改变。这意味着首次创建服务时存在的设置默认值不会改变(除非您已经覆盖了这些默认值,在这种情况下,它们将在升级后保持不变)。
...
有时,我们可能需要更新您的服务,这可能需要禁用某些功能,例如扩缩容或空闲。在极少数情况下,我们可能需要对出现问题的服务采取措施,将其恢复到正常状态。在此类维护期间,您将在服务页面上看到一个横幅,显示 _"维护进行中"_。在此期间,您可能仍然可以使用该服务进行查询。
...
- 适用于希望在生产升级之前有额外时间在非生产环境中测试 ClickHouse 版本的客户。非生产环境可以选择通过快速或常规发布渠道获取升级以进行测试和验证。
...
选择需要指定升级计划的服务,然后从左侧菜单中选择 `Settings`,滚动到 `Scheduled upgrades` 部分。
...
<br />
:::note 虽然计划升级会遵循预定的时间表,但对于关键安全补丁和漏洞修复存在例外情况。当发现紧急安全问题时,升级可能会在计划时间窗口之外执行。如有此类例外情况,我们会及时通知客户。 :::
