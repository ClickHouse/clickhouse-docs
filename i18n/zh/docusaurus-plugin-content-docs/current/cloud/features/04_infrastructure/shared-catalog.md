...
共享目录中的所有元数据和 DDL 查询历史记录都集中存储在 ZooKeeper 中。本地磁盘上不会持久化任何数据。这种架构确保:
...
**Shared 数据库引擎**与 Shared Catalog 协同工作,用于管理使用**无状态表引擎**(如 `SharedMergeTree`)的表所在的数据库。这些表引擎不会将持久化状态写入磁盘,适用于动态计算环境。

Shared 数据库引擎基于 Replicated 数据库引擎构建并进行了改进,同时提供了额外的保障和运维优势。
...
- **改进的计算层分离**
  与 Replicated 数据库引擎不同(该引擎要求所有副本在线才能处理 DROP 查询),Shared Catalog 执行集中式元数据删除。这使得即使某些副本离线,操作也能成功执行。
