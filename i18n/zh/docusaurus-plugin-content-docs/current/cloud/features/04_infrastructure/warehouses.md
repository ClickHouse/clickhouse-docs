...
- 一个端点（或通过 ClickHouse Cloud UI 控制台创建的多个端点），这是用于连接服务的服务 URL（例如 `https://dv2fzne24g.us-east-1.aws.clickhouse.cloud:8443`）。
- 一个对象存储文件夹，服务在其中存储所有数据和部分元数据：

:::note
与单副本父服务不同，单副本子服务可以垂直扩展。
:::
...
每个计算节点组都有自己的端点，因此您可以选择为工作负载使用哪组副本。某些工作负载可能只需要一个小规模副本即可满足，而其他工作负载可能需要完全高可用性（HA）和数百 GB 的内存。计算-计算分离还允许您将读操作与写操作分离，使它们不会相互干扰：
...
您可以创建与现有服务共享相同数据的额外服务，或创建一个全新的配置，其中多个服务共享相同的数据。


## 什么是 Warehouse？ {#what-is-a-warehouse}

在 ClickHouse Cloud 中，_warehouse_（仓库）是指一组共享相同数据的服务集合。

每个 warehouse 包含一个主服务（最先创建的服务）和一个或多个辅助服务。例如，在下面的截图中可以看到名为 "DWH Prod" 的 warehouse，其中包含两个服务：
...
限制特定服务被其他应用程序或临时用户使用通常很有用。这可以通过网络限制来实现,类似于当前为常规服务配置的方式(在 ClickHouse Cloud 控制台中导航到特定服务的服务选项卡中的**设置**)。
...
有时限制对特定服务的写入访问并仅允许仓库中的部分服务进行写入是很有用的。这可以在创建第二个及后续服务时完成(第一个服务应始终为读写):
...
1. 只读服务当前允许用户管理操作(创建、删除等)。此行为可能在未来发生变化。
2. 当前,可刷新物化视图在仓库中的所有服务上执行,包括只读服务。但是,此行为将在未来发生变化,它们将仅在读写服务上执行。
   :::


## 扩展 {#scaling}

仓库中的每个服务可以根据工作负载在以下方面进行调整：

- 节点（副本）数量。主服务（仓库中最先创建的服务）应至少包含 2 个节点。每个辅助服务可以包含 1 个或多个节点。
- 节点（副本）规格
- 服务是否自动扩展
- 服务在不活动时是否进入空闲状态（不适用于组中的第一个服务 - 请参阅**限制**部分）


## 行为变更 {#changes-in-behavior}

一旦为服务启用了计算-计算功能(至少创建了一个辅助服务),使用 `default` 集群名称调用 `clusterAllReplicas()` 函数时,将仅使用调用该函数所在服务的副本。这意味着,如果有两个服务连接到同一个数据集,并且从服务 1 调用 `clusterAllReplicas(default, system, processes)`,则只会显示服务 1 上运行的进程。如果需要访问所有副本,您仍然可以调用 `clusterAllReplicas('all_groups.default', system, processes)`。


## 限制 {#limitations}

1. **主服务必须始终保持运行且不能进入空闲状态(此限制将在正式发布后的某个时间移除)。** 在私有预览期间以及正式发布后的一段时间内,主服务(通常是您希望通过添加其他服务来扩展的现有服务)将始终保持运行,并且空闲设置将被禁用。如果至少存在一个辅助服务,您将无法停止主服务或使其进入空闲状态。移除所有辅助服务后,您才可以再次停止原始服务或使其进入空闲状态。
...
5. **如果启用了空闲功能,一个读写服务中的插入操作可能会阻止另一个读写服务进入空闲状态。** 因此,第二个服务会为第一个服务执行后台合并操作。这些后台操作可能会阻止第二个服务在空闲时进入休眠状态。后台操作完成后,该服务将进入空闲状态。只读服务不受影响,将立即进入空闲状态。
...
仓库中所有服务(主服务和辅助服务)的计算价格相同。存储费用仅收取一次——包含在第一个(原始)服务中。
...
## 使用 Warehouse {#using-warehouses}

### 创建 Warehouse {#creating-a-warehouse}

要创建 Warehouse,您需要创建第二个服务,该服务将与现有服务共享数据。可以通过点击任何现有服务上的加号来完成此操作:

<Image
  img={compute_7}
  size='md'
  alt='在 Warehouse 中创建新服务'
  border
  background='white'
/>
...
_图 7 - 点击加号在 Warehouse 中创建新服务_

在服务创建界面上,原始服务将在下拉菜单中被选中作为新服务的数据源。创建完成后,这两个服务将组成一个 Warehouse。

### 重命名 Warehouse {#renaming-a-warehouse}

有两种方式可以重命名 Warehouse:

- 您可以在服务页面右上角选择"按 Warehouse 排序",然后点击 Warehouse 名称旁边的铅笔图标
- 您可以在任何服务上点击 Warehouse 名称,并在那里重命名 Warehouse

### 删除 Warehouse {#deleting-a-warehouse}

删除 Warehouse 意味着删除所有计算服务和数据(表、视图、用户等)。此操作无法撤销。
您只能通过删除首次创建的服务来删除 Warehouse。具体操作如下:

1. 删除除首次创建的服务之外创建的所有服务;
2. 删除首次创建的服务(警告:此步骤将删除 Warehouse 的所有数据)。
