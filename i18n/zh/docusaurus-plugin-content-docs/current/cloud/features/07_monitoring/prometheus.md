...
该功能支持集成 [Prometheus](https://prometheus.io/) 来监控 ClickHouse Cloud 服务。可通过 [ClickHouse Cloud API](/cloud/manage/api/api-overview) 端点获取 Prometheus 指标，从而安全地连接并将指标导出到 Prometheus 指标采集器。这些指标可以与仪表盘（例如 Grafana、Datadog）集成以进行可视化。

要开始使用，请先[生成一个 API 密钥](/cloud/manage/openapi)。

## Prometheus 端点 API，用于获取 ClickHouse Cloud 指标 {#prometheus-endpoint-api-to-retrieve-clickhouse-cloud-metrics}
...
| GET    | `https://api.clickhouse.cloud/v1/organizations/:organizationId/prometheus?filtered_metrics=[true \| false]` | 返回某个组织内所有服务的指标 |

**请求参数**

| Name             | Location               | Type               |
| ---------------- | ------------------ |------------------ |
| Organization ID  | Endpoint 地址 | uuid               |
| Service ID       | Endpoint 地址 | uuid（可选）               |
| filtered_metrics | Query 参数 | boolean（可选） |
...
ClickHouse Cloud 提供了一个特殊的指标 `ClickHouse_ServiceInfo`，它是一个始终为 `1` 的 `gauge` 类型指标。该指标包含所有 **Metric Labels**，以及以下标签：

|Label|Description|
|---|---|
|clickhouse_cluster_status|服务状态。可能为以下值之一：[`awaking` \| `running` \| `degraded` \| `idle` \| `stopped`]|
|clickhouse_version|服务当前运行的 ClickHouse 服务器版本|
|scrape|指示上一次抓取（scrape）的状态。可能为 `full` 或 `partial`|
|full|指示在上一次指标抓取期间未发生错误|
|partial|指示在上一次指标抓取期间发生了一些错误，并且只返回了 `ClickHouse_ServiceInfo` 指标。|

用于获取指标的请求不会唤醒处于空闲状态的服务。如果服务处于 `idle` 状态，则只会返回 `ClickHouse_ServiceInfo` 指标。

对于 ClickPipes，有一个类似的 `ClickPipes_Info` `gauge` 类型指标，除了包含 **Metric Labels** 外，还包含以下标签：
...
Prometheus 服务器会按指定的时间间隔从已配置的目标收集指标。下面是一个 Prometheus 服务器使用 ClickHouse Cloud 的 Prometheus Endpoint 的示例配置：
...
请注意，需要将 `honor_labels` 配置参数设置为 `true`，以便 instance 标签能够被正确填充。此外，上述示例中将 `filtered_metrics` 设置为 `true`，但应根据实际需求进行配置。
...
- **Metrics Endpoint** – 此方式的优点是无需任何额外组件或基础设施。该方案仅适用于 Grafana Cloud，只需要 ClickHouse Cloud Prometheus Endpoint 的 URL 和凭证即可。
- **Grafana Alloy** - Grafana Alloy 是与厂商无关的 OpenTelemetry (OTel) Collector 发行版，用于替代 Grafana Agent。它可以作为抓取器使用，可部署在自身的基础设施中，并与任意 Prometheus 端点兼容。

下文将说明如何使用这些选项，并重点介绍 ClickHouse Cloud Prometheus Endpoint 相关的细节。

### Grafana Cloud 与 metrics endpoint 集成 {#grafana-cloud-with-metrics-endpoint}

- 登录到您的 Grafana Cloud 帐号
- 选择 **Metrics Endpoint** 添加一个新的连接
- 将 Scrape URL 配置为指向 Prometheus endpoint，并使用 basic auth 和 API key/secret 配置连接
- 测试连接以确保可以成功连接
...
完成配置后，您应该可以在下拉列表中看到可用于配置仪表盘的指标：
...
如果使用 Grafana Cloud，可以在 Grafana 中进入 Alloy 菜单并按照屏幕提示安装 Alloy：

<Image img={prometheus_grafana_alloy} size="md" alt="Grafana Alloy" border />

<br />

这会将 Alloy 配置为使用 `prometheus.remote_write` 组件，通过身份验证令牌将数据发送到 Grafana Cloud 端点。之后用户只需修改 Alloy 配置（在 Linux 上位于 `/etc/alloy/config.alloy`），为 ClickHouse Cloud Prometheus 端点添加一个 scraper 即可。

下面是一个 Alloy 示例配置，其中包含一个 `prometheus.scrape` 组件，用于从 ClickHouse Cloud 端点抓取指标，以及自动配置的 `prometheus.remote_write` 组件。请注意，`basic_auth` 配置部分中包含 Cloud API key 的 ID 和 secret，分别用作用户名和密码。
...
请注意，需要将 `honor_labels` 配置项设置为 `true`，才能正确填充 `instance` 标签的值。


### 使用 Alloy 的自管理 Grafana {#grafana-self-managed-with-alloy}

使用自管理部署的 Grafana 的用户可以在[此处](https://grafana.com/docs/alloy/latest/get-started/install/)找到安装 Alloy agent 的说明。我们假定用户已经将 Alloy 配置为将 Prometheus 指标发送到其期望的目标。下面的 `prometheus.scrape` 组件会使 Alloy 抓取 ClickHouse Cloud 端点。我们假定 `prometheus.remote_write` 会接收已抓取的指标。如不存在该目标，请将 `forward_to` 键调整为实际目标端点。
...
完成配置后，你应该能在指标浏览器（Metrics Explorer）中看到与 ClickHouse 相关的指标：

<Image img={prometheus_grafana_metrics_explorer} size="md" alt="Grafana Metrics Explorer" border />

<br />

请注意，需要将 `honor_labels` 配置参数设置为 `true`，才能正确填充 `instance` 标签。


## 集成 Datadog {#integrating-with-datadog}

可以使用 Datadog 的 [Agent](https://docs.datadoghq.com/agent/?tab=Linux) 和 [OpenMetrics 集成](https://docs.datadoghq.com/integrations/openmetrics/) 从 ClickHouse Cloud 端点收集指标。下面是该 Agent 和集成的一份简单示例配置。请注意，通常建议只选择最关心的那部分指标。下面这个“全量”示例会导出成千上万种指标与实例的组合，Datadog 会将这些视为自定义指标。
