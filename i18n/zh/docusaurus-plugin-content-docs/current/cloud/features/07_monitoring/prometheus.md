...
# HELP ClickPipes_Info 始终等于 1。标签 "clickpipe_state" 表示当前管道状态：Stopped/Provisioning/Running/Paused/Failed
...
# HELP ClickPipes_SentBytes_Total 发送到 ClickHouse 的未压缩字节总量。
...
这将为 Alloy 配置一个 `prometheus.remote_write` 组件,用于将数据发送到带有身份验证令牌的 Grafana Cloud 端点。用户只需修改 Alloy 配置文件(在 Linux 上位于 `/etc/alloy/config.alloy`)以包含 ClickHouse Cloud Prometheus 端点的采集器配置。
