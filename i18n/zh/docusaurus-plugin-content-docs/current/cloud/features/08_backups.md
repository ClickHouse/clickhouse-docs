...
数据库备份通过确保在数据因任何不可预见的原因丢失时，能够从最近一次成功备份中将服务恢复到先前状态，从而提供了一层安全保障。
...
第 1 天,执行完全备份以启动备份链。第 2 天,执行增量备份,此时我们有一个完全备份和一个增量备份可供恢复使用。到第 7 天,备份链中有一个完全备份和六个增量备份,用户可以看到最近的两个增量备份。第 8 天,我们执行新的完全备份,第 9 天,当新链中有两个备份后,之前的链将被丢弃。
...
所有服务默认每天执行一次备份,从 Scale 层级开始,可以通过 Cloud 控制台的"设置"选项卡配置更多备份。
每个备份至少保留 24 小时。
...
- **保留期限**:每个备份的保留天数。保留期限最短可设置为 1 天,最长可设置为 30 天,中间有多个可选值。
...
在某些罕见情况下,备份调度器可能不会遵守为备份指定的**开始时间**。具体来说,如果在当前计划备份时间的 24 小时内已触发过成功的备份,就会出现这种情况。这可能是由于我们为备份设置的重试机制所致。在这种情况下,调度器将跳过当天的备份,并在次日的计划时间重新执行备份。
:::
...
本指南将介绍如何将备份导出到您的 AWS、GCP、Azure 对象存储,以及如何将这些备份恢复到您账户中的新 ClickHouse Cloud 服务。
