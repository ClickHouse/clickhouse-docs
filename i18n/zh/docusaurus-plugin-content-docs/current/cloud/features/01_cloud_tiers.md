...
ClickHouse Cloud 中提供了多个层级。
层级可以分配到任意组织级别，因此一个组织内的所有服务都属于同一层级。
本页将介绍哪些层级最适合您的具体使用场景。
...
- 支持单副本部署的经济实惠选项。
- 适用于数据量较小且对可靠性无严格要求的部门级使用场景。

:::note
基础版服务的规模固定,不支持自动或手动扩展。
如需扩展服务,您可以升级到规模版或企业版。
:::
...
- 支持企业级安全:
  - 单点登录(SSO)
  - 增强加密:适用于 AWS 和 GCP 服务。服务默认使用我们的密钥进行加密,可以轮换为客户密钥以启用客户管理的加密密钥(CMEK)。
