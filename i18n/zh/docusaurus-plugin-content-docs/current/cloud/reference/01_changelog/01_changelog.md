...
title: 'Cloud 更新日志'
description: 'ClickHouse Cloud 更新日志，说明各个 ClickHouse Cloud 版本中的最新变更与新增功能'
doc_type: 'changelog'
...
除了本 ClickHouse Cloud 更新日志外，请参阅 [Cloud Compatibility](/whats-new/cloud-compatibility) 页面。
...
- 我们很高兴地宣布 **ClickHouse Cloud** 现已在**两个新的公共区域**上线:
  ...
  - **AWS 首尔 (亚太地区, ap-northeast-2)** — 现在也支持 **ClickPipes**

  这些区域此前作为**私有区域**提供,现在**向所有用户开放**。
...
- ClickHouse Cloud 控制台现在支持以 1 vCPU、4 GiB 为增量配置副本大小。
  这些选项在创建新服务时以及在设置页面配置副本大小的最小值和最大值时均可使用。
- 自定义硬件配置(企业版功能)现已支持空闲模式。
...
- ClickHouse Cloud 中的 ClickStack 用户现已可以使用告警功能。
...
- **服务监控 - 资源利用率仪表板**  
  CPU 利用率和内存利用率指标的显示方式将从平均值改为显示特定时间段内的最大利用率指标,以便更好地识别资源配置不足的情况。
...
## 2025 年 8 月 22 日 {#august-22-2025}

- **ClickHouse Connector for AWS Glue**  
  现在可以使用官方的 [ClickHouse Connector for AWS Glue](/integrations/glue),该连接器可从 [AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-eqvmuopqzdg7s) 获取。它利用 AWS Glue 基于 Apache Spark 的无服务器引擎,实现 ClickHouse 与其他数据源之间的数据提取、转换和加载集成。请参阅发布[博客文章](http://clickhouse.com/blog/clickhouse-connector-aws-glue)开始使用,了解如何创建表以及在 ClickHouse 和 Spark 之间写入和读取数据。
- **服务最小副本数变更**  
  已扩容的服务现在可以[缩容](/manage/scaling)至使用单个副本(之前最小值为 2 个副本)。注意:单副本服务的可用性降低,不建议用于生产环境。
...
- **ClickPipes for MongoDB CDC 现已开放私有预览**
  现在您可以使用 ClickPipes 只需几次点击即可将数据从 MongoDB 复制到 ClickHouse Cloud,无需外部 ETL 工具即可实现实时分析。该连接器支持持续复制和一次性迁移,兼容 MongoDB Atlas 和自托管 MongoDB 部署。阅读[博客文章](https://clickhouse.com/blog/mongodb-cdc-clickhouse-preview)了解 MongoDB CDC 连接器概述,并[在此注册申请抢先体验](https://clickhouse.com/cloud/clickpipes/mongodb-cdc-connector)!


## August 8, 2025 {#august-08-2025}

- **通知**:当服务开始升级到新的 ClickHouse 版本时,用户将在 UI 中收到通知。可通过通知中心添加额外的邮件和 Slack 通知。
- **ClickPipes**:ClickHouse Terraform provider 现已支持 Azure Blob Storage (ABS) ClickPipes。有关如何以编程方式创建 ABS ClickPipe 的示例,请参阅 provider 文档。
  - [Bug 修复] 使用 Null 引擎写入目标表的对象存储 ClickPipes 现在会在 UI 中显示"总记录数"和"已摄取数据"指标。
...
除了副本数量(水平扩展)之外,此功能还允许您控制每个副本的规格大小。
每个 ClickPipe 的详情页面现在还包含各副本的 CPU 和内存使用率,帮助您更好地了解工作负载并更有把握地规划资源调整操作。
...
即可将 MySQL(或 MariaDB)数据实时直接同步到 ClickHouse Cloud,
...
- 我们现已正式支持用于管理数据库权限的 Terraform provider,
  ...
  了解更多信息。
- 企业版服务现在可以选择加入[慢速发布通道](/manage/updates/#slow-release-channel-deferred-upgrades),
  在常规发布后延迟两周进行升级,以便获得更多时间进行测试。
...
- 我们很高兴地宣布 **ClickPipes for Postgres CDC** 在 ClickHouse Cloud 中正式发布。只需点击几下,您现在就可以复制 Postgres 数据库并实现超快速的实时分析。该连接器提供更快的数据同步、低至几秒的延迟、自动模式变更、完全安全的连接等功能。更多信息请参阅[博客](https://clickhouse.com/blog/postgres-cdc-connector-clickpipes-ga)。要开始使用,请参阅[此处](https://clickhouse.com/docs/integrations/clickpipes/postgres)的说明。

- SQL 控制台仪表板新增改进功能:
  - 共享:您可以与团队成员共享仪表板。支持四个访问级别,可以在全局和每个用户级别进行调整:
    - _写入权限_:添加/编辑可视化、刷新设置、通过筛选器与仪表板交互。
    - _所有者_:共享仪表板、删除仪表板,以及具有"写入权限"的用户的所有其他权限。
...
- 我们正在招募 AWS 和 GCP 的[分布式缓存](https://clickhouse.com/cloud/distributed-cache-waitlist)预览参与者。更多信息请阅读[博客](https://clickhouse.com/blog/building-a-distributed-cache-for-s3)。
...
- 引入了资源利用率仪表板,用于查看 ClickHouse Cloud 服务所使用的资源。以下指标从系统表中采集并显示在此仪表板上:
  ...
  - 数据传输:显示 ClickHouse Cloud 数据流入和流出的图表。了解更多信息请访问[此处](/cloud/manage/network-data-transfer)。
- 我们很高兴地宣布推出全新的 ClickHouse Cloud Prometheus/Grafana 混合组件,旨在简化 ClickHouse Cloud 服务的监控。该混合组件使用我们兼容 Prometheus 的 API 端点,将 ClickHouse 指标无缝集成到您现有的 Prometheus 和 Grafana 环境中。它包含一个预配置的仪表板,让您能够实时查看服务的健康状况和性能表现。请参阅发布[博客](https://clickhouse.com/blog/monitor-with-new-prometheus-grafana-mix-in)了解更多信息。
...
- 引入了 **AWS PrivateLink for ClickPipes**。您可以使用 AWS PrivateLink 在 VPC、AWS 服务、本地系统和 ClickHouse Cloud 之间建立安全连接。在从 AWS 上的 Postgres、MySQL 和 MSK 等数据源移动数据时,可以在不将流量暴露到公共互联网的情况下完成此操作。它还支持通过 VPC 服务端点进行跨区域访问。PrivateLink 连接设置现在可以通过 ClickPipes [完全自助完成](https://clickhouse.com/docs/integrations/clickpipes/aws-privatelink)。
...
- 运行生产环境和开发环境服务的用户现在可在账单中看到 ClickPipes 和数据传输的使用费用。
...
- 新增 `UsageCost` API 端点:API 规范现已支持用于获取使用情况信息的新端点。这是一个组织级端点,支持查询最长 31 天的使用成本。可获取的指标包括存储、计算、数据传输和 ClickPipes。详情请参阅[文档](https://clickhouse.com/docs/cloud/manage/api/usageCost-api-reference)。
...
### ClickHouse Bring Your Own Cloud (BYOC) AWS版现已正式发布 {#clickhouse-byoc-for-aws-ga}

在此部署模式中,数据平面组件(计算、存储、备份、日志、指标)
运行在客户VPC中,而控制平面(Web访问、API和计费)
保留在ClickHouse VPC中。此架构非常适合需要
遵守严格数据驻留要求的大规模工作负载,可确保所有数据保留
在安全的客户环境中。

- 有关更多详细信息,您可以参考BYOC的[文档](/cloud/reference/byoc/overview)
...
### ClickPipes的Postgres CDC连接器 {#postgres-cdc-connector-for-clickpipes}

ClickPipes的Postgres CDC连接器允许用户无缝地将Postgres数据库复制到ClickHouse Cloud。

- 要开始使用,请参考ClickPipes Postgres CDC连接器的[文档](https://clickhouse.com/docs/integrations/clickpipes/postgres)。
...
### ClickHouse Cloud AWS版的PCI合规性 {#pci-compliance-for-clickhouse-cloud-on-aws}

ClickHouse Cloud现在为**us-east-1**和**us-west-2**区域的**企业级**
客户提供**PCI合规服务**支持。希望在PCI合规环境中启动
服务的用户可以联系[支持团队](https://clickhouse.com/support/program)
获取帮助。

### Google Cloud Platform上的透明数据加密和客户管理的加密密钥 {#tde-and-cmek-on-gcp}

**Google Cloud Platform (GCP)**上的ClickHouse Cloud现已支持**透明数据加密(TDE)**和**客户管理的
加密密钥(CMEK)**。

- 有关这些功能的更多信息,请参考[文档](https://clickhouse.com/docs/cloud/security/cmek#transparent-data-encryption-tde)。

### AWS中东(阿联酋)区域可用性 {#aws-middle-east-uae-availability}

ClickHouse Cloud新增区域支持,现已在
**AWS中东(阿联酋)me-central-1**区域可用。

### ClickHouse Cloud防护机制 {#clickhouse-cloud-guardrails}

为了推广最佳实践并确保ClickHouse Cloud的稳定使用,我们正在
引入针对使用中的表、数据库、分区和数据部分数量的防护机制。

- 有关详细信息,请参考文档的[使用限制](https://clickhouse.com/docs/cloud/bestpractices/usage-limits)
  部分。
- 如果您的服务已经超过这些限制,我们将允许增加10%。
...
我们新增了 **Enterprise 企业层级**,以满足最严苛的客户和工作负载需求,重点提供行业特定的安全性和合规性功能、对底层硬件和升级的更精细控制,以及高级灾难恢复功能。
...
您可以在此[博客文章](https://clickhouse.com/blog/evolution-of-clickhouse-cloud-new-features-superior-performance-tailored-offerings)中了解这些功能变更的详细信息。现有客户需要主动选择[新的服务计划](https://clickhouse.com/pricing)。我们已通过电子邮件向组织管理员发送了相关通知。

### Warehouses:计算-计算分离(正式发布){#warehouses-compute-compute-separation-ga}

计算-计算分离(也称为 "Warehouses")现已正式发布;请参阅[博客文章](https://clickhouse.com/blog/introducing-warehouses-compute-compute-separation-in-clickhouse-cloud)了解更多详情,以及查阅[文档](/cloud/reference/warehouses)。

### 单副本服务 {#single-replica-services}

我们推出了"单副本服务"概念,既可作为独立产品使用,也可在 Warehouses 中使用。作为独立产品时,单副本服务在规模上有所限制,适用于小型测试工作负载。在 Warehouses 中,单副本服务可以部署更大的规模,适用于不需要大规模高可用性的工作负载,例如可重启的 ETL 作业。

### 垂直自动扩展改进 {#vertical-auto-scaling-improvements}

我们为计算副本引入了一种新的垂直扩展机制,称为"先建后断"(Make Before Break,MBB)。该方法在移除旧副本之前先添加一个或多个新规模的副本,从而避免在扩展操作期间出现任何容量损失。通过消除移除现有副本和添加新副本之间的间隙,MBB 实现了更加流畅、中断更少的扩展过程。该机制在扩容场景中尤其有益,因为在高资源利用率触发额外容量需求时,过早移除副本只会加剧资源紧张。

### 水平扩展(正式发布){#horizontal-scaling-ga}

水平扩展现已正式发布。用户可以通过 API 和云控制台添加额外的副本来横向扩展服务。请参阅[文档](/manage/scaling#manual-horizontal-scaling)了解相关信息。
...
安全的托管升级为用户提供了重要价值,使他们能够在数据库不断添加新功能的同时保持版本最新。在本次发布中,我们将"先建后断"(MBB)方法应用于升级过程,进一步降低对运行中工作负载的影响。
...
用户可以为其服务安排升级计划。此功能仅支持 Enterprise 企业层级服务。有关计划升级的更多信息,请参阅[文档](/manage/updates)。
...
### DBT 对可刷新物化视图的支持 {#dbt-support-for-refreshable-materialized-views}
...
- **组织级端点**。我们为 ClickHouse Cloud 的 Prometheus 集成引入了一项增强功能。除服务级指标外,API 现在还包含一个**组织级指标**端点。该端点会自动收集组织内所有服务的指标,简化了将指标导出到 Prometheus 收集器的流程。这些指标可以与 Grafana 和 Datadog 等可视化工具集成,以更全面地了解组织的性能表现。
  ...
  此功能现已面向所有用户开放。您可以在[此处](/integrations/prometheus)找到更多详细信息。
...
- **过滤指标**。我们在 ClickHouse Cloud 的 Prometheus 集成中添加了对返回过滤指标列表的支持。此功能使您能够专注于对监控服务健康状况至关重要的指标,从而有助于减小响应负载大小。
  ...
  过滤指标功能现已面向所有用户开放。您可以在[此处](/integrations/prometheus)找到更多详细信息。
...
ClickHouse Cloud 现已支持针对多种计费和扩缩容事件的控制台内通知和邮件通知。客户可以通过云控制台通知中心配置这些通知,选择仅在界面中显示、接收邮件或同时启用两者。您可以在服务级别配置所接收通知的类别和严重程度。

未来,我们将为其他事件添加通知功能,并提供更多接收通知的方式。
...
### ClickHouse Cloud 现在为 GCP 提供符合 HIPAA 标准的服务(Beta 版){#clickhouse-cloud-now-offers-hipaa-ready-services-in-beta-for-gcp}

寻求增强受保护健康信息(PHI)安全性的客户现在可以在 [Google Cloud Platform (GCP)](https://cloud.google.com/) 上使用 ClickHouse Cloud。ClickHouse 已实施 [HIPAA 安全规则](https://www.hhs.gov/hipaa/for-professionals/security/index.html)规定的管理、物理和技术保障措施,并提供可配置的安全设置,可根据您的具体用例和工作负载进行实施。有关可用安全设置的更多信息,请查看我们的[安全功能页面](/cloud/security)。
...
我们很高兴分享 ClickPipes 的最新更新,它是将数据导入 ClickHouse 服务的最简单方式。这些新功能旨在增强您对数据导入的控制,并提供更好的性能指标可见性。
...
我们的 SOC 2 Type II 报告证明了我们对为 ClickHouse 用户提供的服务在安全性、可用性、处理完整性和机密性方面的持续承诺。如需了解更多信息,请查看美国注册会计师协会 (AICPA) 发布的 [SOC 2 - 服务组织控制:信任服务标准](https://www.aicpa-cima.com/resources/landing/system-and-organization-controls-soc-suite-of-services),以及国际标准化组织 (ISO) 的 [什么是 ISO/IEC 27001](https://www.iso.org/standard/27001)。

另请访问我们的[信任中心](https://trust.clickhouse.com/)获取安全与合规文档和报告。
...
我们的新计算-计算分离功能使您能够创建多个计算节点组,每个组都有自己的端点,它们使用相同的对象存储文件夹,因此具有相同的表、视图等。阅读更多关于[计算-计算分离](/cloud/reference/warehouses)的信息。如果您希望在私有预览阶段访问此功能,请[联系支持团队](https://clickhouse.com/support/program)。
...
在上一次的云更新日志中,我们宣布了从 ClickHouse Cloud 导出 [Prometheus](https://prometheus.io/) 指标的私有预览版。此功能允许您使用 [ClickHouse Cloud API](/cloud/manage/api/api-overview) 将指标导入 [Grafana](https://grafana.com/) 和 [Datadog](https://www.datadoghq.com/) 等工具进行可视化。我们很高兴地宣布,此功能现已**正式发布**。请参阅[我们的文档](/integrations/prometheus)以了解有关此功能的更多信息。
...
我们在[今年5月](https://clickhouse.com/blog/clickhouse-cloud-is-now-on-azure-in-public-beta)首次发布了 Microsoft Azure 支持的 Beta 版本。在此次最新的云版本中,我们很高兴地宣布 Azure 支持已从 Beta 版过渡到正式发布(GA)。ClickHouse Cloud 现已在三大主流云平台上可用:AWS、Google Cloud Platform 以及 Microsoft Azure。

此版本还支持通过 [Microsoft Azure Marketplace](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/clickhouse.clickhouse_cloud) 进行订阅。该服务最初将在以下区域提供支持:
...
我们在 Cloud 控制台中推出的全新查询洞察 UI 使 ClickHouse 内置的查询日志更易于使用。ClickHouse 的 `system.query_log` 表是查询优化、调试以及监控整体集群健康状况和性能的关键信息来源。但有一点需要注意:由于每个查询包含70多个字段和多条记录,解读查询日志具有较高的学习门槛。此初始版本的查询洞察为未来简化查询调试和优化模式的工作提供了基础框架。我们非常希望听到您的反馈,在我们持续迭代此功能时,请随时联系我们——您的意见将非常宝贵。
...
- [可配置备份](/cloud/manage/backups/configurable-backups)现已正式发布,可用于配置自定义备份策略,如频率、保留期和计划。
...
在此之前,每当您设置新的 [Kafka Connector for ClickPipes](/integrations/clickpipes/kafka) 时,它总是从 Kafka 主题的起始位置开始消费数据。这种方式在某些特定场景下灵活性不足,例如当您需要重新处理历史数据、监控新传入的数据或从精确的位置恢复时。

ClickPipes for Kafka 新增了一项功能,增强了从 Kafka 主题消费数据时的灵活性和控制能力。您现在可以配置数据消费的起始偏移量。

可用选项如下:

- 从起始位置:从 Kafka 主题的最开始位置消费数据。此选项适合需要重新处理所有历史数据的用户。
- 从最新位置:从最新的偏移量开始消费数据。此选项适合只关注新消息的用户。
- 从指定时间戳:从特定时间戳或之后生成的消息开始消费数据。此功能提供更精确的控制,使用户能够从确切的时间点恢复处理。
...
快速发布渠道允许您的服务提前接收更新,无需等待常规发布计划。以前,启用此功能需要联系支持团队协助。现在,您可以直接通过 ClickHouse Cloud 控制台为您的服务启用此功能。只需导航到**设置**,然后点击**注册快速发布**。您的服务将在更新可用时立即接收。
...
ClickHouse Cloud 支持[水平扩展](/manage/scaling#how-scaling-works-in-clickhouse-cloud),即为您的服务添加相同规格的额外副本。水平扩展可以提高性能和并行化能力,以支持并发查询。以前,添加更多副本需要使用 ClickHouse Cloud 控制台或 API。现在,您可以使用 Terraform 为服务添加或删除副本,从而能够根据需要以编程方式扩展您的 ClickHouse 服务。
...
当您编写SQL查询时,团队中的其他成员很可能也会觉得该查询有用。以前,您必须通过Slack或电子邮件发送查询,而且如果您对查询进行了编辑,团队成员也无法自动接收更新。

我们很高兴地宣布,您现在可以通过ClickHouse Cloud控制台轻松共享查询。在查询编辑器中,您可以直接与整个团队或特定团队成员共享查询。您还可以指定他们拥有只读权限还是写入权限。点击查询编辑器中的**共享**按钮即可试用新的共享查询功能。

<Image
  img={share_queries}
  size='lg'
  alt='ClickHouse Cloud查询编辑器显示带有权限选项的共享功能'
  border
/>

### ClickHouse Cloud for Microsoft Azure现已进入公测阶段 {#clickhouse-cloud-for-microsoft-azure-is-now-in-beta}

我们终于推出了在Microsoft Azure上创建ClickHouse Cloud服务的功能。作为私有预览计划的一部分,已经有许多客户在生产环境中使用Azure上的ClickHouse Cloud。现在,任何人都可以在Azure上创建自己的服务。AWS和GCP上支持的所有ClickHouse功能也将在Azure上可用。

我们预计在未来几周内ClickHouse Cloud for Azure将正式发布(General Availability)。[阅读此博客文章](https://clickhouse.com/blog/clickhouse-cloud-is-now-on-azure-in-public-beta)了解更多信息,或通过ClickHouse Cloud控制台在Azure上创建新服务。

注意:目前暂不支持Azure的**开发**服务。

### 通过Cloud控制台设置Private Link {#set-up-private-link-via-the-cloud-console}

我们的Private Link功能允许您将ClickHouse Cloud服务与云提供商账户中的内部服务连接,而无需将流量导向公共互联网,从而节省成本并增强安全性。以前,这项功能设置起来比较困难,需要使用ClickHouse Cloud API。

现在,您只需在ClickHouse Cloud控制台中点击几下即可配置私有端点。只需转到服务的**设置**,进入**安全**部分,然后点击**设置私有端点**。

<Image
  img={private_endpoint}
  size='lg'
  alt='ClickHouse Cloud控制台显示安全设置中的私有端点设置界面'
  border
/>
...
当您为 ClickHouse 编写 SQL 查询时,仍然需要通过驱动程序连接到 ClickHouse 才能将查询公开给您的应用程序。现在通过我们的**查询端点**功能,您可以直接从 API 执行 SQL 查询,无需任何配置。您可以指定查询端点返回 JSON、CSV 或 TSV 格式。在云控制台中点击"共享"按钮,即可使用您的查询试用此新功能。在[此处](https://clickhouse.com/blog/automatic-query-endpoints)了解有关查询端点的更多信息。
...
我们最新的 ClickPipes 更新提供了直接从 Amazon S3 和 Google Cloud Storage 上传数据的能力。虽然您仍然可以使用我们的内置表函数,但 ClickPipes 是通过我们的用户界面提供的完全托管服务,只需点击几下即可从 S3 和 GCS 导入数据。此功能目前仍处于私有预览阶段,但您可以立即通过云控制台试用。
...
ClickHouse 可以快速查询您的所有大型数据集,但数据必须首先导入到 ClickHouse 中。借助 Fivetran 全面的连接器系列,用户现在可以快速从 500 多个数据源加载数据。无论您需要从 Zendesk、Slack 还是任何您喜爱的应用程序加载数据,Fivetran 的新 ClickHouse 目标连接器现在都允许您将 ClickHouse 用作应用程序数据的目标数据库。
...
本次发布为 ClickHouse Cloud 引入了新的 AWS 东京区域(`ap-northeast-1`)。为了让 ClickHouse 成为最快的数据库,我们持续为各个云平台添加更多区域,以尽可能降低延迟。您可以在更新后的云控制台中创建东京区域的新服务。
...
- NodeJS 客户端主要稳定版本发布:为查询 + ResultSet 提供高级 TypeScript 支持,支持 URL 配置
...
每月有数千名 ClickHouse Cloud 用户在我们的 SQL 控制台上执行数十亿次查询,正因如此,我们决定加大投入打造世界级控制台,让您能够比以往更轻松地使用 ClickHouse Cloud 服务。我们全新的云控制台体验将独立 SQL 编辑器与管理控制台整合到一个直观的用户界面中。

部分客户将率先体验我们的全新云控制台——一种统一且沉浸式的方式来探索和管理您在 ClickHouse 中的数据。如果您希望优先获得访问权限,请通过 support@clickhouse.com 与我们联系。
...
- 引入了发布渠道 – 可根据环境类型指定升级时间的功能。在此版本中,我们添加了"快速"发布渠道,使您能够在生产环境之前升级非生产环境(请联系支持团队以启用)。

### 管理变更 {#administration-changes}

- 添加了通过 API 进行水平扩展配置的支持(私有预览版,请联系支持团队以启用)
- 改进了自动扩展功能,以扩展在启动时遇到内存不足错误的服务
...
- ClickHouse Golang Client:[修复了](https://github.com/ClickHouse/clickhouse-go/pull/1236) Enum 作为 Map 键的错误;[修复了](https://github.com/ClickHouse/clickhouse-go/pull/1237)错误连接留在连接池中的问题(社区贡献)
...
此版本以早期访问形式提供了全新的 Cloud 控制台体验、用于从 S3 和 GCS 批量加载数据的 ClickPipes,以及 ClickPipes for Kafka 对 Avro 格式的支持。此版本还将 ClickHouse 数据库版本升级至 24.1,带来了对新函数的支持以及性能和资源使用优化。

### 控制台变更 {#console-changes-2}

- 全新的 Cloud 控制台体验以早期访问形式提供(如果您有兴趣参与,请联系支持团队)。
- 用于从 S3 和 GCS 批量加载数据的 ClickPipes 以早期访问形式提供(如果您有兴趣参与,请联系支持团队)。
- ClickPipes for Kafka 对 Avro 格式的支持以早期访问形式提供(如果您有兴趣参与,请联系支持团队)。

### ClickHouse 版本升级 {#clickhouse-version-upgrade}

- FINAL 优化、向量化改进、更快的聚合 - 详情请参阅 [23.12 版本博客](https://clickhouse.com/blog/clickhouse-release-23-12#optimizations-for-final)。
- 用于处理 punycode、字符串相似度、检测异常值的新函数,以及针对合并和 Keeper 的内存优化 - 详情请参阅 [24.1 版本博客](https://clickhouse.com/blog/clickhouse-release-24-01)和[演示文稿](https://presentations.clickhouse.com/release_24.1/)。
- 此 ClickHouse Cloud 版本基于 24.1,包含数十项新功能、性能改进和错误修复。详情请参阅核心数据库[变更日志](/whats-new/changelog/2023#2312)。

### 集成变更 {#integrations-changes-2}

- Grafana:修复了 v4 的仪表板迁移、即席过滤逻辑
- Tableau Connector:修复了 DATENAME 函数和 "real" 参数的舍入
- Kafka Connector:修复了连接初始化中的 NPE,添加了指定 JDBC 驱动程序选项的功能
- Golang 客户端:减少了处理响应的内存占用,修复了 Date32 极值,修复了启用压缩时的错误报告
- Python 客户端:改进了 datetime 参数中的时区支持,提升了 Pandas DataFrame 的性能
...
- Metabase:修复了与 23.8 版本以下的 ClickHouse 的不兼容问题
- DBT:添加了向模型创建传递设置的功能
...
- [Grafana 的 ClickHouse 数据源](https://grafana.com/grafana/plugins/grafana-clickhouse-datasource/) v4 版本发布
  - 完全重构了查询构建器,为表、日志、时间序列和追踪提供专用编辑器
  - 完全重构了 SQL 生成器,以支持更复杂和动态的查询
  - 在日志和追踪视图中新增对 OpenTelemetry 的原生支持
  - 扩展了配置功能,允许为日志和追踪指定默认表和列
  - 新增指定自定义 HTTP 标头的功能
...
- Java 客户端:
  ...
  - 添加了对 RowBinaryWithDefaults 格式的支持,以减少 ClickHouse 实例的批处理大小和工作负载(由 Exabeam 请求)
...
- 面向用户的向后不兼容变更:以前,两个功能([is_deleted](/engines/table-engines/mergetree-family/replacingmergetree#is_deleted) 和 `OPTIMIZE CLEANUP`)在某些条件下可能导致 ClickHouse 中的数据损坏。为了保护用户数据的完整性,同时保留功能的核心部分,我们调整了此功能的工作方式。具体而言,MergeTree 设置 `clean_deleted_rows` 现已弃用且不再有任何效果。默认情况下不允许使用 `CLEANUP` 关键字(要使用它,您需要启用 `allow_experimental_replacing_merge_with_cleanup`)。如果您决定使用 `CLEANUP`,则需要确保始终将其与 `FINAL` 一起使用,并且必须保证在运行 `OPTIMIZE FINAL CLEANUP` 后不会插入具有旧版本的行。
...
此版本为亚洲地区的开发服务增加了更多区域支持,引入了客户管理加密密钥的密钥轮换功能,改进了账单控制台中税务设置的粒度,并修复了各支持语言客户端中的多个错误。
...
- Python 客户端
  ...
  - 修复了 NumPy/Pandas Date32 问题
    ​​- Golang 客户端
  - 修复了向 JSON 列插入空映射、压缩缓冲区清理、查询转义、IPv4 和 IPv6 零值/空值时的 panic 问题
  - 新增对已取消插入操作的监控
- DBT
  - 改进了分布式表支持并增加了测试
...
此版本为 SQL 控制台带来了易用性和性能改进,优化了 Metabase 连接器中的 IP 数据类型处理,并为 Java 和 Node.js 客户端增加了新功能。
...
此版本正式发布了适用于 Kafka、Confluent Cloud 和 Amazon MSK 的 ClickPipes 以及 Kafka Connect ClickHouse Sink,提供了通过 IAM 角色保护 Amazon S3 访问的自助服务工作流,以及 AI 辅助查询建议(私有预览)。
...
此版本带来了 ClickPipes for Kafka 的私有预览、全新的数据加载体验,以及通过云控制台从 URL 加载文件的能力。

### 集成变更 {#integrations-changes-14}

- 推出了 [ClickPipes](https://clickhouse.com/cloud/clickpipes) for Kafka 的私有预览,这是一个云原生集成引擎,只需点击几下即可从 Kafka 和 Confluent Cloud 摄取海量数据。请在[此处](https://clickhouse.com/cloud/clickpipes#joinwaitlist)注册候补名单。
- JavaScript 客户端:发布了对基于 Web 环境(浏览器、Cloudflare workers)的支持。代码已重构,允许社区为自定义环境创建连接器。
- Kafka Connector:新增了对带有 Timestamp 和 Time Kafka 类型的内联模式的支持
...
- 改进了邀请流程,新增了加入不同组织和查看所有待处理邀请的选项
...
此版本正式推出 GCP 上的 ClickHouse Cloud,为 Cloud API 提供 Terraform provider,并将 ClickHouse 版本更新至 23.4。

### 常规更新 {#general-updates-4}

- GCP 上的 ClickHouse Cloud 现已正式发布,集成 GCP Marketplace、支持 Private Service Connect 并提供自动备份功能(详情请参阅[博客](https://clickhouse.com/blog/clickhouse-cloud-on-google-cloud-platform-gcp-is-generally-available)和[新闻稿](https://clickhouse.com/blog/clickhouse-cloud-expands-choice-with-launch-on-google-cloud-platform))
...
- Kafka Connector:可上传并安装至 Confluent Cloud,为临时连接问题增加了重试机制,自动重置错误的连接器状态

### ClickHouse 23.4 版本升级 {#clickhouse-234-version-upgrade}

- 为并行副本新增 JOIN 支持(请联系[支持团队](https://console.clickhouse.cloud/support)进行配置)
- 提升了轻量级删除的性能
- 改进了处理大批量插入时的缓存性能
...
此版本正式发布了 ClickHouse Cloud 控制平面操作的程序化 API(详见[博客](https://clickhouse.com/blog/using-the-new-clickhouse-cloud-api-to-automate-deployments)),支持使用 IAM 角色访问 S3,以及新增扩展选项。

### 常规变更 {#general-changes-2}

- ClickHouse Cloud 的 API 支持。通过新的 Cloud API,您可以无缝地将服务管理集成到现有的 CI/CD 流水线中,并以编程方式管理您的服务
...
- Golang 客户端:修复了导致原生协议中连接不平衡的问题,新增了对原生协议中自定义设置的支持
...
- Grafana:在 Grafana 数据源设置中新增指定 ClickHouse 自定义设置和代理环境变量的功能
- Kafka Connector:改进了空记录的处理

### 控制台变更 {#console-changes-18}

- 在用户列表中新增多因素身份验证(MFA)使用指示器
...
此版本引入了新的热力图图表类型,改进了账单使用页面,并提升了服务启动时间。

### 控制台变更 {#console-changes-19}

- 在 SQL 控制台中添加了热力图图表类型
- 改进了账单使用页面,可显示每个计费维度内消耗的额度

### 集成变更 {#integrations-changes-19}

- Kafka 连接器:为瞬态连接错误添加了重试机制
- Python 客户端:添加了 max_connection_age 设置,以确保 HTTP 连接不会被永久重用。这有助于解决某些负载均衡问题
...
- 数十项新功能、性能改进和错误修复——详情请参阅 23.3 和 23.2 的详细[变更日志](/whats-new/changelog/index.md)
...
- 在数据加载模态框的架构推断中新增尽力而为的日期时间检测功能

### 集成变更 {#integrations-changes-21}

- [Metabase](/integrations/data-visualization/metabase-and-clickhouse.md): 新增对多架构的支持
- [Go client](/integrations/language-clients/go/index.md): 修复了 TLS 连接的空闲连接活性检查
...
- 修复了从 SQL 控制台运行 `INSERT INTO ... SELECT ...` 时错误地应用与 SELECT 查询相同的行数限制的问题
...
- Python 客户端: 修复了临时表插入问题,并新增对 Pandas Null 的支持
...
- 优化了备份调度，仅在数据被修改时运行备份
- 缩短了大型备份的完成时间

### 配置变更 {#configuration-changes-1}

- 新增了通过在查询或连接级别覆盖 `max_table_size_to_drop` 和 `max_partition_size_to_drop` 设置来提高删除表和分区限制的功能
- 在查询日志中添加了源 IP，以支持基于源 IP 的配额和访问控制
...
- [Python](/integrations/language-clients/python/index.md) 客户端:为插入操作新增[流式支持](/integrations/language-clients/python/advanced-querying.md#streaming-queries)
...
- 新增用于固定精度十进制运算的数学函数
- 完整的变更列表请参阅 [22.12 版本发布博客](https://clickhouse.com/blog/clickhouse-release-22-12)和[详细的 22.12 变更日志](/whats-new/cloud#clickhouse-2212-version-upgrade)
...
- [Metabase 插件](/integrations/data-visualization/metabase-and-clickhouse.md)发布了期待已久的 v0.9.1 重大更新。现在它与最新的 Metabase 版本兼容,并已针对 ClickHouse Cloud 进行了全面测试。
...
- 新增对复合时间间隔语法的支持
- 通过插入重试机制提高了插入可靠性
- 有关完整的变更列表,请参阅[详细的 22.11 更新日志](/whats-new/cloud#clickhouse-2211-version-upgrade)
...
- 修复了当某些分区存在单个大文件时可能导致备份失败的错误
- 修复了在同时应用访问列表更改时从备份恢复失败的错误
...
- 出于稳定性考虑,用户不再能够更改设置 `allow_suspicious_low_cardinality_types`、`allow_suspicious_fixed_string_types` 和 `allow_suspicious_codecs`(默认值为 false)。

### 控制台变更 {#console-changes-31}

- 将付费客户的自助服务垂直扩展上限提高至 720GB 内存
...
此版本显著降低了小型工作负载的计算资源消耗,降低了计算定价(详情请参阅[定价](https://clickhouse.com/pricing)页面),通过更优的默认配置提高了稳定性,并增强了 ClickHouse Cloud 控制台中的账单和使用情况视图。
...
- 将 max_parts_in_total 从 100k 降低至 10k。MergeTree 表的 `max_parts_in_total` 设置的默认值已从 100,000 降低至 10,000。此变更的原因是我们观察到大量数据分片可能会导致云服务启动时间变慢。大量分片通常表明选择了过于细粒度的分区键,这通常是无意中造成的,应该避免。默认值的变更将有助于更早地发现这些情况。
...
## 2022年10月4日 - Beta {#october-4-2022---beta}

ClickHouse Cloud 于2022年10月4日开始公开 Beta 测试。更多信息请参阅此[博客](https://clickhouse.com/blog/clickhouse-cloud-public-beta)。

ClickHouse Cloud 版本基于 ClickHouse 核心版本 v22.10。有关兼容功能列表,请参阅 [Cloud 兼容性](/whats-new/cloud-compatibility)指南。
