...
* **数据目录集成：** 现在可以将外部数据目录作为数据源进行连接。ClickHouse Cloud 支持 AWS Glue 和 Unity Catalog（后续会支持更多）。连接后，该目录会以数据库的形式出现，使你可以直接读取 Iceberg 表，而无需复制数据。若需开通访问权限，请联系支持团队进行申请。
* **ClickPipes：**
  * MongoDB 连接器已升级为 Public Beta，并现已在所有服务级别向新老 ClickHouse Cloud 客户开放。阅读该[博客文章](https://clickhouse.com/blog/mongodb-cdc-connector-clickpipes-beta)了解新特性的概览，并前往[文档](/integrations/clickpipes/mongodb)开始使用。
  * [S3 ClickPipe](/integrations/clickpipes/object-storage/s3/overview) 现在兼容 OVH Object Storage，这是 OVHcloud 提供的、兼容 S3 API 的对象存储服务。
...
* 我们对 SQL 控制台仪表板进行了改进：
  * 共享：您可以与团队成员共享仪表板。支持四种访问级别，可在全局和按用户维度进行调整：
    * *写入访问（Write access）*：添加/编辑可视化、刷新设置，并通过筛选器与仪表板交互。
    * *所有者（Owner）*：共享仪表板、删除仪表板，以及拥有“写入访问”用户的所有权限。
    * *只读访问（Read-only access）*：通过筛选器查看并与仪表板交互。
    * *无访问（No access）*：无法查看仪表板。
...
* Kafka Connect Sink：新增代理支持、`topic-tablename` 映射，以及 Keeper 的 *exactly-once* 投递属性的可配置能力。
* Node.js 客户端：新增对 Parquet 格式的支持。
