...
* 当元数据事务提交后，重写磁盘对象存储的事务会删除之前的远程 blob。[#81787](https://github.com/ClickHouse/ClickHouse/pull/81787) ([Sema Checherinda](https://github.com/CheSema)).
* 使 S3 重试策略可配置，并在配置 XML 文件发生变更时支持对 S3 磁盘设置进行热加载。 [#82642](https://github.com/ClickHouse/ClickHouse/pull/82642) ([RinChanNOW](https://github.com/RinChanNOWWW)).
* 修复了在结果类型的 LowCardinality 在优化前后不一致时，对冗余相等表达式的优化阶段。 [#82651](https://github.com/ClickHouse/ClickHouse/pull/82651) ([Yakov Olkhovskiy](https://github.com/yakov-olkhovskiy)).
...
* 当 HTTP 客户端在设置 `Expect: 100-continue` 的同时额外设置请求头 `X-ClickHouse-100-Continue: defer` 时，ClickHouse 在完成 QUOTA 校验之前不会向客户端发送 `100 Continue` 响应，从而避免为那些最终会被丢弃的请求体浪费网络带宽。这在 INSERT 查询场景中尤为相关：此时查询本身可以通过 URL 查询字符串发送，而数据则放在请求体中。在未发送完整请求体的情况下中止请求，会导致在 HTTP/1.1 下无法复用连接，但与在插入大批量数据时整体 INSERT 操作的耗时相比，为建立新连接所引入的额外延迟通常可以忽略不计。[#84304](https://github.com/ClickHouse/ClickHouse/pull/84304) ([c-end](https://github.com/c-end))。
...
* 通过使用 `system.completions` 替代对 system 表的多次查询，客户端自动补全会更快且行为更一致。[#84694](https://github.com/ClickHouse/ClickHouse/pull/84694) ([|2ustam](https://github.com/RuS2m))。
* 简化了与在 `best_effort` 格式下解析 Time[64] 相关的逻辑，并规避了一些 bug。[#84730](https://github.com/ClickHouse/ClickHouse/pull/84730) ([Yarik Briukhovetskyi](https://github.com/yariks5s)).
...
* 将 jemalloc 更新到较新版本。基于 jemalloc 的内部工具改进了内存分配分析。现在可以通过配置项 `jemalloc_enable_global_profiler` 启用全局 jemalloc 分析器。通过启用配置项 `jemalloc_collect_global_profile_samples_in_trace_log`，可以将采样的全局分配和释放记录到 `system.trace_log` 中，类型为 `JemallocSample`。现在可以使用设置项 `jemalloc_enable_profiler` 为每个查询单独启用 jemalloc 分析。是否将采样数据存储到 `system.trace_log` 中，可以通过设置项 `jemalloc_collect_profile_samples_in_trace_log` 在每个查询级别进行控制。[#85438](https://github.com/ClickHouse/ClickHouse/pull/85438) ([Antonio Andelic](https://github.com/antonio2368)).
* 新增 `deltaLakeAzureCluster` 函数（类似于用于集群的 `deltaLakeAzure` 函数）以及 `deltaLakeS3Cluster` 函数（是 `deltaLakeCluster` 的别名），修复了 [#85358](https://github.com/ClickHouse/ClickHouse/issues/85358)。[#85547](https://github.com/ClickHouse/ClickHouse/pull/85547)（[Smita Kulkarni](https://github.com/SmitaRKulkarni)）。
...
* 修复了在生存时间 (TTL) 中包含 `materialize` 函数时出现的格式不一致问题。关闭 [#82828](https://github.com/ClickHouse/ClickHouse/issues/82828)。[#85749](https://github.com/ClickHouse/ClickHouse/pull/85749)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* 在普通拷贝操作中也像在备份时一样应用 azure&#95;max&#95;single&#95;part&#95;copy&#95;size 设置。[#85767](https://github.com/ClickHouse/ClickHouse/pull/85767) ([Ilya Golshtein](https://github.com/ilejn))。
...
* 改进了 S3(Azure)Queue 表引擎，使其在 ZooKeeper 连接丢失时仍能继续工作，并避免潜在的重复数据。需要启用 S3Queue 设置项 `use_persistent_processing_nodes`（可通过 `ALTER TABLE MODIFY SETTING` 修改）。[#85995](https://github.com/ClickHouse/ClickHouse/pull/85995)（[Kseniia Sumarokova](https://github.com/kssenii)）。
* Iceberg 表状态不再存储在对象存储的单个对象中。这应当使得在 ClickHouse 中可以对 Iceberg 执行并发查询。 [#86062](https://github.com/ClickHouse/ClickHouse/pull/86062) ([Daniil Ivanik](https://github.com/divanik)).
* 新增设置 `query_condition_cache_selectivity_threshold`（默认值：1.0），用于将选择性较低的谓词对应的扫描结果排除在写入查询条件缓存之外。这样可以在一定程度上减少查询条件缓存的内存消耗，但会以降低缓存命中率为代价。[#86076](https://github.com/ClickHouse/ClickHouse/pull/86076) ([zhongyuankai](https://github.com/zhongyuankai))。
* 在 `http_handlers` 中支持根据完整 URL 字符串（`full_url` 指令，包含 scheme 和 host:port）进行过滤。[#86155](https://github.com/ClickHouse/ClickHouse/pull/86155) ([Azat Khuzhin](https://github.com/azat)).
* 为 Delta Lake 写入功能新增一个实验性设置项 `allow_experimental_delta_lake_writes`，默认禁用。 [#86180](https://github.com/ClickHouse/ClickHouse/pull/86180) ([Kseniia Sumarokova](https://github.com/kssenii)).
* 修复 init.d 脚本中对 systemd 的检测逻辑（修正 “Install packages” 检查）。 [#86187](https://github.com/ClickHouse/ClickHouse/pull/86187) ([Azat Khuzhin](https://github.com/azat)).
...
* Iceberg 写入支持更多分区类型。此更改关闭了 [#86206](https://github.com/ClickHouse/ClickHouse/issues/86206)。[#86298](https://github.com/ClickHouse/ClickHouse/pull/86298)（[scanhex12](https://github.com/scanhex12)）。
...
* 新增仅为特定通道启用 JSON 日志记录的能力，为此将 `logger.formatting.channel` 设置为 `syslog`、`console`、`errorlog` 或 `log` 之一。[#86331](https://github.com/ClickHouse/ClickHouse/pull/86331)（[Azat Khuzhin](https://github.com/azat)）。
...
* 在 Azure 中为磁盘访问检查（`skip_access_check=0`）增加额外重试次数，因为在开通访问权限时可能会持续相当长的时间。[#86419](https://github.com/ClickHouse/ClickHouse/pull/86419) ([Alexander Tokmakov](https://github.com/tavplubix))。
...
* 为在执行 `iceberg drop` 时删除文件新增设置。此更改关闭了 [#86211](https://github.com/ClickHouse/ClickHouse/issues/86211)。[#86501](https://github.com/ClickHouse/ClickHouse/pull/86501)（[scanhex12](https://github.com/scanhex12)）。
...
* 将 `timeSeries*()` 函数中的陈旧时间窗口调整为左开右闭区间。 [#86588](https://github.com/ClickHouse/ClickHouse/pull/86588) ([Vitaly Baranov](https://github.com/vitlibar)).
...
* 使 S3Queue 在有序模式下的 bucket 锁成为持久模式，类似于在 `use_persistent_processing_nodes = 1` 时的处理节点。在测试中添加 Keeper 故障注入。 [#86628](https://github.com/ClickHouse/ClickHouse/pull/86628) ([Kseniia Sumarokova](https://github.com/kssenii)).
* 修复了通过配置文件添加名称中包含点号的用户时的处理方式。[#86633](https://github.com/ClickHouse/ClickHouse/pull/86633)（[Mikhail Koviazin](https://github.com/mkmkme)）。
...
* 你可以使用 `clickhouse-benchmark --precise` 参数，更精确地统计 QPS 和其他按时间间隔计算的指标。当查询持续时间与报告间隔 `--delay D` 相近时，这有助于获得稳定的 QPS。[#86684](https://github.com/ClickHouse/ClickHouse/pull/86684) ([Sergei Trifonov](https://github.com/serxa)).
* Make Linux 线程的 nice 值可配置，从而可以为某些线程（merge/mutate、查询、materialized view、ZooKeeper 客户端）设置更高或更低的优先级。[#86703](https://github.com/ClickHouse/ClickHouse/pull/86703) ([Miсhael Stetsyuk](https://github.com/mstetsyuk)).
* 修复具有误导性的“specified upload does not exist”错误，该错误会在多部分上传过程中由于竞争条件导致原始异常丢失时出现。[#86725](https://github.com/ClickHouse/ClickHouse/pull/86725) ([Julia Kartseva](https://github.com/jkartseva)).
...
* 新增用于调整挂起信号的能力，以尝试规避 `CANNOT_CREATE_TIMER` 错误（适用于查询分析器：`query_profiler_real_time_period_ns`/`query_profiler_cpu_time_period_ns`）。同时从 `/proc/self/status` 中收集 `SigQ` 以便进行自省/诊断（如果 `ProcessSignalQueueSize` 接近 `ProcessSignalQueueLimit`，则很可能会出现 `CANNOT_CREATE_TIMER` 错误）。 [#86760](https://github.com/ClickHouse/ClickHouse/pull/86760) ([Azat Khuzhin](https://github.com/azat)).
* 支持在数据湖上执行分布式 INSERT/SELECT。[#86783](https://github.com/ClickHouse/ClickHouse/pull/86783) ([scanhex12](https://github.com/scanhex12))。
...
* 将复制去重窗口增加至 10000。[#86820](https://github.com/ClickHouse/ClickHouse/pull/86820) ([Sema Checherinda](https://github.com/CheSema))。
...
* 将 `yesterday()` 函数改为不区分大小写，使其与 `today()` 函数保持一致。[#86914](https://github.com/ClickHouse/ClickHouse/pull/86914) ([Kaviraj Kanagaraj](https://github.com/kavirajk))。
...
* 在一些已知会分配大量（&gt;16MiB）内存的位置（排序、异步插入、FileLog）中遵守内存限制。 [#87035](https://github.com/ClickHouse/ClickHouse/pull/87035) ([Azat Khuzhin](https://github.com/azat)).
* 防止在查询中非布尔类型的 setting 无法生效。对 [#85800](https://github.com/ClickHouse/ClickHouse/issues/85800) 的改进。[#87084](https://github.com/ClickHouse/ClickHouse/pull/87084) ([thraeka](https://github.com/thraeka))。
...
* 如果没有投影时，远程副本会跳过索引分析。 [#87096](https://github.com/ClickHouse/ClickHouse/pull/87096) ([zoomxi](https://github.com/zoomxi)).
* 如果在设置 `network_compression_method` 时使用的值不是受支持的通用编解码器，则抛出异常。 [#87097](https://github.com/ClickHouse/ClickHouse/pull/87097) ([Robert Schulze](https://github.com/rschu1ze)).
...
* 为 `S3` 表引擎和 `s3` 表函数新增一个名为 `storage_class_name` 的参数，用于指定 AWS 支持的 Intelligent-Tiering 存储级别。该参数同时支持键值对格式和位置（已弃用）参数格式。[#87122](https://github.com/ClickHouse/ClickHouse/pull/87122) ([alesapin](https://github.com/alesapin))。
* 允许在 Ytsaurus 表中禁用 UTF-8 编码。[#87150](https://github.com/ClickHouse/ClickHouse/pull/87150) ([MikhailBurdukov](https://github.com/MikhailBurdukov))。
* 为数据湖磁盘新增 Azure 支持。 [#87173](https://github.com/ClickHouse/ClickHouse/pull/87173) ([scanhex12](https://github.com/scanhex12)).
* 新增用于控制字典压缩的 `dictionary_block_frontcoding_compression` 文本索引参数。默认启用，并使用 `front-coding` 压缩。[#87175](https://github.com/ClickHouse/ClickHouse/pull/87175) ([Elmi Ahmadov](https://github.com/ahmadov)).
* 为 parseDateTime 函数启用短路求值。[#87184](https://github.com/ClickHouse/ClickHouse/pull/87184) ([Pavel Kruglov](https://github.com/Avogar)).
* 支持通过 `alter table ... materialize statistics all` 将表的所有统计信息物化。[#87197](https://github.com/ClickHouse/ClickHouse/pull/87197) ([Han Fei](https://github.com/hanfei1991))。
...
* 新增了一个 `system.aggregated_zookeeper_log` 表。该表包含按会话 ID、父路径和操作类型分组的 ZooKeeper 操作的统计信息（例如操作次数、平均延迟、错误次数），并会定期刷新到磁盘。[#87208](https://github.com/ClickHouse/ClickHouse/pull/87208) ([Miсhael Stetsyuk](https://github.com/mstetsyuk)).
...
* 更新了 `clickhouse-benchmark`，现在在其命令行参数中允许使用 `-` 来代替 `_`。 [#87251](https://github.com/ClickHouse/ClickHouse/pull/87251) ([Ahmed Gouda](https://github.com/0xgouda)).
...
* 在系统表 `system.parts_columns` 中新增一列 `statistics`。 [#87259](https://github.com/ClickHouse/ClickHouse/pull/87259) ([Han Fei](https://github.com/hanfei1991)).
* 添加了名为 `inject_random_order_for_select_without_order_by` 的设置项，它会为不包含 `ORDER BY` 子句的顶层 `SELECT` 查询自动注入 `ORDER BY rand()`。 [#87261](https://github.com/ClickHouse/ClickHouse/pull/87261) ([Rui Zhang](https://github.com/zhangruiddn)).
...
* 在根据设置项 `min_insert_block_size_rows_for_materialized_views` 和 `min_insert_block_size_bytes_for_materialized_views` 将来自所有线程的数据先进行合并后，再插入到 materialized view 中。此前，如果启用了 `parallel_view_processing`，每个向特定 materialized view 插入数据的线程都会独立执行合并插入，这可能会导致生成的分区片段数量更多。 [#87280](https://github.com/ClickHouse/ClickHouse/pull/87280) ([Antonio Andelic](https://github.com/antonio2368)).
* 此补丁在写入事务期间新增了检查任意 Keeper 节点 stat 的能力，可帮助检测 ABA 问题。 [#87282](https://github.com/ClickHouse/ClickHouse/pull/87282) ([Mikhail Artemenko](https://github.com/Michicosun)).
...
* 此补丁修复了在任意工作负载下，基于磁盘事务的元数据在回滚 unlink/rename/removeRecursive/removeDirectory 等操作以及维护硬链接计数时出现的问题，并简化了相关接口，使其更加通用，从而可以在其他元数据存储中复用。 [#87358](https://github.com/ClickHouse/ClickHouse/pull/87358) ([Mikhail Artemenko](https://github.com/Michicosun))。
* 新增了 `keeper_server.tcp_nodelay` 配置参数，可用于在 Keeper 中禁用 `TCP_NODELAY`。[#87363](https://github.com/ClickHouse/ClickHouse/pull/87363) (Copilot)。
* 在 `clickhouse-benchmarks` 中支持 `--connection`。其行为与 `clickhouse-client` 中所支持的相同，可以在客户端 `config.xml`/`config.yaml` 配置文件中的 `connections_credentials` 路径下预先定义连接，从而避免在命令行参数中显式指定用户名/密码。同时为 `clickhouse-benchmark` 增加对 `--accept-invalid-certificate` 的支持。[#87370](https://github.com/ClickHouse/ClickHouse/pull/87370) ([Azat Khuzhin](https://github.com/azat)).
* 现在，为 Iceberg 表设置 `max_insert_threads` 将生效。[#87407](https://github.com/ClickHouse/ClickHouse/pull/87407) ([alesapin](https://github.com/alesapin))。
* 将直方图指标和维度型指标添加到 `PrometheusMetricsWriter`。这样，`PrometheusRequestHandler` 处理器就会具备所有关键指标，可用于在云环境中进行可靠、低开销的指标采集。[#87521](https://github.com/ClickHouse/ClickHouse/pull/87521) ([Miсhael Stetsyuk](https://github.com/mstetsyuk))。
...
* 为 `Array` 和 `Map`（`mapKeys` 和 `mapValues`）值添加文本索引支持。支持的函数为 `mapContainsKey` 和 `has`。 [#87602](https://github.com/ClickHouse/ClickHouse/pull/87602) ([Elmi Ahmadov](https://github.com/ahmadov)).
...
* 使用带有备份专用设置（例如 `backup_slow_all_threads_after_retryable_s3_error`）的 S3 存储客户端，用于在服务器端（原生方式）将数据复制到备份目标。将 `s3_slow_all_threads_after_retryable_error` 标记为弃用。 [#87660](https://github.com/ClickHouse/ClickHouse/pull/87660) ([Julia Kartseva](https://github.com/jkartseva)).
* 修复在使用实验性 `make_distributed_plan` 进行查询计划序列化时对设置项 `max_joined_block_size_rows` 和 `max_joined_block_size_bytes` 的不正确处理。 [#87675](https://github.com/ClickHouse/ClickHouse/pull/87675) ([Vladimir Cherkasov](https://github.com/vdimir)).
...
* 新增了一个名为 `keeper_hosts` 的设置项，用于公开 ClickHouse 可连接的 [Zoo]Keeper 主机列表。[#87718](https://github.com/ClickHouse/ClickHouse/pull/87718)（[Nikita Mikhaylov](https://github.com/nikitamikhaylov)）。
* 新增 `ALTER TABLE REWRITE PARTS` —— 使用当前所有设置从头重写表的分区片段（因为某些设置，例如 `use_const_adaptive_granularity`，只会应用于新的分区片段）。 [#87774](https://github.com/ClickHouse/ClickHouse/pull/87774) ([Azat Khuzhin](https://github.com/azat)).
* 在系统仪表板中添加 `from` 和 `to` 参数，以便于进行历史分析和排查。 [#87823](https://github.com/ClickHouse/ClickHouse/pull/87823) ([Mikhail f. Shiryaev](https://github.com/Felixoid)).
...
* 添加新的 `joined_block_split_single_row` 设置，用于在每个键对应大量匹配项的哈希 JOIN 中降低内存占用。该设置允许哈希 JOIN 结果即便在单个左表行的匹配范围内也能被分块，这在当左表的一行匹配右表成千上万甚至数百万行时尤其有用。此前，所有匹配项必须一次性在内存中物化。此改动可以降低峰值内存占用，但可能会增加 CPU 使用。 [#87913](https://github.com/ClickHouse/ClickHouse/pull/87913) ([Vladimir Cherkasov](https://github.com/vdimir))。
...
* 新增支持为 `Keeper` 限制请求大小（`max_request_size` 设置，与 `ZooKeeper` 的 `jute.maxbuffer` 相同，为保持向后兼容默认值为 **OFF**，该默认值将在后续版本中调整）。 [#87952](https://github.com/ClickHouse/ClickHouse/pull/87952) ([Azat Khuzhin](https://github.com/azat)).
* 修复 `clickhouse-benchmark`，使其默认情况下不再在错误消息中包含堆栈跟踪。 [#87954](https://github.com/ClickHouse/ClickHouse/pull/87954) ([Ahmed Gouda](https://github.com/0xgouda)).
...
* Ytsaurus：允许在创建表/表函数/字典时只包含部分列。[#87982](https://github.com/ClickHouse/ClickHouse/pull/87982) ([MikhailBurdukov](https://github.com/MikhailBurdukov))。
...
* 移除用于读取 Arrow/ORC/Parquet 的自定义 MemoryPools。在 [https://github.com/ClickHouse/ClickHouse/pull/84082](https://github.com/ClickHouse/ClickHouse/pull/84082) 之后，该组件似乎已不再需要，因为现在无论如何我们都会跟踪所有内存分配。[#88035](https://github.com/ClickHouse/ClickHouse/pull/88035)（[Nikita Mikhaylov](https://github.com/nikitamikhaylov)）。
...
* Add support to connect to tls 端口 of clickhouse-keeper, kept flag names same as in the clickhouse-client. [#88065](https://github.com/ClickHouse/ClickHouse/pull/88065) ([Pradeep Chhetri](https://github.com/chhetripradeep)).
...
* 为 `generateSerialID` 函数新增可选参数 `start_value`，用于给新序列指定自定义起始值。 [#88085](https://github.com/ClickHouse/ClickHouse/pull/88085) ([Manuel](https://github.com/raimannma)).
* 启用针对 `CREATE/ALTER TABLE` 列默认表达式的验证分析器。 [#88087](https://github.com/ClickHouse/ClickHouse/pull/88087) ([Max Justus Spransy](https://github.com/maxjustus)).
...
* 默认启用全局采样分析器：按 CPU 时间和真实时间每 10 秒收集一次所有线程的堆栈跟踪。[#88209](https://github.com/ClickHouse/ClickHouse/pull/88209) ([Alexander Tokmakov](https://github.com/tavplubix)).
* 修复了在 `Alias` 引擎表上执行 `EXCHANGE TABLES` 操作时的支持逻辑。该引擎现在将目标表存储为数据库名和表名，而不是常量存储 ID，从而可以在表交换后正确解析目标表。[#88233](https://github.com/ClickHouse/ClickHouse/pull/88233) ([Kai Zhu](https://github.com/nauu))。
* 添加 `temporary_files_buffer_size` 设置项，用于控制临时文件写入缓冲区的大小。 * 优化 `scatter` 操作（例如在 Grace 哈希连接中使用）在处理 `LowCardinality` 列时的内存占用。 [#88237](https://github.com/ClickHouse/ClickHouse/pull/88237) ([Vladimir Cherkasov](https://github.com/vdimir)).
* 添加了在并行副本下直接从文本索引读取的支持。优化了从对象存储读取文本索引的性能。 [#88262](https://github.com/ClickHouse/ClickHouse/pull/88262) ([Anton Popov](https://github.com/CurtizJ)).
* 现在，函数 `generateSerialID` 支持将序列名称作为非常量参数。关闭了 [#83750](https://github.com/ClickHouse/ClickHouse/issues/83750)。[#88270](https://github.com/ClickHouse/ClickHouse/pull/88270)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
* Datalakes 用于分布式处理的目录数据库。 [#88273](https://github.com/ClickHouse/ClickHouse/pull/88273) ([scanhex12](https://github.com/scanhex12)).
* 更新 Azure SDK，包含针对在复制和创建容器操作中暴露出的 `Content-Length` 问题的修复。[#88278](https://github.com/ClickHouse/ClickHouse/pull/88278) ([Smita Kulkarni](https://github.com/SmitaRKulkarni))。
...
* 添加配置项 `keeper_server.coordination_settings.check_node_acl_on_remove`。如果启用该配置，在每次删除某个节点之前，将同时验证该节点自身及其父节点的 ACL。否则，仅验证父节点的 ACL。 [#88513](https://github.com/ClickHouse/ClickHouse/pull/88513) ([Antonio Andelic](https://github.com/antonio2368)).
...
* 将 `clickhouse-client` 文件（如查询历史）存储在 [XDG Base Directories](https://specifications.freedesktop.org/basedir-spec/latest/index.html) 规范所描述的位置，而不是直接放在主目录根路径下。如果 `~/.clickhouse-client-history` 已经存在，则仍会继续使用该文件。[#88538](https://github.com/ClickHouse/ClickHouse/pull/88538)（[Konstantin Bogdanov](https://github.com/thevar1able)）。
...
* 为 `hasAny`/`hasAllTokens` 新增接受字符串输入的重载版本。[#88679](https://github.com/ClickHouse/ClickHouse/pull/88679) ([George Larionov](https://github.com/george-larionov)).
* 应用此补丁后，启发式规则 `to_remove_small_parts_at_right` 将会在计算合并区间评分之前执行。在此之前，合并选择器会先选择较宽的合并区间，然后再过滤该合并区间的后缀部分。修复：[#85374](https://github.com/ClickHouse/ClickHouse/issues/85374)。[#88736](https://github.com/ClickHouse/ClickHouse/pull/88736)（[Mikhail Artemenko](https://github.com/Michicosun)）。
...
* 在违反约束时限制异常消息的长度。在之前的版本中，当插入一个非常长的字符串时，可能会生成同样非常长的异常消息，并最终被写入 `query_log` 中。修复了 [#87032](https://github.com/ClickHouse/ClickHouse/issues/87032)。[#88801](https://github.com/ClickHouse/ClickHouse/pull/88801)（[Alexey Milovidov](https://github.com/alexey-milovidov)）。
