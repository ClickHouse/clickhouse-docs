...
* 通过新的设置 `enable_replacing_merge_with_cleanup_for_min_age_to_force_merge`，允许在可配置的超时时间后自动对整个分区执行清理合并。 [#76440](https://github.com/ClickHouse/ClickHouse/pull/76440) ([Christoph Wurm](https://github.com/cwurm))。
...
## Bug 修复（官方稳定版中用户可见的错误行为） {#bug-fix}
...
## 构建/测试/打包方面的改进 {#build-testing-packaging-improvement}



* 内部依赖 LLVM 已从 16 升级至 18。[#66053](https://github.com/ClickHouse/ClickHouse/pull/66053)（[Nikita Mikhaylov](https://github.com/nikitamikhaylov)）。
* 恢复已删除的 nats 集成测试并修复错误。 - 修复了 nats 引擎中的一些竞争条件 - 修复了在连接丢失情况下向 nats 进行流式数据传输时发生的数据丢失问题 - 修复了从 nats 的流式传输结束时接收最后一块数据会卡死的问题 - `nats_max_reconnect` 已被弃用且不再生效，现在会基于 `nats_reconnect_wait` 超时时间持续执行重连。 [#69772](https://github.com/ClickHouse/ClickHouse/pull/69772) ([Dmitry Novikov](https://github.com/dmitry-sles-novikov)).
* 修复 contrib 中 openssl 的 asm 文件无法生成的问题。[#72622](https://github.com/ClickHouse/ClickHouse/pull/72622) ([RinChanNOW](https://github.com/RinChanNOWWW)).
* 提升测试 03210&#95;variant&#95;with&#95;aggregate&#95;function&#95;type 的稳定性。[#74012](https://github.com/ClickHouse/ClickHouse/pull/74012) ([Anton Ivashkin](https://github.com/ianton-ru)).
* 在 ARM 和 Intel 架构的 Mac 上均支持构建 HDFS。[#74244](https://github.com/ClickHouse/ClickHouse/pull/74244) ([Yan Xin](https://github.com/yxheartipp))。
...
* *潜在破坏性变更*：改进默认配置，使其更加严格。当前默认设置已经是安全的。用户必须显式指定选项才能将端口对外暴露。但当 `default` 用户既没有通过 `CLICKHOUSE_PASSWORD` 设置密码，也没有通过 `CLICKHOUSE_USER` 环境变量更改用户名时，为了提供额外一层保护，它应当只能从本地系统访问。[#75259](https://github.com/ClickHouse/ClickHouse/pull/75259)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
* 集成测试对每一批并行运行的测试设定了 1 小时的超时时间。当达到该超时时间时，将终止 `pytest`，可能会缺失部分日志。pytest 内部超时时间设置为 55 分钟，以便在会话中打印结果且不触发外部超时信号。关闭 [#75532](https://github.com/ClickHouse/ClickHouse/issues/75532)。[#75533](https://github.com/ClickHouse/ClickHouse/pull/75533)（[Ilya Yatsishin](https://github.com/qoega)）。
* 将所有与 clickhouse-server 相关的操作封装为函数，并仅在 `entrypoint.sh` 中启动默认二进制文件时执行。在 [#50724](https://github.com/ClickHouse/ClickHouse/issues/50724) 中曾提出这一早已拖延的改进。为 `clickhouse-extract-from-config` 新增了 `--users` 开关，用于从 `users.xml` 中获取值。[#75643](https://github.com/ClickHouse/ClickHouse/pull/75643)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
* 在压力测试中，如果在通过 gdb 收集堆栈跟踪时服务器尚未退出，将增加额外的等待时间，以减少 `Possible deadlock on shutdown (see gdb.log)` 检测的噪声。只有在测试未成功完成时才会引入该延迟。[#75668](https://github.com/ClickHouse/ClickHouse/pull/75668)（[Ilya Yatsishin](https://github.com/qoega)）。
* 恢复已删除的 nats 集成测试并修复错误。 - 修复了 nats 引擎中的一些竞争条件问题 - 修复了在向 nats 流式发送数据时连接丢失情况下的数据丢失问题 - 修复了从 nats 流式读取结束时接收最后一块数据会卡死的问题 - `nats_max_reconnect` 已弃用且不再产生任何效果，重连将持续进行，并使用 `nats_reconnect_wait` 作为重连等待超时时间。 [#75850](https://github.com/ClickHouse/ClickHouse/pull/75850) ([Dmitry Novikov](https://github.com/dmitry-sles-novikov)).
...
* 修复了由于在终止进程组时调用 `sleep` 而导致拆分测试输出异常的问题。[#76090](https://github.com/ClickHouse/ClickHouse/pull/76090)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
* 不要在运行结束时收集 `docker-compose` 日志，因为脚本经常会被终止。应在后台收集这些日志。 [#76140](https://github.com/ClickHouse/ClickHouse/pull/76140) ([Mikhail f. Shiryaev](https://github.com/Felixoid)).
* 将 Kafka 存储相关的测试拆分为多个文件。修复了 [#69452](https://github.com/ClickHouse/ClickHouse/issues/69452)。[#76208](https://github.com/ClickHouse/ClickHouse/pull/76208)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
...
* 从二进制文件中移除约 20 MB 的死代码。 [#76226](https://github.com/ClickHouse/ClickHouse/pull/76226) ([Alexey Milovidov](https://github.com/alexey-milovidov)).
* 由于引入 `block()`，将 CMake 的最低版本要求提升至 3.25。 [#76316](https://github.com/ClickHouse/ClickHouse/pull/76316) ([Konstantin Bogdanov](https://github.com/thevar1able)).
* 将 fmt 更新到 11.1.3。[#76547](https://github.com/ClickHouse/ClickHouse/pull/76547) ([Raúl Marín](https://github.com/Algunenano))。
* 将 `lz4` 更新至 `1.10.0`。[#76571](https://github.com/ClickHouse/ClickHouse/pull/76571)（[Konstantin Bogdanov](https://github.com/thevar1able)）。
...
* 该测试在串行模式下执行超过 10 秒，这对快速测试来说过长。[#76948](https://github.com/ClickHouse/ClickHouse/pull/76948)（[Mikhail f. Shiryaev](https://github.com/Felixoid)）。
...
* 将 `minizip-ng` 更新为 `4.0.9`。 [#78917](https://github.com/ClickHouse/ClickHouse/pull/78917) ([Konstantin Bogdanov](https://github.com/thevar1able))。
