...
作为注册用户,您可以通过 ClickHouse 查看和管理个人账户数据,包括联系信息。根据您的角色,您还可能有权访问组织内其他用户的联系信息、API 密钥详情以及其他相关信息。您可以直接通过 ClickHouse 控制台自助管理这些信息。

**什么是数据主体访问请求 (DSAR)**

根据您所在的地区,适用法律可能赋予您针对 ClickHouse 所持有的您的个人数据的额外权利(数据主体权利),具体说明请参见 ClickHouse 隐私政策。行使数据主体权利的流程称为数据主体访问请求 (DSAR)。

**个人数据范围**

请查阅 ClickHouse 隐私政策,了解 ClickHouse 收集的个人数据详情及其使用方式。
...
以下是 ClickHouse 在账户设置和服务使用期间收集的数据摘要,以及在 ClickHouse 控制台中查看特定个人数据的位置信息。
...
| https://console.clickhouse.cloud/organizations/OrgID/audit   | 活动日志,列出各个用户的操作 | 电子邮件                  |
...
注意:包含 `OrgID` 的 URL 需要更新为您特定账户的 `OrgID`。

### 现有客户 {#current-customers}

如果您拥有我们的账户且自助服务选项未能解决您的个人数据问题,您可以根据隐私政策提交数据主体访问请求。为此,请登录您的 ClickHouse 账户并创建[支持工单](https://console.clickhouse.cloud/support)。这有助于我们验证您的身份并简化处理您请求的流程。
...
如果您没有我们的账户且上述自助服务选项未能解决您的个人数据问题,并且您希望根据隐私政策提交数据主体访问请求,您可以通过电子邮件将这些请求发送至 [privacy@clickhouse.com](mailto:privacy@clickhouse.com)。
