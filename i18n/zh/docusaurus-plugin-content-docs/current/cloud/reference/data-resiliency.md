...
本页面介绍 ClickHouse Cloud 的灾难恢复建议，以及客户从中断事件中恢复的指导。
ClickHouse Cloud 目前不支持自动故障转移或跨多个地理区域的自动同步。

:::tip
客户应定期执行备份恢复测试，以了解其服务规模和配置的具体 RTO。
:::
...
**RPO(恢复点目标)**: 发生中断事件后,以时间衡量的最大可接受数据丢失量。示例:RPO 为 30 分钟意味着在发生故障时,数据库应能恢复到不早于 30 分钟前的数据。这当然取决于备份的执行频率。

**RTO(恢复时间目标)**: 发生中断后,恢复正常运营前的最大允许停机时间。示例:RTO 为 30 分钟意味着在发生故障时,团队能够在 30 分钟内恢复数据和应用程序并恢复正常运营。
...
这些备份位于与服务相同的区域,并存储在 ClickHouse CSP(云服务提供商)存储桶中。
当主服务中的数据发生损坏时,可以使用备份恢复到新服务。
...
备份和恢复时长取决于多个因素,包括数据库大小、模式结构以及表的数量。

在我们的测试中,约 1 TB 的小型备份需要 10-15 分钟或更长时间才能完成。
...
目前不支持在相同或不同区域的不同 ClickHouse Cloud 服务之间自动同步数据,即不支持主-主复制。
:::
...
采用此选项时,您可以在不同区域设置 2 个独立集群并同时向两者写入数据。
此选项会带来更高的成本,因为需要运行多个服务,但可以在某个区域不可用时提供更高的可用性。

2. **利用云服务提供商复制**

采用此选项时,您将利用云服务提供商的原生对象存储复制功能来复制数据。
例如,使用 BYOB 时,您可以将备份导出到主区域中您拥有的存储桶,然后通过 [AWS 跨区域复制](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html)将其复制到另一个区域。
