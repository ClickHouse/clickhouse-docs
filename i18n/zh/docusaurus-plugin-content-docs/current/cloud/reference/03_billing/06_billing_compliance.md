...
但是,由于云市场的要求,我们按小时报告您的按量付费使用费用。您将根据与该云市场的协议条款收到账单 - 通常按自然月计费周期。

例如,如果您在 1 月 18 日创建第一个组织服务,您在 ClickHouse Cloud 的第一个计费使用周期将从 1 月 18 日运行到 2 月 17 日当天结束。但是,您可能会在 2 月初收到来自云市场的第一张账单。

但是,如果您的云市场按量付费订阅被取消或无法自动续订,计费将回退到组织登记的信用卡(如果有)。要添加信用卡,请[联系支持](/about-us/support)寻求帮助。如果未提供有效的信用卡,将应用上述[直接按量付费](#direct-payg)中概述的相同未付账单限制 - 包括服务暂停和最终数据删除。
...
积分从授予当日开始消耗,计费周期基于首次选择组织层级(基础版、规模版或企业版)的日期。
...
在 ClickHouse Cloud 用户界面中转到计费部分,然后点击"添加信用卡"按钮(如下所示)以完成设置。如有任何问题,请[联系支持团队](/about-us/support)寻求帮助。
...
如有任何与云市场计费相关的问题,请直接联系相应的云服务提供商。
