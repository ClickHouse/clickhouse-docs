...
ClickHouse 在组织级别收取服务费用。如果我们无法使用您当前的计费方式处理付款,您必须将其更新为上述三种方式之一以避免服务中断。有关基于所选计费方式的付款合规详情,请参见下文。
...
通常会在月度计费周期结束时对存档的信用卡进行扣款,但如果周期内应付金额达到 10,000 美元,将提前扣款(有关付款阈值的更多信息请参见[此处](/cloud/billing/payment-thresholds))。

存档的信用卡必须有效、未过期,并且有足够的可用额度来支付您的账单总额。如果因任何原因我们无法收取全部应付金额,将立即实施以下未付账单限制:
...
例如,如果您在 1 月 18 日创建第一个组织服务,您在 ClickHouse Cloud 中的第一个计费使用周期将从 1 月 18 日运行到 2 月 17 日当天结束。但是,您可能会在 2 月初收到来自云市场的第一张账单。

但是,如果您的云市场按量付费订阅被取消或自动续订失败,计费将回退到组织存档的信用卡(如果有)。要添加信用卡,请[联系支持](/about-us/support)寻求帮助。如果未提供有效的信用卡,将实施与上述[直接按量付费](#direct-payg)相同的未付账单限制 - 包括服务暂停和最终数据删除。

### 承诺合同计费 {#committed-spend-contract}

您可以通过以下方式为组织购买承诺合同额度:


1. 直接联系销售团队购买积分,支付方式包括 ACH
   或电汇。付款条款将在相应的订单表格中列明。
2. 联系销售团队,通过我们支持的云市场(AWS、GCP 或 Azure)订阅购买积分。
   费用将在您接受私有报价时报告给相应的云市场,此后按照报价条款执行,
   但您将根据与该云市场的协议条款收到账单。要通过云市场付款,请参阅以下
   [说明](#marketplace-payg)。

应用于组织的积分(例如通过承诺合同或退款获得的积分)可在订单表格或已接受的私有报价中
指定的期限内使用。
积分从授予之日起开始消耗,计费周期基于首次选择组织层级(Basic、Scale 或 Enterprise)的日期计算。
...
在任一情况下,如果我们无法对配置的信用卡进行扣款,上述针对使用信用卡的
...
前往 ClickHouse Cloud 用户界面中的"计费"部分,点击"添加信用卡"按钮(如下所示)
以完成设置。如有任何问题,请[联系支持团队](/about-us/support)寻求帮助。
