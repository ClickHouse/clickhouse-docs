...
如果您是按使用量付费 (pay as you go) 客户，并且在某个计费周期内对 ClickHouse Cloud 的应付金额达到 1,000 美元 (或等值金额)，系统会自动向为您的组织配置的付款方式扣款。

:::tip
在第一次成功支付发票后，阈值会提高到 $5,000 美元。第二次成功支付发票后，阈值会提高到 $10,000 美元。如果您希望调整支付阈值金额，请[联系支持](mailto:support@clickhouse.com)。
:::
...
如果您的信用卡不符合 RBI 要求，您必须添加一种符合 RBI 要求的新付款方式，或者在您能够在该卡上设置电子授权之前，继续进行手动付款。

下面的示例展示了 Cloud Console 中的计费界面：

<Image img={billing_1} size="sm" alt="在 UI 中查找计费的位置" />

<Image img={billing_2} size="lg" alt="账单" />

从上述示例可以看到，在 2 月 28 日至 3 月 31 日的计费周期内发送了一张阈值发票，随后在同一计费周期内，又针对超过 1,000 美元的剩余用量发送了另一张发票。

## 支付阈值通知 \{#threshholds-notifications\}

如果你的组织达到支付阈值的 90%，并且预计会在该周期中途超过支付阈值，与该组织关联的计费联系人邮箱将会收到一封电子邮件通知。
