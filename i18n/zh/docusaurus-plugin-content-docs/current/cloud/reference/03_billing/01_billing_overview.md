...
doc_type: 'reference'
keywords: ['ClickHouse Cloud', '定价', '计费', '云成本', '计算资源定价']
---

有关价格信息，请参阅 [ClickHouse Cloud 定价](https://clickhouse.com/pricing#pricing-calculator) 页面。
ClickHouse Cloud 会根据计算、存储、[数据传输](/cloud/manage/network-data-transfer)（通过互联网的出站流量和跨区域传输）以及 [ClickPipes](/integrations/clickpipes) 的使用量进行计费。
要了解可能影响账单的因素，以及管理支出的方法，请继续阅读。
...
### 什么是 ClickHouse 积分（CHC）？ {#what-is-chc}

ClickHouse 积分是客户使用 ClickHouse Cloud 的信用单位,等值于一（1）美元,将根据 ClickHouse 当前公布的价格表进行应用。

:::note
如果您通过 Stripe 进行计费,您将在 Stripe 发票上看到 1 CHC 等于 \$0.01 美元。这是为了在 Stripe 上实现准确计费,因为 Stripe 无法对我们的标准 SKU（1 CHC = \$1 美元）按小数数量进行计费。
:::

### 在哪里可以找到旧版定价？ {#find-legacy-pricing}

旧版定价信息可以在[此处](https://clickhouse.com/pricing?legacy=true)找到。

### 计算资源如何计量？ {#how-is-compute-metered}

ClickHouse Cloud 按分钟计量计算资源,以 8G RAM 为增量单位。
计算成本将根据服务层级、区域和云服务提供商而有所不同。

### 磁盘存储如何计算？ {#how-is-storage-on-disk-calculated}

ClickHouse Cloud 使用云对象存储,使用量按存储在 ClickHouse 表中的数据压缩大小计量。
存储成本在各服务层级中相同,但会根据区域和云服务提供商而有所不同。
...
压缩率因数据集而异。
压缩率的差异取决于数据本身的可压缩性（高基数字段与低基数字段的数量）,
以及用户如何设置模式（例如是否使用可选编解码器）。
对于常见类型的分析数据,压缩率可以达到 10 倍左右,但也可能显著更低或更高。
请参阅[优化文档](/optimize/asynchronous-inserts)获取指导,以及这篇 [Uber 博客](https://www.uber.com/blog/logging/)了解详细的日志用例示例。
准确了解压缩率的唯一实用方法是将您的数据集导入 ClickHouse,然后比较数据集的大小与存储在 ClickHouse 中的大小。
...
- 直销年付/多年付（通过预付"ClickHouse 积分",以美元计价,提供额外的支付选项）。
- 通过 AWS、GCP 和 Azure 市场（按需付费（PAYG）或通过市场与 ClickHouse Cloud 签订合同）。

:::note
按需付费的 ClickHouse Cloud 积分以 \$0.01 为单位开具发票,使我们能够根据客户的使用情况对部分 ClickHouse 积分进行收费。这与承诺消费的 ClickHouse 积分不同,后者以完整的 \$1 为单位预先购买。
:::
...
### ClickHouse Cloud 为 Scale 和 Enterprise 服务提供哪些成本管理控制功能？ {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-scale-and-enterprise-services}

- Trial 和 Annual Commit 客户在消耗达到特定阈值时会自动收到电子邮件通知：`50%`、`75%` 和 `90%`。这使用户能够主动管理其使用量。
...
### ClickHouse Cloud 为 Basic 服务提供哪些成本管理控制功能？ {#what-controls-does-clickhouse-cloud-offer-to-manage-costs-for-basic-services}

- [高级扩展控制](/manage/scaling)允许您控制非活动期间的暂停/空闲行为。Basic 服务不支持调整内存分配。
- 请注意，默认设置会在一段时间不活动后暂停服务。

### 如果我有多个服务，我会收到每个服务的单独发票还是合并发票？ {#if-i-have-multiple-services-do-i-get-an-invoice-per-service-or-a-consolidated-invoice}
...
ClickHouse Cloud 控制台提供了使用量显示功能，详细列出每个服务的使用情况。此细分按使用维度组织，帮助您了解与每个计量单位相关的成本。

### 我如何访问 ClickHouse Cloud 服务订阅的发票？ {#how-do-i-access-my-invoice-for-my-subscription-to-the-clickhouse-cloud-service}

对于使用信用卡的直接订阅：

要查看您的发票，请从 ClickHouse Cloud UI 左侧导航栏中选择您的组织，然后转到 Billing。您的所有发票将列在 Invoices 部分下。
...
如果这些日期不同，使用量和发票日期将会有所差异。由于使用量报表按天跟踪给定服务的使用情况，用户可以依靠报表查看成本明细。
...
预付额度未涵盖的任何超额使用必须通过信用卡支付或市场月度计费来支付。
...
无论哪种情况,您对 ClickHouse Cloud 的使用都以 ClickHouse Cloud 积分(CHC)进行跟踪,
计量方式相同并据此计费。
...
如果选择共享,这两个服务将形成一个[仓库](/cloud/reference/warehouses)。
仓库中存储数据,多个计算服务可以访问这些数据。

由于数据仅存储一次,您只需为一份数据副本付费,即使有多个服务在访问它。
您照常为计算付费——计算-计算分离/仓库不收取额外费用。
通过在此部署中利用共享存储,用户可以在存储和备份方面节省成本。

在某些情况下,计算-计算分离可以为您节省大量 ClickHouse 积分。
以下设置是一个很好的例子:
...
然而,在仓库中组合两个服务并将其中一个设置为空闲状态可以帮助您节省费用。
