...
title: '用于 PostgreSQL CDC 的 ClickPipes'
...
1. **数据摄入量**：从 Postgres 传入并摄入到 ClickHouse 的原始未压缩字节数。
...
此维度涵盖每个服务专门为 Postgres ClickPipes 配置的计算单元。计算资源在服务内的所有 Postgres 管道之间共享。**计算资源在创建第一个 Postgres 管道时配置，当不再有 Postgres CDC 管道时释放**。配置的计算资源量取决于您组织的层级：
...
默认情况下,计算资源扩展不可由用户配置。预配置的资源已经过优化,可以最佳地处理大多数客户工作负载。如果您的使用场景需要更多或更少的计算资源,请提交支持工单,以便我们评估您的请求。
