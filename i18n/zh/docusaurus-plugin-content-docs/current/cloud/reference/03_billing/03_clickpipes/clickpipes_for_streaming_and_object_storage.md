---
sidebar_label: '流式处理与对象存储'
slug: /cloud/reference/billing/clickpipes/streaming-and-object-storage
title: '用于流式处理与对象存储的 ClickPipes'
description: '用于流式处理与对象存储的 ClickPipes 计费概览'
doc_type: 'reference'
keywords: ['计费', 'clickpipes', '流式处理定价', '成本', '价格']
---
...
本节介绍 ClickPipes 针对流式传输和对象存储的定价模型。
...
- **计算资源**：按**每单位每小时**计费。
  计算资源表示运行 ClickPipes 副本 Pod 的成本，无论其是否正在主动摄取数据。
  此项适用于所有 ClickPipes 类型。
- **摄取数据**：按**每 GB** 计费。
  摄取数据费率适用于所有流式 ClickPipes
  (Kafka、Confluent、Amazon MSK、Amazon Kinesis、Redpanda、WarpStream、Azure Event Hubs)
  通过副本 Pod 传输的数据。摄取数据大小 (GB) 按从源接收的字节数（未压缩或已压缩）计费。


## 什么是 ClickPipes 副本？ {#what-are-clickpipes-replicas}

ClickPipes 通过专用基础设施从远程数据源中采集数据，该基础设施独立于 ClickHouse Cloud 服务运行和扩展。因此，它使用专用计算副本。


## 默认的副本数量及其大小是多少？ {#what-is-the-default-number-of-replicas-and-their-size}

每个 ClickPipe 默认配置为 1 个副本,提供 512 MiB 内存和 0.125 vCPU (XS)。
...
## 流式传输和对象存储 ClickPipes 常见问题 {#faq-streaming-and-object-storage}
