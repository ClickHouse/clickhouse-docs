...
您可以通过 AWS、GCP 和 Azure 的 Marketplace 订阅 ClickHouse Cloud。这样，您即可通过现有云服务商的账单支付 ClickHouse Cloud 的费用。

您可以选择按量付费（PAYG），或者通过 Marketplace 与 ClickHouse Cloud 签订承诺合约。计费将由云服务商处理，您会收到一张涵盖所有云服务的统一账单。

* [AWS Marketplace 按量付费（PAYG）](/cloud/billing/marketplace/aws-marketplace-payg)
* [AWS Marketplace 承诺合约](/cloud/billing/marketplace/aws-marketplace-committed-contract)
* [GCP Marketplace 按量付费（PAYG）](/cloud/billing/marketplace/gcp-marketplace-payg)
* [GCP Marketplace 承诺合约](/cloud/billing/marketplace/gcp-marketplace-committed-contract)
* [Azure Marketplace 按量付费（PAYG）](/cloud/billing/marketplace/azure-marketplace-payg)
...
在 ClickHouse Cloud 控制台中,导航至**计费**页面。您应该能在**付款详情**部分看到云市场名称和相关链接。

### 我是现有的 ClickHouse Cloud 用户。通过 AWS / GCP / Azure 云市场订阅 ClickHouse Cloud 时会发生什么?​ {#i-am-an-existing-clickhouse-cloud-user-what-happens-when-i-subscribe-to-clickhouse-cloud-via-aws--gcp--azure-marketplace}

从云提供商市场注册 ClickHouse Cloud 需要两个步骤:

1. 首先在云提供商的市场门户上"订阅" ClickHouse Cloud。完成订阅后,点击"立即付款"或"在提供商处管理"(具体取决于所用市场)。这将把您重定向到 ClickHouse Cloud。
2. 在 ClickHouse Cloud 上,您可以注册新账户或使用现有账户登录。无论采用哪种方式,系统都会为您创建一个新的 ClickHouse Cloud 组织,该组织将关联到您的云市场计费。

注意:您之前注册 ClickHouse Cloud 时创建的现有服务和组织将保留,且不会连接到云市场计费。ClickHouse Cloud 允许您使用同一账户管理多个组织,每个组织可以采用不同的计费方式。

您可以从 ClickHouse Cloud 控制台左下角的菜单切换组织。

### 我是现有的 ClickHouse Cloud 用户。如果希望现有服务通过云市场计费,应该怎么做?​ {#i-am-an-existing-clickhouse-cloud-user-what-should-i-do-if-i-want-my-existing-services-to-be-billed-via-marketplace}

您需要通过云提供商市场订阅 ClickHouse Cloud。在市场上完成订阅并重定向到 ClickHouse Cloud 后,您可以选择将现有的 ClickHouse Cloud 组织关联到云市场计费。从那时起,您的现有资源将通过云市场进行计费。
...
您可以从组织的计费页面确认计费已成功关联到云市场。如果遇到任何问题,请联系 [ClickHouse Cloud 支持](https://clickhouse.com/support/program)。

:::note
您之前注册 ClickHouse Cloud 时创建的现有服务和组织将保留,且不会连接到云市场计费。
:::

### 我作为云市场用户订阅了 ClickHouse Cloud。如何取消订阅?​ {#i-subscribed-to-clickhouse-cloud-as-a-marketplace-user-how-can-i-unsubscribe}

请注意,您可以直接停止使用 ClickHouse Cloud 并删除所有现有的 ClickHouse Cloud 服务。即使订阅仍然有效,您也不会产生任何费用,因为 ClickHouse Cloud 没有定期费用。

如果您想取消订阅,请导航到云提供商控制台并在那里取消订阅续订。订阅结束后,所有现有服务将被停止,系统将提示您添加信用卡。如果未添加信用卡,两周后所有现有服务将被删除。

### 我作为云市场用户订阅了 ClickHouse Cloud,然后取消了订阅。现在想重新订阅,流程是什么?​ {#i-subscribed-to-clickhouse-cloud-as-a-marketplace-user-and-then-unsubscribed-now-i-want-to-subscribe-back-what-is-the-process}

在这种情况下,请按照常规流程订阅 ClickHouse Cloud(请参阅通过云市场订阅 ClickHouse Cloud 的相关章节)。
...
### 为什么使用情况报表上的日期与我的云市场发票不匹配?​ {#why-do-the-dates-on-the-usage-statements-not-match-my-marketplace-invoice}

云市场计费遵循自然月周期。例如,对于 12 月 1 日至 1 月 1 日之间的使用情况,发票将在 1 月 3 日至 1 月 5 日之间生成。
