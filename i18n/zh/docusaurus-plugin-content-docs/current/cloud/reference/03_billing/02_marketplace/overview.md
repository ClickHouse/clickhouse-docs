...
您可以通过 AWS、GCP 和 Azure 的 Marketplace 订阅 ClickHouse Cloud。这样，您就可以通过现有云服务商的账单来支付 ClickHouse Cloud 的费用。

您可以在 Marketplace 中选择按需付费（PAYG），或通过 Marketplace 与 ClickHouse Cloud 签订承诺合约。账单将由云服务商统一处理，您会收到一份涵盖所有云服务的综合账单。
...
### 我是现有的 ClickHouse Cloud 用户。通过 AWS / GCP / Azure 云市场订阅 ClickHouse Cloud 会发生什么？​ {#i-am-an-existing-clickhouse-cloud-user-what-happens-when-i-subscribe-to-clickhouse-cloud-via-aws--gcp--azure-marketplace}
...
注意:您之前注册 ClickHouse Cloud 时创建的现有服务和组织将保留,它们不会连接到云市场计费。ClickHouse Cloud 允许您使用同一账户管理多个组织,每个组织可以采用不同的计费方式。
...
您需要通过云提供商市场订阅 ClickHouse Cloud。在市场上完成订阅并重定向到 ClickHouse Cloud 后,您将可以选择将现有的 ClickHouse Cloud 组织关联到云市场计费。从那时起,您的现有资源将通过云市场进行计费。
...
如果您想取消订阅,请前往云提供商控制台并在那里取消订阅续订。订阅结束后,所有现有服务将被停止,系统会提示您添加信用卡。如果未添加信用卡,两周后所有现有服务将被删除。
...
在这种情况下,请按照常规流程订阅 ClickHouse Cloud(参见通过云市场订阅 ClickHouse Cloud 的相关章节)。
...
ClickHouse Cloud 使用报表采用不同的计费周期,从注册之日起以 30 天为周期计量和报告使用情况。

如果使用日期和发票日期不一致,两者将会有所差异。由于使用报表按天跟踪特定服务的使用情况,用户可以通过报表查看成本明细。
...
通过市场计费与直接注册 ClickHouse 的定价没有差异。无论哪种方式,您对 ClickHouse Cloud 的使用都以 ClickHouse Cloud Credits (CHCs) 进行跟踪,计量方式相同并据此计费。
...
如果您的市场承诺消费合同处于有效状态且您的额度已用完,我们将自动将您的组织转为按量付费(PAYG)计费。但是,当现有合同到期时,您需要将新的市场合同关联到您的组织,或将您的组织转为通过信用卡直接计费。
