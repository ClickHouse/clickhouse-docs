...
<SettingsInfoBlock type="UInt64" default_value="10" />用于异步从远程 Iceberg 目录获取最新元数据的线程池大小；所有活跃表共享该线程池。
...
<SettingsInfoBlock type="UInt64" default_value="0" />可同时保留的轻量级快照最大数量。0 表示不受限制。如果快照数量达到该限制，尝试创建新快照时会抛出异常。
