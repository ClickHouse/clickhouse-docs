...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="是" />在发生 LOGICAL&#95;ERROR 异常时让服务器崩溃。仅供专家使用。
...
<SettingsInfoBlock type="GroupArrayActionWhenLimitReached" default_value="throw" changeable_without_restart="No" />当 groupArray 中的数组元素数量达到上限时要执行的操作：抛出 `throw` 异常，或丢弃 (`discard`) 多余的值

## aggregate_function_group_array_max_element_size \{#aggregate_function_group_array_max_element_size\}

<SettingsInfoBlock type="UInt64" default_value="16777215" changeable_without_restart="No" />groupArray 函数的最大数组元素大小 (字节) 。该限制在序列化时检查，有助于避免状态大小过大。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />`enable_webterminal` 先前的 (Experimental) 名称。在未设置 `enable_webterminal` 时，出于向后兼容性仍会被识别。该设置已弃用。

## allow_feature_tier \{#allow_feature_tier\}

<SettingsInfoBlock type="UInt32" default_value="0" changeable_without_restart="是" />
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用或禁用 IMPERSONATE (EXECUTE AS target&#95;user) 功能。该设置已被弃用。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />允许使用 jemalloc 进行内存分配。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="否" />如果为 true，则在优雅关闭时会刷新异步插入队列

## async_insert_threads \{#async_insert_threads\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="No" />后台用于实际解析和插入数据的最大线程数。0 表示禁用异步插入模式
...
系统表的异步加载。如果 `system` 数据库中包含大量日志表和 parts，此设置会非常有用。该设置独立于 `async_load_databases`。
...
<SettingsInfoBlock type="UInt32" default_value="120" changeable_without_restart="No" />更新高开销异步指标的时间间隔 (秒) 。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用对开销较大的异步指标的计算。

## asynchronous_metrics_keeper_metrics_only \{#asynchronous_metrics_keeper_metrics_only\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />仅让异步指标计算 Keeper 相关指标。

## asynchronous_metrics_update_period_s \{#asynchronous_metrics_update_period_s\}

<SettingsInfoBlock type="UInt32" default_value="1" changeable_without_restart="No" />用于更新异步指标的时间周期 (以秒为单位) 。
...
<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="仅可增加" />用于在后台对 [Buffer 引擎表](/engines/table-engines/special/buffer) 执行刷新操作的最大线程数。

## background_common_pool_size \{#background_common_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="No" />用于在后台对 [*MergeTree 引擎表](/engines/table-engines/mergetree-family) 表执行各种操作 (主要是垃圾回收) 的最大线程数。
...
<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="否" />在后台从其他副本获取 [*MergeTree 引擎表](/engines/table-engines/mergetree-family) 表的数据 parts 时可使用的最大线程数。

## background_merges_mutations_concurrency_ratio \{#background_merges_mutations_concurrency_ratio\}

<SettingsInfoBlock type="Float" default_value="2" changeable_without_restart="No" />

设置可并发执行的后台合并与变更 (merges 和 mutation) 数量与线程数之间的比值。
...
用于对后台合并 (merge) 和 mutation 操作 (mutation) 进行调度的策略。可选值为：`round_robin` 和 `shortest_task_first`。

用于从后台线程池中选择下一个要执行的合并或 mutation 的算法。该策略可以在运行时更改，无需重启服务器。
可以通过 `default` profile 应用，以保持向后兼容性。

可选值：

* `round_robin` — 所有并发合并和 mutation 按轮询顺序执行，以确保不会出现“饥饿”情况。较小的合并由于需要合并的数据块更少，通常会比较大的合并更快完成。
* `shortest_task_first` — 始终优先执行较小的合并或 mutation。合并和 mutation 会根据其结果大小被分配优先级。结果较小的合并会被严格优先于较大的合并。该策略能够以尽可能快的速度完成小 parts 的合并，但在被大量 `INSERT` 操作严重压载的分区中，可能会导致大型合并长期处于饥饿状态。

## background_message_broker_schedule_pool_size \{#background_message_broker_schedule_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="仅可增加" />用于执行消息流后台操作所能使用的最大线程数。

## background_move_pool_size \{#background_move_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="No" />在后台将数据 parts 移动到其他磁盘或卷时可使用的最大线程数，适用于 *MergeTree 引擎表*。

## background_pool_size \{#background_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="No" />

设置用于对 MergeTree 引擎表执行后台合并和 mutation 操作的线程数量。
...
<SettingsInfoBlock type="Float" default_value="0.8" changeable_without_restart="否" />线程池中可用于同时执行同一类型任务的线程的最大比例。

## background_schedule_pool_size \{#background_schedule_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="512" changeable_without_restart="仅可增加" />用于持续执行副本表、Kafka 流式处理以及 DNS 缓存更新等轻量级周期性操作时所能使用的最大线程数。
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="16" changeable_without_restart="No" />执行 `BACKUP` 请求时使用的最大线程数。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />设置缓存大小与 RAM 最大值的比例。可在低内存系统上减少缓存占用。

## cannot_allocate_thread_fault_injection_probability \{#cannot_allocate_thread_fault_injection_probability\}

<SettingsInfoBlock type="Double" default_value="0" changeable_without_restart="No" />用于测试。
...
<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />设置[已编译表达式](../../operations/caches.md)缓存的大小 (以元素个数计) 。

## compiled_expression_cache_size \{#compiled_expression_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="134217728" changeable_without_restart="No" />设置用于[编译表达式](../../operations/caches.md)的缓存大小 (单位：字节) 。
...
<SettingsInfoBlock type="String" default_value="max_min_fair" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="2000" changeable_without_restart="是" />
...
* 当为 `true` (默认) 时，核算基于实际消耗的 CPU 时间。会为相互竞争的工作负载分配公平的 CPU 时间。槽位 (槽位) 会在有限时间内被分配，到期后需要重新请求。在 CPU 资源过载的情况下，请求 槽位 可能会阻塞线程执行，即可能发生抢占，从而确保 CPU 时间使用的公平性。
* 当为 `false` 时，核算基于分配的 CPU 槽位 数量。会为相互竞争的工作负载分配公平数量的 CPU 槽位。线程启动时分配一个 槽位，在线程执行期间持续持有，并在线程结束执行时释放。为查询执行分配的线程数量只能从 1 增加到 `max_threads`，且不会减少。对此长时间运行的查询更有利，但可能导致短查询出现 CPU 资源饥饿。
...
<SettingsInfoBlock type="UInt64" default_value="480" changeable_without_restart="否" />
...
<SettingsInfoBlock type="UInt64" default_value="5" changeable_without_restart="No" />如果删除表失败，ClickHouse 会在等待该超时时间后再重试此操作。

## database_catalog_drop_table_concurrency \{#database_catalog_drop_table_concurrency\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="否" />用于执行表删除操作的线程池大小。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />允许在 Replicated 数据库中永久地分离表

## database_replicated_drop_broken_tables \{#database_replicated_drop_broken_tables\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />从 Replicated 数据库中删除意外出现的表，而不是将它们移动到单独的本地数据库中
...
<SettingsInfoBlock type="String" default_value="default" changeable_without_restart="No" />默认数据库名。
...
<SettingsInfoBlock type="String" default_value="{replica}" changeable_without_restart="否" />
...
<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />对启用了 `background_reconnect` 的、连接失败的 MySQL 和 Postgres 字典进行重新连接尝试的时间间隔 (以毫秒为单位) 。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />禁用内部 DNS 缓存。建议在基础设施经常变动的系统 (例如 Kubernetes) 中运行 ClickHouse 时使用。
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="否" />当在创建时达到此限制时会抛出异常。将其设置为 0 可关闭硬限制。该限制适用于磁盘连接数。

## disk_connections_rcvbuf \{#disk_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />磁盘 (S3、Azure、GCS) 连接的 `SO_RCVBUF` 选项大小。如果设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 仅会对新创建的连接恢复自动调优；现有池化连接会保留固定的缓冲区大小，直到重新创建为止。

## disk_connections_sndbuf \{#disk_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />用于磁盘 (S3、Azure、GCS) 连接的 SO&#95;SNDBUF 选项大小。如果设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，只有新创建的连接才会恢复自动调优；现有的池化连接仍会保留固定的缓冲区大小，直到重新创建为止。

## disk_connections_soft_limit \{#disk_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="No" />超出该限制的连接的存活时间会显著缩短。该限制适用于磁盘连接。

## disk_connections_store_limit \{#disk_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="否" />超过此限制的连接在使用后会被重置。将该值设置为 0 可关闭连接缓存。该限制适用于磁盘连接。

## disk_connections_warn_limit \{#disk_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="8000" changeable_without_restart="No" />如果正在使用的连接数超过此限制，将在日志中记录警告信息。该限制适用于磁盘连接。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />缓存服务器是否应采用从客户端接收的限流设置。

## distributed_cache_keep_up_free_connections_ratio \{#distributed_cache_keep_up_free_connections_ratio\}

<SettingsInfoBlock type="Float" default_value="0.1" changeable_without_restart="No" />分布式缓存尝试保持的空闲连接数量的软限制。当空闲连接数低于 distributed&#95;cache&#95;keep&#95;up&#95;free&#95;connections&#95;ratio * max&#95;connections 时，将按最近活动时间最早的顺序关闭连接，直到空闲连接数重新高于该限制。
...
<SettingsInfoBlock type="UInt64" default_value="60" changeable_without_restart="否" />在接收到新节点事件后，如果距离上一次清理已有至少 `<cleanup_delay_period>` 秒，则开始执行清理。
...
<SettingsInfoBlock type="String" default_value="/clickhouse/task_queue/ddl/" changeable_without_restart="No" />在 Keeper 中用于 DDL 查询的 `<task_queue>` 路径

## distributed_ddl.pool_size \{#distributed_ddl.pool_size\}

<SettingsInfoBlock type="Int32" default_value="1" changeable_without_restart="否" />可同时运行的 `<ON CLUSTER>` 查询数量
...
<SettingsInfoBlock type="UInt64" default_value="604800" changeable_without_restart="No" />当节点存在时间超过该值时将其删除。

## distributed_ddl_use_initial_user_and_roles \{#distributed_ddl_use_initial_user_and_roles\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />如果启用，ON CLUSTER 查询在远程分片上执行时将保留并使用发起该查询的用户及其角色。这可以确保整个集群中访问控制的一致性，但要求该用户和角色在所有节点上都已创建。

## dns_allow_resolve_names_to_ipv4 \{#dns_allow_resolve_names_to_ipv4\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="是" />允许将主机名解析为 IPv4 地址。

## dns_allow_resolve_names_to_ipv6 \{#dns_allow_resolve_names_to_ipv6\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="是" />允许将主机名解析为 IPv6 地址。

## dns_cache_max_entries \{#dns_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />内部 DNS 缓存的最大记录数。

## dns_cache_update_period \{#dns_cache_update_period\}

<SettingsInfoBlock type="Int32" default_value="15" changeable_without_restart="No" />内部 DNS 缓存的更新周期 (秒) 。
...
<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="否" />用于清理分布式缓存的线程池大小。

## drop_distributed_cache_queue_size \{#drop_distributed_cache_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="否" />用于清理分布式缓存的线程池的队列大小。

## enable_azure_sdk_logging \{#enable_azure_sdk_logging\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用 Azure SDK 的日志记录功能

## enable_webterminal \{#enable_webterminal\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />启用位于 `/webterminal` HTTP 端点的 Web 终端界面。它通过 WebSocket 在浏览器中提供交互式 `clickhouse-client` 会话。当为 `false` 时，对 `/webterminal` 的请求将返回 HTTP 状态 `403 Forbidden`。
...
<SettingsInfoBlock type="UInt64" default_value="10000000000" changeable_without_restart="No" />全局 profiler 的 CPU 时钟定时周期 (单位：纳秒) 。将该值设置为 0 可关闭全局 CPU 时钟 profiler。推荐取值：针对单个查询至少为 10000000 (每秒 100 次) ，针对集群范围的 profiling 至少为 1000000000 (每秒 1 次) 。

## global_profiler_real_time_period_ns \{#global_profiler_real_time_period_ns\}

<SettingsInfoBlock type="UInt64" default_value="10000000000" changeable_without_restart="No" />全局 profiler 的实时时钟计时周期 (单位：纳秒) 。将该值设为 0 可关闭实时时钟全局 profiler。对于单个查询，推荐至少设置为 10000000 (每秒 100 次) ；对于集群级 profiling，推荐设置为 1000000000 (每秒一次) 。
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="No" />当达到此限制时，尝试创建时会抛出异常。将该值设置为 0 可关闭硬限制。该限制适用于不属于任何磁盘或存储的 HTTP 连接。

## http_connections_rcvbuf \{#http_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />通用 HTTP 连接的 SO&#95;RCVBUF 选项大小。如果设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 只会为新创建的连接恢复自动调优；现有连接池中的连接会保留固定的缓冲区大小，直到重新创建为止。

## http_connections_sndbuf \{#http_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />通用 HTTP 连接的 SO&#95;SNDBUF 选项大小。若设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，只有新创建的连接才会恢复自动调优；现有连接池中的连接会继续保留固定的缓冲区大小，直到重新创建。

## http_connections_soft_limit \{#http_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="No" />超过此限制的连接其存活时间会显著缩短。该限制适用于不隶属于任何磁盘或存储的 HTTP 连接。

## http_connections_store_limit \{#http_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="否" />超出该限制的连接在使用后将被重置。设置为 0 可关闭连接缓存功能。该限制适用于不属于任何磁盘或存储的 HTTP 连接。

## http_connections_warn_limit \{#http_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />如果在用连接数高于此限制，将向日志写入警告消息。该限制适用于不属于任何磁盘或存储的 HTTP 连接。
...
<SettingsInfoBlock type="UInt64" default_value="10" changeable_without_restart="否" />用于异步从远程 Iceberg 目录获取最新元数据的线程池大小；所有活跃表共享该线程池。

## iceberg_catalog_threadpool_pool_size \{#iceberg_catalog_threadpool_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="50" changeable_without_restart="No" />Iceberg 目录 后台线程池的大小
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />二级索引标记缓存策略的名称。

## index_mark_cache_prewarm_ratio \{#index_mark_cache_prewarm_ratio\}

<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="否" />预热期间需要填充的索引标记缓存总大小比例。
...
<SettingsInfoBlock type="Double" default_value="0.3" changeable_without_restart="No" />在二级索引标记缓存中，受保护队列的大小占该缓存总大小的比例 (在使用 SLRU 策略时) 。

## index_uncompressed_cache_policy \{#index_uncompressed_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />二级索引未压缩缓存的策略名称。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />在使用 SLRU 策略时，二级索引未压缩缓存中受保护队列的大小占缓存总大小的比例。
...
值 `old_separate_hashes` 表示 ClickHouse 会为同步和异步插入使用不同的去重哈希 (行为与之前相同) 。
如果没有计划开始迁移，应将此值作为默认值使用。
...
在未来的 ClickHouse 版本中，默认值将首先设置为 `compatible_double_hashes`，随后再切换为 `new_unified_hash`，以便分两步安全地完成迁移。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="否" />启用 jemalloc 后台线程。jemalloc 使用后台线程来清理未使用的内存页。禁用该功能可能会导致性能下降。

## jemalloc_enable_global_profiler \{#jemalloc_enable_global_profiler\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />为所有线程启用 jemalloc 的分配分析器。jemalloc 将对分配进行采样，并对已采样分配的所有释放操作进行采样。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />当全局峰值内存使用量在上一次刷新基础上增加了 jemalloc&#95;flush&#95;profile&#95;interval&#95;bytes 后，将刷新 jemalloc profile
...
<SettingsInfoBlock type="UInt64" default_value="300" changeable_without_restart="No" />Keeper 套接字接收超时。

## keeper_server.socket_send_timeout_sec \{#keeper_server.socket_send_timeout_sec\}

<SettingsInfoBlock type="UInt64" default_value="300" changeable_without_restart="No" />Keeper 套接字发送超时时间 (秒) 。
...
<SettingsInfoBlock type="UInt32" default_value="4096" changeable_without_restart="否" />
...
<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />可加入预取线程池队列的最大任务数
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />当为 `<true>` (默认) 时，日志记录将以异步方式执行 (每个输出通道对应一个后台线程) 。否则，将在调用 LOG 的线程中执行日志记录。

## logger.async_queye_max_size \{#logger.async_queye_max_size\}

<SettingsInfoBlock type="UInt64" default_value="65536" changeable_without_restart="否" />使用异步日志时，队列中可保留、等待刷新到存储的日志消息的最大数量。超出的消息将被丢弃。

## logger.console \{#logger.console\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用将日志输出到控制台。将其设置为 `<1>` 或 `<true>` 以启用。如果 ClickHouse 不以守护进程模式运行，则默认值为 `<1>`，否则为 `<0>`。

## logger.console_log_level \{#logger.console_log_level\}

<SettingsInfoBlock type="String" default_value="trace" changeable_without_restart="否" />控制台日志输出级别。默认为 `<level>`。

## logger.count \{#logger.count\}

<SettingsInfoBlock type="UInt64" default_value="1" changeable_without_restart="否" />轮转策略：最多保留多少个 ClickHouse 历史日志文件。
...
<SettingsInfoBlock type="String" default_value="json" changeable_without_restart="No" />控制台输出的日志格式。目前仅支持 `<json>` 格式。
...
<SettingsInfoBlock type="String" default_value="100M" changeable_without_restart="No" />轮转策略：用于控制何时对日志文件进行轮转。轮转可以基于大小、时间，或两者的组合。例如：100M、daily、100M,daily。当日志文件超过指定大小或达到指定时间间隔时，会被重命名并归档，并创建一个新的日志文件。
...
<SettingsInfoBlock type="String" default_value="100M" changeable_without_restart="否" />轮转策略：日志文件的最大大小，以字节为单位。超过该阈值后，日志文件会被重命名并归档，并创建一个新的日志文件。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />使用 LZ4 压缩日志消息。将此参数设置为 `<1>` 或 `<true>` 以启用。

## logger.syslog_level \{#logger.syslog_level\}

<SettingsInfoBlock type="String" default_value="trace" changeable_without_restart="否" />记录到 syslog 的日志级别。

## logger.use_syslog \{#logger.use_syslog\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="否" />还会将日志输出转发到 syslog。
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />标记缓存策略名称。

## mark_cache_prewarm_ratio \{#mark_cache_prewarm_ratio\}

<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="No" />在预热期间要填充的标记缓存总大小的比例。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="否" />在标记缓存中 (在使用 SLRU 策略时) 受保护队列的大小，占该缓存总大小的比例。

## max_active_parts_loading_thread_pool_size \{#max_active_parts_loading_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="64" changeable_without_restart="是" />用于在启动时加载活动分区片段 (Active) 的线程数。

## max_authentication_methods_per_user \{#max_authentication_methods_per_user\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="否" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />服务器上所有备份操作的最大读取速度 (以字节/秒为单位) 。0 表示不限制。

## max_backups_io_thread_pool_free_size \{#max_backups_io_thread_pool_free_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />如果备份 IO 线程池中**空闲**线程的数量超过 `max_backup_io_thread_pool_free_size`，ClickHouse 会释放这些空闲线程占用的资源，并缩减线程池大小。必要时会重新创建线程。

## max_backups_io_thread_pool_size \{#max_backups_io_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="是" />ClickHouse 使用 Backups IO 线程池中的线程来进行 S3 备份的 IO 操作。`max_backups_io_thread_pool_size` 用于限制该线程池中的最大线程数。
...
<SettingsInfoBlock type="Int32" default_value="4096" changeable_without_restart="No" />服务器允许的最大连接数。

## max_database_num_to_throw \{#max_database_num_to_throw\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />如果数据库数量大于该值，服务器将抛出异常。0 表示无限制。
...
<SettingsInfoBlock type="UInt32" default_value="1" changeable_without_restart="No" />在 DatabaseReplicated 中用于在副本恢复期间创建表的线程数量。当设置为 0 时，线程数量等于 CPU 核心数。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />服务器上从分布式缓存读取数据的总最大速度 (字节/秒) 。0 表示无限制。

## max_distributed_cache_write_bandwidth_for_server \{#max_distributed_cache_write_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />服务器上分布式缓存的最大总写入带宽，单位为字节/秒。0 表示不限制。

## max_entries_for_hash_table_stats \{#max_entries_for_hash_table_stats\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />聚合时收集的哈希表统计信息中允许的最大条目数

## max_fetch_partition_thread_pool_size \{#max_fetch_partition_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="64" changeable_without_restart="是" />用于执行 ALTER TABLE FETCH PARTITION 的线程数量。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />可同时保留的轻量级快照最大数量。0 表示不受限制。如果快照数量达到该限制，尝试创建新快照时会抛出异常。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />服务器上所有合并操作的最大读取带宽 (字节/秒) 。0 表示无限制。

## max_mutations_bandwidth_for_server \{#max_mutations_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />服务器上所有 mutation 操作的最大读取带宽，单位为字节每秒。0 表示不限制。
...
<SettingsInfoBlock type="Float" default_value="0" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="32" changeable_without_restart="是" />在启动时用于加载非活动 (过时) 分区片段的线程数。

## max_part_num_to_warn \{#max_part_num_to_warn\}

<SettingsInfoBlock type="UInt64" default_value="100000" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="50000000000" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="128" changeable_without_restart="是" />用于并发移除不活跃数据分区片段的线程数。
...
<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />用于副本抓取的网络数据交换最大速度，单位为字节/秒。0 表示无限制。

## max_replicated_sends_network_bandwidth_for_server \{#max_replicated_sends_network_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />副本发送时通过网络进行数据交换的最大速率，单位为字节/秒。0 表示不限制。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />如果快照提交线程池中的空闲线程数超过 `max_snapshot_commit_thread_pool_free_size`，ClickHouse 将释放这些空闲线程占用的资源，并缩小线程池的大小。必要时可以重新创建线程。

## max_snapshot_commit_thread_pool_size \{#max_snapshot_commit_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="64" changeable_without_restart="是" />用于提交快照的线程数量。
...
<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="否" />
...
<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="No" />启动时用于加载非活跃意外分区片段的线程数量。

## max_view_num_to_throw \{#max_view_num_to_throw\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />

如果浏览次数的数量超过该值，服务器将抛出异常。
...
如果附加的浏览次数数量超过指定值，ClickHouse 服务器会向 `system.warnings` 表中添加警告消息。
...
后台内存工作线程的轮询周期，用于修正内存跟踪器的内存使用量，并在高内存使用期间清理未使用的内存页。若设置为 0，则将使用默认值，该默认值取决于内存使用来源。

## memory_worker_purge_dirty_pages_threshold_ratio \{#memory_worker_purge_dirty_pages_threshold_ratio\}

<SettingsInfoBlock type="Double" default_value="0.2" changeable_without_restart="否" />
...
<SettingsInfoBlock type="Double" default_value="0.9" changeable_without_restart="否" />
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />根据当前 cgroup 的内存使用信息修正内存跟踪。
...
<SettingsInfoBlock type="String" default_value="default" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="5000000000" changeable_without_restart="No" />执行 `<mlockall>` 所需的最小总内存阈值

## mmap_cache_size \{#mmap_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1024" changeable_without_restart="是" />
...
用于调节 mutation 与其他工作负载之间的资源使用与共享方式。指定的值会作为所有后台 mutation 操作的 `workload` 设置值。可以通过 MergeTree 的设置进行覆盖。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />如果设置为 true，则要求客户端必须通过 [mysql&#95;port](/operations/server-configuration-parameters/settings#mysql_port) 进行安全通信。使用 `<--ssl-mode=none>` 选项的连接将被拒绝。将其与 [OpenSSL](/operations/server-configuration-parameters/settings#openssl) 设置配合使用。

## oom_score \{#oom_score\}

<SettingsInfoBlock type="Int32" default_value="0" changeable_without_restart="No" />在 Linux 系统上，此设置可用于控制 OOM killer 的行为。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用或禁用会话缓存功能。必须与 `<sessionIdContext>` 配合使用。可接受的值：`<true>`、`<false>`。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />如果启用，则验证证书的 CN 或 SAN 是否与对端的主机名一致。

## openSSL.client.fips \{#openssl.client.fips\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="否" />启用 OpenSSL FIPS 模式。仅当库所使用的 OpenSSL 版本支持 FIPS 时才受支持。

## openSSL.client.invalidCertificateHandler.name \{#openssl.client.invalidcertificatehandler.name\}

<SettingsInfoBlock type="String" default_value="RejectCertificateHandler" changeable_without_restart="No" />用于验证无效证书的类 (CertificateHandler 的子类) 。例如：`<<invalidCertificateHandler> <name>RejectCertificateHandler</name> </invalidCertificateHandler>>`。

## openSSL.client.loadDefaultCAFile \{#openssl.client.loaddefaultcafile\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />决定是否使用 OpenSSL 的内置 CA 证书。ClickHouse 假定内置 CA 证书位于文件 `</etc/ssl/cert.pem>` (或目录 `</etc/ssl/certs>`) 中，或者位于由环境变量 `<SSL_CERT_FILE>` (或 `<SSL_CERT_DIR>`) 指定的文件 (或目录) 中。

## openSSL.client.preferServerCiphers \{#openssl.client.preferserverciphers\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />客户端是否优先采用服务器端的密码套件。
...
<SettingsInfoBlock type="String" default_value="KeyConsoleHandler" changeable_without_restart="No" />用于获取访问私钥所需口令的类 (PrivateKeyPassphraseHandler 子类) 。例如：`<<privateKeyPassphraseHandler>>`, `<<name>KeyFileHandler</name>>`, `<<options><password>test</password></options>>`, `<</privateKeyPassphraseHandler>>`

## openSSL.client.requireTLSv1 \{#openssl.client.requiretlsv1\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />要求使用 TLSv1 连接。允许的取值：`<true>`、`<false>`。

## openSSL.client.requireTLSv1_1 \{#openssl.client.requiretlsv1_1\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />要求使用 TLSv1.1 协议的连接。可接受的取值：`<true>`、`<false>`。
...
<SettingsInfoBlock type="UInt64" default_value="9" changeable_without_restart="No" />验证链的最大长度。如果证书链长度超过设置的值，则验证将失败。

## openSSL.client.verificationMode \{#openssl.client.verificationmode\}

<SettingsInfoBlock type="String" default_value="relaxed" changeable_without_restart="No" />用于校验节点证书的方法。详细信息参见 [Context](https://github.com/ClickHouse/poco/blob/master/NetSSL_OpenSSL/include/Poco/Net/Context.h) 类的描述。可选值：`<none>`、`<relaxed>`、`<strict>`、`<once>`。
...
<SettingsInfoBlock type="String" default_value="ALL:!ADH:!LOW:!EXP:!MD5:!3DES:@STRENGTH" changeable_without_restart="No" />支持的 OpenSSL 加密算法。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />若启用，将验证证书的 CN 或 SAN 是否与对端主机名匹配。

## openSSL.server.fips \{#openssl.server.fips\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用 OpenSSL 的 FIPS 模式。仅当该库所使用的 OpenSSL 版本支持 FIPS 时才受支持。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="否" />使用客户端偏好的服务器端加密套件。
...
<SettingsInfoBlock type="String" default_value="KeyConsoleHandler" changeable_without_restart="No" />用于请求输入访问私钥所需口令的类 (PrivateKeyPassphraseHandler 子类) 。例如：`<<privateKeyPassphraseHandler>>`、`<<name>KeyFileHandler</name>>`、`<<options><password>test</password></options>>`、`<</privateKeyPassphraseHandler>>`
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />是否要求使用 TLSv1.1 连接。可接受的值：`<true>`、`<false>`。

## openSSL.server.requireTLSv1_2 \{#openssl.server.requiretlsv1_2\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />是否要求使用 TLSv1.2 连接。可接受的取值：`<true>`、`<false>`。

## openSSL.server.sessionCacheSize \{#openssl.server.sessioncachesize\}

<SettingsInfoBlock type="UInt64" default_value="20480" changeable_without_restart="否" />服务器缓存的最大会话数。值为 0 表示会话数不受限制。

## openSSL.server.sessionIdContext \{#openssl.server.sessionidcontext\}

<SettingsInfoBlock type="String" default_value="application.name" changeable_without_restart="No" />服务器附加到每个生成的会话标识符上的一组唯一随机字符。字符串长度不得超过 `<SSL_MAX_SSL_SESSION_ID_LENGTH>`。始终建议设置此参数，因为无论服务器是否缓存会话，还是客户端是否请求缓存，它都有助于避免相关问题。

## openSSL.server.sessionTimeout \{#openssl.server.sessiontimeout\}

<SettingsInfoBlock type="UInt64" default_value="2" changeable_without_restart="否" />会话在服务器端的缓存时长 (以小时为单位) 。

## openSSL.server.verificationDepth \{#openssl.server.verificationdepth\}

<SettingsInfoBlock type="UInt64" default_value="9" changeable_without_restart="No" />验证链的最大长度。如果证书链的长度超过该值，则验证将失败。

## openSSL.server.verificationMode \{#openssl.server.verificationmode\}

<SettingsInfoBlock type="String" default_value="relaxed" changeable_without_restart="No" />用于校验节点证书的方法。详情参见 [Context](https://github.com/ClickHouse/poco/blob/master/NetSSL_OpenSSL/include/Poco/Net/Context.h) 类的说明。可选值：`<none>`、`<relaxed>`、`<strict>`、`<once>`。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />启用从 /proc/pressure/ 文件采集 PSI 指标。

## os_cpu_busy_time_threshold \{#os_cpu_busy_time_threshold\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="No" />操作系统 CPU 忙碌时间 (OSCPUVirtualTimeMicroseconds 指标) 的阈值，单位为微秒。仅当忙碌时间超过该值时，才认为 CPU 在执行有用工作；如果忙碌时间低于该值，则不会认为 CPU 处于过载状态。
...
<SettingsInfoBlock type="Double" default_value="0.15" changeable_without_restart="否" />在用户态页缓存中预留为空闲的内存限制比例。类似于 Linux 的 min&#95;free&#95;kbytes 设置。

## page_cache_history_window_ms \{#page_cache_history_window_ms\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />在释放的内存可以被用户态页缓存重新使用前需要等待的延迟时间。

## page_cache_max_size \{#page_cache_max_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />用户态页缓存的最大容量。设置为 0 可禁用该缓存。如果该值大于 page&#95;cache&#95;min&#95;size，则缓存大小会在该范围内持续调整，在保持总内存使用量低于限制 (max&#95;server&#95;memory&#95;usage[&#95;to&#95;ram&#95;ratio]) 的前提下尽可能多地使用可用内存。

## page_cache_min_size \{#page_cache_min_size\}

<SettingsInfoBlock type="UInt64" default_value="104857600" changeable_without_restart="No" />用户态页缓存的最小大小。
...
<SettingsInfoBlock type="UInt64" default_value="4" changeable_without_restart="否" />将用户态页缓存拆分为相应数量的分片 (shard) ，以减少互斥锁争用。属于实验性功能，一般不太可能带来性能提升。

## page_cache_size_ratio \{#page_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="否" />用户态页缓存中受保护队列的大小，占该缓存总大小的比例。

## parquet_metadata_cache_max_entries \{#parquet_metadata_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="No" />Parquet 元数据文件缓存的最大条目数量。设置为 0 表示禁用。
...
<SettingsInfoBlock type="UInt64" default_value="536870912" changeable_without_restart="是" />Parquet 元数据缓存的最大容量，单位为字节。设置为 0 表示禁用。

## parquet_metadata_cache_size_ratio \{#parquet_metadata_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />在 parquet 元数据缓存中，受保护队列 (在使用 SLRU 策略时) 的大小相对于缓存总大小的比例。
...
<SettingsInfoBlock type="UInt64" default_value="128" changeable_without_restart="否" />
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />如果设置为 true，则必须通过 [postgresql&#95;port](/operations/server-configuration-parameters/settings#postgresql_port) 与客户端进行安全通信。带有 `<sslmode=disable>` 选项的连接将被拒绝。将其与 [OpenSSL](/operations/server-configuration-parameters/settings#openssl) 设置配合使用。

## prefetch_threadpool_pool_size \{#prefetch_threadpool_pool_size\}

<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="No" />用于处理远程对象存储预取操作的后台线程池大小

## prefetch_threadpool_queue_size \{#prefetch_threadpool_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="否" />可以推入预取线程池的任务数量上限
...
<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="否" />预热期间要填充的 mark 缓存大小占其总大小的比例。

## primary_index_cache_size \{#primary_index_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="5368709120" changeable_without_restart="是" />主索引缓存的最大容量 (MergeTree 系列表的主索引) 。

## primary_index_cache_size_ratio \{#primary_index_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="否" />在主索引缓存中，受保护队列的大小 (在使用 SLRU 策略时) 相对于缓存总大小的比例。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />导出 keeper 相关的指标
...
<SettingsInfoBlock type="UInt64" default_value="1024" changeable_without_restart="否" />缓存中可存储的 SELECT 查询结果的最大数量。

## query_cache.max_entry_size_in_bytes \{#query_cache.max_entry_size_in_bytes\}

<SettingsInfoBlock type="UInt64" default_value="1048576" changeable_without_restart="否" />可保存到缓存中的 SELECT 查询结果的最大大小 (字节) 。

## query_cache.max_entry_size_in_rows \{#query_cache.max_entry_size_in_rows\}

<SettingsInfoBlock type="UInt64" default_value="30000000" changeable_without_restart="否" />可写入缓存的 SELECT 查询结果的最大行数。

## query_cache.max_size_in_bytes \{#query_cache.max_size_in_bytes\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="是" />最大缓存大小 (字节) 。0 表示禁用查询缓存。
...
<SettingsInfoBlock type="UInt64" default_value="104857600" changeable_without_restart="是" />
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />查询条件缓存中受保护队列的大小 (在采用 SLRU 策略时) ，相对于缓存总大小的比例。
...
<SettingsInfoBlock type="Seconds" default_value="0" changeable_without_restart="否" />用于数据分片拉取请求的 HTTP 连接超时时间。如果未显式设置，则继承自默认 profile `http_connection_timeout`。

## replicated_fetches_http_receive_timeout \{#replicated_fetches_http_receive_timeout\}

<SettingsInfoBlock type="Seconds" default_value="0" changeable_without_restart="No" />用于 fetch part 请求的 HTTP 接收超时时间。若未显式设置，则从默认 profile 的 `http_receive_timeout` 继承。

## replicated_fetches_http_send_timeout \{#replicated_fetches_http_send_timeout\}

<SettingsInfoBlock type="秒" default_value="0" changeable_without_restart="否" />用于数据分片拉取请求的 HTTP 发送超时时间。若未显式设置，则继承默认 profile `http_send_timeout` 的值。
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="16" changeable_without_restart="否" />用于执行 RESTORE 请求的最大线程数。

## s3_credentials_provider_max_cache_size \{#s3_credentials_provider_max_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="否" />可缓存的 S3 凭证提供者最大数量

## s3_max_redirects \{#s3_max_redirects\}

<SettingsInfoBlock type="UInt64" default_value="10" changeable_without_restart="No" />S3 重定向允许的最大跳转次数。

## s3_retry_attempts \{#s3_retry_attempts\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />用于 Aws::Client::RetryStrategy 的设置，Aws::Client 会自行执行重试；0 表示不进行重试

## s3queue_disable_streaming \{#s3queue_disable_streaming\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="是" />即使已创建表并附加了物化视图，也在 S3Queue 中禁用流式处理。
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />当设置为 true 时，将在堆栈跟踪中显示地址

## shutdown_wait_backups_and_restores \{#shutdown_wait_backups_and_restores\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />当设置为 true 时，ClickHouse 会在关闭前等待所有正在进行的备份和恢复操作完成。

## shutdown_wait_unfinished \{#shutdown_wait_unfinished\}

<SettingsInfoBlock type="UInt64" default_value="5" changeable_without_restart="No" />等待未完成查询的时间 (秒)

## shutdown_wait_unfinished_queries \{#shutdown_wait_unfinished_queries\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />当设置为 true 时，ClickHouse 会在关闭前等待正在运行的查询完成。

## skip_binary_checksum_checks \{#skip_binary_checksum_checks\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />跳过对 ClickHouse 二进制文件的校验和完整性检查
...
<SettingsInfoBlock type="UInt64" default_value="120" changeable_without_restart="否" />
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />如果设置为 true，则在脚本执行期间发生错误时，服务器将不会启动。
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="No" />当达到该限制时，在尝试创建时会抛出异常。将其设置为 0 可关闭硬性限制。该限制适用于存储连接数。

## storage_connections_rcvbuf \{#storage_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="是" />存储连接 (复制、分布式查询) 的 SO&#95;RCVBUF 选项大小。如果将其设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：仅将此设置修改回 0，只会对新创建的连接恢复自动调优；现有池化连接会保留固定的缓冲区大小，直到重新创建后才会恢复。

## storage_connections_sndbuf \{#storage_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />存储连接 (复制、分布式查询) 的 `SO_SNDBUF` 选项大小。若设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，仅新创建的连接会恢复自动调优；连接池中的现有连接会继续保留固定的缓冲区大小，直到被重新创建。

## storage_connections_soft_limit \{#storage_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="否" />超过此限制的连接，其存活时间会显著缩短。该限制适用于存储连接。

## storage_connections_store_limit \{#storage_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />超过此限制的连接在使用后会被重置。将其设置为 0 可关闭连接缓存。该限制适用于存储连接。

## storage_connections_warn_limit \{#storage_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />如果正在使用的连接数超过此限制，将在日志中记录警告信息。该限制适用于存储连接。

## storage_metadata_write_full_object_key \{#storage_metadata_write_full_object_key\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />使用 VERSION&#95;FULL&#95;OBJECT&#95;KEY 格式写入磁盘元数据文件。默认开启。该设置已弃用。

## storage_shared_set_join_use_inner_uuid \{#storage_shared_set_join_use_inner_uuid\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />启用后，在创建 SharedSet 和 SharedJoin 时会生成内部 UUID。仅适用于 ClickHouse Cloud。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />在关闭连接之前，每个 TCP 连接允许的最大查询次数。设置为 0 表示不限制查询次数。

## tcp_close_connection_after_queries_seconds \{#tcp_close_connection_after_queries_seconds\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />TCP 连接在被关闭前的最大存续时间 (秒) 。设置为 0 表示连接存续时间不限。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />将临时数据存储在分布式缓存中。

## text_index_header_cache_max_entries \{#text_index_header_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="100000" changeable_without_restart="No" />头部缓存的大小 (以条目数计) 。值为 0 表示禁用。
...
<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="是" />头部缓存的大小。为 0 表示禁用。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="否" />文本索引头部缓存中受保护队列 (在使用 SLRU 策略时) 的大小相对于缓存总大小的比例。

## text_index_postings_cache_max_entries \{#text_index_postings_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="No" />文本索引倒排列表缓存的大小 (按条目数计) 。设置为 0 表示禁用。

## text_index_postings_cache_policy \{#text_index_postings_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />文本索引倒排列表缓存策略名称。

## text_index_postings_cache_size \{#text_index_postings_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="2147483648" changeable_without_restart="是" />文本索引 倒排列表的缓存大小。0 表示禁用。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />在文本索引 倒排列表缓存中 (在使用 SLRU 策略时) ，受保护队列大小与缓存总大小的比例。

## text_index_tokens_cache_max_entries \{#text_index_tokens_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="No" />文本索引 token 缓存的最大条目数量。设置为 0 表示禁用该缓存。

## text_index_tokens_cache_policy \{#text_index_tokens_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />文本索引 token 缓存策略的名称。

## text_index_tokens_cache_size \{#text_index_tokens_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="是" />文本索引 token 缓存大小。0 表示禁用。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />文本索引 token 缓存中 (采用 SLRU 策略时) 受保护队列大小与该缓存总大小的比例。
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="No" />当 `local_filesystem_read_method = 'pread_threadpool'` 时，用于从本地文件系统读取数据的线程池中的线程数量。

## threadpool_local_fs_reader_queue_size \{#threadpool_local_fs_reader_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="否" />用于从本地文件系统读取数据的线程池中可调度的最大作业数量。

## threadpool_remote_fs_reader_pool_size \{#threadpool_remote_fs_reader_pool_size\}

<SettingsInfoBlock type="NonZeroUInt64" default_value="250" changeable_without_restart="No" />当 `remote_filesystem_read_method = 'threadpool'` 时，用于从远程文件系统读取的线程池中的线程数量。

## threadpool_remote_fs_reader_queue_size \{#threadpool_remote_fs_reader_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />用于从远程文件系统读取的线程池中可调度任务的最大数量。

## threadpool_writer_pool_size \{#threadpool_writer_pool_size\}

<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="No" />用于处理向对象存储发送写入请求的后台线程池大小

## threadpool_writer_queue_size \{#threadpool_writer_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />可推送到用于对象存储写请求的后台线程池中的最大任务数

## throw_on_unknown_workload \{#throw_on_unknown_workload\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="是" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />以概率 `total_memory_profiler_sample_probability` 收集大小小于或等于指定值的随机内存分配。0 表示禁用。你可能需要将 `max_untracked_memory` 设置为 0，才能使该阈值按预期生效。

## total_memory_profiler_sample_min_allocation_size \{#total_memory_profiler_sample_min_allocation_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />以 `total_memory_profiler_sample_probability` 的概率收集大小大于或等于指定值的随机内存分配。0 表示禁用。可以将 &#39;max&#95;untracked&#95;memory&#39; 设置为 0，以便该阈值按预期生效。

## total_memory_profiler_step \{#total_memory_profiler_step\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="否" />每当服务器内存使用量 (以字节计) 超过下一个步长阈值时，内存分析器都会收集当前分配操作的堆栈跟踪。值为 0 表示禁用内存分析器。将该值设置为低于数兆字节会减慢服务器。
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />未压缩缓存策略的名称。
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />确定在接收到查询数据包时，是否启用客户端信息验证。
...
<SettingsInfoBlock type="UInt64" default_value="10000000" changeable_without_restart="否" />向量相似度索引缓存的大小 (按条目数量计) 。设置为 0 表示禁用。
...
<SettingsInfoBlock type="UInt64" default_value="5368709120" changeable_without_restart="是" />向量相似度索引缓存的大小。为 0 表示禁用。
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />在向量相似度索引缓存中，受保护队列 (在使用 SLRU 策略时) 的大小与该缓存总大小的比例。
...
<SettingsInfoBlock type="String" default_value="wasmtime" changeable_without_restart="No" />用于执行 WebAssembly UDF 的引擎。可选值为 &#39;wasmtime&#39; 和 &#39;wasmedge&#39;。

## webterminal_allowed_origins \{#webterminal_allowed_origins\}

允许打开 `/webterminal` WebSocket 会话的完整来源 (协议 + 主机 + 可选端口) 列表，多个来源之间用逗号分隔。为空时，将严格执行同源策略 (Origin 必须与请求的协议、主机和端口一致) 。如果部署位于执行 TLS 终止的反向代理之后，请设置此项；在这种情况下，即使浏览器使用的是 `https`，`request.isSecure()` 也会是 `false`。示例：`https://example.com,https://app.example.com:8443`。
...
| `pass_opentelemetry_tracing_context` (optional) | 启用将 OpenTelemetry 跟踪上下文传递到 Keeper 请求。启用后，会为 Keeper 操作创建跟踪 span，从而在 ClickHouse 与 Keeper 之间实现分布式追踪。有关更多详细信息，参见 [Tracing ClickHouse Keeper Requests](/operations/opentelemetry#tracing-clickhouse-keeper-requests)。默认值：`false`。 |
