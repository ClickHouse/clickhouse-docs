...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />磁盘 (S3、Azure、GCS) 连接的 `SO_RCVBUF` 选项大小。如果设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 仅会对新创建的连接恢复自动调优；现有池化连接会保留固定的缓冲区大小，直到重新创建为止。

## disk_connections_sndbuf \{#disk_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />用于磁盘 (S3、Azure、GCS) 连接的 SO&#95;SNDBUF 选项大小。如果设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，只有新创建的连接才会恢复自动调优；现有的池化连接仍会保留固定的缓冲区大小，直到重新创建为止。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />通用 HTTP 连接的 SO&#95;RCVBUF 选项大小。如果设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 只会为新创建的连接恢复自动调优；现有连接池中的连接会保留固定的缓冲区大小，直到重新创建为止。

## http_connections_sndbuf \{#http_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />通用 HTTP 连接的 SO&#95;SNDBUF 选项大小。若设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，只有新创建的连接才会恢复自动调优；现有连接池中的连接会继续保留固定的缓冲区大小，直到重新创建。
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />存储连接 (复制、分布式查询) 的 SO&#95;RCVBUF 选项大小。如果将其设置为大于 0 的值，则会重写内核对接收缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：仅将此设置修改回 0，只会对新创建的连接恢复自动调优；现有池化连接会保留固定的缓冲区大小，直到重新创建后才会恢复。

## storage_connections_sndbuf \{#storage_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />存储连接 (复制、分布式查询) 的 `SO_SNDBUF` 选项大小。若设置为大于 0 的值，则会重写内核对发送缓冲区的 TCP 自动调优。0 = 内核默认值 (自动调优) 。注意：将此设置改回 0 后，仅新创建的连接会恢复自动调优；连接池中的现有连接会继续保留固定的缓冲区大小，直到被重新创建。
