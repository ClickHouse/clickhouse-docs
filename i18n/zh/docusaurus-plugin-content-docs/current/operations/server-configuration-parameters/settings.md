...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />启用位于 `/webterminal` HTTP 端点的 Experimental Web 终端界面。它通过 WebSocket 在浏览器中提供交互式 `clickhouse-client` 会话。
