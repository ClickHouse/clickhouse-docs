...
| `disallow_config_defined_profiles_for_sql_defined_users` | 启用后，将阻止把 config 中定义的 settings profile (即在 XML configuration files 中定义的 profile，例如 `users.xml`) 应用于通过 SQL 定义的用户，但默认 profile 除外。同时也会检查分配给授予 SQL 定义用户的角色的 profile。                                                                                                                                   | `false` |
...
<SettingsInfoBlock type="UInt64" default_value="5000" />Parquet 元数据文件缓存的最大条目数量。设置为 0 表示禁用。

## parquet_metadata_cache_policy \{#parquet_metadata_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />Parquet 元数据缓存策略名称。

## parquet_metadata_cache_size \{#parquet_metadata_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="536870912" />Parquet 元数据缓存的最大容量，单位为字节。设置为 0 表示禁用。

## parquet_metadata_cache_size_ratio \{#parquet_metadata_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" />在 parquet 元数据缓存中，受保护队列（在使用 SLRU 策略时）的大小相对于缓存总大小的比例。
...
<SettingsInfoBlock type="UInt64" default_value="1000000" />文本索引 token 缓存的最大条目数量。设置为 0 表示禁用该缓存。

## text_index_tokens_cache_policy \{#text_index_tokens_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />文本索引 token 缓存策略的名称。

## text_index_tokens_cache_size \{#text_index_tokens_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" />文本索引 token 缓存大小。0 表示禁用。
...
<SettingsInfoBlock type="Double" default_value="0.5" />文本索引 token 缓存中（采用 SLRU 策略时）受保护队列大小与该缓存总大小的比例。
