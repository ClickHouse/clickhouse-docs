...
`aiEmbed` 发起的单个 HTTP 请求中可包含的最大文本数。为减少 API 调用开销，文本会按此大小分成多个批次。例如，500 个唯一文本在批次大小为 100 时，会发起 5 个 HTTP 请求。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "新设置"}]}]} />

单个查询中所有 AI 函数 API 调用的输入 (提示) 标记总数上限。根据提供商返回的响应进行累计统计。请注意，由于单次调用的输入标记数无法预先得知，此限制可能会被单次调用的输入标记数超出。设置为 0 可禁用。

此限制仅对在响应中报告 `usage` 对象的提供商生效 (OpenAI、Anthropic、vLLM) 。未报告标记用量的提供商 (尤其是 HuggingFace TEI) 会导致计数器保持为 0 —— 请改用 `ai_function_max_api_calls_per_query` 来限制此类调用。
...
此限制仅对在响应中返回 `usage` 对象的提供商生效 (OpenAI、Anthropic、vLLM) 。它不适用于嵌入函数 (尤其是 aiEmbed) ，因为嵌入函数不会生成输出标记。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.3"},{"label": "0"},{"label": "允许使用实验性数据库引擎 DataLakeCatalog（catalog_type = 'glue'）"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "25.3"},{"label": "0"},{"label": "允许使用实验性数据库引擎 DataLakeCatalog（catalog_type = 'unity'）"}]}]}/>
...
允许您指定对于由 [`ALTER`](../../sql-reference/statements/alter/index.md)、[`OPTIMIZE`](../../sql-reference/statements/optimize.md) 或 [`TRUNCATE`](../../sql-reference/statements/truncate.md) 查询发起、需要在副本上执行的操作，应采用何种等待行为。
...
<VersionHistory
  ...
  {
    id: "row-1",
    items: [{ label: "26.4" }, { label: "0" }, { label: "新设置" }]
  }
]}
/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.2"},{"label": "200"},{"label": "异步插入超时的最小毫秒数；async_insert_busy_timeout_ms 是 async_insert_busy_timeout_max_ms 的别名"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.10"},{"label": "1"},{"label": "用于 ClickHouse Cloud 的设置"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.10"},{"label": "10000"},{"label": "用于 ClickHouse Cloud 的设置"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "432000000"},{"label": "新设置"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "新设置"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "用于限制随机分布函数中分布形状参数的新设置，可防止极端输入导致卡住。"}]}]} />

随机分布函数 (如 `randChiSquared`、`randStudentT` 和 `randFisherF`) 中分布形状参数的最大值。此设置可防止参数值过于极端时导致计算时间过长。

## max_rand_distribution_trials \{#max_rand_distribution_trials\}

<SettingsInfoBlock type="UInt64" default_value="1000000000" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000000"},{"label": "新设置，用于限制随机分布函数中的试验次数，防止在极端输入下长时间卡住。"}]}]} />

`randBinomial` 和 `randNegativeBinomial` 等随机分布函数允许的最大试验次数。该设置可防止试验次数过大时导致计算时间过长。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "用于限制在启用 skip_unavailable_shards 时可静默跳过的分片数量的新设置。"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "用于限制在启用 skip_unavailable_shards 时可静默跳过的分片比例的新设置。"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1000000"},{"label": "新增设置，用于限制 WKB 几何对象解析中的元素数量，防止格式错误的数据导致过大的内存分配。"}]}]} />

由 `readWKB` 及相关函数解析时，单个 WKB 几何元素中允许包含的点、环或多边形的最大数量。此设置可防止格式错误的 WKB 数据导致过大的内存分配。将其设置为 0 可使用硬编码的限制值 (1 亿) 。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "现在默认启用将 arrayExists 重写为 has 的优化，因为在重写前会先检查类型兼容性。"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "新增设置。禁用后，用于并行读取的副本将完全由负载均衡算法选择，不会强制将本地副本纳入其中。"}]}]} />

启用时 (默认) ，本地副本始终会包含在用于并行读取的副本集合中。
禁用时，本地副本不会被优先考虑，副本将完全由负载均衡算法选择。
这样一来，设置了 `max_parallel_replicas = 1` 的查询就可以被定向到另一台主机；当大量短查询分布在集群中时，这有助于提高缓存局部性。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "新增设置，用于将谓词选择性统计信息收集到 system.predicate_statistics_log"}]}]} />

将谓词选择性统计信息收集到 `system.predicate_statistics_log` 中。设置为 N &gt; 0 时，将按查询 ID 对约 1/N 的查询进行采样。0 表示禁用。
...
设置 [query profiler](../../operations/optimizing-performance/sampling-query-profiler.md) 的实时时钟计时器周期。实时时钟计时器按挂钟时间计时。
