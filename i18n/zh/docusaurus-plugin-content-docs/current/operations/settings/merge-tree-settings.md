...
支持的统计类型：basic、tdigest、countmin、minmax、uniq。
...
从其他副本拉取分区片段时的最小 level。低于此阈值的 parts 会被延后
 (保留在复制队列中，并在每个调度周期重新评估，而不是被永久跳过) 。
将其设为 1 可延后拉取 level-0 (未合并) 的 parts，从而在高强度摄取期间降低复制开销。
默认值：0 (无论 level 如何，都会拉取所有 parts) 。
...
超过此超时时间 (以秒为单位) 后，即使某个低于 replicated&#95;fetches&#95;min&#95;part&#95;level 的分区片段，也会照常拉取。
使用 0 可禁用该超时 (低于最小 level 的 parts 将被无限期延后，直到合并完成) 。
默认值：300 (5 分钟后强制拉取) 。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "设置为 false 时，名称中带点号且具有共同前缀的 Array 列会被视为彼此独立的列，而不是按旧版 Nested 语义共享 offsets 文件"}]}]} />

启用时 (默认) ，名称中带点号且具有共同前缀的 Array 列 (例如 `n.a` 和 `n.b`) 
会被视为 Nested 结构的一部分：它们在磁盘上共享同一个 offsets 文件 (例如 `n.size0`) ，
并且会在 INSERT 时验证它们的数组大小是否相等。
禁用时，每个 Array 列都会有自己独立的 offset 文件，带点号的名称不再具有特殊
语义，并且标量列可以与具有相同前缀的带点号 Array 列共存
 (例如 `n UInt32` 与 `n.a Array(String)` 同时存在) 。该设置在表创建后不可更改。
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "默认启用"}]}, {"id": "row-2","items": [{"label": "25.9"},{"label": "0"},{"label": "New setting"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "auto"},{"label": "将设置设为 auto：每个副本的最大合并任务数 * 活跃副本数"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "100"},{"label": "New setting"}]}]} />

协调器需要准备并在各个 worker 之间分发的合并任务数量。
当设置为 &#39;auto&#39; 时，其值等于单个副本允许的最大合并任务数乘以活跃副本数。
...
例外情况：`value = 0` 不遵循该规则；
实现中会采用最小 200 毫秒，因此下一次运行会在 [0, 200] 毫秒范围内抖动。
