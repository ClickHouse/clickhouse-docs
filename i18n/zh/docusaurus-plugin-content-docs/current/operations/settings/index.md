...
| [会话设置](/operations/settings/settings) | 存在于 ``system.settings`` 表中的设置。 |
| [设置概述](/operations/settings/overview) | 设置的概述页面。 |
| [用户与角色设置](/operations/settings/settings-users) | 配置用户和角色的设置。 |
...
| [MergeTree 表设置](/operations/settings/merge-tree-settings) | 存在于 `system.merge_tree_settings` 中的 MergeTree 设置 |
| [设置限制](/operations/settings/constraints-on-settings) | 可以在 `user.xml` 配置文件的 `profiles` 部分定义设置的限制，并禁止用户通过 `SET` 查询更改某些设置。 |
| [内存超承诺](/operations/settings/memory-overcommit) | 一种实验性技术，旨在允许为查询设置更灵活的内存限制。 |
