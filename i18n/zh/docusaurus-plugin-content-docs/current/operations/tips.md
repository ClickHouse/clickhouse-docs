...
始终将 Transparent huge pages 设置为 `madvise`。在较旧的内核上 (5.9 之前) ，将 THP 设为 `always` 可能会导致显著的性能下降——内核会在内存碎片整理上花费过多时间，尤其是在 RAM 为 64 GB 以上的系统中。内核 5.9 引入了主动合并整理机制，对 THP 的处理改善了很多；但如果 THP 设为 `always`，ClickHouse 在启动时仍会发出警告，因此无论内核版本如何，仍然建议使用 `madvise`。

```bash
$ echo 'madvise' | sudo tee /sys/kernel/mm/transparent_hugepage/enabled
```

如果您想永久修改 transparent huge pages 设置，请编辑 `/etc/default/grub`，在 `GRUB_CMDLINE_LINUX_DEFAULT` 选项中添加 `transparent_hugepage=madvise`：
