...
如果要发送一组查询，请创建一个文本文件，每个查询放在该文件的单独字符串中。例如：
...
- `-r`, `--randomize` — 如果有多个输入查询，则随机执行查询的顺序。
- `-s`, `--secure` — 使用 `TLS` 连接。
- `-t N`, `--timelimit=N` — 超时时间（秒）。当达到指定的时间限制时，`clickhouse-benchmark` 停止发送查询。默认值：0（禁用时间限制）。
- `--port=N` — 服务器端口。默认值：9000。对于 [比较模式](#clickhouse-benchmark-comparison-mode)，可以使用多个 `--port` 键。
- `--confidence=N` — T 检验的置信度水平。可能的值：0（80%）、1（90%）、2（95%）、3（98%）、4（99%）、5（99.5%）。默认值：5。在 [比较模式](#clickhouse-benchmark-comparison-mode) 中，`clickhouse-benchmark` 执行 [独立双样本 Student's t 检验](https://en.wikipedia.org/wiki/Student%27s_t-test#Independent_two-sample_t-test)，以确定在所选置信度水平下两个分布是否没有差异。
- `--cumulative` — 打印累积数据而不是每个间隔的数据。
- `--database=DATABASE_NAME` — ClickHouse 数据库名称。默认值：`default`。
- `--user=USERNAME` — ClickHouse 用户名称。默认值：`default`。
- `--password=PSWD` — ClickHouse 用户密码。默认值：空字符串。
- `--stacktrace` — 堆栈跟踪输出。当设置此键时，`clickhouse-benchmak` 输出异常的堆栈跟踪。
...
- `--reconnect=N` - 控制重新连接行为。可能的值 0（从不重新连接）、1（每个查询重新连接）或 N（每 N 个查询后重新连接）。默认值：1。
- `--help` — 显示帮助信息。

如果想要为查询应用一些 [设置](/operations/settings/overview)，请将它们作为键 `--<session setting name>= SETTING_VALUE` 传递。例如，`--max_memory_usage=1048576`。

## 输出 {#clickhouse-benchmark-output}

默认情况下，`clickhouse-benchmark` 会在每个 `--delay` 间隔中报告。
...
- 包含状态字符串（按顺序）：
    ...
    - MiB/s：在 `--delay` 参数指定的时间段内，服务器每秒读取的 mebibyte 数量。
    - result RPS：在 `--delay` 参数指定的时间段内，服务器每秒放入查询结果的行数。
    - result MiB/s：在 `--delay` 参数指定的时间段内，服务器每秒放入查询结果的 mebibyte 数量。
...
要使用比较模式，通过两对 `--host` 和 `--port` 键指定两个服务器的端点。键按参数列表中的位置匹配，第一个 `--host` 与第一个 `--port` 匹配，依此类推。`clickhouse-benchmark` 建立与两个服务器的连接，然后发送查询。每个查询都指向随机选择的服务器。结果将显示在一个表中。
