...
1. 在 [query_views_log](../../operations/server-configuration-parameters/settings.md#query_views_log) 部分中配置参数。
...
- `view_type` ([Enum8](../../sql-reference/data-types/enum.md)) — 视图的类型。值：
    - `'Default' = 1` — [默认视图](/sql-reference/statements/create/view#normal-view)。不应该出现在此日志中。
...
- `peak_memory_usage` ([Int64](../../sql-reference/data-types/int-uint.md)) — 此视图分配和释放内存的最大差异。
- `ProfileEvents` ([Map(String, UInt64)](../../sql-reference/data-types/array.md)) — 用于测量不同指标的 ProfileEvents。它们的描述可以在表 [system.events](/operations/system-tables/events) 中找到。
- `status` ([Enum8](../../sql-reference/data-types/enum.md)) — 视图的状态。值：
    - `'QueryStart' = 1` — 视图执行的成功开始。不应出现。
    - `'QueryFinish' = 2` — 视图执行的成功结束。
    - `'ExceptionBeforeStart' = 3` — 视图执行开始前的异常。
    - `'ExceptionWhileProcessing' = 4` — 视图执行过程中发生的异常。
