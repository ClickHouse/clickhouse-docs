...
ClickHouse不会自动从表中删除数据。有关更多详细信息，请参阅 [Introduction](/operations/system-tables/overview#system-tables-introduction)。
...
- `event_time_microseconds` ([DateTime64](../../sql-reference/data-types/datetime64.md)) — 异步插入完成执行的日期和时间（以微秒为精度）。
...
- `query_id` ([String](../../sql-reference/data-types/string.md)) — 初始查询的ID。
...
- `flush_time_microseconds` ([DateTime64](../../sql-reference/data-types/datetime64.md)) — 刷新发生的日期和时间（以微秒为精度）。
- `flush_query_id` ([String](../../sql-reference/data-types/string.md)) — 刷新查询的ID。
...
- [system.query_log](../../operations/system-tables/query_log) — 关于查询执行的一般信息的 `query_log` 系统表的描述。
