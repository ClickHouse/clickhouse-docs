...
- `num_entries` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 备份中的条目数量，即如果备份存储为文件夹，则文件夹中的文件数量；如果备份存储为归档，则归档中的文件数量。如果是增量备份，或包含空文件或重复文件，则与 `num_files` 不同。以下是始终成立的： `num_entries <= num_files`。
- `uncompressed_size` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 备份的未压缩大小。
- `compressed_size` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 备份的压缩大小。如果备份没有存储为归档，则等于 `uncompressed_size`。
