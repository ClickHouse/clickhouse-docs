...
Keeper 服务器读取操作所用线程池中的线程数。

### KeeperReadThreadsActive \{#keeperreadthreadsactive\}

Keeper 服务器读取线程池中的活动线程数。

### KeeperReadThreadsScheduled \{#keeperreadthreadsscheduled\}

Keeper 服务器读取线程池中处于排队或正在执行状态的作业数量。该指标没有实际参考意义，因为此线程池中的实际读取任务是通过另一种机制调度的。
...
主索引缓存的总大小 (以字节为单位) 。保存按需加载的主索引，即在 `primary_key_lazy_load=1` 且 `use_primary_key_cache=1` 时加载的索引。内存分配位于专用的缓存 jemalloc arena (`jemalloc.cache_arena.*`) 中。绝不会与 `system.parts.primary_key_bytes_in_memory[_allocated]` 重叠——某个 part 的索引要么位于此缓存中 (计入这里) ，要么位于 part 自身中 (计入那里) ；绝不会同时出现在两者中。若要获取所有 parts 的主索引总内存，请将两者相加。
