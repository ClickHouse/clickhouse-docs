...
* `table_engine` ([Enum8(&#39;Kafka&#39; = 1, &#39;RabbitMQ&#39; = 2)](/sql-reference/data-types/enum)) — 流类型。可能的值：&#39;Kafka&#39;、&#39;RabbitMQ&#39;。
* `event_date` ([Date](/sql-reference/data-types/date)) — 消费消息的日期。
* `event_time` ([DateTime](/sql-reference/data-types/datetime)) — 消费消息的日期和时间。
* `event_time_microseconds` ([DateTime64(6)](/sql-reference/data-types/datetime64)) — 精确到微秒的查询开始时间。
* `database` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — Kafka 表所属的 ClickHouse 数据库。
* `table` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — ClickHouse 表名。
* `error` ([String](/sql-reference/data-types/string)) — 错误文本。
* `raw_message` ([String](/sql-reference/data-types/string)) — 消息体。
* `kafka_topic_name` ([String](/sql-reference/data-types/string)) — Kafka topic 名称。
* `kafka_partition` ([UInt64](/sql-reference/data-types/int-uint)) — 该 topic 的 Kafka 分区。
* `kafka_offset` ([UInt64](/sql-reference/data-types/int-uint)) — 消息的 Kafka 偏移量。
* `kafka_key` ([String](/sql-reference/data-types/string)) — 消息的 Kafka 键。
* `rabbitmq_exchange_name` ([String](/sql-reference/data-types/string)) — RabbitMQ 交换机名称。
* `rabbitmq_message_id` ([String](/sql-reference/data-types/string)) — RabbitMQ 消息 ID。
* `rabbitmq_message_timestamp` ([DateTime](/sql-reference/data-types/datetime)) — RabbitMQ 消息时间戳。
* `rabbitmq_message_redelivered` ([UInt8](/sql-reference/data-types/int-uint)) — RabbitMQ 重新投递标志。
* `rabbitmq_message_delivery_tag` ([UInt64](/sql-reference/data-types/int-uint)) — RabbitMQ 投递标签。
* `rabbitmq_channel_id` ([String](/sql-reference/data-types/string)) — RabbitMQ 通道 ID。
