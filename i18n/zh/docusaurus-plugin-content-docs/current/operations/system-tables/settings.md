...
- `changed` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — 显示设置是否在配置中显式定义或显式更改。
...
- `tier` ([Enum8](../../sql-reference/data-types/enum.md)) — 该功能的支持级别。ClickHouse 的功能按层次组织，具体取决于其开发的当前状态和使用时可能的期望。值：
    - `'Production'` — 功能稳定、安全使用，与其他 **production** 功能没有问题。
    - `'Beta'` — 功能稳定且安全。与其他功能一起使用的结果未知，并且准确性不能保证。欢迎进行测试和报告。
    - `'Experimental'` — 功能正在开发中。仅针对开发人员和 ClickHouse 爱好者。该功能可能工作，也可能无法工作，并可能随时被删除。
    - `'Obsolete'` — 不再支持。要么已被删除，要么将来版本中将被删除。
...
使用 `WHERE changed` 可能会很有用，例如，当你想检查：
