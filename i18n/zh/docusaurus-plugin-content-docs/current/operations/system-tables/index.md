...
| [system.certificates](/operations/system-tables/certificates)                                                 | 包含可用证书及其来源信息。                                                                          |
...
| [system.collations](/operations/system-tables/collations)                                                     | 包含所有可用排序规则列表，用于按字母顺序比较字符串。                                                             |
| [system.columns](/operations/system-tables/columns)                                                           | 包含所有表各列信息的系统表                                                                          |
| [system.completions](/operations/system-tables/completions)                                                   | 包含补全标记的列表。                                                                             |
...
| [system.dashboards](/operations/system-tables/dashboards)                                                     | 包含 HTTP 接口中 `/dashboard` 页面所使用的查询语句，可用于监控和故障排查。                                        |
...
| [system.dropped&#95;tables&#95;parts](/operations/system-tables/dropped_tables_parts)                         | 用于存储来自 `system.dropped_tables` 的 MergeTree 已删除表分区片段信息的系统表                              |
...
| [system.fail&#95;points](/operations/system-tables/fail_points)                                               | 包含所有可用 failpoint 及其类型和当前状态列表的系统表。                                                      |
| [system.filesystem&#95;cache](/operations/system-tables/filesystem_cache)                                     | 包含远程对象的文件系统缓存中所有条目信息。                                                                  |
| [system.filesystem&#95;cache&#95;settings](/operations/system-tables/filesystem_cache_settings)               | 包含所有文件系统缓存设置信息                                                                         |
| [system.formats](/operations/system-tables/formats)                                                           | 包含所有格式及其标志的列表，用于指示某种格式是否适用于输入/输出以及是否支持并行化。                                             |
...
| [system.replicated&#95;merge&#95;tree&#95;settings](/operations/system-tables/replicated_merge_tree_settings) | 包含所有 ReplicatedMergeTree 引擎特有设置及其当前值、默认值和说明的列表。您可以在 CREATE 查询的 SETTINGS 部分修改其中任意一项。    |
| [system.replication&#95;queue](/operations/system-tables/replication_queue)                                   | 系统表，包含关于存储在 ClickHouse Keeper 或 ZooKeeper 中的复制队列中各任务的信息，适用于 `ReplicatedMergeTree` 系列表。 |
...
| [system.s3queue&#95;metadata&#95;cache](/operations/system-tables/s3queue_metadata_cache)                     | 包含 S3Queue 元数据的内存状态，以及当前每个文件正在处理的行。                                                    |
...
| [system.user&#95;directories](/operations/system-tables/user_directories)                                     | 包含已配置用户目录信息的系统表——即文件系统中 ClickHouse 服务端允许读取用户提供数据的目录。                                   |
