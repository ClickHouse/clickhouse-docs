...
* `thread_name` ([String](../../sql-reference/data-types/)) — 线程名称。
* `thread_id` ([UInt64](../../sql-reference/data-types/)) — 线程标识符。
* `query_id` ([String](../../sql-reference/data-types/)) — 该线程所属查询的 ID。
* `trace` ([Array(UInt64)](../../sql-reference/data-types/)) — 该线程的堆栈跟踪，本质上只是一个地址数组。
* `untracked_memory` ([Int64](../../sql-reference/data-types/)) — 每个线程中尚未传播到父级 MemoryTracker 的无原子操作内存分配计数器。如果自上次 flush 以来释放的内存多于分配的内存，则该值可能为负数。
...
* [自省函数](../../sql-reference/functions/introspection.md) — 可用的自省函数列表及其用法。
