...
* `last_error_time` ([DateTime](../../sql-reference/data-types/datetime.md))  — 上一次发生错误的时间。
* `last_error_message` ([String](../../sql-reference/data-types/string.md)) — 上一次错误的消息。
* `last_error_query_id` ([String](../../sql-reference/data-types/string.md)) — 导致上一次错误的查询 ID (如果可用) 。
* `last_error_trace` ([Array(UInt64)](../../sql-reference/data-types/array.md)) — 堆栈跟踪，表示已调用方法所在物理地址的列表。
