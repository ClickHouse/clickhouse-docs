...
* `code` ([Int32](/sql-reference/data-types/int-uint)) — 错误代码。
* `error` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — 错误名称。
* `value` ([UInt64](/sql-reference/data-types/int-uint)) — 在该时间间隔内发生的错误数量。
* `remote` ([UInt8](/sql-reference/data-types/int-uint)) — 远程异常 (即在某次分布式查询期间接收到的异常) 。
* `last_error_time` ([DateTime](/sql-reference/data-types/datetime)) — 最近一次发生错误的时间。
* `last_error_message` ([String](/sql-reference/data-types/string)) — 最近一次错误的消息。
* `last_error_query_id` ([String](/sql-reference/data-types/string)) — 导致最近一次错误的查询 ID (如果有) 。
* `last_error_trace` ([Array(UInt64)](/sql-reference/data-types/array)) — 表示堆栈跟踪的物理地址列表，即存储被调用方法的位置。
