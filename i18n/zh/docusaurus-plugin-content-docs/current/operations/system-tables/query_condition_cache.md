---
description: '显示查询条件缓存内容的 system 表。'
...
- `part_name` ([String](../../sql-reference/data-types/)) — 数据分区名称。
- `condition` ([String](../../sql-reference/data-types/)) — 哈希处理后的过滤条件。仅当设置 query_condition_cache_store_conditions_as_plaintext = true 时才会设置。
- `condition_hash` ([UInt64](../../sql-reference/data-types/)) — 过滤条件的哈希值。
- `entry_size` ([UInt64](../../sql-reference/data-types/)) — 条目大小（字节）。
