...
- `trace_id` ([UUID](../../sql-reference/data-types/uuid.md)) — 执行查询的 trace ID。
...
- `kind` ([Enum8](../../sql-reference/data-types/enum.md)) — span 的 [SpanKind](https://opentelemetry.io/docs/reference/specification/trace/api/#spankind)。
    ...
    - `SERVER` — 表示 span 涉及对一个同步 RPC 或其他远程请求的服务器端处理。
    - `CLIENT` — 表示 span 描述对某个远程服务的请求。
    - `PRODUCER` — 表示 span 描述异步请求的发起者。这个父 span 的结束时间通常会在对应的子 CONSUMER span 之前，甚至可能在子 span 开始之前结束。
...
- `attribute.names` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — 根据 `trace span` 的 [Attribute](https://opentelemetry.io/docs/go/instrumentation/#attributes) 名称。它们根据 [OpenTelemetry](https://opentelemetry.io/) 标准中的建议填写。
- `attribute.values` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — 根据 `trace span` 的属性值。它们根据 `OpenTelemetry` 标准中的建议填写。
