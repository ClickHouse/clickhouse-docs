---
'description': '系统表，包含一个特定表的过滤器，以及应使用此行策略的角色和/或用户列表。'
'keywords':
- 'system table'
- 'row_policies'
...
包含特定表的过滤器，以及应该使用此行策略的角色和/或用户列表。
...
- `table` ([String](../../sql-reference/data-types/string.md)) — 表名称。如果政策适用于数据库，则为空。
...
- `is_restrictive` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — 显示行策略是否限制对行的访问，参见 [CREATE ROW POLICY](/sql-reference/statements/create/row-policy)。值：
  - `0` — 行策略是与 `AS PERMISSIVE` 子句一起定义的。
  - `1` — 行策略是与 `AS RESTRICTIVE` 子句一起定义的。

- `apply_to_all` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — 表示行策略适用于所有角色和/或用户。

- `apply_to_list` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — 行策略应用的角色和/或用户列表。

- `apply_to_except` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — 行策略应用于所有角色和/或用户，除了列出的角色和/或用户。
