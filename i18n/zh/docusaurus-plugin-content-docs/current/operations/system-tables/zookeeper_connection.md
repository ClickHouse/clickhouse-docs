...
如果未配置 ZooKeeper，则该表不存在。`system.zookeeper&#95;connection` 表显示当前与 ZooKeeper 的连接（包括辅助 ZooKeeper）。每行表示一个连接的信息。
...
* `index` ([Nullable(UInt8)](../../sql-reference/data-types/int-uint.md)) — ClickHouse 所连接的 ZooKeeper 节点的索引。索引来自 ZooKeeper 配置。若未连接，该列为 NULL。
