---
'description': '仅在配置了 ZooKeeper 的情况下存在的系统表。显示当前与 ZooKeeper 的连接（包括辅助 ZooKeeper）。'
'keywords':
- 'system table'
...
-   `host` ([String](../../sql-reference/data-types/string.md)) — ClickHouse 连接到的 ZooKeeper 节点的主机名/IP。
-   `port` ([UIn16](../../sql-reference/data-types/int-uint.md)) — ClickHouse 连接到的 ZooKeeper 节点的端口。
-   `index` ([Nullable(UInt8)](../../sql-reference/data-types/int-uint.md)) — ClickHouse 连接到的 ZooKeeper 节点的索引。该索引来自 ZooKeeper 配置。如果未连接，则此列为 NULL。
