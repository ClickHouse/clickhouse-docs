包含已执行 DROP TABLE 的表的信息，但尚未进行数据清理。
...
- `metadata_dropped_path` ([String](../../sql-reference/data-types/string.md)) — metadata_dropped 目录中表的元数据文件的路径。
- `table_dropped_time` ([DateTime](../../sql-reference/data-types/datetime.md)) — 安排下次尝试移除表数据的时间。通常是表被删除后加上 `database_atomic_delay_before_drop_table_sec` 的时间。
