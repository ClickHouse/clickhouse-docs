...
包含执行查询的线程的信息，例如，线程名称、线程开始时间、查询处理持续时间。
...
您可以使用 [log_queries_probability](/operations/settings/settings#log_queries_probability) 设置来减少注册在 `query_thread_log` 表中的查询数量。
...
- `written_rows` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 对于 `INSERT` 查询，已写入的行数。对于其他查询，列值为 0。
- `written_bytes` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 对于 `INSERT` 查询，已写入的字节数。对于其他查询，列值为 0。
- `memory_usage` ([Int64](../../sql-reference/data-types/int-uint.md)) — 此线程中分配和释放的内存量之间的差异。
- `peak_memory_usage` ([Int64](../../sql-reference/data-types/int-uint.md)) — 此线程中分配和释放的内存量之间的最大差异。
...
- `master_thread_id` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 初始线程的操作系统初始 ID。
...
- `initial_address` ([IPv6](../../sql-reference/data-types/ipv6.md)) — 父查询启动时的 IP 地址。
...
- `quota_key` ([String](../../sql-reference/data-types/string.md)) — 在 [quotas](../../operations/quotas.md) 设置中指定的“配额键”（请参阅 `keyed`）。
- `revision` ([UInt32](../../sql-reference/data-types/int-uint.md)) — ClickHouse 修订版本。
- `ProfileEvents` ([Map(String, UInt64)](../../sql-reference/data-types/array.md)) — 测量此线程各种指标的 ProfileEvents。关于它们的描述可以在 [system.events](/operations/system-tables/events) 表中找到。
...
- [system.query_log](/operations/system-tables/query_log) — 描述 `query_log` 系统表的文档，该表包含有关查询执行的常见信息。
- [system.query_views_log](/operations/system-tables/query_views_log) — 此表包含查询期间执行的每个视图的信息。
