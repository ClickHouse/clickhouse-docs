...
keywords: ['系统数据库', 'information_schema']
slug: /operations/system-tables/information_schema
title: 'INFORMATION_SCHEMA'
doc_type: 'reference'
---

`INFORMATION_SCHEMA`（或：`information_schema`）是一个系统数据库，它为数据库对象的元数据提供一个（某种程度上）标准化的、[与 DBMS 无关的视图](https://en.wikipedia.org/wiki/Information_schema)。`INFORMATION_SCHEMA` 中的视图在功能上通常不如普通系统表完备，但各种工具可以使用它们以跨不同 DBMS 的方式获取基本信息。`INFORMATION_SCHEMA` 中视图的结构和内容应当以向后兼容的方式演进，即只添加新功能，而不更改或删除现有功能。从内部实现的角度来看，`INFORMATION_SCHEMA` 中的视图通常映射到普通系统表，例如 [system.columns](../../operations/system-tables/columns.md)、[system.databases](../../operations/system-tables/databases.md) 和 [system.tables](../../operations/system-tables/tables.md)。
...
出于与其他数据库兼容性的考虑，提供了不区分大小写的等效视图，例如 `INFORMATION_SCHEMA.columns`。这些视图中的所有列也同样如此——同时提供小写（例如 `table_name`）和大写（`TABLE_NAME`）两种形式。


## COLUMNS {#columns}

包含从 [system.columns](../../operations/system-tables/columns.md) 系统表读取的列,以及 ClickHouse 不支持或无实际意义(始终为 `NULL`)但按标准必须存在的列。
...
包含从 [system.databases](../../operations/system-tables/databases.md) 系统表读取的列,以及 ClickHouse 不支持或无实际意义的列(始终为 `NULL`),但根据标准必须存在。
...
包含从 [system.tables](../../operations/system-tables/tables.md) 系统表中读取的列,当使用 [View](../../engines/table-engines/special/view.md) 表引擎时。
...
- `is_insertable_into` ([Enum8](../../sql-reference/data-types/enum.md)) — 显示创建的视图是否为[物化视图](/sql-reference/statements/create/view#materialized-view)。可能的值:
  - `NO` — 创建的视图不是物化视图。
  - `YES` — 创建的视图是物化视图。
...
- `table_schema` ([String](../../sql-reference/data-types/string.md)) — 表所属的模式（数据库）名称。
- `table_name` ([String](../../sql-reference/data-types/string.md)) — 包含该约束的表名称。
- `column_name` ([Nullable](../../sql-reference/data-types/nullable.md)([String](../../sql-reference/data-types/string.md))) — 包含该约束的列名称。
...
包含外键的相关信息。目前返回空结果(无数据行),这足以提供与 Tableau Online 等第三方工具的兼容性。
...
提供有关表索引的信息。目前返回空结果(无行),仅用于提供与第三方工具(如 Tableau Online)的兼容性。
