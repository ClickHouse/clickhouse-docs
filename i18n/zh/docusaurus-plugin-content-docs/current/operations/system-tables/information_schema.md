...
包含从 [system.columns](../../operations/system-tables/columns.md) 系统表中读取的列，以及 ClickHouse 中不受支持或无实际意义 (始终为 `NULL`) ，但按标准要求必须存在的列。
...
* `constraint_schema` ([String](../../sql-reference/data-types/string.md)) — 约束所属schema (数据库) 的名称。
...
* `table_schema` ([String](../../sql-reference/data-types/string.md)) — 表所属schema (数据库) 的名称。
