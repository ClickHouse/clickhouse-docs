---
description: '系统表，包含 `system.asynchronous_metrics` 的历史值，这些值会按时间间隔保存（默认每秒一次）'
...
包含 `system.asynchronous_metrics` 的历史值，这些值按时间间隔（默认为每秒一次）保存。默认开启。
...
* [asynchronous&#95;metric&#95;log 设置](../../operations/server-configuration-parameters/settings.md#asynchronous_metric_log) — 启用或禁用该设置。
* [system.asynchronous&#95;metrics](../system-tables/asynchronous_metrics.md) — 包含在后台定期计算的指标。
* [system.metric&#95;log](../system-tables/metric_log.md) — 包含来自 `system.metrics` 和 `system.events` 表的指标值历史记录，并会定期写入磁盘。
