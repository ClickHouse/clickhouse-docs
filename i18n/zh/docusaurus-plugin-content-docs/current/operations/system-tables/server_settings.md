...
包含关于服务器的全局设置的信息，这些设置在 `config.xml` 中指定。 目前，该表仅显示 `config.xml` 第一个层级的设置，并且不支持嵌套配置（例如 [logger](../../operations/server-configuration-parameters/settings.md#logger)）。
...
- `changed` ([UInt8](/sql-reference/data-types/int-uint#integer-ranges)) — 显示该设置是否在 `config.xml` 中指定。
- `description` ([String](../../sql-reference/data-types/string.md)) — 简短的服务器设置描述。
...
以下示例演示如何获取名称包含 `thread_pool` 的服务器设置的信息。
...
使用 `WHERE changed` 是有用的，例如，当您想检查配置文件中的设置是否正确加载并在使用中时。
