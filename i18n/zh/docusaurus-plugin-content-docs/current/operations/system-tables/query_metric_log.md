...
包含来自表 `system.events` 的个别查询的内存和指标值的历史记录，定期刷新到磁盘。

一旦查询开始，数据将在 `query_metric_log_interval` 毫秒的周期性间隔内收集（默认设置为 1000）。如果查询持续的时间超过 `query_metric_log_interval`，则在查询完成时也会收集数据。
