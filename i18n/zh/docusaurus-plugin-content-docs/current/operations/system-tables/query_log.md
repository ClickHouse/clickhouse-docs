...
可以通过设置 [log_queries = 0](/operations/settings/settings#log_queries) 来禁用查询日志记录。不建议关闭日志记录，因为此表中的信息对于排查问题非常重要。
...
* `query_start_time_microseconds` ([DateTime64](../../sql-reference/data-types/datetime64.md)) — 查询开始执行的时间，微秒级精度。
...
* `result_rows` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — `SELECT` 查询结果中的行数，或 `INSERT` 查询中写入的行数。
* `result_bytes` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 用于在 RAM 中存储查询结果的字节数。
* `memory_usage` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 查询消耗的内存量。
...
* `formatted_query` ([String](../../sql-reference/data-types/string.md)) — 格式化后的查询字符串。
* `normalized_query_hash` ([UInt64](/sql-reference/data-types/int-uint#integer-ranges)) — 一个数值哈希值，对于仅在字面量取值上不同的查询，该值相同。
...
* `connection_address` ([IPv6](../../sql-reference/data-types/ipv6.md)) — 发起该连接的客户端 IP 地址。如果通过代理连接，则为代理的地址。
* `connection_port` ([UInt16](../../sql-reference/data-types/int-uint.md)) — 建立该连接的客户端端口。如果是通过代理连接，则该端口为代理端口。
...
* `initial_query_start_time_microseconds` ([DateTime64](../../sql-reference/data-types/datetime64.md)) — 分布式查询执行时的初始查询开始时间，具有微秒级精度。
* `interface` ([UInt8](../../sql-reference/data-types/int-uint.md)) — 发起该查询的接口。可能的取值：
  * 1 — TCP。
  * 2 — HTTP。
* `os_user` ([String](../../sql-reference/data-types/string.md)) — 运行 [clickhouse-client](../../interfaces/client.md) 的操作系统用户名。
* `client_hostname` ([String](../../sql-reference/data-types/string.md)) — 客户端所在机器的主机名，即运行 [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端的那台机器的主机名。
* `client_name` ([String](../../sql-reference/data-types/string.md)) — [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端名称。
* `client_revision` ([UInt32](../../sql-reference/data-types/int-uint.md)) — [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端的修订版本号。
* `client_version_major` ([UInt32](../../sql-reference/data-types/int-uint.md)) — [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端的主版本号。
* `client_version_minor` ([UInt32](../../sql-reference/data-types/int-uint.md)) — [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端的次要版本号。
* `client_version_patch` ([UInt32](../../sql-reference/data-types/int-uint.md)) — [clickhouse-client](../../interfaces/client.md) 或其他 TCP 客户端的补丁版本号。
* `script_query_number` ([UInt32](../../sql-reference/data-types/int-uint.md)) — 在供 [clickhouse-client](../../interfaces/client.md) 使用的包含多个查询的脚本中，该查询的序号。
* `script_line_number` ([UInt32](../../sql-reference/data-types/int-uint.md)) — 在包含多个查询、供 [clickhouse-client](../../interfaces/client.md) 使用的脚本中的该查询起始行号。
* `http_method` (UInt8) — 发起查询的 HTTP 方法。可能的取值如下：
  * 0 — 查询通过 TCP 接口发起。
  * 1 — 使用了 `GET` 方法。
  * 2 — 使用了 `POST` 方法。
* `http_user_agent` ([String](../../sql-reference/data-types/string.md)) — 在 HTTP 查询中传递的 HTTP 请求头 `UserAgent`。
