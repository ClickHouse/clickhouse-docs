...
包含有关致命错误堆栈跟踪的信息。该表在数据库中默认不存在，仅在发生致命错误时才会创建。
...
* `timestamp_ns` ([UInt64](../../sql-reference/data-types/int-uint.md)) — 具有纳秒精度的事件时间戳。
...
* `trace_full` ([Array](../../sql-reference/data-types/array.md)([String](../../sql-reference/data-types/string.md))) — 崩溃时的堆栈跟踪。每个元素包含 ClickHouse 服务器进程中被调用的方法。
...
* `build_id` ([String](../../sql-reference/data-types/string.md)) — 由编译器生成的 BuildID。
