...
来自 ICU 的 Unicode 码点属性名称会被转换为 `snake_case`，并用作列名。

## 列 \{#columns\}

{/*AUTOGENERATED_START*/ }

* `code_point` ([String](../../sql-reference/data-types/)) — 以 U+XXXX 表示的 Unicode 码点。
* `code_point_value` ([Int32](../../sql-reference/data-types/)) — Unicode 码点的整数值。
* `notation` ([String](../../sql-reference/data-types/)) — 字符记法 (码点的可视化表示) 。
