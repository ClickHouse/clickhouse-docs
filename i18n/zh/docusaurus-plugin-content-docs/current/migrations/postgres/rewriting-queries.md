...
> 这是关于从 PostgreSQL 迁移到 ClickHouse 的指南的 **第二部分**。通过一个实际的例子，它演示了如何使用实时复制（CDC）方法高效地进行迁移。许多所涵盖的概念同样适用于从 PostgreSQL 到 ClickHouse 的手动批量数据传输。

您在 PostgreSQL 设置中的大多数 SQL 查询应不经修改即可在 ClickHouse 中运行，并且可能执行得更快。

## 使用 CDC 进行去重 {#deduplication-cdc}

使用实时复制与 CDC 时，请记住，更新和删除可能导致重复的行。为了管理这些情况，您可以使用涉及视图和可刷新的物化视图的技术。

请参考此[指南](/integrations/clickpipes/postgres/deduplication#query-like-with-postgres)，了解如何在使用实时复制和 CDC 迁移应用程序时最大限度地减少摩擦，将其从 PostgreSQL 迁移到 ClickHouse。

## 在 ClickHouse 中优化查询 {#optimize-queries-in-clickhouse}

虽然可以在最小化查询重写的情况下进行迁移，但建议利用 ClickHouse 的特性来显著简化查询并进一步提高查询性能。

这里的示例涵盖了常见的查询模式，并展示了如何使用 ClickHouse 对它们进行优化。它们使用了完整的 [Stack Overflow 数据集](/getting-started/example-datasets/stackoverflow)（截至 2024 年 4 月）与 PostgreSQL 和 ClickHouse 中的等效资源（8 核心，32GiB RAM）。

> 为了简单起见，下面的查询省略了去重数据的技术。

> 这里的计数会略有不同，因为 Postgres 数据仅包含满足外键引用完整性的行。ClickHouse 不施加此类约束，因此拥有完整的数据集，例如，包括匿名用户。

获取最多查看次数的用户（问答数量超过 10）：
...
如有可能，用户应利用 ClickHouse 的聚合函数。下面我们展示了如何使用 [argMax](/sql-reference/aggregate-functions/reference/argmax) 函数来计算每年观看次数最多的问题。
...
条件和数组函数使查询变得显著简便。以下查询计算标签（出现次数超过 10000）在 2022 年到 2023 年间增长百分比最大的标签。请注意，由于有条件、数组函数并且能够在 HAVING 和 SELECT 子句中重用别名，以下 ClickHouse 查询变得简洁。
