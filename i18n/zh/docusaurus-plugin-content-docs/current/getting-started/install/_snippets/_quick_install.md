...
如果您无需在生产环境中安装 ClickHouse,最快捷的安装方式是使用 curl 运行安装脚本。该脚本会自动为您的操作系统选择合适的二进制文件。
...
在服务器启动并运行后,打开一个新的终端窗口并运行以下命令来启动 `clickhouse-client`:
...
表数据存储在当前目录中,ClickHouse 服务器重启后数据仍然可用。如有必要,您可以将 `-C config.xml` 作为额外的命令行参数传递给 `./clickhouse server`,并在配置文件中提供进一步的配置。所有可用的配置设置都记录在[此处](/operations/server-configuration-parameters/settings)以及[示例配置文件模板](https://github.com/ClickHouse/ClickHouse/blob/master/programs/server/config.xml)中。
