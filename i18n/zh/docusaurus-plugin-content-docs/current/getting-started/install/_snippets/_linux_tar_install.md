...
# 使用 tgz 归档安装 ClickHouse

> 建议使用官方预编译的 `tgz` 归档来安装所有 Linux 发行版，特别是在无法安装 `deb` 或 `rpm` 包的情况下。

<VerticalStepper>

## 下载并安装最新的稳定版本 {#install-latest-stable}

所需版本可以使用 `curl` 或 `wget` 从库 https://packages.clickhouse.com/tgz/ 下载。
下载后，应解压下载的归档并使用安装脚本进行安装。

以下是安装最新稳定版本的示例。

:::note
对于生产环境，建议使用最新的 `stable` 版本。
您可以在这个 [GitHub 页面](https://github.com/ClickHouse/ClickHouse/tags) 上找到 release 版本号，后缀为 `-stable`。
:::
...
下载每个 ClickHouse 组件的 tarball。循环首先尝试架构特定的包，然后回退到通用包。
