...
[Docker Hub](https://hub.docker.com/r/clickhouse/clickhouse-server/) 上的指南
为方便阅读在下方进行了重现。可用的 Docker 镜像基于官方 ClickHouse deb 包构建。
...
ClickHouse 配置使用 "config.xml" 文件表示（[文档](https://clickhouse.com/docs/operations/configuration_files/)）
...
要在基于此镜像的派生镜像中执行额外的初始化操作,请在 `/docker-entrypoint-initdb.d` 目录下添加一个或多个 `*.sql`、`*.sql.gz` 或 `*.sh` 脚本。入口点调用 `initdb` 后,会依次运行该目录中的所有 `*.sql` 文件、执行所有可执行的 `*.sh` 脚本,并 source 所有不可执行的 `*.sh` 脚本,以便在启动服务前完成进一步的初始化。  
