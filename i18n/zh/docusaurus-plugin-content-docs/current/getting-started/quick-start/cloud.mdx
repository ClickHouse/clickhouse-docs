...
<VerticalStepper type="numbered">
  ...
  * 使用刚刚创建的用户名和密码登录
  ...
  默认情况下,新组织将采用 Scale 层级,并创建 3 个副本,每个副本配置 4 个 vCPUs 和 16 GiB 内存。Scale 层级默认启用[垂直自动扩展](/cloud/features/autoscaling/vertical)。您可以稍后在&#39;Plans&#39;页面更改组织层级。
  ...
  * 数据是否被复制
  ...
  有多种表引擎可供选择，但对于单节点 ClickHouse 服务端上的简单表，建议使用 [`MergeTree`](/engines/table-engines/mergetree-family/mergetree.md)。
  :::
  ...
  * 在 ClickHouse 中，表中的主键对每一&#x884C;***都不是唯一的***
