...
> 在本快速入门教程中，我们将通过 8 个简单步骤帮助你完成开源版 ClickHouse 的设置。你将下载适用于你操作系统的二进制文件，学习如何运行 ClickHouse server，并使用 ClickHouse client 创建一张表，然后向其中插入数据并运行查询来选取这些数据。
>
> 你要找的是 Cloud 快速开始吗？[前往此处](/get-started/quick-start)。
...
<VerticalStepper>
  ...
  您可以使用熟悉的 `INSERT INTO TABLE` 命令操作 ClickHouse,但需要理解的是,每次向 `MergeTree` 表插入数据都会在存储中创建我们所称的 **分区片段**。这些 parts 随后会由 ClickHouse 在后台进行合并。
  ...
  <Tabs groupId="read_data">
    <TabItem value="S3" label="S3" default>
      使用 [`s3` 表函数](/sql-reference/table-functions/s3.md) 来
      从 S3 中读取文件。它是一个表函数，这意味着其返回结果是一个表，
      该表可以：

      1. 作为 `SELECT` 查询的数据源 (允许你运行即席查询，同时
         将数据保留在 S3 中) ，或者……
      2. 将该结果表插入到一个 `MergeTree` 表中 (当你准备好将
         数据迁移到 ClickHouse 中时)
      ...
      查看我们的 [AWS S3 文档页面汇总](/integrations/data-ingestion/s3/index.md)，了解更多关于将 S3 与 ClickHouse 配合使用的详细说明和示例。

      <br />
    </TabItem>

    <TabItem value="GCS" label="GCS">
      用于在 AWS S3 中读取数据的 [`s3` 表函数](/sql-reference/table-functions/s3.md) 也同样适用于 Google Cloud Storage 中的文件。
    ...
    <TabItem value="local_file" label="本地">
      使用 [`file` 表引擎](/sql-reference/table-functions/file) 来
      读取本地文件。为方便起见，将文件复制到 `user_files` 目录
      (该目录位于您下载 ClickHouse 二进制文件的目录下) 。
      ...
      请注意，ClickHouse 会通过分析大量行数据来推断列名和数据类型。如果 ClickHouse 无法从文件名中确定文件格式，则可以将其作为第二个参数进行指定：
      ...
      有关更多详细信息，请参阅 [`file` 表函数](/sql-reference/table-functions/file)
      的文档页面。
    ...
    <TabItem value="其他数据库管理系统" label="ODBC/JDBC">
      ...
      和 [`jdbc` 表函数](/sql-reference/table-functions/jdbc) 文档页面，了解更多详细信息。
    ...
    <TabItem value="其他" label="其他">
      查看我们的[ClickHouse 集成完整列表](/integrations)，以了解如何将您现有的框架和数据源连接到 ClickHouse。
  ...
  * 查看我们的[核心概念](/managing-data/core-concepts)部分，了解一些 ClickHouse 内部工作机制的基本原理。
  * 请查看 [进阶教程](tutorial.md)，该教程将更深入地探讨 ClickHouse 的核心概念和功能。
  * 通过在 [ClickHouse Academy](https://learn.clickhouse.com/visitor_class_catalog) 参加我们的免费按需培训课程来继续学习。
