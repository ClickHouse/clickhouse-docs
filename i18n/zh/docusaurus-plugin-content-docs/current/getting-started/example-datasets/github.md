...
这对于后续分析非常重要，因为我们只想考虑仓库中当前存在的文件。我们将当前有效文件近似定义为那些尚未被重命名或删除 (然后再次添加/重新命名) 的文件。
...
### 代码仓库中代码年龄的分布 \{#distribution-of-code-age-across-repository\}

我们仅分析当前文件。为简洁起见，结果限制为深度 2，且每个根文件夹只显示 5 个文件。可按需调整。
