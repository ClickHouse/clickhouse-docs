...
用户可以下载预先生成的 Parquet 格式数据版本（包含截至 2024 年 4 月的所有帖子），或者下载最新的 XML 格式数据并自行加载。Stack Overflow 会定期更新这些数据——历史上通常为每 3 个月更新一次。

下图展示了在假定使用 Parquet 格式时，可用数据表的模式（schema）。

<Image img={stackoverflow} alt="Stack Overflow schema" size="md" />

可以在[此处](https://meta.stackexchange.com/questions/2677/database-schema-documentation-for-the-public-data-dump-and-sede)找到该数据模式的说明。


## 预准备数据 {#pre-prepared-data}

我们提供了该数据的 Parquet 格式副本,数据更新至 2024 年 4 月。虽然对于 ClickHouse 来说行数相对较少(6000 万条帖子),但该数据集包含大量文本和较大的 String 列。

```sql
CREATE DATABASE stackoverflow
```

以下时间统计基于位于 `eu-west-2` 的 96 GiB、24 vCPU ClickHouse Cloud 集群。数据集位于 `eu-west-3`。
...
帖子数据也可按年份获取,例如 [https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2020.parquet](https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2020.parquet)
...
这些文件最大可达 35GB,根据网络连接情况,下载可能需要约 30 分钟 - 下载服务器的限速约为 20MB/秒。

### 转换为 JSON {#convert-to-json}

截至本文撰写时,ClickHouse 尚不原生支持 XML 作为输入格式。要将数据加载到 ClickHouse 中,我们首先需要将其转换为 NDJSON。

要将 XML 转换为 JSON,我们推荐使用 [`xq`](https://github.com/kislyuk/yq) Linux 工具,这是一个用于 XML 文档的简单 `jq` 包装器。
...
以下步骤适用于上述任何文件。我们以 `stackoverflow.com-Posts.7z` 文件为例。请根据需要进行修改。
...
以下操作将 xml 文件拆分为多个文件,每个文件包含 10000 行。
...
运行上述命令后，用户将得到一组文件，每个文件包含 10000 行。这样可以确保下一条命令的内存开销不会过大（从 XML 到 JSON 的转换是在内存中完成的）。
...
使用以下命令将其加载到 ClickHouse 中。请注意，这里为 `posts.json` 文件显式指定了 schema，需要根据各字段的数据类型进行调整，以与目标表的定义保持一致。
...
我们感谢 Stack Overflow 在 `cc-by-sa 4.0` 许可证下提供此数据,并对其所做的贡献以及数据原始来源 [https://archive.org/details/stackexchange](https://archive.org/details/stackexchange) 表示感谢。
