...
该数据集中的数据来源于完整的 OpenSky 数据集，并经过清理，以展示 COVID-19 大流行期间航空交通的发展。它涵盖了自 2019 年 1 月 1 日以来，网络上的 2500 多名成员所见的所有航班。直到 COVID-19 大流行结束，数据集将定期包含更多数据。

来源: https://zenodo.org/records/5092942

Martin Strohmeier, Xavier Olive, Jannis Luebbe, Matthias Schaefer 和 Vincent Lenders
“来自 OpenSky 网络的众包航空交通数据 2019–2020”
...
在良好的互联网连接下，下载大约需要 2 分钟。总共有 30 个文件，总大小为 4.3 GB。
...
`xargs -P100` 指定最多使用 100 个并行工作进程，但由于我们只有 30 个文件，因此工作进程的数量将只有 30。
- 对于每个文件，`xargs` 将运行一个带有 `bash -c` 的脚本。该脚本的替换形式为 `{}`，并且 `xargs` 命令会将文件名替换为此（我们已经通过 `-I{}` 请求了 `xargs`）。
- 该脚本会将文件解压（`gzip -c -d "{}"`）到标准输出（`-c` 参数），并将输出重定向到 `clickhouse-client`。
- 我们还要求使用扩展解析器解析 [DateTime](../../sql-reference/data-types/datetime.md) 字段（[--date_time_input_format best_effort](/operations/settings/formats#date_time_input_format)）以识别带有时区偏移的 ISO-8601 格式。

最后，`clickhouse-client` 将执行插入。它会以 [CSVWithNames](../../interfaces/formats.md#csvwithnames) 格式读取输入数据。
...
数据集在 ClickHouse 中的大小仅为 2.66 GiB，检查一下。
...
您可以使用交互式资源 [在线演示平台](https://sql.clickhouse.com) 测试对该数据集的其他查询。例如，[像这样](https://sql.clickhouse.com?query_id=BIPDVQNIGVEZFQYFEFQB7O)。不过，请注意，您无法在此处创建临时表。
