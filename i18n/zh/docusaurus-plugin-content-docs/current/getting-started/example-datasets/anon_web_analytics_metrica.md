...
### 导入 hits 表数据 {#import-the-hits-data}
...
hits 和 visits 数据集用于 ClickHouse 的测试例程，这是测试套件中的一个查询。其余测试在本页末尾的 *下一步* 部分中有说明。
...
[A Practical Introduction to Sparse Primary Indexes in ClickHouse](/guides/best-practices/sparse-primary-indexes.md) 使用 hits 数据集来讨论 ClickHouse 索引与传统关系型数据库索引的差异、ClickHouse 如何构建并使用稀疏主索引，以及索引的最佳实践。
...
:::note
测试套件使用名为 `test` 的数据库，表名为 `hits` 和 `visits`。您可以重命名自己的数据库和表，或者修改测试文件中的 SQL。
:::