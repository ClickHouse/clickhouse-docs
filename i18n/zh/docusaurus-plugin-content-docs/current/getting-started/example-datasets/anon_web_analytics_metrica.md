---
description: '由两个表组成的数据集，包含已匿名化的网站分析数据（hits 和 visits）'
...
# 匿名化的 Web 分析数据 {#anonymized-web-analytics-data}

此数据集由两个表构成，分别存储匿名化的 Web 分析数据中的点击（`hits_v1`）和访问（`visits_v1`）。

这些表可以以压缩的 `tsv.xz` 文件形式下载。除本文中使用的示例数据之外，还提供一个体积为 7.5GB 的 `hits` 表扩展版本，包含 1 亿行数据，可作为 TSV 文件从 [https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz) 下载。
...
### 导入 hits 数据 {#import-the-hits-data}
...
### 下载 visits 表的压缩 TSV 文件 {#download-the-visits-compressed-tsv-file}
...
### 导入 `visits` 表数据 {#import-the-visits-data}
...
hits 和 visits 数据集用于 ClickHouse 的测试程序，下面是测试套件中的一个查询示例。本页末尾的 *后续步骤* 部分中引用了其余测试。
...
[A Practical Introduction to Sparse Primary Indexes in ClickHouse](/guides/best-practices/sparse-primary-indexes.md) 使用 hits 数据集，讨论 ClickHouse 索引与传统关系型数据库索引之间的差异、ClickHouse 如何构建和使用稀疏主键索引，以及索引最佳实践。

针对这些表的其他查询示例可以在 ClickHouse 的 [stateful tests](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql) 中找到。

:::note
测试套件使用名为 `test` 的数据库，表名为 `hits` 和 `visits`。可以重命名您的数据库和数据表，或者编辑测试文件中的 SQL。
:::