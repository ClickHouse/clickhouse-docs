...
星型模式基准大致基于 [TPC-H](tpch.md) 的表和查询，但与 TPC-H 不同的是，它采用星型模式布局。
大部分数据存储在一个巨大的事实表中，周围是多个小型维度表。
查询会将事实表与一个或多个维度表进行关联，以应用筛选条件，例如 `MONTH = 'JANUARY'`。
...
然后生成数据。参数 `-s` 用于指定规模因子。比如，当使用 `-s 100` 时，将生成 6 亿行数据。
...
在许多 ClickHouse 的使用场景中，会将多张表转换为一张去范式的扁平表。
此步骤是可选的，下面列出了查询的原始形式以及为去范式表改写后的形式。
