---
description: '星型模式基准（SSB）数据集及查询'
sidebar_label: '星型模式基准'
slug: /getting-started/example-datasets/star-schema
title: '星型模式基准（SSB，2009）'
doc_type: 'guide'
keywords: ['示例数据集', '星型模式', '示例数据', '数据建模', '基准测试']
---

星型模式基准大致基于 [TPC-H](tpch.md) 的表和查询，但与 TPC-H 不同，它采用星型模式结构。
大部分数据存放在一个巨大的事实表中，其周围是多个小型维度表。
查询通过将事实表与一个或多个维度表进行连接来应用过滤条件，例如 `MONTH = 'JANUARY'`。
...
然后生成数据。参数 `-s` 用于指定数据规模因子。例如，当使用 `-s 100` 时，会生成 6 亿行数据。
...
在 ClickHouse 的许多使用场景中，会将多张表转换为一张去范式化的扁平表。
这一步是可选的，下面的查询既以其原始形式列出，也提供了针对去范式化扁平表改写后的形式。
...
这些查询由 `./qgen -s <scaling_factor>` 生成。对于 `s = 100`，示例查询为：
