...
> 在本教程中，你将把 2800 万行 Hacker News 数据从 CSV 和 Parquet 两种格式导入 ClickHouse
> 表中，并运行一些简单查询来探索这些数据。
...
<VerticalStepper headerLevel="h3">
  ...
  [`clickhouse-local`](/operations/utilities/clickhouse-local/) 允许用户对本地文件进行快速处理,而无需部署和配置 ClickHouse 服务端。
  ...
  最重要的是,schema 会自动从文件内容中推断出来。
  另请注意,`clickhouse-local` 能够读取压缩文件,并通过扩展名推断出 gzip 格式。
  `Vertical` 格式用于更直观地查看每列的数据。

  ### 使用 schema inference 加载数据

  用于数据加载的最简单且最强大的工具是 `clickhouse-client`:一个功能丰富的原生命令行客户端。
  加载数据时，您可以再次利用 schema 推断，由 ClickHouse 自动确定各列的数据类型。

  运行以下命令创建表并直接从远程 CSV 文件插入数据,通过 [`url`](/sql-reference/table-functions/url) 函数访问文件内容。
  schema 会自动推断:
  ...
  运行以下查询以删除旧 schema 并创建改进后的 schema:
...
<VerticalStepper headerLevel="h3">
  ...
  与之前处理 CSV 文件时一样,您可以手动指定 schema 以更精确地控制所选类型,并直接从 S3 插入数据:
