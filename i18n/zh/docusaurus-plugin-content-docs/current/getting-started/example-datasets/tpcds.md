...
我们尽可能严格遵循 TPC-DS 规范的规则。
规范中的抽象数据类型映射到 ClickHouse 的原生数据类型如下：

| TPC-DS Type    | ClickHouse Type                  |
| -------------- | -------------------------------- |
| `identifier`   | `Int64`（或用于日期/时间维度键的 `UInt32`）   |
| `integer`      | `Int32`                          |
...
关于可空性，在规范中标记为 &quot;N&quot;（not null）的列不会使用 `Nullable` 包装，因为它们从不包含空值。
所有其他列都使用 `Nullable()` 包装。
对于字符串类型，`Nullable` 放置在 `LowCardinality` 内部，例如 `LowCardinality(Nullable(String))`。
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/95299，此查询在当前环境中无法直接使用。下面这个做了少量修改的替代写法可以正常工作：
...
::::note
当仅对单个值调用 `stddev_samp` 时，该查询会返回 `nan` 而不是 `NULL`。对应的 Issue：https://github.com/ClickHouse/ClickHouse/issues/94683。除此之外，结果是正确的。
::::
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/95299，该查询无法直接使用。下面这个带有小幅修正的替代写法可以正常工作：
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/94858，此查询不能直接使用。下面这个稍作修改的替代写法可以正常工作：
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/94858，该查询无法直接使用。如下这个经过小幅修改的替代写法可以正常工作：
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/94976，该查询无法直接使用。下面这一经过轻微修改的替代写法可以正常运行：
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/94671，这条查询语句无法直接使用。下面这个稍作修改的替代写法可以正常工作：
...
::::note
由于 https://github.com/ClickHouse/ClickHouse/issues/95299，该查询无法直接开箱即用。这个带有小幅修正的替代写法可以正常工作：
