---
description: '超过 1.5 亿条 Amazon 产品的客户评论'
sidebar_label: 'Amazon 客户评论'
slug: /getting-started/example-datasets/amazon-reviews
title: 'Amazon 客户评论'
doc_type: 'guide'
keywords: ['Amazon reviews', '客户评论数据集', '电商数据', '示例数据集', '快速入门']
---

该数据集包含超过 1.5 亿条 Amazon 产品的客户评论。数据以 Snappy 压缩的 Parquet 文件形式存储在 AWS S3 中，压缩后总大小为 49GB。下面我们将逐步演示如何将其导入 ClickHouse。

:::note
下文中的查询是在 ClickHouse Cloud 的 **Production** 实例上执行的。欲了解更多信息，请参阅
...
1. 无需将数据插入 ClickHouse,我们可以直接就地查询。让我们获取一些行数据,查看它们的内容:
...
review_headline:   保护壳坚固且符合我的保护需求
...
3. 以下 `INSERT` 命令使用 `s3Cluster` 表函数,该函数允许使用集群的所有节点并行处理多个 S3 文件。我们还使用通配符来插入所有以 `https://datasets-documentation.s3.eu-west-3.amazonaws.com/amazon_reviews/amazon_reviews_*.snappy.parquet` 开头的文件:
...
:::tip
在 ClickHouse Cloud 中，集群名称为 `default`。请将 `default` 修改为你的集群名称；如果你没有集群，则使用 `s3` 表函数（而不是 `s3Cluster`）。
:::

5. 该查询耗时不长——平均每秒处理约 300,000 行。大约 5 分钟左右你就应该能看到所有行都已插入：
...
原始数据约为 70G，但在 ClickHouse 中压缩后只占用约 30G 的空间。
