...
2. 让我们定义一个新的 `MergeTree` 表 `amazon_reviews` 来存储这些数据到 ClickHouse：
...
3. 以下的 `INSERT` 命令使用了 `s3Cluster` 表函数，它允许使用集群中的所有节点并行处理多个 S3 文件。我们还使用了通配符插入任何以 `https://datasets-documentation.s3.eu-west-3.amazonaws.com/amazon_reviews/amazon_reviews_*.snappy.parquet` 开头的文件：
...
5. 这个查询不会花太长时间——每秒大约 300,000 行。大约 5 分钟后，您应该会看到所有行被插入：
...
7. 让我们运行一些查询……以下是数据集中最有帮助的前10条评论：
...
10 行结果。耗时: 0.897 秒。处理了 150.96百万 行，15.36 GB (168.36百万行/秒，17.13 GB/秒。)
```

8. 以下是 Amazon 上评论最多的前10款产品：
...
10 行结果。耗时: 20.059 秒。处理了 150.96百万 行，12.78 GB (7.53百万行/秒，637.25 MB/秒。)
```

9. 以下是每个产品每个月的平均评论评分（实际上是 [Amazon 职位面试问题](https://datalemur.com/questions/sql-avg-review-ratings)!）：
...
20 行结果。耗时: 43.055 秒。处理了 150.96百万 行，13.24 GB (3.51百万行/秒，307.41 MB/秒。)
峰值内存使用: 41.73 GiB。
```

10. 下面是每个产品类别的总投票数。这个查询快速，因为 `product_category` 在主键中：
...
43 行结果。耗时: 0.201 秒。处理了 150.96百万 行，754.79 MB (750.85百万行/秒，3.75 GB/秒。)
```

11. 让我们找到在评论中 **"awful"** 一词出现频率最高的产品。这是一项大任务 - 需要解析超过 151M 字符串查找单一单词：
...
50 行结果。耗时: 4.072 秒。处理了 150.96百万 行，68.93 GB (37.07百万行/秒，16.93 GB/秒。)
峰值内存使用: 1.82 GiB。
```

12. 我们可以再运行相同的查询，只不过这次我们在评论中搜索 **awesome**：
...
50 行结果。耗时: 4.079 秒。处理了 150.96百万 行，68.95 GB (37.01百万行/秒，16.90 GB/秒。)
峰值内存使用: 2.18 GiB。
```
