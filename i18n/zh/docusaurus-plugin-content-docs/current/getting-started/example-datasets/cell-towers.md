---
'description': '学习如何将 OpenCelliD 数据加载到 ClickHouse，连接 Apache Superset 到 ClickHouse
  并基于数据构建仪表盘'
'sidebar_label': '地理数据'
'sidebar_position': 3
...
<Image img={cell_towers_1} size="md" alt="按 mcc 204 中的无线电类型显示基站的仪表板"/>

## 获取数据集 {#get-the-dataset}

该数据集来自 [OpenCelliD](https://www.opencellid.org/) - 世界上最大的基站开放数据库。

截至 2021 年，它包含超过 4000 万条关于全球基站（GSM、LTE、UMTS 等）的记录，以及它们的地理坐标和元数据（国家代码、网络等）。

OpenCelliD 项目遵循知识共享署名-相同方式共享 4.0 国际许可，并且我们根据相同许可条款重新分发该数据集的快照。数据集的最新版本可在登录后下载。
...
ClickHouse Cloud 提供了一个简单的按钮，可以从 S3 上传此数据集。登录到您的 ClickHouse Cloud 组织，或在 [ClickHouse.cloud](https://clickhouse.cloud) 创建一个免费试用帐户。
<ActionsMenu menu="Load Data" />

在 **示例数据** 选项卡中选择 **基站** 数据集，然后 **加载数据**：

<Image img={cloud_load_data_sample} size='md' alt='加载基站数据集' />

### 检查 cell_towers 表的架构 {#examine-the-schema-of-the-cell_towers-table}
```sql
...
2. 按 [移动国家代码 (MCC)](https://en.wikipedia.org/wiki/Mobile_country_code) 计算基站：
...
您可能想在 ClickHouse 中创建一个 [字典](../../sql-reference/dictionaries/index.md) 来解码这些值。
...
## 架构审查 {#review-of-the-schema}

在 Superset 中构建可视化之前，请查看您将使用的列。该数据集主要提供全球移动基站的位置信息（经度和纬度）和无线电类型。列的描述可以在 [社区论坛](https://community.opencellid.org/t/documenting-the-columns-in-the-downloadable-cells-database-csv/186) 中找到。用于可视化的列描述如下：

以下是来自 OpenCelliD 论坛的列描述：
...
:::tip mcc
要查找您的 MCC，请查看 [移动网络代码](https://en.wikipedia.org/wiki/Mobile_country_code)，并在 **移动国家代码** 列中使用这三位数字。
:::

该表的架构是为在磁盘上的紧凑存储和查询速度设计的。
- `radio` 数据作为 `Enum8` (`UInt8`) 存储，而不是字符串。
- `mcc` 或移动国家代码，存储为 `UInt16`，因为我们知道范围是 1 - 999。
- `lon` 和 `lat` 为 `Float64`。

本指南中的查询或可视化未使用其他字段，但如果您感兴趣，可以在上面链接的论坛中找到它们的描述。

## 使用 Apache Superset 构建可视化 {#build-visualizations-with-apache-superset}

Superset 可以很方便地从 Docker 运行。如果您已经运行 Superset，您只需使用 `pip install clickhouse-connect` 将 ClickHouse 连接添加到其中。如果您需要安装 Superset，请直接打开 **在 Docker 中启动 Apache Superset**。

<SupersetDocker />

要使用 OpenCelliD 数据集构建 Superset 仪表板，您应该：
- 将您的 ClickHouse 服务作为 Superset **数据库** 添加
- 将表 **cell_towers** 作为 Superset **数据集** 添加
- 创建一些 **图表**
- 将图表添加到 **仪表板**

### 将 ClickHouse 服务添加为 Superset 数据库 {#add-your-clickhouse-service-as-a-superset-database}

<ConnectionDetails />

在 Superset 中，可以通过选择数据库类型并提供连接详细信息来添加数据库。打开 Superset，查找 **+** 符号，它有一个菜单，包含 **数据** 和 **连接数据库** 选项。
...
:::note
如果 **ClickHouse Connect** 不是您的选项之一，那么您需要安装它。命令是 `pip install clickhouse-connect`，更多信息可以在 [这里](https://pypi.org/project/clickhouse-connect/) 找到。
:::

#### 添加连接详细信息： {#add-your-connection-details}

:::tip
确保在连接到 ClickHouse Cloud 或其他强制使用 SSL 的 ClickHouse 系统时启用 **SSL**。
:::
...
在 Superset 中，**数据集** 与数据库中的表相映射。单击添加数据集并选择您的 ClickHouse 服务、包含您表的数据库（`default`），并选择 `cell_towers` 表：

<Image img={add_cell_towers_table_as_dataset} size="md" alt="将 cell_towers 表添加为数据集"/>

### 创建一些 **图表** {#create-some-charts}

当您选择在 Superset 中添加图表时，必须指定数据集（`cell_towers`）和图表类型。由于 OpenCelliD 数据集为基站提供经度和纬度坐标，因此我们将创建一个 **地图** 图表。 **deck.gL 散点图** 类型适合此数据集，因为它在地图上处理密集数据点表现良好。

<Image img={create_a_map_in_superset} size="md" alt="在 Superset 中创建地图"/>

#### 指定用于地图的查询 {#specify-the-query-used-for-the-map}

deck.gl 散点图需要经度和纬度，并且还可以对查询应用一个或多个过滤器。在此示例中，应用了两个过滤器，一个是用于有 UMTS 无线电的基站，另一个是指分配给荷兰的移动国家代码。
...
<Image img={superset_mcc_2024} size="md" alt="筛选 MCC 204"/>

添加过滤器 `radio` = `'UMTS'`（或替换为任何其他 `radio` 值，您可以在 `DESCRIBE TABLE cell_towers` 的输出中查看选择）：

<Image img={superset_radio_umts} size="md" alt="筛选无线电等于 UMTS"/>

这是针对 `radio = 'UMTS'` 和 `mcc = 204` 的图表完整配置：

<Image img={superset_umts_netherlands} size="md" alt="MCC 204 中 UMTS 无线电的图表"/>

单击 **更新图表** 以渲染可视化。

### 将图表添加到 **仪表板** {#add-the-charts-to-a-dashboard}

此截图显示了 LTE、UMTS 和 GSM 无线电的基站位置。所有图表均以相同方式创建，并添加到仪表板中。

<Image img={superset_cell_tower_dashboard} size="md" alt="按 mcc 204 中的无线电类型显示基站的仪表板"/>

:::tip
数据还可以在 [Playground](https://sql.clickhouse.com) 中进行交互式查询。

此 [示例](https://sql.clickhouse.com?query_id=UV8M4MAGS2PWAUOAYAAARM) 会为您填充用户名甚至查询。

虽然您无法在 Playground 中创建表，但您可以运行所有查询，并且甚至可以使用 Superset（调整主机名和端口号）。
:::
