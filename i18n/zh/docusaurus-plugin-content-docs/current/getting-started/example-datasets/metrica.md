...
该数据集包含两个表，它们包含匿名化的网络分析数据，分别为点击量（`hits_v1`）和访问量（`visits_v1`）。

这些表可以作为压缩的 `tsv.xz` 文件下载。除了本文中使用的示例外，`hits` 表的扩展版本（7.5GB），包含1亿行数据，以TSV格式提供，下载链接为 [https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz)。

## 下载并导入数据 {#download-and-ingest-the-data}

### 下载点击量压缩的TSV文件： {#download-the-hits-compressed-tsv-file}
...
### 下载访问量压缩的TSV文件： {#download-the-visits-compressed-tsv-file}
...
### 创建访问量表 {#create-the-visits-table}
...
### 导入访问量数据 {#import-the-visits-data}
```bash
...
## 示例JOIN {#an-example-join}

点击量和访问量数据集用于 ClickHouse 测试
例程，这是测试套件中的其中一个查询。其余
的测试在本页面末尾的 *后续步骤* 部分引用。
...
[ClickHouse中稀疏主索引的实用介绍](/guides/best-practices/sparse-primary-indexes.md) 使用点击量数据集讨论 ClickHouse 索引与传统关系数据库的差异，以及 ClickHouse 如何构建和使用稀疏主索引，以及索引最佳实践。

对于这些表的其他查询示例可以在 ClickHouse [有状态测试](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql)中找到。

:::note
测试套件使用的数据库名称为 `test`，表的名称为 `hits` 和 `visits`。您可以重命名您的数据库和表，或编辑测试文件中的 SQL。
:::
