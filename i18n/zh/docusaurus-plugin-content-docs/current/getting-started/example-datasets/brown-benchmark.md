...
`MgBench` 是一个用于机器生成日志数据的新分析基准， [Andrew Crotty](http://cs.brown.edu/people/acrotty/)。
...
下面的查询 3.5 使用了 UNION。 设置组合 SELECT 查询结果的模式。 该设置仅在与 UNION 一起共享时使用，而不明确指定 UNION ALL 或 UNION DISTINCT。
```sql
...
数据也可以在 [Playground](https://sql.clickhouse.com) 中用于交互查询， [示例](https://sql.clickhouse.com?query_id=1MXMHASDLEQIP4P1D1STND)。
