...
新的设置项 `allow_asynchronous_read_from_io_pool_for_merge_tree` 允许读取线程 (流) 的数量超过查询执行流水线其余部分的线程数量。

通常，[`max_threads`](/operations/settings/settings/#max_threads) 设置[控制](https://clickhouse.com/company/events/query-performance-introspection)并行读取线程和并行查询处理线程的数量：
