...
## 为单个查询配置设置 \{#configure-a-setting-for-a-single-query\}
...
## 为会话配置设置 \{#configure-a-setting-for-a-session\}
...
## 为特定用户配置设置 \{#configure-a-setting-for-a-particular-user\}
...
:::note
通过这种方式配置设置将会覆盖该用户当前的所有设置；也就是说，如果该用户之前已经应用了其他设置项，但你在 `ALTER USER` 语句中没有显式指定，它们将会丢失。

如果你希望保留现有设置，请改用 `ALTER USER ... MODIFY SETTING ...`。
:::
