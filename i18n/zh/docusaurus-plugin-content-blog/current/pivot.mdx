...
这些值包含很多小数位，我们可以使用 [`mapApply`](/sql-reference/functions/tuple-map-functions#mapApply) 函数，对映射中的每个值调用 [`floor`](/sql-reference/functions/rounding-functions#floor) 函数，将其取整：
