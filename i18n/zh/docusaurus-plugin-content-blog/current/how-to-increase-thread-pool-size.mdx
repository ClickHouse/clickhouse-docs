...
## 增加 ClickHouse 中的线程数量 \{#increasing-the-number-of-threads-in-clickhouse\}

ClickHouse 使用 **全局线程池** 中的线程来处理查询，并执行合并、变更等后台操作。如果没有空闲线程可用于处理查询，则会在线程池中创建一个新线程。
...
请参阅[文档](/operations/server-configuration-parameters/settings#max_thread_pool_size)，了解上述设置以及其他影响全局线程池的设置的更多详细信息。