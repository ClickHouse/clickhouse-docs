---
title: 如何查看分配给角色的用户及反向对应关系
description: 了解如何查询 ClickHouse 的 `system.role_grants` 系统表，以查找分配给各角色的用户，以及分配给特定用户的角色。
date: 2023-09-07
tags: ['服务器管理', '系统表', '云管理']
keywords: ['用户角色']
---
...
如何查看分配给某个角色的用户，以及反过来查看某个用户被分配了哪些角色？

## 解答 \{#answer\}

```sql
-- 以 default 用户身份登录(管理员权限)
