...
date: 2023-01-14
title: 以分贝比较查询之间的指标
description: 用于在 ClickHouse 中比较两个查询指标的查询。
tags: ['性能与优化']
...
## 比较两个查询的指标 \{#compare-metrics-between-two-queries\}

要比较两个查询的指标，首先必须获取这两个查询各自的 `query_id`。
...
返回 85 行。用时:0.074 秒。已处理 216 万行,102.28 MB(2919 万行/秒,1.38 GB/秒)
峰值内存使用量:114.84 MiB。
```