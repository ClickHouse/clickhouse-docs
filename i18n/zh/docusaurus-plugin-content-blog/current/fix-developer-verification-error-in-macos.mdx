...
1. 前往 **Privacy & Security**（隐私与安全）：

    <Image img={privacy_default} size="md" alt="MacOS 隐私与安全设置的默认视图" border />

1. 滚动到窗口底部，找到一条消息，内容为：_“clickhouse-macos-aarch64” was blocked from use because it is not from an identified developer_。
1. 点击 **Allow Anyway**（仍要允许）。
