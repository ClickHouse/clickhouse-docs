...
首先，确认 Homebrew 将 `clickhouse` 可执行文件安装在何处：
...
运行 `xattr -d com.apple.quarantine` 命令，并在其后追加上一个命令输出的路径，将 `clickhouse` 从隔离区中移除：
...
## 通过重新安装 ClickHouse 解决问题 \{#fix-the-issue-by-reinstalling-clickhouse\}
