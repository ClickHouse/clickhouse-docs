...
:::note[分布式执行并不是 `query_id` 值不同的唯一来源]
单个查询也可能派生出内部子查询，这些子查询会以各自的 `query_id` 和 `is_initial_query = 0` 记录下来——最常见的是 ClickHouse 为求值[视图](/operations/system-tables/query_views_log)而运行的内部查询。这些内部标识符由 ClickHouse 分配，其格式可能与返回给客户端的格式不同；在与视图相关的步骤中，你可能会看到带有 `queryView...` 之类标签的 `query_id` 值。当你想要精确获取自己提交的那条查询时，请按 `is_initial_query = 1` (或 `query_id = initial_query_id`) 进行过滤。这是 `query_log` 的服务端行为，从任何客户端看到的结果都相同，包括 Cloud SQL 控制台。
:::
