...
keywords: [сравнение запросов, сравнение метрик, производительность запросов]
---
...
## Сравнение метрик между двумя запросами \{#compare-metrics-between-two-queries\}

Чтобы сравнить метрики между двумя запросами, сначала необходимо получить `query_id` обоих запросов.
...
Вы получите таблицу с метриками для сравнения двух запросов:
...
Получено 85 строк. Затрачено: 0,074 сек. Обработано 2,16 млн строк, 102,28 МБ (29,19 млн строк/с., 1,38 ГБ/с.)
Пиковое использование памяти: 114,84 МиБ.
```