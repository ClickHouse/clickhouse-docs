...
- В нашем подмножестве каждая строка содержит три столбца, которые указывают на интернет-пользователя (столбец `UserID`), который кликнул на URL (столбец `URL`) в определённое время (столбец `EventTime`).

С помощью этих трёх столбцов мы уже можем сформулировать некоторые типичные запросы веб-аналитики, такие как:

- «Какие 10 URL чаще всего кликал конкретный пользователь?»
- «Какие 10 пользователей чаще всего кликали на конкретный URL?»
- «В какое время (например, дни недели) пользователь чаще всего кликает на конкретный URL?»
...
Затем вставим подмножество набора данных hits в таблицу с помощью следующей SQL-команды вставки.
...
Вывод результатов клиента ClickHouse показывает, что приведённый выше оператор вставил в таблицу 8,87 миллиона строк.
...
Теперь выполним наш первый запрос веб-аналитики. Следующий запрос определяет топ‑10 URL с наибольшим числом кликов для интернет-пользователя с `UserID` 749927693:
...
Обработано 8.87 миллионов строк,
70.45 МБ (398.53 миллиона строк/с, 3.17 ГБ/с)

```

Вывод клиента ClickHouse показывает, что ClickHouse выполнил полное сканирование таблицы! Каждая из 8,87 миллионов строк нашей таблицы была обработана ClickHouse. Это не масштабируется.

Чтобы сделать это намного более эффективным и быстрым, необходимо использовать таблицу с подходящим первичным ключом. Это позволит ClickHouse автоматически создать разреженный первичный индекс (на основе столбца или столбцов первичного ключа), который затем можно использовать для значительного ускорения выполнения нашего примера запроса.
```


## Дизайн индексов ClickHouse {#clickhouse-index-design}

### Дизайн индексов для массивных объёмов данных {#an-index-design-for-massive-data-scales}
  ...
  <li>
    ...
    <ul>
      ...
      <li>
        ...
        <ul>
          <li>
            Если <code>n</code> меньше 8192 и размер объединённых данных
            для этих <code>n</code> строк больше или равен 10 МБ
            (значение по умолчанию для <code>index_granularity_bytes</code>).
          </li>
          <li>
            Если размер объединённых данных для <code>n</code> строк меньше
...
Первичный ключ в приведённой выше DDL-инструкции инициирует создание первичного индекса на основе двух указанных ключевых столбцов.
...
- В общей сложности файлы данных и меток таблицы, а также файл первичного индекса занимают на диске 207.07 МБ.
...
:::note
ClickHouse позволяет вставлять несколько строк с идентичными значениями столбцов первичного ключа. В этом случае (см. строку 1 и строку 2 на диаграмме ниже) окончательный порядок определяется указанным ключом сортировки и, следовательно, значением столбца `EventTime`.
:::
...
- Мы упоминали в начале этого руководства в разделе «Детали DDL-инструкции», что мы отключили [адаптивную гранулярность индекса](/whats-new/changelog/2019.md/#experimental-features-1) (чтобы упростить обсуждение в этом руководстве, а также сделать диаграммы и результаты воспроизводимыми).
...
В общей сложности индекс имеет 1083 записи для нашей таблицы с 8,87 миллионами строк и 1083 гранулами:
...
:::note
- Для таблиц с [адаптивной зернистостью индекса](/whats-new/changelog/2019.md/#experimental-features-1) в первичном индексе также хранится одна дополнительная «финальная» метка, в которой записаны значения столбцов первичного ключа для последней строки таблицы. Но поскольку мы отключили адаптивную зернистость индекса (чтобы упростить обсуждение в этом руководстве, а также сделать диаграммы и результаты воспроизводимыми), индекс нашей примерной таблицы не содержит этой финальной метки.
...
В кластере ClickHouse с самостоятельным управлением мы можем использовать <a href="https://clickhouse.com/docs/sql-reference/table-functions/file/" target="_blank">табличную функцию file</a> для анализа содержимого первичного индекса нашей примерной таблицы.
...
<ul>
<li>Шаг 1: Получить путь к партиции, которая содержит файл первичного индекса</li>
`
...
в Linux вы можете проверить, изменялся ли он: `$ grep user_files_path /etc/clickhouse-server/config.xml`
...
Записи первичного ключа называются метками индекса (index marks), потому что каждая запись индекса помечает начало определённого диапазона данных. В частности, для нашей примерной таблицы:
- Метки индекса по UserID:

  Сохранённые значения `UserID` в первичном индексе отсортированы по возрастанию.<br/>
  «метка 1» на диаграмме выше, таким образом, означает, что значения `UserID` всех строк таблицы в грануле 1 и во всех последующих гранулах гарантированно больше или равны 4.073.710.



[Как мы увидим далее](#the-primary-index-is-used-for-selecting-granules), этот глобальный порядок позволяет ClickHouse <a href="https://github.com/ClickHouse/ClickHouse/blob/22.3/src/Storages/MergeTree/MergeTreeDataSelectExecutor.cpp#L1452" target="_blank">использовать алгоритм бинарного поиска</a> по индексным меткам для первого ключевого столбца, когда запрос выполняет фильтрацию по первому столбцу первичного ключа.

- Индексные метки URL:

  Близкая кардинальность столбцов первичного ключа `UserID` и `URL`
  означает, что индексные метки для всех ключевых столбцов после первого столбца обычно указывают диапазон данных только до тех пор, пока значение предшествующего ключевого столбца остается одинаковым для всех строк таблицы как минимум в пределах текущей гранулы.<br/>
  ...
  Мы подробнее рассмотрим влияние этого на производительность выполнения запросов далее.
...
Если <a href="https://clickhouse.com/docs/operations/server-configuration-parameters/settings/#server_configuration_parameters-logger" target="_blank">детализированное логирование (trace)</a> включено, то в журнале сервера ClickHouse будет видно, что ClickHouse выполнял <a href="https://github.com/ClickHouse/ClickHouse/blob/22.3/src/Storages/MergeTree/MergeTreeDataSelectExecutor.cpp#L1452" target="_blank">двоичный поиск</a> по 1083 меткам индекса UserID, чтобы определить гранулы, которые потенциально могут содержать строки со значением столбца UserID `749927693`. Для этого требуется 19 шагов при средней вычислительной сложности `O(log2 n)`:

```response
...Executor): Условие ключа: (column 0 in [749927693, 749927693])
# highlight-next-line
...Executor): Выполняется бинарный поиск по диапазону индекса для части all_1_9_2 (1083 marks)
...Executor): Найдена (ЛЕВАЯ) граничная отметка: 176
...Executor): Найдена (ПРАВАЯ) граничная отметка: 177
...Executor): Найден непрерывный диапазон за 19 шагов
...Executor): Выбрано 1/1 частей по ключу партиционирования, 1 частей по первичному ключу,
# highlight-next-line
              1/1083 отметок по первичному ключу, 1 отметок для чтения из 1 диапазонов
...Reading ...примерно 8192 строк, начиная с 1441792
```

В журнале трассировки выше мы видим, что одна метка из 1083 существующих меток удовлетворила запросу.
...
<details>
  ...
  <p>
    Была определена метка 176 (&#39;found left boundary mark&#39; включительно, &#39;found right boundary mark&#39; исключительно), и, соответственно, все 8192 строки из гранулы 176 (которая начинается с строки 1.441.792 — мы увидим это далее в данном руководстве) затем передаются в ClickHouse для поиска фактических строк со значением столбца UserID, равным `749927693`.
  </p>
</details>
...
:::note Вывод
Когда запрос фильтрует данные по столбцу, который является частью составного ключа и первым ключевым столбцом, ClickHouse выполняет алгоритм бинарного поиска по индексным меткам этого столбца.
:::

<br />

Как обсуждалось выше, ClickHouse использует свой разреженный первичный индекс для быстрого выбора (посредством бинарного поиска) гранул, которые потенциально могут содержать строки, соответствующие запросу.
...
Как обсуждалось выше, посредством бинарного поиска по 1083 меткам UserID индекса была идентифицирована метка 176. Следовательно, соответствующая ей гранула 176 потенциально может содержать строки со значением столбца UserID, равным 749.927.693.
...
Мы обсудили, что первичный индекс представляет собой плоский несжатый файл-массив (primary.idx), содержащий индексные метки, нумерация которых начинается с 0.

Аналогично, файл меток также является плоским несжатым файлом-массивом (\*.mrk), содержащим метки, нумерация которых начинается с 0.

После того как ClickHouse идентифицировал и выбрал индексную метку для гранулы, которая потенциально может содержать соответствующие запросу строки, выполняется позиционный поиск в файлах меток для получения физических местоположений гранулы.

Каждая запись файла меток для конкретного столбца хранит два местоположения в виде смещений:


- Первое смещение (`block_offset` на диаграмме выше) указывает расположение <a href="https://clickhouse.com/docs/development/architecture/#block" target="_blank">блока</a> в <a href="https://clickhouse.com/docs/introduction/distinctive-features/#data-compression" target="_blank">сжатом</a> файловом столбцовом данных, который содержит сжатую версию выбранной гранулы. Этот сжатый блок потенциально содержит несколько сжатых гранул. Найденный сжатый блок файла при чтении распаковывается в оперативную память.

- Второе смещение (`granule_offset` на диаграмме выше) из mark-файла указывает расположение гранулы внутри распакованных данных блока.

Затем все 8192 строки, относящиеся к найденной распакованной грануле, передаются в ClickHouse для дальнейшей обработки.

:::note

- Для таблиц с [широким форматом](/engines/table-engines/mergetree-family/mergetree.md/#mergetree-data-storage) и без [адаптивной зернистости индекса](/whats-new/changelog/2019.md/#experimental-features-1) ClickHouse использует mark-файлы `.mrk`, как показано выше. Они содержат записи с двумя 8-байтовыми адресами на запись. Эти записи — физические расположения гранул, все одного и того же размера.

 Зернистость индекса по умолчанию [адаптивная](/operations/settings/merge-tree-settings#index_granularity_bytes), но для нашей примерной таблицы мы отключили адаптивную зернистость индекса (чтобы упростить объяснения в этом руководстве, а также сделать диаграммы и результаты воспроизводимыми). Наша таблица использует широкий формат, потому что размер данных больше, чем [min_bytes_for_wide_part](/operations/settings/merge-tree-settings#min_bytes_for_wide_part) (по умолчанию это 10 МБ для кластеров с самостоятельным управлением).

- Для таблиц с широким форматом и с адаптивной зернистостью индекса ClickHouse использует mark-файлы `.mrk2`, которые содержат записи, похожие на записи в `.mrk`, но с дополнительным третьим значением в каждой записи: количеством строк гранулы, с которой связана текущая запись.
...
Почему первичный индекс напрямую не содержит физические расположения гранул, которые соответствуют меткам индекса?

Потому что на тех очень больших масштабах, для которых спроектирован ClickHouse, важно максимально эффективно использовать диск и память.

Файл первичного индекса должен помещаться в оперативной памяти.

В нашем примерном запросе ClickHouse использовал первичный индекс и выбрал единственную гранулу, которая потенциально может содержать строки, соответствующие нашему запросу. Только для этой одной гранулы ClickHouse затем нужны физические расположения, чтобы потоково передать соответствующие строки для дальнейшей обработки.
...
Косвенный уровень, обеспечиваемый mark-файлами, позволяет избежать хранения непосредственно в первичном индексе записей с физическими расположениями всех 1083 гранул для всех трёх столбцов, тем самым избегая наличия лишних (потенциально неиспользуемых) данных в оперативной памяти.
:::
...
Ранее в этом руководстве мы обсуждали, что ClickHouse выбрал метку первичного индекса 176 и, следовательно, гранулу 176 как потенциально содержащую подходящие строки для нашего запроса.

Теперь ClickHouse использует выбранный номер метки (176) из индекса для позиционного поиска по массиву в mark-файле `UserID.mrk`, чтобы получить два смещения для нахождения гранулы 176.

Как показано, первое смещение указывает расположение сжатого блока файла внутри файла данных `UserID.bin`, который, в свою очередь, содержит сжатую версию гранулы 176.

После того как найденный блок файла будет распакован в оперативную память, второе смещение из mark-файла можно использовать, чтобы найти гранулу 176 внутри распакованных данных.

ClickHouse должен найти (и потоково прочитать все значения) гранулы 176 как из файла данных `UserID.bin`, так и из файла данных `URL.bin`, чтобы выполнить наш примерный запрос (top-10 наиболее кликаемых URL для интернет‑пользователя с `UserID` 749.927.693).



Диаграмма выше показывает, как ClickHouse находит гранулу для файла данных UserID.bin.

Параллельно ClickHouse делает то же самое для гранулы 176 файла данных URL.bin. Две соответствующие гранулы выравниваются и передаются потоком в движок ClickHouse для дальнейшей обработки, то есть агрегации и подсчёта значений URL по группам для всех строк, где UserID равен 749.927.693, после чего на выходе формируются 10 крупнейших групп URL в порядке убывания количества.
...
Используем запрос, который вычисляет топ-10 пользователей, которые наиболее часто
переходили по URL "http://public_search":
...
Из приведённого выше примера трассировочного лога видно, что 1076 гранул (через метки) из 1083 были выбраны как потенциально содержащие строки с совпадающим значением URL.
...
Однако, как мы увидим позже, только 39 гранул из этих выбранных 1076 гранул действительно содержат совпадающие строки.

Хотя первичный индекс на основе составного первичного ключа (UserID, URL) был очень полезен для ускорения запросов с фильтрацией строк по конкретному значению UserID, этот индекс не обеспечивает существенного ускорения запроса, который фильтрует строки по конкретному значению URL.
...
Чтобы проиллюстрировать это, мы приведём некоторые детали о том, как работает общий поиск с исключением.
...
Предположим, что UserID имеет низкую кардинальность. В этом случае вероятно, что одно и то же значение UserID распределено по нескольким строкам таблицы и гранулам, а следовательно, и индексным меткам. Для индексных меток с одинаковым UserID значения URL для индексных меток отсортированы в порядке возрастания (поскольку строки таблицы упорядочены сначала по UserID, а затем по URL). Это позволяет эффективно фильтровать, как описано ниже:
...
1.  Индексная метка 0, для которой **значение URL меньше W3 и для которой значение URL непосредственно следующей индексной метки также меньше W3**, может быть исключена, поскольку метки 0 и 1 имеют одинаковое значение UserID. Обратите внимание, что это предусловие исключения гарантирует, что гранула 0 полностью состоит из значений UserID равных U1, так что ClickHouse может предположить, что максимальное значение URL в грануле 0 также меньше W3, и исключить гранулу.
...
Конкретное значение URL, которое ищет запрос (например, 'http://public_search'), с большой вероятностью находится между минимальным и максимальным значением, сохранённым индексом для каждой группы гранул, что вынуждает ClickHouse выбирать группу гранул (поскольку они могут содержать строки, соответствующие запросу).

### Необходимость использования нескольких первичных индексов {#a-need-to-use-multiple-primary-indexes}

Следовательно, если мы хотим значительно ускорить наш примерный запрос, фильтрующий строки с конкретным URL, нам необходимо использовать первичный индекс, оптимизированный для этого запроса.
...
Поскольку мы изменили порядок столбцов в первичном ключе, вставленные строки теперь хранятся на диске в другом лексикографическом порядке (по сравнению с нашей [исходной таблицей](#a-table-with-a-primary-key)), и поэтому 1083 гранулы этой таблицы содержат иные значения, чем раньше:
...
Обработано 319.49 тысячи строк,
11.38 MB (18.41 миллиона строк/с., 655.75 MB/s.)
...
73.04 MB (340.26 миллиона строк/с, 3.10 GB/с)
...
- материализованное представление использует **неявно созданную таблицу**, порядок строк и первичный индекс которой основаны на заданном определении первичного ключа
- неявно созданная таблица отображается в результатах запроса `SHOW TABLES` и имеет имя, начинающееся с `.inner`
- также можно сначала явно создать базовую таблицу для материализованного представления, а затем указать её в представлении через [конструкцию](/sql-reference/statements/create/view.md) `TO [db].[table]`
...
Неявно созданная таблица (и её первичный индекс), лежащая в основе материализованного представления, теперь может использоваться для значительного ускорения выполнения нашего примера запроса с фильтрацией по столбцу URL:
...
10 строк в наборе. Потрачено: 0.026 сек.
...
- проекция создаёт **скрытую таблицу**, порядок строк и первичный индекс которой основаны на указанной конструкции `ORDER BY` проекции
...
- запрос всегда (синтаксически) направлен на исходную таблицу hits_UserID_URL, но если порядок строк и первичный индекс скрытой таблицы позволяют более эффективное выполнение запроса, то вместо исходной таблицы будет использована скрытая таблица
...
Скрытая таблица (и её первичный индекс), созданная проекцией, теперь может быть (неявно) использована для значительного ускорения выполнения нашего примера запроса с фильтрацией по столбцу URL. Обратите внимание, что запрос синтаксически направлен на исходную таблицу проекции.
...
1.38 MB (11.05 миллионов строк/с., 393.58 MB/s.)
...
Из-за одинаково высокой кардинальности столбцов первичного ключа UserID и URL запрос с фильтрацией по второму ключевому столбцу [не получает существенного преимущества от присутствия второго ключевого столбца в индексе](#generic-exclusion-search-algorithm).
...
Чтобы продемонстрировать это, мы будем использовать версию нашего [примера набора данных веб-трафика](#data-set),
где каждая строка содержит три столбца, которые указывают, был ли доступ интернет-пользователя (столбец `UserID`) к URL (столбец `URL`) отмечен как трафик бота (столбец `IsRobot`).
...
### Эффективная фильтрация по неосновным столбцам ключа {#efficient-filtering-on-secondary-key-columns}

Когда запрос фильтрует хотя бы по одному столбцу, который является частью составного ключа и при этом является первым ключевым столбцом, [ClickHouse выполняет алгоритм бинарного поиска по индексным меткам этого столбца](#the-primary-index-is-used-for-selecting-granules).

Когда запрос фильтрует (только) по столбцу, который является частью составного ключа, но не является первым ключевым столбцом, [ClickHouse использует общий алгоритм поиска с исключениями по индексным меткам этого столбца](/guides/best-practices/sparse-primary-indexes#secondary-key-columns-can-not-be-inefficient).

Во втором случае порядок следования ключевых столбцов в составном первичном ключе существенно влияет на эффективность [общего алгоритма поиска с исключениями](https://github.com/ClickHouse/ClickHouse/blob/22.3/src/Storages/MergeTree/MergeTreeDataSelectExecutor.cpp#L1444).

Ниже приведен запрос с фильтрацией по столбцу `UserID` таблицы, в которой ключевые столбцы `(URL, UserID, IsRobot)` упорядочены по кардинальности в порядке убывания:
...
Высокий коэффициент сжатия данных столбца таблицы на диске не только экономит место на диске, но и ускоряет запросы (особенно аналитические), которым требуется чтение данных из этого столбца, так как требуется меньше операций ввода-вывода для перемещения данных столбца с диска в основную память (файловый кэш операционной системы).

Далее мы покажем, почему для улучшения коэффициента сжатия столбцов таблицы выгодно упорядочивать столбцы первичного ключа по кардинальности по возрастанию.

Диаграмма ниже схематично показывает порядок строк на диске для первичного ключа, в котором ключевые столбцы упорядочены по кардинальности по возрастанию:

<Image img={sparsePrimaryIndexes14a} size="md" alt="Sparse Primary Indices 14a" background="white" />

Ранее мы обсудили, что [данные строк таблицы на диске хранятся в порядке столбцов первичного ключа](#data-is-stored-on-disk-ordered-by-primary-key-columns).
...
Одним конкретным примером является сервис для вставки текста [https://pastila.nl](https://pastila.nl), который разработал Алексей Миловидов и о котором [написал в блоге](https://clickhouse.com/blog/building-a-paste-service-with-clickhouse/).
