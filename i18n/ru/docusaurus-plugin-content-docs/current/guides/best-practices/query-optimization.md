...
description: 'Простое руководство по оптимизации запросов, описывающее основные подходы к повышению производительности запросов'
doc_type: 'guide'
...
В этом разделе на распространённых сценариях показывается, как использовать различные приёмы повышения производительности и оптимизации, такие как [анализатор](/operations/analyzer), [профилирование запросов](/operations/optimizing-performance/sampling-query-profiler) или [избегание Nullable-столбцов](/optimize/avoid-nullable-columns), чтобы улучшить производительность ваших запросов в ClickHouse.
...
ClickHouse предоставляет богатый набор инструментов, которые помогают понять, как выполняется ваш запрос и какие ресурсы потребляются для его выполнения.
...
Для каждого выполненного запроса ClickHouse записывает статистику, такую как время выполнения запроса, количество прочитанных строк и использование ресурсов, включая CPU, память и попадания в кэш файловой системы. 

Таким образом, журнал запросов — это хорошая отправная точка при анализе медленных запросов. Вы можете легко выявить запросы, которые выполняются долго, и просмотреть информацию об использовании ресурсов для каждого из них. 

Давайте найдём пять самых долго выполняющихся запросов в нашем наборе данных NYC taxi.
...
Поле `query_duration_ms` показывает, сколько времени потребовалось на выполнение конкретного запроса. Анализируя результаты из журналов запросов, мы видим, что первый запрос выполняется 2967 мс, что можно оптимизировать. 
...
Давайте выделим найденные нами долго выполняющиеся запросы и запустим их повторно несколько раз, чтобы оценить время ответа. 

На этом этапе важно отключить кэш файловой системы, установив настройку `enable_filesystem_cache` в 0, чтобы повысить воспроизводимость.
...
----
Получена 1 строка. Затрачено: 1.699 сек. Обработано 329.04 млн строк, 8.88 ГБ (193.72 млн строк/с., 5.23 ГБ/с.)
...
---
Получено 4 строки. Затрачено: 1.419 сек. Обработано 329.04 млн строк, 5.72 ГБ (231.86 млн строк/с., 4.03 ГБ/с.)
...
---
Получена 1 строка. Затрачено: 1.414 сек. Обработано 329.04 млн строк, 8.88 ГБ (232.63 млн строк/с., 6.28 ГБ/с.)
Пиковое использование памяти: 451.53 МиБ.
```

Сведём данные в таблицу для удобства.
...
Разберёмся чуть подробнее, что делают эти запросы. 
...
Ни один из этих запросов не выполняет очень сложную обработку, за исключением первого, который каждый раз при выполнении заново рассчитывает время поездки. Однако каждый из этих запросов выполняется более одной секунды, что в мире ClickHouse — очень долго. Можно также отметить потребление памяти этими запросами: порядка 400 МБ на каждый запрос — это довольно много. Кроме того, каждый запрос, по-видимому, читает одно и то же количество строк (то есть 329,04 миллиона). Давайте быстро проверим, сколько строк в этой таблице.
...
Результат очевиден. Запрос начинается с чтения данных из таблицы `nyc_taxi.trips_small_inferred`. Затем применяется предложение WHERE для фильтрации строк на основе вычисленных значений. Отфильтрованные данные подготавливаются к агрегированию, и вычисляются квантили. Наконец, результат сортируется и выводится. 

Здесь стоит отметить, что первичные ключи не используются, что логично, так как при создании таблицы мы их не задавали. В результате ClickHouse выполняет полный скан таблицы для этого запроса. 

**Explain Pipeline**

EXPLAIN PIPELINE показывает конкретную стратегию выполнения запроса. Здесь вы можете увидеть, как ClickHouse фактически выполнил общий план запроса, который мы рассматривали ранее.
...
Здесь мы можем отметить количество потоков, использованных для выполнения запроса: 59 потоков, что говорит о высокой степени параллелизма. Это ускоряет выполнение запроса, который на менее мощной машине выполнялся бы дольше. Количество потоков, работающих параллельно, может объяснять большой объём памяти, который использует запрос. 

В идеале вы должны анализировать все свои медленные запросы таким же образом, чтобы выявлять ненужно сложные планы выполнения и понимать количество строк, прочитанных каждым запросом, а также потреблённые ресурсы.



## Методология {#methodology}

Выявление проблемных запросов в продуктивном развёртывании может быть затруднительным, поскольку в любой момент времени в вашем развёртывании ClickHouse, вероятно, выполняется большое количество запросов.
...
После того как вы определите запросы, которые хотите оптимизировать, можно приступать к работе над ними. Одна из распространённых ошибок, которую допускают разработчики на этом этапе, — это одновременное изменение нескольких параметров, проведение разовых экспериментов, что обычно приводит к неоднозначным результатам и, что более важно, к отсутствию чёткого понимания того, что именно ускорило запрос.

Оптимизация запросов требует структурированного подхода. Речь идёт не о продвинутом бенчмаркинге, а о наличии простого процесса, позволяющего понять, как ваши изменения влияют на производительность запросов.
...
Первичные ключи в ClickHouse работают иначе, чем в большинстве традиционных СУБД. В таких системах первичные ключи обеспечивают уникальность и целостность данных. Любая попытка вставить дублирующиеся значения первичного ключа отклоняется, и обычно создаётся индекс на основе B-дерева или хеш-таблицы для быстрого поиска. 

В ClickHouse [назначение](/guides/best-practices/sparse-primary-indexes#a-table-with-a-primary-key) первичного ключа другое: он не обеспечивает уникальность и не помогает поддерживать целостность данных. Вместо этого он предназначен для оптимизации производительности запросов. Первичный ключ определяет порядок хранения данных на диске и реализован в виде разреженного индекса, который хранит указатели на первую строку каждой гранулы.
...
Выбор правильного набора первичных ключей важен для производительности, и на практике часто используется подход, при котором одни и те же данные хранятся в разных таблицах с различными наборами первичных ключей для ускорения определённых запросов. 

Другие возможности ClickHouse, такие как проекции или материализованные представления, позволяют использовать различные наборы первичных ключей для одних и тех же данных. Вторая часть этой серии статей рассмотрит это более подробно. 
...
- Рассмотрите возможность включения временной компоненты в первичный ключ, поскольку фильтрация по времени в наборах данных с временными метками является довольно распространённой. 
...
Затем мы повторно выполняем наши запросы. Мы собираем результаты трёх экспериментов, чтобы увидеть улучшения во времени выполнения, количестве обработанных строк и потреблении памяти. 
...
Мы видим заметное улучшение по всем метрикам — как по времени выполнения, так и по использованию памяти. 

Запрос 2 получает наибольший выигрыш от первичного ключа. Давайте посмотрим, чем теперь отличается построенный план запроса по сравнению с прежним.
...
Идентификатор запроса: 30116a77-ba86-4e9f-a9a2-a01670ad2e15
...
Благодаря первичному ключу был выбран только поднабор гранул таблицы. Уже это значительно улучшает производительность запроса, поскольку ClickHouseу приходится обрабатывать существенно меньше данных.
