...
doc_type: 'guide'
keywords: ['индексы пропуска данных', 'пропуск данных', 'производительность', 'индексация', 'лучшие практики']
---
...
ClickHouse поддерживает шесть типов индексов пропуска данных:
...
| **ngrambf&#95;v1**                                  | N-граммный bloom-фильтр для поиска подстрок                                     |
...
## Текстовый индекс (text) для полнотекстового поиска \{#textindex-for-full-text-search\}

`text` — это инвертированный индекс по токенизированным текстовым данным.
Он разработан специально для полнотекстовых поисковых нагрузок и обеспечивает эффективный и детерминированный поиск токенов и терминов.
Рекомендуется для сценариев поиска по естественному языку или крупномасштабного текстового поиска.

Подробности и примеры см. в разделе [Полнотекстовый поиск с текстовыми индексами](/engines/table-engines/mergetree-family/textindexes).
...
Более полный пример обсервабилити см. в документации [здесь](/use-cases/observability/schema-design#text-index-for-full-text-search).

Текстовый индекс полностью детерминирован и гибко настраивается с точки зрения токенизации и обработки текста, ценой несколько большего потребления места в хранилище по сравнению с индексами на основе фильтра Блума.
