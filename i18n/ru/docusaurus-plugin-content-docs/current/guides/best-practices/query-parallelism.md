...
sidebar_position: 20
description: 'ClickHouse распараллеливает выполнение запросов, используя потоки обработки и параметр max_threads.'
title: 'Как ClickHouse выполняет запросы параллельно'
doc_type: 'guide'
keywords: ['параллельная обработка', 'оптимизация запросов', 'производительность', 'потоки', 'лучшие практики']
---
...
ClickHouse [создан для скорости](/concepts/why-clickhouse-is-so-fast). Он выполняет запросы в высокопараллельном режиме, используя все доступные ядра процессора, распределяя данные по потокам обработки и часто нагружая аппаратное обеспечение до предела его возможностей.
 
Это руководство описывает, как работает параллелизм запросов в ClickHouse и как его можно настроить или отслеживать для повышения производительности при больших нагрузках.

Для иллюстрации ключевых концепций мы используем агрегационный запрос к набору данных [uk_price_paid_simple](/parts).
...
В примере выше предполагается `4` ядра.
...
В ClickHouse Cloud такой же параллелизм достигается с помощью [параллельных реплик](https://clickhouse.com/docs/deployment-guides/parallel-replicas), которые функционируют аналогично шардам в кластерах с архитектурой shared-nothing. Каждая реплика ClickHouse Cloud — stateless вычислительный узел — обрабатывает часть данных параллельно и вносит вклад в итоговый результат, точно так же, как это делал бы независимый шард.
:::


## Мониторинг параллелизма запросов {#monitoring-query-parallelism}

Используйте эти инструменты для проверки того, что ваш запрос полностью использует доступные ресурсы процессора, и для диагностики случаев, когда этого не происходит.
...
Альтернативно, мы можем использовать оператор [EXPLAIN](/sql-reference/statements/explain#explain-pipeline) для проверки [плана физических операторов](/academic_overview#4-2-multi-core-parallelization) — также известного как «конвейер запроса» — для агрегирующего запроса:
...
[Встроенный веб-интерфейс](/interfaces/http) ClickHouse (доступный по адресу `/play`) может отобразить физический план, приведенный выше, в виде графической визуализации. В этом примере мы установили `max_threads` равным `4`, чтобы сохранить визуализацию компактной, показывая только 4 параллельных потока обработки:
...
Как показано в выводе выше, запрос обработал 2,31 миллиона строк и прочитал 13,66 МБ данных. Это связано с тем, что на этапе анализа индекса ClickHouse выбрал **282 гранулы** для обработки, каждая из которых содержит 8 192 строки, что в сумме составляет приблизительно 2,31 миллиона строк:
...
Для кластеров с раздельным хранением:
...
Кроме того, существует жесткий нижний предел для размера задачи чтения, контролируемый следующими параметрами:
...
Мы не рекомендуем изменять эти настройки в production-окружении. Они приведены здесь исключительно для того, чтобы показать, почему `max_threads` не всегда определяет фактический уровень параллелизма.
:::

В демонстрационных целях рассмотрим физический план с переопределёнными настройками для принудительного достижения максимального параллелизма:
...
Это демонстрирует, что для запросов к небольшим наборам данных ClickHouse намеренно ограничивает параллелизм. Используйте переопределение настроек только для тестирования, а не в продакшене, так как это может привести к неэффективному выполнению или конфликтам за ресурсы.


## Ключевые выводы {#key-takeaways}

- ClickHouse распараллеливает запросы, используя каналы обработки, количество которых определяется параметром `max_threads`.
- Фактическое количество каналов зависит от объёма данных, выбранных для обработки.
- Используйте `EXPLAIN PIPELINE` и журналы трассировки для анализа использования каналов.
