...
sidebar_position: 21
description: 'PREWHERE уменьшает объем операций ввода-вывода, позволяя не читать ненужные данные столбцов.'
...
[Клауза PREWHERE](/sql-reference/statements/select/prewhere) — это оптимизация выполнения запросов в ClickHouse. Она сокращает объём операций ввода-вывода и ускоряет выполнение запросов за счёт предотвращения ненужных чтений данных и отбрасывания нерелевантных строк до чтения с диска столбцов, не участвующих в фильтрации.
...
③ Он сканирует записи индекса, чтобы определить, какие гранулы столбца town могут содержать строки, удовлетворяющие предикату.
...
Следующие анимации демонстрируют, как обрабатывается запрос с применением условия PREWHERE ко всем предикатам запроса.
...
Теперь, благодаря условию PREWHERE, следующий этап отличается: вместо того чтобы сразу читать все необходимые столбцы, ClickHouse фильтрует данные столбец за столбцом, загружая только то, что действительно необходимо. Это значительно сокращает операции ввода-вывода, особенно для широких таблиц.
...
<br />
Наконец, ClickHouse ① читает две выбранные гранулы из столбца `price` для
...
соответствующие строки. Это приводит к меньшему использованию памяти, сокращению дисковых операций ввода-вывода и
...
- PREWHERE позволяет избежать чтения данных столбцов, которые впоследствии будут отфильтрованы, экономя операции ввода-вывода и память.
- Работает автоматически при включенной настройке `optimize_move_to_prewhere` (включена по умолчанию).
- Порядок фильтрации имеет значение: небольшие и селективные столбцы должны обрабатываться первыми.
- Используйте `EXPLAIN` и логи для проверки применения PREWHERE и оценки его эффекта.
