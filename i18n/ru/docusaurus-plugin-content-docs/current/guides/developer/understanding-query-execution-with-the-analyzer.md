...
sidebar_label: 'Как выполняются запросы в анализаторе'
title: 'Как выполняются запросы в анализаторе'
description: 'Рассказывает, как использовать анализатор, чтобы понять, как ClickHouse выполняет ваши запросы'
doc_type: 'guide'
...
# Как выполняется запрос с помощью анализатора

ClickHouse обрабатывает запросы чрезвычайно быстро, но выполнение запроса — это не такая уж простая вещь. Попробуем разобраться, как выполняется запрос `SELECT`. Чтобы проиллюстрировать это, добавим немного данных в таблицу в ClickHouse:
...
Теперь, когда в ClickHouse есть данные, мы хотим выполнить несколько запросов и разобраться, как они выполняются. Выполнение запроса разбивается на множество шагов. Каждый шаг выполнения можно проанализировать и диагностировать с помощью соответствующего запроса `EXPLAIN`. Эти шаги показаны на диаграмме ниже:

<Image img={analyzer1} alt="Шаги выполнения запроса EXPLAIN" size="md" />

Рассмотрим каждую из этих сущностей в действии во время выполнения запроса. Мы возьмём несколько запросов и изучим их с помощью оператора `EXPLAIN`.
...
В настоящее время в ClickHouse существует две архитектуры анализатора. Вы можете использовать старую архитектуру, установив параметр `enable_analyzer=0`. Новая архитектура включена по умолчанию. Здесь мы будем описывать только новую архитектуру, так как старая будет объявлена устаревшей после того, как новый анализатор станет общедоступным.
...
Хотя это дает некоторую информацию, можно получить больше. Например, может потребоваться узнать имена столбцов, над которыми выполняются проекции. Для этого можно добавить заголовок к запросу:
...
Конвейер запроса формируется на основе плана запроса. Конвейер запроса очень похож на план запроса, но отличается тем, что представляет собой не дерево, а граф. Он показывает, как ClickHouse будет выполнять запрос и какие ресурсы будут задействованы. Анализ конвейера запроса очень полезен для выявления узких мест в плане входов и выходов. Рассмотрим наш предыдущий запрос и посмотрим на выполнение конвейера запроса:
...
В скобках указан шаг плана запроса, а рядом с ним — процессор. Это полезная информация, но поскольку это граф, было бы удобно визуализировать его соответствующим образом. Для этого можно установить параметр `graph` в значение 1 и указать формат вывода TSV:
...
Затем можно скопировать этот вывод и вставить его [сюда](https://dreampuf.github.io/GraphvizOnline), что сгенерирует следующий граф:

<Image img={analyzer3} alt='Вывод графа' size='md' />

Белый прямоугольник соответствует узлу конвейера, серый прямоугольник — шагам плана запроса, а символ `x` с последующим числом — количеству используемых входов и выходов. Если вы не хотите видеть их в компактной форме, можно добавить параметр `compact=0`:
...
Почему ClickHouse не читает таблицу в несколько потоков? Давайте попробуем добавить в неё больше данных:
...
В данном случае исполнитель решил не выполнять операции параллельно, поскольку объем данных оказался недостаточно большим. После добавления большего числа строк исполнитель принял решение использовать несколько потоков, что и показано на графике.


## Исполнитель {#executor}

Наконец, последний этап выполнения запроса осуществляется исполнителем. Он получает конвейер запроса и выполняет его. Существуют различные типы исполнителей в зависимости от того, выполняется ли запрос `SELECT`, `INSERT` или `INSERT SELECT`.
