...
sidebar_label: 'TTL (время жизни данных)'
sidebar_position: 2
keywords: ['ttl', 'время жизни данных', 'clickhouse', 'старые', 'данные']
description: 'TTL (время жизни данных) — это механизм, позволяющий перемещать, удалять или агрегировать строки или столбцы по прошествии заданного интервала времени.'
title: 'Управление данными с помощью TTL (время жизни данных)'
...
# Управление данными с использованием TTL (time-to-live)



## Обзор TTL {#overview-of-ttl}

TTL (time-to-live, время жизни) — это механизм перемещения, удаления или агрегирования строк или столбцов по истечении определённого интервала времени. Хотя термин «время жизни» подразумевает только удаление старых данных, TTL имеет несколько сценариев применения:

- Удаление старых данных: как и ожидается, можно удалять строки или столбцы по истечении заданного интервала времени
- Перемещение данных между дисками: по истечении определённого времени можно перемещать данные между томами хранения — это полезно для развёртывания архитектуры горячего/тёплого/холодного хранения
- Агрегирование данных: агрегирование старых данных в различные полезные агрегаты и вычисления перед их удалением
...
:::note
Правила TTL могут быть изменены или удалены. Подробнее см. на странице [Манипуляции с TTL таблицы](/sql-reference/statements/alter/ttl.md).
:::


## Запуск событий TTL {#triggering-ttl-events}

Удаление или агрегация устаревших строк происходит не мгновенно — это выполняется только во время слияния частей таблицы. Если таблица не выполняет активное слияние (по какой-либо причине), существуют две настройки для запуска событий TTL:
...
Чтобы удалять строки из таблицы по истечении определённого времени, определите правило TTL на уровне таблицы:
...
Кроме того, можно определить правило TTL на основе значения записи.
Это легко реализуется с помощью указания условия WHERE.
...
Вместо удаления всей строки предположим, что вы хотите установить срок действия только для столбцов balance и address. Давайте изменим таблицу `customers` и добавим TTL для обоих столбцов, равный 2 часам:
...
## Реализация сворачивания данных {#implementing-a-rollup}
...
Предположим, в следующей таблице `hits` мы хотим удалить старые строки, но сохранить сумму и максимум столбца `hits` перед удалением строк. Нам потребуется поле для хранения этих значений, и нам нужно будет добавить секцию `GROUP BY` в секцию `TTL`, которая выполняет агрегирование суммы и максимума:
...
:::note
Если вы используете ClickHouse Cloud, шаги данного урока неприменимы. Вам не требуется заботиться о перемещении старых данных в ClickHouse Cloud.
:::
...
2. Приведённая выше конфигурация ссылается на три диска, которые указывают на каталоги, из которых ClickHouse может читать и в которые может записывать данные. Тома могут содержать один или несколько дисков — мы определили том для каждого из трёх дисков. Просмотрим диски:
...
5. Новое правило `TTL` должно материализоваться автоматически, но вы можете принудительно выполнить его для уверенности:
...
6. Проверьте, что данные переместились на ожидаемые диски, используя таблицу `system.parts`:
