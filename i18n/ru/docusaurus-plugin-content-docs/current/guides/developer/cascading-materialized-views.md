...
title: 'Каскадные materialized view'
description: 'Как использовать несколько materialized view из исходной таблицы.'
keywords: ['materialized view', 'агрегация']
doc_type: 'руководство'
---

Этот пример демонстрирует, как создать materialized view, а затем — как создать второе materialized view на основе первого (каскадно). На этой странице вы увидите, как это сделать, узнаете о многих возможностях и ограничениях. Разные сценарии использования можно реализовать, создавая materialized view, использующее в качестве источника другое materialized view.
...
Подготовка данных с помощью materialized view позволит ограничить объём данных и вычислений, которые ClickHouse должен выполнять, что сделает ваши запросы SELECT быстрее.
...
## Таблица с годовой агрегацией и материализованное представление \{#yearly-aggregated-table-and-materialized-view\}
...
Теперь можно протестировать наше каскадное materialized view, вставив некоторые данные:
...
Создайте два материализованных представления, ссылающихся на одну и ту же таблицу `Target`. Нет необходимости явно указывать отсутствующие столбцы:
...
Теперь при вставке значений эти значения будут агрегированы в соответствующие столбцы таблицы `Target`:
...
Этот запрос должен вывести примерно такой результат:
