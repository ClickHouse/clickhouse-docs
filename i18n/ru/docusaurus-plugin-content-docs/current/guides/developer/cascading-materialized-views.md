...
title: 'Каскадирование материализованных представлений'
description: 'Как использовать несколько материализованных представлений на основе одной исходной таблицы.'
...
В этом примере показано, как создать материализованное представление, а затем сделать второе материализованное представление, каскадно зависящее от первого. На этой странице вы увидите, как это делается, какие есть варианты и ограничения. Разные варианты использования можно реализовать, создавая `MATERIALIZED VIEW`, использующее другое `MATERIALIZED VIEW` в качестве источника.
...
Мы будем использовать тестовый набор данных с количеством просмотров в час для группы доменных имён.
...
- Подготавливать данные при записи в новом формате
- Подготавливать данные при записи в виде заранее рассчитанной агрегации.

Подготовка данных с использованием материализованных представлений позволит сократить объём данных и количество вычислений, которые должен выполнять ClickHouse, делая запросы `SELECT` быстрее.
...
При выполнении SELECT из `analytics.hourly_data` вы увидите следующее, так как движок таблицы — `Null`, но данные были обработаны.
...
Мы использовали небольшой набор данных, чтобы проверить и сравнить результат с ожидаемым. После того как процесс будет корректно работать на небольшом наборе данных, можно переходить к большим объемам.


## Результаты {#results}

Если вы попытаетесь запросить целевую таблицу, выбрав поле `sumCountViews`, вы увидите бинарное представление (в некоторых терминалах), поскольку значение хранится не как число, а как тип AggregateFunction.
...
Материализованные представления также можно использовать для объединения нескольких исходных таблиц в одну целевую таблицу. Это удобно для создания материализованного представления, работающего аналогично логике `UNION ALL`.
