...
При включении мутаций на лету обновлённые строки помечаются как обновлённые немедленно, и последующие запросы `SELECT` автоматически возвращают изменённые значения. Если мутации на лету не включены, для получения изменённых значений может потребоваться дождаться применения мутаций фоновым процессом.

Мутации на лету можно включить для таблиц семейства `MergeTree` с помощью настройки уровня запроса `apply_mutations_on_fly`.
...
-- поведения по умолчанию при выключенных мутациях на лету
