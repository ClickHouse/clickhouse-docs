...
keywords: ['стратегии дедупликации', 'дедупликация данных', 'upserts', 'обновления и удаления', 'руководство для разработчиков']
doc_type: 'guide'
---
...
### Отказ от использования FINAL \{#avoiding-final\}
...
Сейчас в таблице 6 строк, так как фактическое слияние ещё не произошло (произошло только слияние на этапе выполнения запроса, когда мы использовали `FINAL`).
...
Обратите внимание, что добавление `FINAL` возвращает строку текущего состояния:
...
Мы выполним тот же запрос, что и раньше, который за счет столбца sign складывает и вычитает значения:
