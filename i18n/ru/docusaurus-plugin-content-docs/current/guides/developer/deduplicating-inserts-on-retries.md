...
keywords: ['дедупликация', 'дедупликация', 'повторные вставки', 'операции вставки']
doc_type: 'guide'
---

Операции вставки иногда могут завершаться сбоем из-за ошибок, например тайм-аутов. Когда вставка завершается неудачно, данные могли как вставиться, так и не вставиться. В этом руководстве описано, как включить дедупликацию при повторных попытках вставки, чтобы одни и те же данные не вставлялись более одного раза.

Когда выполняется повторная попытка вставки, ClickHouse пытается определить, были ли данные уже успешно вставлены. Если вставленные данные помечены как дубликат, ClickHouse не вставляет их в целевую таблицу. Однако пользователь по-прежнему получит статус успешного выполнения операции, как если бы данные были вставлены обычным образом.
...
В данный момент поведение для `INSERT ... SELECT` контролируется настройкой `insert_select_deduplicate`. Эта настройка определяет, применяется ли дедупликация к данным, вставляемым с помощью запросов `INSERT ... SELECT`. Подробности и примеры использования см. в соответствующей документации.
...
Здесь мы видим, что в таблицу `mv_dst` были вставлены 2 части. Эти части содержат одни и те же данные, однако они не были дедуплицированы.
...
Повторная вставка была дедуплицирована, как и ожидалось.
