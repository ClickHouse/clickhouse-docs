...
Операции вставки иногда могут завершаться с ошибкой, например из‑за тайм-аутов. В таких случаях данные могли быть как успешно вставлены, так и нет. В этом руководстве объясняется, как включить удаление дубликатов при повторных вставках, чтобы одни и те же данные не вставлялись более одного раза.

Когда вставка выполняется повторно, ClickHouse пытается определить, были ли данные уже успешно вставлены. Если вставленные данные помечены как дубликат, ClickHouse не вставляет их в целевую таблицу. Однако пользователь всё равно получит статус успешного выполнения операции, как если бы данные были вставлены обычным образом.
...
Если во время последовательности повторных попыток происходит более `*_deduplication_window` других операций вставки, дедупликация может работать не так, как предполагается. В этом случае одни и те же данные могут быть вставлены несколько раз.
...
При вставке данных в ClickHouse данные разделяются на блоки на основе количества строк и байтов.

Для таблиц, использующих движки семейства `*MergeTree`, каждому блоку присваивается уникальный идентификатор `block_id`, представляющий собой хеш данных в этом блоке. Этот `block_id` используется в качестве уникального ключа для операции вставки. Если такой же `block_id` найден в журнале дедупликации, блок считается дубликатом и не вставляется в таблицу.
...
Для запросов `INSERT ... VALUES` разделение вставляемых данных на блоки является детерминированным и определяется настройками. Поэтому при повторных попытках вставки следует использовать те же значения настроек, что и при первоначальной операции.
...
Приведенные выше настройки позволяют выполнить выборку из таблицы с серией блоков, содержащих только одну строку. Эти небольшие блоки не объединяются и остаются неизменными до момента их вставки в таблицу.
...
При указанных выше настройках запрос select возвращает два блока — следовательно, в таблицу `dst` должны быть вставлены два блока. Однако мы видим, что в таблицу `dst` был вставлен только один блок. Это произошло потому, что второй блок был дедуплицирован. Он содержит те же данные, и ключ дедупликации `block_id`, который вычисляется как хеш от вставляемых данных, совпадает. Такое поведение не соответствует ожиданиям. Подобные случаи встречаются редко, но теоретически возможны. Чтобы корректно обрабатывать такие ситуации, пользователь должен указать `insert_deduplication_token`. Рассмотрим, как это исправить, на следующих примерах:
...
Каждый раз мы вставляем разные данные. Однако в таблицу `mv_dst` попадают одни и те же данные. Данные не дедуплицируются, потому что исходные данные были разными.

### Разные материализованные представления вставляют в одну базовую таблицу эквивалентные данные {#different-materialized-view-inserts-into-one-underlying-table-with-equivalent-data}
