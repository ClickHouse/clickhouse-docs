...
При работе с данными временных рядов могут возникать пропуски из-за отсутствующих данных или периодов неактивности.
Обычно мы не хотим, чтобы такие пропуски присутствовали в результатах запросов. В этом случае может пригодиться предложение `WITH FILL`.
...
Мы будем анализировать изображения, созданные в период с `00:24:03` до `00:24:04` 24 марта 2023 года. Для этого создадим параметры для указанных временных точек:
...
По умолчанию для типов `DateTime` это значение равно 1 секунде, но нам нужно заполнить пропуски длиной 100 мс, поэтому зададим интервал 100 мс в качестве значения шага:
...
Из результатов видно, что теперь все интервалы с `00:24:03.000` по `00:24:03.500` присутствуют в выводе.
...
Все строки, содержащие `0` в столбце `count`, также содержат `0` в столбце cumulative, тогда как мы хотели бы использовать предыдущее значение из столбца `cumulative`.
