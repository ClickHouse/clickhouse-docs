...
keywords: ['time series', 'gap fill', 'временные ряды', 'заполнение пропусков']
...
При работе с данными временных рядов в них могут возникать пропуски из‑за отсутствующих данных или периодов неактивности.
Как правило, мы не хотим, чтобы такие пропуски присутствовали в результатах запросов. В таких случаях может пригодиться предложение `WITH FILL`.
...
Результирующий набор включает только те интервалы, в которых было создано изображение, однако для анализа временных рядов может потребоваться возвращать каждый 100-миллисекундный интервал, даже если в нём нет записей.


## WITH FILL {#with-fill}

Для заполнения этих пропусков можно использовать конструкцию `WITH FILL`.
Также необходимо указать `STEP` — размер заполняемых интервалов.
По умолчанию для типов `DateTime` используется значение 1 секунда, но в данном случае требуется заполнить пропуски длиной 100 мс, поэтому укажем интервал 100 мс в качестве значения шага:
...
Как видно, пропуски были заполнены нулевыми значениями в столбце `count`.
...
Из результатов видно, что теперь отображаются все интервалы от `00:24:03.000` до `00:24:03.500`.


## WITH FILL...TO {#with-fillto}

Однако в конце временного диапазона всё ещё отсутствуют некоторые интервалы, которые можно заполнить, указав значение `TO`.
`TO` не включает граничное значение, поэтому добавим небольшое количество времени к конечному моменту, чтобы гарантировать его включение:
...
Все пропуски теперь заполнены, и имеются записи для каждых 100 мс от `00:24:03.000` до `00:24:05.000`.
...
Значения в столбце `cumulative` работают не так, как ожидалось.


## WITH FILL...INTERPOLATE {#with-fillinterpolate}

Любые строки, имеющие `0` в столбце `count`, также имеют `0` в столбце cumulative, тогда как мы хотели бы использовать предыдущее значение из столбца `cumulative`.
...
Теперь, чтобы завершить работу, добавим столбчатую диаграмму с помощью функции `bar`, не забыв добавить новый столбец в конструкцию `INTERPOLATE`.
