...
При возникновении проблем с памятью или утечек памяти важно знать, какие запросы и ресурсы потребляют значительный объем памяти. Ниже приведены запросы, которые помогут вам выполнить отладку проблем с памятью, определив, какие запросы, базы данных и таблицы можно оптимизировать:


## Список текущих выполняемых процессов по пиковому потреблению памяти {#list-currently-running-processes-by-peak-memory}
