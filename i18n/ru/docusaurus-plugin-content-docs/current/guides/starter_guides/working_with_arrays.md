...
> В этом руководстве вы узнаете, как использовать массивы в ClickHouse, а также познакомитесь с некоторыми из наиболее часто используемых [функций работы с массивами](/sql-reference/functions/array-functions).
...
Эти значения называются _элементами_ массива, и к каждому элементу можно обратиться по индексу, который указывает позицию элемента в группе.
...
Может возникнуть желание создать массив из элементов разных типов следующим образом:

```sql
SELECT array('Hello', 'world', 1, 2, 3)
```

Однако элементы массива всегда должны иметь общий супертип — наименьший тип данных, который может представлять значения из двух или более различных типов без потерь, позволяя использовать их совместно.
Если общего супертипа не существует, при попытке создать массив возникнет исключение:
...
Можно использовать настройку `use_variant_as_common_type` для изменения описанного выше поведения по умолчанию.
Это позволяет использовать тип [Variant](/sql-reference/data-types/variant) в качестве результирующего типа для функций `if`/`multiIf`/`array`/`map`, когда для типов аргументов отсутствует общий тип.
...
Затем можно также извлекать типы из массива по имени типа:
...
Использование индекса в квадратных скобках обеспечивает удобный способ доступа к элементам массива.
В ClickHouse важно знать, что индексация массива всегда начинается с **1**.
...
```sql
WITH array('привет', 'мир') AS string_array
...
Несмотря на то что массивы индексируются с 1, вы всё равно можете обращаться к элементам с индексом 0.
Будет возвращено *значение по умолчанию* для типа элементов массива.
...
Функция `length` используется для получения количества элементов в массиве:
...
Если необходимо найти индекс конкретного значения, можно использовать функцию `indexOf`:
...
Обратите внимание, что эта функция возвращает первый найденный индекс, если в массиве присутствует несколько одинаковых значений.
...
Мы выбрали этот набор данных, поскольку массивы часто хорошо подходят для работы с данными временных рядов и могут помочь упростить
в противном случае сложные запросы.

:::tip
Нажмите кнопку воспроизведения ниже, чтобы выполнить запросы непосредственно в документации и увидеть результат в реальном времени.
:::
...
Давайте посмотрим, сколько из этих рейсов вылетели вовремя, были задержаны на 15-30 минут или задержаны более чем на 30 минут.
...
Для пар отправление/назначение он показывает бортовой номер и статус каждого рейса:

```sql runnable
WITH arrayMap(
              d -> if(d >= 30, 'ЗАДЕРЖАН', if(d >= 15, 'ПРЕДУПРЕЖДЕНИЕ', 'ВОВРЕМЯ')),
...
В приведенном выше запросе функция `arrayMap` принимает одноэлементный массив `[DepDelayMinutes]` и применяет лямбда-функцию `d -> if(d >= 30, 'DELAYED', if(d >= 15, 'WARNING', 'ON-TIME'` для его категоризации.
...
Далее определим, какие пары крупных аэропортов США обслуживают наибольшее количество общих направлений, с помощью функций [`arraySort`](/sql-reference/functions/array-functions#arraySort) и [`arrayIntersect`](/sql-reference/functions/array-functions#arrayIntersect).
...
Сначала он создает временный набор данных `airport_routes` с использованием обобщенного табличного выражения (CTE), которое анализирует все рейсы на 1 января 2024 года и для каждого аэропорта отправления формирует отсортированный список всех уникальных направлений, которые обслуживает этот аэропорт.
...
Это выполняется с помощью перекрестного соединения (cross join), которое создает все комбинации этих аэропортов.
...
Наконец, запрос сортирует результаты, чтобы показать, какие пары аэропортов имеют наибольшее количество общих направлений, и возвращает только первые 10.
...
Пока мы рассматриваем задержки, давайте используем ещё одну функцию высшего порядка для работы с массивами — `arrayReduce` — чтобы найти среднюю и максимальную задержку
...
В приведённом выше примере мы использовали `arrayReduce` для нахождения средней и максимальной задержки для различных исходящих рейсов из `DEN`.
`arrayReduce` применяет агрегатную функцию, указанную в первом параметре, к элементам массива, указанного во втором параметре.
...
Рассмотрим приведённый ниже запрос, который возвращает массив значений от 0 до 100 с шагом 10.
...
Приведённый ниже запрос создаёт гистограмму, показывающую распределение задержек рейсов из Денвера (DEN) в Майами (MIA) 1 января 2024 года, используя кумулятивные интервалы задержек:
...
В приведённом выше запросе мы возвращаем массив задержек, используя CTE-выражение (предложение `WITH`).
...
Для каждого порога задержки (`del`) запрос подсчитывает, сколько рейсов имели задержки, большие или равные этому порогу,
используя `countIf(DepDelayMinutes >= del)`.

У `arrayJoin` также есть эквивалентная SQL-команда `ARRAY JOIN`.
Приведённый выше запрос воспроизведён ниже с эквивалентной SQL-командой для сравнения:
...
Возможно, вам также будет полезно изучить связанные структуры данных, такие как типы [`tuples`](/sql-reference/data-types/tuple#creating-tuples), [JSON](/sql-reference/data-types/newjson) и [Map](/sql-reference/data-types/map), которые хорошо работают вместе с массивами.
Практикуйтесь в применении этих концепций к вашим собственным наборам данных и экспериментируйте с различными запросами в SQL-песочнице или на других примерах наборов данных.
