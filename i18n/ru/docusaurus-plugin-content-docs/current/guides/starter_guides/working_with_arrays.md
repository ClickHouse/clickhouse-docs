...
> В этом руководстве вы узнаете, как использовать массивы в ClickHouse, а также познакомитесь с некоторыми из наиболее распространённых [функций для работы с массивами](/sql-reference/functions/array-functions).
...
Массивы в ClickHouse можно создать с помощью функции [`array`](/sql-reference/data-types/array):
...
Если общего супертипа нет, при попытке создать массив вы получите исключение:
...
Использование индекса с квадратными скобками обеспечивает удобный способ доступа к элементам массива.
...
Так вы можете выбирать элементы относительно последнего элемента:
...
В этом случае будет возвращено *значение по умолчанию* для типа массива.
...
Чтобы найти индекс конкретного значения, используйте функцию `indexOf`:
...
Наши данные содержат одну строку на рейс, но было бы удобно сгруппировать данные по аэропорту отправления и собрать пункты назначения в массив.
...
Функция [`toStringCutToZero`](/sql-reference/functions/type-conversion-functions#tostringcuttozero) в запросе выше используется для удаления нулевых символов, которые появляются после трехбуквенного обозначения некоторых аэропортов.

Имея данные в таком формате, мы можем легко определить порядок самых загруженных аэропортов, найдя длину агрегированных массивов «Destinations»:
...
Функция `arrayIntersect` принимает несколько массивов и возвращает массив, содержащий элементы, присутствующие во всех переданных массивах.
...
Затем для каждой пары используется функция `arrayIntersect` для поиска направлений, которые присутствуют в списках обоих аэропортов.
...
Мы можем написать запрос с использованием `arrayJoin`, чтобы определить, сколько было задержек до указанного количества минут между двумя аэропортами.
...
Практикуйтесь в применении этих концепций на своих собственных наборах данных и экспериментируйте с различными запросами в SQL-песочнице или на других примерах данных.

Массивы — это фундаментальная возможность ClickHouse, обеспечивающая эффективные аналитические запросы. По мере освоения функций для работы с массивами вы обнаружите, что они могут значительно упростить сложные агрегации и анализ временных рядов.
Для более глубокого погружения в работу с массивами рекомендуем посмотреть видео на YouTube от Марка, нашего эксперта по данным:
