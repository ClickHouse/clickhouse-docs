...
Это достигается за счет сочетания высокопараллелизованной архитектуры и эффективного колоночного сжатия, но ценой компромисса в отношении немедленной согласованности данных.
Более конкретно, ClickHouse оптимизирован для операций только на добавление и предоставляет лишь гарантии согласованности в конечном итоге.

В отличие от этого, OLTP-базы данных, такие как Postgres, специально оптимизированы для транзакционных вставок с полным соблюдением ACID, обеспечивая строгие гарантии согласованности и надежности.
...
Эти транзакции могут затрагивать небольшое количество строк за раз, при этом возникают значительные накладные расходы из-за гарантий надежности, что ограничивает производительность вставки.
...
Соблюдение этих правил поможет избежать проблем, с которыми пользователи обычно сталкиваются при первом использовании ClickHouse, пытаясь применить стратегию вставки, работающую для OLTP-баз данных.
...
По умолчанию каждая вставка, отправленная в ClickHouse, приводит к немедленному созданию части хранилища, содержащей данные из вставки вместе с другими необходимыми метаданными.
Поэтому отправка меньшего количества вставок, каждая из которых содержит больше данных, по сравнению с отправкой большего количества вставок с меньшим объемом данных, сократит количество необходимых операций записи.
...
Кроме того, существует риск всплесков событий в наблюдаемых системах, что может потенциально вызвать большие всплески потребления памяти и связанные с этим проблемы при попытке буферизации данных мониторинга на стороне клиента.
...
Полную информацию о настройке асинхронных вставок можно найти [здесь](/optimize/asynchronous-inserts#enabling-asynchronous-inserts), а подробный разбор — [здесь](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse).
:::
...
Он более эффективен с точки зрения сжатия, сетевых издержек и обработки на сервере по сравнению с альтернативными строчными форматами, такими как [JSON](/interfaces/formats/JSON).
Формат [JSONEachRow](/interfaces/formats/JSONEachRow) можно рассмотреть для пользователей с низкой пропускной способностью записи, стремящихся к быстрой интеграции. Следует учитывать, что этот формат создаст дополнительную нагрузку на процессор ClickHouse при парсинге.
...
Его можно использовать как для вставки, так и для запроса данных с использованием любого из вышеперечисленных форматов.
...
Вы можете использовать знакомую команду `INSERT INTO TABLE` в ClickHouse. Давайте вставим некоторые данные в таблицу, которую мы создали в начальном руководстве ["Создание таблиц в ClickHouse"](./creating-tables).
...
Чтобы убедиться, что всё работает, выполним следующий запрос `SELECT`:
...
:::note
При вставке данных с помощью clickhouse-client в интерактивном режиме можно позволить ClickHouse самостоятельно выполнить распаковку при вставке, используя параметр `COMPRESSION`. ClickHouse может автоматически определить тип сжатия по расширению файла, но вы также можете указать его явно.
