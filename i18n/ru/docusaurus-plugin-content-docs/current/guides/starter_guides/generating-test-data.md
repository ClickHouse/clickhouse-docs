...
Генерация случайных данных полезна при тестировании новых сценариев использования или бенчмаркинга вашей реализации.
В ClickHouse есть [широкий набор функций для генерации случайных данных](/sql-reference/functions/random-functions), который во многих случаях позволяет обойтись без внешнего генератора данных.

В этом руководстве приведено несколько примеров генерации случайных наборов данных в ClickHouse с различными требованиями к случайности.



## Простой равномерный набор данных {#simple-uniform-dataset}

**Сценарий использования**: Быстрая генерация набора данных пользовательских событий со случайными временными метками и типами событий.
...
**Сценарий использования**: Моделирование сумм покупок, где большинство значений небольшие, но встречаются и высокие.
...
- `randExponential(1/10)` — большинство значений близки к 0, с минимальным смещением 15 ([ClickHouse][1], [ClickHouse][2], [Atlantic.Net][3], [GitHub][4])
...
- События достигают пика около полудня с отклонением, распределённым по Пуассону

---


## Нормальное распределение, изменяющееся во времени {#time-varying-normal-distribution}
...
- Три оценки для каждого интереса на пользователя
...
Вы также можете использовать seed (начальное значение), чтобы каждый раз получать одну и ту же структуру:
...
Теперь создадим реальную таблицу и заполним её случайными данными:
...
Посмотрите первую строку как пример сгенерированных данных:
