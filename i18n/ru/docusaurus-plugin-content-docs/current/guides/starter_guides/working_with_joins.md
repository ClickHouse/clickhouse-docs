...
Этот запрос находит жанры для каждого фильма, выполняя соединение таблицы `movies` с таблицей `genres`:
...
И поскольку, как уже упоминалось выше, ключевое слово `OUTER` можно опустить для `RIGHT OUTER JOIN`, а необязательное ключевое слово `ALL` можно добавить, вы можете написать `ALL RIGHT JOIN`, и всё будет работать корректно.
