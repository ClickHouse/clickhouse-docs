...
Создайте инкрементное материализованное представление, которое будет действовать как триггер вставки для вставляемых данных и поддерживать минимальную и максимальную температуры для каждого местоположения.
...
Эти показания автоматически обрабатываются материализованным представлением. Проверим текущее состояние:
...
Просмотрите обновленные экстремумы после добавления новых данных:
...
Обратите внимание, что выше для каждого местоположения присутствует два значения. Это связано с тем, что куски еще не были объединены (и агрегированы движком `AggregatingMergeTree`). Чтобы получить окончательный результат из частичных состояний, необходимо добавить `GROUP BY`:
...
Теперь получаем ожидаемый результат:
