...
Предположим, вы хотите узнать для каждого отдельного домена посадочной страницы (`URLDomain`)
на вашем сайте, какие уникальные коды ОС из User Agent (`OS`) были зафиксированы для
посетителей, попавших на этот домен. Это поможет вам понять разнообразие
...
WHERE URLDomain != '' -- Учитываем только обращения с указанным доменом
GROUP BY URLDomain
