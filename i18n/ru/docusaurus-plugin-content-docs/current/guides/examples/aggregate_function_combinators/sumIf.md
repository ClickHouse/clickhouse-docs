...
и используем `sumIf` для вычисления общей суммы успешных транзакций.
...
Функция `sumIf` суммирует только те значения amount, для которых `is_successful = 1`.
В данном случае будут просуммированы: 100.50 + 200.75 + 300.00 + 175.25.
...
чтобы посчитать объём торгов по тикерам акций за 2006 год для трёх крупнейших технологических
