...
для поиска значения `arg`, соответствующего максимальному значению `val` для строк, удовлетворяющих условию,
...
В этом примере мы используем набор данных о продажах товаров, чтобы продемонстрировать, как
работает `argMaxIf`. Мы найдём название товара с наибольшей ценой, но
...
Функция `argMaxIf` вернёт название товара с наибольшей ценой
среди всех товаров, которые были проданы не менее 10 раз (sales_count >= 10).
В данном случае она вернёт 'Laptop', так как у него наибольшая цена (999.99)
