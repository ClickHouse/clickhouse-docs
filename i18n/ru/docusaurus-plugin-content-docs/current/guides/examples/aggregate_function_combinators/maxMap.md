...
Комбинатор [`Map`](/sql-reference/aggregate-functions/combinators#-map) может применяться к функции [`max`](/sql-reference/aggregate-functions/reference/max) для вычисления максимального значения в Map по каждому ключу с помощью агрегатной функции-комбинатора `maxMap`.
...
`maxMap` для нахождения максимального количества для каждого кода статуса в каждом временном интервале.
