...
:::note
Политики строк имеют смысл только если у вас есть доступ `readonly`. Если вы можете изменять таблицу или копировать партиции между таблицами, это сводит на нет ограничения политик строк.
:::
...
Квота содержит набор ограничений для определённых периодов времени, а также список ролей и/или пользователей, к которым применяется эта квота.
...
Использование `GRANT ALTER on *.* TO my_user` повлияет только на операторы верхнего уровня `ALTER TABLE` и `ALTER VIEW`, остальные операторы `ALTER` должны предоставляться или отзываться по отдельности.
...
Привилегии будут выданы следующим образом:
...
1. Чтобы выдать администратору привилегию и предоставить ему возможность управлять набором привилегий
...
2. Добавьте пользователю привилегию на добавление столбцов

```sql
GRANT ADD COLUMN ON my_db.my_table TO my_user;
```

3. Войдите под учетной записью пользователя с ограниченными правами
...
6. Войдите в систему под пользователем my&#95;alter&#95;admin

```bash
clickhouse-client --user my_alter_admin --password password --port 9000 --host <my_clickhouse_host>
```

7. Выдача подпривилегии
...
**Краткое содержание**
Привилегии `ALTER` имеют иерархическую структуру для операций `ALTER` над таблицами и представлениями, но не для других операторов `ALTER`. Права могут задаваться на детализированном уровне или в виде групп привилегий, а также аналогичным образом отзываться. Пользователь, выдающий или отзывающий права, должен иметь `WITH GRANT OPTION`, чтобы назначать привилегии пользователям, включая себя как действующего пользователя, и при этом уже обладать соответствующей привилегией. Действующий пользователь не может отозвать собственные привилегии, если сам не обладает правом `WITH GRANT OPTION`.
