...
description: 'В этом руководстве приведены простые и минимальные настройки для использования в ClickHouse сертификатов OpenSSL для проверки соединений.'
...
В этом руководстве описаны простые и минимально необходимые настройки для конфигурации ClickHouse с использованием сертификатов OpenSSL для проверки соединений. В качестве демонстрации создаются самоподписанный сертификат и ключ центра сертификации (CA), а также сертификаты узлов для установления соединений с соответствующими настройками.

:::note
Реализация TLS — сложная задача, и существует множество параметров, которые необходимо учитывать, чтобы обеспечить полностью безопасное и надежное развертывание. Это базовый учебный пример с простыми примерами конфигурации SSL/TLS. Обратитесь к вашей команде, отвечающей за PKI/безопасность, чтобы сгенерировать корректные сертификаты для вашей организации.

Ознакомьтесь с этим [базовым учебным пособием по использованию сертификатов](https://ubuntu.com/server/docs/security-certificates) для вводного обзора.
:::
...
:::note
Самоподписанные сертификаты предназначены только для демонстрационных целей и не должны использоваться в продакшене. Запросы на сертификаты должны создаваться для подписания организацией и проверяться с использованием цепочки CA, которая будет настроена в параметрах. Однако эти шаги можно использовать для настройки и тестирования параметров, а затем заменить их фактическими сертификатами, которые будут использоваться.
:::
...
7. Убедитесь, что новые сертификаты проходят проверку по сертификату CA:
   ```bash
...
1. Создайте папку в директории, доступной для ClickHouse, на каждом узле. Рекомендуется использовать директорию конфигурации по умолчанию (например, `/etc/clickhouse-server`):

   ```bash
   mkdir /etc/clickhouse-server/certs
   ```

2. Скопируйте сертификат CA, сертификат узла и ключ, соответствующие каждому узлу, в новую директорию certs.
...
1. Добавьте следующее внутри тега `<clickhouse>` в файле `config.xml` сервера ClickHouse
   ...
   :::note
   Для производственных окружений рекомендуется использовать отдельный конфигурационный файл `.xml` в директории `config.d`.
...
4. Задайте значения макросов, чтобы можно было создать таблицу `ReplicatedMergeTree` для тестирования. На `chnode1`:
    ```xml
    <macros>
        <shard>1</shard>
...
6.  Настройте OpenSSL с указанием сертификатов и путей

    :::note
    Каждое имя файла и путь должны соответствовать узлу, на котором выполняется настройка.
    Например, при настройке на хосте `chnode2` измените запись `<certificateFile>` на `chnode2.crt`.
    :::
    ...
    Подробнее см. https://clickhouse.com/docs/operations/server-configuration-parameters/settings/#server_configuration_parameters-openssl
...
7.  Настройте gRPC для работы с SSL на каждом узле:
    ...
    Подробнее см. https://clickhouse.com/docs/interfaces/grpc/
...
2. Убедитесь, что защищённые порты запущены и прослушиваются. Вывод должен быть похож на этот пример на каждом узле:
...
3. Проверьте работоспособность ClickHouse Keeper
   Стандартные команды [4 letter word (4lW)](/guides/sre/keeper/index.md#four-letter-word-commands) не будут работать при использовании `echo` без TLS. Вот как использовать команды с `openssl`.
...
5. Войдите в интерфейс Play по `https` по адресу `https://chnode1.marsnet.local:8443/play`.

   <Image img={configuringSsl01} alt="Настройка SSL" size="md" border />

   :::note
   Браузер отобразит недоверенный сертификат, поскольку доступ осуществляется с рабочей станции, а сертификаты не находятся в хранилищах корневых центров сертификации на клиентской машине.
   При использовании сертификатов, выданных публичным удостоверяющим центром или корпоративным CA, сертификат будет отображаться как доверенный.
   :::
...
В этой статье рассматривалась настройка окружения ClickHouse с использованием SSL/TLS. Параметры будут различаться в зависимости от требований производственных сред; например, уровни проверки сертификатов, протоколы, шифры и т. д. Теперь у вас должно быть чёткое представление о шагах, необходимых для настройки и реализации защищённых соединений.
