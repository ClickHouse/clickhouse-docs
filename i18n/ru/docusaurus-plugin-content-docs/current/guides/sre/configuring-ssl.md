...
description: 'В этом руководстве описаны простые и минимальные настройки, позволяющие настроить ClickHouse на использование сертификатов OpenSSL для проверки подключений.'
...
В этом руководстве приведены простые и минимальные настройки для конфигурирования ClickHouse на использование сертификатов OpenSSL для проверки соединений. В целях демонстрации создаются самоподписанные сертификат и ключ центра сертификации (CA), а также сертификаты узлов для установления соединений с использованием соответствующих настроек.

:::note
Реализация TLS сложна, и существует множество вариантов, которые необходимо учитывать для обеспечения полностью безопасного и надежного развертывания. Это базовое руководство с примерами простой конфигурации SSL/TLS. Обратитесь к вашей команде, отвечающей за PKI/безопасность, чтобы сгенерировать корректные сертификаты для вашей организации.
...
:::note
Самоподписанные сертификаты предназначены только для демонстрационных целей и не должны использоваться в production-среде. Запросы на сертификаты должны создаваться для подписи организацией и проверяться с использованием цепочки CA, которая будет настроена в конфигурации. Однако эти шаги можно использовать для настройки и тестирования параметров, а затем заменить их на реальные сертификаты, которые будут использоваться.
:::
...
2. Сгенерируйте новый самоподписанный сертификат CA. Следующая команда создаст новый сертификат, который будет использоваться для подписи других сертификатов с помощью ключа CA:
   ...
   :::note
   Создайте резервную копию ключа и сертификата CA в безопасном месте вне кластера. После генерации сертификатов узлов ключ следует удалить с узлов кластера.
   :::
...
7. Убедитесь, что новые сертификаты успешно проверяются с помощью сертификата CA:
   ```bash
...
1. Добавьте следующее внутри тега `<clickhouse>` в файле `config.xml` сервера ClickHouse
   ...
   :::note
   Для production-окружений рекомендуется использовать отдельный конфигурационный файл `.xml` в директории `config.d`.
...
6.  Настройте OpenSSL, указав сертификаты и пути к ним
    ...
    Например, измените значение `<certificateFile>` на `chnode2.crt` при настройке на хосте `chnode2`.
    :::
...
3. Проверьте работоспособность ClickHouse Keeper
   Стандартные команды [4 letter word (4lW)](/guides/sre/keeper/index.md#four-letter-word-commands) не будут работать с `echo` без TLS. Вот как использовать эти команды с `openssl`.
...
CONNECTED(00000003)
depth=0 CN = chnode1
verify error:num=20:не удалось получить локальный сертификат издателя
verify return:1
depth=0 CN = chnode1
verify error:num=21:не удалось проверить первый сертификат
verify return:1
---
...
В этой статье рассматривалась настройка окружения ClickHouse с использованием SSL/TLS. Параметры будут отличаться в зависимости от требований производственных сред; например, уровни проверки сертификатов, протоколы, шифры и т. д. Теперь у вас должно быть чёткое представление о шагах, необходимых для настройки и внедрения защищённых соединений.
