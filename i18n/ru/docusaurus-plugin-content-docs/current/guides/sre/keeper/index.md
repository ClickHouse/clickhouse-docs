...
#### Настройки конфигурации ClickHouse Keeper \{#keeper-configuration-settings\}
...
| `server_id`                         | Уникальный идентификатор сервера, каждый участник кластера ClickHouse Keeper должен иметь уникальный номер (1, 2, 3...).                                                                                                                                                | -                                                                                                            |
...
| `http_control`                      | Конфигурация интерфейса [HTTP-контроля](#http-control).                                                                                                                                                                                                                 | -                                                                                                            |
...
ClickHouse Keeper входит в пакет сервера ClickHouse: просто добавьте конфигурацию `<keeper_server>` в `/etc/your_path_to_config/clickhouse-server/config.xml` и запустите сервер ClickHouse как обычно. Если вы хотите запустить ClickHouse Keeper как отдельный сервис, вы можете сделать это аналогичным образом:

```bash
clickhouse-keeper --config /etc/your_path_to_config/config.xml
```

Если у вас нет символической ссылки (`clickhouse-keeper`), вы можете создать её или указать `keeper` в качестве аргумента команды `clickhouse`:
...
ClickHouse Keeper также предоставляет команды 4lw, которые почти такие же, как у ZooKeeper. Каждая команда состоит из четырёх букв, таких как `mntr`, `stat` и т. д. Есть несколько более интересных команд: `stat` даёт общую информацию о сервере и подключённых клиентах, `srvr` предоставляет расширенные сведения о сервере, а `cons` — расширенные сведения о соединениях.

Для команд 4lw существует настройка списка разрешённых команд `four_letter_word_white_list`, которая по умолчанию имеет значение `conf,cons,crst,envi,ruok,srst,srvr,stat,wchs,dirs,mntr,isro,rcvr,apiv,csnp,lgif,rqld,ydld`.

Вы можете отправлять команды в ClickHouse Keeper через telnet или nc по клиентскому порту.
...
* `ruok`: Проверяет, работает ли сервер в исправном состоянии. Сервер ответит `imok`, если он запущен. В противном случае он не ответит вовсе. Ответ `imok` не обязательно означает, что сервер присоединился к кворуму, а лишь то, что серверный процесс активен и привязан к указанному клиентскому порту. Используйте &quot;stat&quot; для получения подробной информации о состоянии сервера в отношении кворума и о подключениях клиентов.
...
* `cons`: Выводит полную информацию о подключениях/сеансах для всех клиентов, подключённых к этому серверу. Включает сведения о количестве полученных/отправленных пакетов, идентификаторе сеанса, задержках операций, последней выполненной операции и т. д.
...
* `crst`: Сбрасывает статистику подключений/сессий для всех соединений.

```response
Connection stats reset.
```

* `envi`: Выводит сведения о среде сервера
...
* `dirs`: Показывает общий размер файлов снимков и логов в байтах
...
* `csnp`: Планирует задачу создания снимка. Возвращает индекс последнего зафиксированного журнала запланированного снимка в случае успеха или `Failed to schedule snapshot creation task.` в случае ошибки. Команда `lgif` может помочь определить, завершено ли создание снимка.

```response
100
```

* `lgif`: Информация о логе Keeper. `first_log_idx` : мой первый индекс лога в хранилище логов; `first_log_term` : мой первый терм лога; `last_log_idx` : мой последний индекс лога в хранилище логов; `last_log_term` : мой последний терм лога; `last_committed_log_idx` : мой последний зафиксированный индекс лога в машине состояний; `leader_committed_log_idx` : зафиксированный индекс лога лидера в моём представлении; `target_committed_log_idx` : целевой индекс лога, который должен быть зафиксирован; `last_snapshot_idx` : максимальный зафиксированный индекс лога в последнем снимке.
...
* `rqld`: Запрос на назначение нового лидера. Возвращает `Sent leadership request to leader.` если запрос отправлен, или `Failed to send leadership request to leader.` если запрос не был отправлен. Если узел уже является лидером, результат будет таким же, как при отправке запроса.

```response
Sent leadership request to leader.
```

* `ftfl`: Выводит список всех флагов возможностей и их состояние (включены/выключены) для данного экземпляра Keeper.
...
* `ydld`: Запрос на передачу лидерства и переход в состояние follower. Если сервер, получающий запрос, является лидером, он сначала приостановит операции записи, дождётся, пока преемник (текущий лидер никогда не может быть преемником) синхронизируется с последним логом, и затем откажется от лидерства. Преемник будет выбран автоматически. Возвращает `Sent yield leadership request to leader.` если запрос отправлен, или `Failed to send yield leadership request to leader.` если запрос не отправлен. Если узел уже является follower, результат будет тем же, что и при отправке запроса.
...
ClickHouse Keeper предоставляет HTTP-интерфейс для проверки готовности реплики к приёму трафика. Его можно использовать в облачных средах, например в [Kubernetes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-readiness-probes).
...
:::note
Начиная с версии 25.7 некоторые флаги возможностей включены по умолчанию.
...
Бесшовная миграция с ZooKeeper на ClickHouse Keeper невозможна. Необходимо остановить кластер ZooKeeper, преобразовать данные и запустить ClickHouse Keeper. Утилита `clickhouse-keeper-converter` преобразует журналы и снимки состояния ZooKeeper в снимок состояния ClickHouse Keeper. Для ее работы требуется ZooKeeper 3.4 или более поздней версии.

#### Подготовка к миграции \{#pre-migration-preparation\}

Для миграции требуется остановить ингестию данных. Перед началом запланируйте окно обслуживания.

Перед остановкой ZooKeeper остановите фоновые задачи ClickHouse, изменяющие координационные метаданные. Например:

```sql
SYSTEM STOP MERGES;
```

Зафиксируйте контрольные метрики перед миграцией, чтобы затем проверить согласованность.
...
6. Скопируйте снимок на все узлы ClickHouse Keeper. Снимок должен присутствовать на каждом узле до запуска любого из них — если узел запустится без снимка, он может избрать себя лидером с пустым состоянием.
...
9. Сравните метрики с исходными значениями до миграции, чтобы проверить согласованность.
...
Если вы используете несколько кластеров ZooKeeper — например, по одному для каждой группы сегментов, — их можно объединить в один кластер ClickHouse Keeper. Официальный инструмент `clickhouse-keeper-converter` поддерживает только преобразование один к одному (один кластер ZooKeeper в один снимок Keeper), поэтому для такого объединения потребуется изменить исходный код конвертера, чтобы слить несколько снимков:

1. Запустите `clickhouse-keeper-converter` отдельно для каждого кластера ZooKeeper, записав каждый результат в отдельный каталог.
...
#### Обработка шифрования и ACL \{#handling-encryption-and-acls\}

ClickHouse Keeper поддерживает те же схемы ACL, что и ZooKeeper (`world`, `auth`, `digest`). Способ обработки ACL при преобразовании зависит от конфигурации ZooKeeper:

* **Полностью зашифровано или полностью не зашифровано**: Выполните преобразование напрямую. Конвертер сохраняет существующую информацию об ACL.
* **Частично зашифровано**: Перед преобразованием назначьте аккаунту права суперадминистратора и очистите ACL с помощью `setAcl -R` для затронутых путей. Выполните преобразование, затем при необходимости снова включите шифрование в ClickHouse Keeper.

#### Проверка миграции \{#verifying-the-migration\}

После запуска ClickHouse Keeper и перезапуска ClickHouse сравните ключевые метрики с базовым уровнем до миграции, чтобы убедиться, что миграция прошла успешно.

При консолидации нескольких кластеров ZooKeeper различайте:

* **Общие пути**: пути, присутствующие в нескольких исходных кластерах с идентичными данными, — их дубликаты следует удалить в объединенном результате.
* **Различающиеся пути**: пути, существующие только в определенных кластерах (например, в `/clickhouse/tables` для каждой группы сегментов), — их необходимо сохранить из соответствующего источника.

Не обходите большие деревья ZooKeeper напрямую для сравнения. Вместо этого во время преобразования выведите все преобразованные пути в файл.

#### Настройка после миграции \{#post-migration-tuning\}

После миграции при необходимости скорректируйте эти параметры для более крупных кластеров или более высокой пропускной способности:

| Параметр                              | По умолчанию | Рекомендуется | Примечания                                                         |
| ------------------------------------- | ------------ | ------------- | ------------------------------------------------------------------ |
| `max_requests_batch_size`             | `100`        | `10000`       | Увеличьте для кластеров с большим количеством частей или сегментов |
| `force_sync`                          | `true`       | `false`       | Асинхронная запись логов повышает пропускную способность           |
| `compress_logs`                       | `false`      | `true`        | Сжимает файлы лога Raft, чтобы уменьшить дисковый I/O              |
| `compress_snapshots_with_zstd_format` | `true`       | —             | Уже включено по умолчанию; сжимает снимки в формате zstd           |

Эти параметры задаются в разделе `coordination_settings` в [конфигурации Keeper](#internal-coordination-settings).
...
К этому следует прибегать только в самом крайнем случае, если вы не можете заново запустить узлы или развернуть новый экземпляр на той же конечной точке.
...
Чтобы использовать диск для снимков, параметр конфигурации `keeper_server.snapshot_storage_disk` должен содержать имя диска.
Кроме того, `keeper_server.latest_log_storage_disk` можно использовать для последних логов, а `keeper_server.latest_snapshot_storage_disk` — для последних снимков.
...
Такая же логика применяется к снимкам: все, кроме последних снимков, будут храниться на `snapshot_s3_plain`, а последний снимок — на диске `snapshot_local`.
...
При запуске все файлы журнала будут перенесены с дисков `log_local` и `log_s3_plain` на диск `log_local2`.
Также все файлы снимков будут перенесены с дисков `snapshot_local` и `snapshot_s3_plain` на диск `snapshot_local2`.
...
* `endpoint` – HTTP-конечная точка для сбора метрик сервером Prometheus. Должен начинаться с &#39;/&#39;.
...
1. Настроим простой кластер с 2 сегментами и только одной репликой на 2 из узлов. Третий узел будет использоваться для достижения кворума, требуемого в ClickHouse Keeper. Обновите конфигурацию на `chnode1` и `chnode2`. Следующий кластер определяет по 1 сегменту на каждом узле, всего 2 сегмента без репликации. В этом примере часть данных будет находиться на одном узле, а часть — на другом:
   ...
   | Parameter | Description                                                                              | Example                            |
   | --------- | ---------------------------------------------------------------------------------------- | ---------------------------------- |
   | сегмент   | список реплик в определении кластера                                                     | список реплик для каждого сегмента |
   | replica   | список настроек для каждой реплики                                                       | записи настроек для каждой реплики |
   | host      | hostname, IP или FQDN сервера, на котором будет размещаться реплика сегмента             | `chnode1.domain.com`               |
...
### 3. Создайте и протестируйте distributed таблицу \{#3-create-and-test-distributed-table\}

1.  Создайте новую базу данных в новом кластере, используя клиент ClickHouse на `chnode1`. Оператор `ON CLUSTER` автоматически создаёт базу данных на обоих узлах.
    ```sql
...
обеспечивает три узла для ClickHouse Keeper (включая узел-арбитр) и один сегмент
...
3. Просмотрите записи в distributed таблице
...
для динамической переконфигурации кластера, если параметр `keeper_server.enable_reconfiguration` включен.

:::note
Если этот параметр выключен, вы можете переконфигурировать кластер, вручную изменив раздел `raft_configuration`
реплики. Убедитесь, что вы редактируете файлы на всех репликах, так как изменения применит только лидер.
В качестве альтернативы вы можете отправить команду `reconfig` через любой клиент, совместимый с ZooKeeper.
:::
...
* Каждая запись о сервере разделяется переводом строки.
...
* `server_priority` — это неотрицательное целое число, указывающее, [каким узлам следует отдавать приоритет при выборах лидера](https://github.com/eBay/NuRaft/blob/master/docs/leader_election_priority.md).
...
Вы можете использовать команду `reconfig` для добавления новых серверов, удаления существующих и изменения приоритетов уже работающих серверов, ниже приведены примеры (с использованием `clickhouse-keeper-client`):
...
* В отличие от ZooKeeper, отсутствует возможность ожидания переконфигурации кластера путем отправки команды `sync`.
  Новая конфигурация будет *в конечном итоге* применена, но без гарантий по времени.
...
Хотя ClickHouse Keeper стремится быть полностью совместимым с ZooKeeper, есть несколько функций, которые ещё не реализованы (разработка продолжается):
