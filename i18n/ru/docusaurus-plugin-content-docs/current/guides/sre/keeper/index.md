...
* `rqld`: Запрос на назначение нового лидера. Возвращает `Sent leadership request to leader.` если запрос отправлен, или `Failed to send leadership request to leader.` если запрос не был отправлен. Обратите внимание, что если узел уже является лидером, результат будет таким же, как при отправке запроса.
...
* `ydld`: Запрос на передачу лидерства и переход в состояние follower. Если сервер, получающий запрос, является лидером, он сначала приостановит операции записи, дождётся, пока преемник (текущий лидер никогда не может быть преемником) синхронизируется с последним логом, и затем откажется от лидерства. Преемник будет выбран автоматически. Возвращает `Sent yield leadership request to leader.` если запрос отправлен, или `Failed to send yield leadership request to leader.` если запрос не отправлен. Обратите внимание, что если узел уже является follower, результат будет тем же, как если бы запрос был отправлен.
