...
По умолчанию ClickHouse Keeper предоставляет те же гарантии, что и ZooKeeper: линеаризуемую запись и нелинеаризуемое чтение. Он имеет совместимый клиент-серверный протокол, поэтому любой стандартный клиент ZooKeeper может использоваться для взаимодействия с ClickHouse Keeper. Снимки и журналы имеют несовместимый с ZooKeeper формат, но инструмент `clickhouse-keeper-converter` позволяет конвертировать данные ZooKeeper в снимки ClickHouse Keeper. Межсерверный протокол в ClickHouse Keeper также несовместим с ZooKeeper, поэтому смешанный кластер ZooKeeper / ClickHouse Keeper невозможен.
...
ClickHouse Keeper может использоваться как автономная замена ZooKeeper или как внутренняя часть сервера ClickHouse. В обоих случаях конфигурация представляет собой практически одинаковый файл `.xml`.
...
| `heart_beat_interval_ms`           | Как часто лидер ClickHouse Keeper отправляет heartbeat-сигналы последователям (мс)                                                                                                                                       | `500`                                                                                                        |
| `election_timeout_lower_bound_ms`  | Если последователь не получает heartbeat-сигнал от лидера в течение этого интервала, он может инициировать выборы лидера. Должен быть меньше либо равен `election_timeout_upper_bound_ms`. В идеале они не должны быть равны. | `1000`                                                                                                       |
| `election_timeout_upper_bound_ms`  | Если последователь не получает heartbeat-сигнал от лидера в течение этого интервала, он должен инициировать выборы лидера.                                                                                              | `2000`                                                                                                       |
...
| `snapshot_distance`                | Как часто ClickHouse Keeper будет создавать новые снимки (по количеству записей в журналах).                                                                                                                             | `100000`                                                                                                     |
| `snapshots_to_keep`                | Сколько снимков хранить.                                                                                                                                                                                                 | `3`                                                                                                          |
| `stale_log_gap`                    | Порог, при котором лидер считает последователь устаревшим и отправляет ему снимок вместо журналов.                                                                                                                      | `10000`                                                                                                      |
| `fresh_log_gap`                    | Порог, после достижения которого узел считается «актуальным».                                                                                                                                                           | `200`                                                                                                        |
...
| `auto_forwarding`                  | Разрешить пересылку запросов на запись от узлов-последователей к лидеру.                                                                                                                                                 | `true`                                                                                                       |
| `shutdown_timeout`                 | Ожидание завершения внутренних подключений и остановки (мс).                                                                                                                                                             | `5000`                                                                                                       |
...
| `commit_logs_cache_size_threshold` | Максимальный общий размер кэша в памяти для записей журнала, необходимых для следующей фиксации (commit)                                                                                                                | `500MiB`                                                                                                     |
| `disk_move_retries_wait_ms`        | Как долго ждать между повторами после сбоя, произошедшего во время перемещения файла между дисками                                                                                                                       | `1000`                                                                                                       |
...
| `experimental_use_rocksdb`         | Использовать `rocksdb` как backend-хранилище                                                                                                                                                                             | `0`                                                                                                          |

Конфигурация кворума расположена в секции `<keeper_server>.<raft_configuration>` и содержит описание серверов.

Единственный параметр для всего кворума — `secure`, который включает зашифрованное соединение для обмена между участниками кворума. Параметр может быть установлен в `true`, если для внутреннего взаимодействия между узлами требуется SSL-соединение, или оставлен неуказанным в противном случае.

Основные параметры для каждого `<server>` следующие:
...
:::note
`async_replication` по умолчанию отключена для сохранения обратной совместимости. Если все экземпляры Keeper в кластере работают на версии, поддерживающей `async_replication` (v23.9+), рекомендуется включить её, так как это может улучшить производительность без каких-либо недостатков.
:::
...
ClickHouse Keeper входит в состав пакета сервера ClickHouse. Просто добавьте конфигурацию `<keeper_server>` в файл `/etc/your_path_to_config/clickhouse-server/config.xml` и запустите сервер ClickHouse как обычно. Если вы хотите запустить автономный ClickHouse Keeper, запустите его аналогичным образом:
...
ClickHouse Keeper также предоставляет 4lw-команды, которые практически идентичны командам Zookeeper. Каждая команда состоит из четырёх букв, таких как `mntr`, `stat` и т. д. Наиболее полезные команды: `stat` предоставляет общую информацию о сервере и подключённых клиентах, а `srvr` и `cons` предоставляют расширенные сведения о сервере и соединениях соответственно.
...
* `srvr`: Показывает полную информацию о сервере.
...
* `stat`: Показывает краткую информацию о сервере и подключённых клиентах.
...
* `conf`: Выводит сведения о конфигурации сервиса.
...
* `cons`: Выводит полную информацию о соединениях/сессиях для всех клиентов, подключённых к этому серверу. Включает сведения о количестве полученных/отправленных пакетов, идентификаторе сессии, задержках операций, последней выполненной операции и т. д.
...
* `crst`: Сброс статистики соединений/сессий для всех соединений.

```response
Статистика подключений сброшена.
```

* `envi`: Выводит сведения о среде запуска
...
* `dirs`: Показывает общий размер файлов snapshot и журналов в байтах
...
* `isro`: Проверяет, запущен ли сервер в режиме только чтения. Сервер ответит `ro`, если он работает в режиме только чтения, или `rw`, если нет.
...
* `wchc`: Выводит подробную информацию о наблюдателях (`watches`) для сервера по сессиям. Результат содержит список сессий (соединений) с соответствующими наблюдателями (путями). Обратите внимание, что в зависимости от количества наблюдателей эта операция может быть ресурсоёмкой (влиять на производительность сервера), поэтому используйте её с осторожностью.
...
* `csnp`: Планирует задачу создания снепшота. Возвращает индекс последнего зафиксированного лога запланированного снепшота в случае успеха или `Failed to schedule snapshot creation task.` в случае сбоя. Обратите внимание, что команда `lgif` может помочь определить, завершено ли создание снепшота.

```response
100
```

* `lgif`: Информация о журнале Keeper. `first_log_idx` : мой первый индекс записи в журнале; `first_log_term` : мой первый термин журнала; `last_log_idx` : мой последний индекс записи в журнале; `last_log_term` : мой последний термин журнала; `last_committed_log_idx` : мой последний зафиксированный индекс журнала в конечном автомате; `leader_committed_log_idx` : зафиксированный индекс журнала лидера с моей точки зрения; `target_committed_log_idx` : целевой индекс журнала, который должен быть зафиксирован; `last_snapshot_idx` : наибольший зафиксированный индекс журнала в последнем снимке.
...
* `rqld`: Запрос на назначение нового лидера. Возвращает `Sent leadership request to leader.` если запрос отправлен или `Failed to send leadership request to leader.` если запрос не был отправлен. Обратите внимание, что если узел уже является лидером, результат будет таким же, как и в случае отправки запроса.

```response
Запрос на лидерство отправлен ведущему узлу.
```

* `ftfl`: Перечисляет все feature-флаги и показывает, включены ли они для экземпляра Keeper.
...
* `ydld`: Запрос на передачу лидерства и переход в состояние follower. Если сервер, получающий запрос, является лидером, он сначала приостановит операции записи, дождётся, пока преемник (текущий лидер никогда не может быть преемником) не закончит догонять состояние по последнему журналу, а затем откажется от лидерства. Преемник будет выбран автоматически. Возвращает `Sent yield leadership request to leader.` если запрос отправлен, или `Failed to send yield leadership request to leader.` если запрос не отправлен. Обратите внимание, что если узел уже находится в состоянии follower, результат будет таким же, как если бы запрос был отправлен.

```response
Отправлен запрос на передачу лидерства ведущему узлу.
```
...
4. Отправьте четырёхбуквенную команду `rcvr` выбранному узлу, что переведёт узел в режим восстановления, ИЛИ остановите экземпляр Keeper на выбранном узле и запустите его снова с аргументом `--force-recovery`.
5. По одному запускайте экземпляры Keeper на новых узлах, убеждаясь, что `mntr` возвращает `follower` для `zk_server_state` перед запуском следующего.
...
7. После достижения кворума узел-лидер вернётся к нормальному режиму работы, принимая все запросы с использованием Raft — проверьте с помощью `mntr`, которая должна возвращать `leader` для `zk_server_state`.
...
Чтобы использовать диск для журналов, необходимо установить параметр конфигурации `keeper_server.log_storage_disk` равным имени диска.
Чтобы использовать диск для снимков состояния, необходимо установить параметр конфигурации `keeper_server.snapshot_storage_disk` равным имени диска.
Кроме того, для последних журналов или снимков состояния можно использовать отдельные диски, указав соответственно `keeper_server.latest_log_storage_disk` и `keeper_server.latest_snapshot_storage_disk`.
...
Чтобы использовать диск для файла состояния, необходимо установить параметр конфигурации `keeper_server.state_storage_disk` равным имени диска.
...
Если используется `force_sync`, то `log_storage_disk` должен быть диском типа `local`, если не используется `latest_log_storage_disk`.
...
Если значения по умолчанию слишком велики, можно уменьшить потребление памяти, снизив значения этих двух параметров.
...
- `metrics` – Флаг, определяющий предоставление метрик из таблицы [system.metrics](/operations/system-tables/metrics).
- `events` – Флаг, определяющий предоставление метрик из таблицы [system.events](/operations/system-tables/events).
- `asynchronous_metrics` – Флаг, определяющий предоставление текущих значений метрик из таблицы [system.asynchronous_metrics](/operations/system-tables/asynchronous_metrics).
...
Это руководство содержит простые и минимальные настройки для конфигурирования ClickHouse Keeper с примером тестирования распределённых операций. Пример выполняется с использованием 3 узлов на Linux.
...
2. На каждом узле добавьте следующую запись для разрешения внешних подключений через сетевой интерфейс.
...
6. База данных `system` содержит таблицу `zookeeper`, в которой хранятся сведения о ваших экземплярах ClickHouse Keeper. Просмотрим таблицу:
   ```sql
   SELECT *
...
1. Установите значения по умолчанию для таблиц на каждом узле
...
:::tip
Вы также можете определить макрос `{database}` на каждом узле, если узлы используются для определённых баз данных.
:::
...
приоритетов существующих серверов. Вот примеры (с использованием `clickhouse-keeper-client`):
...
- Поддерживается только инкрементальная переконфигурация. Запросы с непустым полем `new_members` отклоняются.
  ...
  Изменение типа сервера (participant/learner) также невозможно, поскольку это не поддерживается NuRaft, а
  единственным способом было бы удаление и повторное добавление сервера, что также могло бы ввести в заблуждение.
...
- Поле `from_version` не используется. Все запросы с установленным `from_version` отклоняются.
  ...
  Такое решение было принято, чтобы избежать дублирования данных, поскольку NuRaft уже хранит эту конфигурацию.
- В отличие от ZooKeeper, нет способа ожидать переконфигурации кластера путем отправки команды `sync`.
  Новая конфигурация будет _в конечном итоге_ применена, но без гарантий по времени.
- Команда `reconfig` может завершиться неудачей по различным причинам. Вы можете проверить состояние кластера и убедиться, было ли применено обновление.
```


## Преобразование однонодового keeper в кластер {#converting-a-single-node-keeper-into-a-cluster}

Иногда необходимо расширить экспериментальный узел keeper до кластера. Ниже приведена пошаговая инструкция для кластера из 3 узлов:
