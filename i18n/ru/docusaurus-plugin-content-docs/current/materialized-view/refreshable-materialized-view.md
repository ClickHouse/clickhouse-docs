...
[Обновляемые материализованные представления](/sql-reference/statements/create/view#refreshable-materialized-view) концептуально похожи на материализованные представления в традиционных OLTP-базах данных: они хранят результат заданного запроса для быстрого доступа и снижают необходимость многократно выполнять ресурсоёмкие запросы. В отличие от [инкрементальных материализованных представлений](/materialized-view/incremental-materialized-view) в ClickHouse, в этом случае требуется периодический запуск запроса по всему набору данных, результаты которого сохраняются в целевой таблице для последующих запросов. Теоретически этот результирующий набор должен быть меньше исходного набора данных, что позволяет выполнять такие запросы быстрее.
...
Также вы можете посмотреть следующее видео:
...
Инкрементные материализованные представления ClickHouse обладают огромной мощностью и, как правило, масштабируются значительно лучше, чем подход с использованием обновляемых материализованных представлений, особенно в случаях, когда требуется выполнить агрегацию по одной таблице. Вычисляя агрегацию только для каждого блока данных при его вставке и объединяя инкрементные состояния в итоговой таблице, запрос всегда выполняется только на подмножестве данных. Этот метод масштабируется до потенциальных петабайтов данных и обычно является предпочтительным.

Однако существуют сценарии использования, где этот инкрементный процесс не требуется или неприменим. Некоторые задачи либо несовместимы с инкрементным подходом, либо не требуют обновлений в реальном времени — в таких случаях более подходящим является периодическое перестроение. Например, может потребоваться регулярно выполнять полный пересчёт представления по всему набору данных, поскольку оно использует сложное соединение, несовместимое с инкрементным подходом.

> Обновляемые материализованные представления могут выполнять пакетные процессы, такие как денормализация. Между обновляемыми материализованными представлениями можно создавать зависимости таким образом, чтобы одно представление зависело от результатов другого и выполнялось только после его завершения. Это может заменить запланированные рабочие процессы или простые DAG, такие как задания [dbt](https://www.getdbt.com/). Чтобы узнать больше о том, как настроить зависимости между обновляемыми материализованными представлениями, перейдите к разделу `Dependencies` в [CREATE VIEW](/sql-reference/statements/create/view#refresh-dependencies).
...
Чтобы узнать, когда обновляемое материализованное представление обновлялось в последний раз, выполните запрос к системной таблице [`system.view_refreshes`](/operations/system-tables/view_refreshes), как показано ниже:
...
В этом наборе данных содержится `4096` значений в столбце `uuid`. Мы можем написать следующий запрос, чтобы найти значения с наибольшим суммарным количеством:
...
После этого мы можем запросить `events_snapshot`, чтобы получить изменение количества во времени для конкретного `uuid`:
...
Получение результата занимает немного времени, но предположим, что мы хотим сделать его ещё более быстрым и менее ресурсоёмким.
Также предположим, что этот набор данных постоянно обновляется: выходят новые фильмы, появляются новые актёры и режиссёры.
...
Представление будет выполнено сразу, а затем будет запускаться каждую минуту, как и настроено, чтобы изменения в исходной таблице автоматически отражались. Наш предыдущий запрос для получения сводной информации об актёрах стал синтаксически проще и значительно быстрее!
...
Предположим, мы добавим в наши исходные данные нового актёра по имени &quot;Clicky McClickHouse&quot;, который, как оказалось, снялся во множестве фильмов!
...
Менее чем через 60 секунд наша целевая таблица обновляется, отражая огромную продуктивность актёрской деятельности Clicky:
