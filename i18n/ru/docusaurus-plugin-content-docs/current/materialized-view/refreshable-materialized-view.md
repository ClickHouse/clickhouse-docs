...
[Обновляемые материализованные представления](/sql-reference/statements/create/view#refreshable-materialized-view) концептуально похожи на материализованные представления в традиционных OLTP-базах данных: они хранят результат заданного запроса для быстрого доступа и снижают потребность в повторном выполнении ресурсоёмких запросов. В отличие от [инкрементальных материализованных представлений](/materialized-view/incremental-materialized-view) в ClickHouse, в их случае требуется периодическое выполнение запроса по всему набору данных, результаты которого сохраняются в целевой таблице для последующих запросов. Теоретически этот результирующий набор данных должен быть меньше исходного, что позволяет последующим запросам выполняться быстрее.
...
Инкрементные материализованные представления ClickHouse чрезвычайно мощны и обычно масштабируются гораздо лучше, чем подход, используемый обновляемыми материализованными представлениями, особенно в случаях, когда необходимо выполнить агрегацию по одной таблице. Вычисляя агрегацию только для каждого блока данных при его вставке и объединяя инкрементные состояния в конечной таблице, запрос всегда выполняется только на подмножестве данных. Этот метод масштабируется до потенциально петабайтов данных и обычно является предпочтительным.

Однако существуют сценарии использования, где этот инкрементный процесс не требуется или неприменим. Некоторые задачи либо несовместимы с инкрементным подходом, либо не требуют обновлений в реальном времени, и периодическое перестроение является более подходящим вариантом. Например, может потребоваться регулярно выполнять полный пересчет представления по всему набору данных, поскольку оно использует сложное соединение, несовместимое с инкрементным подходом.

> Обновляемые материализованные представления могут выполнять пакетные процессы, такие как денормализация. Между обновляемыми материализованными представлениями можно создавать зависимости таким образом, что одно представление зависит от результатов другого и выполняется только после его завершения. Это может заменить запланированные рабочие процессы или простые направленные ациклические графы (DAG), такие как задание [dbt](https://www.getdbt.com/). Чтобы узнать больше о том, как настроить зависимости между обновляемыми материализованными представлениями, перейдите к разделу `Dependencies` в [CREATE VIEW](/sql-reference/statements/create/view#refresh-dependencies).


## Как обновить обновляемое материализованное представление? {#how-do-you-refresh-a-refreshable-materialized-view}

Обновляемые материализованные представления обновляются автоматически с интервалом, который задается при создании.
...
Если необходимо принудительно обновить материализованное представление, используйте команду `SYSTEM REFRESH VIEW`:
...
Одно из применений этой функции — создание снимков значений в определённый момент времени. Например, предположим, что у нас есть таблица `events`, заполняемая потоком сообщений из [Kafka](https://kafka.apache.org/), [Redpanda](https://www.redpanda.com/) или другой платформы потоковой передачи данных.
...
Этот набор данных содержит `4096` значений в столбце `uuid`. Мы можем написать следующий запрос для поиска значений с наибольшим общим количеством:
...
Предположим, мы хотим фиксировать количество для каждого `uuid` каждые 10 секунд и сохранять его в новой таблице `events_snapshot`. Схема таблицы `events_snapshot` будет выглядеть следующим образом:
...
После этого мы можем выполнить запрос к `events_snapshot`, чтобы получить количество по времени для конкретного `uuid`:
...
[Руководство по денормализации данных](/data-modeling/denormalization) демонстрирует различные методы денормализации данных с использованием набора данных Stack Overflow. Мы заполняем данными следующие таблицы: `votes`, `users`, `badges`, `posts` и `postlinks`.
...
Затем можно написать следующий запрос для вычисления сводки по каждому актёру, упорядоченной по наибольшему количеству появлений в фильмах.
...
Получено 5 строк. Время выполнения: 0.393 сек. Обработано 5.45 млн строк, 86.82 МБ (13.87 млн строк/с., 221.01 МБ/с.)
Пиковое потребление памяти: 1.38 ГиБ.
```

Получение результата занимает не так уж много времени, но предположим, что мы хотим сделать его ещё быстрее и менее вычислительно затратным.
Предположим также, что этот набор данных постоянно обновляется — фильмы непрерывно выходят, появляются новые актёры и режиссёры.
...
Представление начнёт выполняться немедленно, а затем — каждую минуту, как задано в настройках, чтобы изменения в исходной таблице отражались в результатах. Наш предыдущий запрос для получения сводных данных по актёрам становится синтаксически проще и значительно быстрее!
...
Предположим, мы добавим в наши исходные данные нового актёра по имени «Clicky McClickHouse», который снялся во множестве фильмов!
...
Менее чем через 60 секунд наша целевая таблица обновляется и отражает впечатляющую плодовитость Clicky как актёра:
