...
title: 'Пошаговое материализованное представление'
description: 'Как использовать пошаговые материализованные представления для ускорения запросов'
...
Чтобы продемонстрировать работу материализованного представления, предположим, что таблица votes пуста и ещё не получила никаких данных. Наше материализованное представление выполняет указанный выше запрос `SELECT` для данных, вставляемых в `votes`, при этом результаты отправляются в `up_down_votes_per_day`:
...
После завершения можем проверить размер таблицы `up_down_votes_per_day` — должна быть 1 строка на каждый день:
...
- Агрегировать по ключу сортировки, используемому в целевой таблице, т.е. `CreationDate`, и суммировать метрики. Обычно это более эффективно и гибко (таблицу можно использовать для других целей), но первый вариант может быть проще для некоторых запросов. Ниже показаны оба варианта:
...
В большинстве случаев столбцы, используемые в предложении `GROUP BY` преобразования материализованного представления, должны соответствовать столбцам в предложении `ORDER BY` целевой таблицы при использовании движков `SummingMergeTree` или `AggregatingMergeTree`. Эти движки используют столбцы `ORDER BY` для объединения строк с идентичными значениями во время фоновых операций слияния. Несоответствие между столбцами `GROUP BY` и `ORDER BY` может привести к неэффективной производительности запросов, неоптимальным слияниям или даже расхождениям в данных.
:::

### Более сложный пример {#a-more-complex-example}


Приведённый выше пример использует Materialized Views для вычисления и поддержания двух сумм в день. Суммы представляют собой самую простую форму агрегации для поддержания частичных состояний — мы можем просто добавлять новые значения к существующим по мере их поступления. Однако ClickHouse Materialized Views могут использоваться для агрегации любого типа.

Предположим, мы хотим вычислять некоторые статистики по постам на каждый день: 99.9-й процентиль для `Score` и среднее значение `CommentCount`. Запрос для вычисления этого может выглядеть так:
...
Как и раньше, мы можем создать материализованное представление, которое будет выполнять приведённый выше запрос при добавлении новых записей в таблицу `posts`.

В качестве примера и чтобы избежать загрузки данных постов из S3, мы создадим дополнительную таблицу `posts_null` с той же схемой, что и у `posts`. Однако эта таблица не будет хранить данные и будет использоваться материализованным представлением только при вставке строк. Чтобы избежать хранения данных, мы можем использовать [движок таблицы `Null`](/engines/table-engines/special/null).

```sql
CREATE TABLE posts_null AS posts ENGINE = Null
```

Движок таблицы Null — это мощная оптимизация — думайте о нём как о `/dev/null`. Наше материализованное представление будет вычислять и сохранять сводную статистику в момент вставки строк в таблицу `posts_null` — оно всего лишь выступает в роли триггера. Однако исходные данные сохранены не будут. Хотя в нашем случае мы, вероятно, всё же хотим хранить исходные записи, этот подход можно использовать для вычисления агрегатов, избегая накладных расходов на хранение «сырых» данных.

Материализованное представление, таким образом, будет выглядеть так:
...
Обратите внимание, что мы добавляем суффикс `State` к нашим агрегатным функциям. Это гарантирует, что будет возвращено агрегатное состояние функции, а не окончательный результат. Оно будет содержать дополнительную информацию, позволяющую объединять это частичное состояние с другими состояниями. Например, в случае вычисления среднего оно будет включать количество и сумму значений столбца.

> Частичные состояния агрегации необходимы для вычисления корректных результатов. Например, при вычислении среднего простое усреднение средних по поддиапазонам даёт некорректный результат.
...
Хотя ранее `SummingMergeTree` было достаточно для хранения счетчиков, для других функций нам требуется более продвинутый тип движка: [`AggregatingMergeTree`](/engines/table-engines/mergetree-family/aggregatingmergetree).
Чтобы ClickHouse «знал», что будут храниться агрегатные состояния, мы задаём для `Score_quantiles` и `AvgCommentCount` тип `AggregateFunction`, указывая саму функцию — источник частичных состояний, а также тип соответствующих исходных столбцов. Как и в `SummingMergeTree`, строки с одинаковым значением ключа `ORDER BY` будут объединяться (`Day` в примере выше).
...
> В рабочей среде вы, скорее всего, привязали бы материализованное представление к таблице `posts`. Здесь мы использовали `posts_null`, чтобы продемонстрировать null-таблицу.
...
Предположим, необходимо отфильтровать по конкретному `UserId` и вычислить средний `Score`:
...
1 строка в наборе. Время выполнения: 0.012 сек. Обработано 88.61 тыс. строк, 771.37 KB (7.09 млн строк/с, 61.73 MB/с.)
...
Теперь, если этот пользователь получит новый значок и будет добавлена строка, наше представление обновится:

```sql
INSERT INTO badges VALUES (53505058, 2936484, 'gingerwizard', now(), 'Gold', 0);

1 строка в наборе. Прошло: 7.517 sec.
...
9 строк в наборе. Прошло: 0.017 сек. Обработано 32.77 тыс. строк, 642.27 KB (1.96 млн строк/с., 38.50 MB/s.)
```

:::warning
Обратите внимание на задержку операции вставки. Вставляемая строка пользователя соединяется со всей таблицей `users`, что существенно снижает производительность вставки. Ниже мы предлагаем подходы к решению этой проблемы в разделе [&quot;Использование исходной таблицы в фильтрах и JOINах&quot;](/materialized-view/incremental-materialized-view#using-source-table-in-filters-and-joins-in-materialized-views).
:::

Напротив, если мы сначала вставим `badge` для нового пользователя, а затем строку для этого пользователя, наша материальная представление не сможет корректно зафиксировать метрики пользователей.
...
- **Ограничивайте количество столбцов, извлекаемых из объединений.** Выбирайте только необходимые столбцы из объединяемых таблиц, чтобы минимизировать использование памяти и снизить задержку во время вставки (см. ниже).

- **Оценивайте производительность во время вставки.** JOIN увеличивают стоимость вставок, особенно при больших таблицах справа. Проводите тестирование скорости вставки с использованием репрезентативных производственных данных.
...
- **Используйте явные псевдонимы столбцов.** Когда таблицы имеют пересекающиеся имена столбцов, используйте псевдонимы, чтобы предотвратить неоднозначность и обеспечить правильные результаты в целевой таблице.
...
В приведенном выше примере у нас есть два материализованных представления `mvw1` и `mvw2`, которые выполняют похожие операции, но с небольшим отличием в способе обращения к исходной таблице `t0`.
...
Ключевое отличие заключается в том, как ClickHouse обрабатывает исходную таблицу в запросе материализованного представления. Когда материализованное представление активируется вставкой, исходная таблица (`t0` в данном случае) заменяется вставленным блоком данных. Это поведение можно использовать для оптимизации запросов, но оно также требует внимательного подхода во избежание неожиданных результатов.
...
0 строк в наборе. Время: 132.118 сек. Обработано 323.43 млн строк, 4.69 GB (2.45 млн строк/с, 35.49 MB/s.)
...
В приведённой выше операции из таблицы `users` извлекается только одна строка для пользователя с идентификатором `2936484`. Этот поиск также оптимизирован с помощью ключа упорядочивания таблицы `Id`.
...
Предположим, мы хотим создать единое представление активности пользователей, показывающее последнюю активность каждого пользователя путём объединения этих двух таблиц:
...
Если мы хотим, чтобы эта таблица обновлялась при вставке новых строк в `badges` или `comments`, наивным подходом к решению этой задачи может быть попытка создать материализованное представление с предыдущим запросом объединения:
...
Хотя это синтаксически корректно, это приведёт к непредвиденным результатам — представление будет срабатывать только при вставках в таблицу `comments`. Например:
...
1 строка в наборе. Время: 0.005 сек.

````

Вставки в таблицу `badges` не будут активировать представление, из-за чего `user_activity` не получит обновлений:
...
Чтобы решить эту задачу, мы просто создаём материализованное представление для каждого оператора SELECT:
...
SELECT
 ...
 argMaxState('комментарий', CreationDate) AS activity_type,
...
SELECT
 ...
 argMaxState('значок', Date) AS activity_type,
    max(Date) AS last_activity
FROM stackoverflow.badges
GROUP BY UserId;
```

Вставка в любую из таблиц теперь даёт корректный результат. Например, если мы вставим данные в таблицу `comments`:
...
┌─UserId──┬─description──────┬─activity_type─┬───────────last_activity─┐
│ 2936484 │ The answer is 42 │ комментарий       │ 2025-04-15 10:18:47.000 │
└─────────┴──────────────────┴───────────────┴─────────────────────────┘
...
- Вы осведомлены о возможностях системы обрабатывать одновременное выполнение представлений

Оставьте его отключённым, когда:

- Материализованные представления имеют зависимости друг от друга
- Вам требуется предсказуемое упорядоченное выполнение
- Вы выполняете отладку или аудит поведения вставки и хотите детерминированное воспроизведение
...
В ClickHouse CTE встраиваются, что означает, что они фактически копируются в запрос во время оптимизации и **не** материализуются. Это означает:
...
В этом случае CTE `users` повторно вычисляется при каждой вставке в таблицу `posts`, и материализованное представление не будет обновляться при добавлении новых пользователей — только при вставках в `posts`.

В общем случае используйте CTE для логики, которая работает с той же исходной таблицей, к которой привязано материализованное представление, или убедитесь, что используемые таблицы невелики и с малой вероятностью станут узким местом по производительности. В качестве альтернативы рассмотрите [те же оптимизации, что и для JOIN в материализованных представлениях](/materialized-view/incremental-materialized-view#join-best-practices).
