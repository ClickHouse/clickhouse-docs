...
Операции вставки в таблицу `badges` не будут приводить к срабатыванию представления, поэтому `user_activity` не будет получать обновления:
...
Чтобы решить эту задачу, просто создаём по одному materialized view для каждого оператора SELECT:
...
Теперь вставка в любую из таблиц приводит к корректным результатам. Например, если мы выполним INSERT в таблицу `comments`:
...
Обратите внимание, что каждое из представлений делает паузу на 1 секунду перед вставкой своих строк в таблицу `target`, одновременно добавляя своё имя и время вставки.
...
Напротив, рассмотрим, что происходит, если мы вставляем строку с включённым `parallel_view_processing=1`. При таком режиме представления выполняются параллельно, и порядок поступления строк в целевую таблицу не гарантируется:
...
Хотя в нашем случае порядок поступления строк от каждого представления оказался одинаковым, это не гарантируется — на что указывает близость времени вставки каждой строки. Также обратите внимание на улучшенную производительность вставки.
