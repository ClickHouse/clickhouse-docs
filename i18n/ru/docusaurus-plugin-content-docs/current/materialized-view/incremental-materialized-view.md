...
10 строк в наборе. Затрачено: 0.133 сек. Обработано 238.98 млн строк, 2.15 ГБ (1.79 млрд строк/с., 16.14 ГБ/с.)
Пиковое использование памяти: 363.22 МиБ.
```
...
0 строк в наборе. Затрачено: 111.964 сек. Обработано 477.97 млн строк, 3.89 ГБ (4.27 млн строк/с., 34.71 МБ/с.)
Пиковое использование памяти: 283.49 МиБ.
```
...
Получено 10 строк. Затрачено: 0,113 сек. Обработано 59,82 млн строк, 777,65 МБ (528,48 млн строк/с., 6,87 ГБ/с.)
Пиковое использование памяти: 658,84 МиБ.
```
...
0 строк в наборе. Прошло: 46.357 сек. Обработано 90.38 млн строк, 11.14 ГБ (1.95 млн строк/с., 240.22 МБ/с.)
```
...
1 строка в наборе. Затрачено: 0.778 сек. Обработано 90.38 млн строк, 361.59 МБ (116.16 млн строк/с., 464.74 МБ/с.)
Пиковое использование памяти: 217.08 МиБ.
```
...
0 строк в наборе. Прошло: 5.163 сек. Обработано 90.38 млн строк, 17.25 ГБ (17.51 млн строк/сек., 3.34 ГБ/сек.)
```
...
0 строк в наборе. Затрачено: 433.762 сек. Обработано 1.16 миллиарда строк, 28.50 ГБ (2.67 миллиона строк/с., 65.70 МБ/с.)
```
...
Получено 8 строк. Прошло: 0.018 сек. Обработано 32.77 тыс. строк, 642.14 КБ (1.86 млн строк/сек., 36.44 МБ/сек.)
```
...
0 строк в наборе. Прошло: 0.017 сек. Обработано 32.77 тыс. строк, 644.32 КБ (1.98 млн строк/сек., 38.94 МБ/сек.)
```
...
Обработана 1 строка. Затрачено: 7.517 сек.
```
...
┌─UserId──┬─description──────┬─activity_type─┬───────────last_activity─┐
│ 2936484 │ Ответ — 42 │ комментарий       │ 2025-04-15 10:18:47.000 │
└─────────┴──────────────────┴───────────────┴─────────────────────────┘

Получена 1 строка. Время выполнения: 0.006 сек.
```
...
Обработана 1 строка. Время выполнения: 3.786 сек.
```
...
3 строки в наборе. Затрачено: 0.015 сек.
```
...
Получено 3 строки. Затрачено: 0,004 сек.
```
