...
## Настоящая колоночно-ориентированная система управления базами данных {#true-column-oriented-database-management-system}

В настоящей колоночно-ориентированной СУБД вместе со значениями не хранятся никакие дополнительные данные. Это означает, что должна быть поддержка значений фиксированной длины, чтобы избежать хранения их длины в виде «числа» рядом со значениями. Например, миллиард значений типа UInt8 должен занимать около 1 ГБ в несжатом виде, иначе это существенно влияет на использование процессора. Крайне важно хранить данные компактно (без какого-либо «мусора») даже в несжатом виде, поскольку скорость распаковки (использование процессора) в основном зависит от объёма несжатых данных.

Это отличается от систем, которые могут хранить значения различных колонок раздельно, но не способны эффективно обрабатывать аналитические запросы из-за их оптимизации под другие сценарии, таких как HBase, Bigtable, Cassandra и Hypertable. В этих системах вы получите пропускную способность около ста тысяч строк в секунду, но не сотни миллионов строк в секунду.
...
ClickHouse спроектирован для работы на обычных жестких дисках, что обеспечивает низкую стоимость хранения данных в расчете на гигабайт. При этом SSD и дополнительная оперативная память также полностью используются при наличии.
...
В ClickHouse данные могут размещаться на разных шардах. Каждый шард может представлять собой группу реплик для обеспечения отказоустойчивости. Все шарды используются для параллельного выполнения запросов, что происходит прозрачно для пользователя.
...
ClickHouse поддерживает таблицы с первичным ключом. Для быстрого выполнения запросов по диапазону первичного ключа данные инкрементально сортируются с помощью дерева слияний. Благодаря этому данные могут непрерывно добавляться в таблицу. При добавлении новых данных блокировки не применяются.
...
В отличие от других систем управления базами данных, вторичные индексы в ClickHouse не указывают на конкретные строки или диапазоны строк. Вместо этого они позволяют базе данных заранее узнать, что все строки в некоторых частях данных не соответствуют условиям фильтрации запроса, и не читать их вовсе, поэтому они называются [индексами с пропуском данных](../engines/table-engines/mergetree-family/mergetree.md#table_engine-mergetree-data_skipping-indexes).


## Подходит для онлайн-запросов {#suitable-for-online-queries}

Большинство OLAP-систем управления базами данных не ориентированы на выполнение онлайн-запросов с задержкой менее секунды. В альтернативных системах время построения отчётов в десятки секунд или даже минуты часто считается приемлемым. Иногда требуется ещё больше времени, что вынуждает системы готовить отчёты в офлайн-режиме (заранее или с ответом «зайдите позже»).

В ClickHouse «низкая задержка» означает, что запросы могут обрабатываться без задержки и без предварительной подготовки ответа, непосредственно в момент загрузки страницы пользовательского интерфейса — другими словами, _в режиме реального времени_.
...
ClickHouse адаптивно выбирает способ выполнения [JOIN](../sql-reference/statements/select/join.md) для нескольких таблиц, предпочитая hash join и переходя на merge join при наличии более одной большой таблицы.


## Репликация данных и обеспечение целостности данных {#data-replication-and-data-integrity-support}

ClickHouse использует асинхронную многомастерную репликацию. После записи данных на любую доступную реплику все остальные реплики получают свою копию в фоновом режиме. Система поддерживает идентичность данных на разных репликах. Восстановление после большинства сбоев выполняется автоматически или полуавтоматически в сложных случаях.
...
ClickHouse реализует управление учётными записями пользователей с помощью SQL-запросов и позволяет [настраивать управление доступом на основе ролей](/guides/sre/user-management/index.md) аналогично стандарту ANSI SQL и популярным системам управления реляционными базами данных.
...
2.  Отсутствие возможности изменять или удалять уже вставленные данные с высокой скоростью и низкой задержкой. Доступны пакетные удаления и обновления для очистки или изменения данных, например, для соблюдения требований [GDPR](https://gdpr-info.eu).
3.  Разреженный индекс делает ClickHouse менее эффективным для точечных запросов, извлекающих отдельные строки по их ключам.
