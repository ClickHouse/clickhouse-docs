...
'description': 'ClickHouse имеет бета и экспериментальные функции. Эта страница документации
  обсуждает определение.'
'slug': '/beta-and-experimental-features'
'doc_type': 'reference'
---

Поскольку ClickHouse является проектом с открытым исходным кодом, он получает множество взносов не только от сотрудников ClickHouse, но и от сообщества. Эти взносы часто развиваются с разной скоростью; определенные функции могут потребовать длительной фазы прототипирования или больше времени для получения достаточной обратной связи от сообщества и итераций, чтобы быть признанными общедоступными (GA).

Из-за неопределенности в том, когда функции классифицируются как общедоступные, мы делим функции на две категории: **Beta** и **Experimental**.

**Beta** функции официально поддерживаются командой ClickHouse. **Experimental** функции являются ранними прототипами, разработанными либо командой ClickHouse, либо сообществом, и не поддерживаются официально.

Ниже представлены свойства **Beta** и **Experimental** функций:

## Beta функции {#beta-features}

- Активно разрабатываются, чтобы стать общедоступными (GA)
...
Ниже представлены функции, считающиеся бета в ClickHouse Cloud и доступные для использования в ваших службах ClickHouse Cloud.

Примечание: убедитесь, что вы используете актуальную версию настройки ClickHouse [совместимости](/operations/settings/settings#compatibility), чтобы использовать недавно введенную функцию.

## Experimental функции {#experimental-features}

- Могут никогда не стать GA
- Могут быть удалены
- Могут вносить изменения, нарушающие совместимость
- Функциональность может измениться в будущем
- Необходимо намеренно включать
- Команда ClickHouse **не поддерживает** экспериментальные функции
- Может отсутствовать важная функциональность и документация
- Не может быть включена в облаке

Пожалуйста, обратите внимание: дополнительные экспериментальные функции не могут быть включены в ClickHouse Cloud, кроме тех, что указаны выше как Beta.

<!-- Внутреннее содержимое тегов ниже заменяется на этапе сборки на таблицу, сгенерированную из источника
     Пожалуйста, не изменяйте и не удаляйте теги
-->

<!--AUTOGENERATED_START-->
## Beta настройки {#beta-settings}
...
## Experimental настройки {#experimental-settings}
