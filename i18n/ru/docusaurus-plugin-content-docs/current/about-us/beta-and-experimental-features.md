...
Поскольку ClickHouse является открытым программным обеспечением, он получает множество вкладов не только от сотрудников ClickHouse, но и от сообщества. Эти вклады часто разрабатываются с разной скоростью; определенные функции могут потребовать длительной фазы прототипирования или больше времени на получение достаточной обратной связи от сообщества и итераций, чтобы считаться общедоступными (GA).
...
**Бета** функции официально поддерживаются командой ClickHouse. **Экспериментальные** функции — это ранние прототипы, разработанные либо командой ClickHouse, либо сообществом, и они не имеют официальной поддержки.

Ниже в разделах четко описаны свойства **Бета** и **Экспериментальных** функций:
...
Следующие функции считаются Бета в ClickHouse Cloud и доступны для использования в службах ClickHouse Cloud, даже если они могут быть в настоящее время под настройкой ClickHouse с именем ```allow_experimental_*```:

Примечание: убедитесь, что вы используете текущую версию настройки ClickHouse [совместимость](/operations/settings/settings#compatibility), чтобы использовать недавно введенную функцию.

## Экспериментальные функции {#experimental-features}

- Могут никогда не стать GA
- Могут быть удалены
- Могут вводить поломки совместимости
- Функциональность может измениться в будущем
...
- Не может быть включено в облаке

Пожалуйста, обратите внимание: никаких дополнительных экспериментальных функций не разрешено включать в ClickHouse Cloud, кроме перечисленных выше как Бета.

<!-- Внутреннее содержание тегов ниже заменяется во время сборки на таблицу, созданную из источника
     Пожалуйста, не изменяйте и не удаляйте теги
-->

<!--AUTOGENERATED_START-->
## Настройки Бета {#beta-settings}
