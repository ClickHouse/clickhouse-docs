...
description: 'В ClickHouse есть бета- и экспериментальные функции. На этой странице документации рассматриваются их определения.'
slug: /beta-and-experimental-features
doc_type: 'reference'
---

Поскольку ClickHouse является проектом с открытым исходным кодом, он получает множество вкладов не только от сотрудников ClickHouse, но и от сообщества. Эти вкладчики часто работают с разной скоростью: отдельные функции могут требовать длительной фазы прототипирования или большего времени для получения достаточной обратной связи от сообщества и последующих итераций, прежде чем их можно будет считать общедоступными (GA).

Из-за неопределенности сроков, когда функции приобретают статус общедоступных (GA), мы делим функции на две категории: **Beta** и **Experimental**.

Функции в статусе **Beta** официально поддерживаются командой ClickHouse. Функции в статусе **Experimental** — это ранние прототипы, разрабатываемые либо командой ClickHouse, либо сообществом, и они не поддерживаются официально.

В разделах ниже подробно описаны свойства функций **Beta** и **Experimental**:



## Бета-функции {#beta-features}

- Активно разрабатываются, чтобы впоследствии стать общедоступными (GA)
...
Ниже перечислены функции, которые считаются бета-версией в ClickHouse Cloud и доступны для использования в ваших сервисах ClickHouse Cloud.

Примечание: убедитесь, что вы используете актуальное значение настройки ClickHouse [compatibility](/operations/settings/settings#compatibility), чтобы иметь доступ к недавно добавленным функциям.
...
* Функциональность может измениться в рамках данной функции
* Должны быть включены явно
* Команда ClickHouse **не поддерживает** экспериментальные функции
* Могут отсутствовать важная функциональность и документация
* Не могут быть включены в ClickHouse Cloud

Обратите внимание: в ClickHouse Cloud нельзя включать дополнительные экспериментальные функции, помимо перечисленных выше как Beta.

{/* Содержимое тегов ниже на этапе сборки заменяется таблицей, сгенерированной из исходников.
     Пожалуйста, не изменяйте и не удаляйте сами теги.
  */ }
...
| [merge&#95;selector&#95;algorithm](/operations/settings/merge-tree-settings#merge_selector_algorithm)                                                                                                                 | `Простой`               |
