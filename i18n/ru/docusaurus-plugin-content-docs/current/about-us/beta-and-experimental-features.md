...
description: 'В ClickHouse есть бета- и экспериментальные возможности. На этой странице документации объясняется, что это значит.'
slug: /beta-and-experimental-features
doc_type: 'reference'
---

Поскольку ClickHouse — проект с открытым исходным кодом, вклад в него вносят не только сотрудники ClickHouse, но и сообщество. Эти доработки часто развиваются с разной скоростью: отдельные возможности могут требовать длительной стадии прототипирования или дополнительного времени для получения достаточного объёма обратной связи и итераций в сообществе, прежде чем их можно будет считать общедоступными (GA).

Из-за неопределённости момента, когда возможности становятся общедоступными, мы выделяем две их категории: **бета** и **экспериментальные**.

**Бета**-возможности официально поддерживаются командой ClickHouse. **Экспериментальные** возможности — это ранние прототипы, разрабатываемые либо командой ClickHouse, либо сообществом, и официально не поддерживаются.

В следующих разделах явно описаны свойства **бета**- и **экспериментальных** возможностей:



## Бета-функции {#beta-features}

- Находятся в активной разработке для перехода в статус общедоступных (GA)
...
Ниже приведены функции, которые находятся в статусе бета в ClickHouse Cloud и доступны для использования в ваших сервисах ClickHouse Cloud.

Примечание: для использования недавно добавленных функций убедитесь, что вы используете актуальную версию настройки [compatibility](/operations/settings/settings#compatibility) ClickHouse.
...
- Недоступны в облачной версии

Обратите внимание: в ClickHouse Cloud запрещено включать дополнительные экспериментальные возможности, кроме тех, которые перечислены выше как бета-версии.
...
## Настройки бета-версии {#beta-settings}
