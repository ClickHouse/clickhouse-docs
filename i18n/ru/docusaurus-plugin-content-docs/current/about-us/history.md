...
Его основная задача — построение отчётов в режиме онлайн с использованием неагрегированных данных. Используется кластер из 374 серверов, в базе данных которых хранится более 20,3 триллионов строк. Объём сжатых данных составляет около 2 ПБ без учёта дубликатов и реплик. Объём несжатых данных (в формате TSV) составил бы приблизительно 17 ПБ.
...
- При агрегации ключей с высокой кардинальностью (таких как URL) объём данных уменьшается незначительно (менее чем в два раза).
...
Однако при агрегации значительная часть работы выполняется в фоновом режиме и завершается относительно спокойно. В отличие от этого, онлайн-вычисления требуют максимально быстрого расчёта, поскольку пользователь ожидает результата.
...
OLAPServer хорошо работал с неагрегированными данными, но имел множество ограничений, которые не позволяли использовать его для всех отчётов по желанию. К ним относились отсутствие поддержки типов данных (только числа) и невозможность инкрементального обновления данных в реальном времени (это можно было делать только путём ежедневной перезаписи данных). OLAPServer — это не СУБД, а специализированная БД.
