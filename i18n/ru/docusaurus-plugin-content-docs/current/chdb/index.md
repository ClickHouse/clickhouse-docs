...
sidebar_label: 'Обзор'
slug: /chdb
description: 'chDB — это внутррипроцессный SQL-движок OLAP на базе ClickHouse'
...
chDB — это быстрый встраиваемый (in-process) SQL OLAP-движок на базе [ClickHouse](https://github.com/clickhouse/clickhouse).
Вы можете использовать его, когда вам нужны возможности ClickHouse в языке программирования без необходимости подключаться к серверу ClickHouse.



## Ключевые возможности {#key-features}

- **Встроенный SQL OLAP движок** — На базе ClickHouse, не требует установки сервера ClickHouse
- **Множество форматов данных** — Поддержка ввода и вывода для Parquet, CSV, JSON, Arrow, ORC и [более 70 форматов](/interfaces/formats)
...
- **Глубокая интеграция с экосистемой Python** — Нативная поддержка Pandas, Arrow, DB API 2.0, легко встраивается в существующие рабочие процессы анализа данных
- **Нулевые зависимости** — Не требуется установка внешних баз данных


## Какие языки программирования поддерживает chDB? {#what-languages-are-supported-by-chdb}

chDB предоставляет привязки для следующих языков программирования:
...
- Если вы используете [Go](install/go.md), [Rust](install/rust.md), [NodeJS](install/nodejs.md), [Bun](install/bun.md) или [C и C++](install/c.md), обратитесь к соответствующим страницам для этих языков.
- Если вы используете Python, см. [руководство разработчика по началу работы](getting-started.md) или [курс chDB по запросу](https://learn.clickhouse.com/user_catalog_class/show/1901178). Также доступны руководства по выполнению типичных задач:
  - [Работа с JupySQL](guides/jupysql.md)
...
Вы можете ознакомиться с кратким введением в проект chDB от Алексея Миловидова, создателя ClickHouse:
...
chDB обеспечивает высокую производительность в различных сценариях:
...
* **[Производительность обработки DataFrame](https://colab.research.google.com/drive/1FogLujJ_-ds7RGurDrUnK-U0IW8a8Qd0)** - сравнительный анализ с другими библиотеками для работы с DataFrame
...
- Больше примеров см. на (https://github.com/chdb-io/chdb/tree/main/examples)


## Лицензия {#license}

chDB распространяется под лицензией Apache License, версия 2.0. Подробнее см. в файле [LICENSE](https://github.com/chdb-io/chdb/blob/main/LICENSE.txt).
