---
title: 'chDB'
sidebar_label: 'Обзор'
slug: /chdb
description: 'chDB — это встроенный SQL OLAP-движок на базе ClickHouse'
keywords: ['chdb', 'embedded', 'clickhouse-lite', 'in-process', 'in process']
doc_type: 'guide'
---

import Image from '@theme/IdealImage';
import dfBench from '@site/static/images/chdb/df_bench.png';


# chDB

chDB — это быстрый встроенный SQL-OLAP‑движок на базе [ClickHouse](https://github.com/clickhouse/clickhouse).
Вы можете использовать его, когда вам нужны возможности ClickHouse прямо в коде, без подключения к серверу ClickHouse.



## Ключевые возможности {#key-features}

- **Встроенный SQL OLAP движок** - На основе ClickHouse, не требует установки сервера ClickHouse
- **Множество форматов данных** - Поддержка ввода и вывода для Parquet, CSV, JSON, Arrow, ORC и [более 70 форматов](/interfaces/formats)
- **Минимизация копирования данных** - Из C++ в Python с помощью [python memoryview](https://docs.python.org/3/c-api/memoryview.html)
- **Глубокая интеграция с экосистемой Python** - Нативная поддержка Pandas, Arrow, DB API 2.0, легко встраивается в существующие процессы обработки данных
- **Без зависимостей** - Не требуется установка внешних баз данных


## Какие языки программирования поддерживает chDB? {#what-languages-are-supported-by-chdb}

chDB имеет привязки для следующих языков:

- [Python](install/python.md) - [Справочник API](api/python.md)
- [Go](install/go.md)
- [Rust](install/rust.md)
- [NodeJS](install/nodejs.md)
- [Bun](install/bun.md)
- [C и C++](install/c.md)


## Как начать работу? {#how-do-i-get-started}

- Если вы используете [Go](install/go.md), [Rust](install/rust.md), [NodeJS](install/nodejs.md), [Bun](install/bun.md) или [C и C++](install/c.md), ознакомьтесь с соответствующими страницами для этих языков.
- Если вы используете Python, см. [руководство разработчика по началу работы](getting-started.md) или [курс chDB по запросу](https://learn.clickhouse.com/user_catalog_class/show/1901178). Также доступны руководства по выполнению типичных задач:
  - [JupySQL](guides/jupysql.md)
  - [Запросы к Pandas](guides/querying-pandas.md)
  - [Запросы к Apache Arrow](guides/querying-apache-arrow.md)
  - [Запросы к данным в S3](guides/querying-s3-bucket.md)
  - [Запросы к файлам Parquet](guides/querying-parquet.md)
  - [Запросы к удалённому ClickHouse](guides/query-remote-clickhouse.md)
  - [Использование базы данных clickhouse-local](guides/clickhouse-local.md)


## Вводное видео {#an-introductory-video}

Вы можете посмотреть краткое введение в проект chDB от Алексея Миловидова, создателя ClickHouse:

<div class='vimeo-container'>
  <iframe
    width='560'
    height='315'
    src='https://www.youtube.com/embed/cuf_hYn7dqU?si=SzUm7RW4Ae5-YwFo'
    title='Видеоплеер YouTube'
    frameborder='0'
    allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share'
    referrerpolicy='strict-origin-when-cross-origin'
    allowfullscreen
  ></iframe>
</div>


## Бенчмарки производительности {#performance-benchmarks}

chDB обеспечивает высокую производительность в различных сценариях:


* **[ClickBench of embedded engines](https://benchmark.clickhouse.com/#)** - Комплексное сравнение производительности
* **[Производительность обработки DataFrame](https://colab.research.google.com/drive/1FogLujJ_-ds7RGurDrUnK-U0IW8a8Qd0)** — сравнительный анализ с другими библиотеками для работы с DataFrame
* **[DataFrame Benchmark](https://benchmark.clickhouse.com/#)**



<Image img={dfBench} alt="Результаты тестирования DataFrame" size="md"/>



## О chDB {#about-chdb}

- Прочитайте полную историю создания проекта chDB в [блоге](https://clickhouse.com/blog/chdb-embedded-clickhouse-rocket-engine-on-a-bicycle)
- Узнайте о chDB и сценариях его использования в [блоге](https://clickhouse.com/blog/welcome-chdb-to-clickhouse)
- Пройдите [курс по chDB в удобное время](https://learn.clickhouse.com/user_catalog_class/show/1901178)
- Познакомьтесь с chDB прямо в браузере с помощью [примеров codapi](https://antonz.org/trying-chdb/)
- Больше примеров см. на (https://github.com/chdb-io/chdb/tree/main/examples)


## Лицензия {#license}

chDB распространяется по лицензии Apache License, Version 2.0. Подробнее см. в файле [LICENSE](https://github.com/chdb-io/chdb/blob/main/LICENSE.txt).
