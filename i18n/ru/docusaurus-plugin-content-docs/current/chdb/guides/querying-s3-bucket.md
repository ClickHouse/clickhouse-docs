...
Большая часть мировых данных хранится в бакетах Amazon S3.
...
Также вы можете использовать код в Python-скрипте или в любом удобном notebook-редакторе.
...
Для этого используем [табличную функцию `s3`](/sql-reference/table-functions/s3), передав путь к файлу или маску для набора файлов.

:::tip
Если передать только имя бакета, будет выброшено исключение.
:::

Также используем входной формат [`One`](/interfaces/formats/One), чтобы файл не парсился — вместо этого для каждого файла возвращается одна строка, и мы можем получить доступ к имени файла через виртуальную колонку `_file`, а к пути — через виртуальную колонку `_path`.
...
Чтобы подсчитать количество строк в каждом из файлов, выполните следующий запрос:
...
Рассмотрим схему этих Parquet-файлов с помощью оператора `DESCRIBE`:
