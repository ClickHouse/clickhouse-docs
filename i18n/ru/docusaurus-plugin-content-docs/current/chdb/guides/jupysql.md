...
Давайте приведем дату в порядок и используем оператор `REPLACE`, чтобы вернуть очищенное значение `ranking_date`:
...
Теперь создадим таблицу `rankings`, схема которой будет определена на основе структуры данных в CSV-файлах:
...
При запуске сохранённого запроса он преобразуется в обобщённое табличное выражение (CTE) перед выполнением.
В следующем запросе вычисляется максимальное количество очков, набранное игроками при достижении 1-го места в рейтинге:
