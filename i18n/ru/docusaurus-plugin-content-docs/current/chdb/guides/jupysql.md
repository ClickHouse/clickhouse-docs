...
[JupySQL](https://jupysql.ploomber.io/en/latest/quick-start.html) — это библиотека Python, которая позволяет запускать SQL-код в Jupyter notebooks и в оболочке IPython. В этом руководстве мы научимся запрашивать данные с использованием chDB и JupySQL.
...
:::note
Если вы используете Jupyter Lab, вам нужно создать блокнот перед тем, как следовать остальной части руководства.
:::

## Скачивание набора данных {#downloading-a-dataset}

Мы будем использовать один из наборов данных [Джеффа Сакмана tennis_atp](https://github.com/JeffSackmann/tennis_atp), который содержит метаданные о игроках и их позициях в рейтингах с течением времени. Давайте начнем с загрузки файлов с рейтингами:
...
И создадим подключение к chDB. Все данные, которые мы сохраним, будут сохранены в директории `atp.chdb`:
...
Далее, мы установим предел отображения, чтобы результаты запросов не обрезались:

```python
%config SqlMagic.displaylimit = None
```

## Запрос данных в CSV файлах {#querying-data-in-csv-files}

Мы скачали несколько файлов с префиксом `atp_rankings`. Давайте используем оператор `DESCRIBE`, чтобы понять схему:
...
Формат данных немного странный. Давайте проработаем эту дату и используем оператор `REPLACE`, чтобы вернуть очищенную `ranking_date`:
...
Выглядит хорошо - результат, как и ожидалось, совпадает с запрашиваемыми данными из CSV файлов.

Мы будем следовать тому же процессу для метаданных игроков. На этот раз данные находятся в одном CSV файле, так что давайте загрузим этот файл:
...
А затем создадим таблицу с названием `players`, основываясь на содержимом CSV файла. Мы также очистим поле `dob`, чтобы оно было типа `Date32`.

> В ClickHouse тип `Date` поддерживает только даты с 1970 года. Поскольку столбец `dob` содержит даты до 1970 года, мы вместо этого используем тип `Date32`.
...
Когда выполнение завершится, мы сможем взглянуть на данные, которые мы загрузили:
...
Довольно интересно, что некоторые игроки в этом списке накопили много очков, не будучи номером 1 с этим количеством очков.
