...
[clickhouse-local](/operations/utilities/clickhouse-local) — это интерфейс командной строки (CLI) со встроенной версией ClickHouse.
Он предоставляет пользователям возможности ClickHouse без необходимости установки сервера.
В этом руководстве мы рассмотрим, как использовать базу данных clickhouse-local через chDB.
...
Для выполнения команд в остальной части руководства будем использовать `ipython`. Запустить его можно следующей командой:
