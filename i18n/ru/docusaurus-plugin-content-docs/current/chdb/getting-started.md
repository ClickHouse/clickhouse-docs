...
В этом руководстве мы начнем работу с варианта chDB для Python. 
Сначала мы сделаем запрос к JSON-файлу на S3, затем создадим таблицу в chDB на основе этого JSON-файла и выполним несколько запросов к данным. 
Мы также увидим, как вернуть данные в разных форматах, включая Apache Arrow и Panda, и, наконец, научимся выполнять запросы к DataFrames Pandas.
...
[Набор данных о дизлайках на YouTube](/getting-started/example-datasets/youtube-dislikes) содержит более 4 миллиардов строк дизлайков под видео на YouTube до 2021 года. 
...
Формат вывода по умолчанию — `CSV`, но мы можем изменить его через параметр `output_format`. 
chDB поддерживает форматы данных ClickHouse, а также [некоторые из своих](/chdb/reference/data-formats.md), включая `DataFrame`, который возвращает DataFrame Pandas:
...
Далее давайте посмотрим, как создать таблицу в chDB. 
...
Если оставить его пустым, база данных будет находиться в памяти и будет потеряна, как только мы завершим процесс Python.
...
Мы будем использовать настройку [`schema_inference_make_columns_nullable`](/operations/settings/formats/#schema_inference_make_columns_nullable), чтобы типы столбцов не были всеми `Nullable`.
...
Мы также могли бы выполнить оба этих шага за один раз, используя технику `CREATE...AS`. 
Давайте создадим другую таблицу, используя этот метод:
...
Предположим, мы затем добавляем дополнительный столбец в DataFrame, чтобы вычислить отношение лайков к дизлайкам. 
...
Вы также можете узнать больше о запросах к DataFrames Pandas в [руководстве разработчика по запросам Pandas](guides/querying-pandas.md).
