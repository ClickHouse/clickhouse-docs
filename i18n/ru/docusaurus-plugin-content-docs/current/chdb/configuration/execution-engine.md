...
DataStore может выполнять операции с разными бэкендами. В этом руководстве объясняется, как настроить и оптимизировать выбор движка.
...
:::tip Режим повышенной производительности
Если вы выполняете ресурсоёмкие задачи агрегации и вам не требуется полная совместимость вывода с pandas (порядок строк, MultiIndex, корректировки dtype), рассмотрите возможность использования [режима повышенной производительности](performance-mode.md). Он автоматически устанавливает движок `chdb` и убирает все накладные расходы, связанные с обеспечением совместимости с pandas.
:::
