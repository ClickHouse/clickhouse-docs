...
description: 'Полный справочник по Python API для chDB'
...
Это основная функция выполнения запросов, которая исполняет SQL-операторы с использованием встроенного движка ClickHouse. Поддерживает различные форматы вывода и может работать как с базами данных в памяти, так и с файловыми базами данных.
...
| `output_format` | str  | `"CSV"`     | Формат вывода результатов. Поддерживаемые форматы:<br/>• `"CSV"` — значения, разделённые запятыми<br/>• `"JSON"` — формат JSON<br/>• `"Arrow"` — формат Apache Arrow<br/>• `"Parquet"` — формат Parquet<br/>• `"DataFrame"` — DataFrame библиотеки Pandas<br/>• `"ArrowTable"` — таблица PyArrow<br/>• `"Debug"` — включает подробное логирование |
...
| Исключение                | Условие                                                          |
| ------------------------- | ---------------------------------------------------------------- |
| [`ChdbError`](#chdberror) | Если выполнение SQL-запроса завершилось ошибкой                 |
...
```pycon
>>> # Запрос с пользовательской функцией (UDF)
>>> result = chdb.query("SELECT my_udf('test')", udf_path="/path/to/udfs")
```
...
Преобразует результат запроса chDB в PyArrow Table для эффективной обработки колоночных данных.
...
Преобразует результат запроса chDB в pandas DataFrame путём предварительного преобразования в
PyArrow Table с последующим преобразованием в pandas с использованием многопоточности для повышения производительности.
...
Повторные вызовы с одной и той же строкой подключения вернут один и тот же объект подключения.
...
Если path равен None, будет создан временный каталог и использован в качестве пути к базе данных,
...
сценариях обработки ошибок или когда необходимо обеспечить очистку независимо
от состояния сессии.
...
память. Для больших наборов результатов рекомендуется использовать [`send_query()`](#chdb-session-session-send_query) для
...
Этот метод выполняет SQL-запрос к базе данных сессии и возвращает
объект потоковых результатов, который позволяет итерировать по результатам без
...
Создает объект [Connection](#chdb-state-sqlitelike-connection) для подключения к фоновому серверу chDB.

Эта функция устанавливает соединение с движком базы данных chDB (ClickHouse).
Допускается только одно открытое соединение на процесс. Повторные вызовы с той же
строкой подключения возвращают один и тот же объект соединения.
...
| Тип возвращаемого значения | Описание                                                                                                                                                                                                                                  |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Connection`               | Объект соединения с базой данных, который поддерживает:<br/>• создание курсоров с помощью `Connection.cursor()`<br/>• прямые запросы с помощью `Connection.query()`<br/>• потоковые запросы с помощью `Connection.send_query()`<br/>• протокол контекстного менеджера для автоматической очистки ресурсов |

**Исключения**

| Исключение     | Условие                                         |
| -------------- | ----------------------------------------------- |
| `RuntimeError` | Ошибка подключения к базе данных               |

:::warning Warning
Поддерживается только одно соединение на процесс.
Создание нового соединения приведет к закрытию существующего.
:::
...
соединение становится недействительным и не может быть использовано для дальнейших операций.
...
:::note
Этот метод идемпотентен — вызывать его несколько раз безопасно.
:::

:::warning Warning
Все выполняющиеся потоковые запросы будут отменены при закрытии
соединения. Убедитесь, что все важные данные обработаны до закрытия.
:::
...
набор результатов. Поддерживает различные форматы вывода и автоматически применяет
...
| Исключение     | Условие                                                    |
| -------------- | ---------------------------------------------------------- |
| `RuntimeError` | Если выполнение запроса завершилось неудачей               |
...
результатов рекомендуется использовать [`send_query()`](#chdb-state-sqlitelike-connection-send_query) для потоковой обработки.
:::
...
который позволяет итерироваться по результатам без загрузки всех данных
...
Этот метод извлекает все оставшиеся строки из текущего набора результатов запроса, начиная с текущей позиции курсора. Возвращает кортеж кортежей строк с соответствующим преобразованием типов Python.
...
:::warning Предупреждение
Этот метод загружает все оставшиеся строки в память одновременно. Для больших наборов результатов рекомендуется использовать [`fetchmany()`](#chdb-state-sqlitelike-cursor-fetchmany) для обработки результатов пакетами.
:::
...
Этот метод извлекает до `size` строк из текущего набора результатов запроса. Возвращает кортеж кортежей строк, где каждая строка содержит значения столбцов с соответствующим преобразованием типов Python.
...
:::note
Этот метод соответствует спецификации DB-API 2.0. Он вернёт меньше строк, чем `size`, если набор результатов исчерпан.
:::
...
Этот метод извлекает следующую доступную строку из текущего набора результатов запроса. Возвращает кортеж, содержащий значения столбцов с соответствующим преобразованием типов Python.
...
:::note
Этот метод соответствует спецификации DB-API 2.0. Значения столбцов автоматически преобразуются в соответствующие типы Python на основе типов столбцов ClickHouse.
:::
...
Преобразование результата запроса в таблицу PyArrow.

Эта функция преобразует результаты запросов chdb в формат таблицы PyArrow, который обеспечивает эффективный доступ к столбцовым данным и совместимость с другими библиотеками обработки данных.
...
chDB предоставляет совместимый с Python DB-API 2.0 интерфейс для подключения к базе данных, позволяя использовать chDB с инструментами и фреймворками, ожидающими стандартные интерфейсы баз данных.

Интерфейс chDB DB-API 2.0 включает:

- **Соединения**: Управление подключениями к базе данных с помощью строк подключения
...
- **Потокобезопасность**: Уровень 1 потокобезопасности (потоки могут совместно использовать модули, но не соединения)
...
| Исключение                           | Условие                                  |
| ------------------------------------ | ---------------------------------------- |
| [`err.Error`](#chdb-dbapi-err-error) | Если не удается установить соединение    |
...
| Параметр | Тип | Описание                                      |
| -------- | --- | --------------------------------------------- |
| `x`      | -   | Входные данные для преобразования в бинарный формат |
...
выполнения запросов, управления транзакциями (без эффекта для ClickHouse) и создания курсоров.
...
транзакции. Предоставлено для соответствия DB-API 2.0.
:::
...
| Исключение                                             | Условие                                        |
| ------------------------------------------------------ | ---------------------------------------------- |
| [`err.InterfaceError`](#chdb-dbapi-err-interfaceerror) | Если соединение закрыто или запрос не выполнен |
...
Этот метод предоставлен для соответствия DB-API 2.0, но не выполняет
...
Этот метод показывает финальный SQL-запрос после подстановки параметров,
...
| Тип         | Описание                                                      |
| ----------- | ------------------------------------------------------------- |
| `str`       | Финальная строка SQL-запроса с подставленными параметрами      |
...
chDB автоматически обрабатывает размеры параметров внутренне.
:::
...
:::note
Этот метод ничего не делает, но требуется спецификацией DB-API 2.0.
chDB автоматически обрабатывает размеры выходных данных внутренне.
:::
...
Это исключение возникает, когда операции с базой данных завершаются неудачей из-за проблем
с обрабатываемыми данными, таких как:
...
| Исключение                               | Условие                                           |
| ---------------------------------------- | ------------------------------------------------- |
| [`DataError`](#chdb-dbapi-err-dataerror) | Когда проверка или обработка данных завершается с ошибкой |
...
Исключение, являющееся базовым классом для всех остальных исключений ошибок (кроме Warning).

Это базовый класс для всех исключений ошибок в chdb, за исключением предупреждений.
Служит родительским классом для всех состояний ошибок базы данных, которые препятствуют
...
- Нарушения ограничений проверки (CHECK)
...
Исключение, возникающее при внутренней ошибке базы данных.
...
внимания администратора базы данных. Эти ошибки обычно невозможно устранить
...
и не обязательно находящихся под контролем программиста, включая:
...
- Недопустимые операции SQL (например, DROP несуществующих объектов)
...
>>> # Таблица не найдена
>>> cursor.execute("SELECT * FROM nonexistent_table")
ProgrammingError: таблица 'nonexistent_table' не существует
```

```pycon
>>> # Синтаксическая ошибка SQL
>>> cursor.execute("SELCT * FROM users")
ProgrammingError: ошибка в синтаксисе SQL
```

```pycon
>>> # Неверное количество параметров
>>> cursor.execute("INSERT INTO users (name, age) VALUES (%s)", ('John',))
ProgrammingError: количество столбцов не совпадает с количеством значений
```
...
:::note
Этот класс исключения следует спецификации Python DB API 2.0
...
Исключение, выбрасываемое для важных предупреждений, таких как усечение данных при вставке и т. п.

Это исключение возникает, когда операция с базой данных завершается,
но при этом возникают важные предупреждения, на которые приложение должно обратить внимание.
Типичные сценарии включают:

- усечение данных при вставке;
- потеря точности при числовых преобразованиях;
- предупреждения при преобразовании наборов символов.
...
Создаёт новый строковый объект из переданного объекта. Если заданы
параметры `encoding` или `errors`, объект должен предоставлять
буфер данных, который будет декодирован с использованием указанной
кодировки и обработчика ошибок. В противном случае возвращается
результат `object._\_str_\_()` (если определён) или `repr(object)`.
...
аргументы не переданы. Если `x` — число, возвращается `x._\_int_\_()`.
Для чисел с плавающей запятой выполняется усечение к нулю.

Если `x` не является числом или указан параметр `base`, то `x` должен
быть строкой, объектом `bytes` или `bytearray`, представляющим целочисленный
литерал в указанной системе счисления. Литерал может начинаться с символов
`+` или `-` и может быть окружён пробельными символами. Значение `base`
по умолчанию — 10. Допустимые значения `base`: 0 и диапазон 2–36.
Значение `base = 0` означает, что система счисления определяется по строке
как по целочисленному литералу.
...
DB-API 2.0. Он позволяет выполнять гибкую проверку типов, когда отдельные
значения могут сравниваться с набором с использованием операторов равенства
и неравенства.

Он используется для констант типов, таких как `STRING`, `BINARY`, `NUMBER` и т. д.,
чтобы обеспечить возможность сравнений вида `field_type == STRING`, где
`field_type` — отдельное значение типа.
...
с набором с использованием как операторов равенства, так и неравенства.

Это используется для таких констант типов, как STRING, BINARY, NUMBER и т. д., чтобы выполнять
...
Это используется для констант типов, таких как STRING, BINARY, NUMBER и т. д., чтобы выполнять
сравнения вида «field_type == STRING», где field_type — это отдельное значение типа.
...
print(&quot;Имена столбцов:&quot;, [desc[0] for desc in cur.description])
...
# База данных в оперативной памяти (по умолчанию)
conn1 = dbapi.connect()
...
# Подключение в режиме только чтения
...
:::note
- Интерфейс DB-API 2.0 в chDB совместим с большинством инструментов для работы с базами данных в Python
...
в chDB. Он позволяет расширить возможности chDB, создавая пользовательские функции на Python,
которые можно вызывать из SQL-запросов.

### `chdb.udf.chdb_udf` {#chdb-udf}

Декоратор для пользовательских функций chDB на Python (UDF).
...
вывод типов данных, помощники для преобразования данных и утилиты для отладки.
...
где каждый ключ соответствует столбцу, а каждое значение — списку значений столбца.
...
| Тип возвращаемого значения | Описание                                                                    |
| -------------------------- | --------------------------------------------------------------------------- |
| `Dict[str, List[Any]]`     | Словарь с ключами в виде имен столбцов и значениями в виде списков значений столбцов |
...
| `parent_key` | str              | `""`         | Базовый ключ для добавления к каждому ключу    |
| `sep`        | str              | `"_"`        | Разделитель для использования между объединенными ключами |
...
тип данных, который может представить все значения в списке. Она рассматривает целочисленный,
...
Функция анализирует значения в каждом столбце и определяет наиболее подходящий
...
| Тип возврата | Описание                              |
| ----------- | -------------------------------------- |
| `List[Any]` | Список последовательностей, по одной на столбец |
...
### **класс** `chdb.ChdbError` {#chdberror}
...
таблицы/столбцы и другие проблемы при выполнении запросов.
...
## Информация о версиях {#version-information}
...
Если передан итерируемый объект, кортеж инициализируется его элементами.
...
Создаёт новый строковый объект из переданного объекта. Если заданы
параметры encoding или errors, объект должен предоставлять буфер данных,
который будет декодирован с использованием указанной кодировки и обработчика ошибок.
В противном случае возвращается результат вызова object._\_str_\_() (если определён)
или repr(object).
