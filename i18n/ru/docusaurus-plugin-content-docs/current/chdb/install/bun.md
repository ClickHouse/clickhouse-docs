...
description: 'Как устанавливать и использовать chDB в среде выполнения Bun'
...
chDB-bun предоставляет экспериментальные FFI-привязки (Foreign Function Interface) для chDB, позволяя выполнять запросы ClickHouse напрямую в приложениях Bun без внешних зависимостей.
...
chDB-bun поддерживает два режима выполнения запросов: временные запросы для одноразовых операций и постоянные сессии для сохранения состояния базы данных.

### Временные запросы {#ephemeral-queries}

Для простых одноразовых запросов, не требующих сохранения состояния:
