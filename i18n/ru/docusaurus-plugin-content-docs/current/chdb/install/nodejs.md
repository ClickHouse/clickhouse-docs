...
chDB-node предоставляет привязки для chDB в Node.js, позволяя выполнять запросы ClickHouse непосредственно в ваших приложениях Node.js без каких-либо внешних зависимостей.
...
function safeSessionQuery() {
  ...
  try {
    ...
    console.error("Ошибка запроса сессии:", error.message)
  } finally {
