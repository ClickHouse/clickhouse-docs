...
chDB предоставляет нативный API для C/C++, который позволяет встраивать функциональность ClickHouse непосредственно в приложения. API поддерживает как простые запросы, так и более продвинутые возможности, такие как постоянные соединения и потоковая обработка результатов запросов.
