...
sidebar_position: 1
title: 'Написание запросов в ClickHouse, используя данные GitHub'
```

Этот набор данных содержит все коммиты и изменения для репозитория ClickHouse. Он может быть сгенерирован с помощью встроенного инструмента `git-import`, который распространяется вместе с ClickHouse.

Сгенерированные данные предоставляют файл `tsv` для каждой из следующих таблиц:

- `commits` - коммиты со статистикой.
- `file_changes` - измененные файлы в каждом коммите с информацией о изменении и статистикой.
...
- `commits` - 7.8M - 266051 строк
- `file_changes` - 53M - 266051 строк
- `line_changes` - 2.7G - 7535157 строк
...
Следующие данные можно использовать для воспроизведения рабочей среды. Кроме того, этот набор данных доступен на play.clickhouse.com - смотрите [Запросы](#queries) для получения дополнительной информации.
...
Вставьте данные с использованием `INSERT INTO SELECT` и [функции s3](/sql-reference/table-functions/s3). Например, ниже мы вставляем файлы ClickHouse в каждую из их соответствующих таблиц:
...
Этот набор данных доступен на [play.clickhouse.com](https://sql.clickhouse.com?query_id=DCQPNPAIMAQXRLHYURLKVJ) в базах данных `git_clickhouse`. Мы предоставляем ссылку на эту среду для всех запросов, адаптируя имя базы данных по мере необходимости. Обратите внимание, что результаты игры могут отличаться от представленных здесь из-за различий во времени сбора данных.
### История одного файла {#history-of-a-single-file}

Самый простой из запросов. Здесь мы смотрим на все сообщения коммитов для `StorageReplicatedMergeTree.cpp`. Поскольку они, вероятно, более интересны, мы сортируем по самым недавним сообщениям сначала.
...
Обратите внимание, что существует более сложный вариант этого запроса, в котором мы находим [историю коммитов построчно для файла](#line-by-line-commit-history-of-a-file), учитывая переименования.
...
**Обратите внимание, что, похоже, была нарушена история коммитов в отношении файлов в директориях `dbms`, `libs`, `tests/testflows/` во время их переименований. Поэтому мы также исключаем их.**
...
Обратите внимание, что это позволяет файлам переименовываться, а затем снова переименовываться в их первоначальные значения. Сначала мы агрегируем `old_path` для списка удаленных файлов в результате переименования. Затем мы объединяем это с последней операцией для каждого `path`. Наконец, мы фильтруем этот список на те, где конечное событие не является `Delete`.
...
│      18559 │
└────────────┘
1 row in set. Elapsed: 0.089 sec. Processed 532.10 тысяч rows, 8.68 MB (6.01 million rows/s., 97.99 MB/s.)
```
...
Применяя этот шаблон к `git list-files`, количество файлов составляет 18155.
...
- Переименование может происходить наряду с другими изменениями в файле. Эти изменения перечислены как отдельные события в `file_changes`, но с одинаковым временем. Функция `argMax` не может отличить их - она выбирает первое значение. Естественный порядок вставок (единственное средство, чтобы знать правильный порядок) не поддерживается через объединение, поэтому могут быть выбраны измененные события. Например, ниже файл `src/Functions/geometryFromColumn.h` имеет несколько модификаций до того, как он был переименован в `src/Functions/geometryConverters.h`. Наше текущее решение может выбрать событие Modify как последнее изменение, что приводит к сохранению `src/Functions/geometryFromColumn.h`.
...
```
### Какой день недели коммиты обычно происходят? {#what-day-of-the-week-do-commits-usually-occur}
...
### История подкаталога/файла - число строк, коммитов и участников с течением времени {#history-of-subdirectoryfile---number-of-lines-commits-and-contributors-over-time}

Это приведет к большому результату запроса, который будет нереалистично показать или визуализировать, если он не отфильтрован. Поэтому мы позволяем фильтровать файл или подкаталог в следующем примере. Здесь мы группируем по неделям, используя функцию `toStartOfWeek` - адаптируйте по мере необходимости.
...
10 строк в наборе. Затраченное время: 1.101 сек. Обработано 8.07 миллионов строк, 905.86 МБ (7.33 миллиона строк/с., 823.13 МБ/с.)
```
...
10 строк в наборе. Затраченное время: 0.124 сек. Обработано 798.15 тысяч строк, 14.71 МБ (6.44 миллиона строк/с., 118.61 МБ/с.)
```
...
**Во время захвата данных изменения в папке `docs/` были отфильтрованы из-за очень запутанной истории коммитов. Результаты этого запроса, следовательно, неточные.**

Пишем ли мы больше документации в определенные периоды месяца, например, вблизи дат релизов? Мы можем использовать функцию `countIf`, чтобы вычислить простое соотношение, визуализировав результат с помощью функции `bar`.
...
31 строк в наборе. Затраченное время: 0.043 сек. Обработано 7.54 миллиона строк, 40.53 МБ (176.71 миллиона строк/с., 950.40 МБ/с.)
```
...
### Авторы с наиболее разнообразным влиянием {#authors-with-the-most-diverse-impact}
...
10 строк в наборе. Затраченное время: 0.041 сек. Обработано 266.05 тысяч строк, 4.92 МБ (6.56 миллиона строк/с., 121.21 МБ/с.)
```
...
10 строк в наборе. Затраченное время: 0.106 сек. Обработано 266.05 тысяч строк, 21.04 МБ (2.52 миллиона строк/с., 198.93 МБ/с.)
```
...
10 строк в наборе. Затраченное время: 0.106 сек. Обработано 798.15 тысяч строк, 13.97 МБ (7.51 миллиона строк/с., 131.41 МБ/с.)
```
...
│ TCPHandler.cpp                 │ 205 │
└────────────────────────────────┴─────┘
10 строк в наборе. Затраченное время: 0.032 сек. Обработано 266.05 тысяч строк, 5.68 МБ (8.22 миллиона строк/с., 175.50 МБ/с.)
```

Это может быть более отражающим его области интересов.
### Самые большие файлы с наименьшим количеством авторов {#largest-files-with-lowest-number-of-authors}
...
10 строк в наборе. Затраченное время: 0.138 сек. Обработано 798.15 тысяч строк, 16.57 МБ (5.79 миллиона строк/с., 120.11 МБ/с.)
```
...
└───────────────────────────────────────┴───────────┴─────────────┴────────────────────┘
10 строк в наборе. Затраченное время: 0.140 сек. Обработано 798.15 тысяч строк, 16.84 МБ (5.70 миллиона строк/с., 120.32 МБ/с.)
```
...
10 строк в наборе. Затраченное время: 0.143 сек. Обработано 798.15 тысяч строк, 18.00 МБ (5.58 миллионов строк/с., 125.87 МБ/с.)
```
```yaml
title: 'Коммиты и распределение строк кода по времени; по дню недели, по автору; для конкретных подсрок'
sidebar_label: 'Коммиты и строки кода по времени'
keywords: ['коммиты', 'распределение строк кода', 'день недели', 'автор']
description: 'Анализ коммитов и строк кода в ClickHouse по времени и авторам.'
```

### Коммиты и строки кода распределение по времени; по дню недели, по автору; для конкретных подсрок {#commits-and-lines-of-code-distribution-by-time-by-weekday-by-author-for-specific-subdirectories}

Мы интерпретируем это как количество строк, добавленных и удалённых по дням недели. В данном случае мы фокусируемся на [каталоге Functions](https://github.com/ClickHouse/ClickHouse/tree/master/src/Functions)
...
7 строк в множестве. Затраченное время: 0.034 сек. Обработано 266.05 тысяч строк, 14.66 МБ (7.73 миллиона строк/с., 425.56 МБ/с.)
```
...
24 строки в множестве. Затраченное время: 0.039 сек. Обработано 266.05 тысяч строк, 14.66 МБ (6.77 миллиона строк/с., 372.89 МБ/с.)
```

Это распределение имеет смысл, учитывая, что большая часть нашей команды разработки находится в Амстердаме. Функция `bar` позволяет нам визуализировать эти распределения:
...
24 строки в множестве. Затраченное время: 0.038 сек. Обработано 266.05 тысяч строк, 14.66 МБ (7.09 миллион строк/с., 390.69 МБ/с.)
```
### Матрица авторов, показывающая, какие авторы переписывают код других авторов {#matrix-of-authors-that-shows-what-authors-tends-to-rewrite-another-authors-code}
...
20 строк в множестве. Затраченное время: 0.098 сек. Обработано 7.54 миллиона строк, 42.16 МБ (76.67 миллион строк/с., 428.99 МБ/с.)
```

Диаграмма Санки (SuperSet) позволяет визуализировать это удобно. Обратите внимание, что мы увеличиваем наш `LIMIT BY` до 3, чтобы получить тройку главных удалителей кода для каждого автора, чтобы улучшить разнообразие в визуализации.
...
### Кто является автором с наибольшим процентом вклада по дням недели? {#who-is-the-highest-percentage-contributor-per-day-of-week}
...
7 строк в множестве. Затраченное время: 0.012 сек. Обработано 62.78 тысяч строк, 395.47 КБ (5.44 миллиона строк/с., 34.27 МБ/с.)
```
...
7 строк в множестве. Затраченное время: 0.004 сек. Обработано 21.82 тысячи строк, 140.02 КБ (4.88 миллиона строк/с., 31.29 МБ/с.)
```
...
7 строк в множестве. Затраченное время: 0.014 сек. Обработано 106.12 тысяч строк, 1.38 МБ (7.61 миллиона строк/с., 98.65 МБ/с.)
```
...
24 строки в множестве. Затраченное время: 0.129 сек. Обработано 798.15 тысяч строк, 15.11 МБ (6.19 миллиона строк/с., 117.08 МБ/с.)
```
### Какой процент кода автором был удалён другими авторами? {#what-percentage-of-code-for-an-author-has-been-removed-by-other-authors}
...
10 строк в множестве. Затраченное время: 0.126 сек. Обработано 15.07 миллионов строк, 73.51 МБ (119.97 миллионов строк/с., 585.16 МБ/с.)
```
### Перечислите файлы, которые были переписаны наибольшее количество раз? {#list-files-that-were-rewritten-most-number-of-times}
...
10 строк в множестве. Затраченное время: 0.160 сек. Обработано 8.07 миллионов строк, 98.99 МБ (50.49 миллионов строк/с., 619.49 МБ/с.)
```
...
Запрос ограничен только текущими файлами. Мы перечисляем все изменения файлов, группируя по `path` и `commit_hash`, возвращая количество добавленных и удалённых строк. Используя оконную функцию, мы оцениваем общий размер файла в любой момент времени, выполняя кумулятивную сумму и оценивая влияние каждого изменения на размер файла как `строки добавлены - строки удалены`. Используя эту статистику, мы можем рассчитать процент файла, который был добавлен или удалён для каждого изменения. Наконец, мы подсчитываем количество изменений файла, которые составляют переписывание на файл т.е. `(процент добавления >= 0.5) AND (процент удаления >= 0.5) AND текущий размер > 50`. Обратите внимание, что мы требуем, чтобы файлы содержали более 50 строк, чтобы избежать ранних вкладов в файл, которые могут быть учтены как перезапись. Это также избегает предвзятости очень маленьких файлов, которые могут быть более склонны к переписыванию.
...
10 строк в множестве. Затраченное время: 0.299 сек. Обработано 798.15 тысяч строк, 31.52 МБ (2.67 миллиона строк/с., 105.29 МБ/с.)
```
### В какой день недели код имеет наибольшую вероятность остаться в репозитории? {#what-weekday-does-the-code-have-the-highest-chance-to-stay-in-the-repository}
...
7 строк в множестве. Затраченное время: 3.965 сек. Обработано 15.07 миллионов строк, 1.92 ГБ (3.80 миллиона строк/с., 483.50 МБ/с.)
```

### Файлы, отсортированные по среднему возрасту кода {#files-sorted-by-average-code-age}

Этот запрос использует тот же принцип, что и [В какой день недели код имеет наибольшую вероятность остаться в репозитории](#what-weekday-does-the-code-have-the-highest-chance-to-stay-in-the-repository) - пытаясь уникально идентифицировать строку кода, используя путь и содержимое строки. Это позволяет нам определить время между добавлением и удалением строки. Однако мы фильтруем только текущие файлы и код, а затем берем среднее время для каждого файла по строкам.
...
10 строк в наборе. Время выполнения: 3.134 сек. Обработано 16.13 миллионов строк, 1.83 ГБ (5.15 миллионов строк/с., 582.99 МБ/с.)
```
### Кто, как правило, пишет больше тестов / кода / комментариев на C++? {#who-tends-to-write-more-tests--cpp-code--comments}

Существует несколько способов подойти к этому вопросу. Сосредоточившись на соотношении кода к тестам, этот запрос относительно прост - подсчитайте количество вкладов в папки, содержащие `tests`, и вычислите отношение к общему количеству вкладов.
...
20 строк в наборе. Время выполнения: 0.034 сек. Обработано 266.05 тысяч строк, 4.65 МБ (7.93 миллиона строк/с., 138.76 МБ/с.)
```
...
└────────────────────┴────────────────────┴───────────────────────────────┘
10 строк в наборе. Время выполнения: 0.051 сек. Обработано 266.05 тысяч строк, 4.65 МБ (5.24 миллиона строк/с., 91.64 МБ/с.)
```
...
└────────────────────┴─────────────────────┴─────────┘
10 строк в наборе. Время выполнения: 0.290 сек. Обработано 7.54 миллиона строк, 394.57 МБ (26.00 миллионов строк/с., 1.36 ГБ/с.)
```
...
### Как изменяются коммиты авторов с течением времени с точки зрения процента кода/комментариев? {#how-does-an-authors-commits-change-over-time-with-respect-to-codecomments-percentage}
...
10 строк в наборе. Время выполнения: 0.145 сек. Обработано 7.54 миллиона строк, 51.09 МБ (51.83 миллиона строк/с., 351.44 МБ/с.)
```
...
Чтобы вычислить это, мы сначала вычисляем соотношение комментариев для каждого автора по времени - аналогично [Кто, как правило, пишет больше тестов / кода / комментариев?](#who-tends-to-write-more-tests--cpp-code--comments). Это соединяется с датой начала каждого автора, что позволяет нам вычислить соотношение комментариев по недельным интервалам.
...
20 строк в наборе. Время выполнения: 0.167 сек. Обработано 15.07 миллионов строк, 101.74 МБ (90.51 миллиона строк/с., 610.98 МБ/с.)
```
...
1 строка в наборе. Время выполнения: 0.388 сек. Обработано 266.05 тысяч строк, 22.85 МБ (685.82 тысячи строк/с., 58.89 МБ/с.)
```
### Каково наихудшее время для написания кода с точки зрения того, что код имеет наибольшую вероятность переписывания? {#what-is-the-worst-time-to-write-code-in-sense-that-the-code-has-highest-chance-to-be-re-written}

Похоже на [Каково среднее время до перезаписи кода и медиана (период полураспада кода)?](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) и [Список файлов, которые были переписаны наибольшее количество раз или большинством авторов](#list-files-that-were-rewritten-most-number-of-times), за исключением того, что мы агрегируем по дням недели. При необходимости отрегулируйте, например, по месяцам года.
...
7 строк в наборе. Время выполнения: 0.466 сек. Обработано 7.54 миллиона строк, 701.52 МБ (16.15 миллионов строк/с., 1.50 ГБ/с.)
```
### Код какого автора самый «липкий»? {#which-authors-code-is-the-most-sticky}

Мы определяем «липкий» как то, как долго код автора остается, прежде чем его перепишут. Похоже на предыдущий вопрос [Каково среднее время до перезаписи кода и медиана (период полураспада кода)?](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) - используя тот же критерий для переписываний, т.е. 50% добавлений и 50% удалений файлов. Мы вычисляем среднее время переписывания по автору и учитываем только вкладчиков с более чем двумя файлами.
...
10 строк в наборе. Время выполнения: 0.555 сек. Обработано 7.54 миллиона строк, 720.60 МБ (13.58 миллионов строк/с., 1.30 ГБ/с.)
```
### Наибольшее количество последовательных дней коммитов автором {#most-consecutive-days-of-commits-by-an-author}
...
Наши последующие функции массивов вычисляют самую длинную последовательность последовательных единиц для каждого автора. Сначала используется функция `groupArray`, чтобы собрать все значения `consecutive_day` для автора. Этот массив из 1 и 0 затем разбивается по значениям 0 в подмассивы. Наконец, мы вычисляем самую длинную подмассив.
...
10 строк в наборе. Время выполнения: 0.025 сек. Обработано 62.78 тысячи строк, 395.47 КБ (2.54 миллиона строк/с., 16.02 МБ/с.)
```
### История коммитов файла построчно {#line-by-line-commit-history-of-a-file}

Файлы могут быть переименованы. Когда это происходит, мы получаем событие переименования, где колонка `path` установлена на новый путь файла, а `old_path` представляет собой предыдущее местоположение, например:
...
Вызывая `file_path_history('src/Storages/StorageReplicatedMergeTree.cpp')`, мы рекурсивно проходим через историю переименования, где каждая функция вызывает следующий уровень с `old_path`. Результаты объединяются с помощью `arrayConcat`.
...
## Неразрешенные вопросы {#unsolved-questions}
### Git blame {#git-blame}

Это особенно сложно, чтобы получить точный результат из-за невозможности в настоящее время сохранять состояние в функциях массива. Это будет возможно с помощью `arrayFold` или `arrayReduce`, которые позволяют хранить состояние на каждой итерации.
...
- Блог: [Коммиты Git и наше сообщество](https://clickhouse.com/blog/clickhouse-git-community-commits)
...
- Блог: [История активности на GitHub с использованием ClickHouse + Grafana](https://clickhouse.com/blog/introduction-to-clickhouse-and-grafana-webinar)
