...
└────────────────────┴────────────────────┴───────────────────────────────┘
10 строк в наборе. Время: 0.051 сек. Обработано 266.05 тысяч строк, 4.65 МБ (5.24 миллиона строк/с., 91.64 МБ/с.)
```
...
└────────────────────┴─────────────────────┴─────────┘
10 строк в наборе. Время: 0.290 сек. Обработано 7.54 миллионов строк, 394.57 МБ (26.00 миллионов строк/с., 1.36 ГБ/с.)
```
...
### Как меняются коммиты авторов со временем по отношению к проценту кода/комментариев? {#how-does-an-authors-commits-change-over-time-with-respect-to-codecomments-percentage}
...
10 строк в наборе. Время: 0.145 сек. Обработано 7.54 миллиона строк, 51.09 МБ (51.83 миллиона строк/с., 351.44 МБ/с.)
```
...
Чтобы вычислить это, мы сначала вычисляем соотношение комментариев для каждого автора со временем - аналогично [Кто склонен писать больше тестов / CPP кода / комментариев?](#who-tends-to-write-more-tests--cpp-code--comments). Затем мы присоединяем это к дате начала каждого автора, позволяя нам рассчитать соотношение комментариев по неделям.
...
20 строк в наборе. Время: 0.167 сек. Обработано 15.07 миллионов строк, 101.74 МБ (90.51 миллионов строк/с., 610.98 МБ/с.)
```
...
Мы можем использовать тот же принцип, что и [Список файлов, которые переписывались наибольшее количество раз или большинством авторов](#list-files-that-were-rewritten-most-number-of-times), чтобы определить переписывания, но учитываем все файлы. Оконная функция используется для вычисления времени между переписываниями для каждого файла. Из этого мы можем вычислить среднее и медиану по всем файлам.
...
1 строка в наборе. Время: 0.388 сек. Обработано 266.05 тысяч строк, 22.85 МБ (685.82 тысячи строк/с., 58.89 МБ/с.)
```

### Когда наихудшее время для написания кода в том смысле, что код имеет наибольшую вероятность быть переписанным? {#what-is-the-worst-time-to-write-code-in-sense-that-the-code-has-highest-chance-to-be-re-written}

По аналогии с [Каково среднее время до переписывания кода и медиана (период полураспада кода)?](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) и [Список файлов, которые переписывались наибольшее количество раз или большинством авторов](#list-files-that-were-rewritten-most-number-of-times), мы агрегируем по дню недели. Скорректируйте по мере необходимости, например, по месяцу года.
...
7 строк в наборе. Время: 0.466 сек. Обработано 7.54 миллиона строк, 701.52 МБ (16.15 миллионов строк/с., 1.50 ГБ/с.)
```

### Код каких авторов наименее подвержен изменениям? {#which-authors-code-is-the-most-sticky}

Мы определяем "липкий" как то, насколько долго код автора остается прежним, прежде чем его переписывают. Аналогично предыдущему вопросу [Каково среднее время до переписывания кода и медиана (период полураспада кода)?](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) - используя тот же критерий для переписываний, т.е. 50% добавлений и 50% удалений из файла. Мы вычисляем среднее время переписывания по авторам и учитываем только тех авторов, которые внесли изменения в более чем два файла.
...
10 строк в наборе. Время: 0.555 сек. Обработано 7.54 миллиона строк, 720.60 МБ (13.58 миллиона строк/с., 1.30 ГБ/с.)
```

### Наибольшее количество подряд идущих дней коммитов автором {#most-consecutive-days-of-commits-by-an-author}
...
10 строк в наборе. Время: 0.025 сек. Обработано 62.78 тысячи строк, 395.47 КБ (2.54 миллиона строк/с., 16.02 МБ/с.)
```
...
- Блог: [История активности с открытым исходным кодом на GitHub с использованием ClickHouse + Grafana](https://clickhouse.com/blog/introduction-to-clickhouse-and-grafana-webinar)
