...
Этот набор данных содержит все коммиты и изменения в репозитории ClickHouse. Он может быть сгенерирован с помощью встроенного инструмента `git-import`, распространяемого вместе с ClickHouse.
...
Вставьте данные с помощью `INSERT INTO SELECT` и [табличной функции s3](/sql-reference/table-functions/s3). Например, ниже показана вставка файлов ClickHouse в соответствующие таблицы:
...
Инструмент предлагает несколько запросов в своей справочной информации. Мы ответили на них, а также на некоторые дополнительные вопросы, представляющие интерес. Эти запросы расположены в порядке приблизительно возрастающей сложности, в отличие от произвольного порядка инструмента.
...
Самый простой из запросов. Здесь мы рассматриваем все сообщения коммитов для файла `StorageReplicatedMergeTree.cpp`. Поскольку наиболее интересны последние изменения, мы сортируем результаты, начиная с самых свежих сообщений.
...
10 строк в наборе. Затрачено времени: 0.085 сек. Обработано 532.10 тысяч строк, 8.68 MB (6.30 миллионов строк/с., 102.64 MB/с.)
...
│      18559 │
└────────────┘
1 строка в наборе. Затрачено: 0.089 сек. Обработано 532.10 тыс. строк, 8.68 МБ (6.01 млн строк/сек., 97.99 МБ/сек.)
````

Обратите внимание, что мы пропустили импорт нескольких директорий, а именно:
...
**Таким образом, наше текущее решение даёт лишь оценку актуального набора файлов**

Разница здесь обусловлена несколькими факторами:

* Переименование может происходить вместе с другими изменениями файла. Эти изменения фиксируются как отдельные события в `file_changes`, но с одинаковым временем. Функция `argMax` не может их различить — она выбирает первое попавшееся значение. Естественный порядок вставок (единственный способ определить правильную последовательность) не сохраняется при выполнении `UNION`, поэтому могут быть выбраны события с изменениями. Например, ниже файл `src/Functions/geometryFromColumn.h` несколько раз изменяется, прежде чем будет переименован в `src/Functions/geometryConverters.h`. Наше текущее решение может выбрать событие Modify как последнее изменение, из‑за чего `src/Functions/geometryFromColumn.h` будет сохранён.
...
```
- Неполная история коммитов — отсутствуют события удаления. Источник и причина будут определены.

Эти различия не должны существенно повлиять на анализ. **Мы приветствуем улучшенные версии этого запроса**.

### Список файлов с наибольшим числом изменений {#list-files-with-most-modifications}

Ограничиваясь текущими файлами, число изменений определяется как сумма удалений и добавлений.
...
Без фильтрации этот запрос выдаст большой результат, который сложно отобразить или визуализировать. Поэтому в следующем примере мы применяем фильтрацию по файлу или подкаталогу. Здесь мы группируем данные по неделям с помощью функции `toStartOfWeek` — при необходимости адаптируйте запрос.
...
Ограничение только текущими файлами.
...
10 rows in set. Прошло: 0.124 сек. Обработано 798.15 тысяч строк, 14.71 MB (6.44 млн строк/с., 118.61 MB/с.)

````

Наша основная структура данных, Merge Tree, очевидно, находится в постоянном развитии с длительной историей изменений!

### Распределение вкладов в документацию и код в течение месяца {#distribution-of-contributors-with-respect-to-docs-and-code-over-the-month}

**При сборе данных изменения в папке `docs/` были отфильтрованы из-за сильно загрязнённой истории коммитов. Поэтому результаты этого запроса не являются точными.**
...
Под разнообразием здесь понимается количество уникальных файлов, в которые автор внес вклад.
...
Посмотрим, у кого самые разнообразные коммиты в недавней работе. Вместо ограничения по дате мы ограничимся последними N коммитами автора (в данном случае мы использовали 3, но вы можете изменить это значение):
...
Для оценки, ограничившись текущими файлами, суммируем добавленные строки и вычитаем удалённые. Затем можно вычислить соотношение длины к количеству авторов.
...
Текстовые словари, возможно, не очень реалистичны, поэтому давайте ограничимся только кодом, отфильтровав его по расширению файла!
...
Здесь есть некоторая предвзятость в пользу новых файлов — у них меньше возможностей для коммитов. Что если ограничиться файлами, которым как минимум год?
...
Такое распределение закономерно, поскольку большая часть нашей команды разработки находится в Амстердаме. Функция `bar` помогает нам визуализировать эти распределения:
...
Значение `sign = -1` означает удаление кода. Знаки пунктуации и вставка пустых строк не учитываются.
...
### Кто является наибольшим процентным вкладчиком по дням недели? {#who-is-the-highest-percentage-contributor-per-day-of-week}
...
Здесь у самого активного контрибьютора — нашего основателя Алексея — могут быть определённые преимущества. Ограничим анализ последним годом.
...
Это всё ещё слишком упрощённо и не отражает реальный вклад участников.

Более точной метрикой может быть определение главного контрибьютора каждого дня как доли от общего объёма работы за последний год. Обратите внимание, что удаление и добавление кода учитываются одинаково.
...
Анализ ограничен текущими файлами. Для краткости результаты ограничены глубиной 2 с 5 файлами на корневую папку. Настройте параметры по необходимости.
...
10 rows in set. Elapsed: 0.160 sec. Обработано 8.07 million rows, 98.99 MB (50.49 million rows/s., 619.49 MB/s.)
...
10 строк в наборе. Затрачено: 0.299 сек. Обработано 798.15 тыс. строк, 31.52 МБ (2.67 млн. строк/сек., 105.29 МБ/сек.)
...
Мы запрашиваем добавленные строки, объединяя их с удалёнными строками и фильтруя случаи, когда последние произошли позже первых. Это даёт нам удалённые строки, из которых мы можем вычислить время между этими двумя событиями.

Наконец, мы агрегируем данные по этому набору для вычисления среднего количества дней, в течение которых строки остаются в репозитории, по дням недели.
...
7 строк в наборе. Затрачено: 3.965 сек. Обработано 15.07 млн. строк, 1.92 ГБ (3.80 млн. строк/сек., 483.50 МБ/сек.)
````
...
Это позволяет нам определить время между добавлением и удалением строки. Однако мы фильтруем только текущие файлы и код, и усредняем время для каждого файла по строкам.
...
Существует несколько способов ответить на этот вопрос. Если сосредоточиться на соотношении кода к тестам, запрос получается относительно простым — подсчитываем количество изменений в папках, содержащих `tests`, и вычисляем соотношение к общему количеству изменений.

Обратите внимание, что мы ограничиваемся пользователями с более чем 20 изменениями, чтобы сосредоточиться на постоянных контрибьюторах и избежать смещения в сторону единичных вкладов.
...
20 строк в наборе. Затрачено: 0.034 сек. Обработано 266.05 тыс. строк, 4.65 МБ (7.93 млн строк/с, 138.76 МБ/с).
...
Большинство контрибьюторов пишут больше кода, чем тестов, что вполне ожидаемо.
...
Обратите внимание, что сортировка выполняется по объёму внесённого кода. Удивительно высокий процент комментариев у всех наших крупнейших контрибьюторов — это одна из причин, почему наш код настолько читаем.
...
Вычислить это для конкретного автора несложно,
...
В идеале нам хотелось бы увидеть, как это в совокупности меняется для всех авторов, начиная с первого дня, когда они начинают делать коммиты. Со временем они сокращают количество пишемых комментариев?

Чтобы это вычислить, мы сначала рассчитываем долю комментариев каждого автора во времени — аналогично разделу [Who tends to write more tests / CPP code / comments?](#who-tends-to-write-more-tests--cpp-code--comments). Затем мы соединяем эти данные с датой начала работы каждого автора, что позволяет нам вычислить долю комментариев по смещению в неделях.

После вычисления среднего значения по смещению в неделях для всех авторов мы проводим выборку этих результатов, выбирая каждую 10-ю неделю.
...
Получена 1 строка. Затрачено: 0.388 сек. Обработано 266.05 тыс. строк, 22.85 МБ (685.82 тыс. строк/сек., 58.89 МБ/сек.)

````

### В какое время писать код наименее выгодно с точки зрения вероятности его переписывания? {#what-is-the-worst-time-to-write-code-in-sense-that-the-code-has-highest-chance-to-be-re-written}

Аналогично вопросам [Каково среднее время до переписывания кода и медиана (период полураспада кода)?](#what-is-the-average-time-before-code-will-be-rewritten-and-the-median-half-life-of-code-decay) и [Список файлов, которые были переписаны наибольшее количество раз или наибольшим количеством авторов](#list-files-that-were-rewritten-most-number-of-times), но здесь агрегация выполняется по дням недели. При необходимости можно изменить группировку, например, на месяцы года.
...
Последующие функции для работы с массивами вычисляют самую длинную последовательность единиц для каждого автора. Сначала функция `groupArray` собирает все значения `consecutive_day` для автора. Полученный массив из единиц и нулей затем разбивается по нулевым значениям на подмассивы. В конце вычисляется длина самого длинного подмассива.
...
Для решения этой проблемы можно использовать пользовательские функции (UDF). В настоящее время они не поддерживают рекурсию, поэтому для отслеживания истории файла необходимо определить серию UDF, которые явно вызывают друг друга.

Это означает, что отслеживать переименования можно только до определенной глубины — в примере ниже глубина составляет 5. Маловероятно, что файл будет переименован большее количество раз, поэтому на данный момент этого достаточно.
...
При вызове `file_path_history('src/Storages/StorageReplicatedMergeTree.cpp')` мы рекурсивно проходим по истории переименований, и на каждом шаге функция вызывает следующий уровень с `old_path`. Результаты объединяются с помощью `arrayConcat`.
...
Теперь мы можем воспользоваться этой возможностью, чтобы собрать коммиты для всей истории файла. В этом примере мы показываем по одному коммиту для каждого значения `path`.
...
Получить точный результат особенно сложно из-за невозможности в настоящее время сохранять состояние в функциях для работы с массивами. Это станет возможным с помощью `arrayFold` или `arrayReduce`, которые позволяют сохранять состояние на каждой итерации.
...
└─────────────────┴──────────────────────┴───────────────────────────────────────────────────────────────┘
Получено 20 строк. Затрачено: 0.547 сек. Обработано 7.88 млн строк, 679.20 МБ (14.42 млн строк/сек., 1.24 ГБ/сек.)
```
