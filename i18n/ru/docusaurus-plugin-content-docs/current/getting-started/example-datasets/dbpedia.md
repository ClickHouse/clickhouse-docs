...
Этот набор данных отлично подходит для начала работы с векторными эмбеддингами, векторным поиском по сходству и генеративным ИИ. Мы используем этот набор данных для демонстрации [приближенного поиска ближайших соседей](../../engines/table-engines/mergetree-family/annindexes.md) в ClickHouse и простого, но мощного приложения для ответов на вопросы.
...
- Генерация вектора представления для поискового запроса с помощью LLM-модели
...
до поискового вектора-эмбеддинга с последующей сортировкой расстояний для получения ближайших соседей. Для набора данных `dbpedia`
...
Зафиксируйте задержку выполнения запроса, чтобы затем сравнить её с задержкой запроса ANN (с использованием векторного индекса).
Также измерьте задержку запроса с холодным файловым кешем ОС и с параметром `max_threads=1`, чтобы оценить реальное потребление
...
Запросы поиска по сходству, рассмотренные ранее, используют один из существующих векторов в таблице `dbpedia`
в качестве поискового вектора. В реальных приложениях поисковый вектор необходимо
сгенерировать для пользовательского запроса, который может быть на естественном языке. Поисковый вектор
следует генерировать с использованием той же LLM-модели, которая использовалась для генерации векторов эмбеддингов
для набора данных.

Ниже приведен пример скрипта на Python, демонстрирующий, как программно вызывать API OpenAI для
генерации векторов эмбеддингов с использованием модели `text-embedding-3-large`. Поисковый вектор эмбеддинга
затем передается в качестве аргумента функции `cosineDistance()` в запросе `SELECT`.

Для запуска скрипта необходимо установить ключ API OpenAI в переменной окружения `OPENAI_API_KEY`.
...
while True:
    # Принять поисковый запрос от пользователя
...
приложения. Для запуска приложения требуется установить ключ OpenAI API в переменной окружения `OPENAI_API_KEY`. Ключ OpenAI API можно получить после регистрации на https://platform.openai.com.

```shell
$ python3 QandA.py

Введите тему: FIFA world cup 1990
Генерация эмбеддинга для 'FIFA world cup 1990' и сбор 100 связанных статей из ClickHouse...
...
Генерация эмбеддинга для 'Cricket world cup' и сбор 100 связанных статей из ClickHouse...

Введите ваш вопрос: Which country has hosted the world cup most times
Англия и Уэльс принимали чемпионат мира по крикету больше всего раз, турнир проводился в этих странах пять раз — в 1975, 1979, 1983, 1999 и 2019 годах.
