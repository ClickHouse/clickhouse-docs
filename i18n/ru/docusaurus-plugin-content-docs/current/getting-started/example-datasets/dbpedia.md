...
keywords: ['семантический поиск', 'сходство векторов', 'поиск приблизительных ближайших соседей', 'эмбеддинги']
doc_type: 'guide'
---
...
Этот набор данных — отличная отправная точка для изучения векторных эмбеддингов, поиска по сходству векторов и генеративного ИИ. Мы используем этот набор данных, чтобы продемонстрировать [поиск приблизительных ближайших соседей](../../engines/table-engines/mergetree-family/annindexes.md) в ClickHouse и простое, но мощное Q&A‑приложение.



## Подробности о наборе данных {#dataset-details}

Набор данных содержит 26 файлов `Parquet`, расположенных на [huggingface.co](https://huggingface.co/api/datasets/Qdrant/dbpedia-entities-openai3-text-embedding-3-large-1536-1M/parquet/default/train/). Файлы имеют имена `0.parquet`, `1.parquet`, ..., `25.parquet`. Чтобы просмотреть несколько примеров строк набора данных, посетите эту [страницу Hugging Face](https://huggingface.co/datasets/Qdrant/dbpedia-entities-openai3-text-embedding-3-large-1536-1M).
...
Также можно по отдельности выполнить SQL-запросы, как показано ниже, чтобы загрузить каждый из 25 файлов Parquet:
...
Рекомендуемая статья: [руководство по векторным эмбеддингам](https://platform.openai.com/docs/guides/embeddings)
...
- Принять поисковый запрос от пользователя на естественном языке, например: _«Расскажи о живописных железнодорожных путешествиях»_, _«Триллеры, действие которых происходит в Европе»_ и т. д.
- Сгенерировать вектор эмбеддинга для поискового запроса с помощью модели LLM
- Найти ближайших соседей для вектора эмбеддинга запроса в наборе данных

_Ближайшие соседи_ — это документы, изображения или другой контент, представляющие собой релевантные результаты для пользовательского запроса.
Извлечённые результаты являются основным входом для Retrieval Augmented Generation (RAG) в приложениях генеративного ИИ.



## Выполнить поиск сходства векторов полным перебором

Поиск kNN (k - ближайших соседей) или поиск полным перебором (brute force) предполагает вычисление расстояния от каждого вектора в наборе данных
до искомого вектора-эмбеддинга, а затем упорядочивание этих расстояний для получения ближайших соседей. С набором данных `dbpedia`
быстрый способ наглядно увидеть работу семантического поиска — использовать векторы-эмбеддинги из самого набора данных в качестве поисковых
векторов. Например:
...
20. │ <dbpedia:Kazuo_Ishiguro>                  │ Kazuo Ishiguro                  │
    └───────────────────────────────────────────┴─────────────────────────────────┘
#highlight-next-line
20 строк в наборе. Выполнено за 0.261 сек. Обработано 1.00 млн строк, 6.22 ГБ (3.84 млн строк/с, 23.81 ГБ/с).
```
...
Также измерьте задержку запроса при холодном файловом кэше ОС и с `max_threads=1`, чтобы оценить реальное использование вычислительных ресурсов и пропускную способность хранилища (а затем экстраполируйте результаты на промышленный набор данных с миллионами векторов!).


## Создание индекса векторного сходства

Выполните следующий SQL-запрос, чтобы определить и создать индекс векторного сходства по столбцу `vector`:
...
Параметры и вопросы производительности при создании индекса и поиске описаны в [документации](../../engines/table-engines/mergetree-family/annindexes.md).

Построение и сохранение индекса может занять несколько минут в зависимости от числа доступных ядер CPU и пропускной способности хранилища.


## Выполнить приближённый поиск ближайших соседей (ANN)

*Approximate Nearest Neighbours* или ANN обозначает набор методик (например, специальные структуры данных, такие как графы и случайные леса), которые позволяют получать результаты значительно быстрее, чем при точном векторном поиске. Точность результатов обычно «достаточно хороша» для практического использования. Многие приближённые методы предоставляют параметры для настройки баланса между точностью результата и временем поиска.

После того как индекс векторного сходства построен, запросы векторного поиска будут автоматически использовать этот индекс:
...
В рассмотренных выше примерах запросов на поиск по схожести в качестве вектора поиска использовался один из существующих векторов в таблице `dbpedia`. В реальных приложениях вектор поиска должен генерироваться для пользовательского текстового запроса, который может быть сформулирован на естественном языке. Вектор поиска следует генерировать с использованием той же LLM‑модели, которая применялась для получения эмбеддингов для набора данных.

Ниже приведён пример скрипта на Python, демонстрирующий, как программно вызывать API OpenAI для
генерации эмбеддингов с использованием модели `text-embedding-3-large`. Полученный эмбеддинг
поискового запроса затем передаётся в качестве аргумента функции `cosineDistance()` в запросе `SELECT`.

Для запуска скрипта необходимо задать ключ API OpenAI в переменной окружения `OPENAI_API_KEY`.
Ключ API OpenAI можно получить после регистрации на [https://platform.openai.com](https://platform.openai.com).
...
while True:
    # Примите поисковый запрос от пользователя
    print("Введите поисковый запрос:")
    input_query = sys.stdin.readline();

    # Вызовите конечную точку OpenAI API для получения эмбеддинга
    print("Формируем эмбеддинг для ", input_query);
    embedding = get_embedding(input_query,
                              model='text-embedding-3-large')

    # Выполните векторный поисковый запрос в ClickHouse
...
## Демонстрационное приложение для вопросов и ответов (Q&amp;A)

Приведённые выше примеры демонстрировали семантический поиск и извлечение документов с использованием ClickHouse. Далее представлено очень простое, но обладающее большим потенциалом пример приложения генеративного ИИ.

Приложение выполняет следующие шаги:

1. Принимает *topic* (тему) в качестве входного параметра от пользователя
2. Генерирует вектор эмбеддинга для *topic*, вызывая OpenAI API с моделью `text-embedding-3-large`
3. Извлекает наиболее релевантные статьи и документы Wikipedia с помощью векторного поиска по сходству в таблице `dbpedia`
4. Принимает произвольный вопрос на естественном языке от пользователя, связанный с *topic*
5. Использует Chat API OpenAI `gpt-3.5-turbo` для ответа на вопрос на основе знаний из документов, полученных на шаге 3.
   Документы, извлечённые на шаге 3, передаются в Chat API как *context* (контекст) и являются ключевым звеном в генеративном ИИ.

Ниже сначала приведено несколько примеров диалога при запуске Q&amp;A-приложения, а затем — код
Q&amp;A-приложения. Для запуска приложения требуется задать ключ API OpenAI в переменной окружения
`OPENAI_API_KEY`. Ключ API OpenAI можно получить после регистрации на [https://platform.openai.com](https://platform.openai.com).
...
ch_client = clickhouse_connect.get_client(compress=False) # Укажите здесь учетные данные ClickHouse
openai_client = OpenAI() # Задайте переменную окружения OPENAI_API_KEY
...
Question: {question}"""
    ...
    response = openai_client.chat.completions.create(
        messages=[
        {'role': 'system', 'content': "Вы отвечаете на вопросы по теме {input_query}."},
