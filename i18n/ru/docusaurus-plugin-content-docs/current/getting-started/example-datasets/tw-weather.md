...
- [Предварительно обработанная версия](#pre-processed-data) данных для ClickHouse, которая была очищена, переструктурирована и обогащена. Этот набор данных охватывает годы с 1896 по 2023.
- [Скачать оригинальные необработанные данные](#original-raw-data) и преобразовать в формат, требуемый ClickHouse. Пользователи, желающие добавить свои собственные столбцы, могут исследовать или завершить свои методы.
...
Этот набор данных доступен по следующему адресу Google CloudStorage. Вы можете либо скачать набор данных на свою локальную файловую систему (и вставить его с помощью клиента ClickHouse), либо вставить его непосредственно в ClickHouse (см. [Вставка по URL](#inserting-from-url)).
...
#### Получить метеорологические станции Тайваня {#retrieve-the-taiwan-weather-stations}
...
```
Чтобы узнать, как ускорить это, пожалуйста, просмотрите наш блог-пост о [настройке больших загрузок данных](https://clickhouse.com/blog/supercharge-your-clickhouse-data-loads-part2).
...
### Q1: Получить максимальную температуру точки росы для каждой метеорологической станции в определенном году {#q1-retrieve-the-highest-dew-point-temperature-for-each-weather-station-in-the-specific-year}
...
### Q2: Получение необработанных данных с определенным диапазоном по времени, полями и метеорологической станцией {#q2-raw-data-fetching-with-the-specific-duration-time-range-fields-and-weather-station}
