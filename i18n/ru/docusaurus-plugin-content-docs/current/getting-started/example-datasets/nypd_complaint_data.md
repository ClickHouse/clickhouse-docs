...
Строки 2 и 3 выше содержат конкатенацию из предыдущего шага, а строки 4 и 5 преобразуют эти строки в `DateTime64`. Поскольку время окончания жалобы может отсутствовать, используется `parseDateTime64BestEffortOrNull`.
