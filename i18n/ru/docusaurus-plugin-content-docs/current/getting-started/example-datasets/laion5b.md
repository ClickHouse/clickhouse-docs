...
[Набор данных LAION 5b](https://laion.ai/blog/laion-5b/) содержит 5,85 миллиарда векторных представлений пар изображение-текст и связанные метаданные изображений. Векторные представления были сгенерированы с использованием модели `Open AI CLIP` [ViT-L/14](https://huggingface.co/sentence-transformers/clip-ViT-L-14). Размерность каждого вектора составляет `768`.

Этот набор данных можно использовать для моделирования проектирования, определения размеров и оценки производительности крупномасштабного приложения векторного поиска в реальных условиях. Набор данных подходит как для поиска изображений по текстовому запросу, так и для поиска изображений по изображению.
...
Поле `id` представляет собой инкрементное целое число. Дополнительные атрибуты могут использоваться в предикатах для реализации поиска по векторному сходству в сочетании с постфильтрацией/префильтрацией, как описано в [документации](../../engines/table-engines/mergetree-family/annindexes.md)
...
Альтернативно можно выполнить отдельные SQL-команды для загрузки определенного количества файлов/строк.
...
Поиск методом KNN (k ближайших соседей) или методом полного перебора включает вычисление расстояния от каждого вектора в наборе данных до искомого вектора эмбеддинга с последующей сортировкой расстояний для получения ближайших соседей. В качестве искомого вектора можно использовать один из векторов самого набора данных. Например:
...
Запишите время выполнения запроса, чтобы сравнить его с временем выполнения ANN-запроса (с использованием векторного индекса).
При работе со 100 миллионами строк выполнение приведённого выше запроса без векторного индекса может занять от нескольких секунд до нескольких минут.

### Построение индекса векторного сходства {#build-vector-similarity-index}

Выполните следующий SQL-запрос для определения и построения индекса векторного сходства на столбце `vector` таблицы `laion_5b_100m`:
