...
2. Давайте определим новую таблицу `MergeTree` с именем `amazon_reviews`, чтобы хранить эти данные в ClickHouse:
...
5. Этот запрос не занимает много времени — в среднем около 300 000 строк в секунду. Через 5 минут вы должны увидеть все строки, вставленные:
...
keywords: ['queries', 'sql', 'clickhouse', 'example']
description: 'Примеры запросов к ClickHouse с различными сценариями использования и результатами.'
```
...
10 строк в результате. Время выполнения: 0.897 сек. Обработано 150.96 миллионов строк, 15.36 ГБ (168.36 миллионов строк/с., 17.13 ГБ/с.)
```

8. Вот 10 самых популярных продуктов на Amazon с наибольшим количеством отзывов:
...
10 строк в результате. Время выполнения: 20.059 сек. Обработано 150.96 миллионов строк, 12.78 ГБ (7.53 миллиона строк/с., 637.25 МБ/с.)
```
...
20 строк в результате. Время выполнения: 43.055 сек. Обработано 150.96 миллионов строк, 13.24 ГБ (3.51 миллиона строк/с., 307.41 МБ/с.)
Пиковое потребление памяти: 41.73 ГБ.
```
...
43 строки в результате. Время выполнения: 0.201 сек. Обработано 150.96 миллионов строк, 754.79 МБ (750.85 миллионов строк/с., 3.75 ГБ/с.)
```

11. Давайте найдем продукты, в которых слово **"ужасный"** встречается чаще всего в отзыве. Это большая задача — нужно проанализировать более 151M строк в поисках одного слова:
...
50 строк в результате. Время выполнения: 4.072 сек. Обработано 150.96 миллионов строк, 68.93 ГБ (37.07 миллионов строк/с., 16.93 ГБ/с.)
Пиковое потребление памяти: 1.82 ГБ.
```

12. Мы можем снова выполнить тот же запрос, но на этот раз будем искать **великолепный** в отзывах:
...
50 строк в результате. Время выполнения: 4.079 сек. Обработано 150.96 миллионов строк, 68.95 ГБ (37.01 миллиона строк/с., 16.90 ГБ/с.)
Пиковое потребление памяти: 2.18 ГБ.
```
