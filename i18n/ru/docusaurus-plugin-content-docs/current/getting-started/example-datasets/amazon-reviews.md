...
2. Давайте определим новую таблицу `MergeTree`, названную `amazon_reviews`, чтобы хранить эти данные в ClickHouse:
...
sidebar_label: 'Примеры запросов'
keywords: ['запросы', 'Amazon', 'отзывы']
description: 'Примеры запросов для анализа данных Amazon в ClickHouse.'
```

## Примеры запросов {#example-queries}

7. Давайте запустим несколько запросов... вот 10 наиболее полезных отзывов в наборе данных:
...
10 строк в наборе. Время выполнения: 0.897 сек. Обработано 150.96 миллионов строк, 15.36 ГБ (168.36 миллионов строк/с., 17.13 ГБ/с.)
```

8. Вот 10 лучших продуктов на Amazon с наибольшим количеством отзывов:
...
10 строк в наборе. Время выполнения: 20.059 сек. Обработано 150.96 миллионов строк, 12.78 ГБ (7.53 миллионов строк/с., 637.25 МБ/с.)
```

9. Вот средние рейтинги отзывов за месяц для каждого продукта (фактический [вопрос на собеседовании Amazon](https://datalemur.com/questions/sql-avg-review-ratings)!) :
...
20 строк в наборе. Время выполнения: 43.055 сек. Обработано 150.96 миллионов строк, 13.24 ГБ (3.51 миллиона строк/с., 307.41 МБ/с.)
Пиковое использование памяти: 41.73 ГБ.
```
...
43 строки в наборе. Время выполнения: 0.201 сек. Обработано 150.96 миллионов строк, 754.79 МБ (750.85 миллионов строк/с., 3.75 ГБ/с.)
```

11. Давайте найдем продукты, в отзывах о которых слово **"ужасный"** встречается чаще всего. Это большая задача - необходимо проанализировать более 151M строк в поисках единственного слова:
...
50 строк в наборе. Время выполнения: 4.072 сек. Обработано 150.96 миллионов строк, 68.93 ГБ (37.07 миллионов строк/с., 16.93 ГБ/с.)
Пиковое использование памяти: 1.82 ГБ.
```

12. Мы можем запустить тот же запрос снова, только на этот раз мы ищем **потрясающее** в отзывах:
...
50 строк в наборе. Время выполнения: 4.079 сек. Обработано 150.96 миллионов строк, 68.95 ГБ (37.01 миллионов строк/с., 16.90 ГБ/с.)
Пиковое использование памяти: 2.18 ГБ.
```
