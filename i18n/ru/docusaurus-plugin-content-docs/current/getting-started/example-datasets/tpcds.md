...
Затем сгенерируйте данные. Параметр `-scale` определяет коэффициент масштабирования.
...
Мы максимально строго придерживаемся правил спецификации TPC-DS.
Абстрактные типы данных из спецификации сопоставляются с нативными типами данных ClickHouse следующим образом:

| Тип TPC-DS     | Тип ClickHouse                                           |
| -------------- | -------------------------------------------------------- |
| `identifier`   | `Int64` (или `UInt32` для ключей измерений даты/времени) |
| `integer`      | `Int32`                                                  |
...
Что касается nullability, столбцы, помеченные в спецификации как &quot;N&quot; (not null), не имеют обёртки `Nullable`, так как они никогда не содержат значения NULL.
Все остальные столбцы оборачиваются в `Nullable()`.
Для строковых типов `Nullable` размещается внутри `LowCardinality`, например `LowCardinality(Nullable(String))`.
...
:::note
Следующие настройки необходимо включить, чтобы получать корректные результаты в соответствии со стандартом SQL:
...
::::note
Запрос не работает по умолчанию из-за проблемы, описанной в https://github.com/ClickHouse/ClickHouse/issues/95299. Вместо него работает следующая альтернативная формулировка с небольшим исправлением:
...
::::note
Запрос возвращает `nan` вместо `NULL`, когда `stddev_samp` вызывается для одного значения. Соответствующий issue: https://github.com/ClickHouse/ClickHouse/issues/94683. В остальном результат корректен.
::::
...
::::note
Запрос не работает из коробки из-за ошибки, описанной в https://github.com/ClickHouse/ClickHouse/issues/95299. Работает следующая альтернативная запись с небольшим исправлением:
...
::::note
Запрос не работает «из коробки» из‑за проблемы, описанной в https://github.com/ClickHouse/ClickHouse/issues/94858. Вот альтернативная формулировка с небольшим исправлением, которая работает:
...
::::note
Запрос не работает из коробки из-за следующей проблемы: https://github.com/ClickHouse/ClickHouse/issues/94858. Работает следующая альтернативная формулировка с небольшой правкой:
...
::::note
Запрос не работает «из коробки» из‑за https://github.com/ClickHouse/ClickHouse/issues/94976. Вот рабочий альтернативный вариант с небольшим исправлением:
...
::::note
Запрос не работает из коробки из‑за https://github.com/ClickHouse/ClickHouse/issues/94671. Вот альтернативный вариант с небольшим исправлением, который работает:
...
::::note
Запрос не работает «из коробки» из-за проблемы https://github.com/ClickHouse/ClickHouse/issues/95299. Этот альтернативный вариант с небольшим исправлением работает:
