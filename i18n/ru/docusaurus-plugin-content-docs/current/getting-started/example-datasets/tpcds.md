...
Аналогично [Star Schema Benchmark (SSB)](star-schema.md), TPC-DS основан на [TPC-H](tpch.md), но пошёл по противоположному пути, т.е. увеличил количество необходимых соединений, храня данные в сложной схеме типа «снежинка» (24 таблицы вместо 8).
Распределение данных смещённое (например, нормальное и распределение Пуассона).
Бенчмарк включает 99 отчётных и ad-hoc запросов со случайными подстановками.
...
Затем сгенерируйте запросы (используя тот же коэффициент масштабирования):
...
Вы можете использовать либо исходные определения таблиц в tools/tpcds.sql, либо «оптимизированные» определения таблиц с корректно заданными индексами первичного ключа и типами столбцов LowCardinality там, где это целесообразно.
...
В результате многие из приведённых выше запросов бенчмарка завершатся ошибкой.
::::
