---
description: 'Набор данных, содержащий более чем 28 миллионов публикаций Hacker News и их векторные эмбеддинги'
...
Этот набор данных можно использовать для изучения аспектов проектирования, определения размеров и производительности крупномасштабного
...
Рекомендуется сначала выполнить расчет размеров для оценки требований к хранилищу и памяти для этого набора данных, обратившись к [документации](../../engines/table-engines/mergetree-family/annindexes.md).
...
Поле `id` представляет собой инкрементируемое целое число. Дополнительные атрибуты могут использоваться в предикатах для реализации поиска по векторному сходству в сочетании с постфильтрацией/префильтрацией, как описано в [документации](../../engines/table-engines/mergetree-family/annindexes.md)

### Загрузка данных {#load-table}

Для загрузки набора данных из файла `Parquet` выполните следующую SQL-команду:
...
Выполните следующую SQL-команду для определения и построения индекса векторного сходства на столбце `vector` таблицы `hackernews`:
...
Приведенная выше команда использует значения 64 и 512 соответственно для гиперпараметров HNSW `M` и `ef_construction`.
...
Ниже приведен пример выполнения Python-скрипта и результаты поиска по сходству
...
Ниже сначала приведен пример запуска приложения для суммаризации, за которым следует код
приложения для суммаризации. Для запуска приложения требуется установить ключ OpenAI API в переменную окружения
`OPENAI_API_KEY`. Ключ OpenAI API можно получить после регистрации на https://platform.openai.com.

Это приложение демонстрирует вариант использования генеративного ИИ, применимый ко множеству корпоративных областей, таких как:
...
протоколы совещаний, финансовые отчеты и т. д.
...
при обработке крупномасштабных аналитических нагрузок, хотя упоминаются некоторые проблемы, такие как операции DML и сложности с резервным копированием.
ClickHouse признан за свои возможности вычисления агрегатов в реальном времени и надежную
