---
description: 'Набор данных из двух таблиц с анонимизированными данными веб-аналитики о хитах и визитах'
...
Этот набор данных состоит из двух таблиц, содержащих обезличенные данные веб-аналитики с запросами (`hits_v1`) и визитами (`visits_v1`).

Таблицы можно скачать в виде сжатых файлов `tsv.xz`. Помимо примера, используемого в этом документе, расширенная версия (7,5 ГБ) таблицы `hits`, содержащая 100 миллионов строк, доступна в формате TSV по адресу [https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz).
...
### Импортируйте данные о посещениях {#import-the-visits-data}
...
процедурах ClickHouse; это один из запросов из набора тестов. Остальные
тесты описаны в разделе *Next Steps* в
конце этой страницы.
...
Дополнительные примеры запросов к этим таблицам можно найти среди [stateful tests](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql) ClickHouse.

:::note
Набор тестов использует базу данных с именем `test`, а таблицы называются `hits` и `visits`. Вы можете переименовать свою базу данных и таблицы или отредактировать SQL из тестового файла.
:::