...
Этот набор данных состоит из двух таблиц с обезличенными данными веб‑аналитики: по хитам (`hits_v1`) и визитам (`visits_v1`).

Таблицы можно скачать в виде сжатых файлов `tsv.xz`. Помимо примера, с которым работает этот документ, расширенная (7,5 ГБ) версия таблицы `hits`, содержащая 100 миллионов строк, доступна в формате TSV по адресу [https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz).
...
Наборы данных hits и visits используются в тестовых процедурах ClickHouse, 
это один из запросов из тестового набора. Остальные тесты описаны 
в разделе _Следующие шаги_ в конце этой страницы.
