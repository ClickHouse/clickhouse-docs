...
Данные хранятся в третьей нормальной форме, что требует большого количества операций `JOIN` при выполнении запросов.
Несмотря на возраст и нереалистичное допущение о равномерном и независимом распределении данных, TPC-H по-прежнему остается самым популярным OLAP-бенчмарком на сегодняшний день.
...
o&#95;comment        String)
ORDER BY (o&#95;orderkey);
-- Следующий вариант ключа сортировки не соответствует официальным правилам TPC-H, но рекомендован в разд. 4.5 статьи
...
:::note
Вместо использования `tpch-kit` и самостоятельной генерации таблиц вы можете импортировать данные из общедоступного S3-бакета. Перед этим обязательно создайте пустые таблицы с помощью приведённых выше операторов `CREATE`.
...
Эта альтернативная формулировка работает и проверена — она возвращает эталонные результаты.
