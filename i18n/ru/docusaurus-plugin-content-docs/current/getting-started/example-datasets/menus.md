...
description: 'Набор данных, содержащий 1,3 миллиона записей исторических данных о меню
  отелей, ресторанов и кафе с блюдами и их ценами.'
sidebar_label: 'Набор данных "Что на меню?" Нью-Йоркской публичной библиотеки'
slug: /getting-started/example-datasets/menus
title: 'Набор данных "Что на меню?" Нью-Йоркской публичной библиотеки'
---
...
Данные поступают из архива библиотеки, и они могут быть неполными и сложными для статистического анализа. Тем не менее, они также очень аппетитны.
Размер составляет всего 1,3 миллиона записей о блюдах в меню — это очень небольшой объем данных для ClickHouse, но это все равно хороший пример.
...
Мы отключаем [input_format_null_as_default](/operations/settings/formats#input_format_null_as_default), так как наши данные не имеют [NULL](/operations/settings/formats#input_format_null_as_default). В противном случае ClickHouse попытается разобрать последовательности `\N` и может спутать их с `\` в данных.

Настройка [date_time_input_format best_effort](/operations/settings/formats#date_time_input_format) позволяет разбирать [DateTime](../../sql-reference/data-types/datetime.md) поля в широком диапазоне форматов. Например, ISO-8601 без секунд, таких как '2000-01-01 01:02', будет распознано. Без этой настройки разрешен только фиксированный формат DateTime.

## Денормализовать данные {#denormalize-data}

Данные представлены в нескольких таблицах в [нормализованной форме](https://en.wikipedia.org/wiki/Database_normalization#Normal_forms). Это означает, что вам нужно выполнить [JOIN](/sql-reference/statements/select/join), если вы хотите запросить, например, названия блюд из элементов меню.
Для типичных аналитических задач гораздо эффективнее иметь дело с предварительно объединенными данными, чтобы избежать выполнения `JOIN` каждый раз. Это называется "денормализованные" данные.
