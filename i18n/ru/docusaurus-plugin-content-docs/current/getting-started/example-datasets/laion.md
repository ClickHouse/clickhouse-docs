...
Набор данных содержит URL изображения, эмбеддинги как для изображения, так и для подписи, оценку сходства между изображением и подписью, а также метаданные, например ширину и высоту изображения, лицензию и флаг NSFW. Мы можем использовать этот набор данных для демонстрации [приближённого поиска ближайших соседей](../../engines/table-engines/mergetree-family/annindexes.md) в ClickHouse.
...
Набор данных разделён на 410 файлов, каждый файл содержит примерно 1 миллион строк. Если вы хотите работать с меньшим подмножеством данных, просто скорректируйте диапазон, например `seq 0 9 | ...`.

(Приведённый выше скрипт на Python работает очень медленно (~2–10 минут на файл), потребляет много памяти (41 ГБ на файл), а итоговые CSV-файлы получаются крупными (10 ГБ каждый), поэтому будьте осторожны. Если у вас достаточно RAM, увеличьте значение параметра `-P1` для большего параллелизма. Если этого всё ещё недостаточно, подумайте о более эффективной процедуре загрузки — например, сначала конвертировать файлы .npy в Parquet, а затем выполнять всю дальнейшую обработку в ClickHouse.)
...
Для импорта CSV-файлов в ClickHouse выполните:
...
Пока что мы можем использовать эмбеддинг изображения случайного набора LEGO в качестве `target`.
...
## Выполнение приближённого векторного поиска по сходству с индексом векторного сходства {#run-an-approximate-vector-similarity-search-with-a-vector-similarity-index}
...
10 строк в наборе. Прошло: 0.019 сек. Обработано 137.27 тысяч строк, 24.42 MB (7.38 миллионов строк/с., 1.31 GB/s.)
...
Поиск по векторному сходству с использованием векторного индекса может возвращать результаты, незначительно отличающиеся от результатов поиска полным перебором.
Индекс HNSW потенциально может достичь полноты, близкой к 1 (той же точности, что и при поиске полным перебором), при тщательном подборе параметров HNSW и оценке качества индекса.
```
...
Первый запуск будет медленным, так как загружается модель, но последующие запуски будут быстрыми. Затем мы можем скопировать результат в `SET param_target=...` и легко писать запросы. Альтернативно, функция `encode_text()` может быть напрямую использована в качестве аргумента функции `cosineDistance`:
...
Обратите внимание, что сама UDF `encode_text()` может требовать несколько секунд для вычисления и возврата вектора эмбеддинга.

### Эмбеддинги изображений {#image-embeddings}

Эмбеддинги изображений можно создавать аналогичным образом, и мы предоставляем скрипт Python, который может генерировать эмбеддинг изображения, сохраненного локально в виде файла.
...
Полный поисковый запрос выглядит так:
