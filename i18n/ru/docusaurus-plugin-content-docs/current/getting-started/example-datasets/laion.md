...
[Набор данных Laion-400M](https://laion.ai/blog/laion-400-open-dataset/) содержит 400 миллионов изображений с англоязычными подписями. В настоящее время Laion предоставляет [ещё более крупный набор данных](https://laion.ai/blog/laion-5b/), но работа с ним устроена аналогично.

Набор данных содержит URL изображения, эмбеддинги как для самого изображения, так и для подписи к нему, оценку сходства между изображением и подписью, а также метаданные, например ширину и высоту изображения, лицензию и флаг NSFW. Мы можем использовать этот набор данных для демонстрации [поиска приблизительных ближайших соседей](../../engines/table-engines/mergetree-family/annindexes.md) в ClickHouse.

## Подготовка данных \{#data-preparation\}

В исходных данных эмбеддинги и метаданные хранятся в отдельных файлах. На этапе подготовки выполняется загрузка и объединение файлов, их преобразование в CSV и импорт в ClickHouse. Для этого вы можете использовать следующий скрипт `download.sh`:
...
Набор данных разбит на 410 файлов, каждый файл содержит примерно 1 миллион строк. Если вы хотите работать только с частью данных, просто скорректируйте пределы, например `seq 0 9 | ...`.

(Приведённый выше скрипт на Python работает очень медленно (~2–10 минут на файл), потребляет много памяти (до 41 ГБ на файл), а итоговые CSV‑файлы получаются большими (10 ГБ каждый), поэтому будьте осторожны. Если у вас достаточно RAM, увеличьте значение `-P1` для большей параллельности. Если этого всё ещё недостаточно, рассмотрите возможность более эффективной процедуры ингестии — например, сначала конвертировать файлы .npy в parquet, а затем выполнять всю остальную обработку в ClickHouse.)


## Создание таблицы \{#create-table\}

Чтобы создать таблицу без индексов, выполните команду:
...
## Выполнение векторного поиска по сходству полным перебором \{#run-a-brute-force-vector-similarity-search\}

Чтобы выполнить приближённый векторный поиск по сходству полным перебором, выполните:

```sql
SELECT url, caption FROM laion ORDER BY cosineDistance(image_embedding, {target:Array(Float32)}) LIMIT 10
```

`target` — это массив из 512 элементов, который передаётся клиентом в качестве параметра.
Удобный способ получения таких массивов будет представлен в конце статьи.
А пока мы можем запустить построение эмбеддинга для случайного изображения набора LEGO и использовать его в качестве `target`.
...
## Выполните приблизительный поиск по векторному сходству с использованием индекса векторного сходства \{#run-an-approximate-vector-similarity-search-with-a-vector-similarity-index\}

Теперь определим в таблице два индекса векторного сходства.
...
Параметры и особенности производительности для создания индекса и поиска описаны в [документации](../../engines/table-engines/mergetree-family/annindexes.md).
В приведённом выше определении индекса задаётся индекс HNSW, использующий «косинусное расстояние» в качестве метрики расстояния, с параметром «hnsw&#95;max&#95;connections&#95;per&#95;layer», равным 64, и параметром «hnsw&#95;candidate&#95;list&#95;size&#95;for&#95;construction», равным 256.
Индекс использует числа с плавающей запятой половинной точности формата bfloat16 в качестве квантования для оптимизации использования памяти.
...
Задержка запроса значительно уменьшилась, потому что ближайшие соседи извлекались с использованием векторного индекса.
...
Индекс HNSW при тщательном подборе параметров HNSW и оценке качества индекса потенциально может обеспечить полноту (recall), близкую к 1 (такую же точность, как поиск полным перебором).


## Создание эмбеддингов с помощью UDF \\{#creating-embeddings-with-udfs\\}

Обычно требуется создавать эмбеддинги для новых изображений или новых подписей к изображениям и выполнять поиск в данных похожих пар «изображение — подпись к изображению». Мы можем использовать [UDF](/sql-reference/functions/udf), чтобы создать вектор `target`, не выходя за пределы клиента. Важно использовать одну и ту же модель как для создания данных, так и для новых эмбеддингов, применяемых при поиске. В следующих скриптах используется модель `ViT-B/32`, которая также лежит в основе набора данных.
...
Затем создайте `encode_text_function.xml` в каталоге (по пути), указанном в `<user_defined_executable_functions_config>/path/to/*_function.xml</user_defined_executable_functions_config>` в файле конфигурации сервера ClickHouse.
...
Первый запуск будет медленным, так как он загружает модель, но последующие запуски будут быстрыми. Затем мы можем скопировать результат в `SET param_target=...` и легко писать запросы. Либо функцию `encode_text()` можно напрямую использовать как аргумент для функции `cosineDistance`:
...
Обратите внимание, что самому UDF `encode_text()` может потребоваться несколько секунд, чтобы вычислить и сформировать вектор эмбеддинга.


### Векторные представления изображений \{#image-embeddings\}

Векторные представления изображений можно создавать аналогичным образом. Мы предоставляем скрипт на Python, который может сгенерировать векторное представление изображения, сохранённого локально в виде файла.
...
Полный поисковый запрос выглядит следующим образом:
