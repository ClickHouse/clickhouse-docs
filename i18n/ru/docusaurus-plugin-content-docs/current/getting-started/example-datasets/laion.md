...
Набор данных содержит URL изображения, эмбеддинги как для изображения, так и для подписи, оценку степени сходства между изображением и подписью, а также метаданные, например ширину/высоту изображения, лицензию и флаг NSFW. Мы можем использовать этот набор данных для демонстрации [поиска приблизительных ближайших соседей](../../engines/table-engines/mergetree-family/annindexes.md) в ClickHouse.



## Подготовка данных {#data-preparation}

Эмбеддинги и метаданные хранятся в отдельных файлах исходных данных. На этапе подготовки данных происходит загрузка данных, объединение файлов,
...
# преобразуем np.array в списки
data['image_embedding'] = data['image_embedding'].apply(lambda x: x.tolist())
data['text_embedding'] = data['text_embedding'].apply(lambda x: x.tolist())



# этот небольшой хак нужен, потому что caption иногда содержит разные виды кавычек
...
# удаляем файлы с исходными данными
...
(Приведённый выше скрипт на Python работает очень медленно (~2–10 минут на файл), требует много памяти (41 ГБ на файл), а получающиеся CSV‑файлы большие (по 10 ГБ каждый), так что будьте осторожны. Если у вас достаточно ОЗУ, увеличьте число в параметре `-P1`, чтобы повысить степень параллелизма. Если это всё равно слишком медленно, подумайте о более эффективной процедуре загрузки — например, сначала конвертировать .npy‑файлы в Parquet, а затем выполнять всю остальную обработку в ClickHouse.)
...
Пока что в качестве `target` можно использовать эмбеддинг изображения случайного набора LEGO.
...
Получено 10 строк. Затрачено: 4.605 сек. Обработано 100.38 млн строк, 309.98 ГБ (21.80 млн строк/с., 67.31 ГБ/с.)
```


## Выполнение приближённого векторного поиска по сходству с использованием индекса векторного сходства {#run-an-approximate-vector-similarity-search-with-a-vector-similarity-index}
...
Поиск по векторному сходству с использованием индекса может возвращать результаты, незначительно отличающиеся от результатов полного перебора.
Индекс HNSW может достичь полноты (recall), близкой к 1 (той же точности, что и при полном переборе), при тщательном подборе параметров HNSW и оценке качества индекса.
```


## Создание эмбеддингов с помощью UDF {#creating-embeddings-with-udfs}

Как правило, требуется создавать эмбеддинги для новых изображений или новых подписей к изображениям и искать в данных похожие пары «изображение / подпись». Мы можем использовать [UDF](/sql-reference/functions/udf), чтобы создавать вектор `target`, не выходя за пределы клиента. Важно использовать одну и ту же модель как для построения исходных данных, так и для создания новых эмбеддингов для поиска. В следующих скриптах используется модель `ViT-B/32`, которая также лежит в основе набора данных.

### Текстовые эмбеддинги {#text-embeddings}

Сначала сохраните следующий скрипт на Python в каталоге `user_scripts/` в каталоге данных ClickHouse и сделайте его исполняемым (`chmod +x encode_text.py`).
...
Затем создайте файл `encode_text_function.xml` в расположении, на которое ссылается `<user_defined_executable_functions_config>/path/to/*_function.xml</user_defined_executable_functions_config>` в конфигурационном файле сервера ClickHouse.
...
Первый запуск будет медленным, так как загружается модель, но последующие запуски будут быстрыми. Затем мы можем скопировать результат в `SET param_target=...` и легко писать запросы. В качестве альтернативы функцию `encode_text()` можно напрямую использовать как аргумент функции `cosineDistance`:
...
Обратите внимание, что UDF `encode_text()` сам по себе может потребовать несколько секунд на вычисление и возврат вектора эмбеддинга.

### Эмбеддинги изображений {#image-embeddings}

Эмбеддинги изображений можно создавать аналогичным образом; ниже приведён скрипт на Python, который может сформировать эмбеддинг для изображения, локально сохранённого в файле.
...
```sql
SELECT encode_image('/путь/к/вашему/изображению');
```
