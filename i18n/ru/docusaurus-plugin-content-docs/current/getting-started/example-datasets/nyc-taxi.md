...
doc_type: 'guide'
keywords: ['пример набора данных', 'nyc taxi', 'учебник', 'выборка данных', 'начало работы']
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Выборка данных о поездках на такси в Нью-Йорке содержит более 3 миллиардов поездок на такси и заказных автомобилях (Uber, Lyft и т. д.), совершённых в Нью-Йорке с 2009 года. В этом вводном руководстве используется выборка на 3 млн строк.
...
См. [https://github.com/toddwschneider/nyc-taxi-data](https://github.com/toddwschneider/nyc-taxi-data) и [http://tech.marksblogg.com/billion-nyc-taxi-rides-redshift.html](http://tech.marksblogg.com/billion-nyc-taxi-rides-redshift.html) с описанием набора данных и инструкциями по скачиванию.

В результате скачивания будет получено около 227 ГБ несжатых данных в файлах CSV. Скачивание занимает около часа при соединении со скоростью 1 Gbit/c (параллельное скачивание с s3.amazonaws.com позволяет задействовать как минимум половину пропускной способности канала 1 Gbit/c).
Некоторые файлы могут скачаться не полностью. Проверьте размеры файлов и заново скачайте те, которые кажутся неполными.
...
Использовалась следующая серверная конфигурация:

Два Intel(R) Xeon(R) CPU E5-2650 v2 @ 2.60GHz, всего 16 физических ядер, 128 GiB RAM, 8×6 ТБ HDD в аппаратном RAID-5

Время выполнения — лучшее из трёх запусков. Начиная со второго запуска, запросы читают данные из кэша файловой системы. Дальнейшего кэширования не происходит: при каждом запуске данные читаются и обрабатываются заново.
...
Здесь нет ничего неожиданного, так как запросы масштабируются линейно.
