---
description: 'COVID-19 Open-Data — это крупная открытая (open source) база эпидемиологических
  данных по COVID-19 и сопутствующих факторов, таких как демография, экономика и меры правительств'
sidebar_label: 'COVID-19 open-data'
slug: /getting-started/example-datasets/covid19
title: 'COVID-19 Open-Data'
keywords: ['данные COVID-19', 'эпидемиологические данные', 'набор данных по здравоохранению', 'пример набора данных', 'начало работы']
doc_type: 'guide'
---

COVID-19 Open-Data стремится собрать крупнейшую эпидемиологическую базу данных по COVID-19, а также мощный набор дополнительных ковариат. Она включает открытые, общедоступные, лицензированные данные, относящиеся к демографии, экономике, эпидемиологии, географии, здравоохранению, госпитализациям, мобильности населения, мерам правительств, погоде и многому другому.

Подробности см. на GitHub [здесь](https://github.com/GoogleCloudPlatform/covid-19-open-data).

Вставить эти данные в ClickHouse очень просто...
...
5. Это занимает совсем немного времени — давайте посмотрим, сколько строк было вставлено:
...
7. Вы заметите, что в данных много нулей по датам — либо это выходные дни, либо дни, когда показатели не сообщались каждый день. Мы можем воспользоваться оконной функцией, чтобы сгладить дневные средние значения новых случаев:
...
8. Этот запрос определяет самые последние значения для каждой локации. Мы не можем использовать `max(date)`, потому что не все страны отчитывались каждый день, поэтому мы берем последнюю строку с помощью `ROW_NUMBER`:
...
9. Мы можем использовать `lagInFrame`, чтобы вычислить `LAG` для числа новых случаев по каждому дню. В этом запросе мы отфильтровываем данные по местоположению `US_DC`:
...
10. Этот запрос вычисляет процент изменения числа новых случаев для каждого дня и добавляет в результирующий набор простой столбец с значением `increase` или `decrease`:
...
SELECT
  ...
  percent_change,
  CASE
    WHEN percent_change > 0 THEN 'рост'
...
:::note
Как указано в [репозитории GitHub](https://github.com/GoogleCloudPlatform/covid-19-open-data), набор данных перестал обновляться с 15 сентября 2022 года.
:::
