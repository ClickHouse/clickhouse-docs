...
description: 'COVID-19 Open-Data — это большая, открытая база эпидемиологических
  данных по COVID-19 и связанных факторов, таких как демография, экономика и реакция правительства'
...
COVID-19 Open-Data стремится собрать крупнейшую эпидемиологическую базу данных по Covid-19, а также мощный набор расширенных ковариат. Она включает открытые, общедоступные, лицензированные данные, относящиеся к демографии, экономике, эпидемиологии, географии, здравоохранению, госпитализациям, мобильности, реакции правительства, погоде и многому другому.

Подробнее см. в GitHub [здесь](https://github.com/GoogleCloudPlatform/covid-19-open-data).
...
1. Давайте посмотрим, как выглядят эти данные:
...
2. Теперь посмотрим на несколько строк:
...
Обратите внимание, что функция `url` позволяет с лёгкостью прочитать данные из CSV‑файла:
...
5. Это выполняется довольно быстро — давайте посмотрим, сколько строк было добавлено:
...
7. Вы заметите, что в данных много нулей по датам — либо это выходные, либо дни, когда значения не сообщались ежедневно. Мы можем использовать оконную функцию, чтобы сгладить ежедневные средние значения по числу новых случаев:
...
8. Этот запрос определяет последние значения для каждого местоположения. Мы не можем использовать `max(date)`, потому что не все страны отчитывались каждый день, поэтому получаем последнюю строку с помощью `ROW_NUMBER`:
...
9. Мы можем использовать `lagInFrame`, чтобы вычислить `LAG` для числа новых случаев по дням. В этом запросе мы фильтруем по региону `US_DC`:
...
10. Этот запрос вычисляет процент изменения числа новых случаев за каждый день и добавляет в результирующий набор простой столбец `increase` или `decrease`:
