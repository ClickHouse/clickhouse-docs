...
description: 'COVID-19 Open-Data — это большая открытая база эпидемиологических
  данных по COVID-19 и связанных факторов, таких как демография, экономика и меры реагирования правительств'
...
COVID-19 Open-Data стремится собрать крупнейшую эпидемиологическую базу данных по Covid-19, а также мощный набор дополнительных ковариатов. Она включает открытые, общедоступные, лицензированные данные, относящиеся к демографии, экономике, эпидемиологии, географии, здравоохранению, госпитализациям, мобильности, мерам реагирования правительств, погоде и многому другому.
...
2. Теперь давайте посмотрим на некоторые строки:
...
Обратите внимание, что функция `url` легко считывает данные из файла CSV:
...
4. Следующая команда вставляет весь датасет в таблицу `covid19`:
...
5. Это прошло довольно быстро — давайте посмотрим, сколько строк было вставлено:
...
6. Посмотрим, сколько всего случаев COVID-19 было зарегистрировано:
...
7. Вы заметите, что в данных много нулей по датам — либо это выходные, либо дни, когда показатели не публиковались ежедневно. Мы можем использовать оконную функцию, чтобы сгладить средние дневные значения новых случаев:
...
8. Этот запрос определяет последние значения для каждой локации. Мы не можем использовать `max(date)`, потому что не все страны отчитывались каждый день, поэтому выбираем последнюю строку с помощью `ROW_NUMBER`:
...
9. Мы можем использовать `lagInFrame`, чтобы вычислить `LAG` новых случаев по дням. В этом запросе мы фильтруем по локации `US_DC`:
...
10. Этот запрос вычисляет процент изменения числа новых случаев за каждый день и добавляет в набор результатов простой столбец с указанием `increase` или `decrease`:
...
:::note
Как указано в [репозитории GitHub](https://github.com/GoogleCloudPlatform/covid-19-open-data), набор данных больше не обновляется с 15 сентября 2022 года.
:::
