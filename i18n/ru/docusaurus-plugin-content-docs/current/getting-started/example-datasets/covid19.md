---
description: 'COVID-19 Open-Data — это большая база данных с открытыми источниками эпидемиологических данных по COVID-19 и связанным факторам, таким как демография, экономика и государственные меры.'
slug: /getting-started/example-datasets/covid19
sidebar_label: COVID-19 Open-Data
title: 'COVID-19 Open-Data'
---

COVID-19 Open-Data пытается собрать крупнейшую эпидемиологическую базу данных по Covid-19, наряду с мощным набором обширных ковариат. Она включает открытые, публично доступные лицензированные данные, касающиеся демографии, экономики, эпидемиологии, географии, здравоохранения, госпитализаций, мобильности, государственных мер, погоды и многого другого.
...
:::note
Следующие команды были выполнены на **Production** экземпляре [ClickHouse Cloud](https://clickhouse.cloud). Вы можете легко запустить их на локальной установке.
:::
...
10 строк в наборе. Время выполнения: 0.745 сек.
```
...
7. Вы заметите, что в данных много нулей по датам - либо выходные, либо дни, когда данные не были сообщены каждый день. Мы можем использовать оконную функцию, чтобы сгладить средние значения новых случаев за день:
...
8. Этот запрос определяет последние значения для каждого местоположения. Мы не можем использовать `max(date)`, так как не все страны сообщали данные каждый день, поэтому мы берем последнюю строку, используя `ROW_NUMBER`:
...
10. Этот запрос вычисляет процент изменения новых случаев каждый день и включает простую колонку `increase` или `decrease` в результирующем наборе:
