...
title: 'Открытые данные по COVID-19'
...
5. Это заняло совсем немного времени — давайте посмотрим, сколько строк было вставлено:
...
8. Этот запрос определяет последние значения для каждой локации. Мы не можем использовать `max(date)`, потому что не все страны отчитывались каждый день, поэтому извлекаем последнюю строку с помощью `ROW_NUMBER`:
...
:::note
Как указано в [репозитории на GitHub](https://github.com/GoogleCloudPlatform/covid-19-open-data), после 15 сентября 2022 года набор данных больше не обновляется.
:::
