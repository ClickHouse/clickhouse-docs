---
description: 'Набор данных, состоящий из двух таблиц с анонимными данными веб-аналитики 
  с хитами и визитами'
...
Таблицы можно загрузить в виде сжатых файлов `tsv.xz`. В дополнение к образцу, рассмотренному в этом документе, доступна расширенная (7.5 ГБ) версия таблицы `hits`, которая содержит 100 миллионов строк, в формате TSV по [ссылке](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz).

## Загрузка и прием данных {#download-and-ingest-the-data}

### Загрузка сжатого TSV файла хитовой таблицы: {#download-the-hits-compressed-tsv-file}
...
### Импорт данных с хитами: {#import-the-hits-data}
...
### Загрузка сжатого TSV файла визитов: {#download-the-visits-compressed-tsv-file}
...
[Практическое введение в разреженные первичные индексы в ClickHouse](/guides/best-practices/sparse-primary-indexes.md) использует набор данных хитов, чтобы обсудить различия в индексации ClickHouse по сравнению с традиционными реляционными базами данных, как ClickHouse строит и использует разреженный первичный индекс, и лучшие практики индексации.

Дополнительные примеры запросов к этим таблицам можно найти среди [состояний тестов](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql).
