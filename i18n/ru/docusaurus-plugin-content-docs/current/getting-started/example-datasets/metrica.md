---
description: 'Набор данных, состоящий из двух таблиц, содержащих анонимизированные данные веб-аналитики с переходами и посещениями'
sidebar_label: 'Данные веб-аналитики'
slug: /getting-started/example-datasets/metrica
title: 'Анонимизированные данные веб-аналитики'
---


# Анонимизированные данные веб-аналитики

Этот набор данных состоит из двух таблиц, содержащих анонимизированные данные веб-аналитики с переходами (`hits_v1`) и посещениями (`visits_v1`).

Таблицы можно скачать в виде сжатых файлов `tsv.xz`. В дополнение к образцу, с которым работали в этом документе, доступна расширенная версия таблицы `hits`, содержащая 100 миллионов строк, размером 7,5 ГБ в формате TSV по адресу [https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz](https://datasets.clickhouse.com/hits/tsv/hits_100m_obfuscated_v1.tsv.xz).

## Загрузка и прием данных {#download-and-ingest-the-data}

### Загрузка сжатого файла TSV для переходов: {#download-the-hits-compressed-tsv-file}
...
### Импорт данных переходов: {#import-the-hits-data}
...
### Загрузка сжатого файла TSV для посещений: {#download-the-visits-compressed-tsv-file}
...
### Создание таблицы посещений {#create-the-visits-table}
...
### Импорт данных посещений {#import-the-visits-data}
```bash
...
[Практическое введение в разреженные первичные индексы в ClickHouse](/guides/best-practices/sparse-primary-indexes.md) использует набор данных переходов для обсуждения различий в индексировании ClickHouse по сравнению с традиционными реляционными базами данных, как ClickHouse строит и использует разреженный первичный индекс, и лучшие практики индексирования.

Дополнительные примеры запросов к этим таблицам можно найти среди [состоявшихся тестов](https://github.com/ClickHouse/ClickHouse/blob/d7129855757f38ceec3e4ecc6dafacdabe9b178f/tests/queries/1_stateful/00172_parallel_join.sql).

:::note
Тестовый набор использует имя базы данных `test`, а таблицы называются `hits` и `visits`. Вы можете переименовать вашу базу данных и таблицы или отредактировать SQL из тестового файла.
:::
