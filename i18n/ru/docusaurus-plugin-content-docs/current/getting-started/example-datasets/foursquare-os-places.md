---
description: 'Набор данных с более чем 100 миллионами записей, содержащих информацию о местах на карте, таких как магазины, 
...
который предоставляет полнофункциональный движок ClickHouse. Также можно использовать
ClickHouse Cloud, `clickhouse-client` или даже `chDB`.

Выполните следующий запрос для выборки данных из S3-бакета, где хранятся данные:
...
Мы видим, что довольно много полей содержат `ᴺᵁᴸᴸ`, поэтому можно добавить дополнительные условия
в запрос, чтобы получить более пригодные данные:
...
fsq_category_labels: ['Путешествия и транспорт > Проживание > Аренда жилья на отпуск']
placemaker_url:      https://foursquare.com/placemakers/review-place/59b2c754b54618784f259654
geom:                ᴺᵁᴸᴸ
bbox:                (NULL,NULL,NULL,NULL)
```

Выполните следующий запрос, чтобы просмотреть автоматически определённую схему данных с помощью `DESCRIBE`:
...
Разберём, что происходит для каждого столбца.
...
Указание `MATERIALIZED` гарантирует, что ClickHouse вычислит значения для этих
