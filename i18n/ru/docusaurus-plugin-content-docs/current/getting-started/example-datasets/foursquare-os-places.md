---
description: 'Набор данных с более чем 100 миллионами записей, содержащий сведения о местах на карте — магазинах, ресторанах, парках, детских площадках и памятниках.'
...
дополнительные метаданные об этих местах, например категории и данные из социальных сетей.



## Исследование данных

Для исследования данных мы будем использовать [`clickhouse-local`](https://clickhouse.com/blog/extracting-converting-querying-local-files-with-sql-clickhouse-local) — небольшой инструмент командной строки,
который содержит полноценный движок ClickHouse, хотя вы также можете использовать
ClickHouse Cloud, `clickhouse-client` или даже `chDB`.

Выполните следующий запрос, чтобы получить данные из S3-бакета, в котором они хранятся:
...
fsq_category_labels: ['Путешествия и транспорт > Транспортный узел > Аэропорт > Самолёт']
...
Мы видим, что довольно много полей содержат значение `ᴺᵁᴸᴸ`, поэтому можем добавить в запрос несколько дополнительных условий, чтобы получить более полезные данные:
...
Выполните следующий запрос, чтобы просмотреть автоматически определённую схему данных с помощью оператора `DESCRIBE`:
...
для нескольких столбцов, который изменяет внутреннее представление типов данных на кодирование с помощью словаря. Операции с такими данными значительно
повышают производительность запросов `SELECT` для многих приложений.

Кроме того, создаются два материализованных столбца типа `UInt32`, `mercator_x` и `mercator_y`,
...
* `longitude + 180` сдвигает диапазон долготы с [-180, 180] до [0, 360]
* Деление на 360 нормализует это значение в диапазон от 0 до 1
* Умножение на `0xFFFFFFFF` (шестнадцатеричная запись максимального 32-битного беззнакового целого) масштабирует нормализованное значение до всего диапазона 32-битного целого числа

**mercator&#95;y**

Этот столбец преобразует значение широты в координату Y в проекции Меркатора:

* `latitude + 90` сдвигает диапазон широты с [-90, 90] до [0, 180]
* Деление на 360 и умножение на pi() преобразует значение в радианы для использования в тригонометрических функциях
* Часть `log(tan(...))` является ядром формулы проекции Меркатора
* Умножение на `0xFFFFFFFF` масштабирует значение до всего диапазона 32-битного целого числа
...
Таблица упорядочена по `mortonEncode(mercator_x, mercator_y)`, который формирует
заполняющую пространство кривую Z-порядка для `mercator_x`, `mercator_y`, чтобы значительно
улучшить производительность геопространственных запросов. Такое упорядочивание по кривой Z-порядка гарантирует, что данные
...
Как видите, в ClickHouse есть абсолютно всё, что нужно для картографических приложений в реальном времени!
...
Чтобы увидеть, что можно сделать с этим набором данных, перейдите на [adsb.exposed](https://adsb.exposed/?dataset=Places&zoom=5&lat=52.3488&lng=4.9219).
adsb.exposed изначально был создан сооснователем и техническим директором Алексеем Миловидовым для визуализации полётных данных ADS-B (Automatic Dependent Surveillance-Broadcast), 
которые в тысячу раз больше по объёму. Во время корпоративного хакатона Алексей добавил в инструмент данные Foursquare.

Ниже приведены некоторые из наших любимых визуализаций, которые мы отобрали для вас.
