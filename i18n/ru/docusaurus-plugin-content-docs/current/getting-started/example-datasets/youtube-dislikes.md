---
description: 'Подборка дизлайков к видео на YouTube.'
...
В ноябре 2021 года YouTube убрал публичный счетчик ***дизлайков*** у всех своих видео. Хотя авторы по-прежнему видят количество дизлайков, зрители могут видеть только, сколько ***лайков*** набрало видео.

:::important
Набор данных содержит более 4,55 миллиарда записей, поэтому не стоит просто копировать и выполнять приведённые ниже команды, не убедившись, что ваши ресурсы способны обработать такой объём. Эти команды выполнялись на **Production**-инстансе [ClickHouse Cloud](https://clickhouse.cloud).
:::

Данные представлены в формате JSON и могут быть загружены с [archive.org](https://archive.org/download/dislikes_youtube_2021_12_video_json_files). Мы также разместили те же данные в S3, чтобы их можно было эффективнее загружать в инстанс ClickHouse Cloud.

Ниже приведены шаги по созданию таблицы в ClickHouse Cloud и вставке данных.

:::note
Эти шаги также без проблем подойдут для локальной установки ClickHouse. Единственное изменение — использовать функцию `s3` вместо `s3cluster` (если только у вас не настроен кластер — в этом случае замените `default` на имя вашего кластера).
:::
...
Следующая команда выполняет потоковую загрузку записей из файлов S3 в таблицу `youtube`.


:::important
Эта операция вставляет большой объем данных — 4,65 миллиарда строк. Если вам не нужен весь набор данных, просто добавьте условие `LIMIT` с нужным количеством строк.
:::
...
Несколько комментариев о команде `INSERT`:

- Функция `parseDateTimeBestEffortUSOrZero` удобна, когда входящие поля дат могут иметь неправильный формат. Если `fetch_date` не удастся корректно разобрать, значение будет установлено в `0`
...
После вставки данных вы можете подсчитать количество дизлайков ваших любимых видео или каналов. Давайте посмотрим, сколько видео было загружено каналом ClickHouse:
...
1 строка в наборе. Затрачено: 0.570 сек. Обработано 237,57 тыс. строк, 5,77 МБ (416,54 тыс. строк/сек., 10,12 МБ/сек.)
```

:::note
Запрос выше выполняется так быстро, потому что мы выбрали `uploader` в качестве первого столбца первичного ключа — поэтому пришлось обработать только 237 тыс. строк.
:::
...
84 строки в наборе. Затрачено: 0.013 сек. Обработано 155,65 тыс. строк, 16,94 МБ (11,96 млн. строк/сек., 1,30 ГБ/сек.)
```

Вот пример поиска видео с **ClickHouse** в полях `title` или `description`:
...
Этот запрос должен обработать каждую строку и проанализировать два столбца строк. Тем не менее производительность остается высокой — 4,15 млн строк в секунду:

```response
1174 строки в наборе. Затрачено: 1099,368 сек. Обработано 4,56 млрд строк, 1,98 ТБ (4,15 млн строк/с., 1,80 ГБ/с.)
```
...
### Снижает ли отключение комментариев вероятность того, что пользователи будут ставить лайки или дизлайки? {#if-someone-disables-comments-does-it-lower-the-chance-someone-will-actually-click-like-or-dislike}

Когда комментирование отключено, чаще ли люди используют лайки или дизлайки для выражения своего отношения к видео?
...
С развитием технологий распознавания речи создание субтитров для видео стало проще, чем когда-либо. YouTube добавил функцию автоматических субтитров в конце 2009 года — произошел ли скачок именно тогда?
...
### Топ загрузчиков с течением времени {#top-uploaders-over-time}
...
### Как меняется соотношение лайков с ростом просмотров? {#how-do-like-ratio-changes-as-views-go-up}
...
### Как распределяются просмотры? {#how-are-views-distributed}


```sql
SELECT
    labels AS процентиль,
    round(quantiles) AS просмотры
FROM
(
    SELECT
        quantiles(0.999, 0.99, 0.95, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1)(view_count) AS quantiles,
        ['99,9-й', '99-й', '95-й', '90-й', '80-й', '70-й', '60-й', '50-й', '40-й', '30-й', '20-й', '10-й'] AS labels
    FROM youtube
)
ARRAY JOIN
    quantiles,
