---
description: 'Сборище дизлайков YouTube видео.'
...
:::important
В наборе данных более 4.55 миллиардов записей, поэтому будьте осторожны, просто копируя и вставляя команды ниже, если ваши ресурсы не могут справиться с таким объемом. Команды ниже были выполнены на **Production** экземпляре [ClickHouse Cloud](https://clickhouse.cloud).
:::

Данные находятся в формате JSON и могут быть загружены с [archive.org](https://archive.org/download/dislikes_youtube_2021_12_video_json_files). Мы предоставили эти же данные в S3, чтобы их можно было более эффективно загрузить в экземпляр ClickHouse Cloud.
...
2. На основе извлеченной схемы, мы очистили типы данных и добавили первичный ключ. Определите следующую таблицу:
...
3. Следующая команда передает записи из S3 файлов в таблицу `youtube`.
...
4. Откройте новую вкладку в SQL Console ClickHouse Cloud (или новое окно `clickhouse-client`) и следите, как увеличивается счетчик. Вставка 4.56B строк займет некоторое время в зависимости от ресурсов вашего сервера. (Без каких-либо изменений в настройках это занимает около 4.5 часов.)
...
5. После вставки данных, вперед и посчитайте количество дизлайков ваших любимых видео или каналов. Давайте посмотрим, сколько видео было загружено ClickHouse:
...
7. Вот запрос на видео с **ClickHouse** в полях `title` или `description`:
...
### Если кто-то отключает комментарии, снижает ли это вероятность того, что кто-то нажмет "лайк" или "дизлайк"? {#if-someone-disables-comments-does-it-lower-the-chance-someone-will-actually-click-like-or-dislike}
...
Всплеск числа загрузчиков [вокруг COVID хорошо заметен](https://www.theverge.com/2020/3/27/21197642/youtube-with-me-style-videos-views-coronavirus-cook-workout-study-home-beauty).


### Больше субтитров с течением времени и когда {#more-subtitles-over-time-and-when}
...
Результаты данных показывают всплеск в 2009 году. По всей видимости, в это время YouTube удаляла свою функцию общественных субтитров, которая позволяла загружать субтитры для видео других людей.
Это привело к очень успешной кампании по добавлению субтитров в видео для людей с нарушениями слуха и глухих зрителей.


### Лучшие загрузчики с течением времени {#top-uploaders-over-time}
...
### Как меняется соотношение лайков по мере увеличения просмотров? {#how-do-like-ratio-changes-as-views-go-up}
