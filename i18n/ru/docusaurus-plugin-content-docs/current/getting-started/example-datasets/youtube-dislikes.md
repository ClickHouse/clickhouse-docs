...
В ноябре 2021 года YouTube убрал публичный счетчик ***dislike*** со всех своих видео. Хотя авторы по-прежнему видят количество дизлайков, зрителям теперь доступна только информация о том, сколько ***like*** набрало видео.

:::important
Набор данных содержит более 4,55&nbsp;миллиарда записей, поэтому не копируйте и не запускайте приведённые ниже команды без учёта того, выдержат ли ваши ресурсы такой объём. Эти команды были выполнены на **Production**-инстансе [ClickHouse Cloud](https://clickhouse.cloud).
:::

Данные представлены в формате JSON и могут быть скачаны с [archive.org](https://archive.org/download/dislikes_youtube_2021_12_video_json_files). Мы также разместили эти же данные в S3, чтобы их можно было эффективнее загружать в инстанс ClickHouse Cloud.

Ниже приведены шаги по созданию таблицы в ClickHouse Cloud и загрузке данных.

:::note
Приведённые ниже шаги также без проблем подойдут для локальной установки ClickHouse. Единственное изменение — использовать функцию `s3` вместо `s3cluster` (если у вас не настроен кластер; в этом случае замените `default` на имя вашего кластера).
:::
...
Посмотрим, как выглядят данные. Табличная функция `s3cluster` возвращает таблицу, поэтому мы можем применить `DESCRIBE` к результату:
...
- Функция `parseDateTimeBestEffortUSOrZero` полезна, когда входящие поля дат могут быть в неправильном формате. Если `fetch_date` не удастся корректно разобрать, значение будет установлено в `0`
...
:::note
Этот запрос выполняется так быстро, потому что мы выбрали `uploader` в качестве первого столбца первичного ключа — поэтому пришлось обработать только 237 тысяч строк.
:::
...
Этот запрос должен обработать каждую строку и проанализировать два столбца строк. Тем не менее производительность остается хорошей — 4,15 млн строк в секунду:
...
### Как меняется количество видео с течением времени — значимые события? {#how-does-the-number-of-videos-change-over-time---notable-events}
...
Заметен всплеск количества авторов [в период пандемии COVID](https://www.theverge.com/2020/3/27/21197642/youtube-with-me-style-videos-views-coronavirus-cook-workout-study-home-beauty).

### Увеличение количества субтитров с течением времени {#more-subtitles-over-time-and-when}

С развитием технологий распознавания речи создание субтитров для видео стало проще, чем когда-либо. YouTube добавил функцию автоматических субтитров в конце 2009 года — произошел ли скачок именно в этот момент?
...
Данные показывают всплеск в 2009 году. Очевидно, в то время YouTube удалял функцию субтитров сообщества, которая позволяла загружать субтитры к видео других пользователей.
