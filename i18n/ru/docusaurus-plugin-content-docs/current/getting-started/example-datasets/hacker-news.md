...
> из файлов в форматах CSV и Parquet, а затем выполните несколько простых запросов, чтобы изучить эти данные.
...
<VerticalStepper headerLevel="h3">
  ...
  Для загрузки данных можно снова воспользоваться выводом схемы, предоставив ClickHouse возможность самостоятельно определить типы столбцов.

  Выполните следующую команду, чтобы создать таблицу и вставить данные напрямую из удалённого CSV-файла, обращаясь к его содержимому через функцию [`url`](/sql-reference/table-functions/url).
  ...
  Это создаёт пустую таблицу, используя схему, определённую из данных.
  ...
  В ClickHouse первичный ключ id для данных определяется с помощью конструкции `ORDER BY`.
