...
> из файлов в форматах CSV и Parquet и выполните несколько простых запросов для изучения данных.
...
В этой команде заложено множество возможностей.
...
Просмотрите записи Hacker News и отдельные столбцы, выполнив следующий запрос:
...
Помимо объявления поля time как типа `DateTime`, мы определяем подходящий тип для каждого из полей ниже после удаления существующего набора данных.
В ClickHouse первичный ключ id для данных определяется через предложение `ORDER BY`.

Выбор подходящих типов и определение того, какие столбцы включить в предложение `ORDER BY`, поможет улучшить скорость выполнения запросов и сжатие данных.
...
Поле score содержит метрику популярности публикаций, а поле `id` и оператор конкатенации `||` можно использовать для формирования ссылки на исходную публикацию.
...
Растет ли количество упоминаний ClickHouse со временем? Здесь показана полезность определения поля `time` как `DateTime`, поскольку использование правильного типа данных позволяет применять функцию `toYYYYMM()`:
...
#### Кто является самыми активными комментаторами статей о ClickHouse? {#top-commenters}
...
Одна из сильных сторон ClickHouse — это способность работать с любым количеством [форматов](/interfaces/formats).
CSV представляет собой довольно удобный вариант использования, но не является наиболее эффективным для обмена данными.

Далее вы загрузите данные из файла Parquet, который является эффективным колоночно-ориентированным форматом.

Parquet имеет минимальный набор типов, которые ClickHouse должен учитывать, и эта информация о типах закодирована в самом формате.
Автоматическое определение типов для файла Parquet неизбежно приведет к несколько иной схеме, чем для CSV-файла.
...
Выполните следующую команду для просмотра автоматически определенной схемы:
...
Как и ранее с CSV-файлом, вы можете указать схему вручную для большего контроля над выбранными типами и вставить
данные напрямую из s3:
...
Выполните запрос снова после создания индекса:
...
ускорив тем самым выполнение запроса.

Теперь также возможен эффективный поиск одного или нескольких терминов одновременно:
