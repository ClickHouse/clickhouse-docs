...
Этот датасет содержит все записи таблиц `Posts`, `Users`, `Votes`, `Comments`, `Badges`, `PostHistory` и `PostLinks`, когда-либо появлявшиеся на Stack Overflow.

Вы можете либо скачать предварительно подготовленные версии данных в формате Parquet, содержащие все сообщения до апреля 2024 года, либо скачать последние данные в формате XML и загрузить их. Stack Overflow периодически публикует обновления этих данных — по историческим данным, раз в три месяца.

На следующей диаграмме показана схема доступных таблиц при условии использования формата Parquet.
...
Мы предоставляем копию этого набора данных в формате Parquet, актуальную по состоянию на апрель 2024 года. Хотя по количеству строк (60 миллионов постов) этот набор данных невелик для ClickHouse, он содержит значительный объём текстовых данных и крупные столбцы типа String.

```sql
CREATE DATABASE stackoverflow
```

Приведённые ниже значения времени относятся к кластеру ClickHouse Cloud с 96 GiB и 24 vCPU, расположенному в `eu-west-2`. Набор данных находится в `eu-west-3`.
...
Посты также доступны по отдельным годам, например: [https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2020.parquet](https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2020.parquet)
...
Размер этих файлов может достигать 35 ГБ, и их скачивание может занять около 30 минут в зависимости от интернет-соединения — сервер загрузки ограничивает скорость примерно до 20 МБ/с.
...
Для преобразования XML в JSON мы рекомендуем утилиту для Linux [`xq`](https://github.com/kislyuk/yq) — простую обёртку `jq` для XML‑документов.
...
Следующие шаги применимы к любому из указанных выше файлов. В качестве примера мы используем файл `stackoverflow.com-Posts.7z`. При необходимости измените команды.

Извлеките архив с помощью [p7zip](https://p7zip.sourceforge.net/). В результате будет создан один XML‑файл — в данном случае `Posts.xml`.
...
Следующий запрос разбивает XML‑файл на несколько файлов, в каждом из которых по 10 000 строк.
...
После выполнения приведённой выше команды у вас будет набор файлов по 10 000 строк каждый. Это гарантирует, что объём потребляемой памяти следующей командой не будет чрезмерным (преобразование XML в JSON выполняется в памяти).
...
Загрузите данные в ClickHouse следующей командой. Обратите внимание, что схема указана для файла `posts.json`. Её потребуется скорректировать в зависимости от типа данных, чтобы она соответствовала целевой таблице.
...
### Наиболее спорные публикации {#most-controversial-posts}
...
Мы благодарим Stack Overflow за предоставление этих данных по лицензии `cc-by-sa 4.0` и признаём их вклад, а также исходный источник данных: [https://archive.org/details/stackexchange](https://archive.org/details/stackexchange).