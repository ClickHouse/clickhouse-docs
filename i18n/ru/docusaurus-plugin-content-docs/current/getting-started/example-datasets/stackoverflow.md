...
Пользователи могут либо скачать заранее подготовленные версии данных в формате Parquet, содержащие все публикации до апреля 2024 года, либо скачать самые свежие данные в формате XML и загрузить их. Stack Overflow периодически публикует обновления этих данных — исторически примерно раз в 3 месяца.

Следующая диаграмма показывает схему доступных таблиц для формата Parquet.
...
### Ссылки на публикации {#postlinks}
...
На момент написания ClickHouse не имеет встроенной поддержки XML в качестве входного формата. Для загрузки данных в ClickHouse их необходимо сначала преобразовать в NDJSON.

Для преобразования XML в JSON рекомендуется использовать инструмент [`xq`](https://github.com/kislyuk/yq) для Linux — простую обёртку `jq` для работы с XML-документами.
...
Следующие шаги применимы к любому из перечисленных выше файлов. В качестве примера используется файл `stackoverflow.com-Posts.7z`. При необходимости внесите соответствующие изменения.
...
Следующая команда разделяет XML-файл на несколько файлов, каждый из которых содержит 10000 строк.
...
После выполнения описанной выше команды у пользователя будет набор файлов, каждый из которых содержит по 10 000 строк. Это гарантирует, что накладные расходы по памяти для следующей команды не будут чрезмерными (преобразование XML в JSON выполняется в памяти).
...
Загрузите данные в ClickHouse следующей командой. Обратите внимание, что схема задана для файла `posts.json`. Ее необходимо скорректировать с учетом типов данных, чтобы она соответствовала целевой таблице.
...
### Публикации, связанные с ClickHouse, с наибольшим количеством просмотров {#clickhouse-related-posts-with-the-most-views}
