---
description: 'Новый аналитический бенчмарк для машинно-генерируемых логов'
...
`MgBench` — это новый аналитический бенчмарк для машинно-генерируемых логов, разработанный [Эндрю Кроти](http://cs.brown.edu/people/acrotty/).
...
```sql
-- Q1.2: Какие машины компьютерных лабораторий были недоступны за последние сутки?
...
```sql
-- Q1.4: Как часто за месяц каждый сервер блокировался на дисковом вводе-выводе?
...
```sql
-- Q1.5: На каких доступных извне виртуальных машинах заканчивалась память?
...
```sql
-- Q1.6: Каков общий почасовой сетевой трафик по всем файловым серверам?
...
```sql
-- Q3.1: Опускалась ли температура в помещении до точки замерзания в выходные?
...
Запрос 3.5 ниже использует UNION. Задайте режим объединения результатов запросов SELECT. Этот параметр используется только при использовании с UNION без явного указания UNION ALL или UNION DISTINCT.
...
```sql
-- Q3.5: Где в здании происходят значительные колебания температуры зимой и летом?
