...
Этот набор данных содержит метеорологические измерения за последние 120 лет. Каждая строка — это измерение для конкретного момента времени и станции.

Более точно, согласно [источнику этих данных](https://github.com/awslabs/open-data-docs/tree/main/docs/noaa/noaa-ghcn):

> GHCN-Daily — это набор данных, содержащий ежедневные наблюдения на суше по всему миру. Он включает измерения с наземных станций со всего мира, около двух третей из которых предназначены только для измерения осадков (Menne et al., 2012). GHCN-Daily представляет собой совокупность климатических записей из многочисленных источников, которые были объединены и прошли общий набор проверок качества (Durre et al., 2010). Архив включает следующие метеорологические элементы:

    - Максимальная суточная температура
    - Минимальная суточная температура
    - Температура на момент наблюдения
    - Осадки (дождь, талый снег)
    - Снегопад
    - Глубина снежного покрова
    - Другие элементы при наличии

Разделы ниже содержат краткий обзор шагов, которые потребовались для загрузки этого набора данных в ClickHouse. Если вы хотите подробнее узнать о каждом шаге, рекомендуем ознакомиться с нашей статьей в блоге ["Exploring massive, real-world data sets: 100+ Years of Weather Records in ClickHouse"](https://clickhouse.com/blog/real-world-data-noaa-climate-data).
...
- [Загрузите исходные данные](#original-data) и преобразуйте их в формат, требуемый ClickHouse. Пользователи, желающие добавить собственные столбцы, могут воспользоваться этим подходом.
...
Краткое описание [документации по формату](https://github.com/awslabs/open-data-docs/tree/main/docs/noaa/noaa-ghcn):

Краткое описание формата документации и столбцов в порядке их следования:
...
Структура с одним измерением на строку приведет к разреженной таблице в ClickHouse. Следует преобразовать данные так, чтобы каждая строка соответствовала времени и станции, а измерения стали колонками. Сначала мы ограничиваем набор данных строками без проблем, т.е. где `qFlag` равен пустой строке.
...
Хотя структура с одним измерением на строку может использоваться в ClickHouse, она излишне усложнит будущие запросы. В идеале нам нужна одна строка на идентификатор станции и дату, где каждый тип измерения и связанное с ним значение представлены отдельной колонкой, т.е.
...
(
   ...
   `percentDailySun` UInt8 COMMENT 'Дневной процент возможной солнечной активности (проценты)',
   `averageWindSpeed` UInt32 COMMENT 'Средняя суточная скорость ветра (десятые доли метра в секунду)',
...
Получено 5 строк. Прошло: 0.750 сек. Обработано 689.10 млн строк, 3.20 ГБ (918.20 млн строк/сек., 4.26 ГБ/сек.)
