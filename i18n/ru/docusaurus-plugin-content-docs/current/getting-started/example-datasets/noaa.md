...
Этот набор данных содержит измерения погоды за последние 120 лет. Каждая строка соответствует измерению в определённый момент времени на конкретной станции.

Точнее, в соответствии с [источником этих данных](https://github.com/awslabs/open-data-docs/tree/main/docs/noaa/noaa-ghcn):

> GHCN-Daily — это набор данных, содержащий ежедневные наблюдения над сушей по всему миру. Он включает основанные на станциях измерения с наземных станций по всему миру, около двух третей из которых предназначены только для измерения количества осадков (Menne et al., 2012). GHCN-Daily — это составной набор климатических записей из многочисленных источников, которые были объединены и подвергнуты единому набору процедур контроля качества (Durre et al., 2010). Архив включает следующие метеорологические элементы:

    - Ежедневная максимальная температура
    - Ежедневная минимальная температура
    - Температура на момент наблюдения
    - Осадки (то есть дождь, растаявший снег)
    - Количество выпавшего снега
    - Высота снежного покрова
    - Другие элементы, при наличии

В разделах ниже приведён краткий обзор шагов, которые были выполнены для загрузки этого набора данных в ClickHouse. Если вы хотите более подробно прочитать о каждом шаге, рекомендуем обратить внимание на наш пост в блоге под названием ["Исследование масштабных реальных наборов данных: более 100 лет метеорологических записей в ClickHouse"](https://clickhouse.com/blog/real-world-data-noaa-climate-data).
...
В частности, были удалены строки, которые не прошли проверки контроля качества NOAA. Данные также были реструктурированы из формата с одним измерением на строку в формат с одной строкой на идентификатор станции и дату, т.е.
...
Кратко резюмируя [документацию по формату](https://github.com/awslabs/open-data-docs/tree/main/docs/noaa/noaa-ghcn):

Кратко резюмируя документацию по формату и перечисляя столбцы по порядку:
...
- ELEMENT = 4-символьный индикатор типа элемента. По сути, это тип измерения. Хотя доступно множество измерений, мы выбираем следующие:
  ...
  - PSUN - Ежедневный процент возможной продолжительности солнечного сияния (проценты)
  - AWND - Средняя суточная скорость ветра (десятые доли метров в секунду)
...
- Q-FLAG - флаг качества измерения с 14 возможными значениями. Нас интересуют только данные с пустым значением, т.е. прошедшие все проверки контроля качества.
...
Структура с одним измерением на строку приведет к разреженной таблице в ClickHouse. Следует преобразовать данные так, чтобы каждая строка соответствовала времени и станции, а измерения стали столбцами. Сначала мы ограничиваем набор данных строками без проблем, т.е. где `qFlag` равен пустой строке.
...
При более чем 2,6 миллиардах строк это не быстрый запрос, поскольку он включает разбор всех файлов. На нашей 8-ядерной машине это занимает около 160 секунд.
...
Используя ClickHouse local и простой `GROUP BY`, мы можем преобразовать наши данные в эту структуру. Чтобы ограничить использование памяти, мы обрабатываем по одному файлу за раз.
...
(
   ...
   `maxWindSpeed` UInt32 COMMENT 'Максимальная скорость порыва ветра (десятые доли метров в секунду)',
   `weatherType` Enum8('Normal' = 0, 'Fog' = 1, 'Heavy Fog' = 2, 'Thunder' = 3, 'Small Hail' = 4, 'Hail' = 5, 'Glaze' = 6, 'Dust/Ash' = 7, 'Smoke/Haze' = 8, 'Blowing/Drifting Snow' = 9, 'Tornado' = 10, 'High Winds' = 11, 'Blowing Spray' = 12, 'Mist' = 13, 'Drizzle' = 14, 'Freezing Drizzle' = 15, 'Rain' = 16, 'Freezing Rain' = 17, 'Snow' = 18, 'Unknown Precipitation' = 19, 'Ground Fog' = 21, 'Freezing Fog' = 22) COMMENT 'Тип погоды',
   `location` Point,
   `elevation` Float32,
...
Информацию о том, как ускорить эту загрузку, см. [здесь](https://clickhouse.com/blog/real-world-data-noaa-climate-data#load-the-data).
...
Информацию о том, как ускорить этот процесс, см. в нашей статье в блоге о [настройке загрузки больших объемов данных](https://clickhouse.com/blog/supercharge-your-clickhouse-data-loads-part2).
...
Используя [список горнолыжных курортов](https://gist.githubusercontent.com/gingerwizard/dd022f754fd128fdaf270e58fa052e35/raw/622e03c37460f17ef72907afe554cb1c07f91f23/ski_resort_stats.csv) в Соединенных Штатах и их координаты, мы объединяем их с топ-1000 метеостанций с наибольшим количеством снега за любой месяц за последние 5 лет. Сортируя результаты объединения по [geoDistance](/sql-reference/functions/geo/coordinates/#geodistance) и ограничивая выборку теми записями, где расстояние меньше 20 км, мы выбираем лучший результат для каждого курорта и сортируем по общему количеству снега. Обратите внимание, что мы также ограничиваем выборку курортами, расположенными выше 1800 м, что является общим индикатором хороших условий для катания на лыжах.
...
Мы выражаем признательность Global Historical Climatology Network за подготовку, очистку и распространение этих данных. Благодарим вас за проделанную работу.
