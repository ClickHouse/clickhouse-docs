...
>
> Ищете быстрый старт для Cloud? [Перейдите сюда](/get-started/quick-start).
...
<VerticalStepper>
  ...
  Если вам необходимо установить экземпляр ClickHouse для промышленной эксплуатации, обратитесь к [странице установки](/install).
  :::
  ...
  того, что мы называем **партом** (part) в ClickHouse. Эти части впоследствии
  ...
  <Tabs groupId="read_data">
    ...
    <TabItem value="local_file" label="Локальный">
      ...
      читать локальный файл. Для простоты скопируйте файл в каталог `user_files`
      (он находится в каталоге, куда вы загрузили бинарный файл ClickHouse).
    ...
    <TabItem value="очередь сообщений" label="Очереди сообщений">
      Очереди сообщений могут передавать данные в ClickHouse с помощью соответствующего табличного движка, в том числе:

      * **Kafka**: интегрируйте с Kafka с помощью [табличного движка `Kafka`](/engines/table-engines/integrations/kafka)
      * **Amazon MSK**: интегрируйте с [Amazon Managed Streaming for Apache Kafka (MSK)](/integrations/kafka/cloud/amazon-msk/)
      * **RabbitMQ**: интегрируйте с RabbitMQ с помощью [табличного движка `RabbitMQ`](/engines/table-engines/integrations/rabbitmq)
