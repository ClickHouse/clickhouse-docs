...
## Как использовать этот образ Docker \{#how-to-use-image\}
...
изнутри контейнера на порт хоста:
...
Или можно разрешить контейнеру использовать [порты хоста напрямую](https://docs.docker.com/network/host/) с помощью `--network=host`
...
Они необязательны и могут быть включены с помощью следующих [параметров командной строки Docker](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities):
