---
description: 'Песочница ClickHouse позволяет пользователям экспериментировать с ClickHouse,
  мгновенно выполняя запросы без необходимости настраивать собственный сервер или кластер.'
...
[ClickHouse Playground](https://sql.clickhouse.com) позволяет экспериментировать с ClickHouse, выполняя запросы мгновенно, без необходимости настраивать сервер или кластер.
В Playground доступно несколько примерных наборов данных.

Вы можете отправлять запросы в Playground с помощью любого HTTP‑клиента, например [curl](https://curl.haxx.se) или [wget](https://www.gnu.org/software/wget/), или настроить подключение с использованием драйверов [JDBC](../interfaces/jdbc.md) или [ODBC](../interfaces/odbc.md). Дополнительная информация о программных продуктах, поддерживающих ClickHouse, доступна [здесь](../integrations/index.mdx).
...
- 256 ГиБ дискового пространства и 59 виртуальных процессоров на каждую реплику.
