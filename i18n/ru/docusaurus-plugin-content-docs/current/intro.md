...
description: 'ClickHouse® — это колонко-ориентированная система управления базами данных (СУБД) SQL для онлайн-аналитической обработки (OLAP). Она доступна как в виде программного обеспечения с открытым исходным кодом, так и как облачный сервис.'
...
ClickHouse® — это высокопроизводительная колоночная система управления базами данных (СУБД) SQL для онлайн-аналитической обработки (OLAP). Она доступна как в виде [программного обеспечения с открытым исходным кодом](https://github.com/ClickHouse/ClickHouse), так и в виде [облачного сервиса](https://clickhouse.com/cloud).


## Что такое аналитика? {#what-are-analytics}

Аналитика, также известная как OLAP (Online Analytical Processing, оперативная аналитическая обработка), — это SQL-запросы со сложными вычислениями (например, агрегацией, обработкой строк, арифметическими операциями) над большими объёмами данных.
...
## Строковое и колоночное хранение данных {#row-oriented-vs-column-oriented-storage}
...
В строковой базе данных последовательные строки таблицы хранятся одна за другой. Такая структура позволяет быстро извлекать строки, поскольку значения всех столбцов каждой строки хранятся вместе.

ClickHouse — это колоночная база данных. В таких системах таблицы хранятся как набор столбцов, то есть значения каждого столбца хранятся последовательно одно за другим. Такая структура усложняет восстановление отдельных строк (поскольку между значениями строк появляются промежутки), но операции со столбцами, такие как фильтрация или агрегация, выполняются гораздо быстрее, чем в строковой базе данных.
...
<Image
  img={column_example}
  alt='Пример запроса в колоночной базе данных'
  size='lg'
/>
...
В строковой базе данных, даже если приведенный выше запрос обрабатывает только несколько из существующих столбцов, системе все равно необходимо загрузить данные из всех остальных столбцов с диска в память. Причина в том, что данные хранятся на диске фрагментами, называемыми [блоками](<https://en.wikipedia.org/wiki/Block_(data_storage)>) (обычно фиксированного размера, например 4 КБ или 8 КБ). Блоки являются наименьшими единицами данных, считываемых с диска в память. Когда приложение или база данных запрашивает данные, подсистема дискового ввода-вывода операционной системы считывает необходимые блоки с диска. Даже если требуется только часть блока, весь блок считывается в память (это обусловлено архитектурой диска и файловой системы):
...
Поскольку блочное хранение и передача данных с диска в память согласованы с характерным для аналитических запросов шаблоном доступа к данным, с диска считываются только те столбцы, которые требуются для запроса, что позволяет избежать лишних операций ввода-вывода для неиспользуемых данных. Это [значительно быстрее](https://benchmark.clickhouse.com/) по сравнению со строковым хранением, при котором считываются целые строки (включая неиспользуемые столбцы):
...
ClickHouse использует асинхронную схему multi-master репликации для обеспечения избыточного хранения данных на нескольких узлах. После записи на любую доступную реплику все остальные реплики получают свою копию в фоновом режиме. Система поддерживает идентичность данных на разных репликах. Восстановление после большинства сбоев выполняется автоматически или полуавтоматически в сложных случаях.
...
ClickHouse широко известен исключительно высокой скоростью выполнения запросов.
...
В остальной части этой статьи объясняется, почему колоночные базы данных хорошо работают для таких сценариев и почему ClickHouse в частности [превосходит](/concepts/why-clickhouse-is-so-fast/concepts/why-clickhouse-is-so-fast#storage-layer-concurrent-inserts-and-selects-are-isolated) другие решения в этой категории.
...
1.  Для аналитического запроса необходимо прочитать лишь небольшое количество столбцов таблицы. В колоночной базе данных можно читать только те данные, которые действительно нужны. Например, если требуется 5 столбцов из 100, можно ожидать 20-кратного сокращения объёма операций ввода-вывода.
...
Поскольку выполнение запроса требует обработки большого количества строк, целесообразно выполнять все операции для целых векторов, а не для отдельных строк, или реализовать движок запросов таким образом, чтобы затраты на диспетчеризацию были практически нулевыми. Если этого не сделать, то при любой сколько-нибудь приличной дисковой подсистеме интерпретатор запросов неизбежно будет простаивать в ожидании процессора. Имеет смысл как хранить данные в столбцах, так и обрабатывать их, когда это возможно, по столбцам.

Существует два способа это реализовать:

1.  Векторный движок. Все операции реализованы для векторов, а не для отдельных значений. Это означает, что операции не нужно вызывать слишком часто, и затраты на диспетчеризацию незначительны. Код операции содержит оптимизированный внутренний цикл.
...
Обратите внимание, что для эффективной работы процессора язык запросов должен быть декларативным (SQL или MDX) или хотя бы векторным (J, K). Запрос должен содержать только неявные циклы, что позволяет проводить оптимизацию.
-->
