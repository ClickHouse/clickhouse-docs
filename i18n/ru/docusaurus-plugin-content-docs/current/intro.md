...
description: 'ClickHouse® — это колоночная система управления базами данных (СУБД) с поддержкой SQL для оперативной аналитической обработки данных (OLAP). Она доступна как в виде программного обеспечения с открытым исходным кодом, так и в виде облачного сервиса.'
title: 'Что такое ClickHouse?'
keywords: ['ClickHouse', 'колоночная база данных', 'OLAP-база данных', 'аналитическая база данных', 'высокопроизводительная база данных']
doc_type: 'guide'
---
...
ClickHouse® — это высокопроизводительная столбцовая SQL-СУБД для онлайн-аналитической обработки (OLAP). Она доступна как в виде [программного обеспечения с открытым исходным кодом](https://github.com/ClickHouse/ClickHouse), так и в виде [облачного сервиса](https://clickhouse.com/cloud).
...
В отличие от транзакционных запросов (или OLTP, Online Transaction Processing, оперативная обработка транзакций), которые читают и записывают лишь несколько строк на запрос и поэтому выполняются за миллисекунды, аналитические запросы обрабатывают миллиарды и триллионы строк.
...
В построчной базе данных последовательные строки таблицы хранятся одна за другой. Такая структура позволяет быстро извлекать строки, так как значения столбцов каждой строки хранятся вместе.

ClickHouse — это поколоночная база данных. В таких системах таблицы хранятся как набор столбцов, то есть значения каждого столбца хранятся последовательно одно за другим. Такая структура усложняет восстановление отдельных строк (так как теперь между значениями строк есть промежутки), но операции со столбцами, такие как фильтрация или агрегация, выполняются гораздо быстрее, чем в построчной базе данных.

Разницу лучше всего объяснить на примере запроса, выполняемого над 100 миллионами строк [реальных анонимизированных данных веб-аналитики](/getting-started/example-datasets/metrica):
...
Как видно из раздела статистики на диаграмме выше, запрос обработал 100 миллионов строк за 92 миллисекунды — это пропускная способность более 1 миллиарда строк в секунду или чуть менее 7 ГБ передаваемых данных в секунду.

**Построчная СУБД**

В построчной базе данных, даже если приведенный выше запрос обрабатывает только несколько из существующих столбцов, системе все равно необходимо загрузить данные из других существующих столбцов с диска в память. Причина этого заключается в том, что данные хранятся на диске фрагментами, называемыми [блоками](<https://en.wikipedia.org/wiki/Block_(data_storage)>) (обычно фиксированного размера, например 4 КБ или 8 КБ). Блоки являются наименьшими единицами данных, считываемых с диска в память. Когда приложение или база данных запрашивает данные, подсистема дискового ввода-вывода операционной системы считывает необходимые блоки с диска. Даже если требуется только часть блока, весь блок считывается в память (это обусловлено архитектурой диска и файловой системы):
...
Поскольку блочное хранение и передача данных с диска в память согласованы с характером доступа к данным аналитических запросов, с диска считываются только те столбцы, которые необходимы для запроса, что позволяет избежать лишних операций ввода-вывода для неиспользуемых данных. Это [намного быстрее](https://benchmark.clickhouse.com/) по сравнению со строчно-ориентированным хранением, при котором читаются целые строки (включая столбцы, не относящиеся к запросу):
...
ClickHouse использует асинхронную схему репликации с несколькими мастерами, чтобы обеспечить избыточное хранение данных на нескольких узлах. После записи на любую доступную реплику все остальные реплики получают свою копию в фоновом режиме. Система поддерживает идентичность данных на разных репликах. Восстановление после большинства сбоев выполняется автоматически, а в сложных случаях — полуавтоматически.


## Управление доступом на основе ролей {#role-based-access-control}

ClickHouse реализует управление учетными записями пользователей с помощью SQL-запросов и поддерживает настройку управления доступом на основе ролей, аналогичную стандарту ANSI SQL и популярным системам управления реляционными базами данных.
...
ClickHouse позволяет жертвовать точностью ради производительности. Например, некоторые агрегатные функции вычисляют количество уникальных значений, медиану и квантили приближённо. Также запросы могут выполняться на выборке данных для быстрого получения приближённого результата. Наконец, агрегации могут выполняться с ограниченным количеством ключей вместо обработки всех ключей. В зависимости от степени неравномерности распределения ключей это может обеспечить достаточно точный результат при использовании значительно меньшего объёма ресурсов по сравнению с точным вычислением.


## Адаптивные алгоритмы соединения {#adaptive-join-algorithms}

ClickHouse адаптивно выбирает алгоритм соединения: сначала используются быстрые хеш-соединения, а при наличии более одной большой таблицы происходит переход к соединениям слиянием.
...
## Колоночные и строчные базы данных {#column-oriented-vs-row-oriented-databases}

В строчных СУБД данные хранятся построчно, при этом все значения, относящиеся к одной строке, физически располагаются рядом друг с другом.

В колоночных СУБД данные хранятся по столбцам, при этом значения из одного столбца хранятся вместе.
...
В остальной части этой статьи объясняется, почему колоночные базы данных хорошо работают в таких сценариях и почему ClickHouse [превосходит](/concepts/why-clickhouse-is-so-fast/concepts/why-clickhouse-is-so-fast#storage-layer-concurrent-inserts-and-selects-are-isolated) другие системы в этой категории.


## Почему ClickHouse такой быстрый? {#why-is-clickhouse-so-fast}

ClickHouse использует все доступные системные ресурсы на полную мощность для максимально быстрой обработки каждого аналитического запроса. Это достигается благодаря уникальному сочетанию аналитических возможностей и внимания к низкоуровневым деталям, необходимым для создания самой быстрой OLAP-базы данных.
...
Легко заметить, что сценарий OLAP сильно отличается от других популярных сценариев (таких как OLTP или доступ типа «ключ-значение»). Поэтому не имеет смысла пытаться использовать OLTP или базу данных типа «ключ-значение» для обработки аналитических запросов, если вы хотите получить приемлемую производительность. Например, если вы попытаетесь использовать MongoDB или Redis для аналитики, вы получите очень низкую производительность по сравнению с базами данных OLAP.


### Ввод/вывод {#inputoutput}

1.  Для аналитического запроса необходимо прочитать лишь небольшое количество столбцов таблицы. В колоночной базе данных можно читать только те данные, которые действительно нужны. Например, если требуется 5 столбцов из 100, можно ожидать 20-кратного сокращения операций ввода-вывода.
2.  Поскольку данные читаются пакетами, их проще сжимать. Данные в столбцах также сжимаются лучше. Это дополнительно сокращает объём операций ввода-вывода.
3.  Благодаря сокращению операций ввода-вывода в системном кэше помещается больше данных.

Например, запрос «подсчитать количество записей для каждой рекламной платформы» требует чтения одного столбца «идентификатор рекламной платформы», который занимает 1 байт в несжатом виде. Если большая часть трафика не связана с рекламными платформами, можно ожидать как минимум 10-кратного сжатия этого столбца. При использовании быстрого алгоритма сжатия распаковка данных возможна со скоростью не менее нескольких гигабайт несжатых данных в секунду. Другими словами, такой запрос может обрабатываться со скоростью приблизительно несколько миллиардов строк в секунду на одном сервере. Такая скорость действительно достигается на практике.
...
1.  Векторный движок. Все операции написаны для векторов, а не для отдельных значений. Это означает, что операции не нужно вызывать часто, а затраты на диспетчеризацию незначительны. Код операции содержит оптимизированный внутренний цикл.
...
Обратите внимание, что для эффективности процессора язык запросов должен быть декларативным (SQL или MDX) или, по крайней мере, векторным (J, K). Запрос должен содержать только неявные циклы, что позволяет проводить оптимизацию.
-->
