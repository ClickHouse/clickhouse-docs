...
* `path_to_logs` – Путь к файлам журнала для чтения. Это может быть путь к каталогу с файлами журнала или к отдельному файлу журнала. Обратите внимание, что ClickHouse допускает только пути внутри каталога `user_files`.
...
* `poll_timeout_ms` - Тайм-аут для одного опроса файла журнала. Значение по умолчанию: [stream&#95;poll&#95;timeout&#95;ms](../../../operations/settings/settings.md#stream_poll_timeout_ms).
...
Чтобы прекратить приём потоковых данных или изменить логку преобразования, отсоедините материализованное представление:
