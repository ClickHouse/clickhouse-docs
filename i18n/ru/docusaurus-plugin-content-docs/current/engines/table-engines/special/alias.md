...
:::note
Когда `target_db` опущен и `target_table` не полностью квалифицировано (например, `Alias('my_table')`), целевая таблица определяется в той же базе данных, что и сам алиас, а не в текущей базе данных сессии.
:::
