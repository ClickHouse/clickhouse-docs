...
* У постфильтрации есть общая проблема: она может вернуть меньше строк, чем было запрошено в предложении `LIMIT <N>`. Такая ситуация возникает, когда одна или несколько строк результата, возвращённых индексом векторного сходства, не удовлетворяют дополнительным фильтрам.
...
:::note
Тип данных `QBit` и связанные с ним функции расстояния в настоящее время находятся в статусе Beta. Чтобы их включить, выполните `SET enable_qbit_type = 1`.
Если вы столкнётесь с проблемами, пожалуйста, создайте issue в [репозитории ClickHouse](https://github.com/clickhouse/clickhouse/issues).
:::
...
- [Мы построили движок векторного поиска, который позволяет выбирать степень точности во время выполнения запроса](https://clickhouse.com/blog/qbit-vector-search)