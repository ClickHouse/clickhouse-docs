...
keywords: ['поиск по сходству векторов', 'ann', 'knn', 'hnsw', 'индексы', 'индекс', 'ближайший сосед', 'векторный поиск']
...
В качестве альтернативы можно добавить индекс сходства векторов в существующую таблицу:
...
В отличие от обычных пропускающих индексов, которые используют значение гранулярности индекса по умолчанию, равное 1, индексы сходства векторов используют значение по умолчанию 100 миллионов.
Это значение гарантирует, что даже для больших частей внутри системы будет создано лишь небольшое количество индексов.
Мы рекомендуем изменять гранулярность индекса только продвинутым пользователям, которые понимают последствия своих действий (см. [ниже](#differences-to-regular-skipping-indexes)).
...
Следующие сообщения журнала (`system.text_log`) указывают на то, что индекс векторного сходства загружается.
Если такие сообщения появляются многократно для разных запросов векторного поиска, это указывает на то, что размер кэша слишком мал.
...
Информацию о попаданиях и промахах кэша для запроса с определённым идентификатором можно получить из [system.query&#95;log](../../../operations/system-tables/query_log.md):
...
Для производственных сценариев использования мы рекомендуем выбирать такой размер кэша, чтобы все векторные индексы постоянно размещались в памяти.

**Настройка квантования**

[Quantization](https://huggingface.co/blog/embedding-quantization) — это техника для уменьшения памяти, занимаемой векторами, и вычислительных затрат на построение и обход векторных индексов.
В ClickHouse векторные индексы поддерживают следующие варианты квантования:
