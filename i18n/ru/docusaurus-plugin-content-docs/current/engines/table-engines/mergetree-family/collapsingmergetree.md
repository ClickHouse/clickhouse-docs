---
description: 'Наследуется от MergeTree, но добавляет логику схлопывания строк в процессе слияния.'
...
Поскольку нам нужно только последнее состояние активности пользователя, исходную строку состояния и строку
отмены, которую мы вставили, можно удалить, как показано ниже, таким образом схлопывая недействительное (старое) состояние объекта:
...
1. Программа, которая записывает данные, должна запоминать состояние объекта, чтобы впоследствии иметь возможность его отменить. Строка отмены должна содержать копии полей ключа сортировки строки состояния и противоположный `Sign`. Это увеличивает первоначальный объём хранилища, но позволяет быстро записывать данные.
...
каждая группа последовательных строк с одинаковым ключом сортировки (`ORDER BY`) сводится не более чем к двум строкам:
строке состояния с `Sign` = `1` и строке отмены с `Sign` = `-1`.
...
|    |                                                                                                                                                            |
| -- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. | Первую строку отмены и последнюю строку состояния, если количество строк состояния и отмены совпадает и последняя строка является строкой состояния. |
...
Кроме того, когда строк состояния как минимум на две больше, чем строк отмены,
