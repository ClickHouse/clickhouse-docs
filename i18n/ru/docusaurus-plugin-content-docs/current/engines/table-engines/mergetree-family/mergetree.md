...
```sql
SELECT <выражение списка столбцов> [GROUP BY] <выражение ключей группировки> [ORDER BY] <выражение>
```
...
```sql
TTL expr
    [DELETE|RECOMPRESS codec_name1|НА ДИСК 'xxx'|НА ТОМ 'xxx'][, DELETE|RECOMPRESS codec_name2|НА ДИСК 'aaa'|НА ТОМ 'bbb'] ...
    [WHERE условия]
    [GROUP BY key_expr [SET v1 = aggr_func(v1) [, v2 = aggr_func(v2) ...]] ]
```
...
* `TO VOLUME 'bbb'` — перенести часть в том `bbb`;
...
```sql
TTL time_column + INTERVAL 1 MONTH УДАЛИТЬ ГДЕ column = 'value'
```
