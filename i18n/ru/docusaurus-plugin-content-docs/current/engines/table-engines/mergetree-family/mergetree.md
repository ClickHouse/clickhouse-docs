...
Чтобы выбирать данные в исходном порядке, используйте [однопоточные](/operations/settings/settings.md/#max_threads) запросы `SELECT`.
...
Для каждой гранулы индекса хранится не более `max_rows` уникальных значений заданного выражения.
...
Для каждой гранулы индекса хранится [фильтр Блума](https://en.wikipedia.org/wiki/Bloom_filter) для указанных столбцов.
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `n`                             | размер n-граммы                                                                                                                |
| `size_of_bloom_filter_in_bytes` | размер фильтра Блума в байтах. Можно использовать большое значение, например, `256` или `512`, потому что он хорошо сжимается. |
| `number_of_hash_functions`      | количество хэш-функций, используемых в фильтре Блума.                                                                          |
| `random_seed`                   | начальное значение (seed) для хэш-функций фильтра Блума.                                                                       |
...
Для оценки параметров `ngrambf_v1` вы можете использовать следующие [пользовательские функции (UDF)](/sql-reference/statements/create/function.md).
...
Эти функции соответствуют калькулятору фильтра Блума, доступному [здесь](https://hur.st/bloomfilter).
...
#### Блум-фильтр по разрежённым граммам \{#sparse-grams-bloom-filter\}

Блум-фильтр по разрежённым граммам аналогичен `ngrambf_v1`, но использует [токены разрежённых грамм](/sql-reference/functions/string-functions.md/#sparseGrams) вместо n-грамм.
...
### Табличный TTL \{#mergetree-table-ttl\}

Таблица может содержать выражение для удаления просроченных строк и несколько выражений для автоматического перемещения частей между [дисками или томами](#table_engine-mergetree-multiple-volumes). Когда срок жизни строк в таблице истекает, ClickHouse удаляет все соответствующие строки. Для перемещения или перекомпрессии частей все строки части должны удовлетворять условию выражения `TTL`.
...
#### Создание таблицы, в которой устаревшие строки повторно сжимаются: \{#creating-a-table-where-expired-rows-are-recompressed\}
...
Если вы выполняете запрос `SELECT` между слияниями, вы можете получить просроченные данные. Чтобы этого избежать, используйте запрос [OPTIMIZE](/sql-reference/statements/optimize.md) перед `SELECT`.
...
В настоящее время отсечение частей поддерживается только статистиками `MinMax` и `Basic`. Если такая статистика определена для столбца, ClickHouse отслеживает минимальные и максимальные значения этого столбца в каждой части.
...
* `Basic`
  ...
  Компактный набор однозначных сводных характеристик, вычисляемых по столбцу. В зависимости от типа столбца заполняются следующие данные:
  ...
  * для столбцов `String` и `FixedString`: суммарная длина в байтах значений, отличных от `NULL` (на её основе можно вычислить среднюю длину строки);
  * для столбцов `Nullable` и `LowCardinality(Nullable)`: количество значений `NULL`, которое оптимизатор использует, чтобы исключать строки с `NULL` из оценок селективности.
    ...
    Одна статистика `Basic` может заполнять сразу несколько таких значений — например, для столбца `Nullable(UInt32)` она отслеживает и числовые min/max, и количество `NULL`. По сравнению с `MinMax`, `Basic` дополнительно работает со столбцами `String` / `FixedString`, а также может быть объявлена для обёрток `Nullable` над типами вроде `UUID` или `IPv6` исключительно для отслеживания количества `NULL`.
...
Все перечисленные выше варианты также поддерживают обёртки `Nullable` и `LowCardinality(Nullable)` для указанных типов. `Basic` также можно объявлять для обёрток `Nullable` у таких типов, как `UUID` или `IPv6`, исключительно для подсчёта количества значений NULL.
...
Для `Basic` в столбцах `String` / `FixedString` статистика хранит только суммарный
размер не-`NULL` значений в байтах (используется для оценки средней длины строки) и количество `NULL`;
фильтры диапазона и отсечение частей на её основе не используются.
