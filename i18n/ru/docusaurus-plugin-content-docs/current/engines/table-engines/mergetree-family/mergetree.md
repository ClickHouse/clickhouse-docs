...
* `NullCount`
  ...
  Отслеживает количество значений `NULL` в столбцах `Nullable`. Используется для точной оценки селективности предикатов `IS NULL`/`IS NOT NULL` при оптимизации PREWHERE и позволяет выполнять отсечение частей на основе наличия `NULL`.
