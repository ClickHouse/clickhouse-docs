...
Этот движок предоставляет доступ только для чтения к существующим таблицам Apache [Paimon](https://paimon.apache.org/) в Amazon S3, Azure, HDFS и к локально хранимым таблицам.
Он поддерживает чтение снимков, инкрементальное чтение и базовое отсечение партиций, которое обеспечивает этот движок.
