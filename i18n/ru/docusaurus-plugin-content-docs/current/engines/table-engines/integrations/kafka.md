...
* `kafka_consumer_acquire_timeout_ms` — Таймаут в миллисекундах для получения доступа к консьюмеру Kafka при прямых запросах `SELECT` к таблице `Kafka2` (с хранением смещений в Keeper). Когда на одной и той же таблице одновременно выполняются несколько прямых запросов `SELECT`, каждый из них должен ждать, пока консьюмеры не станут доступны. Этот таймаут предотвращает взаимные блокировки, когда запросы занимают разные подмножества консьюмеров. По умолчанию: `30000`.
...
* `kafka_autodetect_client_rack` — Автоматически задаёт параметр `client.rack` для `librdkafka`, чтобы выбирать ближайшие реплики Kafka.
  Поддерживаемые источники:
  `AWS_ZONE_ID` для идентификатора зоны доступности AWS IMDSv2, например `euc1-az1`;
  `AWS_ZONE_NAME` для имени зоны доступности AWS IMDSv2, например `eu-central-1a`;
  `GCP_ZONE` для зоны из сервиса метаданных GCP, например `europe-central2-a`;
  `CLICKHOUSE` для использования внутреннего механизма определения в ClickHouse, который может опираться на облачные метаданные или конфигурацию;
  `AWS_ZONE_NAME_THEN_GCP_ZONE` — чтобы сначала попробовать `AWS_ZONE_NAME`, а затем `GCP_ZONE`.
  По умолчанию: пустая строка, отключено.
  Совет: в разных окружениях используются разные форматы зон доступности. Amazon MSK обычно использует идентификаторы зон, поэтому предпочтителен `AWS_ZONE_ID`. Confluent Cloud обычно использует имена зон, поэтому предпочтителен `AWS_ZONE_NAME`. Если вы не уверены, используйте `AWS_ZONE_NAME_THEN_GCP_ZONE` или проверьте значение `broker.rack` в кластере.
  Примечание: брокеры Kafka должны быть настроены с параметрами `broker.rack` и `replica.selector.class=org.apache.kafka.common.replica.RackAwareReplicaSelector`.
...
* `kafka_map_virtual_columns_on_write` — Если включено, столбцы со специальными именами `_key`, `_timestamp`, `_headers.name` и `_headers.value` в schema таблицы сопоставляются с соответствующими метаданными сообщения Kafka при `INSERT` и исключаются из полезной нагрузки сообщения. См. [Соответствие столбцов метаданным сообщений Kafka](#mapping-columns-to-kafka-message-metadata). По умолчанию: `false`.
...
## Соответствие столбцов метаданным сообщений Kafka \{#mapping-columns-to-kafka-message-metadata\}

При отправке сообщений с помощью `INSERT INTO` движок Kafka всегда использует столбец `_key` (типа `String`) как ключ сообщения Kafka, а столбец `_timestamp` (типа `DateTime`) — как таймстамп сообщения Kafka, если такие столбцы есть в таблице. По умолчанию эти столбцы также включаются в полезную нагрузку отправляемого сообщения наряду с остальными столбцами.

При `kafka_map_virtual_columns_on_write = 1` поведение меняется:

* `_key` (тип `String`) — сопоставляется с ключом сообщения Kafka.
* `_timestamp` (тип `DateTime`) — сопоставляется с таймстампом сообщения Kafka.
* `_headers.name` (тип `Array(String)`) и `_headers.value` (тип `Array(String)`) — сопоставляются с заголовками сообщения Kafka. Каждая пара `(_headers.name[i], _headers.value[i])` становится одним заголовком Kafka. Поскольку `_headers.name` и `_headers.value` имеют общий вложенный префикс `_headers`, ClickHouse требует, чтобы в каждой строке оба массива были одинакового размера.

Столбцы с этими именами **исключаются из полезной нагрузки сообщения** только в том случае, если их типы соответствуют перечисленным выше; в противном случае они остаются в полезной нагрузке, поэтому схемы, в которых эти имена случайно используются для несвязанных данных, продолжают работать.
...
Сформированное сообщение Kafka содержит полезную нагрузку `{"event_json":"{\"a\":1}"}`, ключ `session-42`, текущий таймстамп и два заголовка: `source=api` и `trace_id=abc-123`.
