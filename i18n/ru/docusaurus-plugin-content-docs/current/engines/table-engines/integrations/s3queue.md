...
Если `true`, при перемещении успешно обработанного файла полный путь исходного объекта добавляется к `after_processing_move_prefix`, поэтому в пункте назначения сохраняется структура исходного каталога внутри бакета. Если `false`, используется только имя файла, а структура исходного каталога становится плоской.
...
Путь к метаданным очереди в ZooKeeper. Если не указан явно, ClickHouse формирует путь из `s3queue_default_zookeeper_path`, UUID базы данных и UUID таблицы. Абсолютные значения (начинающиеся с `/`) используются без изменений, а относительные значения дописываются к настроенному префиксу. Макросы, такие как `{database}` или `{uuid}`, раскрываются до подключения движка к ZooKeeper.

Чтобы использовать вспомогательный кластер ZooKeeper, добавьте к значению префикс с настроенным именем, например `analytics_keeper:/clickhouse/queue/orders`. Имя должно существовать в `<auxiliary_zookeepers>`, иначе движок выдаст `Unknown auxiliary ZooKeeper name ...`. Полная строка (включая префикс) сохраняется в `SHOW CREATE TABLE`, так что оператор можно реплицировать дословно.
...
## Ролевой доступ к S3 \{#s3-role-based-access\}
...
Режим обработки `S3Queue` позволяет хранить меньше метаданных в ZooKeeper, но имеет ограничение: файлы, добавленные позже по времени, должны иметь алфавитно-цифровые имена, которые больше.

Режим `ordered` в `S3Queue`, так же как и `unordered`, поддерживает настройку `(s3queue_)processing_threads_num` (префикс `s3queue_` является необязательным), которая позволяет управлять количеством потоков, которые будут локально на сервере обрабатывать файлы `S3`.

Для режима `ordered` без партиционирования ClickHouse может возобновлять перечисление S3 с последнего обработанного ключа, чтобы избежать повторного перечисления всей истории префикса. В режиме `ordered` с бакетами точка возобновления консервативно выбирается как наименьший обработанный ключ среди всех бакетов, чтобы избежать пропуска необработанных файлов.
Эта оптимизация возобновляемого перечисления используется только для очередей на базе S3 в режиме `ordered` без партиционирования (не для AzureQueue и не когда задан `partitioning_mode`).
Кроме того, режим `ordered` также вводит ещё одну настройку под названием `(s3queue_)buckets`, которая означает «логические потоки». Это означает, что в распределённом сценарии, когда есть несколько серверов с репликами таблицы `S3Queue`, эта настройка определяет количество единиц обработки. Например, каждый поток обработки на каждой реплике `S3Queue` будет пытаться захватить определённый `бакет` для обработки, при этом каждый `бакет` сопоставляется определённым файлам по хэшу имени файла. Поэтому в распределённом сценарии настоятельно рекомендуется, чтобы значение настройки `(s3queue_)buckets` было как минимум равно количеству реплик или больше. Допустимо, чтобы количество бакетов было больше количества реплик. Наиболее оптимальный сценарий — когда значение настройки `(s3queue_)buckets` равно произведению `number_of_replicas` и `(s3queue_)processing_threads_num`.
...
## SELECT из движка таблиц S3Queue \{#select\}
...
`SELECT` мало полезен для потокового импорта (кроме отладки), потому что каждый файл можно импортировать только один раз. Более практично создавать потоки в реальном времени с помощью [materialized views](../../../sql-reference/statements/create/view.md). Для этого:

1. Используйте этот движок для создания таблицы, которая будет читать данные из указанного пути в S3 и рассматривать её как поток данных.
2. Создайте таблицу с требуемой структурой.
3. Создайте materialized view, которое преобразует данные из этого движка и помещает их в ранее созданную таблицу.
