...
description: 'Этот движок обеспечивает интеграцию с экосистемой Amazon S3. Аналогичен
  движку HDFS, но предоставляет специфичные для S3 возможности.'
sidebar_label: 'S3'
...
Этот движок обеспечивает интеграцию с экосистемой [Amazon S3](https://aws.amazon.com/s3/). Он аналогичен движку [HDFS](/engines/table-engines/integrations/hdfs), но предоставляет специфичные для S3 возможности.
...
* `path` — URL бакета с путем к файлу. В режиме только для чтения поддерживаются следующие подстановочные шаблоны: `*`, `**`, `?`, `{abc,def}` и `{N..M}`, где `N`, `M` — числа, `'abc'`, `'def'` — строки. Дополнительную информацию см. [ниже](#wildcards-in-path).
...
* `partition_strategy` – Варианты: `WILDCARD` или `HIVE`. `WILDCARD` требует наличия `{_partition_id}` в пути, который заменяется ключом партиционирования. `HIVE` не допускает подстановочные шаблоны, предполагает, что путь — это корень таблицы, и генерирует директории партиций в стиле Hive с идентификаторами Snowflake ID в качестве имен файлов и форматом файла в качестве расширения. По умолчанию используется настройка `file_like_engine_default_partition_strategy` (`WILDCARD` в настройках `compatibility` старше `26.6`, иначе `HIVE`).
...
* `extra_credentials` — Необязательный параметр. Используется для передачи `role_arn` для доступа на основе ролей в ClickHouse Cloud. Шаги настройки см. в разделе [Secure S3](/cloud/data-sources/secure-s3).
...
`WILDCARD`: заменяет подстановочный знак `{_partition_id}` в пути к файлу на фактический ключ партиционирования. Чтение не поддерживается. Выбирается по умолчанию только при настройке `compatibility` ниже `26.6`; в противном случае по умолчанию используется `HIVE` (см. настройку `file_like_engine_default_partition_strategy`).
...
партиционированных данных в S3 — перенос этих данных в другую
систему ClickHouse (например, при переходе с локальных систем на ClickHouse
Cloud). Поскольку датасеты ClickHouse часто очень велики, а надёжность сети
иногда далека от идеальной, имеет смысл передавать датасеты
по частям — отсюда и запись с партиционированием.
:::
...
## Ролевой доступ \{#role-based-access\}

В ClickHouse Cloud для аутентификации в S3 вместо ключей доступа можно использовать ролевой доступ. Шаги настройки см. в разделе [Secure S3](/cloud/data-sources/secure-s3).

После этого `roleARN` можно передать через параметр `extra_credentials`:
