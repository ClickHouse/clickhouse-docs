...
Этот движок обеспечивает интеграцию с экосистемой [Azure Blob Storage](https://azure.microsoft.com/en-us/products/storage/blobs), позволяя выполнять потоковый импорт данных.
...
1. Используйте `system.azure_queue_metadata_cache` для представления состояния очереди в памяти для версий сервера &gt;= 25.1. Для более старых версий используйте `system.s3queue_metadata_cache` (он также будет содержать информацию для таблиц `azure`).
...
Эта постоянная таблица содержит ту же информацию, что и `system.s3queue_metadata_cache`, но для обработанных и завершившихся ошибкой файлов.
