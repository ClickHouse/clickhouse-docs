...
Поддерживает изменение структуры таблиц (`ALTER TABLE ... ADD|DROP COLUMN`). Если параметр `use_table_cache` (см. параметры движка ниже) установлен в значение `1`, структура таблицы кэшируется и не проверяется на наличие изменений, но может быть обновлена с помощью запросов `DETACH` и `ATTACH`.
...
- `use_table_cache` — определяет, кэшируется ли структура таблиц базы данных. Необязательный параметр. Значение по умолчанию: `0`.
...
Поскольку при создании базы данных параметр `use_table_cache` был установлен в `1`, структура таблицы в ClickHouse была закэширована и поэтому не изменилась:
...
После отсоединения и повторного присоединения таблицы структура была обновлена:
