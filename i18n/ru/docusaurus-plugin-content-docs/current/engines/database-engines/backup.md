...
Резервное место может быть любым допустимым резервным [местом](../../operations/backup#configure-a-backup-destination), таким как `Disk`, `S3`, `File`.

При использовании резервного места `Disk`, запрос на создание базы данных из резервной копии будет выглядеть следующим образом:
...
- `backup_destination` — Резервное место.

## Пример использования {#usage-example}

Давайте приведем пример с резервным местом `Disk`. Сначала настроим диск для резервных копий в `storage.xml`:
...
Пример использования. Создадим тестовую базу данных, таблицы, вставим некоторые данные и затем создадим резервную копию:
