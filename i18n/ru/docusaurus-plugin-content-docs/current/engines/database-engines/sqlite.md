---
description: 'Позволяет подключаться к базам данных SQLite и выполнять операции `INSERT` и `SELECT` для обмена данными между ClickHouse и SQLite.'
...
SQLite не требует управления сервисами (например, скриптов запуска) или контроля доступа на основе `GRANT` и паролей. Контроль доступа осуществляется с помощью прав файловой системы, назначенных самому файлу базы данных.
