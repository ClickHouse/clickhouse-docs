...
Возвращает последнее значение, оцененное в пределах своего упорядоченного окна. По умолчанию аргументы NULL пропускаются, однако модификатор `RESPECT NULLS` может быть использован для переопределения этого поведения.
...
:::note
Использование необязательного модификатора `RESPECT NULLS` после `first_value(column_name)` обеспечит, что аргументы `NULL` не будут пропущены.
...
- Последнее значение, оцененное в пределах его упорядоченного окна.

**Пример**

В этом примере функция `last_value` используется для нахождения наименьшей зарплаты футболиста из фиктивного набора данных о зарплатах футболистов Премьер-Лиги.
