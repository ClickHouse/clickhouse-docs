---
description: 'Документация для оконной функции last_value'
...
Возвращает последнее значение, рассчитанное в пределах его упорядоченной рамки. По умолчанию аргументы NULL пропускаются, однако модификатор `RESPECT NULLS` может быть использован для переопределения этого поведения.
...
:::note
Использование необязательного модификатора `RESPECT NULLS` после `first_value(column_name)` гарантирует, что аргументы `NULL` не будут пропущены.
...
- Последнее значение, рассчитанное в пределах его упорядоченной рамки.

**Пример**

В этом примере функция `last_value` используется для нахождения наименее оплачиваемого футболиста из вымышленного набора данных о зарплатах футболистов английской Премьер-лиги.
