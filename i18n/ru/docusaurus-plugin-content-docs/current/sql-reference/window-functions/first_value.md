...
Возвращает первое значение, вычисленное в его упорядоченной области. По умолчанию аргументы NULL пропускаются, однако можно использовать модификатор `RESPECT NULLS`, чтобы переопределить это поведение.
...
- Первое значение, вычисленное в его упорядоченной области.

**Пример**

В этом примере функция `first_value` используется для нахождения наиболее высокооплачиваемого футболиста из вымышленного набора данных о зарплатах игроков Премьер-лиги.
