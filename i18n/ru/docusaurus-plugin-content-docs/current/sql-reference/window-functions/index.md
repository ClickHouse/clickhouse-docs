...
Их можно использовать для вычислений, аналогичных тем, что выполняются с агрегатными функциями, но оконная функция не приводит к объединению строк в единый результирующий набор — отдельные строки по‑прежнему возвращаются.

## Стандартные оконные функции \{#standard-window-functions\}

ClickHouse поддерживает стандартный синтаксис SQL для окон и оконных функций.
В таблице ниже показано, какие возможности поддерживаются на данный момент:
...
| Рамка окна `ROWS`                                                               | ✅               |                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Рамка окна `RANGE`                                                              | ✅               | Используется по умолчанию, если рамка окна не указана явно (`RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`).                                                                                                                                                                                                                                                                                                       |
| Синтаксис `INTERVAL` для рамки `RANGE OFFSET` с `DateTime`                      | ❌               | Вместо этого указывайте количество секунд (`RANGE` работает с любым числовым типом).                                                                                                                                                                                                                                                                                                                                    |
| Рамка окна `GROUPS`                                                             | ❌               |                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `percent_rank()`/`percentRank()`                                                | ✅               | Эффективно вычисляет относительное положение значения в пределах партиции. Заменяет более многословное и вычислительно затратное ручное вычисление на SQL, выраженное как `ifNull((rank() OVER (PARTITION BY x ORDER BY y) - 1) / nullif(count(1) OVER (PARTITION BY x) - 1, 0), 0)`.                                                                                                                                   |
| `cume_dist()`                                                                   | ✅               | Вычисляет кумулятивное распределение значения в группе значений. Возвращает процент строк со значениями, меньшими или равными значению текущей строки.                                                                                                                                                                                                                                                                  |
| `lag/lead(value, offset)`                                                       | ✅               | Также можно использовать один из следующих обходных вариантов:<br /> 1) `any(value) OVER (... ROWS BETWEEN <offset> PRECEDING AND <offset> PRECEDING)` или `FOLLOWING` вместо `PRECEDING` для `lead` <br /> 2) `lagInFrame/leadInFrame`, которые работают аналогично, но учитывают рамку окна. Чтобы получить поведение, идентичное `lag/lead`, используйте `ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING`. |
| `ntile(buckets)`                                                                | ✅               | Например, окно можно задать так: `(PARTITION BY x ORDER BY y ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)`.                                                                                                                                                                                                                                                                                                |
...
* `ROWS or RANGE` - задает границы рамки окна, `aggregate_function` вычисляется внутри этой рамки окна.
...
## Функции, используемые только как оконные функции \{#functions\}

Следующие функции можно использовать только как оконные функции. Большинство из них — стандартные SQL-функции; `lagInFrame`, `leadInFrame` и `nonNegativeDerivative` — расширения ClickHouse.

| Функция                                                                                                    | Описание                                                                                                                                            |
| ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`row_number()`](./row_number.md)                                                                          | Нумерует текущую строку в пределах её партиции, начиная с 1.                                                                                        |
| [`first_value(x)`](./first_value.md)                                                                       | Возвращает первое значение, вычисленное в пределах упорядоченной рамки окна.                                                                        |
| [`last_value(x)`](./last_value.md)                                                                         | Возвращает последнее значение, вычисленное в пределах упорядоченной рамки окна.                                                                     |
| [`nth_value(x, offset)`](./nth_value.md)                                                                   | Возвращает первое не-NULL значение, вычисленное для n-й строки (offset) в пределах упорядоченной рамки окна.                                        |
| [`rank()`](./rank.md)                                                                                      | Присваивает текущей строке ранг в пределах её партиции с пропусками.                                                                                |
| [`dense_rank()`](./dense_rank.md)                                                                          | Присваивает текущей строке ранг в пределах её партиции без пропусков.                                                                               |
| [`lagInFrame(x)`](./lagInFrame.md)                                                                         | Возвращает значение, вычисленное для строки, находящейся на заданное физическое смещение перед текущей строкой в пределах упорядоченной рамки окна. |
| [`leadInFrame(x)`](./leadInFrame.md)                                                                       | Возвращает значение, вычисленное для строки, находящейся на offset строк после текущей строки в пределах упорядоченной рамки окна.                  |
| [`nonNegativeDerivative(metric_column, timestamp_column[, INTERVAL X UNITS])`](./nonNegativeDerivative.md) | Вычисляет неотрицательную производную `metric_column` по `timestamp_column`. Специфично для ClickHouse.                                             |
...
### Границы рамки окна \{#frame-bounding\}
...
Следующие примеры решают распространённые практические задачи.

### Максимальная/суммарная зарплата по отделу \{#maximumtotal-salary-per-department\}
...
Температура хранится с точностью до секунды, но с помощью `Range` и `ORDER BY toDate(ts)` мы задаём рамку окна размером в 10 единиц, и благодаря `toDate(ts)` такой единицей становится день.
