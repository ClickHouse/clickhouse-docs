...
Приведённое выше создаёт представление таблицы, которое можно использовать как табличную функцию, подставляя параметры, как показано ниже.
...
* Нет триггера на вставку. Т.е. когда новые данные вставляются в таблицу, указанную в SELECT, они *не* отправляются автоматически в обновляемое материализованное представление. Периодическое обновление каждый раз выполняет весь запрос целиком.
...
* `refresh_retries` - Сколько раз повторять попытку, если запрос обновления завершился с исключением. Если все попытки не удались, переход к следующему запланированному времени обновления. 0 означает отсутствие повторных попыток, -1 означает бесконечное количество повторов. По умолчанию: 0.
...
[Функции временных окон](../../functions/time-window-functions.md) используются для получения нижней и верхней границ окна для записей. Оконное представление должно использоваться совместно с одной из функций временных окон.
...
По умолчанию окно срабатывает при поступлении водяного знака, а элементы, которые приходят после водяного знака, отбрасываются. Представление window view поддерживает обработку поздних событий через параметр `ALLOWED_LATENESS = INTERVAL`. Пример обработки таких опоздавших событий:
...
`LIMIT` может быть указан для задания количества обновлений, которые нужно получить перед завершением запроса. Предложение `EVENTS` можно использовать для получения сокращённой формы запроса `WATCH`, при которой вместо результата запроса вы будете получать только последний водяной знак (watermark) запроса.
...
Сначала создадим оконное представление с фиксированным временным окном (tumbling window) в 10 секунд:
...
Когда логи записываются в таблицу `data`,

```sql
INSERT INTO data VALUES(1,now())
```

Запрос `WATCH` должен выводить результаты в следующем виде:
