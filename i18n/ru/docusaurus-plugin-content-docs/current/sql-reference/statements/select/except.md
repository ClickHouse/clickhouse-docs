...
- Результат `EXCEPT` может содержать дублирующиеся строки. Используйте `EXCEPT DISTINCT`, если это нежелательно.
- Множественные операторы `EXCEPT` выполняются слева направо, если не указаны скобки.
- Оператор `EXCEPT` имеет такой же приоритет, как и клаузула `UNION`, и более низкий приоритет, чем клаузула `INTERSECT`.
...
### Фильтрация чисел с использованием клаузы `EXCEPT` {#filtering-numbers-using-the-except-clause}
...
### Исключение конкретных колонок с использованием `EXCEPT()` {#excluding-specific-columns-using-except}
