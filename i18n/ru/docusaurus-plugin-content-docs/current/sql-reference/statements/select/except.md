---
description: 'Документация по оператору EXCEPT, которая возвращает только те строки, которые получены из первого запроса, исключая результаты второго.'
...
> Оператор `EXCEPT` возвращает только те строки, которые получены из первого запроса, исключая результаты второго.
...
`EXCEPT` и `INTERSECT` во многих случаях можно использовать как взаимозаменяемые операторы при разной булевой логике; оба они полезны, когда у вас есть две таблицы с общим столбцом (или несколькими столбцами).
Например, предположим, что у нас есть несколько миллионов строк исторических данных о криптовалюте, содержащих цены сделок и объём торгов:
...
Теперь предположим, что у нас есть таблица с именем `holdings`, которая содержит список принадлежащих нам криптовалют вместе с количеством монет:
...
Мы можем использовать `EXCEPT`, чтобы ответить на такой вопрос: **&quot;Какие монеты из тех, которыми мы владеем, никогда не торговались ниже $10?&quot;**:
...
Это означает, что из четырёх криптовалют, которыми мы владеем, только Bitcoin ни разу не опускался ниже $10 (судя по ограниченным данным в этом примере).

### Использование `EXCEPT DISTINCT` \{#using-except-distinct\}

Обратите внимание, что в результате предыдущего запроса было несколько строк с Bitcoin. Вы можете добавить `DISTINCT` к `EXCEPT`, чтобы удалить повторяющиеся строки из результата:
