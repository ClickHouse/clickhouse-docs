---
description: 'Документация по оператору INTERSECT'
...
# Оператор INTERSECT \\{#intersect-clause\\}

Оператор `INTERSECT` возвращает только те строки, которые присутствуют одновременно и в первом, и во втором запросе. Запросы должны совпадать по количеству столбцов, их порядку и типам данных. Результат `INTERSECT` может содержать повторяющиеся строки.
...
Условие может быть любым выражением, соответствующим вашим требованиям.
...
Это означает, что в какой‑то момент Bitcoin и Ethereum торговались выше $100, тогда как DOGEFI и Bitcoin Diamond никогда не поднимались выше этой отметки (по крайней мере, судя по данным, которые у нас есть в этом примере).
