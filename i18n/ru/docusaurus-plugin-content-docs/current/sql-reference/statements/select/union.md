...
Запросы, входящие в `UNION/UNION ALL/UNION DISTINCT`, могут выполняться параллельно, а их результаты — объединяться.
