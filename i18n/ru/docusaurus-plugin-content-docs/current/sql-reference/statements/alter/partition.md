...
## DROP DETACHED PARTITION|PART — удаление отсоединённого раздела/части {#drop-detached-partitionpart}
...
Удаляет данные в указанном разделе, которые соответствуют заданному фильтрующему выражению. Операция реализована как [мутация](/sql-reference/statements/alter/index.md#mutations).
...
`IN PARTITION` задает раздел, к которому применяются выражения [UPDATE](/sql-reference/statements/alter/update) или [DELETE](/sql-reference/statements/alter/delete) в результате выполнения запроса `ALTER TABLE`. Новые парты создаются только из указанного раздела. Таким образом, `IN PARTITION` помогает снизить нагрузку, когда таблица разбита на множество разделов, а вам необходимо обновить данные точечно.
