...
Вы можете изменить запрос `SELECT`, указанный при создании [материализованного представления](/sql-reference/statements/create/view#materialized-view), с помощью оператора `ALTER TABLE ... MODIFY QUERY` без прерывания процесса загрузки данных.

Эта команда предназначена для изменения материализованного представления, созданного с использованием клаузы `TO [db.]name`. Она не изменяет структуру базовой таблицы хранения и не изменяет определение столбцов материализованного представления, поэтому область применения этой команды для материализованных представлений, созданных без клаузы `TO [db.]name`, очень ограничена.
...
-- Столбцы не обязаны совпадать в материализованном представлении и в TO
-- (целевой таблице), поэтому следующий ALTER не нарушает вставку данных.
...
-- Новые столбцы `browser` и `cost` пусты, так как мы еще не изменили материализованное представление.
...
-- !!! При выполнении `MODIFY ORDER BY` PRIMARY KEY был неявно создан.
...
-- !!! Определение столбцов не изменилось, но это не важно: мы не выполняем запрос
-- к MATERIALIZED VIEW, мы выполняем запрос к таблице-хранилищу TO.
...
Применение сильно ограничено, так как можно изменять только секцию `SELECT` без возможности добавления новых столбцов.
```
