...
Вы можете изменить запрос `SELECT`, который был указан при создании [materialized view](/sql-reference/statements/create/view#materialized-view), с помощью оператора `ALTER TABLE ... MODIFY QUERY` без прерывания процесса ингестии.

Эта команда предназначена для изменения materialized view, созданного с использованием предложения `TO [db.]name`. Она не изменяет структуру базовой таблицы хранилища и не изменяет определение столбцов materialized view, поэтому область её применения в отношении materialized view, созданных без предложения `TO [db.]name`, крайне ограничена.
...
Область применения очень ограничена, так как вы можете изменять только секцию `SELECT`, не добавляя новые столбцы.
