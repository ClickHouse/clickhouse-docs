...
Переименование столбцов, в именах которых присутствуют точки, частично поддерживается. Точки зарезервированы для доступа к подстолбцам типа [Nested](/sql-reference/data-types/nested-data-structures/nested), поэтому префикс (имя родителя) должен оставаться тем же. Можно изменять только суффикс (имя подстолбца). Например, `a.b` можно переименовать в `a.c`, но переименование `a.b` в `b.d` не допускается, так как при этом изменяется префикс родителя Nested.
