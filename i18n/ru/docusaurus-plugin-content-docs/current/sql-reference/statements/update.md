...
- **Становятся видимыми немедленно** в запросах `SELECT` за счёт применения патчей
...
- [`APPLY PATCHES`](/sql-reference/statements/alter/apply-patches) — принудительная физическая материализация патчей в частях данных (операция мутации)