...
В качестве токенизатора использует функцию [splitByNonAlpha](/sql-reference/functions/splitting-merging-functions.md/#splitByNonAlpha), то есть токен определяется как максимально длинная подпоследовательность подряд идущих символов `[0-9A-Za-z_]` (цифр, ASCII‑букв и символа подчёркивания).
