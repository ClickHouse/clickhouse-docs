...
`1`, если точка находится внутри хотя бы одного из элипсов; `0`, если не находится.
...
> • Когда аргумент многоугольника является константой, а точка задаётся через индексированные столбцы ключа (например, `pointInPolygon((x, y), constant_polygon)` в таблице, где `x, y` являются частью `PRIMARY KEY` или покрываются индексом `minmax`), ClickHouse может использовать как первичный ключ, так и индексы пропуска данных `minmax`, чтобы отсекать нерелевантные гранулы.
