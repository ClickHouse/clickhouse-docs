...
Возвращает геометрический объект WKT (Well Known Text) из различных [геотипов данных](../../data-types/geo.md). Поддерживаются следующие объекты WKT:
...
`geo_data` — один из следующих [геотипов данных](../../data-types/geo.md) или их базовых примитивных типов:
...
- Для Point возвращается геометрический объект WKT `POINT`.
- Для Polygon возвращается геометрический объект WKT `POLYGON`.
- Для MultiPolygon возвращается геометрический объект WKT `MULTIPOLYGON`.
- Для LineString возвращается геометрический объект WKT `LINESTRING`.
- Для MultiLineString возвращается геометрический объект WKT `MULTILINESTRING`.
...
Функция `readWKTPoint` в ClickHouse разбирает представление геометрии точки (Point) в формате Well-Known Text (WKT) и возвращает точку во внутреннем формате ClickHouse.
...
Функция возвращает внутреннее представление ClickHouse геометрии типа LineString.
...
Разбирает представление геометрии MultiLineString в формате Well-Known Text (WKT) и возвращает её во внутреннем формате ClickHouse.
...
Функция возвращает внутреннее представление ClickHouse геометрии типа MultiLineString.
...
Разбирает представление геометрии Polygon в формате Well-Known Text (WKT) и возвращает кольцо (замкнутую ломаную) во внутреннем формате ClickHouse.
...
Функция возвращает внутреннее представление ClickHouse геометрии кольца (замкнутой ломаной).
...
Возвращает true или false в зависимости от того, полностью ли один полигон находится внутри другого полигона. Справка: https://www.boost.org/doc/libs/1_62_0/libs/geometry/doc/html/geometry/reference/algorithms/within/within_2.html
...
Преобразует WKB (Well Known Binary) MultiPolygon в тип MultiPolygon.
...
Преобразует WKB (Well Known Binary) MultiPolygon в тип Polygon.
...
Функция `readWKBPoint` в ClickHouse разбирает представление геометрии точки (Point) в формате Well-Known Binary (WKB) и возвращает точку во внутреннем формате ClickHouse.
...
- `wkb_string` — входная строка WKB, представляющая геометрию точки (Point).
...
Разбирает представление геометрии LineString в формате Well-Known Binary (WKB) и возвращает его во внутреннем формате ClickHouse.
...
- `wkb_string` — входная строка WKB, представляющая геометрию LineString.
...
Разбирает представление геометрии MultiLineString в формате Well-Known Binary (WKB) и возвращает её во внутреннем формате ClickHouse.
...
- `wkb_string` — входная строка WKB, представляющая геометрию MultiLineString.
...
Вычисляет минимальное расстояние между двумя точками, где одна точка принадлежит первому полигону, а вторая — второму полигону. Сферический означает, что координаты интерпретируются как координаты на идеальной сфере, что не соответствует реальной форме Земли. Использование такой системы координат ускоряет выполнение, но не обеспечивает высокой точности.
...
Вычисляет пространственную теоретико-множественную симметрическую разность (XOR) между двумя многоугольниками
...
Аналогично `polygonsSymDifferenceSpherical`, но координаты задаются в декартовой системе координат, которая ближе к модели реальной Земли.
...
Вычисляет пересечение (AND) между полигонами в сферических координатах.
...
UInt8, 0 для ложного значения, 1 для истинного
...
Возвращает true, если два полигона пересекаются (имеют общую область или границу). Справка: https://www.boost.org/doc/libs/1_62_0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Вычисляет площадь многоугольника на сфере.
...
Вычисляет объединение полигонов (операция ИЛИ).
...
#### Многоугольник, представляющий Зимбабве {#polygon-representing-zimbabwe}
