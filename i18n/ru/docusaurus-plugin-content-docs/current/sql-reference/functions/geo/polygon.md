...
description: 'Документация для Polygon'
title: 'Функции для работы с многоугольниками'
doc_type: reference
---
## WKT {#wkt}

Возвращает геометрический объект WKT (Well Known Text) из различных [Геоданных](../../data-types/geo.md). Поддерживаемые объекты WKT:
...
`geo_data` может быть одним из следующих [Геоданных](../../data-types/geo.md) или их базовых примитивных типов:
...
Функция `readWKTPoint` в ClickHouse разбирает представление Well-Known Text (WKT) геометрии Point и возвращает точку во внутреннем формате ClickHouse.
...
- `wkt_string`: Входная строка WKT, представляющая геометрию Point.

### Возвращаемое значение {#returned-value-2}

Функция возвращает внутреннее представление ClickHouse геометрии Point.
...
Разбирает представление Well-Known Text (WKT) геометрии LineString и возвращает его во внутреннем формате ClickHouse.
...
## readWKTMultiLineString {#readwkbmultilinestring}

Разбирает представление Well-Known Text (WKT) геометрии MultiLineString и возвращает его во внутреннем формате ClickHouse.
...
Разбирает представление Well-Known Text (WKT) геометрии Polygon и возвращает кольцо (замкнутый LineString) во внутреннем формате ClickHouse.
...
Функция возвращает внутреннее представление ClickHouse кольца (замкнутого LineString) геометрии.
...
Функция `readWKBPoint` в ClickHouse разбирает представление Well-Known Binary (WKB) геометрии Point и возвращает точку во внутреннем формате ClickHouse.
...
- `wkb_string`: Входная строка WKB, представляющая геометрию Point.
...
Разбирает представление Well-Known Binary (WKB) геометрии LineString и возвращает его во внутреннем формате ClickHouse.
...
Разбирает представление Well-Known Binary (WKB) геометрии MultiLineString и возвращает его во внутреннем формате ClickHouse.
...
Вызывает минимальное расстояние между двумя точками, где одна точка принадлежит первому многоугольнику, а вторая - другому многоугольнику. Сферические координаты означают, что координаты интерпретируются как координаты на чистом и идеальном шаре, что не соответствует действительности для Земли. Использование этой системы координат ускоряет выполнение, но, конечно, не является точным.
...
Вычисляет пространственнный теоретический симметричный разностный (XOR) результат между двумя многоугольниками.
...
Возвращает true, если два многоугольника пересекаются (имеют общую площадь или границу).
...
Возвращает true, если два многоугольника пересекаются (имеют общую площадь или границу). Ссылка https://www.boost.org/doc/libs/1_62_0/libs/geometry/doc/html/geometry/reference/algorithms/intersects.html
...
Для получения дополнительной информации о геометрических системах смотрите эту [презентацию](https://archive.fosdem.org/2020/schedule/event/working_with_spatial_trajectories_in_boost_geometry/attachments/slides/3988/export/events/attachments/working_with_spatial_trajectories_in_boost_geometry/slides/3988/FOSDEM20_vissarion.pdf) о библиотеке Boost, которая используется ClickHouse.