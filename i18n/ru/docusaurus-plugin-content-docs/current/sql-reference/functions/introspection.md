...
**Получение имени файла и номера строки исходного кода для одного адреса**
...
* Установите пакет `clickhouse-common-static-dbg`.
* Задайте для настройки [`allow_introspection_functions`](../../operations/settings/settings.md#allow_introspection_functions) значение `1`.
...
Возвращает массив, первый элемент которого — имя файла исходного кода и номер строки, разделённые двоеточием. Второй, третий и т. д. элементы содержат имена файлов исходного кода, номера строк и имена встроенных функций. Если отладочная информация не найдена, возвращается массив с одним элементом, равным имени бинарного файла, в противном случае если адрес некорректен, возвращается пустой массив. [`Array(String)`](/sql-reference/data-types/array)
...
Преобразует виртуальный адрес памяти внутри серверного процесса ClickHouse в символ из объектных файлов ClickHouse.
...
Возвращает символ из объектных файлов ClickHouse или пустую строку, если адрес недействителен. [`String`](/sql-reference/data-types/string)
...
**Применение функции к всей трассировке стека**
