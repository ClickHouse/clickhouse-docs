...
Вы можете использовать функции, описанные в этой главе, для интроспекции [ELF](https://ru.wikipedia.org/wiki/Executable_and_Linkable_Format) и [DWARF](https://ru.wikipedia.org/wiki/DWARF) для профилирования запросов.
...
Функция [arrayMap](/sql-reference/functions/array-functions#arraymapfunc-arr1-) позволяет обрабатывать каждый отдельный элемент массива `trace` с помощью функции `addressToLine`. Результат этой обработки вы видите в столбце `trace_source_code_lines` вывода.
...
Функции [arrayJoin](/sql-reference/functions/array-join) разобьют массив на строки.
...
Функция [arrayMap](/sql-reference/functions/array-functions#arraymapfunc-arr1-) позволяет обрабатывать каждый отдельный элемент массива `trace` с помощью функции `addressToSymbols`. Результат этой обработки вы видите в столбце `trace_symbols` вывода.
...
- Имя C++ функции или пустая строка, если символ недействителен. [String](../data-types/string.md).
...
Функция [arrayMap](/sql-reference/functions/array-functions#arraymapfunc-arr1-) позволяет обрабатывать каждый отдельный элемент массива `trace` с помощью функции `demangle`. Результат этой обработки вы видите в столбце `trace_functions` вывода.
...
Отправляет сообщение трассировки в лог сервера для каждого [Block](/development/architecture/#block).
...
- `message` — Сообщение, которое отправляется в лог сервера. [String](/sql-reference/data-types/string).
