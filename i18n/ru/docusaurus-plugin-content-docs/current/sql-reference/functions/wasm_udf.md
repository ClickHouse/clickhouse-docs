...
## Настройки \{#host-api-available-to-modules\}

Следующие настройки на уровне запроса управляют выполнением WebAssembly UDF:

* `webassembly_udf_max_fuel` (UInt64, по умолчанию: 100000) — лимит топлива на выполнение экземпляра WebAssembly UDF. Каждая инструкция WebAssembly потребляет некоторое количество топлива. Установите значение 0 для отключения ограничения.

* `webassembly_udf_max_memory` (UInt64, по умолчанию: 128&#95;MiB) — лимит памяти в байтах на экземпляр WebAssembly UDF.

* `webassembly_udf_max_input_block_size` (UInt64, по умолчанию: 0) — максимальное количество строк, передаваемых в WebAssembly UDF в одном блоке. Установите значение 0, чтобы обрабатывать все строки сразу.

* `webassembly_udf_max_instances` (UInt64, по умолчанию: 128) — максимальное количество экземпляров WebAssembly UDF, которые могут выполняться одновременно для одной функции.
