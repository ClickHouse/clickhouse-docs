---
description: 'Документация для функций массивов'
...
:::note
Можно оптимизировать, включив [`optimize_functions_to_subcolumns` настройку](/operations/settings/settings#optimize_functions_to_subcolumns). При `optimize_functions_to_subcolumns = 1` функция читает только [size0](/sql-reference/data-types/array#array-size) подколонку вместо чтения и обработки всей колонки массива. Запрос `SELECT empty(arr) FROM TABLE;` преобразуется в `SELECT arr.size0 = 0 FROM TABLE;`.
:::
...
:::note
Можно оптимизировать, включив [optimize_functions_to_subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) настройку. При `optimize_functions_to_subcolumns = 1` функция читает только [size0](/sql-reference/data-types/array#array-size) подколонку вместо чтения и обработки всей колонки массива. Запрос `SELECT notEmpty(arr) FROM table` преобразуется в `SELECT arr.size0 != 0 FROM TABLE`.
:::
...
Можно оптимизировать, включив [optimize_functions_to_subcolumns](/operations/settings/settings#optimize_functions_to_subcolumns) настройку. При `optimize_functions_to_subcolumns = 1` функция читает только [size0](/sql-reference/data-types/array#array-size) подколонку вместо чтения и обработки всей колонки массива. Запрос `SELECT length(arr) FROM table` преобразуется в `SELECT arr.size0 FROM TABLE`.
...
- `start` — Первый элемент массива. Необязательный, необходим, если используется `step`. Значение по умолчанию: 0.
- `end` — Число, перед которым создается массив. Обязательный.
- `step` — Определяет прирост между каждым элементом в массиве. Необязательный. Значение по умолчанию: 1.
...
- Все аргументы `start`, `end`, `step` должны быть нижеперечисленными типами данных: `UInt8`, `UInt16`, `UInt32`, `UInt64`, `Int8`, `Int16`, `Int32`, `Int64`, а также элементы возвращаемого массива, чей тип является суперттипом всех аргументов.
- Исключение выбрасывается, если результат запроса приводит к массивам с общей длиной более количества элементов, заданного настройкой [function_range_max_elements_in_block](../../operations/settings/settings.md#function_range_max_elements_in_block).
- Возвращает Null, если какой-либо аргумент имеет тип Nullable(Nothing). Исключение выбрасывается, если какой-либо аргумент имеет значение Null (тип Nullable(T)).
...
Аргументы должны быть константами и иметь типы, которые имеют наименьший общий тип. Должен быть передан хотя бы один аргумент, поскольку в противном случае неясно, какой тип массива создать. То есть, вы не можете использовать эту функцию для создания пустого массива (для этого используйте функцию 'emptyArray\*', описанную выше).
...
Создает массив длиной `length`, заполненный значением `elem`.
...
Проверяет, есть ли элемент `elem` в массиве `arr`.
...
- `subset` – Массив любого типа, который имеет общий суперттип с `set`, содержащий элементы, которые должны быть протестированы на принадлежность к `set`.
...
Выбрасывается исключение `NO_COMMON_TYPE`, если элементы `set` и `subset` не имеют общего супертипа.
...
- `array2` – Массив любого типа, который имеет общий супертип с `array1`.
...
Выбрасывается исключение `NO_COMMON_TYPE`, если элементы в `array1` и `array2` не имеют общего супертипа.
...
Проверяет, появляются ли все элементы `array2` в `array1` в том же точном порядке. Следовательно, функция вернет 1, если и только если `array1 = prefix + array2 + suffix`.

```sql
hasSubstr(array1, array2)
```

Другими словами, функции проверят, содержатся ли все элементы `array2` в `array1`, как это делает функция `hasAll`. В дополнение к этому, будет проверяться, что элементы встречаются в одном и том же порядке в `array1` и `array2`.
...
Выбрасывается исключение `NO_COMMON_TYPE`, если элементы `array1` и `array2` не имеют общего супертипа.
...
Если внутренний массив типа Nullable, будет вызвана функция 'indexOf'.
...
Возвращает количество элементов, для которых `func(arr1[i], ..., arrN[i])` возвращает что-то, отличное от 0. Если `func` не указана, возвращает количество ненулевых элементов в массиве.

Обратите внимание, что `arrayCount` является [высокой функцией](/sql-reference/functions/overview#higher-order-functions). Вы можете передать лямбда-функцию в качестве первого аргумента.
...
:::note
Тип возвращаемого значения определяется типом аргументов. Если массивы или кортежи содержат смешанные типы элементов, то тип результата является суперттипом.
:::
...
В этом примере Reaches — это количество конверсий (строк, полученных после применения ARRAY JOIN), а Hits — это количество просмотров страниц (строк до ARRAY JOIN). В данном случае можно получить тот же результат более простым способом:
...
Эта функция также может быть использована в высоких порядковых функциях. Например, вы можете использовать ее, чтобы получить индексы массива для элементов, соответствующих условию.
## arrayEnumerateUniq {#arrayenumerateuniq}

Возвращает массив такого же размера, как исходный массив, указывающий для каждого элемента, какое у него положение среди элементов с таким же значением.
...
В этом примере для каждого идентификатора цели проводится расчет количества конверсий (каждый элемент в вложенной структуре данных Goals — это цель, которая была достигнута, которую мы называем конверсией) и количества сессий. Без ARRAY JOIN мы бы посчитали количество сессий как sum(Sign). Но в данном случае строки были умножены на вложенную структуру Goals, поэтому, чтобы подсчитать каждую сессию один раз после этого, мы применяем условие к значению функции arrayEnumerateUniq(Goals.ID).

Функция arrayEnumerateUniq может принимать несколько массивов одинакового размера в качестве аргументов. В этом случае уникальность считается для кортежей элементов в одних и тех же позициях во всех массивах.
...
Возвращает массив такого же размера, как исходный массив, указывающий для каждого элемента, какое у него положение среди элементов с таким же значением. Позволяет перечислять многомерный массив с возможностью указать, насколько глубоко искать внутри массива.
...
- `clear_depth`: Переименовать элементы на указанном уровне отдельно. Положительное [целое число](../data-types/int-uint.md), меньшее или равное `max_arr_depth`.
...
При `clear_depth=1` и `max_array_depth=1`, результат `arrayEnumerateUniqRanked` идентичен тому, что бы `arrayEnumerateUniq` дал для того же массива.
...
В этом примере `arrayEnumerateUniqRanked` используется для получения массива, указывающего для каждого элемента многомерного массива, какое у него положение среди элементов с одинаковым значением. Для первой строки переданного массива `[1,2,3]` соответствующий результат — `[1,1,1]`, указывающий, что это первый раз, когда встречаются `1`, `2` и `3`. Для второй строки предоставленного массива `[2,2,1]` соответствующий результат — `[2,3,3]`, указывающий, что `2` встречается во второй и третий раз, а `1` во второй раз. Точно так же, для третьей строки предоставленного массива `[3]` соответствующий результат — `[2]`, указывающий, что `3` встречается во второй раз.
...
Изменяя `clear_depth=2`, результаты будут указывать элементы отдельно для каждой строки.
...
- Если `size` больше начального размера массива, массив расширяется вправо значениями `extender` или значениями по умолчанию для типа данных элементов массива.
...
- `array` – Массив данных.
- `offset` – Смещение от края массива. Положительное значение указывает смещение слева, а отрицательное — смещение справа. Нумерация элементов массива начинается с 1.
- `length` – Длина необходимого среза. Если вы укажете отрицательное значение, функция вернет открытый срез `[offset, array_length - length]`. Если вы не укажете значение, функция вернет срез `[offset, the_end_of_array]`.
...
Элементы массива, установленные в `NULL`, обрабатываются как нормальные значения.
...
Сортирует элементы массива `arr` в порядке возрастания. Если указана функция `func`, порядок сортировки определяется результатом применения функции `func` к элементам массива. Если `func` принимает несколько аргументов, функции `arraySort` передается несколько массивов, которые будут соответствовать аргументам `func`. Подробные примеры приведены в конце описания `arraySort`.
...
- Значения `NaN` находятся сразу перед `NULL`.
- Значения `Inf` находятся сразу перед `NaN`.

Обратите внимание, что `arraySort` является [высшего порядка функции](/sql-reference/functions/overview#higher-order-functions). Вы можете передать ему лямбда-функцию в качестве первого аргумента. В этом случае порядок сортировки определяется результатом применения лямбда-функции к элементам массива.
...
Для каждого элемента исходного массива лямбда-функция возвращает ключ сортировки, то есть \[1 –\> -1, 2 –\> -2, 3 –\> -3\]. Поскольку функция `arraySort` сортирует ключи в порядке возрастания, результатом является \[3, 2, 1\]. Таким образом, лямбда-функция `(x) –> -x` задает [убывающий порядок](#arrayreversesort) при сортировке.

Лямбда-функция может принимать несколько аргументов. В этом случае необходимо передать функции `arraySort` несколько массивов одинаковой длины, которые будут соответствовать аргументам лямбда-функции. Результирующий массив будет состоять из элементов первого входного массива; элементы из следующих входных массивов будут задавать ключи сортировки. Например:
...
Здесь элементы, переданные во втором массиве (\[2, 1\]), определяют ключ сортировки для соответствующего элемента из исходного массива (\['hello', 'world'\]), то есть \['hello' –\> 2, 'world' –\> 1\]. Поскольку лямбда-функция не использует `x`, фактические значения исходного массива не влияют на порядок в результате. Таким образом, 'hello' будет вторым элементом в результате, а 'world' будет первым.
...
:::note
Для повышения эффективности сортировки используется [Schwartzian transform](https://en.wikipedia.org/wiki/Schwartzian_transform).
:::
## arrayPartialSort(\[func,\] limit, arr, ...) {#arraypartialsortfunc-limit-arr-}

Работает аналогично `arraySort`, с дополнительным аргументом `limit`, позволяющим частичную сортировку. Возвращает массив того же размера, что и исходный массив, где элементы в диапазоне `[1..limit]` отсортированы в порядке возрастания. Оставшиеся элементы `(limit..N]` должны содержать элементы в неопределенном порядке.
## arrayReverseSort {#arrayreversesort}

Сортирует элементы массива `arr` в порядке убывания. Если указана функция `func`, массив `arr` сортируется в соответствии с результатом функции `func`, примененной к элементам массива, а затем отсортированный массив обращается. Если `func` принимает несколько аргументов, функции `arrayReverseSort` передаются несколько массивов, которые будут соответствовать аргументам `func`. Подробные примеры приведены в конце описания `arrayReverseSort`.
...
Обратите внимание, что `arrayReverseSort` является [высшего порядка функции](/sql-reference/functions/overview#higher-order-functions). Вы можете передать ему лямбда-функцию в качестве первого аргумента. Пример приведен ниже.
...
2. Массив, полученный на предыдущем шаге, обращается. Таким образом, окончательный результат — \[1, 2, 3\].

Лямбда-функция может принимать несколько аргументов. В этом случае необходимо передать функции `arrayReverseSort` несколько массивов одинаковой длины, которые будут соответствовать аргументам лямбда-функции. Результирующий массив будет состоять из элементов первого входного массива; элементы из следующих входных массивов будут задавать ключи сортировки. Например:
...
2. Массив, который был отсортирован на предыдущем шаге, обращается. Таким образом, окончательный результат — \['hello', 'world'\].
...
Работает аналогично `arrayReverseSort` с дополнительным аргументом `limit`, позволяющим частичную сортировку. Возвращает массив того же размера, что и исходный массив, где элементы в диапазоне `[1..limit]` отсортированы в порядке убывания. Оставшиеся элементы `(limit..N]` должны содержать элементы в неопределенном порядке.
## arrayShuffle {#arrayshuffle}

Возвращает массив того же размера, что и исходный массив, содержащий элементы в перемешанном порядке. Элементы перераспределяются таким образом, что каждая возможная перестановка этих элементов имеет равную вероятность появления.
...
- `arr`: Массив, который необходимо частично перемешать. [Array](/sql-reference/data-types/array).
- `seed` (необязательно): начальное значение, которое будет использовано для генерации случайных чисел. Если не задано, используется случайное значение. [UInt или Int](../data-types/int-uint.md).
...
В этом примере `arrayShuffle` используется без указания `seed`, и поэтому она сама сгенерирует его случайным образом. 
...
Примечание: при использовании [ClickHouse Fiddle](https://fiddle.clickhouse.com/) точный ответ может отличаться из-за случайной природы функции.
...
В этом примере `arrayShuffle` предоставлен `seed`, и она будет производить стабильные результаты.
...
Дан входной массив с кардинальностью `N`, возвращает массив размером N, в котором элементы в диапазоне `[1...limit]` перемешаны, а оставшиеся элементы в диапазоне `(limit...n]` — не перемешаны.
...
- `arr`: Массив размером `N`, который необходимо частично перемешать. [Array](/sql-reference/data-types/array).
- `limit` (необязательно): количество, ограничивающее количество замен элементов, в диапазоне `[1..N]`. [UInt или Int](../data-types/int-uint.md).
- `seed` (необязательно): значение начального значения, которое будет использоваться для генерации случайных чисел. Если не указано, будет использовано случайное. [UInt или Int](../data-types/int-uint.md)
...
Значение `limit` должно находиться в диапазоне `[1..N]`. Значения, выходящие за этот диапазон, эквивалентны выполнению полной [arrayShuffle](#arrayshuffle).
:::
...
Порядок элементов сохранен (`[2,3,4,5], [7,8,9,10]`), за исключением двух перемешанных элементов `[1, 6]`. Поскольку `seed` не предоставлен, функция выбирает его случайным образом.

```response
[6,2,3,4,5,1,7,8,9,10]
```

В этом примере `limit` увеличен до `2`, и задано значение `seed`. Порядок 
...
Порядок элементов сохранен (`[4, 5, 6, 7, 8], [10]`), за исключением четырех перемешанных элементов `[1, 2, 3, 9]`.
...
Если передан один аргумент, он подсчитывает количество различных элементов в массиве. Если передано несколько аргументов, подсчитывает количество различных кортежей элементов в соответствующих позициях в нескольких массивах.
...
Вычисляет массив разностей между соседними элементами массива. Первый элемент результирующего массива будет равен 0, второй — `a[1] - a[0]`, третий — `a[2] - a[1]` и т. д. Тип элементов результирующего массива определяется правилами вывода типов для вычитания (например, `UInt8` - `UInt8` = `Int16`).
...
Возвращает массив разностей между соседними элементами массивов. [UInt\*](/sql-reference/data-types/int-uint#integer-ranges), [Int\*](/sql-reference/data-types/int-uint#integer-ranges), [Float\*](/sql-reference/data-types/float).
...
Возвращает массив такого же размера, как исходный массив, указывая, где каждый элемент впервые появляется в исходном массиве. Позволяет перечислять многомерный массив с возможностью указать, насколько глубоко следует просматривать массив.
...
- `clear_depth`: Перечисление элементов на указанном уровне отдельно. Положительное [Целое число](../data-types/int-uint.md), меньшее или равное `max_arr_depth`.
- `arr`: Н-мерный массив для перечисления. [Array](/sql-reference/data-types/array).
- `max_array_depth`: Максимальная эффективная глубина. Положительное [Целое число](../data-types/int-uint.md), меньшее или равное глубине `arr`.
...
В этом примере `arrayEnumerateDenseRanked` используется для получения массива, указывающего, для каждого элемента многомерного массива, какова его позиция среди элементов одного и того же значения. Для первой строки переданного массива ` [10,10,30,20]` соответствующая первая строка результата — `[1,1,2,3]`, что указывает на то, что `10` — первое число, встреченное на позициях 1 и 2, `30` — второе число, встреченное на позиции 3, а `20` — третье число, встреченное на позиции 4. Для второй строки `[40, 50, 10, 30]` соответствующая вторая строка результата — `[4,5,1,2]`, что указывает на то, что `40` и `50` — четвертое и пятое числа, встреченные на позициях 1 и 2 этой строки, что еще одно `10` (первое встреченное число) находится на позиции 3, а `30` (второе встреченное число) — на последней позиции.
...
Изменяя `clear_depth=2`, нумерация выполняется отдельно для каждой строки заново.
...
Принимает несколько массивов и возвращает массив, содержащий элементы, которые присутствуют во всех исходных массивах. Результат содержит только уникальные значения.
...
- [Array](/sql-reference/data-types/array) с уникальными элементами, которые отсутствуют во всех исходных массивах.
...
Применяет агрегатную функцию к элементам массива и возвращает ее результат. Название функции агрегации передается как строка в одинарных кавычках `'max'`, `'sum'`. При использовании параметрических агрегатных функций параметр указывается после названия функции в круглых скобках `'uniqUpTo(6)'`.
...
- `agg_func` — Название агрегатной функции, которая должна быть константной [строкой](../data-types/string.md).
- `arr` — любое количество столбцов типа [array](/sql-reference/data-types/array) в качестве параметров функции агрегации.
...
- Массив, содержащий результаты функции агрегации для указанных диапазонов. [Array](/sql-reference/data-types/array).
...
- Применяется к любой глубине вложенных массивов.
- Не изменяет массивы, которые уже являются плоскими.

Развёрнутый массив содержит все элементы из всех исходных массивов.
...
Удаляет последовательные дублирующиеся элементы из массива. Порядок результатов определяется порядком в исходном массиве.
...
`arr` — Массив для проверки. [Array](/sql-reference/data-types/array).
...
Объединяет несколько массивов в один массив. Р resulting массив содержит соответствующие элементы исходных массивов, сгруппированные в кортежи в указанном порядке аргументов.
...
Функция может принимать любое количество массивов различных типов. Все входные массивы должны иметь одинаковый размер.

**Возвращаемое значение**

- Массив с элементами из исходных массивов, сгруппированными в [кортежи](../data-types/tuple.md). Типы данных в кортеже такие же, как типы входных массивов и в том же порядке, в каком массивы передаются. [Array](/sql-reference/data-types/array).
...
Объединяет несколько массивов в один массив, позволяя невыравненные массивы. Р resulting массив содержит соответствующие элементы исходных массивов, сгруппированные в кортежи в указанном порядке аргументов.
...
- Массив с элементами из исходных массивов, сгруппированными в [кортежи](../data-types/tuple.md). Типы данных в кортеже такие же, как типы входных массивов и в том же порядке, в каком массивы передаются. [Array](/sql-reference/data-types/array). Если массивы имеют разные размеры, более короткие массивы будут дополнены значениями `null`.
...
Вычисляет площадь под кривой приема-сигнала (ROC). Кривая ROC создается путем построения отношения истинно положительных (TPR) по оси y и ложно положительных (FPR) по оси x для всех порогов. Полученное значение колеблется от 0 до 1, где более высокое значение указывает на более высокую эффективность модели. ROC AUC (также известный просто как AUC) является концепцией в машинном обучении. Для получения дополнительной информации, пожалуйста, смотрите [здесь](https://developers.google.com/machine-learning/glossary#pr-auc-area-under-the-pr-curve), [здесь](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc#expandable-1) и [здесь](https://en.wikipedia.org/wiki/Receiver_operating_characteristic#Area_under_the_curve).
...
- `arr_scores` — Оценки, которые дает модель предсказания. [Array](/sql-reference/data-types/array) целых [чисел](../data-types/int-uint.md) или [чисел с плавающей точкой](../data-types/float.md).
...
- `scale` — Определяет, следует ли возвращать нормализованную площадь. Если `false`, возвращает площадь под кривой TP (истинные положительные) x FP (ложные положительные). Значение по умолчанию: `true`. [Bool](../data-types/boolean.md). Необязательный параметр.
- `arr_partial_offsets` — Массив из четырех ненегативных целых чисел для вычисления частичной площади под кривой ROC (эквивалентно вертикальной полосе в ROC-пространстве), вместо всей AUC. Эта опция полезна для распределенного вычисления ROC AUC. Массив должен содержать следующие элементы [`higher_partitions_tp`, `higher_partitions_fp`, `total_positives`, `total_negatives`]. [Array](/sql-reference/data-types/array) ненегативных [целых чисел](../data-types/int-uint.md). Необязательный параметр.
    - `higher_partitions_tp`: количество положительных меток в более высоко оцененных разделах.
    - `higher_partitions_fp`: количество отрицательных меток в более высоко оцененных разделах.
    - `total_positives`: общее число положительных образцов во всей выборке.
    - `total_negatives`: общее число отрицательных образцов во всей выборке.

::::note
При использовании `arr_partial_offsets`, `arr_scores` и `arr_labels` должны составлять только часть всей выборки, содержащей интервал оценок. Данные должны быть разделены на смежные разделы, где каждый раздел содержит подмножество данных, чьи оценки попадают в определенный диапазон. Например:
...
Возвращает площадь под кривой приема-сигнала (ROC). [Float64](../data-types/float.md).
...
Кривая точности-отзыва создается путем построения точности по оси y и отзыва по оси x для всех порогов.
Результирующее значение варьируется от 0 до 1, причем более высокое значение указывает на лучшую производительность модели.
PR AUC особенно полезен для несбалансированных наборов данных, обеспечивая более четкое сравнение производительности по сравнению с ROC AUC в этих случаях.
...
- `arr_scores` — Оценки, которые получает предсказательная модель. [Массив](/sql-reference/data-types/array) [Целых чисел](../data-types/int-uint.md) или [чисел с плавающей запятой](../data-types/float.md).
- `arr_labels` — Метки образцов, обычно 1 для положительного образца и 0 для отрицательного. [Массив](/sql-reference/data-types/array) [Целых чисел](../data-types/int-uint.md) или [Перечислений](../data-types/enum.md).
- `arr_partial_offsets` — Опционально. [Массив](/sql-reference/data-types/array) из трех неотрицательных целых чисел для вычисления частичной площади под кривой PR (эквивалент вертикальной полосы пространства PR) вместо всей AUC. Эта опция полезна для распределенных расчетов PR AUC. Массив должен содержать следующие элементы [`higher_partitions_tp`, `higher_partitions_fp`, `total_positives`]. [Массив](/sql-reference/data-types/array) неотрицательных [Целых чисел](../data-types/int-uint.md). Опционально.
    - `higher_partitions_tp`: Количество положительных меток в более высоко оцененных партициях.
    - `higher_partitions_fp`: Количество отрицательных меток в более высоко оцененных партициях.
...
Набор данных должен быть разделен на смежные партиции, каждая из которых содержит подмножество данных, оценки которых находятся в определенном диапазоне.
Например:
...
Возвращает массив, полученный из оригинальных массивов путем применения `func(arr1[i], ..., arrN[i])` для каждого элемента. Массивы `arr1` ... `arrN` должны иметь одинаковое количество элементов.
...
Просматривает `arr1` от первого элемента до последнего и заменяет `arr1[i]` на `arr1[i - 1]`, если `func(arr1[i], ..., arrN[i])` возвращает 0. Первый элемент `arr1` не будет заменен.
...
Просматривает `arr1` от последнего элемента до первого элемента и заменяет `arr1[i]` на `arr1[i + 1]`, если `func(arr1[i], ..., arrN[i])` возвращает 0. Последний элемент `arr1` не будет заменен.
...
Возвращает 1, если есть хотя бы один элемент в `arr`, для которого `func(arr1[i], ..., arrN[i])` возвращает что-то, отличное от 0. В противном случае возвращает 0.
...
- Первый элемент переданного массива.
...
- Последний элемент переданного массива.
- В противном случае возвращает `NULL`
...
Если функция `func` указана, возвращает минимум элементов, преобразованных этой функцией.
...
:::note
Если `func` указан, то возвращаемый тип соответствует типу возвращаемого значения `func`, иначе он соответствует типу элементов массива.
:::
...
Если функция `func` указана, возвращает максимум элементов, преобразованных этой функцией.
...
Если функция `func` указана, возвращает сумму элементов, преобразованных этой функции.
...
Если функция `func` указана, возвращает среднее значение элементов, преобразованных этой функцией.
...
Возвращает массив частичных (пробегаемых) сумм элементов в исходном массиве `arr1`. Если `func` указана, то сумма считается путем применения `func` к `arr1`, `arr2`, ..., `arrN`, т.е. `func(arr1[i], ..., arrN[i])`.
...
Аналогично `arrayCumSum`, возвращает массив частичных (пробегаемых) сумм элементов в исходном массиве. Если `func` указана, то сумма считается путем применения `func` к `arr1`, `arr2`, ..., `arrN`, т.е. `func(arr1[i], ..., arrN[i])`. В отличие от `arrayCumSum`, если текущая пробегаемая сумма меньше 0, она заменяется на 0.
...
Умножает элементы массива [массив](/sql-reference/data-types/array).
...
Поворачивает [массив](/sql-reference/data-types/array) влево на указанный номер элементов.
...
Поворачивает [массив](/sql-reference/data-types/array) вправо на указанный номер элементов.
...
Сдвигает [массив](/sql-reference/data-types/array) влево на указанное число элементов.
...
Сдвигает [массив](/sql-reference/data-types/array) вправо на указанное число элементов.
Новые элементы заполняются предоставленным аргументом или значением по умолчанию для типа элемента массива.
...
Функция `arrayRandomSample` возвращает подмножество с `samples`-числом случайных элементов исходного массива. Если `samples` превышает размер исходного массива, размер выборки ограничивается размером массива, т.е. возвращаются все элементы массива, но их порядок не гарантируется. Функция может обрабатывать как плоские массивы, так и вложенные массивы.
...
- `arr` — Исходный массив, из которого будут выборку элементы. ([Array(T)](/sql-reference/data-types/array))
- `samples` — Количество элементов, включаемых в случайную выборку ([UInt*](../data-types/int-uint.md))

**Возвращаемое значение**

- Массив, содержащий случайную выборку элементов из исходного массива. [Array](/sql-reference/data-types/array).
