...
Вычисляет внутреннюю норму доходности (IRR) для серии денежных потоков, возникающих через регулярные интервалы.
IRR — это ставка дисконтирования, при которой чистая приведённая стоимость (NPV) равна нулю.

IRR решает следующее уравнение:
...
Возвращает внутреннюю норму доходности или `NaN`, если вычисление не сходится, входной массив пуст или содержит только один элемент, все денежные потоки равны нулю или возникают другие ошибки вычисления. [`Float64`](/sql-reference/data-types/float)
...
Вычисляет расширенную внутреннюю норму доходности (XIRR) для серии денежных потоков, происходящих через нерегулярные интервалы. XIRR — это ставка дисконтирования, при которой чистая приведённая стоимость (NPV) всех денежных потоков равна нулю.
...
- `[, guess]` — Необязательный параметр. Начальное приближение (константное значение) для вычисления XIRR. [`Float*`](/sql-reference/data-types/float)
- `[, daycount]` —
...
Вычисляет чистую приведённую стоимость (NPV) серии денежных потоков при условии равных временных интервалов между каждым денежным потоком.
...
- `rate` — Ставка дисконтирования. [`Float*`](/sql-reference/data-types/float)
- `cashflows` — Массив денежных потоков. Каждое значение представляет собой платёж (отрицательное значение) или доход (положительное значение). [`Array(Int8/16/32/64)`](/sql-reference/data-types/array) или [`Array(Float*)`](/sql-reference/data-types/array)
- `[, start_from_zero]` — Необязательный логический параметр, указывающий, начинать ли расчёт NPV с периода `0` (true) или с периода `1` (false, совместимо с Excel). По умолчанию: true. [`Bool`](/sql-reference/data-types/boolean)
