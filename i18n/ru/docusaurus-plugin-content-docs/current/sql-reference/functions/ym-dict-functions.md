---
description: 'Документация для функций работы со встроенными словарями'
...
Таким образом, примеры, приведенные в этом разделе, будут вызывать исключение в [ClickHouse Fiddle](https://fiddle.clickhouse.com/) и в развертываниях быстрого релиза и производства по умолчанию, если не будут предварительно настроены.
:::

Для получения информации о создании списков ссылок смотрите раздел ["Словари"](../dictionaries#embedded-dictionaries).
...
Все словари перезагружаются во время выполнения (раз в определенное количество секунд, как определено в параметре конфигурации [`builtin_dictionaries_reload_interval`](/operations/server-configuration-parameters/settings#builtin_dictionaries_reload_interval), или раз в час по умолчанию). Однако список доступных словарей определяется один раз, когда сервер запускается.
...
Преобразует регион в область (тип 5 в геобазе). Во всех остальных отношениях эта функция такая же, как ['regionToCity'](#regiontocity).
...
Получает население региона. Население может быть записано в файлы с геобазой. Смотрите раздел ["Словари"](../dictionaries#embedded-dictionaries). Если население не записано для региона, возвращает 0. В геобазе население может быть записано для дочерних регионов, но не для родительских регионов.
