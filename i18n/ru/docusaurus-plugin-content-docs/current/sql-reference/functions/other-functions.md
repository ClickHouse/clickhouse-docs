...
Возвращает идентификатор сборки (build ID), сгенерированный компилятором для запущенного бинарного файла сервера ClickHouse.
...
Входной цвет задаётся в цветовом пространстве OKLab. Если входные значения находятся вне
...
* L: перцептивная светлота (обычно в диапазоне [0..1])
  * a: зелёно-красная оппонентная ось
  * b: жёлто-синяя оппонентная ось

Компоненты a и b теоретически не ограничены, но на практике лежат в диапазоне от -0.4 до 0.4.
OKLab разработан как перцептивно равномерное пространство,
при этом остаётся вычислительно недорогим.

Преобразование предназначено быть обратным к colorSRGBToOKLAB и состоит из
следующих этапов:

1. Преобразование из OKLab в линейный sRGB.
   2) Преобразование из линейного sRGB в гамма-кодированный sRGB.

Необязательный аргумент gamma задаёт показатель степени, используемый при преобразовании из линейного
sRGB в гамма-кодированные значения RGB. Если параметр не указан, используется значение gamma
по умолчанию для согласованности с colorSRGBToOKLAB.
...
:::note
**OKLAB** — это перцептивно равномерное цветовое пространство.
Его три координаты — `L` (светлота в диапазоне `[0...1]`), `a (ось Green-Red)` и `b (ось Blue-Yellow)`.
OKLab спроектировано как перцептивно равномерное, при этом остаётся вычислительно недорогим.
:::
...
Возвращает кортеж (L, a, b), представляющий значения в цветовом пространстве OKLAB. [`Tuple(Float64, Float64, Float64)`](/sql-reference/data-types/tuple)
...
При анализе индекса предполагается, что аргумент этой функции не обёрнут в `indexHint`.
...
<details>
  ...
  1. Использует индекс, чтобы определить, какие гранулы (блоки примерно по 8192 строки) могут содержать `key = 123`.
  2. Читает эти гранулы и построчно фильтрует их, возвращая только строки, где `key = 123`.

  То есть, даже если с диска читается 8192 строки, возвращается только одна строка, которая действительно совпадает с условием.

  При использовании `indexHint`, когда вы выполняете:
  ...
  1. Использует индекс, чтобы найти гранулы, которые могут содержать `key = 123`, и возвращает все строки из этих гранул **без** фильтрации.

  Возвращаются все 8192 строки, включая строки, где `key = 456`, `key = 789` и т. д. (всё, что оказалось сохранено в той же грануле).
  ...
  * Какие гранулы выбираются моим условием?
  * Сколько строк в этих гранулах?
  * Насколько эффективно используется мой индекс?
</details>

Примечание: оптимизировать запрос с помощью функции `indexHint` невозможно. Функция `indexHint` не оптимизирует запрос, так как не предоставляет никакой дополнительной информации для анализа запроса. Наличие выражения внутри функции `indexHint` никак не лучше, чем без функции `indexHint`. Функция `indexHint` может использоваться только для целей интроспекции и отладки и не улучшает производительность. Если вы видите использование `indexHint` кем-либо, кроме участников команды ClickHouse, скорее всего, это ошибка, и её следует удалить.
...
Возвращает номер TCP-порта [native интерфейса](/interfaces/tcp), на котором слушает сервер.
Если выполняется в контексте distributed таблицы, эта функция генерирует обычный столбец со значениями, соответствующими каждому сегменту.
В противном случае она возвращает константное значение.
