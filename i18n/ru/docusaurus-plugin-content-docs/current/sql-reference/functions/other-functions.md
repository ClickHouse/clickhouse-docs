...
Эта функция полезна для анонимизации запросов перед логированием или передачей для отладки.
...
используется в запросе несколько раз. Это гарантирует, что каждый вызов даст отдельный обфусцированный результат.
...
* `query` — SQL-запрос, который нужно обфусцировать. [`String`](/sql-reference/data-types/string)
* `tag` — Необязательно. Значение, позволяющее предотвратить устранение общих подвыражений, если один и тот же вызов функции используется несколько раз.

**Возвращаемое значение**

Обфусцированный запрос, в котором заменены идентификаторы и литералы, при этом сохраняется исходная структура запроса. [`String`](/sql-reference/data-types/string)
...
**С тегом, предотвращающим устранение общих подвыражений**
...
Выполняет обфускацию SQL-запроса с использованием указанного seed для получения детерминированных результатов.

В отличие от `obfuscateQuery()`, эта функция возвращает детерминированные результаты при использовании одного и того же seed.
Это полезно, когда нужна единообразная обфускация при нескольких запусках или когда требуется
...
* Единообразная анонимизация при нескольких запусках
* Отладка с использованием единообразно обфусцированных запросов
...
* `query` — SQL-запрос, который нужно обфусцировать. [`String`](/sql-reference/data-types/string)
* `seed` — Начальное значение для обфускации. Одно и то же значение `seed` даёт детерминированные результаты. [`Integer`](/sql-reference/data-types/int-uint) или [`String`](/sql-reference/data-types/string)

**Возвращаемое значение**

Обфусцированный запрос, детерминированно сгенерированный на основе указанного значения `seed`. [`String`](/sql-reference/data-types/string)
...
**Одинаковое начальное значение даёт одинаковый результат**
