...
Возвращает названное значение из раздела [macros](../../operations/server-configuration-parameters/settings.md#macros) конфигурации сервера.
...
- `name` — Имя макроса, которое нужно извлечь из секции `<macros>`. [String](/sql-reference/data-types/string).
...
- Строка с полным доменным именем. [String](../data-types/string.md).
...
- `expr` — Значение типа [String](../data-types/string.md). Обратные косые черты должны быть экранированы.
...
Вычисляет предполагаемую ширину при выводе значений в текстовом формате (разделенном табуляцией). Эта функция используется системой для реализации [Pretty formats](../../interfaces/formats.md).
...
В ClickHouse запросы обрабатываются в [блоках](/development/architecture#block) (кусках). Эта функция возвращает размер (количество строк) блока, на котором вызывается функция.
...
- `seconds`: [UInt*](../data-types/int-uint.md) или [Float](../data-types/float.md) Количество секунд, на которое нужно приостановить выполнение запроса, максимум 3 секунды. Это может быть значение с плавающей точкой для указания дробных секунд.
...
- `seconds`: [UInt*](../data-types/int-uint.md) или [Float*](../data-types/float.md) Количество секунд, на которое нужно приостановить выполнение запроса для каждой строки в результирующем наборе, максимум 3 секунды. Это может быть значение с плавающей точкой для указания дробных секунд.
...
Как и у функции [`sleep()`](#sleep), важно использовать `sleepEachRow()` с умом и только когда это необходимо, так как это может значительно повлиять на общую производительность и отзывчивость вашей системы ClickHouse, особенно при работе с большими наборами результатов.
...
:::note
Логический аргумент игнорируется. Он только существует для совместимости с [реализацией](https://www.postgresql.org/docs/7.3/functions-misc.html) этой функции в PostgreSQL.
:::
...
Постоянное выражение — это выражение, результат которого известен во время анализа запроса, т.е. до выполнения. Например, выражения над [литералами](../../sql-reference/syntax.md#literals) являются постоянными выражениями.
...
- `0`, если `x` неконстантный. [UInt8](../data-types/int-uint.md).
...
- `column` : имя столбца. [String literal](/sql-reference/syntax#string)
...
Преобразует значение в соответствии с явно определенной сопоставлением некоторых элементов с другими.
...
SELECT
    ...
    formatReadableTimeDelta(elapsed, 'minutes', 'наносекунды') AS time_delta
```
...
│                100 │ 1 minute and 40 seconds                        │
│              12345 │ 205 minutes and 45 seconds                     │
│ 432546534.00000006 │ 7209108 minutes, 54 seconds and 60 наносекунд │
└────────────────────┴────────────────────────────────────────────────┘
```
## parseReadableSize {#parsereadablesize}

Учитывая строку, содержащую размер в байтах и `B`, `KiB`, `KB`, `MiB`, `MB` и т.д. в качестве единицы (т.е. [ISO/IEC 80000-13](https://en.wikipedia.org/wiki/ISO/IEC_80000) или десятичная байтовая единица), эта функция возвращает соответствующее количество байт.  
...
- `x` : Читаемый размер с единицей ISO/IEC 80000-13 или десятичной байт. ([String](../../sql-reference/data-types/string.md)).

**Возвращаемое значение**

- Количество байт, округленное до ближайшего целого числа ([UInt64](../../sql-reference/data-types/int-uint.md)).
...
- Количество байт, округленное до ближайшего целого числа, или NULL, если не удалось разобрать входное значение (Nullable([UInt64](../../sql-reference/data-types/int-uint.md))).
...
Учитывая строку, содержащую размер в байтах и `B`, `KiB`, `KB`, `MiB`, `MB` и т.д. в качестве единицы (т.е. [ISO/IEC 80000-13](https://en.wikipedia.org/wiki/ISO/IEC_80000) или десятичная байтовая единица), эта функция возвращает соответствующее количество байт. Если функция не может разобрать входное значение, она возвращает `0`.
...
- Количество байт, округленное до ближайшего целого числа, или 0, если не удалось разобрать входное значение ([UInt64](../../sql-reference/data-types/int-uint.md)).
...
:::note
Версия [24.12](/whats-new/changelog/2024#a-id2412a-clickhouse-release-2412-2024-12-19) внесла изменения, несовместимые с предыдущими версиями, так что значения `NULL` игнорируются, в то время как ранее возвращалось `NULL`, если один из аргументов был `NULL`. Чтобы сохранить предыдущее поведение, установите настройку `least_greatest_legacy_null_behavior` (по умолчанию: `false`) в `true`. 
:::
## greatest {#greatest}
...
Возвращает идентификатор сборки, сгенерированный компилятором для работающего бинарного файла сервера ClickHouse.
Если выполняется в контексте распределенной таблицы, эта функция генерирует обычный столбец со значениями, относящимися к каждой шард. В противном случае она возвращает постоянное значение.
...
Возвращаемый номер блока обновляется на основе усилий, т.е. он может быть не полностью точным.
...
- Порядковый номер блока данных, в котором находится строка. [UInt64](../data-types/int-uint.md).
...
Возвращает для каждого [блока](../../development/architecture.md#block), обрабатываемого `rowNumberInBlock`, номер текущей строки.
...
Заменяет литералы, последовательности литералов на плейсхолдер `?`, но не заменяет сложные псевдонимы (содержащие пробелы, больше двух цифр
или длиной как минимум 36 байт, таких как UUID). Это помогает лучше анализировать сложные журналы запросов.
...
- Значение хеша. [UInt64](/sql-reference/data-types/int-uint#integer-ranges).
...
Как [normalizedQueryHash](#normalizedqueryhash), возвращает идентичные 64-битные хеш-значения без значений литералов для схожих запросов, но не заменяет сложные псевдонимы (содержащие пробелы, больше двух цифр
или длиной как минимум 36 байт, такие как UUID) на плейсхолдер перед хешированием. Может быть полезно для анализа журналов запросов.
...
Из-за этого ненадежного поведения функция DEPRECATED, рекомендуется использовать соответствующие функции окон вместо этого.
:::

Порядок строк во время вычисления `neighbor()` может отличаться от порядка строк, возвращаемых пользователю.
Чтобы предотвратить это, вы можете создать подзапрос с [ORDER BY](../../sql-reference/statements/select/order-by.md) и вызвать функцию извне подзапроса.

**Аргументы**

- `column` — Имя столбца или скалярное выражение.
- `offset` — Количество строк, которое нужно посмотреть до или после текущей строки в `column`. [Int64](../data-types/int-uint.md).
...
Вызывает разницу между двумя последовательными значениями строк в блоке данных.
...
Порядок строк во время вычисления `runningDifference()` может отличаться от порядка строк, возвращаемых пользователю.
Чтобы предотвратить это, вы можете создать подзапрос с [ORDER BY](../../sql-reference/statements/select/order-by.md) и вызвать функцию извне подзапроса.
...
Та же, что и [runningDifference](/sql-reference/functions/other-functions#runningDifference), но возвращает значение первой строки как значение для первой строки.
## runningConcurrency {#runningconcurrency}

Вычисляет количество параллельных событий.
Каждое событие имеет время начала и окончания. Время начала включено в событие, в то время как время окончания исключено. Столбцы с временем начала и окончания должны быть одного и того же типа данных.
...
- Количество параллельных событий в каждое время начала события. [UInt32](../data-types/int-uint.md)
...
Исключение выбрасывается, если тип не `Enum`.
...
В этом примере функция `indexHint` позволяет увидеть смежные даты.
...
Возвращает количество свободного места в файловой системе, хранящей постоянные данные базы данных. Возвращаемое значение всегда меньше общего свободного места ([filesystemUnreserved](#filesystemunreserved)), так как часть пространства зарезервирована для операционной системы.
...
Возвращает общее количество свободного пространства на файловой системе, хранящей постоянные данные базы данных. (ранее `filesystemFree`). См. также [`filesystemAvailable`](#filesystemavailable).
...
Возвращает емкость файловой системы в байтах. Необходим [путь](../../operations/server-configuration-parameters/settings.md#path) к каталогу данных.
...
- `aggregate_function` — Название агрегатной функции для инициализации. [String](../data-types/string.md).
...
Учитывая состояние агрегатной функции, эта функция возвращает результат агрегации (или завершенное состояние при использовании комбинатора [-State](/sql-reference/aggregate-functions/combinators#-state)).
...
- `grouping` — Ключ группировки. Необязательно. Состояние функции сбрасывается, если значение `grouping` изменяется. Это может быть любым из [поддерживаемых типов данных](../data-types/index.md), для которых определен оператор равенства.
...
Функция позволяет извлекать данные из таблицы так же, как из [словаря](../../sql-reference/dictionaries/index.md). Получает данные из таблиц [Join](../../engines/table-engines/special/join.md#creating-a-table) с использованием указанного ключа соединения.
...
- `join_storage_table_name` — [идентификатор](/sql-reference/syntax#identifiers), указывающий, где производится поиск.
...
:::note
Если конкретный ключ отсутствует в исходной таблице, то будет возвращено `0` или `null` в зависимости от настройки [join_use_nulls](../../operations/settings/settings.md#join_use_nulls) во время создания таблицы. Более подробную информацию о `join_use_nulls` см. в [операции Join](../../engines/table-engines/special/join.md).
:::
...
Как [joinGet](#joinget), но возвращает `NULL`, когда ключ отсутствует, вместо возвращения значения по умолчанию.
...
Оценка внешней модели CatBoost. [CatBoost](https://catboost.ai) — это библиотека градиентного бустинга с открытым исходным кодом, разработанная Яндексом для машинного обучения. Принимает путь к модели CatBoost и аргументы модели (признаки). Возвращает Float64.
...
По соображениям безопасности и изоляции оценка модели не выполняется в процессе сервера, а в процессе моста clickhouse-library-bridge. При первом выполнении `catboostEvaluate()`, сервер запускает процесс библиотеки до того, как его еще нет. Оба процесса
взаимодействуют с помощью HTTP интерфейса. Порт `9012` используется по умолчанию. Другой порт можно задать следующим образом - это полезно, если порт
...
См. [Обучение и применение моделей](https://catboost.ai/docs/features/training.html#training) о том, как обучать модели CatBoost на основе обучающего набора данных.
## throwIf {#throwif}
...
Code: 395. DB::Exception: Получено от localhost:9000. DB::Exception: Слишком много.
```
## identity {#identity}
...
- `p` — точность. Необязательно. Если опущено, используется начальная точность первого аргумента. Этот параметр может быть полезен для миграции данных из/в другую базу данных или файл. [UInt8](/sql-reference/data-types/int-uint#integer-ranges).
...
Возвращает [номер порта TCP](../../interfaces/tcp.md) интерфейса, слушаемого этим сервером. Если выполнен в контексте распределенной таблицы, эта функция генерирует обычный столбец со значениями, относящимися к каждому шардированию. В противном случае она выдает константное значение.
...
- Список активных профилей настроек. [Array](../data-types/array.md)([String](../data-types/string.md)).
...
Возвращает роли, назначенные текущему пользователю. Роли могут быть изменены оператором [SET ROLE](/sql-reference/statements/set-role). Если оператор `SET ROLE` не использовался, функция `currentRoles` возвращает то же самое, что и `defaultRoles`.
...
- Список текущих ролей текущего пользователя. [Array](../data-types/array.md)([String](../data-types/string.md)).
...
- Список активных ролей для текущего пользователя. [Array](../data-types/array.md)([String](../data-types/string.md)).
## defaultRoles {#defaultroles}

Возвращает роли, которые включены по умолчанию для текущего пользователя при входе. Изначально это все роли, предоставленные текущему пользователю (см. [GRANT](../../sql-reference/statements/grant.md#select)), но это может быть изменено оператором [SET DEFAULT ROLE](/sql-reference/statements/set-role#set-default-role).
...
Возвращает ID текущего запроса. Другие параметры запроса могут быть извлечены из таблицы [system.query_log](../../operations/system-tables/query_log.md) через `query_id`.

В отличие от функции [initialQueryID](#initialqueryid), `queryID` может возвращать разные результаты на разных шардерах (см. пример).
...
Возвращает ID первоначального текущего запроса. Другие параметры запроса могут быть извлечены из таблицы [system.query_log](../../operations/system-tables/query_log.md) через `initial_query_id`.

В отличие от функции [queryID](/sql-reference/functions/other-functions#queryid), `initialQueryID` возвращает одни и те же результаты на разных шардерах (см. пример).
...
- ID первоначального текущего запроса. [String](../data-types/string.md)
...
- Время начала первоначального текущего запроса. [DateTime](../data-types/datetime.md)
...
Вычисляет [ID раздела](../../engines/table-engines/mergetree-family/custom-partitioning-key.md).
...
- ID раздела, к которому принадлежит строка. [String](../data-types/string.md).
...
- Индекс шардера или константное значение `0`. [UInt32](../data-types/int-uint.md).

**Пример**

В следующем примере используется конфигурация с двумя шардерами. Запрос выполняется на таблице [system.one](../../operations/system-tables/one.md) на каждом шардировании.
...
```
```yaml
title: 'сгенерироватьСлучайнуюСтруктуру'
sidebar_label: 'сгенерироватьСлучайнуюСтруктуру'
keywords: ['сгенерироватьСлучайнуюСтруктуру', 'ClickHouse']
description: 'Генерирует случайную структуру таблицы в формате `column1_name column1_type, column2_name column2_type, ...`.'
```
...
- `type_name` — Имя варианта типа для извлечения. [String](../data-types/string.md).
...
Использует формулу, описанную в [этом статье](https://towardsdatascience.com/required-sample-size-for-a-b-testing-6f6608dd330a). Предполагает равные размеры групп обработки и контроля. Возвращает размер выборки, необходимый для одной группы (т.е. размер выборки, необходимый для всего эксперимента, составляет вдвое больше возвращаемого значения).

**Аргументы**

- `baseline` — Базовая конверсия. [Float](../data-types/float.md).
- `mde` — Минимально обнаруживаемый эффект (MDE) в процентных пунктах (например, для базовой конверсии 0.25 MDE 0.03 означает ожидаемое изменение до 0.25 ± 0.03). [Float](../data-types/float.md).
...
- `"detect_range_lower"` — Нижний предел диапазона значений, которые нельзя обнаружить с возвращаемым необходимым размером выборки (т.е. все значения, меньшие или равные `"detect_range_lower"`, можно обнаружить с предоставленным `alpha` и `power`). Рассчитывается как `baseline - mde`. [Float64](../data-types/float.md).
- `"detect_range_upper"` — Верхний предел диапазона значений, которые нельзя обнаружить с возвращаемым необходимым размером выборки (т.е. все значения, большие или равные `"detect_range_upper"`, можно обнаружить с предоставленным `alpha` и `power`). Рассчитывается как `baseline + mde`. [Float64](../data-types/float.md).
...
- `mde` — Минимально обнаруживаемый эффект (MDE) в процентах от базового значения (например, для базового значения 112.25 MDE 0.03 означает ожидаемое изменение до 112.25 ± 112.25\*0.03). [Integer](../data-types/int-uint.md) или [Float](../data-types/float.md).
- `power` — Необходимая статическая мощность теста (1 - вероятность ошибки второго рода). [Integer](../data-types/int-uint.md) или [Float](../data-types/float.md).
...
- `"minimum_sample_size"` — Необходимый размер выборки. [Float64](../data-types/float.md).
- `"detect_range_lower"` — Нижний предел диапазона значений, которые нельзя обнаружить с возвращаемым необходимым размером выборки (т.е. все значения, меньшие или равные `"detect_range_lower"`, можно обнаружить с предоставленным `alpha` и `power`). Рассчитывается как `baseline * (1 - mde)`. [Float64](../data-types/float.md).
- `"detect_range_upper"` — Верхний предел диапазона значений, которые нельзя обнаружить с возвращаемым необходимым размером выборки (т.е. все значения, большие или равные `"detect_range_upper"`, можно обнаружить с предоставленным `alpha` и `power`). Рассчитывается как `baseline * (1 + mde)`. [Float64](../data-types/float.md).
...
Показывает информацию о текущем сертификате безопасности Secure Sockets Layer (SSL) сервера, если он был настроен. См. [Настройка SSL-TLS](/guides/sre/configuring-ssl) для получения дополнительной информации о том, как настроить ClickHouse для использования сертификатов OpenSSL для проверки соединений.
...
- Карта пар ключ-значение, относящаяся к настроенному SSL-сертификату. [Map](../data-types/map.md)([String](../data-types/string.md), [String](../data-types/string.md)).
...
Возвращает позицию значения в словаре [LowCardinality](../data-types/lowcardinality.md) столбца. Позиции начинаются с 1. Поскольку LowCardinality имеют словари по частям, эта функция может возвращать разные позиции для одного и того же значения в разных частях.
...
- `col` — столбец с низкой кардинальностью. [LowCardinality](../data-types/lowcardinality.md).
...
Возвращает значения словаря [LowCardinality](../data-types/lowcardinality.md) столбца. Если блок меньше или больше размера словаря, результат будет обрезан или расширен с использованием значений по умолчанию. Поскольку LowCardinality имеют словари по частям, эта функция может возвращать разные значения словаря в разных частях.
...
Возвращает значение `display_name` из [конфигурации](/operations/configuration-files) или полное квалифицированное доменное имя (FQDN) сервера, если оно не установлено.
...
<!-- Это имя, которое будет отображаться в clickhouse-client.
     По умолчанию все, что содержит "production", будет выделено красным в командной строке запросов.
-->
...
Возвращает идентификатор [транзакции](/guides/developer/transactional#transactions-commit-and-rollback).
...
- `start_csn`: Глобальный последовательный номер, самая новая метка времени коммита, которая была видна, когда началась эта транзакция. [UInt64](../data-types/int-uint.md).
...
Возвращает самый старый снимок (Commit Sequence Number), который виден для некоторых запущенных [транзакции](/guides/developer/transactional#transactions-commit-and-rollback).
