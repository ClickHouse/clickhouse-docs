...
Классифицирует входной текст с использованием модели Naive Bayes с n-граммами и сглаживанием Лапласа. Перед использованием модель должна быть настроена в ClickHouse.
...
При `n=3` и в режиме `codepoint` это может выглядеть так:
...
Человекочитаемый формат ClickHouse напрямую не использует; его необходимо преобразовать в бинарный формат, описанный ниже.

**Подробности бинарного формата**
...
Перед созданием модели на основе корпуса документов необходимо предварительно обработать документы, чтобы извлечь n-граммы в соответствии с указанными `mode` и `n`. Ниже приведены шаги предварительной обработки:
...
1. **Добавьте маркеры границ в начало и конец каждого документа в зависимости от режима токенизации:**
   ...
   *Примечание:* В начало и конец документа добавляются `(n - 1)` токенов.
