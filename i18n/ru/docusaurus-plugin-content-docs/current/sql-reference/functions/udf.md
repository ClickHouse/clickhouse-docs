...
| `send_chunk_header`           | Определяет, нужно ли отправлять количество строк перед отправкой фрагмента данных в процесс                                                                                                                                                                                                                                                                                                                                                                                  | Optional | `false`                   |
...
| `stderr_reaction`             | Способ обработки вывода команды в stderr. Значения: `none` (игнорировать), `log` (немедленно записывать в журнал весь stderr), `log_first` (записывать в журнал первые 4 КиБ после завершения), `log_last` (записывать в журнал последние 4 КиБ после завершения), `throw` (немедленно сгенерировать исключение при любом выводе в stderr). При использовании `log_first` или `log_last` с ненулевым кодом завершения содержимое stderr включается в сообщение об исключении | Optional | `log_last`                |
| `check_exit_code`             | Если установлено значение true, ClickHouse будет проверять код завершения команды. Ненулевой код завершения вызывает исключение                                                                                                                                                                                                                                                                                                                                              | Optional | `true`                    |

Команда должна читать аргументы из `STDIN` и выводить результат в `STDOUT`. Команда должна обрабатывать аргументы итеративно, то есть после обработки фрагмента аргументов она должна ожидать следующий фрагмент.
