...
- `attr_names` — Название колонки словаря, [Строковый литерал](/sql-reference/syntax#string) или кортеж имен колонок, [Tuple](/sql-reference/data-types/tuple)([Строковый литерал](/sql-reference/syntax#string).
- `id_expr` — Ключевое значение. [Выражение](/sql-reference/syntax#expressions) возвращающее значение ключа словаря или [Tuple](../data-types/tuple.md)-тип значения в зависимости от конфигурации словаря.
...
Первая колонка `id`, вторая — `c1`, третья — `c2`.
...
- `dict_name` — Название словаря. [Строковый литерал](/sql-reference/syntax#string).
- `id_expr` — Ключевое значение. [Выражение](/sql-reference/syntax#expressions) возвращающее значение типа ключа словаря или [Tuple](../data-types/tuple.md)-тип значения в зависимости от конфигурации словаря.

**Возвращаемое значение**

- 0, если ключа нет. [UInt8](../data-types/int-uint.md).
- 1, если ключ есть. [UInt8](../data-types/int-uint.md).
...
- `dict_name` — Название словаря. [Строковый литерал](/sql-reference/syntax#string).
- `key` — Ключевое значение. [Выражение](/sql-reference/syntax#expressions) возвращающее значение типа [UInt64](../data-types/int-uint.md).
...
- `dict_name` — Название словаря. [Строковый литерал](/sql-reference/syntax#string).
- `child_id_expr` — Ключ для проверки. [Выражение](/sql-reference/syntax#expressions) возвращающее значение типа [UInt64](../data-types/int-uint.md).
...
- 0, если `child_id_expr` не является ребенком `ancestor_id_expr`. [UInt8](../data-types/int-uint.md).
- 1, если `child_id_expr` является ребенком `ancestor_id_expr` или если `child_id_expr` является `ancestor_id_expr`. [UInt8](../data-types/int-uint.md).

## dictGetChildren {#dictgetchildren}

Возвращает детей первого уровня в виде массива индексов. Это обратное преобразование для [dictGetHierarchy](#dictgethierarchy).
...
- Дети первого уровня для ключа. [Array](../data-types/array.md)([UInt64](../data-types/int-uint.md)).
...
Возвращает всех потомков так, как если бы функция [dictGetChildren](#dictgetchildren) применялась `level` раз рекурсивно.
...
Извлекает значения атрибутов всех узлов, которые соответствуют каждому ключу в [словаре-дереве регулярных выражений](../../sql-reference/dictionaries/index.md#regexp-tree-dictionary).

Кроме возвращения значений типа `Array(T)` вместо `T`, эта функция ведет себя аналогично [`dictGet`](#dictget-dictgetordefault-dictgetornull).
...
- `dict_name` — Название словаря. [Строковый литерал](/sql-reference/syntax#string).
- `attr_names` — Название колонки словаря, [Строковой литерал](/sql-reference/syntax#string) или кортеж имен колонок, [Tuple](/sql-reference/data-types/tuple)([Строковой литерал](/sql-reference/syntax#string)).
- `id_expr` — Ключевое значение. [Выражение](/sql-reference/syntax#expressions) возвращающее массив значений типа ключа словаря или [Tuple](../data-types/tuple)-тип значения в зависимости от конфигурации словаря.
- `limit` - Максимальная длина для каждого возвращаемого массива значений. При усечении дочерние узлы имеют приоритет над родительскими узлами, если не указано, длина массива не ограничена.

**Возвращаемое значение**

- Если ClickHouse успешно разбирает атрибут в типе данных атрибута, как определено в словаре, возвращает массив значений атрибутов словаря, соответствующих `id_expr` для каждого атрибута, указанного в `attr_names`.
...
- `dict_name` — Название словаря. [Строковый литерал](/sql-reference/syntax#string).
- `attr_name` — Название колонки словаря. [Строковой литерал](/sql-reference/syntax#string).
- `id_expr` — Ключевое значение. [Выражение](/sql-reference/syntax#expressions) возвращающее значение [UInt64](../data-types/int-uint.md) или [Tuple](../data-types/tuple.md)-тип в зависимости от конфигурации словаря.
