...
Токенизатор по умолчанию использует небуквенно-цифровые символы ASCII в качестве разделителей.
...
Чтобы этого добиться, передавайте разделители в порядке убывания их длины.
Например, при separators = `['%21', '%']` строка `%21abc` будет токенизирована как `['abc']`, тогда как при separators = `['%', '%21']` результатом будет `['21ac']` (что, вероятно, не то, что вам нужно).
