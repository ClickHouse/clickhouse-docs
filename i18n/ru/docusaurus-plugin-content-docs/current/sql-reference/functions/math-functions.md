...
чем больше раундов, тем ниже вероятность ложноположительного результата и тем больше время выполнения. Для любого составного числа
вероятность ложноположительного результата ограничена `4^(-rounds)`; значение по умолчанию `25` удерживает её ниже `10^-15`.

Функция детерминированная: одни и те же входные данные и значение `rounds` всегда дают один и тот же результат.
...
* `rounds` — Необязательная положительная целочисленная константа. Количество раундов Миллера—Рабина для `UInt128`/`UInt256` (для более узких типов игнорируется). По умолчанию — `25`. [`UInt8`](/sql-reference/data-types/int-uint) или [`UInt16`](/sql-reference/data-types/int-uint) или [`UInt32`](/sql-reference/data-types/int-uint) или [`UInt64`](/sql-reference/data-types/int-uint)
...
Впервые была добавлена в: v23.3.0
