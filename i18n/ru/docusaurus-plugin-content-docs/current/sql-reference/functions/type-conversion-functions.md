...
ClickHouse, как правило, использует [такое же поведение, как и C++ программы](https://en.cppreference.com/w/cpp/language/implicit_conversion).

Функции `to<type>` и [cast](#cast) ведут себя по-разному в некоторых случаях, например, в случае [LowCardinality](../data-types/lowcardinality.md): [cast](#cast) удаляет признак [LowCardinality](../data-types/lowcardinality.md), а функции `to<type>` — нет. То же самое с [Nullable](../data-types/nullable.md), это поведение несовместимо со стандартом SQL и может быть изменено с помощью настройки [cast_keep_nullable](../../operations/settings/settings.md/#cast_keep_nullable).
...
## Заметки о функциях `toDate`/`toDateTime` {#to-date-and-date-time-functions}
...
Также см. функцию [`toUnixTimestamp`](#toUnixTimestamp).
## toBool {#tobool}
...
- Возвращает `true` или `false` на основе оценки аргумента. [Bool](../data-types/boolean.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int8](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой. Например: `SELECT toInt8(128) == -128;`.
:::
...
:::note
Функция использует [округление по направлению к нулю](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero), что означает, что она обрезает дробные цифры чисел.
:::
...
:::note
Если входное значение не может быть представлено в пределах [Int8](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 8-битное целое число, если успешно, иначе `0`. [Int8](../data-types/int-uint.md).
...
- 8-битное целое число, если успешно, иначе `NULL`. [Int8](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как [`toInt8`](#toint8), эта функция преобразует входное значение в значение типа [Int8](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `default` (опционально) — значение по умолчанию, возвращаемое, если преобразование к типу `Int8` не удалось. [Int8](../data-types/int-uint.md).
...
- 8-битное целое число, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Int8](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int16](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой. Например: `SELECT toInt16(32768) == -32768;`.
:::

**Возвращаемое значение**

- 16-битное целое число. [Int16](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int16](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 16-битное целое число, если успешно, иначе `0`. [Int16](../data-types/int-uint.md).
...
- 16-битное целое число, если успешно, иначе `NULL`. [Int16](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как [`toInt16`](#toint16), эта функция преобразует входное значение в значение типа [Int16](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (опционально) — значение по умолчанию, возвращаемое, если преобразование к типу `Int16` не удалось. [Int16](../data-types/int-uint.md).
...
- 16-битное целое число, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Int16](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int32](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой. Например: `SELECT toInt32(2147483648) == -2147483648;`
:::
...
:::note
Если входное значение не может быть представлено в пределах [Int32](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 32-битное целое число, если успешно, иначе `0`. [Int32](../data-types/int-uint.md)
...
- 32-битное целое число, если успешно, иначе `NULL`. [Int32](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как [`toInt32`](#toint32), эта функция преобразует входное значение в значение типа [Int32](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (опционально) — значение по умолчанию, возвращаемое, если преобразование к типу `Int32` не удалось. [Int32](../data-types/int-uint.md).
...
- 32-битное целое число, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Int32](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int64](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой. Например: `SELECT toInt64(9223372036854775808) == -9223372036854775808;`
:::
...
:::note
Если входное значение не может быть представлено в пределах [Int64](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 64-битное целое число, если успешно, иначе `0`. [Int64](../data-types/int-uint.md).
...
- 64-битное целое число, если успешно, иначе `NULL`. [Int64](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как [`toInt64`](#toint64), эта функция преобразует входное значение в значение типа [Int64](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (опционально) — значение по умолчанию, возвращаемое, если преобразование к типу `Int64` не удалось. [Int64](../data-types/int-uint.md).
...
- 64-битное целое число, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Int64](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int128](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой.
:::
...
:::note
Если входное значение не может быть представлено в пределах [Int128](../data-types/int-uint.md), возникает переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 128-битное целое число, если успешно, иначе `0`. [Int128](../data-types/int-uint.md).
...
- 128-битное целое число, если успешно, иначе `NULL`. [Int128](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как и [`toInt128`](#toint128), эта функция преобразует входное значение в значение типа [Int128](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (необязательный) — Значение по умолчанию, которое будет возвращено, если разбор типа `Int128` не удался. [Int128](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int128](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 128-битовое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [Int128](../data-types/int-uint.md).

:::note
- Функция использует [округление к нулю](https://en.wikipedia.org/wiki/Rounding#Rounding_towards_zero), то есть она обрезает дробные цифры чисел.
...
:::note
Если входное значение не может быть представлено в пределах [Int256](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 256-битовое целое значение. [Int256](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [Int256](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 256-битовое целое значение, если успешно, иначе `0`. [Int256](../data-types/int-uint.md).
...
- 256-битовое целое значение, если успешно, иначе `NULL`. [Int256](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как и [`toInt256`](#toint256), эта функция преобразует входное значение в значение типа [Int256](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (необязательный) — Значение по умолчанию, которое будет возвращено, если разбор типа `Int256` не удался. [Int256](../data-types/int-uint.md).
...
- 256-битовое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [Int256](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt8](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой. Например: `SELECT toUInt8(256) == 0;`.
:::

**Возвращаемое значение**

- 8-битовое беззнаковое целое значение. [UInt8](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt8](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 8-битовое беззнаковое целое значение, если успешно, иначе `0`. [UInt8](../data-types/int-uint.md).
...
- 8-битовое беззнаковое целое значение, если успешно, иначе `NULL`. [UInt8](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как и [`toUInt8`](#touint8), эта функция преобразует входное значение в значение типа [UInt8](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (необязательный) — Значение по умолчанию, которое будет возвращено, если разбор типа `UInt8` не удался. [UInt8](../data-types/int-uint.md).
...
- 8-битовое беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [UInt8](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt16](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой. Например: `SELECT toUInt16(65536) == 0;`.
:::

**Возвращаемое значение**

- 16-битовое беззнаковое целое значение. [UInt16](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt16](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 16-битовое беззнаковое целое значение, если успешно, иначе `0`. [UInt16](../data-types/int-uint.md).
...
- 16-битовое беззнаковое целое значение, если успешно, иначе `NULL`. [UInt16](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как и [`toUInt16`](#touint16), эта функция преобразует входное значение в значение типа [UInt16](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (необязательный) — Значение по умолчанию, которое будет возвращено, если разбор типа `UInt16` не удался. [UInt16](../data-types/int-uint.md).
...
- 16-битовое беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [UInt16](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt32](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой. Например: `SELECT toUInt32(4294967296) == 0;`
:::

**Возвращаемое значение**

- 32-битовое беззнаковое целое значение. [UInt32](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt32](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 32-битовое беззнаковое целое значение, если успешно, иначе `0`. [UInt32](../data-types/int-uint.md)
...
- 32-битовое беззнаковое целое значение, если успешно, иначе `NULL`. [UInt32](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
Как и [`toUInt32`](#touint32), эта функция преобразует входное значение в значение типа [UInt32](../data-types/int-uint.md), но возвращает значение по умолчанию в случае ошибки. Если значение `default` не передано, то в случае ошибки возвращается `0`.
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Expression](/sql-reference/syntax#expressions) / [String](../data-types/string.md).
- `default` (необязательный) — Значение по умолчанию, которое будет возвращено, если разбор типа `UInt32` не удался. [UInt32](../data-types/int-uint.md).
...
- 32-битовое беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [UInt32](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt64](../data-types/int-uint.md), результат переполняется или недополняется. Это не считается ошибкой. Например: `SELECT toUInt64(18446744073709551616) == 0;`
:::
...
:::note
Если входное значение не может быть представлено в пределах [UInt64](../data-types/int-uint.md), происходит переполнение или недополнение результата. Это не считается ошибкой.
:::

**Возвращаемое значение**

- 64-битовое беззнаковое целое значение, если успешно, иначе `0`. [UInt64](../data-types/int-uint.md).
...
- 64-битовое беззнаковое целое значение, если успешно, иначе `NULL`. [UInt64](../data-types/int-uint.md) / [NULL](../data-types/nullable.md).
...
- `defauult` (необязательно) — Значение по умолчанию, которое будет возвращено, если разбор к типу `UInt64` не удался. [UInt64](../data-types/int-uint.md).
...
- 64-битное беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если нет. [UInt64](../data-types/int-uint.md).
...
- 128-битное беззнаковое целое значение, если успешно, в противном случае `0`. [UInt128](../data-types/int-uint.md).
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Выражение](/sql-reference/syntax#expressions) / [Строка](../data-types/string.md).
- `default` (необязательно) — Значение по умолчанию, которое будет возвращено, если разбор к типу `UInt128` не удался. [UInt128](../data-types/int-uint.md).
...
- 128-битное беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если нет. [UInt128](../data-types/int-uint.md).
...
:::note
Если входное значение не может быть представлено в пределах [UInt256](../data-types/int-uint.md), результат переполняется или недополняется.
Это не считается ошибкой.
:::
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Выражение](/sql-reference/syntax#expressions) / [Строка](../data-types/string.md).
- `default` (необязательно) — Значение по умолчанию, которое будет возвращено, если разбор к типу `UInt256` не удался. [UInt256](../data-types/int-uint.md).
...
- 256-битное беззнаковое целое значение, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если нет. [UInt256](../data-types/int-uint.md).
...
- 32-битное значение с плавающей точкой, если успешно, иначе `0`. [Float32](../data-types/float.md).
...
- 32-битное значение с плавающей точкой, если успешно, иначе `\N`. [Float32](../data-types/float.md).
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Выражение](/sql-reference/syntax#expressions) / [Строка](../data-types/string.md).
- `default` (необязательно) — Значение по умолчанию, которое будет возвращено, если разбор к типу `Float32` не удался. [Float32](../data-types/float.md).
...
- 32-битное значение с плавающей точкой, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если нет. [Float32](../data-types/float.md).
...
- 64-битное значение с плавающей точкой, если успешно, иначе `0`. [Float64](../data-types/float.md).
...
- 64-битное значение с плавающей точкой, если успешно, иначе `\N`. [Float64](../data-types/float.md).
...
- `expr` — Выражение, возвращающее число или строковое представление числа. [Выражение](/sql-reference/syntax#expressions) / [Строка](../data-types/string.md).
- `default` (необязательно) — Значение по умолчанию, которое будет возвращено, если разбор к типу `Float64` не удался. [Float64](../data-types/float.md).
...
- 64-битное значение с плавающей точкой, если успешно, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если нет. [Float64](../data-types/float.md).
...
Если аргумент является [DateTime](../data-types/datetime.md) или [DateTime64](../data-types/datetime64.md), он усечен и сохраняется только компонент даты из DateTime:
...
Если аргумент является [Строкой](../data-types/string.md), он разбирается как [Date](../data-types/date.md) или [DateTime](../data-types/datetime.md). Если он был разобран как [DateTime](../data-types/datetime.md), используется компонент даты:
...
Если аргумент является числом и выглядит как метка времени UNIX (больше 65535), он интерпретируется как [DateTime](../data-types/datetime.md), а затем усечен до [Date](../data-types/date.md) в текущем часовом поясе. Аргумент часовых поясов может быть указан в качестве второго аргумента функции. Усечение до [Date](../data-types/date.md) зависит от часового пояса:
...
Если аргумент является числом и меньше 65536, он интерпретируется как количество дней с 1970-01-01 (первый UNIX день) и преобразуется в [Date](../data-types/date.md). Это соответствует внутреннему числовому представлению типа `Date`. Пример:
...
То же самое, что и [toDate](#todate), но возвращает нижнюю границу [Date](../data-types/date.md), если получен недопустимый аргумент. Поддерживается только аргумент [Строка](../data-types/string.md).
...
Как и [toDate](#todate), но если не удается, возвращает значение по умолчанию, которое либо является вторым аргументом (если указан), либо иначе нижней границей [Date](../data-types/date.md).
...
- `expr` — Значение. [Строка](../data-types/string.md), [Int](../data-types/int-uint.md), [Date](../data-types/date.md) или [DateTime](../data-types/datetime.md).
...
Если `expr` является [Строкой](../data-types/string.md), он может быть интерпретирован как метка времени Unix или как строковое представление даты / даты с временем.
Таким образом, разбор строковых представлений коротких чисел (до 4 цифр) явно отключен из-за двусмысленности, например, строка `'1999'` может быть как годом (неполное строковое представление даты / даты и времени), так и временной меткой Unix. Более длинные числовые строки разрешены.
:::
...
То же самое, что и [toDateTime](#todatetime), но возвращает нижнюю границу [DateTime](../data-types/datetime.md), если получен недопустимый аргумент. Поддерживается только аргумент [Строка](../data-types/string.md).
...
Как и [toDateTime](#todatetime), но если не удается, возвращает значение по умолчанию, которое либо является третьим аргументом (если указан), либо иначе нижней границей [DateTime](../data-types/datetime.md).
...
Преобразует аргумент в тип [Date32](../data-types/date32.md). Если значение находится вне диапазона, `toDate32` возвращает граничные значения, поддерживаемые [Date32](../data-types/date32.md). Если аргумент имеет тип [Date](../data-types/date.md), учитываются его границы.
...
- `expr` — Значение. [Строка](../data-types/string.md), [UInt32](../data-types/int-uint.md) или [Date](../data-types/date.md).
...
2. Значение находится вне диапазона:
...
То же самое, что и [toDate32](#todate32), но возвращает минимальное значение [Date32](../data-types/date32.md), если получен недопустимый аргумент.
...
Преобразует аргумент в тип данных [Date32](../data-types/date32.md). Если значение находится вне диапазона, `toDate32OrDefault` возвращает минимально допустимое значение, поддерживаемое [Date32](../data-types/date32.md). Если аргумент имеет тип [Date](../data-types/date.md), его границы принимаются во внимание. Возвращает значение по умолчанию, если получен недопустимый аргумент.
...
2. В виде десятичного числа с точностью:
...
Как [toDateTime64](#todatetime64), эта функция преобразует входное значение в значение типа [DateTime64](../data-types/datetime64.md), но возвращает минимальное значение [DateTime64](../data-types/datetime64.md), если был получен недопустимый аргумент.
...
- Календарная дата и время суток с точностью до долей секунды, иначе минимальное значение `DateTime64`: `1970-01-01 01:00:00.000`. [DateTime64](../data-types/datetime64.md).
...
Как [toDateTime64](#todatetime64), эта функция преобразует входное значение в значение типа [DateTime64](../data-types/datetime64.md), но возвращает `NULL`, если был получен недопустимый аргумент.
...
- Календарная дата и время суток с точностью до долей секунды, иначе `NULL`. [DateTime64](../data-types/datetime64.md)/[NULL](../data-types/nullable.md).
...
либо предоставленное значение по умолчанию, если был получен недопустимый аргумент.
...
- `scale` - Размер тика (точность): 10<sup>-precision</sup> секунд. Допустимый диапазон: [ 0 : 9 ].
- `timezone` (опционально) - Часовой пояс указанного объекта DateTime64.
- `default` (опционально) - Значение по умолчанию для возврата, если был получен недопустимый аргумент. [DateTime64](../data-types/datetime64.md).

**Возвращаемое значение**

- Календарная дата и время суток с точностью до долей секунды, иначе минимальное значение `DateTime64` или значение `default`, если оно предоставлено. [DateTime64](../data-types/datetime64.md).
...
Преобразует входное значение в значение типа [`Decimal(9, S)`](../data-types/decimal.md) с масштабом `S`. Выдает исключение в случае ошибки.
...
Например: `toDecimal32(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
Как [`toDecimal32`](#todecimal32), эта функция преобразует входное значение в значение типа [Decimal(9, S)](../data-types/decimal.md), но возвращает значение по умолчанию в случае ошибки.
...
- `expr` — Строковое представление числа. [String](../data-types/string.md).
- `S` — Параметр масштаба от 0 до 9, указывающий, сколько цифр может иметь дробная часть числа. [UInt8](../data-types/int-uint.md).
- `default` (опционально) — Значение по умолчанию для возврата, если преобразование к типу `Decimal32(S)` не удалось. [Decimal32(S)](../data-types/decimal.md).
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal32OrDefault(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
- Значение типа `Decimal(9, S)`, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Decimal32(S)](../data-types/decimal.md).
...
Преобразует входное значение в значение типа [`Decimal(18, S)`](../data-types/decimal.md) с масштабом `S`. Выдает исключение в случае ошибки.
...
:::note
Переполнение может произойти, если значение `expr` превышает пределы `Decimal64`: `( -1 * 10^(18 - S), 1 * 10^(18 - S) )`.
Слишком много цифр в дробной части отбрасывается (не округляется).
Слишком много цифр в целой части приведет к исключению.
:::
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal64(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
Как [`toDecimal64`](#todecimal64), эта функция преобразует входное значение в значение типа [Decimal(18, S)](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
- Значение типа `Decimal(18, S)`, если успешно, иначе `0` с `S` десятичными знаками. [Decimal64(S)](../data-types/decimal.md).
...
Как [`toDecimal64`](#todecimal64), эта функция преобразует входное значение в значение типа [Nullable(Decimal(18, S))](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
- Значение типа `Nullable(Decimal(18, S))`, если успешно, иначе значение `NULL` того же типа. [Decimal64(S)](../data-types/decimal.md).
...
Как [`toDecimal64`](#todecimal64), эта функция преобразует входное значение в значение типа [Decimal(18, S)](../data-types/decimal.md), но возвращает значение по умолчанию в случае ошибки.
...
- `expr` — Строковое представление числа. [String](../data-types/string.md).
- `S` — Параметр масштаба от 0 до 18, указывающий, сколько цифр может иметь дробная часть числа. [UInt8](../data-types/int-uint.md).
- `default` (опционально) — Значение по умолчанию для возврата, если преобразование к типу `Decimal64(S)` не удалось. [Decimal64(S)](../data-types/decimal.md).
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal64OrDefault(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
- Значение типа `Decimal(18, S)` если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Decimal64(S)](../data-types/decimal.md).
...
Преобразует входное значение в значение типа [`Decimal(38, S)`](../data-types/decimal.md) с масштабом `S`. Выдает исключение в случае ошибки.
...
:::note
Переполнение может произойти, если значение `expr` превышает пределы `Decimal128`: `( -1 * 10^(38 - S), 1 * 10^(38 - S) )`.
Слишком много цифр в дробной части отбрасывается (не округляется).
Слишком много цифр в целой части приведет к исключению.
:::
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal128(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
Как [`toDecimal128`](#todecimal128), эта функция преобразует входное значение в значение типа [Decimal(38, S)](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
Как [`toDecimal128`](#todecimal128), эта функция преобразует входное значение в значение типа [Nullable(Decimal(38, S))](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
- Значение типа `Nullable(Decimal(38, S))`, если успешно, иначе значение `NULL` того же типа. [Decimal128(S)](../data-types/decimal.md).
...
Как [`toDecimal128`](#todecimal128), эта функция преобразует входное значение в значение типа [Decimal(38, S)](../data-types/decimal.md), но возвращает значение по умолчанию в случае ошибки.
...
- `expr` — Строковое представление числа. [String](../data-types/string.md).
- `S` — Параметр масштаба от 0 до 38, указывающий, сколько цифр может иметь дробная часть числа. [UInt8](../data-types/int-uint.md).
- `default` (опционально) — Значение по умолчанию для возврата, если преобразование к типу `Decimal128(S)` не удалось. [Decimal128(S)](../data-types/decimal.md).
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal128OrDefault(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
- Значение типа `Decimal(38, S)`, если успешно, иначе возвращает значение по умолчанию, если оно передано, или `0`, если нет. [Decimal128(S)](../data-types/decimal.md).
...
Преобразует входное значение в значение типа [`Decimal(76, S)`](../data-types/decimal.md) с масштабом `S`. Выдает исключение в случае ошибки.
...
:::note
Переполнение может произойти, если значение `expr` превышает пределы `Decimal256`: `( -1 * 10^(76 - S), 1 * 10^(76 - S) )`.
Слишком много цифр в дробной части отбрасывается (не округляется).
Слишком много цифр в целой части приведет к исключению.
:::
...
:::warning
Преобразования отбрасывают лишние цифры и могут работать неожиданным образом при работе с входными данными Float32/Float64, так как операции выполняются с использованием инструкций с плавающей запятой.
Например: `toDecimal256(1.15, 2)` равен `1.14`, потому что 1.15 * 100 в формате с плавающей запятой — это 114.99.
...
Как [`toDecimal256`](#todecimal256), эта функция преобразует входное значение в значение типа [Decimal(76, S)](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
- Значение типа `Decimal(76, S)`, если успешно, иначе `0` с `S` десятичными знаками. [Decimal256(S)](../data-types/decimal.md).
...
Как [`toDecimal256`](#todecimal256), эта функция преобразует входное значение в значение типа [Nullable(Decimal(76, S))](../data-types/decimal.md), но возвращает `0` в случае ошибки.
...
- Значение типа `Nullable(Decimal(76, S))`, если успешно, иначе значение `NULL` того же типа. [Decimal256(S)](../data-types/decimal.md).
...
- `expr` — строковое представление числа. [String](../data-types/string.md).
- `S` — параметр масштаба от 0 до 76, указывающий, сколько цифр может содержать дробная часть числа. [UInt8](../data-types/int-uint.md).
- `default` (опционально) — значение по умолчанию, которое возвращается, если преобразование в тип `Decimal256(S)` не удалось. [Decimal256(S)](../data-types/decimal.md).
...
Например: `toDecimal256OrDefault(1.15, 2)` равняется `1.14`, потому что 1.15 * 100 в формате с плавающей запятой равно 114.99.
...
- Значение типа `Decimal(76, S)`, если операция успешна, иначе возвращает значение по умолчанию, если оно было передано, или `0`, если не было. [Decimal256(S)](../data-types/decimal.md).
...
Для аргументов DateTime функция может принимать второй строковый аргумент, содержащий название временной зоны.
...
- `timezone`: Необязательно. Название временной зоны для преобразования `DateTime`. [`String`](/sql-reference/data-types/string).
...
Преобразует аргумент типа [String](../data-types/string.md) в тип [FixedString(N)](../data-types/fixedstring.md) (строка фиксированной длины N).
Если строка имеет меньше байт, чем N, она дополнена нулевыми байтами справа. Если строка имеет больше байт, чем N, будет выброшено исключение.
...
- Строка фиксированной длины N из `s`. [FixedString](../data-types/fixedstring.md).
...
- `scale` — количество дробных знаков, [UInt8](../data-types/int-uint.md).
  * Максимальный масштаб для типов [Decimal](../data-types/decimal.md) и [Int, UInt](../data-types/int-uint.md) равен 77 (это максимальное возможное количество значащих цифр для Decimal),
  * Максимальный масштаб для [Float](../data-types/float.md) равен 60.

**Возвращаемое значение**

- Входное значение, представленное как [String](../data-types/string.md) с заданным количеством дробных знаков (масштаба).
  Число округляется вверх или вниз согласно общим арифметическим правилам в случае, если запрашиваемый масштаб меньше масштаба исходного числа.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt8. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt16. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt32. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt64. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt128. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа UInt256. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int8. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int16. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int32. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int64. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int128. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Int256. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Float32. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
Выполняет переинтерпретацию байтов, рассматривая входное значение как значение типа Float64. В отличие от [`CAST`](#cast), функция не пытается сохранить исходное значение - если целевой тип не может представить входной тип, результат оказывается бессмысленным.
...
:::note
В дополнение к функциям UUID, перечисленным здесь, существует специализированная [документация по функциям UUID](../functions/uuid-functions.md).
:::
...
- `fixed_string` — байтовая строка в порядке big-endian. [FixedString](/sql-reference/data-types/fixedstring).
...
Преобразует входное значение в указанный тип данных. В отличие от функции [reinterpret](#reinterpret), `CAST` пытается представить то же значение, используя новый тип данных. Если преобразование невозможно, будет вызвано исключение.
...
Поддерживается преобразование типов в [Nullable](../data-types/nullable.md) и обратно.
...
Отличие от [cast](#cast) заключается в том, что `accurateCast` не допускает переполнение числовых типов при преобразовании, если значение типа `x` не помещается в пределы типа `T`. Например, `accurateCast(-1, 'UInt8')` вызывает исключение.
...
Преобразует входное значение `x` в указанный тип данных `T`. Всегда возвращает тип [Nullable](../data-types/nullable.md) и возвращает [NULL](/sql-reference/syntax#null), если значение при преобразовании не может быть представлено в целевом типе.
...
Создает значение типа [Interval](../../sql-reference/data-types/special-data-types/interval.md) из числового значения и единицы интервала (например, 'секунда' или 'день').
...
- `value` — длина интервала. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество лет. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество кварталов. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество месяцев. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество недель. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество дней. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество часов. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество минут. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество секунд. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
- `n` — количество миллисекунд. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
Возвращает интервал в `n` микросекундах типа данных [IntervalMicrosecond](../data-types/special-data-types/interval.md).
...
- `n` — Количество микросекунд. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
Возвращает интервал в `n` наносекундах типа данных [IntervalNanosecond](../data-types/special-data-types/interval.md).
...
- `n` — Количество наносекунд. Целые числа или строковые представления, а также дробные числа. [(U)Int*](../data-types/int-uint.md)/[Float*](../data-types/float.md)/[String](../data-types/string.md).
...
Эта функция является обратным действием функции [formatDateTime](/sql-reference/functions/date-time-functions#formatDateTime).
...
То же самое, что и для [parseDateTime](#parsedatetime), за исключением того, что он возвращает дату ноль, когда встречает формат даты, который не может быть обработан.
...
Похоже на [parseDateTime](#parsedatetime), за исключением того, что строка формата находится в формате [Joda](https://joda-time.sourceforge.net/apidocs/org/joda/time/format/DateTimeFormat.html), а не в синтаксисе MySQL.

Эта функция является обратным действием функции [formatDateTimeInJodaSyntax](/sql-reference/functions/date-time-functions#formatDateTimeInJodaSyntax).
...
Все спецификаторы формата, перечисленные в [`formatDateTimeInJodaSyntax`](/sql-reference/functions/date-time-functions#formatDateTimeInJodaSyntax), поддерживаются, за исключением:
- S: дробная секунда
- z: часовой пояс
...
То же самое, что и для [parseDateTimeInJodaSyntax](#parsedatetimeinjodasyntax), за исключением того, что он возвращает дату ноль, когда встречает формат даты, который не может быть обработан.
...
То же самое, что и для [parseDateTime64](#parsedatetime64), за исключением того, что он возвращает дату ноль, когда встречает формат даты, который не может быть обработан.
...
То же самое, что и для [parseDateTime64InJodaSyntax](#parsedatetime64injodasyntax), за исключением того, что он возвращает дату ноль, когда встречает формат даты, который не может быть обработан.
...
Функция разбирает [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601), [RFC 1123 - 5.2.14 RFC-822 Указания по дате и времени](https://tools.ietf.org/html/rfc1123#page-55), форматы даты и времени ClickHouse и некоторые другие.
...
- Строка, содержащая временную метку [unix](https://en.wikipedia.org/wiki/Unix_time) в 9..10 цифр.
...
- [временная метка syslog](https://datatracker.ietf.org/doc/html/rfc3164#section-4.1.2): `Mmm dd hh:mm:ss`. Например, `Jun  9 14:20:32`.
...
- [toDate](#todate)
- [toDateTime](#todatetime)
- [ISO 8601 анонс от @xkcd](https://xkcd.com/1179/)
- [RFC 3164](https://datatracker.ietf.org/doc/html/rfc3164#section-4.1.2)
## parseDateTimeBestEffortUS {#parsedatetimebesteffortus}

Эта функция ведет себя как [parseDateTimeBestEffort](#parsedatetimebesteffort) для ISO форматов даты, например, `YYYY-MM-DD hh:mm:ss`, и других форматов даты, где компоненты месяца и даты могут быть однозначно извлечены, например, `YYYYMMDDhhmmss`, `YYYY-MM`, `DD hh` или `YYYY-MM-DD hh:mm:ss ±h:mm`. Если компоненты месяца и даты не могут быть однозначно извлечены, например, `MM/DD/YYYY`, `MM-DD-YYYY` или `MM-DD-YY`, он предпочитает формат даты США вместо `DD/MM/YYYY`, `DD-MM-YYYY` или `DD-MM-YY`. В качестве исключения из последнего, если месяц больше 12 и меньше или равен 31, эта функция возвращается к поведению [parseDateTimeBestEffort](#parsedatetimebesteffort), например, `15/08/2020` разбирается как `2020-08-15`.
...
То же самое, что и для [parseDateTimeBestEffort](#parsedatetimebesteffort), за исключением того, что он возвращает дату ноль или время даты ноль, когда встречает формат даты, который не может быть обработан.
...
То же самое, что и функция [parseDateTimeBestEffortUS](#parsedatetimebesteffortus), за исключением того, что он возвращает дату ноль (`1970-01-01`) или дату ноль с временем (`1970-01-01 00:00:00`), когда встречает формат даты, который не может быть обработан.
...
То же самое, что и для [parseDateTime64BestEffort](#parsedatetime64besteffort), за исключением того, что эта функция предпочитает формат даты США (`MM/DD/YYYY` и т.д.) в случае неоднозначности.
...
То же самое, что и для [parseDateTime64BestEffort](#parsedatetime64besteffort), за исключением того, что он возвращает дату ноль или время даты ноль, когда встречает формат даты, который не может быть обработан.
## parseDateTime64BestEffortUSOrNull {#parsedatetime64besteffortusornull}

То же самое, что и для [parseDateTime64BestEffort](#parsedatetime64besteffort), за исключением того, что эта функция предпочитает формат даты США (`MM/DD/YYYY` и т.д.) в случае неоднозначности и возвращает `NULL`, когда встречает формат даты, который не может быть обработан.
## parseDateTime64BestEffortUSOrZero {#parsedatetime64besteffortusorzero}

То же самое, что и для [parseDateTime64BestEffort](#parsedatetime64besteffort), за исключением того, что эта функция предпочитает формат даты США (`MM/DD/YYYY` и т.д.) в случае неоднозначности и возвращает дату ноль или время даты ноль, когда встречает формат даты, который не может быть обработан.
## toLowCardinality {#tolowcardinality}

Преобразует входной параметр в [LowCardinality](../data-types/lowcardinality.md) версию того же типа данных.
...
- `expr` — [Выражение](/sql-reference/syntax#expressions), которое приводит к одному из [поддерживаемых типов данных](/sql-reference/data-types).
...
- `timezone`: Необязательно. Часовой пояс, используемый для преобразования. Если не указано, используется часовой пояс сервера. [`String`](/sql-reference/data-types/string)
