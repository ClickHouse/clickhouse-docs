...
:::note
Тип данных `QBit` и связанные с ним функции расстояния являются возможностями в статусе Beta.
...
`QBit` реализует модель доступа к подстолбцам, которая позволяет обращаться к отдельным битовым плоскостям хранимых векторов. К каждой позиции бита можно обратиться с помощью синтаксиса `.N`, где `N` — позиция бита:
...
Это функции расстояния для поиска по сходству векторов, использующие тип данных `QBit`:
