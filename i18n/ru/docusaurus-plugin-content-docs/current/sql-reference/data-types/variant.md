...
Порядок вложенных типов не имеет значения: Variant(T1, T2) = Variant(T2, T1). Вложенные типы могут быть произвольными типами, кроме Nullable(...), LowCardinality(Nullable(...)) и Variant(...) типов.

:::note
Не рекомендуется использовать подобные типы в качестве вариаций (например, разные численные типы, такие как `Variant(UInt32, Int64)` или разные типы дат, такие как `Variant(Date, DateTime)`), поскольку работа с такими значениями может привести к неоднозначности. По умолчанию создание такого типа `Variant` приведет к исключению, но это можно включить с помощью настройки `allow_suspicious_variant_types`.
:::

## Создание Variant {#creating-variant}

Используя тип `Variant` в определении столбца таблицы:
...
## Чтение вложенных типов Variant как подполя {#reading-variant-nested-types-as-subcolumns}
