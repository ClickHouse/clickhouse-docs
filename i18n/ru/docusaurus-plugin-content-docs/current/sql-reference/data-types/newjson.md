---
description: 'Документация для типа данных JSON в ClickHouse, который обеспечивает
  нативную поддержку работы с данными в формате JSON'
...
:::note
Пути JSON хранятся в развернутом виде. Это означает, что когда JSON-объект формируется из пути, например, `a.b.c`, невозможно узнать, следует ли объект строить как `{ "a.b.c" : ... }` или `{ "a" : {"b" : {"c" : ... }}}`.
...
```
:::
## Чтение JSON-путей как под-колонок {#reading-json-paths-as-sub-columns}

Тип `JSON` поддерживает чтение каждого пути в качестве отдельной под-колонки. 
...
```sql title="Запрос (Чтение JSON-путей как под-колонок)"
SELECT json.a.b, json.a.g, json.c, json.d FROM test;
```

```text title="Ответ (Чтение JSON-путей как под-колонок)"
...
Как мы видим, для `a.b` тип — это `UInt32`, как мы указали в декларации типа JSON, 
...
Вероятно, переданный UUID не в кавычках: 
при выполнении 'FUNCTION CAST(__table1.json.a.g :: 2, 'UUID'_String :: 1) -> CAST(__table1.json.a.g, 'UUID'_String) UUID : 0'. 
(НЕ_РЕАЛИЗОВАНО)
```
## Чтение JSON-подобных объектов как под-колонок {#reading-json-sub-objects-as-sub-columns}
...
При анализе `JSON` ClickHouse пытается обнаружить наиболее подходящий тип данных для каждого пути JSON. 
Это работает аналогично [автоматическому выводу схемы на основе входных данных](/interfaces/schema-inference.md),
...
JSON пути, которые содержат массив объектов, анализируются как тип `Array(JSON)` и вставляются в динамический столбец для пути. 
...
Тип данных `JSON` может хранить только ограниченное количество путей в качестве отдельных под-колонок внутри. 
По умолчанию этот лимит составляет `1024`, но вы можете изменить его в декларации типа, используя параметр `max_dynamic_paths`.

Когда лимит достигнут, все новые пути, вставленные в столбец `JSON`, будут храниться в одной общей структуре данных. 
...
Во время разбора объектов `JSON` из данных, когда лимит достигается для текущего блока данных, 
...
└─arrayJoin(distinctJSONPathsAndTypes(json))──────────────────┘
```
## ИЗМЕНЕНИЕ КОЛОНКИ на тип JSON {#alter-modify-column-to-json-type}
...
- [Сравнение JSON: ClickHouse против MongoDB, Elasticsearch и других](https://clickhouse.com/blog/json-bench-clickhouse-vs-mongodb-elasticsearch-duckdb-postgresql)
