...
Массив элементов типа `T`, при этом индексация в массиве начинается с 1. `T` может быть любым типом данных, включая массив.
...
Если вложенный тип `T` внутри `Array` имеет подстолбцы (например, если это [именованный кортеж](./tuple.md)), вы можете читать его подстолбцы из типа `Array(T)` с теми же именами подстолбцов. Тип подстолбца будет `Array` от типа исходного подстолбца.
