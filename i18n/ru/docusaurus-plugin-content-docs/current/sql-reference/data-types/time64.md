...
Значение [Time64](time64.md) можно прибавить к значению [Date](date.md) или [Date32](date32.md), чтобы получить [DateTime64](datetime64.md) с той же точностью, что и у `Time64`:
...
См. раздел [Сложение даты и времени](../operators/index.md#date-time-addition), чтобы узнать подробности обо всех поддерживаемых комбинациях и результирующих типах.
