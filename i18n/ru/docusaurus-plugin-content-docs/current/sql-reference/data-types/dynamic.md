---
description: 'Документация по типу данных Dynamic в ClickHouse, предназначенному для
  хранения значений различных типов в одном столбце'
...
Этот тип позволяет хранить значения произвольного типа, не перечисляя все возможные типы заранее.
...
Где `N` — это необязательный параметр в диапазоне от `0` до `254`, определяющий, сколько различных типов данных может быть сохранено как отдельные подстолбцы внутри столбца типа `Dynamic` в пределах одного отдельно хранимого блока данных (например, в пределах одной части данных таблицы MergeTree). Если этот предел превышен, все значения с новыми типами будут храниться вместе в специальной общей структуре данных в двоичном виде. Значение по умолчанию для `max_types` — `32`.
...
Использование CAST для преобразования из обычного столбца:
...
Использование CAST для преобразования из столбца `Variant`:
...
Таким образом, если у вас есть столбец `d Dynamic`, вы можете прочитать подстолбец любого допустимого типа `T`, используя синтаксис `d.T`.
Этот подстолбец будет иметь тип `Nullable(T)`, если `T` может находиться внутри `Nullable`, и `T` в противном случае. Этот подстолбец будет
иметь тот же размер, что и исходный столбец `Dynamic`, и будет содержать значения `NULL` (или пустые значения, если `T` не может находиться внутри `Nullable`)
во всех строках, в которых исходный столбец `Dynamic` не имеет тип `T`.
...
Чтобы узнать, какой вариант хранится в каждой строке, можно использовать функцию `dynamicType(dynamic_column)`. Она возвращает значение типа `String` с именем типа для каждой строки (или `'None'`, если значение в строке равно `NULL`).
...
## Преобразование столбца Dynamic в другие столбцы и обратно {#conversion-between-dynamic-column-and-other-columns}

Со столбцом `Dynamic` можно выполнить 4 типа преобразований.

### Преобразование обычного столбца в столбец Dynamic {#converting-an-ordinary-column-to-a-dynamic-column}
...
### Преобразование столбца String в столбец Dynamic с помощью парсинга {#converting-a-string-column-to-a-dynamic-column-through-parsing}

Для парсинга значений типа `Dynamic` из столбца `String` необходимо включить настройку `cast_string_to_dynamic_use_inference`:
...
### Преобразование столбца Dynamic в обычный столбец {#converting-a-dynamic-column-to-an-ordinary-column}
...
### Преобразование столбца Variant в столбец Dynamic {#converting-a-variant-column-to-dynamic-column}
...
### Преобразование столбца Dynamic(max_types=N) в другой столбец Dynamic(max_types=K) {#converting-a-dynamicmax_typesn-column-to-another-dynamicmax_typesk}
...
Если `K < N`, то значения с наиболее редкими типами будут помещены в единый специальный подстолбец, но останутся доступными:
...
Функция `isDynamicElementInSharedData` возвращает `true` для строк, которые хранятся в специальной разделяемой структуре данных внутри `Dynamic`, и, как мы видим, результирующий столбец содержит только два типа, которые не хранятся в этой разделяемой структуре данных.
...
Все текстовые форматы (TSV, CSV, CustomSeparated, Values, JSONEachRow и т. д.) поддерживают чтение типа `Dynamic`. При парсинге данных ClickHouse пытается определить тип каждого значения и использует его при вставке в столбец `Dynamic`.
...
Если тип результата функции зависит от типов аргументов, результатом такой функции, выполненной с аргументами `Dynamic`, будет `Dynamic`. Если тип результата функции не зависит от типов аргументов, результатом будет `Nullable(T)`, где `T` — стандартный тип результата этой функции.
...
Если для какого-либо типа внутри столбца `Dynamic` функцию невозможно выполнить, будет сгенерировано исключение:
...
По умолчанию тип `Dynamic` не разрешён в ключах `GROUP BY`/`ORDER BY`. Если вы хотите его использовать, учитывайте его особое правило сравнения и включите настройки `allow_suspicious_types_in_group_by`/`allow_suspicious_types_in_order_by`.
...
**Примечание:** значения динамических типов с различными числовыми типами считаются разными значениями и не сравниваются между собой — вместо этого сравниваются имена их типов.
...
## Достижение лимита количества различных типов данных, хранящихся внутри Dynamic {#reaching-the-limit-in-number-of-different-data-types-stored-inside-dynamic}

Тип данных `Dynamic` может хранить только ограниченное количество различных типов данных в виде отдельных подстолбцов. По умолчанию этот лимит составляет 32, но его можно изменить в объявлении типа, используя синтаксис `Dynamic(max_types=N)`, где N находится в диапазоне от 0 до 254 (из-за особенностей реализации невозможно хранить более 254 различных типов данных в виде отдельных подстолбцов внутри Dynamic).
При достижении лимита все новые типы данных, вставляемые в столбец `Dynamic`, будут помещены в единую общую структуру данных, которая хранит значения различных типов данных в бинарном виде.
...
Как видно, после вставки 3 различных типов данных `Int64`, `Array(Int64)` и `String` все новые типы были помещены в специальную общую структуру данных.

### Во время слияния частей данных в движках таблиц MergeTree {#during-merges-of-data-parts-in-mergetree-table-engines}

Во время слияния нескольких частей данных в таблице MergeTree столбец `Dynamic` в результирующей части данных может достичь лимита различных типов данных, которые могут храниться в отдельных подстолбцах, и не сможет сохранить все типы в виде подстолбцов из исходных частей.
В этом случае ClickHouse выбирает, какие типы останутся в виде отдельных подстолбцов после слияния, а какие типы будут помещены в общую структуру данных. В большинстве случаев ClickHouse пытается сохранить наиболее частые типы и поместить наиболее редкие типы в общую структуру данных, но это зависит от реализации.

Рассмотрим пример такого слияния. Сначала создадим таблицу со столбцом `Dynamic`, установим лимит различных типов данных равным `3` и вставим значения с `5` различными типами:
...
Каждая вставка создаст отдельную часть данных со столбцом `Dynamic`, содержащим один тип:
...
Теперь давайте объединим все части воедино и посмотрим, что произойдёт:
...
Как мы видим, ClickHouse сохранил наиболее часто встречающиеся типы `UInt64` и `Array(UInt64)` в виде подколонок, а все остальные типы поместил в общую область данных.


## Функции JSONExtract с типом Dynamic {#jsonextract-functions-with-dynamic}
...
### Бинарный формат вывода {#binary-output-format}
