...
description: 'Документация для типа данных Dynamic в ClickHouse, который может хранить
  значения различных типов в одной колонке'
...
│ 42            │ Int64          │ ᴺᵁᴸᴸ                        │                         42 │ []                                │                      ᴺᵁᴸᴸ │ []                                 │
│ Hello, World! │ String         │ Hello, World!               │                       ᴺᵁᴸᴸ │ []                                │                      ᴺᵁᴸᴸ │ []                                 │
│ [1,2,3]       │ Array(Int64)   │ ᴺᵁᴸᴸ                        │                       ᴺᵁᴸᴸ │ [1,2,3]                           │                      ᴺᵁᴸᴼ  │ []                                 │
└───────────────┴────────────────┴─────────────────────────────┴────────────────────────────┴───────────────────────────────────┴───────────────────────────┴────────────────────────────────────┘
```
...
### Преобразование колонки Dynamic(max_types=N) в другую колонку Dynamic(max_types=K) {#converting-a-dynamicmax_typesn-column-to-another-dynamicmax_typesk}
...
```text
Получено исключение:
Код: 43. DB::Exception: Неправильные типы Array(Int64) и UInt8 в аргументах функции плюс: при выполнении 'FUNCTION plus(__table1.d : 3, 1_UInt8 :: 1) -> plus(__table1.d, 1_UInt8) Dynamic : 0'. (ILLEGAL_TYPE_OF_ARGUMENT)
```
...
При использовании `ORDER BY` и `GROUP BY` значения типов `Dynamic` сравниваются так же, как значения типа `Variant`:
