...
CUBE в следующем запросе, `CUBE(datacenter,distro,version)`, задаёт иерархию, которая может быть не вполне осмысленной. Нет смысла рассматривать version сразу для двух дистрибутивов, поскольку у Arch и RHEL разные циклы выпуска и разные стандарты именования версий. Следующий пример с GROUPING SETS более уместен, поскольку в нём `distro` и `version` группируются в одном наборе.
