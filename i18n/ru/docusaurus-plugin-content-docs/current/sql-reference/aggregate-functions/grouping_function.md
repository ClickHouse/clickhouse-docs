...
Анализ иерархических данных — хороший пример использования модификаторов ROLLUP, CUBE и GROUPING SETS. В данном примере представлена таблица с данными о том, какой дистрибутив Linux и какая версия этого дистрибутива установлены в двух дата-центрах. Может быть полезно рассмотреть данные по дистрибутиву, версии и расположению.
...
Получить количество серверов в каждом дата-центре по дистрибутивам:
...
Оператор CUBE в следующем запросе, `CUBE(datacenter,distro,version)`, создает иерархию, которая может не иметь смысла. Нецелесообразно рассматривать версию для двух дистрибутивов (поскольку Arch и RHEL имеют разные циклы выпуска и стандарты именования версий). Следующий пример с GROUPING SETS более уместен, так как он группирует `distro` и `version` в одном наборе.
...
:::note
Версия в приведённом выше примере может быть бессмысленной, когда она не связана с дистрибутивом; если бы мы отслеживали версию ядра, это было бы более логично, поскольку версия ядра может быть связана с любым из дистрибутивов. Использование GROUPING SETS, как в следующем примере, может оказаться более удачным выбором.
:::
