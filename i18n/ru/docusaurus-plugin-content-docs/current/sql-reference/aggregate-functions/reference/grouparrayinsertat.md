---
description: 'Вставляет значение в массив на указанную позицию.'
...
* `default_x` — значение по умолчанию для подстановки в пустые позиции. Необязательный параметр. [Выражение](/sql-reference/syntax#expressions), результатом которого является тип данных, заданный для параметра `x`. Если `default_x` не определён, используются [значения по умолчанию](/sql-reference/statements/create/table).
* `size` — длина результирующего массива. Необязательный параметр. При использовании этого параметра значение по умолчанию `default_x` должно быть задано. [UInt32](/sql-reference/data-types/int-uint#integer-ranges).
