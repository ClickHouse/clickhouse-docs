---
description: 'Вычисляет сумму чисел и одновременно считает количество строк. Функция используется оптимизатором запросов ClickHouse: если в запросе присутствует несколько функций `sum`, `count` или `avg`, они могут быть заменены одной функцией `sumCount`, чтобы избежать повторных вычислений. Необходимость явного использования этой функции возникает редко.'
