...
:::note
Функция использует численно нестабильный алгоритм. Если в вычислениях требуется [численная устойчивость](https://en.wikipedia.org/wiki/Numerical_stability), используйте функцию [`covarPopStable`](../reference/covarPopStable.md). Она работает медленнее, но обеспечивает меньшую вычислительную погрешность.
:::
...
- Ковариация генеральной совокупности для `x` и `y`. [Float64](../../data-types/float.md).
