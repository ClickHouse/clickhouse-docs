---
description: 'Агрегатная функция, которая строит флеймграф по списку стек-трейсов.'
...
- `size` — размер выделения для профилирования памяти (необязательный параметр, по умолчанию `1`). [UInt64](../../data-types/int-uint.md).
