...
:::note
Результат, показанный выше, будет отличаться в зависимости от кодека сжатия по умолчанию, настроенного на сервере. См. раздел [Кодеки сжатия столбцов](/sql-reference/statements/create/table#column_compression_codec).
:::
...
Функция также позволяет указывать несколько кодеков:
