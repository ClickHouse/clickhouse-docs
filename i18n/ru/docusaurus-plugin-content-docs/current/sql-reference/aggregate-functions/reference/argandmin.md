---
description: 'Вычисляет значения `arg` и `val` для минимального значения `val`. Если есть несколько строк с одинаковым минимальным значением `val`, то то, какие связанные значения `arg` и `val` будут возвращены, недетерминировано.'
...
Вычисляет пару значений `arg` и `val` для минимального значения `val`. Если существует несколько строк с одинаковым минимальным значением `val`, то то, какая из соответствующих пар `arg` и `val` будет возвращена, не определено.
Обе части — `arg` и `min` — ведут себя как [агрегатные функции](/sql-reference/aggregate-functions/index.md), обе они [пропускают `Null`](/sql-reference/aggregate-functions/index.md#null-processing) при обработке и возвращают значения, отличные от `Null`, если такие значения доступны.
...
Тип: кортеж, элементы которого имеют те же типы, что и `arg` и `val` соответственно.
...
┌─argAndMin((a), b)─┐
│ ((NULL),0)        │ -- `Tuple` 'a', содержащий только значение `NULL`, сам не равен `NULL`, поэтому агрегатные функции не пропустят эту строку из-за этого значения `NULL`
└───────────────────┘

SELECT (argAndMin((a, b), b) as t).1 argMinA, t.2 argMinB from test;
┌─argMinA──┬─argMinB─┐
│ (NULL,0) │       0 │ -- можно использовать `Tuple` и получить оба столбца (все — tuple(*)) для соответствующего min(b)
└──────────┴─────────┘
...
│ ('a',(1,'a'))        │ (0,NULL)    │ -- 'a' является первым значением, отличным от `NULL`, для min
└──────────────────────┴─────────────┘
