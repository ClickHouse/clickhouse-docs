...
Отличается от функции [sumMap](../reference/summap.md) тем, что выполняет суммирование с переполнением - т.е. возвращает тот же тип данных для суммирования, что и тип данных аргумента.
...
Сначала создадим таблицу под названием `sum_map`, и вставим в неё некоторые данные. Массивы ключей и значений хранятся отдельно в столбце под названием `statusMap` типа [Nested](../../data-types/nested-data-structures/index.md), и вместе в столбце под названием `statusMapTuple` типа [tuple](../../data-types/tuple.md) для иллюстрации использования двух различных синтаксисов этой функции, описанных выше.
...
Если мы выполнением запрос к таблице, используя функции `sumMap`, `sumMapWithOverflow` с синтаксисом типа массива и `toTypeName`, то мы можем увидеть, что для функции `sumMapWithOverflow` тип данных массива суммированных значений остается тем же, что и тип аргумента, оба `UInt8` (т.е. суммирование выполнено с переполнением). Для функции `sumMap` тип данных суммированных массивов значений изменился с `UInt8` на `UInt64`, чтобы избежать переполнения.
