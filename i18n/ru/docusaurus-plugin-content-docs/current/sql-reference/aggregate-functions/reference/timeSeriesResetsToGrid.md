---
description: 'Агрегатная функция, которая вычисляет сбросы во временных рядах в стиле PromQL на указанной сетке.'
...
Агрегатная функция, которая принимает данные временных рядов в виде пар «метка времени — значение» и вычисляет [сбросы в стиле PromQL](https://prometheus.io/docs/prometheus/latest/querying/functions/#resets) для этих данных на регулярной временной сетке, задаваемой начальной меткой времени, конечной меткой времени и шагом. Для каждой точки сетки образцы, используемые для вычисления `resets`, рассматриваются в пределах заданного временного окна.
...
* `staleness` - задаёт максимальное «устаревание» рассматриваемых образцов в секундах
...
значения `resets` на указанной сетке в виде `Array(Nullable(Float64))`. Возвращаемый массив содержит по одному значению для каждой точки временной сетки. Значение равно NULL, если в окне нет образцов для вычисления значения сбросов для соответствующей точки сетки.
