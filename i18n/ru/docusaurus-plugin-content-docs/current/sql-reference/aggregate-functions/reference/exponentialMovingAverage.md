---
description: 'Вычисляет экспоненциальное скользящее среднее значений за заданный
  интервал времени.'
...
Каждому `value` соответствует определённая `timeunit`.
Период полураспада `x` — это временной интервал, при котором экспоненциальные веса уменьшаются вдвое.
Функция возвращает взвешенное среднее: чем старше временная точка, тем меньший вес имеет соответствующее значение.
...
* `timeunit` — Единица времени. Единица времени — это не метка времени (в секундах), а индекс временного интервала. Может быть вычислена с помощью `intDiv`. [`(U)Int*`](/sql-reference/data-types/int-uint) или [`Float*`](/sql-reference/data-types/float) или [`Decimal`](/sql-reference/data-types/decimal)

**Возвращаемое значение**

Возвращает экспоненциально сглаженное скользящее среднее значений за последние `x` единиц времени на текущий момент времени. [`Float64`](/sql-reference/data-types/float)
...
**Использование оконных функций для временных вычислений**
