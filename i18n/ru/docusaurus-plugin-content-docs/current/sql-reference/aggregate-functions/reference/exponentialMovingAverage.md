...
Каждое `value` соответствует определённой `timeunit`. Период полураспада `x` — это временной лаг, при котором экспоненциальные веса уменьшаются вдвое. Функция возвращает взвешенное среднее: чем старше момент времени, тем меньший вес имеет соответствующее значение.
...
* `timeunit` — Единица времени. [Integer](../../../sql-reference/data-types/int-uint.md), [Float](../../../sql-reference/data-types/float.md) или [Decimal](../../../sql-reference/data-types/decimal.md). Единица времени — это не метка времени в секундах, а индекс временного интервала. Может быть вычислена с помощью [intDiv](/sql-reference/functions/arithmetic-functions#intDiv).
...
* Возвращает [экспоненциально сглаженное скользящее среднее](https://en.wikipedia.org/wiki/Moving_average#Exponential_moving_average) значений за последние `x` единиц времени на последний момент времени.
