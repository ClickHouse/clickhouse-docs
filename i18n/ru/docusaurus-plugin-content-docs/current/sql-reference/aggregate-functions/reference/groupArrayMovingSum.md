...
Функция может принимать размер окна в качестве параметра. Если параметр не указан, функция использует размер окна, равный количеству строк в столбце.
...
* `window_size` — размер расчетного окна.
