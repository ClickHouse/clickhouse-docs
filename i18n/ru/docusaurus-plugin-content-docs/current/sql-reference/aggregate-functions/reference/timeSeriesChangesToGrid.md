...
* `start_timestamp` — задаёт начало сетки. - `end_timestamp` — задаёт конец сетки. - `grid_step` — задаёт шаг сетки в секундах. - `staleness` — задаёт максимально допустимую «устарелость» в секундах для учитываемых выборок.

**Аргументы**

* `timestamp` — метка времени выборки. Может задаваться как отдельными значениями, так и массивами. - `value` — значение временного ряда, соответствующее метке времени. Может задаваться как отдельными значениями, так и массивами.
...
**Тот же запрос, но с аргументами-массивами**
