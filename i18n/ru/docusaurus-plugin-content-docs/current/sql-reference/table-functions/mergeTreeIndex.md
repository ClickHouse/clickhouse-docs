---
description: 'Представляет содержимое индексных и меточных файлов таблиц MergeTree. Может быть использовано для инстроспекции.'
...
Представляет содержимое индексных и меточных файлов таблиц MergeTree. Может быть использовано для инстроспекции.
...
- `database` - Имя базы данных, из которой читать индексы и метки.
- `table` - Имя таблицы, из которой читать индексы и метки.
- `with_marks` - Включать ли столбцы с метками в результат.

**Возвращаемое Значение**

Объект таблицы со столбцами со значениями первичного индекса исходной таблицы, столбцами со значениями меток (если включено) для всех возможных файлов в частях данных исходной таблицы и виртуальными столбцами:
...
Столбец меток может содержать значение `(NULL, NULL)` в случае, если столбец отсутствует в части данных или метки для одного из его подпотоков не записаны (например, в компактных частях).
