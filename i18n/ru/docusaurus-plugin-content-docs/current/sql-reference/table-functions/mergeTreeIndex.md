---
description: 'Представляет содержимое файлов индексов и меток таблиц MergeTree.
  Может использоваться для анализа внутреннего состояния.'
...
Представляет содержимое файлов индексов и меток таблиц MergeTree. Может использоваться для внутреннего анализа.
...
| Аргумент      | Описание                                          |
| ------------- | ------------------------------------------------- |
| `database`    | Имя базы данных, из которой читать индекс и метки.|
| `table`       | Имя таблицы, из которой читать индекс и метки.    |
...
Табличный объект со столбцами, содержащими значения первичного индекса и min-max индекса (если включён) исходной таблицы, столбцами со значениями меток (если включены) для всех возможных файлов в кусках данных исходной таблицы и виртуальными столбцами:

- `part_name` — имя куска данных.
- `mark_number` — номер текущей метки в куске данных.
- `rows_in_granule` — количество строк в текущей гранулe.

Столбец меток может содержать значение `(NULL, NULL)` в случае, когда столбец отсутствует в куске данных или метки для одного из его подпотоков не записаны (например, в компактных кусках).
