...
Какие снимки сохраняются, определяется [политикой хранения](#iceberg-snapshot-retention-policy) (свойствами таблицы `min-snapshots-to-keep`, `max-snapshot-age-ms` и переопределениями для отдельных ссылок). Политика хранения применяется всегда, независимо от того, передан ли аргумент временной метки.

Необязательный аргумент `timestamp` — это строка даты и времени (например, `'2024-06-01 00:00:00'`), интерпретируемая в **часовом поясе сервера**. Он служит предохранителем: снимки, у которых `timestamp-ms` равен этому значению или больше него, защищены от удаления, даже если по политике хранения они должны были бы быть удалены. Это позволяет гарантировать, что не будет удалён ни один снимок новее указанного момента времени.

Если временная метка не указана, какие снимки будут удалены, определяется только политикой хранения.
...
Команда возвращает таблицу с двумя столбцами (`metric_name String`, `metric_value Int64`), содержащую по одной строке для каждой метрики. Имена метрик соответствуют [спецификации Iceberg](https://iceberg.apache.org/docs/latest/spark-procedures/#output):
...
5. Создаёт новые метаданные без удалённых снимков
6. Физически удаляет недостижимые списки манифестов, файлы манифестов и файлы данных
...
| -------------------------------------- | ----- | ------------------ | --------------------------------------------------------------------------------------------------- |
| `history.expire.min-snapshots-to-keep` | Table | 1                  | Минимальное количество снимков, которое нужно сохранять в цепочке предков каждой ветки              |
| `history.expire.max-snapshot-age-ms`   | Table | 432000000 (5 days) | Максимальный возраст (в мс) снимков, сохраняемых в ветке                                            |
| `history.expire.max-ref-age-ms`        | Table | ∞ (never)          | Максимальный возраст (в мс) ссылки на снимок (ветки или тега), после которого сама ссылка удаляется |

Каждая ссылка на снимок (`refs` в метаданных Iceberg) может переопределять эти значения через поля уровня ссылки: `min-snapshots-to-keep`, `max-snapshot-age-ms` и `max-ref-age-ms`.
