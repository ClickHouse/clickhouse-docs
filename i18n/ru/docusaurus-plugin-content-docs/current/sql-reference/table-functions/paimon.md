...
Предоставляет табличный интерфейс только для чтения к таблицам Apache [Paimon](https://paimon.apache.org/) в Amazon S3, Azure, HDFS или в локальное хранилище.
...
Для `paimonS3` можно использовать необязательный параметр `extra_credentials` для передачи `role_arn` при доступе на основе ролей в ClickHouse Cloud. Инструкции по настройке см. в разделе [Secure S3](/cloud/data-sources/secure-s3).
