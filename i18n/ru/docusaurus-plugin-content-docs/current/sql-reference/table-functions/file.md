...
Движок таблицы, который предоставляет табличный интерфейс для выполнения SELECT из файлов и INSERT в файлы, аналогично табличной функции [s3](/sql-reference/table-functions/s3.md). Используйте `file` при работе с локальными файлами и `s3` при работе с бакетами в объектном хранилище, например S3, GCS или MinIO.
...
| Параметр          | Описание                                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `path`            | Относительный путь к файлу от [user&#95;files&#95;path](operations/server-configuration-parameters/settings.md#user_files_path) или `Array(String)` путей в запросах `SELECT`. В режиме только чтения поддерживает следующие [глоб-шаблоны](#globs-in-path): `*`, `?`, `{abc,def}` (где `'abc'` и `'def'` — строки) и `{N..M}` (где `N` и `M` — числа). |
...
### Партиционированная запись в несколько файлов в формате TSV \{#partitioned-write-to-multiple-tsv-files\}

Если при вставке данных в табличную функцию типа `file`, указать выражение `PARTITION BY`, то для каждой партиции создаётся отдельный файл. Разбиение данных на отдельные файлы помогает повысить производительность операций чтения.
