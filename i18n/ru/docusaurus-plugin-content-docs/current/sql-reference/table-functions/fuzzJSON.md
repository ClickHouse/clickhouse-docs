---
description: 'Искажает JSON-строку случайными изменениями.'
...
| `reuse_output` (boolean)           | Повторно использовать результат фаззинга в качестве входных данных для следующего фаззера.  |
| `malform_output` (boolean)         | Генерировать строку, которая не может быть разобрана как объект JSON.                       |
| `max_output_length` (UInt64)       | Максимально допустимая длина сгенерированной или модифицированной строки JSON.              |
...
Табличный объект с одним столбцом, содержащим изменённые JSON-строки.
