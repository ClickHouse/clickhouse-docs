...
* Самые быстрые формы — это простые запросы с диапазоном и точечным фильтром, которые используют шаг по умолчанию (`1`), например `primes(N)` или `primes() LIMIT N`. Эти формы используют оптимизированный генератор простых чисел для эффективного вычисления очень больших простых чисел. Например, следующий запрос выполняется почти мгновенно:
