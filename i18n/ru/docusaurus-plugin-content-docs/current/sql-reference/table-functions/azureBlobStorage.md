...
| `partition_strategy`             | Необязательный параметр. Поддерживаемые значения: `WILDCARD` или `HIVE`. `WILDCARD` требует наличия `{_partition_id}` в пути, который заменяется ключом партиционирования. `HIVE` не допускает подстановочные шаблоны, предполагает, что путь — это корень таблицы, и создает каталоги партиций в стиле Hive с идентификаторами Snowflake ID в качестве имен файлов и форматом файла в качестве расширения. По умолчанию используется настройка `file_like_engine_default_partition_strategy` (`WILDCARD` при настройках `compatibility` старее `26.6`, в противном случае `HIVE`). |
...
`WILDCARD`: заменяет подстановочный символ `{_partition_id}` в пути к файлу фактическим ключом партиции. По умолчанию выбирается только при значении настройки `compatibility` ниже `26.6`; в противном случае по умолчанию используется `HIVE` (см. настройку `file_like_engine_default_partition_strategy`).
