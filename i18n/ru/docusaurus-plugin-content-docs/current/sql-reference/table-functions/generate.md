---
description: 'Генерирует случайные данные с заданной схемой данных. Позволяет заполнять тестовые таблицы этими данными. Поддерживаются не все типы данных.'
...
| `random_seed`       | Задает начальное значение генератора случайных чисел вручную для получения стабильных результатов. Если `NULL` — начальное значение генерируется случайным образом. |
| `max_string_length` | Максимальная длина строки для всех генерируемых строк. По умолчанию `10`.                       |
| `max_array_length`  | Максимальное количество элементов для всех генерируемых массивов или map-структур. По умолчанию `10`. |
...
В комбинации с [generateRandomStructure](../../sql-reference/functions/other-functions.md#generateRandomStructure):
...
Без аргумента `structure` (в этом случае структура генерируется случайно):
...
Со случайным значением seed как для случайной структуры, так и для случайных данных:
...
:::note
`generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` при достаточно большом `max_array_length` может сгенерировать очень большой вывод из‑за потенциально большой глубины вложенности (до 16) составных типов (`Array`, `Tuple`, `Map`, `Nested`).
:::
