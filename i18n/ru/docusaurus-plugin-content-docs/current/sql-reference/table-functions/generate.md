...
sidebar_position: 75
description: 'Генерирует случайные данные с заданной схемой. Позволяет заполнять'
...
При отсутствии аргумента `structure` (в этом случае структура случайная):
...
:::note
`generateRandom(generateRandomStructure(), [random seed], max_string_length, max_array_length)` с достаточно большим `max_array_length` может генерировать действительно огромный вывод из-за возможной большой глубины вложенности (до 16) сложных типов (`Array`, `Tuple`, `Map`, `Nested`).
:::

## Смежный контент {#related-content}
