...
Создаёт значение типа [Interval](../../sql-reference/data-types/special-data-types/interval.md), которое следует использовать в арифметических операциях со значениями типов [Date](../../sql-reference/data-types/date.md) и [DateTime](../../sql-reference/data-types/datetime.md).
...
* [toInterval](/sql-reference/functions/type-conversion-functions#tointervalyear) — функция преобразования типов
...
Условный оператор вычисляет значения `b` и `c`, затем проверяет, выполняется ли условие `a`, и после этого возвращает соответствующее значение. Если `b` или `c` является функцией [arrayJoin()](/sql-reference/functions/array-join), каждая строка будет продублирована независимо от условия `a`.
