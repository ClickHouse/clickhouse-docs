...
В целях обратной совместимости, если правая часть представляет собой одно выражение типа `tuple`, оно может интерпретироваться либо как набор значений, либо как одно значение-кортеж — в зависимости от левой части оператора `IN`. Если левая часть является скалярным значением, ClickHouse рассматривает элементы этого единственного выражения `tuple` в правой части как отдельные значения для `IN`:
...
Это работает аналогично `SELECT 1 IN (1, 2)`. Если левая часть также является кортежем, правая часть интерпретируется как набор значений кортежей:
...
Эта специальная обработка применяется только в том случае, когда правая часть является одним выражением типа `tuple`. Скалярная левая часть не может быть сопоставлена с правой частью, содержащей несколько значений кортежа:
...
Подзапрос в предложении `IN` всегда выполняется один раз на одном сервере. Зависимых подзапросов нет.
...
Для `GLOBAL ... JOIN` то, какая сторона соединения вычисляется как подзапрос, зависит от типа соединения: для `LEFT` и `INNER` соединений вычисляется правая таблица; для `RIGHT` соединений вместо неё вычисляется левая таблица, поскольку правая таблица является сохраняемой стороной и должна считываться из сегментов.
...
Это эффективнее, чем использовать обычный `IN`. Однако имейте в виду следующие моменты:
