...
Если `some_predicate` недостаточно селективен, он вернёт большой объём данных и приведёт к проблемам с производительностью. В таких случаях целесообразно ограничить передачу данных по сети. Также обратите внимание, что [`set_overflow_mode`](/operations/settings/settings#set_overflow_mode) по умолчанию установлен в `throw`, что означает генерацию исключения при достижении этих порогов.
