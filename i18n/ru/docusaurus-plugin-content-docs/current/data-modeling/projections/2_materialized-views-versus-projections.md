...
hide_title: false
description: 'Статья, сравнивающая материализованные представления и проекции в ClickHouse, включая сценарии их использования, производительность и ограничения.'
doc_type: 'reference'
keywords: ['materialized views', 'projections', 'differences']
---

> Один из часто задаваемых пользователями вопросов — когда следует использовать материализованные представления, а когда 
> проекции. В этой статье мы рассмотрим ключевые различия между ними и объясним, почему в определённых сценариях 
> может быть предпочтительнее выбрать один вариант вместо другого.
...
| Механизм обновления                                                              | Работают **синхронно** при выполнении `INSERT` в исходную таблицу (для инкрементных материализованных представлений). Примечание: они также могут выполняться **по расписанию** с использованием обновляемых материализованных представлений.                                                                                                                                          | **Асинхронные** обновления в фоновом режиме при выполнении `INSERT` в основную таблицу.                                                                                                                                                                                                            |
...
| Операционные издержки                                                            | Требуют явного создания целевой таблицы и часто ручного заполнения данных. Управление согласованностью при операциях `UPDATE`/`DELETE` увеличивает сложность.                                                                                                                                                                                                                         | Проекции автоматически поддерживаются и синхронизируются, и в целом имеют меньшую операционную нагрузку.                                                                                                                                                                                           |
...
### Когда использовать материализованные представления {#choosing-materialized-views}

Материализованные представления следует использовать в следующих случаях:

- Работа с **конвейерами данных в реальном времени и многоэтапными ETL-процессами:** когда необходимо выполнять сложные преобразования, агрегации или маршрутизацию данных по мере их поступления, потенциально через несколько этапов путём создания цепочки представлений.
...
### Когда не следует использовать материализованные представления {#avoid-materialized-views}

От использования материализованных представлений следует отказаться в следующих случаях:
...
- **Предпочтительны простота и автоматическая оптимизация**: если нужно избежать управления отдельными целевыми таблицами.

### Когда использовать проекции {#choosing-projections}

Проекции следует использовать в следующих случаях:
...
### Когда не следует использовать проекции {#avoid-projections}

От использования проекций следует отказаться в следующих случаях:

- **Требуются сложные преобразования данных или многоэтапные ETL-процессы**: проекции не поддерживают операции `JOIN` в своих определениях, не могут быть изменены для построения многошаговых конвейеров и не могут обрабатывать некоторые возможности SQL, такие как оконные функции или сложные операторы `CASE`. Поэтому они не подходят для сложных преобразований данных.
...
- Запросы с `FINAL` критически важны: проекции не работают с запросами `FINAL`, которые иногда используются для дедупликации.
...
образом, чтобы извлечь максимум из ваших запросов. Таким образом, выбор между материализованными
...
Как правило, следует использовать материализованные представления, когда
необходимо агрегировать данные из одной или нескольких исходных таблиц в целевую таблицу или
выполнять сложные преобразования в больших объемах. Материализованные представления отлично подходят для переноса
работы ресурсоемких агрегаций с момента выполнения запроса на момент вставки данных. Они являются
...
которые фильтруют данные по столбцам, отличным от тех, что используются в первичном ключе таблицы,
определяющем физический порядок данных на диске. Они особенно
полезны, когда изменить первичный ключ таблицы уже невозможно или когда
ваши шаблоны доступа к данным более разнообразны, чем может обеспечить первичный ключ.
