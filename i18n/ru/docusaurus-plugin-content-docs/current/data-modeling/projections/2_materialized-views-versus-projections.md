...
sidebar_label: 'Материализованные представления против проекций'
...
> Один из частых вопросов пользователей — когда лучше использовать материализованные представления, а когда 
проекции. В этой статье мы рассмотрим ключевые различия между ними и объясним, почему в определённых сценариях 
может быть предпочтительно выбрать один подход вместо другого.
...
| Операционные издержки                                                             | Требуют явного создания целевой таблицы и часто ручного заполнения данными. Управление согласованностью при операциях `UPDATE`/`DELETE` увеличивает сложность.                                                                                                                                                                                                                   | Проекции автоматически поддерживаются и синхронизируются, и в целом имеют меньшую операционную нагрузку.                                                                                                                                                                                               |
| Совместимость с запросами `FINAL`                                                      | В целом совместимы, но часто требуют использования `GROUP BY` в целевой таблице.                                                                                                                                                                                                                                                                                                   | **Не работают** с запросами `FINAL`.                                                                                                                                                                                                                                                                  |
...
- **Работа с конвейерами данных реального времени ETL и многоэтапными конвейерами данных:** когда необходимо выполнять сложные преобразования, агрегации или маршрутизацию данных по мере их поступления, потенциально через несколько этапов путем объединения представлений в цепочки.
...
- **Требуется явное управление схемой**: когда необходима отдельная целевая таблица с собственной схемой и движком для предварительно вычисленных результатов, что обеспечивает большую гибкость для моделирования данных.
- **Требуется фильтрация при приеме данных**: когда необходимо фильтровать данные _до_ их материализации, уменьшая объем данных, записываемых в целевую таблицу.

### Когда следует избегать материализованных представлений {#avoid-materialized-views}

Рекомендуется избегать использования материализованных представлений в следующих случаях:
...
- **Оптимизация запросов для одной таблицы**: когда основная цель — ускорить запросы к одной базовой таблице путем предоставления альтернативных порядков сортировки, оптимизации фильтров по столбцам, которые не являются частью первичного ключа, или предварительного вычисления агрегаций для одной таблицы.
- **Требуется прозрачность запросов**: когда запросы должны быть направлены на исходную таблицу без изменений, полагаясь на то, что ClickHouse выберет наилучшую структуру данных для данного запроса.

### Когда следует избегать проекций {#avoid-projections}

Рекомендуется избегать использования проекций в следующих случаях:

- **Требуются сложные преобразования данных или многоэтапные процессы ETL**: проекции не поддерживают операции `JOIN` в своих определениях, не могут быть изменены для построения многошаговых конвейеров и не могут обрабатывать некоторые возможности SQL, такие как оконные функции или сложные операторы `CASE`. Таким образом, они не подходят для сложных преобразований данных.
- **Требуется явная фильтрация материализованных данных**: проекции не поддерживают предложения `WHERE` в своем определении для фильтрации данных, которые материализуются в саму проекцию.
...
- **Запросы с `FINAL` являются необходимыми**: проекции не работают с запросами `FINAL`, которые иногда используются для дедупликации.
...
их использование как взаимоисключающий выбор. Напротив, они могут применяться взаимодополняющим
образом для достижения максимальной эффективности запросов. Таким образом, выбор между материализованными
представлениями и проекциями в ClickHouse зависит от конкретного сценария использования и
паттернов доступа к данным.

Как правило, материализованные представления следует использовать, когда
требуется агрегировать данные из одной или нескольких исходных таблиц в целевую таблицу или
выполнять сложные преобразования в больших масштабах. Материализованные представления отлично подходят для переноса
ресурсоёмких агрегаций с момента выполнения запроса на момент вставки данных. Они являются
отличным выбором для ежедневных или ежемесячных сводок, дашбордов реального времени или агрегированных данных.

С другой стороны, проекции следует использовать, когда необходимо оптимизировать запросы,
фильтрующие данные по столбцам, отличным от тех, которые используются в первичном
ключе таблицы, определяющем физический порядок данных на диске. Они особенно
полезны, когда изменение первичного ключа таблицы уже невозможно или когда
паттерны доступа к данным более разнообразны, чем может обеспечить первичный ключ.
