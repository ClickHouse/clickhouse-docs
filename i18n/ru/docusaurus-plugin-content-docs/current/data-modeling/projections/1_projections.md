...
title: 'Проекции'
description: 'На этой странице рассказывается, что такое проекции, как их можно использовать для повышения производительности запросов и чем они отличаются от материализованных представлений.'
...
ClickHouse предлагает различные механизмы ускорения аналитических запросов к большим
...
запросы путём переупорядочивания данных по интересующим атрибутам. Это может быть:
...
сгенерировать тот же корректный результат, но требует минимального объёма данных для
...
- **Хранение полных столбцов (исходное поведение)**: Проекция содержит полные
...
- **Хранение только ключа сортировки + `_part_offset`**: Проекция работает как индекс.
  ...
  фактические данные из базовой таблицы. Это снижает накладные расходы на хранение за счёт
  незначительного увеличения операций ввода-вывода во время выполнения запроса.

Вышеуказанные подходы также можно комбинировать, храня некоторые столбцы в проекции, а
...
пользователи должны знать, поэтому их следует использовать с осторожностью.

- Проекции не позволяют использовать различные TTL для исходной таблицы и
  (скрытой) целевой таблицы, материализованные представления позволяют использовать различные TTL.
- Лёгкие обновления и удаления не поддерживаются для таблиц с проекциями.
- Материализованные представления могут быть объединены в цепочку: целевая таблица одного материализованного представления
  может быть исходной таблицей другого материализованного представления, и так далее. Это невозможно
  для проекций.
...
- Требуется полная переупорядоченность данных. Хотя выражение в
  ...
  будет использовать проекции, которые применяют простое переупорядочивание, то есть `SELECT * ORDER BY x`.
...
доступный на [sql.clickhouse.com](https://sql.clickhouse.com/), упорядоченный
по `pickup_datetime`.
...
выполнил полное сканирование таблицы. Давайте ускорим этот запрос.
...
Для добавления проекции используется оператор `ALTER TABLE` вместе с оператором `ADD PROJECTION`:
...
Теперь, когда мы добавили проекцию, выполним запрос снова:
...
Обратите внимание, что время выполнения запроса существенно сократилось, а количество просканированных
строк уменьшилось.

Мы можем подтвердить, что наш запрос действительно использовал созданную проекцию,
...
содержащую 30,03 миллиона строк. Этот набор данных также доступен в нашей
среде [sql.clickhouse.com](https://sql.clickhouse.com/?query_id=6IDMHK3OMR1C97J6M9EUQS).

Если вы хотите узнать, как была создана таблица и загружены данные, обратитесь
к странице ["Набор данных о ценах на недвижимость в Великобритании"](/getting-started/example-datasets/uk-price-paid).

Выполним два простых запроса к этому набору данных. Первый выводит список графств в Лондоне с
наибольшими уплаченными ценами, а второй вычисляет среднюю цену для графств:
...
Обратите внимание, что, несмотря на очень высокую скорость, для обоих запросов было выполнено полное сканирование таблицы — всех 30,03 миллиона строк, — поскольку ни `town`, ни `price` не были указаны в операторе `ORDER BY` при создании таблицы:
...
Давайте посмотрим, сможем ли мы ускорить выполнение этого запроса с помощью проекций.

Чтобы сохранить исходную таблицу и результаты, создадим новую таблицу и скопируем данные с помощью оператора `INSERT INTO SELECT`:
...
дополнительную (скрытую) таблицу с первичным ключом, упорядоченную по городу и цене, чтобы
оптимизировать запрос, выводящий список округов в заданном городе с самыми высокими
ценами:
...
которая инкрементально предварительно вычисляет агрегатные значения функции avg(price) для всех
130 существующих графств Великобритании:
...
:::note
Если в проекции, как в проекции `prj_gby_county` выше, используется предложение `GROUP BY`,
то базовым движком хранения для (скрытой) таблицы становится `AggregatingMergeTree`, а все агрегатные функции
преобразуются в `AggregateFunction`. Это обеспечивает корректную поэтапную (инкрементальную) агрегацию данных.
:::
...
Если теперь повторно выполнить запрос, который выводит графства Лондона с тремя
наибольшими ценами сделок, мы увидим улучшение производительности запроса:
...
Аналогично, для запроса, который выводит графства Великобритании с тремя самыми высокими средними ценами покупки:
...
Обратите внимание, что оба запроса обращаются к исходной таблице и что оба привели
к полному сканированию таблицы (все 30,03 млн строк были прочитаны с диска) до того, как мы
создали две проекции.

Также обратите внимание, что запрос, который перечисляет графства в Лондоне с тремя
самыми высокими ценами продажи, считывает 2,17 млн строк. Когда мы напрямую использовали вторую таблицу,
оптимизированную под этот запрос, с диска было прочитано только 81,92 тыс. строк.

Причина этой разницы состоит в том, что в настоящее время оптимизация `optimize_read_in_order`,
упомянутая выше, не поддерживается для проекций.

Мы просматриваем таблицу `system.query_log`, чтобы увидеть, что ClickHouse
автоматически использовал две проекции для обоих указанных выше запросов (см. столбец projections ниже):
...
В следующих примерах используется тот же набор данных о ценах в Великобритании, сравниваются запросы с проекциями и без них.
...
Следующие запросы сравнивают производительность с проекциями и без них. Чтобы отключить использование проекций, применяется настройка [`optimize_use_projections`](/operations/settings/settings#optimize_use_projections), которая включена по умолчанию.
...
:::note
Примечание: Таблица использует специальные настройки для иллюстрации, такие как гранулы из одной строки
...
не помогает в данном случае, поэтому ClickHouse использует:
...
Результат выполнения `EXPLAIN` (показан выше) раскрывает логический план запроса, сверху вниз:
...
Комбинируя анализ индексов нескольких проекций, ClickHouse существенно сокращает объём сканируемых данных,
повышая производительность при низких накладных расходах на хранилище.
