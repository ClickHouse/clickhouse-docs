...
Понимание принципов эффективного проектирования схем — ключ к оптимизации производительности ClickHouse. Оно предполагает выбор решений, часто связанных с компромиссами, при этом оптимальный подход зависит от выполняемых запросов, а также таких факторов, как частота обновления данных, требования по задержке и объём данных. В этом руководстве представлен обзор лучших практик проектирования схем и методов моделирования данных для оптимизации производительности ClickHouse.


## Набор данных Stack Overflow {#stack-overflow-dataset}

Для примеров в этом руководстве мы используем подмножество набора данных Stack Overflow. Оно содержит все публикации, голоса, пользователей, комментарии и значки, которые появились на Stack Overflow с 2008 года по апрель 2024 года. Эти данные доступны в формате Parquet по схемам, приведенным ниже, в S3-бакете `s3://datasets-documentation/stackoverflow/parquet/`:

> Указанные первичные ключи и связи не обеспечиваются ограничениями (Parquet — это формат файла, а не таблицы) и служат исключительно для указания того, как данные связаны между собой и какие уникальные ключи они содержат.
...
Мы указали MergeTree в качестве движка таблицы. MergeTree — это наиболее распространённый движок таблиц ClickHouse, который вы, скорее всего, будете использовать. Это универсальный инструмент в вашем арсенале ClickHouse, способный обрабатывать петабайты данных и обслуживать большинство аналитических сценариев. Существуют и другие движки таблиц для таких сценариев, как CDC, которые требуют поддержки эффективных обновлений.

Конструкция `ORDER BY ()` означает, что у нас нет индекса и, более конкретно, нет порядка в данных. Подробнее об этом позже. Пока просто имейте в виду, что все запросы будут требовать линейного сканирования.
...
После определения начальной схемы можно заполнить таблицу данными с помощью `INSERT INTO SELECT`, считывая данные через табличную функцию s3. Следующий запрос загружает данные `posts` примерно за 2 минуты на 8-ядерном инстансе ClickHouse Cloud.
...
> Приведённый выше запрос загружает 60 млн строк. Хотя для ClickHouse это немного, пользователи с более медленным интернет-соединением могут предпочесть загрузить только часть данных. Это можно сделать, просто указав годы, которые нужно загрузить, с помощью glob-шаблона, например `https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/2008.parquet` или `https://datasets-documentation.s3.eu-west-3.amazonaws.com/stackoverflow/parquet/posts/{2008, 2009}.parquet`. О том, как использовать glob-шаблоны для выбора подмножеств файлов, см. [здесь](/sql-reference/table-functions/file#globs-in-path).


## Оптимизация типов данных {#optimizing-types}

Один из секретов высокой производительности запросов в ClickHouse — это сжатие данных.
...
- **Используйте LowCardinality** — Числовые столбцы, строки, столбцы Date или DateTime с небольшим количеством уникальных значений потенциально могут быть закодированы с использованием типа LowCardinality. Этот словарный метод кодирует значения, уменьшая размер на диске. Рассмотрите это для столбцов с менее чем 10 тысячами уникальных значений.
...
- **Enum для валидации данных** — Тип Enum можно использовать для эффективного кодирования перечислимых типов. Enum может быть 8 или 16 бит в зависимости от количества уникальных значений, которые необходимо хранить. Рассмотрите возможность использования этого типа, если вам нужна соответствующая валидация при вставке (необъявленные значения будут отклонены) или вы хотите выполнять запросы, использующие естественный порядок значений Enum, например, представьте столбец обратной связи, содержащий ответы пользователей `Enum(':(' = 1, ':|' = 2, ':)' = 3)`.

> Совет: Чтобы найти диапазон всех столбцов и количество уникальных значений, пользователи могут использовать простой запрос `SELECT * APPLY min, * APPLY  max, * APPLY uniq FROM table FORMAT Vertical`. Рекомендуем выполнять это на меньшем подмножестве данных, так как это может быть ресурсоемко. Этот запрос требует, чтобы числовые значения были определены как таковые для получения точного результата, то есть не как String.

Применяя эти простые правила к нашей таблице постов, мы можем определить оптимальный тип для каждого столбца:


| Столбец                 | Является числовым | Мин, Макс                                                    | Уникальные значения | NULL-значения | Комментарий                                                                                                       | Оптимальный тип                                                                                                                                              |
| ----------------------- | ----------------- | ------------------------------------------------------------ | ------------------- | ------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
| `CreationDate`          | Нет               | 2008-07-31 21:42:52.667000000, 2024-03-31 23:59:17.697000000 | *                   | Нет           | Если милисекундная точность не нужна, используйте DateTime                                                        | DateTime                                                                                                                                                     |
...
| `LastEditorUserId`      | Да                | -1, 9999993                                                  | 1104694             | Да            | 0 — неиспользуемое значение, которое можно использовать для значений Null                                         | Int32                                                                                                                                                        |
| `LastEditorDisplayName` | Нет               | *                                                            | 70952               | Да            | Считайте Null пустой строкой. Протестировали LowCardinality — выигрыша нет                                        | Строка                                                                                                                                                       |
...
| `Title`                 | Нет               | -                                                            | *                   | Нет           | Считать Null пустой строкой                                                                                       | Строка                                                                                                                                                       |
| `Теги`                  | Нет               | -                                                            | *                   | Нет           | Считать значение NULL пустой строкой                                                                              | Строка                                                                                                                                                       |
...
| `FavoriteCount`         | Да                | 0, 225                                                       | 6                   | Да            | Считать Null и 0 равными                                                                                          | UInt8                                                                                                                                                        |
...
Мы можем заполнить её с помощью простого `INSERT INTO SELECT`, считывая данные из нашей предыдущей таблицы и вставляя их в эту:
...
Мы не храним значения `null` в нашей новой схеме. Приведённый выше `INSERT` неявно преобразует их в значения по умолчанию для соответствующих типов — `0` для целых чисел и пустую строку для строковых типов. ClickHouse также автоматически приводит любые числовые значения к требуемой точности.
Первичные (упорядочивающие) ключи в ClickHouse
Пользователи, переходящие с OLTP-баз данных, часто ищут эквивалентную концепцию в ClickHouse.
...
Возможно, в нашем случае мы ожидаем, что пользователи всегда будут фильтровать по `PostTypeId`. Его кардинальность равна 8, и он является логичным выбором для первого элемента в нашем ключе сортировки. Понимая, что фильтрации по дате с нужной точностью, скорее всего, будет достаточно (она по‑прежнему принесет пользу фильтрам по datetime), мы используем `toDate(CreationDate)` как второй компонент нашего ключа. Это также приведет к меньшему индексу, так как дату можно представить 16‑битным значением, что ускорит фильтрацию. Наш заключительный элемент ключа — `CommentCount`, который помогает находить посты с наибольшим числом комментариев (для окончательной сортировки).
...
Пользователи, которых интересуют улучшения сжатия, достигаемые за счет использования определенных типов и подходящих ключей упорядочивания, могут обратиться к разделу [Compression in ClickHouse](/data-compression/compression-in-clickhouse). Если требуется еще больше повысить степень сжатия, мы также рекомендуем раздел [Choosing the right column compression codec](/data-compression/compression-in-clickhouse#choosing-the-right-column-compression-codec).
...
> В ClickHouse отсутствует понятие внешних ключей. Это не запрещает соединения, но означает, что ссылочная целостность остается на усмотрение пользователя и управляется на уровне приложения. В OLAP-системах, таких как ClickHouse, целостность данных часто обеспечивается на уровне приложения или в процессе загрузки данных, а не самой базой данных, где это создает значительные накладные расходы. Такой подход обеспечивает большую гибкость и более быструю вставку данных. Это соответствует фокусу ClickHouse на скорости и масштабируемости запросов чтения и вставки при работе с очень большими наборами данных.

Чтобы минимизировать использование соединений во время выполнения запросов, пользователи могут применять несколько инструментов и подходов:
...
- [**Инкрементальные материализованные представления**](/materialized-view/incremental-materialized-view) — функция ClickHouse для переноса вычислительных затрат с момента выполнения запроса на момент вставки данных, включая возможность инкрементального вычисления агрегированных значений.
- [**Обновляемые материализованные представления**](/materialized-view/refreshable-materialized-view) — аналогично материализованным представлениям, используемым в других СУБД, позволяет периодически вычислять результаты запроса и кэшировать их.

Мы рассматриваем каждый из этих подходов в соответствующих руководствах, указывая, когда каждый из них уместен, с примерами применения для решения задач с набором данных Stack Overflow.
