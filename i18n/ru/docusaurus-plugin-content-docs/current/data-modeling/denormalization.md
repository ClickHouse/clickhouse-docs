...
Распространенная техника, популяризированная NoSQL-решениями, заключается в денормализации данных при отсутствии поддержки `JOIN`, фактически сохраняя всю статистику или связанные строки в родительской строке в виде столбцов и вложенных объектов. Например, в примере схемы для блога мы можем хранить все комментарии (`Comments`) в виде массива (`Array`) объектов в соответствующих постах.


## Когда использовать денормализацию {#when-to-use-denormalization}

В целом мы рекомендуем применять денормализацию в следующих случаях:

- Денормализуйте таблицы, которые изменяются нечасто или для которых допустима задержка перед тем, как данные станут доступны для аналитических запросов, то есть когда данные можно полностью перезагрузить пакетно.
...
- Избегайте денормализации связей с высокой кардинальностью. Если каждая строка в таблице имеет тысячи связанных записей в другой таблице, их потребуется представить в виде массива `Array` — либо примитивного типа, либо кортежей. Как правило, массивы с более чем 1000 кортежей не рекомендуются.
- Вместо денормализации всех столбцов в виде вложенных объектов рассмотрите возможность денормализации только агрегированных показателей с использованием материализованных представлений (см. ниже).
...
Рассмотрим несколько практических примеров, когда денормализация может быть оправдана, а также случаи, когда предпочтительнее использовать альтернативные подходы.
...
Голоса за посты представлены в виде отдельных таблиц. Оптимизированная схема для этого показана ниже вместе с командой вставки для загрузки данных:
...
Голоса часто добавляются к постам. Хотя со временем это может уменьшаться для отдельных постов, следующий запрос показывает, что у нас около 40 тысяч голосов в час для более чем 30 тысяч постов.
...
Основное наблюдение здесь заключается в том, что агрегированной статистики голосов для каждого поста было бы достаточно для большинства анализов — нам не нужно денормализовать всю информацию о голосах. Например, текущий столбец `Score` представляет такую статистику, то есть общее количество положительных голосов минус отрицательные голоса. В идеале мы могли бы просто получать эту статистику во время выполнения запроса с помощью простого поиска (см. [словари](/dictionary)).
...
<p></p>
Сначала вставляем данные с помощью следующей команды:
<p></p>
...
Можно убедиться, что ни один пост не имеет чрезмерного количества ссылок, препятствующих денормализации:
...
Мы используем этот пример для демонстрации денормализации ниже.

### Пример простой статистики {#simple-statistic-example}

В большинстве случаев денормализация требует добавления одного столбца или статистики к родительской строке. Например, мы можем захотеть обогатить наши посты количеством дублирующихся постов — для этого достаточно добавить один столбец.
...
Для заполнения этой таблицы используем `INSERT INTO SELECT`, объединяя статистику дубликатов с нашими постами.
...
Для выполнения денормализации часто требуется использовать сложные типы. Если денормализуется связь один-к-одному с небольшим количеством столбцов, можно просто добавить их как строки с исходными типами, как показано выше. Однако это часто нежелательно для больших объектов и невозможно для связей один-ко-многим.

В случае сложных объектов или связей один-ко-многим можно использовать:
...
В качестве примера продемонстрируем денормализацию `PostLinks` в `Posts`.

Каждый пост может содержать несколько ссылок на другие посты, как показано в схеме `PostLinks` ранее. В виде типа Nested эти связанные и дублирующиеся посты можно представить следующим образом:
...
> Обратите внимание на использование настройки `flatten_nested=0`. Рекомендуется отключить выравнивание вложенных данных.

Эту денормализацию можно выполнить с помощью `INSERT INTO SELECT` с запросом `OUTER JOIN`:
...
> Обратите внимание на время выполнения. Нам удалось денормализовать 66 млн строк примерно за 2 минуты. Как мы увидим далее, эту операцию можно запланировать.


Обратите внимание на использование функций `groupArray`, с помощью которых `PostLinks` сворачиваются в массив для каждого `PostId` перед выполнением соединения. Затем этот массив фильтруется на два подсписка: `LinkedPosts` и `DuplicatePosts`, при этом из них также исключаются любые пустые результаты внешнего соединения.
...
Применение денормализации требует процесса преобразования данных, в рамках которого она может быть выполнена и оркестрирована.
...
У пользователей есть несколько вариантов оркестрации этого процесса в ClickHouse, если приемлем периодический пакетный процесс загрузки:
...
Пользователи также могут выполнять это за пределами ClickHouse, до вставки данных, используя потоковые технологии, такие как [Apache Flink](https://flink.apache.org/). Альтернативный вариант — использование инкрементных [материализованных представлений](/guides/developer/cascading-materialized-views) для выполнения этого процесса по мере вставки данных.
