...
- Добавлена настройка `low_cardinality_allow_in_native_format` (включена по умолчанию). При отключении столбцы LowCardinality будут преобразовываться в обычные столбцы для запросов SELECT, а для запросов INSERT будут ожидаться обычные столбцы. [#3879](https://github.com/ClickHouse/ClickHouse/pull/3879)
...
* Теперь вы можете использовать `KILL QUERY` для отмены запросов, которые ещё не запустились, так как ожидают блокировки таблицы. [#3517](https://github.com/ClickHouse/ClickHouse/pull/3517)
* Исправлены вычисления даты и времени в случае, когда перевод часов назад выполнялся в полночь (это происходит в Иране, а также происходило в Москве с 1981 по 1983 год). Ранее это приводило к тому, что время сбрасывалось на день раньше, чем требовалось, а также вызывало некорректное форматирование даты и времени в текстовом виде. [#3819](https://github.com/ClickHouse/ClickHouse/pull/3819)
* Исправлены ошибки, возникавшие в некоторых случаях использования `VIEW` и подзапросов без указания базы данных. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3521)
...
* Исправлена обработка запросов при включённой опции `compile_expressions` (по умолчанию она включена). Недетерминированные константные выражения, такие как функция `now`, больше не раскрываются. [#3457](https://github.com/ClickHouse/ClickHouse/pull/3457)
...
* Исправлено непрерывное протоколирование ошибок в `DDLWorker`, когда ZooKeeper недоступен. [8f50c620](https://github.com/ClickHouse/ClickHouse/commit/8f50c620334988b28018213ec0092fe6423847e2)
* Исправлен возвращаемый тип для функций `quantile*` с аргументами типов `Date` и `DateTime`. [#3580](https://github.com/ClickHouse/ClickHouse/pull/3580)
* Исправлена обработка предложения `WITH`, если в нём задаётся простой псевдоним без выражений. [#3570](https://github.com/ClickHouse/ClickHouse/pull/3570)
...
* Исправлена ошибка, приводившая к сбою при передаче некоторых некорректных аргументов функции `arrayReverse`. [73e3a7b6](https://github.com/ClickHouse/ClickHouse/commit/73e3a7b662161d6005e7727d8a711b930386b871)
...
* Исправлено переполнение буфера в функциях `lowerUTF8` и `upperUTF8`. Удалена возможность выполнять эти функции над аргументами типа `FixedString`. [#3662](https://github.com/ClickHouse/ClickHouse/pull/3662)
* Исправлена редкая состояние гонки при удалении таблиц `MergeTree`. [#3680](https://github.com/ClickHouse/ClickHouse/pull/3680)
...
* Исправлено падение с ошибкой сегментации при превышении лимита `max_temporary_non_const_columns`. [#3788](https://github.com/ClickHouse/ClickHouse/pull/3788)

#### Улучшения: {#improvements-1}


* Сервер не записывает обработанные файлы конфигурации в каталог `/etc/clickhouse-server/`. Вместо этого он сохраняет их в каталоге `preprocessed_configs` внутри `path`. Это означает, что у каталога `/etc/clickhouse-server/` нет прав на запись для пользователя `clickhouse`, что повышает безопасность. [#2443](https://github.com/ClickHouse/ClickHouse/pull/2443)
* Опция `min_merge_bytes_to_use_direct_io` по умолчанию установлена в 10 GiB. Слияние, при котором формируются крупные части таблиц семейства MergeTree, будет выполняться в режиме `O_DIRECT`, что предотвращает чрезмерное вытеснение страниц из кеша. [#3504](https://github.com/ClickHouse/ClickHouse/pull/3504)
* Ускорен запуск сервера при очень большом числе таблиц. [#3398](https://github.com/ClickHouse/ClickHouse/pull/3398)
* Добавлен пул подключений и HTTP `Keep-Alive` для соединений между репликами. [#3594](https://github.com/ClickHouse/ClickHouse/pull/3594)
...
* Удалён вывод логов в `stderr` из библиотеки `re2` для некорректных или слишком сложных регулярных выражений. [#3723](https://github.com/ClickHouse/ClickHouse/pull/3723)
* Добавлено для движка таблиц `Kafka`: проверка наличия подписок перед началом чтения из Kafka; настройка kafka&#95;max&#95;block&#95;size для таблицы. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3396)
...
* Для `FULL` и `RIGHT JOIN` настройка `max_block_size` используется для потока несоединённых данных из правой таблицы. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3699)
...
* Преобразование типов, когда структура таблицы типа `Buffer` не совпадает со структурой таблицы назначения. [Vitaly Baranov](https://github.com/ClickHouse/ClickHouse/pull/3603)
* Добавлена настройка `tcp_keep_alive_timeout` для включения keep-alive-пакетов после заданного периода неактивности. [#3441](https://github.com/ClickHouse/ClickHouse/pull/3441)
...
* Добавлены синонимы для функций `POWER`, `LN`, `LCASE`, `UCASE`, `REPLACE`, `LOCATE`, `SUBSTR` и `MID`. [#3774](https://github.com/ClickHouse/ClickHouse/pull/3774) [#3763](https://github.com/ClickHouse/ClickHouse/pull/3763) Некоторые имена функций становятся регистронезависимыми для совместимости со стандартом SQL. Добавлен синтаксический сахар `SUBSTRING(expr FROM start FOR length)` для совместимости с SQL. [#3804](https://github.com/ClickHouse/ClickHouse/pull/3804)
* Добавлена возможность выполнять `mlock` для страниц памяти, соответствующих исполняемому коду `clickhouse-server`, чтобы предотвратить их вытеснение из памяти. Эта функция по умолчанию отключена. [#3553](https://github.com/ClickHouse/ClickHouse/pull/3553)
* Улучшена производительность чтения с использованием `O_DIRECT` (при включённой опции `min_bytes_to_use_direct_io`). [#3405](https://github.com/ClickHouse/ClickHouse/pull/3405)
...
* Возможность задавать пользовательские переменные окружения для запуска `clickhouse-server` с помощью скрипта `SYS-V init.d`, определяя `CLICKHOUSE_PROGRAM_ENV` в `/etc/default/clickhouse`.
...
* Удалена возможность сравнивать тип `Date` с числом, чтобы избежать потенциальных ошибок вида `date = 2018-12-17`, когда кавычки вокруг значения даты по ошибке опущены. [#3687](https://github.com/ClickHouse/ClickHouse/pull/3687)
* Исправлено поведение состояниях функций, таких как `rowNumberInAllBlocks`. Ранее они возвращали результат, увеличенный на единицу, из‑за запуска во время анализа запроса. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3729)
* Если файл `force_restore_data` не удаётся удалить, отображается сообщение об ошибке. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3794)
...
- Добавлен фаззинг-тест выражений в SELECT-запросах. [#3442](https://github.com/ClickHouse/ClickHouse/pull/3442)
...
- Исправлена взаимоблокировка запроса в случае, когда создание потока запроса завершается с ошибкой `Resource temporarily unavailable`. [#3643](https://github.com/ClickHouse/ClickHouse/pull/3643)
...
- Вложенные структуры данных могут читаться как вложенные объекты в формате `JSONEachRow`. Добавлена настройка `input_format_import_nested_json`. [Veloman Yunkan](https://github.com/ClickHouse/ClickHouse/pull/3144)
...
- Теперь можно использовать предопределённые макросы `database` и `table` при объявлении таблиц `Replicated`. [#3251](https://github.com/ClickHouse/ClickHouse/pull/3251)
...
* Значительно снижено потребление памяти для запросов с `ORDER BY` и `LIMIT`. См. настройку `max_bytes_before_remerge_sort`. [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
...
* Движок таблиц `ODBC` корректно выбирает способ кавычкования идентификаторов в SQL-диалекте удалённой базы данных. [Alexandr Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/3210)
* Настройка `compile_expressions` (JIT-компиляция выражений) по умолчанию включена.
...
* Выражения LIKE и IN с константной правой частью теперь передаются на удалённый сервер при выполнении запросов к таблицам MySQL или ODBC. [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
...
* Корректный расчёт ширины строки в терминале для форматов `Pretty`, включая строки с иероглифами. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3257).
* `ON CLUSTER` может быть указан в запросах `ALTER UPDATE`.
* Повышена производительность чтения данных в формате `JSONEachRow`. [#3332](https://github.com/ClickHouse/ClickHouse/pull/3332)
* Добавлены синонимы функций `LENGTH` и `CHARACTER_LENGTH` для совместимости. Функция `CONCAT` больше не чувствительна к регистру. [#3306](https://github.com/ClickHouse/ClickHouse/pull/3306)
* Добавлен синоним `TIMESTAMP` для типа `DateTime`. [#3390](https://github.com/ClickHouse/ClickHouse/pull/3390)
* В журналах сервера всегда зарезервировано место под query&#95;id, даже если строка журнала не связана с запросом. Это упрощает разбор текстовых журналов сервера с помощью сторонних инструментов.
* Потребление памяти запросом записывается в лог, когда оно превышает очередной целый гигабайт. [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* Добавлен режим совместимости для случая, когда клиентская библиотека, использующая протокол Native, по ошибке отправляет меньше столбцов, чем сервер ожидает в запросе INSERT. Такая ситуация могла возникать при использовании библиотеки clickhouse-cpp. Ранее она приводила к сбою сервера. [#3171](https://github.com/ClickHouse/ClickHouse/pull/3171)
* В пользовательском выражении WHERE в `clickhouse-copier` теперь можно использовать псевдоним `partition_key` (для дополнительной фильтрации по партиции исходной таблицы). Это полезно, если во время копирования схема партиционирования меняется, но лишь незначительно. [#3166](https://github.com/ClickHouse/ClickHouse/pull/3166)
* Работа движка `Kafka` была перенесена в фоновый пул потоков, чтобы автоматически снижать скорость чтения данных при высокой нагрузке. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215).
* Поддержка чтения значений типов `Tuple` и `Nested` для структур, аналогичных `struct`, в формате `Cap'n'Proto`. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3216)
* Список доменов верхнего уровня для функции `firstSignificantSubdomain` теперь включает зону `biz`. [decaseal](https://github.com/ClickHouse/ClickHouse/pull/3219)
* В конфигурации внешних словарей параметр `null_value` интерпретируется как значение типа данных по умолчанию. [#3330](https://github.com/ClickHouse/ClickHouse/pull/3330)
...
* Поддержка использования типов `Date`, `DateTime`, `UUID` и `Decimal` в качестве ключа для агрегатной функции `sumMap`. [#3281](https://github.com/ClickHouse/ClickHouse/pull/3281)
* Поддержка типа данных `Decimal` для внешних словарей. [#3324](https://github.com/ClickHouse/ClickHouse/pull/3324)
...
* Уменьшено количество системных вызовов `open` и `close` при чтении из таблиц `MergeTree`. [#3283](https://github.com/ClickHouse/ClickHouse/pull/3283)
* Запрос `TRUNCATE TABLE` можно выполнять на любой реплике (он будет передан на ведущую реплику). [Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/3375)

#### Исправления ошибок: {#bug-fixes-10}


* Исправлена проблема с таблицами `Dictionary` для словарей типа `range_hashed`. Ошибка проявлялась в версии 18.12.17. [#1702](https://github.com/ClickHouse/ClickHouse/pull/1702)
* Исправлена ошибка при загрузке словарей `range_hashed` (сообщение `Unsupported type Nullable (...)`), возникавшая в версии 18.12.17. [#3362](https://github.com/ClickHouse/ClickHouse/pull/3362)
...
* Если после слияния кусков данных `checksum` для результирующей части отличается от результата того же слияния на другой реплике, результат слияния удаляется, а часть данных загружается с другой реплики (это корректное поведение). Но после загрузки части данных её не удавалось добавить в рабочий набор из‑за ошибки о том, что часть уже существует (поскольку часть данных была удалена с некоторой задержкой после слияния). Это приводило к циклическим попыткам загрузки одних и тех же данных. [#3194](https://github.com/ClickHouse/ClickHouse/pull/3194)
* Исправлено некорректное вычисление общего потребления памяти запросами (из‑за неправильного вычисления настройка `max_memory_usage_for_all_queries` работала некорректно, а метрика `MemoryTracking` имела неверное значение). Эта ошибка возникла в версии 18.12.13. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3344)
...
* Исправлена избыточная подготовка структур данных для `JOIN` на сервере, инициирующем запрос, если `JOIN` выполняется только на удалённых серверах. [#3340](https://github.com/ClickHouse/ClickHouse/pull/3340)
* Исправлены ошибки в движке `Kafka`: взаимоблокировки после исключений при начале чтения данных и блокировки при завершении работы [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215).
...
* Если в кластере серверов ZooKeeper есть серверы, которые принимают соединение, но затем сразу же закрывают его, не отвечая на handshake, ClickHouse переключается на подключение к другому серверу. Ранее это приводило к ошибке `Cannot read all data. Bytes read: 0. Bytes expected: 4.`, и сервер не мог запуститься. [8218cf3a](https://github.com/ClickHouse/ClickHouse/commit/8218cf3a5f39a43401953769d6d12a0bb8d29da9)
...
* Исправлена ошибка разбора формата CSV с числами с плавающей запятой при использовании нестандартного разделителя CSV, такого как `;` [#3155](https://github.com/ClickHouse/ClickHouse/pull/3155)
...
* Исправлены ошибки в табличной функции `odbc`, возникавшие в версии 18.12. [#3197](https://github.com/ClickHouse/ClickHouse/pull/3197)
* Исправлена работа агрегатных функций с комбинаторами `StateArray`. [#3188](https://github.com/ClickHouse/ClickHouse/pull/3188)
* Исправлено аварийное завершение работы при делении значения типа `Decimal` на ноль. [69dd6609](https://github.com/ClickHouse/ClickHouse/commit/69dd6609193beb4e7acd3e6ad216eca0ccfb8179)
...
* Исправлена ошибка сегментации при `GROUP BY` по полю типа `Decimal128`. [3359ba06](https://github.com/ClickHouse/ClickHouse/commit/3359ba06c39fcd05bfdb87d6c64154819621e13a)
* Настройка `log_query_threads` (логирование информации о каждом потоке выполнения запроса) теперь действует только при условии, что параметр `log_queries` (логирование информации о запросах) установлен в 1. Поскольку параметр `log_query_threads` включён по умолчанию, ранее информация о потоках логировалась даже при отключённом логировании запросов. [#3241](https://github.com/ClickHouse/ClickHouse/pull/3241)
* Исправлена ошибка в распределённой обработке агрегатной функции quantiles (сообщение об ошибке `Not found column quantile...`). [292a8855](https://github.com/ClickHouse/ClickHouse/commit/292a885533b8e3b41ce8993867069d14cbd5a664)
* Исправлена проблема совместимости при одновременной работе в кластере с серверами версии 18.12.17 и более старыми серверами. Для распределённых запросов с ключами GROUP BY как фиксированной, так и переменной длины при большом объёме данных для агрегации возвращаемые данные не всегда были полностью агрегированы (две разные строки содержали одинаковые ключи агрегации). [#3254](https://github.com/ClickHouse/ClickHouse/pull/3254)
* Исправлена обработка подстановок в `clickhouse-performance-test`, когда запрос содержит только часть подстановок, объявленных в тесте. [#3263](https://github.com/ClickHouse/ClickHouse/pull/3263)
...
* Исправлена ошибка сегментации, которая в редких случаях могла возникать после оптимизации, заменяющей цепочки условий AND с проверкой на равенство соответствующим выражением IN. [liuyimin-bytedance](https://github.com/ClickHouse/ClickHouse/pull/3339)
* Незначительные исправления в `clickhouse-benchmark`: ранее информация о клиенте не отправлялась на сервер; теперь при завершении работы и при ограничении числа итераций количество выполненных запросов определяется более точно. [#3351](https://github.com/ClickHouse/ClickHouse/pull/3351) [#3352](https://github.com/ClickHouse/ClickHouse/pull/3352)
...
- Добавлена опция `min_merge_bytes_to_use_direct_io` для движков `MergeTree`, которая позволяет установить порог для общего размера слияния (при превышении порога файлы частей данных будут обрабатываться с использованием O_DIRECT). [#3117](https://github.com/ClickHouse/ClickHouse/pull/3117)
...
- Исправлена ошибка при обновлении внешних словарей с источником `ODBC` и типом хранилища `hashed`. Эта ошибка появилась в версии 18.12.13.
...
* В запросах с JOIN символ «*» разворачивается в список столбцов во всех таблицах, в соответствии со стандартом SQL. Вы можете вернуть старое поведение, установив параметр `asterisk_left_columns_only` в значение 1 на уровне пользовательской конфигурации. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2787)
...
* Ctrl+C в clickhouse-client очищает введённый запрос. [#2877](https://github.com/ClickHouse/ClickHouse/pull/2877)
* Добавлена настройка `join_default_strictness` (значения: `"`, `'any'`, `'all'`). Она позволяет не указывать `ANY` или `ALL` для `JOIN`. [#2982](https://github.com/ClickHouse/ClickHouse/pull/2982)
* Каждая строка серверного журнала, относящаяся к обработке запроса, содержит идентификатор запроса. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Теперь вы можете получать журналы выполнения запросов в clickhouse-client (используйте настройку `send_logs_level`). При распределённой обработке запросов журналы передаются (каскадируются) со всех серверов. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Таблицы `system.query_log` и `system.processes` (`SHOW PROCESSLIST`) теперь содержат информацию обо всех изменённых настройках при выполнении запроса (во вложенной структуре данных `Settings`). Добавлена настройка `log_query_settings`. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Таблицы `system.query_log` и `system.processes` теперь показывают информацию о числе потоков, участвующих в выполнении запроса (см. столбец `thread_numbers`). [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Добавлены счетчики `ProfileEvents`, измеряющие время, затраченное на чтение и запись по сети и на диск, количество сетевых ошибок, а также время ожидания при ограничении пропускной способности сети. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Добавлены счётчики `ProfileEvents`, содержащие системные метрики из rusage (их можно использовать для получения информации об использовании CPU в пространстве пользователя и ядра, сбоях страниц и переключениях контекста), а также метрики taskstats (их можно использовать для получения информации о времени ожидания I/O, времени ожидания CPU и объёме прочитанных и записанных данных — как с использованием page cache, так и без него). [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Счетчики `ProfileEvents` применяются глобально, для каждого запроса, а также для каждого потока выполнения запроса, что позволяет детально профилировать потребление ресурсов по каждому запросу. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
...
* Теперь вы можете складывать (объединять) состояния агрегатных функций с помощью оператора `+` и умножать состояния агрегатных функций на неотрицательную константу. [#3062](https://github.com/ClickHouse/ClickHouse/pull/3062) [#3034](https://github.com/ClickHouse/ClickHouse/pull/3034)
...
- Добавлено кеширование JIT-компилируемых функций и счётчик числа использований до компиляции. Для JIT-компиляции выражений включите настройку `compile_expressions`. [#2990](https://github.com/ClickHouse/ClickHouse/pull/2990) [#3077](https://github.com/ClickHouse/ClickHouse/pull/3077)

#### Улучшения: {#improvements-4}


* Исправлена проблема с неограниченным ростом журнала репликации при наличии заброшенных реплик. Добавлен эффективный режим восстановления для реплик с большим лагом.
...
* Улучшена производительность при использовании `PREWHERE` и при неявном переносе выражений в `PREWHERE`.
* Улучшена производительность разбора для текстовых форматов (`CSV`, `TSV`). [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2977) [#2980](https://github.com/ClickHouse/ClickHouse/pull/2980)
...
* Повышена производительность и снижено потребление памяти при выполнении запросов к `system.tables` и `system.columns` в случае очень большого количества таблиц на одном сервере. [#2953](https://github.com/ClickHouse/ClickHouse/pull/2953)
* Исправлена проблема с производительностью при большом потоке запросов, завершающихся ошибкой (функция `_dl_addr` видна в `perf top`, но сервер почти не нагружает CPU). [#2938](https://github.com/ClickHouse/ClickHouse/pull/2938)
* Условия проталкиваются в представление (View) (при включенной настройке `enable_optimize_predicate_expression`). [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
...
* Когда настройка `input_format_skip_unknown_fields` включена, поля объектов в формате `JSONEachRow` корректно игнорируются. [BlahGeek](https://github.com/ClickHouse/ClickHouse/pull/2958)
* Для выражения `CASE` с условиями теперь можно опустить `ELSE`, что будет эквивалентно `ELSE NULL`. [#2920](https://github.com/ClickHouse/ClickHouse/pull/2920)
* Таймаут операции теперь можно настроить при работе с ZooKeeper. [urykhy](https://github.com/ClickHouse/ClickHouse/pull/2971)
* Вы можете задать смещение для `LIMIT n, m` в виде `LIMIT n OFFSET m`. [#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
...
* Дублирующиеся столбцы могут использоваться в секции `USING` для `JOIN`. [#3006](https://github.com/ClickHouse/ClickHouse/pull/3006)
* Форматы `Pretty` теперь ограничивают выравнивание столбцов по ширине. Используйте настройку `output_format_pretty_max_column_pad_width`. Если значение шире этого ограничения, оно по-прежнему будет отображаться полностью, но остальные ячейки таблицы не будут чрезмерно широкими. [#3003](https://github.com/ClickHouse/ClickHouse/pull/3003)
...
* Добавлена рандомизация при периодическом запуске потока очистки для таблиц `ReplicatedMergeTree`, чтобы избежать периодических пиков нагрузки при очень большом количестве таких таблиц.
* Поддержка запроса `ATTACH TABLE ... ON CLUSTER`. [#3025](https://github.com/ClickHouse/ClickHouse/pull/3025)

#### Исправления ошибок: {#bug-fixes-13}


* Исправлена проблема с таблицами `Dictionary` (выбрасывалось исключение `Size of offsets does not match size of column` или `Unknown compression method`). Эта ошибка появилась в версии 18.10.3. [#2913](https://github.com/ClickHouse/ClickHouse/issues/2913)
* Исправлена ошибка при слиянии таблиц `CollapsingMergeTree`, если одна из частей данных пуста (такие части формируются в процессе слияния или при выполнении `ALTER DELETE`, когда все данные были удалены) и для слияния использовался алгоритм `vertical`. [#3049](https://github.com/ClickHouse/ClickHouse/pull/3049)
...
* Исправлено падение (segfault) во время `JOIN ... ON`. [#3000](https://github.com/ClickHouse/ClickHouse/pull/3000)
...
* Исправлена ошибка &quot;Not found column&quot;, возникавшая при выполнении распределённых запросов, если с удалённого сервера запрашивался единственный столбец, который представляет собой выражение IN с подзапросом. [#3087](https://github.com/ClickHouse/ClickHouse/pull/3087)
* Исправлена ошибка `Block structure mismatch in UNION stream: different number of columns`, возникавшая в распределённых запросах, если один из шардов локальный, а другой — нет, и срабатывала оптимизация переноса в `PREWHERE`. [#2226](https://github.com/ClickHouse/ClickHouse/pull/2226) [#3037](https://github.com/ClickHouse/ClickHouse/pull/3037) [#3055](https://github.com/ClickHouse/ClickHouse/pull/3055) [#3065](https://github.com/ClickHouse/ClickHouse/pull/3065) [#3073](https://github.com/ClickHouse/ClickHouse/pull/3073) [#3090](https://github.com/ClickHouse/ClickHouse/pull/3090) [#3093](https://github.com/ClickHouse/ClickHouse/pull/3093)
* Исправлена функция `pointInPolygon` для некоторых случаев невыпуклых многоугольников. [#2910](https://github.com/ClickHouse/ClickHouse/pull/2910)
* Исправлена некорректная работа сравнения `nan` с целыми числами. [#3024](https://github.com/ClickHouse/ClickHouse/pull/3024)
...
* Исправлена утечка памяти при вставке в таблицу со столбцами типа `AggregateFunction`, если состояние агрегатной функции не является простым (выделяет память отдельно) и если один запрос на вставку приводит к появлению нескольких небольших блоков. [#3084](https://github.com/ClickHouse/ClickHouse/pull/3084)
* Исправлено состояние гонки при одновременном создании и удалении одной и той же таблицы `Buffer` или `MergeTree`.
* Исправлена возможная ошибка сегментации при сравнении кортежей, состоящих из некоторых нетривиальных типов, например кортежей. [#2989](https://github.com/ClickHouse/ClickHouse/pull/2989)
* Исправлена потенциальная причина segfault при выполнении некоторых запросов `ON CLUSTER`. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2960)
...
* Исправлена ошибка сегментации при повторной инициализации сессии ZooKeeper. [#2917](https://github.com/ClickHouse/ClickHouse/pull/2917)
* Исправлена возможная блокировка при работе с ZooKeeper.
* Исправлен некорректный код для добавления вложенных структур данных в `SummingMergeTree`.
* При выделении памяти для состояний агрегатных функций теперь корректно учитывается выравнивание, что позволяет использовать операции, требующие выравнивания, при реализации состояний агрегатных функций. [chenxing-xc](https://github.com/ClickHouse/ClickHouse/pull/2808)
...
- Добавлена поддержка типов Nullable в драйвере ClickHouse ODBC (формат вывода `ODBCDriver2`). [#2834](https://github.com/ClickHouse/ClickHouse/pull/2834)
- Добавлена поддержка `UUID` в ключевых столбцах.

#### Улучшения: {#improvements-5}

- Кластеры можно удалять без перезапуска сервера при их удалении из конфигурационных файлов. [#2777](https://github.com/ClickHouse/ClickHouse/pull/2777)
- Внешние словари можно удалять без перезапуска сервера при их удалении из конфигурационных файлов. [#2779](https://github.com/ClickHouse/ClickHouse/pull/2779)
...
- Улучшения для типа данных `UUID` (ещё не завершены). [#2618](https://github.com/ClickHouse/ClickHouse/pull/2618)
...
- Агрегатная функция `quantileExact` возвращает `nan` в случае агрегации по пустому набору `Float32` или `Float64`. [Sundy Li](https://github.com/ClickHouse/ClickHouse/pull/2855)

#### Исправления ошибок: {#bug-fixes-14}


- Удалено ненужное экранирование параметров строки подключения для ODBC, из-за которого было невозможно установить соединение. Эта ошибка появилась в версии 18.6.0.
- Исправлена логика обработки команд `REPLACE PARTITION` в очереди репликации. Если для одной партиции существуют две команды `REPLACE`, некорректная логика могла привести к тому, что одна из них оставалась в очереди репликации и не выполнялась. [#2814](https://github.com/ClickHouse/ClickHouse/pull/2814)
...
- Исправлена ошибка `Block structure mismatch in UNION stream: different number of columns`, которая возникала в запросах `UNION ALL` внутри подзапроса, если один из запросов `SELECT` содержал дублирующиеся имена столбцов. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2094)
- Исправлена утечка памяти при возникновении исключения во время подключения к серверу MySQL.
- Исправлен некорректный код возврата clickhouse-client в случае ошибки запроса.
...
- Удалена поддержка запросов CHECK TABLE для распределённых таблиц.

#### Изменения сборки: {#build-changes-3}

- Заменён аллокатор памяти: теперь используется `jemalloc` вместо `tcmalloc`. В некоторых сценариях это увеличивает скорость до 20%. Однако есть запросы, которые замедлились до 20%. Потребление памяти снижено примерно на 10% в некоторых сценариях, улучшена стабильность. При высококонкурентных нагрузках использование CPU в пользовательском пространстве и в системе показывает лишь незначительное увеличение. [#2773](https://github.com/ClickHouse/ClickHouse/pull/2773)
...
- Запрос `ALTER TABLE t DELETE WHERE` не перезаписывает куски данных, на которые не повлияло условие WHERE [#2694](https://github.com/ClickHouse/ClickHouse/pull/2694).
...
- Функция `has` теперь корректно работает для массивов с элементами типа Nullable ([#2115](https://github.com/ClickHouse/ClickHouse/issues/2115)).
...
- Запросы можно отправлять в формате `multipart/form-data` (в поле `query`), что полезно при передаче внешних данных для обработки запроса ([Olga Hvostikova](https://github.com/ClickHouse/ClickHouse/pull/2490)).
...
- Удалена избыточная проверка контрольных сумм при добавлении куска данных. Это важно при большом количестве реплик, так как в таких случаях общее количество проверок было равно N^2.
...
- Логи сервера теперь можно отправлять в syslog ([Alexander Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/2459)).
...
- Добавлена возможность вычисления аргументов `and` только там, где это необходимо ([Anastasia Tsarkova](https://github.com/ClickHouse/ClickHouse/pull/2272))
...
- Исправлена ошибка в клиентской библиотеке ZooKeeper, которая приводила к потере отслеживаний (watches), зависанию очереди распределённых DDL и замедлению очереди репликации при использовании непустого префикса `chroot` в конфигурации ZooKeeper.
...
- `SELECT ... FINAL` и `OPTIMIZE ... FINAL` можно использовать, даже если таблица содержит только один кусок данных.
...
- При вставке данных в таблицу типа `Replicated` выполняется меньше запросов к `ZooKeeper` (и большинство пользовательских ошибок исчезло из лога `ZooKeeper`).
...
- Исправлена ошибка `Illegal PREWHERE` при чтении из таблиц типа Merge для таблиц `Distributed`.
...
* Агрегация без `GROUP BY` для пустого набора (например, `SELECT count(*) FROM table WHERE 0`) теперь возвращает результат с одной строкой с `NULL`-значениями для агрегатных функций, в соответствии со стандартом SQL. Чтобы вернуть старое поведение (возврат пустого результата), установите параметр `empty_result_for_aggregation_by_empty_set` в 1.
* Добавлено преобразование типов для `UNION ALL`. В позициях `SELECT` в `UNION ALL` теперь допускаются разные имена алиасов в соответствии со стандартом SQL.
* В предложениях `LIMIT BY` поддерживаются произвольные выражения. Ранее можно было использовать только столбцы, возвращаемые оператором `SELECT`.
* Индекс таблиц `MergeTree` используется, когда `IN` применяется к кортежу выражений, построенных из столбцов первичного ключа. Пример: `WHERE (UserID, EventDate) IN ((123, '2000-01-01'), ...)` (Анастасия Царькова).
* Добавлен инструмент `clickhouse-copier` для копирования данных между кластерами и перешардирования (бета).
* Добавлены функции консистентного хеширования: `yandexConsistentHash`, `jumpConsistentHash`, `sumburConsistentHash`. Их можно использовать в качестве ключа шардинга, чтобы сократить объем сетевого трафика при последующем перераспределении шардов.
* Добавлены функции: `arrayAny`, `arrayAll`, `hasAny`, `hasAll`, `arrayIntersect`, `arrayResize`.
* Добавлена функция `arrayCumSum` (Хави Сантана).
* Добавлены функции `parseDateTimeBestEffort`, `parseDateTimeBestEffortOrZero` и `parseDateTimeBestEffortOrNull` для чтения значения `DateTime` из строки, содержащей текст в самом широком диапазоне возможных форматов.
* Данные могут частично перезагружаться из внешних словарей во время обновления (загружаются только записи, в которых значение указанного поля больше, чем при предыдущей загрузке данных) (Arsen Hakobyan).
...
* Запрос `SHOW TABLES` теперь также отображает временные таблицы. Во `system.tables` добавлены временные таблицы и столбец `is_temporary` (zhang2014).
* Добавлены запросы `DROP TEMPORARY TABLE` и `EXISTS TEMPORARY TABLE` (zhang2014).
* Поддержка команды `SHOW CREATE TABLE` для временных таблиц (zhang2014).
...
* Чтение значения `null` как значения по умолчанию при загрузке данных для внешнего словаря из источника `MongoDB` (Pavel Litvinenko).
* Чтение значений `DateTime` в формате `Values` из Unix-метки времени без одинарных кавычек.
* Режим отказоустойчивости поддерживается в табличных функциях `remote` на случай, когда на некоторых репликах отсутствует запрошенная таблица.
...
* Реализована функция `empty` для аргумента типа `FixedString`: функция возвращает 1, если строка целиком состоит из нулевых байтов (zhang2014).
* Добавлен параметр конфигурации `listen_try`, позволяющий продолжать работу при невозможности прослушивать некоторые из указанных адресов и прослушивать хотя бы один из них (полезно для систем с отключённой поддержкой IPv4 или IPv6).
* Добавлен движок таблицы `VersionedCollapsingMergeTree`.
* Поддержка строковых и произвольных числовых типов для источника словаря `library`.
* Таблицы `MergeTree` можно использовать без первичного ключа (необходимо указать `ORDER BY tuple()`).
* Тип `Nullable` может быть приведён с помощью `CAST` к типу без `Nullable`, если аргумент не равен `NULL`.
* `RENAME TABLE` можно выполнять для `VIEW`.
* Добавлена функция `throwIf`.
* Добавлена опция `odbc_default_field_size`, которая позволяет увеличить максимальный размер значения, загружаемого из источника ODBC (по умолчанию — 1024).
* В таблицу `system.processes` и в результат `SHOW PROCESSLIST` добавлены столбцы `is_cancelled` и `peak_memory_usage`.
...
- Меньше ложных срабатываний `force_restore_data` при проверке состояния реплицируемых таблиц во время запуска сервера.
...
- Добавлена возможность интерпретировать пустую строку в текстовых форматах как число 0 для типов данных `Float`. Эта функциональность была доступна ранее, но была утеряна в релизе 1.1.54342.
- Значения `Enum` могут использоваться в функциях `min`, `max`, `sum` и некоторых других. В этих случаях используются соответствующие числовые значения. Эта функциональность была доступна ранее, но была утеряна в релизе 1.1.54337.
- Добавлена настройка `max_expanded_ast_elements` для ограничения размера AST после рекурсивного раскрытия псевдонимов.
...
- Добавлена поддержка макросов для определения имён кластеров в распределённых DDL-запросах и конструкторах Distributed-таблиц: `CREATE TABLE distr ON CLUSTER '{cluster}' (...) ENGINE = Distributed('{cluster}', 'db', 'table')`.
...
- Добавлена возможность передавать аргумент типа `AggregateFunction` для агрегатной функции `groupArray` (что позволяет создать массив состояний некоторой агрегатной функции).
- Сняты ограничения на различные комбинации комбинаторов агрегатных функций. Например, можно использовать агрегатные функции `avgForEachIf`, а также `avgIfForEach`, которые имеют разное поведение.
...
- Пользователям с настройкой `readonly = 2` разрешено работать с TEMPORARY таблицами (CREATE, DROP, INSERT...) (Kirill Shvakov).
...
- Добавлены настройки: `http_connection_timeout`, `http_send_timeout`, `http_receive_timeout`. В частности, эти настройки используются для загрузки кусков данных при репликации. Изменение этих настроек позволяет ускорить переключение при отказе в случае перегрузки сети.
...
- Исправлена дедупликация данных после выполнения запроса `DROP` или `DETACH PARTITION`. В предыдущей версии удаление партиции и повторная вставка тех же данных не работали, так как вставленные блоки считались дубликатами.
...
- Добавлена недостающая поддержка типа данных `UUID` для `DISTINCT`, `JOIN` и агрегатной функции `uniq`, а также для внешних словарей (Evgeniy Ivanov). Поддержка `UUID` всё ещё неполная.
...
- Исправлена неатомарность обновления очереди репликации. Это могло привести к рассинхронизации реплик до перезапуска сервера.
...
- Переработана поддержка типов `Nullable`. Исправлены ошибки, которые могли привести к падению сервера. Исправлены почти все остальные ошибки, связанные с поддержкой `NULL`: некорректные преобразования типов в INSERT SELECT, недостаточная поддержка Nullable в HAVING и PREWHERE, режим `join_use_nulls`, типы Nullable в качестве аргументов оператора `OR` и т. д.
...
- Одна сборка используется для разных версий ОС. Пакеты и бинарные файлы совместимы с широким спектром систем Linux.
- Добавлен пакет `clickhouse-test`. Его можно использовать для запуска функциональных тестов.
- Архив с исходным кодом теперь можно публиковать в репозитории. Его можно использовать для воспроизведения сборки без использования GitHub.
...
- Добавлена поддержка `systemd` (Владимир Смирнов). По умолчанию отключена из-за несовместимости с некоторыми образами ОС и может быть включена вручную.
...
- Типы данных Enum нельзя использовать в агрегатных функциях min/max. Эта возможность будет возвращена в следующем релизе.
