...
doc_type: 'changelog'
keywords: ['ClickHouse 2018', 'список изменений 2018', 'примечания к релизу', 'история версий', 'старые версии']
---
...
- JIT-компиляция агрегатных функций теперь работает со столбцами типа LowCardinality. [#3838](https://github.com/ClickHouse/ClickHouse/issues/3838)
...
* Исправления и улучшения производительности для типа данных `LowCardinality`. `GROUP BY` с использованием `LowCardinality(Nullable(...))`. Получение значений `extremes`. Обработка функций высшего порядка. `LEFT ARRAY JOIN`. Распределённый `GROUP BY`. Функции, которые возвращают `Array`. Выполнение `ORDER BY`. Запись в таблицы типа `Distributed` (nicelulu). Обратная совместимость для запросов `INSERT` от старых клиентов, использующих протокол `Native`. Поддержка `LowCardinality` для `JOIN`. Повышена производительность при работе в одном потоке. [#3823](https://github.com/ClickHouse/ClickHouse/pull/3823) [#3803](https://github.com/ClickHouse/ClickHouse/pull/3803) [#3799](https://github.com/ClickHouse/ClickHouse/pull/3799) [#3769](https://github.com/ClickHouse/ClickHouse/pull/3769) [#3744](https://github.com/ClickHouse/ClickHouse/pull/3744) [#3681](https://github.com/ClickHouse/ClickHouse/pull/3681) [#3651](https://github.com/ClickHouse/ClickHouse/pull/3651) [#3649](https://github.com/ClickHouse/ClickHouse/pull/3649) [#3641](https://github.com/ClickHouse/ClickHouse/pull/3641) [#3632](https://github.com/ClickHouse/ClickHouse/pull/3632) [#3568](https://github.com/ClickHouse/ClickHouse/pull/3568) [#3523](https://github.com/ClickHouse/ClickHouse/pull/3523) [#3518](https://github.com/ClickHouse/ClickHouse/pull/3518)
* Исправлена работа параметра `select_sequential_consistency`. Ранее при включении этого параметра иногда возвращался неполный результат после начала записи данных в новый раздел. [#2863](https://github.com/ClickHouse/ClickHouse/pull/2863)
* Базы данных теперь корректно задаются при выполнении DDL-запросов `ON CLUSTER` и `ALTER UPDATE/DELETE`. [#3772](https://github.com/ClickHouse/ClickHouse/pull/3772) [#3460](https://github.com/ClickHouse/ClickHouse/pull/3460)
* Базы данных теперь корректно указываются в подзапросах внутри VIEW. [#3521](https://github.com/ClickHouse/ClickHouse/pull/3521)
* Исправлена ошибка в `PREWHERE` при использовании `FINAL` для `VersionedCollapsingMergeTree`. [7167bfd7](https://github.com/ClickHouse/ClickHouse/commit/7167bfd7b365538f7a91c4307ad77e552ab4e8c1)
* Теперь вы можете использовать `KILL QUERY`, чтобы отменять ещё не запущенные запросы, ожидающие получения блокировки на таблицу. [#3517](https://github.com/ClickHouse/ClickHouse/pull/3517)
* Исправлены вычисления даты и времени при переводе часов назад в полночь (это происходит в Иране, а также происходило в Москве с 1981 по 1983 год). Ранее это приводило к тому, что время сбрасывалось на сутки раньше, чем нужно, а также вызывало некорректное форматирование даты и времени при выводе в текстовом виде. [#3819](https://github.com/ClickHouse/ClickHouse/pull/3819)
* Исправлены ошибки в некоторых случаях использования `VIEW` и подзапросов, в которых опущено указание базы данных. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3521)
...
* Исправлена обработка запросов при включённой опции `compile_expressions` (она включена по умолчанию). Недетерминированные константные выражения, такие как функция `now`, больше не разворачиваются. [#3457](https://github.com/ClickHouse/ClickHouse/pull/3457)
* Исправлен сбой при указании неконстантного аргумента масштаба в функциях `toDecimal32/64/128`.
* Исправлена ошибка при попытке вставить массив с элементами `NULL` в формате `Values` в столбец типа `Array` без модификатора `Nullable` (если `input_format_values_interpret_expressions` = 1). [#3487](https://github.com/ClickHouse/ClickHouse/pull/3487) [#3503](https://github.com/ClickHouse/ClickHouse/pull/3503)
* Исправлена проблема непрерывного логирования ошибок в `DDLWorker` при недоступности ZooKeeper. [8f50c620](https://github.com/ClickHouse/ClickHouse/commit/8f50c620334988b28018213ec0092fe6423847e2)
...
* Исправлена обработка предложения `WITH`, если в нём указан простой псевдоним без выражений. [#3570](https://github.com/ClickHouse/ClickHouse/pull/3570)
...
* Исправлен сбой, возникавший при передаче некоторых некорректных аргументов в функцию `arrayReverse`. [73e3a7b6](https://github.com/ClickHouse/ClickHouse/commit/73e3a7b662161d6005e7727d8a711b930386b871)
...
* Исправлено переполнение буфера в функциях `lowerUTF8` и `upperUTF8`. Удалена возможность выполнять эти функции для аргументов типа `FixedString`. [#3662](https://github.com/ClickHouse/ClickHouse/pull/3662)
* Исправлено редкое состояние гонки при удалении таблиц `MergeTree`. [#3680](https://github.com/ClickHouse/ClickHouse/pull/3680)
* Исправлено условие гонки при чтении из таблиц `Buffer` при одновременном выполнении операций `ALTER` или `DROP` над целевыми таблицами. [#3719](https://github.com/ClickHouse/ClickHouse/pull/3719)
* Исправлена ошибка сегментации, возникавшая при превышении лимита `max_temporary_non_const_columns`. [#3788](https://github.com/ClickHouse/ClickHouse/pull/3788)

#### Улучшения: {#improvements-1}


* Сервер не записывает обработанные файлы конфигурации в каталог `/etc/clickhouse-server/`. Вместо этого он сохраняет их в подкаталоге `preprocessed_configs` внутри `path`. Это означает, что пользователь `clickhouse` не имеет прав на запись в каталог `/etc/clickhouse-server/`, что повышает безопасность. [#2443](https://github.com/ClickHouse/ClickHouse/pull/2443)
* Параметр `min_merge_bytes_to_use_direct_io` по умолчанию установлен в значение 10 GiB. Слияние, формирующее крупные части таблиц семейства MergeTree, будет выполняться в режиме `O_DIRECT`, что предотвращает чрезмерное вытеснение страниц из кеша. [#3504](https://github.com/ClickHouse/ClickHouse/pull/3504)
...
* Если синтаксис запроса неверен, в интерфейсе `HTTP` возвращается код `400 Bad Request` (ранее возвращался код 500). [31bc680a](https://github.com/ClickHouse/ClickHouse/commit/31bc680ac5f4bb1d0360a8ba4696fa84bb47d6ab)
...
* Удалено логирование в `stderr` в библиотеке `re2` для некорректных или чрезмерно сложных регулярных выражений. [#3723](https://github.com/ClickHouse/ClickHouse/pull/3723)
* Добавлено для движка таблиц `Kafka`: проверка наличия подписок перед началом чтения данных из Kafka; настройка kafka&#95;max&#95;block&#95;size для таблицы. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3396)
* Функции `cityHash64`, `farmHash64`, `metroHash64`, `sipHash64`, `halfMD5`, `murmurHash2_32`, `murmurHash2_64`, `murmurHash3_32` и `murmurHash3_64` теперь работают с произвольным количеством аргументов, а также с аргументами в виде кортежей. [#3451](https://github.com/ClickHouse/ClickHouse/pull/3451) [#3519](https://github.com/ClickHouse/ClickHouse/pull/3519)
...
* Для `FULL` и `RIGHT JOIN` параметр `max_block_size` используется для потока несоединённых данных из правой таблицы. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3699)
* В утилиты `clickhouse-benchmark` и `clickhouse-performance-test` добавлен параметр командной строки `--secure` для включения TLS. [#3688](https://github.com/ClickHouse/ClickHouse/pull/3688) [#3690](https://github.com/ClickHouse/ClickHouse/pull/3690)
* Преобразование типов при несовпадении структуры таблицы типа `Buffer` со структурой целевой таблицы. [Vitaly Baranov](https://github.com/ClickHouse/ClickHouse/pull/3603)
* Добавлена опция `tcp_keep_alive_timeout` для включения keep-alive-пакетов после периода неактивности, продолжающегося заданный интервал времени. [#3441](https://github.com/ClickHouse/ClickHouse/pull/3441)
* Удалено избыточное заключение в кавычки значений ключа партиционирования в таблице `system.parts`, если он состоит из одного столбца. [#3652](https://github.com/ClickHouse/ClickHouse/pull/3652)
* Функция `modulo` поддерживается для типов данных `Date` и `DateTime`. [#3385](https://github.com/ClickHouse/ClickHouse/pull/3385)
* Добавлены синонимы для функций `POWER`, `LN`, `LCASE`, `UCASE`, `REPLACE`, `LOCATE`, `SUBSTR` и `MID`. [#3774](https://github.com/ClickHouse/ClickHouse/pull/3774) [#3763](https://github.com/ClickHouse/ClickHouse/pull/3763) Некоторые имена функций обрабатываются без учета регистра для совместимости со стандартом SQL. Добавлена синтаксическая конструкция `SUBSTRING(expr FROM start FOR length)` для совместимости с SQL. [#3804](https://github.com/ClickHouse/ClickHouse/pull/3804)
* Добавлена возможность выполнять `mlock` для страниц памяти, содержащих исполняемый код `clickhouse-server`, чтобы предотвратить их вытеснение из памяти. Эта возможность по умолчанию отключена. [#3553](https://github.com/ClickHouse/ClickHouse/pull/3553)
* Повышена производительность при чтении с использованием `O_DIRECT` (при включённой опции `min_bytes_to_use_direct_io`). [#3405](https://github.com/ClickHouse/ClickHouse/pull/3405)
* Улучшена производительность функции `dictGet...OrDefault` для константного ключевого аргумента и неконстантного значения по умолчанию. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3563)
* Функция `firstSignificantSubdomain` теперь обрабатывает домены `gov`, `mil` и `edu`. [Igor Hatarist](https://github.com/ClickHouse/ClickHouse/pull/3601) Производительность улучшена. [#3628](https://github.com/ClickHouse/ClickHouse/pull/3628)
* Возможность задавать произвольные переменные окружения для запуска `clickhouse-server` через скрипт `SYS-V init.d` путём определения `CLICKHOUSE_PROGRAM_ENV` в `/etc/default/clickhouse`.
...
* Таблица `system.metrics` теперь содержит метрику `VersionInteger`, а в `system.build_options` добавлена строка `VERSION_INTEGER`, которая содержит числовое представление версии ClickHouse, например `18016000`. [#3644](https://github.com/ClickHouse/ClickHouse/pull/3644)
* Исключена возможность сравнивать тип `Date` с числом, чтобы избежать потенциальных ошибок, например `date = 2018-12-17`, когда кавычки вокруг даты по ошибке опускаются. [#3687](https://github.com/ClickHouse/ClickHouse/pull/3687)
* Исправлено поведение функций с сохранением состояния, таких как `rowNumberInAllBlocks`. Ранее они возвращали результат, на единицу больше, из‑за запуска уже на этапе анализа запроса. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3729)
...
- Добавлена возможность запуска интеграционных тестов при наличии в системе только `Docker`. [#3650](https://github.com/ClickHouse/ClickHouse/pull/3650)
...
#### Исправление ошибок: {#bug-fixes-2}

- Исправлена ошибка, приводившая к проблемам с обновлением словарей с источником ODBC. [#3825](https://github.com/ClickHouse/ClickHouse/issues/3825), [#3829](https://github.com/ClickHouse/ClickHouse/issues/3829)
...
#### Исправление ошибок: {#bug-fixes-3}

- Исправлена ошибка в функции `dictGet...` для словарей типа `range`, когда один из аргументов является константой, а другой — нет. [#3751](https://github.com/ClickHouse/ClickHouse/pull/3751)
- Исправлена ошибка, приводившая к выводу сообщений `netlink: '...': attribute type 1 has an invalid length` в журнале ядра Linux, что происходило только на достаточно новых версиях ядра Linux. [#3749](https://github.com/ClickHouse/ClickHouse/pull/3749)
...
- Исправлены случаи, когда процесс ODBC-моста не завершался вместе с основным процессом сервера. [#3642](https://github.com/ClickHouse/ClickHouse/pull/3642)
...
- Исправлено редкое состояние гонки, которое может привести к сбою при удалении таблицы MergeTree. [#3643](https://github.com/ClickHouse/ClickHouse/pull/3643)
...
- Исправлен разбор конструкции `ENGINE` при использовании синтаксиса `CREATE AS table`, когда конструкция `ENGINE` указывалась перед `AS table` (ошибка приводила к игнорированию указанного движка). [#3692](https://github.com/ClickHouse/ClickHouse/pull/3692)

### Релиз ClickHouse 18.14.15, 2018-11-21 {#clickhouse-release-18-14-15-2018-11-21}

#### Исправление ошибок: {#bug-fixes-5}
...
#### Исправление ошибок: {#bug-fixes-6}
...
- Настройка `compile_expressions` (JIT-компиляция выражений) отключена по умолчанию. [#3410](https://github.com/ClickHouse/ClickHouse/pull/3410)
- Настройка `enable_optimize_predicate_expression` отключена по умолчанию.
...
- Добавлен движок таблиц `JDBC` и табличная функция `jdbc` (требуется установка clickhouse-jdbc-bridge). [Alexandr Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/3210)
...
* Значительно уменьшено потребление памяти при выполнении запросов с `ORDER BY` и `LIMIT`. См. настройку `max_bytes_before_remerge_sort`. [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* Если тип `JOIN` (`LEFT`, `INNER`, ...) не указан, используется `INNER JOIN`. [#3147](https://github.com/ClickHouse/ClickHouse/pull/3147)
* Квалифицированные звездочки корректно обрабатываются в запросах с `JOIN`. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/3202)
* Табличный движок `ODBC` корректно выбирает способ заключения идентификаторов в кавычки в SQL-диалекте удалённой базы данных. [Alexandr Krasheninnikov](https://github.com/ClickHouse/ClickHouse/pull/3210)
* Настройка `compile_expressions` (JIT-компиляция выражений) включена по умолчанию.
* Скорректировано поведение при одновременном выполнении `DROP DATABASE/TABLE IF EXISTS` и `CREATE DATABASE/TABLE IF NOT EXISTS`. Ранее запрос `CREATE DATABASE ... IF NOT EXISTS` мог возвращать сообщение об ошибке «File ... already exists», а запросы `CREATE TABLE ... IF NOT EXISTS` и `DROP TABLE IF EXISTS` могли возвращать «Table ... is creating or attaching right now». [#3101](https://github.com/ClickHouse/ClickHouse/pull/3101)
* Выражения LIKE и IN с константным правым операндом передаются на удалённый сервер при запросах из таблиц MySQL или ODBC. [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* Сравнения с константными выражениями в условии WHERE передаются на удалённый сервер при выполнении запросов к таблицам MySQL и ODBC. Ранее передавались только сравнения с константами. [#3182](https://github.com/ClickHouse/ClickHouse/pull/3182)
* Корректный расчет ширины строк в терминале для форматов `Pretty`, включая строки, содержащие иероглифы. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/3257).
* `ON CLUSTER` может быть указан в запросах `ALTER UPDATE`.
...
* Добавлены синонимы функций `LENGTH` и `CHARACTER_LENGTH` для совместимости. Функция `CONCAT` теперь регистронезависима. [#3306](https://github.com/ClickHouse/ClickHouse/pull/3306)
* Добавлен синоним `TIMESTAMP` для типа `DateTime`. [#3390](https://github.com/ClickHouse/ClickHouse/pull/3390)
* В серверных логах всегда зарезервировано поле для query&#95;id, даже если строка лога не относится к запросу. Это упрощает разбор текстовых логов сервера с помощью сторонних инструментов.
* Потребление памяти запросом записывается в лог, когда оно превышает очередное целое число гигабайт. [#3205](https://github.com/ClickHouse/ClickHouse/pull/3205)
* Добавлен режим совместимости для случая, когда клиентская библиотека, использующая протокол Native, по ошибке отправляет меньше столбцов, чем сервер ожидает получить для запроса INSERT. Этот сценарий был возможен при использовании библиотеки clickhouse-cpp. Ранее это приводило к сбою сервера. [#3171](https://github.com/ClickHouse/ClickHouse/pull/3171)
* В пользовательском выражении WHERE в `clickhouse-copier` теперь можно использовать псевдоним `partition_key` (для дополнительной фильтрации по партиции исходной таблицы). Это полезно, если схема партиционирования во время копирования меняется лишь незначительно. [#3166](https://github.com/ClickHouse/ClickHouse/pull/3166)
* Логика работы движка `Kafka` была перенесена в пул фоновых потоков для автоматического снижения скорости чтения данных при высокой нагрузке. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215).
...
* Список доменов верхнего уровня для функции `firstSignificantSubdomain` теперь также включает домен `biz`. [decaseal](https://github.com/ClickHouse/ClickHouse/pull/3219)
...
* Поддержка функций `intDiv` и `intDivOrZero` для `Decimal`. [b48402e8](https://github.com/ClickHouse/ClickHouse/commit/b48402e8712e2b9b151e0eef8193811d433a1264)
...
* Добавлены специализации для типа `UUID` в функции `if`. [#3366](https://github.com/ClickHouse/ClickHouse/pull/3366)
* Сокращено количество системных вызовов `open` и `close` при чтении из таблицы `MergeTree`. [#3283](https://github.com/ClickHouse/ClickHouse/pull/3283)
* Запрос `TRUNCATE TABLE` может быть выполнен на любой реплике (запрос передаётся на реплику-лидер). [Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/3375)

#### Исправления ошибок: {#bug-fixes-10}


* Исправлена ошибка с таблицами `Dictionary` для словарей типа `range_hashed`. Ошибка проявлялась в версии 18.12.17. [#1702](https://github.com/ClickHouse/ClickHouse/pull/1702)
...
* Исправлены ошибки в функции `pointInPolygon`, вызванные накоплением погрешностей вычислений для полигонов с большим числом вершин, расположенных близко друг к другу. [#3331](https://github.com/ClickHouse/ClickHouse/pull/3331) [#3341](https://github.com/ClickHouse/ClickHouse/pull/3341)
* Если после слияния частей данных контрольная сумма получившейся части отличается от результата того же слияния на другой реплике, результат слияния удаляется, и часть данных загружается с другой реплики (это корректное поведение). Но после загрузки часть данных не удавалось добавить в рабочий набор из-за ошибки о том, что часть уже существует (потому что часть данных была удалена с некоторой задержкой после слияния). Это приводило к циклическим попыткам загрузить одни и те же данные. [#3194](https://github.com/ClickHouse/ClickHouse/pull/3194)
* Исправлено некорректное вычисление общего объёма потребляемой памяти всеми запросами (из-за этого настройка `max_memory_usage_for_all_queries` работала неправильно, а метрика `MemoryTracking` имела неверное значение). Эта ошибка присутствовала в версии 18.12.13. [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3344)
* Исправлена ошибка в работе `CREATE TABLE ... ON CLUSTER ... AS SELECT ...`. Эта ошибка возникала в версии 18.12.13. [#3247](https://github.com/ClickHouse/ClickHouse/pull/3247)
* Исправлена ненужная подготовка структур данных для операций `JOIN` на сервере, инициирующем запрос, если `JOIN` выполняется только на удалённых серверах. [#3340](https://github.com/ClickHouse/ClickHouse/pull/3340)
* Исправлены ошибки в движке `Kafka`, приводившие к взаимным блокировкам после исключений в начале чтения данных и к блокировкам при завершении работы [Marek Vavruša](https://github.com/ClickHouse/ClickHouse/pull/3215).
...
* Если в ансамбле серверов ZooKeeper есть серверы, которые принимают соединение, но затем сразу же закрывают его вместо завершения handshake, ClickHouse подключается к другому серверу. Ранее это приводило к ошибке `Cannot read all data. Bytes read: 0. Bytes expected: 4.` и сервер не мог запуститься. [8218cf3a](https://github.com/ClickHouse/ClickHouse/commit/8218cf3a5f39a43401953769d6d12a0bb8d29da9)
* Если в ансамбле серверов ZooKeeper есть серверы, для которых при выполнении DNS-запроса возникает ошибка, эти серверы игнорируются. [17b8e209](https://github.com/ClickHouse/ClickHouse/commit/17b8e209221061325ad7ba0539f03c6e65f87f29)
...
* Исправлено преобразование типов между `Decimal` и целочисленными типами. [#3211](https://github.com/ClickHouse/ClickHouse/pull/3211)
...
* Исправлена функция `arrayCumSumNonNegative` (теперь она не накапливает отрицательные значения, если аккумулятор меньше нуля). [Aleksey Studnev](https://github.com/ClickHouse/ClickHouse/pull/3163)
...
* Исправлено поведение агрегатных функций с комбинатором `StateArray`. [#3188](https://github.com/ClickHouse/ClickHouse/pull/3188)
...
* Исправлен вывод типов для операций с аргументами типа `Decimal` и целочисленных типов. [#3224](https://github.com/ClickHouse/ClickHouse/pull/3224)
* Исправлен segfault, возникавший при выполнении `GROUP BY` по `Decimal128`. [3359ba06](https://github.com/ClickHouse/ClickHouse/commit/3359ba06c39fcd05bfdb87d6c64154819621e13a)
* Настройка `log_query_threads` (логирование информации о каждом потоке выполнения запроса) теперь применяется только в том случае, если настройка `log_queries` (логирование информации о запросах) установлена в значение 1. Поскольку настройка `log_query_threads` включена по умолчанию, информация о потоках ранее записывалась в журнал даже при отключённом логировании запросов. [#3241](https://github.com/ClickHouse/ClickHouse/pull/3241)
* Исправлена ошибка в распределённом выполнении агрегатной функции quantiles (сообщение об ошибке `Not found column quantile...`). [292a8855](https://github.com/ClickHouse/ClickHouse/commit/292a885533b8e3b41ce8993867069d14cbd5a664)
* Исправлена проблема совместимости при одновременной работе в кластере серверов версии 18.12.17 и более ранних версий. Для распределённых запросов с ключами GROUP BY как фиксированной, так и произвольной длины, при большом объёме данных для агрегирования возвращаемые данные не всегда были полностью агрегированы (две разные строки содержали одинаковые ключи агрегации). [#3254](https://github.com/ClickHouse/ClickHouse/pull/3254)
* Исправлена обработка подстановок в `clickhouse-performance-test`, если запрос содержит только часть подстановок, заданных в тесте. [#3263](https://github.com/ClickHouse/ClickHouse/pull/3263)
* Исправлена ошибка при использовании `FINAL` с `PREWHERE`. [#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* Исправлена ошибка при использовании `PREWHERE` над столбцами, которые были добавлены оператором `ALTER`. [#3298](https://github.com/ClickHouse/ClickHouse/pull/3298)
* Добавлена проверка, не допускающая использование `arrayJoin` в выражениях `DEFAULT` и `MATERIALIZED`. Ранее использование `arrayJoin` приводило к ошибке при вставке данных. [#3337](https://github.com/ClickHouse/ClickHouse/pull/3337)
* Добавлена проверка на отсутствие вызова `arrayJoin` в предложении `PREWHERE`. Ранее это приводило к сообщениям об ошибках вида `Size ... does not match` или `Unknown compression method` при выполнении запросов. [#3357](https://github.com/ClickHouse/ClickHouse/pull/3357)
* Исправлена ошибка сегментации, которая в редких случаях могла возникать после оптимизации, заменявшей цепочки AND из сравнений на равенство соответствующим выражением IN. [liuyimin-bytedance](https://github.com/ClickHouse/ClickHouse/pull/3339)
* Незначительные исправления в `clickhouse-benchmark`: ранее данные о клиенте не отправлялись на сервер, теперь количество выполненных запросов рассчитывается более точно при завершении работы и при ограничении числа итераций. [#3351](https://github.com/ClickHouse/ClickHouse/pull/3351) [#3352](https://github.com/ClickHouse/ClickHouse/pull/3352)
...
- Исправлена ошибка при обновлении внешних словарей с источником `ODBC` и хранилищем `hashed`. Эта ошибка возникла в версии 18.12.13.
...
* Добавлен тип данных `DECIMAL(digits, scale)` (`Decimal32(scale)`, `Decimal64(scale)`, `Decimal128(scale)`). Для его включения используйте настройку `allow_experimental_decimal_type`. [#2846](https://github.com/ClickHouse/ClickHouse/pull/2846) [#2970](https://github.com/ClickHouse/ClickHouse/pull/2970) [#3008](https://github.com/ClickHouse/ClickHouse/pull/3008) [#3047](https://github.com/ClickHouse/ClickHouse/pull/3047)
...
* В запросах с JOIN звёздочка раскрывается в список столбцов во всех таблицах, в соответствии со стандартом SQL. Вы можете восстановить старое поведение, установив параметр `asterisk_left_columns_only` равным 1 на уровне конфигурации пользователя. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2787)
* Добавлена поддержка операторов JOIN с табличными функциями. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
* Автодополнение при нажатии клавиши Tab в clickhouse-client. [Sergey Shcherbin](https://github.com/ClickHouse/ClickHouse/pull/2447)
* Ctrl+C в clickhouse-client сбрасывает введённый запрос. [#2877](https://github.com/ClickHouse/ClickHouse/pull/2877)
* Добавлена настройка `join_default_strictness` (значения: `"`, `'any'`, `'all'`). Она позволяет не указывать `ANY` или `ALL` в операторе `JOIN`. [#2982](https://github.com/ClickHouse/ClickHouse/pull/2982)
* Каждая строка журнала сервера, связанная с обработкой запросов, содержит идентификатор запроса. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Теперь в clickhouse-client можно получать логи выполнения запросов (используйте настройку `send_logs_level`). При распределённой обработке запросов логи каскадно собираются со всех серверов. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Таблицы `system.query_log` и `system.processes` (`SHOW PROCESSLIST`) теперь содержат информацию обо всех изменённых настройках при выполнении запроса (вложенную структуру данных `Settings`). Добавлена настройка `log_query_settings`. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Таблицы `system.query_log` и `system.processes` теперь показывают информацию о количестве потоков, участвующих в выполнении запроса (см. столбец `thread_numbers`). [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Добавлены счетчики `ProfileEvents`, которые измеряют время, затраченное на чтение и запись по сети, чтение и запись на диск, количество сетевых ошибок, а также время ожидания при ограничении пропускной способности сети. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Добавлены счетчики `ProfileEvents`, которые содержат системные метрики из rusage (их можно использовать для получения информации об использовании CPU в пространстве пользователя и в ядре, об ошибках страниц памяти (page faults) и переключениях контекста), а также метрики taskstats (с помощью них можно получить информацию о времени ожидания операций ввода-вывода (I/O), времени ожидания CPU и объёме прочитанных и записанных данных — как с использованием кэша страниц (page cache), так и без него). [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Счётчики `ProfileEvents` применяются на глобальном уровне, для каждого запроса, а также для каждого потока его выполнения, что позволяет детально профилировать потребление ресурсов каждым запросом. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Добавлена таблица `system.query_thread_log`, которая содержит информацию о каждом потоке, выполняющем запрос. Добавлена настройка `log_query_threads`. [#2482](https://github.com/ClickHouse/ClickHouse/pull/2482)
* Таблицы `system.metrics` и `system.events` получили встроенную документацию. [#3016](https://github.com/ClickHouse/ClickHouse/pull/3016)
...
* Теперь вы можете складывать (объединять) состояния агрегатных функций с помощью оператора сложения и умножать их на неотрицательную константу. [#3062](https://github.com/ClickHouse/ClickHouse/pull/3062) [#3034](https://github.com/ClickHouse/ClickHouse/pull/3034)
* Таблицы семейства MergeTree теперь содержат виртуальный столбец `_partition_id`. [#3089](https://github.com/ClickHouse/ClickHouse/pull/3089)
...
- Добавлен кеш JIT-компилируемых функций и счётчик количества использований перед компиляцией. Для JIT-компиляции выражений включите настройку `compile_expressions`. [#2990](https://github.com/ClickHouse/ClickHouse/pull/2990) [#3077](https://github.com/ClickHouse/ClickHouse/pull/3077)

#### Улучшения: {#improvements-4}


* Исправлена проблема с неограниченным ростом журнала репликации при наличии заброшенных реплик. Добавлен эффективный режим восстановления для реплик с длительным лагом.
* Улучшена производительность `GROUP BY` с несколькими полями группировки, когда одно из них имеет строковый тип, а остальные — фиксированной длины.
* Повышена производительность при использовании `PREWHERE` и при неявном переносе выражений в `PREWHERE`.
* Повышена производительность разбора текстовых форматов (`CSV`, `TSV`). [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2977) [#2980](https://github.com/ClickHouse/ClickHouse/pull/2980)
* Улучшена производительность при чтении строк и массивов в двоичных форматах. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2955)
* Повышена производительность и снижено потребление памяти для запросов к `system.tables` и `system.columns`, когда на одном сервере находится очень большое число таблиц. [#2953](https://github.com/ClickHouse/ClickHouse/pull/2953)
* Исправлена проблема с производительностью при большом потоке запросов, которые завершаются ошибкой (функция `_dl_addr` видна в `perf top`, но сервер почти не нагружает CPU). [#2938](https://github.com/ClickHouse/ClickHouse/pull/2938)
* Условия передаются в представление (View) (когда `enable_optimize_predicate_expression` включён). [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2907)
...
* Для выражений `CASE` с условиями теперь можно опускать `ELSE`, что эквивалентно `ELSE NULL`. [#2920](https://github.com/ClickHouse/ClickHouse/pull/2920)
* Тайм-аут операций теперь можно задавать при работе с ZooKeeper. [urykhy](https://github.com/ClickHouse/ClickHouse/pull/2971)
* Вы можете задать смещение в `LIMIT n, m` с помощью `LIMIT n OFFSET m`. [#2840](https://github.com/ClickHouse/ClickHouse/pull/2840)
...
* Агрегатная функция `windowFunnel` теперь поддерживает события, которые соответствуют нескольким условиям. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2801)
* Дублирующиеся столбцы можно использовать в секции `USING` оператора `JOIN`. [#3006](https://github.com/ClickHouse/ClickHouse/pull/3006)
* Форматы `Pretty` теперь имеют ограничение на выравнивание столбцов по ширине. Используйте настройку `output_format_pretty_max_column_pad_width`. Если значение превышает этот предел по ширине, оно всё равно будет отображено полностью, но другие ячейки таблицы не будут чрезмерно широкими. [#3003](https://github.com/ClickHouse/ClickHouse/pull/3003)
* Табличная функция `odbc` теперь позволяет задавать имя базы данных/схемы. [Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2885)
...
* Запросы `ALTER DELETE` применимы к материализованным представлениям.
* Добавлена рандомизация при периодическом запуске потока очистки таблиц `ReplicatedMergeTree`, чтобы избежать пиков нагрузки при очень большом их количестве.
...
* Исправлена проблема с таблицами типа `Dictionary` (при которой выбрасывалось исключение `Size of offsets does not match size of column` или `Unknown compression method`). Эта ошибка появилась в версии 18.10.3. [#2913](https://github.com/ClickHouse/ClickHouse/issues/2913)
* Исправлена ошибка при слиянии таблиц `CollapsingMergeTree`, если одна из частей данных пуста (эти части формируются во время слияния или операции `ALTER DELETE`, если все данные были удалены) и для слияния использовался алгоритм `vertical`. [#3049](https://github.com/ClickHouse/ClickHouse/pull/3049)
* Исправлено состояние гонки при выполнении `DROP` или `TRUNCATE` для таблиц `Memory` при одновременном выполнении `SELECT`, которое могло приводить к сбоям сервера. Эта ошибка появилась в версии 1.1.54388. [#3038](https://github.com/ClickHouse/ClickHouse/pull/3038)
* Исправлена проблема, при которой могла происходить потеря данных при вставке в таблицы `Replicated`, если возвращалась ошибка `Session is expired` (потерю данных можно обнаружить по метрике `ReplicatedDataLoss`). Эта ошибка проявлялась в версии 1.1.54378. [#2939](https://github.com/ClickHouse/ClickHouse/pull/2939) [#2949](https://github.com/ClickHouse/ClickHouse/pull/2949) [#2964](https://github.com/ClickHouse/ClickHouse/pull/2964)
* Исправлена ошибка сегментации при выполнении `JOIN ... ON`. [#3000](https://github.com/ClickHouse/ClickHouse/pull/3000)
* Исправлена ошибка поиска имён столбцов, когда выражение `WHERE` полностью состоит из квалифицированного имени столбца, например `WHERE table.column`. [#2994](https://github.com/ClickHouse/ClickHouse/pull/2994)
* Исправлена ошибка &quot;Not found column&quot;, которая возникала при выполнении распределённых запросов, если с удалённого сервера запрашивался единственный столбец, представляющий собой выражение IN с подзапросом. [#3087](https://github.com/ClickHouse/ClickHouse/pull/3087)
* Исправлена ошибка `Block structure mismatch in UNION stream: different number of columns`, возникавшая при выполнении распределённых запросов, если один из шардов локальный, а другой — нет, и включалась оптимизация переноса условий в `PREWHERE`. [#2226](https://github.com/ClickHouse/ClickHouse/pull/2226) [#3037](https://github.com/ClickHouse/ClickHouse/pull/3037) [#3055](https://github.com/ClickHouse/ClickHouse/pull/3055) [#3065](https://github.com/ClickHouse/ClickHouse/pull/3065) [#3073](https://github.com/ClickHouse/ClickHouse/pull/3073) [#3090](https://github.com/ClickHouse/ClickHouse/pull/3090) [#3093](https://github.com/ClickHouse/ClickHouse/pull/3093)
* Исправлена функция `pointInPolygon` в отдельных случаях для невыпуклых многоугольников. [#2910](https://github.com/ClickHouse/ClickHouse/pull/2910)
...
* Исправлена утечка памяти при вставке в таблицу со столбцами типа `AggregateFunction`, если состояние агрегатной функции является непростым (память выделяется отдельно) и если один запрос вставки приводит к нескольким небольшим блокам. [#3084](https://github.com/ClickHouse/ClickHouse/pull/3084)
* Исправлена гонка при одновременном создании и удалении одной и той же таблицы `Buffer` или `MergeTree`.
* Устранена потенциальная ошибка сегментирования при сравнении кортежей, содержащих некоторые нетривиальные типы (например, кортежи). [#2989](https://github.com/ClickHouse/ClickHouse/pull/2989)
* Устранена возможная ошибка сегментации (segfault) при выполнении некоторых запросов `ON CLUSTER`. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2960)
* Исправлена ошибка в функции `arrayDistinct` для элементов массивов типа `Nullable`. [#2845](https://github.com/ClickHouse/ClickHouse/pull/2845) [#2937](https://github.com/ClickHouse/ClickHouse/pull/2937)
* Опция `enable_optimize_predicate_expression` теперь корректно работает в запросах с `SELECT *`. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2929)
* Исправлен сегфолт при повторной инициализации сессии ZooKeeper. [#2917](https://github.com/ClickHouse/ClickHouse/pull/2917)
...
- Безопасное использование источников данных ODBC. Взаимодействие с драйверами ODBC использует отдельный процесс `clickhouse-odbc-bridge`. Ошибки в сторонних драйверах ODBC больше не приводят к проблемам со стабильностью сервера или уязвимостям. [#2828](https://github.com/ClickHouse/ClickHouse/pull/2828) [#2879](https://github.com/ClickHouse/ClickHouse/pull/2879) [#2886](https://github.com/ClickHouse/ClickHouse/pull/2886) [#2893](https://github.com/ClickHouse/ClickHouse/pull/2893) [#2921](https://github.com/ClickHouse/ClickHouse/pull/2921)
...
- В запросах с JOIN символ звездочки раскрывается в список столбцов из всех таблиц в соответствии со стандартом SQL. Вы можете восстановить прежнее поведение, установив параметр `asterisk_left_columns_only` в значение 1 на уровне конфигурации пользователя.
...
- Поддержка типов Nullable в драйвере ClickHouse ODBC (формат вывода `ODBCDriver2`). [#2834](https://github.com/ClickHouse/ClickHouse/pull/2834)
...
- Поддержка пустых кусков после слияний в движках `SummingMergeTree`, `CollapsingMergeTree` и `VersionedCollapsingMergeTree`. [#2815](https://github.com/ClickHouse/ClickHouse/pull/2815)
- Старые записи о завершенных мутациях удаляются (`ALTER DELETE`). [#2784](https://github.com/ClickHouse/ClickHouse/pull/2784)
- Добавлена таблица `system.merge_tree_settings`. [Kirill Shvakov](https://github.com/ClickHouse/ClickHouse/pull/2841)
- В таблице `system.tables` добавлены столбцы зависимостей: `dependencies_database` и `dependencies_table`. [Winter Zhang](https://github.com/ClickHouse/ClickHouse/pull/2851)
...
- Исправлена ошибка слияния, когда все части данных были пустыми (части, сформированные в результате слияния или команды `ALTER DELETE`, если все данные были удалены). Эта ошибка появилась в версии 18.1.0. [#2930](https://github.com/ClickHouse/ClickHouse/pull/2930)
...
- Добавлена поддержка выражений ON для синтаксиса JOIN ON:
  ...
  Выражение должно представлять собой цепочку равенств, соединённых оператором AND. Каждая сторона равенства может быть произвольным выражением над столбцами одной из таблиц. Поддерживается использование полностью квалифицированных имён столбцов (`table.name`, `database.table.name`, `table_alias.name`, `subquery_alias.name`) для правой таблицы. [#2742](https://github.com/ClickHouse/ClickHouse/pull/2742)
...
- Изменена схема нумерации версий релизов. Теперь первая часть содержит год выпуска (н.э., московский часовой пояс, минус 2000), вторая часть содержит номер основных изменений (увеличивается для большинства релизов), а третья часть — версию патча. Релизы по-прежнему обратно совместимы, если иное не указано в журнале изменений.
- Ускорено преобразование чисел с плавающей точкой в строку ([Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2664)).
- Если некоторые строки были пропущены во время вставки из-за ошибок парсинга (это возможно при включенных настройках `input_allow_errors_num` и `input_allow_errors_ratio`), количество пропущенных строк теперь записывается в лог сервера ([Leonardo Cecchi](https://github.com/ClickHouse/ClickHouse/pull/2669)).
...
- Исправлен неверный результат функции `maxIntersection()` при совпадении границ интервалов ([Michael Furmur](https://github.com/ClickHouse/ClickHouse/pull/2657)).
...
- Запросы можно отправлять в формате `multipart/form-data` (в поле `query`), что полезно при отправке внешних данных для обработки запроса ([Olga Hvostikova](https://github.com/ClickHouse/ClickHouse/pull/2490)).
- Добавлена возможность включать или отключать обработку одинарных или двойных кавычек при чтении данных в формате CSV. Это настраивается с помощью параметров `format_csv_allow_single_quotes` и `format_csv_allow_double_quotes` ([Amos Bird](https://github.com/ClickHouse/ClickHouse/pull/2574)).
...
- Удалена избыточная проверка контрольных сумм при добавлении куска данных. Это важно при большом количестве реплик, поскольку в таких случаях общее количество проверок составляло N^2.
...
- Файл `format_version.txt` для таблиц `MergeTree` пересоздаётся, если он отсутствует, что актуально при запуске ClickHouse после копирования структуры каталогов без файлов ([Ciprian Hacman](https://github.com/ClickHouse/ClickHouse/pull/2593)).

#### Исправления ошибок: {#bug-fixes-19}


- Исправлена ошибка при работе с ZooKeeper, из-за которой было невозможно восстановить сессию и состояния readonly таблиц до перезапуска сервера.
...
### Релиз ClickHouse 1.1.54388, 2018-06-28 {#clickhouse-release-1-1-54388-2018-06-28}
...
- Добавлена возможность записи в таблицу с движком MySQL и соответствующая табличная функция ([sundy-li](https://github.com/ClickHouse/ClickHouse/pull/2294)).
- Добавлена табличная функция `url()` и движок таблиц `URL` ([Alexander Sapin](https://github.com/ClickHouse/ClickHouse/pull/2501)).
...
- Пароль для `clickhouse-client` может быть введён в интерактивном режиме.
...
- Добавлена настройка `date_time_input_format`. При установке этой настройки в значение `'best_effort'` значения DateTime будут читаться в широком диапазоне форматов.
...
- Исправлена невозможность выполнения запроса `OPTIMIZE` на реплике, не являющейся лидером, если для таблицы была выполнена операция `RENAME`.
...
- Исправлена ошибка в клиентской библиотеке ZooKeeper, которая приводила к потере наблюдателей, зависанию очереди распределённых DDL и замедлению очереди репликации при использовании непустого префикса `chroot` в конфигурации ZooKeeper.

#### Обратно несовместимые изменения: {#backward-incompatible-changes-8}

- Удалена поддержка выражений вида `(a, b) IN (SELECT (a, b))` (можно использовать эквивалентное выражение `(a, b) IN (SELECT a, b)`). В предыдущих релизах эти выражения приводили к неопределённой фильтрации `WHERE` или вызывали ошибки.
...
- Удалена специальная интерпретация выражения IN, если слева указан массив. Ранее выражение `arr IN (set)` интерпретировалось как «хотя бы один элемент `arr` принадлежит `set`». Чтобы получить такое же поведение в новой версии, используйте `arrayExists(x -> x IN (set), arr)`.
...
- При вставке данных в таблицу `Replicated` выполняется меньше запросов к `ZooKeeper` (и большинство пользовательских ошибок исчезли из лога `ZooKeeper`).
...
* Агрегация без `GROUP BY` для пустого набора (например, `SELECT count(*) FROM table WHERE 0`) теперь возвращает результат с одной строкой со значениями NULL для агрегатных функций, в соответствии со стандартом SQL. Чтобы вернуть старое поведение (возврат пустого результата), установите `empty_result_for_aggregation_by_empty_set` в 1.
* Добавлено приведение типов для `UNION ALL`. В позициях `SELECT` в `UNION ALL` теперь допускаются разные имена псевдонимов, в соответствии со стандартом SQL.
* В предложениях `LIMIT BY` поддерживаются произвольные выражения. Ранее можно было использовать только столбцы, полученные из `SELECT`.
...
* Добавлены функции консистентного хеширования: `yandexConsistentHash`, `jumpConsistentHash`, `sumburConsistentHash`. Их можно использовать в качестве ключа шардинга для снижения объёма сетевого трафика при последующих перешардингах.
* Добавлены функции: `arrayAny`, `arrayAll`, `hasAny`, `hasAll`, `arrayIntersect`, `arrayResize`.
* Добавлена функция `arrayCumSum` (Javi Santana).
* Добавлены функции `parseDateTimeBestEffort`, `parseDateTimeBestEffortOrZero` и `parseDateTimeBestEffortOrNull` для чтения значения DateTime из строки, содержащей текст в самом широком спектре возможных форматов.
* При обновлении данные могут частично перезагружаться из внешних словарей (загружать только записи, в которых значение указанного поля больше, чем было при предыдущей загрузке) (Arsen Hakobyan).
* Добавлена табличная функция `cluster`. Пример: `cluster(cluster_name, db, table)`. Табличная функция `remote` может принимать имя кластера в качестве первого аргумента, если оно передано как идентификатор.
* Табличные функции `remote` и `cluster` могут использоваться в запросах `INSERT`.
...
* В таблицу `system.part_log` добавлена дополнительная информация о слияниях.
* Для таблицы `system.query_log` может использоваться произвольный ключ партиционирования (Kirill Shvakov).
* Запрос `SHOW TABLES` теперь также показывает временные таблицы. В таблицу `system.tables` добавлены временные таблицы и столбец `is_temporary` (zhang2014).
* Добавлены запросы `DROP TEMPORARY TABLE` и `EXISTS TEMPORARY TABLE` (zhang2014).
* Реализована поддержка `SHOW CREATE TABLE` для временных таблиц (zhang2014).
* Добавлен конфигурационный параметр `system_profile` для настроек, используемых внутренними процессами.
* Поддержка загрузки `object_id` как атрибута в словари `MongoDB` (Pavel Litvinenko).
* Чтение `null` как значения по умолчанию при загрузке данных для внешнего словаря из источника `MongoDB` (Pavel Litvinenko).
* Чтение значений `DateTime` в формате `Values` из Unix-метки времени без одинарных кавычек.
* Механизм отказоустойчивости поддерживается в табличных функциях `remote` на случай, когда на некоторых репликах отсутствует запрошенная таблица.
* Параметры конфигурации можно переопределить из командной строки при запуске `clickhouse-server`. Пример: `clickhouse-server -- --logger.level=information`.
* Реализована функция `empty` с аргументом типа `FixedString`: функция возвращает значение 1, если строка полностью состоит из нулевых байтов (zhang2014).
* Добавлен конфигурационный параметр `listen_try`, позволяющий продолжать работу при неудаче привязки к части адресов и обеспечивающий прослушивание как минимум одного из указанных адресов (полезно для систем с отключенной поддержкой IPv4 или IPv6).
...
* Таблицы `MergeTree` можно использовать без первичного ключа (следует указать `ORDER BY tuple()`).
* Тип `Nullable` можно привести (`CAST`) к типу без `Nullable`, если аргумент не равен `NULL`.
* `RENAME TABLE` можно применять к `VIEW`.
* Добавлена функция `throwIf`.
* Добавлен параметр `odbc_default_field_size`, который позволяет увеличить максимальный размер значения, загружаемого из источника ODBC (по умолчанию он равен 1024).
* Таблица `system.processes` и команда `SHOW PROCESSLIST` теперь имеют столбцы `is_cancelled` и `peak_memory_usage`.
...
- Уменьшено количество ложных срабатываний `force_restore_data` при проверке состояния таблиц `Replicated` во время запуска сервера.
...
- Улучшена производительность функции `arrayElement` в случае константного многомерного массива с пустым массивом в качестве одного из элементов. Пример: `[[1], []][x]`.
...
- Добавлена возможность парсить пустую строку в текстовых форматах как число 0 для типов данных `Float`. Эта функциональность была доступна ранее, но была утрачена в релизе 1.1.54342.
...
- Исправлена ошибка сегментации для некоторых случаев использования `FULL` и `RIGHT JOIN` с дублирующимися столбцами в подзапросах.
- Исправлена ошибка сегментации для некоторых случаев использования `replace_running_query` и `KILL QUERY`.
...
- Исправлена низкая точность функций `uniqHLL12` и `uniqCombined` для мощностей более 100 миллионов элементов (Alex Bocharov).
...
- Исправлено падение, которое происходило при выполнении запроса `CHECK` для таблиц `Distributed`, если все шарды локальные (chenxing.xc).
...
- Исправлена ошибка, которая могла привести к появлению дополнительной секции `FORMAT` в `.sql`-файле с метаданными.
...
- Исправлена ошибка при чтении элементов столбцов `Nested` структур, которые были добавлены с помощью `ALTER`, но пусты для старых партиций, когда условия для этих столбцов перемещались в `PREWHERE`.
...
### Релиз ClickHouse 1.1.54343, 2018-02-05 {#clickhouse-release-1-1-54343-2018-02-05}
...
- Улучшена обработка дубликатов при вставке в реплицируемые таблицы, благодаря чему они больше не замедляют выполнение очереди репликации.

### Релиз ClickHouse 1.1.54342, 2018-01-22 {#clickhouse-release-1-1-54342-2018-01-22}
...
- Исправлено недетерминированное поведение GraphiteMergeTree (можно увидеть в сообщениях лога `Data after merge is not byte-identical to the data on another replicas`).
- Исправлена ошибка, которая может привести к несогласованным слияниям после запроса OPTIMIZE к реплицируемым таблицам (можно увидеть в сообщениях лога `Part ... intersects the previous part`).
...
### Релиз ClickHouse 1.1.54337, 2018-01-18 {#clickhouse-release-1-1-54337-2018-01-18}
...
- Поддержка табличных функций для запросов `DESCRIBE` и `INSERT`. Добавлена поддержка подзапросов в `DESCRIBE`. Примеры: `DESC TABLE remote('host', default.hits)`; `DESC TABLE (SELECT 1)`; `INSERT INTO TABLE FUNCTION remote('host', default.hits)`. Поддержка `INSERT INTO TABLE` в дополнение к `INSERT INTO`.
- Улучшена поддержка часовых поясов. Тип данных `DateTime` может быть аннотирован часовым поясом, который используется для разбора и форматирования в текстовых форматах. Пример: `DateTime('Asia/Istanbul')`. Когда часовые пояса указаны в функциях для аргументов `DateTime`, возвращаемый тип будет отслеживать часовой пояс, и значение будет отображаться корректно.
...
- Добавлен конфигурационный параметр `format_schema_path` (Marek Vavruşa). Он используется для указания схемы в формате `Cap'n Proto`. Файлы схем могут находиться только в указанном каталоге.
- Добавлена поддержка подстановок конфигурации (`incl` и `conf.d`) для настройки внешних словарей и моделей (Pavel Yakunin).
- Добавлен столбец с документацией для таблицы `system.settings` (Kirill Shvakov).
...
- Добавлена возможность передачи аргумента типа `AggregateFunction` для агрегатной функции `groupArray` (что позволяет создать массив состояний некоторой агрегатной функции).
- Сняты ограничения на различные комбинации комбинаторов агрегатных функций. Например, можно использовать агрегатные функции `avgForEachIf`, а также `avgIfForEach`, которые имеют различное поведение.
- Комбинатор агрегатных функций `-ForEach` расширен для случая агрегатных функций с несколькими аргументами.
- Добавлена поддержка агрегатных функций с аргументами `Nullable` даже для случаев, когда функция возвращает не-`Nullable` результат (добавлено при участии Silviu Caragea). Пример: `groupArray`, `groupUniqArray`, `topK`.
...
- Добавлена поддержка предложений `WITH` для запроса `INSERT SELECT` (автор: zhang2014).
- Добавлены настройки: `http_connection_timeout`, `http_send_timeout`, `http_receive_timeout`. В частности, эти настройки используются для загрузки частей данных при репликации. Изменение этих настроек позволяет ускорить переключение при отказе в случае перегрузки сети.
...
- Улучшена производительность разбора и форматирования значений типов `Date` и `DateTime` в текстовом формате.
- Улучшена производительность и точность разбора чисел с плавающей точкой.
...
- Исправлена ошибка, которая могла привести к неправильной интерпретации условия `WHERE` для запросов `CREATE MATERIALIZED VIEW` с `POPULATE`.
- Исправлена ошибка при использовании параметра `root_path` в конфигурации `zookeeper_servers`.
- Исправлены неожиданные результаты при передаче аргумента `Date` в функцию `toStartOfDay`.
...
- Добавлена недостающая поддержка типа данных `UUID` для `DISTINCT`, `JOIN` и агрегатных функций `uniq`, а также внешних словарей (Evgeniy Ivanov). Поддержка `UUID` всё ещё неполная.
...
- Улучшены сообщения об ошибках о несовместимых типах, переданных в функции `multiIf`, `array` и некоторые другие.
...
- Одна сборка используется для различных версий ОС. Пакеты и бинарные файлы совместимы с широким спектром систем Linux.
- Добавлен пакет `clickhouse-test`. Он может использоваться для запуска функциональных тестов.
- Архив с исходным кодом теперь может публиковаться в репозитории. Его можно использовать для воспроизведения сборки без использования GitHub.
...
- При выполнении последовательного обновления кластера, в момент, когда некоторые реплики работают на старой версии ClickHouse, а некоторые — на новой версии, репликация временно останавливается и в логе появляется сообщение `unknown parameter 'shard'`. Репликация продолжится после обновления всех реплик кластера.
