...
**2.** Для имен функций (методов) используйте стиль camelCase, начиная со строчной буквы.
...
В `.h`-файле библиотеки вы можете использовать `namespace detail`, чтобы скрыть детали реализации, которые не нужны прикладному коду.
