...
Тест может иметь ноль или более *тегов*, определяющих ограничения на то, в каких контекстах тест запускается в CI.

Для `.sql`‑тестов теги указываются в первой строке в виде SQL‑комментария:
...
Для `.sh`‑тестов теги указываются в комментарии на второй строке:
...
| `no-fasttest`                  | Тест не выполняется в режиме [Быстрый тест](#test-types)                            | Тест использует движок таблиц `MySQL`, который отключён в Быстрый тест                                        |
| `fasttest-only`                | Тест выполняется только в режиме [Быстрый тест](#test-types)                        |                                                                                                               |
...
| `no-darwin`                    | Отключает тесты в macOS (Darwin)                                                    | Тест зависит от специфичных для Linux возможностей, таких как распределённый запрос, `procfs` или HTTP-сервер |

Также поддерживаются следующие опции: `no-polymorphic-parts`, `no-random-settings`, `no-random-merge-tree-settings`, `no-backward-compatibility-check`, `no-cpu-x86_64`, `no-cpu-aarch64`, `no-cpu-ppc64le`, `no-s3-storage`.
...
### Указание ограничений для случайных значений настроек \{#specifying-limits-for-random-settings\}

Тест может задать минимальные и максимальные допустимые значения для настроек, которые могут принимать случайные значения во время выполнения теста.

Для `.sh`-тестов ограничения указываются как комментарий в строке рядом с тегами или во второй строке, если теги не заданы:
...
Параллельно могут быть добавлены другие тесты с тем же числовым префиксом, но это нормально и не приводит к каким-либо проблемам, ничего потом менять не потребуется.


### Проверка ошибки, которая должна произойти \{#checking-for-an-error-that-must-occur\}
