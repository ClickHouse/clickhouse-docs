...
:::note
`clickhouse-server`, `clickhouse-client` и т.д. являются символическими ссылками на бинарный файл `clickhouse` в каталоге `programs/`, создаваемыми после завершения сборки. Поэтому достаточно собрать только цель `clickhouse`.
:::
