---
description: 'Обзор политики бэкпорта и автоматизации в ClickHouse'
...
В этом документе описаны политика бэкпорта ClickHouse и автоматизированная система, реализующая её.
...
Активная ветка релиза — это любая ветка, для которой соответствующий release PR (с меткой `release`) всё ещё открыт на GitHub. Автоматизация бэкпорта определяет такие ветки динамически при каждом запуске, поэтому при выпуске нового релиза или завершении жизненного цикла старого изменения конфигурации не требуются.

Ветка релиза может находиться в состоянии **раскатки** (если её release PR содержит метку `rolling-out`) в период, когда разворачивается новый релиз. Для веток в состоянии раскатки обычные бэкпорты приостанавливаются, чтобы не усложнять раскатку. Метки для конкретных версий (например, `v25.3-must-backport`) отменяют это ограничение и принудительно включают бэкпорт даже во время раскатки.

Метка для конкретной версии задаёт *самый ранний* релиз, в который должен попасть PR: бэкпорт выполняется в этот релиз **и во все более новые активные ветки релиза**, а не только в явно указанную. Например, `v25.3-must-backport` в PR, слитом в ветку development, приводит к бэкпорту в `25.3` и во все последующие активные ветки релиза (`25.4`, `25.5`, …). Если присутствует несколько меток для конкретных версий, выбирается наименьшая версия, поскольку она уже охватывает все более новые.
...
| `v{VER}-must-backport` (например, `v25.3-must-backport`) | Бэкпортирование в эту ветку релиза **и во все более новые активные ветки релиза** — версия указывает *самый старый* релиз, в который PR должен попасть. При наличии нескольких таких меток выбирается наименьшая версия. Override пропуск по `rolling-out` для этих веток |
| `pr-backports-created`                                   | Устанавливается ботом, когда созданы все необходимые PR для бэкпортирования; снимается, если PR cherry-pick открывают повторно                                                                                                                                            |
| `pr-cherrypick`                                          | Применяется к PR cherry-pick, созданным ботом                                                                                                                                                                                                                              |
| `pr-backport`                                            | Применяется к бэкпорт PR, созданным ботом                                                                                                                                                                                                                                  |
