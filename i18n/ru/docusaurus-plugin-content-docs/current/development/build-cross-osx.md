---
description: 'Руководство по кросс-компиляции ClickHouse на Linux для систем macOS'
...
Этот документ описывает случай, когда у вас есть машина с Linux, и вы хотите использовать её для сборки исполняемого файла `clickhouse`, который будет запускаться под OS X.
Основной сценарий использования — проверки системы непрерывной интеграции, выполняемые на машинах с Linux.
...
Например, замените `x86_64-apple-darwin` на `aarch64-apple-darwin` на всех шагах.



## Установка набора инструментов для кросс-компиляции {#install-cross-compilation-toolset}

Сохраним путь установки `cctools` в переменной `${CCTOOLS}`
...
Также необходимо загрузить macOS X SDK в рабочий каталог.
...
Полученный исполняемый файл будет иметь формат Mach-O и не может быть запущен в Linux.
