---
description: 'Всесторонний обзор архитектуры ClickHouse и её колоночной
  структуры'
...
ClickHouse — это по-настоящему колоночная СУБД. Данные хранятся по столбцам и при выполнении запросов обрабатываются массивами (векторами или фрагментами столбцов).
...
:::note
Для большинства внешних приложений мы рекомендуем использовать HTTP‑интерфейс, поскольку он простой и удобный. TCP‑протокол более тесно связан с внутренними структурами данных: он использует внутренний формат для передачи блоков данных и собственный протокол фрейминга для сжатых данных.
:::
...
ClickHouse управляет настройками посредством иерархии контекстов:

* **Глобальный контекст** — настройки всего сервера, заданные через конфигурационные файлы
* **Сессионный контекст** — настройки пользовательской сессии из профилей, пользовательской конфигурации и команд SET
* **Контекст запроса** — настройки на уровне запроса из предложения SETTINGS
* **Фоновый контекст** — серверные настройки фоновых операций (Mutate, Merge), определённые через профиль `background`

При планировании выполнения операции (запросы, мутации и т.д.) сервер формирует конкретный контекст, объединяя настройки в следующем порядке (более поздние секции переопределяют более ранние):

1. Глобальные значения по умолчанию
2. Глобальная конфигурация
3. Настройки профиля (из секции `<profiles>`)
4. Пользовательские настройки (из секции `<users>`)
5. Сессионные настройки (из команды SET)
6. Настройки запроса (из предложения SETTINGS)

:::note
Фоновые операции могут быть сконфигурированы с помощью глобальных настроек и настроек профиля `background`; сессионные и настройки запроса в этом случае не влияют на поведение. Если явная конфигурация не задана, настройки наследуются из глобального контекста. Имя профиля по умолчанию для таких операций — `background`, его можно переопределить с помощью серверной настройки `background_profile`.
:::
