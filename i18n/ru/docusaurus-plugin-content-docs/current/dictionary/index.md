...
description: 'Словарь предоставляет представление данных в формате ключ–значение для быстрого доступа.'
doc_type: 'reference'
---
...
Словарь в ClickHouse представляет данные в памяти в формате [ключ–значение](https://en.wikipedia.org/wiki/Key%E2%80%93value_database) из различных [внутренних и внешних источников](/sql-reference/dictionaries#dictionary-sources), оптимизированном для запросов поиска с минимальной задержкой.
...
В этом случае ClickHouse может использовать словарь для выполнения [прямого соединения (Direct Join)](https://clickhouse.com/blog/clickhouse-fully-supports-joins-direct-join-part4#direct-join). Это самый быстрый алгоритм соединения в ClickHouse, применимый в случаях, когда базовый [движок таблиц](/engines/table-engines) для правой таблицы поддерживает низколатентные запросы ключ-значение. В ClickHouse есть три движка таблиц, обеспечивающих это: [Join](/engines/table-engines/special/join) (по сути, предварительно вычисленная хеш-таблица), [EmbeddedRocksDB](/engines/table-engines/integrations/embedded-rocksdb) и [Dictionary](/engines/table-engines/special/dictionary). Мы опишем подход на основе словаря, но механизм работы одинаков для всех трёх движков.

Алгоритм прямого соединения требует, чтобы правая таблица была основана на словаре, так что данные из этой таблицы, подлежащие соединению, уже находятся в памяти в виде низколатентной структуры данных ключ-значение.

### Пример {#example}

Используя набор данных Stack Overflow, ответим на вопрос:
_Какой пост о SQL на Hacker News является наиболее спорным?_

Мы определим спорность как ситуацию, когда посты имеют примерно одинаковое количество положительных и отрицательных голосов. Мы вычисляем абсолютную разницу, где значение ближе к 0 означает большую спорность. Предположим, что пост должен иметь как минимум 10 положительных и 10 отрицательных голосов — посты, за которые люди не голосуют, не являются особо спорными.
...
> **Используйте меньшие наборы данных в правой части `JOIN`**: Этот запрос может показаться более многословным, чем требуется, с фильтрацией по `PostId` как во внешнем, так и во вложенном запросах. Это оптимизация производительности, которая обеспечивает быстрое время отклика запроса. Для оптимальной производительности всегда следите за тем, чтобы правая часть `JOIN` была меньшим набором данных и настолько малой, насколько это возможно. Для получения советов по оптимизации производительности JOIN и понимания доступных алгоритмов мы рекомендуем [эту серию статей в блоге](https://clickhouse.com/blog/clickhouse-fully-supports-joins-part1).

Хотя этот запрос выполняется быстро, он требует тщательного написания `JOIN` для достижения хорошей производительности. В идеале мы бы просто отфильтровали посты до тех, которые содержат «SQL», прежде чем смотреть на количество `UpVote` и `DownVote` для подмножества постов для вычисления нашей метрики.
...
Данные будут храниться в несжатом виде в нашем словаре, поэтому нам потребуется как минимум 4 ГБ памяти, если бы мы хранили все столбцы (мы этого делать не будем) в словаре. Словарь будет реплицирован по всему нашему кластеру, поэтому этот объем памяти должен быть зарезервирован *для каждого узла*.

> В примере ниже данные для нашего словаря берутся из таблицы ClickHouse. Хотя это и является наиболее распространённым источником словарей, поддерживается [ряд источников](/sql-reference/dictionaries#dictionary-sources), включая файлы, HTTP и базы данных, в том числе [Postgres](/sql-reference/dictionaries#postgresql). Как мы покажем, словари могут автоматически обновляться, что является идеальным способом обеспечить, чтобы небольшие наборы данных, подверженные частым изменениям, были доступны для прямых соединений (JOIN).

Наш словарь требует первичного ключа, по которому будет выполняться поиск. Концептуально это идентично первичному ключу в транзакционной базе данных и должно быть уникальным. Наш запрос выше требует поиска по ключу соединения — `PostId`. Словарь, в свою очередь, должен быть заполнен суммарным количеством голосов «за» и «против» по `PostId` из нашей таблицы `votes`. Вот запрос для получения этих данных для словаря:
...
Для создания нашего словаря используется следующий DDL — обратите внимание, что здесь используется приведённый выше запрос:
...
> В самостоятельной установке OSS приведённую выше команду необходимо выполнить на всех узлах. В ClickHouse Cloud словарь будет автоматически реплицирован на все узлы. Команда выше была выполнена на узле ClickHouse Cloud с 64 ГБ ОЗУ, загрузка заняла 36 секунд.

Чтобы подтвердить объём памяти, потребляемой нашим словарём:
...
Этот запрос не только гораздо проще, но и более чем в два раза быстрее! Его можно ещё сильнее оптимизировать, загружая в словарь только публикации с более чем 10 голосами «за» и «против» и сохраняя только предварительно вычисленное значение степени спорности.


## Обогащение данных во время выполнения запроса {#query-time-enrichment}

Словари можно использовать для поиска значений во время выполнения запроса. Эти значения могут возвращаться в результатах или использоваться в агрегациях. Предположим, мы создаём словарь для сопоставления идентификаторов пользователей с их местоположением:
...
Аналогично приведённому выше примеру с соединением, мы можем использовать тот же словарь для эффективного определения, откуда происходит большинство постов:
...
В приведенном выше примере мы использовали словарь во время выполнения запроса для устранения соединения. Словари также можно использовать для обогащения строк во время вставки. Это обычно целесообразно, если значение обогащения не изменяется и существует во внешнем источнике, который можно использовать для заполнения словаря. В этом случае обогащение строки во время вставки позволяет избежать обращения к словарю во время выполнения запроса.

Предположим, что `Location` пользователя в Stack Overflow никогда не меняется (в действительности меняется) — в частности, столбец `Location` таблицы `users`. Предположим, мы хотим выполнить аналитический запрос к таблице постов по местоположению. Эта таблица содержит `UserId`.
...
В приведенном выше примере `Location` объявлен как столбец `MATERIALIZED`. Это означает, что значение может быть предоставлено как часть запроса `INSERT` и всегда будет вычисляться.

> ClickHouse также поддерживает [столбцы `DEFAULT`](/sql-reference/statements/create/table#default_values) (где значение может быть вставлено или вычислено, если не предоставлено).
...
Теперь можно получить название местоположения, из которого происходит большинство постов:
...
### Выбор параметра `LAYOUT` словаря {#choosing-the-dictionary-layout}
