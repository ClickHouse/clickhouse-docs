...
## Релизы Terraform-провайдеров \{#terraform-provider-releases\}

ClickHouse поддерживает два официальных Terraform-провайдера — провайдер ClickHouse Cloud для облачной инфраструктуры и провайдер DBops для объектов на уровне базы данных. Оба используют одну и ту же модель релизов.

### Стабильные версии и альфа-версии \{#stable-vs-alpha\}

Стабильные версии (например, 3.11.1, 1.9.0) включают только ресурсы для функций со статусом GA. Альфа-версии (например, 3.12.0-alpha2, 1.10.0-alpha1) включают всё, что есть в стабильных версиях, а также ресурсы для функций, которые всё ещё находятся в статусе бета или закрытой предварительной версии, и для использования должны быть явно зафиксированы.

### Версионирование \{#versioning\}

Оба провайдера используют семантическое версионирование (MAJOR.MINOR.PATCH). Мажорная версия увеличивается при несовместимых изменениях, минорная — при добавлении новых функций или ресурсов, а
патч-версия — при исправлении ошибок. В альфа-релизах к следующей минорной версии добавляется суффикс предварительного выпуска (например, 3.12.0-alpha1), при этом номер альфа увеличивается по мере добавления исправлений или изменений до перевода в стабильный релиз (например, alpha1 → alpha2 → alpha3). Релизы выпускаются по мере необходимости, а не по фиксированному расписанию. Новый альфа-релиз создаётся, когда для функции, ещё не достигшей стадии GA, добавляется ресурс или когда исправление требует ранней проверки. Новый стабильный релиз создаётся, когда накопленные изменения — включая функции, которые к этому моменту уже достигли стадии GA, — готовы для продакшена, как правило после периода получения отзывов от клиентов. До объединения в один стабильный релиз может накопиться несколько альфа-версий с разными минорными версиями.

### Переход из альфа в стабильный \{#promotion\}

Когда возможность Terraform готова к GA, соответствующий ресурс Terraform переводится из альфа в стабильный в следующем стабильном релизе. До этого ресурс доступен только в альфа-сборках.
