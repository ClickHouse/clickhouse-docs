...
организациями и сервисами в ClickHouse Cloud. С помощью нашего Cloud API вы можете 
создавать и управлять сервисами, выдавать API-ключи и добавлять или удалять участников в вашей 
организации и многое другое.

[Узнайте, как создать свой первый API-ключ и начать использовать API ClickHouse Cloud.](/cloud/manage/openapi)

## Эндпоинт и UI Swagger (OpenAPI) \{#swagger-openapi-endpoint-and-ui\}
...
Это повлияет на объекты, возвращаемые запросами `POST`, `GET` и `PATCH` для сервисов. Поэтому любой код, использующий эти API, может потребовать доработки для корректной обработки этих изменений.
:::
