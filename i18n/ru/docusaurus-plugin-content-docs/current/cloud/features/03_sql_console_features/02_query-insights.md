...
description: 'Визуализация данных system.query_log для упрощения отладки запросов и оптимизации производительности'
...
После выбора сервиса элемент навигации **Monitoring** в левой боковой панели раскроется, отобразив новый подпункт **Query insights**. При нажатии на него откроется страница Query insights:
...
Как видно из боковой панели, этот запрос был выполнен более 3000 раз за последние 24 часа. Все метрики на вкладке **Query info** являются агрегированными, но можно также просмотреть метрики отдельных выполнений, перейдя на вкладку **Query history**:
