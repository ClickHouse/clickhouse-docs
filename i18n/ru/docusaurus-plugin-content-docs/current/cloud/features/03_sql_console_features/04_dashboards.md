...
Функция панелей мониторинга в SQL Console позволяет собирать и совместно использовать визуализации на основе сохранённых запросов. Начните с сохранения и визуализации запросов, добавления визуализаций запросов на панель, а затем сделайте панель интерактивной с помощью параметров запросов.

## Базовые понятия \{#core-concepts\}

### Общий доступ к запросам \{#query-sharing\}

Чтобы поделиться панелью с коллегами, обязательно предоставьте общий доступ к базовому сохранённому запросу. Для просмотра визуализации вы должны иметь как минимум доступ «только для чтения» к этому сохранённому запросу.
...
Создадим панель мониторинга для нашего сервиса ClickHouse, используя системную таблицу [query&#95;log](/operations/system-tables/query_log).
