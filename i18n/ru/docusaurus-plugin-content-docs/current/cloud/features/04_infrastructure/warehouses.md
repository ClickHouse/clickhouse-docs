...
В ClickHouse Cloud *warehouse* — это набор сервисов, которые используют одни и те же данные.
...
2. Refreshable materialized views выполняются **только** на сервисах с правами чтения-записи (RW) в рамках warehouse. Они **не** выполняются на сервисах только для чтения (RO).
   :::
...
7. **В настоящее время действует мягкий лимит — до 5 сервисов на один warehouse.** Обратитесь в службу поддержки, если вам требуется более 5 сервисов в одном warehouse.
