...
Все сервисы в хранилище используют одни и те же:
...
Поскольку все сервисы в хранилище используют один и тот же набор таблиц, они также используют общие настройки управления доступом. Это означает, что все пользователи базы данных, созданные в Сервисе 1, смогут использовать Сервис 2 с теми же правами доступа (гранты на таблицы, представления и т. д.), и наоборот. Пользователи будут использовать разные конечные точки для каждого сервиса, но с одним и тем же именем пользователя и паролем. Другими словами, _пользователи являются общими для всех сервисов, работающих с одним хранилищем:_
...
Иногда полезно ограничить доступ на запись к определенному сервису и разрешить запись только подмножеству сервисов в хранилище. Это можно сделать при создании второго и последующих сервисов (первый сервис всегда должен иметь права на чтение и запись):

<Image
  img={compute_5}
  size='lg'
  alt='Сервисы с правами чтения-записи и только чтения в хранилище'
/>

<br />

_Рис. 6 — Сервисы с правами чтения-записи и только чтения в хранилище_
...
2. В настоящее время обновляемые материализованные представления выполняются на всех сервисах в хранилище, включая сервисы только для чтения. Однако это поведение будет изменено в будущем, и они будут выполняться только на сервисах с правами чтения-записи.
   :::
...
2. **Иногда рабочие нагрузки невозможно изолировать.** Хотя цель состоит в том, чтобы предоставить вам возможность изолировать рабочие нагрузки баз данных друг от друга, могут возникать пограничные случаи, когда одна рабочая нагрузка в одном сервисе будет влиять на другой сервис, использующий те же данные. Это довольно редкие ситуации, которые в основном связаны с OLTP-подобными рабочими нагрузками.
...
7. **В настоящее время существует мягкое ограничение в 5 сервисов на хранилище.** Свяжитесь с командой поддержки, если вам требуется более 5 сервисов в одном хранилище.


## Цены {#pricing}

Стоимость вычислительных ресурсов одинакова для всех сервисов в хранилище (основного и дополнительных). Хранилище оплачивается только один раз — оно включено в первый (основной) сервис.

Воспользуйтесь калькулятором стоимости на странице [pricing](https://clickhouse.com/pricing), который поможет оценить затраты в зависимости от объёма рабочей нагрузки и выбранного уровня сервиса.
...
На экране создания сервиса исходный сервис будет автоматически выбран в выпадающем списке в качестве источника данных для нового сервиса. После создания эти два сервиса образуют хранилище.

### Переименование хранилища {#renaming-a-warehouse}

Существует два способа переименовать хранилище:

- Выберите «Сортировать по хранилищам» на странице сервисов в правом верхнем углу, а затем нажмите на значок карандаша рядом с названием хранилища
- Нажмите на название хранилища в любом из сервисов и переименуйте хранилище там
...
Удалить хранилище можно только путем удаления первого созданного сервиса. Для этого:
