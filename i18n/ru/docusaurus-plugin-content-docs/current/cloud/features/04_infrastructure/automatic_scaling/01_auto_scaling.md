...
Масштабирование — это возможность изменять доступные ресурсы в соответствии с запросами клиентов. Сервисы тарифов Scale и Enterprise (со стандартным профилем 1:4) можно масштабировать горизонтально, вызывая API программно или изменяя настройки в интерфейсе для регулировки системных ресурсов. Эти сервисы также могут **автоматически масштабироваться** вертикально в соответствии с потребностями приложения.

<ScalePlanFeatureBadge feature="Automatic vertical scaling"/>

:::note
Тарифы Scale и Enterprise поддерживают как одно-, так и многорепликовые сервисы, тогда как тариф Basic поддерживает только однорепликовые сервисы. Однорепликовые сервисы имеют фиксированный размер и не поддерживают вертикальное или горизонтальное масштабирование. Пользователи могут перейти на тариф Scale или Enterprise, чтобы масштабировать свои сервисы.
:::
...
Это обеспечивает баланс между необходимостью завершения существующих запросов и гарантией того, что старые реплики не будут задерживаться слишком долго.
...
:::note
Сервисы могут масштабироваться горизонтально до максимум 20 реплик. Если вам требуется больше реплик, пожалуйста, свяжитесь с нашей службой поддержки.
:::

### Горизонтальное масштабирование через API {#horizontal-scaling-via-api}

Для горизонтального масштабирования кластера выполните `PATCH`-запрос через API для изменения количества реплик. Скриншоты ниже показывают API-вызов для масштабирования кластера с `3` реплик до `6` реплик и соответствующий ответ.
...
Если вы отправите новый запрос на масштабирование или несколько последовательных запросов, пока один уже выполняется, служба масштабирования проигнорирует промежуточные состояния и сойдется к финальному количеству реплик.
...
После масштабирования сервиса панель метрик в облачной консоли должна показать корректное распределение ресурсов для сервиса. Скриншот ниже показывает кластер, масштабированный до общей памяти `96 GiB`, что соответствует `6` репликам, каждая с выделением памяти `16 GiB`.
...
На странице **Settings** вы также можете выбрать, разрешить ли автоматический переход сервиса в режим простоя при отсутствии активности, как показано на изображении выше (т. е. когда сервис не выполняет пользовательские запросы). Автоматический переход в режим простоя снижает стоимость сервиса, поскольку вычислительные ресурсы не тарифицируются, когда сервис приостановлен.
...
упреждающего масштабирования сервиса, чтобы справиться с пиком, и уменьшить его масштаб после
снижения нагрузки.

Чтобы узнать текущее количество ядер CPU и объём используемой памяти для
