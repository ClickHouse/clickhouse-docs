...
На странице **Settings** вы также можете выбрать, разрешать ли автоматический переход сервиса в режим ожидания, когда он неактивен в течение определенного времени (то есть когда сервис не выполняет никаких пользовательских запросов). Автоматический переход в режим ожидания снижает стоимость работы сервиса, так как с вас не взимается плата за вычислительные ресурсы, пока сервис приостановлен.

### Адаптивный переход в режим ожидания \{#adaptive-idling\}

ClickHouse Cloud реализует адаптивный переход в режим ожидания, чтобы избежать сбоев при одновременной оптимизации затрат. Перед переводом сервиса в режим ожидания система оценивает несколько условий. Адаптивный переход в режим ожидания переопределяет настройку длительности ожидания, когда выполняется любое из перечисленных ниже условий:

- Когда количество частей превышает максимальный порог для перехода в режим ожидания (значение по умолчанию: 10 000), сервис не переводится в режим ожидания, чтобы фоновое обслуживание могло продолжаться
- Когда выполняются операции слияния (merge), сервис не переводится в режим ожидания до завершения этих слияний, чтобы не прерывать критически важную консолидацию данных
- Кроме того, сервис адаптирует тайм-ауты ожидания в зависимости от времени инициализации сервера:
  - Если время инициализации сервера менее 15 минут, адаптивный тайм-аут не применяется, и используется настроенный пользователем тайм-аут ожидания по умолчанию
  - Если время инициализации сервера от 15 до 30 минут, тайм-аут ожидания устанавливается в 15 минут
  - Если время инициализации сервера от 30 до 60 минут, тайм-аут ожидания устанавливается в 30 минут
  - Если время инициализации сервера более 60 минут, тайм-аут ожидания устанавливается в 1 час
