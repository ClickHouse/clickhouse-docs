...
description: 'Описание компонента Shared Catalog и движка Shared Database в ClickHouse Cloud'
doc_type: 'reference'
---
...
Shared Catalog — это облачный компонент, отвечающий за репликацию метаданных и DDL-операций баз данных и таблиц, использующих stateless-движки, между репликами в ClickHouse Cloud. Он обеспечивает согласованное и централизованное управление состоянием этих объектов, гарантируя консистентность метаданных даже в динамических средах или при частичной недоступности узлов.
...
**Движок базы данных Shared** работает совместно со Shared Catalog для управления базами данных, таблицы которых используют **движки таблиц без сохранения состояния** (stateless), такие как `SharedMergeTree`. Эти движки таблиц не записывают постоянное состояние на диск и совместимы с динамическими вычислительными средами.
...
- **Атомарная операция CREATE TABLE ... AS SELECT**
  Создание таблицы и вставка данных выполняются атомарно — либо вся операция завершается успешно, либо таблица не создаётся вообще.
...
- **Автоматическая репликация метаданных**
  Shared Catalog гарантирует, что определения баз данных автоматически реплицируются на все серверы при запуске. Операторам не требуется вручную настраивать или синхронизировать метаданные на новых экземплярах.

- **Централизованное версионированное состояние метаданных**
  Shared Catalog хранит единый источник истины в ZooKeeper. При запуске реплика получает актуальное состояние и применяет изменения для достижения согласованности. Во время выполнения запроса система может ожидать, пока другие реплики достигнут как минимум требуемой версии метаданных для обеспечения корректности.
...
- Надёжную и автоматическую репликацию метаданных для движков без сохранения состояния
- Вычисления без сохранения состояния и без локального хранения метаданных
- Атомарные операции для сложных DDL-команд
