...
Мониторинг системы баз данных в рабочем (продуктивном) окружении имеет ключевое значение для
понимания состояния развертывания и своевременного предотвращения или устранения сбоев.

Расширенная панель мониторинга — это легковесный инструмент, предназначенный для получения глубокого представления
о системе ClickHouse и её окружении, помогая вам своевременно выявлять
узкие места производительности, сбои системы и неэффективность.
...
Для доступа к встроенной расширенной панели мониторинга выполните следующие действия:

- Левая боковая панель
  - `Monitoring` → `Advanced dashboard`
  - Нажмите `You can still access the native advanced dashboard.`

Встроенная расширенная панель мониторинга откроется в новой вкладке. Для доступа к панели потребуется пройти аутентификацию.

<Image img={NativeAdvancedDashboard} size='lg' alt='Расширенная панель мониторинга' />

Каждая визуализация связана с SQL-запросом, который заполняет её данными. Вы можете отредактировать этот запрос, нажав на значок карандаша.
...
Графики по умолчанию в расширенной панели управления предназначены для обеспечения
...
| Total MergeTree Parts     | Показывает количество активных кусков в таблицах MergeTree, помогая выявить небатчированные вставки |
...
| Load Average (15 minutes) | Сообщает текущую среднюю нагрузку за 15 минут                            |
...
несколько проблем, которые можно обнаружить с помощью расширенной панели мониторинга.
...
Ниже приведен пример пикового использования CPU без значительного изменения
...
Еще одна проблема, которую можно обнаружить с помощью расширенной панели мониторинга, — это неудачный дизайн первичного ключа.
...
Одна из метрик, которую можно отслеживать для выявления потенциальных улучшений в первичных ключах,
