...
description: 'Быстро разверните конечные точки REST API на основе сохранённых запросов'
...
Функция **Query API Endpoints** позволяет создать конечную точку API напрямую из любого сохранённого SQL-запроса в консоли ClickHouse Cloud. Вы сможете обращаться к конечным точкам API по HTTP, чтобы выполнять сохранённые запросы без необходимости подключаться к сервису ClickHouse Cloud через нативный драйвер.
...
Для выполнения запросов к API-эндпоинту API-ключу требуется организационная роль `Member` с доступом к сервису `Query Endpoints`. Роль базы данных настраивается при создании эндпоинта.
:::
...
Откройте новую вкладку запроса. В демонстрационных целях мы будем использовать [набор данных youtube](/getting-started/example-datasets/youtube-dislikes), который содержит приблизительно 4,5 миллиарда записей.
...
Руководство по примеру набора данных вставляет большой объем данных — 4,65 миллиарда строк, что может занять значительное время.
Для целей данного руководства рекомендуется использовать оператор `LIMIT` для вставки меньшего объема данных,
...
Редактор запросов SQL-консоли автоматически обнаруживает выражения параметров запросов ClickHouse и предоставляет поле ввода для каждого параметра.
...
Параметры запроса можно указать с помощью синтаксиса `{parameter_name: type}`. Эти параметры будут автоматически обнаружены, и пример полезной нагрузки запроса будет содержать объект `queryVariables`, через который можно передавать эти параметры.

### Тестирование и мониторинг {#testing-and-monitoring}

После создания API-эндпоинта запроса можно проверить его работу с помощью `curl` или любого другого HTTP-клиента:
...
Эта конечная точка выполняет запросы к сохранённым конечным точкам Query API.
Она поддерживает несколько версий, гибкие форматы ответов, параметризованные запросы и опциональные потоковые ответы (только версия 2).
...
**Права доступа:** Соответствующие права доступа для конечной точки запроса
...
| ---------------- | ------ | ----------- | ------------------------------ |
| `queryVariables` | object | Нет         | Переменные для использования в запросе |
...
| ------------------ | -------------------------------------------------- |
| `400 Bad Request`  | Запрос сформирован неправильно                     |
| `401 Unauthorized` | Отсутствует аутентификация или недостаточно прав доступа |
...
Это обеспечивает доступ к возможностям версии 2, включая:
...
### Запрос с переменными и версией 2 в формате JSONCompactEachRow {#request-with-query-variables-and-version-2-on-jsoncompacteachrow-format}
