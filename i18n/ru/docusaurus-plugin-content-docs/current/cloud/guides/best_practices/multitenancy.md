...
sidebar_label: 'Многопользовательская архитектура'
title: 'Многопользовательская архитектура'
description: 'Рекомендации по реализации многопользовательской архитектуры'
doc_type: 'guide'
keywords: ['multitenancy', 'изоляция', 'рекомендации', 'архитектура', 'многопользовательская']
---

На платформе аналитики данных в модели SaaS обычно несколько клиентов (тенантов), таких как организации, заказчики или бизнес‑подразделения, используют одну и ту же инфраструктуру базы данных, при этом сохраняя логическую изоляцию своих данных. Это позволяет разным пользователям безопасно получать доступ к своим данным в рамках одной платформы.

В зависимости от требований существуют разные способы реализации многопользовательской архитектуры. Ниже приведено руководство по их реализации в ClickHouse Cloud.
...
> **Мы рекомендуем этот подход, так как он наиболее прост в управлении, особенно когда все клиенты используют одинаковую схему данных, а объемы данных умеренные (< ТБ)**

Консолидация данных всех клиентов в одной таблице повышает эффективность хранения за счет оптимизированного сжатия данных и снижения накладных расходов на метаданные. Кроме того, обновление схемы упрощается, поскольку все данные управляются централизованно.
...
В случаях, когда существует значительная разница в объеме данных между клиентами, более мелкие клиенты могут испытывать ненужное снижение производительности запросов. Обратите внимание, что эта проблема в значительной степени смягчается включением поля клиента в первичный ключ.
...
```sql
-- Вставка демонстрационных строк
...
Затем с помощью общей роли предоставьте общей таблице права [`GRANT SELECT`](/sql-reference/statements/grant#usage).
...
При таком подходе данные каждого клиента хранятся в отдельной таблице в рамках одной базы данных, что устраняет необходимость в специальном поле для идентификации клиентов. Доступ пользователей контролируется с помощью [оператора GRANT](/sql-reference/statements/grant), что гарантирует доступ каждого пользователя только к таблицам, содержащим данные его клиентов.

> **Использование отдельных таблиц — хороший выбор, когда у клиентов различаются схемы данных.**

Для сценариев с небольшим количеством клиентов, имеющих очень большие наборы данных, где критична производительность запросов, этот подход может превосходить модель с общей таблицей. Поскольку нет необходимости фильтровать данные других клиентов, запросы выполняются более эффективно. Кроме того, первичные ключи можно дополнительно оптимизировать, так как не требуется включать в них дополнительное поле (например, идентификатор клиента).

Обратите внимание, что этот подход не масштабируется на тысячи клиентов. См. [ограничения использования](/cloud/bestpractices/usage-limits).
...
```sql
-- Создание таблицы для клиента 1
...
-- Создание таблицы для клиента 2
...
> **Этот менее распространённый метод может быть решением, если данные тенантов должны храниться в разных регионах — по юридическим причинам, соображениям безопасности или территориальной близости.**
