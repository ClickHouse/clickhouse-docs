...
На SaaS‑платформе аналитики данных распространён сценарий, когда несколько арендаторов, например организации, заказчики или бизнес‑подразделения, используют одну и ту же инфраструктуру базы данных, при этом сохраняя логическое разделение своих данных. Это позволяет разным пользователям безопасно получать доступ к собственным данным в рамках одной платформы.

В зависимости от требований мультиарендность можно реализовать по‑разному. Ниже приведено руководство по реализации таких вариантов с помощью ClickHouse Cloud.

## Общая таблица  \\{#shared-table\\}

В этом подходе данные всех арендаторов хранятся в одной общей таблице, при этом поле (или набор полей) используется для идентификации данных каждого арендатора. Для максимальной производительности это поле должно быть включено в [первичный ключ](/sql-reference/statements/create/table#primary-key). Чтобы гарантировать доступ только к данным соответствующих арендаторов, используется [управление доступом на основе ролей](/operations/access-rights), реализованное с помощью [политик на уровне строк](/operations/access-rights#row-policy-management).

> **Мы рекомендуем этот подход, поскольку им проще всего управлять, особенно когда все арендаторы используют одну и ту же схему данных, а объемы данных умеренные (< ТБ)**

Объединяя все данные арендаторов в одной таблице, вы повышаете эффективность хранения за счет оптимизированного сжатия данных и снижения накладных расходов на метаданные. Кроме того, обновление схемы упрощается, поскольку все данные централизованно управляются.

Этот метод особенно эффективен при работе с большим количеством арендаторов (потенциально миллионами).

Однако альтернативные подходы могут быть более подходящими, если у арендаторов разные схемы данных или ожидается, что со временем они будут существенно различаться.

В случаях, когда существует значительная разница в объеме данных между арендаторами, меньшие арендаторы могут испытывать ненужное ухудшение производительности запросов. Обратите внимание, что эта проблема в значительной степени смягчается за счет включения поля арендатора в первичный ключ.

### Пример \{#shared-table-example\}

Это пример реализации многоклиентной модели с общей таблицей.
...
Мы [создаём политики строк](/sql-reference/statements/create/row-policy), так что `user_1` и `user_2` могут получать доступ только к данным своих арендаторов.
...
Затем, используя общую роль, предоставьте привилегии [`GRANT SELECT`](/sql-reference/statements/grant#usage) на общую таблицу.
...
Теперь вы можете подключиться под пользователем `user_1` и выполнить простой SELECT. Будут возвращены только строки первого арендатора.
...
В этом подходе данные каждого арендатора хранятся в отдельной таблице в рамках одной базы данных, что устраняет необходимость в специальном поле для идентификации арендаторов. Доступ пользователей обеспечивается с использованием [команды GRANT](/sql-reference/statements/grant) и гарантирует, что каждый пользователь может получить доступ только к таблицам, содержащим данные его арендаторов.

> **Использование отдельных таблиц — хороший выбор, когда у арендаторов разные схемы данных.**

В случаях с небольшим количеством арендаторов, но с очень большими наборами данных, где критична производительность запросов, этот подход может превосходить модель общей таблицы. Поскольку нет необходимости отфильтровывать данные других арендаторов, запросы могут быть более эффективными. Кроме того, первичные ключи можно дополнительно оптимизировать, так как нет необходимости включать дополнительное поле (например, идентификатор арендатора) в первичный ключ. 
...
> **Этот подход полезен, если каждому арендатору требуется большое количество таблиц и, возможно, materialized view, а также своя схема данных. Однако управление таким решением может стать сложным при большом количестве арендаторов.**

Реализация схожа с подходом с отдельными таблицами, но вместо выдачи привилегий на уровне таблиц привилегии выдаются на уровне баз данных.

Обратите внимание, что этот подход не масштабируется на тысячи арендаторов. См. раздел [ограничения использования](/cloud/bestpractices/usage-limits).

### Пример \{#separate-databases-example\}

Это пример реализации многотенантной модели с отдельными базами данных.
...
Теперь вы можете подключиться под пользователем `user_1` и выполнить простой SELECT из таблицы `events` соответствующей базы данных. Будут возвращены только строки первого арендатора.
...
Три подхода, описанные выше, также могут быть дополнительно изолированы с помощью [Warehouses](/cloud/reference/warehouses#what-is-a-warehouse). Данные хранятся в общем объектном хранилище, но каждый арендатор может иметь собственный вычислительный сервис благодаря [compute-compute separation](/cloud/reference/warehouses#what-is-compute-compute-separation) с разным соотношением CPU/Memory. 

Управление пользователями аналогично подходам, описанным ранее, поскольку все сервисы в одном Warehouse [используют единые настройки доступа](/cloud/reference/warehouses#database-credentials). 

Обратите внимание, что количество дочерних сервисов в одном Warehouse ограничено небольшим числом. См. [ограничения Warehouse](/cloud/reference/warehouses#limitations).
...
> **Этот менее распространённый метод может быть решением, если данные арендаторов необходимо хранить в разных регионах — по юридическим соображениям, требованиям безопасности или из соображений близости.**

Для каждого сервиса, на котором пользователь будет получать доступ к данным соответствующего арендатора, должна быть создана учётная запись пользователя.

Такой подход сложнее в администрировании и влечёт накладные расходы на каждый сервис, поскольку каждому из них требуется собственная инфраструктура для работы. Сервисами можно управлять через [ClickHouse Cloud API](/cloud/manage/api/api-overview), а оркестрация также возможна через [официальный Terraform-провайдер](https://registry.terraform.io/providers/ClickHouse/clickhouse/latest/docs).

### Пример \{#separate-service-example\}

Это пример реализации модели мультиарендности с отдельным сервисом для каждого арендатора. Обратите внимание, что в примере показано создание таблиц и пользователей на одном сервисе ClickHouse; то же самое потребуется настроить на всех сервисах.
...
Затем выдайте привилегию `SELECT` на соответствующую таблицу с помощью команды `GRANT`.
...
Теперь вы можете подключиться под пользователем `user_1` к сервису для первого арендатора и выполнить простой запрос SELECT. Будут возвращены только строки из первого арендатора.
