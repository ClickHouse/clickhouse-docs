...
Данное руководство описывает два способа управления пользователями базы данных: через SQL-консоль и непосредственно в базе данных.

### Аутентификация в SQL-консоли без пароля {#sql-console-passwordless-authentication}

Пользователи SQL-консоли создаются для каждой сессии и проходят аутентификацию с использованием сертификатов X.509, которые автоматически ротируются. Пользователь удаляется при завершении сессии. При формировании списков доступа для аудита перейдите на вкладку «Настройки» (Settings) для сервиса в консоли и обратите внимание на доступ к SQL-консоли в дополнение к пользователям базы данных, существующим в базе данных. Если настроены пользовательские роли, доступ пользователя указывается в роли, имя которой заканчивается именем пользователя.


## Пользователи и роли SQL-консоли {#sql-console-users-and-roles}

Базовые роли SQL-консоли могут быть назначены пользователям с разрешениями Service Read Only и Service Admin. Дополнительную информацию см. в разделе [Управление назначением ролей SQL-консоли](/cloud/guides/sql-console/manage-sql-console-role-assignments). В данном руководстве показано, как создать пользовательскую роль для пользователя SQL-консоли.
...
Пользователю будет назначена роль, связанная с его адресом электронной почты, каждый раз при использовании SQL-консоли.
...
Поскольку пользователи без административных привилегий не могут самостоятельно установить свой пароль, попросите пользователя хешировать пароль с помощью генератора,
...
### Пользователь базы данных с аутентификацией через защищенную оболочку (SSH) {#database-ssh}

Для настройки SSH-аутентификации пользователя базы данных ClickHouse Cloud:
...
- Роли базы данных являются аддитивными. Это означает, что если пользователь является членом двух ролей, он получает максимальный доступ, предоставленный этим двум ролям. Пользователи не теряют доступ при добавлении ролей.
...
#### Получение списка всех разрешений базы данных {#get-a-list-of-all-database-grants}

Выполните следующие запросы, чтобы получить список всех разрешений в базе данных.
...
#### Сопоставление списка разрешений с пользователями консоли, имеющими доступ к SQL-консоли {#associate-grant-list-to-console-users-with-access-to-sql-console}
...
Пользователи хранилища являются общими для всех сервисов в рамках одного хранилища. Дополнительную информацию см. в разделе [управление доступом к хранилищу](/cloud/reference/warehouses#access-controls).
