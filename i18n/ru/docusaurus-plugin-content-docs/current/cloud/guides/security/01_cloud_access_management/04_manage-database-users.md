...
Это руководство описывает два способа управления пользователями базы данных: через SQL-консоль и непосредственно в базе данных.
...
#### Создание роли `database_developer` и предоставление прав {#create-role-grant-permissions}

Создайте роль `database_developer` и предоставьте права `SHOW`, `CREATE`, `ALTER` и `DELETE`.
...
### Пользователь базы данных с аутентификацией через SSH {#database-ssh}
...
Пользователи могут использовать генератор хеша SHA256 или функцию кода, такую как `hashlib` в Python, для преобразования пароля длиной не менее 12 символов с соответствующей сложностью в строку SHA256, которую можно предоставить системному администратору в качестве пароля. Это гарантирует, что администратор не видит и не обрабатывает пароли в открытом виде.
