...
В настоящее время мы поддерживаем SSO, инициируемый поставщиком услуг, несколько организаций с использованием отдельных соединений и JIT-подготовку пользователей. Мы пока не поддерживаем System for Cross-domain Identity Management (SCIM) или сопоставление атрибутов.
...
<VerticalStepper headerLevel="h3">
  ...
  * [Настройка Duo SAML](#configure-duo-saml)
  ...
  После завершения настройки SAML вы можете задать роль или роли по умолчанию, которые будут назначаться всем пользователям при входе, а также настроить параметры тайм-аута сеанса. Список доступных системных ролей, которые можно назначить, см. в разделе [Console roles and permissions](/cloud/security/console-roles).
...
<details>
   ...
   4. Введите имя приложения, установите флажок **Don't display application icon to users** и нажмите **Next**. 
...
### Мультиорганизационное SSO \{#configure-your-admin-user\}
...
:::note
Если вы не хотите, чтобы пользователи с доменом вашей компании перенаправлялись в организацию при вводе адреса электронной почты на https://console.clickhouse.cloud, отправьте запрос в службу поддержки, чтобы вручную обновить настройки SSO и отключить это поведение.
:::
