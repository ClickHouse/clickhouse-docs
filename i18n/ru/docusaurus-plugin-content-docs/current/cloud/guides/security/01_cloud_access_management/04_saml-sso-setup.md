...
ClickHouse Cloud поддерживает единый вход (SSO) с использованием Security Assertion Markup Language (SAML). Это позволяет безопасно входить в вашу организацию ClickHouse Cloud, проходя аутентификацию у вашего поставщика удостоверений (IdP).

В настоящее время поддерживаются инициируемый поставщиком услуг (service provider-initiated) SSO, несколько организаций с отдельными подключениями и подготовка учетных записей «по требованию» (just-in-time provisioning). Пока что не поддерживаются система управления идентификацией между доменами (SCIM) и сопоставление атрибутов.
...
Мы рекомендуем настроить **прямую ссылку на вашу организацию** в дополнение к SAML-подключению для упрощения процесса входа. Каждый IdP обрабатывает это по-своему. Ниже описано, как это сделать для вашего IdP.


## Как настроить IdP {#how-to-configure-your-idp}
...
<details> 
   ...
   ClickHouse использует SAML-соединения, инициируемые поставщиком услуг (service provider). Это означает, что вы можете войти через https://console.clickhouse.cloud или по прямой ссылке. В настоящее время мы не поддерживаем соединения, инициируемые поставщиком идентификации (identity provider). Базовая конфигурация SAML включает следующие параметры:
...
<details>
   ...
   8. Мы завершим настройку в ClickHouse Cloud и сообщим, когда она будет готова к тестированию.
...
<details>
  <summary>
    {" "}
    2. Создайте приложение-закладку для беспрепятственного входа пользователей{" "}
  </summary>
  ...
  **Bookmark App**. 4. Нажмите **Add integration**. 5. Выберите метку для приложения.
  6. Введите URL: `https://console.clickhouse.cloud/?connection=
...
<details>
   ...
   7. Введите следующий Attribute Statement.
...
<Image img={samlGoogleApp} size='md' alt='Google SAML-приложение' force />
...
14. Чтобы включить приложение, нажмите **OFF** для всех и измените параметр на **ON** для всех. Доступ также можно ограничить группами или организационными единицами, выбрав соответствующие параметры в левой части экрана.
...
<details>
   ...
   Вы настроите одну интеграцию приложения с отдельным URL-адресом для входа для каждой организации.
   
   1. Войдите в центр администрирования Microsoft Entra.
   
   2. В левой части меню перейдите в **Applications > Enterprise applications**.
   ...
   9. Используйте следующие настройки, чтобы заполнить экран Basic SAML Configuration.
   ...
   11. Добавьте (A) или обновите (U) следующие элементы в разделе Attributes & Claims:
...
<details>
   ...
   1. Следуйте инструкциям в разделе [Duo Single Sign-On for Generic SAML Service Providers](https://duo.com/docs/sso-generic). 
   
   2. Используйте следующую схему сопоставления Bridge Attribute:

      |  Bridge Attribute  |  Атрибут ClickHouse    |
      |:-------------------|:-----------------------|
      | Email Address      | email                  |

3.  Используйте следующие значения, чтобы обновить облачное приложение в Duo:
...
### Управление пользователями с SAML SSO {#user-management-with-saml-sso}

Дополнительную информацию об управлении правами пользователей и ограничении доступа только к SAML-подключениям см. в разделе [Управление пользователями облака](/cloud/security/manage-cloud-users).

### SSO, инициируемый поставщиком сервиса {#service-provider-initiated-sso}

Мы используем только SSO, инициируемый поставщиком сервиса. Это означает, что пользователи переходят на `https://console.clickhouse.cloud` и вводят свой адрес электронной почты, после чего перенаправляются к IdP для аутентификации. Пользователи, уже прошедшие аутентификацию через ваш IdP, могут использовать прямую ссылку для автоматического входа в организацию без ввода адреса электронной почты на странице входа.

### SSO для нескольких организаций {#multi-org-sso}

ClickHouse Cloud поддерживает SSO для нескольких организаций, предоставляя отдельное подключение для каждой из них. Используйте прямую ссылку (`https://console.clickhouse.cloud/?connection={organizationid}`) для входа в соответствующую организацию. Не забудьте выйти из одной организации перед входом в другую.
...
- **Мы обрабатываем только потоки аутентификации, инициированные поставщиком услуг.** Пользователи должны перейти на `https://console.clickhouse.cloud` и ввести адрес электронной почты, чтобы быть перенаправленными к вашему провайдеру идентификации. Для вашего удобства предоставлены инструкции по добавлению закладки или ярлыка, чтобы вашим пользователям не требовалось запоминать URL.
...
## Устранение распространенных проблем {#troubleshooting-common-issues}
...
| Вас перенаправляет к провайдеру идентификации, а затем обратно на страницу входа                                                                                            | У провайдера идентификации отсутствует сопоставление атрибута email                       | Следуйте инструкциям для вашего провайдера идентификации выше, чтобы настроить атрибут email пользователя и войти снова                                                                                                                |
...
| У вас несколько организаций ClickHouse, интегрированных с SAML SSO, и вы всегда входите в одну и ту же организацию, независимо от того, какую ссылку или плитку вы используете | Вы все еще авторизованы в первой организации                                     | Выйдите из системы, затем войдите в другую организацию                                                                                                                                                                                 |
| В URL кратковременно отображается `access denied`                                                                                                                              | Ваш домен email не совпадает с настроенным доменом                        | Обратитесь в службу поддержки за помощью в устранении этой ошибки                                                                                                                                                                                       |
