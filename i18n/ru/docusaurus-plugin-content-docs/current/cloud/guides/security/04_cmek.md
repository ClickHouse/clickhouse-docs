...
Данные в состоянии покоя по умолчанию шифруются с использованием ключей AES 256, управляемых облачным провайдером. Клиенты могут включить прозрачное шифрование данных (TDE) для обеспечения дополнительного уровня защиты данных сервиса или предоставить собственный ключ для реализации шифрования с использованием ключей, управляемых клиентом (CMEK).
...
4. Откройте выпадающий список функций Enterprise и включите опцию «Включить прозрачное шифрование данных (TDE)»
...
3. В нижней части экрана разверните раздел «Информация о сетевой безопасности»
4. Скопируйте идентификатор роли шифрования (AWS) или учетную запись службы шифрования (GCP) — это понадобится на следующем шаге
...
2. Нажмите на «Настройки» слева
3. В нижней части экрана разверните раздел «Информация о сетевой безопасности»
4. Скопируйте учетную запись службы шифрования (GCP) — это понадобится на следующем шаге
...
10. Вернитесь в ClickHouse Cloud и вставьте путь к ресурсу ключа в раздел «Прозрачное шифрование данных» в настройках сервиса
...
После настройки CMEK выполните ротацию ключа, следуя приведенным выше процедурам создания нового ключа KMS и предоставления разрешений. Вернитесь в настройки сервиса, чтобы вставить новый ARN (AWS) или путь к ресурсу ключа (GCP), и сохраните настройки. Сервис перезапустится для применения нового ключа.
