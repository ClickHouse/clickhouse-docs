...
doc_type: 'guide'
keywords: ['byoc', 'безопасность', 'playbook', 'лучшие практики', 'соответствие требованиям']
---
...
ClickHouse предоставляет решение Bring Your Own Cloud (BYOC) в рамках модели разделённой ответственности за безопасность, которую можно скачать из нашего Центра доверия по адресу https://trust.clickhouse.com. Приведённая ниже информация предназначена для клиентов BYOC в качестве примеров выявления потенциальных инцидентов безопасности. Клиентам рекомендуется рассматривать эту информацию в контексте своей программы безопасности для определения необходимости дополнительных средств обнаружения и оповещения.
...
ClickHouse использует предварительно созданные роли для выполнения системных функций. В данном разделе предполагается, что клиент использует AWS с CloudTrail и имеет доступ к журналам CloudTrail.
...
ClickHouse BYOC работает внутри EKS. В этом разделе предполагается, что клиент использует CloudTrail и CloudWatch в AWS и имеет доступ к журналам.

Если инцидент может быть результатом компрометации кластера EKS, используйте приведенные ниже запросы к журналам EKS CloudWatch для выявления конкретных угроз.
