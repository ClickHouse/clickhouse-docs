...
Таблица [system.session_log](/operations/system-tables/session_log) регистрирует успешные и неудачные попытки входа в систему, а также местоположение попытки аутентификации. Эта информация может использоваться для выявления атак с подбором учётных данных (credential stuffing) или атак методом перебора (brute force) на экземпляр ClickHouse.
