...
Информация записывается непосредственно в системные таблицы и по умолчанию хранится до 30 дней. Этот период может быть больше или меньше в зависимости от частоты слияний в системе. Пользователи могут принять дополнительные меры для более длительного хранения журналов или экспортировать журналы в систему управления информацией о безопасности и событиями (SIEM) для долгосрочного хранения. Подробности ниже.
:::


## Журналы событий безопасности {#security-relevant-logs}

ClickHouse регистрирует события базы данных, связанные с безопасностью, преимущественно в журналах сессий и запросов.

Таблица [system.session_log](/operations/system-tables/session_log) регистрирует успешные и неудачные попытки входа в систему, а также местоположение попытки аутентификации. Эта информация может использоваться для выявления атак с использованием украденных учетных данных или атак методом перебора против экземпляра ClickHouse.

Пример запроса, показывающего неудачные попытки входа в систему
...
## Хранение данных журналов внутри сервисов {#reatining-log-data-within-services}

Пользователи, которым требуется более длительное хранение или повышенная надёжность журналов, могут использовать материализованные представления для достижения этих целей. Подробнее о материализованных представлениях, их назначении, преимуществах и способах реализации см. в наших видео и документации по [материализованным представлениям](/materialized-views).


## Экспорт логов {#exporting-logs}

Системные логи можно записывать или экспортировать в хранилище, используя различные форматы, совместимые с SIEM-системами. Подробнее см. в документации по [табличным функциям](/sql-reference/table-functions). Наиболее распространённые методы:
