...
Маскирование данных — это метод защиты, при котором исходные данные заменяются таким образом, что сохраняются их формат и структура, но удаляются персональные данные (PII) и другая конфиденциальная информация.

В этом руководстве описано, как выполнять маскирование данных в ClickHouse.



## Использование функций замены строк {#using-string-functions}

Для базовых задач маскирования данных семейство функций `replace` предоставляет удобный способ маскировки:
...
Также можно замаскировать номер социального страхования, оставив только последние 4 цифры, используя функцию `replaceRegexpAll`.
...
В секции `SELECT` приведенного выше запроса создания представления мы определяем преобразования с использованием функции `replaceRegexpOne` для полей `name`, `email`, `phone` и `shipping_address`, которые содержат конфиденциальную информацию, которую мы хотим частично маскировать.
...
Вы также можете создать несколько представлений с разной степенью маскирования в зависимости от уровня привилегированного доступа к информации у пользователя, просматривающего данные.

Чтобы пользователи могли получать доступ только к представлению, возвращающему замаскированные данные, а не к таблице с исходными немаскированными данными, следует использовать [Role Based Access Control](/cloud/security/console-roles), чтобы гарантировать, что конкретные роли имеют права только на выборку из представления.
...
Поскольку роли в ClickHouse являются накопительными, вы должны убедиться, что пользователи, которые должны видеть только маскированное представление, не имеют привилегий `SELECT` на базовую таблицу ни через одну из ролей.
...
только замаскированные данные из представления, а не исходные незамаскированные данные из таблицы.


## Использование колонок `MATERIALIZED` и ограничений доступа на уровне колонок {#materialized-ephemeral-column-restrictions}

В случаях, когда вы не хотите создавать отдельное представление, вы можете хранить маскированные версии данных вместе с исходными данными.
...
Возьмем предыдущий пример: вместо создания отдельного представления `VIEW` для маскированных данных мы создадим маскированные колонки с помощью `MATERIALIZED`:
...
Если вы выполните следующий запрос select, вы увидите, что маскированные данные материализуются во время вставки и хранятся вместе с исходными, немаскированными данными.
...
Чтобы гарантировать, что пользователи могут получать доступ только к столбцам с маскированными данными, вы снова можете использовать [Role Based Access Control](/cloud/security/console-roles), чтобы определённые роли имели права только на выборку маскированных столбцов из `orders`.
...
Если вам нужно хранить в таблице `orders` только маскированные данные,
вы можете пометить чувствительные немаскированные столбцы как [`EPHEMERAL`](/sql-reference/statements/create/table#ephemeral),
что гарантирует, что столбцы этого типа не будут физически записываться в таблицу.
...
Если мы запустим тот же запрос, что и раньше, вы увидите, что в таблицу были вставлены только материализованные данные с маскированием:
...
Пользователи ClickHouse OSS, которые хотят маскировать данные в журналах, могут использовать [правила маскирования запросов](/operations/server-configuration-parameters/settings#query_masking_rules) (маскирование журналов).
