...
Маскировка данных — это метод защиты, при котором исходные данные заменяются их вариантом, сохраняющим формат и структуру, но не содержащим персональные данные (PII) или другую чувствительную информацию.

В этом руководстве показано, как выполнять маскировку данных в ClickHouse.



## Использование функций замены строк {#using-string-functions}

Для простых сценариев маскирования семейство функций `replace` предоставляет удобный способ скрытия данных:

| Функция                                                                                  | Описание                                                                                                                                               |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`replaceOne`](/sql-reference/functions/string-replace-functions#replaceOne)             | Заменяет первое вхождение шаблона в исходной строке указанной строкой-заменой.                                                                        |
| [`replaceAll`](/sql-reference/functions/string-replace-functions#replaceAll)             | Заменяет все вхождения шаблона в исходной строке указанной строкой-заменой.                                                                           |
| [`replaceRegexpOne`](/sql-reference/functions/string-replace-functions#replaceRegexpOne) | Заменяет первое вхождение подстроки, соответствующей шаблону регулярного выражения (в синтаксисе re2), в исходной строке указанной строкой-заменой.   |
| [`replaceRegexpAll`](/sql-reference/functions/string-replace-functions#replaceRegexpAll) | Заменяет все вхождения подстрок, соответствующих шаблону регулярного выражения (в синтаксисе re2), в исходной строке указанной строкой-заменой.       |

Например, вы можете заменить имя «John Smith» шаблоном `[CUSTOMER_NAME]`, используя функцию `replaceOne`:
...
Более универсально, вы можете использовать `replaceRegexpOne`, чтобы заменить любое имя клиента:
...
Или вы можете замаскировать номер социального страхования, оставив только последние 4 цифры с помощью функции `replaceRegexpAll`.
...
В приведённом выше запросе `\3` используется для подстановки третьей захваченной группы в результирующую строку, что даёт:
...
Мы хотим, чтобы группа сотрудников могла просматривать информацию, но при этом не видела полные данные клиентов.
...
В секции `SELECT` приведенного выше запроса создания представления мы определяем преобразования с помощью функции `replaceRegexpOne` для полей `name`, `email`, `phone` и `shipping_address`, которые содержат конфиденциальную информацию, требующую частичного маскирования.
...
Обратите внимание, что данные, возвращаемые представлением, частично маскированы, закрывая конфиденциальную информацию.
Вы также можете создать несколько представлений с разной степенью маскирования в зависимости от уровня привилегированного доступа к информации у пользователя, который просматривает данные.

Чтобы пользователи могли получать доступ только к представлению, возвращающему замаскированные данные, а не к таблице с исходными незамаскированными данными, следует использовать [Role Based Access Control](/cloud/security/console-roles), чтобы обеспечить, что определённые роли имеют права только на выборку из представления.
...
Затем предоставьте роли право `SELECT` на представление:

```sql
GRANT SELECT ON masked_orders TO masked_orders_viewer;
```

Поскольку роли в ClickHouse являются аддитивными, вы должны убедиться, что пользователи, которые должны видеть только маскированное представление, не имеют никаких привилегий `SELECT` на базовую таблицу ни через одну из ролей.

Поэтому, чтобы избежать риска, следует явно отозвать доступ к базовой таблице:
...
только маскированные данные из представления, а не исходные немаскированные данные из таблицы.
...
Возьмем предыдущий пример: вместо создания отдельного `VIEW` для маскированных данных создадим маскированные колонки с помощью `MATERIALIZED`:
...
Чтобы гарантировать, что пользователи могут получать доступ только к столбцам, содержащим маскированные данные, вы можете снова использовать [Role Based Access Control](/cloud/security/console-roles), чтобы обеспечить, что определённые роли имеют права только на выборку (`SELECT`) маскированных столбцов из `orders`.

Создайте заново роль, которую мы создавали ранее:
...
Далее предоставьте право `SELECT` на таблицу `orders`:

```sql
GRANT SELECT ON orders TO masked_data_reader;
```

Отзовите доступ ко всем конфиденциальным столбцам:
...
вы можете пометить чувствительные незамаскированные столбцы как [`EPHEMERAL`](/sql-reference/statements/create/table#ephemeral),
...
Если запустить тот же запрос, что и раньше, вы увидите, что в таблицу были вставлены только материализованные маскированные данные:
...
## Использование правил маскирования запросов для данных в логах {#use-query-masking-rules}

Пользователи ClickHouse OSS, которые хотят маскировать данные в логах, могут использовать [правила маскирования запросов](/operations/server-configuration-parameters/settings#query_masking_rules) (маскирование логов).

Для этого можно определить правила маскирования на основе регулярных выражений в конфигурации сервера.
Эти правила применяются к запросам и всем сообщениям в логах перед их сохранением в логи сервера или системные таблицы (такие как `system.query_log`, `system.text_log` и `system.processes`).

Это помогает предотвратить утечку конфиденциальных данных только в **логи**.
Обратите внимание, что маскирование не применяется к данным в результатах запросов.
