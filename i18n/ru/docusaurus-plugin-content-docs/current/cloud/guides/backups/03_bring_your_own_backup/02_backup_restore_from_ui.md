---
sidebar_label: 'Резервное копирование и восстановление в интерфейсе'
slug: /cloud/manage/backups/backup-restore-via-ui
title: 'Создание и восстановление резервной копии через пользовательский интерфейс'
description: 'Страница, описывающая, как создать резервную копию или восстановить её через пользовательский интерфейс с использованием собственного бакета'
sidebar_position: 2
doc_type: 'guide'
keywords: ['резервные копии', 'аварийное восстановление', 'защита данных', 'восстановление', 'облачные возможности']
---
...
Для этого создайте политику разрешений для роли с JSON, аналогичным приведенному ниже, где вы подставляете ARN вашей корзины в качестве ресурса в обоих местах.
...
##### Настройте внешнюю резервную копию {#configure-external-bucket}

На странице «Настройки» нажмите «Настроить внешнюю резервную копию»:

<Image img={change_external_backup} alt='Настройка внешней резервной копии' size='lg' />
...
<Image
  ...
  alt='Настройка ARN роли AWS IAM и параметров корзины S3'
  size='lg'
/>

##### Сохраните изменения {#save-changes}

Нажмите «Сохранить внешнюю корзину», чтобы сохранить настройки
...
##### Просмотр резервных копий в вашей корзине {#view-backups-stored-in-your-bucket}

На странице «Резервные копии» эти резервные копии из вашей корзины будут отображаться в отдельной таблице,
как показано ниже:

<Image img={view_backups} alt='Просмотр резервных копий, хранящихся в вашей корзине' size='lg' />
...
#### Шаги в GCP {#gcp-steps-to-follow}
...
##### Сгенерируйте ключ HMAC и секрет {#generate-an-hmac-key-and-secret}

Сгенерируйте ключ HMAC и секрет, которые необходимы для аутентификации на основе пароля. Выполните следующие шаги для генерации ключей:
...
- a. Создайте сервисную учетную запись
  ...
  - IV. Нажмите `Done`, чтобы завершить создание сервисной учетной записи.
...
#### Шаги в ClickHouse Cloud {#gcp-cloud-steps}
...
##### Настройте ключ HMAC и секрет GCP {#gcp-configure-gcp-hmac-key-and-secret}

В диалоговом окне укажите путь к бакету GCP, ключ HMAC и секрет, созданные в предыдущем разделе.
...
Вы можете выбрать нужную резервную копию из выпадающего списка, чтобы получить команду восстановления
для конкретной резервной копии. Вам необходимо добавить ваш секретный ключ доступа
в команду:
...
Это позволяет выполнять восстановление асинхронно, поэтому при потере соединения процесс восстановления продолжит работу.
...
Необходимо отслеживать таблицу `system.backups`, чтобы проверить, завершилось ли восстановление и был ли результат успешным.
:::
...
Создайте учетную запись хранения или выберите существующую учетную запись хранения на портале Azure, где вы хотите хранить резервные копии.

##### Получите строку подключения {#azure-get-connection-string}

- a. В обзоре вашей учетной записи хранения найдите раздел `Security + networking` и нажмите на `Access keys`.
...
На следующем экране укажите строку подключения и имя контейнера для вашей учетной записи хранения Azure, созданной в предыдущем разделе:
...
Внешние резервные копии теперь будут создаваться в вашем хранилище по расписанию по умолчанию. Кроме того, вы можете настроить расписание резервного копирования на странице «Settings». Если настроено пользовательское расписание, оно используется для записи резервных копий в ваше хранилище, а расписание по умолчанию (резервные копии каждые 24 часа) используется для резервных копий в хранилище ClickHouse Cloud.

##### Просмотр резервных копий в вашем хранилище {#azure-view-backups-stored-in-your-bucket}

На странице резервных копий эти резервные копии в вашем хранилище должны отображаться в отдельной таблице, как показано ниже:
...
Нажмите на ссылку `access or restore a backup` над списком резервных копий в интерфейсе, чтобы получить SQL-команду для восстановления резервной копии. Команда должна выглядеть следующим образом, и вы можете выбрать соответствующую резервную копию из выпадающего списка, чтобы получить команду восстановления для этой конкретной резервной копии. Вам потребуется добавить строку подключения вашей учетной записи хранения Azure в команду.

<Image img={restore_backups_azure} alt='Restore backups in Azure' size='md' />

:::warning Перемещение резервных копий в другое расположение
Если вы переместите резервные копии в другое расположение, вам потребуется изменить команду восстановления, чтобы она ссылалась на новое расположение.
:::

:::tip Команда ASYNC
Для команды восстановления вы также можете дополнительно добавить команду `ASYNC` в конце для больших восстановлений.
...
Важно отметить, что команда ASYNC немедленно возвращает статус успеха.
Это не означает, что восстановление было успешным.
Вам потребуется отслеживать таблицу `system.backups`, чтобы проверить, завершилось ли восстановление и было ли оно успешным или неудачным.
:::
