...
Выполните следующие шаги, чтобы восстановить резервные копии из AWS:

<VerticalStepper headerLevel="h5">

##### Создание нового сервиса для восстановления \{#create-new-service-to-restore-to\}
...
<Image img={backup_command} alt="Получение SQL-команды для восстановления резервной копии" size="md" />
...
:::tip Команда ASYNC
Для команды Restore вы также можете при необходимости добавить директиву `ASYNC` в конец команды при больших объемах восстановления.
Это позволяет выполнять восстановление асинхронно, чтобы в случае потери соединения процесс восстановления продолжал выполняться.
Важно отметить, что команда ASYNC немедленно возвращает статус «успешно».
Это не означает, что восстановление действительно завершилось успешно.
Вам потребуется отслеживать таблицу `system.backups`, чтобы узнать, завершилось ли восстановление и было ли оно успешным или завершилось с ошибкой.
:::

##### Выполните SQL-команду для восстановления резервной копии \{#gcp-run-sql-command-to-restore-backup\}

Выполните команду восстановления из SQL-консоли в только что созданном сервисе, чтобы
