---
sidebar_label: 'Резервное копирование и восстановление через интерфейс'
slug: /cloud/manage/backups/backup-restore-via-ui
title: 'Создание резервной копии и восстановление из нее через интерфейс'
description: 'Страница, описывающая, как создать резервную копию или восстановить ее через интерфейс, используя собственный бакет'
sidebar_position: 2
doc_type: 'guide'
...
:::note
Эти шаги аналогичны настройке безопасного доступа к S3, описанной в разделе [«Безопасный доступ к данным S3»](/cloud/data-sources/secure-s3), однако для разрешений роли требуются дополнительные действия
:::
...
AWS использует аутентификацию на основе ролей, поэтому создайте роль IAM, которую сервис ClickHouse Cloud сможет принять для записи в эту корзину.
...
```json
{
  ...
  "Statement": [
    {
      "Sid": "сервис резервного копирования",
      "Effect": "Allow",
      "Principal": {
...
##### Настройка внешнего резервного копирования {#configure-external-bucket}

На странице настроек нажмите «Настроить внешнее резервное копирование»:

<Image img={change_external_backup} alt='Настройка внешнего резервного копирования' size='lg' />
...
Cloud) в политику доверия для IAM-роли. Это аналогично
[второму шагу](#create-iam-role) в разделе шагов AWS выше. Это необходимо,
чтобы новый сервис мог получить доступ к бакету S3.
...
Создайте бакет хранилища в вашем аккаунте GCP для экспорта резервных копий.
...
Альтернативно, вы можете настроить расписание резервного копирования на странице `Settings`.
...
и вы можете выбрать соответствующую резервную копию из выпадающего списка, чтобы получить команду восстановления
для этой конкретной резервной копии. Вам потребуется добавить ваш секретный ключ доступа
в команду:
...
:::warning Перемещение резервных копий в другое расположение
Если вы переместите резервные копии в другое расположение, вам потребуется настроить команду восстановления для указания нового расположения.
:::


:::tip Команда ASYNC
Для команды восстановления можно дополнительно указать команду `ASYNC` в конце при восстановлении больших объемов данных.
Это позволяет выполнять восстановление асинхронно: если соединение будет потеряно, процесс восстановления продолжит работу.
Важно учитывать, что команда ASYNC сразу возвращает статус успешного выполнения.
Это не означает, что восстановление завершилось успешно.
...
<Image img={change_external_backup} alt='Изменение настроек внешнего резервного копирования' size='lg' />
...
<Image
  ...
  alt='Укажите строку подключения и имя контейнера для вашей учетной записи хранилища Azure'
  size='md'
/>
...
<Image
  ...
  alt='Просмотр резервных копий в вашем хранилище'
  size='md'
/>
...
Нажмите на ссылку `access or restore a backup` над списком резервных копий в интерфейсе, чтобы получить SQL-команду для восстановления резервной копии. Команда должна выглядеть следующим образом. Вы можете выбрать нужную резервную копию из выпадающего списка, чтобы получить команду восстановления для этой конкретной резервной копии. Вам потребуется добавить строку подключения вашей учетной записи хранилища Azure в команду.

<Image img={restore_backups_azure} alt='Восстановление резервных копий в Azure' size='md' />
