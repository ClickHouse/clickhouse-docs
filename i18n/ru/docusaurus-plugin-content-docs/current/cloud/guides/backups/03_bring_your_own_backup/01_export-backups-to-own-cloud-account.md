...
:::note
При любом сценарии, когда резервные копии экспортируются в другой регион того же облачного провайдера, будет взиматься плата за [передачу данных](/cloud/manage/network-data-transfer).

Резервное копирование между разными облаками поддерживается только с помощью команд резервного копирования/восстановления, описанных на этой странице, и не поддерживается через UI.
:::
