...
**Cloud NAT Gateway**
Убедитесь, что для VPC развернут [шлюз Cloud NAT](https://cloud.google.com/nat/docs/overview). Компонентам ClickHouse BYOC требуется исходящий доступ в интернет для взаимодействия с плоскостью управления Tailscale. Tailscale используется для обеспечения безопасной сети с моделью нулевого доверия для частных операций управления. Шлюз Cloud NAT предоставляет это исходящее подключение для экземпляров без внешних IP-адресов.
