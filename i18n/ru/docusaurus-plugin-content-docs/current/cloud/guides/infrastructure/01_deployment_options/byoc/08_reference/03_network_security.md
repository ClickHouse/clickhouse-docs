...
* Инженеры должны запрашивать доступ через внутренний процесс утверждения, прежде чем Tailscale сможет маршрутизировать их к конечной точке клиента
* Доступ ограничен по времени и автоматически истекает
* Все действия по доступу аудируются и логируются

Полную политику доступа к данным — что могут видеть инженеры, аутентификацию на основе сертификатов и аудит на стороне клиента — см. в разделе [Доступ ClickHouse к данным](/cloud/reference/byoc/reference/clickhouse_data_access).
...
Более подробную техническую информацию о том, как Tailscale реализован в BYOC, см. в [записи блога Building ClickHouse BYOC on AWS](https://clickhouse.com/blog/building-clickhouse-byoc-on-aws#tailscale-connection). О том, какие данные инженеры ClickHouse могут просматривать после подключения и как ClickHouse аудирует такой доступ, см. в разделе [Доступ ClickHouse к данным](/cloud/reference/byoc/reference/clickhouse_data_access).
...
Инженерам ClickHouse Cloud требуется доступ для устранения неполадок по Tailscale. Им предоставляется аутентификация just-in-time на основе сертификатов для развертываний BYOC. Полную политику доступа см. в разделе [доступ ClickHouse к данным](/cloud/reference/byoc/reference/clickhouse_data_access).
