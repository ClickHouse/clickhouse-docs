...
description: 'В этом руководстве представлен обзор функциональных и операционных аспектов ClickHouse Cloud.'
...
В этом руководстве представлен обзор того, чего ожидать от ClickHouse Cloud в функциональном и эксплуатационном плане. Хотя ClickHouse Cloud основан на дистрибутиве ClickHouse с открытым исходным кодом, возможны некоторые отличия в архитектуре и реализации. В качестве вводного материала может быть полезно и интересно прочитать эту статью в блоге о том, [как мы создали ClickHouse Cloud](https://clickhouse.com/blog/building-clickhouse-cloud-from-scratch-in-a-year).



## Архитектура ClickHouse Cloud {#clickhouse-cloud-architecture}

ClickHouse Cloud значительно упрощает операционные задачи и снижает затраты на эксплуатацию ClickHouse в масштабе. Нет необходимости заранее рассчитывать размер развертывания, настраивать репликацию для обеспечения высокой доступности, вручную шардировать данные, увеличивать мощность серверов при росте нагрузки или уменьшать её при простое — мы делаем это за вас.

Эти преимущества являются результатом архитектурных решений, лежащих в основе ClickHouse Cloud:

- Вычислительные ресурсы и хранилище разделены и могут автоматически масштабироваться независимо друг от друга, поэтому вам не нужно избыточно выделять ресурсы хранилища или вычислений в статических конфигурациях инстансов.
- Многоуровневое хранилище на основе объектного хранилища и многоуровневое кэширование обеспечивают практически неограниченное масштабирование и хорошее соотношение цены и производительности, поэтому вам не нужно заранее рассчитывать размер раздела хранилища и беспокоиться о высоких затратах на хранение данных.
...
- Автоматическое масштабирование для переменных непрерывных рабочих нагрузок включено по умолчанию, поэтому вам не нужно заранее рассчитывать размер сервиса, увеличивать мощность серверов при росте нагрузки или вручную уменьшать её при снижении активности.
...
ClickHouse Cloud предоставляет доступ к отобранному набору возможностей из дистрибутива ClickHouse с открытым исходным кодом. В таблицах ниже описаны некоторые функции, которые в настоящее время отключены в ClickHouse Cloud.
...
- Поддержка `CREATE AS SELECT`, которая в настоящее время недоступна. В качестве обходного решения рекомендуется использовать `CREATE ... EMPTY ... AS SELECT`, а затем вставлять данные в эту таблицу (см. пример в [этой статье блога](https://clickhouse.com/blog/getting-data-into-clickhouse-part-1)).
...
- Не используйте параметры `ON CLUSTER` в ClickHouse Cloud — они не требуются. Хотя в основном это функции без операций, они все равно могут вызвать ошибку при попытке использовать [макросы](/operations/server-configuration-parameters/settings#macros). Макросы часто не работают и не нужны в ClickHouse Cloud.
...
Экспериментальные возможности отключены в сервисах ClickHouse Cloud для обеспечения стабильности развертывания сервисов.
...
Эта настройка для каждого сервера была увеличена со значения по умолчанию `100` до `1000` для обеспечения большей параллельности.
...
Мы добавляем поддержку исполняемых пользовательских функций (UDF) в ClickHouse Cloud и оцениваем востребованность других функций. Если у вас есть отзывы или вы хотите запросить конкретную функцию, пожалуйста, [отправьте запрос здесь](https://console.clickhouse.cloud/support).
