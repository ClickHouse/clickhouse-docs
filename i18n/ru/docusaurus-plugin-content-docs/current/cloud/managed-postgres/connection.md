...
:::tip[SQL Console]
На этой странице описано, как подключить производственные системы к Postgres. Если вам нужен быстрый способ выполнить запрос, вы можете подключиться через [SQL Console](/integrations/sql-clients/sql-console) так же, как и к экземплярам ClickHouse.
:::
