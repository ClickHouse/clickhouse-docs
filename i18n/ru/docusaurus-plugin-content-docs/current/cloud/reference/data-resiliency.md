...
description: 'В этом руководстве представлен обзор механизмов восстановления после аварий.'
doc_type: 'reference'
keywords: ['ClickHouse Cloud', 'отказоустойчивость данных', 'восстановление после аварий']
---
...
ClickHouse Cloud в настоящее время не поддерживает автоматическое переключение при отказе или автоматическую синхронизацию между несколькими географическими регионами.
...
**RPO (Recovery Point Objective, целевая точка восстановления)**: Максимально допустимая потеря данных, измеряемая во времени после сбойного события. Пример: RPO в 30 минут означает, что в случае сбоя база данных должна быть восстановлена до состояния не старше 30 минут. Разумеется, это зависит от частоты создания резервных копий.
...
**Резервные копии и снимки базы данных**: Резервные копии обеспечивают надежное долгосрочное хранение с отдельной копией данных. Снимки не создают дополнительную копию данных, обычно выполняются быстрее и обеспечивают более низкие показатели RPO.


## Резервные копии баз данных {#database-backups}

Наличие резервной копии основного сервиса — это эффективный способ восстановления данных в случае недоступности основного сервиса.
...
В случае повреждения данных в основном сервисе резервную копию можно использовать для восстановления в новый сервис.

2. **Внешние резервные копии (в собственном хранилище клиента)**

Клиенты уровня Enterprise могут [экспортировать резервные копии](/cloud/manage/backups/export-backups-to-own-cloud-account) в своё объектное хранилище в собственной учётной записи — в том же регионе или в другом регионе.
Поддержка экспорта резервных копий между облаками появится в ближайшее время.
К межрегиональным и межоблачным резервным копиям применяются соответствующие тарифы на передачу данных.
...
В ходе наших тестов мы наблюдали, что резервное копирование небольших объёмов данных (~1 ТБ) занимает до 10–15 минут или дольше.
...
При больших объёмах резервное копирование становится более эффективным, и мы наблюдали, что резервное копирование до 1 ПБ для некоторых внутренних сервисов завершается менее чем за 10 часов.
...
Как упоминалось выше, рекомендуем провести тестирование на вашей собственной базе данных, чтобы понять, сколько времени займёт восстановление из резервной копии.
...
Для восстановления из существующей резервной копии:
...
- Перезапуск сервисов приложений для установления новых соединений
...
Существуют дополнительные варианты, которые следует рассмотреть.

1. **Двойная запись в отдельные кластеры**

При использовании этого варианта можно настроить 2 отдельных кластера в разных регионах и выполнять двойную запись в оба.
Этот вариант по своей природе связан с более высокими затратами, поскольку предполагает работу нескольких сервисов, но обеспечивает более высокую доступность в случае недоступности одного из регионов.

2. **Использование репликации облачного провайдера**

При использовании этого варианта используется встроенная репликация объектного хранилища облачного провайдера для копирования данных.
