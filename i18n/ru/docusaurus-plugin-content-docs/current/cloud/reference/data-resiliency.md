...
title: 'План восстановления после аварий'
description: 'В этом руководстве представлен обзор плана восстановления после аварий.'
doc_type: 'reference'
...
Эти резервные копии хранятся в том же регионе, что и сервис, в хранилище облачного провайдера (CSP) ClickHouse.
...
Для межрегиональных и межоблачных резервных копий будут применяться соответствующие тарифы на передачу данных.
...
Время резервного копирования и восстановления зависит от нескольких факторов, таких как размер базы данных, схема и количество таблиц.

По результатам наших тестов резервное копирование небольших объемов данных (~1 ТБ) занимает от 10 до 15 минут или больше.
Резервное копирование объемов менее 20 ТБ обычно завершается в течение часа, а резервное копирование ~50 ТБ данных занимает 2–3 часа.
При больших объемах проявляется эффект масштаба: резервное копирование до 1 ПБ для некоторых внутренних сервисов завершается менее чем за 10 часов.

Рекомендуем провести тестирование на вашей собственной базе данных или тестовых данных для получения более точных оценок, так как фактическое время зависит от нескольких факторов, описанных выше.

Время восстановления сопоставимо со временем резервного копирования для аналогичных объемов.
Как упоминалось выше, рекомендуем провести тестирование на вашей базе данных, чтобы понимать, сколько времени займет восстановление из резервной копии.
...
Рассмотрим несколько дополнительных вариантов.

1. **Двойная запись в отдельные кластеры**

При использовании этого варианта можно настроить два отдельных кластера в разных регионах и выполнять двойную запись в оба кластера.
Этот вариант требует больших затрат, поскольку предполагает работу нескольких сервисов, но обеспечивает более высокую доступность в случае недоступности одного из регионов.
...
Например, при использовании BYOB можно экспортировать резервную копию в собственный бакет в основном регионе и реплицировать её в другой регион с помощью [межрегиональной репликации AWS](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html).
