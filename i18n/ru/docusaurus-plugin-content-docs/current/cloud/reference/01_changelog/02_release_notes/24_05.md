...
Основные изменения в сервисах ClickHouse Cloud на базе релиза v24.5.



## Критические изменения {#breaking-changes}

- Изменено имя столбца с duration_ms на duration_microseconds в таблице system.zookeeper, чтобы отразить фактическое разрешение длительности в микросекундах. [#60774](https://github.com/ClickHouse/ClickHouse/pull/60774) (Duc Canh Le).

- Запрещена установка max_parallel_replicas в 0, так как это не имеет смысла. Установка значения 0 может привести к неожиданным логическим ошибкам. Закрывает #60140. [#61201](https://github.com/ClickHouse/ClickHouse/pull/61201) (Kruglov Pavel).
...
- Использование функций neighbor, runningAccumulate, runningDifferenceStartingWithFirstValue, runningDifference объявлено устаревшим (из-за подверженности ошибкам). Вместо них следует использовать соответствующие оконные функции. Чтобы включить их обратно, установите allow_deprecated_error_prone_window_functions=1. [#63132](https://github.com/ClickHouse/ClickHouse/pull/63132) (Nikita Taranov).
...
- Запросы к system.columns будут выполняться быстрее при большом количестве столбцов, когда для многих баз данных или таблиц отсутствуют права на SHOW TABLES. Обратите внимание, что в предыдущих версиях, если вы предоставляли права SHOW COLUMNS для отдельных столбцов без предоставления SHOW TABLES для соответствующих таблиц, таблица system.columns отображала эти столбцы, но в новой версии она будет полностью пропускать такую таблицу. Удалены трассировочные сообщения журнала «Access granted» и «Access denied», которые замедляли выполнение запросов. [#63439](https://github.com/ClickHouse/ClickHouse/pull/63439) (Alexey Milovidov).

- Исправлено аварийное завершение в функции largestTriangleThreeBuckets. Это изменяет поведение функции: теперь она игнорирует значения NaN в предоставленных рядах данных. Таким образом, результирующий набор может отличаться от результатов в предыдущих версиях. [#62646](https://github.com/ClickHouse/ClickHouse/pull/62646) (Raúl Marín).
...
- Движок таблиц теперь поддерживает выдачу прав доступа, и это не повлияет на поведение существующих пользователей. [#60117](https://github.com/ClickHouse/ClickHouse/pull/60117) (jsc0218).
...
- Поддержка соединений с условиями неравенства, включающими столбцы из левой и правой таблиц, например `t1.y < t2.y`. Для включения используйте `SET allow_experimental_join_condition = 1`. [#60920](https://github.com/ClickHouse/ClickHouse/pull/60920) (lgbo).
...
- Обработка строковых функций XXXUTF8 в ASCII-режиме, если входные строки содержат только ASCII-символы. Вдохновлено apache/doris#29799. Общее ускорение в 1.07x~1.62x раза. Обратите внимание, что пиковое потребление памяти снижено в некоторых случаях. [#61632](https://github.com/ClickHouse/ClickHouse/pull/61632) (李扬).
...
- Улучшение простых INSERT SELECT из файлов в табличных функциях file/s3/hdfs/url/... Добавлена отдельная настройка max_parsing_threads для управления количеством потоков, используемых при параллельном парсинге. [#62404](https://github.com/ClickHouse/ClickHouse/pull/62404) (Kruglov Pavel).
...
- Функции parseDateTimeOrNull, parseDateTimeOrZero, parseDateTimeInJodaSyntaxOrNull и parseDateTimeInJodaSyntaxOrZero теперь работают значительно быстрее (в 10x - 1000x раз), когда входные данные содержат преимущественно непарсируемые значения. [#62634](https://github.com/ClickHouse/ClickHouse/pull/62634) (LiuNeng).
...
- Анализ индексов теперь работает при сравнении DateTime с DateTime64. Закрывает #63441. [#63443](https://github.com/ClickHouse/ClickHouse/pull/63443) (Alexey Milovidov).
...
* Удалена настройка `optimize_monotonous_functions_in_order_by`, так как она превращается в пустую операцию. [#63004](https://github.com/ClickHouse/ClickHouse/pull/63004) (Raúl Marín).

* В `Map` теперь можно использовать `Float32`, `Float64`, `Array(T)`, `Map(K,V)` и `Tuple(T1, T2, ...)` в качестве ключей. Закрывает #54537. [#59318](https://github.com/ClickHouse/ClickHouse/pull/59318) (李扬).
...
* Сделано так, что `RabbitMQ` отправляет `nack` для повреждённых сообщений. Закрывает #45350. [#60312](https://github.com/ClickHouse/ClickHouse/pull/60312) (Kseniia Sumarokova).

* Добавлена настройка `first_day_of_week`, которая влияет на первый день недели, учитываемый функциями `toStartOfInterval(..., INTERVAL ... WEEK)`. Это позволяет добиться согласованности с функцией `toStartOfWeek`, которая по умолчанию использует воскресенье как первый день недели. [#60598](https://github.com/ClickHouse/ClickHouse/pull/60598) (Jordi Villar).

* Добавлен постоянный виртуальный столбец `_block_offset`, который хранит исходный номер строки в блоке, присвоенный при вставке. Постоянство столбца `_block_offset` можно включить настройкой `enable_block_offset_column`. Добавлен виртуальный столбец `_part_data_version`, который содержит либо минимальный номер блока, либо версию модификации части. Постоянный виртуальный столбец `_block_number` больше не считается экспериментальным. [#60676](https://github.com/ClickHouse/ClickHouse/pull/60676) (Anton Popov).

* Функции `date_diff` и `age` теперь вычисляют результат с наносекундной, а не микросекундной точностью. Они также поддерживают наносекунду (`nanosecond`, `nanoseconds` или `ns`) как возможное значение для параметра единицы измерения. [#61409](https://github.com/ClickHouse/ClickHouse/pull/61409) (Austin Kothig).

* Теперь метки не загружаются для широких кусков (`wide parts`) во время слияний. [#61551](https://github.com/ClickHouse/ClickHouse/pull/61551) (Anton Popov).

* `output_format_pretty_row_numbers` включён по умолчанию. Это лучше с точки зрения удобства использования. [#61791](https://github.com/ClickHouse/ClickHouse/pull/61791) (Alexey Milovidov).

* Индикатор прогресса теперь работает для простых запросов с `LIMIT` к `system.zeros`, `system.zeros_mt` (он уже работал для `system.numbers` и `system.numbers_mt`) и табличной функции `generateRandom`. В качестве бонуса, если общее число записей больше лимита `max_rows_to_read`, исключение будет выброшено раньше. Это закрывает #58183. [#61823](https://github.com/ClickHouse/ClickHouse/pull/61823) (Alexey Milovidov).
...
* Исправлена грамматика с «a» на «the» в предупреждающем сообщении. Существует только один движок `Atomic`, поэтому должно быть «to the new Atomic engine», а не «to a new Atomic engine». [#61952](https://github.com/ClickHouse/ClickHouse/pull/61952) (shabroo).
...
* Автоматически выводятся типы столбцов `Nullable` из схемы Apache Arrow. [#61984](https://github.com/ClickHouse/ClickHouse/pull/61984) (Maksim Kita).
...
* OPTIMIZE FINAL для ReplicatedMergeTree теперь будет дожидаться завершения текущих активных слияний и затем повторно пытаться запланировать финальное слияние. Это сделает поведение более близким к обычному MergeTree. [#62067](https://github.com/ClickHouse/ClickHouse/pull/62067) (Nikita Taranov).

* При чтении данных из текстового файла Hive использовалась первая строка файла для изменения количества входных полей, и иногда количество полей в первой строке не совпадало с определением таблицы Hive. Например, таблица Hive определена с 3 столбцами, как test_tbl(a Int32, b Int32, c Int32), но первая строка текстового файла содержит только 2 поля. В этой ситуации количество входных полей будет изменено на 2, и если следующая строка текстового файла содержит 3 поля, то третье поле не будет прочитано, а вместо него будет установлено значение по умолчанию 0, что неверно. [#62086](https://github.com/ClickHouse/ClickHouse/pull/62086) (KevinyhZou).
...
* Добавлена настройка lightweight_deletes_sync (значение по умолчанию: 2 — синхронно ждать все реплики). Аналогична настройке mutations_sync, но влияет только на поведение легковесных удалений. [#62195](https://github.com/ClickHouse/ClickHouse/pull/62195) (Anton Popov).

* Добавлено различение булевых значений и целых чисел при разборе значений для пользовательских настроек: SET custom_a = true; SET custom_b = 1;. [#62206](https://github.com/ClickHouse/ClickHouse/pull/62206) (Vitaly Baranov).
...
* Для функции date_trunc добавлены единицы nano-, micro-, milliseconds. [#62335](https://github.com/ClickHouse/ClickHouse/pull/62335) (Misz606).
...
* Исключено вычисление выражений DEFAULT таблиц при выполнении RESTORE. [#62601](https://github.com/ClickHouse/ClickHouse/pull/62601) (Vitaly Baranov).

* Разрешено использование quota key с другой схемой аутентификации в HTTP-запросах. [#62842](https://github.com/ClickHouse/ClickHouse/pull/62842) (Kseniia Sumarokova).



* Закрывать сессию, если для пользователя наступил срок действия `valid_until`. [#63046](https://github.com/ClickHouse/ClickHouse/pull/63046) (Konstantin Bogdanov).
