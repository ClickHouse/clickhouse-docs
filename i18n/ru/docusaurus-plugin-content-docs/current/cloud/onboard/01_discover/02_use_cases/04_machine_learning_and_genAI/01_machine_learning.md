...
Независимо от того, соответствует ли этот миф действительности, факт остается фактом: данные находятся в центре задач машинного обучения от начала до конца.
Независимо от того, создаете ли вы RAG-конвейеры, выполняете тонкую настройку, обучаете собственную модель или оцениваете производительность модели, данные лежат в основе каждой из этих задач.

Управление данными может быть непростым делом, и как следствие, в этой области наблюдается распространение инструментов, предназначенных для повышения производительности путем решения конкретных аспектов проблем с данными в машинном обучении.
Часто это принимает форму уровня абстракции вокруг более универсального решения с определенным интерфейсом, который на первый взгляд упрощает применение к конкретной подзадаче.
По сути, это снижает гибкость, присущую универсальному решению, в пользу простоты использования и упрощения выполнения конкретной задачи.

<Image img={machine_learning_data_layer} size='sm' />

У этого подхода есть несколько недостатков.
Каскадный набор специализированных инструментов, продуктов и сервисов, в отличие от универсального решения в сочетании с поддерживающим прикладным кодом, создает риск избыточной архитектурной сложности и затрат на данные.
Легко случайно оказаться с бесконечным списком инструментов и сервисов, каждый из которых используется только для одного этапа.

Эти риски имеют два основных аспекта:

1. **Затраты на обучение, обслуживание и переход**

Архитектуры машинного обучения могут стать настолько перегруженными различными инструментами и компонентами, что это создает фрагментированную и сложную среду для изучения и управления, с увеличением точек отказа и постепенным ростом расходов.

2. **Затраты на дублирование и передачу данных**

Использование нескольких отдельных, но пересекающихся систем данных в конвейере машинного обучения может привести к ненужным и часто дорогостоящим накладным расходам на передачу данных между ними.
...
Хотя это может быть правильным выбором в некоторых архитектурах, в других векторная база данных может оказаться ненужным новым дополнением к технологическому стеку, поскольку это еще одна система для интеграции, управления и передачи данных.
Большинство современных универсальных баз данных поддерживают векторы из коробки (или через плагин) и обладают более широкими и комплексными возможностями.
Другими словами, в таких архитектурах может вообще не быть необходимости в отдельной новой базе данных специально для работы с векторами.
Важность сводится к тому, являются ли специфичные для векторов удобные функции (например, встроенные модели эмбеддингов) критически важными и оправдывают ли они свою стоимость.
...
Этот процесс оценки и понимания является итеративным и часто приводит к выполнению серии ad-hoc запросов к наборам данных, где критически важна скорость отклика запросов (наряду с другими факторами, такими как экономическая эффективность и точность).
...
Некоторые крупные игроки значительно увеличивают затраты для сокращения времени выполнения запросов и препятствуют ad-hoc оценке, взимая плату за каждый запрос или по количеству просканированных байтов.
...
Благодаря поддержке типов данных и кодеков с максимальной детализацией точности вам не нужно беспокоиться о снижении детализации ваших данных.
...
Таким образом, инженеры данных могут выполнять преобразования на стороне клиента, а результаты могут материализоваться в виде таблиц признаков в централизованном экземпляре ClickHouse.

### Подготовка данных и извлечение признаков {#data-preparation-and-feature-extraction}

Затем данные подготавливаются: очищаются, преобразуются и используются для извлечения признаков, на основе которых модель будет обучаться и оцениваться.
...
Однако, поскольку это отдельные инструменты от базы данных, с которой они работают, они могут быть ненадежными и вызывать сбои, требующие ручного исправления.
...
Для тех наборов данных, которые удобнее обрабатывать локально, [ClickHouse local](/operations/utilities/clickhouse-local) является отличной альтернативой, наряду с [chDB](/chdb), позволяющей пользователям использовать ClickHouse со стандартными библиотеками данных Python, такими как Pandas.
...
На этом этапе конвейера часто вводится еще один специализированный инструмент в уровень данных машинного обучения — хранилище признаков.
Хранилище признаков чаще всего представляет собой уровень абстракции вокруг базы данных, который предоставляет удобные функции, специфичные для управления данными для обучения, вывода и оценки моделей.
...
Разделяя хранение и вычисления с использованием объектного хранилища, ClickHouse Cloud дополнительно позволяет хранить данные неограниченно долго — с уменьшением вычислительных ресурсов или полным переводом их в режим простоя для минимизации затрат.
...
Полностью распараллеленный движок выполнения запросов в сочетании с колоночным форматом хранения позволяет ClickHouse выполнять агрегации по петабайтам данных за секунды — в отличие от преобразований датафреймов в памяти, пользователи не ограничены объемом памяти.
Кроме того, материализованные представления позволяют преобразовывать данные во время вставки, тем самым перенося вычислительную нагрузку со времени запроса на время загрузки данных.
...
Офлайн-хранилище признаков используется для обучения моделей.
...
Поскольку преобразования часто группируются по идентификатору сущности и возвращают несколько столбцов в качестве результатов, автоматический вывод схемы ClickHouse может определить необходимые типы данных из этих результатов и создать подходящую схему таблицы для их хранения.
...
Это означает, что эти признаки должны вычисляться с минимальной задержкой, поскольку они используются как часть сервиса машинного обучения в режиме реального времени.
...
Важно отметить, что ClickHouse способна обеспечивать такую производительность запросов даже при высоких нагрузках на запись благодаря своей структуре log-structured merge tree.
...
При рассмотрении компромиссов между использованием продукта хранилища признаков на этом этапе и прямым использованием хранилища данных реального времени стоит подчеркнуть, что удобные функции, такие как версионирование, могут быть достигнуты с помощью проверенных временем парадигм баз данных, таких как проектирование таблиц или схем.
Другие функции, такие как преобразование определений признаков в SQL-операторы, могут обеспечить большую гибкость как часть логики приложения или бизнес-логики, а не существовать в виде навязанного слоя абстракции.
...
Основная операция в векторной базе данных — это «поиск по сходству», позволяющий найти векторы, которые являются наиболее «близкими» друг к другу согласно математической метрике.
Векторные базы данных стали популярными благодаря применению специальных методов, направленных на максимальное ускорение этой операции — сравнения векторов.
Эти методы, как правило, подразумевают приближённое сравнение векторов вместо сравнения входного вектора с каждым хранимым вектором.

Проблема этого нового класса инструментов заключается в том, что многие универсальные базы данных, включая ClickHouse, предоставляют поддержку векторов из коробки и часто имеют встроенные реализации этих приближённых подходов.
ClickHouse, в частности, спроектирован для высокопроизводительной крупномасштабной аналитики, что позволяет выполнять точные (неприближённые) сравнения векторов с высокой эффективностью.
Это означает, что вы можете получать точные результаты вместо использования приближений, при этом не жертвуя скоростью.

### Наблюдаемость {#observability}

После запуска вашего приложения машинного обучения в продакшен оно будет генерировать данные, включая логи и данные трассировки, которые предоставляют ценную информацию о поведении модели, производительности и потенциальных областях для улучшения.

Наблюдаемость на основе SQL — это ещё один ключевой сценарий использования ClickHouse, где ClickHouse оказывается в 10–100 раз более экономически эффективным по сравнению с альтернативами.
Фактически, многие продукты для наблюдаемости сами построены на базе ClickHouse.
Благодаря лучшим в своём классе скоростям приёма данных и коэффициентам сжатия ClickHouse обеспечивает экономическую эффективность и высочайшую скорость для обеспечения наблюдаемости машинного обучения в любом масштабе.
