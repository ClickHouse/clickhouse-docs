...
Хотя пользователи могут трансформировать данные непосредственно в ClickHouse или перед вставкой с помощью SQL‑запросов, ClickHouse также может использоваться в программных окружениях, таких как Python, через [chDB](/chdb).
Это позволяет встраиваемому ClickHouse выступать в роли модуля Python и использоваться для трансформации и обработки больших датафреймов в ноутбуках.
Инженеры по данным таким образом могут выполнять преобразования на стороне клиента, при этом результаты потенциально могут материализовываться как таблицы признаков в централизованном экземпляре ClickHouse.
...
Благодаря разделению хранения и вычислений с помощью объектного хранилища, ClickHouse Cloud дополнительно позволяет хранить данные неограниченно долго — с возможностью уменьшать вычислительные ресурсы или полностью останавливать их для минимизации затрат.
...
Как правило, это означает, что сами признаки формируются пакетными конвейерами трансформации данных (как описано в предыдущем разделе), и, как правило, нет жёстких требований по задержке на доступность этих признаков.
...
Vector store — это специализированный тип базы данных, оптимизированный для хранения и выборки векторов, обычно эмбеддингов некоторого фрагмента данных (таких как текст или изображения), которые численно отражают их внутренний смысл.
Векторы лежат в основе нынешней волны генеративного ИИ и используются во множестве приложений.
...
Обзервабилити на основе SQL — ещё один ключевой сценарий использования ClickHouse, в котором ClickHouse, как показала практика, оказывается в 10–100 раз более экономичным по сравнению с альтернативами.
Фактически многие продукты в области наблюдаемости сами построены на ClickHouse «под капотом».
Обладая лучшими в классе скоростями ингестии и коэффициентами сжатия, ClickHouse обеспечивает экономичность и очень высокую скорость для реализации наблюдаемости за системами машинного обучения в масштабах любого уровня.