...
В ClickHouse партиционирование задаётся для таблицы при её создании с помощью выражения [`PARTITION BY`](/engines/table-engines/mergetree-family/custom-partitioning-key). В этом выражении может использоваться SQL-выражение по любому столбцу или нескольким столбцам, результат которого определяет, в какую партицию будет помещена строка.
...
Партиционирование в ClickHouse используется для схожих целей, что и в BigQuery, но с некоторыми тонкими отличиями. В частности:

* **Управление данными** — В ClickHouse в первую очередь следует рассматривать партиционирование как средство управления данными, а не как технику оптимизации запросов. Логически разделяя данные по ключу, можно независимо работать с каждой партицией, например, удалять её. Это позволяет эффективно перемещать партиции, а значит и подмножества данных, между [уровнями хранилища](/integrations/s3#storage-tiers) по времени или [удалять устаревшие данные/эффективно очищать кластер](/sql-reference/statements/alter/partition). В примере ниже мы удаляем записи за 2008 год:
...
материализованное представление отправляет строки в целевую таблицу с иным ключом упорядочивания
...
Этот запрос требует сканирования всех 90 млн строк (хотя и быстро), так как `UserId`
не является ключом упорядочивания. Ранее мы решали это с помощью материализованного представления,
...
Если мы повторно выполним приведённый выше запрос, мы увидим, что производительность значительно улучшилась за счёт увеличения объёма хранимых данных.
...
вам следует знать, поэтому их следует использовать с осторожностью. Для получения дополнительной 
...
Где это возможно, вам следует использовать агрегатные функции ClickHouse. Ниже показано использование [функции `argMax`](/sql-reference/aggregate-functions/reference/argmax) для вычисления самого просматриваемого вопроса за каждый год.
