...
> Это **Часть 2** руководства по миграции с PostgreSQL на ClickHouse. На практическом примере показывается, как эффективно выполнить миграцию с использованием подхода репликации в реальном времени (CDC). Многие из рассматриваемых концепций также применимы к ручной массовой передаче данных из PostgreSQL в ClickHouse.

Большинство SQL-запросов из вашей инсталляции PostgreSQL должны выполняться в ClickHouse без изменений и, скорее всего, будут работать быстрее.



## Дедупликация с использованием CDC {#deduplication-cdc}

При использовании репликации в реальном времени с CDC следует учитывать, что операции обновления и удаления могут приводить к появлению дублирующихся строк. Для решения этой проблемы можно использовать представления (Views) и обновляемые материализованные представления (Refreshable Materialized Views).

См. это [руководство](/integrations/clickpipes/postgres/deduplication#query-like-with-postgres), чтобы узнать, как перенести приложение из PostgreSQL в ClickHouse с минимальными сложностями при миграции с использованием репликации в реальном времени с CDC.
...
Приведенные здесь примеры охватывают распространенные шаблоны запросов и показывают, как оптимизировать их с помощью ClickHouse. В них используется полный [набор данных Stack Overflow](/getting-started/example-datasets/stackoverflow) (по апрель 2024 года) на эквивалентных ресурсах в PostgreSQL и ClickHouse (8 ядер, 32 ГиБ оперативной памяти).

> Для простоты в приведенных ниже запросах не используются техники дедупликации данных.
...
Где это возможно, пользователям следует использовать агрегатные функции ClickHouse. Ниже мы показываем использование функции [argMax](/sql-reference/aggregate-functions/reference/argmax) для вычисления наиболее просматриваемого вопроса каждого года.
...
Это заметно проще (и быстрее), чем аналогичный запрос в Postgres:
...
...
 ...
 2023 | Как решить ошибку "error: externally-managed-environment" при каждом использовании pip 3?                               |       506822
...
Условные выражения и функции для работы с массивами значительно упрощают запросы. Следующий запрос вычисляет теги (с числом вхождений более 10 000) с наибольшим процентным ростом с 2022 по 2023 год. Обратите внимание, насколько лаконичен этот запрос ClickHouse благодаря условным выражениям, функциям для работы с массивами и возможности повторно использовать псевдонимы в предложениях HAVING и SELECT.
...
5 строк в наборе. Время: 0.247 сек. Обработано 5.08 млн строк, 155.73 MB (20.58 млн строк/с, 630.61 MB/s.)
...
[Часть 3 — по этой ссылке](/migrations/postgresql/data-modeling-techniques)
