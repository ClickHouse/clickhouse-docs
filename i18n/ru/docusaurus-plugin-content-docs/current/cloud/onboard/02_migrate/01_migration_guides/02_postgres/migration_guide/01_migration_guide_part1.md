...
> Это **Часть 1** руководства по миграции с PostgreSQL на ClickHouse. На практическом примере показывается, как эффективно выполнить миграцию с использованием подхода репликации изменений в реальном времени (CDC). Многие рассматриваемые концепции также применимы к ручной массовой загрузке данных из PostgreSQL в ClickHouse.
...
<Image
  ...
  size='lg'
  alt='Схема PostgreSQL для Stack Overflow'
/>
...
Эта схема, хотя и не является оптимальной, использует ряд популярных возможностей PostgreSQL, включая первичные ключи, внешние ключи, партиционирование и индексы.
...
Хотя для ClickHouse этот набор данных невелик, для Postgres он достаточно объёмен. Приведённое выше представляет собой подмножество, охватывающее первые три месяца 2024 года.

> Хотя в наших примерах используется полный набор данных для демонстрации различий в производительности между Postgres и ClickHouse, все описанные ниже шаги функционально идентичны при работе с меньшим подмножеством. Пользователи, желающие загрузить полный набор данных в Postgres, могут ознакомиться с инструкциями [здесь](https://pastila.nl/?00d47a08/1c5224c0b61beb480539f15ac375619d#XNj5vX3a7ZjkdiX7In8wqA==). Из-за ограничений внешних ключей, накладываемых приведённой выше схемой, полный набор данных для PostgreSQL содержит только строки, соответствующие требованиям ссылочной целостности. При необходимости [версия в формате Parquet](/getting-started/example-datasets/stackoverflow) без таких ограничений может быть легко загружена непосредственно в ClickHouse.
```
...
При использовании ручного подхода начальную массовую загрузку набора данных можно выполнить следующими способами:
...
- **Экспорт** — экспорт в промежуточные форматы, такие как CSV или файл SQL-скрипта. Эти файлы затем можно загрузить в ClickHouse либо из клиента с помощью конструкции `INSERT FROM INFILE`, либо используя объектное хранилище и связанные с ним функции, например s3, gcs.
...
Мы можем заполнить её с помощью простого `INSERT INTO SELECT`, считывая данные из PostgreSQL и вставляя их в ClickHouse:
...
Инкрементальные загрузки, в свою очередь, можно выполнять по расписанию. Если таблица Postgres получает только вставки и в ней есть монотонно растущий id или timestamp, пользователи могут воспользоваться описанным выше подходом с табличной функцией для загрузки инкрементов, то есть к `SELECT` может быть применено условие `WHERE`. Этот подход также можно использовать для поддержки обновлений, если гарантируется, что изменяется один и тот же столбец. Поддержка удалений, однако, потребует полной перезагрузки, что может быть сложно реализовать по мере роста таблицы.

Мы демонстрируем начальную загрузку и инкрементальную загрузку, используя `CreationDate` (мы предполагаем, что это поле обновляется при обновлении строк).
...
> ClickHouse будет выполнять проталкивание простых выражений `WHERE`, таких как `=`, `!=`, `>`,`>=`, `<`, `<=` и IN, на сервер PostgreSQL. Таким образом, инкрементальные загрузки можно сделать более эффективными, обеспечив наличие индекса по столбцам, используемым для идентификации набора изменений.

> Один из возможных способов выявления операций UPDATE при использовании репликации запросов — использовать [системный столбец `XMIN`](https://www.postgresql.org/docs/9.1/ddl-system-columns.html) (идентификаторы транзакций) в качестве водяного знака: изменение этого столбца указывает на изменение строки и, следовательно, такая строка должна быть применена к целевой таблице. Пользователям, применяющим этот подход, следует учитывать, что значения `XMIN` могут переполняться и начинаться сначала (wrap around), а сравнения требуют полного сканирования таблицы, что усложняет отслеживание изменений.
