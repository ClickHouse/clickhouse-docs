...
description: 'Дополнительная информация по миграции с PostgreSQL'
doc_type: 'reference'
---
...
В ClickHouse возможность грязного чтения — когда вы можете записать данные в одну реплику, а затем читать потенциально ещё не реплицированные данные с другой — возникает из‑за модели репликации с eventual consistency, управляемой через Keeper. Эта модель делает акцент на производительности и масштабируемости в распределённых системах, позволяя репликам работать независимо и синхронизироваться асинхронно. В результате недавно вставленные данные могут быть не сразу видны на всех репликах — это зависит от лага репликации и времени, необходимого для распространения изменений по системе.
...
> При создании Distributed table вы указываете кластер. Это определение кластера, задаваемое в config.xml, перечисляет шарды (и их реплики), что позволяет пользователям контролировать порядок их использования с каждого узла. Благодаря этому вы можете обеспечить детерминированный выбор.

## Последовательная согласованность \\{#sequential-consistency\\}

В исключительных случаях вам может потребоваться последовательная согласованность.
...
ClickHouse обеспечивает свойства ACID при [ограниченных конфигурациях](/guides/developer/transactional) — в самом простом случае при использовании нереплицированного экземпляра движка таблиц MergeTree с одним разделом. Вам не следует ожидать наличия этих свойств вне этих случаев и необходимо убедиться, что они не являются обязательным требованием.
