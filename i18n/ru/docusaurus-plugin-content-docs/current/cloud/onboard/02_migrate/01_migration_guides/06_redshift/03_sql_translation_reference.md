...
description: 'Справочник по преобразованию SQL с Amazon Redshift в ClickHouse'
keywords: ['Redshift']
title: 'Руководство по преобразованию SQL с Amazon Redshift в ClickHouse'
doc_type: 'reference'
---



# Руководство по преобразованию SQL из Amazon Redshift
...
с пользователя, храня строки без кодирования в байтах. Таким образом, тип String в ClickHouse не имеет ограничений или требований к указанию длины.

Кроме того, пользователи могут использовать типы Array, Tuple и Enum — отсутствующие в
Redshift как полноценные типы (хотя массивы и структуры можно имитировать
с помощью `SUPER`), что является распространённой проблемой пользователей. ClickHouse дополнительно
позволяет сохранять состояния агрегации как во время выполнения запроса, так и непосредственно в таблице.
Это позволяет предварительно агрегировать данные, обычно
с использованием материализованных представлений, что может значительно улучшить производительность
типовых запросов.
...
| [`DECIMAL`](https://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html#r_Numeric_types201-decimal-or-numeric-type)       | [`UInt128`, `UInt256`, `Int128`, `Int256`](/sql-reference/data-types/int-uint),[`Decimal(P, S)`, `Decimal32(S)`, `Decimal64(S)`, `Decimal128(S)`, `Decimal256(S)`](/sql-reference/data-types/decimal)- (поддерживает высокую точность и широкие диапазоны)          |
...
| [`GEOMETRY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                                | [Геоданные: типы данных](/sql-reference/data-types/geo)                                                                                                                                                                                                             |
| [`GEOGRAPHY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                               | [Геоданные: типы](/sql-reference/data-types/geo)(менее развито, например, нет систем координат — можно эмулировать[с использованием функций](/sql-reference/functions/geo/)(менее развито, например, нет систем координат — их можно эмулировать с помощью функций) |
...
<sub><span>**</span>Тип String в ClickHouse по умолчанию неограничен по длине, но её можно задать с помощью <a href='http://clickhouse.com/docs/sql-reference/statements/create/table#constraints'>ограничений (Constraints)</a>.</sub>
...
Redshift поддерживает несколько удобных возможностей для ключей сортировки. Первая —
...
который придаёт равный вес подмножеству столбцов в ключе сортировки для повышения
...
концепцию РСУБД, предназначенную для обеспечения ограничений целостности. Однако в Redshift они не применяются
строго и вместо этого служат подсказками для планировщика запросов и распределения данных
между узлами. В ClickHouse первичный ключ обозначает столбцы, используемые
