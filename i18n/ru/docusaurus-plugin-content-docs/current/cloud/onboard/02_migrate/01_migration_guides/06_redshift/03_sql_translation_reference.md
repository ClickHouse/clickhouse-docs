...
description: 'Справочник по переводу SQL с Amazon Redshift на ClickHouse'
keywords: ['Redshift']
title: 'Руководство по переводу SQL из Amazon Redshift'
doc_type: 'reference'
---



# Руководство по преобразованию SQL с Amazon Redshift
...
с пользователя, храня строки в виде байтов без кодирования. Таким образом, тип String в ClickHouse не имеет ограничений и не требует указания длины.

Кроме того, пользователи могут использовать массивы (Array), кортежи (Tuple) и перечисления (Enum) — 
типы, отсутствующие в Redshift как полноценные (хотя массивы и структуры можно имитировать
с помощью `SUPER`), что является распространённой проблемой пользователей. ClickHouse также
позволяет сохранять состояния агрегации как во время выполнения запроса, так и непосредственно в таблице. Это позволяет предварительно агрегировать данные, обычно
...
| [`DECIMAL`](https://docs.aws.amazon.com/redshift/latest/dg/r_Numeric_types201.html#r_Numeric_types201-decimal-or-numeric-type)       | [`UInt128`, `UInt256`, `Int128`, `Int256`](/sql-reference/data-types/int-uint),[`Decimal(P, S)`, `Decimal32(S)`, `Decimal64(S)`, `Decimal128(S)`, `Decimal256(S)`](/sql-reference/data-types/decimal)- (возможна высокая точность и широкий диапазон значений)                          |
...
| [`GEOMETRY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                                | [Геоданные и геометрические типы](/sql-reference/data-types/geo)                                                                                                                                                                                                                        |
| [`GEOGRAPHY`](https://docs.aws.amazon.com/redshift/latest/dg/geospatial-overview.html)                                               | [Геопространственные типы данных](/sql-reference/data-types/geo)(менее развито, например, нет систем координат — можно эмулировать[с использованием функций](/sql-reference/functions/geo/)) (менее развито: например, нет систем координат, но их можно эмулировать с помощью функций) |
...
| [`VARBYTE`](https://docs.aws.amazon.com/redshift/latest/dg/r_VARBYTE_type.html)**                                                    | [`String`](/sql-reference/data-types/string)в сочетании с[`Битовые`](/sql-reference/functions/bit-functions)и[Кодировки](/sql-reference/functions/encoding-functions/#hex)функции                                                                                                       |



<sub><span>*</span> ClickHouse дополнительно поддерживает беззнаковые целые числа с расширенными диапазонами, то есть <a href='http://clickhouse.com/docs/sql-reference/data-types/int-uint'>`UInt8`, `UInt16`, `UInt32` и `UInt64`</a>.</sub><br />
<sub><span>**</span>Тип String в ClickHouse по умолчанию не имеет ограничения длины, но может быть ограничен до конкретной длины с помощью <a href='http://clickhouse.com/docs/sql-reference/statements/create/table#constraints'>ограничений (Constraints)</a>.</sub>
...
порядок сортировки данных при их сохранении. В Redshift ключ сортировки задаётся с помощью
конструкции `SORTKEY`:
...
в Redshift необходимо выполнять команды `VACUUM` и `ANALYZE` для пересортировки новых данных
и обновления статистики для планировщика запросов — в противном случае объём несортированных данных
будет расти. В ClickHouse такой процесс не требуется.
...
для начала работы, явные ключи сортировки обеспечивают наилучшую производительность и
эффективность хранения при оптимальном выборе ключа сортировки. Вторая возможность —
ключ сортировки `INTERLEAVED`, который придаёт равный вес подмножеству столбцов в ключе
сортировки для повышения производительности, когда запрос использует один или несколько
вторичных столбцов сортировки. ClickHouse поддерживает явные [проекции](/data-modeling/projections),
которые достигают того же результата при несколько иной настройке.

Пользователям следует учитывать, что концепция «первичного ключа» имеет разное значение
в ClickHouse и Redshift. В Redshift первичный ключ напоминает традиционную концепцию СУБД,
предназначенную для обеспечения ограничений целостности. Однако в Redshift они не применяются
строго, а вместо этого служат подсказками для планировщика запросов и распределения данных
между узлами. В ClickHouse первичный ключ обозначает столбцы, используемые для построения
разреженного первичного индекса, который обеспечивает упорядоченность данных на диске,
максимизируя сжатие и предотвращая загрязнение первичного индекса и нерациональное использование памяти.
