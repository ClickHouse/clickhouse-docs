...
<Image img={object_storage_01} size='md' alt='Миграция автономного ClickHouse' background='white' />

Если вы используете облачное объектное хранилище как data lake и хотите импортировать эти данные в ClickHouse Cloud,
...
Если ваша текущая система баз данных не может напрямую выгружать данные в облачное объектное хранилище, вы можете использовать [сторонний ETL/ELT-инструмент](/cloud/migration/etl-tool-to-clickhouse) или [clickhouse-local](/cloud/migration/clickhouse-local), чтобы перенести данные
из вашей текущей системы баз данных в облачное объектное хранилище, а затем на втором шаге мигрировать эти данные в таблицу ClickHouse Cloud.
...
Также вы можете использовать продвинутые сжатые форматы, такие как [Parquet](/interfaces/formats/Parquet).

Есть [статья в блоге](https://clickhouse.com/blog/getting-data-into-clickhouse-part-3-s3) с конкретными примерами кода, показывающими, как можно загружать данные в ClickHouse Cloud с помощью S3.