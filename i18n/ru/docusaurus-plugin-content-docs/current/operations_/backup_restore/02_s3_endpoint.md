...
title: 'Резервное копирование и восстановление через S3 endpoint'
doc_type: 'guide'
---
...
В этой статье рассматривается создание резервных копий и восстановление данных в корзину S3 или из неё
...
Создайте следующую базу данных и таблицу, а затем вставьте в неё случайные данные:
...
Для инкрементного резервного копирования требуется _базовая_ резервная копия в качестве отправной точки. Первый параметр места назначения S3 — это конечная точка S3, за которой следует каталог внутри корзины для этой резервной копии. В данном примере каталог называется `my_backup`.
...
Этот запрос сравнивает содержимое исходной таблицы `test_table` с восстановленной таблицей `test_table_restored`:
