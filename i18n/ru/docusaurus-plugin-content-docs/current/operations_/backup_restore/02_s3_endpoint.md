...
title: 'Резервное копирование и восстановление с/на конечную точку S3'
doc_type: 'guide'
---
...
# BACKUP / RESTORE в конечную точку S3 или из неё {#backup-to-a-local-disk}

В этой статье рассматривается резервное копирование и восстановление данных в корзину S3 и из неё
...
### Инкрементное резервное копирование на S3-endpoint {#incremental-backup-to-an-s3-endpoint}

В этом примере мы создадим резервную копию на S3-endpoint, а затем восстановим из неё данные.
...
| Параметр          | Пример                                                       |
| ----------------- | ------------------------------------------------------------ |
| S3-endpoint       | `https://backup-ch-docs.s3.us-east-1.amazonaws.com/backups/` |
| Идентификатор ключа доступа | `BKIOZLE2VYN3VXXTP9RC`                                       |
| Секретный ключ доступа | `40bwYnbqN7xU8bVePaUCh3+YEyGXu8UOMV9ANpwL`                   |

:::tip
Создание S3-bucket описано в разделе ["использование S3 Object Storage в качестве диска ClickHouse"](/integrations/data-ingestion/s3/index.md#configuring-s3-for-clickhouse-use)
:::
...
Для инкрементного резервного копирования требуется _базовая_ резервная копия в качестве отправной точки. Первый параметр назначения S3 — это S3-endpoint, за которым следует директория внутри bucket для хранения этой резервной копии. В данном примере директория называется `my_backup`.
...
Эта команда резервного копирования аналогична команде для базовой резервной копии, но добавляет `SETTINGS base_backup` и расположение базовой резервной копии. Обратите внимание, что место назначения для инкрементной резервной копии — это не та же директория, что и для базовой, а тот же endpoint с другой целевой директорией внутри bucket. Базовая резервная копия находится в `my_backup`, а инкрементная будет записана в `my_incremental`:
...
Обратите внимание, что при восстановлении инкрементной резервной копии базовая резервная копия также включается автоматически.
...
В исходную таблицу `data` было выполнено две вставки: одна с 1000 строками и одна со 100 строками, всего 1100 строк.
Проверьте, что восстановленная таблица содержит 1100 строк:
...
Сравнение содержимого исходной таблицы `test_table` с восстановленной таблицей `test_table_restored`:
