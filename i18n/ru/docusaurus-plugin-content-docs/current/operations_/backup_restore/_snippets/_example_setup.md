Выполните следующие команды, чтобы создать тестовую базу данных и таблицу, для которых в этом примере мы будем
создавать резервную копию и выполнять восстановление:

<details>
<summary>Команды для подготовки</summary>
...
Сгенерируйте и вставьте тысячу строк случайных данных:
...
Далее необходимо создать файл, задающий место назначения резервной копии, по
следующему пути:
...
:::note
Если clickhouse-server уже запущен, его нужно перезапустить, чтобы изменения
вступили в силу.
:::
