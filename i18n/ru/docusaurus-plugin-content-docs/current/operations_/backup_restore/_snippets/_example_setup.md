Выполните следующие команды, чтобы создать тестовую базу данных и таблицу, резервную копию и восстановление которых мы будем выполнять в этом примере:
...
Заполните таблицу одной тысячей строк случайных данных:
...
Далее нужно создать файл, в котором будет указано место назначения для резервных копий, по следующему пути:
...
:::note
Если clickhouse-server запущен, необходимо перезагрузить его, чтобы изменения вступили в силу.
:::
