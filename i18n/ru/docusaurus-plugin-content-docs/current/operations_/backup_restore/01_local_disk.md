...
Также возможно выполнять операции `BACKUP`/`RESTORE` в S3, настроив диск S3 в
...
- Этот диск не следует использовать для самого `MergeTree`, только для операций `BACKUP`/`RESTORE`
...
Для восстановления таблицы, которая уже содержит данные, выполните:
...
для возможности восстановления из инкрементной резервной копии.
:::
...
Если необходимо восстановить конкретные партиции, связанные с таблицей, их можно указать.

Создадим простую партиционированную таблицу из четырёх частей, вставим в неё данные, а затем создадим резервную копию только первой и четвёртой партиций:
