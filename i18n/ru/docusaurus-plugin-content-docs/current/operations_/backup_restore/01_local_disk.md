---
description: 'Резервное копирование и восстановление на локальный диск и с локального диска'
...
В приведённых ниже примерах место назначения резервной копии указывается как `Disk('backups', '1.zip')`.  
...
Например, приведённая ниже конфигурация определяет диск с именем `backups` и затем добавляет этот диск в
...
`BACKUP`/`RESTORE` для диска S3 выполняется так же, как и для локального диска:
...
Для создания резервной копии таблицы выполните:
...
Таблицу можно восстановить из резервной копии с помощью следующей команды, если таблица пуста:
...
:::note
Приведенная выше команда `RESTORE` завершится ошибкой, если таблица `test.table` содержит данные.
Настройка `allow_non_empty_tables=true` позволяет команде `RESTORE TABLE` вставлять данные
в непустые таблицы. Это приведет к смешиванию существующих данных в таблице с данными, извлеченными из резервной копии.
Поэтому данная настройка может вызвать дублирование данных в таблице и должна использоваться с осторожностью.
:::

Для восстановления таблицы, уже содержащей данные, выполните:
...
Таблицы можно восстанавливать или копировать с новыми именами:

```sql
RESTORE TABLE test_db.table_table AS test_db.test_table_renamed FROM Disk('backups', '1.zip')
```

Архив резервной копии имеет следующую структуру:
...
Можно использовать форматы, отличные от zip. Дополнительные сведения см. в разделе [«Резервные копии в виде tar-архивов»](#backups-as-tar-archives)
ниже.

### Инкрементные резервные копии на диск {#incremental-backups}

Базовая резервная копия в ClickHouse — это начальная полная резервная копия, на основе которой создаются последующие
инкрементные резервные копии. Инкрементные резервные копии хранят только изменения,
внесенные после создания базовой резервной копии, поэтому базовая резервная копия должна оставаться доступной для
восстановления из любой инкрементной резервной копии. Расположение базовой резервной копии можно задать с помощью настройки
`base_backup`.

:::note
Инкрементные резервные копии зависят от базовой резервной копии. Базовая резервная копия должна оставаться доступной,
чтобы можно было выполнить восстановление из инкрементной резервной копии.
:::

Для создания инкрементной резервной копии таблицы сначала создайте базовую резервную копию:
...
Все данные из инкрементной резервной копии и базовой резервной копии можно восстановить в
...
К резервным копиям, записанным на диск, можно применить пароль для защиты файла.
...
Для восстановления защищенной паролем резервной копии необходимо снова
...
Функциональность такая же, как для zip, за исключением того, что защита паролем не
поддерживается для tar-архивов. Кроме того, tar-архивы поддерживают различные
методы сжатия.

Для создания резервной копии таблицы в виде tar-архива:
...
Метод сжатия и уровень сжатия можно указать с помощью настроек `compression_method` и `compression_level` соответственно.
