---
description: 'Обзор поддерживаемых ClickHouse форматов ввода и вывода данных'
...
ClickHouse поддерживает большинство известных текстовых и бинарных форматов данных. Это позволяет легко интегрировать ClickHouse практически в любой существующий конвейер обработки данных и использовать его преимущества.
...
- Выполнения запросов `SELECT` из таблиц на основе файлов, таких как `File`, `URL` или `HDFS`
- Чтения словарей

Выбор правильного формата ввода имеет решающее значение для эффективной загрузки данных в ClickHouse. При наличии более 70 поддерживаемых форматов выбор наиболее производительного варианта может существенно повлиять на скорость вставки, использование ЦП и памяти, а также на общую эффективность системы. Чтобы помочь в выборе, мы провели тестирование производительности загрузки данных в различных форматах и выявили ключевые выводы:

- **Формат [Native](formats/Native.md) является наиболее эффективным форматом ввода**, обеспечивая лучшее сжатие, минимальное использование ресурсов и минимальные накладные расходы на обработку на стороне сервера.
- **Сжатие необходимо** — LZ4 уменьшает размер данных при минимальных затратах ЦП, в то время как ZSTD обеспечивает более высокое сжатие за счёт дополнительного использования ЦП.
- **Предварительная сортировка имеет умеренное влияние**, поскольку ClickHouse уже эффективно выполняет сортировку.
- **Пакетная обработка значительно повышает эффективность** — большие пакеты снижают накладные расходы на вставку и улучшают пропускную способность.

Для подробного изучения результатов и лучших практик ознакомьтесь с полным [анализом тестирования](https://www.clickhouse.com/blog/clickhouse-input-format-matchup-which-is-fastest-most-efficient).
Для просмотра полных результатов тестирования изучите онлайн-панель [FastFormats](https://fastformats.clickhouse.com/).
...
- Выполнения операций `INSERT` в таблицы на основе файлов
...
| [Шаблон](./formats/Template/Template.md)                                                                   | ✔    | ✔     |
...
Вы можете управлять некоторыми параметрами обработки форматов с помощью настроек ClickHouse. Подробнее см. раздел [Settings](/operations/settings/settings-formats.md).
...
Этот параметр обязателен при использовании форматов `Cap'n Proto` и `Protobuf`.
...
При вводе или выводе данных через [клиент](/interfaces/cli.md) в интерактивном режиме имя файла, указанное в схеме формата,
может содержать абсолютный путь или путь относительно текущего каталога на клиенте.
При использовании клиента в [пакетном режиме](/interfaces/cli.md/#batch-mode) путь к схеме должен быть относительным по соображениям безопасности.

При вводе или выводе данных через [HTTP-интерфейс](/interfaces/http.md) имя файла, указанное в схеме формата,
должно находиться в каталоге, заданном параметром [format_schema_path](/operations/server-configuration-parameters/settings.md/#format_schema_path)
в конфигурации сервера.


## Пропуск ошибок {#skippingerrors}

Некоторые форматы, такие как `CSV`, `TabSeparated`, `TSKV`, `JSONEachRow`, `Template`, `CustomSeparated` и `Protobuf`, могут пропускать повреждённую строку при возникновении ошибки разбора и продолжать разбор со следующей строки. См. настройки [input_format_allow_errors_num](/operations/settings/settings-formats.md/#input_format_allow_errors_num) и
[input_format_allow_errors_ratio](/operations/settings/settings-formats.md/#input_format_allow_errors_ratio).
Ограничения:

- В случае ошибки разбора `JSONEachRow` пропускает все данные до новой строки (или EOF), поэтому строки должны разделяться символом `\n` для корректного подсчёта ошибок.
- `Template` и `CustomSeparated` используют разделитель после последнего столбца и разделитель между строками для поиска начала следующей строки, поэтому пропуск ошибок работает только в том случае, если хотя бы один из них не является пустым.
