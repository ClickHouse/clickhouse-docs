...
Формат `CapnProto` — это двоичный формат сообщений, похожий на формат [`Protocol Buffers`](https://developers.google.com/protocol-buffers/) и [Thrift](https://en.wikipedia.org/wiki/Apache_Thrift), но не на [JSON](./JSON/JSON.md) или [MessagePack](https://msgpack.org/).
Сообщения CapnProto строго типизированы и не являются самоописывающимися, то есть им требуется внешнее описание схемы. Схема применяется динамически и кэшируется для каждого запроса.

См. также раздел [Format Schema](/interfaces/formats/#formatschema).



## Соответствие типов данных {#data_types-matching-capnproto}

В таблице ниже перечислены поддерживаемые типы данных и то, как они сопоставляются с [типами данных](/sql-reference/data-types/index.md) ClickHouse в запросах `INSERT` и `SELECT`.
...
- Целочисленные типы могут быть преобразованы друг в друга при чтении и записи данных.
...
- Массивы могут быть вложенными и иметь элементы типа `Nullable` в качестве аргумента. Типы `Tuple` и `Map` также могут быть вложенными.
...
Вы можете вставить данные CapnProto из файла в таблицу ClickHouse, выполнив следующую команду:

```bash
$ cat capnproto_messages.bin | clickhouse-client --query "INSERT INTO test.hits SETTINGS format_schema = 'schema:Message' FORMAT CapnProto"
```

Файл `schema.capnp` имеет следующий вид:
...
Если у вас нет внешней схемы `CapnProto` для ваших данных, вы всё равно можете выводить/считывать данные в формате `CapnProto`, используя автоматически сгенерированную схему.
...
Вы также можете прочитать файл CapnProto с автоматически сгенерированной схемой (при этом файл должен быть создан с использованием той же схемы):
...
Вы также можете сохранить автоматически сгенерированную схему в файл при вводе и выводе данных, используя настройку [`output_format_schema`](/operations/settings/formats#output_format_schema).
