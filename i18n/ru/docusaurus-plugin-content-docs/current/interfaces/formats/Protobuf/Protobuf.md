...
Поддерживаются вложенные сообщения (см. [basic-examples](#basic-examples)). Пустые сообщения также поддерживаются.
...
<details>
  ...
  Запустите скрипт для генерации двоичного файла:
