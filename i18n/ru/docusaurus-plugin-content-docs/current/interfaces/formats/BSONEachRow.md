...
Формат `BSONEachRow` обрабатывает данные как последовательность документов Binary JSON (BSON) без разделителей между ними.
Каждая строка представляет собой отдельный документ, а каждый столбец — отдельное поле документа BSON, где имя столбца используется в качестве ключа.
...
Другие типы BSON не поддерживаются. Также выполняется преобразование между различными целочисленными типами. 
Например, можно вставить значение BSON `int32` в ClickHouse как [`UInt8`](../../sql-reference/data-types/int-uint.md).

Большие целые и десятичные числа, такие как `Int128`/`UInt128`/`Int256`/`UInt256`/`Decimal128`/`Decimal256`, могут быть прочитаны из BSON Binary со значением подтипа `\x00`. 
...
Используя BSON-файл со следующими данными с именем `football.bson`:
...
Чтение данных с использованием формата `BSONEachRow`:
...
:::tip
BSON — это бинарный формат, который не отображается в удобочитаемом виде в терминале. Используйте `INTO OUTFILE` для вывода BSON-файлов.
:::
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------- |
| [`output_format_bson_string_as_string`](../../operations/settings/settings-formats.md/#output_format_bson_string_as_string)                                                                           | Использовать тип BSON String вместо Binary для столбцов String.                                   | `false` |
