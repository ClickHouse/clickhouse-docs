...
Каждая строка форматируется как отдельный документ, а каждый столбец — как отдельное поле BSON-документа с именем столбца в качестве ключа.



## Соответствие типов данных {#data-types-matching}

При выводе данных используется следующее соответствие между типами ClickHouse и типами BSON:
...
| [Decimal128](/sql-reference/data-types/decimal.md)                                                                   | `\x05` binary, `\x00` двоичный подтип, размер = 16                                                            |
| [Decimal256](/sql-reference/data-types/decimal.md)                                                                   | `\x05` binary, `\x00` двоичный подтип, размер = 32                                                            |
| [Int128/UInt128](/sql-reference/data-types/int-uint.md)                                                              | `\x05` binary, `\x00` двоичный подтип, размер = 16                                                            |
| [Int256/UInt256](/sql-reference/data-types/int-uint.md)                                                              | `\x05` binary, `\x00` двоичный подтип, размер = 32                                                            |
| [String](/sql-reference/data-types/string.md)/[FixedString](/sql-reference/data-types/fixedstring.md)               | `\x05` binary, `\x00` двоичный подтип или \x02 string, если включена настройка `output_format_bson_string_as_string` |
...
| [IPv6](/sql-reference/data-types/ipv6.md)                                                                            | `\x05` binary, `\x00` двоичный подтип                                                                         |

При вводе данных используется следующее соответствие между типами BSON и типами ClickHouse:
...
Например, можно вставить значение BSON `int32` в ClickHouse как [`UInt8`](../../sql-reference/data-types/int-uint.md).

Большие целые числа и десятичные значения, такие как `Int128`/`UInt128`/`Int256`/`UInt256`/`Decimal128`/`Decimal256`, могут быть прочитаны из значения BSON типа Binary с двоичным подтипом `\x00`.  
В этом случае формат проверяет, что размер двоичных данных равен размеру ожидаемого значения.

:::note
Этот формат работает некорректно на платформах с порядком байт Big-Endian.
:::
...
Считайте данные в формате `BSONEachRow`:
...
:::tip
BSON — это двоичный формат, который не отображается в человекочитаемом виде в терминале. Используйте `INTO OUTFILE` для вывода BSON‑файлов.
:::
