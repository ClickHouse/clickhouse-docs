...
[Apache Arrow](https://arrow.apache.org/) поставляется с двумя встроенными колонночными форматами хранения данных. ClickHouse поддерживает операции чтения и записи для этих форматов.
`Arrow` — это «файловый» формат Apache Arrow. Он предназначен для произвольного доступа к данным в памяти.
...
Тип `DICTIONARY` поддерживается для запросов `INSERT`, а для запросов `SELECT` есть настройка [`output_format_arrow_low_cardinality_as_dictionary`](/operations/settings/formats#output_format_arrow_low_cardinality_as_dictionary), которая позволяет выводить значения типа [LowCardinality](/sql-reference/data-types/lowcardinality.md) в виде типа `DICTIONARY`.
...
Вы можете вставить данные в формате Arrow из файла в таблицу ClickHouse, используя следующую команду:
...
|--------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|------------------------|
| `input_format_arrow_allow_missing_columns`                                                                               | Разрешать отсутствующие столбцы при чтении входных форматов Arrow                                                             | `1`                    |
| `input_format_arrow_case_insensitive_column_matching`                                                                    | Игнорировать регистр при сопоставлении столбцов Arrow со столбцами CH                                                        | `0`                    |
| `input_format_arrow_import_nested`                                                                                       | Устаревшая настройка, не оказывает никакого эффекта                                                                           | `0`                    |
...
| `output_format_arrow_compression_method`                                                                                 | Метод сжатия для формата вывода Arrow. Поддерживаемые кодеки: lz4_frame, zstd, none (без сжатия)                             | `lz4_frame`            |
