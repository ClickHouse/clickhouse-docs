...
Этот формат объединяет компактный построчный вывод JSONCompactEachRow с потоковой информацией о ходе выполнения.
Он выводит данные в виде отдельных JSON-объектов для метаданных, отдельных строк, обновлений хода выполнения,
итогов и исключений. Значения представлены в их исходных типах.

Ключевые особенности:

- Сначала выводятся метаданные с именами и типами столбцов
- Каждая строка представляет собой отдельный JSON-объект с ключом "row", содержащим массив значений
- Включает обновления хода выполнения во время выполнения запроса (в виде объектов `{"progress":...}`)
