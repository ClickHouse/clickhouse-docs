...
Этот формат сочетает компактный построчный вывод JSONCompactEachRow с потоковой информацией
о ходе выполнения запроса.
Он выводит данные в виде отдельных JSON-объектов для метаданных, отдельных строк, обновлений прогресса,
итогов и исключений. Значения представлены в их родных типах.

Ключевые особенности:
- Сначала выводит метаданные с именами и типами столбцов
- Каждая строка — отдельный JSON-объект с ключом "row", значением которого является массив значений
- Включает обновления прогресса во время выполнения запроса (в виде объектов `{"progress":...}`)
- Поддерживает totals и extremes
- Значения сохраняют свои родные типы (числа — как числа, строки — как строки)
