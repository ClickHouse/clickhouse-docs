...
В этом формате один объект JSON интерпретируется как одно значение типа [JSON](/sql-reference/data-types/newjson.md). Если во входных данных содержится несколько объектов JSON (разделённых запятыми), они интерпретируются как отдельные строки. Если входные данные заключены в квадратные скобки, они интерпретируются как массив объектов JSON.

Этот формат может быть обработан только для таблицы с одним полем типа [JSON](/sql-reference/data-types/newjson.md). Остальные столбцы должны иметь значение [`DEFAULT`](/sql-reference/statements/create/table.md/#default) или [`MATERIALIZED`](/sql-reference/statements/create/view#materialized-view).
