...
В этом формате один объект JSON интерпретируется как одно значение типа [JSON](/sql-reference/data-types/newjson.md). Если на входе — несколько объектов JSON (через запятую), они интерпретируются как отдельные строки. Если входные данные заключены в `[]`, они интерпретируются как массив значений JSON.
