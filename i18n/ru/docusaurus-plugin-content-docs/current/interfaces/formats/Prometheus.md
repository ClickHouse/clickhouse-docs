...
Для этого формата выходная таблица должна быть структурирована в соответствии со следующими правилами:
...
- Столбец `type` ([String](/sql-reference/data-types/string.md)) должен принимать одно из значений: `counter`, `gauge`, `histogram`, `summary`, `untyped` или быть пустым.
- Каждое значение метрики может также содержать `labels` ([Map(String, String)](/sql-reference/data-types/map.md)).
...
Для меток `histogram` и `summary` существуют специальные требования — подробности см. в [документации Prometheus](https://prometheus.io/docs/instrumenting/exposition_formats/#histograms-and-summaries).
...
│ http_requests_total                 │ counter   │ Общее число HTTP-запросов                 │ {'method':'post','code':'200'} │     1027 │ 1395066363000 │
...
Будет отформатировано следующим образом:
...
# HELP http_requests_total Общее число HTTP-запросов
...
# HELP rpc&#95;duration&#95;seconds Сводная статистика по длительности RPC в секундах.
