...
\* По умолчанию используется `bytes`; поведение определяется настройкой [`output_format_avro_string_column_pattern`](/operations/settings/settings-formats.md/#output_format_avro_string_column_pattern)
...
В результате при формировании Avro из ClickHouse мы всегда должны включать тип `null` в набор типов Avro `union`, так как при выводе схемы мы не знаем, является ли какое-либо значение фактически `null`.
