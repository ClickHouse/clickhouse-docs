...
Формат вывода `Hash` вычисляет единое хеш-значение по всем столбцам и строкам результата.
Это полезно для вычисления «отпечатка» результата, например в ситуациях, когда узким местом является передача данных.
...
Прочитать данные в формате `Hash`:
