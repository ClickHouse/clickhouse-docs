...
| `--inline-insert-data`          | Отправляет `INSERT ... VALUES` (и другие встроенные форматы) как есть в тексте запроса вместо преобразования данных в блоки в нативном формате. Сервер сам разбирает встроенные данные, что позволяет избежать лишнего обмена данными при передаче структуры таблицы и значений по умолчанию столбцов обратно клиенту. Это может повысить производительность при большом количестве небольших вставок по собственному протоколу. Автоматически устанавливает [`send_table_structure_on_insert_with_inline_data`](/operations/settings/settings#send_table_structure_on_insert_with_inline_data) в `0`. Нельзя использовать одновременно со встроенными и внешними данными (из stdin или `INFILE`). |
...
| `--pager <command>`               | Передавайте весь вывод в эту команду. Обычно это `less` (например, `less -S` для отображения широких результирующих наборов) или аналогичная команда.                                                                            | -                                                                      |
...
| `--echo [ <bool> ]`               | Выводите каждый запрос перед выполнением. Принимает необязательное логическое значение.                                                                                                                                          | `true` в интерактивном режиме, `false` в неинтерактивном (батч) режиме |
| `--echo-formatted [ <bool> ]`     | Форматируйте выводимые запросы. Принимает необязательное логическое значение.                                                                                                                                                    | `true` в интерактивном режиме, `false` в неинтерактивном (батч) режиме |
| `--echo-query-id [ <bool> ]`      | Выводите Query id перед выполнением. Принимает необязательное логическое значение.                                                                                                                                               | `true` в интерактивном режиме, `false` в неинтерактивном (батч) режиме |
| `--highlight [ --hilite ] <bool>` | Включайте или отключайте подсветку синтаксиса в приглашении командной строки и выводимых запросах.                                                                                                                               | `true`                                                                 |
