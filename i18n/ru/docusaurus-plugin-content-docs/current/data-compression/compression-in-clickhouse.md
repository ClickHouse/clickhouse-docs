...
> Причины, по которым ClickHouse так хорошо сжимает данные, рассмотрены в [этой статье](https://clickhouse.com/blog/optimize-clickhouse-codecs-compression-schema). Вкратце: наша столбцовая база данных записывает значения по столбцам. Когда эти значения отсортированы, одинаковые значения оказываются рядом друг с другом, и алгоритмы сжатия используют непрерывные шаблоны в данных. Помимо этого, в ClickHouse есть кодеки и более детализированные типы данных, которые позволяют вам ещё проще и точнее настраивать сжатие.
...
Чтобы получить общий объём данных в таблице, мы можем упростить приведённый выше запрос:
...
Результаты сжатия для этих столбцов показаны ниже:
