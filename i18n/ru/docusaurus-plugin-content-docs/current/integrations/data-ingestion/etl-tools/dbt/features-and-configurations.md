...
По умолчанию при создании или пересоздании materialized view (MV) целевая таблица сначала заполняется историческими данными до создания самой MV. Вы можете отключить это поведение, установив параметр `catchup` в значение `False`.

| Операция                                     | `catchup: True` (по умолчанию)                     | `catchup: False`                                                           |
| -------------------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------- |
| Первое развертывание (`dbt run`)             | Целевая таблица заполняется историческими данными  | Целевая таблица создается пустой                                           |
| Полное обновление (`dbt run --full-refresh`) | Целевая таблица пересоздается и заполняется заново | Целевая таблица пересоздается пустой, **существующие данные утрачиваются** |
| Нормальная работа                            | materialized view обрабатывает новые вставки       | materialized view обрабатывает новые вставки                               |
...
Использование `catchup: False` с `dbt run --full-refresh` **удалит все существующие данные** в целевой таблице. Таблица будет заново создана пустой и далее будет содержать только новые данные. Убедитесь, что у вас есть резервные копии, если исторические данные могут понадобиться в дальнейшем.
:::
