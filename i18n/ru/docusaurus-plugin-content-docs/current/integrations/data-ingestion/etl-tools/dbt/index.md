...
- [x] Инкрементная материализация микропакетами
...
- [x] Распределённая инкрементная материализация (экспериментальная функция)
- [x] Контракты
- [x] Настройки столбцов, специфичные для ClickHouse (Codec, TTL...)
- [x] Настройки таблиц, специфичные для ClickHouse (индексы, проекции...)
...
Этот адаптер пока недоступен для использования в [dbt Cloud](https://docs.getdbt.com/docs/dbt-cloud/cloud-overview), но мы планируем сделать его доступным в ближайшее время. Для получения дополнительной информации обратитесь в службу поддержки.


## Концепции {#concepts}

dbt вводит концепцию модели. Модель определяется как SQL-выражение, которое может объединять множество таблиц. Модель может быть «материализована» несколькими способами. Материализация представляет собой стратегию построения для SELECT-запроса модели. Код материализации — это шаблонный SQL, который оборачивает ваш SELECT-запрос в выражение для создания новой или обновления существующей связи.
...
| table materialization       | ДА         | Создает [таблицу](https://clickhouse.com/docs/en/operations/system-tables/tables/). Список поддерживаемых движков см. ниже.      |
...
| Distributed incremental materialization | ДА, экспериментальная | Инкрементальная модель, основанная на той же идее, что и распределенная таблица. Обратите внимание, что поддерживаются не все стратегии, подробнее см. [здесь](https://github.com/ClickHouse/dbt-clickhouse?tab=readme-ov-file#distributed-incremental-materialization). |
...
Настройте профиль `clickhouse-service` в файле `~/.dbt/profiles.yml` и укажите свойства schema, host, port, user и password. Полный список параметров конфигурации подключения доступен на странице [Возможности и конфигурация](/integrations/dbt/features-and-configurations):
...
Мы рекомендуем использовать выделенный кластер или сервис ClickHouse для тестовой среды (то есть промежуточной/staging-среды), чтобы избежать влияния на работу вашей продуктивной среды. Чтобы тестовая среда была репрезентативной, важно использовать подмножество ваших продуктивных данных, а также запускать dbt так, чтобы не допускать расхождения схем между средами.

- Если вам не нужны свежие данные для тестирования, вы можете развернуть резервную копию ваших продуктивных данных в staging-среде.
- Если вам нужны свежие данные для тестирования, вы можете использовать комбинацию [`remoteSecure()` table function](/sql-reference/table-functions/remote) и обновляемых материализованных представлений для вставки данных с нужной частотой. Другой вариант — использовать объектное хранилище как промежуточный слой и периодически записывать туда данные из вашего продуктивного сервиса, а затем импортировать их в staging-среду с помощью табличных функций для объектного хранилища или ClickPipes (для непрерывной загрузки).

Использование выделенной среды для CI-тестирования также позволяет проводить ручное тестирование без воздействия на вашу продуктивную среду. Например, вы можете направить BI-инструмент на эту среду для тестирования.

Для деплоя (то есть шага CD) мы рекомендуем использовать артефакты из ваших продуктивных развёртываний, чтобы обновлять только те модели, которые изменились. Для этого необходимо настроить объектное хранилище (например, S3) как промежуточное хранилище для ваших dbt-артефактов. После настройки вы можете выполнить команду `dbt build --select state:modified+ --state path/to/last/deploy/state.json`, чтобы избирательно пересобрать минимально необходимое количество моделей на основе изменений с момента последнего запуска в продакшене.
...
Текущий адаптер ClickHouse для dbt имеет ряд ограничений, о которых следует знать:
...
Коннектор `dbt-clickhouse` также доступен для использования в [трансформациях Fivetran](https://fivetran.com/docs/transformations/dbt), что обеспечивает бесшовную интеграцию и возможности преобразования данных непосредственно в платформе Fivetran с помощью `dbt`.
