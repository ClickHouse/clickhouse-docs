...
**Вариант 1: поведение `--full-refresh` по умолчанию. Всё пересоздаётся, но во время пересоздания materialized view (MV) целевая таблица будет пустой или частично загруженной.**

Всё удаляется и пересоздаётся. Если вы хотите повторно вставить данные с помощью SQL materialized view (MV), оставьте настройку `catchup=True`:
...
Таким образом, вы не увидите пустых данных в своей таблице, пока MV пересоздаются.
...
* При создании refreshable materialized view (MV) в ClickHouse, у которой есть зависимость, ClickHouse не выдаёт
  ...
  не будет своевременно обеспечена. Перед созданием refreshable materialized view следует убедиться, что все
