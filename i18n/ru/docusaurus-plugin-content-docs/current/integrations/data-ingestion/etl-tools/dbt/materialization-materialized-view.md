...
По умолчанию dbt-clickhouse создаёт и управляет как целевой таблицей, так и materialized view в рамках одной модели (подход [неявная целевая таблица](#implicit-target), описанный выше). У этого подхода есть несколько ограничений:
...
Функция **явная целевая таблица** позволяет определить целевую таблицу отдельно как обычную материализацию `table`, а затем ссылаться на неё из ваших моделей materialized view.
...
* **Больше нет необходимости соблюдать соглашения об именовании**: теперь все ресурсы создаются с именем, которое вы задаёте, а не с добавленным суффиксом `_mv` для MVs.
...
| Operation                      | Internal process                                                                             | Safety while inserts are happening                                                                                                                                                                                                                                                                      |
| ------------------------------ | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First `dbt run`                | 1. Создать MV (с `TO`-предложением)<br />2. Выполнить догоняющее заполнение (если `catchup=True`) | ✅ MV создаётся первой, поэтому новые вставки сразу же попадают в неё.<br />⚠️ **Догоняющее заполнение может дублировать данные** — запрос дозаполнения может пересекаться со строками, которые уже обрабатываются MV. Безопасно при использовании дедуплицирующего движка (например, `ReplacingMergeTree`). |
...
| `dbt run --full-refresh` (по умолчанию)                               | Пересоздать таблицу (оставив её пустой)                                                                                 | ⚠️ **Целевая таблица пуста**, пока MV не заполнят её в ходе дозаполнения. MV продолжают вставлять данные в новую таблицу, как только она появляется.                    |
