...
description: 'Передавайте потоковые данные в ClickHouse с помощью конвейеров данных NiFi'
...
— это программное обеспечение с открытым исходным кодом для управления рабочими процессами, предназначенное для автоматизации потоков данных
между программными системами. Оно позволяет создавать конвейеры ETL и
поставляется с более чем 300 процессорами данных. Это пошаговое руководство показывает, как
подключить Apache NiFi к ClickHouse в качестве источника и приёмника данных, а также
...
## Загрузка и запуск Apache NiFi {#2-download-and-run-apache-nifi}

Для новой установки скачайте бинарный файл с https://nifi.apache.org/download.html и запустите его командой `./bin/nifi.sh start`


## Загрузка JDBC-драйвера ClickHouse {#3-download-the-clickhouse-jdbc-driver}
...
2. В разделе релиза нажмите на «Show all xx assets» и найдите JAR-файл, содержащий ключевое слово «shaded» или «all», например `clickhouse-jdbc-0.5.0-all.jar`
3. Поместите JAR-файл в папку, доступную для Apache NiFi, и запомните абсолютный путь к нему


## Добавление Controller Service `DBCPConnectionPool` и настройка его свойств {#4-add-dbcpconnectionpool-controller-service-and-configure-its-properties}

1. Чтобы настроить Controller Service в Apache NiFi, перейдите на страницу конфигурации потока NiFi, нажав на кнопку с изображением шестерёнки

   <Image
     img={nifi01}
     size='sm'
     border
     alt='Страница конфигурации потока NiFi с выделенной кнопкой шестерёнки'
   />

2. Выберите вкладку Controller Services и добавьте новый Controller Service, нажав на кнопку `+` в правом верхнем углу

   <Image
     img={nifi02}
     size='lg'
     border
     alt='Вкладка Controller Services с выделенной кнопкой добавления'
   />
...
3. Найдите `DBCPConnectionPool` и нажмите кнопку «Add»
   ...
   <Image
     ...
     size='lg'
     border
     alt='Диалоговое окно выбора Controller Service с выделенным DBCPConnectionPool'
   />

4. Только что добавленный `DBCPConnectionPool` по умолчанию будет находиться в состоянии Invalid. Нажмите на кнопку с изображением шестерёнки, чтобы начать настройку

   <Image
     img={nifi04}
     size='lg'
     border
     alt='Список Controller Services с недействительным DBCPConnectionPool и выделенной кнопкой шестерёнки'
   />
...
6. В разделе Settings измените имя Controller Service на «ClickHouse JDBC» для удобства
   ...
   <Image
     ...
     size='lg'
     border
     alt='Диалоговое окно настройки DBCPConnectionPool с заполненными свойствами'
   />

7. Активируйте Controller Service `DBCPConnectionPool`, нажав на кнопку с изображением молнии, а затем на кнопку «Enable»

   <Image
     img={nifi06}
     size='lg'
     border
     alt='Список Controller Services с выделенной кнопкой молнии'
   />
   ...
   <Image
     ...
     size='lg'
     border
     alt='Диалоговое окно подтверждения включения Controller Service'
   />
...
8. Проверьте вкладку Controller Services и убедитесь, что Controller Service включён
   ...
   <Image
     ...
     size='lg'
     border
     alt='Список Controller Services с включённым сервисом ClickHouse JDBC'
   />


## Чтение данных из таблицы с помощью процессора `ExecuteSQL` {#5-read-from-a-table-using-the-executesql-processor}
...
1. Добавьте процессор `ExecuteSQL` вместе с соответствующими предшествующими и последующими процессорами
   ...
   <Image
     ...
     size='md'
     border
     alt='Рабочая область NiFi с процессором ExecuteSQL в потоке обработки'
   />

2. В разделе «Properties» процессора `ExecuteSQL` укажите следующие значения

   | Свойство                            | Значение                      | Примечание                                              |
   | ----------------------------------- | ----------------------------- | ------------------------------------------------------- |
   | Database Connection Pooling Service | ClickHouse JDBC               | Выберите Controller Service, настроенный для ClickHouse |
   | SQL select query                    | SELECT \* FROM system.metrics | Укажите ваш запрос здесь                                |
...
3. Запустите процессор `ExecuteSQL`
   ...
   <Image
     ...
     size='lg'
     border
     alt='Конфигурация процессора ExecuteSQL с заполненными свойствами'
   />

4. Чтобы убедиться в успешной обработке запроса, проверьте один из объектов `FlowFile` в выходной очереди

   <Image
     img={nifi11}
     size='lg'
     border
     alt='Диалоговое окно списка очереди с объектами FlowFile, готовыми к проверке'
   />

5. Переключитесь на представление «formatted», чтобы просмотреть результат выходного объекта `FlowFile`

   <Image
     img={nifi12}
     size='lg'
     border
     alt='Просмотрщик содержимого FlowFile с результатами запроса в форматированном представлении'
   />
...
2. В разделе "Properties" процессора `MergeRecord` введите следующие значения
   ...
   | Свойство                  | Значение            | Примечание                                                                                                                 |
   | ------------------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------- |
   | Record Reader             | `JSONTreeReader`    | Выберите соответствующий обработчик чтения записей                                                                         |
   | Record Writer             | `JSONReadSetWriter` | Выберите соответствующий обработчик записи записей                                                                         |
...
4. В разделе "Properties" процессора `PutDatabaseRecord` введите следующие значения
   ...
   | Translate Field Names               | false            | Установите значение "false", чтобы имена вставляемых полей соответствовали именам столбцов                                                                       |
   | Maximum Batch Size                  | 1000             | Максимальное количество строк на одну операцию вставки. Это значение не должно быть меньше значения "Minimum Number of Records" в процессоре `MergeRecord`      |

5. Чтобы убедиться, что каждая операция вставки содержит несколько строк, проверьте, что количество строк в таблице увеличивается как минимум на значение "Minimum Number of Records", определенное в `MergeRecord`.
