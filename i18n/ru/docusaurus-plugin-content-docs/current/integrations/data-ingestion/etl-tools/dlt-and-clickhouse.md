...
```bash
# разместите это в начале вашего файла toml, перед началом любых секций.
...
2. Укажите HMAC-ключи, а также `client_email`, `project_id` и `private_key` для вашей учетной записи службы в настройках назначения ClickHouse вашего проекта dlt в файле `config.toml`:
