...
<a href="https://dlthub.com/docs/intro" target="_blank">dlt</a> — это библиотека с открытым исходным кодом, которую вы можете добавить в свои Python-скрипты, чтобы загружать данные из различных, зачастую «грязных» источников в хорошо структурированные, актуальные наборы данных.
...
:::note HTTP_PORT
Параметр `http_port` указывает номер порта, который следует использовать при подключении к HTTP-интерфейсу сервера ClickHouse. Это отличается от порта по умолчанию 9000, который используется для нативного протокола TCP.
...
```bash
# разместите это в начале вашего toml-файла, перед началом любой секции.
...
ClickHouse не поддерживает несколько наборов данных в одной базе данных, тогда как `dlt` полагается на наборы данных по ряду причин. Для обеспечения совместной работы ClickHouse с `dlt` имена таблиц, создаваемых `dlt` в вашей базе данных ClickHouse, будут иметь префикс с именем набора данных, отделённый настраиваемым разделителем `dataset_table_separator`. Кроме того, будет создана специальная сигнальная таблица без данных, позволяющая `dlt` определять, какие виртуальные наборы данных уже существуют в целевой базе данных ClickHouse.
...
4. `ClickHouse` допускает добавление ненулевых колонок в заполненную таблицу.
