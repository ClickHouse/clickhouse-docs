...
<a href="https://dlthub.com/docs/intro" target="_blank">dlt</a> — это библиотека с открытым исходным кодом, которую можно добавить в ваши Python-скрипты, чтобы загружать данные из различных, зачастую «шумных» источников в хорошо структурированные, актуальные наборы данных.



## Установка dlt с ClickHouse {#install-dlt-with-clickhouse}

### Установка библиотеки `dlt` с зависимостями для ClickHouse: {#to-install-the-dlt-library-with-clickhouse-dependencies}
...
3. Выполните следующие SQL-команды для создания новой базы данных, пользователя и предоставления необходимых прав доступа:
...
port = 9000                              # Порт ClickHouse для нативного протокола, по умолчанию 9000
...
Вы должны установить `http_port`, если не используете внешнее промежуточное хранилище (т.е. не устанавливаете параметр staging в вашем конвейере). Это связано с тем, что встроенное промежуточное хранилище ClickHouse использует библиотеку <a href="https://github.com/ClickHouse/clickhouse-connect">clickhouse-connect</a>, которая взаимодействует с ClickHouse по протоколу HTTP.
...
Данные загружаются в ClickHouse наиболее эффективным методом в зависимости от источника данных:
...
ClickHouse не поддерживает несколько наборов данных в одной базе данных, тогда как `dlt` использует наборы данных по ряду причин. Для обеспечения совместной работы ClickHouse с `dlt` таблицы, создаваемые `dlt` в вашей базе данных ClickHouse, будут иметь имена с префиксом в виде названия набора данных, отделённым настраиваемым параметром `dataset_table_separator`. Кроме того, будет создана специальная сигнальная таблица без данных, которая позволит `dlt` определять, какие виртуальные наборы данных уже существуют в целевой системе ClickHouse.
...
1. В `Clickhouse` есть экспериментальный тип данных `object`, но мы обнаружили, что он работает несколько непредсказуемо, поэтому назначение dlt clickhouse будет загружать сложные типы данных в текстовую колонку. Если вам нужна эта функция, свяжитесь с нашим сообществом в Slack, и мы рассмотрим возможность её добавления.
2. `Clickhouse` не поддерживает тип данных `time`. Время будет загружено в колонку типа `text`.
3. `Clickhouse` не поддерживает тип данных `binary`. Вместо этого бинарные данные будут загружены в колонку типа `text`. При загрузке из `jsonl` бинарные данные будут представлены в виде строки base64, а при загрузке из parquet объект `binary` будет преобразован в `text`.
4. `Clickhouse` позволяет добавлять ненулевые колонки в заполненную таблицу.
5. `Clickhouse` может создавать ошибки округления при определённых условиях при использовании типов данных float или double. Если вы не можете допустить ошибки округления, обязательно используйте тип данных decimal. Например, загрузка значения 12.7001 в колонку типа double при формате файла загрузчика `jsonl` предсказуемо приведёт к ошибке округления.
...
Табличная функция GCS в ClickHouse поддерживает только аутентификацию с использованием ключей HMAC (Hash-based Message Authentication Code). Для этого GCS предоставляет режим совместимости с S3, эмулирующий API Amazon S3. ClickHouse использует это для доступа к корзинам GCS через свою интеграцию с S3.
...
Примечание: В дополнение к ключам HMAC (`gcp_access_key_id` и `gcp_secret_access_key`) необходимо указать `client_email`, `project_id` и `private_key` для вашей сервисной учётной записи в разделе `[destination.filesystem.credentials]`. Это связано с тем, что поддержка промежуточного хранилища GCS в настоящее время реализована как временное решение и ещё не оптимизирована.

dlt передаст эти учётные данные в ClickHouse, который обработает аутентификацию и доступ к GCS.

Ведётся активная работа по упрощению и улучшению настройки промежуточного хранилища GCS для назначения ClickHouse в dlt. Полноценная поддержка промежуточного хранилища GCS отслеживается в следующих задачах GitHub:
