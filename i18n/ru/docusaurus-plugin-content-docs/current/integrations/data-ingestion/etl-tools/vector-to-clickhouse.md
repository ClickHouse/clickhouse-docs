...
<VerticalStepper headerLevel="h2">
  ...
  2. Вставьте всё событие лога как одну строку. Очевидно, что это не лучший формат для проведения аналитики по данным логов, но ниже мы разберёмся с этим, используя ***materialized views***.
  ...
  **ORDER BY** установлен в **tuple()** (пустой кортеж), так как первичный ключ пока не требуется.
  ...
  1. Следующая директива `access_log` записывает логи в `/var/log/nginx/my_access.log` в формате **combined**.
     Это значение указывается в разделе `http` файла `nginx.conf`:
  ...
  2. Обязательно перезапустите Nginx, если вы изменяли файл `nginx.conf`.

  3. Сгенерируйте несколько событий в журнале доступа (access log), переходя по страницам вашего веб-сервера.
  ...
  Vector собирает, преобразует и маршрутизирует логи, метрики и трейсы (далее — **источники**) в различные сервисы (далее — **приёмники**), включая встроенную «из коробки» совместимость с ClickHouse.
  ...
  2. Запустите Vector с приведённой выше конфигурацией. За дополнительной информацией об определении источников и приёмников обращайтесь к [документации](https://vector.dev/docs/) Vector.

  3. Убедитесь, что логи доступа записываются в ClickHouse, выполнив следующий запрос. В своей таблице вы должны увидеть эти логи доступа:

  ```sql
  SELECT * FROM nginxdb.access_logs
  ```

  <Image img={vector01} size="lg" border alt="Просмотр логов ClickHouse в табличном виде" />
  ...
  Далее мы рассмотрим, как разбирать события логов с помощью [materialized view](/materialized-view/incremental-materialized-view).

  **Materialized view** работает подобно триггеру на INSERT в SQL. Когда строки данных вставляются в исходную таблицу, materialized view выполняет над ними некоторые преобразования и вставляет результаты в целевую таблицу.
  Materialized view можно настроить для формирования разобранного представления событий логов в **access&#95;logs**.
  Пример одного такого события лога приведён ниже:
  ...
  В некоторых строках есть лишние символы, а user agent (сведения о браузере) не требовалось парсить, но
  ...
  Строка с датой начинается с символа `[` и при этом имеет формат, который ClickHouse не может разобрать как дату.
  ...
  Вы должны увидеть, что логи доступа аккуратно разобраны по столбцам:

  ```sql
  SELECT * FROM nginxdb.access_logs_view
  ```

  <Image img={vector02} size="lg" border alt="Просматривайте разобранные логи ClickHouse в виде таблицы" />

  :::note
  В приведённом выше примере данные сохранялись в двух таблицах, но вы можете изменить исходную таблицу `nginxdb.access_logs`, чтобы она использовала движок таблиц [`Null`](/engines/table-engines/special/null).
