...
description: 'Как в режиме реального времени передавать данные из лог-файла в ClickHouse с помощью Vector'
...
Это руководство показывает, как использовать популярный конвейер данных [Vector](https://vector.dev/docs/about/what-is-vector/) для чтения файла логов Nginx и отправки данных в ClickHouse.
Приведенные ниже шаги аналогичны для чтения любого типа файла логов.
...
Определите таблицу для хранения событий журнала:
...
2. Вставьте всё событие журнала как одну строку. Очевидно, что это не лучший формат для выполнения аналитики данных журнала, но мы разберёмся с этим ниже, используя **_материализованные представления_**.
...
Vector собирает, преобразует и маршрутизирует логи, метрики и трассировки (называемые **источниками**) к различным поставщикам (называемым **приёмниками**), включая готовую совместимость с ClickHouse.
...
2. Запустите Vector, используя приведённую выше конфигурацию. Для получения более подробной информации об определении источников и приёмников обратитесь к [документации](https://vector.dev/docs/) Vector.

3. Убедитесь, что логи доступа вставляются в ClickHouse, выполнив следующий запрос. В таблице должны отобразиться логи доступа:
...
Хранить логи в ClickHouse — это хорошо, но сохранение каждого события в виде одной строки не позволяет проводить полноценный анализ данных.
Далее мы рассмотрим, как разобрать события логов с помощью [материализованного представления](/materialized-view/incremental-materialized-view).

**Материализованное представление** работает аналогично триггеру вставки в SQL. Когда строки данных вставляются в исходную таблицу, материализованное представление выполняет преобразование этих строк и вставляет результаты в целевую таблицу.
Материализованное представление можно настроить для создания разобранного представления событий логов в **access_logs**.
...
В ClickHouse существуют различные функции для разбора приведённой выше строки. Функция [`splitByWhitespace`](/sql-reference/functions/splitting-merging-functions#splitByWhitespace) разбирает строку по пробельным символам и возвращает каждый токен в массиве.
...
Некоторые строки содержат дополнительные символы, и user agent (информация о браузере) не требовал разбора, но
...
Строка времени имеет ведущую квадратную скобку и находится в формате, который ClickHouse не может разобрать как дату.
Однако, если мы изменим разделитель с двоеточия (**:**) на пробел (**,**), то разбор будет работать отлично:
