...
10. Дополнительная информация: вы также можете просмотреть историю операций ClickPipe и журналы ошибок через интерфейс ClickHouse Cloud, чтобы отладить и мониторить процесс приёма данных.

11. **Поздравляем!** Вы успешно настроили свой первый ClickPipe. Если это потоковый ClickPipe, он будет постоянно работать, выполняя приём данных в реальном времени из удалённого источника данных. В противном случае приём будет выполнен пакетно и завершится.
