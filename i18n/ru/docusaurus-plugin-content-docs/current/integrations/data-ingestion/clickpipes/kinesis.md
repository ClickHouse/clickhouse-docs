...
Мы настоятельно рекомендуем ограничивать число одновременно активных сегментов так, чтобы оно соответствовало вашим требованиям по пропускной способности. Для потока Kinesis типа "On Demand" AWS автоматически назначит соответствующее число сегментов исходя из пропускной способности,
но для потоков типа "Provisioned" выделение слишком большого числа сегментов может приводить к задержкам, как описано ниже, а также к повышенным затратам, поскольку стоимость таких потоков в Kinesis рассчитывается по модели "за сегмент".

Если ваше приложение-продюсер непрерывно записывает в большое количество активных сегментов, это может вызывать задержки, если ваш pipe масштабирован недостаточно для эффективной обработки этих сегментов. Основываясь на лимитах пропускной способности Kinesis,
ClickPipes назначает определённое количество "workers" на реплику для чтения данных сегмента. Например, в наименьшей конфигурации реплика ClickPipes будет иметь 4 таких worker-потока. Если продюсер записывает
одновременно более чем в 4 сегмента, данные из "лишних" сегментов не будут обрабатываться, пока не освободится worker-поток. В частности, если pipe использует "enhanced fanout", каждый worker-поток будет подписан на
один сегмент на 5 минут и в течение этого времени не сможет читать другие сегменты. Это может вызывать всплески задержки, кратные 5 минутам.
