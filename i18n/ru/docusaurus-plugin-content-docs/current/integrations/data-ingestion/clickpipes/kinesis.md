...
description: 'Бесшовно подключайте свои источники данных Amazon Kinesis к ClickHouse Cloud.'
...
Вы ознакомились с [вступлением ClickPipes](./index.md) и настроили [учетные данные IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html) или [IAM роль](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html). Следуйте [руководству по роли контроль доступа на основе ролей для Kinesis](./secure-kinesis.md) для получения информации о том, как настроить роль, которая работает с ClickHouse Cloud.
...
4. Заполните форму, указав вашему ClickPipe имя, описание (необязательно), вашу IAM роль или учетные данные и другие сведения о подключении.
...
<Image img={cp_step3_kinesis} alt="Установка формата данных и темы" size="lg" border/>

6. На следующем шаге вы можете выбрать, хотите ли вы загружать данные в новую таблицу ClickHouse или использовать существующую. Следуйте инструкциям на экране для изменения имени вашей таблицы, схемы и настроек. Вы можете видеть реальный предварительный просмотр ваших изменений в примерной таблице вверху.
...
<Image img={cp_step4a3} alt="Установка расширенных элементов управления" size="lg" border/>

7. В качестве альтернативы вы можете решить загрузить свои данные в существующую таблицу ClickHouse. В этом случае интерфейс позволит вам сопоставить поля от источника с полями ClickHouse в выбранной целевой таблице.
...
9. Нажимая "Завершить настройку", система зарегистрирует ваш ClickPipe, и вы сможете увидеть его в итоговой таблице.
...
10. **Поздравляем!** вы успешно настроили свой первый ClickPipe. Если это потоковый ClickPipe, он будет постоянно работать, загружая данные в реальном времени из вашего удаленного источника данных. В противном случае он загрузит пакет и завершит работу.
...
|-------------------|-----------------------------------------------------------|--------------------------|
| _key              | Ключ раздела Kinesis                                      | String                   |
...
Поле _raw_message можно использовать в случаях, когда требуется только полная запись Kinesis в формате JSON (например, при использовании функций ClickHouse [`JsonExtract*`](/sql-reference/functions/json-functions#jsonextract-functions) для заполнения потокового материализованного представления). В таких трубах может улучшиться производительность ClickPipes за счет удаления всех «не виртуальных» столбцов.
...
Задержка (определяемая как время между отправкой сообщения Kinesis в поток и доступностью сообщения в ClickHouse) будет зависеть от ряда факторов (например, задержка Kinesis, задержка сети, размер/формат сообщения). [Пакетная загрузка](#batching), описанная в предыдущем разделе, также повлияет на задержку. Мы всегда рекомендуем тестировать ваш конкретный случай, чтобы понять, какую задержку вы можете ожидать.
