...
:::info
Загрузка данных из MySQL в ClickHouse Cloud с помощью ClickPipes находится в публичной бета-версии.
:::

Вы можете использовать ClickPipes для загрузки данных из исходной базы данных MySQL в ClickHouse Cloud. Исходная база данных MySQL может быть развернута в локальной инфраструктуре или в облаке с использованием таких сервисов, как Amazon RDS, Google Cloud SQL и другие.



## Предварительные требования {#prerequisites}

Для начала работы необходимо убедиться, что ваша база данных MySQL правильно настроена для репликации через binlog. Шаги настройки зависят от способа развертывания MySQL, поэтому воспользуйтесь соответствующим руководством ниже:
...
После настройки исходной базы данных MySQL можно приступить к созданию ClickPipe.
...
4. Заполните параметры подключения к исходной базе данных MySQL, которую вы настроили на этапе выполнения предварительных требований.
   ...
   :::info
   Перед добавлением параметров подключения убедитесь, что вы внесли IP-адреса ClickPipes в белый список правил брандмауэра. Список IP-адресов ClickPipes можно найти на [следующей странице](../index.md#list-of-static-ips).
...
3. Для использования аутентификации на основе ключей нажмите "Revoke and generate key pair", чтобы сгенерировать новую пару ключей, и скопируйте сгенерированный публичный ключ на SSH-сервер в файл `~/.ssh/authorized_keys`.
4. Нажмите "Verify Connection" для проверки подключения.

:::note
Убедитесь, что вы внесли [IP-адреса ClickPipes](../clickpipes#list-of-static-ips) в белый список правил брандмауэра для SSH-бастиона, чтобы ClickPipes мог установить SSH-туннель.
:::
...
- **Sync interval**: Интервал, с которым ClickPipes будет опрашивать исходную базу данных на предмет изменений. Этот параметр влияет на целевой сервис ClickHouse. Для пользователей, чувствительных к затратам, рекомендуется устанавливать более высокое значение (более `3600`).
- **Parallel threads for initial load**: Количество параллельных рабочих потоков для получения начального снимка данных. Этот параметр полезен при работе с большим количеством таблиц, когда требуется контролировать количество параллельных потоков для получения начального снимка. Параметр применяется для каждой таблицы.
- **Pull batch size**: Количество строк для получения в одном пакете. Это параметр с приоритетом наилучшего усилия, который может не соблюдаться во всех случаях.
- **Snapshot number of rows per partition**: Количество строк, которые будут получены в каждой партиции во время создания начального снимка. Этот параметр полезен при работе с таблицами, содержащими большое количество строк, когда требуется контролировать количество строк, получаемых в каждой партиции.
- **Snapshot number of tables in parallel**: Количество таблиц, которые будут получены параллельно во время создания начального снимка. Этот параметр полезен при работе с большим количеством таблиц, когда требуется контролировать количество таблиц, получаемых параллельно.
...
После настройки ClickPipe для репликации данных из MySQL в ClickHouse Cloud вы можете сосредоточиться на запросах и моделировании данных для достижения оптимальной производительности. Ответы на распространённые вопросы о MySQL CDC и устранении неполадок см. на [странице часто задаваемых вопросов по MySQL](/integrations/data-ingestion/clickpipes/mysql/faq.md).
