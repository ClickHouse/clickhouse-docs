...
description: 'Подробное описание поддерживаемых форматов, источников, семантики доставки, аутентификации и экспериментальных функций Kafka ClickPipes'
...
| ---------------- | ------------------------------------------------------------------------------------------- | --------- | ------ | ---------------------------------------------------------------------------------------------------- |
| Apache Kafka     | <Kafkasvg class="image" alt="Apache Kafka logo" style={{width: '3rem', 'height': '3rem'}}/> | Потоковая передача | Стабильно | Настройте ClickPipes и начните приём потоковых данных из Apache Kafka в ClickHouse Cloud.     |
| Confluent Cloud  | <Confluentsvg class="image" alt="Confluent Cloud logo" style={{width: '3rem'}}/>            | Потоковая передача | Стабильно | Используйте всю мощь совместной работы Confluent и ClickHouse Cloud благодаря прямой интеграции.          |
| Redpanda         | <Image img={redpanda_logo} size="logo" alt="Redpanda logo"/>                                | Потоковая передача | Стабильно | Настройте ClickPipes и начните приём потоковых данных из Redpanda в ClickHouse Cloud.         |
| AWS MSK          | <Msksvg class="image" alt="AWS MSK logo" style={{width: '3rem', 'height': '3rem'}}/>        | Потоковая передача | Стабильно | Настройте ClickPipes и начните приём потоковых данных из AWS MSK в ClickHouse Cloud.          |
| Azure Event Hubs | <Azureeventhubssvg class="image" alt="Azure Event Hubs logo" style={{width: '3rem'}}/>      | Потоковая передача | Стабильно | Настройте ClickPipes и начните приём потоковых данных из Azure Event Hubs в ClickHouse Cloud. |
| WarpStream       | <Warpstreamsvg class="image" alt="WarpStream logo" style={{width: '3rem'}}/>                | Потоковая передача | Стабильно | Настройте ClickPipes и начните приём потоковых данных из WarpStream в ClickHouse Cloud.       |
...
- Map с ключами и значениями любых из вышеперечисленных типов (включая Nullable)
- Tuple и Array с элементами любых из вышеперечисленных типов (включая Nullable, только один уровень вложенности)
