...
description: 'Лучшие практики работы с Kafka ClickPipes'
...
Если вы выполняете аутентификацию в MSK с использованием ARN роли IAM, необходимо добавить доверительные отношения между вашим экземпляром ClickHouse Cloud, чтобы роль могла быть принята.
...
ClickPipes вставляет данные в ClickHouse пакетами. Это позволяет избежать создания слишком большого количества партов в базе данных, что может привести к проблемам с производительностью кластера.
...
- Тестовые данные включали вложенные данные со смешанными типами данных `UUID`, `String` и `Int`. Другие типы данных, такие как `Float`, `Decimal` и `DateTime`, могут быть менее производительными.
