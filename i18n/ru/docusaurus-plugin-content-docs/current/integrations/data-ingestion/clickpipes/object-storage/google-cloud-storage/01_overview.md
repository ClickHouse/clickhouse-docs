...
##### Настройка уведомлений Pub/Sub \{#pubsub-setup\}

Чтобы использовать неупорядоченный режим, необходимо настроить автоматические уведомления из бакета GCS в тему Pub/Sub. Следуйте [официальной документации](https://docs.cloud.google.com/storage/docs/pubsub-notifications) по уведомлениям Pub/Sub, чтобы создать тему и подписку Pub/Sub, а затем настроить уведомления для бакета.
...
##### Предоставление прав учетной записи службы \{#pubsub-permissions\}
...
##### Настройка ClickPipe \{#pubsub-clickpipe-config\}
...
<Image img={gcs_subscription_input} alt="Неупорядоченный режим для GCS" size="lg" border />
