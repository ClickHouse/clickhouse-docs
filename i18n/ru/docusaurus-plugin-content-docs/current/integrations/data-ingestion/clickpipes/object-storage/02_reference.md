...
description: 'Подробная информация о поддерживаемых форматах, семантике exactly-once, поддержке представлений, масштабировании, ограничениях и аутентификации ClickPipes при работе с объектным хранилищем'
...
ClickPipes для объектного хранилища масштабируются на основе минимального размера сервиса ClickHouse, определяемого [настройками вертикального автомасштабирования](/manage/scaling#configuring-vertical-auto-scaling). Размер ClickPipe определяется при создании конвейера. Последующие изменения настроек сервиса ClickHouse не влияют на размер ClickPipe.
...
- ClickPipes будет пытаться загружать только объекты размером 10 ГБ или меньше. Если размер файла превышает 10 ГБ, ошибка будет добавлена в выделенную таблицу ошибок ClickPipes.
...
Чтобы использовать роль IAM, создайте её согласно инструкциям [в этом руководстве](/cloud/data-sources/secure-s3). После создания скопируйте ARN новой роли IAM и вставьте его в конфигурацию ClickPipe в поле «IAM ARN role».
...
В настоящее время для Azure Blob Storage поддерживаются только защищённые бакеты. Аутентификация выполняется через строку подключения, которая поддерживает ключи доступа и общие ключи. Дополнительную информацию см. в [этом руководстве](https://learn.microsoft.com/en-us/azure/storage/common/storage-configure-connection-string).
