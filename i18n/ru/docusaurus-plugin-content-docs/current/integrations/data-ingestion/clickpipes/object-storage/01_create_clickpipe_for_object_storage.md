---
sidebar_label: 'Создайте первый ClickPipe для объектного хранилища'
...
ClickPipes для объектных хранилищ обеспечивают простой и отказоустойчивый способ загрузки данных из Amazon S3, Google Cloud Storage, Azure Blob Storage и DigitalOcean Spaces в ClickHouse Cloud. Поддерживается как однократная, так и непрерывная загрузка с семантикой «ровно один раз» (exactly-once).
...
В облачной консоли выберите кнопку `Data Sources` в меню слева и нажмите на «Set up a ClickPipe»
...
Дополнительную информацию см. в [документации по использованию подстановочных символов в пути](/integrations/clickpipes/object-storage/reference/#limitations).
...
Выберите формат данных (в настоящее время поддерживается подмножество форматов ClickHouse) и укажите, требуется ли включить непрерывную загрузку данных.
...
Альтернативный вариант — загрузить данные в существующую таблицу ClickHouse.
...
## Настройка разрешений {#6-configure-permissions}

Наконец, можно настроить разрешения для внутреннего пользователя ClickPipes.

**Разрешения:** ClickPipes создаст выделенного пользователя для записи данных в целевую таблицу. Вы можете выбрать роль для этого внутреннего пользователя, используя пользовательскую роль или одну из предопределённых ролей:

- `Full access`: полный доступ к кластеру. Требуется при использовании материализованного представления или словаря с целевой таблицей.
- `Only destination table`: разрешения `INSERT` только для целевой таблицы.
...
А также элементы управления для удаления ClickPipe и отображения сводки задачи загрузки данных.

<Image img={cp_overview} alt='Просмотр обзора' size='lg' border />

**Поздравляем!** Вы успешно настроили свой первый ClickPipe.
Если это потоковый ClickPipe, он будет непрерывно работать, загружая данные в режиме реального времени из вашего удалённого источника данных.
В противном случае он загрузит пакет данных и завершит работу.
