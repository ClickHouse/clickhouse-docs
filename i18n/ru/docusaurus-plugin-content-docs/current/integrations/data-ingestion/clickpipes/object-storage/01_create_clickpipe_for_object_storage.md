---
sidebar_label: 'Создание первого ClickPipe для объектного хранилища'
...
ClickPipes для объектного хранилища предоставляют простой и надежный способ загрузки данных из Amazon S3, Google Cloud Storage, Azure Blob Storage и DigitalOcean Spaces в ClickHouse Cloud. Поддерживаются как разовая, так и непрерывная загрузка с семантикой «ровно один раз».
...
В облачной консоли выберите кнопку `Data Sources` в левом меню и нажмите «Set up a ClickPipe»
...
Предварительный просмотр изменений в реальном времени отображается в примере таблицы вверху экрана.
...
**Права доступа:** ClickPipes создаст выделенного пользователя для записи данных в целевую таблицу. Вы можете выбрать роль для этого внутреннего пользователя, используя пользовательскую роль или одну из предопределённых ролей:
...
Если это потоковый ClickPipe, он будет непрерывно работать, загружая данные в реальном времени из вашего удалённого источника данных.
