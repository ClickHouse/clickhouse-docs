...
**1.** Настройте [сервисный аккаунт](http://docs.cloud.google.com/iam/docs/keys-create-delete) с [необходимыми правами доступа](/integrations/clickpipes/object-storage/gcs/overview#permissions), чтобы разрешить ClickPipes перечислять и получать объекты в указанном бакете, а также получать и отслеживать уведомления из подписки Pub/Sub.
