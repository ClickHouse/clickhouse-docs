...
keywords: ['clickpipes', 'kinesis', 'потоковая передача', 'aws', 'ингестия данных', 'сжатие', 'gzip', 'zstd', 'lz4', 'snappy']
integration:
  - support_level: 'core'
  - category: 'clickpipes'
---
...
Kinesis ClickPipes можно развертывать и управлять ими вручную через интерфейс ClickPipes, а также программно с помощью [OpenAPI](/integrations/clickpipes/programmatic-access/openapi) и [Terraform](/integrations/clickpipes/programmatic-access/terraform).
...
ClickPipes для Kinesis автоматически определяет и распаковывает сжатые записи. В отличие от Kafka, где клиентская библиотека прозрачно выполняет распаковку, Kinesis передаёт необработанные байты — ClickPipes делает это за вас без какой-либо дополнительной конфигурации.
...
* **zstd**
* **lz4**
* **snappy** (фреймовый формат)

Сжатие определяется автоматически по сигнатуре magic bytes в каждой записи. Если известная сигнатура сжатия не найдена, запись считается несжатой. Обнаруженный тип сжатия также отображается при определении схемы, поэтому предварительный просмотр образца данных в интерфейсе будет корректно показывать распакованные данные.

:::note
Автоопределение безопасно для текстовых форматов, таких как JSON и CSV, поскольку печатные символы ASCII никогда не совпадут с magic bytes сжатия.
:::
