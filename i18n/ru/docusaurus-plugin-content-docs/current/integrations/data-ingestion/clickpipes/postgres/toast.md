...
Важно отметить, что при захвате изменений данных (CDC) неизменённые TOAST-столбцы не включаются в поток репликации. Это может привести к неполной репликации данных, если не обрабатывать эту ситуацию должным образом.

Во время начальной загрузки (snapshot) все значения столбцов, включая TOAST-столбцы, реплицируются корректно независимо от их размера. Ограничения, описанные в этом руководстве, в первую очередь касаются непрерывного процесса CDC после начальной загрузки.
...
Этот запрос вернёт имена и типы данных столбцов, которые потенциально могут быть подвергнуты TOAST-обработке. Однако важно отметить, что данный запрос лишь определяет столбцы, подходящие для TOAST-хранения на основе их типа данных и атрибутов хранения. Чтобы определить, действительно ли эти столбцы содержат данные, обработанные TOAST, необходимо проверить, превышают ли значения в этих столбцах пороговый размер. Фактическая TOAST-обработка данных зависит от конкретного содержимого, хранящегося в этих столбцах.
...
Ознакомьтесь с [этой статьей в блоге](https://xata.io/blog/replica-identity-full-performance), чтобы узнать о влиянии на производительность при установке `REPLICA IDENTITY FULL`.
...
Правильная обработка TOAST-столбцов необходима для обеспечения целостности данных при репликации из PostgreSQL в ClickHouse. Определив TOAST-столбцы и установив соответствующий параметр `REPLICA IDENTITY`, вы сможете обеспечить точную и полную репликацию данных.
