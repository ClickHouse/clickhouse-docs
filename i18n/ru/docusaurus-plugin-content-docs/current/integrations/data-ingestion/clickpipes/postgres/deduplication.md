...
Обновления и удаления, реплицируемые из Postgres в ClickHouse, приводят к дублированию строк в ClickHouse из‑за его структуры хранения данных и процесса репликации. На этой странице объясняется, почему это происходит, и рассматриваются стратегии работы с дубликатами в ClickHouse.
...
ClickPipes сопоставляет таблицы Postgres с ClickHouse, используя движок [ReplacingMergeTree](/engines/table-engines/mergetree-family/replacingmergetree). ClickHouse показывает наилучшую производительность при работе с нагрузками типа append-only и не рекомендует частые операции UPDATE. Именно здесь ReplacingMergeTree проявляет свою особую эффективность.

В ReplacingMergeTree обновления моделируются как вставки с более новой версией (`_peerdb_version`) строки, а удаления — как вставки с более новой версией и `_peerdb_is_deleted`, помеченным как true. Движок ReplacingMergeTree дедуплицирует и объединяет данные в фоновом режиме, сохраняя последнюю версию строки для заданного первичного ключа (id), что обеспечивает эффективную обработку операций UPDATE и DELETE в виде версионированных вставок.
...
**Шаг 3** показывает, как ClickPipes реплицирует операции INSERT, UPDATE и DELETE в ClickHouse в виде версионированных вставок. UPDATE отображается как новая версия строки с ID 2, а DELETE — как новая версия ID 1, помеченная как true с помощью `_is_deleted`. Из-за этого в ClickHouse на три строки больше по сравнению с PostgreSQL.
...
Затем можно выполнять запросы к представлениям, используя те же запросы, что и в PostgreSQL.
...
Затем можно выполнять обычные запросы к таблице `deduplicated_posts`.
