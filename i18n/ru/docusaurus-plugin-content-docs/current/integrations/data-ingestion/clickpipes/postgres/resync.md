...
4. Затем таблицы `_resync` заменяются на исходные таблицы. Мягко удалённые строки переносятся из исходных таблиц в таблицы `_resync` перед заменой.
...
1. На вкладке «Источники данных» нажмите на Postgres ClickPipe, который вы хотите повторно синхронизировать.
...
6. Примерно через 5 секунд (а также при обновлении страницы) статус конвейера должен быть **Setup** или **Snapshot**.
...
9. После завершения замены конвейер перейдёт в состояние **Running** и будет выполнять CDC, если это включено.
