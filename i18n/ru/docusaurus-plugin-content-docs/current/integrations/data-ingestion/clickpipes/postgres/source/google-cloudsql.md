...
Если вы используете одного из поддерживаемых провайдеров (в боковой панели), обратитесь к соответствующему руководству по работе с ним.
...
**Вам не требуется** выполнять приведенные ниже шаги, если параметр `cloudsql.logical_decoding` включен, а `wal_sender_timeout` равен 0. Эти параметры, как правило, уже настроены, если вы мигрируете с другого инструмента репликации данных.
...
2. Перейдите в раздел Flags и измените значение `cloudsql.logical_decoding` на on, а `wal_sender_timeout` — на 0. Для применения этих изменений потребуется перезапуск сервера Postgres.
...
Выполните следующие действия, чтобы добавить IP-адреса ClickPipes в вашу сеть.

:::note

Если вы используете SSH-туннелирование, необходимо добавить [IP-адреса ClickPipes](../../index.md#list-of-static-ips) в правила брандмауэра Jump Server/Bastion.
