...
Если вы используете одного из поддерживаемых провайдеров (в боковой панели), обратитесь к отдельному руководству для этого провайдера.
...
2. Кроме того, рекомендуется установить на экземпляре Postgres следующие параметры:
...
Вы можете увеличить значение параметра `max_slot_wal_keep_size` для вашего экземпляра PostgreSQL (минимум до 100 ГБ или `102400`), обновив файл `postgresql.conf`.
