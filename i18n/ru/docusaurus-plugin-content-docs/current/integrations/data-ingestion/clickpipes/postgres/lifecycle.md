...
:::
Это состояние указывает, что pipe находится в фазе повторной синхронизации, в ходе которой выполняется атомарная замена таблиц _resync на исходные таблицы. Дополнительную информацию о повторной синхронизации см. в [документации по повторной синхронизации](./resync).
...
Если в конвейере возникает неустранимая ошибка, он переходит в состояние `Failed`. Вы можете обратиться в службу поддержки или [повторно синхронизировать](./resync) конвейер, чтобы вывести его из этого состояния.