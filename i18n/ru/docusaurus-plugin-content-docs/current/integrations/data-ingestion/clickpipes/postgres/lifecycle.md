...
Этот документ описывает различные фазы работы Postgres ClickPipe, возможные статусы и их значения.
...
После завершения настройки система переходит в состояние `Snapshot` (если это не конвейер только для CDC, который перейдёт в состояние `Running`). Термины `Snapshot`, `Initial Snapshot` и `Initial Load` (наиболее распространённый) являются взаимозаменяемыми. В этом состоянии создаётся снимок таблиц исходной базы данных и выполняется их загрузка в ClickHouse. При этом не используется логическая репликация, однако слот репликации создаётся на данном этапе, поэтому параметры `max_slot_wal_keep_size` и хранилища должны учитывать рост слота во время начальной загрузки. Дополнительную информацию о начальной загрузке см. в [документации по параллельной начальной загрузке](./parallel_initial_load). Конвейер также перейдёт в состояние `Snapshot` при запуске повторной синхронизации или при добавлении новых таблиц в существующий конвейер.
