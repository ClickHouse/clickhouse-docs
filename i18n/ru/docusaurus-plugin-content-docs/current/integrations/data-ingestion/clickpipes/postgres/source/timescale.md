...
которое призвано повысить производительность аналитических запросов без необходимости миграции с Postgres.
Это достигается путем
создания «гипертаблиц», которые управляются расширением и поддерживают автоматическое разбиение на «чанки».
Гипертаблицы также поддерживают прозрачное сжатие и гибридное строчно-столбцовое хранилище (известное как «hypercore»), однако эти
...
Существуют сторонние поставщики, предлагающие управляемые сервисы, которые позволяют использовать расширение TimescaleDB, однако из-за
...
**выполняемую по мере возможности**.
...
Необходимые действия зависят от способа развертывания вашего экземпляра Postgres с TimescaleDB.
...
Для других управляемых сервисов создайте запрос в службу поддержки вашего провайдера для получения помощи в включении логической репликации, если она еще не включена.

:::info
Timescale Cloud не поддерживает включение логической репликации, которая необходима для каналов Postgres в режиме CDC.
...
1. Создайте пользователя Postgres для конвейера и предоставьте ему права на `SELECT` для таблиц, которые требуется реплицировать.

```sql
  CREATE USER clickpipes_user PASSWORD 'clickpipes_password';
  GRANT USAGE ON SCHEMA "public" TO clickpipes_user;
  -- При необходимости можно ограничить эти права GRANT только отдельными таблицами, а не всей схемой
  -- Но при добавлении новых таблиц в ClickPipe их также потребуется добавить для пользователя.
...
2. От имени суперпользователя/администратора Postgres создайте публикацию на исходном экземпляре, которая содержит таблицы и гипертаблицы
   для репликации, и **также включает всю схему `_timescaledb_internal`**. При создании ClickPipe необходимо выбрать эту публикацию.

```sql
-- При добавлении новых таблиц в ClickPipe их также потребуется вручную добавить в публикацию.
  CREATE PUBLICATION clickpipes_publication FOR TABLE <...>, <...>, TABLES IN SCHEMA _timescaledb_internal;
```

:::tip
Не рекомендуется создавать публикацию `FOR ALL TABLES`, так как это приводит к увеличению трафика от Postgres к ClickPipes (отправке изменений для других таблиц, не входящих в конвейер) и снижает общую эффективность.

Для публикаций, созданных вручную, добавьте все необходимые таблицы в публикацию перед добавлением их в конвейер.
:::

:::info
Некоторые управляемые сервисы не предоставляют своим пользователям-администраторам необходимые права для создания публикации для всей схемы.
Если это ваш случай, обратитесь в службу поддержки вашего провайдера. В качестве альтернативы можно пропустить этот и следующие
...
После выполнения этих шагов можно приступить к [созданию ClickPipe](../index.md).
