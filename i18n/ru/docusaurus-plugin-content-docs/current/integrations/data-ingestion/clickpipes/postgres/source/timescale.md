...
title: 'Руководство по настройке источника Postgres с TimescaleDB'
...
для повышения производительности аналитических запросов без необходимости миграции с Postgres. Это достигается путём
создания «гипертаблиц» (hypertables), которые управляются расширением и поддерживают автоматическое разбиение на «чанки» (chunks).
...
возможности доступны только в версии расширения с проприетарной лицензией.
...
Существуют сторонние поставщики, предлагающие управляемые сервисы с поддержкой расширения TimescaleDB, однако из-за
лицензионных ограничений эти поставщики поддерживают только версию расширения с открытым исходным кодом.

Гипертаблицы Timescale ведут себя иначе, чем обычные таблицы Postgres, в нескольких аспектах. Это создаёт определённые сложности
при их репликации, поэтому возможность репликации гипертаблиц Timescale следует рассматривать как
**наилучшую попытку** (best effort).
...
- Если вы развертываете TimescaleDB самостоятельно, следуйте общей инструкции.
...
ClickPipe для Postgres содержит логику автоматического переназначения изменений из таблиц-чанков в родительскую гипертаблицу,
...
```sql
  ...
  -- При необходимости можно ограничить эти GRANT только отдельными таблицами, а не всей схемой
  -- Но при добавлении новых таблиц в ClickPipe их также нужно будет добавить пользователю.
...
```sql
-- При добавлении новых таблиц в ClickPipe их также нужно будет вручную добавить в публикацию.
...
Для публикаций, созданных вручную, добавляйте нужные таблицы в публикацию перед добавлением их в пайп.
:::
...
3. Предоставьте права репликации созданному ранее пользователю.

```sql
-- Предоставить права репликации пользователю
