...
description: 'Как задавать пользовательские ключи сортировки.'
...
Ключи сортировки (Ordering Keys, также известные как sorting keys) определяют, как данные упорядочиваются на диске и индексируются в таблице ClickHouse. При репликации из Postgres ClickPipes по умолчанию использует первичный ключ таблицы в Postgres как ключ сортировки для соответствующей таблицы в ClickHouse. В большинстве случаев первичного ключа Postgres достаточно в качестве ключа сортировки, так как ClickHouse уже оптимизирован для быстрых сканирований, и пользовательские ключи сортировки часто не требуются.
