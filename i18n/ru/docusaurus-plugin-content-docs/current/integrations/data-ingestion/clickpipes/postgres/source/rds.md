...
Если вы хотите ограничить трафик к вашему экземпляру RDS, добавьте [документированные статические NAT IP-адреса](../../index.md#list-of-static-ips) в правила `Inbound rules` группы безопасности вашего RDS.
...
Для подключения к вашему экземпляру RDS через частную сеть можно использовать AWS PrivateLink. Следуйте нашему [руководству по настройке AWS PrivateLink для ClickPipes](/knowledgebase/aws-privatelink-setup-for-clickpipes), чтобы настроить соединение.
...
1. С помощью задания cron периодически определяйте IP-адрес конечной точки RDS и обновляйте NLB при его изменении.
2. С помощью уведомлений о событиях RDS с EventBridge/SNS: автоматически запускайте обновления, используя уведомления о событиях AWS RDS.
