...
| DigitalOcean Spaces                                | <DOsvg class="image" alt="Логотип Digital Ocean" style={{width: '3rem', height: 'auto'}} />                    | Объектное хранилище | Стабильно                       | Настройте ClickPipes для приёма больших объёмов данных из объектного хранилища.                                                                                                                           |
...
| [GCP Pub/Sub](/integrations/clickpipes/pubsub)     | <GoogleCloudPubSub class="image" alt="Логотип Google Cloud Pub/Sub" style={{width: '3rem', height: 'auto'}} /> | Streaming           | Публичная бета                  | Настройте ClickPipes и начните приём потоковых данных из Google Cloud Pub/Sub в ClickHouse Cloud.                                                                                                         |
...
Ниже в таблицах перечислены статические NAT IP-адреса, которые ClickPipes использует для подключения к вашим внешним сервисам. Добавьте IP-адреса региона ClickPipes, обслуживающего ваш сервис ClickHouse Cloud, в список разрешённых IP-адресов. В случае пайпов Объектного хранилища вам также следует добавить [IP-адреса кластера ClickHouse](/manage/data-sources/cloud-endpoints-api) в список разрешённых IP-адресов.

Сервисы в регионах Google Cloud, перечисленных в таблице Google Cloud ниже, используют эти IP-адреса Google Cloud только в том случае, если сервис был создан 27 мая 2026 г. или позднее. Сервисы в этих регионах, созданные до 27 мая 2026 г., продолжают использовать IP-адреса региона по умолчанию, перечисленные ниже.

Для других сервисов трафик ClickPipes будет исходить из региона по умолчанию, определяемого на основе расположения вашего сервиса:

* **eu-central-1**: Для всех регионов ЕС, явно не перечисленных, а также для регионов Azure в ЕС и сервисов Google Cloud в ЕС, созданных до 27 мая 2026 г.
...
* **mx-central-1**: Для сервисов в AWS `mx-central-1`, созданных 19 мая 2026 г. или позднее (сервисы, созданные до этой даты, используют IP-адреса `us-east-2`).
...
* **us-east-2**: Для всех остальных регионов, не подпадающих под правила выше, включая регионы Azure и сервисы Google Cloud, созданные до 27 мая 2026 г.

### Статические NAT IP-адреса в AWS \{#aws-static-nat-ips\}
...
### Статические NAT IP-адреса Google Cloud \{#google-cloud-static-nat-ips\}

| Регион Google Cloud                            | IP-адреса                                                                             |
| ---------------------------------------------- | ------------------------------------------------------------------------------------- |
| **asia-northeast1** - Токио (с 27 мая 2026)    | `104.198.114.210`, `35.221.66.81`, `35.243.126.127`, `136.110.107.86`, `34.85.18.112` |
| **asia-southeast1** - Сингапур (с 27 мая 2026) | `34.21.197.28`, `35.197.141.23`, `35.197.157.90`, `136.110.17.200`, `35.185.179.231`  |
| **europe-west2** - Лондон (с 27 мая 2026)      | `35.242.131.178`, `34.39.77.101`, `34.39.47.179`, `34.89.53.234`, `8.228.63.151`      |
| **europe-west4** - Нидерланды (с 27 мая 2026)  | `34.34.86.3`, `34.6.175.56`, `34.178.6.187`, `34.91.204.220`, `34.12.85.206`          |
| **us-central1** - Айова (с 27 мая 2026)        | `34.28.24.54`, `34.42.56.195`, `34.63.141.9`, `35.238.146.37`, `34.10.251.49`         |
| **us-east1** - Южная Каролина (с 27 мая 2026)  | `34.24.134.232`, `34.24.214.165`, `34.24.20.1`, `35.243.193.248`, `34.23.98.76`       |
