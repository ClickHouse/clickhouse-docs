...
description: 'Легко подключайте ваши внешние источники данных к ClickHouse Cloud.'
...
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Apache Kafka](/integrations/clickpipes/kafka)     | <Kafkasvg class="image" alt="Apache Kafka logo" style={{width: '3rem', 'height': '3rem'}}/>      | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из Apache Kafka в ClickHouse Cloud.                                                                                                         |
| Confluent Cloud                                    | <Confluentsvg class="image" alt="Confluent Cloud logo" style={{width: '3rem'}}/>                 | Потоковый      | Стабильный      | Используйте совместные возможности Confluent и ClickHouse Cloud благодаря нашей прямой интеграции.                                                                                                    |
| Redpanda                                           | <Image img={redpanda_logo} size="logo" alt="Redpanda logo"/>                                     | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из Redpanda в ClickHouse Cloud.                                                                                                             |
| AWS MSK                                            | <Msksvg class="image" alt="AWS MSK logo" style={{width: '3rem', 'height': '3rem'}}/>             | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из AWS MSK в ClickHouse Cloud.                                                                                                              |
| Azure Event Hubs                                   | <Azureeventhubssvg class="image" alt="Azure Event Hubs logo" style={{width: '3rem'}}/>           | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из Azure Event Hubs в ClickHouse Cloud. См. раздел [Azure Event Hubs FAQ](/integrations/clickpipes/kafka/faq/#azure-eventhubs) для получения рекомендаций. |
| WarpStream                                         | <Warpstreamsvg class="image" alt="WarpStream logo" style={{width: '3rem'}}/>                     | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из WarpStream в ClickHouse Cloud.                                                                                                           |
...
| [Amazon Kinesis](/integrations/clickpipes/kinesis) | <Amazonkinesis class="image" alt="Amazon Kenesis logo" style={{width: '3rem', height: 'auto'}}/> | Потоковый      | Стабильный      | Настройте ClickPipes и начните загружать потоковые данные из Amazon Kinesis в ClickHouse Cloud.                                                                                                       |
...
| [MySQL](/integrations/clickpipes/mysql)            | <Mysqlsvg class="image" alt="MySQL logo" style={{width: '3rem', height: '3rem'}}/>               | СУБД           | Публичная бета  | Настройте ClickPipes и начните загружать данные из MySQL в ClickHouse Cloud.                                                                                                                          |
...
В ClickPipes со временем появится больше коннекторов; узнать подробности можно, [связавшись с нами](https://clickhouse.com/company/contact?loc=clickpipes).


## Список статических IP-адресов {#list-of-static-ips}

Ниже приведены статические NAT IP-адреса (по регионам), которые ClickPipes использует для подключения к вашим внешним сервисам. Добавьте IP-адреса региона вашего экземпляра в список разрешённых IP-адресов для разрешения трафика.
...
ClickHouse Cloud предоставляет оптимальные значения по умолчанию для большинства сценариев использования. Однако если вам необходимо настроить некоторые параметры ClickHouse для целевых таблиц ClickPipes, наиболее гибким решением является выделенная роль для ClickPipes.
Шаги:
...
ClickPipes предоставляет разумные значения по умолчанию, которые покрывают требования большинства сценариев использования. Если ваш сценарий требует дополнительной тонкой настройки, вы можете изменить следующие параметры:
...
| `Object storage polling interval`    | 30s           | Задает максимальный период ожидания перед вставкой данных в кластер ClickHouse.                                                       |
| `Parallel distributed insert select` | 2             | [Настройка параллельной распределенной вставки с выборкой](/operations/settings/settings#parallel_distributed_insert_select).                             |
| `Parallel view processing`           | false         | Включать ли отправку данных в присоединенные представления [параллельно вместо последовательно](/operations/settings/settings#parallel_view_processing). |
...
| Параметр                         | Значение по умолчанию | Описание                                                                           |
| -------------------------------- | ------------- | ------------------------------------------------------------------------------------- |
| `Streaming max insert wait time` | 5s            | Задает максимальный период ожидания перед вставкой данных в кластер ClickHouse. |
...
ClickPipes создаёт таблицу рядом с целевой таблицей с постфиксом `<destination_table_name>_clickpipes_error`. Эта таблица содержит все ошибки, связанные с некорректными данными или несоответствием схемы, и включает полное содержимое невалидного сообщения. Для этой таблицы установлен [TTL](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-ttl) 7 дней.

### Системные ошибки {#system-errors}

Ошибки, связанные с работой ClickPipe, сохраняются в таблице `system.clickpipes_log`. В ней хранятся все прочие ошибки, связанные с работой вашего ClickPipe (сеть, подключение и т. д.). Для этой таблицы установлен [TTL](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-ttl) 7 дней.
...
- **Поддерживает ли ClickPipes преобразование данных?**
  ...
  Да, ClickPipes поддерживает базовое преобразование данных через создание DDL. Вы можете применить более сложные преобразования к данным при их загрузке в целевую таблицу сервиса ClickHouse Cloud, используя [материализованные представления](/guides/developer/cascading-materialized-views) ClickHouse.

- **Влечет ли использование ClickPipes дополнительные расходы?**

  ClickPipes тарифицируется по двум параметрам: объем загруженных данных и вычислительные ресурсы. Полная информация о ценах доступна на [этой странице](/cloud/reference/billing/clickpipes). Работа ClickPipes также может создавать косвенные затраты на вычислительные ресурсы и хранение в целевом сервисе ClickHouse Cloud, как и любая другая нагрузка по загрузке данных.

- **Как обрабатываются ошибки и сбои при использовании ClickPipes для Kafka?**

  Да, ClickPipes для Kafka автоматически повторяет попытки при возникновении сбоев во время получения данных из Kafka при любых операционных проблемах, включая проблемы с сетью, подключением и т. д. В случае некорректных данных или недопустимой схемы ClickPipes сохранит запись в таблице record_error и продолжит обработку.
