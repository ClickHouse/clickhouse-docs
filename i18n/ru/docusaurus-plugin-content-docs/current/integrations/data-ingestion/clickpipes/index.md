...
doc_type: 'guide'
keywords: ['ClickPipes', 'платформа приема данных', 'потоковые данные', 'интеграционная платформа', 'ClickHouse Cloud']
---
...
| Confluent Cloud                                    | <Confluentsvg class="image" alt="Confluent Cloud logo" style={{width: '3rem'}}/>                 | Потоковая передача      | Стабильный          | Используйте всю мощь совместной работы Confluent и ClickHouse Cloud благодаря прямой интеграции.                                                                                                            |
...
| Azure Event Hubs                                   | <Azureeventhubssvg class="image" alt="Azure Event Hubs logo" style={{width: '3rem'}}/>           | Потоковая передача      | Стабильный          | Настройте ClickPipes и начните загружать потоковые данные из Azure Event Hubs в ClickHouse Cloud. Инструкции см. в разделе [FAQ по Azure Event Hubs](/integrations/clickpipes/kafka/faq/#azure-eventhubs). |
...
| Azure Blob Storage                                 | <ABSsvg class="image" alt="Azure Blob Storage logo" style={{width: '3rem', height: 'auto'}}/>    | Объектное хранилище | Стабильный          | Настройте ClickPipes для загрузки больших объемов данных из объектного хранилища.                                                                                                                              |
...
Для всех сервисов трафик ClickPipes будет исходить из региона по умолчанию в зависимости от расположения вашего сервиса:
...
| `Parallel distributed insert select` | 2             | [Параметр параллельной распределенной вставки с выборкой](/operations/settings/settings#parallel_distributed_insert_select).                             |
| `Parallel view processing`           | false         | Определяет, следует ли отправлять данные в присоединенные представления [параллельно, а не последовательно](/operations/settings/settings#parallel_view_processing). |
...
ClickPipes сохраняет ошибки в двух отдельных таблицах в зависимости от типа ошибки, возникшей в процессе приема данных.

### Ошибки записей {#record-errors}

ClickPipes создает таблицу рядом с целевой таблицей с постфиксом `<destination_table_name>_clickpipes_error`. Эта таблица содержит ошибки, связанные с некорректными данными или несоответствием схемы, и включает полное содержимое недопустимого сообщения. Для этой таблицы установлен [TTL](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-ttl) 7 дней.
...
- **Что такое ClickPipes?**
  ...
  ClickPipes — это функция ClickHouse Cloud, которая упрощает подключение сервисов ClickHouse к внешним источникам данных, в частности к Kafka. С помощью ClickPipes для Kafka пользователи могут легко организовать непрерывную загрузку данных в ClickHouse, обеспечивая их доступность для аналитики в реальном времени.

- **Поддерживает ли ClickPipes преобразование данных?**

  Да, ClickPipes поддерживает базовое преобразование данных через предоставление доступа к созданию DDL. Затем вы можете применять более сложные преобразования к данным при их загрузке в целевую таблицу в сервисе ClickHouse Cloud, используя [функцию материализованных представлений](/guides/developer/cascading-materialized-views) ClickHouse.

- **Влечет ли использование ClickPipes дополнительные расходы?**

  ClickPipes тарифицируется по двум параметрам: объем загруженных данных и вычислительные ресурсы. Полная информация о ценообразовании доступна на [этой странице](/cloud/reference/billing/clickpipes). Работа ClickPipes также может приводить к косвенным затратам на вычислительные ресурсы и хранение данных в целевом сервисе ClickHouse Cloud, как и любая другая рабочая нагрузка по загрузке данных.

- **Существует ли способ обработки ошибок или сбоев при использовании ClickPipes для Kafka?**

  Да, ClickPipes для Kafka автоматически повторяет попытки в случае сбоев при получении данных из Kafka при любых операционных проблемах, включая проблемы с сетью, проблемы с подключением и т. д. В случае некорректных данных или недопустимой схемы ClickPipes сохранит запись в таблице record_error и продолжит обработку.
