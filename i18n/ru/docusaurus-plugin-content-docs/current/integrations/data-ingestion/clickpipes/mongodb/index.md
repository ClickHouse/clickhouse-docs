...
description: 'Описывает, как легко подключить вашу MongoDB к ClickHouse Cloud.'
...
:::info
Загрузка данных из MongoDB в ClickHouse Cloud с помощью ClickPipes находится в стадии публичного бета-тестирования.
:::

:::note
В консоли и документации ClickHouse Cloud для MongoDB термины «table» и «collection» используются как синонимы.
:::
...
- **Pull batch size**: Количество строк для извлечения в одном пакете. Это параметр с приоритетом наилучшего результата, который может не соблюдаться во всех случаях.
- **Snapshot number of tables in parallel**: Количество таблиц, которые будут извлекаться параллельно во время создания начального снимка. Это полезно при работе с большим количеством таблиц, когда требуется контролировать количество таблиц, извлекаемых параллельно.
...
После настройки ClickPipe для репликации данных из MongoDB в ClickHouse Cloud вы можете сосредоточиться на запросах и моделировании данных для оптимальной производительности.
...
- Документы из MongoDB по умолчанию реплицируются в ClickHouse с типом JSON. Это обеспечивает гибкое управление схемой и позволяет использовать богатый набор операторов JSON в ClickHouse для выполнения запросов и аналитики. Подробнее о работе с JSON-данными можно узнать [здесь](https://clickhouse.com/docs/sql-reference/data-types/newjson).
- Самостоятельная настройка PrivateLink в настоящее время недоступна. Если вы используете AWS и вам требуется PrivateLink, обратитесь по адресу db-integrations-support@clickhouse.com или создайте тикет в службу поддержки — мы поможем вам с настройкой.
