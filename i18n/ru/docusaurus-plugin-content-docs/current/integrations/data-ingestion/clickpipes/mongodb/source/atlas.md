...
Для репликации требуется минимальное время хранения oplog в 24 часа. Рекомендуется устанавливать время хранения oplog на 72 часа или более, чтобы гарантировать, что oplog не будет усечён до завершения начального снимка. Чтобы настроить время хранения oplog через пользовательский интерфейс:
...
ClickPipes требуется пользователь со следующими ролями:
