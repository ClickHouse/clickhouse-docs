...
description: 'Типовые подходы к работе с JSON-данными, реплицируемыми из MongoDB в ClickHouse с помощью ClickPipes'
doc_type: 'guide'
...
В этом руководстве рассматриваются типовые паттерны работы с JSON‑данными, реплицируемыми из MongoDB в ClickHouse с помощью ClickPipes.
...
Коннектор CDC для MongoDB реплицирует документы MongoDB в ClickHouse, используя родной тип данных JSON. Реплицированная таблица `t1` в ClickHouse будет содержать следующую строку:
...
ClickPipes отображает коллекции MongoDB в ClickHouse с использованием семейства движков таблиц `ReplacingMergeTree`. При использовании этого движка обновления моделируются как вставки с более новой версией (`_peerdb_version`) документа для заданного первичного ключа (`_id`), что обеспечивает эффективную обработку обновлений, замен и удалений в виде версионированных вставок.
...
Удаления из MongoDB передаются как новые строки, помеченные как удаленные с помощью столбца `_peerdb_is_deleted`. Обычно их необходимо отфильтровывать в запросах:
...
Чтобы узнать базовый тип данных для поля, используйте функцию `dynamicType`. Обратите внимание, что для одного и того же имени поля в разных строках могут быть разные типы данных:
...
:::note
Приведение из типа Dynamic к базовому типу данных (определяемому функцией `dynamicType`) выполняется очень эффективно, поскольку ClickHouse уже хранит значение во внутреннем представлении базового типа.
:::
...
Теперь вы можете запрашивать представление аналогично тому, как вы запрашиваете плоскую таблицу:
...
Ключевое преимущество этого метода заключается в том, что запрос с использованием ключевого слова `FINAL` выполняется только один раз во время обновления, устраняя необходимость использования `FINAL` в последующих запросах к целевой таблице.
