...
Для прямого доступа к полям, таким как `{"user_id": 123}`, используйте **точечную нотацию**:
...
Подробнее о работе с JSON см. в нашем [руководстве по работе с JSON](./quickstart).
...
### Можно ли подключаться к базам данных MongoDB без публичного IP-адреса или находящимся в частных сетях? {#can-i-connect-mongodb-databases-that-dont-have-a-public-ip-or-are-in-private-networks}
...
### Что произойдет при удалении базы данных или таблицы из базы данных MongoDB? {#what-happens-if-i-delete-a-database-table-from-my-mongodb-database}
