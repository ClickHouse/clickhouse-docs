...
keywords: ['Amazon S3', 'объектное хранилище', 'облачное хранилище', 'озеро данных', 'интеграция S3']
doc_type: 'guide'
integration:
  - support_level: 'core'
...
Вы можете загружать данные из S3 в ClickHouse, а также использовать S3 в качестве целевого хранилища при экспорте, что позволяет интегрироваться с архитектурами типа «Data Lake». Кроме того, S3 может обеспечивать уровни «холодного» хранилища и помогать в разделении хранения и вычислительных ресурсов. В разделах ниже мы используем датасет поездок на такси в Нью‑Йорке, чтобы продемонстрировать процесс переноса данных между S3 и ClickHouse, а также выделить ключевые параметры конфигурации и дать рекомендации по оптимизации производительности.
...
- path — URL бакета с путём к файлу. В режиме только для чтения поддерживаются следующие подстановочные символы: `*`, `?`, `{abc,def}` и `{N..M}`, где `N`, `M` — числа, `'abc'`, `'def'` — строки. Подробнее см. в документации по [использованию подстановочных символов в пути](/engines/table-engines/integrations/s3/#wildcards-in-path).
...
Результат выполнения оператора `DESCRIBE TABLE` покажет, как ClickHouse автоматически определяет структуру данных из бакета S3. Обратите внимание, что ClickHouse также автоматически распознаёт и распаковывает формат сжатия gzip:
...
Проверим количество строк в этом образце данных. Обратите внимание на использование подстановочных символов для раскрытия файлов — таким образом учитываются все двадцать файлов. Выполнение этого запроса займет около 10 секунд в зависимости от количества ядер экземпляра ClickHouse:
...
Хотя чтение данных непосредственно из S3 полезно для выборки данных и выполнения разовых исследовательских запросов, это не то, что следует делать регулярно. Когда приходит время для серьезной работы, импортируйте данные в таблицу `MergeTree` в ClickHouse.
...
Чтобы использовать все возможности ClickHouse, далее мы читаем и вставляем данные в наш экземпляр.
Для этого мы объединяем функцию `s3` с простым оператором `INSERT`. Обратите внимание, что нам не требуется перечислять столбцы, поскольку целевая таблица предоставляет необходимую структуру. Это требует, чтобы столбцы располагались в порядке, указанном в DDL-операторе таблицы: столбцы сопоставляются в соответствии с их позицией в предложении `SELECT`. Вставка всех 10 млн строк может занять несколько минут в зависимости от экземпляра ClickHouse. Ниже мы вставляем 1 млн строк для обеспечения быстрого ответа. При необходимости настройте предложение `LIMIT` или выбор столбцов для импорта подмножеств данных:
...
Если политики сетевой безопасности не позволяют вашему кластеру ClickHouse устанавливать исходящие соединения, вы можете вставить данные из S3 с помощью `clickhouse-local`. В приведенном ниже примере мы читаем данные из корзины S3 и вставляем их в ClickHouse с помощью функции `remote`:
...
Вы можете записывать данные в файлы в S3 с помощью табличной функции `s3`. Для этого потребуются соответствующие разрешения. Мы передаем необходимые учетные данные в запросе, но для получения дополнительных вариантов см. страницу [Управление учетными данными](#managing-credentials).

В приведенном ниже простом примере мы используем табличную функцию в качестве назначения, а не источника. Здесь мы передаем 10 000 строк из таблицы `trips` в корзину, указывая сжатие `lz4` и тип вывода `CSV`:
...
Маловероятно, что вы захотите экспортировать данные в один файл. Большинство инструментов, включая ClickHouse, достигают более высокой пропускной способности при чтении и записи нескольких файлов благодаря возможности параллелизма. Мы могли бы выполнить команду `INSERT` несколько раз, обрабатывая подмножество данных. ClickHouse предоставляет механизм автоматического разделения файлов с использованием ключа `PARTITION`.

В примере ниже мы создаём десять файлов, используя остаток от деления функции `rand()`. Обратите внимание, как полученный идентификатор партиции используется в имени файла. В результате получается десять файлов с числовым суффиксом, например `trips_0.csv.lz4`, `trips_1.csv.lz4` и т.д.:
...
Все вышеперечисленные функции ограничены выполнением на одном узле. Скорость чтения масштабируется линейно с количеством ядер CPU до тех пор, пока не будут исчерпаны другие ресурсы (обычно сеть), что позволяет пользователям выполнять вертикальное масштабирование. Однако этот подход имеет свои ограничения. Хотя пользователи могут снизить нагрузку на ресурсы, вставляя данные в распределённую таблицу при выполнении запроса `INSERT INTO SELECT`, это всё равно оставляет один узел для чтения, парсинга и обработки данных. Для решения этой задачи и обеспечения горизонтального масштабирования чтения существует функция [s3Cluster](/sql-reference/table-functions/s3Cluster.md).

Узел, который получает запрос, известный как инициатор, создаёт соединение с каждым узлом в кластере. Шаблон glob, определяющий, какие файлы необходимо прочитать, преобразуется в набор файлов. Инициатор распределяет файлы между узлами кластера, которые выступают в роли рабочих узлов. Эти рабочие узлы, в свою очередь, запрашивают файлы для обработки по мере завершения чтения. Этот процесс обеспечивает горизонтальное масштабирование чтения.
...
- `access_key_id` и `secret_access_key` — ключи, которые указывают учётные данные для использования с данной конечной точкой. Необязательные параметры.
- `format` — [формат](/interfaces/formats#formats-overview) файла.
- `structure` — структура таблицы. Формат 'column1_name column1_type, column2_name column2_type, ...'.

Как и для любых функций `s3`, учётные данные являются необязательными, если бакет незащищён или вы определяете безопасность через окружение, например, роли IAM. Однако, в отличие от функции s3, начиная с версии 22.3.1 структура должна быть указана в запросе, то есть схема не выводится автоматически.
...
Вставки выполняются на узле-инициаторе. Это означает, что хотя чтение выполняется на каждом узле, результирующие строки перенаправляются узлу-инициатору для распределения. В сценариях с высокой пропускной способностью это может стать узким местом. Чтобы устранить это, установите параметр [parallel&#95;distributed&#95;insert&#95;select](/operations/settings/settings/#parallel_distributed_insert_select) для функции `s3cluster`.


## Табличные движки S3 {#s3-table-engines}

Хотя функции `s3` позволяют выполнять ad-hoc запросы к данным, хранящимся в S3, их синтаксис довольно громоздкий. Табличный движок `S3` избавляет от необходимости каждый раз указывать URL бакета и учетные данные. Для этого ClickHouse предоставляет табличный движок S3.
...
В следующем примере создается таблица `trips_raw` на основе первых десяти TSV-файлов, расположенных в бакете `https://datasets-documentation.s3.eu-west-3.amazonaws.com/nyc-taxi/`. Каждый файл содержит 1 млн строк:
...
Обратите внимание на использование шаблона `{0..9}` для ограничения первыми десятью файлами. После создания эту таблицу можно запрашивать как любую другую:
...
Для демонстрации записи создайте таблицу, указывающую на доступный для записи бакет S3:
...
Обратите внимание, что строки можно вставлять только в новые файлы. Циклы слияния и операции разбиения файлов отсутствуют. После того как файл записан, последующие вставки будут завершаться с ошибкой. У пользователей здесь есть два варианта:

* Указать настройку `s3_create_new_file_on_insert=1`. Это приведёт к созданию новых файлов при каждой вставке. К концу каждого файла будет добавлен числовой суффикс, который будет монотонно увеличиваться при каждой операции вставки. Для приведённого выше примера последующая вставка приведёт к созданию файла trips&#95;1.bin.
* Указать настройку `s3_truncate_on_insert=1`. Это приведёт к усечению файла, то есть по завершении он будет содержать только вновь вставленные строки.

Обе эти настройки по умолчанию равны 0 — таким образом, пользователь вынужден выбрать одну из них. Если заданы обе, приоритет будет у `s3_truncate_on_insert`.

Некоторые замечания о движке таблиц `S3`:

* В отличие от таблиц семейства `MergeTree`, удаление таблицы `S3` не приведёт к удалению лежащих в её основе данных.
* Полный перечень настроек для этого типа таблиц можно найти [здесь](/engines/table-engines/integrations/s3.md/#settings).
* Имейте в виду следующие ограничения при использовании этого движка:
  * Запросы ALTER не поддерживаются
  * Операции SAMPLE не поддерживаются
  * Индексы отсутствуют — ни первичные, ни пропускающие.


## Управление учетными данными {#managing-credentials}

В предыдущих примерах мы передавали учетные данные в функции `s3` или определении таблицы `S3`. Хотя это может быть приемлемо для эпизодического использования, в производственной среде требуются менее явные механизмы аутентификации. Для решения этой задачи ClickHouse предоставляет несколько вариантов:

- Укажите параметры подключения в файле **config.xml** или эквивалентном конфигурационном файле в каталоге **conf.d**. Содержимое примера файла показано ниже (предполагается установка с использованием пакета debian).
  ...
  Эти учетные данные будут использоваться для любых запросов, где указанная выше конечная точка точно совпадает с префиксом запрашиваемого URL. Также обратите внимание на возможность объявить заголовок авторизации в качестве альтернативы ключам доступа и секретным ключам. Полный список поддерживаемых настроек можно найти [здесь](/engines/table-engines/integrations/s3.md/#settings).
...
Внутри движок MergeTree в ClickHouse использует два основных формата хранения: [`Wide` и `Compact`](/engines/table-engines/mergetree-family/mergetree.md/#mergetree-data-storage). Хотя текущая реализация использует стандартное поведение ClickHouse (управляемое настройками `min_bytes_for_wide_part` и `min_rows_for_wide_part`), ожидается, что в будущих версиях поведение для S3 будет отличаться, например, большее значение по умолчанию для `min_bytes_for_wide_part` будет способствовать более частому использованию формата `Compact` и, следовательно, уменьшению количества файлов. Пользователи могут уже сейчас настроить эти параметры при использовании исключительно хранилища S3.


## MergeTree с хранением в S3 {#s3-backed-mergetree}

Функции `s3` и соответствующий движок таблиц позволяют запрашивать данные в S3, используя привычный синтаксис ClickHouse. Однако они имеют ограничения в части функций управления данными и производительности. Отсутствует поддержка первичных индексов и кэширования, а вставка файлов должна управляться пользователем.

ClickHouse признаёт, что S3 представляет собой привлекательное решение для хранения данных, особенно когда производительность запросов к «холодным» данным менее критична, а пользователи стремятся разделить хранение и вычисления. Для достижения этой цели предоставляется поддержка использования S3 в качестве хранилища для движка MergeTree. Это позволяет пользователям воспользоваться преимуществами масштабируемости и экономической эффективности S3, а также производительностью вставки и запросов движка MergeTree.
...
Уровни хранения открывают возможности для архитектур «горячих-холодных» данных, где самые свежие данные, которые обычно запрашиваются чаще всего, требуют лишь небольшого объёма пространства на высокопроизводительном хранилище, например NVMe SSD. По мере старения данных увеличиваются допустимые значения SLA для времени выполнения запросов, а частота запросов снижается. Этот «длинный хвост» данных может храниться на более медленном, менее производительном хранилище, таком как HDD или объектное хранилище, например S3.
...
Полный список настроек, относящихся к объявлению этого диска, можно найти [здесь](/engines/table-engines/mergetree-family/mergetree.md/#table_engine-mergetree-s3). Обратите внимание, что учётными данными можно управлять, используя те же подходы, описанные в разделе [Управление учётными данными](#managing-credentials), то есть параметр use_environment_credentials может быть установлен в true в приведённом выше блоке настроек для использования ролей IAM.

### Создание политики хранения {#creating-a-storage-policy}

После настройки этот «диск» может использоваться томом хранения, объявленным в рамках политики. В приведённом ниже примере мы предполагаем, что s3 является нашим единственным хранилищем. Это не учитывает более сложные архитектуры «горячих-холодных» данных, где данные могут перемещаться на основе TTL и степени заполнения.
...
Предполагая, что вы настроили диск для использования корзины с правами на запись, вы сможете создать таблицу, как показано в примере ниже. Для краткости мы используем подмножество столбцов набора данных NYC taxi и передаём данные напрямую в таблицу с хранением в S3:
...
В зависимости от оборудования вставка 1 млн строк может занять несколько минут. Вы можете отслеживать прогресс через таблицу system.processes. При желании можно увеличить количество строк до 10 млн и выполнить несколько примеров запросов.
...
* По умолчанию максимальное количество потоков обработки запроса, используемых на любой стадии конвейера обработки запросов, равно количеству ядер. Некоторые стадии лучше поддаются параллелизации, чем другие, поэтому это значение задаёт верхнюю границу. Несколько стадий запроса могут выполняться одновременно, так как данные считываются с диска потоковым образом. Поэтому фактическое число потоков, используемых для запроса, может превышать это значение. Параметр можно изменить с помощью настройки [max_threads](/operations/settings/settings#max_threads).
* Операции чтения из S3 по умолчанию выполняются асинхронно. Такое поведение определяется настройкой `remote_filesystem_read_method`, которая по умолчанию имеет значение `threadpool`. При обслуживании запроса ClickHouse читает гранулы «полосами» (stripes). Каждая из таких полос потенциально содержит много столбцов. Поток последовательно читает столбцы для своих гранул. Вместо выполнения этого синхронно выполняется предварительная выборка (prefetch) всех столбцов ещё до ожидания данных. Это даёт значительный выигрыш в производительности по сравнению с синхронным ожиданием для каждого столбца. В большинстве случаев пользователям не потребуется изменять эту настройку — см. [Оптимизация производительности](#s3-optimizing-performance).
* Запись выполняется параллельно, с максимум 100 одновременными потоками записи файлов. Параметр `max_insert_delayed_streams_for_parallel_write`, по умолчанию равный 1000, контролирует количество объектов (blob) в S3, записываемых параллельно. Поскольку для каждого записываемого файла требуется буфер (~1MB), это фактически ограничивает потребление памяти операцией INSERT. В сценариях с небольшим объёмом памяти сервера может быть целесообразно уменьшить это значение.



## Использование объектного хранилища S3 в качестве диска ClickHouse {#configuring-s3-for-clickhouse-use}

Если вам необходимы пошаговые инструкции по созданию корзин и роли IAM, разверните раздел **Создание корзин S3 и роли IAM** и следуйте указаниям:

<BucketDetails />

### Настройка ClickHouse для использования корзины S3 в качестве диска {#configure-clickhouse-to-use-the-s3-bucket-as-a-disk}

Следующий пример основан на установке пакета Linux Deb в качестве службы с каталогами ClickHouse по умолчанию.
...
2. Добавьте следующую конфигурацию хранилища, подставив путь к корзине, ключ доступа и секретный ключ из предыдущих шагов
...
Приведенная выше конфигурация предназначена для ClickHouse версии 22.8 или выше. Если вы используете более раннюю версию, обратитесь к документации по [хранению данных](/operations/storing-data.md/#using-local-cache).

Для получения дополнительной информации об использовании S3:
Руководство по интеграции: [MergeTree на основе S3](#s3-backed-mergetree)
:::
...
2 строки в выборке. Время выполнения: 0,284 с.
...
Если вам требуются пошаговые инструкции по созданию корзин и роли IAM, разверните раздел **Создание корзин S3 и роли IAM** и следуйте указаниям:

<BucketDetails />

Файлы конфигурации размещаются в каталоге `/etc/clickhouse-server/config.d/`. Ниже приведен пример файла конфигурации для одной корзины. Конфигурация для второй корзины аналогична, отличаются только три выделенные строки:
...
:::note
Во многих шагах данного руководства вам потребуется размещать файлы конфигурации в каталоге `/etc/clickhouse-server/config.d/`. Это стандартное расположение файлов переопределения конфигурации в системах Linux. При размещении файлов в этом каталоге ClickHouse использует их содержимое для переопределения конфигурации по умолчанию. Размещая файлы в каталоге переопределения, вы предотвратите потерю конфигурации при обновлении.
:::
...
`server_id` указывает идентификатор, который будет присвоен хосту, где используется конфигурационный файл. В примере ниже `server_id` равен `3`, и если посмотреть дальше в файле в секцию `<raft_configuration>`, можно увидеть, что сервер 3 имеет имя хоста `keepernode3`. Именно так процесс ClickHouse Keeper определяет, к каким другим серверам подключаться при выборе лидера и выполнении всех остальных операций.
...
Скопируйте конфигурационный файл для ClickHouse Keeper в нужное место (не забудьте установить `<server_id>`):
...
Кластеры ClickHouse определяются в секции `<remote_servers>` конфигурации. В данном примере определен один кластер `cluster_1S_2R`, который состоит из одного шарда с двумя репликами. Реплики расположены на хостах `chnode1` и `chnode2`.
...
:::note
Приведенные выше макросы предназначены для `chnode1`, на `chnode2` установите для `replica` значение `replica_2`.
:::
...
Этот параметр должен иметь значение false по двум причинам: 1) данная функция не готова к использованию в промышленной эксплуатации; 2) в сценарии аварийного восстановления как данные, так и метаданные должны храниться в нескольких регионах. Установите для `allow_remote_fs_zero_copy_replication` значение `false`.
...
При настройке параметров безопасности в AWS обратитесь к списку [сетевых портов](../../../guides/sre/network-ports.md), чтобы серверы могли взаимодействовать друг с другом, а вы могли подключаться к ним.

Все три сервера должны принимать сетевые соединения, чтобы они могли взаимодействовать между собой и с S3. По умолчанию ClickHouse принимает соединения только на адресе обратной петли, поэтому это необходимо изменить. Настройка выполняется в `/etc/clickhouse-server/config.d/`. Ниже приведен пример конфигурации ClickHouse и ClickHouse Keeper для прослушивания на всех интерфейсах IPv4. Дополнительную информацию см. в документации или в файле конфигурации по умолчанию `/etc/clickhouse/config.xml`.
...
1 строка в наборе. Время выполнения: 0.012 сек.
...
* Убедитесь, что данные хранятся в S3.
  ...
  Проверьте размер данных на локальном диске. Из вывода выше видно, что размер данных на диске для миллионов сохранённых строк составляет 36.42 MiB. Эти данные должны находиться в S3, а не на локальном диске. Запрос выше также показывает, где на локальном диске хранятся данные и метаданные. Проверьте локальные данные:
  ...
  Проверьте данные в S3 в каждом бакете S3 (итоговые значения не показаны, но в обоих бакетах после вставок хранится примерно по 36 MiB):
...
[S3Express](https://aws.amazon.com/s3/storage-classes/express-one-zone/) — это новый высокопроизводительный класс хранения в Amazon S3, работающий в пределах одной зоны доступности.
...
2. Установите соответствующую политику бакета для предоставления всех необходимых разрешений вашему пользователю S3 (например, `"Action": "s3express:*"` для предоставления неограниченного доступа)
3. При настройке политики хранения укажите параметр `region`

Конфигурация хранилища аналогична конфигурации для обычного S3 и, например, может выглядеть следующим образом:
...
Можно сохранить резервную копию на диске, созданном выше:
