...
5 строк в результате. Время выполнения: 0.360 сек. Обработано 24.37 миллиона строк, 140.45 МБ (67.73 миллиона строк/с., 390.38 МБ/с.)
Пиковое использование памяти: 510.71 МБ.
```
...
5 строк в результате. Время выполнения: 0.908 сек. Обработано 59.82 миллиона строк, 1.45 ГБ (65.87 миллиона строк/с., 1.59 ГБ/с.)
```
...
По возможности пользователи должны использовать агрегатные функции ClickHouse. Ниже мы демонстрируем использование функции [argMax](/sql-reference/aggregate-functions/reference/argmax) для вычисления наиболее просматриваемого вопроса каждого года.
...
17 строк в результате. Время выполнения: 0.677 сек. Обработано 24.37 миллиона строк, 1.86 ГБ (36.01 миллиона строк/с., 2.75 ГБ/с.)
Пиковое использование памяти: 554.31 МБ.
```
...
5 строк в результате. Время выполнения: 0.247 сек. Обработано 5.08 миллиона строк, 155.73 МБ (20.58 миллиона строк/с., 630.61 МБ/с.)
Пиковое использование памяти: 403.04 МБ.
```
...
Это завершает наше базовое руководство для пользователей, мигрирующих с Postgres на ClickHouse. Рекомендуем пользователям, мигрирующим с Postgres, ознакомиться с [руководством по моделированию данных в ClickHouse](/data-modeling/schema-design), чтобы узнать больше о расширенных функциях ClickHouse.
