...
description: 'Страница, на которой описаны различные способы подключения PostgreSQL к ClickHouse'
...
- использование движка таблицы `PostgreSQL` для чтения из таблицы PostgreSQL;
...
В качестве альтернативы [PeerDB](https://github.com/PeerDB-io/peerdb) доступен как инструмент CDC с открытым исходным кодом, специально разработанный для репликации баз данных PostgreSQL как в самостоятельно управляемый ClickHouse, так и в ClickHouse Cloud.
:::
...
5. Давайте посмотрим, что происходит при добавлении строк в таблицу ClickHouse:
...
**_В приведенных ниже процедурах используются PostgreSQL CLI (psql) и ClickHouse CLI (clickhouse-client). Сервер PostgreSQL установлен на Linux. Далее приведены минимальные настройки для новой тестовой установки базы данных PostgreSQL_**
...
_\*Для ClickHouse требуется как минимум уровень WAL `logical` и как минимум `2` слота репликации_
...
:::info
Дополнительные возможности для расширенных параметров см. в [справочной документации](/engines/database-engines/materialized-postgresql).
:::
