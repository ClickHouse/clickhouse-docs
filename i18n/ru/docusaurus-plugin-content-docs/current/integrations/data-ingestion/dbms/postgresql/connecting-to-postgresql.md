...
В качестве альтернативы [PeerDB](https://github.com/PeerDB-io/peerdb) доступен как open source‑инструмент CDC, специально разработанный для репликации баз данных PostgreSQL как в самоуправляемый ClickHouse, так и в ClickHouse Cloud.
:::
...
Движок таблиц `PostgreSQL` позволяет выполнять операции **SELECT** и **INSERT** над данными, хранящимися на удалённом сервере PostgreSQL, из ClickHouse.
В этой статье показаны базовые методы интеграции на примере одной таблицы.

### 1. Настройка PostgreSQL {#1-setting-up-postgresql}

1.  В файле `postgresql.conf` добавьте следующую запись, чтобы PostgreSQL принимал подключения по сетевым интерфейсам:
...
6. Чтобы настроить PostgreSQL на разрешение подключений к новой базе данных с новым пользователем для репликации, добавьте следующую запись в файл `pg_hba.conf`. Укажите в строке адреса подсеть или IP-адрес вашего сервера PostgreSQL:
...
_\*Для ClickHouse требуется минимум уровень WAL `logical` и минимум `2` слота репликации_
...
:::info
Полное руководство по движку базы данных PostgreSQL см. на https://clickhouse.com/docs/engines/database-engines/materialized-postgresql/#settings
:::
...
:::info
Дополнительные возможности для продвинутых параметров см. в [справочной документации](/engines/database-engines/materialized-postgresql).
:::
