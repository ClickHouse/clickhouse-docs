...
*\* Поддержка JSON в ClickHouse в настоящий момент находится в разработке. В настоящее время пользователи могут либо отображать JSON как String и использовать [JSON функции](/sql-reference/functions/json-functions), либо отображать JSON напрямую на [Tuples](/sql-reference/data-types/tuple) и [Nested](/sql-reference/data-types/nested-data-structures/nested), если структура предсказуема. Узнайте больше о JSON [здесь](/integrations/data-formats/json/overview).*
