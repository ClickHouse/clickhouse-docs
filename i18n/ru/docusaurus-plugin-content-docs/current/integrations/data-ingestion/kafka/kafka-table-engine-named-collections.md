...
keywords: ['именованная коллекция', 'инструкция', 'kafka']
...
# Интеграция ClickHouse с Kafka с помощью именованных коллекций



## Введение {#introduction}

В этом руководстве мы рассмотрим, как подключить ClickHouse к Kafka с помощью именованных коллекций. Использование конфигурационного файла для именованных коллекций дает несколько преимуществ:
...
- Упрощенная проверка и устранение неполадок конфигураций путем просмотра одного конфигурационного файла.
...
Данный документ предполагает, что у вас есть:

1. Работающий кластер Kafka.
2. Настроенный и запущенный кластер ClickHouse.
3. Базовые знания SQL и понимание конфигураций ClickHouse и Kafka.
...
4. В этом примере используется протокол безопасности `SASL_SSL` и механизм `PLAIN`. Настройте эти параметры в соответствии с конфигурацией вашего кластера Kafka.
...
На этом завершается настройка интеграции ClickHouse с Kafka с использованием именованных коллекций. Централизуя конфигурации Kafka в файле `config.xml` ClickHouse, вы можете более удобно управлять настройками и изменять их, обеспечивая упрощённую и эффективную интеграцию.
