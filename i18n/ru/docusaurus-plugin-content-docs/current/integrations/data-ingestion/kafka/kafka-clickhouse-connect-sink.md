...
:::
**ClickHouse Kafka Connect Sink** — это коннектор Kafka, который передаёт данные из топика Kafka в таблицу ClickHouse.
...
- Добавьте путь к директории плагина в конфигурацию [plugin.path](https://kafka.apache.org/documentation/#connectconfigs_plugin.path) в файле свойств Connect, чтобы Confluent Platform могла найти плагин.
...
- Если вы используете Confluent Platform, войдите в интерфейс Confluent Control Center, чтобы убедиться, что ClickHouse Sink доступен в списке доступных коннекторов.
...
- учётные данные пользователя: пароль (**обязательно**) и имя пользователя (необязательно)
...
- конвертеры ключей и значений: устанавливаются в зависимости от типа данных в вашем топике. Обязательны, если ещё не определены в конфигурации worker.
...
| `value.converter` (Обязательно\* — см. описание) | Задайте в зависимости от типа данных в вашем топике. Поддерживаются форматы: JSON, String, Avro или Protobuf. Обязательно здесь, если не определено в конфигурации воркера.                                                        | `"org.apache.kafka.connect.json.JsonConverter"`          |
...
- (2) - Когда структура содержит объединения типа `oneof`, конвертер должен быть настроен так, чтобы НЕ добавлять префикс/суффикс к именам полей. Для этого существует параметр `generate.index.for.unions=false` [для `ProtobufConverter`](https://docs.confluent.io/platform/current/schema-registry/connect.html#protobuf).
...
5. **Мониторинг состояния подключений**: Проверяйте метрики подключений на уровне узлов на наличие сетевых проблем
...
- **Малый размер сообщений**: При работе с большим количеством небольших сообщений, которые выиграют от пакетной обработки на стороне сервера
...
1. **Фреймворк Kafka Connect** извлекает сообщения из топиков Kafka в фоновом режиме
...
Первый уровень оптимизации — управление объёмом данных, которые коннектор получает в одном пакете из Kafka.
...
- **`fetch.min.bytes`**: Минимальный объём данных, прежде чем фреймворк передаст значения коннектору (по умолчанию: 1 байт)
- **`fetch.max.bytes`**: Максимальный объём данных для извлечения в одном запросе (по умолчанию: 52428800 / 50 МБ)
...
- **`max.partition.fetch.bytes`**: Максимальный объём данных на партицию (по умолчанию: 1048576 / 1 МБ)
...
# Увеличьте размер выборки партиции (5 МБ)
consumer.max.partition.fetch.bytes=5242880



# Необязательно: увеличьте минимальный размер выборки, чтобы дождаться большего объёма данных (1 МБ)
...
- **Распределённое развёртывание**: Работает много экземпляров коннектора на разных хостах
...
SETTINGS
    -- Увеличьте максимальное количество потоков вставки для параллельной записи кусков
...
3. **Метрики ClickHouse**:
   ...
   - `system.parts`: Отслеживайте количество кусков для обнаружения проблем со слиянием
...
| Высокая загрузка CPU         | Слишком много мелких кусков          | Включите асинхронные вставки, увеличьте настройки слияния                |
...
2. **Предпочитайте большие пакеты**: стремитесь к 10 000–100 000 строк на вставку, когда это возможно
...
7. **Мониторьте непрерывно**: отслеживайте задержку потребителя, количество кусков и активность слияния
...
- `UnknownHostException` — возникает, когда не удается разрешить хост.
