...
keywords: ['Apache Kafka', 'потоковая обработка событий', 'конвейер обработки данных', 'брокер сообщений', 'данные в реальном времени']
doc_type: 'guide'
integration:
  - support_level: 'core'
...
[Apache Kafka](https://kafka.apache.org/) — это распределённая платформа потоковой передачи событий с открытым исходным кодом, которую используют тысячи компаний для высокопроизводительных конвейеров данных, потоковой аналитики, интеграции данных и критически важных приложений. ClickHouse предоставляет несколько способов **чтения из** и **записи в** Kafka и другие брокеры, совместимые с API Kafka (например, Redpanda, Amazon MSK).
...
- Оптимизирован для ClickHouse Cloud, обеспечивая высочайшую производительность
...
:::tip
Это рекомендуемый вариант, если вы предпочитаете **высокую гибкость настройки** или уже используете Kafka Connect.
:::
...
Чтобы начать использовать движок таблиц Kafka, см. [справочную документацию](./kafka-table-engine.md).
