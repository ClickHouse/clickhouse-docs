...
**Ниже приведены альтернативные подходы к моделированию JSON в ClickHouse. Они задокументированы для полноты и применялись до появления типа JSON, поэтому в большинстве сценариев использования, как правило, не рекомендуются и не применимы.**

:::note Используйте объектный подход
К разным объектам в одной и той же схеме могут применяться разные техники. Например, некоторые объекты удобнее всего хранить в виде типа `String`, а другие — в виде типа `Map`. Обратите внимание, что после выбора типа `String` дополнительных решений по схеме принимать не требуется. Напротив, в ключе `Map` можно вкладывать под-объекты — включая строку `String`, представляющую JSON, — как показано ниже:
:::
...
Обработка данных с использованием структурированного подхода, описанного выше, часто невозможна для пользователей с динамическим JSON, который либо подвержен изменениям, либо имеет недостаточно понятную схему. Для максимальной гибкости можно просто хранить JSON как `String`, а затем использовать функции для извлечения полей по мере необходимости. Это представляет собой полную противоположность обработке JSON как структурированного объекта. Такая гибкость имеет свою цену в виде существенных недостатков — в первую очередь, усложнения синтаксиса запросов и снижения производительности.
...
Обратите внимание, что функциям требуется как ссылка на столбец `String` `tags`, так и путь в JSON для извлечения. Для вложенных путей требуется вложенность функций, например `JSONExtractUInt(JSONExtractString(tags, 'car'), 'year')`, которая извлекает значение `tags.car.year`. Извлечение вложенных путей можно упростить с помощью функций [`JSON_QUERY`](/sql-reference/functions/json-functions#JSON_QUERY) и [`JSON_VALUE`](/sql-reference/functions/json-functions#JSON_VALUE).
...
10 строк в наборе. Затрачено: 0.264 сек. Обработано 2.31 млн строк, 153.57 МБ (8.75 млн строк/сек., 582.58 МБ/сек.)
...
10 строк в наборе. Затрачено: 1.281 сек. Обработано 2.49 млн строк, 4.22 ГБ (1.94 млн строк/сек., 3.29 ГБ/сек.)
...
Строковые функции заметно медленнее (более чем в 10 раз), чем явные преобразования типов с индексами. Приведенные выше запросы всегда требуют полного сканирования таблицы и обработки каждой строки. Хотя эти запросы будут выполняться быстро на таком небольшом наборе данных, производительность снизится на больших наборах данных.

Гибкость этого подхода имеет явную цену в виде снижения производительности и усложнения синтаксиса, и его следует использовать только для высокодинамичных объектов в схеме.

### Простые функции JSON {#simple-json-functions}

Приведенные выше примеры используют семейство функций JSON\*. Они используют полноценный парсер JSON на основе [simdjson](https://github.com/simdjson/simdjson), который строго выполняет разбор и различает одинаковые поля, вложенные на разных уровнях. Эти функции способны обрабатывать JSON, который синтаксически корректен, но плохо отформатирован, например, с двойными пробелами между ключами.
...
- Имена полей уникальны во всех вложенных структурах. Различия между уровнями вложенности не делается, и сопоставление выполняется без учета контекста. В случае нескольких совпадающих полей используется первое вхождение.
...
В некоторых случаях, когда производительность критична, а ваш JSON соответствует вышеуказанным требованиям, использование этих функций может быть оправдано. Ниже показан пример предыдущего запроса, переписанный с использованием функций `simpleJSON*`:
...
При моделировании объектов как `Map` используется ключ типа `String` для хранения имени ключа JSON. Поэтому map всегда будет иметь вид `Map(String, T)`, где `T` зависит от данных.
:::
...
Обработана 1 строка. Прошло: 0.001 сек.
```

Применение `Map` в таком случае обычно редко и указывает на то, что данные следует переразработать так, чтобы динамические имена ключей не имели подобъектов. Например, приведённый выше пример можно переразработать следующим образом, что позволит использовать `Array(Tuple(key String, name String, time DateTime))`.
...
Это позволяет использовать произвольный уровень вложенности и означает, что вложенные столбцы остаются единым массивом `Tuple` — фактически они становятся эквивалентны `Array(Tuple)`.
...
Учитывая ограничения и входной формат JSON, мы вставляем этот образец данных с помощью следующего запроса. Здесь мы устанавливаем `flatten_nested=0`.
...
5 строк в наборе. Время выполнения: 0.383 сек. Обработано 8.22 млн строк, 1.97 GB (21.45 млн строк/с, 5.15 GB/s.)
