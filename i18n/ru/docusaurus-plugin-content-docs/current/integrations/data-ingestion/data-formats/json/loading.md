...
В этом простом случае структура статична, имена столбцов известны, а их типы четко определены.
...
В случаях, когда столбцы имеют фиксированные имена и типы, и новые столбцы не ожидаются, всегда предпочитайте статически определенную схему в продакшене.
...
Здесь мы выбрали ключ сортировки с помощью выражения `ORDER BY`. Для получения дополнительной информации о ключах сортировки и о том, как их выбирать, см. [здесь](/data-modeling/schema-design#choosing-an-ordering-key).
:::
...
Обратите внимание, что нам не требуется указывать формат файла. Вместо этого мы используем шаблон glob для чтения всех файлов `*.json.gz` в бакете. ClickHouse автоматически определяет, что формат — `JSONEachRow` (ndjson), по расширению и содержимому файла. Формат можно указать вручную через параметры функции, если ClickHouse не может его определить автоматически.
...
2 строки в наборе. Затрачено: 0.005 сек. Обработано 8.19 тыс. строк, 908.03 KB (1.63 млн строк/с., 180.38 MB/с.)
...
ClickHouse решает эту задачу с помощью специального типа данных [`JSON`](/sql-reference/data-types/newjson).
...
Столбец tags здесь непредсказуем, и поэтому его невозможно смоделировать. Для загрузки этих данных мы можем использовать предыдущую схему, добавив дополнительный столбец `tags` типа [`JSON`](/sql-reference/data-types/newjson):
...
Обратите внимание на разницу в производительности при загрузке данных. Столбец JSON требует определения типов во время вставки, а также дополнительного места для хранения, если существуют столбцы с более чем одним типом. Хотя тип JSON можно настроить (см. [Designing JSON schema](/integrations/data-formats/json/schema)) для достижения производительности, эквивалентной явному объявлению столбцов, он намеренно гибок по умолчанию. Однако эта гибкость имеет определённую цену.
...
Если структура ваших данных известна и постоянна, редко возникает необходимость в типе JSON, даже если данные представлены в формате JSON. В частности, если ваши данные имеют:
...
* **Предсказуемая структура с различными типами данных**: в этом случае рассмотрите использование типов Dynamic или Variant.

Вы также можете сочетать подходы, как в приведённом выше примере, используя статические столбцы для предсказуемых ключей верхнего уровня и один столбец JSON для динамической части полезной нагрузки.
