...
В случаях, когда столбцы имеют фиксированные имена и типы и не ожидается появление новых столбцов, всегда предпочитайте статически определенную схему в продакшене.
...
Строки также можно загрузить непосредственно, используя [конструкцию `FORMAT`](/sql-reference/statements/select/format), например:
...
ClickHouse решает эту задачу с помощью специального типа [`JSON`](/sql-reference/data-types/newjson).

Рассмотрим следующий пример из расширенной версии набора данных [Python PyPI dataset](https://clickpy.clickhouse.com/), упомянутого выше. Здесь мы добавили произвольный столбец `tags` со случайными парами ключ-значение.
...
* **Предсказуемая структура с различающимися типами**: вместо этого рассмотрите использование типов `Dynamic` или `Variant`.

Вы также можете комбинировать подходы, как показано в приведённом выше примере, используя статические столбцы для предсказуемых ключей верхнего уровня и один столбец JSON для динамической части полезной нагрузки.
