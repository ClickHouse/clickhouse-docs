---
sidebar_label: 'Шаблоны и регулярные выражения'
sidebar_position: 3
slug: /integrations/data-formats/templates-regexp
title: 'Импорт и экспорт произвольных текстовых данных с помощью форматов Templates и Regex в ClickHouse'
description: 'Страница, описывающая, как импортировать и экспортировать произвольный текст с помощью шаблонов и регулярных выражений (Templates и Regex) в ClickHouse'
doc_type: 'guide'
keywords: ['форматы данных', 'шаблоны', 'regex', 'пользовательские форматы', 'разбор данных']
---
...
Нам часто приходится работать с данными в произвольных текстовых форматах. Это может быть нестандартный формат, некорректный JSON или «сломанный» CSV. Использование стандартных парсеров, таких как CSV или JSON, подходит не во всех таких случаях. Но в ClickHouse для этого есть мощные форматы Template и Regex.
...
Здесь `rows_read` и `time` — это системные метрики, доступные для каждого запроса. `data` обозначает сгенерированные строки (`${data}` всегда должен быть первым заполнителем в этом файле), формируемые на основе шаблона, определенного в [**файле шаблона строки**](assets/output.rows):
...
Формат шаблона можно использовать для генерации любых текстовых форматов файлов, включая XML. Просто укажите соответствующий шаблон и выполните экспорт.

Также рассмотрите возможность использования формата [XML](/interfaces/formats/XML) для получения стандартных XML-результатов, включая метаданные:
