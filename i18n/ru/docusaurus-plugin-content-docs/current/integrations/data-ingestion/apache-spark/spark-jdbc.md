...
как использовать [официальный JDBC-коннектор ClickHouse](/integrations/language-clients/java/jdbc) со Spark.
...
При использовании Spark JDBC Spark считывает данные, используя одну партицию. Чтобы повысить параллелизм, необходимо указать
`partitionColumn`, `lowerBound`, `upperBound` и `numPartitions`, которые определяют, как разбить таблицу на партиции при
параллельном чтении несколькими воркерами.
Подробнее см. в официальной документации Apache Spark
по [конфигурации JDBC](https://spark.apache.org/docs/latest/sql-data-sources-jdbc.html#data-source-option).
...
* На данный момент через JDBC можно выполнять вставку данных только в уже существующие таблицы (сейчас нельзя автоматически создавать
  таблицу при вставке DF, как это Spark делает с другими коннекторами).