...
description: 'Узнайте больше о загрузке локальных файлов'
...
Вы можете использовать `clickhouse-client` для потоковой загрузки локальных файлов в ваш сервис ClickHouse. Это позволяет предварительно обрабатывать данные с помощью множества мощных и удобных функций ClickHouse. Рассмотрим пример...

1. Допустим, у нас есть TSV‑файл `comments.tsv`, который содержит некоторые комментарии с Hacker News, а строка заголовка содержит имена столбцов. При вставке данных необходимо указать [формат ввода](/interfaces/formats), который в нашем случае — `TabSeparatedWithNames`:
...
19467048        comment karambahh       2019-03-22 21:15:41     "Мне кажется, вы здесь сравниваете несравнимое.<p>Если вы освобождаете парковочное место для другого использования (например, для строительства жилья для семей или приюта для животных), вы не лишаете автомобиль чего-либо — это дорогой, большой кусок металла, который не обладает сознанием.<p>Дальше вы скажете, что лишаете владельцев автомобилей удобства парковать свои машины где угодно. Я совершенно спокойно отношусь к тому, чтобы лишить владельцев автомобилей этого удобства ради того, чтобы у человека была крыша над головой. (говорю из личного опыта, так как буквально несколько минут назад мне пришлось припарковать машину в километре от дома, потому что город сейчас строит жилье и ограничил парковку поблизости)<p>Затем некоторые могут возразить, что стыдно помогать животным, пока страдают люди. Это точно такая же логика, как «мы не можем принимать больше мигрантов, мы должны позаботиться о „своих" бездомных».<p>Это ложная дихотомия. Неравенство в западных обществах растет все больше и больше. Мои попытки внести свой вклад незначительны. Мои пожертвования на помощь людям или животным — это маленькая царапина на горах неравенства, на которых мы живем. Но все мы вместе действительно меняем ситуацию — пожертвованиями, голосованием и просто тем, что держим глаза открытыми на мир, в котором живем...<p>И наконец, совершенно субъективное наблюдение: я несколько раз был свидетелем того, как крайне бедные люди изо всех сил проявляли солидарность с животными или людьми. Я также был свидетелем того, как очень много крайне богатых людей жаловались на то, что бедные доставляют им неудобства самим своим присутствием, и при этом их богатство было прямым следствием эксплуатации их предками тех самых бедных людей."      [19467512]
```

2. Создадим таблицу для наших данных Hacker News:
...
:::note
Функция `input` здесь полезна, так как позволяет преобразовывать данные в момент их вставки в таблицу `hackernews`. Аргументом `input` является формат входящих «сырых» данных, и вы увидите этот подход во многих других табличных функциях (где вы задаёте схему для входящих данных).
:::
...
│ 4900 │ comment │ lupin_sansei │ 2007-03-19 00:26:30 │ "Суть Ajax заключается в обеспечении взаимодействия Javascript с сервером без перезагрузки страницы. Хотя XmlHttpRequest наиболее удобен, до появления XmlHttpRequest существовали и другие методы, такие как <p>- загрузка изображения размером 1 пиксель и отправка данных в cookie изображения<p>- загрузка данных с сервера через небольшой фрейм, содержащий XML или данные javascipt<p>- использование java-апплета для получения данных от имени javascript" │ []       │ ['The','essence','of','Ajax','is','getting','Javascript','to','communicate','with','the','server','without','reloading','the','page','Although','XmlHttpRequest','is','most','convenient','there','were','other','methods','of','doing','this','before','XmlHttpRequest','such','as','p','loading','a','1','pixel','image','and','sending','data','in','the','image','s','cookie','p','loading','server','data','through','a','tiny','frame','which','contained','XML','or','javascipt','data','p','Using','a','java','applet','to','fetch','the','data','on','behalf','of','javascript'] │
│ 5102 │ comment │ staunch      │ 2007-03-20 02:42:47 │ "Что ж, это именно то, что не совсем очевидно. Похоже, что как только вы становитесь богатым, появляется новый набор правил, по которым приходится жить. Обидно, что каждому пришлось заново изучать эти вещи самостоятельно из-за того, что некоторые люди не могут справиться со своей завистью.<p>Очень приятно слышать, что это где-то в вашем списке эссе. Постараюсь не разбогатеть до того, как вы это напишете, чтобы хоть примерно знать, чего ожидать :-)" │ []       │ ['Well','this','is','exactly','the','kind','of','thing','that','isn','t','very','obvious','It','sounds','like','once','you','re','wealthy','there','s','a','new','set','of','rules','you','have','to','live','by','It','s','a','shame','everyone','has','had','to','re','learn','these','things','for','themselves','because','a','few','bad','apples','can','control','their','jealousy','p','Very','good','to','hear','it','s','somewhere','in','your','essay','queue','though','I','ll','try','not','to','get','rich','before','you','write','it','so','I','have','some','idea','of','what','to','expect'] │
```

5. Другой вариант — использовать утилиту `cat`, чтобы передать содержимое файла в `clickhouse-client`. Например, следующая команда даёт тот же результат, что и использование оператора `<`:
