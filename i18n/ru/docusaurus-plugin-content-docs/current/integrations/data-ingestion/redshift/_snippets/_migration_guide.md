...
3. **[ПЕРЕНЕСТИ](#pivot-data-from-redshift-to-clickhouse-using-s3)** через объектное хранилище S3 по принципу «выгрузка, затем загрузка»
...
- Использование существующего каталога коннекторов из ПО ETL/ELT.
...
- Необходимость настройки и обслуживания инфраструктуры ETL/ELT.
...
Для использования этого варианта необходимо настроить ClickHouse JDBC Bridge. ClickHouse JDBC Bridge — это автономное Java-приложение, которое обрабатывает подключения JDBC и выступает в качестве прокси между экземпляром ClickHouse и источниками данных. В этом руководстве мы использовали предварительно заполненный экземпляр Redshift с [тестовой базой данных](https://docs.aws.amazon.com/redshift/latest/dg/c_sampledb.html).
...
После развертывания и запуска ClickHouse JDBC Bridge вы можете начать выполнять запросы к экземпляру Redshift из ClickHouse
...
:::note
В этом примере в качестве промежуточного формата используется CSV. Однако для производственных нагрузок мы рекомендуем Apache Parquet как лучший вариант для крупных миграций, поскольку он поддерживает сжатие и позволяет снизить затраты на хранение, одновременно сокращая время передачи данных (по умолчанию каждая группа строк сжимается с использованием SNAPPY). ClickHouse также использует колоночную ориентацию Parquet для ускорения загрузки данных.
:::
