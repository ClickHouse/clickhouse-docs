...
title: 'Интеграция EMQX и ClickHouse'
doc_type: 'guide'
integration:
  - support_level: 'partner'
  - category: 'data_ingestion'
keywords: ['интеграция EMQX с ClickHouse', 'коннектор MQTT для ClickHouse', 'EMQX Cloud и ClickHouse', 'данные IoT в ClickHouse', 'брокер MQTT для ClickHouse']
---
...
<Image
  ...
  size='sm'
  border
  alt='Настройки подключения ClickHouse Cloud с конфигурацией доступа по IP'
/>
...
<Image
  ...
  size='lg'
  border
  alt='Выполнение SQL-запроса для создания базы данных и таблицы в ClickHouse Cloud'
/>
...
### Создание учетной записи {#get-an-account}
...
Перейдите на страницу [регистрации EMQX Cloud](https://accounts.emqx.com/signup?continue=https%3A%2F%2Fwww.emqx.com%2Fen%2Fcloud) и нажмите кнопку начала бесплатного использования для регистрации учетной записи, если вы впервые работаете с EMQX Cloud.
...
<Image
  ...
  size='lg'
  border
  alt='EMQX Cloud: Создание развертывания, шаг 1, отображающий варианты развертывания'
/>
...
Выберите версию Pro и регион `N.Virginia`, затем нажмите `Create Now`. Всего через несколько минут вы получите полностью управляемый MQTT-брокер:

<Image
  img={emqx_cloud_create_2}
  size='lg'
  border
  alt='EMQX Cloud: Создание развертывания, шаг 2, отображающий выбор региона'
/>
...
<Image
  ...
  size='lg'
  border
  alt='Панель обзора EMQX Cloud, отображающая метрики брокера'
/>
...
Нажмите «Confirm», и теперь у нас есть готовый полностью управляемый MQTT-брокер.
...
Вернитесь на страницу обзора и прокрутите вниз до конца страницы, где вы увидите виджет NAT-шлюза. Нажмите кнопку Subscribe и следуйте инструкциям. Обратите внимание, что NAT Gateway является дополнительной платной услугой, но также предлагает 14-дневный бесплатный пробный период.
...
После создания вы найдете публичный IP-адрес в виджете. Обратите внимание, что если вы выбрали «Connect from a specific location» во время настройки ClickHouse Cloud, вам потребуется добавить этот IP-адрес в белый список.


## Интеграция EMQX Cloud с ClickHouse Cloud {#integration-emqx-cloud-with-clickhouse-cloud}

[EMQX Cloud Data Integrations](https://docs.emqx.com/en/cloud/latest/rule_engine/introduction.html#general-flow) используется для настройки правил обработки потоков сообщений EMQX и событий устройств. Data Integrations не только предоставляет понятное и гибкое настраиваемое архитектурное решение, но также упрощает процесс разработки, повышает удобство использования и снижает степень связанности между бизнес-системой и EMQX Cloud. Кроме того, он обеспечивает надежную инфраструктуру для настройки собственных возможностей EMQX Cloud.
...
<Image
  ...
  size='lg'
  border
  alt='Сведения о коннекторе интеграции данных EMQX Cloud ClickHouse'
/>
...
<Image
  ...
  size='lg'
  border
  alt='Форма настройки ресурса EMQX Cloud ClickHouse с параметрами подключения'
/>

### Создание нового правила {#create-a-new-rule}

При создании ресурса появится всплывающее окно, и нажатие «New» приведет вас на страницу создания правила.

EMQX предоставляет мощный [механизм правил](https://docs.emqx.com/en/cloud/latest/rule_engine/rules.html), который может преобразовывать и обогащать исходные сообщения MQTT перед отправкой в сторонние системы.
...
Оно будет читать сообщения из топика `temp_hum/emqx` и обогащать объект JSON, добавляя информацию о client_id, topic и timestamp.
...
Теперь нажмите кнопку «NEXT». На этом шаге необходимо указать EMQX Cloud, как вставлять обработанные данные в вашу базу данных ClickHouse.
...
Вам нужно только задать шаблон SQL. Вот пример, используемый в этом руководстве:
...
<Image
  ...
  size='md'
  border
  alt='Настройка действия правила интеграции данных EMQX Cloud с шаблоном SQL'
/>

Это шаблон для вставки данных в ClickHouse, в котором используются переменные.

### Просмотр сведений о правилах {#view-rules-details}

Нажмите «Confirm» и «View Details». Теперь все должно быть настроено правильно. Вы можете увидеть работу интеграции данных на странице сведений о правиле.

<Image
  img={data_integration_details}
  size='md'
  border
  alt='Сведения о правиле интеграции данных EMQX Cloud с сводкой конфигурации'
/>
...
Мы смоделируем данные о температуре и влажности и передадим их в EMQX Cloud через MQTT X, а затем используем интеграцию данных EMQX Cloud для сохранения данных в ClickHouse Cloud.

<Image
  img={work_flow}
  size='lg'
  border
  alt='Диаграмма рабочего процесса от EMQX Cloud к ClickHouse, показывающая поток данных'
/>

### Публикация MQTT-сообщений в EMQX Cloud {#publish-mqtt-messages-to-emqx-cloud}

Вы можете использовать любой MQTT-клиент или SDK для публикации сообщений. В этом руководстве мы будем использовать [MQTT X](https://mqttx.app/) — удобное клиентское приложение MQTT от EMQ.

<Image
  img={mqttx_overview}
  size='lg'
  border
  alt='Обзор MQTTX, показывающий интерфейс клиента'
/>
...
- Username/Password: Используйте учетные данные, созданные ранее, которые в этом руководстве должны быть `emqx` и `xxxxxx`.
...
Нажмите кнопку «Connect» в правом верхнем углу, и подключение будет установлено.
...
Данные, отправленные в EMQX Cloud, будут обработаны механизмом правил и автоматически вставлены в ClickHouse Cloud.

<Image
  img={mqttx_publish}
  size='lg'
  border
  alt='Интерфейс публикации MQTT-сообщений MQTTX, показывающий составление сообщения'
/>
...
<Image
  ...
  size='lg'
  border
  alt='Панель мониторинга правил EMQX Cloud, показывающая метрики обработки сообщений'
/>

### Проверка сохраненных данных {#check-the-data-persisted}

Теперь пришло время проверить данные в ClickHouse Cloud. В идеале данные, которые вы отправляете с помощью MQTTX, поступят в EMQX Cloud и будут сохранены в базе данных ClickHouse Cloud с помощью встроенной интеграции данных.
...
<Image
  ...
  size='lg'
  border
  alt='Результаты запроса ClickHouse, показывающие сохраненные IoT-данные'
/>

### Резюме {#summary}

Вы не написали ни строчки кода, и теперь MQTT-данные перемещаются из EMQX Cloud в ClickHouse Cloud. С EMQX Cloud и ClickHouse Cloud вам не нужно управлять инфраструктурой — вы можете просто сосредоточиться на разработке IoT-приложений, а данные будут надежно храниться в ClickHouse Cloud.
