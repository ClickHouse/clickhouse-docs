...
description: 'MergeTree на базе Google Cloud Storage (GCS)'
...
ClickHouse учитывает, что GCS является привлекательным вариантом хранилища для пользователей, стремящихся разделить хранение и вычисления. Для этого предусмотрена поддержка использования GCS в качестве хранилища для движка MergeTree. Это позволяет пользователям воспользоваться масштабируемостью и выгодной стоимостью GCS, а также высокой скоростью вставки и выполнения запросов в движке MergeTree.
...
Чтобы использовать бакет GCS в качестве диска, его необходимо сначала объявить в конфигурации ClickHouse в файле в каталоге `conf.d`. Ниже приведён пример объявления диска GCS. Эта конфигурация включает несколько разделов для настройки «диска» GCS, кэша и политики, которая указывается в DDL-запросах при создании таблиц на диске GCS. Каждый из этих разделов описан ниже.
...
Приведённая ниже конфигурация включает кэш размером 10 ГиБ для диска `gcs`.
...
Политики конфигурации хранилища позволяют выбирать место хранения данных. Политика, выделенная ниже, позволяет хранить данные на диске `gcs` путём указания политики `gcs_main`. Например, `CREATE TABLE ... SETTINGS storage_policy='gcs_main'`.
...
Предполагая, что вы настроили диск для использования бакета с правами на запись, вы сможете создать таблицу, как показано в примере ниже. Для краткости мы используем подмножество столбцов NYC taxi и передаём данные напрямую в таблицу с хранилищем GCS:
...
Репликация с дисками GCS может быть реализована с использованием движка таблиц `ReplicatedMergeTree`. Подробности см. в руководстве [репликация одного шарда между двумя регионами GCP с использованием GCS](#gcs-multi-region).
...
Для получения дополнительной информации о настройке потоков см. раздел [Оптимизация производительности](../s3/index.md#s3-optimizing-performance).


## Использование Google Cloud Storage (GCS) {#gcs-multi-region}

:::tip
В ClickHouse Cloud объектное хранилище используется по умолчанию, поэтому если вы работаете в ClickHouse Cloud, выполнять эту процедуру не требуется.
:::

### Планирование развертывания {#plan-the-deployment}

В этом руководстве описывается реплицированное развертывание ClickHouse в Google Cloud с использованием Google Cloud Storage (GCS) в качестве «типа» дискового хранилища ClickHouse.

В руководстве вы развернете узлы сервера ClickHouse на виртуальных машинах Google Compute Engine, каждая из которых будет иметь связанный бакет GCS для хранения данных. Репликация координируется набором узлов ClickHouse Keeper, также развернутых на виртуальных машинах.
...
Для работы ClickHouse Keeper требуется минимум два узла, поэтому для обеспечения высокой доступности необходимо три узла.
...
Разместите `chnode1` в одном регионе GCP, а `chnode2` — во втором. В этом руководстве для виртуальных машин Compute Engine и бакетов GCS используются регионы `us-east1` и `us-east4`.

:::note
Не запускайте `clickhouse server` до завершения его настройки. Просто установите его.
:::
...
В разделе **Cloud Storage > Buckets** выберите **CREATE BUCKET**. Для этого руководства создаются два бакета — по одному в каждом из регионов `us-east1` и `us-east4`. Бакеты являются однорегиональными, со стандартным классом хранения и не являются публичными. При появлении запроса включите защиту от публичного доступа. Не создавайте папки — они будут созданы автоматически, когда ClickHouse начнет записывать данные в хранилище.
...
По умолчанию ClickHouse прослушивает интерфейс обратной петли. В реплицированной конфигурации необходима сетевая связь между машинами. Настройте прослушивание на всех интерфейсах:
