...
title: 'Использование HTTP-интерфейса ClickHouse для передачи данных из Azure в ClickHouse'
doc_type: 'guide'
---
...
В таких сценариях можно использовать Azure Data Factory вместе с
...
[Создание и настройка self-hosted integration
...
Эту возможность можно использовать для прямой вставки данных в ClickHouse
...
Для данного примера достаточно указать целевую таблицу, определить
...
Чтобы отправить этот запрос как часть HTTP-запроса, передайте его в виде
...
:::info
Azure Data Factory может выполнить кодирование автоматически с помощью встроенной
...
полностью совместим с форматом ввода `JSONEachRow` в ClickHouse.

Как видно, для этого шага не требуется выполнять какие-либо специальные действия на стороне ClickHouse.
HTTP-интерфейс уже предоставляет всё необходимое для работы в качестве
...
На следующих шагах мы создадим экземпляр Azure Data Factory, настроим связанную
службу (Linked Service) для экземпляра ClickHouse, определим набор данных (Dataset) для
[приёмника REST](https://learn.microsoft.com/en-us/azure/data-factory/connector-rest)
и создадим действие копирования данных (Copy Data activity) для отправки данных из Azure в ClickHouse.


## Создание экземпляра Azure Data Factory {#create-an-azure-data-factory-instance}

Это руководство предполагает, что у вас есть доступ к учетной записи Microsoft Azure и вы
...
4. Нажмите **Review + Create**, затем нажмите **Create** для запуска развертывания.
   ...
   <Image
     ...
     size='lg'
     alt='Подтверждение создания Data Factory на портале Azure'
     border
   />
   ...
   <Image
     ...
     size='lg'
     alt='Успешное создание Data Factory на портале Azure'
     border
   />
...
9. Вернувшись в основную форму, выберите Basic authentication, введите имя пользователя и
   ...
   connection**. Если всё настроено правильно, вы увидите сообщение об успешном
   подключении.
...
В этом примере мы вставим небольшую часть [данных датчиков окружающей среды
(Environmental Sensors Data)](https://clickhouse.com/docs/getting-started/example-datasets/environmental-sensors).
...
2. В Azure Data Factory Studio выберите Author на левой панели. Наведите курсор
   ...
   <Image img={adfNewDatasetItem} size='lg' alt='Элемент нового набора данных' border />
...
4. Теперь вы должны увидеть созданный набор данных в разделе Datasets
   ...
   <Image
     ...
     size='lg'
     alt='Свойства нового набора данных'
     border
   />
...
5. На открывшейся панели вставьте следующий запрос:
   ...
   не экранируете их, вы можете не увидеть ошибку сразу — но операция завершится неудачей
...
6. Нажмите OK, чтобы сохранить выражение. Нажмите Test connection. Если всё
   ...
   <Image
     ...
     size='xl'
     alt='Успешное подключение нового набора данных'
     border
   />

### Настройка примера набора данных {#setting-up-an-example-dataset}

В этом примере мы не будем использовать полный набор данных Environmental Sensors Dataset, а
...
4. Перейдите на вкладку **Sink** и выберите набор данных ClickHouse, созданный для
   ...
   :::
   :::info
   Рекомендуется сохранить размер пакета по умолчанию 10 000 или даже увеличить его.
   ...
   <Image
     ...
     size='lg'
     alt='Выбор POST для приемника копирования данных'
     border
   />
...
5. Нажмите **Debug** в верхней части холста, чтобы запустить конвейер. После короткого
   ...
   <Image
     ...
     size='lg'
     alt='Успешная отладка копирования данных'
     border
   />

6. После завершения нажмите **Publish all**, чтобы сохранить изменения конвейера и наборов данных.
