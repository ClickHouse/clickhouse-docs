...
[marimo](https://marimo.io/) — это открытый реактивный ноутбук для Python со встроенной поддержкой SQL. Когда вы запускаете ячейку или взаимодействуете с элементом интерфейса, marimo автоматически выполняет все затронутые ячейки (или помечает их как неактуальные), поддерживая согласованность кода и результатов и предотвращая ошибки ещё до их возникновения. Каждый ноутбук marimo хранится как чистый Python-файл, может выполняться как скрипт и развёртываться как приложение.
...
Вам будет предложено ввести параметры подключения к базе данных.

<Image
  img={add_db_details}
  size='md'
  border
  alt='Введите параметры подключения к базе данных'
/>
...
<Image
  ...
  size='md'
  border
  alt='Запустите ячейку для подключения к ClickHouse'
/>


## 3. Выполнение SQL {#run-sql}

После настройки соединения можно создать новую SQL-ячейку и выбрать движок clickhouse.
...
Теперь результаты можно просматривать в виде датафрейма. Визуализируем самые дорогие высадки из заданной точки посадки. marimo предоставляет несколько UI-компонентов для этой задачи. Используем выпадающий список для выбора местоположения и altair для построения графиков.
...
Реактивная модель выполнения marimo распространяется на SQL-запросы, поэтому изменения в SQL автоматически запускают последующие вычисления для зависимых ячеек (или опционально помечают ячейки как устаревшие для ресурсоёмких вычислений). Таким образом, график и таблица обновляются при изменении запроса.

Также можно переключиться в режим App View для удобного интерфейса исследования данных.
