...
После загрузки данных мы визуализируем их с помощью построения графиков с использованием SQL.

Интеграция между JupySQL и ClickHouse стала возможной благодаря использованию библиотеки clickhouse&#95;sqlalchemy. Эта библиотека упрощает взаимодействие между двумя системами и позволяет подключаться к ClickHouse и задавать диалект SQL. После подключения вы можете выполнять SQL-запросы непосредственно из родного интерфейса ClickHouse или напрямую из блокнота Jupyter.
...
**На следующих этапах необходимо, чтобы ваш ClickHouse был запущен и доступен. Можно использовать как локальную, так и облачную версию.**

**Примечание:** вам потребуется изменить строку подключения в соответствии с типом экземпляра, к которому вы подключаетесь (URL, пользователь, пароль). В примере ниже мы используем локальный экземпляр. Чтобы узнать больше, см. [это руководство](/get-started/quick-start).
...
<Image img={jupysql_plot_1} size="md" alt="Гистограмма, показывающая распределение расстояний поездок с 10 интервалами из набора данных short-trips" border />
...
<Image img={jupysql_plot_2} size="md" alt="Гистограмма распределения дистанций поездок с 50 интервалами и сеткой, с заголовком «Дистанция поездки для поездок < 6,3»" border />
