...
:::tip
Подробное описание каждого параметра, включая значения по умолчанию, параметры DSN, рекомендации и устранение неполадок, см. в разделе [Configuration Reference](/integrations/language-clients/go/config-reference).
:::
