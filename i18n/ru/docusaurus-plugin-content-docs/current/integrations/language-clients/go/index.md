...
description: 'Клиенты Go для ClickHouse позволяют подключаться к ClickHouse с использованием стандартного интерфейса Go database/sql или оптимизированного нативного интерфейса.'
title: 'ClickHouse Go'
doc_type: 'reference'
integration:
  - support_level: 'core'
...
Рассмотрим простой пример. Он подключится к ClickHouse и выполнит выборку из системной базы данных. Для начала работы вам понадобятся параметры подключения.
...
### Настройка параметров подключения {#set-your-connection-details}

Ранее вы получили параметры подключения. Укажите их в файле `main.go` в функции `connect()`:
...
Остальная документация в этом разделе подробно описывает работу с Go-клиентом ClickHouse.
...
Для рабочих нагрузок с запросами, ориентированными на агрегацию, или для вставок с меньшей пропускной способностью клиент [clickhouse-go](https://github.com/ClickHouse/clickhouse-go) предоставляет привычный интерфейс `database/sql` и более простую семантику работы со строками. Пользователи также могут опционально использовать HTTP в качестве транспортного протокола и воспользоваться вспомогательными функциями для маршалинга строк в структуры и обратно.
...
|                    | Нативный формат | Нативный протокол | HTTP-протокол | Поддержка массовой записи | Маршалинг структур | Сжатие | Плейсхолдеры запросов |
| :----------------: | :-------------: | :---------------: | :-----------: | :-----------------------: | :----------------: | :----: | :-------------------: |
|   API ClickHouse   |       ✅        |        ✅         |               |            ✅             |         ✅         |   ✅   |          ✅           |
| API `database/sql` |       ✅        |        ✅         |      ✅       |            ✅             |                    |   ✅   |          ✅           |
...
Клиент выпускается независимо от ClickHouse. Версия 2.x представляет текущую основную ветку разработки. Все версии 2.x совместимы между собой.
...
- Все версии ClickHouse за последние 2 года с даты выпуска клиента. Обратите внимание, что активно тестируются только LTS-версии.
...
- `MaxOpenConns` — максимальное количество соединений для одновременного использования. В пуле простаивающих соединений может быть больше или меньше соединений, но одновременно может использоваться только это количество. По умолчанию `MaxIdleConns+5`.
...
Также обратите внимание, что значение `ConnMaxLifetime` по умолчанию составляет 1 час. Это может привести к ситуациям, когда нагрузка на ClickHouse становится несбалансированной при выходе узлов из кластера. Это может произойти, когда узел становится недоступным — соединения перераспределяются на другие узлы. Эти соединения сохраняются и не обновляются в течение 1 часа по умолчанию, даже если проблемный узел возвращается в кластер. Рассмотрите возможность уменьшения этого значения при высоких нагрузках.
...
Этой минимальной конфигурации `TLS.Config` обычно достаточно для подключения к защищенному нативному порту (обычно 9440) сервера ClickHouse. Если у сервера ClickHouse нет действительного сертификата (истек срок действия, неверное имя хоста, не подписан общепризнанным корневым центром сертификации), можно установить `InsecureSkipVerify` в true, но это крайне не рекомендуется.
...
- `ConnOpenInOrder` (по умолчанию) — адреса используются по порядку. Последующие адреса применяются только в случае неудачного подключения к адресам, расположенным ранее в списке. Фактически это стратегия отказоустойчивости (failover).
...
Произвольные SQL-операторы можно выполнять с помощью метода `Exec`. Он удобен для DDL-команд и простых операторов. Не рекомендуется использовать его для больших вставок данных или итерации по результатам запросов.
...
Обратите внимание на возможность передачи Context в запрос. Это позволяет задавать настройки на уровне конкретного запроса — см. раздел [Использование Context](#using-context).

### Пакетная вставка {#batch-insert}

Для вставки большого количества строк клиент предоставляет пакетную семантику. Для этого необходимо подготовить пакет, к которому можно добавлять строки. Отправка данных выполняется методом `Send()`. Пакеты хранятся в памяти до вызова `Send`.

Рекомендуется вызывать `Close` для пакета, чтобы предотвратить утечку соединений. Это можно сделать с помощью ключевого слова `defer` сразу после подготовки пакета. Это освободит соединение, если метод `Send` так и не будет вызван. Обратите внимание, что в этом случае в журнале запросов будет зафиксирована вставка 0 строк, если в пакет не было добавлено ни одной строки.
...
Из приведенного выше примера обратите внимание на необходимость соответствия типов переменных типам столбцов при добавлении строк. Хотя соответствие обычно очевидно, этот интерфейс обеспечивает гибкость, и типы будут преобразованы при условии отсутствия потери точности. Например, следующий код демонстрирует вставку строки в тип datetime64.
...
Обратите внимание, что в обоих случаях необходимо передать указатель на переменные, в которые будут десериализованы соответствующие значения столбцов. Они должны передаваться в порядке, указанном в операторе `SELECT` — по умолчанию в случае `SELECT *` будет использоваться порядок объявления столбцов, как показано выше.


Как и при вставке, метод Scan требует, чтобы целевые переменные имели соответствующий тип. Здесь также обеспечивается гибкость: типы преобразуются там, где это возможно, при условии отсутствия потери точности. Например, в приведенном выше примере столбец UUID читается в строковую переменную. Полный список поддерживаемых типов Go для каждого типа столбца см. в разделе [Преобразование типов](#type-conversions).
...
Вставки могут выполняться в колоночном формате. Это может повысить производительность, если данные уже организованы в такой структуре, избегая необходимости преобразования в строки.
...
Структуры Golang обеспечивают логическое представление строки данных в ClickHouse. Для удобства работы с ними нативный интерфейс предоставляет несколько полезных функций.
...
- **Date/Date32**
  - При **вставке** часовой пояс даты учитывается при преобразовании даты в unix timestamp, то есть дата смещается на величину часового пояса перед сохранением, поскольку типы Date в ClickHouse не содержат информации о локали. Если часовой пояс не указан в строковом значении, используется локальный часовой пояс.
  - При **выборке** даты сканируются в экземпляры `time.Time{}` или `sql.NullTime{}` и возвращаются без информации о часовом поясе.

#### Массивы {#array}

Массивы должны вставляться как срезы (slices). Правила типизации элементов соответствуют правилам для [примитивных типов](#type-conversions), то есть элементы преобразуются там, где это возможно.
...
Map должны вставляться как Golang-карты (maps) с ключами и значениями, соответствующими правилам типизации, определённым [ранее](#type-conversions).
...
Примечание: типизированные срезы и карты поддерживаются при условии, что все подстолбцы в именованном кортеже имеют одинаковые типы.

#### Nested {#nested}

Поле типа Nested эквивалентно массиву именованных кортежей. Использование зависит от того, установлен ли параметр [flatten_nested](/operations/settings/settings#flatten_nested) в значение 1 или 0.

При установке flatten_nested в значение 0 столбцы Nested остаются единым массивом кортежей. Это позволяет использовать срезы карт для вставки и извлечения данных с произвольными уровнями вложенности. Ключ карты должен совпадать с именем столбца, как показано в примере ниже.
...
По умолчанию срезы (slices) разворачиваются в список значений, разделенных запятыми, при передаче в качестве параметра запроса. Если необходимо вставить набор значений с обрамлением `[ ]`, следует использовать `ArraySet`.

Если требуются группы или кортежи с обрамлением `( )`, например, для использования с операторами IN, можно использовать `GroupSet`. Это особенно полезно в случаях, когда требуется несколько групп, как показано в примере ниже.

Наконец, поля DateTime64 требуют указания точности для корректного представления параметров. Однако уровень точности поля неизвестен клиенту, поэтому пользователь должен указать его самостоятельно. Для этого предоставляется параметр `DateNamed`.
...
Контексты Go предоставляют механизм передачи сроков выполнения, сигналов отмены и других значений в области запроса через границы API. Все методы соединения принимают контекст в качестве первого параметра. Хотя в предыдущих примерах использовался context.Background(), пользователи могут использовать эту возможность для передачи настроек и сроков выполнения, а также для отмены запросов.

Передача контекста, созданного с помощью `withDeadline`, позволяет устанавливать ограничения времени выполнения для запросов. Обратите внимание, что это абсолютное время, и по истечении срока соединение будет освобождено, а в ClickHouse будет отправлен сигнал отмены. В качестве альтернативы можно использовать `WithCancel` для явной отмены запроса.

Вспомогательные функции `clickhouse.WithQueryID` и `clickhouse.WithQuotaKey` позволяют указать идентификатор запроса и ключ квоты. Идентификаторы запросов могут быть полезны для отслеживания запросов в логах и для целей отмены. Ключ квоты можно использовать для установки ограничений на использование ClickHouse на основе уникального значения ключа — подробнее см. [Управление квотами](/operations/access-rights#quotas-management).
...
// мы можем использовать контекст для передачи настроек конкретному вызову API
...
// установка предельного времени выполнения запроса - запрос будет отменен после достижения указанного времени.
...
Чтобы отправить внешние данные на сервер вместе с запросом, пользователь должен создать внешнюю таблицу с помощью `ext.NewTable` перед передачей её через контекст.
...
API `database/sql` или «стандартный» API позволяет использовать клиент в сценариях, где код приложения должен быть независим от конкретной базы данных, соответствуя стандартному интерфейсу. Это имеет определённую цену — дополнительные уровни абстракции и косвенного обращения, а также примитивы, которые не обязательно согласованы с ClickHouse. Однако эти издержки обычно приемлемы в сценариях, где инструментарию необходимо подключаться к нескольким базам данных.
...
Далее структура документации повторяет структуру документации для API ClickHouse.
...
Пользователи могут управлять использованием предоставленного списка адресов узлов, как описано в разделе [Подключение к нескольким узлам](#connecting-to-multiple-nodes). Однако управление соединениями и пулинг по умолчанию делегированы `sql.DB`.
...
- `gzip` - `-2` (максимальная скорость) до `9` (максимальное сжатие)
- `deflate` - `-2` (максимальная скорость) до `9` (максимальное сжатие)
- `br` - `0` (максимальная скорость) до `11` (максимальное сжатие)
- `zstd`, `lz4` - игнорируется
...
// установите query id для отслеживания запросов в логах, например, см. system.query_log
var one uint8
...
Аналогично [ClickHouse API](#dynamic-scanning), доступна информация о типах столбцов, что позволяет пользователям создавать экземпляры переменных с правильными типами во время выполнения, которые можно передать в Scan. Это позволяет читать столбцы, тип которых неизвестен.
