...
description: 'Клиенты Go для ClickHouse позволяют пользователям подключаться к ClickHouse либо через стандартный интерфейс Go database/sql, либо через оптимизированный нативный интерфейс.'
title: 'ClickHouse Go'
doc_type: 'reference'
integration:
  - support_level: 'core'
...
Рассмотрим простой пример. Он подключится к ClickHouse и выполнит выборку из системной базы данных. Для начала работы вам потребуются данные для подключения.
...
### Запуск go mod tidy {#run-go-mod-tidy}

```bash
go mod tidy
```

### Указание данных для подключения {#set-your-connection-details}

Ранее вы получили данные для подключения. Укажите их в файле `main.go` в функции `connect()`:
...
Остальная документация в этом разделе подробно описывает клиент ClickHouse для Go.


## Клиент ClickHouse для Go {#clickhouse-go-client}

ClickHouse поддерживает два официальных клиента для Go. Эти клиенты дополняют друг друга и предназначены для различных сценариев использования.
...
Начиная с версии 2.3, clickhouse-go использует ch-go для низкоуровневых функций, таких как кодирование, декодирование и сжатие. Обратите внимание, что clickhouse-go также поддерживает стандартный интерфейс Go `database/sql`. Оба клиента используют нативный формат для кодирования, чтобы обеспечить оптимальную производительность, и могут взаимодействовать через нативный протокол ClickHouse. clickhouse-go также поддерживает HTTP в качестве транспортного механизма для случаев, когда пользователям требуется проксирование или балансировка нагрузки трафика.

При выборе клиентской библиотеки пользователи должны учитывать их преимущества и недостатки — см. раздел «Выбор клиентской библиотеки».
...
Выбор клиентской библиотеки зависит от ваших сценариев использования и требований к оптимальной производительности. Для случаев с интенсивной вставкой данных, когда требуются миллионы вставок в секунду, мы рекомендуем использовать низкоуровневый клиент [ch-go](https://github.com/ClickHouse/ch-go). Этот клиент позволяет избежать накладных расходов, связанных с преобразованием данных из строчно-ориентированного формата в колоночный, которого требует нативный формат ClickHouse. Кроме того, он не использует рефлексию и тип `interface{}` (`any`), что упрощает работу с ним.

Для рабочих нагрузок с запросами, ориентированными на агрегацию, или для вставок с меньшей пропускной способностью клиент [clickhouse-go](https://github.com/ClickHouse/clickhouse-go) предоставляет знакомый интерфейс `database/sql` и более простую семантику работы со строками. Пользователи также могут опционально использовать HTTP в качестве транспортного протокола и воспользоваться вспомогательными функциями для маршалинга строк в структуры и обратно.
...
Клиент выпускается независимо от ClickHouse. Версия 2.x представляет текущую основную ветку разработки. Все версии 2.x должны быть совместимы друг с другом.

#### Совместимость с ClickHouse {#clickhouse-compatibility}

Клиент поддерживает:

- Все актуальные поддерживаемые версии ClickHouse, перечисленные [здесь](https://github.com/ClickHouse/ClickHouse/blob/master/SECURITY.md). Версии ClickHouse, которые больше не поддерживаются, также больше не тестируются активно с выпусками клиента.
...
**Для всех последующих примеров, если явно не указано иное, предполагается, что переменная `conn` ClickHouse создана и доступна.**
...
- `DialContext` — пользовательская функция установки соединения для определения способа установки соединений.
...
- `MaxOpenConns` — максимальное количество соединений для использования в любой момент времени. В пуле простаивающих соединений может быть больше или меньше соединений, но одновременно можно использовать только это количество. По умолчанию `MaxIdleConns+5`.
- `MaxIdleConns` — количество соединений для поддержания в пуле. Соединения будут повторно использоваться, если это возможно. По умолчанию `5`.
- `ConnMaxLifetime` — максимальное время жизни для поддержания соединения доступным. По умолчанию 1 час. Соединения уничтожаются по истечении этого времени, при необходимости в пул добавляются новые соединения.
...
Клиент поддерживает пул соединений, повторно используя их для запросов по мере необходимости. Одновременно будет использоваться не более `MaxOpenConns` соединений, при этом максимальный размер пула контролируется параметром `MaxIdleConns`. Клиент получает соединение из пула для выполнения каждого запроса и возвращает его в пул для повторного использования. Соединение используется в течение всего времени жизни пакета и освобождается при вызове `Send()`.

Нет гарантии, что для последующих запросов будет использоваться то же самое соединение из пула, если только пользователь не установит `MaxOpenConns=1`. Это требуется редко, но может быть необходимо в случаях, когда используются временные таблицы.

Также обратите внимание, что значение `ConnMaxLifetime` по умолчанию составляет 1 час. Это может привести к ситуациям, когда нагрузка на ClickHouse становится несбалансированной при выходе узлов из кластера. Это может произойти, когда узел становится недоступным — соединения будут перераспределены на другие узлы. Эти соединения сохранятся и не будут обновлены в течение 1 часа по умолчанию, даже если проблемный узел вернется в кластер. Рассмотрите возможность уменьшения этого значения при высоких нагрузках.

### Использование TLS {#using-tls}

На низком уровне все методы подключения клиента (`DSN/OpenDB/Open`) используют [пакет Go tls](https://pkg.go.dev/crypto/tls) для установления защищенного соединения. Клиент определяет необходимость использования TLS, если структура Options содержит ненулевой указатель `tls.Config`.
...
Этой минимальной конфигурации `TLS.Config` обычно достаточно для подключения к защищенному нативному порту (обычно 9440) на сервере ClickHouse. Если сервер ClickHouse не имеет действительного сертификата (истек срок действия, неверное имя хоста, не подписан общепризнанным корневым центром сертификации), параметр `InsecureSkipVerify` может быть установлен в true, но это крайне не рекомендуется.
...
Если требуются дополнительные параметры TLS, код приложения должен установить необходимые поля в структуре `tls.Config`. Это может включать конкретные наборы шифров, принудительное использование определенной версии TLS (например, 1.2 или 1.3), добавление цепочки сертификатов внутреннего центра сертификации, добавление клиентского сертификата (и закрытого ключа), если это требуется сервером ClickHouse, и большинство других параметров, связанных с более специализированной настройкой безопасности.

### Аутентификация {#authentication}

Укажите структуру Auth в параметрах подключения для указания имени пользователя и пароля.
...
- `ConnOpenInOrder` (по умолчанию) — адреса используются по порядку. Последующие адреса применяются только в случае неудачного подключения к адресам, расположенным ранее в списке. Это фактически стратегия отказоустойчивости.
...
Для вставки большого количества строк клиент предоставляет пакетную семантику. Это требует подготовки пакета, к которому можно добавлять строки. Затем пакет отправляется с помощью метода `Send()`. Пакеты хранятся в памяти до выполнения `Send`.

Рекомендуется вызывать `Close` для пакета, чтобы предотвратить утечку соединений. Это можно сделать с помощью ключевого слова `defer` после подготовки пакета. Это очистит соединение, если `Send` так и не будет вызван. Обратите внимание, что в этом случае в журнале запросов будет зафиксирована вставка 0 строк, если ни одна строка не была добавлена в пакет.
...
Рекомендации для ClickHouse применимы [здесь](/guides/inserting-data#best-practices-for-inserts). Пакеты не следует использовать совместно между go-подпрограммами — создавайте отдельный пакет для каждой подпрограммы.
...
Пользователи могут либо запросить одну строку с помощью метода `QueryRow`, либо получить курсор для итерации по набору результатов через `Query`. Первый метод принимает место назначения для десериализации данных, второй требует вызова `Scan` для каждой строки.
...
Обратите внимание, что в обоих случаях необходимо передать указатель на переменные, в которые мы хотим десериализовать соответствующие значения столбцов. Они должны передаваться в порядке, указанном в операторе `SELECT` — по умолчанию при использовании `SELECT *` будет использоваться порядок объявления столбцов, как показано выше.


Аналогично вставке, метод Scan требует, чтобы целевые переменные имели соответствующий тип. Это также обеспечивает гибкость: типы преобразуются там, где это возможно, при условии отсутствия потери точности. Например, в приведенном выше примере показано, как столбец UUID считывается в строковую переменную. Полный список поддерживаемых типов Go для каждого типа столбца см. в разделе [Преобразования типов](#type-conversions).

Наконец, обратите внимание на возможность передачи `Context` в методы `Query` и `QueryRow`. Это можно использовать для настроек на уровне запроса — подробнее см. в разделе [Использование Context](#using-context).

### Асинхронная вставка {#async-insert}

Асинхронные вставки поддерживаются через метод Async. Это позволяет пользователю указать, должен ли клиент ожидать завершения вставки сервером или ответить сразу после получения данных. Фактически это управляет параметром [wait_for_async_insert](/operations/settings/settings#wait_for_async_insert).
...
Вставки могут выполняться в колоночном формате. Это может обеспечить преимущества в производительности, если данные уже организованы в такой структуре, избегая необходимости преобразования в строки.
...
Для пользователей структуры Golang обеспечивают логическое представление строки данных в ClickHouse. Для этого нативный интерфейс предоставляет несколько удобных функций.

#### Select с сериализацией {#select-with-serialize}

Метод Select позволяет преобразовать набор строк ответа в срез структур одним вызовом.
...
`AppendStruct` позволяет добавить структуру к существующему [пакету](#batch-insert) и интерпретировать её как полную строку. Для этого требуется, чтобы поля структуры совпадали с колонками таблицы как по имени, так и по типу. Хотя все колонки должны иметь соответствующее поле в структуре, некоторые поля структуры могут не иметь соответствующей колонки. Такие поля будут просто проигнорированы.
...
Клиент стремится обеспечить максимальную гибкость при работе с различными типами переменных как для вставки данных, так и для преобразования ответов. В большинстве случаев для типа колонки ClickHouse существует эквивалентный тип Golang, например [UInt64](/sql-reference/data-types/int-uint/) соответствует [uint64](https://pkg.go.dev/builtin#uint64). Эти логические соответствия всегда должны поддерживаться. Пользователи могут использовать типы переменных, которые можно вставить в колонки или использовать для получения ответа, если предварительно выполняется преобразование переменной или полученных данных. Клиент поддерживает эти преобразования прозрачно, чтобы пользователям не требовалось преобразовывать свои данные для точного соответствия перед вставкой, а также обеспечивает гибкое преобразование при выполнении запросов. Прозрачное преобразование не допускает потери точности. Например, uint32 нельзя использовать для получения данных из колонки UInt64. С другой стороны, строку можно вставить в поле datetime64 при условии, что она соответствует требованиям формата.
...
Эта работа продолжается и может быть разделена на операции вставки (`Append`/`AppendRow`) и чтения (через `Scan`). Если вам требуется поддержка конкретного преобразования, пожалуйста, создайте issue.

### Сложные типы {#complex-types}

#### Типы Date/DateTime {#datedatetime-types}

Клиент ClickHouse для Go поддерживает типы даты и времени `Date`, `Date32`, `DateTime` и `DateTime64`. Даты можно вставлять в виде строки в формате `2006-01-02` или с использованием встроенных типов Go `time.Time{}` или `sql.NullTime`. Типы DateTime также поддерживают эти типы, но требуют передачи строк в формате `2006-01-02 15:04:05` с необязательным смещением часового пояса, например `2006-01-02 15:04:05 +08:00`. При чтении поддерживаются `time.Time{}` и `sql.NullTime`, а также любая реализация интерфейса `sql.Scanner`.

Обработка информации о часовом поясе зависит от типа ClickHouse и от того, выполняется ли вставка или чтение значения:


- **DateTime/DateTime64**
  - При **вставке** значение отправляется в ClickHouse в формате временной метки UNIX. Если часовой пояс не указан, клиент использует локальный часовой пояс. `time.Time{}` или `sql.NullTime` будут соответствующим образом преобразованы в эпоху.
  - При **выборке** используется часовой пояс столбца, если он установлен, при возврате значения `time.Time`. В противном случае используется часовой пояс сервера.
- **Date/Date32**
  - При **вставке** часовой пояс даты учитывается при преобразовании даты в временную метку unix, то есть дата смещается на часовой пояс перед сохранением, поскольку типы Date не имеют локали в ClickHouse. Если часовой пояс не указан в строковом значении, используется локальный часовой пояс.
  - При **выборке** даты сканируются в экземпляры `time.Time{}` или `sql.NullTime{}` и возвращаются без информации о часовом поясе.

#### Массив {#array}

Массивы должны вставляться как срезы. Правила типизации для элементов соответствуют правилам для [примитивных типов](#type-conversions), то есть элементы будут преобразованы, где это возможно.
...
Карты должны вставляться как карты Golang с ключами и значениями, соответствующими правилам типизации, определенным [ранее](#type-conversions).
...
Кортежи представляют собой группу столбцов произвольной длины. Столбцы могут быть либо явно именованы, либо указывать только тип, например:
...
Из этих подходов именованные кортежи обеспечивают большую гибкость. В то время как неименованные кортежи должны вставляться и читаться с использованием срезов, именованные кортежи также совместимы с картами.
...
// как именованные, так и неименованные кортежи могут быть добавлены с помощью срезов. Обратите внимание, что мы можем использовать строго типизированные списки и карты, если все элементы одного типа
...
// именованные кортежи могут быть извлечены в карту или срезы, неименованные - только в срезы
...
Примечание: поддерживаются типизированные срезы и карты при условии, что все подстолбцы в именованном кортеже имеют одинаковый тип.

#### Nested {#nested}

Поле Nested эквивалентно массиву именованных кортежей. Использование зависит от того, установлен ли параметр [flatten_nested](/operations/settings/settings#flatten_nested) в значение 1 или 0.

При установке flatten_nested в 0 столбцы Nested остаются единым массивом кортежей. Это позволяет использовать срезы карт для вставки и извлечения данных с произвольными уровнями вложенности. Ключ карты должен совпадать с именем столбца, как показано в примере ниже.

Примечание: поскольку карты представляют кортеж, они должны иметь тип `map[string]interface{}`. В настоящее время значения не являются строго типизированными.
...
Если для параметра `flatten_nested` используется значение по умолчанию 1, вложенные столбцы разворачиваются в отдельные массивы. Это требует использования вложенных срезов для вставки и извлечения данных. Хотя произвольные уровни вложенности могут работать, официальная поддержка для них отсутствует.
...
Благодаря более простому интерфейсу и официальной поддержке вложенности рекомендуется использовать `flatten_nested=0`.

#### Геотипы {#geo-types}


Клиент поддерживает геотипы Point, Ring, Polygon и MultiPolygon. Эти поля используются в Golang с помощью пакета [github.com/paulmach/orb](https://github.com/paulmach/orb).
...
Тип UUID поддерживается пакетом [github.com/google/uuid](https://github.com/google/uuid). Пользователи также могут отправлять и сериализовать UUID как строку или любой тип, реализующий `sql.Scanner` или `Stringify`.
...
Поддержка методов сжатия зависит от используемого протокола. Для нативного протокола клиент поддерживает сжатие `LZ4` и `ZSTD`. Сжатие выполняется только на уровне блоков. Чтобы включить сжатие, необходимо добавить конфигурацию `Compression` при установке соединения.
...
Контексты Go предоставляют механизм передачи дедлайнов, сигналов отмены и других значений в области запроса через границы API. Все методы соединения принимают контекст в качестве первого параметра. Хотя в предыдущих примерах использовался context.Background(), пользователи могут использовать эту возможность для передачи настроек и дедлайнов, а также для отмены запросов.

Передача контекста, созданного с помощью `withDeadline`, позволяет устанавливать временные ограничения на выполнение запросов. Обратите внимание, что это абсолютное время, и по истечении срока соединение будет освобождено, а в ClickHouse будет отправлен сигнал отмены. В качестве альтернативы можно использовать `WithCancel` для явной отмены запроса.

Вспомогательные функции `clickhouse.WithQueryID` и `clickhouse.WithQuotaKey` позволяют указать идентификатор запроса и ключ квоты. Идентификаторы запросов могут быть полезны для отслеживания запросов в логах и для целей отмены. Ключ квоты можно использовать для установки ограничений на использование ClickHouse на основе уникального значения ключа — подробнее см. в разделе [Управление квотами](/operations/access-rights#quotas-management).

Пользователи также могут использовать контекст, чтобы применить настройку только для конкретного запроса, а не для всего соединения, как показано в разделе [Настройки соединения](#connection-settings).

Наконец, пользователи могут управлять размером буфера блоков с помощью `clickhouse.WithBlockSize`. Это переопределяет настройку уровня соединения `BlockBufferSize` и контролирует максимальное количество блоков, которые декодируются и хранятся в памяти в любой момент времени. Большие значения потенциально означают большую степень параллелизации за счёт памяти.
...
// можно использовать контекст для передачи настроек конкретному вызову API
...
// установка предельного времени для запроса - запрос будет отменен после достижения указанного времени.
...
Для получения этой информации необходимо использовать [Context](#using-context), которому можно передать функции обратного вызова.
...
[Внешние таблицы](/engines/table-engines/special/external-data/) позволяют клиенту отправлять данные в ClickHouse вместе с запросом SELECT. Эти данные помещаются во временную таблицу и могут использоваться непосредственно в запросе для вычислений.

Для отправки внешних данных на сервер вместе с запросом необходимо создать внешнюю таблицу с помощью `ext.NewTable` и передать её через контекст.
...
Далее приводится структура, соответствующая документации для API ClickHouse.
...
Подключение может быть выполнено либо через строку DSN в формате `clickhouse://<host>:<port>?<query_option>=<value>` и метод `Open`, либо через метод `clickhouse.OpenDB`. Последний не является частью спецификации `database/sql`, но возвращает экземпляр `sql.DB`. Этот метод предоставляет функциональность, такую как профилирование, для которой нет очевидных способов предоставления через спецификацию `database/sql`.
...
В строке DSN могут быть переданы следующие параметры:
...
- `connection_open_strategy` — `random/in_order` (по умолчанию `random`) — см. [Подключение к нескольким узлам](#connecting-to-multiple-nodes)
  - `round_robin` — выбор сервера по циклическому алгоритму из набора
...
При использовании `OpenDB` для подключения к нескольким хостам применяется тот же подход с параметрами, что и для ClickHouse API, с возможностью указания `ConnOpenStrategy`.

Для соединений на основе DSN строка поддерживает указание нескольких хостов и параметр `connection_open_strategy`, для которого можно установить значение `round_robin` или `in_order`.
...
При использовании `OpenDB` данные для аутентификации передаются через стандартные параметры. Для соединений на основе DSN имя пользователя и пароль можно передать в строке подключения — либо как параметры, либо как учетные данные, закодированные в адресе.
...
После установления соединения можно выполнять `sql`-инструкции с помощью метода Exec.
...
Пакетная обработка реализуется путем создания `sql.Tx` с помощью метода `Begin`. Затем можно получить пакет, используя метод `Prepare` с инструкцией `INSERT`. Этот метод возвращает `sql.Stmt`, к которому можно добавлять строки с помощью метода `Exec`. Пакет будет накапливаться в памяти до выполнения `Commit` на исходном `sql.Tx`.
...
Асинхронные вставки можно выполнить, используя метод `ExecContext`. Ему следует передать контекст с включенным асинхронным режимом, как показано ниже. Это позволяет указать, должен ли клиент ожидать завершения вставки на сервере или получить ответ сразу после получения данных сервером. Фактически это управляет параметром [wait_for_async_insert](/operations/settings/settings#wait_for_async_insert).
...
// 250
fmt.Printf("Количество с позиционной привязкой: %d\n", count)
// нумерованная привязка
if err = conn.QueryRow(ctx, "SELECT count() FROM example WHERE Col1 <= $2 AND Col3 > $1", now.Add(time.Duration(150)*time.Second), 250).Scan(&count); err != nil {
    return err
}
// 100
fmt.Printf("Количество с нумерованной привязкой: %d\n", count)
// именованная привязка
if err = conn.QueryRow(ctx, "SELECT count() FROM example WHERE Col1 <= @col1 AND Col3 > @col3", clickhouse.Named("col1", 100), clickhouse.Named("col3", now.Add(time.Duration(50)*time.Second))).Scan(&count); err != nil {
    return err
}
...
// для создания столбца JSON необходимо установить allow_experimental_object_type=1
...
// установка срока выполнения для запроса — это отменит запрос после достижения абсолютного времени. Опять же, завершается только соединение,
// запросы продолжат выполняться до завершения в ClickHouse
ctx, cancel = context.WithDeadline(context.Background(), time.Now().Add(-time.Second))
defer cancel()
if err := conn.PingContext(ctx); err == nil {
    return fmt.Errorf("ожидалось превышение срока выполнения")
}
...
Аналогично [ClickHouse API](#dynamic-scanning), доступна информация о типах столбцов, что позволяет пользователям создавать экземпляры правильно типизированных переменных во время выполнения, которые могут быть переданы в Scan. Это позволяет читать столбцы с неизвестным типом.
...
Чтобы отправить внешние данные на сервер вместе с запросом, необходимо создать внешнюю таблицу с помощью `ext.NewTable` и передать её через контекст.
...
ClickHouse позволяет передавать [контекст трассировки](/operations/opentelemetry/) как часть нативного протокола. Клиент позволяет создать Span с помощью функции `clickhouse.withSpan` и передать его через Context. Эта функциональность не поддерживается при использовании HTTP в качестве транспорта.
...
- При чтении больших наборов данных рассмотрите возможность изменения параметра [`BlockBufferSize`](#connection-settings). Это увеличит потребление памяти, но позволит декодировать больше блоков параллельно во время итерации по строкам. Значение по умолчанию 2 является консервативным и минимизирует накладные расходы по памяти. Более высокие значения означают большее количество блоков в памяти. Это требует тестирования, поскольку разные запросы могут создавать блоки различного размера. Поэтому данный параметр можно установить на [уровне запроса](#using-context) через Context.
- Будьте точны в указании типов при вставке данных. Хотя клиент стремится быть гибким, например, позволяя разбирать строки для UUID или IP-адресов, это требует валидации данных и влечет дополнительные затраты во время вставки.
