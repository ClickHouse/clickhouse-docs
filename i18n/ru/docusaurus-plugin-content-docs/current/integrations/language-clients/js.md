...
Он не имеет зависимостей, оптимизирован для максимальной производительности и протестирован с различными версиями и конфигурациями ClickHouse (on-premise‑развёртывание с одним узлом, on-premise‑кластер и ClickHouse Cloud).
...
| 20.x            | ✔          |
| 18.x            | ✔          |
| 16.x            | В меру возможности |


## Требования к окружению (веб) {#environment-requirements-web}

Веб-версия клиента официально протестирована в последних версиях браузеров Chrome и Firefox и может использоваться в качестве зависимости, например, в приложениях React/Vue/Angular или в Cloudflare Workers.
...
Если что-то неясно или отсутствует в примерах или в документации ниже, не стесняйтесь [связаться с нами](./js.md#contact-us).
...
Если ваше окружение не поддерживает модули ESM, можно использовать синтаксис CJS:
...
| **roles**?: string \| string[]                                           | Имя (имена) ролей ClickHouse, присоединяемых к исходящим запросам.                  | -                       | [Использование ролей с HTTP-интерфейсом](/interfaces/http#setting-role-with-query-parameters) |
...
| -------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------- |
| **max_open_connections**?: number                                          | Максимальное количество одновременных подключений на хост.    | `10`          | -                                                                                                    |
...
| **set_basic_auth_header**?: boolean <br/><ExperimentalBadge/>              | Устанавливать заголовок `Authorization` с учетными данными базовой аутентификации. | `true`        | [использование этой настройки в документации по HTTP-агенту](./js.md#custom-httphttps-agent-experimental-nodejs-only) |

### Конфигурация URL {#url-configuration}

:::important
Конфигурация URL _всегда_ переопределяет жестко заданные значения, при этом будет записано предупреждение.
:::
...
если эта таблица включена в [конфигурации сервера](/operations/server-configuration-parameters/settings), или для отмены долго выполняющихся запросов (см. [пример](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/cancel_query.ts)). При необходимости `query_id` может быть переопределен пользователем в параметрах методов `command`/`query`/`exec`/`insert`.
...
:::note
Для вставки данных существует специальный метод [insert](./js.md#insert-method), а для DDL-операторов — [command](./js.md#command-method).
:::
...
Вы можете обработать `ResultSet`, вызвав методы `text` или `json` на объекте `ResultSet` и загрузив весь набор строк, возвращенных запросом, в память.


Вы должны начинать читать содержимое `ResultSet` как можно раньше, поскольку он удерживает поток ответа открытым и, как следствие, занимает используемое соединение. Клиент не буферизует входящие данные, чтобы избежать потенциально чрезмерного использования памяти приложением.

Если данные слишком большие, чтобы поместиться в памяти целиком, вы можете вызвать метод `stream` и обрабатывать данные в потоковом режиме. Каждый из фрагментов ответа будет преобразован в относительно небольшой массив строк (размер этого массива зависит от размера конкретного фрагмента, который клиент получает от сервера, так как он может меняться, и от размера отдельной строки), обрабатываемый по одному фрагменту за раз.

Обратитесь к списку [поддерживаемых форматов данных](./js.md#supported-data-formats), чтобы определить, какой формат лучше всего подходит для потоковой передачи в вашем случае. Например, если вы хотите передавать объекты JSON, вы можете выбрать формат [JSONEachRow](/interfaces/formats/JSONEachRow), и каждая строка будет разобрана как объект JS, либо, возможно, более компактный формат [JSONCompactColumns](/interfaces/formats/JSONCompactColumns), при котором каждая строка будет представлять собой компактный массив значений. См. также: [потоковая передача файлов](./js.md#streaming-files-nodejs-only).

:::important
Если `ResultSet` или его поток не будут полностью прочитаны, они будут уничтожены по истечении периода бездействия `request_timeout`.
:::
...
**Пример:** (Node.js/Web) Запрос с результирующим набором данных в формате `JSONEachRow`, при котором весь поток считывается и его содержимое разбирается в объекты JS.
...
**Пример:** (только Node.js) потоковое получение результата запроса в формате `JSONEachRow` с использованием классического подхода `on('data')`. Взаимозаменяемо с синтаксисом `for await const`. [Исходный код](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_streaming_json_each_row.ts).
...
**Пример:** (только для Node.js) Потоковая выборка результата запроса в формате `CSV` с использованием классического подхода `on('data')`. Взаимозаменяемо с синтаксисом `for await const`.
...
Возвращаемый тип минимален, поскольку мы не ожидаем получения данных от сервера и немедленно опустошаем поток ответа.

Если методу insert передан пустой массив, оператор вставки не будет отправлен на сервер; вместо этого метод немедленно вернёт `{ query_id: '...', executed: false }`. Если в этом случае `query_id` не был указан в параметрах метода, в результате будет пустая строка, поскольку возврат случайного UUID, сгенерированного клиентом, может вызвать путаницу — запрос с таким `query_id` не будет существовать в таблице `system.query_log`.
...
Дано определение таблицы следующего вида:
...
Дополнительные подробности см. в [исходном коде](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/insert_exclude_columns.ts).

**Пример**: Вставка в базу данных, отличную от той, которая была указана при создании экземпляра клиента. [Исходный код](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/insert_into_different_db.ts).
...
Следовательно, интерфейс `InsertParams` для веб-версии немного отличается от версии для Node.js,
...
Может использоваться для операторов, которые не возвращают результат, когда предложение format неприменимо, или когда вас вообще не интересует ответ. Примерами таких операторов могут быть `CREATE TABLE` или `ALTER TABLE`.
...
:::important
Запрос, отменённый с помощью `abort_signal`, не гарантирует, что оператор не был выполнен сервером.
:::
...
/\*\* Параметры запроса для проверки работоспособности — с использованием встроенного эндпоинта `/ping`.
...
- Это поведение по умолчанию для версии Node.js. _/
  ...
  /** Параметры запроса для проверки работоспособности — с использованием запроса SELECT.
- Это поведение по умолчанию для веб-версии, так как эндпоинт `/ping` не поддерживает CORS.
- Большинство стандартных параметров метода `query`, например `query_id`, `abort_signal`, `http_headers` и т. д., будут работать,
- за исключением `query_params`, которое в этом методе не имеет смысла разрешать. \*/
...
Ping может быть полезным инструментом для проверки доступности сервера при запуске приложения, особенно в ClickHouse Cloud, где экземпляр может простаивать и «просыпаться» после ping-запроса: в таком случае вы можете повторить попытку несколько раз с паузой между ними.

Обратите внимание, что по умолчанию версия для Node.js использует эндпоинт `/ping`, тогда как веб-версия использует простой запрос `SELECT 1` для достижения аналогичного результата, поскольку эндпоинт `/ping` не поддерживает CORS.

**Пример:** (Node.js/Web) простой ping экземпляра сервера ClickHouse. Примечание: для веб-версии перехватываемые ошибки будут отличаться.
...
**Пример:** если вы хотите дополнительно проверить учетные данные при вызове метода `ping` или указать дополнительные параметры, такие как `query_id`, вы можете использовать его следующим образом:
...
Единственное отличие — формат, указываемый при вызове `query` (`JSONEachRow`, `CSV` и т. д.), и имя выходного файла.
...
Клиент поддерживает потоковую передачу JSON-объектов в таких форматах, как [JSONEachRow](/interfaces/formats/JSONEachRow) (см. обзор таблицы для других форматов с поддержкой потоковой передачи; см. также [примеры `select_streaming_` в репозитории клиента](https://github.com/ClickHouse/clickhouse-js/tree/main/examples/node)).
...
Для Parquet основным сценарием использования `SELECT`-запросов, вероятнее всего, будет запись результирующего потока в файл. См. [пример](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_parquet_as_file.ts) в репозитории клиента.

`JSONEachRowWithProgress` — это формат только для вывода, который поддерживает отображение прогресса в потоке. Для получения дополнительных сведений см. [этот пример](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/node/select_json_each_row_with_progress.ts).
...
Поскольку клиент вставляет значения без дополнительного преобразования типов, столбцы типа `Date`/`Date32` можно вставлять только в виде
строк.
...
Хотя сервер может принимать значения как числа, в выходных форматах семейства `JSON*` они возвращаются в виде строк во избежание переполнения целых чисел, так как максимальные значения для этих типов превышают `Number.MAX_SAFE_INTEGER`.
...
Примечание: сжатие запросов в настоящее время недоступно в веб-версии. Сжатие ответов работает в обычном режиме. Версия для Node.js поддерживает оба варианта.
...
В настоящее время клиент логирует следующие события:
...
- `WARN` — некритичные ошибки; неудачный запрос `ping` логируется как предупреждение, поскольку базовая ошибка включена в возвращаемый результат
- `ERROR` — критичные ошибки из методов `query`/`insert`/`exec`/`command`, такие как неудачный запрос
...
Клиент Node.js опционально поддерживает как базовый TLS (только центр сертификации),
так и взаимный TLS (центр сертификации и клиентские сертификаты).

Пример базовой конфигурации TLS, предполагая, что ваши сертификаты находятся в папке `certs`,
...
Если вы изменяете `keep_alive.idle_socket_ttl`, помните, что это значение должно всегда быть синхронизировано с конфигурацией Keep-Alive вашего сервера/балансировщика нагрузки и должно быть **всегда ниже**, гарантируя, что сервер никогда не закроет открытое соединение первым.
...
- Если эта ошибка возникает во время длительных запросов без входящих или исходящих данных (например, длительный `INSERT FROM SELECT`), это может быть связано с тем, что балансировщик нагрузки закрывает простаивающие соединения. Вы можете попробовать принудительно передавать данные во время длительных запросов, используя комбинацию следующих настроек ClickHouse:
  ...
  Также возможно использовать совершенно другой подход, полностью избегая времени ожидания на канале связи; это можно сделать, используя «особенность» HTTP-интерфейса, заключающуюся в том, что мутации не отменяются при потере соединения. См. [этот пример (часть 2)](https://github.com/ClickHouse/clickhouse-js/blob/main/examples/long_running_queries_timeouts.ts) для получения дополнительной информации.
...
При использовании клиента с [пользователем readonly=1](/operations/settings/permissions-for-queries#readonly) сжатие ответов не может быть включено, поскольку для этого требуется настройка `enable_http_compression`. Следующая конфигурация приведёт к ошибке:
...
- Параметры `max_open_connections` и `tls` _не будут иметь эффекта_ и будут проигнорированы клиентом, так как они являются частью конфигурации базового агента.
...
- Хотя управление неактивными keep-alive-сокетами будет продолжать работать (так как оно привязано не к агенту, а к конкретному сокету), теперь можно полностью отключить его, установив значение `keep_alive.idle_socket_ttl` равным `0`.
...
При использовании сертификатов _и_ пользовательского _HTTPS_-агента, вероятно, потребуется отключить заголовок авторизации по умолчанию через параметр `set_basic_auth_header` (введён в версии 1.2.0), так как он конфликтует с заголовками TLS. Все заголовки TLS должны быть предоставлены вручную.
...
- Потоковая передача для SELECT-запросов работает, но для INSERT отключена (в том числе на уровне типов).
...
- Для снижения потребления памяти приложением рекомендуется использовать потоки для больших вставок (например, из файлов) и выборок, где это применимо. Для обработчиков событий и аналогичных сценариев использования хорошим вариантом могут быть [асинхронные вставки](/optimize/asynchronous-inserts), позволяющие минимизировать или даже полностью избежать пакетной обработки на стороне клиента. Примеры асинхронных вставок доступны в [репозитории клиента](https://github.com/ClickHouse/clickhouse-js/tree/main/examples) с префиксом имени файла `async_insert_`.
- Клиент не включает сжатие запросов или ответов по умолчанию. Однако при выборке или вставке больших наборов данных можно рассмотреть возможность включения сжатия через параметр `ClickHouseClientConfigOptions.compression` (либо только для `request`, либо для `response`, либо для обоих).
- Сжатие существенно снижает производительность. Его включение для `request` или `response` негативно повлияет на скорость выборок или вставок соответственно, но уменьшит объём сетевого трафика, передаваемого приложением.


## Свяжитесь с нами {#contact-us}

Если у вас возникли вопросы или вам нужна помощь, обращайтесь к нам в [Community Slack](https://clickhouse.com/slack) (канал `#clickhouse-js`) или через [GitHub issues](https://github.com/ClickHouse/clickhouse-js/issues).
