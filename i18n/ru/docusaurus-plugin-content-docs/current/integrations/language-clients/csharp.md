...
description: 'Официальный клиент на C# для подключения к ClickHouse.'
title: 'Драйвер ClickHouse для C#'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
Флаг `UseSession` включает сохранение серверной сессии, позволяя использовать операторы `SET` и временные таблицы. Сессия будет сброшена после 60 секунд неактивности (таймаут по умолчанию). Время жизни сессии можно продлить, установив настройки сессии через операторы ClickHouse.
...
- Соединения могут оставаться активными после того, как объект `ClickHouseConnection` был удален.
- Это поведение можно настроить, передав специальный `HttpClient` с пользовательским `HttpClientHandler`.

Для сред с внедрением зависимостей (DI) существует специальный конструктор `ClickHouseConnection(string connectionString, IHttpClientFactory httpClientFactory, string httpClientName = "")`, который позволяет обобщить настройки HTTP-клиента.

**Рекомендации:**

- `ClickHouseConnection` представляет собой «сессию» с сервером. Он выполняет обнаружение возможностей путем запроса версии сервера (поэтому при открытии есть небольшие накладные расходы), но в целом безопасно создавать и уничтожать такие объекты несколько раз.
- Рекомендуемое время жизни соединения — один объект соединения на большую «транзакцию», охватывающую несколько запросов. При запуске соединения есть небольшие накладные расходы, поэтому не рекомендуется создавать объект соединения для каждого запроса.
...
- Значения `DateTime` возвращаются в формате UTC. Пользователь может самостоятельно преобразовать их или использовать метод `ToLocalTime()` экземпляра `DateTime`.
- При вставке значения `DateTime` обрабатываются следующим образом:
  - Значения `DateTime` в формате `UTC` вставляются «как есть», поскольку ClickHouse хранит их внутренне в UTC.
  - Значения `DateTime` в формате `Local` преобразуются в UTC в соответствии с настройками локального часового пояса пользователя.
  - Значения `DateTime` в формате `Unspecified` считаются находящимися в часовом поясе целевого столбца и преобразуются в UTC в соответствии с этим часовым поясом.
- Для столбцов без указанного часового пояса по умолчанию используется часовой пояс клиента (устаревшее поведение). Для использования часового пояса сервера можно указать флаг `UseServerTimezone` в строке подключения.
...
Клиент ClickHouse .NET интегрируется с абстракциями `Microsoft.Extensions.Logging` для обеспечения легковесного логирования по требованию. При включении драйвер генерирует структурированные сообщения о событиях жизненного цикла соединения, выполнении команд, транспортных операциях и массовой загрузке данных. Логирование полностью опционально — приложения, не настроившие логгер, продолжают работать без дополнительных накладных расходов.
...
| `ClickHouse.Driver.Command`    | `ClickHouseCommand`    | Начало/завершение выполнения запросов, время выполнения, идентификаторы запросов, статистика сервера, детали ошибок. |
| `ClickHouse.Driver.Transport`  | `ClickHouseConnection` | Низкоуровневые HTTP-запросы с потоковой передачей, флаги сжатия, коды статуса ответов, сбои транспорта.              |
...
Для помощи в диагностике сетевых проблем библиотека драйвера включает вспомогательный инструмент, который активирует низкоуровневую трассировку внутренних механизмов сети .NET. Чтобы включить его, необходимо передать LoggerFactory с уровнем Trace и установить EnableDebugMode в true (или включить вручную через класс `ClickHouse.Driver.Diagnostic.TraceHelper`). Предупреждение: это создаст чрезвычайно подробные логи и повлияет на производительность. Не рекомендуется включать режим отладки в production-окружении.
