...
title: 'Драйвер C# для ClickHouse'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
Вложенные типы ClickHouse (`Nested(...)`) можно читать и записывать, используя семантику массивов.
...
- SQL-параметры передаются как параметры HTTP URI-запроса, поэтому использование слишком большого их количества может привести к исключению «URL too long».
...
- При вставке значения `DateTime` обрабатываются следующим образом:
  - `UTC` `DateTime` вставляются «как есть», поскольку ClickHouse хранит их внутренне в UTC.
...
Драйвер использует выделенные категории, позволяющие точно настроить уровни логирования для каждого компонента:
...
Для диагностики сетевых проблем библиотека драйвера включает вспомогательный инструмент, активирующий низкоуровневую трассировку внутренних механизмов сети .NET. Чтобы включить его, необходимо передать LoggerFactory с уровнем Trace и установить EnableDebugMode в true (или включить вручную через класс `ClickHouse.Driver.Diagnostic.TraceHelper`). Предупреждение: это создаст чрезвычайно подробные логи и повлияет на производительность. Не рекомендуется включать режим отладки в продакшене.
...
var loggerFactory = LoggerFactory.Create(builder =>
{
    builder
        .AddConsole()
        .SetMinimumLevel(LogLevel.Trace); // Должен быть уровень Trace для просмотра сетевых событий
});
...
{
LoggerFactory = loggerFactory,
EnableDebugMode = true, // Включить трассировку на сетевом уровне
};
