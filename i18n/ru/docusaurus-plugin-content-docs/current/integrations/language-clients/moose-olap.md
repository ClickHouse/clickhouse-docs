---
description: 'Начните работу со стеком Moose — подходом code-first к разработке поверх ClickHouse с типобезопасными схемами и локальной средой разработки'
...
[Moose OLAP](https://docs.fiveonefour.com/moose/olap) — это основной модуль [Moose Stack](https://docs.fiveonefour.com/moose), открытого набора инструментов для разработчиков для создания аналитических бекендов реального времени на TypeScript и Python. 

Moose OLAP предлагает удобные для разработчиков абстракции и функциональность, аналогичную ORM, изначально разработанные для ClickHouse.



## Ключевые возможности Moose OLAP {#key-features}

- **Схемы как код**: Определяйте таблицы ClickHouse на TypeScript или Python с безопасностью типов и автодополнением в IDE
...
**Важно**: Ваш продакшн-экземпляр ClickHouse останется нетронутым. Будет инициализирован только новый проект Moose OLAP с моделями данных, созданными на основе ваших таблиц ClickHouse.


```bash
# TypeScript
moose init my-project --from-remote <ВАША_СТРОКА_ПОДКЛЮЧЕНИЯ_CLICKHOUSE> --language typescript
```


# Python

moose init my-project --from-remote <СТРОКА_ПОДКЛЮЧЕНИЯ_CLICKHOUSE> --language python
...
```bash
https://имя_пользователя:пароль@хост:порт/?database=имя_базы_данных
````
...
**Важно**: Ваш продакшн-экземпляр ClickHouse останется нетронутым. Это создаёт локальную среду разработки.
