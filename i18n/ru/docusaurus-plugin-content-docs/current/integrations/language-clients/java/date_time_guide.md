...
Драйвер не преобразует целочисленные значения ни в один из типов даты/времени. Вызов `pstmt.setLong("timestamp", 1772132359L)` приведёт к тому, что значение `1772132359` будет записано на сервер как число, которое будет интерпретировано сервером как
Unix timestamp в секундах в часовом поясе UTC.
