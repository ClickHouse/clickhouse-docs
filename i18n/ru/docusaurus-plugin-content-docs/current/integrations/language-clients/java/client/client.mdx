...
>
  <Version>
    ...
    * проверьте, что CN в клиентском сертификате соответствует имени пользователя:
    ...
    <Tabs groupId="client-config">
      <TabItem value="connection" label="Подключение и конечные точки">
      ...
      <TabItem value="Аутентификация" label="Аутентификация">
        ...
        | `setPassword(String password)`                       | `password` - секретное значение                                | Задает секретное значение для аутентификации по паролю и фактически выбирает этот метод аутентификации                                                                  | -         | `password`            |
        ...
        | `useSSLAuthentication(boolean useSSLAuthentication)` | `useSSLAuthentication` - флаг для включения SSL-аутентификации | Устанавливает аутентификацию по клиентскому SSL-сертификату.                                                                                                            | -         | `ssl_authentication`  |
        | `useHTTPBasicAuth(boolean useBasicAuth)`             | `useBasicAuth` - флаг включения/отключения                     | Определяет, следует ли использовать базовую HTTP-аутентификацию для аутентификации по имени пользователя и паролю. Решает проблемы с паролями, содержащими спецсимволы. | `true`    | `http_use_basic_auth` |
      ...
      <TabItem value="Расширенные" label="Расширенные">
        | Method                                                                    | Arguments                                                                       | Description                                                                                                                                                                | Default  | Key                          |
        | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------- |
    ...
    * `-` — операция не поддерживается в ClickHouse для этого формата
    ...
    | [RowBinaryWithNames](/interfaces/formats/RowBinaryWithNamesAndTypes)                                         |      full      |      полный     |
    ...
    **Методы настройки**
    ...
    | `setUseTimeZone(String timeZone)`                 | Запрашивает, чтобы сервер использовал часовой пояс `timeZone` для преобразования времени. См. [session&#95;timezone](/operations/settings/settings#session_timezone).  |
    ...
    * Справочная реализация сервиса Spring: [implementation](https://github.com/ClickHouse/clickhouse-java/tree/main/examples/demo-service)
    ...
    **Возвращаемое значение**
    ...
    Извлекает схему из SQL-команды.
  ...
  <Version>
    ...
    **Кодирование в формате RowBinary**
    ...
    | Свойство                      | По умолчанию                             | Описание                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
    | ----------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | load&#95;balancing&#95;policy | `""`                                     | Политика балансировки нагрузки может иметь одно из следующих значений: <li>`firstAlive` — запрос отправляется первому работоспособному узлу из списка управляемых узлов</li><li>`random` — запрос отправляется случайному узлу из списка управляемых узлов </li><li>`roundRobin` — запросы по очереди отправляются на каждый узел из списка управляемых узлов</li><li>полное имя класса, реализующего интерфейс `ClickHouseLoadBalancingPolicy`, — пользовательская политика балансировки нагрузки</li>Если политика не указана, запрос отправляется на первый узел из списка управляемых узлов |
    ...
    | check&#95;all&#95;nodes       | `false`                                  | Определяет, выполнять ли проверку работоспособности для всех узлов или только для проблемных.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
    ...
    | Свойство                           | По умолчанию | Описание                                                                                                                                                                                                                                                                     |
    | ---------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | failover                           | `0`          | Максимальное число переключений (failover) для одного запроса. Нулевое или отрицательное значение означает отсутствие failover. При failover неуспешный запрос отправляется на другой узел (в соответствии с политикой балансировки нагрузки) для восстановления после сбоя. |
