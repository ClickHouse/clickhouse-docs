...
title: 'Клиент ClickHouse на Rust'
doc_type: 'reference'
---
...
Для решения этой проблемы можно использовать версию v0.11+ и включить функцию `wa-37420`. Примечание: данную функцию не следует использовать с более новыми версиями ClickHouse.
...
:::tip
Повторно используйте созданные клиенты или клонируйте их для повторного использования базового пула соединений hyper.
:::
...
Примечание: поскольку весь ответ передается потоком, курсоры могут вернуть ошибку даже после получения некоторых строк. Если это происходит в вашем случае, попробуйте использовать `query(...).with_option("wait_end_of_query", "1")` для включения буферизации ответа на стороне сервера. [Подробнее](/interfaces/http/#response-buffering). Опция `buffer_size` также может быть полезна.
...
Аналогично `query_id`, вы можете установить `session_id` для выполнения операторов в рамках одной сессии. `session_id` может быть установлен глобально на уровне клиента или для каждого вызова `query`, `insert` или `inserter`.
...
:::danger
При кластерных развертываниях из-за отсутствия «закрепленных сессий» необходимо подключаться к _конкретному узлу кластера_ для корректного использования этой функции, поскольку, например, балансировщик нагрузки с алгоритмом round-robin не гарантирует, что последовательные запросы будут обработаны одним и тем же узлом ClickHouse.
:::
...
:::warning
Этот пример использует устаревший API Hyper и может быть изменен в будущем.
:::
...
- `UUID` сопоставляется с [`uuid::Uuid`](https://docs.rs/uuid/latest/uuid/struct.Uuid.html) с использованием `serde::uuid`. Требуется возможность `uuid`.
...
- `Date` сопоставляется с типом `u16` или обёрткой вокруг него и представляет количество дней, прошедших с `1970-01-01`. Также поддерживается [`time::Date`](https://docs.rs/time/latest/time/struct.Date.html) с использованием `serde::time::date`, что требует возможности `time`.
...
- `Date32` сопоставляется с типом `i32` или обёрткой вокруг него и представляет количество дней, прошедших с `1970-01-01`. Также поддерживается [`time::Date`](https://docs.rs/time/latest/time/struct.Date.html) с использованием `serde::time::date32`, что требует возможности `time`.
...
* `DateTime` сопоставляется с/из `u32` или newtype-обёртки вокруг него и представляет количество секунд, прошедших с начала эпохи UNIX. Также поддерживается [`time::OffsetDateTime`](https://docs.rs/time/latest/time/struct.OffsetDateTime.html) с использованием `serde::time::datetime`, для чего требуется фича `time`.
...
* `DateTime64(_)` мапится из/в `i32` или newtype-обёртку вокруг него и представляет время, прошедшее с момента UNIX-эпохи. Также поддерживается [`time::OffsetDateTime`](https://docs.rs/time/latest/time/struct.OffsetDateTime.html) при использовании `serde::time::datetime64::*`, для чего требуется feature `time`.
...
* `Tuple(A, B, ...)` сопоставляется с `(A, B, ...)` или с newtype-обёрткой вокруг него.
* `Array(_)` сопоставляется с любым срезом, например `Vec<_>`, `&[_]`. Также поддерживаются типы-обёртки (newtype).
* `Map(K, V)` ведёт себя как `Array((K, V))`.
* `LowCardinality(_)` поддерживается без дополнительной настройки.
* `Nullable(_)` сопоставляется с `Option<_>`. Для вспомогательных функций `clickhouse::serde::*` добавьте `::option`.
...
* Тип `Nested` поддерживается за счёт использования нескольких массивов с переименованием.
...
Крейт предоставляет утилиты для имитации сервера ClickHouse и тестирования DDL-запросов, а также запросов `SELECT`, `INSERT` и `WATCH`. Функциональность можно включить с помощью feature `test-util`. Используйте её **только** в качестве dev-зависимости.
...
В данном примере это исправляется корректным определением структуры `EventLog`:
...
Если у вас возникли вопросы или вам требуется помощь, свяжитесь с нами в [Community Slack](https://clickhouse.com/slack) или через [GitHub issues](https://github.com/ClickHouse/clickhouse-rs/issues).
