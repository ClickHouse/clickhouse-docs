...
- `query_column_block_stream` — возвращает данные запроса блоками в виде последовательности столбцов, используя встроенные объекты Python
- `query_row_block_stream` — возвращает данные запроса в виде блока строк, используя встроенные объекты Python
- `query_rows_stream` — возвращает данные запроса в виде последовательности строк, используя встроенные объекты Python
...
- [preferred_block_size_bytes](/operations/settings/settings#preferred_block_size_bytes) — мягкое ограничение на размер блока в байтах. По умолчанию 1 000 000.

Независимо от настройки `preferred_block_size_setting`, каждый блок никогда не будет содержать более `max_block_size` строк. В зависимости от типа запроса фактические возвращаемые блоки могут быть любого размера. Например, запросы к распределённой таблице, охватывающей множество шардов, могут содержать меньшие блоки, полученные непосредственно с каждого шарда.
...
Метод `query_np_stream` возвращает каждый блок в виде двумерного массива NumPy. Внутренне массивы NumPy (обычно) хранятся в виде столбцов, поэтому отдельные методы для строк или столбцов не требуются. «Форма» массива NumPy будет выражена как (столбцы, строки). Библиотека NumPy предоставляет множество методов для работы с массивами NumPy. Обратите внимание, что если все столбцы в запросе имеют одинаковый dtype NumPy, возвращаемый массив NumPy также будет иметь только один dtype и может быть изменён по форме или повёрнут без фактического изменения его внутренней структуры.
...
### DataFrame с поддержкой Arrow {#arrow-backed-dataframes}
...
#### Запрос в DataFrame с поддержкой Arrow {#query-to-arrow-backed-dataframe}
...
# Запрос возвращает объект Pandas DataFrame с типами данных Arrow (требуется pandas 2.x)
...
# Потоковая передача данных пакетами DataFrame (на примере polars)

with client.query_df_arrow_stream(
"SELECT number, toString(number) AS str FROM system.numbers LIMIT 100000", dataframe_library="polars"
) as stream:
for df_batch in stream:
print(f"Received {type(df_batch)} batch with {len(df_batch)} rows and dtypes: {df_batch.dtypes}") # Вывод: # Received <class 'polars.dataframe.frame.DataFrame'> batch with 65409 rows and dtypes: [UInt64, String] # Received <class 'polars.dataframe.frame.DataFrame'> batch with 34591 rows and dtypes: [UInt64, String]

```

#### Примечания и особенности {#notes-and-caveats}
- Сопоставление типов Arrow: При возврате данных в формате Arrow ClickHouse сопоставляет типы с ближайшими поддерживаемыми типами Arrow. Некоторые типы ClickHouse не имеют прямого эквивалента в Arrow и возвращаются в виде необработанных байтов в полях Arrow (обычно `BINARY` или `FIXED_SIZE_BINARY`).
  ...
  - В таких случаях столбец DataFrame будет содержать байтовые значения, поддерживаемые полем Arrow; интерпретация и преобразование этих байтов в соответствии с семантикой ClickHouse возлагается на клиентский код.
- Неподдерживаемые типы данных Arrow (например, UUID/ENUM как нативные типы Arrow) не выводятся; значения представляются с использованием ближайшего поддерживаемого типа Arrow (часто в виде бинарных байтов) при выводе.
- Требования к Pandas: типы данных на основе Arrow требуют pandas 2.x. Для более старых версий pandas используйте вместо этого `query_df` (без Arrow).
- Строки и бинарные данные: опция `use_strings` (при поддержке серверной настройкой `output_format_arrow_string_as_string`) управляет тем, возвращаются ли столбцы ClickHouse `String` как строки Arrow или как бинарные данные.

#### Примеры преобразования несовпадающих типов ClickHouse/Arrow {#mismatched-clickhousearrow-type-conversion-examples}

Когда ClickHouse возвращает столбцы в виде необработанных бинарных данных (например, `FIXED_SIZE_BINARY` или `BINARY`), преобразование этих байтов в соответствующие типы Python является обязанностью кода приложения. Приведенные ниже примеры показывают, что некоторые преобразования можно выполнить с использованием API библиотек DataFrame, в то время как другие могут потребовать чисто Python-подходов, таких как `struct.unpack` (которые снижают производительность, но сохраняют гибкость).

```


Столбцы типа `Date` могут приходить в виде `UINT16` (число дней с начала эпохи Unix, 1970‑01‑01). Преобразование внутри DataFrame выполняется просто и эффективно:
...
Начиная с NumPy 2.3 не существует общедоступного 128-битного целочисленного типа `dtype`, поэтому приходится использовать чистый Python и можно сделать что-то вроде:
...
| String                | string                  | bytes             | Столбцы String в ClickHouse не имеют встроенной кодировки, поэтому они также используются для бинарных данных переменной длины |
...
| DateTime              | datetime.datetime       | int               | ClickHouse хранит DateTime в секундах эпохи. Это значение доступно как int                                        |
...
| Time                  | datetime.timedelta      | int, string, time | Момент времени сохраняется как временная метка Unix. Это значение доступно как int                                |
...
| structure | str or seq of str | Список имён столбцов + типов данных (см. примеры). Требуется либо `structure`, либо `types`                                                                  |
