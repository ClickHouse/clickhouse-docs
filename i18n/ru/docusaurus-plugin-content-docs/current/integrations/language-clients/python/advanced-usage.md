...
description: 'Расширенное использование ClickHouse Connect'
...
Обработка результирующего объекта `bytes` является ответственностью вызывающей стороны. Обратите внимание, что `Client.query_arrow` — это тонкая обёртка вокруг данного метода, использующая формат вывода ClickHouse `Arrow`.
...
| Параметр     | Тип                                    | По умолчанию | Описание                                                                                 |
| ------------ | -------------------------------------- | ---------- | ------------------------------------------------------------------------------------------- |
| table        | str                                    | _Обязательный_ | Простое или квалифицированное базой данных имя таблицы                                          |
...
Обеспечение того, что `insert_block` находится в указанном формате и использует указанный метод сжатия, является ответственностью вызывающей стороны. ClickHouse Connect использует эти прямые вставки для загрузки файлов и таблиц PyArrow, делегируя разбор серверу ClickHouse.


## Сохранение результатов запроса в файлы {#saving-query-results-as-files}

Вы можете передавать данные напрямую из ClickHouse в локальную файловую систему, используя метод `raw_stream`. Например, если вы хотите сохранить результаты запроса в CSV-файл, можно использовать следующий фрагмент кода:
...
Аналогичным образом можно сохранять данные в формате [TabSeparated](/interfaces/formats/TabSeparated) и других форматах. См. раздел [Форматы входных и выходных данных](/interfaces/formats) для обзора всех доступных форматов.
...
Кроме того, в приложении, в котором два или более запроса и/или вставки выполняются одновременно, необходимо учитывать два дополнительных аспекта. Первый — это «сессия» ClickHouse, связанная с запросом/вставкой, а второй — пул HTTP-соединений, используемый экземплярами ClickHouse Connect Client.
