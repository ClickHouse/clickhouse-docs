...
ClickHouse Connect включает диалект SQLAlchemy (`clickhousedb`), реализованный поверх базового драйвера. Он ориентирован на API SQLAlchemy Core и поддерживает SQLAlchemy версии 1.4.40+ и 2.0.x.



## Подключение с помощью SQLAlchemy {#sqlalchemy-connect}

Создайте движок, используя URL-адреса `clickhousedb://` или `clickhousedb+connect://`. Параметры запроса соответствуют настройкам ClickHouse, опциям клиента и параметрам транспорта HTTP/TLS.
...
Полный список поддерживаемых опций см. в разделе [Аргументы подключения и настройки](driver-api.md#connection-arguments) ниже. Эти опции также могут быть переданы через DSN SQLAlchemy.
...
## DDL и отражение схемы {#sqlalchemy-ddl-reflection}

Вы можете создавать базы данных и таблицы, используя предоставленные вспомогательные функции DDL и конструкции типов/движков. Поддерживается отражение схемы таблиц (включая типы столбцов и движок).
...
Отраженные столбцы включают специфичные для диалекта атрибуты, такие как `clickhousedb_default_type`, `clickhousedb_codec_expression` и `clickhousedb_ttl_expression`, если они присутствуют на сервере.
...
- DDL и рефлексия: Создание баз данных и таблиц поддерживается, рефлексия возвращает типы столбцов и метаданные движка таблицы. Традиционные метаданные первичных ключей (PK), внешних ключей (FK) и индексов отсутствуют, поскольку ClickHouse не обеспечивает соблюдение этих ограничений.
- Область применения ORM: Декларативные модели и вставка данных через `Session.add(...)`/`bulk_save_objects(...)` работают для удобства. Расширенные возможности ORM (управление связями, обновления в рамках единицы работы, каскадирование, семантика немедленной/отложенной загрузки) не поддерживаются.
