...
| dict_parameter_format               | 'json'       | 'json', 'map'           | Управляет тем, преобразуют ли параметризованные запросы словарь Python в синтаксис JSON или ClickHouse Map. Значение `json` следует использовать для вставок в столбцы JSON, `map` — для столбцов ClickHouse Map.                                             |
| invalid_setting_action              | 'error'      | 'drop', 'send', 'error' | Действие, которое следует предпринять при предоставлении недопустимой или доступной только для чтения настройки (для клиентской сессии или запроса). Если `drop`, настройка будет проигнорирована, если `send`, настройка будет отправлена в ClickHouse, если `error`, будет вызвана клиентская ошибка ProgrammingError. |
...
| readonly                            | 0            | 0, 1                    | Подразумеваемая настройка ClickHouse "read_only" для версий до 19.17. Может быть установлена в соответствии со значением "read_only" ClickHouse для настроек, чтобы обеспечить работу с очень старыми версиями ClickHouse.                                   |
...
Для получения сжатых данных параметр сервера ClickHouse `enable_http_compression` должен быть установлен в 1, либо пользователь должен иметь разрешение на изменение этой настройки для каждого запроса.

Сжатие управляется параметром `compress` при вызове фабричного метода `clickhouse_connect.get_client`. По умолчанию параметр `compress` установлен в `True`, что активирует настройки сжатия по умолчанию. Для запросов, выполняемых с помощью клиентских методов `query`, `query_np` и `query_df`, ClickHouse Connect добавит заголовок `Accept-Encoding` с кодировками `lz4`, `zstd`, `br` (brotli, если библиотека brotli установлена), `gzip` и `deflate` к запросам, выполняемым методом `query` (и косвенно `query_np` и `query_df`). (Для большинства запросов сервер ClickHouse возвращает данные, сжатые с помощью `zstd`.) Для вставки данных по умолчанию ClickHouse Connect сжимает блоки данных с помощью `lz4` и отправляет HTTP-заголовок `Content-Encoding: lz4`.
...
Экспериментальный тип данных `Object` (или `Object('json')`) устарел и его следует избегать в продуктивной среде. ClickHouse Connect продолжает предоставлять ограниченную поддержку этого типа данных для обеспечения обратной совместимости. Обратите внимание, что эта поддержка не распространяется на запросы, которые должны возвращать JSON-значения «верхнего уровня» или «родительские» значения в виде словарей или их эквивалентов — такие запросы приведут к исключению.
