...
Самый простой способ изменить конфигурацию — через интерфейс Grafana на странице настроек плагина, но источники данных также могут быть [созданы и настроены с помощью YAML-файла](https://grafana.com/docs/grafana/latest/administration/provisioning/#data-sources).

На этой странице приведён список параметров, доступных для настройки в плагине ClickHouse, а также примеры конфигураций для тех, кто создаёт источник данных с помощью YAML.

Для быстрого обзора всех параметров полный список настроек конфигурации доступен [здесь](#all-yaml-options).
...
Заголовки могут быть обычными или защищёнными.
Все ключи заголовков хранятся в виде обычного текста, тогда как значения защищённых заголовков сохраняются в защищённой конфигурации (аналогично полю `password`).
...
Эти дополнительные настройки являются опциональными.
...
jsonData:
  ...
  queryTimeout: 60 # таймаут выполнения запроса в секундах. По умолчанию 60. Требует соответствующих прав пользователя; при ошибке прав доступа попробуйте установить "0" для отключения.
...
Также необходимо настроить эти параметры по умолчанию для включения [связей данных](./query-builder.md#data-links) — функции, обеспечивающей мощные рабочие процессы наблюдаемости.
...
jsonData:
  logs:
    ...
    defaultTable: otel_logs # таблица логов по умолчанию. При использовании OTel должна быть установлена в "otel_logs".
...
jsonData:
  traces:
    ...
    defaultTable: otel_traces # таблица трассировок по умолчанию. При использовании OTel должна быть установлена в "otel_traces".
...
С помощью псевдонимов можно преобразовать вложенную схему в плоскую структуру, чтобы её было легко выбирать в Grafana.
...
По умолчанию Grafana предоставляет предложения столбцов на основе ответа от `DESC table`.
...
Преимущество этого подхода перед псевдонимами, определёнными в таблице, заключается в том, что их можно легко обновлять без необходимости изменения таблицы. В некоторых схемах это могут быть тысячи записей, что может загромождать определение базовой таблицы. Кроме того, это позволяет скрывать столбцы, которые пользователь должен игнорировать.
...
Теперь Grafana будет видеть результаты таблицы псевдонимов вместо результатов из `DESC example_table`:
