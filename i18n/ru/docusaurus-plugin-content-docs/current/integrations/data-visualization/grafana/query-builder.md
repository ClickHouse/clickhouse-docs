...
Конструктор запросов — удобный вариант для более простых запросов, но для сложных запросов потребуется использовать [редактор SQL](#sql-editor).

У всех запросов в конструкторе запросов есть [тип запроса](#query-types), и для них нужно выбрать как минимум один столбец.

Доступны следующие типы запросов:

* [Таблица](#table): самый простой тип запроса для отображения данных в табличном формате. Хорошо подходит как универсальный вариант и для простых, и для сложных запросов, содержащих агрегатные функции.
* [Журналы](#logs): оптимизирован для построения запросов к журналам. Лучше всего работает в представлении Explore при [настроенных значениях по умолчанию](./config.md#logs).
* [Временные ряды](#time-series): лучше всего подходит для построения запросов временных рядов. Позволяет выбрать отдельный столбец времени и добавлять агрегатные функции.
* [Трейсы](#traces): оптимизирован для поиска и просмотра трейсов. Лучше всего работает в представлении Explore при [настроенных значениях по умолчанию](./config.md#traces).
* [редактор SQL](#sql-editor): редактор SQL можно использовать, когда нужен полный контроль над запросом. В этом режиме можно выполнять любой SQL-запрос.
