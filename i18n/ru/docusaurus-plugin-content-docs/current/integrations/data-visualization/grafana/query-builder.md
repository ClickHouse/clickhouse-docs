...
Конструктор запросов — удобный вариант для более простых запросов, но для сложных запросов потребуется использовать [SQL Editor](#sql-editor).

Все запросы в конструкторе запросов имеют [тип](#query-types) и требуют выбора как минимум одного столбца.

Доступные типы:
- [Table](#table): самый простой тип запроса для отображения данных в табличном формате. Хорошо подходит как универсальный вариант и для простых, и для сложных запросов, содержащих агрегатные функции.
...
- [Time Series](#time-series): лучше всего подходит для построения запросов к временным рядам. Позволяет выбрать отдельный столбец с меткой времени и добавлять агрегатные функции.
- [Traces](#traces): оптимизирован для поиска и просмотра трассировок. Лучше всего работает в режиме обзора при [настроенных значениях по умолчанию](./config.md#traces).
...
| Поле         | Описание                                                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Builder Mode | Простые запросы не включают агрегаты и Group By, в то время как агрегатные запросы включают эти опции.                                                                                      |
...
| Поле         | Описание                                                                                                                                                                                                                        |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Builder Mode | Простые запросы не включают агрегаты и Group By, тогда как агрегатные запросы включают эти опции.                                                                                                                                     |
...
Значения по умолчанию можно настроить в [конфигурации трассировки](./config.md#traces) источника данных, чтобы конструктор запросов предварительно загружался с базой данных, таблицей и столбцами по умолчанию. Если значения по умолчанию настроены, выбор столбцов будет свернут.
Также можно включить OpenTelemetry для автоматического выбора столбцов в соответствии с версией схемы.
...
| Поле                  | Описание                                                                                                                                                                                  |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Trace Mode            | Переключает запрос с поиска трассировки на поиск по Trace ID.                                                                                                                                      |
...
| Duration Time Column  | Длительность спана. По умолчанию Grafana ожидает число с плавающей точкой в миллисекундах. Преобразование применяется автоматически через выпадающий список `Duration Unit`.                             |
| Duration Unit         | Единица времени для длительности. По умолчанию наносекунды. Выбранная единица будет преобразована в число с плавающей точкой в миллисекундах, как требуется Grafana.                                       |
...
| Order By              | Список выражений [ORDER BY](/sql-reference/statements/select/order-by.md).                                                                                                              |
| Limit                 | Добавляет оператор [LIMIT](/sql-reference/statements/select/limit.md) в конец запроса. Если установлено значение `0`, оператор будет исключен, но это не рекомендуется для больших наборов данных трассировки. |
| Filters               | Список фильтров, применяемых в предложении `WHERE`.                                                                                                                                       |
| Trace ID              | Trace ID для фильтрации. Используется только в режиме Trace ID и при открытии [ссылки на данные](#data-links) trace ID.                                                                                 |

<Image size='md' img={demo_trace_query} alt='Пример запроса трассировки OTel' border />

Этот тип запроса отображает данные в табличном представлении для режима поиска трассировки и на панели трассировки для режима Trace ID.
...
Эта функция включена в плагине ClickHouse для связывания трассировок с логами и наоборот. Она работает лучше всего при настройке OpenTelemetry как для логов, так и для трассировок в [конфигурации источника данных](./config.md#opentelemetry)
...
При включении столбца с идентификатором трассировки к данным будут прикреплены ссылки «**View Trace**» и «**View Logs**».
...
Макросы можно использовать как в SQL Editor, так и в Query Builder.
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `$__dateFilter(columnName)`                  | Заменяется фильтром временного диапазона для указанного столбца, используя временной диапазон панели Grafana в виде [Date](/sql-reference/data-types/date.md).                       | `columnName >= toDate('2022-10-21') AND columnName <= toDate('2022-10-23')`                                       |
| `$__timeFilter(columnName)`                  | Заменяется фильтром временного диапазона для указанного столбца, используя временной диапазон панели Grafana в виде [DateTime](/sql-reference/data-types/datetime.md).               | `columnName >= toDateTime(1415792726) AND time <= toDateTime(1447328726)`                                         |
| `$__timeFilter_ms(columnName)`               | Заменяется фильтром временного диапазона для указанного столбца, используя временной диапазон панели Grafana в виде [DateTime64](/sql-reference/data-types/datetime64.md).           | `columnName >= fromUnixTimestamp64Milli(1415792726123) AND columnName <= fromUnixTimestamp64Milli(1447328726456)` |
