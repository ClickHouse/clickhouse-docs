...
Тип запроса time series похож на [table](#table), но с акцентом на данные временных рядов.

Оба представления в основном одинаковы, однако есть несколько отличий:
...
- Для поля **Time** автоматически добавляются фильтр по диапазону времени и Order By.

:::important В вашей визуализации не хватает данных?
В некоторых случаях панель временных рядов может казаться обрезанной, потому что значение `LIMIT` по умолчанию равно `1000`.

Попробуйте удалить оператор `LIMIT`, установив его в `0` (если это допускается вашим набором данных).
:::
