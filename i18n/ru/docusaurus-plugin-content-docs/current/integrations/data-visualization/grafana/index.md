...
С помощью Grafana вы можете исследовать и визуализировать все свои данные на дашбордах.
Для подключения к ClickHouse в Grafana нужен плагин, который можно легко установить через её интерфейс.
...
2. Найдите **ClickHouse** и выберите подписанный плагин от Grafana Labs:
...
3. На следующем экране нажмите кнопку **Install**:
   ...
   <Image size='md' img={install} alt='Установка плагина ClickHouse' border />
...
3. Введите параметры сервера и учетные данные. Основные настройки:
...
- **Username** и **Password**: введите учетные данные пользователя ClickHouse. Если пользователи не настроены, попробуйте использовать `default` в качестве имени пользователя. Рекомендуется [настроить пользователя с правами только на чтение](#2-making-a-read-only-user).

Дополнительные параметры см. в документации по [конфигурации плагина](./config.md).

4. Нажмите кнопку **Save & test**, чтобы проверить возможность подключения Grafana к вашему сервису ClickHouse. При успешном подключении отобразится сообщение **Data source is working**:
...
Дополнительные сведения о конфигурации см. в документации по [настройке плагина](./config.md).

Если вам нужна информация, не представленная в этой документации, обратитесь к [репозиторию плагина на GitHub](https://github.com/grafana/clickhouse-datasource).


## Обновление версий плагина {#upgrading-plugin-versions}

Начиная с версии v4, конфигурации и запросы могут обновляться при выходе новых версий.

Конфигурации и запросы из v3 автоматически мигрируются в v4 при открытии. Хотя старые конфигурации и дашборды загружаются в v4, миграция не сохраняется до тех пор, пока они не будут повторно сохранены в новой версии. Если при открытии старой конфигурации или запроса возникнут проблемы, отмените изменения и [сообщите о проблеме на GitHub](https://github.com/grafana/clickhouse-datasource/issues).

Плагин не может быть откачен к предыдущим версиям, если конфигурация или запрос были созданы в более новой версии.
