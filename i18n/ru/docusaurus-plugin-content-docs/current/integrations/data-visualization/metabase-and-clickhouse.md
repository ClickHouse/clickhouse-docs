...
keywords: ['Metabase']
description: 'Metabase — это простой в использовании инструмент с открытым исходным кодом и удобным пользовательским интерфейсом для анализа ваших данных.'
...
Metabase — это простой в использовании инструмент с открытым исходным кодом для работы с данными через пользовательский интерфейс. Metabase — это Java-приложение, которое можно запустить, просто <a href="https://www.metabase.com/start/oss/jar" target="_blank">скачав JAR-файл</a> и выполнив команду `java -jar metabase.jar`. Metabase подключается к ClickHouse с помощью JDBC-драйвера, который нужно скачать и поместить в папку `plugins`:



## Цель {#goal}

В этом руководстве вы будете задавать вопросы к данным ClickHouse с помощью Metabase и визуализировать ответы. Один из ответов будет выглядеть следующим образом:
...
4. Запустите (или перезапустите) Metabase, чтобы драйвер был корректно загружен.
...
3. Если драйвер установлен корректно, в выпадающем меню **Database type** появится пункт **ClickHouse**:

   <Image
     size='md'
     img={metabase_01}
     alt='Выбор базы данных в Metabase с отображением ClickHouse в списке опций'
     border
   />

4. Укажите **Display name** для базы данных — это настройка Metabase, поэтому можно использовать любое удобное имя.

5. Введите параметры подключения к базе данных ClickHouse. Включите безопасное соединение, если сервер ClickHouse настроен на использование SSL. Например:
...
2. В правом верхнем углу откройте меню **+ New** и обратите внимание, что вы можете задавать вопросы, выполнять SQL-запросы и создавать дашборды:
   ...
   <Image
     size='sm'
     img={metabase_03}
     alt='Меню New в Metabase с опциями для создания вопросов, SQL-запросов и дашбордов'
     border
   />
...
2. Нажмите кнопку **Visualize**, чтобы просмотреть результаты в табличном виде.
...
3. Под результатами нажмите кнопку **Visualization**, чтобы изменить тип визуализации на столбчатую диаграмму (или выбрать любой другой доступный вариант):
...
Дополнительную информацию о Metabase и о том, как создавать дашборды, можно найти в <a href="https://www.metabase.com/docs/latest/" target="_blank">документации Metabase</a>.
