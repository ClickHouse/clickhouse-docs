...
description: 'Lightdash — это современный BI‑инструмент с открытым исходным кодом, построенный поверх dbt. Он позволяет командам исследовать и визуализировать данные из ClickHouse через семантический слой. Узнайте, как подключить Lightdash к ClickHouse для быстрой управляемой аналитики на основе dbt.'
...
Lightdash — это **ориентированная на ИИ BI‑платформа**, созданная для современных команд работы с данными, которая сочетает открытость dbt с производительностью ClickHouse. Подключив ClickHouse к Lightdash, команды получают **аналитическую среду самообслуживания на базе ИИ**, опирающуюся на их семантический слой dbt, так что на каждый вопрос даётся ответ с контролируемыми, согласованными метриками.

Разработчики ценят Lightdash за его открытую архитектуру, управляемые версиями YAML‑модели и интеграции, которые органично вписываются в их рабочий процесс — от GitHub до IDE.

Это партнёрство объединяет **скорость ClickHouse** и **разработческую ценность Lightdash**, как никогда раньше упрощая исследование, визуализацию и автоматизацию получения инсайтов с помощью ИИ.



## Создание интерактивного дашборда с Lightdash и ClickHouse {#build-an-interactive-dashboard}

В этом руководстве вы узнаете, как **Lightdash** подключается к **ClickHouse** для работы с вашими dbt-моделями и создания интерактивных дашбордов.  
Ниже показан пример готового дашборда на основе данных из ClickHouse.

<Image size='md' img={lightdash_02} alt='Пример дашборда Lightdash' border />
...
- **Schema:** Схема по умолчанию, используемая dbt для компиляции и выполнения вашего проекта (указана в `profiles.yml`)
...
После настройки профиля dbt для ClickHouse вам также потребуется подключить ваш **dbt-проект** к Lightdash.

Поскольку этот процесс одинаков для всех хранилищ данных, мы не будем подробно его описывать — вы можете следовать официальному руководству Lightdash по импорту dbt-проекта:

[Импорт dbt-проекта → Документация Lightdash](https://docs.lightdash.com/get-started/setup-lightdash/connect-project#2-import-a-dbt-project?utm_source=clickhouse&utm_medium=partner&utm_campaign=integration_docs)

После подключения вашего dbt-проекта Lightdash автоматически определит конфигурацию ClickHouse из файла `profiles.yml`. После успешного тестирования подключения вы сможете начать работу с вашими dbt-моделями и создавать дашборды на основе ClickHouse.

---

### Работа с данными ClickHouse в Lightdash {#exploring-your-clickhouse-data-in-lightdash}
...
- **Дашбордам**, работающим на запросах ClickHouse в реальном времени

Теперь вы можете создавать дашборды, делиться аналитическими данными и даже использовать **Ask AI** для генерации визуализаций непосредственно на основе ClickHouse — без необходимости писать SQL вручную.
...
<Image
  size='md'
  img={lightdash_03}
  alt='Пример определения метрик в .yml-файле'
  border
/>
<br />

Определение их в YAML гарантирует, что ваша команда использует одинаковые определения во всех дашбордах и анализах. Например, вы можете создать переиспользуемые метрики, такие как `total_order_count`, `total_revenue` или `avg_order_value`, прямо рядом с вашими dbt-моделями — без необходимости дублирования в интерфейсе.
...
Вместо написания запросов пользователи могут задавать вопросы на естественном языке — например, _«Каким был наш ежемесячный рост выручки?»_ — и AI-агент автоматически создаёт нужную визуализацию, используя метрики и модели, определённые в dbt, для обеспечения точности и согласованности.
