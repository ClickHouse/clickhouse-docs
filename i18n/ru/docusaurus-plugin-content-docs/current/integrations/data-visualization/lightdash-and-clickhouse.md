...
description: 'Lightdash — это современный BI‑инструмент с открытым исходным кодом, построенный поверх dbt, который позволяет командам исследовать и визуализировать данные из ClickHouse через семантический слой. Узнайте, как подключить Lightdash к ClickHouse для быстрой управляемой аналитики на базе dbt.'
...
Lightdash — это **изначально ориентированная на ИИ BI‑платформа**, созданная для современных дата‑команд, объединяющая открытость dbt с производительностью ClickHouse. Подключив ClickHouse к Lightdash, команды получают **самообслуживаемую аналитику с поддержкой ИИ**, основанную на их семантическом слое dbt и обеспечивающую ответы на любые вопросы на основе управляемых, согласованных метрик.
...
Это партнёрство объединяет **скорость ClickHouse** и **удобство разработки в Lightdash**, упрощая изучение данных, создание визуализаций и автоматизацию получения инсайтов с помощью ИИ.
...
В Lightdash все **метрики** и **измерения** определяются непосредственно в файлах `.yml` ваших dbt-моделей. Это делает вашу бизнес-логику версионируемой, согласованной и полностью прозрачной.
...
Определение этих элементов в YAML гарантирует, что ваша команда использует одинаковые определения во всех панелях мониторинга и анализах. Например, вы можете создать переиспользуемые метрики, такие как `total_order_count`, `total_revenue` или `avg_order_value`, непосредственно рядом с вашими dbt-моделями — без необходимости дублирования в пользовательском интерфейсе.
...
Здесь вы можете создавать и настраивать запросы в интерактивном режиме — перетаскивать поля, добавлять фильтры и переключаться между типами визуализации, такими как таблицы, столбчатые диаграммы или временные ряды.

Подробнее об исследованиях и выполнении запросов к таблицам см.:  
...
После изучения данных и сохранения визуализаций вы можете объединить их в **дашборды** для совместного использования с командой.
...
Вы также можете создавать новые графики **непосредственно в дашборде**, что помогает поддерживать порядок в проектах. Графики, созданные таким образом, являются **эксклюзивными для данного дашборда** — их нельзя использовать в других частях проекта.
...
Вместо написания запросов пользователи могут просто задавать вопросы на естественном языке — например, _«Каким был наш ежемесячный рост выручки?»_ — и AI-агент автоматически генерирует подходящую визуализацию, используя метрики и модели, определённые в dbt, для обеспечения точности и согласованности.

Он работает на основе того же семантического слоя, который используется в dbt, что означает управляемость, объяснимость и быстродействие каждого ответа — всё это при поддержке ClickHouse.
