...
description: 'Amazon QuickSight обеспечивает организации, ориентированные на данные, единой платформой бизнес‑аналитики (BI).'
title: 'QuickSight'
doc_type: 'guide'
integration:
  - support_level: 'core'
...
QuickSight может подключаться к локальному развертыванию ClickHouse (23.11+) по интерфейсу MySQL, используя официальный источник данных MySQL и режим Direct Query.
...
В файле `config.xml` раскомментируйте следующее, чтобы прослушивать все адреса:
...
Используя примерное имя пользователя (`mysql_user`) и пароль (`LZOQYnqQN4L/T6L0`) из приведённого выше примера, команда будет выглядеть так:
...
Теперь можно приступить к публикации набора данных и созданию новой визуализации!
