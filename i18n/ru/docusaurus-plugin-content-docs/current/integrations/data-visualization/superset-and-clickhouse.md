...
<a href="https://superset.apache.org/" target="_blank">Apache Superset</a> — это платформа с открытым исходным кодом для исследования и визуализации данных, написанная на Python. Superset подключается к ClickHouse с помощью Python-драйвера от ClickHouse. Посмотрим, как это работает...
...
:::tip Добавьте данные
Если у вас нет набора данных для работы, вы можете использовать один из примеров. В этом руководстве используется набор данных [UK Price Paid](/getting-started/example-datasets/uk-price-paid.md) — можете выбрать его. В той же категории документации доступны и другие наборы данных.
:::
...
1. Для работы с данными ClickHouse в Superset необходимо определить **_набор данных_** (dataset). В верхнем меню Superset выберите **Data**, затем **Datasets** из выпадающего меню.
...
Если вы знакомы с Superset, то этот раздел не вызовет у вас затруднений. Если же вы впервые работаете с Superset, то... он похож на многие другие популярные инструменты визуализации — начать работу несложно, но детали и нюансы осваиваются со временем по мере использования.
...
2. Чтобы создать новый график, выберите **Charts** в верхнем меню и нажмите кнопку для добавления нового графика. Вам будет предложено множество вариантов. В примере ниже показан график типа **Pie Chart** с использованием набора данных **uk_price_paid** из выпадающего списка **CHOOSE A DATASET**:
...
3. Для круговых диаграмм в Superset требуются **Dimension** и **Metric**, остальные настройки необязательны. Вы можете выбрать собственные поля для измерения и метрики. В данном примере используется поле ClickHouse `district` в качестве измерения и `AVG(price)` в качестве метрики.
