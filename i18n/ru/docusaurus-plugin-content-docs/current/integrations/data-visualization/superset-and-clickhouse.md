...
<a href="https://superset.apache.org/" target="_blank">Apache Superset</a> — платформа с открытым исходным кодом для исследования и визуализации данных, написанная на Python. Superset подключается к ClickHouse с помощью Python-драйвера, предоставленного ClickHouse. Давайте посмотрим, как это работает...
...
:::tip Добавьте данные
Если у вас нет набора данных для работы, вы можете использовать один из примеров. В этом руководстве используется набор данных [UK Price Paid](/getting-started/example-datasets/uk-price-paid.md), поэтому можете выбрать его. В той же категории документации доступны и другие наборы данных.
:::
...
1. В интерфейсе Superset выберите **Data** в верхнем меню, затем **Databases** в выпадающем меню. Добавьте новую базу данных, нажав на кнопку **+ Database**:
...
- Укажите **DISPLAY NAME** — это может быть любое имя на ваш выбор. Если вы планируете подключаться к нескольким базам данных ClickHouse, используйте более описательное имя.

<Image
  size='sm'
  img={superset_03}
  alt='Форма настройки подключения Superset с параметрами подключения к ClickHouse'
  border
/>
<br />
...
Если вы знакомы с Superset, то этот раздел не вызовет у вас затруднений. Если же вы впервые работаете с Superset — это инструмент, похожий на многие другие популярные средства визуализации: начать работу несложно, но детали и тонкости осваиваются постепенно в процессе использования.

1. Начните с создания дашборда. В верхнем меню Superset выберите **Dashboards**. Нажмите кнопку в правом верхнем углу, чтобы добавить новый дашборд. В примере ниже дашборд называется **UK property prices**:
...
2. Чтобы создать новый график, выберите **Charts** в верхнем меню и нажмите кнопку добавления нового графика. Вам будет предложено множество вариантов. В следующем примере показан график типа **Pie Chart** с использованием набора данных **uk_price_paid** из выпадающего списка **CHOOSE A DATASET**:
...
3. Для круговых диаграмм в Superset необходимо указать **Dimension** и **Metric**, остальные настройки необязательны. Вы можете выбрать любые поля для измерения и метрики; в данном примере используется поле ClickHouse `district` в качестве измерения и `AVG(price)` в качестве метрики.

<Image
  size='md'
  img={superset_08}
  alt='Настройка измерения с выбранным полем district для круговой диаграммы'
  border
/>
<Image
  size='md'
  img={superset_09}
  alt='Настройка метрики с агрегатной функцией AVG(price) для круговой диаграммы'
  border
/>
<br />

5. Если вы предпочитаете кольцевые диаграммы круговым, это и другие параметры можно настроить в разделе **CUSTOMIZE**:
...
7. Вот и всё. Создание дашбордов в Superset на основе данных из ClickHouse открывает широкие возможности для молниеносной аналитики данных!
