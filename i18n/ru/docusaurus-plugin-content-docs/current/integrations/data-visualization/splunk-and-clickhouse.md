...
Splunk — это популярная технология для задач безопасности и наблюдаемости. Это также мощная система для поиска и построения дашбордов. Существуют сотни приложений Splunk для решения различных сценариев.

Для ClickHouse мы используем [Splunk DB Connect App](https://splunkbase.splunk.com/app/2686), который обеспечивает простую интеграцию с высокопроизводительным драйвером ClickHouse JDBC для прямого выполнения запросов к таблицам в ClickHouse.

Идеальный сценарий использования этой интеграции — когда вы применяете ClickHouse для хранения больших объемов данных, таких как NetFlow, бинарные данные Avro или Protobuf, DNS, журналы VPC Flow и другие OTEL‑журналы, которые можно предоставлять вашей команде в Splunk для поиска и построения дашбордов. При таком подходе данные не загружаются в индексный слой Splunk, а просто запрашиваются напрямую из ClickHouse, аналогично другим интеграциям для визуализации, таким как [Metabase](https://www.metabase.com/) или [Superset](https://superset.apache.org/).
...
С помощью этого руководства вы сможете создать дашборд, подключенный к ClickHouse, похожий на этот:
...
Загрузите [JDBC-драйвер ClickHouse](https://github.com/ClickHouse/clickhouse-java) в папку драйверов DB Connect, например:

```bash
$SPLUNK_HOME/etc/apps/splunk_app_db_connect/drivers
```

Затем необходимо отредактировать конфигурацию типов подключений в файле `$SPLUNK_HOME/etc/apps/splunk_app_db_connect/default/db_connection_types.conf`, чтобы добавить сведения о классе JDBC-драйвера ClickHouse.
...
Вернитесь в приложение DB Connect и перейдите в раздел Configuration > Settings > Drivers. Рядом с ClickHouse должна отображаться зелёная галочка:
...
Перейдите в DB Connect App Configuration -> Databases -> Identities и создайте идентификатор для вашего ClickHouse.
...
Добавьте данные хоста ClickHouse и убедитесь, что установлен флажок «Enable SSL»:
...
Выберите параметры подключения в SQL Explorer из раздела DataLab приложения DB Connect App. В этой демонстрации мы используем таблицу `trips`:
...
Добавим ещё один запрос, который показывает среднюю стоимость поездки в зависимости от количества пассажиров.
...
Splunk содержит сотни встроенных функций, которые можно использовать в дашбордах для визуализации и представления данных временных рядов. В этом примере SQL и SPL объединяются для создания запроса, работающего с данными временных рядов в Splunk
