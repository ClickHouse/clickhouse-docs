...
keywords: ['clickhouse', 'Mitzu', 'подключение', 'интеграция', 'ui']
description: 'Mitzu — это нативное для хранилища данных приложение для продуктовой аналитики без кода.'
...
Mitzu — это no-code‑платформа для продуктовой аналитики, изначально рассчитанная на работу с хранилищем данных. Подобно таким инструментам, как Amplitude, Mixpanel и PostHog, Mitzu позволяет пользователям анализировать данные об использовании продукта без необходимости владеть SQL или Python.

Однако в отличие от этих платформ Mitzu не дублирует данные компании о взаимодействии с продуктом. Вместо этого она генерирует нативные SQL‑запросы непосредственно по данным в существующем хранилище или озере данных компании.
...
<Image
  size='lg'
  img={mitzu_05}
  alt='Настройка каталога событий Mitzu с отображением параметров сопоставления столбцов'
  border
/>
<br />
После настройки всех таблиц нажмите кнопку `Save & update event catalog`,
и Mitzu обнаружит все события и их свойства из указанных таблиц. Этот шаг
может занять несколько минут в зависимости от размера набора данных.


## 4. Выполнение запросов сегментации {#4-run-segmentation-queries}

Сегментация пользователей в Mitzu выполняется так же просто, как в Amplitude, Mixpanel или PostHog.
...
Получайте мгновенную информацию о коэффициенте конверсии без написания ни одной строки SQL-кода.
...
Выберите до 9 шагов для воронки. Укажите временной интервал, в течение которого пользователи могут завершить путь. Диаграмма пути Mitzu предоставляет визуальную карту всех маршрутов, которые пользователи проходят через выбранные события.

<Image
  size='lg'
  img={mitzu_09}
  alt='Визуализация пути Mitzu, показывающая поток перемещения пользователей между событиями'
  border
/>
<br />

:::tip Детализация шагов
Вы можете выбрать свойство для сегмента `Break down`, чтобы различать пользователей в рамках одного шага.
:::
...
Если вы столкнулись с ограничением интерфейса Mitzu, скопируйте SQL-код и продолжите работу в BI-инструменте.
:::
