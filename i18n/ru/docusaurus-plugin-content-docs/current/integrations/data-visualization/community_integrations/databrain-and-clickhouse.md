...
description: 'Databrain — это встраиваемая аналитическая платформа, которая бесшовно интегрируется с ClickHouse для создания клиентских дашбордов, метрик и визуализаций данных.'
...
[Databrain](https://usedatabrain.com) — это платформа встроенной аналитики, которая позволяет создавать и предоставлять вашим клиентам интерактивные дашборды, метрики и визуализации данных. Databrain подключается к ClickHouse по HTTPS, что упрощает визуализацию и анализ данных ClickHouse в современном, удобном интерфейсе.

<Image size="md" img={databrain_01} alt="Интерфейс дашборда Databrain с визуализацией данных ClickHouse" border />

<br/>

В этом руководстве описаны шаги по подключению Databrain к вашему экземпляру ClickHouse.
...
Если в вашем экземпляре ClickHouse включена фильтрация по IP-адресам, необходимо добавить IP-адреса Databrain в белый список.
...
:::tip
Обратитесь к [документации Databrain по добавлению IP-адресов в белый список](https://docs.usedatabrain.com/guides/datasources/allow-access-to-our-ip) для получения актуального списка IP-адресов.
:::

### 3. Добавьте ClickHouse в качестве источника данных в Databrain {#3-add-clickhouse-as-a-data-source}

1. Войдите в свою учётную запись Databrain и перейдите в рабочее пространство, в которое хотите добавить источник данных.
...
2. В обозревателе данных отобразится список ваших таблиц ClickHouse.
...
- **Многопользовательская и однопользовательская архитектура**: Подключайте базу данных ClickHouse с поддержкой как однопользовательской, так и многопользовательской архитектуры
- **Планирование отчётов**: Настраивайте автоматическую генерацию отчётов и их отправку заинтересованным лицам по электронной почте
- **Аналитика на основе ИИ**: Используйте искусственный интеллект для создания сводок и получения аналитических выводов из данных
- **Встроенная аналитика**: Встраивайте дашборды и метрики непосредственно в свои приложения
...
4. **Оптимизация индексов**: Используйте первичные ключи и индексы с пропуском данных ClickHouse


## Узнать больше {#learn-more}

Дополнительная информация о возможностях Databrain и создании мощной аналитики:
