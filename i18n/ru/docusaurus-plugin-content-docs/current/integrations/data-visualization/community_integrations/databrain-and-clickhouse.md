...
description: 'Databrain — это платформа встроенной аналитики, которая бесшовно интегрируется с ClickHouse для создания клиентских панелей, метрик и визуализаций данных.'
...
[Databrain](https://usedatabrain.com) — это встраиваемая аналитическая платформа, которая позволяет создавать и совместно использовать интерактивные дашборды, метрики и визуализации данных с вашими клиентами. Databrain подключается к ClickHouse через интерфейс HTTPS, что упрощает визуализацию и анализ данных ClickHouse с помощью современного и удобного интерфейса.

<Image size="md" img={databrain_01} alt="Интерфейс панели Databrain с визуализацией данных ClickHouse" border />

<br/>

В этом руководстве пошагово описано, как подключить Databrain к вашему экземпляру ClickHouse.
...
Если в вашем экземпляре ClickHouse включена фильтрация по IP-адресам, необходимо добавить IP-адреса Databrain в список разрешённых.
...
:::tip
Обратитесь к [документации Databrain по добавлению IP-адресов в список разрешённых](https://docs.usedatabrain.com/guides/datasources/allow-access-to-our-ip) для получения актуального списка IP-адресов.
:::

### 3. Добавьте ClickHouse в качестве источника данных в Databrain {#3-add-clickhouse-as-a-data-source}

1. Войдите в свою учётную запись Databrain и перейдите в рабочее пространство, в которое вы хотите добавить источник данных.
...
1. После подключения перейдите в свою рабочую область в Databrain.

2. В обозревателе данных вы увидите список таблиц ClickHouse.
...
3. Используйте интуитивный интерфейс Databrain для:
   ...
   - Добавления пользовательских SQL-запросов для углублённого анализа
...
2. Добавьте метрики на дашборд, перетаскивая сохранённые метрики.
...
4. Поделитесь дашбордом с командой или встройте его в своё приложение.

### Расширенные возможности {#advanced-features}

Databrain предлагает несколько расширенных возможностей при работе с ClickHouse:

- **Консоль пользовательских SQL-запросов**: Пишите и выполняйте пользовательские SQL-запросы непосредственно к базе данных ClickHouse
- **Мультитенантность и однотенантность**: Подключайте базу данных ClickHouse с поддержкой как однотенантной, так и мультитенантной архитектуры
- **Планирование отчётов**: Настраивайте автоматическую генерацию отчётов и отправку их заинтересованным лицам по электронной почте
- **Аналитика на основе ИИ**: Используйте искусственный интеллект для создания сводок и получения инсайтов из данных
- **Встраиваемая аналитика**: Встраивайте дашборды и метрики непосредственно в свои приложения
...
3. **Белый список IP-адресов**: Убедитесь, что IP-адреса Databrain добавлены в белый список в настройках брандмауэра/безопасности ClickHouse
...
Дополнительную информацию о возможностях Databrain и создании мощной аналитики можно найти в следующих материалах:
