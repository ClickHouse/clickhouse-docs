...
description: 'Подключите Chartbrew к ClickHouse, чтобы создавать дашборды и клиентские отчеты в реальном времени.'
doc_type: 'guide'
---
...
[Chartbrew](https://chartbrew.com) — это платформа для визуализации данных, которая позволяет пользователям создавать дашборды и отслеживать данные в реальном времени. Она поддерживает множество источников данных, включая ClickHouse, и предоставляет no-code интерфейс для создания графиков и отчетов.
...
3. Введите параметры подключения к вашей базе данных ClickHouse:
   ...
   - **Database Name**: База данных, к которой необходимо подключиться.
...
5. Если тест выполнен успешно, нажмите **Save connection**. Chartbrew автоматически получит схему из ClickHouse.
...
2. Выберите созданное ранее подключение ClickHouse.

<Image
  img={chartbrew_05}
  size='lg'
  alt='Выбор подключения ClickHouse для набора данных'
/>

Напишите SQL-запрос для получения данных, которые требуется визуализировать. Например, следующий запрос вычисляет среднюю цену за год из набора данных `uk_price_paid`:
...
Если вы не уверены, как составить запрос, можно использовать **AI-ассистент Chartbrew** для генерации SQL-запросов на основе схемы базы данных.
...
2. Просмотрите набор данных, чтобы убедиться в правильности структуры результатов запроса.
3. Выберите тип диаграммы (например, линейная, столбчатая или круговая диаграмма) и добавьте её на дашборд.
...
Вы можете создать любое количество наборов данных для визуализации различных аспектов ваших данных. На основе этих наборов данных можно создать несколько дашбордов для отслеживания различных метрик.
...
Чтобы поддерживать актуальность дашборда, можно настроить автоматическое обновление данных:
...
3. Сохраните настройки для включения автоматического обновления.
