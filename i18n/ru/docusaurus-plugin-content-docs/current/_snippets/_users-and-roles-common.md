...
Создайте таблицы и пользователей, которые будут использоваться в примерах.
...
##### Предоставьте явные разрешения другим ролям для сохранения доступа ко всем строкам {#grant-other-roles-access}
    ...
    :::note
    При присоединении политики к таблице система применит её, и только определённые в ней пользователи и роли смогут выполнять операции с таблицей — всем остальным любые операции будут запрещены. Чтобы ограничительная политика строк не применялась к другим пользователям, необходимо определить дополнительную политику, разрешающую другим пользователям и ролям стандартный или иные типы доступа.
    :::
...
##### Войдите в клиент ClickHouse от имени пользователя `clickhouse_admin` {#login-admin-user}

```bash
clickhouse-client --user clickhouse_admin --password password
```

##### Проверьте доступ к базе данных, таблице и всем строкам от имени администратора {#verify-admin-access}
...
##### Войдите в клиент ClickHouse от имени пользователя `column_user` {#login-column-user}
...
##### Проверьте запрос `SELECT` только с разрешенными столбцами: {#verify-allowed-columns}
...
##### Войдите в клиент ClickHouse от имени пользователя `row_user` {#login-row-user}
...
:::note
Убедитесь, что возвращаются только две указанные выше строки, строки со значением `B` в столбце `column1` должны быть исключены.
:::
...
##### Войдите в клиент ClickHouse под пользователем `row_and_column_user` {#login-restricted-user}
...
Иногда привилегии пересекаются или комбинируются, что приводит к неожиданным результатам. Для диагностики проблемы можно использовать следующие команды от имени учетной записи администратора

### Просмотр привилегий и ролей пользователя {#listing-the-grants-and-roles-for-a-user}
...
### Просмотр списка ролей в ClickHouse {#list-roles-in-clickhouse}
...
### Удаление привилегии у роли {#remove-privilege-from-a-role}
...
В этой статье рассмотрены основы создания пользователей и ролей SQL, а также описаны шаги по настройке и изменению привилегий для пользователей и ролей. Для получения более подробной информации обратитесь к руководствам пользователя и справочной документации.
