...
##### Предоставьте явные разрешения другим ролям для сохранения доступа ко всем строкам {#grant-other-roles-access}
    ...
    :::note
    При привязке политики к таблице система применит её, и только те пользователи и роли, которые в ней определены, смогут выполнять операции с таблицей — всем остальным любые операции будут запрещены. Чтобы ограничительная политика строк не применялась к другим пользователям, необходимо определить отдельную политику, разрешающую другим пользователям и ролям стандартный или иной тип доступа.
    :::
...
0 строк в наборе. Затрачено: 0.005 сек.

Получено исключение от сервера (версия 22.3.2):
Код: 497. DB::Exception: Получено от localhost:9000.
DB::Exception: row_and_column_user: Недостаточно привилегий.
Для выполнения этого запроса необходима привилегия
SELECT(id, column1, column2) ON db1.table1. (ACCESS_DENIED)
```
