...
Datadog предлагает плагин Clickhouse Monitoring для своего агента, который напрямую запрашивает системные таблицы. Эта интеграция обеспечивает комплексный мониторинг базы данных с поддержкой кластерной топологии через функциональность clusterAllReplicas.
:::note
Эта интеграция не рекомендуется для развертываний ClickHouse Cloud из-за несовместимости с оптимизацией затрат в режиме простоя и операционных ограничений облачного прокси-слоя.
:::

### Прямое использование системных таблиц {#system-tables}

Пользователи могут выполнять глубокий анализ производительности запросов, подключаясь к системным таблицам ClickHouse, в частности к `system.query_log`, и выполняя запросы напрямую. Используя SQL-консоль или клиент clickhouse, команды могут выявлять медленные запросы, анализировать использование ресурсов и отслеживать паттерны использования в организации.

**Анализ производительности запросов**

Пользователи могут использовать журналы запросов системных таблиц для анализа производительности запросов.
