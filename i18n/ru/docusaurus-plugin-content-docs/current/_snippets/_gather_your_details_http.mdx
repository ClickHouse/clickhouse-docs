...
| ------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `HOST` and `PORT`         | Как правило, при использовании TLS используется порт 8443, а без TLS — порт 8123.                          |
| `DATABASE NAME`           | По умолчанию имеется база данных `default`; используйте имя базы данных, к которой вы хотите подключиться. |
...
Если вы используете самоуправляемый (self-managed) ClickHouse, параметры подключения настраиваются вашим администратором ClickHouse.
