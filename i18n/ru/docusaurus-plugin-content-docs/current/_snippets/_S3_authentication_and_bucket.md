...
В данной статье описываются основы настройки пользователя AWS IAM, создания корзины S3 и настройки ClickHouse для использования корзины в качестве диска S3. Рекомендуется согласовать используемые разрешения с командой безопасности и рассматривать приведенные инструкции как отправную точку.

### Создание пользователя AWS IAM {#create-an-aws-iam-user}

В данной процедуре создается сервисная учетная запись, а не учетная запись для интерактивного входа.
...
6. Выберите **Create User**
   ...
   :::note
   Предупреждение об отсутствии разрешений у пользователя можно проигнорировать; разрешения будут предоставлены для корзины в следующем разделе
   :::
...
:::note
Рекомендуется согласовать используемые разрешения с вашей командой безопасности; приведенные настройки следует рассматривать как отправную точку.
