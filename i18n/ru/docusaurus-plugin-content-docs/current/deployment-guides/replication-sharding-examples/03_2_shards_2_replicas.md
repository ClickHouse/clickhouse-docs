...
description: 'Пройдя это руководство, вы научитесь настраивать простой кластер ClickHouse.'
doc_type: 'guide'
...
> одновременно обеспечивает репликацию и масштабирование. Он состоит из двух шардов и двух реплик, а также
> трехузлового кластера ClickHouse Keeper, который отвечает за координацию и поддержание кворума
> в кластере.

Архитектура кластера, который вы будете настраивать, показана ниже:

<Image img={SharedReplicatedArchitecture} size="md" alt="Схема архитектуры кластера с 2 шардами и 1 репликой" />
...
Конфигурация журналирования определяется в блоке `<logger>`. Данная конфигурация создает
отладочный журнал с ротацией при достижении размера 1000M три раза:
...
стандартного [файла конфигурации](https://github.com/ClickHouse/ClickHouse/blob/master/programs/server/config.xml) ClickHouse.
...
На практике делать так не рекомендуется.
...
Чтобы убедиться, что кластер работает, подключитесь к любому узлу и выполните
...
Таким образом, вы успешно развернули кластер ClickHouse с двумя шардами и двумя репликами.
...
Набор данных содержит около 30 миллионов записей о ценах на недвижимость
...
запрос был выполнен только из `clickhouse-01`:
...
Теперь, когда база данных создана, следующим шагом будет создание таблицы с репликацией.
...
не применяется к DML-запросам (Data Manipulation Language), таким как `INSERT`, `UPDATE`
