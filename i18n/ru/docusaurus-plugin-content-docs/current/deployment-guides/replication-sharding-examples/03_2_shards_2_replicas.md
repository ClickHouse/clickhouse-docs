...
<VerticalStepper level="h2">
  ...
  Она содержит около 30 миллионов строк о ценах на недвижимость в Англии и Уэльсе с 1995 года.
  ...
  * `{shard}` и `{replica}` — макросы, [ранее определённые](#macros-config-explanation)
    в файле `config.xml` каждого узла ClickHouse.
  ...
  Как описано в [руководстве](/architecture/horizontal-scaling) по настройке кластера с 2 сегментами и 1 репликой, distributed таблицы — это таблицы, имеющие доступ к сегментам, расположенным на разных
  хостах, и определяемые с помощью табличного движка `Distributed`.
  Distributed таблица служит интерфейсом для всех сегментов в кластере.

  С любого из клиентов хоста выполните следующий запрос для создания distributed таблицы на основе существующей реплицируемой таблицы, созданной на предыдущем шаге:
