...
description: 'Страница с примером архитектуры, разработанной для обеспечения масштабируемости'
doc_type: 'guide'
...
> В этом примере вы узнаете, как настроить простой, но масштабируемый кластер ClickHouse.
> Настроено пять серверов: два используются как шарды для данных,
...
Здесь определено имя кластера `cluster_2S_1R`.
...
Параметр `internal_replication` по умолчанию оставлен равным false, поскольку на каждый шард приходится только одна реплика.
...
в производственных средах настоятельно рекомендуется запускать ClickHouse Keeper на выделенных хостах.
:::
...
:::note
Они определяются индивидуально в зависимости от структуры кластера.
:::
...
Docker начнёт загрузку образов ClickHouse и Keeper,
...
следующий запрос. Команда для подключения к первому узлу:
...
Если выполнить следующий запрос на любом из хостов, можно увидеть, что данные
распределены по шардам более или менее равномерно (учитывайте, что выбор шарда
...
Теперь с `clickhouse-02` выполните тот же запрос SELECT к распределённой
таблице, который мы выполняли ранее:
...
Code: 32. DB::Exception: Попытка чтения после EOF. (ATTEMPT&#95;TO&#95;READ&#95;AFTER&#95;EOF) (version 25.5.2.47 (official build))
Code: 209. DB::NetException: Таймаут: время ожидания подключения истекло: 192.168.7.1:9000 (clickhouse-01:9000, 192.168.7.1, local address: 192.168.7.2:37484, connection timeout 1000 ms). (SOCKET&#95;TIMEOUT) (version 25.5.2.47 (official build))
#highlight-next-line
Code: 198. DB::NetException: Не удалось определить адрес хоста: clickhouse-01: (clickhouse-01:9000, 192.168.7.1, local address: 192.168.7.2:37484). (DNS&#95;ERROR) (version 25.5.2.47 (official build))
...
настроить кластер с двумя шардами и двумя репликами, обеспечивающий как масштабируемость, так и
отказоустойчивость.
