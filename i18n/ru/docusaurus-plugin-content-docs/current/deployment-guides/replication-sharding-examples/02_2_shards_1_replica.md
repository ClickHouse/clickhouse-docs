...
<VerticalStepper level="h2">
  ...
  `internal_replication` по умолчанию оставлен со значением false, так как на каждый сегмент приходится только одна реплика.
  ...
  При успешном подключении вы увидите приглашение клиента ClickHouse:
  ...
  В клиенте `clickhouse-01` выполните следующий **распределённый** DDL-запрос с использованием
  ...
  Обратите внимание, что этот запрос идентичен запросу, использованному в исходной инструкции `CREATE` в
  ...
  Конструкция `ON CLUSTER` предназначена для распределённого выполнения DDL-запросов (Data Definition Language — язык описания данных),
  ...
  Прежде чем вставить данные о ценах, уплаченных в Великобритании, давайте проведём небольшой эксперимент, чтобы посмотреть, что произойдёт, если вставлять данные в обычную таблицу с любого из хостов.
  ...
  Для чтения данных из двух сегментов требуется интерфейс, способный обрабатывать запросы
  ко всем сегментам, объединяя данные из обоих сегментов при выполнении SELECT-запросов
  и вставляя данные в оба сегмента при выполнении INSERT-запросов.
  ...
  выполните следующий запрос для создания distributed таблицы на основе существующей таблицы,
  ...
  распределены более или менее равномерно по сегментам (имейте в виду, что выбор сегмента
