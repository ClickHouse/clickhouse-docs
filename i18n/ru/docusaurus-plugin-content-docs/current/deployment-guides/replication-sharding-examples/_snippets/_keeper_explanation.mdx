...
Используемое значение `server_id` должно быть уникальным для данного узла ClickHouse Keeper
в кластере и соответствовать значению `<id>` сервера, определённому в секции `<raft_configuration>`.
