...
> В этом примере вы узнаете, как развернуть простой кластер ClickHouse с репликацией данных. Настроено пять серверов: два используются для хранения копий данных, остальные три — для координации репликации.
...
настройки кластера ClickHouse. Эту конфигурацию можно изменить для работы
на отдельных локальных машинах, виртуальных машинах или облачных экземплярах.
...
выделены ниже, необходимо изменить, чтобы они были уникальны для каждого узла:
...
<clickhouse replace="true">
    ...
    <!--highlight-next-line-->
    <display_name>cluster_1S_2R узел 1</display_name>
...
Каждый раздел приведённого выше файла конфигурации более подробно описан ниже.

#### Сеть и логирование

<ListenHost />

Логирование задаётся в блоке `<logger>`. Эта примерная конфигурация создаёт
отладочный лог с ротацией по размеру: при достижении 1000M будет храниться до трёх файлов.
...
Для получения дополнительной информации о конфигурации логирования см. комментарии
в стандартном конфигурационном файле ClickHouse
...
шаблоном для распределённых DDL-запросов, то есть запросов, которые выполняются по всему
кластеру с использованием клаузы `ON CLUSTER`. По умолчанию распределённые DDL-запросы
разрешены, но их также можно отключить с помощью настройки `allow_distributed_ddl_queries`.

`internal_replication` установлен в значение `true`, поэтому данные записываются только на одну из реплик.
...
для него задаются имя хоста и номер порта с помощью тегов `<host>` и `<port>` соответственно.

Процесс настройки ClickHouse Keeper описан на следующем шаге этого руководства.
...
Кроме того, раздел `<macros>` используется для задания параметров подстановки для
реплицированных таблиц. Они перечислены в `system.macros` и позволяют использовать подстановки
вроде `{shard}` и `{replica}` в запросах.
...
:::note
Они будут заданы по‑разному в зависимости от топологии кластера.
:::

### Конфигурация пользователей

Теперь измените каждый пустой конфигурационный файл `users.xml`, расположенный по пути
...
В этом примере пользователь по умолчанию для упрощения настроен без пароля.
...
### Конфигурация Keeper {#configuration-explanation}
...
Вы увидите, как Docker начнёт загружать образы ClickHouse и Keeper, а затем запустит контейнеры:
...
Выполните следующий запрос, чтобы проверить, какие топологии кластера заданы на каких хостах:
...
Теперь у вас развернут кластер ClickHouse с одним шардом и двумя репликами.
На следующем шаге вы создадите таблицу в кластере.


## Создание базы данных

Теперь, когда вы проверили, что кластер корректно настроен и запущен,
вы воссоздадите ту же таблицу, которая используется в руководстве по набору данных
[UK property prices](/getting-started/example-datasets/uk-price-paid).
Она содержит около 30 миллионов строк с ценами сделок по объектам
недвижимости в Англии и Уэльсе, начиная с 1995 года.

Подключитесь к клиенту на каждом хосте, запустив каждую из следующих команд
в отдельных вкладках или окнах терминала:
...
Вы можете выполнить следующий запрос из clickhouse-client на каждом хосте, чтобы убедиться, что, помимо стандартных, еще не создано ни одной базы данных:
...
С клиента `clickhouse-01` выполните следующий **распределённый** DDL-запрос с использованием `ON CLUSTER`, чтобы создать новую базу данных `uk`:
...
Вы можете ещё раз выполнить тот же запрос, что и ранее, из клиента на каждом хосте,
чтобы убедиться, что база данных была создана во всём кластере, хотя вы запускали
запрос только на `clickhouse-01`:
...
Обратите внимание, что он идентичен запросу, использованному в исходном операторе `CREATE` в учебнике по набору примерных данных
[UK property prices](/getting-started/example-datasets/uk-price-paid),
за исключением конструкции `ON CLUSTER` и использования движка `ReplicatedMergeTree`.

Конструкция `ON CLUSTER` предназначена для распределённого выполнения DDL (Data Definition Language — язык описания данных)
...
Вы можете выполнить приведённый ниже запрос из любого из клиентов `clickhouse-01` или `clickhouse-02`,
...
Поскольку набор данных большой и его полный приём занимает несколько минут, мы
для начала вставим только небольшой поднабор.

Вставьте меньший поднабор данных, выполнив приведённый ниже запрос на `clickhouse-01`:
...
SELECT
    ...
    transform(a, ['T', 'S', 'D', 'F', 'O'], ['таунхаус', 'двухквартирный', 'отдельный', 'квартира', 'другое']) AS type,
    b = 'Y' AS is_new,
    transform(c, ['F', 'L', 'U'], ['собственность', 'аренда', 'неизвестно']) AS duration,
    addr1,
    addr2,
    street,
    locality,
...
Как и для таблицы `uk_price_paid`, мы можем вставлять данные с любого из хостов:

```sql
INSERT INTO test.test_table (id, name) VALUES (1, 'Clicky McClickface');
```

Но что произойдёт, если один из хостов будет недоступен? Чтобы смоделировать такую ситуацию, остановите
...
```sql
INSERT INTO test.test_table (id, name) VALUES (2, 'Алексей Миловидов');
...
Теперь перезапустите `clickhouse-01` следующей командой (после этого можно выполнить `docker-compose ps` для проверки):
...
Преимущество такой топологии кластера в том, что при двух репликах 
ваши данные хранятся на двух отдельных хостах. Если один хост выходит из строя, другая реплика 
продолжает обслуживать запросы без потери данных. Это устраняет единые точки отказа 
на уровне хранилища.
