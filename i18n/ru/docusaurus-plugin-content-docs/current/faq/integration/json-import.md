...
keywords: ['импорт JSON', 'формат JSONEachRow', 'импорт данных', 'загрузка JSON', 'форматы данных']
doc_type: 'guide'
---
...
Вместо ручной вставки данных можно использовать [инструмент интеграции](../../integrations/index.mdx).


## Полезные настройки {#useful-settings}

- `input_format_skip_unknown_fields` позволяет вставлять JSON, даже если в нём присутствуют дополнительные поля, отсутствующие в схеме таблицы (такие поля будут отброшены).
