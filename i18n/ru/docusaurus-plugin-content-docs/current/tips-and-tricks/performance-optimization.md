...
_Если у вас медленные запросы и вы хотите больше примеров, у нас также есть руководство по [оптимизации запросов](/optimize/query-optimization)._
...
SELECT
    ...
    'Намного лучше для ключа сортировки - позволяет пропускать блоки данных'
FROM github.github_events
WHERE created_at >= '2024-01-01'
UNION ALL
SELECT
...
Когда запрос выполняется медленно, не ограничивайтесь анализом средних значений. Спросите: «Почему ЭТОТ конкретный запрос был медленным?» и изучите фактические паттерны использования ресурсов.


## Память и сканирование строк {#memory-and-row-scanning}

Sentry — это платформа отслеживания ошибок, ориентированная на разработчиков, которая обрабатывает миллиарды событий ежедневно от более чем 4 миллионов разработчиков. Их ключевой вывод: _«Кардинальность ключа группировки определяет потребление памяти в данной конкретной ситуации»_ — агрегации с высокой кардинальностью убивают производительность из-за исчерпания памяти, а не из-за сканирования строк.
...
Запрос вида `GROUP BY user_id, error_message, url_path` создает отдельное состояние в памяти для каждой уникальной комбинации всех трех значений. При большом количестве пользователей, типов ошибок и URL-путей можно легко сгенерировать миллионы состояний агрегации, которые должны одновременно храниться в памяти.
...
SELECT
    ...
    -- Сложные многоусловные агрегации по-прежнему используют постоянный объём памяти
...
От инженерной команды Sentry: «Эти тяжёлые запросы работают более чем в 10 раз быстрее, а потребление памяти в 100 раз ниже (и, что более важно, ограничено). Наши крупнейшие клиенты больше не видят ошибок при поиске повторов, и теперь мы можем поддерживать клиентов произвольного размера без исчерпания памяти».


## Видеоматериалы {#video-sources}

- [Lost in the Haystack - Optimizing High Cardinality Aggregations](https://www.youtube.com/watch?v=paK84-EUJCA) - опыт Sentry по оптимизации памяти в production
