...
sidebar_label: 'Советы по отладке'
doc_type: 'guide'
keywords: [
...
title: 'Уроки — советы по отладке'
description: 'Найдите решения наиболее распространённых проблем ClickHouse, включая медленные запросы, ошибки памяти, проблемы с подключением и конфигурационные ошибки.'
---
...
Включите внешнюю агрегацию для операций, требующих большого объёма памяти. Это медленнее, но предотвращает сбои из-за нехватки памяти путём выгрузки данных на диск. Для этого используйте параметр `max_bytes_before_external_group_by`, который помогает предотвратить сбои из-за нехватки памяти при выполнении больших операций `GROUP BY`. Подробнее об этом параметре можно узнать [здесь](/operations/settings/settings#max_bytes_before_external_group_by).
...
| --------------- | -------------------------------- | ---------------------------------- |
| Дисковое пространство | Проверьте общий объем в `system.parts` | Отслеживайте использование, планируйте масштабирование |
| Слишком много частей | Подсчитайте количество частей на таблицу | Используйте пакетные вставки, включите async_insert |
| Задержка репликации | Проверьте задержку в `system.replicas` | Отслеживайте сеть, перезапустите реплики |
| Некорректные данные | Проверьте даты партиций | Реализуйте валидацию временных меток |
| Зависшие мутации | Проверьте статус в `system.mutations` | Сначала протестируйте на небольших данных |
