...
description: 'ClickHouse предлагает пять вариантов развертывания, и все они используют один и тот же мощный движок базы данных, просто по-разному упакованный, чтобы соответствовать вашим конкретным потребностям.'
...
SQL Console в ClickHouse Cloud включает мощную функцию [создания дашбордов](/cloud/manage/dashboards), которая позволяет преобразовывать ваши запросы в интерактивные визуализации. Вы можете создавать и делиться дашбордами, построенными на основе сохранённых запросов, с возможностью добавлять интерактивные элементы через параметры запросов. Эти дашборды можно сделать динамическими с помощью глобальных фильтров, что позволяет пользователям исследовать данные через настраиваемые представления — однако важно учитывать, что вам потребуется как минимум доступ на чтение к соответствующим сохранённым запросам, чтобы просматривать визуализации.

Для мониторинга и оптимизации ClickHouse Cloud включает встроенные графики и [query insights](/cloud/get-started/query-insights). Эти инструменты обеспечивают глубокое понимание производительности вашего кластера, помогая разобраться в шаблонах запросов, использовании ресурсов и потенциальных возможностях оптимизации. Такой уровень обсервабилити особенно ценен для команд, которым необходимо поддерживать высокопроизводительную аналитическую инфраструктуру, не выделяя значительные ресурсы на управление ею.
...
ClickHouse CLI (`clickhousectl`) — это инструмент командной строки для локальной разработки с ClickHouse и управления ClickHouse Cloud. Он упрощает установку различных версий ClickHouse, запуск и управление локальными экземплярами сервера, а также выполнение запросов.

Для локальной разработки `clickhousectl` помогает устанавливать разные версии ClickHouse и запускать серверы. Вы можете установить любую версию ClickHouse, запускать именованные серверы и подключаться к ним через встроенный клиент. Для каждого сервера используется отдельный каталог данных, поэтому можно одновременно запускать несколько изолированных окружений.

`clickhousectl` также предоставляет полный набор средств для управления ресурсами ClickHouse Cloud, включая создание и масштабирование сервисов, управление API-ключами и администрирование организаций, что делает его единым инструментом как для локальных, так и для облачных рабочих процессов.
