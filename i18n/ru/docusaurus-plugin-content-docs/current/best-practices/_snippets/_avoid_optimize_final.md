...
Таблицы ClickHouse, использующие **движок MergeTree**, хранят данные на диске в виде **неизменяемых кусков (parts)**, которые создаются при каждой вставке данных.

Каждая вставка создает новый кусок, содержащий отсортированные и сжатые файловые представления столбцов, а также метаданные, такие как индексы и контрольные суммы. Подробное описание структуры кусков и принципов их формирования приведено в этом [руководстве](/parts).

Со временем фоновые процессы объединяют более мелкие куски в крупные, чтобы уменьшить фрагментацию и повысить производительность запросов.

<Image img={simple_merges} size="md" alt="Простые слияния" />

Хотя может показаться заманчивым вручную инициировать это слияние с помощью:
...
`FINAL` можно использовать, если ваши запросы фильтруют по тем же столбцам, которые
входят в первичный ключ.
:::
...
Эти операции **интенсивно используют CPU и подсистему ввода-вывода** и могут создать значительную нагрузку на систему, особенно при работе с большими объёмами данных.
...
- Это может привести к **длительному времени слияния**, **нехватке памяти** или даже **ошибкам нехватки памяти**
...
ClickHouse уже выполняет интеллектуальные фоновые слияния для оптимизации хранения и эффективности запросов. Они выполняются инкрементально, учитывают доступные ресурсы и соблюдают настроенные пороговые значения. Если у вас нет специфических требований (например, финализация данных перед заморозкой таблицы или экспортом), **лучше позволить ClickHouse управлять слияниями самостоятельно**.
