...
:::note
Отправка множества маленьких батчей в синхронном режиме не рекомендуется, поскольку приводит к созданию большого количества частей (parts). Это ухудшит производительность запросов и приведёт к ошибкам [&quot;too many part&quot;](/knowledgebase/exception-too-many-parts).
:::
...
* Буфер достигает заданного размера данных ([`async_insert_max_data_size`](/operations/settings/settings#async_insert_max_data_size), по умолчанию 10 MiB).
