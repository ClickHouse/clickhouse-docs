...
sidebar_label: 'Рекомендации по выбору конфигурации и аппаратному обеспечению'
sidebar_position: 4
title: 'Рекомендации по выбору конфигурации и аппаратному обеспечению'
description: 'В этом руководстве рассматриваются наши общие рекомендации по аппаратному обеспечению, вычислительным ресурсам, памяти и конфигурациям дисков для пользователей открытой версии ClickHouse.'
doc_type: 'guide'
...
# Рекомендации по выбору размеров и аппаратного обеспечения

В этом руководстве рассматриваются наши общие рекомендации по аппаратному обеспечению, вычислительным ресурсам, памяти и конфигурациям дисков для пользователей открытой версии ClickHouse. Если вы хотите упростить свою инфраструктуру, мы рекомендуем использовать [ClickHouse Cloud](https://clickhouse.com/cloud), так как он автоматически масштабируется и адаптируется к вашим нагрузкам, одновременно минимизируя затраты на управление инфраструктурой.

Конфигурация вашего кластера ClickHouse в значительной степени зависит от сценария использования и характера нагрузки вашего приложения. При планировании архитектуры необходимо учитывать следующие факторы:
...
Также можно реализовать многоуровневое хранилище с использованием SSD и HDD в [архитектуре hot/warm/cold](/guides/developer/ttl#implementing-a-hotwarmcold-architecture). Альтернативный вариант — использование [AWS S3](https://aws.amazon.com/s3/) для хранения данных с разделением вычислений и хранения. Ознакомьтесь с нашим руководством по использованию open-source ClickHouse с разделением вычислений и хранения [здесь](/guides/separation-storage-compute). Разделение вычислений и хранения доступно по умолчанию в ClickHouse Cloud.
...
Тип процессора зависит от вашего сценария использования. Однако в целом приложениям с большим количеством частых параллельных запросов, обрабатывающим большие объемы данных или использующим вычислительно интенсивные пользовательские функции (UDF), потребуется больше ядер процессора.
...
Для ClickHouse не существует стандартного целевого показателя загрузки процессора. Используйте инструмент, такой как [iostat](https://linux.die.net/man/1/iostat), для измерения средней загрузки процессора и соответствующим образом корректируйте размер серверов для управления неожиданными всплесками трафика. Однако для аналитических сценариев или сценариев хранилищ данных с ad-hoc запросами следует ориентироваться на загрузку процессора 10-20%.
...
В целом, однако, чем больше памяти, тем быстрее будут выполняться запросы.
...
Для небольших объёмов данных приемлемо соотношение памяти к хранилищу 1:1, но общий объём памяти не должен быть ниже 8 ГБ.

Для сценариев использования с длительными периодами хранения данных или с большими объёмами данных рекомендуется соотношение памяти к хранилищу от 1:100 до 1:130. Например, 100 ГБ оперативной памяти на реплику при хранении 10 ТБ данных.

Для сценариев использования с частым доступом, таких как клиентские нагрузки, рекомендуется использовать больше памяти при соотношении памяти к хранилищу от 1:30 до 1:50.
...
Для справки (а не в качестве рекомендаций) ниже приведены примеры конфигураций пользователей ClickHouse в продуктивной среде:
...
<table>
  ...
  <tr>
    <td>
      <strong>Общее количество vCPU</strong>
    </td>
    <td>2700</td>
  </tr>
  <tr>
...
<table>
    ...
    <tr>
        <td><strong>Объём логов в месяц</strong></td>
        <td>4860TB</td>
    </tr>
    <tr>
        <td><strong>Общий объём хранилища (сжатые данные)</strong></td>
        <td>608TB</td>
    </tr>
    <tr>
    ...
    <tr>
        <td><strong>Общий объём RAM</strong></td>
        <td>10TB</td>
    </tr>
    <tr>
    ...
    <tr>
        <td><strong>Соотношение RAM:vCPU</strong></td>
        <td>6 GB:1</td>
    </tr>
    <tr>
        <td><strong>Соотношение RAM:диск</strong></td>
        <td>1:60</td>
    </tr>
</table>
