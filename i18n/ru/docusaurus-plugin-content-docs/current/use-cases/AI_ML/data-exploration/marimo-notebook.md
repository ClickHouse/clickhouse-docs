...
sidebar_label: 'Исследование данных в ноутбуках Marimo с помощью chDB'
title: 'Исследование данных в ноутбуках Marimo с помощью chDB'
description: 'В этом руководстве объясняется, как настроить и использовать chDB для исследования данных из ClickHouse Cloud или локальных файлов в ноутбуках Marimo'
...
В этом руководстве вы узнаете, как исследовать набор данных в ClickHouse Cloud в ноутбуке Marimo с помощью [chDB](/docs/chdb) — быстрого встраиваемого SQL OLAP-движка на базе ClickHouse.
...
* Выполнять запросы к удалённым наборам данных и преобразовывать результаты в Pandas DataFrames
...
Выполните в терминале следующую команду для добавления имени пользователя и пароля в качестве переменных окружения:
...
:::note
Указанные выше переменные окружения сохраняются только на время сеанса терминала.
...
Находясь в виртуальном окружении, установите следующие пакеты, которые будут использоваться в этом руководстве:
...
Блокноты Marimo хранятся в виде обычных файлов Python, что упрощает контроль версий и обмен ими с другими пользователями.
...
Теперь, когда набор данных о ценах на недвижимость в Великобритании настроен, а chDB запущен в Marimo notebook, можно приступить к исследованию данных.
...
Создайте новую ячейку со следующим запросом для получения данных о ценах на недвижимость в Великобритании из вашего сервиса ClickHouse Cloud и преобразования их в `pandas.DataFrame`:
...
В запросе используется функция [`remoteSecure`](/sql-reference/table-functions/remote) для подключения к ClickHouse Cloud.
...
В данном случае возвращается средняя цена за год с фильтрацией по `town='LONDON'`.
Результат сохраняется в виде DataFrame в переменной `df`.

### Визуализация данных {#visualizing-the-data}

Теперь, когда данные доступны в привычной форме, давайте исследуем, как изменялись цены на недвижимость в Лондоне с течением времени.
...
Одна из сильных сторон Marimo — реактивная модель выполнения. Давайте создадим интерактивный виджет для динамического выбора различных городов.
...
Диаграмма размаха покажет медиану, квартили и выбросы, что даст намного более полное представление, чем просто средняя цена.
...
# создайте интерактивный ящичный диаграмму (box plot).
...
В этом руководстве показано, как использовать chDB для исследования данных в ClickHouse Cloud с помощью ноутбуков Marimo.
...
Благодаря реактивной модели выполнения chDB и Marimo специалисты по работе с данными могут использовать мощные SQL-возможности ClickHouse вместе со знакомыми инструментами Python, такими как Pandas и Plotly, с дополнительным преимуществом в виде интерактивных виджетов и автоматического отслеживания зависимостей, что делает исследовательский анализ более эффективным и воспроизводимым.
