...
В этом руководстве вы узнаете, как исследовать набор данных в ClickHouse Cloud в блокноте Marimo с помощью [chDB](/docs/chdb) — быстрого встроенного (in‑process) SQL OLAP‑движка на базе ClickHouse.

**Предварительные требования:**

* Python 3.8 или выше
* виртуальное окружение
* работающий сервис ClickHouse Cloud и ваши [параметры подключения](/docs/cloud/guides/sql-console/gather-connection-details)

:::tip
Если у вас ещё нет учётной записи ClickHouse Cloud, вы можете [зарегистрироваться](https://console.clickhouse.cloud/signUp?loc=docs-marimo-chdb),
чтобы начать пробный период и получить $300 в виде бесплатных кредитов.
:::

**Чему вы научитесь:**

* подключаться к ClickHouse Cloud из блокнотов Marimo с использованием chDB
* выполнять запросы к удалённым наборам данных и преобразовывать результаты в фреймы данных Pandas (DataFrame)
...
При наведении курсора мыши на ячейку появятся два кружка с символом "+".
...
Результат отобразится под только что выполненной ячейкой:
...
После настройки набора данных о ценах на недвижимость в Великобритании и запуска chDB в блокноте Marimo мы можем приступить к исследованию данных.
Предположим, что нас интересует, как изменялась цена с течением времени для конкретного региона Великобритании, например столицы — Лондона.
...
Результат затем сохраняется в виде DataFrame в переменной с именем `df`.
...
Одной из сильных сторон Marimo является его реактивная модель выполнения. Давайте создадим интерактивный виджет для динамического выбора различных городов.
...
### Исследование распределения цен с помощью интерактивных ящичных диаграмм {#exploring-price-distributions}

Давайте углубимся в данные, изучив распределение цен на недвижимость в Лондоне за разные годы.
Ящичная диаграмма покажет нам медиану, квартили и выбросы, что даст гораздо более полное представление, чем просто средняя цена.
...
# создадим интерактивную диаграмму размаха (box plot).
