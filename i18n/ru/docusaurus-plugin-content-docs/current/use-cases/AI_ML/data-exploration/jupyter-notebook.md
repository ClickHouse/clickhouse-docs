...
description: 'В этом руководстве рассказывается, как настроить и использовать chDB для исследования данных из ClickHouse Cloud или локальных файлов в Jupyter Notebook'
...
# Исследование данных с помощью блокнотов Jupyter и chDB

В этом руководстве вы узнаете, как исследовать набор данных в ClickHouse Cloud в блокноте Jupyter с помощью [chDB](/chdb) — быстрого встраиваемого SQL OLAP‑движка на базе ClickHouse.
...
:::tip
Если у вас ещё нет аккаунта ClickHouse Cloud, вы можете [зарегистрироваться](https://console.clickhouse.cloud/signUp?loc=docs-juypter-chdb), получить пробный доступ и $300 в виде бесплатных кредитов для начала работы.
:::

**Чему вы научитесь:**
- подключаться к ClickHouse Cloud из блокнотов Jupyter с помощью chDB;
...
Предположим, нас интересует, как изменялась цена со временем для конкретного региона Великобритании, например, для столицы — Лондона.
Функция [`remoteSecure`](/sql-reference/table-functions/remote) ClickHouse позволяет легко получать данные из ClickHouse Cloud.
Вы можете указать chDB вернуть эти данные внутри процесса в виде DataFrame библиотеки Pandas — это удобный и привычный способ работы с данными.
...
В зависимости от объема данных это может занять несколько секунд.
В данном случае возвращается средняя цена за год с фильтрацией по условию `town='LONDON'`.
...
В новой ячейке выполните следующую команду, чтобы построить простой график зависимости цены от времени для Лондона с помощью matplotlib:
...
# Покажем каждый второй год, чтобы избежать загромождения
...
<details>
  ...
  Также можно читать данные из нескольких источников за один шаг. Для этого вы можете использовать следующий запрос с `JOIN`:
...
<Image size="md" img={image_8} alt="предпросмотр DataFrame" />
...
# Построить график проданных домов по левой оси Y
color = 'tab:blue'
ax1.set_xlabel('Year')
...
# Создание второй оси Y для данных о стоимости
ax2 = ax1.twinx()
color = 'tab:red'
...
На примере набора данных UK Property Price мы продемонстрировали, как выполнять запросы к удалённым данным ClickHouse Cloud с помощью функции `remoteSecure()`, читать локальные CSV-файлы с помощью движка таблиц `file()` и преобразовывать результаты непосредственно в Pandas DataFrames для анализа и визуализации.
...
Хотя многие лондонские специалисты по данным вряд ли смогут позволить себе собственное жильё в ближайшее время, по крайней мере, они могут проанализировать рынок, который вытеснил их своими ценами!
