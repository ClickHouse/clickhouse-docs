...
title: 'Как создать агента ИИ LangChain/LangGraph с использованием сервера ClickHouse MCP.'
pagination_prev: null
pagination_next: null
description: 'Узнайте, как создать агента ИИ LangChain/LangGraph, который может взаимодействовать с SQL‑песочницей ClickHouse с использованием сервера ClickHouse MCP.'
...
# Как создать AI-агента LangChain/LangGraph с использованием сервера ClickHouse MCP

В этом руководстве вы узнаете, как создать AI-агента [LangChain/LangGraph](https://github.com/langchain-ai/langgraph),
который может взаимодействовать с [SQL-песочницей ClickHouse](https://sql.clickhouse.com/) с помощью [сервера ClickHouse MCP](https://github.com/ClickHouse/mcp-clickhouse).

:::note Пример ноутбука
Этот пример доступен в виде ноутбука в [репозитории с примерами](https://github.com/ClickHouse/examples/blob/main/ai/mcp/langchain/langchain.ipynb).
:::
...
инструкции по настройке учетных данных можно найти в [документации по провайдерам Langchain](https://python.langchain.com/docs/integrations/providers/)
:::
...
Настройте обработчик для потокового вывода, чтобы упростить его использование:
...
class UltraCleanStreamHandler:
    ...
    def handle_chunk(self, chunk):
        ...
        if event == "on_chat_model_stream":
            ...
            # Обрабатывать только фактическое текстовое содержимое, пропускать потоки вызова инструментов
            if hasattr(chunk_data, 'content'):
                content = chunk_data.content
                if isinstance(content, str) and not content.startswith('{"'):
                    # Добавить пробел после завершения работы инструмента при необходимости
...
Наконец, вызовите агента и спросите, кто внёс больше всего кода в ClickHouse:
...
🔧 run_select_query ✅ На основе данных о git-коммитах ClickHouse **Alexey Milovidov** внёс больше всего кода в ClickHouse по нескольким показателям:
...
1. **Больше всего изменённых строк (всего)**: Alexey Milovidov — **1 696 929 изменённых строк** (853 049 добавлено + 843 880 удалено)
...
## Топ-участники по количеству добавленных строк:
...
Alexey Milovidov, безусловно, является самым продуктивным участником разработки ClickHouse, что логично, поскольку он один из первых создателей и ведущих разработчиков проекта. Его вклад значительно превосходит вклад остальных как по общему объёму кода, так и по количеству коммитов: почти 16 000 коммитов и более 850 000 строк кода, добавленных в проект.
