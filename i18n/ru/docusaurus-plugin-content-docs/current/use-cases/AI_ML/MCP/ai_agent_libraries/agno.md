...
Этот пример доступен в виде блокнота в [репозитории с примерами](https://github.com/ClickHouse/examples/blob/main/ai/mcp/agno/agno.ipynb).
:::
...
┃ запрос к базе данных ClickHouse. Однако сначала нужно собрать информацию и убедиться, что мы работаем с        ┃
┃ правильными данными. Давайте сначала проверим доступные базы данных и таблицы.Спасибо за предоставленный       ┃
┃ список баз данных. Я вижу, что есть база данных "github", которая, вероятно, содержит нужную информацию.       ┃
┃ Давайте проверим таблицы в этой базе данных.Теперь, когда у нас есть информация о таблицах в базе данных       ┃
┃ github, мы можем запросить необходимые данные для ответа на ваш вопрос о проекте с наибольшим количеством      ┃
┃ звёзд в 2025 году. Мы будем использовать таблицу repo_events_per_day, которая содержит ежедневное количество   ┃
┃ событий для каждого репозитория, включая события добавления в избранное (WatchEvents).                         ┃
┃                                                                                                                 ┃
