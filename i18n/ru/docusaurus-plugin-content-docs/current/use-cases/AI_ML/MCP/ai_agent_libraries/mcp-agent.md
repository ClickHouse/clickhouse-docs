...
- На основе таблиц UK Price Paid в ClickHouse, для транзакций, зарегистрированных в 2025 году на текущий момент, имеется 376 633 продажи со средней ценой
£362 283 и медианой £281 000. Данные, по-видимому, включают только месяцы январь–август 2025 года (таким образом, данные за 2025 год неполные). Присутствуют экстремальные
выбросы (мин. £100, макс. £127 700 000), которые смещают среднее значение.
```



Что именно и как я считал
Я выполнил агрегирующие запросы по таблицам uk.price-paid в ClickHouse:
- общая сводка по 2025 году (count, mean, median, min, max) из uk.uk_price_paid_simple_partitioned
- помесячное разбиение за 2025 год (transactions, mean, median)
- топ городов в 2025 году по средней цене (города с >= 50 транзакциями)
- сравнение по годам: 2024 vs 2025 (count, mean, median)
- разбиение по типу недвижимости за 2025 год (counts, avg, median) с использованием uk.uk_price_paid
...
- Топ городов по средней цене (2025, города с ≥50 транзакциями)
  ...
  - EAST MOLESEY, BEACONSFIELD, ESHER, CHALFONT ST GILES, THAMES DITTON также входят в топ‑10 (все это обеспеченные пригородные/коммутивные города с высокой средней ценой).
- Сравнение по годам (2024 vs 2025 по фактическим данным)
  - 2024: 859,960 transactions, mean £390,879, median £280,000
  - 2025: 376,633 transactions, mean £362,283, median £281,000
  (Показатели за 2025 год существенно ниже, поскольку набор данных охватывает только часть года.)
...
- Набор данных по 2025 году выглядит неполным (присутствуют только месяцы Jan–Aug). Любые итоги за «2025» не являются показателями за полный год.
- Присутствуют значительные выбросы (например, max £127.7M и min £100). Вероятно, они включают ошибки ввода данных или нестандартные записи и завышают
mean. В данном случае median часто является более устойчивой метрикой.
- Средние значения для типа недвижимости «other» нестабильны из‑за малого/разнородного числа записей и выбросов.
...
leaseholds).
- Таблицы содержат записи транзакций в формате Price Paid (зарегистрированные продажи) — они не представляют напрямую цены предложений или оценочную стоимость.
