...
# Как создать ИИ-агента с помощью Claude Agent SDK и ClickHouse MCP Server

В этом руководстве вы узнаете, как создать ИИ-агента на основе [Claude Agent SDK](https://docs.claude.com/en/api/agent-sdk/overview), который может взаимодействовать с 
...
Этот пример доступен в виде блокнота в [репозитории примеров](https://github.com/ClickHouse/examples/blob/main/ai/mcp/claude-agent/claude-agent.ipynb).
:::
...
Это логично, учитывая, что Бейкер-стрит является одним из самых престижных адресов Лондона, проходящим через богатые районы, такие как Мэрилебон, и в этом наборе данных зафиксировано 541 продажа.
```
