...
title: 'Как создать агента OpenAI с помощью сервера ClickHouse MCP.'
pagination_prev: null
pagination_next: null
description: 'Узнайте, как создать агента OpenAI, который может взаимодействовать с сервером ClickHouse MCP.'
...
# Как создать агента OpenAI с использованием сервера MCP ClickHouse

В этом руководстве вы узнаете, как создать агента [OpenAI](https://github.com/openai/openai-agents-python), который
может взаимодействовать с [SQL-песочницей ClickHouse](https://sql.clickhouse.com/) с помощью [сервера MCP ClickHouse](https://github.com/ClickHouse/mcp-clickhouse).
...
def simple_render_chunk(chunk):
    """Упрощенная версия, которая фильтрует только важные события"""
    ...
    if (hasattr(chunk, 'type') and
        ...
        if chunk.name == 'tool_called':
            ...
            try:
                # Обработка как строкового, так и уже разобранного вывода
                ...
                if isinstance(output, dict):
                    ...
                    # Запасной вариант - просто вывести необработанный результат
                    print(f"✅ Result: {str(output)[:100]}...")

            except (json.JSONDecodeError, AttributeError, KeyError) as e:
                # Запасной вариант с необработанным выводом при ошибке разбора
