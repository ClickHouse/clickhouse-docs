...
sidebar_label: 'Интеграция с Ollama'
title: 'Настройка сервера ClickHouse MCP с Ollama'
pagination_prev: null
pagination_next: null
description: 'В этом руководстве описывается, как настроить Ollama для работы с сервером ClickHouse MCP.'
...
После завершения загрузки вы можете запустить модель следующим образом:
...
:::note
С MCP Servers будут работать только [модели с поддержкой инструментов](https://ollama.com/search?c=tools).
:::
...
Для получения подробной информации о загруженной модели используйте следующую команду:
...
Из этого вывода видно, что модель qwen3 по умолчанию содержит чуть более 8 миллиардов параметров.


## Установка MCPHost {#install-mcphost}

На момент написания (июль 2025 года) нативная функциональность для использования Ollama с MCP-серверами отсутствует.
...
MCPHost будет искать конфигурационные файлы в вашем домашнем каталоге в следующем порядке:
...
Ниже приведен пример конфигурации сервера ClickHouse MCP, который мы сохраним в файл `~/.mcphost.json`:
...
Тип используется для указания типа транспорта, используемого MCP-сервером.
...
:::note
Теоретически эти переменные можно указать под ключом `environment` в конфигурационном файле MCP, однако мы обнаружили, что это не работает.
:::
...
По нашему опыту, при использовании небольших моделей (модель qwen3 по умолчанию имеет 8 миллиардов параметров) необходимо более точно формулировать запросы.
Например, нужно явно попросить вывести список баз данных и таблиц, а не сразу запрашивать данные из конкретной таблицы.
