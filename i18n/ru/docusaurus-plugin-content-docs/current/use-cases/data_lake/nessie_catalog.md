...
description: 'В этом руководстве мы пошагово покажем, как выполнять запросы к данным с помощью ClickHouse и каталога Nessie.'
...
ClickHouse поддерживает интеграцию с несколькими каталогами (Unity, Glue, REST, Polaris и т. д.). В этом руководстве показано, как выполнять запросы к вашим данным в ClickHouse с использованием каталога [Nessie](https://projectnessie.org/).

Nessie — это транзакционный каталог с открытым исходным кодом для дата-лейков, который обеспечивает:

* **Git-подобный** контроль версий данных с ветками и коммитами
...
* **Готовность к использованию в продакшене** при развёртывании в Docker или Kubernetes

:::note
Поскольку эта функция является экспериментальной, вам нужно включить её с помощью команды:
...
2. **Пример настройки**: Можно использовать официальную настройку docker-compose для Nessie

### Настройка локального каталога Nessie {#setting-up-local-nessie-catalog}

Можно использовать официальную [настройку docker-compose для Nessie](https://projectnessie.org/guides/setting-up/), которая предоставляет полное окружение с Nessie, хранилищем версий в памяти и MinIO для объектного хранилища.
...
:::note
Установка Nessie использует хранилище версий в памяти и требует предварительной загрузки тестовых данных в таблицы Iceberg. Убедитесь, что окружение создало и заполнило таблицы, прежде чем выполнять запросы к ним через ClickHouse.
:::
...
Теперь, когда соединение установлено, можно начать выполнять запросы через каталог Nessie. Например:
...
Если ваша установка включает примеры данных (например, набор данных о такси), вы увидите таблицы следующего вида:
...
1. Окружение еще не создало примеры таблиц
2. Служба каталога Nessie не полностью инициализирована
...
Для загрузки данных из каталога Nessie в ClickHouse сначала создайте локальную таблицу ClickHouse:
