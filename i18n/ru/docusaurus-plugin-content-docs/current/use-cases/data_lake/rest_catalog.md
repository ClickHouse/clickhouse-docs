...
ClickHouse поддерживает интеграцию с несколькими каталогами (Unity, Glue, REST, Polaris и т. д.). В этом руководстве описаны шаги по выполнению запросов к вашим данным с использованием ClickHouse и спецификации [REST Catalog](https://github.com/apache/iceberg/blob/main/open-api/rest-catalog-open-api.yaml/).

REST Catalog — это стандартизованная спецификация API для каталогов Iceberg, поддерживаемая различными платформами, включая:

* **Локальные среды разработки** (на основе docker-compose)
...
**Шаг 4:** Дождитесь готовности всех сервисов. Вы можете проверить логи:
...
## Запрос таблиц REST-каталога с помощью ClickHouse {#querying-rest-catalog-tables-using-clickhouse}

Теперь, когда соединение установлено, можно начать выполнять запросы через REST-каталог. Например:
...
1. Окружение Spark еще не создало тестовые таблицы
2. Служба REST-каталога не полностью инициализирована
