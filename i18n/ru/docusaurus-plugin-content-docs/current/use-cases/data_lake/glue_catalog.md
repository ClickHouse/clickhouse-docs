...
description: 'В этом руководстве мы шаг за шагом покажем, как выполнять запросы к данным в S3-бакетах с помощью ClickHouse и каталога данных AWS Glue (AWS Glue Data Catalog).'
...
и др.). В этом руководстве мы шаг за шагом покажем, как выполнять запросы к данным
в S3-бакетах с помощью ClickHouse и Glue Data Catalog.

:::note
Glue поддерживает множество различных форматов таблиц, но данная интеграция
...
`iceberg-benchmark.hitsparquet`. Запросы к ним выполнить не получится, так как в настоящее время поддерживается только Iceberg.
...
Для просмотра DDL таблицы выполните следующий запрос:
