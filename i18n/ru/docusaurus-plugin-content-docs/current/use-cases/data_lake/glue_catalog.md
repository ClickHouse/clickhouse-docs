...
description: 'В этом руководстве мы проведем вас через шаги для запроса ваших данных в S3-ведрах с использованием ClickHouse и AWS Glue Data Catalog.'
---
...
ClickHouse поддерживает интеграцию с несколькими каталогами (Unity, Glue, Polaris и др.). В этом руководстве мы проведем вас через шаги для запроса ваших данных в S3-ведрах с использованием ClickHouse и Glue Data Catalog.

:::note
Glue поддерживает много разных форматов таблиц, но эта интеграция поддерживает только таблицы Iceberg.
:::
...
:::note
На данный момент каталог Glue поддерживает только ключи доступа и секретные ключи, но в будущем мы будем поддерживать дополнительные подходы к аутентификации.
:::
...
Теперь, когда соединение установлено, вы можете начать запрашивать Glue:
...
Вы можете видеть, что некоторые таблицы выше не являются таблицами Iceberg, например `iceberg-benchmark.hitsparquet`. Вы не сможете выполнять запросы к ним, так как в данный момент поддерживается только Iceberg.
