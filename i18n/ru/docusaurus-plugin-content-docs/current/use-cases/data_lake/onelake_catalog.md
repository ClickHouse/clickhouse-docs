...
description: 'В этом руководстве мы шаг за шагом покажем, как выполнять запросы к данным в Microsoft OneLake.'
...
ClickHouse поддерживает интеграцию с несколькими каталогами (OneLake, Unity, Glue, Polaris и др.). В этом руководстве описаны шаги по выполнению запросов к данным, хранящимся в Microsoft OneLake, с помощью ClickHouse и [OneLake](https://learn.microsoft.com/en-us/fabric/onelake/onelake-overview).

Microsoft OneLake поддерживает несколько форматов таблиц для своего lakehouse. В ClickHouse вы можете выполнять запросы к таблицам Iceberg.
