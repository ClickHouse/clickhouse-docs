...
ClickHouse обеспечивает непревзойдённую скорость, масштаб и экономичность для задач наблюдаемости. В этом руководстве предлагаются два подхода в зависимости от ваших потребностей:
...
**ClickStack** — это платформа наблюдаемости промышленного уровня, построенная на базе ClickHouse и OpenTelemetry (OTel), объединяющая логи, трассировки, метрики и сессии в единое высокопроизводительное масштабируемое решение, которое работает как на однонодовых развертываниях, так и в масштабах **нескольких петабайт**.
...
| [Search](/use-cases/observability/clickstack/search)                    | Как выполнять поиск и запросы к данным наблюдаемости |
| [Production](/use-cases/observability/clickstack/production)            | Лучшие практики для развертывания в продакшене  |
...
| Страница                                                              | Описание                                                                                                                                                            |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Introduction](/use-cases/observability/introduction)                 | Это руководство предназначено для пользователей, желающих создать собственное решение для наблюдаемости с использованием ClickHouse, с акцентом на логи и трассировки. |
