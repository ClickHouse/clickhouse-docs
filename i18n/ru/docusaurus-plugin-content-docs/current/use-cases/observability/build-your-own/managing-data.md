...
description: 'Управление данными для observability'
...
Развертывания ClickHouse для Observability неизбежно связаны с большими объёмами данных, которыми необходимо управлять. ClickHouse предлагает ряд возможностей, помогающих в этом.
...
Партиционирование задаётся для таблицы при её первоначальном определении с помощью секции `PARTITION BY`. Эта секция может содержать SQL-выражение для любого столбца или столбцов, результат которого определит, в какую партицию будет направлена строка.

<Image img={observability_14} alt='Партиции' size='md' />

Куски данных логически связаны (через общий префикс имени папки) с каждой партицией на диске и могут запрашиваться изолированно. В приведённом ниже примере схема `otel_logs` по умолчанию разбивается на партиции по дням с использованием выражения `toDate(Timestamp)`. При вставке строк в ClickHouse это выражение вычисляется для каждой строки и направляет её в соответствующую партицию, если она существует (если строка является первой за день, партиция будет создана).
...
В качестве примера предположим, что наша таблица `otel_logs` разбита на партиции по дням. Если она заполнена структурированным набором данных логов, она будет содержать данные за несколько дней:
...
--подтверждаем, что данные были перемещены
SELECT
...
**Важно: Мы рекомендуем использовать настройку [`ttl_only_drop_parts=1`](/operations/settings/merge-tree-settings#ttl_only_drop_parts)** (применяется схемой по умолчанию). Когда эта настройка включена, ClickHouse удаляет целый кусок, когда все строки в нём истекли. Удаление целых кусков вместо частичной очистки строк с истёкшим TTL (достигаемой через ресурсоёмкие мутации при `ttl_only_drop_parts=0`) позволяет использовать более короткие значения `merge_with_ttl_timeout` и снижает влияние на производительность системы. Если данные партиционированы по той же единице времени, по которой выполняется истечение TTL, например, по дням, куски естественным образом будут содержать только данные из определённого интервала. Это обеспечит эффективное применение `ttl_only_drop_parts=1`.
...
В ClickHouse пользователи могут создавать уровни хранения на различных дисках, например, горячие/актуальные данные на SSD и более старые данные на базе S3. Такая архитектура позволяет использовать более дешёвое хранилище для старых данных, к которым предъявляются менее строгие требования по SLA запросов из-за их редкого использования в аналитических задачах.
...
Создание уровней хранения требует от пользователей создания дисков, которые затем используются для формирования политик хранения с томами, указываемыми при создании таблицы. Данные могут автоматически перемещаться между дисками на основе степени заполнения, размеров кусков и приоритетов томов. Дополнительные сведения можно найти [здесь](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-multiple-volumes).
...
Схемы логов и трассировок неизбежно меняются в течение жизненного цикла системы, например, когда пользователи начинают мониторить новые системы с другими метаданными или метками pod. Благодаря использованию схемы OTel для формирования данных и сохранению исходных данных событий в структурированном формате схемы ClickHouse будут устойчивы к таким изменениям. Однако по мере появления новых метаданных и изменения паттернов доступа к данным пользователям потребуется обновлять схемы для отражения этих изменений.
