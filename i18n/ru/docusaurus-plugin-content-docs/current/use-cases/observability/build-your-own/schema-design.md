...
description: 'Проектирование схемы для обеспечения наблюдаемости'
...
# Проектирование схемы для observability

Мы рекомендуем пользователям всегда создавать собственную схему для логов и трассировок по следующим причинам:

- **Выбор первичного ключа** — Схемы по умолчанию используют `ORDER BY`, оптимизированный под определённые шаблоны доступа. Маловероятно, что ваши шаблоны доступа будут с ними совпадать.
- **Извлечение структуры** — Пользователи могут захотеть извлекать новые столбцы из существующих столбцов, например из столбца `Body`. Это можно сделать с помощью материализованных столбцов (и материализованных представлений в более сложных случаях). Это требует изменения схемы.
- **Оптимизация Maps** — Схемы по умолчанию используют тип Map для хранения атрибутов. Эти столбцы позволяют хранить произвольные метаданные. Несмотря на то что это важная возможность, так как метаданные событий часто не определены заранее и иначе не могут быть сохранены в строго типизированной базе данных вроде ClickHouse, доступ к ключам map и их значениям менее эффективен, чем доступ к обычному столбцу. Мы решаем это путём изменения схемы и вынесения наиболее часто используемых ключей map в столбцы верхнего уровня — см. раздел ["Extracting structure with SQL"](#extracting-structure-with-sql). Это требует изменения схемы.
- **Упрощение доступа к ключам map** — Доступ к ключам в map требует более многословного синтаксиса. Пользователи могут упростить это с помощью алиасов. См. раздел ["Using Aliases"](#using-aliases) для упрощения запросов.
- **Вторичные индексы** — Схема по умолчанию использует вторичные индексы для ускорения доступа к Map и ускорения текстовых запросов. Обычно они не требуются и увеличивают использование дискового пространства. Их можно применять, но следует протестировать, действительно ли они необходимы. См. раздел ["Secondary / Data Skipping indices"](#secondarydata-skipping-indices).
- **Использование кодеков** — Пользователи могут захотеть настроить кодеки для столбцов, если они понимают ожидаемые данные и имеют подтверждение того, что это улучшает сжатие.

_Ниже мы подробно описываем каждый из вышеперечисленных вариантов использования._

**Важно:** хотя пользователям и рекомендуется расширять и модифицировать свою схему для достижения оптимального сжатия и производительности запросов, по возможности им следует придерживаться соглашений OTel по именованию основных столбцов. Плагин ClickHouse для Grafana предполагает наличие некоторых базовых столбцов OTel, упрощающих построение запросов, например Timestamp и SeverityText. Требуемые столбцы для логов и трассировок задокументированы здесь [[1]](https://grafana.com/developers/plugin-tools/tutorials/build-a-logs-data-source-plugin#logs-data-frame-format)[[2]](https://grafana.com/docs/grafana/latest/explore/logs-integration/) и [здесь](https://grafana.com/docs/grafana/latest/explore/trace-integration/#data-frame-structure) соответственно. Вы можете изменить эти имена столбцов, переопределив значения по умолчанию в конфигурации плагина.
...
Предполагая, что `LogAttributes` доступен, запрос для подсчёта того, какие URL-пути сайта получают больше всего POST-запросов:
...
Эквивалентную схему для извлечения данных с помощью JSON-функций из строки `Body` можно найти [здесь](https://pastila.nl/?005cbb97/513b174a7d6114bf17ecc657428cf829#gqoOOiomEjIiG6zlWhE+Sg==).

Наши три материализованных столбца извлекают страницу запроса, тип запроса и домен реферера. Они обращаются к ключам map-колонки и применяют функции к их значениям. Наш последующий запрос выполняется значительно быстрее:
...
:::note
Материализованные столбцы по умолчанию не возвращаются в результате `SELECT *`. Это сделано для сохранения инварианта: результат `SELECT *` всегда можно вставить обратно в таблицу с помощью INSERT. Это поведение можно изменить, установив `asterisk_include_materialized_columns=1`, а также его можно включить в Grafana (см. `Additional Settings -> Custom Settings` в конфигурации источника данных).
:::
...
Запрос, связанный с материализованным представлением, теоретически может быть любым запросом, включая агрегацию, хотя [существуют ограничения при использовании JOIN](https://clickhouse.com/blog/using-materialized-views-in-clickhouse#materialized-views-and-joins). Для задач преобразования и фильтрации, необходимых для работы с логами и трассировками, пользователи могут использовать любой оператор `SELECT`.

Пользователям следует помнить, что запрос — это просто триггер, выполняющийся над строками, вставляемыми в таблицу (исходную таблицу), а результаты отправляются в новую таблицу (целевую таблицу).

Чтобы не сохранять данные дважды (в исходной и целевой таблицах), можно изменить движок исходной таблицы на [движок таблиц Null](/engines/table-engines/special/null), сохранив исходную схему. Наши коллекторы OTel продолжат отправлять данные в эту таблицу. Например, для логов таблица `otel_logs` принимает следующий вид:
...
Движок таблиц Null — это мощная оптимизация, можно представить его как `/dev/null`. Эта таблица не будет хранить никаких данных, но все присоединенные материализованные представления по-прежнему будут выполняться над вставляемыми строками до того, как они будут отброшены.

Рассмотрим следующий запрос. Он преобразует наши строки в формат, который мы хотим сохранить, извлекая все столбцы из `LogAttributes` (предполагается, что это было установлено коллектором с использованием оператора `json_parser`), устанавливая `SeverityText` и `SeverityNumber` (на основе простых условий и определения [этих столбцов](https://opentelemetry.io/docs/specs/otel/logs/data-model/#field-severitytext)). В данном случае мы также выбираем только те столбцы, которые, как мы знаем, будут заполнены, игнорируя такие столбцы, как `TraceId`, `SpanId` и `TraceFlags`.
...
Мы также извлекаем столбец `Body` выше — на случай, если позже будут добавлены дополнительные атрибуты, которые не извлекаются нашим SQL. Этот столбец должен хорошо сжиматься в ClickHouse и будет редко запрашиваться, поэтому не повлияет на производительность запросов. Наконец, мы понижаем точность `Timestamp` до `DateTime` (для экономии места — см. [&quot;Optimizing Types&quot;](#optimizing-types)) с помощью приведения типа.

:::note Conditionals
Обратите внимание на использование [conditionals](/sql-reference/functions/conditional-functions) выше для извлечения `SeverityText` и `SeverityNumber`. Они чрезвычайно полезны для формулирования сложных условий и проверки, установлены ли значения в отображениях (map) — мы наивно предполагаем, что все ключи существуют в `LogAttributes`. Мы рекомендуем пользователям с ними ознакомиться — это ваш союзник при разборе логов, в дополнение к функциям для обработки [null values](/sql-reference/functions/functions-for-nulls)!
:::

Нам требуется таблица для записи этих результатов. Целевая таблица ниже соответствует приведённому выше запросу:
...
:::note
Обратите внимание, насколько сильно мы изменили схему. На практике у пользователей, вероятно, также будут столбцы Trace, которые они захотят сохранить, а также столбец `ResourceAttributes` (как правило, он содержит метаданные Kubernetes). Grafana может использовать столбцы трассировок, чтобы предоставлять функциональность связывания между логами и трассировками — см. [&quot;Using Grafana&quot;](/observability/grafana).
:::


Ниже мы создаём материализованное представление `otel_logs_mv`, которое выполняет указанный выше запрос SELECT к таблице `otel_logs` и отправляет результаты в `otel_logs_v2`.
...
Если теперь перезапустить конфигурацию коллектора, использованную в разделе [&quot;Exporting to ClickHouse&quot;](/observability/integrating-opentelemetry#exporting-to-clickhouse), данные появятся в `otel_logs_v2` в нужном нам формате. Обратите внимание на использование типизированных функций извлечения JSON.
...
Получена 1 строка. Затрачено: 0,010 сек.
```

Эквивалентное материализованное представление, которое извлекает столбцы из столбца `Body` с помощью JSON‑функций, показано ниже:
...
### Учитывайте типы данных {#beware-types}

Приведенные выше материализованные представления используют неявное приведение типов — особенно при работе со словарем `LogAttributes`. ClickHouse часто автоматически приводит извлеченное значение к типу целевой таблицы, упрощая синтаксис. Тем не менее, мы рекомендуем всегда тестировать представления, выполняя запрос `SELECT` представления с инструкцией [`INSERT INTO`](/sql-reference/statements/insert-into) в целевую таблицу с той же схемой. Это позволит убедиться, что типы обрабатываются корректно. Особое внимание следует уделить следующим случаям:

- Если ключ отсутствует в словаре, будет возвращена пустая строка. Для числовых значений потребуется явно сопоставить их с соответствующим значением. Это можно сделать с помощью [условных функций](/sql-reference/functions/conditional-functions), например `if(LogAttributes['status'] = ", 200, LogAttributes['status'])`, или [функций приведения типов](/sql-reference/functions/type-conversion-functions), если допустимы значения по умолчанию, например `toUInt8OrDefault(LogAttributes['status'] )`
- Некоторые типы не всегда приводятся автоматически, например, строковые представления чисел не будут приведены к значениям перечислений.
- Функции извлечения JSON возвращают значения по умолчанию для своего типа, если значение не найдено. Убедитесь, что эти значения имеют смысл в вашем контексте!

:::note Избегайте Nullable
Избегайте использования [Nullable](/sql-reference/data-types/nullable) в ClickHouse для данных наблюдаемости. В логах и трассировках редко требуется различать пустые значения и null. Эта возможность создает дополнительные накладные расходы на хранение и негативно влияет на производительность запросов. Подробнее см. [здесь](/data-modeling/schema-design#optimizing-types).
:::


## Выбор первичного (сортирующего) ключа {#choosing-a-primary-ordering-key}

После извлечения нужных столбцов можно приступить к оптимизации сортирующего/первичного ключа.
...
Запросы к типам данных `Map` выполняются медленнее, чем запросы к обычным столбцам — см. раздел ["Ускорение запросов"](#accelerating-queries). Кроме того, синтаксис таких запросов сложнее и может быть неудобен для пользователей. Чтобы решить последнюю проблему, рекомендуется использовать столбцы‑псевдонимы (`ALIAS`).

Столбцы типа `ALIAS` вычисляются во время выполнения запроса и не хранятся в таблице. Поэтому выполнить `INSERT` значения в столбец этого типа невозможно. С помощью псевдонимов можно обращаться к ключам карты и упрощать синтаксис, прозрачно предоставляя элементы `Map` как обычные столбцы. Рассмотрим следующий пример:
...
В таблице определено несколько материализованных столбцов и столбец типа `ALIAS` — `RemoteAddr`, который обращается к карте `LogAttributes`. Теперь мы можем запрашивать значения `LogAttributes['remote_addr']` через этот столбец, тем самым упрощая наш запрос, например:
...
5 строк в наборе. Время выполнения: 0.011 сек.
```

Кроме того, добавить столбец типа `ALIAS` очень просто с помощью команды `ALTER TABLE`. Такие столбцы становятся доступны сразу, например:
...
5 строк в наборе. Время выполнения: 0.014 сек.
```

:::note Псевдонимы по умолчанию исключаются
По умолчанию `SELECT *` не включает столбцы типа `ALIAS`. Это поведение можно изменить, установив параметр `asterisk_include_alias_columns=1`.
:::


## Оптимизация типов данных {#optimizing-types}

[Общие рекомендации ClickHouse](/data-modeling/schema-design#optimizing-types) по оптимизации типов данных применимы к использованию ClickHouse.
...
[Словари](/sql-reference/dictionaries) — это [ключевая возможность](https://clickhouse.com/blog/faster-queries-dictionaries-clickhouse) ClickHouse, обеспечивающая представление данных в памяти в формате [ключ-значение](https://en.wikipedia.org/wiki/Key%E2%80%93value_database) из различных внутренних и внешних [источников](/sql-reference/dictionaries#dictionary-sources), оптимизированное для запросов поиска со сверхнизкой задержкой.

<Image img={observability_12} alt='Наблюдаемость и словари' size='md' />

Это полезно в различных сценариях: от обогащения поступающих данных на лету без замедления процесса загрузки до улучшения производительности запросов в целом, особенно при использовании JOIN.
Хотя соединения редко требуются в сценариях наблюдаемости, словари всё равно могут быть полезны для обогащения данных — как при вставке, так и при выполнении запросов. Ниже мы приводим примеры обоих подходов.
...
Словари можно использовать для обогащения наборов данных при выполнении запросов или при вставке. Каждый из этих подходов имеет свои преимущества и недостатки. Вкратце:

- **При вставке** — этот подход обычно подходит, если значение для обогащения не меняется и существует во внешнем источнике, который можно использовать для заполнения словаря. В этом случае обогащение строки при вставке позволяет избежать поиска в словаре во время запроса. Это достигается за счёт снижения производительности вставки, а также дополнительных затрат на хранение, поскольку обогащённые значения будут храниться в виде столбцов.
- **При запросе** — если значения в словаре часто меняются, поиск во время запроса обычно более применим. Это позволяет избежать необходимости обновлять столбцы (и переписывать данные) при изменении сопоставленных значений. Такая гибкость достигается за счёт затрат на поиск во время запроса. Эти затраты обычно заметны, если требуется поиск для большого количества строк, например, при использовании поиска в словаре в условии фильтрации. Для обогащения результатов, то есть в `SELECT`, эти накладные расходы обычно незначительны.

Мы рекомендуем пользователям ознакомиться с основами работы со словарями. Словари предоставляют таблицу поиска в памяти, из которой значения можно извлекать с помощью специализированных [функций](/sql-reference/functions/ext-dict-functions#dictgetall).

Простые примеры обогащения данных см. в руководстве по словарям [здесь](/dictionary). Ниже мы рассмотрим типичные задачи обогащения данных наблюдаемости.

### Использование IP-словарей {#using-ip-dictionaries}

Геообогащение логов и трассировок значениями широты и долготы на основе IP-адресов — это распространённое требование в области наблюдаемости. Мы можем реализовать это с помощью структурированного словаря `ip_trie`.
...
Учитывая эту структуру, давайте начнём с просмотра данных с помощью табличной функции [url()](/sql-reference/table-functions/url):
...
Чтобы упростить задачу, давайте используем движок таблиц [`URL()`](/engines/table-engines/special/url) для создания объекта таблицы ClickHouse с нашими именами полей и подтвердим общее количество строк:
...
Поскольку наш словарь `ip_trie` требует, чтобы диапазоны IP-адресов задавались в CIDR-нотации, нам нужно преобразовать `ip_range_start` и `ip_range_end`.

CIDR-блок для каждого диапазона можно вычислить с помощью следующего запроса:
...
:::note
В приведённом выше запросе происходит много всего. Тем, кому интересно, рекомендуем прочитать это отличное [объяснение](https://clickhouse.com/blog/geolocating-ips-in-clickhouse-and-grafana#using-bit-functions-to-convert-ip-ranges-to-cidr-notation). Иначе просто примите, что указанный выше запрос вычисляет CIDR для диапазона IP-адресов.
:::

Для наших целей нам понадобятся только диапазон IP-адресов, код страны и координаты, поэтому давайте создадим новую таблицу и вставим в неё наши данные Geo IP:
...
Чтобы выполнять IP-поиск с низкой задержкой в ClickHouse, мы будем использовать словари для хранения сопоставления ключ → атрибуты для наших Geo IP данных в памяти. ClickHouse предоставляет структуру словаря `ip_trie` ([dictionary structure](/sql-reference/dictionaries#ip_trie)) для сопоставления сетевых префиксов (CIDR-блоков) с координатами и кодами стран. Следующий запрос задаёт словарь с такой структурой и использует в качестве источника приведённую выше таблицу.
...
3 строки в наборе. Время выполнения: 4,662 сек.
...
Возвращаясь к нашему исходному датасету логов, мы можем использовать описанный выше подход, чтобы агрегировать логи по странам. Далее предполагается, что мы используем схему, полученную из нашего предыдущего материализованного представления, в которой есть извлечённый столбец `RemoteAddress`.
...
Получено 5 строк. Прошло: 0.140 сек. Обработано 20.73 млн строк, 82.92 МБ (147.79 млн строк/с., 591.16 МБ/с.)
Пиковое использование памяти: 1.16 МиБ.
```

Поскольку соответствие IP-адреса географическому местоположению может меняться, пользователям, скорее всего, важно знать, откуда пришёл запрос в момент его выполнения, а не каково текущее географическое местоположение для того же адреса. По этой причине здесь, вероятнее всего, предпочтительнее обогащение на этапе индексирования. Это можно сделать с использованием материализованных столбцов, как показано ниже, или в `SELECT` материализованного представления:
...
:::note Периодическое обновление
Пользователям, вероятно, потребуется периодически обновлять словарь обогащения IP на основе новых данных. Это можно реализовать с помощью параметра `LIFETIME` словаря, который обеспечивает периодическую перезагрузку словаря из базовой таблицы. Для обновления базовой таблицы см. ["Обновляемые материализованные представления"](/materialized-view/refreshable-materialized-view).
:::
...
:::note
В приведенных ниже примерах мы используем снимки последних регулярных выражений uap-core для парсинга user-agent от июня 2024 года. Последний файл, который периодически обновляется, можно найти [здесь](https://raw.githubusercontent.com/ua-parser/uap-core/master/regexes.yaml). Пользователи могут следовать инструкциям [здесь](/sql-reference/dictionaries#collecting-attribute-values) для загрузки в CSV-файл, используемый ниже.
:::
...
После загрузки этих словарей мы можем передать пример значения `user-agent` и протестировать наши новые возможности извлечения данных из словарей:
...
Учитывая, что правила, связанные с user agents, будут редко меняться, а словарь потребуется обновлять только при появлении новых браузеров, операционных систем и устройств, имеет смысл выполнять это извлечение на этапе вставки данных.

Мы можем выполнять эту работу либо с помощью материализованного столбца, либо с помощью материализованного представления. Ниже мы модифицируем использованное ранее материализованное представление:
...
После перезапуска коллектора и загрузки структурированных логов, выполненной по ранее описанным шагам, мы можем выполнять запросы к новым извлечённым столбцам Device, Browser и Os.
...
Обратите внимание на использование кортежей для этих столбцов user agent. Кортежи рекомендуются для сложных структур с заранее известной иерархией. Подстолбцы обеспечивают такую же производительность, как и обычные столбцы (в отличие от ключей Map), при этом допуская гетерогенные типы.
:::
...
ClickHouse поддерживает ряд методов ускорения выполнения запросов. Следующие подходы следует рассматривать только после выбора подходящего первичного ключа/ключа сортировки для оптимизации наиболее распространенных шаблонов доступа и максимизации сжатия. Как правило, это оказывает наибольшее влияние на производительность при минимальных усилиях.
...
- Использовать [модификатор `FINAL`](/sql-reference/statements/select/from#final-modifier) для имени таблицы (что мы и сделали в запросе подсчёта выше).
- Агрегировать по ключу сортировки, используемому в финальной таблице, то есть по Timestamp, и суммировать метрики.

Обычно второй вариант более эффективен и гибок (таблицу можно использовать для других целей), но первый может быть проще для некоторых запросов. Ниже показаны оба варианта:
...
Эта схема оптимизирована для фильтрации по `ServiceName`, `SpanName` и `Timestamp`. В трейсинге пользователям также нужна возможность выполнять поиск по конкретному `TraceId` и получать спаны, относящиеся к этому трейсe. Хотя это поле присутствует в ключе сортировки, его положение в конце означает, что [фильтрация будет менее эффективной](/guides/best-practices/sparse-primary-indexes#ordering-key-columns-efficiently) и, скорее всего, приведёт к необходимости сканировать значительные объёмы данных при получении одного трейса.

Коллектор OTel также устанавливает материализованное представление и связанную таблицу, чтобы решить эту задачу. Таблица и представление показаны ниже:
...
Представление по сути гарантирует, что таблица `otel_traces_trace_id_ts` содержит минимальную и максимальную метки времени для трассы. Эта таблица, упорядоченная по `TraceId`, позволяет эффективно получать эти значения. Далее, эти диапазоны меток времени можно использовать при выполнении запросов к основной таблице `otel_traces`. В частности, при получении трассы по её идентификатору Grafana использует следующий запрос:
...
Этот же подход можно применить для аналогичных шаблонов доступа. Похожий пример рассматривается в разделе моделирования данных [здесь](/materialized-view/incremental-materialized-view#lookup-table).
...
В предыдущих разделах мы рассмотрели, как материализованные представления могут использоваться в ClickHouse для предварительного вычисления агрегаций, преобразования строк и оптимизации запросов наблюдаемости под различные шаблоны доступа.
...
:::note Проекции vs материализованные представления
...
Приведенный выше запрос требует линейного сканирования с нашим выбранным ключом сортировки `(ServiceName, Timestamp)`. Хотя мы могли бы добавить `Status` в конец ключа сортировки, улучшив производительность для данного запроса, мы также можем добавить проекцию.
...
Важно: если проекция создается через `ALTER`, её создание выполняется асинхронно при выполнении команды `MATERIALIZE PROJECTION`. Пользователи могут отслеживать прогресс этой операции следующим запросом, ожидая `is_done=1`.
...
При повторном выполнении запроса выше видно, что производительность значительно улучшилась за счет дополнительного использования дискового пространства (см. раздел ["Измерение размера таблицы и сжатия"](#measuring-table-size--compression)).
...
В приведенном выше примере мы указываем в проекции столбцы, используемые в предыдущем запросе. Это означает, что на диске в составе проекции будут храниться только эти столбцы, упорядоченные по Status. Если бы вместо этого использовался `SELECT *`, хранились бы все столбцы. Хотя это позволило бы большему числу запросов (использующих любое подмножество столбцов) воспользоваться проекцией, потребуется дополнительное дисковое пространство. Для измерения дискового пространства и сжатия см. раздел ["Измерение размера таблицы и сжатия"](#measuring-table-size--compression).

### Вторичные индексы/индексы пропуска данных {#secondarydata-skipping-indices}

Независимо от того, насколько хорошо настроен первичный ключ в ClickHouse, некоторые запросы неизбежно потребуют полного сканирования таблицы. Хотя это можно смягчить с помощью материализованных представлений (и проекций для некоторых запросов), они требуют дополнительного обслуживания, и пользователи должны знать об их наличии для эффективного использования. В то время как традиционные реляционные базы данных решают эту проблему с помощью вторичных индексов, в колоночных базах данных, таких как ClickHouse, они неэффективны. Вместо этого ClickHouse использует индексы пропуска (Skip indexes), которые могут значительно улучшить производительность запросов, позволяя базе данных пропускать большие блоки данных без совпадающих значений.

Схемы OTel по умолчанию используют вторичные индексы для ускорения доступа к map-структурам. Хотя мы считаем их в целом неэффективными и не рекомендуем копировать в пользовательские схемы, индексы пропуска все же могут быть полезны.

Пользователям следует изучить [руководство по вторичным индексам](/optimize/skipping-indexes) перед их применением.

**В целом, они эффективны при наличии сильной корреляции между первичным ключом и целевым непервичным столбцом/выражением, когда пользователи ищут редкие значения, т.е. те, которые встречаются в небольшом числе гранул.**

### Фильтры Блума для текстового поиска {#bloom-filters-for-text-search}


Для запросов в сфере Observability вторичные индексы могут быть полезны, когда пользователям нужно выполнять текстовый поиск. В частности, блум-фильтры на основе ngram и токенов — [`ngrambf_v1`](/optimize/skipping-indexes#bloom-filter-types) и [`tokenbf_v1`](/optimize/skipping-indexes#bloom-filter-types) — могут использоваться для ускорения поиска по столбцам типа String с операторами `LIKE`, `IN` и hasToken. Важно, что индекс на основе токенов генерирует токены, используя неалфавитно-цифровые символы в качестве разделителей. Это означает, что во время выполнения запроса могут сопоставляться только токены (или целые слова). Для более детального сопоставления можно использовать [N-gram bloom filter](/optimize/skipping-indexes#bloom-filter-types). Он разбивает строки на n-граммы заданного размера, что позволяет выполнять сопоставление по частям слов.

Чтобы понять, какие токены будут сгенерированы и, соответственно, сопоставляться, можно использовать функцию `tokens`:
...
Функция `ngram` предоставляет схожие возможности: размер `ngram` можно задать вторым параметром.
...
В ClickHouse также есть экспериментальная поддержка инвертированных индексов как вторичных индексов. Сейчас мы не рекомендуем использовать их для наборов данных логов, но ожидаем, что они заменят токен-ориентированные bloom-фильтры, когда будут готовы к промышленной эксплуатации.
:::

Для целей этого примера мы используем набор данных со структурированными логами. Предположим, что мы хотим посчитать логи, в которых столбец `Referer` содержит `ultra`.
...
Здесь нам нужно использовать n-граммы длиной 3. Поэтому мы создаём индекс `ngrambf_v1`.
...
Индекс `ngrambf_v1(3, 10000, 3, 7)` здесь принимает четыре параметра. Последний из них (значение 7) — это seed. Остальные задают размер ngram (3), значение `m` (размер фильтра) и количество хеш‑функций `k` (7). Параметры `k` и `m` требуют настройки и зависят от количества уникальных ngram/токенов и вероятности того, что фильтр вернёт истинный отрицательный результат — тем самым подтверждая, что значение отсутствует в грануле. Мы рекомендуем воспользоваться [этими функциями](/engines/table-engines/mergetree-family/mergetree#bloom-filter), чтобы подобрать эти значения.


При грамотной настройке ускорение может быть существенным:
...
:::note Только пример
Приведённый выше пример предназначен исключительно для иллюстрации. Мы рекомендуем пользователям извлекать структуру из своих логов на этапе вставки, а не пытаться оптимизировать текстовый поиск с помощью токен-ориентированных bloom-фильтров. Тем не менее, есть случаи, когда у пользователей есть stack trace или другие большие значения типа `String`, для которых текстовый поиск может быть полезен из‑за менее детерминированной структуры.
:::

Несколько общих рекомендаций по использованию bloom-фильтров:

Цель bloom-фильтра — отфильтровать [гранулы](/guides/best-practices/sparse-primary-indexes#clickhouse-index-design), тем самым избегая необходимости загружать все значения столбца и выполнять линейное сканирование. Оператор `EXPLAIN` с параметром `indexes=1` можно использовать для определения количества гранул, которые были пропущены. Ниже приведены примеры ответов для исходной таблицы `otel_logs_v2` и таблицы `otel_logs_bloom` с bloom-фильтром ngram.
...
Фильтр Блума, как правило, будет работать быстрее только в том случае, если он меньше самого столбца. Если он больше, выигрыш в производительности, скорее всего, будет пренебрежимо мал. Сравните размер фильтра с размером столбца с помощью следующих запросов:
...
Получена 1 строка. Время выполнения: 0.018 сек.
...
Тип Map широко используется в схемах OTel. Этот тип требует, чтобы значения и ключи имели одинаковый тип — этого достаточно для метаданных, таких как метки Kubernetes. Обратите внимание, что при запросе подключа типа Map загружается весь родительский столбец. Если в map много ключей, это может привести к значительному снижению производительности запроса, так как потребуется считать с диска больше данных, чем если бы ключ существовал как отдельный столбец.

Если вы часто запрашиваете определенный ключ, рассмотрите возможность перемещения его в отдельный выделенный столбец на корневом уровне. Обычно это задача, которая выполняется в ответ на типичные паттерны доступа после развертывания, и может быть сложно предсказать её необходимость до выхода в production. См. раздел ["Управление изменениями схемы"](/observability/managing-data#managing-schema-changes) о том, как изменить схему после развертывания.
```
...
Помимо значительного снижения затрат на хранение, меньший объем данных на диске означает меньше операций ввода-вывода и более быстрые запросы и вставки. Снижение нагрузки на ввод-вывод перевешивает накладные расходы любого алгоритма сжатия с точки зрения использования процессора. Поэтому улучшение сжатия данных должно быть первоочередной задачей при работе над обеспечением быстрой работы запросов ClickHouse.
