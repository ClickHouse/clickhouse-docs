...
Кроме того, добавление `ALIAS` с помощью команды `ALTER TABLE` выполняется очень просто. Эти столбцы сразу же становятся доступными, например:
...
Важно: если проекция создаётся с помощью команды `ALTER`, то при выполнении команды `MATERIALIZE PROJECTION` её создание происходит асинхронно. Вы можете проверить ход выполнения этой операции следующим запросом, ожидая значения `is_done=1`.
