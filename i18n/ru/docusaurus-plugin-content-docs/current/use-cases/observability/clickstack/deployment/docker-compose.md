...
Все компоненты ClickStack распространяются отдельно в виде индивидуальных Docker-образов:
...
Эти порты обеспечивают интеграцию с различными источниками телеметрии и делают коллектор OpenTelemetry готовым к промышленной эксплуатации для разнообразных сценариев приема данных.
...
- Развертывания ClickStack при размещении ClickHouse отдельно, например, с использованием ClickHouse Cloud
...
Для развертывания с помощью Docker Compose клонируйте репозиторий HyperDX, перейдите в его каталог и выполните команду `docker-compose up`:
...
Чтобы установить эти параметры, измените соответствующие сервисы в `docker-compose.yaml`:
