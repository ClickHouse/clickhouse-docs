...
keywords:
  [
    ...
    "развертывание ClickHouse с Helm",
    "установка HyperDX с Helm",
    "стек наблюдаемости Kubernetes",
    "развертывание ClickStack в Kubernetes"
  ]
---
...
Helm-чарт для HyperDX можно найти [здесь](https://github.com/hyperdxio/helm-charts), и это **рекомендуемый** метод для продакшен-развертываний.
...
Чарт поддерживает стандартные best practices Kubernetes, включая:
...
- Кластер Kubernetes (рекомендуется версия 1.20 и выше)
...
Для установки чарта HyperDX со значениями по умолчанию выполните:
...
Перенаправление портов позволяет получить доступ к HyperDX и выполнить его настройку. Для развертывания в производственной среде рекомендуется предоставлять доступ к сервису через ingress или балансировщик нагрузки для обеспечения корректного сетевого доступа, терминирования TLS и масштабируемости. Перенаправление портов подходит для локальной разработки или разовых административных задач, но не для долгосрочного использования или сред с высокой доступностью.
...
При нажатии `Create` будут созданы источники данных для экземпляра ClickHouse, развернутого с помощью чарта Helm.
...
Альтернативно можно отредактировать файл `values.yaml`. Чтобы получить значения по умолчанию, выполните:
...
Для работы с конфиденциальными данными, такими как ключи API или учетные данные базы данных, используйте секреты Kubernetes. Чарты Helm для HyperDX предоставляют файлы секретов по умолчанию, которые можно изменить и применить к вашему кластеру.

#### Использование предварительно настроенных секретов {#using-pre-configured-secrets}

Чарт Helm включает шаблон секрета по умолчанию, расположенный по адресу [`charts/hdx-oss-v2/templates/secrets.yaml`](https://github.com/hyperdxio/helm-charts/blob/main/charts/hdx-oss-v2/templates/secrets.yaml). Этот файл предоставляет базовую структуру для управления секретами.
...
При использовании ClickHouse Cloud необходимо отключить экземпляр ClickHouse, развёрнутый с помощью Helm-чарта, и указать учётные данные Cloud:
...
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `tasks.enabled`               | Включить/отключить задачи cron в кластере. По умолчанию образ HyperDX выполняет задачи cron внутри процесса. Установите значение true, если хотите использовать отдельную задачу cron в кластере. | `false`               |
...
Для удаления развертывания выполните команду:

```shell
helm uninstall my-hyperdx
```

Эта команда удалит все ресурсы, связанные с релизом, однако постоянные данные (при их наличии) могут сохраниться.
