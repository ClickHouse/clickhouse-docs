...
import OtelTracesSchema from '@site/i18n/ru/docusaurus-plugin-content-docs/current/use-cases/observability/clickstack/ingesting-data/_snippets/_schema_otel_traces.md';
...
## Оптимизация 2. Добавление индексов пропуска данных \{#adding-skip-indexes\}
...
Рассмотрим схему трейсов по умолчанию для ClickStack, которая включает индексы пропуска данных:

<OtelTracesSchema />

Эти индексы ориентированы на три распространённых сценария:

* Фильтрация строк с высокой кардинальностью, таких как TraceId, идентификаторы сессий, ключи атрибутов или значения
* Фильтрация по вложенным ключам Map, ускоренная [текстовыми индексами](#text-indexes) для столбцов [`*AttributeItems`](#map-direct-read-optimization)
* Фильтрация по числовым диапазонам, например по длительности спанов

Таблица logs повсеместно использует индексы `text(tokenizer = 'array')` вместо bloom-фильтров, а также добавляет индекс `text(tokenizer = 'splitByNonAlpha')` на `lower(Body)` для полнотекстового поиска. Полный DDL см. в разделе [&quot;Таблицы и схемы, используемые ClickStack&quot;](/use-cases/observability/clickstack/ingesting-data/schemas#logs).
...
[Текстовые индексы](/engines/table-engines/mergetree-family/textindexes) предлагают альтернативу bloom-фильтрам. Bloom-фильтр — это вероятностная структура, которая может однозначно исключать гранулы, но допускает ложноположительные срабатывания, поэтому гранулы, которые он не исключил, всё равно нужно загружать и проверять по условию `WHERE`. Текстовые индексы — это инвертированные индексы, которые сопоставляют токены с точными смещениями внутри части. Поскольку они работают со смещениями, а не с гранулами, и не дают ложноположительных срабатываний, обычно они могут обработать условие `WHERE` без загрузки исходного столбца. Эта оптимизация называется [прямое чтение](https://github.com/ClickHouse/clickhouse-docs/pull/6356/%E2%80%A6). Поскольку загрузка данных часто даёт наибольший вклад во время выполнения запроса, прямое чтение может заметно снизить задержку запроса.

Кроме того, текстовые индексы сами доступны для запросов, что обеспечивает автодополнение и другие возможности диагностики в ClickStack.

Два токенизатора покрывают большинство сценариев в ClickStack:

| Токенизатор       | Используется для                                                 | Типичный столбец                  |
| ----------------- | ---------------------------------------------------------------- | --------------------------------- |
| `array`           | Индексирование элементов `Array(String)` как отдельных токенов   | `mapKeys(...)`, `*AttributeItems` |
| `splitByNonAlpha` | Полнотекстовый поиск на уровне слов по строкам с обычным текстом | `Body`, `lower(Body)`, `SpanName` |

#### Токенизатор Array для Map и столбцов типа Array \{#array-tokenizer\}

В схеме журналов по умолчанию для `mapKeys` и материализованных массивов элементов используется токенизатор `array`:
...
Каждый ключ Map (или элемент массива) становится отдельным токеном. Фильтрация по
известному ключу атрибута затем отсекает любую строку, которая его не содержит, без сканирования
самого столбца Map. Именно этот механизм делает [оптимизацию прямого чтения Map](#map-direct-read-optimization)
эффективной.

#### `splitByNonAlpha` для тел логов \{#text-index-body\}

Полнотекстовый поиск по столбцу `Body` выигрывает от использования текстового
индекса `splitByNonAlpha`. ClickStack определяет этот индекс для `lower(Body)`,
чтобы его можно было использовать для регистронезависимого поиска Lucene:

```sql
INDEX idx_lower_body lower(Body) TYPE text(tokenizer = 'splitByNonAlpha')
```

Когда ClickStack обнаруживает индекс `text(tokenizer = 'splitByNonAlpha')` на
`lower(Body)`, он преобразует Lucene-запросы по неявному столбцу, такие как `error` или
`"connection refused"`, в `hasAllTokens(lower(Body), lower(...))`, которые
индекс может обработать без чтения всего столбца `Body`. Для большинства
нагрузок логов в обсервабилити это самое существенное доступное ускорение
фильтрации.

:::note Текстовые индексы и `tokenbf_v1`
Более старый тип индекса `tokenbf_v1` (по-прежнему используемый в схеме traces по умолчанию для
`lower(SpanName)`) функционально похож, но считается устаревшим в ClickHouse 26.2
и выше. Для новых индексов текстового поиска следует использовать `text(tokenizer = ...)`.
:::

Более подробную справочную информацию о параметрах токенизатора, препроцессорах и проверке см. в [документации по полнотекстовому поиску](/engines/table-engines/mergetree-family/textindexes).

#### Текстовые индексы в схеме журналов по умолчанию \{#text-indexes-in-default-logs-schema\}

Схема `otel_logs` по умолчанию, синхронизируемая с upstream, включает все текстовые индексы, рассмотренные выше: `text(tokenizer = 'array')` для `TraceId`, каждого массива `mapKeys(...)` и `*AttributeItems`, а `text(tokenizer = 'splitByNonAlpha')` — для `lower(Body)` при полнотекстовом поиске. Канонический DDL см. в [&quot;Таблицы и схемы, используемые в ClickStack&quot;](/use-cases/observability/clickstack/ingesting-data/schemas#logs); та же схема приведена ниже.

<OtelLogsSchema />

### Индексы min-max \{#min-max-indexes\}
...
### Материализовать индекс пропуска данных \{#materialize-skip-index\}

После того как индекс пропуска данных был добавлен, он применяется только к вновь поступающим данным. Исторические данные не смогут воспользоваться преимуществами индекса, пока он не будет явно материализован.
...
:::note[Материализация индексов пропуска данных]
Материализация индекса пропуска данных, как правило, легковесна и безопасна для выполнения, особенно для индексов minmax. Для индексов Bloom filter на больших наборах данных пользователи могут предпочесть материализовывать их по партициям, чтобы лучше контролировать использование ресурсов, например:
...
Материализация индекса пропуска данных выполняется как мутация. За ходом её выполнения можно отслеживать с помощью системных таблиц.
...
### Оценка эффективности индексов пропуска данных \{#evaluating-skip-index-effectiveness\}

Наиболее надёжный способ оценить отсечение с помощью индексов пропуска данных — использовать `EXPLAIN indexes = 1`, который показывает, сколько [частей](/parts) и [гранул](/guides/best-practices/sparse-primary-indexes#data-is-organized-into-granules-for-parallel-data-processing) исключается на каждом этапе планирования запроса. В большинстве случаев желательно видеть значительное уменьшение числа гранул на этапе Skip, причём после того, как первичный ключ уже сократил пространство поиска. Индексы пропуска данных оцениваются после отсечения по партициям и первичному ключу, поэтому их влияние лучше всего измерять относительно оставшихся частей и гранул.

`EXPLAIN` подтверждает, происходит ли отсечение, но не гарантирует итоговое ускорение. Оценка индексов пропуска данных тоже стоит ресурсов, особенно если индекс большой. Всегда замеряйте производительность запросов до и после добавления и материализации индекса, чтобы подтвердить реальные улучшения.

Например, рассмотрим индекс пропуска данных Bloom filter по умолчанию для TraceId, включённый в стандартную схему Traces:

```sql
INDEX idx_trace_id TraceId TYPE bloom_filter(0.001) GRANULARITY 1
```

Вы можете использовать `EXPLAIN indexes = 1`, чтобы оценить, насколько эффективен индекс пропуска данных для селективного запроса:
...
В этом случае фильтр по первичному ключу сначала существенно сокращает набор данных (с 35898 гранул до 255), а затем фильтр Блума дополнительно сужает его до одной гранулы (1/255). Это идеальный сценарий для индексов пропуска данных: отсечение по первичному ключу уменьшает область поиска, после чего индекс пропуска данных отбрасывает большую часть оставшихся данных.
...
:::tip
Если `EXPLAIN` показывает минимальное отсечение гранул или индекс пропуска данных очень большой, стоимость его оценки может свести на нет любую выгоду. Используйте `EXPLAIN indexes = 1`, чтобы подтвердить отсечение, затем проведите бенчмаркинг, чтобы подтвердить улучшение сквозной производительности.
:::

### Когда добавлять индексы пропуска данных \{#when-to-add-skip-indexes\}

Индексы пропуска данных следует добавлять выборочно, исходя из типов фильтров, которые пользователи используют чаще всего, и структуры данных в частях и гранулах. Цель состоит в том, чтобы отбросить достаточно гранул, чтобы компенсировать стоимость вычисления самого индекса, поэтому тестирование на данных, близких к боевым, крайне важно.

**Для числовых столбцов, которые используются в фильтрах, minmax-индекс пропуска данных почти всегда является хорошим выбором.** Он легковесный, дешевый в вычислении и может быть эффективен для диапазонных предикатов — особенно когда значения слабо упорядочены или ограничены узкими диапазонами внутри частей. Даже если minmax не помогает для какого‑то конкретного шаблона запроса, его накладные расходы обычно достаточно низки, чтобы его имело смысл оставить.

**Для строковых столбцов по возможности предпочитайте текстовые индексы; в противном случае используйте фильтры Блума.** Текстовые индексы ускоряют те же фильтры на равенство и `IN`, что и фильтры Блума, а также дополнительно поддерживают предикаты на основе токенов (`hasToken`, `hasAllTokens`, `has`), используемые в полнотекстовом поиске и [оптимизации прямого чтения Map](#map-direct-read-optimization). В старых кластерах, которые ещё не поддерживают текстовые индексы, фильтры Блума остаются надежным выбором.
...
## Оптимизация 3. Прямое чтение Map \{#map-direct-read-optimization\}

Когда вы фильтруете по вложенному ключу Map, например `LogAttributes['k8s.pod.name'] =
'checkout'`, ClickHouse должен прочитать с диска весь столбец Map `LogAttributes`
и распаковать каждую строку, чтобы вычислить предикат. [Материализация часто запрашиваемых атрибутов](#materialize-frequently-queried-attributes)
решает эту проблему для ключей, известных заранее, но этот подход не
масштабируется на произвольные атрибуты, по которым пользователи фильтруют данные ad hoc.

Даже если схема содержит индексы по `mapKeys` и `mapValues`, эти индексы могут определить, есть ли в строке заданный ключ и есть ли в ней заданное значение, но не могут определить, относятся ли ключ и значение к одной и той же записи. Иными словами, `mapKeys` отвечает на `mapContainsKey(ResourceAttributes, 'foo')`, а `mapValues` отвечает на `mapContainsValue(ResourceAttributes, 'bar')`, но ни один из них не отвечает на `ResourceAttributes['foo'] = 'bar'`.

Объединяя ключи и значения в один столбец `Array(String)`, оптимизация
прямого чтения Map позволяет отвечать на `ResourceAttributes['foo'] = 'bar'`
без загрузки исходного Map. Map часто бывают большими и
увеличиваются в размере по мере роста объема данных. В сочетании с переписыванием
запроса на уровне приложения фильтры на равенство по любому вложенному ключу Map сводятся к одному вызову `has(...)`,
который использует этот индекс, без десериализации Map во время выполнения запроса. Кроме того,
дополнительные затраты на хранение связаны только с текстовым индексом, так как исходный столбец
является столбцом `ALIAS` и не хранится.

Эта оптимизация применяется автоматически. ClickStack включает необходимые столбцы и
индексы в стандартные таблицы журналов и трассировок, а также переписывает фильтры
по обращению к элементам Map во время выполнения, когда подключенный ClickHouse server поддерживает
соответствующий базовый примитив. Если в вашей схеме нет этих столбцов или у вас есть
дополнительные столбцы Map, которые вы хотите ускорить сверх стандартного набора, читайте далее,
чтобы включить их.

### Схема \{#map-direct-read-schema\}

Для каждого столбца Map, чтение которого вы хотите ускорить, ClickStack определяет
столбец `Array(String)` `ALIAS`, в котором каждый ключ и значение объединены через `=`:
...
Форма ALIAS означает, что массив не занимает места на диске. ClickHouse вычисляет его
во время выполнения запроса и при построении индекса. `text(tokenizer = 'array')` индекс пропуска данных для
столбца `ALIAS` хранит по одному токену для каждой пары `key=value`, и ClickHouse использует его,
чтобы отсеивать гранулы, не обращаясь к исходному Map:
...
После создания индекса для существующей таблицы материализуйте его, чтобы он применялся и к историческим
данным (см. [&quot;Материализация индекса пропуска данных&quot;](#materialize-skip-index)).

Схемы ClickStack по умолчанию включают следующие столбцы и индексы:

| Таблица       | Столбцы ALIAS                                                        | Текстовые индексы                                                  |
| ------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------ |
...
### Переписывание запроса \{#map-direct-read-rewrite\}

Когда пользователь фильтрует по вложенному ключу в Map через интерфейс ClickStack или SDK, ClickStack
переписывает:
...
Переписанная форма использует текстовый индекс для `LogAttributeItems`, отбрасывает целые
строки, не содержащие токен `key=value`, и никогда не десериализует
исходный Map `LogAttributes` для несовпадающих строк. Для рабочих нагрузок
обсервабилити с высокой мощностью это обычно дает сокращение
I/O на порядок по сравнению с доступом по ключу в Map.

Переписывание происходит автоматически — сохраненные запросы, панели мониторинга и алерты,
которые ссылаются на `LogAttributes['key']`, получают ускорение без каких-либо изменений.

### Требования к версии ClickHouse \{#map-direct-read-version\}

Для переписывания запроса требуется версия ClickHouse, которая поддерживает прямое
отсечение на уровне токенов для столбцов типа Array с текстовым индексом. ClickStack определяет
версию подключённого сервера (`SELECT version()`, кэшируется для каждого подключения) и применяет
переписанную форму только если версия сервера равна пороговой или выше. На более старых
серверах автоматически используется исходная форма с обращением по индексу в Map.
...
:::note Почему ALIAS, а не MATERIALIZED
Массив items — это представление данных, которые уже хранятся в столбце Map.
Хранить их дважды — один раз в Map, второй раз в массиве — означало бы удвоить I/O записи
без появления новых шаблонов запросов. Текстовый индекс для столбца `ALIAS`
строится во время вставки из тех же исходных данных, поэтому эта оптимизация добавляет на диск
только объём, занимаемый индексом.
:::
...
Ниже приведены первичные ключи по умолчанию для таблиц ClickStack для логов и трейсов.
...
См. [&quot;Tables and schemas used by ClickStack&quot;](/use-cases/observability/clickstack/ingesting-data/schemas) для первичных ключей, используемых в таблицах для других типов данных. Таблицы трейсов оптимизированы для фильтрации по имени сервиса и имени спана, а затем по временной метке. Таблицы логов начинаются с пятиминутного временного бакета, чтобы сканирование по временному диапазону сначала задействовало первичный индекс, а затем сужалось по имени сервиса внутри каждого бакета — такая структура подходит для типичного сценария «что произошло за последние N минут для сервиса X». Хотя оптимальным является применение фильтров в порядке следования столбцов первичного ключа, запросы всё равно будут существенно выигрывать при фильтрации по любому из этих столбцов в любом порядке, поскольку ClickHouse будет [отсекать данные перед чтением](/optimize/skipping-indexes).
...
Следующий процесс можно использовать, чтобы старые данные были сохранены и по‑прежнему прозрачно запрашивались (с использованием существующего ключа в интерфейсе ClickStack при необходимости), в то время как новые данные выводятся через новую таблицу, оптимизированную под шаблоны доступа пользователей. Такой подход гарантирует, что конвейеры ингестии не нужно изменять: данные по‑прежнему отправляются в таблицы по умолчанию, а все изменения остаются незаметными для пользователей.
...
<VerticalStepper headerLevel="h4">
  ...
  #### Обновить интерфейс ClickStack для чтения из таблицы Merge

  Настройте интерфейс ClickStack на использование `otel_logs_merge` как таблицы для источника данных логов.
  ...
  Записи теперь направляются в новую таблицу `otel_logs` с обновлённым первичным ключом. Существующие данные остаются в `otel_logs_23_01_2025` и по‑прежнему доступны через таблицу Merge. Суффикс указывает дату, когда изменение было применено, и соответствует максимальной временной метке, содержащейся в этой таблице.
...
### Ускорение поиска строк с помощью блочных столбцов

Схема журналов ClickStack по умолчанию включает две настройки MergeTree, которые
не влияют напрямую на производительность запросов, но заметно ускоряют поиск
подробной информации о строках в интерфейсе ClickStack:

```sql
SETTINGS enable_block_number_column = 1, enable_block_offset_column = 1
```

С этими настройками каждая строка в таблице содержит неявную
пару `(_block_number, _block_offset)`, которая однозначно идентифицирует её внутри
part. Когда вы нажимаете на строку журнала в интерфейсе ClickStack, чтобы открыть панель сведений, ClickStack
выполняет дополнительный запрос, чтобы получить именно эту строку. Без block columns
в предложение `WHERE` должны входить достаточно столбцов — обычно первичный ключ,
а также `Body` и `SeverityText`, — чтобы однозначно определить строку. С block columns
достаточно первичного ключа, `_block_number` и `_block_offset`. Крупные
столбцы, такие как `Body`, при lookup вообще не читаются, что фактически ускоряет запрос.

ClickStack определяет эту настройку по оператору `CREATE` таблицы и автоматически
формирует более компактное предложение WHERE, когда включены оба столбца. Никаких
изменений в конфигурации приложения не требуется.

Чтобы включить эту оптимизацию для существующей таблицы logs или traces:
...
Эти настройки применяются к данным, записанным после `ALTER`. Существующие части по-прежнему
используют старый построчный поиск, пока не будут переписаны при слиянии.

## Оптимизация 5. Использование materialized views
...
## Оптимизация 6. Использование PROJECTION
