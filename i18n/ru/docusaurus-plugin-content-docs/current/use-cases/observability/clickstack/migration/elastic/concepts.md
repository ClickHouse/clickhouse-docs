...
|--------------------------|--------------------------------------------------|--------------------------------------------------|--------------|
| **UI и оповещения** | **Kibana** — дашборды, поиск и оповещения      | **ClickStack UI (HyperDX)** — UI в реальном времени, поиск и оповещения   | Оба выступают в роли основного интерфейса для пользователей, включая визуализации и управление оповещениями. UI ClickStack специально разработан для задач наблюдаемости и тесно связан с семантикой OpenTelemetry. |
...
В сценариях обсервабилити одним из наиболее распространённых применений агрегаций является подсчёт того, как часто возникают определённые сообщения логов или события (и оповещение, если частота выглядит аномальной).
...
На диаграмме выше исходная таблица материализованного представления уже содержит часть данных, хранящую некоторые `blue` строки (1–10), относящиеся к одной и той же группе. Для этой группы уже существует и часть данных в целевой таблице представления, которая хранит [промежуточное состояние агрегации](https://www.youtube.com/watch?v=QDAJTKZT8y4) для группы `blue`. Когда выполняются вставки ① ② ③ новых строк в исходную таблицу, для каждой вставки создаётся соответствующая часть данных исходной таблицы и параллельно (только) для каждого блока вновь вставленных строк вычисляется промежуточное состояние агрегации, которое в виде части данных вставляется в целевую таблицу материализованного представления. ④ Во время фонового слияния частей промежуточные состояния агрегации объединяются, что приводит к инкрементальной агрегации данных.

Обратите внимание, что все [агрегатные функции](/sql-reference/aggregate-functions/reference) (их более 90), включая их комбинации с [комбинаторами агрегатных функций](https://www.youtube.com/watch?v=7ApwD0cfAFI), поддерживают [промежуточные состояния агрегации](/sql-reference/data-types/aggregatefunction).
