...
#### Настройте двойную ингестию \{#configure-dual-ingestion\}
...
#### Отрегулируйте сроки хранения данных \{#adjust-retention-period\}
...
#### Проверьте и сравните \{#validate-and-compare\}
...
#### Постепенный переход \{#graudal-transition\}
...
<VerticalStepper headerLevel="h3">
  ...
  Создайте таблицу в ClickHouse для индекса, переносимого из Elasticsearch. Можно сопоставить [типы данных Elasticsearch с их эквивалентами в ClickHouse](/use-cases/observability/clickstack/migration/elastic/types). Альтернативно можно полагаться на тип данных JSON в ClickHouse, который при вставке данных автоматически создаёт столбцы соответствующего типа.
  ...
  * Кортежи используются для представления вложенных структур вместо точечной нотации
  * Использованы соответствующие типы данных ClickHouse на основе маппинга:
    * `keyword` → `String`
    * `date` → `DateTime`
    * `boolean` → `UInt8`
    * `long` → `Int64`
    * `ip` → `Array(Variant(IPv4, IPv6))`. Здесь мы используем [`Variant(IPv4, IPv6)`](/sql-reference/data-types/variant), так как поле содержит одновременно и [`IPv4`](/sql-reference/data-types/ipv4), и [`IPv6`](/sql-reference/data-types/ipv6).
    * `object` → `JSON` для объекта syslog со структурой, не определённой заранее.
  * Столбцы `host.ip` и `host.mac` имеют явный тип `Array`, в отличие от Elasticsearch, где все типы являются массивами.
  * Добавлено предложение `ORDER BY` по временной метке и имени хоста для эффективного выполнения временных запросов
  * `MergeTree`, оптимальный для данных логов, используется в качестве типа движка
  ...
  * **Проверка данных** – применение строгой схемы позволяет избежать риска неконтролируемого разрастания числа столбцов за пределами специально предусмотренных структур.
  * **Снижает риск взрывного роста числа столбцов**: хотя тип JSON может масштабироваться до тысяч столбцов, при этом подстолбцы хранятся как отдельные столбцы, что может привести к «взрыву» файлов столбцов, когда создаётся чрезмерное их количество, негативно влияющее на производительность. Чтобы минимизировать это, базовый тип [Dynamic](/sql-reference/data-types/dynamic), используемый для JSON, предоставляет параметр [`max_dynamic_paths`](/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns), который ограничивает количество уникальных путей, хранимых как отдельные файлы столбцов. Как только порог достигнут, дополнительные пути сохраняются в общем файле столбца с использованием компактного кодированного формата, что позволяет сохранить производительность и эффективность хранения при гибкой ингестии данных. Однако доступ к этому общему файлу столбца менее производителен. Обратите внимание, что столбец JSON может использоваться с [type hints](/integrations/data-formats/json/schema#using-type-hints-and-skipping-paths). Столбцы с подсказками по типам обеспечат ту же производительность, что и выделенные столбцы.
  * **Упрощённая интроспекция путей и типов**: хотя тип JSON поддерживает [функции интроспекции](/sql-reference/data-types/newjson#introspection-functions) для определения выведенных типов и путей, статические структуры зачастую проще исследовать, например, с помощью `DESCRIBE`.
  ...
  В качестве альтернативы вы можете просто создать таблицу с одним столбцом `JSON`.
  ...
  :::note
  Мы указываем подсказку типа для столбцов `host.name` и `timestamp` в определении JSON, так как используем их в порядке сортировки/первичном ключе. Это помогает ClickHouse понять, что этот столбец не будет NULL, и гарантирует, что он выберет правильные подстолбцы (для каждого типа может быть несколько, поэтому в противном случае это неоднозначно).
  :::

  Этот подход, хотя и проще, лучше всего подходит для прототипирования и задач по обработке данных. Для production используйте `JSON` только для динамических подструктур, где это необходимо.
  ...
  Мы рекомендуем использовать [`elasticdump`](https://github.com/elasticsearch-dump/elasticsearch-dump) для экспорта данных из Elasticsearch. Этот инструмент требует установленного `node` и должен быть установлен на машине с минимальной сетевой задержкой до Elasticsearch и ClickHouse. Для большинства экспортов рекомендуется выделенный сервер с минимум 4 ядрами и 16 ГБ оперативной памяти.
  ...
  * Обеспечивает согласованность данных в процессе экспорта с помощью API Point-in-Time (PIT), который создаёт согласованный снимок данных в определённый момент времени.
  * Экспортирует данные непосредственно в формат JSON, который затем можно в потоковом режиме передавать в клиент ClickHouse для вставки.

  По возможности рекомендуется запускать ClickHouse, Elasticsearch и `elastic dump` в одной зоне доступности или центре обработки данных, чтобы минимизировать исходящий сетевой трафик и максимально увеличить пропускную способность.

  ### Установите клиент ClickHouse

  Убедитесь, что ClickHouse [установлен на сервере](/install), на котором расположен `elasticdump`. **Не запускайте сервер ClickHouse** — для этих шагов нужен только клиент.
  ...
  Обратите внимание на использование следующих флагов утилиты `elasticdump`:

  * `type=data` — ограничивает ответ только содержимым документа в Elasticsearch.
  * `input-index` — наш входной индекс Elasticsearch.
  * `output=$` — перенаправляет весь вывод в stdout.
  * Флаг `sourceOnly`, который исключает из ответа поля метаданных.
  * Флаг `searchAfter` для использования API [`searchAfter`](https://www.elastic.co/docs/reference/elasticsearch/rest-apis/paginate-search-results#search-after) для эффективной постраничной выборки результатов.
  * `pit=true` — чтобы обеспечить согласованные результаты между запросами при использовании [API Point-in-Time](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-open-point-in-time).

  <br />

  Параметры клиента ClickHouse (помимо учётных данных):

  * `max_insert_block_size=1000` — клиент ClickHouse отправит данные по достижении этого количества строк. Увеличение значения параметра повышает пропускную способность ценой времени на формирование блока — соответственно увеличивается задержка до появления данных в ClickHouse.
  * `min_insert_block_size_bytes=0` - Отключает объединение блоков на стороне сервера по объёму (в байтах).
  * `min_insert_block_size_rows=1000` — объединяет блоки, поступающие от клиентов, на стороне сервера. В данном случае мы устанавливаем его равным `max_insert_block_size`, чтобы строки появлялись сразу. Увеличьте этот параметр, чтобы повысить пропускную способность.
  * `query="INSERT INTO logs_system_syslog FORMAT JSONAsRow"` — вставка данных в формате [JSONEachRow](/integrations/data-formats/json/other-formats). Этот вариант подходит, если данные отправляются в таблицу с чётко определённой схемой, например `logs_system_syslog.`

  <br />

  **Вы можете рассчитывать на пропускную способность порядка тысяч строк в секунду.**

  :::note Вставка в одну строку JSON
  При вставке данных в один столбец JSON (см. схему `syslog_json` выше) можно использовать ту же команду INSERT. Однако необходимо указать формат `JSONAsObject` вместо `JSONEachRow`, например:
  ...
  Приведённые выше команды предполагают соответствие полей Elasticsearch столбцам ClickHouse в соотношении 1:1. Пользователям часто требуется фильтровать и преобразовывать данные Elasticsearch перед их вставкой в ClickHouse.

  Это можно реализовать с помощью табличной функции [`input`](/sql-reference/table-functions/input), которая позволяет выполнять любой запрос `SELECT` по данным, поступающим из stdout.

  Предположим, что мы хотим сохранять только поля `timestamp` и `hostname` из полученных ранее данных. Схема ClickHouse:
  ...
  Для вставки из `elasticdump` в эту таблицу мы можем просто использовать табличную функцию `input` с типом JSON для динамического определения и выбора необходимых столбцов. Обратите внимание, что в этот запрос `SELECT` при необходимости можно добавить фильтр.
