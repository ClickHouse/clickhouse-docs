...
3. **Упрощенная миграция**: нет необходимости в сложных инструментах или процессах передачи данных для перемещения исторических данных между системами.
   <br />
...
- Хранить только недавние, часто запрашиваемые логи на быстром диске

Рекомендуется согласовать конфигурацию TTL в ClickHouse с существующими политиками хранения в Elastic для поддержания единого жизненного цикла данных во время миграции. Примеры см. в разделе [Настройка TTL для продакшн-среды ClickStack](/use-cases/observability/clickstack/production#configure-ttl).
...
* `Tuple` используется для представления вложенных структур вместо точечной нотации.
...
**Такой подход — статическое определение схемы и выборочное использование типа JSON там, где это требуется, — [рекомендуется](/integrations/data-formats/json/schema#handling-semi-structured-dynamic-structures).**

Такая строгая схема имеет ряд преимуществ:

* **Валидация данных** – применение строгой схемы позволяет избежать риска взрывного роста количества столбцов вне отдельных специфических структур.
* **Минимизирует риск взрывного роста количества столбцов**: хотя тип JSON масштабируется до потенциально тысяч столбцов, где подстолбцы хранятся как отдельные столбцы, это может привести к «взрыву» файлов столбцов, когда создаётся чрезмерное количество файлов столбцов, что ухудшает производительность. Для смягчения этого риска базовый [тип Dynamic](/sql-reference/data-types/dynamic), используемый JSON, предоставляет параметр [`max_dynamic_paths`](/sql-reference/data-types/newjson#reading-json-paths-as-sub-columns), который ограничивает число уникальных путей, хранимых как отдельные файлы столбцов. После достижения порогового значения дополнительные пути хранятся в общем файле столбца с использованием компактного кодированного формата, что сохраняет производительность и эффективность хранения при поддержке гибкой загрузки данных. Однако доступ к этому общему файлу столбца менее производителен. Обратите внимание, что столбец JSON может использоваться с [подсказками типов](/integrations/data-formats/json/schema#using-type-hints-and-skipping-paths). Столбцы с «подсказками» обеспечивают ту же производительность, что и выделенные столбцы.
* **Упрощённое исследование путей и типов**: хотя тип JSON поддерживает [функции интроспекции](/sql-reference/data-types/newjson#introspection-functions) для определения типов и путей, которые были выведены, статические структуры могут быть проще для исследования, например, с помощью `DESCRIBE`.
...
:::note
Мы указываем тип для столбцов `host.name` и `timestamp` в определении JSON, так как используем их в ключе сортировки/первичном ключе. Это позволяет ClickHouse понять, что эти столбцы не будут содержать null, и определить, какие подстолбцы использовать (для каждого типа их может быть несколько, что иначе приводит к неоднозначности).
:::

Этот подход, хотя и проще, лучше всего подходит для прототипирования и задач инженерии данных. В продакшене используйте `JSON` только для динамических подструктур там, где это необходимо.

Для получения более подробной информации об использовании типа JSON в схемах и о том, как эффективно его применять, рекомендуем руководство ["Проектирование схемы"](/integrations/data-formats/json/schema).

### Установка `elasticdump` {#install-elasticdump}

Мы рекомендуем использовать [`elasticdump`](https://github.com/elasticsearch-dump/elasticsearch-dump) для экспорта данных из Elasticsearch. Этот инструмент требует `node` и должен быть установлен на машине с сетевой близостью как к Elasticsearch, так и к ClickHouse. Для большинства экспортов рекомендуется выделенный сервер с минимум 4 ядрами и 16 ГБ оперативной памяти.
...
- Поддерживает согласованность данных во время процесса экспорта с использованием API Point-in-Time (PIT) — это создает согласованный снимок данных в определенный момент времени.
- Экспортирует данные напрямую в формат JSON, который может быть передан в потоковом режиме клиенту ClickHouse для вставки.
...
Убедитесь, что ClickHouse [установлен на сервере](/install), на котором находится `elasticdump`. **Не запускайте сервер ClickHouse** — для этих шагов требуется только клиент.

### Потоковая передача данных {#stream-data}

Для потоковой передачи данных между Elasticsearch и ClickHouse используйте команду `elasticdump`, передавая вывод напрямую клиенту ClickHouse. Следующая команда вставляет данные в хорошо структурированную таблицу `logs_system_syslog`.
