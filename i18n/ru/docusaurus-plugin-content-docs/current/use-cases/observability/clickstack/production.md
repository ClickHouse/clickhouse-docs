...
title: 'Вывод в промышленную эксплуатацию'
sidebar_label: 'Промышленная эксплуатация'
pagination_prev: null
pagination_next: null
description: 'Вывод ClickStack в промышленную эксплуатацию'
doc_type: 'guide'
keywords: ['clickstack', 'промышленная эксплуатация', 'развертывание', 'лучшие практики', 'эксплуатация']
---
...
При развёртывании ClickStack в рабочей среде (production) следует учитывать ряд дополнительных аспектов, чтобы обеспечить безопасность, стабильность и корректную настройку.


## Безопасность сети и портов {#network-security}

По умолчанию Docker Compose открывает порты на хосте, делая их доступными извне контейнера — даже если включены такие инструменты, как `ufw` (Uncomplicated Firewall). Такое поведение связано с сетевым стеком Docker, который может обходить правила межсетевого экрана на уровне хоста, если это явно не настроено.

**Рекомендация:**

Открывайте только те порты, которые необходимы для работы в продакшене. Обычно это конечные точки OTLP, API-сервер и фронтенд.
...
Обратитесь к [документации по сети Docker](https://docs.docker.com/network/) за подробной информацией об изоляции контейнеров и повышении безопасности доступа.


## Настройка секрета сессии {#session-secret}

В продакшене необходимо установить надежное случайное значение для переменной окружения `EXPRESS_SESSION_SECRET`, чтобы защитить данные сессии и предотвратить их подделку.

Вот как добавить его в файл `docker-compose.yml` для сервиса приложения:
...
Избегайте фиксации секретов в системе контроля версий. В продакшене рекомендуется использовать инструменты управления переменными окружения (например, Docker Secrets, HashiCorp Vault или конфигурации CI/CD для конкретных окружений).


## Безопасный приём данных {#secure-ingestion}

Весь приём данных должен осуществляться через порты OTLP, предоставляемые дистрибутивом ClickStack коллектора OpenTelemetry (OTel). По умолчанию для этого требуется защищённый API-ключ приёма данных, который генерируется при запуске. Этот ключ необходим при отправке данных на порты OTel и находится в интерфейсе HyperDX в разделе `Team Settings → API Keys`.
...
Мы рекомендуем создать выделенного пользователя для сборщика OTel для приема данных в ClickHouse и обеспечить отправку данных в конкретную базу данных, например `otel`. См. раздел ["Создание пользователя для приема данных"](/use-cases/observability/clickstack/ingesting-data/otel-collector#creating-an-ingestion-user) для получения дополнительной информации.
...
Следуйте [руководству по началу работы с ClickHouse Cloud](/getting-started/quick-start/cloud/#1-create-a-clickhouse-service) для создания сервиса.
...
Опытные пользователи могут развернуть [коллектор OTel](/use-cases/observability/clickstack/ingesting-data/opentelemetry#standalone) и [HyperDX](/use-cases/observability/clickstack/deployment/hyperdx-only) раздельно с использованием соответствующих автономных режимов развертывания.
:::
...
При нажатии `Create` вам будет предложено ввести параметры подключения.
