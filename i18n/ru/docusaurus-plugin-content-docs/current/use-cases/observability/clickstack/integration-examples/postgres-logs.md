...
В этом руководстве описывается, как настроить мониторинг PostgreSQL с помощью ClickStack путём конфигурирования коллектора OpenTelemetry для приёма логов сервера PostgreSQL. Вы узнаете, как:
...
- Использовать готовый дашборд для визуализации данных из логов PostgreSQL (ошибки, медленные запросы, соединения)
...
Если вы хотите протестировать интеграцию логов PostgreSQL перед настройкой собственной установки, вы можете воспользоваться нашей предварительно настроенной конфигурацией и тестовыми данными в разделе [«Демонстрационный набор данных»](/use-cases/observability/clickstack/integrations/postgresql-logs#demo-dataset).
...
#log&#95;min&#95;duration&#95;statement = 1000  # Логировать запросы, выполняющиеся более 1 секунды
...
ClickStack позволяет расширить базовую конфигурацию OpenTelemetry Collector, примонтировав пользовательский конфигурационный файл и задав переменную окружения. Пользовательская конфигурация объединяется с базовой конфигурацией, управляемой HyperDX через OpAMP.
...
receivers:
  filelog/postgres:
    include:
      - /var/lib/postgresql/*/main/log/postgresql-*.csv # Измените путь в соответствии с вашей установкой PostgreSQL
    start_at: end
    multiline:
...
- Читает CSV‑логи PostgreSQL из их стандартного расположения
...
- Извлекает временные метки, чтобы сохранить исходное время логирования
- Добавляет атрибут `source: postgresql` для фильтрации в HyperDX
- Направляет логи в экспортёр ClickHouse через выделенный конвейер (pipeline)
...
- Измените путь в директиве `include` так, чтобы он соответствовал расположению каталога с логами PostgreSQL
  :::
...
1. Примонтировать пользовательский конфигурационный файл по пути `/etc/otelcol-contrib/custom.config.yaml`
2. Установить переменную окружения `CUSTOM_OTELCOL_CONFIG_FILE=/etc/otelcol-contrib/custom.config.yaml`
3. Примонтировать каталог логов PostgreSQL, чтобы коллектор мог их читать
...
:::note
Убедитесь, что коллектор ClickStack имеет соответствующие права для чтения файлов логов PostgreSQL. В продуктивной среде используйте монтирование только для чтения (`:ro`) и придерживайтесь принципа наименьших привилегий.
:::
...
Для пользователей, которые хотят протестировать интеграцию логов PostgreSQL перед настройкой производственных систем, мы предоставляем образец набора данных с предварительно сгенерированными логами PostgreSQL с реалистичными паттернами.
...
Убедитесь, что действующая конфигурация включает ваш filelog receiver:
