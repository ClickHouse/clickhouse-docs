...
В этом руководстве описывается, как настроить мониторинг Nginx с помощью ClickStack, сконфигурировав коллектор OpenTelemetry для приёма логов доступа Nginx. Вы узнаете, как:
...
Доступен демонстрационный набор данных с примерами логов, если вы хотите протестировать интеграцию перед настройкой продакшн-окружения Nginx.
...
В этом разделе описывается настройка существующей установки Nginx для отправки логов в ClickStack путём изменения конфигурации OTel-коллектора ClickStack.
...
#### Создание пользовательской конфигурации OTel-коллектора {#custom-otel}
...
3. Смонтировать каталоги логов Nginx, чтобы коллектор мог их читать
...
:::note
Убедитесь, что коллектор ClickStack имеет необходимые права для чтения файлов журналов nginx. В рабочей среде используйте примонтированные тома только для чтения (:ro) и придерживайтесь принципа наименьших привилегий.
:::
...
1. Перейдите в интерфейс поиска.
2. Установите источник в Logs и убедитесь, что вы видите записи журнала с такими полями, как request, request_time, upstream_response_time и т. д.

Ниже приведён пример того, что вы должны увидеть:
...
- Разные эндпоинты и HTTP‑методы
- Сочетание успешных запросов и ошибок
- Реалистичное время отклика и объёмы ответов в байтах
...
HyperDX отображает временные метки в локальном часовом поясе браузера. Демонстрационные данные охватывают период с 2025-10-20 11:00:00 по 2025-10-21 11:00:00 UTC. Широкий диапазон времени гарантирует, что вы увидите демо‑логи независимо от своего местоположения. После того как вы увидите логи, можно сузить диапазон до 24 часов для более наглядной визуализации.
:::
...
Чтобы помочь вам начать мониторинг nginx с помощью ClickStack, мы предоставляем необходимые визуализации для логов Nginx.

<VerticalStepper headerLevel="h4">

#### <TrackedLink href={useBaseUrl('/examples/nginx-logs-dashboard.json')} download="nginx-logs-dashboard.json" eventName="docs.nginx_logs_monitoring.dashboard_download">Скачайте</TrackedLink> конфигурацию дашборда {#download}
...
:::note
Для демонстрационного набора данных установите временной диапазон **2025-10-20 11:00:00 - 2025-10-21 11:00:00 (UTC)** (скорректируйте в соответствии с вашим часовым поясом). По умолчанию импортированный дашборд не будет иметь указанного временного диапазона.
:::
