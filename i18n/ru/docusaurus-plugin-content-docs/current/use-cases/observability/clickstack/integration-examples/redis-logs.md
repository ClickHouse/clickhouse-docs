...
В этом руководстве описывается, как настроить мониторинг Redis с помощью ClickStack, сконфигурировав коллектор OpenTelemetry для приёма логов сервера Redis. Вы узнаете, как:
...
Если вы хотите протестировать интеграцию с Redis перед настройкой собственной установки, вы можете воспользоваться нашей предварительно настроенной конфигурацией и демонстрационными данными в разделе [«Демонстрационный набор данных»](/use-cases/observability/clickstack/integrations/redis#demo-dataset).
...
# Задать уровень журналирования (варианты: debug, verbose, notice, warning)
...
:::note
Убедитесь, что коллектор ClickStack имеет соответствующие разрешения для чтения файлов логов Redis. В production-среде используйте монтирование только для чтения (`:ro`) и следуйте принципу наименьших привилегий.
:::
...
Для пользователей, которые хотят протестировать интеграцию с Redis перед настройкой производственных систем, мы предоставляем демонстрационный набор данных с предварительно сгенерированными журналами Redis с реалистичными шаблонами.
...
Чтобы помочь вам начать мониторинг Redis с помощью ClickStack, мы предоставляем необходимые визуализации для журналов Redis.

<VerticalStepper headerLevel="h4">

#### <TrackedLink href={useBaseUrl('/examples/redis-logs-dashboard.json')} download="redis-logs-dashboard.json" eventName="docs.redis_logs_monitoring.dashboard_download">Скачайте</TrackedLink> конфигурацию панели мониторинга {#download}

#### Импорт готовой панели мониторинга {#import-dashboard}
...
:::note
Для демонстрационного набора данных установите временной диапазон **2025-10-27 10:00:00 - 2025-10-28 10:00:00 (UTC)** (скорректируйте в соответствии с вашим часовым поясом). Импортированная панель мониторинга по умолчанию не будет иметь указанного временного диапазона.
:::
...
**Просмотрите содержимое пользовательского файла конфигурации:**
...
**Убедитесь, что Redis активно пишет логи:**
...
**Убедитесь, что сборщик может читать журналы:**

```bash
docker exec <container> cat /var/log/redis/redis-server.log
# Должны отобразиться записи журнала Redis
```
...
**Убедитесь, что формат журнала Redis соответствует ожидаемому формату:**
...
Если формат логов Redis отличается, возможно, вам потребуется скорректировать шаблон регулярного выражения в операторе `regex_parser`. Стандартный формат следующий:
...
Если вы хотите продолжить изучение, вот несколько следующих шагов для экспериментов с вашей панелью мониторинга:
...
- Создайте дополнительные [панели мониторинга](/use-cases/observability/clickstack/dashboards) для конкретных сценариев использования (мониторинг API, события безопасности)
