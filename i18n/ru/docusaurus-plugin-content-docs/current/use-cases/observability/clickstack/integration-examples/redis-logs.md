...
В этом руководстве показано, как настроить мониторинг Redis с помощью ClickStack, сконфигурировав коллектор OpenTelemetry для приёма логов сервера Redis. Вы узнаете, как:
...
Демонстрационный набор данных с примерами логов доступен, если вы хотите протестировать интеграцию перед настройкой вашего production-окружения Redis.
...
Если вы хотите протестировать интеграцию с Redis перед настройкой собственной установки, можете воспользоваться нашей предварительно настроенной конфигурацией и демонстрационными данными в разделе [«Демонстрационный набор данных»](/use-cases/observability/clickstack/integrations/redis#demo-dataset).
...
- Оператор `time_parser` извлекает временные метки из логов Redis для сохранения исходного времени записи
- Эта конфигурация использует `start_at: beginning` для чтения всех существующих логов при запуске сборщика, что позволяет сразу увидеть логи. Для производственных развёртываний, где вы хотите избежать повторной обработки логов при перезапуске сборщика, измените на `start_at: end`.
  :::
...
3. Смонтировать каталог логов Redis, чтобы сборщик мог их читать
...
:::note
Убедитесь, что сборщик ClickStack имеет соответствующие разрешения для чтения файлов логов Redis. В производственной среде используйте монтирование только для чтения (`:ro`) и следуйте принципу минимальных привилегий.
:::
...
1. Откройте [HyperDX](http://localhost:8080/) и войдите в свой аккаунт (возможно, сначала потребуется создать аккаунт)
...
**Просмотр содержимого пользовательского конфига:**
...
**Убедитесь, что сборщик может читать логи:**
...
**Проверьте, что формат журнала Redis соответствует ожидаемому шаблону:**
...
Если журналы Redis имеют другой формат, возможно, вам потребуется скорректировать шаблон регулярного выражения в операторе `regex_parser`. Стандартный формат:
