...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: оставьте поле пустым
  ...
  С помощью функции кластеризации HyperDX вы можете автоматически выявлять ошибки и группировать их в значимые шаблоны. Это ускоряет анализ при работе с большими объёмами логов и трейсов. Чтобы воспользоваться этой функцией, выберите `Event Patterns` в меню `Analysis Mode` на левой панели.
  ...
  Выберите любую из получившихся ошибок. Метаданные журналов будут показаны подробно. Прокрутка как раздела `Overview`, так и `Column Values` указывает на проблему с платёжными картами из‑за кэша:
  ...
  Мы выявили ошибку, связанную с кэшем, которая, вероятно, приводит к сбоям платежей. Необходимо определить, в каком компоненте микросервисной архитектуры возникает данная проблема.
  ...
  Если к трейсам применить кластеризацию событий, выбрав `Event Patterns`, можно сразу обнаружить проблему с кэшем в сервисе `payment`.

  <Image img={step_12} alt="Шаг 12" size="lg" />

  ### Изучение инфраструктуры для трейса
  ...
  Сопоставляя трейсы с метриками, мы видим, что потребление памяти и CPU увеличилось у сервиса `payment`, после чего упало до `0` (это можно объяснить перезапуском пода) — что указывает на то, что проблема с кэшем вызвала дефицит ресурсов. Можно ожидать, что это повлияло на время завершения платежей.

  ### Event deltas для более быстрого устранения

  Event Deltas помогают обнаруживать аномалии, связывая изменения производительности или частоты ошибок с конкретными подмножествами данных — что позволяет быстрее точно определить первопричину.
  ...
  Если выбрать события с длительностью более `1ms` и применить фильтр `Filter by selection`, можно проанализировать различия между «обычными» событиями и группой высокой плотности спанов с длительностью ~0ms:

  <Image img={step_17} alt="Шаг 17" size="lg" />

  Проведя анализ этого подмножества данных, мы видим, что «фоновые» спаны за пределами выборки — это преимущественно транзакции Visa, для которых фиксируется время ответа 0 мс из-за ошибок кэша.
  ...
  * Проблема не позволяла завершить платежи по картам Visa — или, по крайней мере, существенно замедляла их обработку.
  ...
  * `Group By`: `<пусто>`
  ...
  Итак, последовательно проанализировав логи, трейсы и, наконец, метрики, мы пришли к следующим выводам:

  * Проблема на стороне платёжного сервиса
  * Изменение в поведении сервиса, вероятно вызванное развертыванием, привело к медленному росту кэша Visa в течение 4–5 часов, пока его размер не достиг максимального значения `100 000`.
  * Это привело к росту потребления ресурсов по мере увеличения размера кэша — вероятно, из‑за неудачной реализации
  ...
  * После достижения максимального размера кэш начал отклонять платежи и сообщал, что его размер равен `0`.
  ...
  Например, если команда поддержки предоставляет адрес электронной почты пользователя, столкнувшегося с проблемой оплаты `Ronny.Windler@gmail.com` — зачастую эффективнее начать с анализа его сеанса, а не с прямого поиска по логам или трассировкам.
  ...
  Выполните поиск по `SpanAttributes.userEmail: Ronny.Windler`, чтобы найти сеанс нашего клиента. При выборе сеанса слева отобразятся события браузера и связанные спаны для этого сеанса, а справа — повторно воспроизведённый опыт пользователя в браузере:
