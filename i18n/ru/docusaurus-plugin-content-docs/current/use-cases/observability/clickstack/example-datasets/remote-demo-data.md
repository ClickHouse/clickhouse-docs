...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: оставьте пустым
  ...
  <Image img={edit_demo_source} alt="Изменить демо-источник" size="lg" />
  ...
  Нажмите на наиболее очевидный кластер ошибок, который коррелирует с зарегистрированной проблемой невозможности пользователей завершить платежи: `Failed to place order`.
  ...
  * Кэш переполнен
  * Это приводило к росту потребления ресурсов
  * Проблема мешала завершению платежей по картам Visa — или, по крайней мере, значительно увеличивала время их завершения.
  ...
  * `Интервал времени`: `Последние 24 часа`
  ...
  Нажатие `▶️` покажет, как производительность платежей снижалась с течением времени.
  ...
  * `Метрика`: `Maximum`
  * `SQL Column`: `visa_validation_cache.size (gauge)` (просто начните вводить `cache`, чтобы сработало автодополнение)
  * `Где`: `ServiceName: payment`
  * `Group By`: `<пусто>`
  ...
  * Изменение поведения сервиса, вероятно, из‑за развертывания, привело к медленному росту кэша Visa в течение 4–5 часов, пока его размер не достиг максимального значения `100 000`.
