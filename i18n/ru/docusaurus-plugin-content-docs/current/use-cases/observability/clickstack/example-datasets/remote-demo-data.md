...
<VerticalStepper headerLevel="h3">
  ...
  * `Password`: оставьте поле пустым
  ...
  Установите в селекторе времени в правом верхнем углу диапазон `2 дня`, чтобы отобразить все данные за последние двое суток.
  ...
  * Проблема не позволяла завершить платежи по картам Visa — или, по крайней мере, из‑за неё их выполнение занимало очень много времени.
  ...
  * После достижения максимального размера кэш начал отклонять платежи и сообщал, что его размер равен `0`.
  ...
  Например, если команда поддержки предоставляет адрес электронной почты пользователя, столкнувшегося с проблемой оплаты `Eddie86@yahoo.com` — зачастую эффективнее начать с анализа его сеанса, а не с прямого поиска по логам или трассировкам.
  ...
  Выполните поиск по `SpanAttributes.userEmail: Eddie`, чтобы найти сеанс нашего клиента. При выборе сеанса слева отобразятся события браузера и связанные спаны для этого сеанса, а справа — повторно воспроизведённый опыт пользователя в браузере:
