...
doc_type: 'guide'
keywords: ['ClickStack', 'getting started', 'развертывание в Docker', 'интерфейс HyperDX', 'ClickHouse Cloud', 'локальное развертывание']
---
...
Благодаря готовым Docker-образам начать работу с **ClickStack** очень просто. Эти образы основаны на официальном Debian-пакете ClickHouse и доступны в нескольких вариантах, подходящих для разных сценариев использования.
...
Чтобы данные и настройки сохранялись при перезапуске контейнера, можно изменить приведенную выше команду docker, добавив монтирование путей `/data/db`, `/var/lib/clickhouse` и `/var/log/clickhouse-server`.
...
HyperDX автоматически подключится к локальному кластеру и создаст источники данных для логов, трассировок, метрик и сессий, что позволит вам сразу начать работу с продуктом.
...
- [Локальные файлы и метрики](/use-cases/observability/clickstack/getting-started/local-data) — загрузите локальные файлы и мониторьте систему на OSX или Linux с помощью локального коллектора OTel.

<br />
Также вы можете подключиться к демонстрационному кластеру для работы с более крупным набором данных:
...
Локальный режим позволяет развернуть HyperDX без необходимости аутентификации.
...
### Размещённая версия {#hosted-version}

Вы можете использовать размещённую версию HyperDX в локальном режиме, доступную по адресу [play.hyperdx.io](https://play.hyperdx.io).

### Самостоятельное развёртывание {#self-hosted-version}

<VerticalStepper headerLevel="h3">

### Запуск с Docker {#run-local-with-docker}

Образ для самостоятельного развёртывания в локальном режиме поставляется с предварительно настроенными OpenTelemetry Collector и сервером ClickHouse. Это упрощает сбор телеметрических данных из ваших приложений и их визуализацию в HyperDX с минимальной внешней настройкой. Чтобы начать работу с самостоятельно развёрнутой версией, просто запустите Docker-контейнер с проброшенными соответствующими портами:
...
### Ввод учётных данных подключения {#complete-connection-credentials}

Чтобы подключиться к вашему собственному **внешнему кластеру ClickHouse**, вы можете вручную ввести учётные данные подключения.

Альтернативно, для быстрого ознакомления с продуктом вы также можете нажать **Connect to Demo Server**, чтобы получить доступ к предзагруженным наборам данных и попробовать ClickStack без необходимости настройки.
