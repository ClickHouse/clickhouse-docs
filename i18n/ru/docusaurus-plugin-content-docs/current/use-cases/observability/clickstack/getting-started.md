...
Начать работу с **ClickStack** просто благодаря готовым Docker-образам. Эти образы основаны на официальном Debian-пакете ClickHouse и доступны в нескольких вариантах для разных сценариев использования.
...
Следующая команда запустит коллектор OpenTelemetry (на портах 4317 и 4318) и HyperDX UI (на порту 8080).
...
Чтобы сохранить данные и настройки при перезапуске контейнера, можно изменить приведенную выше команду docker, добавив монтирование путей `/data/db`, `/var/lib/clickhouse` и `/var/log/clickhouse-server`.
...
<br />
Также вы можете подключиться к демонстрационному кластеру для работы с более крупным
набором данных:
...
Пользователи могут развернуть ClickStack с ClickHouse Cloud, получая преимущества полностью управляемого и безопасного бэкенда при сохранении полного контроля над процессами приема данных, схемой и рабочими процессами наблюдаемости.
...
### Самостоятельное размещение {#self-hosted-version}

<VerticalStepper headerLevel="h3">

### Запуск с Docker {#run-local-with-docker}

Образ для самостоятельного размещения в локальном режиме поставляется с предварительно настроенными OpenTelemetry collector и сервером ClickHouse. Это упрощает сбор телеметрических данных из ваших приложений и их визуализацию в HyperDX с минимальной внешней настройкой. Чтобы начать работу с самостоятельно размещаемой версией, просто запустите Docker-контейнер с проброшенными соответствующими портами:
