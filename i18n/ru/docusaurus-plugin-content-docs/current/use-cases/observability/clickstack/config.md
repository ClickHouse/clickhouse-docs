...
Альтернативно можно явно переопределить настройки в файле [`docker-compose.yaml`](https://github.com/hyperdxio/hyperdx/blob/main/docker-compose.yml), например:
...
Для каждого из этих источников необходимо указать как минимум одну таблицу при создании, а также набор столбцов, которые позволяют HyperDX запрашивать данные.
...
| `Timestamp Column`               | Столбец или выражение типа datetime, являющееся частью первичного ключа.                                                                           | Да       | Да                         | `Timestamp`                                                                                           |
...
Эти настройки нельзя изменить, когда HyperDX управляется в ClickHouse Cloud.
:::
...
- `HYPERDX_APP_PORT`
  - **Default:** `8000`
  - **Description:** Порт frontend‑приложения HyperDX.
  - **Guidance:**
  - Убедитесь, что этот порт свободен на вашем хосте
  - Измените, если есть конфликты портов
...
- `MINER_API_URL`
  ...
  - **Description:** URL сервиса поиска паттернов в логах.
  - **Guidance:**
  - Используйте значение по умолчанию для локальной разработки с Docker
...
- `OTEL_SERVICE_NAME`
  ...
  - **Guidance:**
  - Используйте информативное имя для вашего сервиса HyperDX. Применимо, если HyperDX самоинструментируется.
...
- `USAGE_STATS_ENABLED`
  - **Default:** `true`
  - **Description:** Включает или отключает сбор статистики использования.
  - **Guidance:**
  - Установите `false`, чтобы отключить сбор статистики использования
  - Полезно для конфиденциальных развёртываний
...
- `IS_OSS`
  ...
  - **Guidance:**
  - Оставьте `true` для open‑source‑развёртываний
...
- `IS_LOCAL_MODE`
  ...
  - Отключает некоторые продакшен‑функции
...
- `ENABLE_SWAGGER`
  - **Default:** `false`
  - **Description:** Включает или отключает документацию Swagger API.
  - **Guidance:**
...
- `BETA_CH_OTEL_JSON_SCHEMA_ENABLED`
  - **Default:** `false`
  - **Description:** Включает beta‑поддержку типа JSON в HyperDX. См. также [`OTEL_AGENT_FEATURE_GATE_ARG`](#otel-collector) для включения поддержки JSON в OTel‑коллекторе.
  - **Guidance:**
...
Подробнее см. в разделе ["Коллектор OpenTelemetry ClickStack"](/use-cases/observability/clickstack/ingesting-data/otel-collector).

- `CLICKHOUSE_ENDPOINT`
  - **По умолчанию:** _Не задано (обязательный параметр)_ для автономного образа. Для дистрибутивов All-in-one или Docker Compose указывает на интегрированный экземпляр ClickHouse.
...
- `CLICKHOUSE_PASSWORD`
  - **По умолчанию:** _Не задано (обязательный параметр, если включена аутентификация)_
  - **Описание:** Пароль для указанного пользователя ClickHouse.
  - **Рекомендации:**
    - Обязателен, если для учётной записи пользователя установлен пароль
    - Храните безопасно через механизм секретов в производственных развёртываниях
...
- `OPAMP_SERVER_URL`
  - **По умолчанию:** _Не задано (обязательный параметр)_ для автономного образа. Для дистрибутивов All-in-one или Docker Compose указывает на развёрнутый экземпляр HyperDX.
...
Для эффективной настройки ClickHouse пользователям необходимо понимать ключевые концепции хранения данных, такие как [куски](/parts), [партиции](/partitions), [шарды и реплики](/shards), а также механизм [слияния](/merges) при вставке данных. Мы рекомендуем изучить основы [первичных индексов](/primary-indexes), [разреженных вторичных индексов](/optimize/skipping-indexes) и индексов пропуска данных, а также методы [управления жизненным циклом данных](/observability/managing-data), например, с использованием TTL.
