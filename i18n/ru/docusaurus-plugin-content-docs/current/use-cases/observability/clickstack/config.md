...
HyperDX требует от пользователя определить источник для каждого из типов данных наблюдаемости:
...
Для каждого из этих источников при создании необходимо указать как минимум одну таблицу, а также набор столбцов, которые позволяют HyperDX запрашивать данные.
...
| `Span Events Expression`         | Выражение для извлечения событий спана. Обычно это столбец типа `Nested`. Позволяет отображать трассировки стека исключений с поддерживаемыми SDK. | Да      | Да                        | `Events`                                                                                              |
...
| `Sum Table`             | Таблица для хранения метрик типа sum (счётчик).  | Да           | Нет                            | `otel_metrics_sum`       |
...
| `Timestamp Column`               | Столбец или выражение типа datetime, являющееся частью первичного ключа.                                 | Да           | Да                             | `TimestampTime`      |
...
| `Implicit Column Expression`     | Столбец для полнотекстового поиска, когда поле не указано (например, при разборе запросов в стиле Lucene). | Да           | Да                             | `Body`               |

### Коррелированные источники {#correlated-sources}

Для включения полной межисточниковой корреляции в ClickStack необходимо настроить коррелированные источники для логов, трассировок, метрик и сессий. Это позволяет HyperDX связывать связанные данные и предоставлять расширенный контекст при отображении событий.
...
Настроив эти корреляции, HyperDX может, например, отображать соответствующие логи вместе с трассировкой или выявлять аномалии метрик, связанные с сессией. Правильная конфигурация обеспечивает единый и контекстуальный опыт наблюдаемости.
...
- `HYPERDX_API_PORT`
  ...
  - Должен совпадать с портом в конфигурациях вашего API‑клиента

- `HYPERDX_APP_PORT`
  - **Default:** `8000`
  - **Description:** Порт для frontend‑приложения HyperDX.
  - **Guidance:**
  - Убедитесь, что этот порт доступен на вашем хосте
  - Измените, если есть конфликт портов
...
- `HYPERDX_APP_URL`
  ...
  - **Description:** Базовый URL для frontend‑приложения.
  - **Guidance:**
...
- `MINER_API_URL`
  ...
  - **Description:** URL сервиса майнинга шаблонов логов.
  - **Guidance:**
  - Используйте значение по умолчанию для локальной разработки с Docker
  - В продакшене укажите URL вашего miner‑сервиса
  - Должен быть доступен из API‑сервиса

- `FRONTEND_URL`
  - **Default:** `http://localhost:3000`
  - **Description:** URL frontend‑приложения.
  - **Guidance:**
...
- `OTEL_SERVICE_NAME`
  ...
  - **Guidance:**
  - Используйте описательное имя для вашего сервиса HyperDX. Применимо, если HyperDX сам себя инструментирует.
...
- `NEXT_PUBLIC_OTEL_EXPORTER_OTLP_ENDPOINT`
  ...
  - **Guidance:**
  - Актуален при самостоятельном инструментировании HyperDX
  - Используйте значение по умолчанию для локальной разработки
...
- `IS_OSS`
  - **Default:** `true`
  - **Description:** Указывает, запущен ли в режиме OSS.
  - **Guidance:**
  - Оставьте `true` для open‑source‑развертываний
  - Установите `false` для корпоративных развертываний
  - Влияет на доступность функциональности

- `IS_LOCAL_MODE`
  - **Default:** `false`
  - **Description:** Указывает, запущен ли в локальном режиме.
  - **Guidance:**
  - Установите `true` для локальной разработки
  - Отключает некоторые функции, предназначенные для продакшена
...
- `EXPRESS_SESSION_SECRET`
  ...
  - Используйте стойкую, случайную строку
