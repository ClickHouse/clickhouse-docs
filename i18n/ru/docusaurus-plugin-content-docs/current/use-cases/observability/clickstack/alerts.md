...
Система оповещений работает со всеми данными ClickStack, помогая отслеживать состояние системы, выявлять снижение производительности и мониторить ключевые бизнес-события.
...
- Установить пороговое значение и указать, сколько раз оно должно быть достигнуто в течение заданного периода. Пороговые значения также могут использоваться как верхние или нижние границы. Период также определяет, как часто будет срабатывать оповещение.
...
Обратите внимание, что к поиску можно добавить несколько оповещений. При повторении описанного выше процесса пользователи увидят текущие оповещения в виде вкладок в верхней части диалогового окна редактирования оповещения, каждому оповещению будет присвоен номер.
...
Когда метрика пересекает заданное пороговое значение, оповещение срабатывает автоматически, позволяя отслеживать KPI, задержки или другие ключевые метрики с течением времени.
...
Откройте меню конфигурации графика и выберите кнопку оповещения. Откроется диалоговое окно редактирования графика.
...
Определите условие (`>=`, `<`), пороговое значение, длительность и webhook. Длительность также определяет, как часто будет срабатывать оповещение.
...
Имя вебхука обязательно, описание — опционально. Другие настройки, которые необходимо заполнить, зависят от типа сервиса.
...
В зависимости от типа сервиса необходимо указать различные данные. А именно:
...
- Тело вебхука (опционально). В настоящее время тело поддерживает переменные шаблона `{{title}}`, `{{body}}` и `{{link}}`.
...
`duration:>5000`) и затем создать оповещение при возникновении слишком большого количества медленных операций.

**Пользовательские события:** Также можно настроить оповещения для команд, работающих с клиентами, чтобы получать уведомления при регистрации новых пользователей или выполнении критически важных действий.
