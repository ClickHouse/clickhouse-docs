...
ClickStack включает встроенную поддержку оповещений, позволяя командам обнаруживать и реагировать на проблемы в режиме реального времени на основе логов, метрик и трассировок.
...
Система оповещений работает со всеми данными ClickStack, помогая отслеживать работоспособность системы, выявлять снижение производительности и мониторить ключевые бизнес-события.


## Типы оповещений {#types-of-alerts}

ClickStack поддерживает два взаимодополняющих способа создания оповещений: **оповещения по поиску** и **оповещения по графикам дашборда**. После создания оповещение привязывается либо к поиску, либо к графику.

### 1. Оповещения по поиску {#search-alerts}

Оповещения по поиску позволяют отправлять уведомления на основе результатов сохраненного поиска. Они помогают обнаруживать случаи, когда определенные события или паттерны происходят чаще (или реже), чем ожидалось.
...
Для создания оповещения по поиску поиск должен быть сохранен. Пользователи могут создать оповещение для существующего сохраненного поиска или сохранить поиск в процессе создания оповещения. В примере ниже предполагается, что поиск не сохранен.

#### Откройте диалог создания оповещения {#open-dialog}

Начните с ввода [поискового запроса](/use-cases/observability/clickstack/search) и нажмите кнопку `Alerts` в правом верхнем углу страницы `Search`.
...
- Установить порог и указать, сколько раз он должен быть достигнут в течение заданного периода. Пороги также могут использоваться как верхние или нижние границы. Период также определяет, как часто будет срабатывать оповещение.
...
Перед сохранением ClickStack визуализирует условие порога, чтобы вы могли убедиться, что оно будет работать как ожидается.

<Image img={search_alert} alt='Search alerts' size='lg' />

</VerticalStepper>

Обратите внимание, что к поиску можно добавить несколько оповещений. При повторении описанного выше процесса пользователи увидят текущие оповещения в виде вкладок в верхней части диалога редактирования оповещения, каждому оповещению будет присвоен номер.
...
Вы можете создать оповещение на основе графика непосредственно из сохраненного дашборда, используя полные SQL-агрегации и функции ClickHouse для сложных вычислений.

Когда метрика пересекает заданный порог, оповещение срабатывает автоматически, позволяя отслеживать KPI, задержки или другие ключевые метрики с течением времени.
...
Откройте меню конфигурации графика и выберите кнопку оповещения. Откроется диалог редактирования графика.
...
Определите условие (`>=`, `<`), порог, длительность и webhook. Длительность также определяет, как часто будет срабатывать оповещение.
...
При создании оповещения можно использовать существующий вебхук или создать новый. После создания вебхук станет доступен для повторного использования в других оповещениях.

Вебхук можно создать для различных типов сервисов, включая Slack и PagerDuty, а также для произвольных целей.

Например, рассмотрим создание оповещения для графика ниже. Перед указанием вебхука можно выбрать `Add New Webhook`.

<Image img={add_new_webhook} alt='Добавить новый вебхук' size='lg' />

Откроется диалоговое окно создания вебхука, в котором можно создать новый вебхук:

<Image img={add_webhook_dialog} alt='Создание вебхука' size='md' />

Имя вебхука является обязательным, описание — опциональным. Остальные настройки, которые необходимо заполнить, зависят от типа сервиса.

Обратите внимание, что в ClickStack Open Source и ClickStack Cloud доступны разные типы сервисов. См. раздел [Интеграции типов сервисов](#integrations).

### Интеграции типов сервисов {#integrations}

Оповещения ClickStack поддерживают готовые интеграции со следующими типами сервисов:
...
- **Webhook**: подключение оповещений к любой пользовательской системе или рабочему процессу через произвольный вебхук.

:::note Интеграции только для ClickHouse Cloud
Интеграции Slack API и PagerDuty поддерживаются только в ClickHouse Cloud.
:::

В зависимости от типа сервиса потребуется указать различные данные. А именно:
...
- URL вебхука
- Заголовки вебхука (опционально)
- Тело вебхука (опционально). В теле поддерживаются переменные шаблона `{{title}}`, `{{body}}` и `{{link}}`.
...
Это представление также отображает историю оценки оповещений. Оповещения оцениваются через регулярные временные интервалы (определяемые периодом/длительностью, заданными при создании оповещения). Во время каждой оценки HyperDX запрашивает ваши данные, чтобы проверить, выполнено ли условие оповещения:

- **Красная полоса**: пороговое условие было выполнено во время этой оценки, и оповещение сработало (уведомление отправлено)
- **Зелёная полоса**: оповещение было оценено, но пороговое условие не было выполнено (уведомление не отправлено)

Каждая оценка независима — оповещение проверяет данные для этого временного окна и срабатывает только в том случае, если условие истинно в данный момент.

В приведённом выше примере первое оповещение срабатывало при каждой оценке, что указывает на постоянную проблему. Второе оповещение показывает решённую проблему — оно сработало дважды изначально (красные полосы), затем при последующих оценках пороговое условие больше не выполнялось (зелёные полосы).
...
Ниже приведены несколько типичных сценариев оповещений, для которых можно использовать HyperDX:

**Ошибки:** Рекомендуется настроить оповещения для сохранённых поисков по умолчанию
`All Error Events` и `HTTP Status >= 400`, чтобы получать уведомления при
превышении допустимого количества ошибок.

**Медленные операции:** Можно настроить поиск медленных операций (например,
`duration:>5000`) и затем создать оповещение при обнаружении слишком большого количества медленных операций.

**События пользователей:** Также можно настроить оповещения для команд, работающих с клиентами, чтобы получать уведомления при регистрации новых пользователей или выполнении критически важных действий.
