...
description: 'Поиск с помощью ClickStack'
doc_type: 'guide'
...
ClickStack позволяет выполнять полнотекстовый поиск по вашим событиям (логам и трейсам). Вы можете начать поиск, просто вводя ключевые слова, которые соответствуют вашим событиям. Например, если ваш лог содержит «Error», вы можете найти его, просто введя «Error» в строку поиска.
...
- По умолчанию поиск выполняется по целым словам (например, `Error` найдёт `Error here`,
  но не `Errors here`). Можно окружить слово подстановочными символами для поиска частичных
...
- Можно искать по столбцам и свойствам JSON/map, используя `column:value` (например, `level:Error`,
  `service:app`)
...
- Ввод времени принимает выражения на естественном языке (например, `1 hour ago`, `yesterday`,
  `last week`)
- Указание одной временной точки приведёт к поиску от этой точки
  до текущего момента
- Временной диапазон всегда будет преобразован в распознанный временной диапазон при поиске для
...
Чтобы указать столбцы для отображения в результатах поиска, можно использовать поле `SELECT`.
