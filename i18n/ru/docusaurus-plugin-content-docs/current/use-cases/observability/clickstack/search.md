...
ClickStack позволяет выполнять полнотекстовый поиск по вашим событиям (логам и трассировкам). Можно сразу приступить к поиску, просто набирая ключевые слова, которые встречаются в ваших событиях. Например, если ваш лог содержит «Error», вы можете найти его, просто введя «Error» в строку поиска.
...
- Можно исключить ключевые слова с помощью `NOT` или `-` (например, `Error NOT Exception` или
  `Error -Exception`)
...
- Можно искать по столбцам и свойствам JSON/map, используя синтаксис `column:value` (например, `level:Error`,
  `service:app`)
- Можно искать диапазон значений с помощью операторов сравнения (`>`, `<`,
  `>=`, `<=`) (например, `Duration:>1000`)
- Можно проверить наличие свойства с помощью `property:*` (например,
  `duration:*`)
...
- Временной диапазон всегда преобразуется в распознанный диапазон при выполнении поиска для
  упрощения отладки временных запросов
- Также можно выделить столбец гистограммы для увеличения конкретного временного диапазона

### Синтаксис SQL-поиска {#sql-syntax}

Можно переключить поля поиска в режим SQL. В этом режиме принимается любое допустимое
...
Сохранённые поиски можно отслеживать с помощью оповещений, которые уведомят вас при выполнении определённых условий. Вы можете настроить оповещения для срабатывания, когда количество событий, соответствующих вашему сохранённому поиску, превышает или опускается ниже заданного порогового значения.

Дополнительную информацию о настройке и конфигурировании оповещений см. в [документации по оповещениям](/use-cases/observability/clickstack/alerts).
