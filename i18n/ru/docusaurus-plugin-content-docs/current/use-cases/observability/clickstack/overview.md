...
**ClickStack** — это готовая к продакшну платформа наблюдаемости, построенная на ClickHouse и объединяющая логи, трейсы, метрики и сессии в одном высокопроизводительном решении. Предназначенная для мониторинга и отладки сложных систем, ClickStack позволяет разработчикам и SRE выполнять сквозное трассирование проблем без переключения между инструментами и без ручного объединения данных по временным меткам или correlation ID.

В основе ClickStack лежит простая, но мощная идея: все данные наблюдаемости должны поступать в виде «широких», насыщенных событий. Эти события хранятся в таблицах ClickHouse по типам данных — логи, трейсы, метрики и сессии, — но при этом остаются полностью доступными для запросов и перекрёстной корреляции на уровне базы данных.

ClickStack создан для эффективной обработки нагрузок с высокой кардинальностью, используя колоночную архитектуру ClickHouse, нативную поддержку JSON и полностью параллелизованный движок выполнения. Это обеспечивает субсекундные запросы по огромным наборам данных, быстрые агрегации на больших временных интервалах и детальный анализ отдельных трейсов. JSON хранится в сжатом колоночном формате, что позволяет эволюцию схемы без ручного вмешательства или предварительного определения структур.
...
- Нативная работа с JSON-строками
...
3. **ClickHouse** – высокопроизводительная аналитическая база данных, лежащая в основе стека

Эти компоненты могут быть развёрнуты независимо друг от друга или совместно. Также доступна браузерная версия HyperDX UI, позволяющая пользователям подключаться к существующим развёртываниям ClickHouse без дополнительной инфраструктуры.
...
ClickStack работает из коробки с любым экземпляром и схемой ClickHouse, требуя минимальной настройки. Независимо от того, начинаете ли вы с нуля или интегрируетесь с существующей установкой, вы можете запустить систему за считанные минуты.
...
ClickStack обеспечивает полную видимость стека — от пользовательских сессий на фронтенде до метрик инфраструктуры бэкенда, журналов приложений и распределенных трассировок. Это единое представление позволяет проводить глубокую корреляцию и анализ по всей системе.

### Создан для ClickHouse {#built-for-clickhouse}

Каждый уровень стека спроектирован для полного использования возможностей ClickHouse. Запросы оптимизированы для использования аналитических функций и колоночного движка ClickHouse, обеспечивая быстрый поиск и агрегацию по огромным объемам данных.

### Нативная интеграция с OpenTelemetry {#open-telemetry-native}
...
ClickStack полностью открыт и может быть развернут где угодно. Схема гибкая и может изменяться пользователем, а интерфейс спроектирован так, чтобы настраиваться под пользовательские схемы без необходимости внесения изменений. Все компоненты — включая коллекторы, ClickHouse и интерфейс — могут масштабироваться независимо для удовлетворения требований к приему данных, запросам или хранению.
