...
**ClickStack** — это платформа обсервабилити промышленного уровня, построенная на ClickHouse и объединяющая логи, трейсы, метрики и сессии в едином высокопроизводительном решении. Предназначенная для мониторинга и отладки сложных систем, ClickStack позволяет разработчикам и SRE выполнять сквозное трассирование проблем без переключения между инструментами или ручного связывания данных по меткам времени или идентификаторам корреляции.

В основе ClickStack лежит простая, но мощная идея: все данные обсервабилити должны поступать в систему в виде широких, информативных событий. Эти события хранятся в таблицах ClickHouse по типам данных — логи, трейсы, метрики и сессии, — но при этом остаются полностью доступными для выполнения запросов и перекрёстной корреляции на уровне базы данных.

ClickStack создан для эффективной обработки рабочих нагрузок с высокой кардинальностью, используя столбцовую архитектуру ClickHouse, встроенную поддержку JSON и полностью параллельный движок выполнения. Это обеспечивает выполнение запросов с задержкой менее секунды по огромным наборам данных, быстрые агрегации на больших интервалах времени и глубокий анализ отдельных трейсов. JSON хранится в сжатом столбцовом формате, что позволяет изменять схему без ручного вмешательства или предварительного определения.
