...
description: 'SDK для языков программирования в ClickStack — стеке наблюдаемости ClickHouse'
...
Данные обычно отправляются в ClickStack через **сборщик OpenTelemetry (OTel collector)** — либо напрямую из SDK для языков программирования, либо через промежуточный сборщик OpenTelemetry, работающий как агент, например для сбора инфраструктурных метрик и логов.

SDK для языков программирования отвечают за сбор телеметрии из вашего приложения — в первую очередь **трейсов** и **логов** — и экспорт этих данных в сборщик OpenTelemetry через OTLP-эндпоинт, который обеспечивает загрузку данных в ClickHouse.

В браузерных средах SDK также могут отвечать за сбор **данных сессий**, включая события UI, клики и переходы между страницами, что позволяет воспроизводить пользовательские сессии. 
