...
title: 'Мониторинг логов AWS Lambda с помощью ClickStack и Rotel'
sidebar_label: 'Логи AWS Lambda'
pagination_prev: null
pagination_next: null
description: 'Мониторинг логов AWS Lambda с помощью ClickStack и Rotel'
doc_type: 'guide'
keywords: ['AWS', 'Lambda', 'OTEL', 'ClickStack', 'логи', 'CloudWatch']
---
...
# Мониторинг логов AWS Lambda в ClickStack с использованием Rotel \{#lambda-clickstack\}

<CommunityMaintainedBadge/>

:::note[Кратко]
В этом руководстве показано, как мониторить функции AWS Lambda с помощью ClickStack, используя Rotel Lambda Extension для сбора и пересылки логов функций, логов расширения и данных OpenTelemetry напрямую в ClickHouse. Вы узнаете, как:
...
Этот подход может значительно снизить расходы на обсервабилити для Lambda, полностью обходя CloudWatch Logs.
...
В этом разделе описывается настройка ваших существующих функций AWS Lambda для отправки журналов и трассировок в ClickStack с помощью расширения Rotel Lambda Extension.

### Предварительные требования \{#prerequisites\}

- Запущенный экземпляр ClickStack
- Функции AWS Lambda, которые нужно мониторить
- AWS CLI, настроенный с соответствующими правами доступа
- Роль выполнения Lambda с правами на добавление слоёв

<VerticalStepper headerLevel="h4">
  #### Выберите соответствующий слой Rotel Lambda Extension

  Выберите слой Lambda, соответствующий архитектуре среды выполнения Lambda. Поле `{version}`
  зависит от региона AWS, в который выполняется развертывание. Актуальные номера версий для вашего региона см. на странице [releases](https://github.com/streamfold/rotel-lambda-extension/releases).
  ...
  *В этих примерах замените `{arch}`, `{region}` и `{version}` соответствующими значениями, указанными выше.*
  ...
  Добавьте следующие переменные окружения в Lambda-функцию:
  ...
  ##### Расширенная конфигурация (использование файла .env)
  ...
  **rotel.env:**
  ...
  Для промышленных развертываний храните конфиденциальные данные, такие как API-ключи, в AWS Secrets Manager или Parameter Store:
  ...
  **Требуемые разрешения IAM:**
  ...
  Вызовите Lambda-функцию, чтобы проверить, что логи отправляются в ClickStack:
  ...
  <Image img={log} alt="Подробности журнала Lambda" />

  Найдите в логах следующие ключевые атрибуты:

  * `service.name`: Имя вашей Lambda-функции
  * `faas.name`: название функции AWS Lambda
...
По умолчанию AWS Lambda отправляет все логи в CloudWatch Logs, что при больших масштабах может быть дорого. После того как вы убедились, что логи поступают в ClickStack, вы можете отключить логирование в CloudWatch для снижения затрат.

<VerticalStepper headerLevel="h4">

#### Удаление прав на CloudWatch из роли выполнения \{#remove-permissions\}
...
5. Отредактируйте роль и удалите все действия `logs:*`:
   ...
   - Если используется управляемая AWS политика `AWSLambdaBasicExecutionRole`, удалите её из роли
6. Сохраните роль

#### Проверка отключения логирования в CloudWatch {#verify-disabled}

Повторно вызовите функцию и убедитесь, что:
1. Новые потоки логов в CloudWatch не создаются
2. Логи продолжают появляться в ClickStack/HyperDX

```bash
# После изменения политики здесь не должно появиться новых потоков логов
...
Расширение Rotel Lambda Extension бесшовно работает со слоями автоинструментирования OpenTelemetry для сбора распределённых трейсов и метрик в дополнение к логам.

<VerticalStepper headerLevel="h4">

#### Выберите слой инструментации для вашего языка {#choose-instrumentation}
...
Актуальные версии можно найти в [репозитории AWS OpenTelemetry Lambda](https://github.com/aws-observability/aws-otel-lambda).

#### Добавьте оба слоя к вашей функции \{#add-both-layers\}

Добавьте **оба** слоя: слой расширения Rotel и слой инструментации:
...
2. Вы должны увидеть трейсы со спанами из вашей Lambda-функции
3. Трейсы будут коррелированы с логами через атрибуты `trace_id` и `span_id`

</VerticalStepper>

## Примеры {#examples}

Посмотрите пример приложения на Python, демонстрирующего Rotel Lambda Extension:

- **[Python + ClickHouse](https://github.com/streamfold/python-aws-lambda-clickhouse-example)**: приложение на Python с ручным инструментированием OpenTelemetry, отправляющее трассировки и логи напрямую в ClickHouse

## Присоединяйтесь к сообществу Rotel {#join-rotel-community}

Если у вас есть вопросы по Rotel, присоединяйтесь к [серверу Discord Rotel](https://rotel.dev) и делитесь своими отзывами и вопросами. Ознакомьтесь с [Rotel Lambda Extension](https://github.com/streamfold/rotel-lambda-extension), чтобы внести улучшения или поучаствовать в развитии проекта.
...
- **[Rotel Core](https://github.com/streamfold/rotel)**: Легковесная плоскость данных OTel, лежащая в основе этого расширения