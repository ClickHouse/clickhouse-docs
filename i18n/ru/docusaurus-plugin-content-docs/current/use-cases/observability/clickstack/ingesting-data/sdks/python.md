...
трейсов). Трейсы автоматически формируются с помощью автоматической инструментализации, поэтому ручная
инструментализация не требуется, чтобы извлечь пользу из трассировки.
...
Для настройки OpenTelemetry на серверах приложений, использующих режим веб-сервера с предварительным созданием процессов (pre-fork), необходимо вызвать метод `configure_opentelemetry` внутри хука post-fork.
...
или с несколькими рабочими процессами (`--workers`). Рекомендуется отключить эти флаги во время тестирования или использовать Gunicorn.
...
OpenTelemetry Python SDK обычно отображает ошибки в консоли при их
возникновении. Однако если вы не сталкиваетесь с ошибками, но замечаете, что ваши данные
...
Подробнее об инструментировании Python OpenTelemetry читайте здесь:
