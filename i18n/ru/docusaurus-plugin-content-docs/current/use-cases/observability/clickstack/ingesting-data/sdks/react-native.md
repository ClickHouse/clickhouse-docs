...
ClickStack React Native SDK позволяет инструментировать React Native‑приложение для отправки событий в ClickStack. Это позволяет видеть сетевые запросы и исключения на мобильных устройствах вместе с событиями бэкенда в единой временной шкале.
...
отредактируйте файл `metro.config.js`, чтобы заставить metro использовать браузерные
пакеты. Например:
