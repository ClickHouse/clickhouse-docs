...
<VerticalStepper headerLevel="h4">
  ...
  Убедитесь, что эти учетные данные сохранены в файле `temporal.key` в том же каталоге, что и создаваемый ниже файл конфигурации. Этот ключ должен храниться в виде текста без пробелов в начале и в конце.
  :::
  ...
  * Процессоры `memory_limiter` и `batch`, а также экспортёр `clickhouse` уже определены в базовой конфигурации ClickStack — достаточно ссылаться на них по имени
  * Процессор `resource` устанавливает обязательный атрибут `service.name` в соответствии с семантическими конвенциями OpenTelemetry
  * Если у вас несколько аккаунтов Temporal Cloud, настройте `service.name`, чтобы их различать (например, `"temporal-prod"`, `"temporal-dev"`)
    :::

  #### Настройте ClickStack для загрузки пользовательской конфигурации

  Чтобы включить пользовательскую конфигурацию сборщика в вашем существующем развертывании ClickStack, вы должны:
...
- Настройте [оповещения](/use-cases/observability/clickstack/alerts) для критически важных метрик (частота сбоев workflow, рост очереди задач, задержка schedule-to-start)
- Создайте дополнительные панели мониторинга для конкретных сценариев использования (мониторинг на уровне пространства имен, производительность типов workflow)
- Отслеживайте несколько учётных записей Temporal Cloud, дублируя конфигурацию приёмника с разными конечными точками и именами сервисов

## Переход к промышленной эксплуатации \{#troubleshooting\}
