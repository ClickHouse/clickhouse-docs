---
title: 'Производительность запросов по временным рядам'
...
После оптимизации хранения следующим шагом будет повышение производительности запросов.
В этом разделе мы рассмотрим два ключевых подхода: оптимизацию ключей `ORDER BY` и использование materialized views.
...
Теперь мы можем выполнять запросы к materialized view вместо исходной таблицы:
...
Наш прирост производительности здесь колоссален.
Раньше вычисление ответа на этот запрос занимало чуть больше 2 секунд, а теперь — всего 4 миллисекунды.
