...
description: 'Оптимизация производительности запросов к временным рядам'
...
Прежде чем приступать к другим оптимизациям, необходимо оптимизировать ключи сортировки, чтобы ClickHouse обеспечивал максимально быструю обработку запросов.
...
Теперь мы можем выполнять запрос к материализованному представлению вместо исходной таблицы:
...
10 строк в наборе. Затрачено: 0.004 сек.
```

Прирост производительности здесь колоссальный.
Раньше на вычисление результата этого запроса уходило чуть больше 2 секунд, а теперь — всего 4 миллисекунды.
