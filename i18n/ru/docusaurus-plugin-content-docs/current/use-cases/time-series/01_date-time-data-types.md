...
Полноценный набор типов данных для даты и времени необходим для эффективной работы с временными рядами, и ClickHouse как раз предоставляет такой набор.
От компактных представлений дат до высокоточных меток времени с точностью до наносекунд — эти типы спроектированы так, чтобы сбалансировать эффективность хранения с практическими требованиями различных приложений, работающих с временными рядами.

Независимо от того, работаете ли вы с историческими финансовыми данными, показаниями IoT‑датчиков или событиями, запланированными на будущее, типы даты и времени в ClickHouse обеспечивают необходимую гибкость для обработки самых разных временных данных.
Поддерживаемый набор типов позволяет оптимизировать и объём хранения, и производительность запросов, при этом сохраняя требуемую для вашего сценария точность.

* Тип [`Date`](/sql-reference/data-types/date) подходит в большинстве случаев. Этот тип использует 2 байта для хранения даты и ограничивает диапазон значениями `[1970-01-01, 2149-06-06]`.

* [`Date32`](/sql-reference/data-types/date32) охватывает более широкий диапазон дат. Он использует 4 байта для хранения даты и ограничивает диапазон значениями `[1900-01-01, 2299-12-31]`.

* [`DateTime`](/sql-reference/data-types/datetime) хранит значения даты и времени с точностью до секунды в диапазоне `[1970-01-01 00:00:00, 2106-02-07 06:28:15]`. Он использует 4 байта на значение.

* В случаях, когда требуется более высокая точность, можно использовать [`DateTime64`](/sql-reference/data-types/datetime64). Он позволяет хранить время с точностью до наносекунды в диапазоне `[1900-01-01 00:00:00, 2299-12-31 23:59:59.99999999]`. Он использует 8 байт на значение.
...
Мы можем использовать функцию [`now()`](/sql-reference/functions/date-time-functions#now) для получения текущего времени и [`now64()`](/sql-reference/functions/date-time-functions#now64), чтобы получить его с указанной точностью через первый аргумент.
...
Это заполнит наши столбцы временем в соответствии с типом каждого столбца:
...
ClickHouse также предоставляет набор функций для преобразования данных между различными типами.

Например, можно использовать [`toDate`](/sql-reference/functions/type-conversion-functions#todate) для преобразования значения `DateTime` в тип `Date`:
...
Для преобразования `Date` или `DateTime64` обратно в `DateTime` можно использовать [`toDateTime`](/sql-reference/functions/type-conversion-functions#todatetime):
