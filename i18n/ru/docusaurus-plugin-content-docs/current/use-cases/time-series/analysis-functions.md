...
Оконная функция [`lagInFrame`](/sql-reference/window-functions/lagInFrame) позволяет нам получить значение предыдущего периода для расчета этих изменений.
...
Затем мы можем использовать [`arrayJoin()`](/sql-reference/functions/array-join) для обработки данных и `bar()` для их визуализации:
