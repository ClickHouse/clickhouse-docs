...
* Тип [`Date`](/docs/sql-reference/data-types/date) должен быть достаточен в большинстве случаев. Этот тип требует 2 байта для хранения даты и ограничивает диапазон значений `[1970-01-01, 2149-06-06]`. 

* Тип [`Date32`](/docs/sql-reference/data-types/date32) охватывает более широкий диапазон дат. Он требует 4 байта для хранения даты и ограничивает диапазон значений `[1900-01-01, 2299-12-31]`.

* Тип [`DateTime`](/docs/sql-reference/data-types/datetime) хранит значения даты и времени с точностью до секунд и диапазоном `[1970-01-01 00:00:00, 2106-02-07 06:28:15]`. Он требует 4 байта на значение.

* В случаях, когда требуется больше точности, можно использовать тип [`DateTime64`](/docs/sql-reference/data-types/datetime64). Это позволяет хранить время с точностью до наносекунд в диапазоне `[1900-01-01 00:00:00, 2299-12-31 23:59:59.99999999]`. Он требует 8 байтов на значение.
...
Мы можем использовать функцию [`now()`](/docs/sql-reference/functions/date-time-functions#now), чтобы вернуть текущее время, и [`now64()`](/docs/sql-reference/functions/date-time-functions#now64), чтобы получить его в заданной точности через первый аргумент.
...
Например, мы можем использовать [`toDate`](/docs/sql-reference/functions/type-conversion-functions#todate), чтобы преобразовать значение `DateTime` в тип `Date`:
...
Мы можем использовать [`toDateTime64`](/docs/sql-reference/functions/type-conversion-functions#todatetime64), чтобы преобразовать `DateTime` в `DateTime64`:
...
И мы можем использовать [`toDateTime`](/docs/sql-reference/functions/type-conversion-functions#todatetime), чтобы вернуться от `Date` или `DateTime64` к `DateTime`:
