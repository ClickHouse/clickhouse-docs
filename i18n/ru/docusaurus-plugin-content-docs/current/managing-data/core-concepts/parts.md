...
Данные каждой таблицы в [семействе движков MergeTree](/engines/table-engines/mergetree-family) ClickHouse организованы на диске в виде набора неизменяемых `частей данных`.
...
В зависимости от конкретного движка таблицы дополнительные преобразования [могут](/operations/settings/settings) выполняться параллельно с сортировкой.

^^Части^^ данных являются самодостаточными и включают все метаданные, необходимые для интерпретации их содержимого, без необходимости в центральном каталоге. Помимо разреженного первичного индекса, ^^части^^ содержат дополнительные метаданные, такие как вторичные [индексы пропуска данных](/optimize/skipping-indexes), [статистика столбцов](https://clickhouse.com/blog/clickhouse-release-23-11#column-statistics-for-prewhere), контрольные суммы, индексы min-max (если используется [партиционирование](/partitions)) и [другое](https://github.com/ClickHouse/ClickHouse/blob/a065b11d591f22b5dd50cb6224fab2ca557b4989/src/Storages/MergeTree/MergeTreeData.h#L104).


## Слияние партов {#part-merges}

Для управления количеством ^^партов^^ в таблице фоновое задание [слияния](/merges) периодически объединяет меньшие ^^парты^^ в более крупные, пока они не достигнут [настраиваемого](/operations/settings/merge-tree-settings#max_bytes_to_merge_at_max_space_in_pool) размера в сжатом виде (обычно ~150 ГБ). Объединённые ^^парты^^ помечаются как неактивные и удаляются через [настраиваемый](/operations/settings/merge-tree-settings#old_parts_lifetime) интервал времени. Со временем этот процесс создаёт иерархическую структуру объединённых ^^партов^^, поэтому таблица называется ^^MergeTree^^:

<Image img={merges} size='lg' />

<br />

Чтобы минимизировать количество начальных ^^партов^^ и накладные расходы на слияния, клиентам баз данных [рекомендуется](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse#data-needs-to-be-batched-for-optimal-performance) либо вставлять данные массово, например, по 20 000 строк за раз, либо использовать [режим асинхронной вставки](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse), при котором ClickHouse буферизует строки из нескольких входящих INSERT в одну и ту же таблицу и создаёт новый парт только после того, как размер буфера превысит настраиваемый порог или истечёт таймаут.


## Мониторинг партиций таблицы {#monitoring-table-parts}

Вы можете [получить](https://sql.clickhouse.com/?query=U0VMRUNUIF9wYXJ0CkZST00gdWsudWtfcHJpY2VfcGFpZF9zaW1wbGUKR1JPVVAgQlkgX3BhcnQKT1JERVIgQlkgX3BhcnQgQVNDOw&run_query=true&tab=results) список всех существующих в данный момент активных ^^партиций^^ нашей примерной таблицы, используя [виртуальный столбец](/engines/table-engines#table_engines-virtual_columns) `_part`:
...
Уровень слияния увеличивается на единицу при каждом дополнительном слиянии партиции. Уровень 0 означает, что это новая партиция, которая ещё ни разу не была слита.
