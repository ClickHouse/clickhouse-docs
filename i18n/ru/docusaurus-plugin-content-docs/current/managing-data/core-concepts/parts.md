...
Данные каждой таблицы в ClickHouse [семейства движков MergeTree](/engines/table-engines/mergetree-family) организованы на диске в виде набора неизменяемых `частей данных`.
...
Для управления количеством ^^частей^^ в таблице фоновое задание [слияния](/merges) периодически объединяет меньшие ^^части^^ в более крупные, пока они не достигнут [настраиваемого](/operations/settings/merge-tree-settings#max_bytes_to_merge_at_max_space_in_pool) размера в сжатом виде (обычно ~150 ГБ). Объединённые ^^части^^ помечаются как неактивные и удаляются через [настраиваемый](/operations/settings/merge-tree-settings#old_parts_lifetime) интервал времени. Со временем этот процесс создаёт иерархическую структуру объединённых ^^частей^^, поэтому таблица и называется ^^MergeTree^^:

<Image img={merges} size='lg' />

<br />

Чтобы минимизировать количество начальных ^^частей^^ и накладные расходы на слияния, клиентам баз данных [рекомендуется](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse#data-needs-to-be-batched-for-optimal-performance) либо вставлять данные массово, например, по 20 000 строк за раз, либо использовать [режим асинхронной вставки](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse), при котором ClickHouse буферизует строки из нескольких входящих INSERT в одну и ту же таблицу и создаёт новую часть только после того, как размер буфера превысит настраиваемый порог или истечёт время ожидания.
...
Уровень слияния увеличивается на единицу с каждым дополнительным слиянием части. Уровень 0 означает, что это новая часть, которая еще не была объединена.
