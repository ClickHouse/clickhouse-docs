...
## refresh_parts_interval и table_disk \{#refresh-parts-interval-and-table-disk\}

Этот параметр предназначен для таблиц на MergeTree без Replicated, в которых части могут записываться внешними средствами и метаданные необходимо периодически обновлять из хранилища.

Параметр MergeTree `refresh_parts_interval` включает периодическое обновление списка частей данных из подлежащего хранилища (например, чтобы подхватить части, записанные внешними средствами). Важно различать **общие метаданные для всех реплик** и **локальные метаданные реплики** (например, S3 с локальными метаданными на реплику): новые части будут видимы всем репликам только тогда, когда метаданные являются общими. Использование одного лишь Объектного хранилища само по себе не означает общих метаданных.

- **Объектное хранилище (например, `disk = 's3'`) само по себе не подразумевает общих метаданных.** Когда метаданные хранятся локально для каждой реплики (поведение по умолчанию), каждая реплика независимо управляет своими указателями на объекты (blobs) в Объектном хранилище. Изменения, сделанные на одной реплике, не видны другим. В этом случае `refresh_parts_interval` не делает новые части видимыми на всех репликах, поскольку метаданные, которые читает каждая реплика, являются локальными для этой реплики.

- **Автоматическое обновление частей требует, чтобы метаданные файловой системы были общими** (или чтобы таблица использовала принадлежащие ей, только для чтения метаданные, к которым обновление может применяться). Установка `table_disk = true` вместе с диском уровня таблицы (например, `SETTINGS disk = disk(type=object_storage, ...), table_disk = true`) — один из способов получить корректную семантику: таблица владеет жизненным циклом метаданных, а хранилище считается только для чтения, поэтому `refresh_parts_interval` выполняется и внешне добавленные части могут быть обнаружены.

- **При глобально определённом диске** (например, `disk = 's3'` в `storage_configuration`) и локальных метаданных по умолчанию каждая реплика имеет собственное состояние метаданных. Хотя объекты могут находиться в S3, хранилище не считается общим для целей `refresh_parts_interval`, и новые части, созданные вне ClickHouse или на другой реплике, не будут обнаружены.

Для автоматического обновления частей убедитесь, что метаданные являются общими, или используйте диск уровня таблицы с `table_disk = true`, как описано выше. Полагаться только на `refresh_parts_interval` при локальных для реплики метаданных недостаточно для обновления частей в ожидаемом виде.
...
Этот параметр применим только к нереплицируемым таблицам MergeTree, в которых части записываются внешними средствами и требуется обновление метаданных.
:::
...
В следующем примере к внешнему хранилищу добавлен кэш.
...
где `web` задаётся в файле конфигурации сервера:
...
| `use_environment_credentials`                                                                                                    | Считывает учетные данные AWS из переменных окружения: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` и `AWS_SESSION_TOKEN`, если они существуют. Примечание: учетные данные из переменных окружения являются общими для всех дисков S3. Чтобы использовать разные учетные данные для разных дисков, вместо этого явно укажите `access_key_id` и `secret_access_key` для каждого диска. | `false`                                  |
...
### Использование S3 Plain Rewritable Storage \{#s3-plain-rewritable-storage\}
...
Табличные движки семейства `MergeTree` могут хранить данные в [Azure Blob Storage](https://azure.microsoft.com/en-us/services/storage/blobs/),
...
#### Параметры ограничения \{#azure-blob-storage-limit-parameters\}
...
Имейте в виду, что в некоторых пограничных сценариях HDFS может работать некорректно.
...
Эти настройки должны быть заданы в секции конфигурации диска.
...
| `use_split_cache`                     | Boolean | `false`               | Включает разделение кэша на системную и пользовательскую части.                                                                                                                                                               |
| `split_cache_ratio`                   | Double  | `0.1`                 | Доля системного сегмента в общем размере кэша при использовании `split_cache`.                                                                                                                                                |

> **Примечание**: Значения размера поддерживают единицы, такие как `ki`, `Mi`, `Gi` и т. д. (например, `10Gi`).


## Параметры файлового кэша для запросов/профилей \{#file-cache-query-profile-settings\}
...
#### Команды работы с кэшем \{#cache-commands-file-cache\}
...
Имя кэша можно получить из команды `SHOW FILESYSTEM CACHES`. (Для версий меньше либо равных `22.8` команда называется `DESCRIBE CACHE`)
...
| Текущие метрики кэша      | Асинхронные метрики кэша | События профилирования кэша                                                               |
| ------------------------- | ------------------------ | ----------------------------------------------------------------------------------------- |
...
Готовый тестовый сценарий. Добавьте эту конфигурацию в config:
