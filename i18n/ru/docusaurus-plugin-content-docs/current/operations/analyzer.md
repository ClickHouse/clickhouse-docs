...
Несмотря на исправление большого числа ошибок и внедрение новых оптимизаций, также были внесены некоторые изменения в поведении ClickHouse, нарушающие обратную совместимость. Пожалуйста, ознакомьтесь со следующими изменениями, чтобы понять, как переписать ваши запросы для анализатора.
...
В анализаторе проверка запроса выполняется до шага оптимизации.
...
В анализаторе использование `*` в запросе `JOIN USING`, в котором используются столбцы `ALIAS` или `MATERIALIZED`, по умолчанию включает эти столбцы в результирующий набор данных.
...
В анализаторе результат этого запроса будет включать столбец `payload` вместе со столбцом `id` из обеих таблиц.
...
Чтобы обеспечить предсказуемые и согласованные результаты, особенно при миграции старых запросов на анализатор, рекомендуется явно указывать столбцы в предложении `SELECT`, а не использовать `*`.
...
В анализаторе вывод типов происходит на этапе первоначального анализа запроса.
...
Анализатор существенно изменяет протокол взаимодействия между серверами в кластере. Поэтому выполнение распределённых запросов на серверах с разными значениями настройки `enable_analyzer` невозможно.
...
Список возможностей, которые анализатор в данный момент не поддерживает, приведён ниже:
...
Причина: старый анализатор позволял выбирать столбцы, отсутствующие в предложении GROUP BY (часто выбирая произвольное значение). Анализатор следует стандарту SQL: каждый столбец в SELECT должен быть либо агрегатом, либо ключом группировки.
...
Причина: старый анализатор допускал определение нескольких Common Table Expressions (WITH ...) с одинаковым именем, при котором более позднее определение перекрывало предыдущее. Анализатор запрещает такую неоднозначность.
...
Причина: запрос ссылается на имя столбца, которое встречается в нескольких таблицах в JOIN, без указания исходной таблицы. Старый анализатор часто выбирал столбец на основе внутренней логики, анализатор требует явного указания.
...
Причина: FINAL — это модификатор для табличного движка (в частности, [Shared]ReplacingMergeTree). Анализатор отклоняет использование FINAL, когда он применяется к:
...
Причина: Имена функций чувствительны к регистру или строго сопоставляются в анализаторе. `countdistinct` (полностью в нижнем регистре) больше не определяется автоматически. 
