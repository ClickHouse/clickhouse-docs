---
description: 'Коллекция настроек, объединённых общим именем.'
...
:::note
ClickHouse также поддерживает [процесс управления на основе SQL](/operations/access-rights#access-control-usage) для работы с профилями настроек. Мы рекомендуем использовать его.
:::

Профиль может иметь любое имя. Вы можете указать один и тот же профиль для разных пользователей. Самое важное, что вы можете задать в профиле настроек, — это `readonly=1`, что обеспечивает доступ только для чтения.
...
В примере определены два профиля: `default` и `web`.
...
Профиль `web` — это обычный профиль, который можно задать с помощью запроса `SET` или с использованием параметра URL в HTTP-запросе.
