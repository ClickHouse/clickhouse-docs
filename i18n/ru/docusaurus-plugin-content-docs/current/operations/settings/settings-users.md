...
Для одного пользователя можно настроить несколько методов аутентификации с помощью элемента `<auth_methods>`. Это позволяет пользователю проходить аутентификацию с использованием любого из перечисленных методов — например, у пользователя могут быть одновременно пароль и учетные данные LDAP, и вход по любому из них будет успешным.

Каждый дочерний элемент `<auth_methods>` — это произвольно именованная обертка, содержащая ровно один тип аутентификации. Имя обертки (например, `<method1>`, `<primary>`, `<a1>`) не имеет значения; используется только внутренний элемент аутентификации.
...
**Пример: сочетание разных типов аутентификации**
...
* **`password_sha256_hex`** — хеш пароля SHA256
* **`password_scram_sha256_hex`** — хеш пароля SCRAM-SHA-256
* **`password_double_sha1_hex`** — хеш пароля double SHA1
...
* `<auth_methods>` **нельзя** использовать вместе с методами аутентификации, заданными на уровне пользователя. Используйте либо один из этих способов, либо другой, но не оба одновременно.
* `<auth_methods>` должен содержать как минимум один метод аутентификации.
* Каждый элемент-обёртка внутри `<auth_methods>` должен содержать ровно один тип аутентификации (за исключением `<ssh_keys>`, который в целях обратной совместимости может содержать несколько).
* TOTP (`<time_based_one_time_password>`) задаётся на уровне пользователя (вне `<auth_methods>`) и применяется ко всем методам на основе пароля в списке. Если TOTP включён, требуется как минимум один метод на основе пароля.
...
В этом примере проверка TOTP применяется к методу на основе пароля (`<password>`), в то время как метод LDAP независимо выполняет аутентификацию через внешний сервер.
