...
Максимальное количество текстов в одном HTTP-запросе, который выполняет `aiEmbed`. Чтобы сократить накладные расходы на вызовы API, тексты группируются в батчи такого размера. Например, 500 уникальных текстов при размере батча 100 дают 5 HTTP-запросов.
...
Максимальное суммарное количество входных токенов (prompt) во всех вызовах API функций ИИ в рамках одного запроса. Отслеживается накопительно по ответам провайдера. Обратите внимание, что этот лимит может быть превышен на количество входных токенов одного вызова, поскольку число входных токенов для вызова заранее неизвестно. Установите 0, чтобы отключить.

Этот лимит применяется только к провайдерам, которые включают объект `usage` в свой ответ (OpenAI, Anthropic, vLLM). Если провайдер не сообщает об использовании токенов (в частности, HuggingFace TEI), счетчик остается равным 0 — в таком случае используйте `ai_function_max_api_calls_per_query`, чтобы ограничить число таких вызовов.
...
Максимальное суммарное число выходных токенов (completion) во всех вызовах API функций ИИ в рамках одного запроса. Учитывается накопительно по ответам провайдера. Обратите внимание, что этот лимит может быть превышен на число выходных токенов одного вызова, поскольку количество выходных токенов для вызова заранее неизвестно. Установите значение 0, чтобы отключить ограничение.

Это ограничение действует только для провайдеров, которые возвращают объект `usage` в своём ответе (OpenAI, Anthropic, vLLM). Оно не распространяется на функции эмбеддингов (в частности, aiEmbed), которые никогда не создают выходные токены.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.2"},{"label": "200"},{"label": "Минимальное значение таймаута асинхронной вставки в миллисекундах; async_insert_busy_timeout_ms является псевдонимом для async_insert_busy_timeout_max_ms"}]}]}/>
...
Максимальный таймаут задержки повторной попытки для операций [Zoo]Keeper во время резервного копирования или восстановления
...
Движок базы данных, разрешённый в Cloud. 1 - переписывать DDL-запросы для использования базы данных Replicated, 2 - переписывать DDL-запросы для использования базы данных Shared
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "Настройка, добавленная в 26.5, управляет поведением отложенного отсечения партиций для FINAL, которое было незаметно введено в 26.3 (https://github.com/ClickHouse/ClickHouse/pull/98242). Существенное изменение семантики зафиксировано в блоке 26.3, поэтому `compatibility = '26.2'` отменяет его; эта запись нужна для того, чтобы проверка обновления с 26.4 принимала новое имя настройки."}]}, {"id": "row-2","items": [{"label": "26.3"},{"label": "1"},{"label": "Управляет безусловным отключением отсечения партиций в планировщике FINAL, когда столбец ключа партиции не входит в ключ сортировки. Само изменение поведения было незаметно введено в 26.3 через https://github.com/ClickHouse/ClickHouse/pull/98242; эта запись задним числом документирует его, чтобы `compatibility = '26.2'` восстанавливало дорегрессионное поведение (0 = отсекать до FINAL, быстрее; 1 = откладывать отсечение, безопасно с точки зрения корректности)."}]}]} />

Если настройка включена (по умолчанию), для запросов `FINAL` к таблицам, у которых
...
введённое в 26.3: `FINAL` может потребоваться дедуплицировать строки с одинаковым первичным ключом, но находящиеся
в разных партициях, а отсечение партиций незаметно исключило бы такие строки из входных данных
для дедупликации.

Если настройка отключена, отсечение партиций применяется даже с `FINAL`, что возвращает поведение
до 26.3. Это может быть значительно быстрее для запросов с предикатами `WHERE` по
столбцу партиции, но корректно только тогда, когда строки с одинаковым первичным ключом не могут находиться
в разных партициях — например, в таблицах логов событий, где столбец партиции задаётся при вставке
...
По умолчанию при вставке данных в таблицу `Distributed` сервер ClickHouse отправляет данные на узлы кластера в фоновом режиме. Когда `distributed_foreground_insert=1`, данные обрабатываются синхронно, и операция `INSERT` считается успешной только после того, как все данные сохранены на всех шардах (как минимум на одной реплике для каждого шарда, если `internal_replication` равно `true`).
...
Если параметр включён, распределённый анализ индексов выполняется только на координаторе.
Это предотвращает запросы O(N^2), порождаемые при наличии подзапросов в предикате (например, `IN (SELECT ...)`),
поскольку в противном случае каждая ведомая реплика независимо запускала бы собственный распределённый анализ индексов,
но снижает эффективность распределённого анализа индексов, если в подзапросах используются большие таблицы.
...
чтобы оптимизатор порядка соединений мог рассматривать прямые планы (A JOIN C).
...
Выполняет некоррелированные подзапросы EXISTS как скалярные подзапросы. Как и для скалярных подзапросов, используется кэш, а к результату применяется свёртка констант.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "Новая настройка для синхронного завершения частей проекций во время INSERT, чтобы уменьшить пиковое потребление памяти."}]}]} />

Если параметр включён, части проекций завершаются синхронно во время INSERT, что снижает пиковое потребление памяти за счёт уменьшения параллелизма загрузки в S3. По умолчанию выходной поток каждой проекции остаётся открытым до тех пор, пока не будет завершена вся часть (включая все проекции), что позволяет выполнять параллельные загрузки в S3, но увеличивает пиковое потребление памяти пропорционально количеству проекций. Эта настройка влияет только на путь INSERT; слияние и мутации уже завершают проекции синхронно.
...
Значение по умолчанию для свойства таблицы Iceberg `history.expire.max-ref-age-ms`, которое `expire_snapshots` использует при отсутствии этого свойства.
...
Значение по умолчанию для свойства таблицы Iceberg `history.expire.max-snapshot-age-ms`, которое `expire_snapshots` использует при отсутствии этого свойства.
...
Значение по умолчанию для свойства таблицы Iceberg `history.expire.min-snapshots-to-keep`, которое `expire_snapshots` использует при отсутствии этого свойства.
...
Если включено, исключения, возникающие при отправке данных в зависимое materialized view (в его `SELECT` или во внутреннюю таблицу-приемник), записываются в журнал как предупреждение, а оператор `INSERT` завершается успешно. Если отключено (по умолчанию), такое исключение пробрасывается дальше, и оператор `INSERT` завершается ошибкой.

Этот параметр влияет только на обработку ошибок. Он не откатывает запись в исходную таблицу и не гарантирует, что исходный block уже был зафиксирован в исходной таблице к моменту возникновения ошибки в конвейере зависимого представления. Если параметр отключен (по умолчанию), при ошибке в представлении `INSERT` завершается неуспешно — повторите вставку с insert deduplication (`insert_deduplicate`, `deduplicate_blocks_in_dependent_materialized_views`), чтобы обеспечить exactly-once доставку в исходную таблицу и все зависимые представления. Если параметр включен, `INSERT` сообщает об успехе, несмотря на частичную доставку в представления, где произошла ошибка, и в их последующие цепочки; используйте это только там, где запись в исходную таблицу не должна блокироваться из-за проблем на стороне представления (например, для таблиц `system.*_log`). Полное описание семантики см. в документации по `CREATE VIEW`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "Новая настройка для ограничения количества сегментов, которые могут быть молча пропущены, когда включён skip_unavailable_shards."}]}]} />

Когда включён `skip_unavailable_shards`, эта настройка ограничивает максимальное количество сегментов, которые могут быть молча пропущены.
Если количество недоступных сегментов превышает это значение, вместо молчаливого пропуска выбрасывается исключение.

Значение 0 означает без ограничений (поведение по умолчанию — можно пропустить все недоступные сегменты).

## max_skip_unavailable_shards_ratio \{#max_skip_unavailable_shards_ratio\}

<SettingsInfoBlock type="Float" default_value="0" />

<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "Новая настройка для ограничения доли сегментов, которые можно молча пропускать, когда включён skip_unavailable_shards."}]}]}/>

Когда `skip_unavailable_shards` включён, эта настройка ограничивает максимальную долю (от 0 до 1) сегментов, которые можно молча пропускать.
Если отношение недоступных сегментов к общему числу сегментов превышает это значение, вместо молчаливого пропуска выбрасывается исключение.

Значение 0 означает отсутствие ограничений (поведение по умолчанию — можно пропустить все недоступные сегменты).
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "1"},{"label": "Переписывает tupleElement(dictGet(..., tuple_of_attrs, ...), N) в вызов dictGet с одним атрибутом."}]}]} />

Переписывает `tupleElement(dictGet('dict', ('a', 'b', 'c'), key), 2)` в `dictGet('dict', 'b', key)`, чтобы не извлекать лишние атрибуты словаря. Поддерживает позиционный (`.1`, `.2`, ...) и именованный (`.b`) доступ, а также применяется к `dictGetOrDefault`, если аргумент значения по умолчанию — константный кортеж или `tuple(...)` из констант.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "10000"},{"label": "Добавлена новая настройка, ограничивающая затраты на хеширование содержимого множеств в предложениях `IN` в механизме сопоставления проекций (на данный момент: агрегатной проекции). Множества, превышающие этот лимит, считаются несовпадающими. Ноль полностью отключает сравнение хешей содержимого (значение для совместимости: сопоставление проекций никогда не бывает успешным для узлов с множествами `IN`)."}]}]} />

Максимальное число строк в множестве предложения `IN`, для которого механизм сопоставления проекций вычисляет и сравнивает хеши содержимого, чтобы определить, равны ли два множества. Множества, превышающие это значение, считаются несовпадающими, и проекция пропускается. Ноль полностью отключает сравнение хешей содержимого: сопоставление проекций никогда не бывает успешным для узлов, содержащих множества предложения `IN`.

Используется механизмом сопоставления агрегатной проекции (и любым будущим механизмом сопоставления проекций, которому потребуется сравнивать множества предложений `IN`). Вычисление хеша содержимого имеет сложность `O(N log N)` по числу элементов множества; эта настройка ограничивает затраты на этапе планирования, когда в запросе или проекции встречается много предложений `IN`.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "0"},{"label": "Новая настройка для случайной генерации статистики порядка соединений при тестировании."}]}]} />
...
Задаёт период таймера реального времени для [профилировщика запросов](../../operations/optimizing-performance/sampling-query-profiler.md). Таймер реального времени измеряет так называемое «настенное» время (wall-clock time).
...
- Положительное целое число в наносекундах.
    ...
    Рекомендуемые значения:
            ...
            - 1000000000 наносекунд (раз в секунду) — для профилирования на уровне всего кластера.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "24.12"},{"label": "1"},{"label": "включено контроль конкуренции по умолчанию"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "21.9"},{"label": "1"},{"label": "включена функция Hedged Requests по умолчанию"}]}]}/>
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.5"},{"label": "0"},{"label": "По умолчанию отключает `use_top_k_dynamic_filtering` для сортировочных столбцов переменной длины (например, `String`); прежнее поведение, при котором оптимизация применялась безусловно, сохраняется при `compatibility`."}]}]} />

Разрешает применять `use_top_k_dynamic_filtering`, если сортировочный столбец имеет тип данных переменной длины (например, `String`, `Array`, `Map`, `Tuple`, содержащий элементы переменной длины).

Для таких типов сравнение с порогом для каждой строки, которое выполняет динамический фильтр, может свести на нет получаемую выгоду, если в столбце преобладает лексикографический минимум (например, в основном пустые строки) и можно пропустить лишь немного гранул. В этом случае динамический фильтр не уменьшает, а увеличивает задержку запроса.

Если эта настройка имеет значение `0`, динамическая фильтрация ограничивается столбцами, значения которых имеют фиксированный максимальный размер в памяти (числа, `Date`, `DateTime`, `FixedString`, `Enum`, `Nullable` таких типов, `Tuple` таких типов). Если установлено значение `1`, динамическая фильтрация также применяется к типам переменной длины.
...
Перед передачей в среду выполнения значение умножается на 1024, поэтому `webassembly_udf_max_fuel = 1` соответствует примерно 1024 единицам «топлива». Установите значение 0, чтобы убрать конечное ограничение. Применяется только к функциям, для которых настройка уровня функции `webassembly_udf_enable_fuel` имеет значение true, что используется по умолчанию.
