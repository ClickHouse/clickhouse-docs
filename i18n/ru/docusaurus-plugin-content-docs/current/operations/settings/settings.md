...
По умолчанию значение `max_threads` соответствует количеству ядер (потоков) одного CPU, доступного для ClickHouse.
Для пользователей Cloud значение по умолчанию отображается как `auto(N)`, где N соответствует vCPU‑размеру вашего сервиса, например 8GiB:2vCPU, 16GiB:4vCPU и т. д.
