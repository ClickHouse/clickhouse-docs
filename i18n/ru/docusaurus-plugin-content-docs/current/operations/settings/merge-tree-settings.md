...
отдельного сетевого запроса (например, одного HTTP `DELETE` на часть в Объектном хранилище), поэтому
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.2"},{"label": "1"},{"label": "По умолчанию ограничивает размер частей даже при использовании min_age_to_force_merge_seconds"}]}, {"id": "row-2","items": [{"label": "25.1"},{"label": "0"},{"label": "Добавлена новая настройка для ограничения максимального числа байт для min_age_to_force_merge."}]}, {"id": "row-3","items": [{"label": "25.1"},{"label": "0"},{"label": "новая настройка"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1"},{"label": "Если установлено значение false, столбцы Array с именами через точку и общим префиксом рассматриваются как независимые столбцы, а не как использующие общие файлы смещений в рамках устаревшей семантики Nested"}]}]} />

Когда параметр включен (по умолчанию), столбцы Array с именами через точку и общим префиксом (например, n.a и n.b)
рассматриваются как часть структуры Nested: они используют один общий файл смещений на диске (например, n.size0),
а во время INSERT проверяется, что размеры их массивов совпадают.
Когда параметр отключен, каждый столбец Array получает собственный независимый файл смещений, имена через точку не имеют
особой семантики, и скалярный столбец может сосуществовать со столбцами Array с именами через точку и тем же префиксом
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "auto"},{"label": "Сделать настройку автоматической: максимальное число задач слияния на реплику * число активных реплик"}]}, {"id": "row-2","items": [{"label": "25.5"},{"label": "100"},{"label": "Новая настройка"}]}]} />
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.4"},{"label": "1800"},{"label": "Интервал фонового обновления набора реплик по умолчанию увеличен до 30 минут"}]}, {"id": "row-2","items": [{"label": "26.2"},{"label": "300"},{"label": "Новая настройка"}]}]} />

Как часто реплики пытаются обновлять набор реплик в фоновом режиме. Следующий запуск
выбирается равномерно случайным образом в диапазоне [0, value] секунд. Исключение: value = 0 не подчиняется этому правилу;
в реализации применяется минимум 200 мс, поэтому следующий запуск выбирается в диапазоне [0, 200] мс.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.6"},{"label": "0"},{"label": "Новая настройка, позволяющая SMT при запуске загружать данные частей из реплик вместо S3"}]}]} />

Если включено, при запуске все реплики пытаются получать хранящиеся в памяти данные частей (например, первичный
ключ, информацию о партиции и т. д.) с других реплик, где эти данные уже есть.
...
Если параметр включён, SharedMergeTree использует одну из сессий ZooKeeper из серверного пула.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "0"},{"label": "Новая настройка для пометки таблицы как только для чтения, предотвращающая вставку и изменение данных"}]}]}/>

Если установлено значение true, таблица переходит в режим только для чтения. Любые попытки вставить данные или изменить таблицу завершатся ошибкой.
...
<VersionHistory rows={[{"id": "row-1","items": [{"label": "26.3"},{"label": "1"},{"label": "Разрешить алгоритм вертикального слияния для слияний, которым нужно удалять строки с истекшим TTL"}]}]}/>

Если установлено значение `true`, удаление строк по TTL оптимизируется при вертикальном слиянии. Вместо принудительного горизонтального слияния
фильтр TTL вычисляется и передаётся алгоритму слияния, который устанавливает флаги пропуска в источниках строк.
