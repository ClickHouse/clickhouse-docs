...
* `--queries-format=FORMAT` — Формат запросов, считываемых из стандартного ввода. Возможные значения: `tsv` (по умолчанию, один запрос с экранированием табуляции на строку) и `script` (разобрать ввод как скрипт из нескольких запросов, разделенных точкой с запятой). Ограничение `script`: запросы `INSERT ... FORMAT` должны находиться в одной строке.
