...
* `-c FILE_PATH`, `--config-file=FILE_PATH` — Установить путь к конфигурационному файлу, чтобы получить строку подключения. Значение по умолчанию: `config.xml`.
* `--password=PASSWORD` — Пароль для аутентификации. Также может быть задан через переменную окружения `CLICKHOUSE_KEEPER_PASSWORD` или в XML-файле конфигурации в разделе `<zookeeper><password>`.
* `--identity=IDENTITY` — Идентификатор для схемы аутентификации `digest`. Также может быть задан через переменную окружения `CLICKHOUSE_KEEPER_IDENTITY` или в XML-файле конфигурации в разделе `<zookeeper><identity>`.
...
## Переменные окружения \{#clickhouse-keeper-client-env\}

* `CLICKHOUSE_KEEPER_PASSWORD` — Используется как пароль по умолчанию, если параметр `--password` не указан в командной строке.
* `CLICKHOUSE_KEEPER_IDENTITY` — Используется как идентификатор по умолчанию, если параметр `--identity` не указан в командной строке.

## Аутентификация \{#clickhouse-keeper-client-auth\}

При подключении к серверу Keeper, требующему аутентификации, пароль определяется в следующем порядке приоритета (используется первое совпадение):
...
Тот же порядок приоритета применяется к `--identity` / `CLICKHOUSE_KEEPER_IDENTITY` / `<zookeeper><identity>`.

Пример XML-файла конфигурации с параметрами аутентификации:
...
* `exists '<path>' [watch_id]` -- Возвращает `1`, если узел существует, и `0` в противном случае. При необходимости устанавливает наблюдение с идентификатором `watch_id`
...
* `watch <watch_id> [timeout_seconds]` -- Ожидает событие наблюдения с идентификатором `watch_id` и выводит тип события и путь. Если указан `timeout_seconds`, возвращает ошибку по истечении заданного времени ожидания
