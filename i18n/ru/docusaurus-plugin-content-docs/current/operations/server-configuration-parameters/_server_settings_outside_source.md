...
* Если он есть только у *некоторых* томов, тома без него имеют наименьший приоритет. Тома, у которых параметр задан, получают приоритет в соответствии со значением параметра, приоритет остальных определяется их порядком описания в конфигурационном файле относительно друг друга.
