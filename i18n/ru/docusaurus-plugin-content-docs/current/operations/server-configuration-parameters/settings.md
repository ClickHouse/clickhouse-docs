...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Да" />Аварийно останавливать сервер при исключениях LOGICAL&#95;ERROR. Только для экспертов.
...
<SettingsInfoBlock type="GroupArrayActionWhenLimitReached" default_value="throw" changeable_without_restart="No" />Действие, выполняемое при превышении максимального размера элемента массива в groupArray: сгенерировать исключение (`throw`) или отбросить (`discard`) лишние значения

## aggregate_function_group_array_max_element_size \{#aggregate_function_group_array_max_element_size\}

<SettingsInfoBlock type="UInt64" default_value="16777215" changeable_without_restart="Нет" />Максимальный размер элемента массива в байтах для функции groupArray. Этот лимит проверяется при сериализации и позволяет избежать чрезмерного размера состояния.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Включает или отключает использование функции IMPERSONATE (EXECUTE AS target&#95;user). Настройка устарела.
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Разрешает использование памяти jemalloc.
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="Нет" />Если включено, очередь асинхронных вставок принудительно обрабатывается при корректном завершении работы сервера

## async_insert_threads \{#async_insert_threads\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="No" />Максимальное количество потоков для непосредственного разбора и вставки данных в фоновом режиме. Ноль означает, что асинхронный режим отключен.
...
<SettingsInfoBlock type="UInt32" default_value="120" changeable_without_restart="No" />Интервал (в секундах) обновления ресурсоёмких асинхронных метрик.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Ограничивает вычисление асинхронных метрик только метриками, относящимися к Keeper.

## asynchronous_metrics_update_period_s \{#asynchronous_metrics_update_period_s\}

<SettingsInfoBlock type="UInt32" default_value="1" changeable_without_restart="Нет" />Период обновления асинхронных метрик в секундах.
...
<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="Только увеличение" />Максимальное количество потоков, которое будет использоваться для выполнения операций фонового сброса данных для таблиц движка [Buffer](/engines/table-engines/special/buffer).

## background_common_pool_size \{#background_common_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="Нет" />Максимальное количество потоков, которые будут использоваться для выполнения различных операций (в основном для сборки мусора) над таблицами движка [*MergeTree](/engines/table-engines/mergetree-family) в фоновом режиме.

## background_distributed_schedule_pool_size \{#background_distributed_schedule_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="Только увеличение" />Максимальное количество потоков, которое будет использовано для выполнения распределённых отправок.

## background_fetches_pool_size \{#background_fetches_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="Нет" />Максимальное количество потоков, которое будет использоваться для загрузки частей данных с другой реплики для таблиц [движок *MergeTree](/engines/table-engines/mergetree-family) в фоновом режиме.

## background_merges_mutations_concurrency_ratio \{#background_merges_mutations_concurrency_ratio\}

<SettingsInfoBlock type="Float" default_value="2" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="Только увеличение" />Максимальное число потоков, используемых для выполнения фоновых операций потоковой передачи сообщений.

## background_move_pool_size \{#background_move_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="Нет" />Максимальное число потоков, используемых для перемещения частей данных на другой диск или том в фоновом режиме для таблиц на движке *MergeTree.
...
<SettingsInfoBlock type="Float" default_value="0.8" changeable_without_restart="Нет" />Максимальная доля потоков пула, которые могут одновременно выполнять задачи одного типа.

## background_schedule_pool_size \{#background_schedule_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="512" changeable_without_restart="Только увеличение" />Максимальное количество потоков, которое будет использоваться для постоянного выполнения легковесных периодических операций для реплицируемых таблиц, потоковой обработки данных из Kafka и обновления DNS-кэша.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Задает максимальное соотношение размера кэша к объему оперативной памяти (RAM). Позволяет уменьшить размер кэша на системах с небольшим объемом памяти.
...
<SettingsInfoBlock type="UInt64" default_value="15" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="Нет" />Устанавливает размер кэша (в элементах) для [скомпилированных выражений](../../operations/caches.md).

## compiled_expression_cache_size \{#compiled_expression_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="134217728" changeable_without_restart="Нет" />Задает размер кэша (в байтах) для [скомпилированных выражений](../../operations/caches.md).
...
* `max_min_fair` — Каждый запрос с настройкой `use_concurrency_control` = 1 выделяет до `max_threads - 1` слотов CPU. Аналогичен `fair_round_robin`, но освободившиеся слоты всегда выдаются запросу с минимальным количеством уже выделенных слотов. Это обеспечивает более справедливое распределение при сильной перегрузке, когда множество запросов конкурируют за ограниченное количество слотов CPU. Краткоживущие запросы не страдают от долгоживущих запросов, которые со временем накопили больше слотов.
...
<SettingsInfoBlock type="String" default_value="config.xml" changeable_without_restart="No" />Путь к конфигурационному файлу сервера.
...
* Если `true` (значение по умолчанию), учет ведется на основе фактически потребленного CPU-времени. Конкурирующим нагрузкам выделяется справедливая доля CPU-времени. Слоты выделяются на ограниченный период и повторно запрашиваются после его истечения. Запрос слота может блокировать выполнение потока в случае перегрузки по ресурсам CPU, т.е. может происходить вытеснение (preemption). Это обеспечивает справедливое распределение CPU-времени.
* Если `false`, учет ведется на основе количества выделенных CPU-слотов. Конкурирующим нагрузкам выделяется справедливая доля CPU-слотов. Слот выделяется при запуске потока, удерживается непрерывно и освобождается при завершении выполнения потока. Количество потоков, выделенных для выполнения запроса, может только увеличиваться от 1 до `max_threads` и никогда не уменьшаться. Это более благоприятно для долго работающих запросов и может приводить к голоданию коротких запросов по CPU.
...
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `buffer_size_rows_flush_threshold` | Порог по количеству строк. Когда порог достигается, в фоновом режиме запускается сброс логов на диск.                                                           | `max_size_rows / 2` |                                                                                                                                  |
| `database`                         | Имя базы данных.                                                                                                                                                |                     |                                                                                                                                  |
| `engine`                           | [Определение движка MergeTree](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-creating-a-table) для системной таблицы.                |                     | Нельзя использовать, если заданы `partition_by` или `order_by`. Если не указано, по умолчанию выбирается `MergeTree`             |
...
| `order_by`                         | [Пользовательский ключ сортировки](/engines/table-engines/mergetree-family/mergetree#order_by) для системной таблицы. Нельзя использовать, если задан `engine`. |                     | Если для системной таблицы указан `engine`, параметр `order_by` должен быть указан непосредственно внутри &#39;engine&#39;       |
...
| `settings`                         | [Дополнительные параметры](/engines/table-engines/mergetree-family/mergetree/#settings), управляющие поведением MergeTree (необязательный параметр).            |                     | Если для системной таблицы указан `engine`, параметр `settings` должен быть указан непосредственно внутри &#39;engine&#39;       |
| `storage_policy`                   | Имя политики хранения, используемой для таблицы (необязательный параметр).                                                                                      |                     | Если для системной таблицы указан `engine`, параметр `storage_policy` должен быть указан непосредственно внутри &#39;engine&#39; |
| `table`                            | Имя системной таблицы.                                                                                                                                          |                     |                                                                                                                                  |
| `ttl`                              | Задает [TTL таблицы](/engines/table-engines/mergetree-family/mergetree#table_engine-mergetree-ttl).                                                             |                     | Если для системной таблицы указан `engine`, параметр `ttl` должен быть указан непосредственно внутри &#39;engine&#39;            |
...
<SettingsInfoBlock type="UInt64" default_value="480" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="5" changeable_without_restart="No" />В случае неудачной попытки удалить таблицу ClickHouse будет ждать в течение этого тайм-аута, прежде чем повторить операцию.

## database_catalog_drop_table_concurrency \{#database_catalog_drop_table_concurrency\}

<SettingsInfoBlock type="UInt64" default_value="16" changeable_without_restart="No" />Размер пула потоков, используемого при удалении таблиц.
...
Если какой-либо подкаталог не используется сервером ClickHouse и этот каталог не изменялся в течение последних
...
удалив все права доступа к нему. Это также применяется к каталогам, которые сервер ClickHouse не
...
<SettingsInfoBlock type="UInt64" default_value="2592000" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Разрешает постоянное отсоединение таблиц в базах данных Replicated

## database_replicated_drop_broken_tables \{#database_replicated_drop_broken_tables\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Удалять неожиданные таблицы из баз данных Replicated вместо перемещения их в отдельную локальную базу данных
...
<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />Интервал в миллисекундах между попытками повторного подключения словарей MySQL и Postgres с включённым `background_reconnect` после сбоя.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Отключает внутренний DNS-кэш. Рекомендуется при эксплуатации ClickHouse в средах с часто меняющейся инфраструктурой, например Kubernetes.
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="No" />При попытке создания, когда этот лимит достигнут, выбрасывается исключение. Установите 0, чтобы отключить жесткое ограничение. Лимит применяется к подключениям к дискам.

## disk_connections_rcvbuf \{#disk_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер буфера сокета SO&#95;RCVBUF для подключений к диску (S3, Azure, GCS). Если задано значение больше 0, оно переопределяет автонастройку TCP в ядре для буфера приёма. 0 = значение ядра по умолчанию (автонастройка). Примечание: если изменить эту настройку обратно на 0, автонастройка восстановится только для вновь создаваемых соединений; существующие соединения в пуле сохранят фиксированные размеры буфера, пока не будут пересозданы.

## disk_connections_sndbuf \{#disk_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер параметра SO&#95;SNDBUF для подключений к диску (S3, Azure, GCS). Если указано значение больше 0, оно отключает автонастройку TCP ядра для буфера отправки. 0 = значение ядра по умолчанию (автонастройка). 注意: при возврате этой настройки к значению 0 автонастройка восстанавливается только для вновь создаваемых соединений; существующие соединения из пула сохраняют фиксированный размер буфера, пока не будут созданы заново.

## disk_connections_soft_limit \{#disk_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="Нет" />Соединения сверх этого лимита имеют значительно меньший срок жизни. Лимит применяется к подключениям к дискам.

## disk_connections_store_limit \{#disk_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="Нет" />Подключения сверх этого лимита очищаются после использования. Установите 0, чтобы отключить кэш подключений. Лимит применяется к подключениям к дискам.

## disk_connections_warn_limit \{#disk_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="8000" changeable_without_restart="No" />Предупреждающие сообщения записываются в журналы, если количество активных соединений превышает этот предел. Предел применяется к подключениям к дискам.
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Определяет, следует ли серверу кэша применять настройки ограничения (throttling), полученные от клиента.
...
<SettingsInfoBlock type="UInt64" default_value="60" changeable_without_restart="Нет" />очистка начинается после получения события о новом узле, если с момента последней очистки прошло не менее `<cleanup_delay_period>` секунд.
...
<SettingsInfoBlock type="Int32" default_value="1" changeable_without_restart="No" />Сколько запросов `<ON CLUSTER>` может выполняться одновременно
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Если параметр включён, запросы ON CLUSTER будут сохранять и использовать пользователя и роли инициатора для выполнения на удалённых сегментах. Это обеспечивает последовательный контроль доступа во всём кластере, но требует, чтобы этот пользователь и роли существовали на всех узлах.

## dns_allow_resolve_names_to_ipv4 \{#dns_allow_resolve_names_to_ipv4\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="Yes" />Разрешает преобразовывать имена в IPv4-адреса.
...
<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="No" />Размер пула потоков, используемого для очистки распределённого кэша.

## drop_distributed_cache_queue_size \{#drop_distributed_cache_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />Размер очереди пула потоков, используемого для очистки распределённого кэша.
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="No" />При попытке создать соединение, когда этот предел достигнут, выбрасывается исключение. Установите 0, чтобы отключить жёсткое ограничение. Ограничение применяется к HTTP-соединениям, которые не принадлежат ни к какому диску или хранилищу.

## http_connections_rcvbuf \{#http_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер буфера SO&#95;RCVBUF для обычных HTTP-соединений. Если задано значение больше 0, оно переопределяет автотюнинг TCP для буфера приёма, выполняемый ядром. 0 = значение ядра по умолчанию (автотюнинг). Внимание: при возврате этой настройки к 0 автотюнинг восстанавливается только для вновь создаваемых соединений; существующие соединения в пуле сохраняют фиксированные размеры буфера, пока не будут пересозданы.

## http_connections_sndbuf \{#http_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер параметра SO&#95;SNDBUF для обычных HTTP-соединений. Если задано значение больше 0, оно отключает автотюнинг ядра TCP для буфера отправки и использует указанное значение. 0 = значение ядра по умолчанию (автотюнинг). Примечание: при изменении этой настройки обратно на 0 автотюнинг восстанавливается только для вновь созданных соединений; существующие соединения в пуле сохраняют фиксированный размер буфера, пока не будут пересозданы.

## http_connections_soft_limit \{#http_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="No" />Соединения, число которых превышает этот лимит, имеют значительно более короткое время жизни. Лимит применяется к HTTP‑соединениям, которые не привязаны ни к какому диску или хранилищу.

## http_connections_store_limit \{#http_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />Подключения сверх этого лимита сбрасываются после использования. Установите значение 0, чтобы отключить кэш подключений. Лимит применяется к HTTP-соединениям, которые не привязаны ни к какому диску или хранилищу.

## http_connections_warn_limit \{#http_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />Предупреждающие сообщения записываются в логи, если число используемых соединений превышает этот предел. Предел применяется к HTTP-соединениям, которые не относятся ни к одному диску или хранилищу.
...
<SettingsInfoBlock type="UInt64" default_value="10" changeable_without_restart="No" />Размер пула потоков для асхронного получения актуальных метаданных из удалённого каталога Iceberg; этот пул используется совместно всеми активными таблицами.

## iceberg_catalog_threadpool_pool_size \{#iceberg_catalog_threadpool_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="50" changeable_without_restart="Нет" />Размер фонового пула потоков для каталога Iceberg
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Название политики кэширования метаданных Iceberg.

## iceberg_metadata_files_cache_size \{#iceberg_metadata_files_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="Yes" />Максимальный размер кэша метаданных Iceberg в байтах. Нулевое значение означает, что кэш отключён.
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="Нет" />Имя политики кэширования меток вторичного индекса.

## index_mark_cache_prewarm_ratio \{#index_mark_cache_prewarm_ratio\}

<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="No" />Доля общего объёма кэша меток индекса, которую нужно заполнить при предварительном прогреве.

## index_mark_cache_size \{#index_mark_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="5368709120" changeable_without_restart="Да" />
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэша несжатого вторичного индекса.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (в случае использования политики SLRU) в кэше несжатого вторичного индекса по отношению к общему размеру кэша.
...
Значение `old_separate_hashes` означает, что ClickHouse будет использовать разные хэши дедупликации для синхронных и асинхронных вставок (как и раньше).
Это значение следует использовать в качестве значения по умолчанию, если нет намерения начинать миграцию.
Значение `compatible_double_hashes` означает, что ClickHouse будет использовать два хэша дедупликации: старый — для синхронных или асинхронных вставок, и новый — для всех вставок. Это значение следует использовать для безопасной миграции существующих экземпляров к новому поведению.
Это значение должно быть включено на некоторое время (см. настройки `replicated_deduplication_window` и `non_replicated_deduplication_window`), чтобы убедиться, что ни одна синхронная или асинхронная вставка не была потеряна во время миграции.
Наконец, значение `new_unified_hash` означает, что ClickHouse будет использовать новый хэш дедупликации для синхронных и асинхронных вставок. Это значение можно включать для новых экземпляров ClickHouse или для экземпляров, которые уже использовали значение `compatible_double_hashes` в течение некоторого времени.
Значение по умолчанию будет установлено в `compatible_double_hashes`, а затем позднее в `new_unified_hash` в будущих версиях ClickHouse, чтобы завершить миграцию безопасным способом в два релиза.
...
<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="Да" />
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Нет" />Хранить выборочные выделения памяти jemalloc в system.trace&#95;log

## jemalloc_enable_background_threads \{#jemalloc_enable_background_threads\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="Нет" />Включает фоновые потоки jemalloc. Jemalloc использует фоновые потоки для очистки неиспользуемых страниц памяти. Его отключение может привести к снижению производительности.

## jemalloc_enable_global_profiler \{#jemalloc_enable_global_profiler\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Включает глобальный профилировщик выделений jemalloc для всех потоков. Jemalloc будет выборочно отслеживать операции выделения и все освобождения для выборочно отслеженных выделений.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Сброс профиля jemalloc будет выполнен после того, как глобальное пиковое потребление памяти увеличится на jemalloc&#95;flush&#95;profile&#95;interval&#95;bytes
...
<SettingsInfoBlock type="UInt64" default_value="300" changeable_without_restart="Нет" />Таймаут ожидания при получении данных сокетом Keeper.

## keeper_server.socket_send_timeout_sec \{#keeper_server.socket_send_timeout_sec\}

<SettingsInfoBlock type="UInt64" default_value="300" changeable_without_restart="No" />Таймаут отправки данных через сокет Keeper.
...
<SettingsInfoBlock type="UInt64" default_value="50" changeable_without_restart="Нет" />Размер фонового пула потоков для загрузки меток

## load_marks_threadpool_queue_size \{#load_marks_threadpool_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />Количество задач, которые можно поставить в очередь пула предварительной выборки
...
| Key                    | Description                                                                                                                                                        |
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `async` | Если `true` (по умолчанию), логирование выполняется асинхронно (по одному фоновому потоку на каждый выходной канал). В противном случае запись в лог выполняется в потоке, который вызывает LOG.           |
...
| `console_shutdown_log_level` | Уровень при остановке, используемый для установки уровня логирования в консоль при остановке сервера.   |
| `console_startup_log_level` | Уровень логирования при запуске используется для установки уровня логирования в консоль при старте сервера. После запуска уровень логирования возвращается к значению настройки `console_log_level`.                                   |   
...
| `rotation` | Политика ротации: определяет, когда файлы журнала ротируются. Ротация может основываться на размере, времени или их комбинации. Примеры: 100M, daily, 100M,daily. Как только размер файла журнала превышает указанный порог или достигается заданный интервал времени, файл переименовывается и архивируется, и создаётся новый файл журнала. |
| `shutdown_level` | Уровень логирования, используемый для установки уровня корневого логгера при остановке сервера.                                                                                            |
| `size` | Политика ротации: максимальный размер файлов журнала в байтах. Как только размер файла журнала превышает этот порог, файл переименовывается и архивируется, и создаётся новый файл журнала. |
| `startup_level` | Уровень логирования при запуске, используется для установки уровня корневого логгера при старте сервера. После запуска уровень логирования возвращается к значению настройки `level`.                                   |
| `stream_compress` | Сжатие сообщений журнала с помощью LZ4. Установите `1` или `true`, чтобы включить.                                                                                                   |
| `syslog_level` | Уровень логирования для вывода в syslog.                                                                                                                                   |
| `use_syslog` | Дополнительно пересылать вывод логов в syslog.                                                                                                                                 |

**Спецификаторы формата журнала**

Имена файлов в путях `log` и `errorLog` поддерживают приведённые ниже спецификаторы формата для результирующего имени файла (для части пути, указывающей каталог, они не поддерживаются).
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Когда установлено значение `<true>` (по умолчанию), логирование выполняется асинхронно (один фоновый поток на каждый канал вывода). В противном случае записи в лог выполняются в потоке, из которого вызывается LOG.

## logger.async_queye_max_size \{#logger.async_queye_max_size\}

<SettingsInfoBlock type="UInt64" default_value="65536" changeable_without_restart="Нет" />При использовании асинхронного логирования задаёт максимальное количество сообщений, которые могут находиться в очереди в ожидании сброса. Сообщения сверх этого лимита будут отброшены.

## logger.console \{#logger.console\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Включает вывод логов в консоль. Установите значение `<1>` или `<true>`, чтобы включить параметр. Значение по умолчанию — `<1>`, если ClickHouse не запущен в режиме демона, и `<0>` в противном случае.
...
<SettingsInfoBlock type="UInt64" default_value="1" changeable_without_restart="No" />Политика ротации: максимальное количество старых файлов журнала ClickHouse, которые могут храниться.
...
<SettingsInfoBlock type="String" default_value="json" changeable_without_restart="No" />Формат логов для вывода в консоль. Сейчас поддерживается только `<json>`.

## logger.level \{#logger.level\}

<SettingsInfoBlock type="String" default_value="trace" changeable_without_restart="No" />Уровень логирования. Допустимые значения: `<none>` (отключить логирование), `<fatal>`, `<critical>`, `<error>`, `<warning>`, `<notice>`, `<information>`, `<debug>`, `<trace>`, `<test>`.
...
<SettingsInfoBlock type="String" default_value="100M" changeable_without_restart="Нет" />Политика ротации: максимальный допустимый размер файлов журнала в байтах. После превышения этого порога файл журнала переименовывается и архивируется, а затем создаётся новый файл журнала.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Сжимает сообщения журнала с помощью LZ4. Установите значение `<1>` или `<true>`, чтобы включить.

## logger.syslog_level \{#logger.syslog_level\}

<SettingsInfoBlock type="String" default_value="trace" changeable_without_restart="No" />Уровень логирования при выводе в syslog.

## logger.use_syslog \{#logger.use_syslog\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Также перенаправлять вывод логов в syslog.
...
<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="Нет" />Доля общего объёма кэша меток, заполняемая при предварительном прогреве.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="Нет" />Размер защищённой очереди (в случае политики SLRU) в кэше меток относительно общего размера кэша.
...
<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальная скорость чтения в байтах в секунду для всех резервных копий на сервере. Значение 0 означает отсутствие ограничения.

## max_backups_io_thread_pool_free_size \{#max_backups_io_thread_pool_free_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Если количество **бездействующих** потоков в пуле потоков ввода-вывода резервных копий (Backups IO Thread pool) превышает `max_backup_io_thread_pool_free_size`, ClickHouse освободит ресурсы, занятые этими потоками, и уменьшит размер пула. Потоки могут быть созданы снова при необходимости.

## max_backups_io_thread_pool_size \{#max_backups_io_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="Yes" />ClickHouse использует потоки из пула потоков ввода-вывода резервных копий для выполнения операций ввода-вывода при резервном копировании в S3. `max_backups_io_thread_pool_size` ограничивает максимальное количество потоков в этом пуле.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Если число баз данных превышает это значение, сервер выбросит исключение. 0 означает отсутствие ограничения.
...
<SettingsInfoBlock type="UInt32" default_value="1" changeable_without_restart="No" />Количество потоков, используемых для создания таблиц во время восстановления реплики в DatabaseReplicated. Ноль означает, что количество потоков равно количеству ядер.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Да" />Максимальная суммарная скорость чтения из распределённого кэша на сервере в байтах в секунду. Ноль означает отсутствие ограничения.

## max_distributed_cache_write_bandwidth_for_server \{#max_distributed_cache_write_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Максимальная суммарная скорость записи в распределённый кэш на сервере в байтах в секунду. Ноль означает отсутствие ограничений.

## max_entries_for_hash_table_stats \{#max_entries_for_hash_table_stats\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />Максимальное количество записей в статистике хеш-таблицы, собираемой во время агрегации

## max_fetch_partition_thread_pool_size \{#max_fetch_partition_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="64" changeable_without_restart="Yes" />Количество потоков для выполнения ALTER TABLE FETCH PARTITION.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальное количество легковесных снимков, которые могут одновременно храниться. Ноль означает отсутствие ограничений. Если количество снимков достигает этого предела, при попытке создать новый снимок будет выброшено исключение.

## max_io_thread_pool_free_size \{#max_io_thread_pool_free_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Да" />

Если количество **бездействующих** потоков в пуле потоков ввода-вывода превышает `max_io_thread_pool_free_size`, ClickHouse освободит ресурсы, занятые этими потоками, и уменьшит размер пула. При необходимости потоки могут быть созданы заново.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Максимальная скорость чтения всех слияний на сервере в байтах в секунду. Ноль означает отсутствие ограничений.

## max_mutations_bandwidth_for_server \{#max_mutations_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Максимальная совокупная скорость чтения всех мутаций на сервере в байтах в секунду. 0 означает отсутствие ограничений.
...
<SettingsInfoBlock type="UInt64" default_value="32" changeable_without_restart="Да" />Количество потоков в пуле для загрузки неактивных (устаревших) частей данных при запуске.
...
<SettingsInfoBlock type="UInt64" default_value="50000000000" changeable_without_restart="Да" />
...
<SettingsInfoBlock type="UInt64" default_value="128" changeable_without_restart="Yes" />Количество потоков для одновременного удаления неактивных частей данных.
...
Если количество ожидающих мутаций превышает заданное значение, сервер ClickHouse добавит предупреждающие сообщения в таблицу `system.warnings`.
...
Если количество **бездействующих** потоков в пуле потоков десериализации префиксов превышает `max_prefixes_deserialization_thread_pool_free_size`, ClickHouse освободит ресурсы, занятые этими потоками, и уменьшит размер пула. При необходимости потоки могут быть созданы заново.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальная скорость обмена данными по сети в байтах в секунду для реплицированных выборок. Ноль означает отсутствие ограничений.

## max_replicated_sends_network_bandwidth_for_server \{#max_replicated_sends_network_bandwidth_for_server\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Максимальная скорость обмена данными по сети, в байтах в секунду, для операций replicated sends. Ноль означает отсутствие ограничений.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Да" />Если количество бездействующих потоков в пуле потоков фиксации снимков превышает `max_snapshot_commit_thread_pool_free_size`, ClickHouse освободит ресурсы, занимаемые бездействующими потоками, и уменьшит размер пула. При необходимости потоки могут быть созданы заново.

## max_snapshot_commit_thread_pool_size \{#max_snapshot_commit_thread_pool_size\}

<SettingsInfoBlock type="UInt64" default_value="64" changeable_without_restart="Yes" />Количество потоков для фиксации снимка.
...
ClickHouse использует потоки из глобального пула потоков (global thread pool) для обработки запросов. Если нет бездействующего потока для обработки запроса, в пуле создаётся новый поток. `max_thread_pool_size` ограничивает максимальное количество потоков в пуле.
...
<SettingsInfoBlock type="UInt64" default_value="8" changeable_without_restart="No" />Количество потоков для загрузки неактивного набора частей данных («неожиданных») при запуске.
...
Максимальное количество сессий ZooKeeper, инициализируемых по мере необходимости, для каждого кластера ZooKeeper в общем пуле.

Значение `0` отключает пул подключений, и по-прежнему используется одна сессия.
...
<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="Нет" />

Длительность в миллисекундах, в течение которой должно сохраняться давление на память, прежде чем будет выполнена динамическая корректировка параметра `dirty_decay_ms` в jemalloc. Когда использование памяти остается выше порогового значения для очистки в течение этого периода, автоматическое старение грязных страниц отключается (`dirty_decay_ms=0`), чтобы агрессивно освобождать память. Когда использование памяти остается ниже порога в течение этого периода, восстанавливается поведение старения по умолчанию. Установите значение 0, чтобы отключить динамическую корректировку и использовать настройки старения по умолчанию в jemalloc.

## memory_worker_period_ms \{#memory_worker_period_ms\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />

Период тиков фонового обработчика памяти, который корректирует значения memory tracker и очищает неиспользуемые страницы при повышенном использовании памяти. Если установлено значение 0, будет использовано значение по умолчанию в зависимости от источника использования памяти.
...
Пороговое значение доли «грязных» страниц jemalloc от объёма памяти, доступной серверу ClickHouse. Когда размер «грязных» страниц превышает эту долю, фоновый обработчик памяти принудительно очищает «грязные» страницы. Если установлено значение 0, принудительная очистка, основанная на доле «грязных» страниц, отключается.

## memory_worker_purge_total_memory_threshold_ratio \{#memory_worker_purge_total_memory_threshold_ratio\}

<SettingsInfoBlock type="Double" default_value="0.9" changeable_without_restart="Нет" />

Пороговое значение доли общего использования памяти по отношению к объёму памяти, доступной серверу ClickHouse, при превышении которого выполняется очистка jemalloc. Когда общее использование памяти превышает эту долю, фоновый обработчик памяти принудительно очищает «грязные» страницы. Если установлено значение 0, принудительная очистка на основе общего объёма памяти отключается.

## memory_worker_use_cgroup \{#memory_worker_use_cgroup\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Использовать сведения о текущем использовании памяти в cgroup для корректировки отслеживания памяти.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Yes" />Отключает вставку из движков очередей сообщений (Kafka, RabbitMQ, NATS) в присоединённые materialized view
...
Минимальный размер общего выделения памяти в C++ в байтах (например, выполняемого стандартными контейнерами, строками, при увеличении `std::vector`, умными указателями и т. п.), при котором после достижения `max_server_memory_usage` допускается генерация `MEMORY_LIMIT_EXCEEDED`. Более мелкие общие выделения памяти по-прежнему учитываются трекером памяти, но им разрешено успешно выполняться даже после превышения лимита, что снижает количество ложных сбоев во время очистки и обработки исключений вблизи OOM.

Выделения памяти, выполняемые собственными крупными буферами ClickHouse — данные столбцов, хеш-таблицы, Arena, конвейеры запросов, буферы ввода-вывода, — всегда учитывают лимит памяти и могут сгенерировать исключение независимо от этого значения. Этот параметр влияет только на неявные выделения памяти, проходящие через `operator new`.

Значение `0` (по умолчанию) сохраняет прежнее поведение: неявные выделения через `operator new` никогда не отклоняются трекером памяти, и только явные выделения памяти ClickHouse могут приводить к `MEMORY_LIMIT_EXCEEDED`.

Обратите внимание: чтобы избежать побочных эффектов, рекомендуется устанавливать значение больше `max_untracked_memory`.
...
<SettingsInfoBlock type="UInt64" default_value="5000000000" changeable_without_restart="No" />Минимальный порог объёма памяти, необходимый для выполнения `<mlockall>`

## mmap_cache_size \{#mmap_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1024" changeable_without_restart="Да" />
...
Используется для регулирования того, как ресурсы используются и распределяются между мутациями и другими типами нагрузки. Указанное значение используется как значение настройки `workload` для всех фоновых мутаций. Может быть переопределено настройкой таблицы MergeTree.
...
<SettingsInfoBlock type="Int32" default_value="0" changeable_without_restart="No" />В системах Linux этот параметр может управлять работой OOM-killer.
...
| `caConfig`                    | Путь к файлу или каталогу, содержащему доверенные сертификаты CA. Если указан файл, он должен быть в формате PEM и может содержать несколько сертификатов CA. Если указан каталог, он должен содержать отдельный .pem-файл для каждого сертификата CA. Имена файлов подбираются по хэшу имени субъекта CA. Подробности можно найти на man-странице [SSL&#95;CTX&#95;load&#95;verify&#95;locations](https://www.openssl.org/docs/man3.0/man3/SSL_CTX_load_verify_locations.html). |                                                                                            |
...
| `extendedVerification`        | При включении параметра проверяется, что CN или SAN сертификата совпадает с именем хоста однорангового узла.                                                                                                                                                                                                                                                                                                                                                                     | `false`                                                                                    |
| `fips`                        | Активирует режим OpenSSL FIPS. Поддерживается, если версия библиотеки OpenSSL поддерживает FIPS.                                                                                                                                                                                                                                                                                                                                                                                 | `false`                                                                                    |
| `invalidCertificateHandler`   | Класс (подкласс CertificateHandler), используемый для проверки недействительных сертификатов. Например: `<invalidCertificateHandler> <name>RejectCertificateHandler</name> </invalidCertificateHandler>`.                                                                                                                                                                                                                                                                        | `RejectCertificateHandler`                                                                 |
| `loadDefaultCAFile`           | Определяет, будут ли использоваться встроенные сертификаты ЦС для OpenSSL. ClickHouse предполагает, что встроенные сертификаты ЦС находятся в файле `/etc/ssl/cert.pem` (соответственно, в каталоге `/etc/ssl/certs`) или в файле (соответственно, каталоге), указанном в переменной окружения `SSL_CERT_FILE` (соответственно, `SSL_CERT_DIR`).                                                                                                                                 | `true`                                                                                     |
...
| `requireTLSv1`                | Требовать соединения по протоколу TLSv1. Допустимые значения: `true`, `false`.                                                                                                                                                                                                                                                                                                                                                                                                   | `false`                                                                                    |
...
| `sessionCacheSize`            | Максимальное количество сеансов, кэшируемых сервером. Значение `0` означает отсутствие ограничения на число сеансов.                                                                                                                                                                                                                                                                                                                                                             | [1024*20](https://github.com/ClickHouse/boringssl/blob/master/include/openssl/ssl.h#L1978) |
| `sessionIdContext`            | Уникальная последовательность случайных символов, которую сервер добавляет к каждому сгенерированному идентификатору. Длина строки не должна превышать `SSL_MAX_SSL_SESSION_ID_LENGTH`. Рекомендуется всегда задавать этот параметр, так как он помогает избежать проблем как при кэшировании сессии на сервере, так и при запросе кэширования со стороны клиента.                                                                                                               | `$\{application.name\}`                                                                    |
| `sessionTimeout`              | Время, в течение которого сессия кэшируется на сервере (в часах).                                                                                                                                                                                                                                                                                                                                                                                                                | `2`                                                                                        |
| `verificationDepth`           | Максимальная длина цепочки проверки сертификатов. Проверка завершится с ошибкой, если длина цепочки сертификатов превысит заданное значение.                                                                                                                                                                                                                                                                                                                                     | `9`                                                                                        |
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Включает или отключает кэширование сеансов. Должен использоваться в сочетании с `<sessionIdContext>`. Допустимые значения: `<true>`, `<false>`.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Нет" />Если параметр включён, проверяется, что значение поля CN или SAN сертификата соответствует имени хоста узла.

## openSSL.client.fips \{#openssl.client.fips\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Активирует режим OpenSSL FIPS. Поддерживается, если версия библиотеки OpenSSL поддерживает FIPS.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Нет" />Требует соединения по TLSv1. Допустимые значения: `<true>`, `<false>`.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Требует соединения по TLSv1.2. Допустимые значения: `<true>`, `<false>`.

## openSSL.client.verificationDepth \{#openssl.client.verificationdepth\}

<SettingsInfoBlock type="UInt64" default_value="9" changeable_without_restart="Нет" />Максимально допустимая длина цепочки проверки. Проверка завершится ошибкой, если длина цепочки сертификатов превысит заданное значение.
...
<SettingsInfoBlock type="String" default_value="ALL:!ADH:!LOW:!EXP:!MD5:!3DES:@STRENGTH" changeable_without_restart="Нет" />Поддерживаемые наборы шифров OpenSSL.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Если параметр включён, проверяется, что CN или SAN в сертификате совпадает с именем хоста удалённого узла.
...
<SettingsInfoBlock type="String" default_value="RejectCertificateHandler" changeable_without_restart="No" />Класс (подкласс CertificateHandler), используемый для обработки недействительных сертификатов. Например: `<<invalidCertificateHandler> <name>RejectCertificateHandler</name> </invalidCertificateHandler>>`.

## openSSL.server.loadDefaultCAFile \{#openssl.server.loaddefaultcafile\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Определяет, будут ли использоваться встроенные CA‑сертификаты для OpenSSL. ClickHouse предполагает, что встроенные CA‑сертификаты находятся в файле `</etc/ssl/cert.pem>` (соответственно, в каталоге `</etc/ssl/certs>`) или в файле (соответственно, каталоге), указанном в переменной окружения `<SSL_CERT_FILE>` (соответственно, `<SSL_CERT_DIR>`).

## openSSL.server.preferServerCiphers \{#openssl.server.preferserverciphers\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Нет" />Серверные шифры, выбираемые в соответствии с предпочтениями клиента.
...
<SettingsInfoBlock type="String" default_value="KeyConsoleHandler" changeable_without_restart="No" />Класс (наследник PrivateKeyPassphraseHandler), который запрашивает парольную фразу для доступа к закрытому ключу. Например: `<<privateKeyPassphraseHandler>>`, `<<name>KeyFileHandler</name>>`, `<<options><password>test</password></options>>`, `<</privateKeyPassphraseHandler>>`

## openSSL.server.requireTLSv1 \{#openssl.server.requiretlsv1\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Требует соединения по протоколу TLSv1. Допустимые значения: `<true>`, `<false>`.

## openSSL.server.requireTLSv1_1 \{#openssl.server.requiretlsv1_1\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Требует соединения TLSv1.1. Допустимые значения: `<true>`, `<false>`.

## openSSL.server.requireTLSv1_2 \{#openssl.server.requiretlsv1_2\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Требует соединения по протоколу TLSv1.2. Допустимые значения: `<true>`, `<false>`.

## openSSL.server.sessionCacheSize \{#openssl.server.sessioncachesize\}

<SettingsInfoBlock type="UInt64" default_value="20480" changeable_without_restart="No" />Максимальное число сеансов, которые сервер может кэшировать. Значение 0 означает неограниченное количество сеансов.

## openSSL.server.sessionIdContext \{#openssl.server.sessionidcontext\}

<SettingsInfoBlock type="String" default_value="application.name" changeable_without_restart="No" />Уникальная последовательность случайных символов, которую сервер добавляет к каждому сгенерированному идентификатору. Длина строки не должна превышать `<SSL_MAX_SSL_SESSION_ID_LENGTH>`. Настоятельно рекомендуется всегда задавать этот параметр, поскольку он помогает избежать проблем как при кэшировании сеанса на сервере, так и при запросе кэширования со стороны клиента.
...
<SettingsInfoBlock type="UInt64" default_value="9" changeable_without_restart="Нет" />Максимальная длина цепочки проверки сертификатов. Проверка завершится с ошибкой, если длина цепочки сертификатов превысит установленное значение.

## openSSL.server.verificationMode \{#openssl.server.verificationmode\}

<SettingsInfoBlock type="String" default_value="relaxed" changeable_without_restart="No" />Метод проверки сертификатов узла. Подробности см. в описании класса [Context](https://github.com/ClickHouse/poco/blob/master/NetSSL_OpenSSL/include/Poco/Net/Context.h). Возможные значения: `<none>`, `<relaxed>`, `<strict>`, `<once>`.
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Включить сбор метрик PSI из файлов /proc/pressure/.

## os_cpu_busy_time_threshold \{#os_cpu_busy_time_threshold\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="Нет" />Порог занятого времени CPU операционной системы в микросекундах (метрика OSCPUVirtualTimeMicroseconds), при превышении которого считается, что CPU выполняет полезную работу; перегрузка CPU не будет зафиксирована, если занятое время ниже этого значения.
...
<SettingsInfoBlock type="Double" default_value="0.15" changeable_without_restart="No" />Доля лимита памяти, которую следует резервировать, не используя под пользовательский кэш страниц (userspace page cache). Аналогично параметру Linux min&#95;free&#95;kbytes.

## page_cache_history_window_ms \{#page_cache_history_window_ms\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="Нет" />Задержка перед тем, как освобождённая память может быть использована кэшем страниц в пространстве пользователя.

## page_cache_max_size \{#page_cache_max_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальный размер кэша страниц в пользовательском пространстве. Установите 0, чтобы отключить кэш. Если значение больше page&#95;cache&#95;min&#95;size, размер кэша будет динамически изменяться в этих пределах, чтобы использовать большую часть доступной памяти, при этом удерживая общее потребление памяти ниже лимита (max&#95;server&#95;memory&#95;usage[&#95;to&#95;ram&#95;ratio]).
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэша страниц в пространстве пользователя.

## page_cache_shards \{#page_cache_shards\}

<SettingsInfoBlock type="UInt64" default_value="4" changeable_without_restart="No" />Распределяет пользовательский кэш страниц (userspace page cache) по указанному числу сегментов, чтобы уменьшить конкуренцию за мьютексы. Экспериментальная опция, маловероятно, что она даст прирост производительности.
...
<SettingsInfoBlock type="UInt64" default_value="5000" changeable_without_restart="No" />Максимальный размер кэша файлов метаданных Parquet, в элементах. Значение 0 означает, что кэш отключён.

## parquet_metadata_cache_policy \{#parquet_metadata_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэша метаданных Parquet.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (при использовании политики SLRU) в кэше метаданных Parquet относительно общего размера кэша.
...
<SettingsInfoBlock type="UInt64" default_value="30" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="128" changeable_without_restart="Нет" />

Потоки для очистки частей (parts killer threads) в общем хранилище SharedMergeTree. Доступно только в ClickHouse Cloud
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="No" />Размер фонового пула для предварительного чтения (prefetch) из удалённых объектных хранилищ

## prefetch_threadpool_queue_size \{#prefetch_threadpool_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />Количество задач, которые можно поставить в очередь пула потоков предварительного чтения
...
<SettingsInfoBlock type="Double" default_value="0.95" changeable_without_restart="No" />Доля общего размера кэша меток, которую нужно заполнить при предварительном прогреве.

## primary_index_cache_size \{#primary_index_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="5368709120" changeable_without_restart="Yes" />Максимальный размер кэша первичного индекса (индекса семейства таблиц MergeTree).

## primary_index_cache_size_ratio \{#primary_index_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (при использовании политики SLRU) в кэше первичного индекса по отношению к общему размеру кэша.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Экспортировать метрики, относящиеся к ClickHouse Keeper
...
<SettingsInfoBlock type="UInt64" default_value="1024" changeable_without_restart="Нет" />Максимальное количество результатов запросов SELECT, хранящихся в кэше.

## query_cache.max_entry_size_in_bytes \{#query_cache.max_entry_size_in_bytes\}

<SettingsInfoBlock type="UInt64" default_value="1048576" changeable_without_restart="No" />Максимальный допустимый размер в байтах результатов запроса SELECT, которые могут быть сохранены в кэше.

## query_cache.max_entry_size_in_rows \{#query_cache.max_entry_size_in_rows\}

<SettingsInfoBlock type="UInt64" default_value="30000000" changeable_without_restart="No" />Максимальное количество строк в результатах запроса SELECT, которые могут быть сохранены в кэше.

## query_cache.max_size_in_bytes \{#query_cache.max_size_in_bytes\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="Да" />Максимальный размер кэша в байтах. 0 означает, что кэш запросов отключен.

## query_condition_cache_policy \{#query_condition_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэша условий запроса.

## query_condition_cache_size \{#query_condition_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="104857600" changeable_without_restart="Да" />
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (в случае политики SLRU) в кэше условий запроса по отношению к общему размеру кэша.
...
<SettingsInfoBlock type="Seconds" default_value="0" changeable_without_restart="No" />Таймаут HTTP-соединения для запросов на выборку частей. Наследуется из профиля по умолчанию `http_connection_timeout`, если не задан явно.

## replicated_fetches_http_receive_timeout \{#replicated_fetches_http_receive_timeout\}

<SettingsInfoBlock type="Seconds" default_value="0" changeable_without_restart="No" />Таймаут ожидания получения HTTP-ответа для запросов на получение частей данных. Наследуется от профиля по умолчанию `http_receive_timeout`, если не задан явно.

## replicated_fetches_http_send_timeout \{#replicated_fetches_http_send_timeout\}

<SettingsInfoBlock type="Seconds" default_value="0" changeable_without_restart="Нет" />Тайм-аут отправки HTTP-запросов при получении частей. Наследуется из профиля по умолчанию `http_send_timeout`, если не задан явно.
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="16" changeable_without_restart="No" />Максимальное количество потоков при выполнении запросов RESTORE.

## s3_credentials_provider_max_cache_size \{#s3_credentials_provider_max_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="Нет" />Максимальное число провайдеров учётных данных S3, которые могут храниться в кэше

## s3_max_redirects \{#s3_max_redirects\}

<SettingsInfoBlock type="UInt64" default_value="10" changeable_without_restart="No" />Максимальное число допустимых переходов при перенаправлениях S3.

## s3_retry_attempts \{#s3_retry_attempts\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />Параметр для Aws::Client::RetryStrategy; повторные попытки выполняются самим Aws::Client, 0 — без повторных попыток

## s3queue_disable_streaming \{#s3queue_disable_streaming\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Yes" />Отключает потоковую обработку (streaming) в S3Queue, даже если таблица уже создана и к ней присоединены материализованные представления
...
<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Если параметр имеет значение true, в трассировках стека отображаются адреса

## shutdown_wait_backups_and_restores \{#shutdown_wait_backups_and_restores\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Если установлено значение true, ClickHouse будет дожидаться завершения выполняющихся операций резервного копирования и восстановления перед завершением работы.

## shutdown_wait_unfinished \{#shutdown_wait_unfinished\}

<SettingsInfoBlock type="UInt64" default_value="5" changeable_without_restart="No" />Время ожидания незавершённых запросов в секундах

## shutdown_wait_unfinished_queries \{#shutdown_wait_unfinished_queries\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Если установлено значение `true`, ClickHouse будет дожидаться завершения выполняющихся запросов перед остановкой.

## skip_binary_checksum_checks \{#skip_binary_checksum_checks\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="Нет" />Пропускает проверки целостности контрольных сумм исполняемого файла ClickHouse
...
<SettingsInfoBlock type="UInt64" default_value="120" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Отладочный параметр для моделирования задержки при создании материализованного представления

## startup_scripts.throw_on_error \{#startup_scripts.throw_on_error\}

<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Если параметр имеет значение `true`, сервер не запустится, если во время выполнения скрипта произойдет ошибка.
...
<SettingsInfoBlock type="UInt64" default_value="200000" changeable_without_restart="No" />При достижении этого лимита при попытке создания соединения выбрасывается исключение. Установите 0, чтобы отключить жесткий предел. Лимит применяется к подключениям к хранилищу.

## storage_connections_rcvbuf \{#storage_connections_rcvbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер опции SO&#95;RCVBUF для подключений к хранилищу (репликация, распределённые запросы). Если задано значение больше 0, оно переопределяет автонастройку TCP ядра для буфера приёма. 0 = значение ядра по умолчанию (автонастройка). Внимание: при изменении этой настройки обратно на 0 автонастройка восстанавливается только для вновь создаваемых подключений; существующие подключения из пула сохраняют фиксированный размер буфера, пока не будут пересозданы.

## storage_connections_sndbuf \{#storage_connections_sndbuf\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Yes" />Размер буфера SO&#95;SNDBUF для подключений к хранилищу (репликация, распределённые запросы). Если задано значение больше 0, оно переопределяет автоматическую настройку TCP ядра для буфера отправки. 0 = значение ядра по умолчанию (автоматическая настройка). Внимание: если изменить эту настройку обратно на 0, автоматическая настройка восстановится только для вновь создаваемых подключений; существующие подключения из пула сохранят фиксированные размеры буфера, пока не будут созданы заново.

## storage_connections_soft_limit \{#storage_connections_soft_limit\}

<SettingsInfoBlock type="UInt64" default_value="100" changeable_without_restart="Нет" />Подключения, превышающие этот лимит, имеют существенно более короткое время жизни. Лимит применяется к подключениям к хранилищу.

## storage_connections_store_limit \{#storage_connections_store_limit\}

<SettingsInfoBlock type="UInt64" default_value="1000" changeable_without_restart="No" />Подключения сверх этого лимита сбрасываются после использования. Установите значение 0, чтобы отключить кэш подключений. Лимит применяется к подключениям к хранилищам.

## storage_connections_warn_limit \{#storage_connections_warn_limit\}

<SettingsInfoBlock type="UInt64" default_value="500" changeable_without_restart="No" />Предупреждающие сообщения записываются в логи, если количество активных подключений превышает этот предел. Предел применяется к подключениям к хранилищам.

## storage_metadata_write_full_object_key \{#storage_metadata_write_full_object_key\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Записывать файлы метаданных диска в формате VERSION&#95;FULL&#95;OBJECT&#95;KEY. Включено по умолчанию. Настройка устарела.

## storage_shared_set_join_use_inner_uuid \{#storage_shared_set_join_use_inner_uuid\}

<SettingsInfoBlock type="Bool" default_value="1" changeable_without_restart="No" />Если параметр включён, при создании SharedSet и SharedJoin генерируется внутренний UUID. Только для ClickHouse Cloud.
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальное количество запросов по одному TCP‑соединению, после которого соединение будет закрыто. Установите значение 0 для неограниченного числа запросов.

## tcp_close_connection_after_queries_seconds \{#tcp_close_connection_after_queries_seconds\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Максимальное время жизни TCP‑соединения в секундах до его закрытия. Установите значение 0 для неограниченного времени жизни соединения.
...
<SettingsInfoBlock type="Bool" default_value="0" changeable_without_restart="No" />Хранит временные данные в распределённом кэше.

## text_index_header_cache_max_entries \{#text_index_header_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="100000" changeable_without_restart="No" />Размер кэша заголовка текстового индекса (в количестве элементов). Ноль отключает кэш.

## text_index_header_cache_policy \{#text_index_header_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэширования заголовков текстового индекса.

## text_index_header_cache_size \{#text_index_header_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="Yes" />Размер кэша заголовков текстового индекса. Ноль — отключено.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (при использовании политики SLRU) в кэше заголовков текстового индекса относительно общего размера кэша.

## text_index_postings_cache_max_entries \{#text_index_postings_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="No" />Размер кэша списков вхождений текстового индекса (в записях). Значение 0 означает отключение.
...
<SettingsInfoBlock type="UInt64" default_value="2147483648" changeable_without_restart="Yes" />Размер кэша списков вхождений текстового индекса. Ноль отключает кэш.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Относительный размер защищённой очереди (в случае политики SLRU) в кэше списков вхождений текстового индекса по отношению к общему размеру этого кэша.

## text_index_tokens_cache_max_entries \{#text_index_tokens_cache_max_entries\}

<SettingsInfoBlock type="UInt64" default_value="1000000" changeable_without_restart="Нет" />Размер кэша для токенов текстового индекса в элементах. Ноль — кэш отключён.
...
<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="Yes" />Размер кэша токенов текстового индекса. Значение 0 означает, что кэш отключён.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищенной очереди (в случае политики SLRU) в кэше токенов текстового индекса относительно общего размера кэша.
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="No" />Количество потоков в пуле для чтения из локальной файловой системы, когда `local_filesystem_read_method = 'pread_threadpool'`.
...
<SettingsInfoBlock type="NonZeroUInt64" default_value="250" changeable_without_restart="No" />Количество потоков в пуле потоков, используемом для чтения из удалённой файловой системы при `remote_filesystem_read_method = 'threadpool'`.

## threadpool_remote_fs_reader_queue_size \{#threadpool_remote_fs_reader_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="No" />Максимальное количество задач, которые может быть запланировано в пуле потоков для чтения из удалённой файловой системы.

## threadpool_writer_pool_size \{#threadpool_writer_pool_size\}

<SettingsInfoBlock type="NonZeroUInt64" default_value="100" changeable_without_restart="Нет" />Размер фонового пула для обработки запросов на запись в объектные хранилища

## threadpool_writer_queue_size \{#threadpool_writer_queue_size\}

<SettingsInfoBlock type="UInt64" default_value="10000" changeable_without_restart="Нет" />Количество задач, которые можно поместить в фоновый пул для обработки запросов на запись в объектные хранилища
...
<SettingsInfoBlock type="String" default_value="/var/lib/clickhouse/top_level_domains/" changeable_without_restart="Нет" />
...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Собирать случайные выделения памяти размером меньше либо равным указанному значению с вероятностью, равной `total_memory_profiler_sample_probability`. 0 означает, что параметр отключён. Имеет смысл установить `max_untracked_memory` в 0, чтобы этот порог работал как ожидается.

## total_memory_profiler_sample_min_allocation_size \{#total_memory_profiler_sample_min_allocation_size\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="Нет" />Случайным образом собирать выделения памяти размером не менее указанного значения с вероятностью `total_memory_profiler_sample_probability`. 0 означает, что функция отключена. Имеет смысл установить `max_untracked_memory` в 0, чтобы этот порог работал как ожидается.

## total_memory_profiler_step \{#total_memory_profiler_step\}

<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Каждый раз, когда использование памяти сервером превышает очередной шаг (в байтах), профилировщик памяти собирает стек трассировки по месту выделения. Нулевое значение означает, что профилировщик памяти отключен. Значения меньше нескольких мегабайт будут замедлять работу сервера.
...
<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэша несжатых данных.
...
<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (в случае политики SLRU) в несжатом кэше относительно общего размера этого кэша.

## unique_key_index_cache_policy \{#unique_key_index_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэширования для индекса UNIQUE KEY (SLRU или LRU).

## unique_key_index_cache_size_bytes \{#unique_key_index_cache_size_bytes\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" changeable_without_restart="No" />Максимальный размер кэша в памяти процесса для блоков индекса UNIQUE KEY (SST), в байтах. Установите 0, чтобы отключить кэш.

## unique_key_index_cache_size_ratio \{#unique_key_index_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" changeable_without_restart="No" />Размер защищённой очереди (при политике SLRU) в кэше индекса UNIQUE KEY по отношению к общему размеру кэша.
...
Каталог с файлами пользовательских скриптов. Используется для исполняемых пользовательских функций [исполняемых пользовательских функций](/sql-reference/functions/udf#executable-user-defined-functions).
...
<SettingsInfoBlock type="UInt64" default_value="10000000" changeable_without_restart="No" />Размер кэша для индекса векторного сходства в элементах. Ноль означает, что кэш отключён.

## vector_similarity_index_cache_policy \{#vector_similarity_index_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" changeable_without_restart="No" />Имя политики кэширования индекса векторного сходства.

## vector_similarity_index_cache_size \{#vector_similarity_index_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="5368709120" changeable_without_restart="Yes" />Размер кэша индексов векторного сходства. 0 — отключено.
