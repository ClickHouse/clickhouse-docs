...
<SettingsInfoBlock type="UInt64" default_value="0" changeable_without_restart="No" />Размер буфера сокета SO&#95;RCVBUF для подключений к диску (S3, Azure, GCS). Если задано значение больше 0, оно переопределяет автонастройку TCP в ядре для буфера приёма. 0 = значение ядра по умолчанию (автонастройка). Примечание: если изменить эту настройку обратно на 0, автонастройка восстановится только для вновь создаваемых соединений; существующие соединения в пуле сохранят фиксированные размеры буфера, пока не будут пересозданы.
