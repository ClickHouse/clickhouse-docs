...
| `throw_on_unmatched_row_policies`               | Определяет, должно ли чтение из таблицы выбрасывать исключение, если для таблицы заданы политики строк (ROW POLICY), но ни одна из них не относится к текущему пользователю.                                                                                                                                                                                                                                                                                                                                                                                                     | `false` |
...
Это значение следует использовать в качестве значения по умолчанию для обеспечения обратной совместимости. Все существующие экземпляры ClickHouse должны использовать это значение, чтобы избежать ломающих изменений.
...
<SettingsInfoBlock type="UInt64" default_value="5000" />Максимальный размер кэша метаданных файлов формата Parquet в элементах. Значение 0 означает, что кэш отключен.

## parquet_metadata_cache_policy \{#parquet_metadata_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />Название политики кэша метаданных Parquet.

## parquet_metadata_cache_size \{#parquet_metadata_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="536870912" />Максимальный размер кэша метаданных Parquet в байтах. Нулевое значение отключает кэш.

## parquet_metadata_cache_size_ratio \{#parquet_metadata_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" />Размер защищённой очереди (в случае политики SLRU) в кэше метаданных Parquet по отношению к общему размеру кэша.
