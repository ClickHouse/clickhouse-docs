...
| `disallow_config_defined_profiles_for_sql_defined_users` | Если параметр включён, запрещает применять к пользователям, определённым через SQL, профили настроек, заданные в config (то есть определённые в XML-файлах конфигурации, таких как `users.xml`), кроме профиля по умолчанию. Также проверяются профили, назначенные ролям, выданным пользователю, определённому через SQL.                                                                                                                                                                                                                                            | `false` |
...
<SettingsInfoBlock type="Bool" default_value="0" />Включает или отключает использование функции IMPERSONATE (EXECUTE AS target_user). Настройка устарела.
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Количество задач, которые можно поместить в очередь пула потоков каталога Iceberg
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Количество задач, которые можно поставить в очередь пула предварительной выборки
...
Пороговое значение доли «грязных» страниц jemalloc от объёма памяти, доступной серверу ClickHouse. Когда размер «грязных» страниц превышает эту долю, фоновый рабочий поток памяти принудительно очищает «грязные» страницы. Если установлено значение 0, принудительная очистка, основанная на доле «грязных» страниц, отключается.

## memory_worker_purge_total_memory_threshold_ratio \{#memory_worker_purge_total_memory_threshold_ratio\}

<SettingsInfoBlock type="Double" default_value="0.9" />

Пороговое значение доли общего использования памяти по отношению к объёму памяти, доступной серверу ClickHouse, при превышении которого выполняется очистка jemalloc. Когда общее использование памяти превышает эту долю, фоновый рабочий поток памяти принудительно очищает «грязные» страницы. Если установлено значение 0, принудительная очистка на основе общего объёма памяти отключается.
...
<SettingsInfoBlock type="UInt64" default_value="5000" />Максимальный размер кэша файлов метаданных Parquet, в элементах. Значение 0 означает, что кэш отключён.

## parquet_metadata_cache_policy \{#parquet_metadata_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />Имя политики кэша метаданных Parquet.

## parquet_metadata_cache_size \{#parquet_metadata_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="536870912" />Максимальный размер кэша метаданных Parquet в байтах. Значение 0 означает, что кэш отключён.

## parquet_metadata_cache_size_ratio \{#parquet_metadata_cache_size_ratio\}

<SettingsInfoBlock type="Double" default_value="0.5" />Размер защищённой очереди (при использовании политики SLRU) в кэше метаданных Parquet относительно общего размера кэша.
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Количество задач, которые можно поставить в очередь пула потоков предварительной выборки
...
<SettingsInfoBlock type="UInt64" default_value="1000000" />Размер кэша для токенов текстового индекса в элементах. Ноль — кэш отключён.

## text_index_tokens_cache_policy \{#text_index_tokens_cache_policy\}

<SettingsInfoBlock type="String" default_value="SLRU" />Имя политики кэширования токенов текстового индекса.

## text_index_tokens_cache_size \{#text_index_tokens_cache_size\}

<SettingsInfoBlock type="UInt64" default_value="1073741824" />Размер кэша токенов текстового индекса. Значение 0 означает, что кэш отключён.
...
<SettingsInfoBlock type="Double" default_value="0.5" />Размер защищенной очереди (в случае политики SLRU) в кэше токенов текстового индекса относительно общего размера кэша.
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Максимальное количество задач, которые могут быть поставлены в очередь пула потоков для чтения из локальной файловой системы.
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Максимальное количество задач, которые может быть запланировано в пуле потоков для чтения из удалённой файловой системы.
...
<SettingsInfoBlock type="UInt64" default_value="10000" />Количество задач, которые можно поместить в фоновый пул для обработки запросов на запись в объектные хранилища
