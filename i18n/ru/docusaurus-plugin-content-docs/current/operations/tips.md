...
Всегда устанавливайте для прозрачных больших страниц значение `madvise`. В старых версиях ядра (до 5.9) значение THP `always` может вызывать значительное снижение производительности — ядро тратит слишком много времени на дефрагментацию памяти, особенно в системах с 64 ГБ+ оперативной памяти. В ядре 5.9 была добавлена proactive compaction, которая значительно лучше справляется с THP, однако ClickHouse по-прежнему выводит предупреждение при запуске, если для THP установлено значение `always`, поэтому `madvise` остаётся рекомендуемой настройкой независимо от версии ядра.

```bash
$ echo 'madvise' | sudo tee /sys/kernel/mm/transparent_hugepage/enabled
```

Если вы хотите изменить настройку прозрачных больших страниц на постоянной основе, отредактируйте `/etc/default/grub`, добавив `transparent_hugepage=madvise` в опцию `GRUB_CMDLINE_LINUX_DEFAULT`:
