...
tickTime=2000
# Количество тиков, которое может занять начальная
# фаза синхронизации
...
# Максимальное значение, которое может запросить клиент и которое примет сервер.
# Допустимо устанавливать высокое значение maxSessionTimeout на сервере, чтобы клиенты могли работать с большим таймаутом сессии при необходимости.
# По умолчанию запрашивается таймаут сессии 30 секунд (можно изменить с помощью параметра session_timeout_ms в конфигурации ClickHouse).
...
# Чтобы избежать операций поиска, ZooKeeper выделяет место в файле журнала транзакций
# блоками размером preAllocSize килобайт. Размер блока по умолчанию — 64M. Одна из причин
# изменения размера блоков — уменьшение размера блока при более частом создании снимков состояния.
# (См. также snapCount).
preAllocSize=131072

# Клиенты могут отправлять запросы быстрее, чем ZooKeeper способен их обработать,
# особенно при большом количестве клиентов. Чтобы предотвратить исчерпание памяти
# из-за запросов в очереди, ZooKeeper ограничивает клиентов так, чтобы
# в системе находилось не более globalOutstandingLimit необработанных запросов.
# Ограничение по умолчанию — 1000.
# globalOutstandingLimit=1000

# ZooKeeper записывает транзакции в журнал транзакций. После записи snapCount транзакций
# в файл журнала создается снимок состояния и начинается новый файл журнала транзакций.
# Значение snapCount по умолчанию — 100000.
snapCount=3000000

# Если эта опция определена, запросы будут записываться в файл трассировки с именем
# traceFile.year.month.day.
#traceFile=

# Лидер принимает клиентские соединения. Значение по умолчанию — "yes". Узел-лидер
# координирует обновления. Для повышения пропускной способности обновлений за счет незначительного снижения
# пропускной способности чтения лидер может быть настроен так, чтобы не принимать клиентов и сосредоточиться
# на координации.
leaderServes=yes
...
# TODO это выглядит неаккуратно
# Как узнать, какие jar-файлы требуются?
# Судя по всему, log4j требует наличия файла log4j.properties в classpath
