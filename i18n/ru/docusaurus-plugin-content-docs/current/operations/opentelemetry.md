...
ClickHouse поддерживает трассировку с использованием OpenTelemetry для запросов [ClickHouse Keeper](../guides/sre/keeper/index.md) (службы координации, совместимой с ZooKeeper). Эта функция обеспечивает детальную наблюдаемость жизненного цикла операций Keeper — от отправки клиентского запроса до обработки на стороне сервера.

### Включение трассировки Keeper \{#enabling-keeper-tracing\}
