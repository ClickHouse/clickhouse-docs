...
Таблица `system.part_log` создается только в том случае, если задан параметр сервера [part_log](/operations/server-configuration-parameters/settings#part_log).
...
- `event_type` ([Enum8](../../sql-reference/data-types/enum.md)) — Тип события, которое произошло с частью данных. Может принимать одно из следующих значений:
...
- `merge_reason` ([Enum8](../../sql-reference/data-types/enum.md)) — Причина события с типом `MERGE_PARTS`. Может принимать одно из следующих значений:
    - `NotAMerge` — Текущие событие имеет тип, отличный от `MERGE_PARTS`.
...
- `partition_id` ([String](../../sql-reference/data-types/string.md)) — ID партиции, в которую была вставлена часть данных. Колонка принимает значение `all`, если партиционирование составляет `tuple()`.
...
- `merged_from` ([Array(String)](../../sql-reference/data-types/array.md)) — Массив имен частей, из которых была составлена текущая часть (после слияния).
...
- `error` ([UInt16](../../sql-reference/data-types/int-uint.md)) — Номер кода возникшей ошибки.
- `exception` ([String](../../sql-reference/data-types/string.md)) — Текстовое сообщение об возникшей ошибке.
