---
description: 'Системная таблица, содержащая информацию о событиях, которые произошли с
...
Таблица `system.part_log` создается только если задана серверная настройка [part_log](/operations/server-configuration-parameters/settings#part_log).
...
- `event_type` ([Enum8](../../sql-reference/data-types/enum.md)) — Тип события, произошедшего с частью данных. Может иметь одно из следующих значений:
    - `NewPart` — Вставка новой части данных.
    - `MergePartsStart` — Начато слияние частей данных.
    - `MergeParts` — Слияние частей данных завершено.
    - `DownloadPart` — Загрузка части数据.
...
- `merge_reason` ([Enum8](../../sql-reference/data-types/enum.md)) — Причина события с типом `MERGE_PARTS`. Может иметь одно из следующих значений:
    ...
    - `TTLRecompressMerge` — Рекомпрессия части данных.
...
- `partition_id` ([String](../../sql-reference/data-types/string.md)) — ID раздела, в который была вставлена часть данных. Столбец принимает значение `all`, если шардирование по `tuple()`.
...
- `merged_from` ([Array(String)](../../sql-reference/data-types/array.md)) — Массив имен частей, из которых состояла текущая часть (после слияния).
- `bytes_uncompressed` ([UInt64](../../sql-reference/data-types/int-uint.md)) — Размер данных в несжатом виде.
...
- `error` ([UInt16](../../sql-reference/data-types/int-uint.md)) — Номер кода произошедшей ошибки.
- `exception` ([String](../../sql-reference/data-types/string.md)) — Текстовое сообщение произошедшей ошибки.
