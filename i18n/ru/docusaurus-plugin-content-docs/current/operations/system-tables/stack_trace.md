...
* `untracked_memory` ([Int64](../../sql-reference/data-types/)) — Счётчик выделений памяти для потока, которые ещё не переданы родительскому MemoryTracker. Может быть отрицательным, если с момента последнего сброса было освобождено больше памяти, чем выделено.
