...
Содержит стек вызовов всех потоков сервера. Позволяет разработчикам исследовать состояние сервера.
...
* `thread_name` ([String](../../sql-reference/data-types/)) — Имя потока.
* `thread_id` ([UInt64](../../sql-reference/data-types/)) — Идентификатор потока.
* `query_id` ([String](../../sql-reference/data-types/)) — Идентификатор запроса, к которому относится этот поток.
* `trace` ([Array(UInt64)](../../sql-reference/data-types/)) — Стек вызовов этого потока. По сути, это просто массив адресов.
* `untracked_memory` ([Int64](../../sql-reference/data-types/)) — Неатомарный счётчик выделений памяти для потока, которые ещё не переданы родительскому MemoryTracker. Может быть отрицательным, если с момента последнего сброса было освобождено больше памяти, чем выделено.
...
* [system.trace&#95;log](../system-tables/trace_log.md) — Содержит стек вызовов, собранные профилировщиком запросов с выборкой.
