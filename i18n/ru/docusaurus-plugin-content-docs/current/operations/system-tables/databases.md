...
* `is_external` ([UInt8](../../sql-reference/data-types/)) — Признак того, что база данных является внешней (например, PostgreSQL/DataLakeCatalog).

{/*AUTOGENERATED_END*/ }

**Псевдонимы:**

* `database` — псевдоним `name`.

Столбец `name` из этой системной таблицы используется при выполнении запроса `SHOW DATABASES`.
