...
* `hostname` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — Имя узла сервера, выполняющего запрос.
...
* `metric` ([LowCardinality(String)](/sql-reference/data-types/lowcardinality)) — Название метрики.
* `value` ([Float64](/sql-reference/data-types/float)) — Значение метрики.
