...
Данные профилирования можно преобразовать в формат трассировки событий Chrome с помощью следующего запроса. Сохраните этот запрос в файл `chrome_trace.sql`:
...
И выполнить его с помощью ClickHouse Client, чтобы экспортировать в файл `trace.json`, который затем можно импортировать либо в [Perfetto](https://ui.perfetto.dev/), либо в [speedscope](https://www.speedscope.app/).

```bash
echo $(clickhouse client --query "$(cat chrome_trace.sql)") > trace.json
```

Мы можем опустить стек, если нам нужна более компактная, но менее информативная трассировка.
