...
| Страница | Описание |
|-----|-----|
| [system.backup_log](/operations/system-tables/backup_log) | Системная таблица, содержащая записи логов с информацией о операциях `BACKUP` и `RESTORE`. |
...
| [system.distribution_queue](/operations/system-tables/distribution_queue) | Системная таблица, содержащая информацию о локальных файлах, находящихся в очереди на отправку на шарды. |
...
| [system.enabled_roles](/operations/system-tables/enabled-roles) | Системная таблица, содержащая все активные роли на данный момент, включая текущую роль текущего пользователя и предоставленные роли для текущей роли. |
...
| [system.blob_storage_log](/operations/system-tables/blob_storage_log) | Системная таблица, содержащая записи логов с информацией о различных операциях с blob-хранилищем, таких как загрузки и удаления. |
...
| [system.table_engine](/operations/system-tables/table_engines) | Системная таблица, содержащая описания движков таблиц, поддерживаемых сервером, и их возможностей. |
| [system.processes](/operations/system-tables/processes) | Системная таблица, используемая для выполнения запроса `SHOW PROCESSLIST`. |
| [system.columns](/operations/system-tables/columns) | Системная таблица, содержащая информацию о столбцах во всех таблицах. |
| [system.quota_usage](/operations/system-tables/quota_usage) | Системная таблица, содержащая информацию об использовании квоты текущим пользователем, такой как сколько квоты использовано и сколько осталось. |
...
| [system.quotas_usage](/operations/system-tables/quotas_usage) | Системная таблица, содержащая информацию о использовании квот всеми пользователями. |
...
| [system.opentelemetry_span_log](/operations/system-tables/opentelemetry_span_log) | Системная таблица, содержащая информацию о трассировочных отрезках для выполненных запросов. |
| [system.s3_queue_settings](/operations/system-tables/s3_queue_settings) | Системная таблица, содержащая информацию о параметрах таблиц S3Queue. Доступно с версии сервера `24.10`. |
...
| [system.symbols](/operations/system-tables/symbols) | Системная таблица, полезная для экспертов по C++ и инженеров ClickHouse, содержащая информацию для интроспекции двоичного файла `clickhouse`. |
| [system.distributed_ddl_queue](/operations/system-tables/distributed_ddl_queue) | Системная таблица, содержащая информацию о распределенных ddl запросах (запросах, использующих оператор ON CLUSTER), которые были выполнены на кластере. |
...
| [system.asynchronous_loader](/operations/system-tables/asynchronous_loader) | Системная таблица, содержащая информацию о недавних асинхронных заданиях (например, для таблиц, которые загружаются). Таблица содержит строку для каждого задания. |
...
| [system.jemalloc_bins](/operations/system-tables/jemalloc_bins) | Системная таблица, содержащая информацию о распределении памяти, выполненном через распределитель jemalloc по различным классам размеров (bins), агрегированная из всех арен. |
| [system.dns_cache](/operations/system-tables/dns_cache) | Системная таблица, содержащая информацию о кэшированных DNS записях. |
| [system.query_thread_log](/operations/system-tables/query_thread_log) | Системная таблица, содержащая информацию о потоках, которые выполняют запросы, например, имя потока, время начала потока и продолжительность обработки запроса. |
...
| [system.merges](/operations/system-tables/merges) | Системная таблица, содержащая информацию о слияниях и мутациях частей, которые в настоящее время находятся в процессе для таблиц в семейства MergeTree. |
...
| [system.azure_queue_settings](/operations/system-tables/azure_queue_settings) | Системная таблица, содержащая информацию о параметрах таблиц AzureQueue. Доступно с версии сервера `24.10`. |
...
| [system.scheduler](/operations/system-tables/scheduler) | Системная таблица, содержащая информацию о состоянии узлов планирования, находящихся на локальном сервере. |
...
| [system.replicated_fetches](/operations/system-tables/replicated_fetches) | Системная таблица, содержащая информацию о текущих запущенных фоновомfetches. |
...
| [system.trace_log](/operations/system-tables/trace_log) | Системная таблица, содержащая стековые трассировки, собранные профилировщиком запросов выборки. |
...
| [system.replicas](/operations/system-tables/replicas) | Системная таблица, содержащая информацию о и состоянии реплицированных таблиц, находящихся на локальном сервере. Полезно для мониторинга. |
...
| [system.dropped_tables](/operations/system-tables/dropped_tables) | Системная таблица, содержащая информацию о таблицах, для которых была выполнена команда drop table, но очистка данных еще не была выполнена. |
...
| [system.moves](/operations/system-tables/moves) | Системная таблица, содержащая информацию о перемещениях частей данных, находящихся в процессе для таблиц MergeTree. Каждое перемещение части данных представлено одной строкой. |
...
| [system.one](/operations/system-tables/one) | Системная таблица, содержащая одну строку с единственным `dummy` UInt8 столбцом, содержащим значение 0. Похоже на таблицу `DUAL`, встречающуюся в других СУБД. |
...
| [system.stack_trace](/operations/system-tables/stack_trace) | Системная таблица, которая содержит стековые трассировки всех потоков сервера. Позволяет разработчикам интроспектировать состояние сервера. |
...
| [system.zookeeper_connection](/operations/system-tables/zookeeper_connection) | Системная таблица, которая существует только если ZooKeeper настроен. Показывает текущие подключения к ZooKeeper (включая вспомогательных ZooKeeper). |
...
| [system.settings_profiles](/operations/system-tables/settings_profiles) | Системная таблица, которая содержит свойства настроенных профилей настроек. |
| [system.zookeeper](/operations/system-tables/zookeeper) | Системная таблица, которая существует только если ClickHouse Keeper или ZooKeeper настроены. Она отображает данные из кластера Keeper, определенного в конфигурации. |
