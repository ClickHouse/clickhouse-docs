...
description: 'Системная база данных, предоставляющая почти стандартизированное,
  не зависящее от СУБД представление о метаданных объектов базы данных.'
...
Содержит столбцы, считываемые из системной таблицы [system.columns](../../operations/system-tables/columns.md), а также столбцы, которые не поддерживаются в ClickHouse или не имеют смысла (всегда `NULL`), но должны присутствовать в соответствии со стандартом.
