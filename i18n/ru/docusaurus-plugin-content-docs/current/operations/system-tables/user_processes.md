---
description: 'Системная таблица, содержащая информацию, полезную для общего обзора использования памяти и событий ProfileEvents пользователей.'
...
Эта системная таблица используется для получения обзора использования памяти и событий ProfileEvents пользователей.
...
- `memory_usage` ([Int64](../../sql-reference/data-types/)) — Суммарный объём оперативной памяти, используемой всеми процессами пользователя. Может не включать некоторые типы выделенной памяти. См. настройку max_memory_usage.
- `peak_memory_usage` ([Int64](../../sql-reference/data-types/)) — Пиковое значение использования памяти пользователем. Может быть сброшено, когда для пользователя не выполняются запросы.
- `ProfileEvents` ([Map(String, UInt64)](../../sql-reference/data-types/)) — Сводка событий ProfileEvents, которые измеряют различные метрики для пользователя. Их описание можно найти в таблице system.events.
<!--AUTOGENERATED_END-->
