...
* `memory_usage` ([Int64](../../sql-reference/data-types/)) — Суммарный объём оперативной памяти, используемый всеми процессами пользователя. Может не включать некоторые типы выделенной памяти. См. настройку `max_memory_usage`.
* `peak_memory_usage` ([Int64](../../sql-reference/data-types/)) — Пиковое использование памяти пользователем. Может быть сброшено, когда для пользователя не выполняются запросы.
* `ProfileEvents` ([Map(LowCardinality(String), UInt64)](../../sql-reference/data-types/)) — Сводка значений `ProfileEvents`, измеряющих различные метрики для пользователя. Их описание можно найти в таблице `system.events`.
