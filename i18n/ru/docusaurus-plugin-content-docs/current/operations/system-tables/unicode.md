...
Имена свойств кодовых точек Unicode из ICU преобразуются в `snake_case` и используются в качестве имен столбцов.

## Столбцы \{#columns\}

{/*AUTOGENERATED_START*/ }

* `code_point` ([String](../../sql-reference/data-types/)) — Кодовая точка Unicode в виде U+XXXX.
* `code_point_value` ([Int32](../../sql-reference/data-types/)) — Целочисленное значение кодовой точки Unicode.
* `notation` ([String](../../sql-reference/data-types/)) — Обозначение символа (визуальное представление кодовой точки).
