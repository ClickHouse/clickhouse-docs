---
description: 'Системная таблица, которая существует только если ZooKeeper настроен. Показывает текущие соединения с ZooKeeper (включая вспомогательные экземпляры ZooKeeper).'
...
Эта таблица отсутствует, если ZooKeeper не настроен. Таблица &#39;system.zookeeper&#95;connection&#39; отображает текущие подключения к ZooKeeper (включая вспомогательные ZooKeeper). Каждая строка содержит информацию об одном подключении.
...
* `index` ([Nullable(UInt8)](../../sql-reference/data-types/int-uint.md)) — Индекс узла ZooKeeper, к которому подключился ClickHouse. Индекс берётся из конфигурации ZooKeeper. Если подключение отсутствует, в этом столбце значение NULL.
...
* `client_id` ([Int64](../../sql-reference/data-types/int-uint.md)) — Идентификатор сессии для соединения.
* `xid` ([Int64](../../sql-reference/data-types/int-uint.md)) — XID текущей сессии.
* `enabled_feature_flags` ([Array(Enum16)](../../sql-reference/data-types/array.md)) — Включённые флаги возможностей. Применимо только к ClickHouse Keeper. Возможные значения: `FILTERED_LIST`, `MULTI_READ`, `CHECK_NOT_EXISTS`, `CREATE_IF_NOT_EXISTS`, `REMOVE_RECURSIVE`.
