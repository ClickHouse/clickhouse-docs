...
sed -i.bak 's/Limit the max number of partitions that can be accessed in one query. <= 0 means unlimited./Limit the max number of partitions that can be accessed in one query. `<=` 0 means unlimited./g; s/\(this merge is created when set min_age_to_force_merge_seconds > 0 and min_age_to_force_merge_on_partition_only = true\)/(this merge is created when set `min_age_to_force_merge_seconds > 0` and `min_age_to_force_merge_on_partition_only = true`)/g; s/"If >= 1, columns will be always written in full serialization\."/"If `>= 1`, columns will be always written in full serialization."/g; s/<candidate partitions for mutations only \(partitions that cannot be merged\)\/>/<candidate partitions for mutations>/`<candidate partitions for mutations only (partitions that cannot be merged)>/<candidate partitions for mutations>`/g; s/<= 0 means unlimited/`<= 0` means unlimited/g' generated_merge_tree_settings.md
