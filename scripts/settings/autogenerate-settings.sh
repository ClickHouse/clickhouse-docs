...
INTO OUTFILE '$root/docs/en/operations/settings/settings-formats.md' TRUNCATE FORMAT LineAsString
...
' AS prefix
SELECT prefix || (SELECT groupConcat(*) FROM main_content)
INTO OUTFILE '$root/docs/en/operations/settings/settings.md' TRUNCATE FORMAT LineAsString
