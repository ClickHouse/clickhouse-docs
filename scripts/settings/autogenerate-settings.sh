...
if [ "$GENERATE_SETTINGS" = "true" ]; then
    ...
    sql_files_found=$(find "$SCRIPT_DIR" -maxdepth 1 -name '*.sql' ! -name 'generate-functions.sql' ! -name 'system-tables.sql' ! -name 'generate-aggregate-functions.sql' ! -name 'generate-events-list.sql' ! -name 'generate-metrics-list.sql' -print -quit)
if [ -z "$sql_files_found" ]; then
    ...
    for SQL_FILE in "$SCRIPT_DIR"/*.sql; do
      if [ -f "$SQL_FILE" ] && [ "$(basename "$SQL_FILE")" != "generate-functions.sql" ] && [ "$(basename "$SQL_FILE")" != "system-tables.sql" ] && [ "$(basename "$SQL_FILE")" != "generate-aggregate-functions.sql" ] && [ "$(basename "$SQL_FILE")" != "generate-events-list.sql" ] && [ "$(basename "$SQL_FILE")" != "generate-metrics-list.sql" ]; then
...
if [ -f "$SYSTEM_TABLES_SQL_FILE" ]; then
    ...
    echo "[$SCRIPT_NAME] Skipping system tables generation."
fi

# --- Insert generated content between AUTOGENERATED tags ---
echo "[$SCRIPT_NAME] Inserting system tables content between AUTOGENERATED_START and AUTOGENERATED_END tags"
...
# --- Generate ProfileEvents (system.events) descriptions list ---
echo "[$SCRIPT_NAME] Generating ProfileEvents descriptions list..."
EVENTS_LIST_SQL_FILE="$SCRIPT_DIR/generate-events-list.sql"
if [ -f "$EVENTS_LIST_SQL_FILE" ]; then
    "$script_path" --queries-file "$EVENTS_LIST_SQL_FILE" || {
        echo "[$SCRIPT_NAME] Warning: Failed to generate events descriptions list."
    }
    if [ -f "temp-events-list.md" ]; then
        dest_full_path="$target_dir/docs/operations/system-tables/events.md"
        if [ -f "$dest_full_path" ]; then
            # Add description tags if they don't exist (source files come from ClickHouse repo)
            if ! grep -q "<!--AUTOGENERATED_DESCRIPTIONS_START-->" "$dest_full_path"; then
                echo "[$SCRIPT_NAME] Adding AUTOGENERATED_DESCRIPTIONS tags to events.md"
...
' "$dest_full_path"
            ...
            echo "[$SCRIPT_NAME] Inserting ProfileEvents descriptions into $dest_full_path"
            insert_content_between_tags "./temp-events-list.md" "$dest_full_path" "<!--AUTOGENERATED_DESCRIPTIONS_START-->" "<!--AUTOGENERATED_DESCRIPTIONS_END-->"
        else
            echo "[$SCRIPT_NAME] Warning: events.md not found at $dest_full_path. Skipping."
        fi
        rm -f "temp-events-list.md"
    else
        echo "[$SCRIPT_NAME] Warning: temp-events-list.md not generated."
    fi
else
    echo "[$SCRIPT_NAME] Warning: Events list SQL file not found at $EVENTS_LIST_SQL_FILE"
fi

# --- Generate CurrentMetrics (system.metrics) descriptions list ---
echo "[$SCRIPT_NAME] Generating CurrentMetrics descriptions list..."
METRICS_LIST_SQL_FILE="$SCRIPT_DIR/generate-metrics-list.sql"
if [ -f "$METRICS_LIST_SQL_FILE" ]; then
    "$script_path" --queries-file "$METRICS_LIST_SQL_FILE" || {
        echo "[$SCRIPT_NAME] Warning: Failed to generate metrics descriptions list."
    }
    if [ -f "temp-metrics-list.md" ]; then
        dest_full_path="$target_dir/docs/operations/system-tables/metrics.md"
        if [ -f "$dest_full_path" ]; then
            # Add description tags if they don't exist (source files come from ClickHouse repo)
            if ! grep -q "<!--AUTOGENERATED_DESCRIPTIONS_START-->" "$dest_full_path"; then
                echo "[$SCRIPT_NAME] Adding AUTOGENERATED_DESCRIPTIONS tags to metrics.md"
                # Remove any existing hardcoded "Metric descriptions" section (from ClickHouse repo)
                # and replace with autogenerated tags
                if grep -q "^## Metric descriptions" "$dest_full_path"; then
                    echo "[$SCRIPT_NAME] Replacing existing hardcoded Metric descriptions section"
                    awk '
                        /^## Metric descriptions/ { in_section=1; next }
                        in_section && /^\*\*See Also\*\*/ { in_section=0; print "## Metric descriptions {#metric-descriptions}"; print ""; print "<!--AUTOGENERATED_DESCRIPTIONS_START-->"; print "<!--AUTOGENERATED_DESCRIPTIONS_END-->"; print ""; print $0; next }
                        in_section { next }
                        { print }
...
' "$dest_full_path"
            ...
            echo "[$SCRIPT_NAME] Inserting CurrentMetrics descriptions into $dest_full_path"
            insert_content_between_tags "./temp-metrics-list.md" "$dest_full_path" "<!--AUTOGENERATED_DESCRIPTIONS_START-->" "<!--AUTOGENERATED_DESCRIPTIONS_END-->"
        else
            echo "[$SCRIPT_NAME] Warning: metrics.md not found at $dest_full_path. Skipping."
        fi
        rm -f "temp-metrics-list.md"
    else
        echo "[$SCRIPT_NAME] Warning: temp-metrics-list.md not generated."
    fi
else
    echo "[$SCRIPT_NAME] Warning: Metrics list SQL file not found at $METRICS_LIST_SQL_FILE"
fi
