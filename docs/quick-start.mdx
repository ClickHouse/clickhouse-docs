...
an `ENGINE` clause. Use [`MergeTree`](/docs/engines/table-engines/mergetree-family/mergetree)
...
(tens of thousands or even millions at once) to minimize the number of [**parts**](/docs/parts)
...
The next step is to get your own data into ClickHouse. We have lots of [table functions](/docs/sql-reference/table-functions/index.md)
and [integrations](/docs/integrations) for ingesting data. We have some examples in the tabs
below, or you can check out our [Integrations](/docs/integrations) page for a long list of
technologies that integrate with ClickHouse.

<Tabs groupId="read_data">
<TabItem value="S3" label="S3" default>

Use the [`s3` table function](/docs/sql-reference/table-functions/s3.md) to
...
View our [collection of AWS S3 documentation pages](/docs/integrations/data-ingestion/s3/index.md) for lots more details and examples of using S3 with ClickHouse.
<br/>
</TabItem>
<TabItem value="GCS" label="GCS">

The [`s3` table function](/docs/sql-reference/table-functions/s3.md) used for
...
Find more details on the [`s3` table function page](/docs/sql-reference/table-functions/s3.md).
<br/>
</TabItem>
<TabItem value="URL" label="Web">

The [`url` table function](/docs/sql-reference/table-functions/url) reads
...
Find more details on the [`url` table function page](/docs/sql-reference/table-functions/url).
<br/>
</TabItem>
<TabItem value="local_file" label="Local">

Use the [`file` table engine](/docs/sql-reference/table-functions/file) to
...
View the [`file` table function](/docs/sql-reference/table-functions/file)
docs page for more details.
<br/>
</TabItem>
<TabItem value="PostgreSQL" label="PostgreSQL">

Use the [`postgresql` table function](/en/sql-reference/table-functions/postgresql)
...
View the [`postgresql` table function](/docs/sql-reference/table-functions/postgresql)
docs page for more details.
<br/>
</TabItem>
<TabItem value="MySQL" label="MySQL">

Use the [`mysql` table function](/docs/sql-reference/table-functions/mysql)
...
View the [`mysql` table function](/docs/sql-reference/table-functions/mysql)
docs page for more details.
<br/>
</TabItem>
...
View the [`odbc` table function](/docs/sql-reference/table-functions/odbc)
and the [`jdbc` table function](/docs/sql-reference/table-functions/jdbc) docs
...
- **Kafka**: integrate with Kafka using the [`Kafka` table engine](/docs/engines/table-engines/integrations/kafka)
- **Amazon MSK**: integrate with [Amazon Managed Streaming for Apache Kafka (MSK)](/docs/integrations/kafka/cloud/amazon-msk/)
- **RabbitMQ**: integrate with RabbitMQ using the [`RabbitMQ` table engine](/docs/engines/table-engines/integrations/rabbitmq)
<br/>
</TabItem>
...
- **Hadoop**: integrate with Apache Hadoop using the [`hdfs` table function](/docs/sql-reference/table-functions/hdfs)
- **Hudi**: read from existing Apache Hudi tables in S3 using the [`hudi` table function](/docs/sql-reference/table-functions/hudi)
- **Iceberg**: read from existing Apache Iceberg tables in S3 using the [`iceberg` table function](/docs/sql-reference/table-functions/iceberg)
- **DeltaLake**: read from existing Delta Lake tables in S3 using the [`deltaLake` table function](/docs/sql-reference/table-functions/deltalake)
<br/>
</TabItem>
<TabItem value="Other" label="Other">

Check out our [long list of ClickHouse integrations](/docs/integrations) to find how to connect your existing frameworks and data sources to ClickHouse.
<br/>
</TabItem>
</Tabs>
...
- Check out our [Core Concepts](/docs/managing-data/core-concepts) section to learn some of the fundamentals of how ClickHouse works under the hood.
...
- We have a list of [example datasets](/docs/getting-started/example-datasets/) with instructions on how to insert them.
- If your data is coming from an external source, view our [collection of integration guides](/docs/integrations/) for connecting to message queues, databases, pipelines and more.
- If you are using a UI/BI visualization tool, view the [user guides for connecting a UI to ClickHouse](/docs/integrations/data-visualization/).
- The user guide on [primary keys](/docs/guides/best-practices/sparse-primary-indexes.md) is everything you need to know about primary keys and how to define them.
