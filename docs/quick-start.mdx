...
2. Run the following command to start the ClickHouse server:
    ...
    :::note
    If your OS is not supported or for other install options, view the [installation details in the technical reference guide](../technical-reference/getting-started/install/).
    :::
...
2. Even the simplest of tables in ClickHouse must specify a **table engine**. The engine determines details about the table like:
    ...
    [MergeTree](../engines/table-engines/mergetree-family/mergetree.md#mergetree) is your likely choice. Run the following command to create a table named `my_first_table` in the `helloworld` database:
    ```sql 
...
1. Even for a simple example, let's insert more than one row at a time:
    ...
    functions <a href="../reference/sql-reference/functions/" target="_blank">that you can view in the **Functions** section of the docs</a>.
    :::
...
2. If you get the smily face prompt, you are ready to run queries! Give it a try:
    ```sql
...
3. Add a `FORMAT` clause to specify one of the <a href="../reference/interfaces/formats/" target="_blank">many supported output formats of ClickHouse</a>:
    ```sql
...
- If you want to play around with larger data, be sure to check out some of the <a href="../category/example-datasets/" target="_blank">example datasets</a>
