...
* **text**: Builds an inverted index over tokenized text data, enabling full-text search in string columns.
...
* **tokenbf_v1 / ngrambf_v1**: Specialized Bloom filter variants that support searching tokens in strings. Such indexes require significantly less storage space than text indexes but they are also harder to configure and they yield false positive matches.
