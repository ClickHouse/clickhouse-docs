...
slug: /en/integrations/language-clients/python
...
'22.10.1.98'
```
- Connecting to `play.clickhouse.com`
```python
import clickhouse_connect
...
ClickHouse Connect provides three core methods for executing SQL statements against a ClickHouse database.
...
- `query` -- Execute a SQL query and return a result set of Python values.  There are also specialized variants:
  - `raw_query` -- Execute a SQL query and return the raw response as a byte array. 
...
#### Parameters Argument for Client _command_ and _query_ methods <a name="params_arg"></a>

ClickHouse Connect Client `query*` and `command` methods accept a `parameters` keyword argument used for binding Python
expressions to a ClickHouse value expression in the rendered SQL.  The `parameters` argument should be a dictionary or a
sequence.  ClickHouse Connect currently uses the Python ["printf" style](https://docs.python.org/3/library/stdtypes.html#old-string-formatting) 
...
#### Settings Argument for Client SQL Methods <a name="settings_arg"></a>

All the core SQL Client ClickHouse database server accept a `settings` keyword argument used for passing ClickHouse
Server users settings for the included SQL statement.  The `settings` argument should be a dictionary.  Each item should
be a ClickHouse setting name and its associated value.  Values will be converted to strings when sent to the server
as query parameters.
...
### Client _command_ Method - Database Commands/DDL 
...
value rather than a full dataset.  The command method takes accepts the following parameters:

| Parameter  | Type             | Description                                                                     |
|------------|------------------|---------------------------------------------------------------------------------|
| cmd        | str              | A ClickHouse SQL statement that returns a single value or a single row of values |                                                                                                                                                                                                                                                                              |
| parameters | dict or iterable | See [parameters description](#params_arg)                                       |
