...
1. From your organization page, open the connection details for the service that you want to connect to:
...
1. The SQL console requires a username and password, the username is populated with `default`; change it to your username if you are not using the default user.  Enter your password in the password field.
...
If you are unable to connect, then please check the IP Access List for the service by expanding and following the steps in **Manage your IP Allow List**.
...
1. This returns the names of the databases, type it in the SQL console, and click **Run**:
  ```sql
  SHOW databases
  ```
