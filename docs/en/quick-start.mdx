...
2. Run the following command to start the ClickHouse server. A user named `default` with no password is created on the initial startup:
    ...
    To run the ClickHouse client on a Mac, use `./clickhouse server` and `./clickhouse client`, respectively.
    :::
...
- If you are using a UI/BI visualization tool, view the [user guides for connecting a UI to ClickHouse](./connect-a-ui/)
- The user guide on [primary keys](./guides/improving-query-performance/sparse-primary-indexes.md) is everything you need to know about primary keys and how to define them
