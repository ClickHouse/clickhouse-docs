...
2. Add a `FORMAT` clause to specify one of the [many supported output formats of ClickHouse](/en/interfaces/formats/):
    ...
    FORMAT Vertical
    ```
    The above query returns the output as a vertical list.  This is very useful for data that contains long strings which would otherwise be hard to read in your terminal or the Play UI. You could also specify CSV, TSV, or many other [FORMAT](/en/interfaces/formats)s.
    ```response
    Row 1:
    ──────
    user_id:   102
...
If you have been using the Play UI (the web interface), switch to the commandline for this step as you will be working with a file on disk.
...
3. Run the following query to validate the insertion:
   ...
   ```
   :::tip
   If you see two sets of rows in your output that means that your data is stored on disk in multiple **parts**.  This is expected, because you ran two insert queries, one in the client, and one from the CSV file.  Read more about background merging of parts in the [MergeTree](./engines/table-engines/mergetree-family/mergetree) documentation.
