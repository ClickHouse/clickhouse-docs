...
2. **Only staging services are allowed for the private preview**. Please only include non-production ClickHouse Cloud services in the private preview. This feature is still in a very early stage and issues can still be found, including potential data loss.

3. **Services are created manually by the ClickHouse team (limitation will be removed in GA).** Once you are ready to create a service that will access the same data as an existing service, please notify the support team. The ClickHouse team will create such a service and you will see the new service in the cloud console.
