...
|-----------------|---------|--------------------------------------------------|
| checksum        | uint128 | [Hash](./hash.mdx) of (header + compressed data) |
...
<Diagram src={useBaseUrl('/img/diagrams/ch_compression_block.drawio.svg')}/>

Header is (raw_size + data_size + mode), raw size consists of len(header + compressed_data).

Checksum is `hash(header + compressed_data)`, using [ClickHouse CityHash](./hash.mdx).
