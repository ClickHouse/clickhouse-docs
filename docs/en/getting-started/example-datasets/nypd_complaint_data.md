...
You will use `clickhouse-client` to create the table.  To connect you will need the connection details collected earlier.  Replace `HOSTNAME.clickhouse.cloud` and `PASSWORD` with your details.  The default username is `default`, if you are using a different user replace the username also:
...
Putting together the changes to data types and the `ORDER BY` tuple gives this table structure. Paste the CREATE statement into `clickhouse-client`:
