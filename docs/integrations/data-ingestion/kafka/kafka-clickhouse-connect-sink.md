...
| `bufferCount`                                   | Number of records to buffer in memory before flushing to ClickHouse. `0` disables internal buffering. Buffering is not supported with `exactlyOnce=true`.                                                                       | `"0"`                                                    |
| `bufferFlushTime`                               | Maximum time in milliseconds to buffer records before flush. `0` disables time-based flushing. Only effective when `bufferCount > 0`.                                                                                           | `"0"`                                                    |
| `reportInsertedOffsets`                         | Enables returning only successfully inserted offsets from `preCommit` (instead of `currentOffsets`) when `exactlyOnce=false`. This does not apply when `ignorePartitionsWhenBatching=true`, where `currentOffsets` are still returned. | `"false"`                                                |
