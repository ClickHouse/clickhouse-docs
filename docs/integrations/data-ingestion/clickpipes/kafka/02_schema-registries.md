...
import cp_glue_schema_registry from '@site/static/images/integrations/data-ingestion/clickpipes/cp_glue_schema_registry.png';
...
:::note Avro only
AWS Glue registries are supported for the Avro format only. The Glue SerDe can also frame JSON and Protobuf, but these are not supported by ClickPipes and are rejected when the pipe is created.
:::
...
<Image img={cp_glue_schema_registry} alt="Schema registry panel with AWS Glue selected" size="lg" border/>
...
1. Obtain your ClickHouse service's IAM role ARN: open the service, select the **Settings** tab, scroll to the **Network security information** section, and copy the **Service role ID (IAM)** value, an ARN of the form `arn:aws:iam::123456789012:role/CH-S3-example-service-Role`. This is referred to below as `{ClickHouse_IAM_ARN}`. Every ClickHouse service has its own role, so this value is different for each service.
   ...
   <Image img={secures3_arn} alt="Service role ID (IAM)" size="lg" border/>
