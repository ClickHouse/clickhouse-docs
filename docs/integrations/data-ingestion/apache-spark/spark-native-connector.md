...
| `timezone`   | Timezone for date/time operations. If not set, the ClickHouse `use_time_zone` client option is used as a fallback (pass it as `.option("option.use_time_zone", "Asia/Shanghai")`); an explicit `timezone` always takes precedence. | `server`       | No       |
...
|----------------------------------------------------|--------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|
| spark.clickhouse.client.queryTimeout               | 60s                                                    | The maximum time the ClickHouse client will wait for a single query or ping operation to complete on a `NodeClient`. Applied as a future-handle timeout on every `client.query(...)` and `client.ping(...)` call. Used on both read and write paths for query and ping operations. Inserts themselves are unaffected — they have no connector-level timeout. | 0.10.1 |
...
| spark.clickhouse.read.pushdown.topN                | true                                                   | If `true`, push down top-N (`ORDER BY ... LIMIT n`) to ClickHouse. Each input partition runs its own local top-N and Spark performs a final global merge across partitions. Push-down is all-or-nothing: if any sort key cannot be translated to ClickHouse (e.g. multi-part column references, arbitrary scalar expressions, unregistered functions), the entire top-N is left for Spark to evaluate. On Spark 3.3 only bare column sort keys are pushed down. Set to `false` to disable.                               | 0.10.1 |
...
| spark.clickhouse.write.repartitionNum              | 0                                                      | Repartition count before writing. When set to a value greater than 0, Spark will repartition data to exactly this many partitions before the write pipeline begins. A value of `0` (default) means no specific partition count is enforced, and Spark falls back to `spark.sql.shuffle.partitions` (default `200`). This is a fixed partition count, not one derived from the size of the data.                                                                                                       | 0.1.0 |
...
[`build-and-test` workflow](https://github.com/ClickHouse/spark-clickhouse-connector/blob/main/.github/workflows/build-and-test.yml)
for tested versions.
...
| **Top-N** | `SupportsPushDownTopN` | `ORDER BY ... LIMIT N` is sent to ClickHouse. Each input partition runs its own local top-N and Spark performs a final global merge. Controlled by `spark.clickhouse.read.pushdown.topN` (default `true`). Push-down is all-or-nothing: if any sort key can't be translated to ClickHouse, the entire top-N is evaluated Spark-side. |
| **Aggregations** | `SupportsPushDownAggregates` | `GROUP BY` + aggregate functions (`SUM`, `COUNT`, `MIN`, `MAX`) are executed in ClickHouse. |
...
For a table with no `PARTITION BY` clause, the connector reads the table in a single Spark task (one partition in the Spark stage), producing minimal read parallelism. Use `PARTITION BY` in your ClickHouse table to increase read parallelism.
...
df.repartition(16, col("SHARD_COLUMN")) \
    .sortWithinPartitions("SHARD_COLUMN", "PARTITION_COLUMN") \
    .write \
    .format("clickhouse") \
    .option("host", "your-host") \
    .option("database", "default") \
...
The two settings are independent — one governs reads, the other writes — so all four combinations are valid:

| `read.distributed.convertLocal` | `write.distributed.useClusterNodes` | Behaviour |
|---|---|---|
| `true` (default) | `true` (default) | Reads connect directly to each shard's local table; writes are spread across all cluster nodes. Maximum parallelism, but every shard hostname must be reachable from Spark. |
| `true` | `false` | Reads connect directly to each shard; writes go through the single coordinator using the `Distributed` engine. |
| `false` | `true` | Reads go through the coordinator using the `Distributed` engine; writes are spread across all cluster nodes. |
| `false` | `false` | Both reads and writes go through the single coordinator. Simplest networking (only the coordinator must be reachable), lowest parallelism. |
...
```text
spark.sql.catalog.clickhouse.option.http_header_X-ClickHouse-Quota   spark_etl_quota
```
...
| Tuning | Configuration | Notes |
|---|---|---|
| **Increase batch size** | `spark.clickhouse.write.batchSize=50000` | Default is 10,000. Larger batches reduce round-trips and could minimize `Too Many Parts` errors. |
...
**Symptom**: 64-bit integer values inside a `VariantType` or `JSON` column come back as **string-typed** entries — Spark reads the value as a string, not a number. Reading a Variant whose `v` path holds `1` returns the string `"1"` (JSON form: `{"v":"1"}`) instead of the number `1`. This happens regardless of the write format — both `arrow` and `json` writes are affected. Floats, strings, and booleans are unaffected.

**Cause**: This is read-side behavior, not a write-path bug. The connector reads `Variant` and `JSON` columns using a JSON format, and integers stored inside are held as 64-bit integers. ClickHouse's `output_format_json_quote_64bit_integers` setting controls whether 64-bit integers are wrapped in quotes in JSON output; when it is `1`, they are emitted as quoted strings and the connector rebuilds them as string-typed entries.

**Fix**: Pick one:
- Disable integer quoting on reads:
  ```text
  spark.clickhouse.read.settings=output_format_json_quote_64bit_integers=0
  ```
- Declare typed JSON paths in ClickHouse, e.g. `data JSON(v Int8)`. A non-64-bit typed path is not subject to the quoting setting.
