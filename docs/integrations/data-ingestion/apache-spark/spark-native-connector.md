...
| spark.clickhouse.read.pushdown.topN                | true                                                   | If `true`, push down top-N (`ORDER BY ... LIMIT n`) to ClickHouse. Each input partition runs its own local top-N and Spark performs a final global merge across partitions. Sort orders that cannot be translated to ClickHouse (e.g. multi-part column references, arbitrary scalar expressions, unregistered functions) are left for Spark to evaluate. On Spark 3.3 only bare column sort keys are pushed down. Set to `false` to disable.                               | 0.10.1 |
...
| **Top-N** | `SupportsPushDownTopN` | `ORDER BY ... LIMIT N` is sent to ClickHouse. Each input partition runs its own local top-N and Spark performs a final global merge. Controlled by `spark.clickhouse.read.pushdown.topN` (default `true`). Sort orders that can't be translated to ClickHouse fall back to Spark-side evaluation. |
