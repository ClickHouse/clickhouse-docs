...
Configure `clickhouse` profile in ~/.dbt/profiles.yml file and provide user, password, schema host properties. The full list of connection configuration options is available in the [Features and configurations](/integrations/data-ingestion/etl-tools/dbt/features-and-configurations) page:
```yaml
clickhouse:
  target: dev
  outputs:
