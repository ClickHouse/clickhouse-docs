...
| `setOption("custom_settings_prefix", value)` | `value` - prefix string | Sets the prefix for custom settings passed to the server. Should be aligned with the server configuration. See [ClickHouse Docs](/operations/settings/query-level#custom_settings). | `custom_` | `custom_settings_prefix` |
