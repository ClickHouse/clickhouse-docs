...
- Service may fallback to a comparable instance type from an older generation to maintain the selected high-availability configuration, if the selected instance type capacity is temporarily restricted in some of the availability zones. You will be billed at the target instance type rate.
