...
Customers are strongly recommended to prepare a **dedicated** AWS account/GCP Project for hosting the ClickHouse BYOC deployment to ensure better isolation in terms of permissions and resources. And ClickHouse will deploy a dedicated set of cloud resources(VPC, K8S, IAM, Buckets etc.) in your account.
...
## Request access {#onboarding-process}

To start the onboarding process, please [contact us](https://clickhouse.com/cloud/bring-your-own-cloud). Our team will guide you through the BYOC requirements and add your account to the allowlist.

## Onboarding 

### Prepare an AWS account/GCP project {#prepare-an-aws-account}

Prepare a fresh AWS account and GCP project under your organization.

Visit our web console: https://console.clickhouse.cloud/byocOnboarding to continue the setup. 

### Choose a Cloud Provider - Step 1

<Image img={byoc_onboarding_1} size="lg" alt="BYOC choose CSP" background='black'/>

### Account/Project setup - Step 2 & 3
...
:::note
S3/Bucket, VPC, K8S and compute resources required for running ClickHouse are not included in this initial setup.
:::
#### Alternative Terraform Module for AWS {#terraform-module-aws}

If you also wish to use Terraform instead of Cloudformation in AWS, we also have a [Terraform module for AWS](https://s3.us-east-2.amazonaws.com/clickhouse-public-resources.clickhouse.cloud/tf/byoc.tar.gz).  
Usage:  
```hcl
...
### Set up BYOC infrastructure {#setup-byoc-infrastructure} - Step 4

You will be prompted to set up the infrastructure, including S3/Bucket, VPC, and the K8S cluster, from the cloud console. Certain configurations must be determined at this stage, as they cannot be changed later. Specifically:
- **The region you want to use**: All **public regions** listed in our [supported regions](https://clickhouse.com/docs/cloud/reference/supported-regions) documentation are available for BYOC deployments. Private regions are not currently supported.
...
- **Availability Zones for BYOC VPC**: If you plan to use VPC peering, aligning availability zones between the source and BYOC accounts can help reduce cross-AZ traffic costs. For example, in AWS, availability zone suffixes (`a, b, c`) may represent different physical zone IDs across accounts. See the [AWS guide](https://docs.aws.amazon.com/prescriptive-guidance/latest/patterns/use-consistent-availability-zones-in-vpcs-across-different-aws-accounts.html) for details.
