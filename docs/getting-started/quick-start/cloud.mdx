...
By default, new organizations are put on the Scale tier and will create 3 ^^replica^^s each with 4 VCPUs and 16 GiB RAM. [Vertical autoscaling](/manage/scaling#vertical-auto-scaling) will be enabled by default in the Scale tier. You can change your organization tier later on the 'Plans' page.
...
:::tip[IP access list filtering]
Before you move on, we recommended setting up IP access list filtering.
See ["Setting IP filters"](/cloud/security/setting-ip-filters) for more details.
:::
