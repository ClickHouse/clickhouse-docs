...
doc_type: 'guide'
sidebar_label: 'Choosing an update strategy'
---

When it comes to handling updates, analytical and transactional databases take different approaches to updates due to their underlying design philosophies and target use cases.
...
ClickHouse on the other hand is a column-oriented database which is optimized for read-heavy analytics and high throughput append-only operations.
In practice tables are often restructured to convert deletes and updates into append operations that are processed asynchronously and/or at read time, capitalising on ClickHouse's strengths for high-throughput data ingestion. While in-place updates and deletes are fully supported in ClickHouse, choosing the right strategy for the workload at hand is important.
...
With mutations via `ALTER TABLE ... UPDATE` you may have to wait for your mutations to be applied via a background process to see the changed values reflected in your queries without use of `FINAL`.
...
| [On-the-fly mutations](/guides/developer/on-the-fly-mutations) | Variable | Variable | Instant visibility; performance degrades if many updates accumulate |
