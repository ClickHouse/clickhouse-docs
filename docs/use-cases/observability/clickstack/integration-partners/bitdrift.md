...
title: 'Sending mobile telemetry to ClickStack with bitdrift'
sidebar_label: 'bitdrift'
pagination_prev: null
pagination_next: null
description: 'Connect bitdrift mobile observability to ClickStack with OpenTelemetry trace context'
doc_type: 'guide'
keywords: ['bitdrift', 'ClickStack', 'ClickHouse', 'OpenTelemetry', 'mobile observability', 'tracing']
---

import Image from '@theme/IdealImage';
import bitdrift_user_journey_dashboard from '@site/static/images/clickstack/bitdrift/bitdrift-user-journey-dashboard.png';
import bitdrift_view_trace from '@site/static/images/clickstack/bitdrift/bitdrift-view-trace.jpg';
import PartnerBadge from '@theme/badges/PartnerBadge';

<PartnerBadge/>

:::note[TL;DR]
This guide shows you how to connect bitdrift mobile telemetry to ClickStack using OpenTelemetry trace context. You'll learn how to:

- Enable bitdrift's networking and tracing integrations on iOS, Android, or React Native
- Enable tracing for your bitdrift tenant and point it at your ClickStack instance
...
- **The [Capture SDK](https://bitdrift.io/feature/performance-centric)**, a lightweight library for iOS, Android, and React Native, built in Rust with thin platform-specific wrappers, that writes telemetry to a fixed-size ring buffer on the device rather than streaming it off immediately.
...
<Image img={bitdrift_user_journey_dashboard} alt="A bitdrift User Journey dashboard showing paths to an ANR" size="lg"/>
...
:::tip[Key takeaways]
- **bitdrift** captures unsampled, mobile-native telemetry on iOS, Android, and React Native, and links captured trace IDs to their backend trace.
- **ClickStack** stores and queries the resulting distributed trace in ClickHouse and surfaces it in HyperDX.
:::

## Prerequisites {#prerequisites}

- **ClickStack** installed and reachable from your backend services, either self-managed or on ClickHouse Cloud. See [Getting started with open source ClickStack](/use-cases/observability/clickstack/getting-started/oss) or [Getting started with managed ClickStack](/use-cases/observability/clickstack/getting-started/managed).
- Your backend services instrumented with **OpenTelemetry** and sending traces to ClickStack. See [Ingesting with OpenTelemetry](/use-cases/observability/clickstack/ingesting-data/opentelemetry).
- The **bitdrift Capture SDK** integrated in your iOS, Android, or React Native app. See the [bitdrift SDK documentation](https://docs.bitdrift.io/) for install instructions.
...
Tracing is an experimental bitdrift feature that must be turned on per tenant. Configuration happens entirely on the bitdrift side: enabling tracing for your tenant, enabling bitdrift's network integration on your platform, and adding a **Start Tracing** workflow action. For setup steps, see [How bitdrift and ClickStack work together](https://docs.bitdrift.io/product/integration-partners/clickhouse#how-bitdrift-and-clickstack-work-together) in the bitdrift documentation.
...
<Image img={bitdrift_view_trace} alt="A bitdrift trace linking to a distributed trace in the ClickStack UI" size="lg"/>

*Following a View Trace link from a bitdrift session into a distributed trace in the ClickStack UI (HyperDX).*

## Advanced configuration {#advanced-configuration}

### Trace propagation formats {#trace-propagation-formats}

bitdrift supports multiple trace context propagation formats. Use the format that matches your existing OpenTelemetry Collector and backend service configuration for ClickStack, so trace context propagates correctly end-to-end. See bitdrift's [Tracing](https://docs.bitdrift.io/sdk/features/tracing) documentation for supported formats.

### Deployment flexibility {#deployment-flexibility}

The integration works whether ClickStack is self-managed or run as a managed offering on ClickHouse Cloud. Telemetry from bitdrift reaches the same OpenTelemetry Collector that ClickStack already uses, via OTLP over HTTP or gRPC. No changes to your ClickStack deployment are required.
...
- [Ingesting with OpenTelemetry](/use-cases/observability/clickstack/ingesting-data/opentelemetry)
