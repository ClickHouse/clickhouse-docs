...
- We assume most queries occur over recent data, with a mean lookback window of six hours, with a normal distribution and a standard deviation of one hour on either side. You may wish to provision dedicated compute to query older data. In ClickHouse Cloud this can be idle (thus not incurring costs) when not in use.
